...
struct Conv1DGenerator
  ...
  FailureOr<Operation *> conv(Conv1DOpOrder conv1DOpOrder) {
    ...
    Value lhs = vector::TransferReadOp::create(rewriter, loc, lhsType, lhsShaped,
                                                        lhsPadding);
    // This is needed only for Conv.
    Value rhs = nullptr;
    if (oper == ConvOperationKind::Conv)
      rhs = vector::TransferReadOp::create(rewriter, loc, rhsType, rhsShaped,
                                                    rhsPadding);
    Value res = vector::TransferReadOp::create(rewriter, loc, resType, resShaped,
                                                        resPadding);
  ...
  FailureOr<Operation *> depthwiseConv(uint64_t channelDimVecSize,
    ...
    Value rhs = vector::TransferReadOp::create(rewriter, loc, rhsType, rhsShaped,
