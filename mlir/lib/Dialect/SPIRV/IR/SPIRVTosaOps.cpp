...
static LogicalResult verifyConvOp(T op) {
  ...
  if (!isa<BoolAttr>(op.getLocalBound())) {
...
LogicalResult TosaArgMaxOp::verify() {
  ...
  auto axisAttr = dyn_cast<IntegerAttr>(getAxis());
  if (!axisAttr && !axisAttr.getType().isInteger(32)) {
    return emitOpError("axis type must be a constant i32 integer");
  }
...
LogicalResult TosaFFT2DOp::verify() {
  ...
  if (!isa<BoolAttr>(getInverse())) {
    return emitOpError("inverse must be a constant boolean");
  }

  if (!isa<BoolAttr>(getLocalBound())) {
...
LogicalResult TosaMulOp::verify() {
  ...
  if (auto resIntType = dyn_cast<IntegerType>(resElemType)) {
    auto lhsIntType = cast<IntegerType>(getInput1Type().getElementType());
    auto rhsIntType = cast<IntegerType>(getInput2Type().getElementType());
...
LogicalResult TosaRescaleOp::verify() {
  ...
  auto scale32Attr = dyn_cast<BoolAttr>(getScale32());
  if (!scale32Attr) {
  ...
  auto perChannelAttr = dyn_cast<BoolAttr>(getPerChannel());
  if (!perChannelAttr) {
  ...
  auto inputUnsignedAttr = dyn_cast<BoolAttr>(getInputUnsigned());
  if (!inputUnsignedAttr) {
    return emitOpError("input_unsigned must be a constant boolean");
  }

  auto outputUnsignedAttr = dyn_cast<BoolAttr>(getOutputUnsigned());
  if (!outputUnsignedAttr) {
...
LogicalResult TosaReverseOp::verify() {
  ...
  auto axisAttr = dyn_cast<IntegerAttr>(getAxis());
  if (!axisAttr && !axisAttr.getType().isInteger(32)) {
    return emitOpError("axis must be a constant i32 integer");
  }
