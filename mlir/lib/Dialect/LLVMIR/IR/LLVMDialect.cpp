...
OpFoldResult LLVM::ExtractValueOp::fold(FoldAdaptor adaptor) {
  ...
  while (insertValueOp) {
    ...
    // Case 3: Try to continue the traversal with the container value, in order
    // to swap out the container operand. This does not work if we decided
    // earlier to continue the traversal with the inserted value (Case 2).
