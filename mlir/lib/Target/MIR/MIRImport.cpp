...
/// AArch64 opcode-record-name -> collapsed op mnemonic, generated by
/// `mlir-tblgen -gen-mir-target-ops --mir-emit-lowering`.
static const llvm::StringMap<std::string> &getLoweringMap() {
  static const llvm::StringMap<std::string> map = [] {
    llvm::StringMap<std::string> m;
    static const std::pair<const char *, const char *> entries[] = {
#include "AArch64MIRLowering.inc"
    };
...
private:
  ...
  /// The mir type of a virtual register: its LLT if generic, otherwise a
  /// scalar sized from its register class (post-instruction-select vregs).
  Type regType(llvm::Register reg) {
    llvm::LLT llt = mri.getType(reg);
    if (llt.isValid())
      return lltToType(llt, ctx);
    if (const llvm::TargetRegisterClass *rc = mri.getRegClassOrNull(reg))
      return mir::ScalarType::get(ctx, tri->getRegSizeInBits(*rc));
    return mir::ScalarType::get(ctx, 0);
  }
  ...
  LogicalResult convert(OpBuilder &b, Location loc,
    ...
    if (llvm::isPreISelGenericOpcode(opcode)) {
      ...
      for (const llvm::MachineOperand &mo : mi.operands()) {
        ...
        if (mo.isImplicit())
          continue;
        if (mo.isDef()) {
          resultTypes.push_back(regType(mo.getReg()));
    ...
    // Selected target opcode -> aarch64_mir.<mnemonic> with a `variant`
    // discriminator naming the concrete opcode.
    StringRef recName = tii->getName(opcode);
    auto it = getLoweringMap().find(recName);
    if (it != getLoweringMap().end()) {
      std::string name = "aarch64_mir." + it->second;
      SmallVector<Value> operands;
      SmallVector<Type> resultTypes;
      SmallVector<llvm::Register> defRegs;
      for (const llvm::MachineOperand &mo : mi.operands()) {
      ...
      state.addOperands(operands);
      state.addTypes(resultTypes);
      state.addAttribute("variant", StringAttr::get(ctx, recName));
      Operation *op = b.create(state);
      for (auto [reg, res] : llvm::zip(defRegs, op->getResults()))
        map(reg, res);
      return success();
...
static OwningOpRef<Operation *> translateMIRToModule(llvm::SourceMgr &sourceMgr,
  ...
  ctx->getOrLoadDialect<aarch64_mir::AArch64MIRDialect>();
  Location loc = UnknownLoc::get(ctx);
...
void registerFromMIRTranslation() {
  TranslateToMLIRRegistration registration(
      ...
      [](DialectRegistry &registry) {
        registry.insert<mir::MIRDialect, aarch64_mir::AArch64MIRDialect>();
      });
}
} // namespace mlir
