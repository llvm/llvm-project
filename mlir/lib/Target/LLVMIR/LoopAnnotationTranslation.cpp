...
llvm::MDNode *LoopAnnotationConversion::convert() {
  ...
  if (!parallelAccessGroups.empty()) {
    ...
    for (AccessGroupAttr group : parallelAccessGroups)
      parallelAccess.push_back(loopAnnotationTranslation.getAccessGroup(group));
...
llvm::MDNode *LoopAnnotationTranslation::getAccessGroup(AccessGroupAttr group) {
  auto it = accessGroupMetadataMapping.find(group);
  if (it != accessGroupMetadataMapping.end())
    return it->getSecond();

  llvm::MDNode *accessGroup =
      llvm::MDNode::getDistinct(llvmModule.getContext(), {});
  accessGroupMetadataMapping.insert({group, accessGroup});
  return accessGroup;
}
...
LoopAnnotationTranslation::getAccessGroups(AccessGroupOpInterface op) {
  ...
  for (auto group : accessGroups.getAsRange<AccessGroupAttr>())
