...
static void processBranchOp(BranchOpInterface branchOp, RunLivenessAnalysis &la,
  ...
  for (unsigned succIdx = 0; succIdx < numSuccessors; ++succIdx) {
    ...
    // determines it is live. This can happen when a branch is in a statically
    // unreachable (dead) block: the forwarded operand appears dead because it
    // is in the dead block, but the successor block argument may still be live
    // because it is also forwarded from other live predecessor branches.
