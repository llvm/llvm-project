  ...
  matchAndRewrite(tosa::MaxPool2dOp op, OpAdaptor adaptor,
    ...
    if (const auto nanMode = op.getNanMode(); nanMode == "IGNORE") {
      auto genericOp = linalg::GenericOp::create(
          rewriter, op->getLoc(), resultOp.getType(0), resultOp.getInputs(),
          ...
          [&](OpBuilder &opBuilder, Location loc, ValueRange blockArgs) {
            ...
            Value isNaN = arith::CmpFOp::create(
                opBuilder, op->getLoc(), arith::CmpFPredicate::UNO,
                blockArgs.front(), blockArgs.front());
            auto selectOp =
                arith::SelectOp::create(opBuilder, op->getLoc(), isNaN,
                                        blockArgs.back(), newOp->getResult(0));
