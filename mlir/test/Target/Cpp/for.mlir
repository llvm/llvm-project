...
func.func @test_for_yield() {
  ...
  emitc.assign %s0 : i32 to %0 : i32
  emitc.assign %p0 : f32 to %1 : f32
  emitc.for %iter = %start to %stop step %step {
    %2 = "emitc.variable"() <{value = #emitc.opaque<"">}> : () -> i32
    emitc.assign %0 : i32 to %2 : i32
    ...
    emitc.assign %sn : i32 to %0 : i32
    emitc.assign %pn : f32 to %1 : f32
    emitc.yield
  }
  %2 = "emitc.variable"() <{value = #emitc.opaque<"">}> : () -> i32
  emitc.assign %0 : i32 to %2 : i32
...
// CPP-DEFAULT-NEXT: int32_t [[SL:[^ ]*]];
// CPP-DEFAULT-NEXT: [[SL]] = [[SI]];
// CPP-DEFAULT-NEXT: float [[PL:[^ ]*]];
// CPP-DEFAULT-NEXT: [[PL]] = [[PI]];
// CPP-DEFAULT-NEXT: int32_t [[SN:[^ ]*]] = add([[SL]], [[ITER]]);
// CPP-DEFAULT-NEXT: float [[PN:[^ ]*]] = mul([[PL]], [[ITER]]);
...
// CPP-DECLTOP-NEXT: int32_t [[SL:[^ ]*]];
// CPP-DECLTOP-NEXT: float [[PL:[^ ]*]];
...
// CPP-DECLTOP-NEXT: ;
// CPP-DECLTOP-NEXT: [[SL]] = [[SI]];
// CPP-DECLTOP-NEXT: ;
// CPP-DECLTOP-NEXT: [[PL]] = [[PI]];
// CPP-DECLTOP-NEXT: [[SN]] = add([[SL]], [[ITER]]);
// CPP-DECLTOP-NEXT: [[PN]] = mul([[PL]], [[ITER]]);
