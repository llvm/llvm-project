...
// A transfer_write masked by an enclosing vector.mask must be left
// unconverted: MMA conversion cannot preserve a region mask.

#map1 = affine_map<(d0, d1, d2) -> (d0, d2)>
#map2 = affine_map<(d0, d1, d2) -> (d1, d2)>
#map3 = affine_map<(d0, d1, d2) -> (d0, d1)>

// CHECK-LABEL: func @m16n8k16_fp16_masked_write
//   CHECK-NOT:   nvgpu.mma.sync
//       CHECK:   vector.contract
//       CHECK:   vector.mask %{{.*}} { vector.transfer_write
//   CHECK-NOT:   nvgpu.
func.func @m16n8k16_fp16_masked_write(%arg0: memref<16x16xf16, #gpu.address_space<workgroup>>, %arg1: memref<8x16xf16, #gpu.address_space<workgroup>>, %arg2: memref<16x8xf16>, %m: index, %n: index) {
  %c0 = arith.constant 0 : index
  %cst = arith.constant 0.000000e+00 : f16
  ...
  %D = vector.contract {indexing_maps = [#map1, #map2, #map3], iterator_types = ["parallel", "parallel", "reduction"], kind = #vector.kind<add>} %A, %B, %C : vector<16x16xf16>, vector<8x16xf16> into vector<16x8xf16>
  %mask = vector.create_mask %m, %n : vector<16x8xi1>
  vector.mask %mask { vector.transfer_write %D, %arg2[%c0, %c0] {in_bounds = [true, true]} : vector<16x8xf16>, memref<16x8xf16> } : vector<16x8xi1>
  return
}
