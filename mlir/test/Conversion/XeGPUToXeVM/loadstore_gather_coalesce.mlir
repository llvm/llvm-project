...
// Lane-level gather/scatter carry one offset and one mask bit per element. A
// lane whose `lane_data` is wider than one element gets a contiguous run, so
// vector offsets here are coalesced into a single block load/store based at
// element 0. The mask must still be uniform: one block access is gated on one
// bit.
...
// vector.step offsets, the run based at 0 that distribution emits for lane 0.
