...
func.func @large_const_shape() -> !tosa.shape<33> {
  // CHECK: tosa.const_shape values({{.*}}) {grapharm.graph_constant_id = 0 : i32} : () -> !tosa.shape<33>
...
func.func @mixed_large_constants() -> (tensor<17xi32>, tensor<18xi32>, !tosa.shape<33>) {
  ...
  // CHECK: tosa.const_shape values({{.*}}) {grapharm.graph_constant_id = 2 : i32} : () -> !tosa.shape<33>
