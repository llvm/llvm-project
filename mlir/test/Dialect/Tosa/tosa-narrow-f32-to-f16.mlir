...
func.func @test_f32_identity_chain(%arg0: tensor<1xf32>) -> tensor<1xf32> {
  // DEFAULT: %[[CAST_IN:.*]] = tosa.cast %arg0 input_unsigned(false) : (tensor<1xf32>) -> tensor<1xf16>
  ...
  // DEFAULT: %[[CAST_OUT:.*]] = tosa.cast %[[ID2]] input_unsigned(false) : (tensor<1xf16>) -> tensor<1xf32>
...
func.func @test_f32_concat(%arg0: tensor<13x21x3xf32>, %arg1: tensor<13x21x3xf32>) -> tensor<26x21x3xf32> {
  // DEFAULT-DAG: %[[CAST0:.*]] = tosa.cast %[[A0]] input_unsigned(false) : (tensor<13x21x3xf32>) -> tensor<13x21x3xf16>
  // DEFAULT-DAG: %[[CAST1:.*]] = tosa.cast %[[A1]] input_unsigned(false) : (tensor<13x21x3xf32>) -> tensor<13x21x3xf16>
  // COMMON: %[[CONCAT:.*]] = tosa.concat %{{.*}}, %{{.*}} axis(0) : (tensor<13x21x3xf16>, tensor<13x21x3xf16>) -> tensor<26x21x3xf16>
  %0 = tosa.concat %arg0, %arg1 axis(0) : (tensor<13x21x3xf32>, tensor<13x21x3xf32>) -> tensor<26x21x3xf32>
  // DEFAULT: %[[CAST_OUT:.*]] = tosa.cast %[[CONCAT]] input_unsigned(false) : (tensor<26x21x3xf16>) -> tensor<26x21x3xf32>
...
func.func @test_f32_pad(%arg0: tensor<13x21x3xf32>, %arg1: tensor<1xf32>) -> tensor<15x23x5xf32> {
  ...
  // DEFAULT-DAG: %[[IN_CAST:.*]] = tosa.cast %arg0 input_unsigned(false) : (tensor<13x21x3xf32>) -> tensor<13x21x3xf16>
  // DEFAULT-DAG: %[[PAD_CAST:.*]] = tosa.cast %arg1 input_unsigned(false) : (tensor<1xf32>) -> tensor<1xf16>
  ...
  // DEFAULT: %[[OUT_CAST:.*]] = tosa.cast %[[PAD]] input_unsigned(false) : (tensor<15x23x5xf16>) -> tensor<15x23x5xf32>
...
func.func @test_f32_reshape(%arg0: tensor<13x21x3xf32>) -> tensor<1x819xf32> {
  ...
  // DEFAULT: %[[OUT_CAST:.*]] = tosa.cast %[[RESHAPE]] input_unsigned(false) : (tensor<1x819xf16>) -> tensor<1x819xf32>
...
func.func @test_f32_reverse(%arg0: tensor<13x21x3xf32>) -> tensor<13x21x3xf32> {
  // COMMON: %[[REV:.*]] = tosa.reverse %{{.*}} axis(0) : (tensor<13x21x3xf16>) -> tensor<13x21x3xf16>
  %0 = tosa.reverse %arg0 axis(0) : (tensor<13x21x3xf32>) -> tensor<13x21x3xf32>
  // DEFAULT: %[[OUT:.*]] = tosa.cast %[[REV]] input_unsigned(false) : (tensor<13x21x3xf16>) -> tensor<13x21x3xf32>
...
func.func @test_f32_slice(%arg0: tensor<13x21x3xf32>) -> tensor<4x11x1xf32> {
  ...
  // DEFAULT: %[[OUT:.*]] = tosa.cast %[[SLICE]] input_unsigned(false) : (tensor<4x11x1xf16>) -> tensor<4x11x1xf32>
...
func.func @test_f32_tile(%arg0: tensor<13x21x3xf32>) -> tensor<39x21x6xf32> {
  ...
  // DEFAULT: %[[OUT:.*]] = tosa.cast %[[TILE]] input_unsigned(false) : (tensor<39x21x6xf16>) -> tensor<39x21x6xf32>
...
func.func @test_f32_transpose(%arg0: tensor<13x21x3xf32>) -> tensor<3x13x21xf32> {
  // COMMON: %[[TRANSPOSE:.*]] = tosa.transpose %{{.*}} perms([2, 0, 1]) : (tensor<13x21x3xf16>) -> tensor<3x13x21xf16>
  %0 = tosa.transpose %arg0 perms([2, 0, 1]) : (tensor<13x21x3xf32>) -> tensor<3x13x21xf32>
  // DEFAULT: %[[OUT:.*]] = tosa.cast %[[TRANSPOSE]] input_unsigned(false) : (tensor<3x13x21xf16>) -> tensor<3x13x21xf32>
...
func.func @test_f32_regions(%arg0: tensor<1xf32>, %arg1: tensor<1xf32>, %arg2: tensor<i1>) -> tensor<1xf32> {
  ...
  // DEFAULT: %[[OUT:.*]] = tosa.cast %[[IF_RESULT]] input_unsigned(false) : (tensor<1xf16>) -> tensor<1xf32>
