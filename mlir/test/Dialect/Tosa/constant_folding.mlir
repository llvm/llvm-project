...
func.func @try_fold_unranked_identity_results(%arg0: tensor<1xf32>) {
  ...
  %0 = tosa.transpose %arg0 perms([0]) : (tensor<1xf32>) -> tensor<*xf32>
  %1 = tosa.reverse %arg0 axis(0) : (tensor<1xf32>) -> tensor<*xf32>
...
func.func @reverse_length_one(%arg0 : tensor<10x1xi32>) -> (tensor<10x1xi32>, tensor<10x1xi32>) {
  %nofold = tosa.reverse %arg0 axis(0) : (tensor<10x1xi32>) -> tensor<10x1xi32>
  %fold = tosa.reverse %arg0 axis(1) : (tensor<10x1xi32>) -> tensor<10x1xi32>
  // CHECK: %[[NOFOLD:.+]] = tosa.reverse %arg0 axis(0)
...
func.func @test_fold_dim(%arg0: tensor<6xi32>) -> !tosa.shape<1> {
  %dim = tosa.dim %arg0 axis(0) : (tensor<6xi32>) -> !tosa.shape<1>
...
func.func @test_no_fold_dim_unranked_input(%arg0: tensor<*xi32>) -> !tosa.shape<1> {
  %dim = tosa.dim %arg0 axis(0) : (tensor<*xi32>) -> !tosa.shape<1>
  return %dim : !tosa.shape<1>
}
...
func.func @test_no_fold_dim_dynamic(%arg0: tensor<4x?xi32>) -> !tosa.shape<1> {
  %dim = tosa.dim %arg0 axis(1) : (tensor<4x?xi32>) -> !tosa.shape<1>
  return %dim : !tosa.shape<1>
}
