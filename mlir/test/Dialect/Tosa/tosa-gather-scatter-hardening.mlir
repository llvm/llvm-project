...
// CHECK-DAG: %[[UPPER:.*]] = tosa.const values(dense<20> : tensor<1x1xi32>) : () -> tensor<1x1xi32>
// CHECK-DAG: %[[DATA:.*]] = tosa.const values(dense<25> : tensor<1x2xi32>) : () -> tensor<1x2xi32>
...
// CHECK-DAG: %[[ZERO:.*]] = tosa.const values(dense<0> : tensor<1x1xi32>) : () -> tensor<1x1xi32>
// CHECK-DAG: %[[UPPER:.*]] = tosa.const values(dense<2147483647> : tensor<1x1xi32>) : () -> tensor<1x1xi32>
// CHECK-NEXT: %[[NONNEGATIVE:.*]] = tosa.maximum %arg1, %[[ZERO]]
// CHECK-NEXT: %[[CLAMPED:.*]] = tosa.minimum %[[NONNEGATIVE]], %[[UPPER]]
// CHECK-NEXT: %[[GATHER:.*]] = tosa.gather %arg0, %[[CLAMPED]]
// CHECK-NEXT: return %[[GATHER]]
...
// CHECK-DAG: %[[ZERO:.*]] = tosa.const values(dense<0> : tensor<1x1xi64>) : () -> tensor<1x1xi64>
// CHECK-DAG: %[[UPPER:.*]] = tosa.const values(dense<20> : tensor<1x1xi64>) : () -> tensor<1x1xi64>
// CHECK-DAG: %[[BAD_BOUND:.*]] = tosa.const values(dense<21> : tensor<1x1xi64>) : () -> tensor<1x1xi64>
...
// CHECK-DAG: %[[ZERO:.*]] = tosa.const values(dense<0> : tensor<1x1xi32>) : () -> tensor<1x1xi32>
// CHECK-DAG: %[[UPPER:.*]] = tosa.const values(dense<20> : tensor<1x1xi32>) : () -> tensor<1x1xi32>
// CHECK-DAG: %[[NEGATIVE:.*]] = tosa.const values(dense<-1> : tensor<1x1xi32>) : () -> tensor<1x1xi32>
...
// CHECK-DAG: %[[ZERO:.*]] = tosa.const values(dense<0> : tensor<1x1xi32>) : () -> tensor<1x1xi32>
// CHECK-DAG: %[[UPPER:.*]] = tosa.const values(dense<20> : tensor<1x1xi32>) : () -> tensor<1x1xi32>
// CHECK-DAG: %[[DATA:.*]] = tosa.const values(dense<-1> : tensor<1x2xi32>) : () -> tensor<1x2xi32>
