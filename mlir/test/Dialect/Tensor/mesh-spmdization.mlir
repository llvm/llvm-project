...
func.func @tensor_empty_static_sharded_dims_sizes() -> () {
  ...
  %sharding = mesh.sharding @mesh_1d_4 split_axis = [[0]] sharded_dims_sizes = [1, 3, 3, 1] : !mesh.sharding
  %sharded= mesh.shard %b to %sharding : tensor<8x16xf32>
  // CHECK:  %[[sharding:.*]] = mesh.sharding @mesh_1d_4 split_axis = {{\[\[}}0]] sharded_dims_sizes = [1, 3, 3, 1] : !mesh.sharding
...
func.func @tensor_empty_same_static_dims_sizes() -> () {
  %b = tensor.empty() : tensor<8x16xf32>
  %sharding = mesh.sharding @mesh_1d_4 split_axis = [[0]] sharded_dims_sizes = [4, 4, 4, 4] : !mesh.sharding
  %sharded= mesh.shard %b to %sharding : tensor<8x16xf32>
