...
func.func @fuse_unary(%arg0: tensor<?x?xf32>, %arg1: tensor<?x?xf32>) -> tensor<?x?xf32> {
  ...
  //     CHECK: return %[[RES]]
  %0 = linalg.elementwise <exp> ins(%arg0 : tensor<?x?xf32>)
                             outs(%arg1: tensor<?x?xf32>) -> tensor<?x?xf32>
  %1 = linalg.elementwise <add> ins(%0, %arg0 : tensor<?x?xf32>, tensor<?x?xf32>)
                             outs(%arg1: tensor<?x?xf32>) -> tensor<?x?xf32>
...
func.func @fuse_no_tiling_packed_tile_sizes(%arg0: tensor<?x?xf32>, %arg1: tensor<?x?xf32>) -> tensor<?x?xf32> {
  ...
  //     CHECK-NOT: scf.for
  //     CHECK:       linalg.elementwise <exp>
  //     CHECK:       %[[RES:.*]] = linalg.elementwise <add>
  //     CHECK: return %[[RES]]
  %0 = linalg.elementwise <exp> ins(%arg0 : tensor<?x?xf32>)
                             outs(%arg1: tensor<?x?xf32>) -> tensor<?x?xf32>
  %1 = linalg.elementwise <add> ins(%0, %arg0 : tensor<?x?xf32>, tensor<?x?xf32>)
...
func.func @unpack_elemwise(%arg0: tensor<16x48x8x8xf32>, %arg1: tensor<128x384xf32>) -> tensor<128x384xf32> {
  ...
  %2 = linalg.elementwise <exp> ins(%1: tensor<128x384xf32>)
...
func.func @pack_elemwise(%arg0: tensor<128x384xf32>, %arg1: tensor<16x48x8x8xf32>) -> tensor<16x48x8x8xf32> {
  ...
  %2 = linalg.elementwise <exp> ins(%1: tensor<16x48x8x8xf32>)
...
func.func @fuse_through_slice(%arg0: tensor<?x?xf32>, %arg1: tensor<?x?xf32>) -> tensor<?x?xf32> {
  ...
  %2 = linalg.elementwise <add> ins(%1, %arg1 : tensor<?x?xf32>, tensor<?x?xf32>)
                             outs(%arg1: tensor<?x?xf32>) -> tensor<?x?xf32>
...
func.func @fuse_through_slice_and_cast_chain(%arg0: tensor<100x100xf32>, %arg1: tensor<?x?xf32>) -> tensor<?x?xf32> {
  ...
  //     CHECK:       linalg.elementwise <add>
  //     CHECK: return %[[RES]]
  %0 = linalg.elementwise <exp> ins(%arg0 : tensor<100x100xf32>)
  ...
  %5 = linalg.elementwise <add> ins(%4, %arg1 : tensor<?x?xf32>, tensor<?x?xf32>)
                             outs(%arg1: tensor<?x?xf32>) -> tensor<?x?xf32>
...
//     CHECK:       linalg.elementwise <exp> ins(%[[EXPAND]]
func.func @bubble_up_extract_slice_through_expand_shape(%0: tensor<60xf32>) -> tensor<2x3x10xf32> {
  ...
  %exp = linalg.elementwise <exp> ins(%expand : tensor<2x3x10xf32>) outs(%empty : tensor<2x3x10xf32>) -> tensor<2x3x10xf32>
...
func.func @bubble_up_extract_slice_through_expand_shape_full_inner_dim(%0: tensor<120xf32>) -> tensor<3x4x10xf32> {
  ...
  %exp = linalg.elementwise <exp> ins(%expand : tensor<3x4x10xf32>) outs(%empty : tensor<3x4x10xf32>) -> tensor<3x4x10xf32>
...
module {
  func.func @bubble_up_extract_slice_through_expand_shape_multiple_expanded_dims(%0: tensor<120x56xf32>) -> tensor<3x4x10x7x8xf32> {
    ...
    %exp = linalg.elementwise <exp> ins(%expand : tensor<3x4x10x7x8xf32>) outs(%empty : tensor<3x4x10x7x8xf32>) -> tensor<3x4x10x7x8xf32>
...
//     CHECK:    %[[ABS:.+]] = linalg.elementwise <abs> ins(%[[SLICE]]
...
module {
  func.func @bubble_up_extract_slice_through_expand_shape_and_fuse_with_expand_producer(%0: tensor<1x1800x256xf32>) -> tensor<1x1800x8x32xf32> {
    ...
    %exp1 = linalg.elementwise <abs> ins(%0 : tensor<1x1800x256xf32>) outs(%empty1 : tensor<1x1800x256xf32>) -> tensor<1x1800x256xf32>
    ...
    %exp2 = linalg.elementwise <exp> ins(%expand : tensor<1x1800x8x32xf32>) outs(%empty2 : tensor<1x1800x8x32xf32>) -> tensor<1x1800x8x32xf32>
...
//     CHECK:       linalg.elementwise <exp> ins(%[[SLICE]]
...
// CHECK:             %[[EXP1:.*]] = linalg.elementwise <exp> ins(%[[COLLAPSE]]
func.func @bubble_up_extract_slice_through_collapse_shape(%0: tensor<1x8x1800x32xf32>) -> tensor<8x1800x32xf32> {
  ...
  %exp = linalg.elementwise <exp> ins(%expand : tensor<8x1800x32xf32>) outs(%empty : tensor<8x1800x32xf32>) -> tensor<8x1800x32xf32>
...
// CHECK:           scf.for %[[X:[A-Za-z0-9]+]] = {{.*}}
// CHECK:             %[[EXTRACT:.*]] = tensor.extract_slice
// CHECK:             %[[ABS:.*]] = linalg.elementwise <abs> ins(%[[EXTRACT]]
// CHECK:             %[[COLLAPSE:.*]] = tensor.collapse_shape %[[ABS]]
// CHECK:             %[[EXP:.*]] = linalg.elementwise <exp> ins(%[[COLLAPSE]]
func.func @bubble_up_extract_slice_through_collapse_shape_with_collapse_producer(%0: tensor<1x8x1800x32xf32>) -> tensor<8x1800x32xf32> {
  ...
  %abs = linalg.elementwise <abs> ins(%0 : tensor<1x8x1800x32xf32>) outs(%empty1 : tensor<1x8x1800x32xf32>) -> tensor<1x8x1800x32xf32>
  ...
  %exp = linalg.elementwise <exp> ins(%expand : tensor<8x1800x32xf32>) outs(%empty2 : tensor<8x1800x32xf32>) -> tensor<8x1800x32xf32>
  return %exp : tensor<8x1800x32xf32>
}
...
func.func @fuse_producer_semi_affine_aligned(%arg0: tensor<12xf32>, %scale: tensor<4xf32>, %init: tensor<12xf32>, %out: tensor<12xf32>) -> tensor<12xf32> {
  ...
  %1 = linalg.elementwise <exp> ins(%0 : tensor<12xf32>) outs(%out : tensor<12xf32>) -> tensor<12xf32>
