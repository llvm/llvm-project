...
func.func @negative_hoist_subview_extra_memory_effect(
  ...
  scf.for %i = %lb to %ub step %step {
    ...
    %u0 = "test.val_use"(%r0) : (vector<2xf32>) -> vector<2xf32>
    vector.transfer_write %u0, %sv0[%c0] : vector<2xf32>, memref<2xf32, strided<[1]>>
    "test.prevent_hoisting"(%mem) : (memref<4xf32>) -> ()
  }
  return
}
...
// CHECK-NEXT:      %[[SV1:.*]] = memref.subview %[[MEM]][2, 0] [2, %[[SZ]]] [1, 1]
// CHECK-NEXT:      %[[R0:.*]] = vector.transfer_read %[[SV0]]
// CHECK-NEXT:      %[[R1:.*]] = vector.transfer_read %[[SV1]]
// CHECK-NEXT:      %[[FOR:.*]]:2 = scf.for {{.*}} iter_args(%[[I0:.*]] = %[[R0]], %[[I1:.*]] = %[[R1]]) -> (vector<2x4xf32>, vector<2x4xf32>) {
// CHECK-NEXT:        %[[U0:.*]] = "test.val_use"(%[[I0]])
// CHECK-NEXT:        %[[U1:.*]] = "test.val_use"(%[[I1]])
// CHECK-NEXT:        scf.yield %[[U0]], %[[U1]]
// CHECK-NEXT:      }
...
func.func @hoist_disjoint_dynamic_size_subviews(
  ...
  %c0 = arith.constant 0 : index
  %pad = arith.constant 0.0 : f32
  %sv0 = memref.subview %mem[0, 0][2, %sz][1, 1] : memref<4x8xf32> to memref<2x?xf32, strided<[8, 1]>>
  %sv1 = memref.subview %mem[2, 0][2, %sz][1, 1] : memref<4x8xf32> to memref<2x?xf32, strided<[8, 1], offset: 16>>
  scf.for %i = %lb to %ub step %step {
    %r0 = vector.transfer_read %sv0[%c0, %c0], %pad : memref<2x?xf32, strided<[8, 1]>>, vector<2x4xf32>
    %u0 = "test.val_use"(%r0) : (vector<2x4xf32>) -> vector<2x4xf32>
    vector.transfer_write %u0, %sv0[%c0, %c0] : vector<2x4xf32>, memref<2x?xf32, strided<[8, 1]>>

    %r1 = vector.transfer_read %sv1[%c0, %c0], %pad : memref<2x?xf32, strided<[8, 1], offset: 16>>, vector<2x4xf32>
    %u1 = "test.val_use"(%r1) : (vector<2x4xf32>) -> vector<2x4xf32>
    vector.transfer_write %u1, %sv1[%c0, %c0] : vector<2x4xf32>, memref<2x?xf32, strided<[8, 1], offset: 16>>
  }
  return
}
...
func.func @negative_hoist_dynamic_size_only_separation(
  ...
  %pad = arith.constant 0.0 : f32
  %sv0 = memref.subview %mem[0, 0][2, %sz][1, 1] : memref<4x8xf32> to memref<2x?xf32, strided<[8, 1]>>
  %sv1 = memref.subview %mem[0, 4][2, %sz][1, 1] : memref<4x8xf32> to memref<2x?xf32, strided<[8, 1], offset: 4>>
  scf.for %i = %lb to %ub step %step {
    %r0 = vector.transfer_read %sv0[%c0, %c0], %pad : memref<2x?xf32, strided<[8, 1]>>, vector<2x4xf32>
    %u0 = "test.val_use"(%r0) : (vector<2x4xf32>) -> vector<2x4xf32>
    vector.transfer_write %u0, %sv0[%c0, %c0] : vector<2x4xf32>, memref<2x?xf32, strided<[8, 1]>>

    %r1 = vector.transfer_read %sv1[%c0, %c0], %pad : memref<2x?xf32, strided<[8, 1], offset: 4>>, vector<2x4xf32>
    %u1 = "test.val_use"(%r1) : (vector<2x4xf32>) -> vector<2x4xf32>
    vector.transfer_write %u1, %sv1[%c0, %c0] : vector<2x4xf32>, memref<2x?xf32, strided<[8, 1], offset: 4>>
  }
  return
}
