...
// CHECK-LABEL: func @multi_reduction_to_arith_ops_2d
// CHECK-SAME:    %[[INPUT:.+]]: vector<4x2xf32>, %[[ACC:.+]]: vector<2xf32>
func.func @multi_reduction_to_arith_ops_2d(%arg0: vector<4x2xf32>, %acc: vector<2xf32>) -> vector<2xf32> {
    // CHECK: %[[V0:.+]] = vector.extract %[[INPUT]][0] : vector<2xf32> from vector<4x2xf32>
    // CHECK: %[[V1:.+]] = vector.extract %[[INPUT]][1] : vector<2xf32> from vector<4x2xf32>
    // CHECK: %[[V2:.+]] = vector.extract %[[INPUT]][2] : vector<2xf32> from vector<4x2xf32>
    // CHECK: %[[V3:.+]] = vector.extract %[[INPUT]][3] : vector<2xf32> from vector<4x2xf32>
    // CHECK: %[[RV0:.+]] = arith.addf %[[V0]], %[[ACC]] : vector<2xf32>
    // CHECK: %[[RV1:.+]] = arith.addf %[[V1]], %[[RV0]] : vector<2xf32>
    // CHECK: %[[RV2:.+]] = arith.addf %[[V2]], %[[RV1]] : vector<2xf32>
    // CHECK: %[[RESULT:.+]] = arith.addf %[[V3]], %[[RV2]] : vector<2xf32>
    %0 = vector.multi_reduction <add>, %arg0, %acc [0] : vector<4x2xf32> to vector<2xf32>
    // CHECK: return %[[RESULT]]
    return %0 : vector<2xf32>
}
...
module attributes {transform.with_named_sequence} {
  transform.named_sequence @__transform_main(%root : !transform.any_op {transform.readonly}) {
    ...
    transform.apply_patterns to %func_op {
      transform.apply_patterns.vector.multi_reduction_rank_reducing_unrolling
