...
func.func @subgroupId(%sz : index, %mem: memref<index, 1>) {
  gpu.launch blocks(%bx, %by, %bz) in (%grid_x = %sz, %grid_y = %sz, %grid_z = %sz)
    ...
    // CHECK-NEXT: %[[T0:.*]] = arith.muli %[[DIMY]], %[[TIDZ]] overflow<nsw, nuw> : index
    // CHECK-NEXT: %[[T1:.*]] = arith.addi %[[T0]], %[[TIDY]] overflow<nsw, nuw> : index
    // CHECK-NEXT: %[[T2:.*]] = arith.muli %[[DIMX]], %[[T1]] overflow<nsw, nuw> : index
    // CHECK-NEXT: %[[T3:.*]] = arith.addi %[[TIDX]], %[[T2]] overflow<nsw, nuw> : index
...
func.func @subgroupIdConsts(%sz : index, %mem: memref<index, 1>) {
  ...
  gpu.launch blocks(%bx, %by, %bz) in (%grid_x = %sz, %grid_y = %sz, %grid_z = %sz)
    ...
    // CHECK-NEXT: %[[T0:.*]] = arith.muli %[[TIDZ]], %[[DIMY]] overflow<nsw, nuw> : index
    // CHECK-NEXT: %[[T1:.*]] = arith.addi %[[T0]], %[[TIDY]] overflow<nsw, nuw> : index
    // CHECK-NEXT: %[[T2:.*]] = arith.muli  %[[T1]], %[[DIMX]] overflow<nsw, nuw> : index
    // CHECK-NEXT: %[[T3:.*]] = arith.addi %[[TIDX]], %[[T2]] overflow<nsw, nuw> : index
    // CHECK-NEXT: %[[T4:.*]] = gpu.subgroup_size : index
    // CHECK-NEXT: %[[T5:.*]] = arith.divui %[[T3]], %[[T4]] : index
    %idz = gpu.subgroup_id : index
