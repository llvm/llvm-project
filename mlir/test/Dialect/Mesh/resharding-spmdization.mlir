...
func.func @split_replicated_tensor_axis_dynamic(
  ...
  %s0 = mesh.sharding @mesh_1d_dynamic split_axis = [[], [], []] : !mesh.sharding
  %0 = mesh.shard %arg0 to %s0 : tensor<?x3x?xf32>
  %s1 = mesh.sharding @mesh_1d_dynamic split_axis = [[0]] : !mesh.sharding
...
func.func @move_split_axis_dynamic_mesh(
  ...
  %s0 = mesh.sharding @mesh_1d_dynamic split_axis = [[0]] : !mesh.sharding
  %0 = mesh.shard %arg0 to %s0 : tensor<10x14xf32>
  %s1 = mesh.sharding @mesh_1d_dynamic split_axis = [[], [0]] : !mesh.sharding
  %1 = mesh.shard %0 to %s1 annotate_for_users : tensor<10x14xf32>
  // CHECK: return %[[RES]] : tensor<10x14xf32>
...
// CHECK-SAME: %[[ARG:.*]]: tensor<10x14xf32>  
  ...
  %s0 = mesh.sharding @mesh_1d_dynamic split_axis = [[0]] : !mesh.sharding
  %0 = mesh.shard %arg0 to %s0 : tensor<10x14xf32>
  %s1 = mesh.sharding @mesh_1d_dynamic split_axis = [[]] : !mesh.sharding
  %1 = mesh.shard %0 to %s1 annotate_for_users : tensor<10x14xf32>
  // CHECK: return %[[RES]] : tensor<10x14xf32>
  return %1 : tensor<10x14xf32>
}
...
// CHECK-SAME: %[[ARG:.*]]: tensor<10x14xf32>  
  ...
  %s0 = mesh.sharding @mesh_1d split_axis = [[]] partial = sum[0] : !mesh.sharding
  %0 = mesh.shard %arg0 to %s0 : tensor<10x14xf32>
  %s1 = mesh.sharding @mesh_1d split_axis = [[]] : !mesh.sharding
  %1 = mesh.shard %0 to %s1 annotate_for_users : tensor<10x14xf32>
