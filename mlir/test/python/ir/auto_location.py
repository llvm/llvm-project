...
def testInferLocations():
    with Context() as ctx, with_infer_location():
        ...
        # fmt: off
        # CHECK: loc("ConstantOp.__init__"("{{.*}}[[SEP]]mlir[[SEP]]dialects[[SEP]]_arith_ops_gen.py":{{\d+}}:4 to :235))
        # fmt: on
        foo()
