...
Usage: symbolize_raw_stacks.py <binary> <memprof_output_file> [llvm-symbolizer]
...
file-relative addresses, and symbolizes them using llvm-symbolizer. Prints one
symbolized function name per line for addresses that fall within the main
binary.
"""
...
def main():
    ...
    for runtime_addr in sorted(addrs):
        ...
        result = subprocess.run(
            [symbolizer, "--obj=" + binary, hex(file_addr)],
            capture_output=True,
            text=True,
        )
