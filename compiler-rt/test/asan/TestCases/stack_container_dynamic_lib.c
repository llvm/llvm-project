...
static void init(struct Stack *s) {
  ...
  __sanitizer_annotate_contiguous_container(s->data, s->data + 5, s->data + 5, s->data);
#endif
}
...
#if __has_feature(address_sanitizer) && !defined(__SANITIZER_DISABLE_CONTAINER_OVERFLOW__)
  __sanitizer_annotate_contiguous_container(s->data, s->data + 5, s->data + s->size,
                                            s->data + s->size + 1);
#endif
  s->data[s->size++] = value;
}
...
#if __has_feature(address_sanitizer) && !defined(__SANITIZER_DISABLE_CONTAINER_OVERFLOW__)
  __sanitizer_annotate_contiguous_container(s->data, s->data + 5, s->data + s->size + 1,
                                            s->data + s->size);
#endif
  return result;
}
