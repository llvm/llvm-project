...
if "darwin" in config.available_features:
    ...
    if config.use_trap_loop:
        trap_loop = "-fsanitize-trap-loop " + \
                    "-include " + config.test_source_root + "/trap_loop_signal_handler.h "
    config.substitutions.append((r"%clang_cfi ", clang_cfi + non_dso + trap_loop))
    config.substitutions.append((r"%clangxx_cfi ", clang_cfi + cxx + non_dso + trap_loop))
