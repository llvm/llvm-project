...
#define __ptrauth_gcc_personality_lpad \
  __ptrauth_gcc_personality_intptr(ptrauth_key_function_pointer, 1, \
                                   __ptrauth_personality_lpad_disc)
...
#endif
{
  ...
  while (p < callSiteTableEnd) {
    uintptr_t __ptrauth_gcc_start start = readEncodedPointer(&p, callSiteEncoding);
    size_t __ptrauth_gcc_length length = readEncodedPointer(&p, callSiteEncoding);
    ...
    if ((start <= pcOffset) && (pcOffset < (start + length))) {
      ...
      const uintptr_t existingDiscriminator =
        ptrauth_blend_discriminator(&landingPad, __ptrauth_personality_lpad_disc);
      uintptr_t newIP /* opaque __ptrauth(ptrauth_key_return_address, stack_pointer, 0) */ =
        (uintptr_t)ptrauth_auth_and_resign(*(void **)&landingPad,
                                           ptrauth_key_function_pointer,
                                           existingDiscriminator, ptrauth_key_return_address,
                                           stack_pointer);
