...
static void initialize_fed_tables() {
  fed_tables = (fed_table_t *)malloc(sizeof(fed_table_t) * NUM_FED_TYPES);
...
// Ensure that the sizeinfo table of the given type has enough memory allocated
// to add a new unit's entries.
...
static inline void compute_inst_counts(instrumentation_counts_t *counts,
