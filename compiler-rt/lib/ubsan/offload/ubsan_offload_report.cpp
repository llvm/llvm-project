...
static_assert(ARRAY_SIZE(kLayout) == UBSAN_OFFLOAD_KIND_COUNT,
              "kLayout must match ubsan_offload_checks.inc");
...
bool InlineValues(unsigned Kind, void *S, uptr LocOff, unsigned NLoc) {
  ...
  switch (Kind) {
  case UBSAN_OFFLOAD_add_overflow:
  case UBSAN_OFFLOAD_sub_overflow:
  case UBSAN_OFFLOAD_mul_overflow:
  case UBSAN_OFFLOAD_divrem_overflow:
  case UBSAN_OFFLOAD_negate_overflow:
  case UBSAN_OFFLOAD_vla_bound_not_positive:
  case UBSAN_OFFLOAD_load_invalid_value:
  case UBSAN_OFFLOAD_float_cast_overflow:
    return Ok(0);
  case UBSAN_OFFLOAD_shift_out_of_bounds:
  case UBSAN_OFFLOAD_implicit_conversion:
    return Ok(0) && Ok(1);
  case UBSAN_OFFLOAD_out_of_bounds:
    return Ok(1);
  default:
    return true;
  }
...
bool Materialize(const __ubsan_offload_report &R, void **Data, ValueHandle *V0,
  ...
  *V2 = static_cast<ValueHandle>(R.val2);
  if (R.kind >= UBSAN_OFFLOAD_KIND_COUNT)
    return false;
...
void Replay(unsigned Kind, void *Data, ValueHandle V0, ValueHandle V1,
  ...
  switch (Kind) {
  case UBSAN_OFFLOAD_type_mismatch:
    handleTypeMismatchImpl(reinterpret_cast<TypeMismatchData *>(Data), V0,
                           Opts);
    break;
  case UBSAN_OFFLOAD_alignment_assumption:
  ...
  case UBSAN_OFFLOAD_out_of_bounds:
    ...
    break;
  case UBSAN_OFFLOAD_local_out_of_bounds:
  ...
  case UBSAN_OFFLOAD_builtin_unreachable:
    handleBuiltinUnreachableImpl(reinterpret_cast<UnreachableData *>(Data),
                                 Opts);
    break;
  case UBSAN_OFFLOAD_missing_return:
