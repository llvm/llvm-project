...
void SimpleShadowMem::insert_access_into_group(bool is_read, const csi_id_t acc_id,
                                               uintptr_t addr,
                                               size_t mem_size, FrameData_t *f,
                                               const call_stack_t &call_stack,
                                               value_type00 *dst) {
...
value_type00 *SimpleShadowMem::find(bool is_read, uintptr_t addr) {
  if (is_read)
...
value_type00 *SimpleShadowMem::find_group(bool is_read, uintptr_t addr,
                                                  size_t max_size,
                                                  size_t &num_elems) {
  if (is_read)
...
value_type00 *SimpleShadowMem::find_exact_group(bool is_read,
                                                        uintptr_t addr,
                                                        size_t max_size,
                                                        size_t &num_elems) {
  if (is_read)
...
bool SimpleShadowMem::does_access_exists(bool is_read, uintptr_t addr,
                                                 size_t mem_size) {
  if (is_read)
...
void SimpleShadowMem::clear(size_t start, size_t size) {
...
void SimpleShadowMem::record_alloc(size_t start, size_t size,
                                           FrameData_t *f,
                                           const call_stack_t &call_stack,
                                           csi_id_t alloca_id) {
...
void SimpleShadowMem::clear_alloc(size_t start, size_t size) {
...
void SimpleShadowMem::check_race(bool prev_read, bool is_read,
...
void SimpleShadowMem::check_race_with_prev_read(const csi_id_t acc_id,
                                                        uintptr_t addr,
                                                        size_t mem_size,
                                                        bool on_stack,
                                                        FrameData_t *f,
...
void SimpleShadowMem::check_race_with_prev_write(bool is_read,
                                                         const csi_id_t acc_id,
                                                         uintptr_t addr,
                                                         size_t mem_size,
                                                         bool on_stack,
...
void SimpleShadowMem::update(bool with_read, const csi_id_t acc_id,
                                     uintptr_t addr,
                                     size_t mem_size, bool on_stack,
                                     FrameData_t *f,
                                     const call_stack_t &call_stack) {
...
void SimpleShadowMem::update_with_write(const csi_id_t acc_id,
...
void SimpleShadowMem::update_with_read(const csi_id_t acc_id,
                                               uintptr_t addr, size_t mem_size,
                                               bool on_stack, FrameData_t *f,
                                               const call_stack_t &call_stack) {
...
void SimpleShadowMem::check_and_update_write(
