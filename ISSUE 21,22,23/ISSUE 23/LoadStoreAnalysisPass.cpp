...
#include "llvm/IR/Module.h" // Include Module header
...
namespace {
  ...
  struct LoadStoreAnalysisPass : public FunctionPass {
    ...
    bool runOnFunction(Function &F) override {
      ...
      reportResults(F, memoryAccessMap);
  ...
  private:
    ...
    void reportResults(Function &F, std::unordered_map<Value*, MemoryAccessInfo> &memoryAccessMap) {
      Module *M = F.getParent(); // Get the module from the function
      errs() << M->getSourceFileName() << ":" << F.getName() << "()\n";

      for (const auto &pair : memoryAccessMap) {
        Value *memoryLocation = pair.first;
        const MemoryAccessInfo &accessInfo = pair.second;
        errs() << "   " << *memoryLocation << ": " << accessInfo.totalMemoryAccessed << " bytes ("
               << accessInfo.memoryAccessFrequency << " times)\n";
      }
