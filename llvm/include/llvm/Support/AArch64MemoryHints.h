...
enum class AArch64MemoryHint {
  MEM_HINT_NONE = 0,
  MEM_HINT_STSHH_KEEP = 1,
  MEM_HINT_STSHH_STRM = 2,
};

template <typename Int> inline bool isValidAArch64MemoryHintValue(Int I) {
  return (Int)AArch64MemoryHint::MEM_HINT_STSHH_KEEP <= I &&
         I <= (Int)AArch64MemoryHint::MEM_HINT_STSHH_STRM;
}

template <typename Int> inline AArch64MemoryHint toAArch64MemoryHint(Int I) {
  switch (I) {
  case 0:
    return AArch64MemoryHint::MEM_HINT_STSHH_KEEP;
  case 1:
    return AArch64MemoryHint::MEM_HINT_STSHH_STRM;
  default:
