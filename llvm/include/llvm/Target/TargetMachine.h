...
public:
  ...
  /// callee-save register spilling and scavenging are not needed or used.
  virtual bool usesPhysRegsForValues() const { return true; }

  /// NEXT32
  /// True if the variadic defs should be allocated vregs. Implicitly defined
  /// registers will still be assumed to be physical registers.
  /// Next32 requires this as a separate flag because "usesPhysRegsForValues"
  /// is too restrictive (it still needs callee-save register spilling and
  /// scavenging).
  virtual bool usesVRegsForVariadicDefs() const { return false; }
