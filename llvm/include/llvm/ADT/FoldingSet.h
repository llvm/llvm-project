...
///
/// This class is implemented as an open-addressing hash set with linear
/// probing and Knuth TAOCP 6.4 Algorithm R deletion (tombstone-free).
/// It uniques heap-allocated objects by computing and comparing their profile
/// IDs.
///
...
protected:
  ...
  /// Packed 1-bit array indicating bucket occupancy.
  UsedT *Used = nullptr;
...
public:
  ...
  unsigned capacity() const { return (NumBuckets * 3) / 4; }
...
private:
  // Insert a node into an empty slot without checking capacity.
  void insertImpl(void *N, uint32_t Hash);

  // Allow the iterator to access the used array and bucket array.
