...
struct inlined_ls_hash {
  size_t operator()(const std::pair<const DILocalScope *, DebugLoc> &P) const {
    return std::hash<const DILocalScope *>()(P.first) * 31 + std::hash<uint64_t>()(P.second.getAsRawInteger());
  }
};
...
class LexicalScope {
public:
  LexicalScope(LexicalScope *P, const DILocalScope *D, DebugLoc I,
               bool A)
  ...
  DebugLoc getInlinedAt() const { return InlinedAtLocation; }
  DILocRef getInlinedAtLocRef() const {
    return InlinedAtLocation
      ? DILocRef(getInlinedAtSubprogram(), InlinedAtLocation)
      : DILocRef();
  }
  ...
  // Search up to the outermost scope and assume that corresponds to the DISubprogram.
  // FIXME: Verify this approach, or else find somewhere to keep a subprogram
  // reference around.
  DISubprogram *getInlinedAtSubprogram() const {
    const LexicalScope *Current = this;
    const LexicalScope *Parent = getParent();
    while (Parent) {
      Current = Parent;
      Parent = Current->getParent();
    }
    return getDISubprogram(getScopeNode());
  }
...
private:
  ...
  DebugLoc InlinedAtLocation;                  // Location at which this
                                               // scope is inlined.
...
public:
  ...
  LexicalScope *findInlinedScope(const DILocalScope *N, DebugLoc IA) {
