...
public:
  ...
  /// Create a chain of casts to convert V to NewTy. This may involve multiple
  /// casts (e.g., ptr -> i64 -> <2 x i32>). The created cast instructions are
  /// inserted into the current basic block. If no casts are needed, V is
  /// returned.
  LLVM_ABI Value *CreateCastChain(const DataLayout &DL, Value *V, Type *NewTy);
