...
/// PrintValueName, when provided, is used for unnamed global and local values.
/// PrintTypeName, when provided, is used for unnamed identified types. The
/// callbacks are owned by this object. When PrintCallAttributesInline is true,
/// function attributes on call-like instructions are printed inline instead
/// of through module attribute-group slots.
class LLVM_ABI FunctionInstructionPrinter {
  struct Impl;
  std::unique_ptr<Impl> P;

public:
  using ValueNameCallback = unique_function<void(raw_ostream &, const Value &)>;
  using TypeNameCallback = unique_function<void(raw_ostream &, const Type &)>;

  FunctionInstructionPrinter(raw_ostream &OS, ModuleSlotTracker &MST,
                             const Function &F, bool IsForDebug = false,
                             ValueNameCallback PrintValueName = nullptr,
                             TypeNameCallback PrintTypeName = nullptr,
