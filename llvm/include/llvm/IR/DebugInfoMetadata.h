...
/// Index for a DILoc in the context of a specific function. Note that this
/// is not unique across functions, therefore if comparing DILocs outside of a
/// function context you must also include the DISubprogram this is associated
/// with; see DILocRef below.
///// class DebugLoc;

class DISrcLocData {
public:
  uint32_t Line;
  // Not clear whether we need more bits for Column or AtomGroup right now;
  // either could be fine with 12 bits.
  uint16_t Column;
  uint16_t AtomGroup : 12;
  uint16_t AtomRank : 3;
  uint16_t IsImplicitCode : 1;
  
  DISrcLocData() = default;
  DISrcLocData(uint32_t Line, uint16_t Column = 0, uint16_t AtomGroup = 0, uint16_t AtomRank = 0, uint16_t IsImplicitCode = 0)
    : Line(Line), Column(Column), AtomGroup(AtomGroup), AtomRank(AtomRank), IsImplicitCode(IsImplicitCode) {}
  static DISrcLocData fromRawInteger(uint64_t RawInt) {
    return llvm::bit_cast<DISrcLocData>(RawInt);
  }
  uint64_t toRawInteger() const {
    return llvm::bit_cast<uint64_t>(*this);
  }
  bool operator==(const DISrcLocData &Other) const {
    return Line == Other.Line && Column == Other.Column && AtomGroup == Other.AtomGroup && AtomRank == Other.AtomRank
  ...
  bool operator<(const DISrcLocData &Other) const {
    ...
    if (AtomGroup != Other.AtomGroup)
      return AtomGroup < Other.AtomGroup;
    if (AtomRank != Other.AtomRank)
      return AtomRank < Other.AtomRank;
    return IsImplicitCode < Other.IsImplicitCode;
  }
};
class DILocScopeData {
public:
  DILocScopeData(DILocalScope *Scope, DebugLoc InlinedAt) :
    Scope(Scope), InlinedAt(InlinedAt) {}
  DILocalScope *Scope;
  DebugLoc InlinedAt;

  inline unsigned getDiscriminator() const;
  bool operator==(const DILocScopeData &Other) const {
    return Scope == Other.Scope && InlinedAt == Other.InlinedAt;
  }
};

class DILocationData {
public:
  DILocationData() : Scope(nullptr) {}
  DILocationData(DILocalScope *Scope, DISrcLocData SrcLoc) :
    Scope(Scope), Line(SrcLoc.Line), Column(SrcLoc.Column),
      AtomGroup(SrcLoc.AtomGroup), AtomRank(SrcLoc.AtomRank), IsImplicitCode(SrcLoc.IsImplicitCode) {}
  operator bool() {
...
class DIInlinedLocations : public SmallVector<DILocationData> {
public:
  DILocationData getInnerLocation() const {
    if (empty())
      return DILocationData();
    return (*this)[0];
  }
};
...
public:
  /// These fields are serialized; the others are transient or can be inferred
  /// from these two, so are not serialized.
  SmallVector<DILocScopeData, 0> FnLocScopes;
  SmallVector<DISrcLocData, 0> FnSrcLocs;

  /// Stores the FnSrcLocs that haven't been normalized; this should only
  /// include merged locations that have been 0-columned or had new AtomGroups
  /// set by optimizations.
  /// Cannot be parsed; must be merged into the normalized SrcLocs before
  /// printing.
  SmallVector<std::pair<DISrcLocData, uint32_t>, 0> NonNormalFnSrcLocs;
  
  /// Can be re-generated by finding the largest AtomGroup in FnSrcLocs.
  uint16_t NextAtomGroup = 1;
  
  /// Used to keep track of the ranges of FnSrcLocs that are associated with
  /// particular inline functions. For each inlined function (including nested)
  /// inlined functions) there will be an entry in this list with the index at
  /// which the inlined call SrcLoc appears, with all following SrcLocs being
  /// associated with that inlined call up to either the next entry in this list
  /// or the end of FnSrcLocs.
  /// Can be re-generated by finding the SrcLoc indices of all inlinedAt entries
  /// in FnLocScopes. 
  SmallVector<uint32_t, 0> FnInlinedAtSrcLocRanges;

/// Methods for managing the function loc data.
public:
  bool isUsingFnDebugLocs() {
    return !FnLocScopes.empty();
  }
  void setupFnDebugLocs() {
    assert(!isUsingFnDebugLocs());
    FnLocScopes.push_back(DILocScopeData(this, DebugLoc()));
    FnSrcLocs.push_back(DISrcLocData(0));
  }

  DISrcLocData getInstSrcLoc(const Instruction *I) const {
    return getSrcLoc(I->getDebugLoc());
  }
  DILocScopeData getInstLocScope(const Instruction *I) const {
    DebugLoc LocIdx = I->getDebugLoc();
    return FnLocScopes[LocIdx.LocScopeIndex];
  }
  DISrcLocData getSrcLoc(DebugLoc LocIdx) const {
    return getSrcLoc(LocIdx.SrcLocIndex);
  }
  DISrcLocData getSrcLoc(uint32_t SrcLocIndex) const {
    if (SrcLocIndex >= FnSrcLocs.size())
      return NonNormalFnSrcLocs[SrcLocIndex - FnSrcLocs.size()].first;
    return FnSrcLocs[SrcLocIndex];
  }
  DILocScopeData getLocScope(DebugLoc LocIdx) const {
    return FnLocScopes[LocIdx.LocScopeIndex];
  }
  /// Returns all the debug location information for a given DebugLoc,
  /// exluding the "inlinedAt" field or any inline frames above this location.
  DILocationData getInnerLocation(DebugLoc LocIdx) const {
    if (!LocIdx)
      return DILocationData();
    DISrcLocData SrcLoc = getSrcLoc(LocIdx);
    DILocScopeData LocScope = getLocScope(LocIdx);
    return DILocationData(LocScope.Scope, SrcLoc);
  }
  /// Returns all the debug location information for a given DebugLoc,
  /// including all the inlinedAt frames above it.
  DIInlinedLocations getFullLocation(DebugLoc LocIdx) const {
    DIInlinedLocations LocationList;
    while (LocIdx) {
      DISrcLocData SrcLoc = getSrcLoc(LocIdx);
      DILocScopeData LocScope = getLocScope(LocIdx);
      LocationList.push_back(DILocationData(LocScope.Scope, SrcLoc));
      LocIdx = LocScope.InlinedAt;
    }
    return LocationList;
  }
  /// Cannot currently be used to find either the location of inlined call sites.
  /// If InsertIfMissing=false, will return 0xFFFFFFFF if the location isn't found.
  uint32_t getSrcLocIndex(DISrcLocData SrcLoc, DebugLoc InlinedAt, bool InsertIfMissing) {
    if (SrcLoc.Line == 0)
      return 0;
    uint32_t SearchStart = 0;
    uint32_t SearchEnd = FnInlinedAtSrcLocRanges.empty() ? FnSrcLocs.size() : FnInlinedAtSrcLocRanges[0];
    if (InlinedAt) {
      SearchStart = InlinedAt.SrcLocIndex + 1;
      auto *SearchEndIdx = find(FnInlinedAtSrcLocRanges, InlinedAt.SrcLocIndex) + 1;
      SearchEnd = SearchEndIdx == FnInlinedAtSrcLocRanges.end() ? FnSrcLocs.size() : *SearchEndIdx;
    }
    auto *EndIt = FnSrcLocs.begin() + SearchEnd;
    auto *FoundIdx = std::lower_bound(FnSrcLocs.begin() + SearchStart, FnSrcLocs.begin() + SearchEnd, SrcLoc);
    if (FoundIdx != EndIt && *const_cast<DISrcLocData*>(FoundIdx) == SrcLoc)
      return std::distance(FnSrcLocs.begin(), FoundIdx);

    if (auto *NonNormalIt = find(NonNormalFnSrcLocs, std::make_pair(SrcLoc, InlinedAt.SrcLocIndex)); NonNormalIt != NonNormalFnSrcLocs.end())
      return std::distance(NonNormalFnSrcLocs.begin(), NonNormalIt) + FnSrcLocs.size();

    if (!InsertIfMissing)
      return 0xFFFFFFFF;

    uint32_t NewIdx = FnSrcLocs.size() + NonNormalFnSrcLocs.size();
    NonNormalFnSrcLocs.push_back(std::make_pair(SrcLoc, InlinedAt.SrcLocIndex));
    return NewIdx;
  }
  uint32_t getLocScopeIndex(DILocScopeData LocScope, bool InsertIfMissing) {
    if (auto *LocScopeIt = find(FnLocScopes, LocScope); LocScopeIt != FnLocScopes.end())
      return std::distance(FnLocScopes.begin(), LocScopeIt);

    if (!InsertIfMissing)
      return 0xFFFFFFFF;

    uint32_t NewIdx = FnLocScopes.size();
    FnLocScopes.push_back(LocScope);
    return NewIdx;
  }
  DebugLoc getDebugLoc(DISrcLocData SrcLoc, DILocScopeData LocScope, bool InsertIfMissing) {
    uint32_t LocScopeIndex = getLocScopeIndex(LocScope, InsertIfMissing);
    uint32_t SrcLocIndex = getSrcLocIndex(SrcLoc, LocScope.InlinedAt, InsertIfMissing);
    return DebugLoc(SrcLocIndex, LocScopeIndex);
  }
  DIInlinedLocations getInstInlinedLocs(const Instruction *I) const {
    DIInlinedLocations InlinedLocs;
    DebugLoc LocIdx = I->getDebugLoc();
    DISrcLocData SrcLoc = getSrcLoc(LocIdx);
    DILocScopeData LocScope = getLocScope(LocIdx);
    InlinedLocs.push_back(DILocationData(LocScope.Scope, SrcLoc));
    while (LocScope.InlinedAt) {
      SrcLoc = getSrcLoc(LocScope.InlinedAt);
      LocScope = getLocScope(LocScope.InlinedAt);
      InlinedLocs.push_back(DILocationData(LocScope.Scope, SrcLoc));
    }
    return InlinedLocs;
  }
  DILocalScope *getInlinedAtScope(DebugLoc LocIdx) const {
    DILocScopeData LocScope = getLocScope(LocIdx);
    while (LocScope.InlinedAt)
      LocScope = getLocScope(LocScope.InlinedAt);
    return LocScope.Scope;
  }

  void absorbDILocations(Function *F, SmallVectorImpl<DILocation *> &TransientDILocations);
  void absorbDILocations(Function *F, SmallVectorImpl<TrackingMDNodeRef> &TransientDILocations);
  SmallVector<DILocation *, 0> expelDILocations(Function *F);

  /// CallDL must be a DebugLoc associated with this Subprogram.
  /// Remaps any InlinedAt indices, and returns a DebugLoc that points to the
  /// outermost inlined scope, and the inlined call location; this should not be
  /// used directly for anything (as that combination is incoherent), but should
  /// instead be used as an offset for all DebugLocs in the inlined function,
  /// with the exception of any 0-index SrcLocs (which should remain 0).
  DebugLoc addInlinedLocations(DISubprogram *OtherSP, DebugLoc CallDL);

  inline unsigned getDiscriminator(DebugLoc LocIdx) const;
  inline unsigned getBaseDiscriminator(DebugLoc LocIdx) const;
  inline std::optional<DebugLoc> cloneByMultiplyingDuplicationFactor(DebugLoc DL, unsigned DF);
  inline std::optional<DebugLoc> cloneWithBaseDiscriminator(DebugLoc DL, unsigned D);

  DebugLoc cloneWithDiscriminator(DebugLoc Idx, unsigned Discriminator);
  DebugLoc getMergedLocation(DebugLoc LocA, DebugLoc LocB);
  DebugLoc getMergedLocations(ArrayRef<DebugLoc> Locs);

  void getUsedLocScopes(Function *F, SmallVectorImpl<DILocScopeData> &UsedLocScopes);
  void getUsedSrcLocs(Function *F, SmallVectorImpl<DISrcLocData> &UsedSrcLocs);
  void getUsedSrcLocScopes(Function *F, SmallVectorImpl<DISrcLocData> &UsedSrcLocs, SmallVectorImpl<DILocScopeData> &UsedLocScopes);

  // During parsing or in the frontend, we continually insert DebugLocs into 
  // DISubprograms for each instruction in the associated function, without
  // sorting or removing duplicates for performance and index stability reasons.
  // After we're done inserting new DebugLocs, this can be called to sort
  // DebugLocs properly and remove duplicates.
  void normalizeDebugLocs(Function *F);
...
inline DISrcLocData getSrcLoc(const Instruction &I) {
  return I.getFunction()->getSubprogram()->getSrcLoc(I.getDebugLoc());
}
inline DILocScopeData getLocScope(const Instruction &I) {
  return I.getFunction()->getSubprogram()->getLocScope(I.getDebugLoc());
}
inline DILocationData getLocationData(const Instruction &I) {
  return I.getFunction()->getSubprogram()->getInnerLocation(I.getDebugLoc());
}
inline DIInlinedLocations getFullLocationData(const Instruction &I) {
  return I.getFunction()->getSubprogram()->getFullLocation(I.getDebugLoc());
}

/// This class can be used to uniquely identify a DebugLoc outside of a
/// specific function context.
class MachineInstr;
class DILocRef {
public:
  DILocRef() : SP(nullptr), Index() {}
  DILocRef(DISubprogram *SP, DebugLoc Index) : SP(SP), Index(Index) {
    assert(SP);
  }
  DILocRef(Instruction *Pos, DebugLoc Index) : SP(Pos->getFunction()->getSubprogram()), Index(Index) {}
  DILocRef(BasicBlock *Pos, DebugLoc Index) : SP(Pos->getParent()->getSubprogram()), Index(Index) {}
  DILocRef(Function *Pos, DebugLoc Index) : SP(Pos->getSubprogram()), Index(Index) {}
  DILocRef(std::nullptr_t) : DILocRef() {}
  DILocRef(Instruction &I) : SP(I.getFunction()->getSubprogram()), Index(I.getDebugLoc()) {
    assert(I.getParent() && I.getFunction() && "Only safe to call this on an instruction inserted into a function!");
  }
  DILocRef(const Instruction &I) : DILocRef(const_cast<Instruction&>(I)) {
    assert(I.getParent() && I.getFunction() && "Only safe to call this on an instruction inserted into a function!");
  }
  DILocRef(DbgRecord &DR) : SP(DR.getFunction()->getSubprogram()), Index(DR.getDebugLoc()) {
    assert(DR.getBlock() && DR.getFunction() && "Only safe to call this on a DbgRecord inserted into a function!");
  }
  DILocRef(const DbgRecord &DR) : DILocRef(const_cast<DbgRecord&>(DR)) {
    assert(DR.getBlock() && DR.getFunction() && "Only safe to call this on a DbgRecord inserted into a function!");
  }
  DILocRef(MachineInstr &DR);
  DILocRef(const MachineInstr &DR);
  DILocRef(DILocRefWrapper Wrapper) : SP(Wrapper.SP), Index(Wrapper.Index) {}
  DILocRefWrapper toWrapper() { return DILocRefWrapper(SP, Index); }
  DISubprogram *SP;
  DebugLoc Index;

  operator bool() const { return (bool)Index; }
  operator DebugLoc() { return Index; }
  DILocRef *operator->() { return this; }
  DILocRef get() { return *this; }
  inline DILocScopeData getLocScope() {
    return SP->getLocScope(Index);
  }
  inline DISrcLocData getSrcLoc() {
    return SP->getSrcLoc(Index);
  }
  std::tuple<DISubprogram *, DebugLoc, uint64_t, uint8_t>
  getAtomInfo() const;
  uint64_t getAtomGroup() const { return SP->getSrcLoc(Index).AtomGroup; }
  uint8_t getAtomRank() const { return SP->getSrcLoc(Index).AtomRank; }
  unsigned getLine() const { return SP->getSrcLoc(Index).Line; }
  unsigned getColumn() const { return SP->getSrcLoc(Index).Column; }
  unsigned getCol() const { return getColumn(); }
  DILocalScope *getScope() const { return SP->getLocScope(Index).Scope; }
  bool isImplicitCode() const { return SP->getSrcLoc(Index).IsImplicitCode; }
  ...
  DILocRef getMergedLocation(DebugLoc Other) {
    return DILocRef(SP, SP->getMergedLocation(Index, Other));
  }

  DILocRef getInlinedAt() const {
    return DILocRef(SP, SP->getLocScope(Index).InlinedAt);
  }
...
public:
  inline unsigned getDiscriminator() const {
    return SP->getDiscriminator(Index);
  }
  inline DILocRef cloneWithDiscriminator(unsigned Discriminator) const {
    return DILocRef(SP, SP->cloneWithDiscriminator(Index, Discriminator));
  }
  inline std::optional<DILocRef> cloneWithBaseDiscriminator(unsigned BD) const {
    if (auto LocOpt = SP->cloneWithBaseDiscriminator(Index, BD))
      return DILocRef(SP, *LocOpt);
    return {};
  };
  inline std::optional<DILocRef>
  cloneByMultiplyingDuplicationFactor(unsigned DF) const {
    if (auto LocOpt = SP->cloneByMultiplyingDuplicationFactor(Index, DF))
      return DILocRef(SP, *LocOpt);
    return {};
  }
  ...
  void print(raw_ostream &OS) const {
    ...
    if (DILocRef InlinedAtDL = getInlinedAt()) {
      OS << " @[ ";
...
inline raw_ostream &operator<<(raw_ostream &OS, DILocRef DL) {
  DL.Index.print(OS, DL.SP);
  return OS;
}

inline DILocationData getInstLocData(const Instruction *I) {
  return I->getFunction()->getSubprogram()->getInnerLocation(I->getDebugLoc());
}
...
inline unsigned DILocationData::getDiscriminator() const {
  if (auto *F = dyn_cast<DILexicalBlockFile>(Scope))
    return F->getDiscriminator();
  return 0;
}
inline unsigned DILocationData::getBaseDiscriminator() const {
  return DILocation::getBaseDiscriminatorFromDiscriminator(
    getDiscriminator(), EnableFSDiscriminator);
}
inline unsigned DILocScopeData::getDiscriminator() const {
  if (auto *F = dyn_cast<DILexicalBlockFile>(Scope))
    return F->getDiscriminator();
  return 0;
}

inline unsigned DISubprogram::getDiscriminator(DebugLoc LocIdx) const {
  DILocScopeData LocScope = getLocScope(LocIdx);
  return LocScope.getDiscriminator();
}
inline unsigned DISubprogram::getBaseDiscriminator(DebugLoc LocIdx) const {
  return DILocation::getBaseDiscriminatorFromDiscriminator(
    getDiscriminator(LocIdx), EnableFSDiscriminator);
}
inline unsigned DILocRef::getBaseDiscriminator() const {
  return DILocation::getBaseDiscriminatorFromDiscriminator(
    SP->getDiscriminator(Index), EnableFSDiscriminator);
}

std::optional<DebugLoc>
DISubprogram::cloneByMultiplyingDuplicationFactor(DebugLoc DL, unsigned DF) {
  ...
  unsigned Discriminator = getDiscriminator(DL);
  if (DILocation::isPseudoProbeDiscriminator(Discriminator))
    return DL;

  DF *= DILocation::getDuplicationFactorFromDiscriminator(Discriminator);
  if (DF <= 1)
    return DL;

  unsigned BD = DILocation::getBaseDiscriminatorFromDiscriminator(
    Discriminator, EnableFSDiscriminator);
  unsigned CI = DILocation::getCopyIdentifierFromDiscriminator(Discriminator);
  if (std::optional<unsigned> D = DILocation::encodeDiscriminator(BD, DF, CI))
    return cloneWithDiscriminator(DL, *D);
  return std::nullopt;
}

std::optional<DebugLoc>
DISubprogram::cloneWithBaseDiscriminator(DebugLoc DL, unsigned D) {
  unsigned BD, DF, CI;
  unsigned Discriminator = getDiscriminator(DL);

  if (EnableFSDiscriminator) {
    BD = DILocation::getBaseDiscriminatorFromDiscriminator(
      Discriminator, EnableFSDiscriminator);
    if (D == BD)
      return DL;
    return cloneWithDiscriminator(DL, D);
  ...
  DILocation::decodeDiscriminator(Discriminator, BD, DF, CI);
  if (D == BD)
    return DL;
  if (std::optional<unsigned> Encoded = DILocation::encodeDiscriminator(D, DF, CI))
    return cloneWithDiscriminator(DL, *Encoded);
  return std::nullopt;
}
...
template <> struct DenseMapInfo<DebugVariable> {
  ...
  static unsigned getHashValue(const DebugVariable &D) {
    ...
    return hash_combine(D.getVariable(), HV, D.getInlinedAt().getAsRawInteger());
  }
