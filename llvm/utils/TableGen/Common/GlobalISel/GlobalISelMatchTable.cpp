...
void AddRegisterRenderer::emitRenderOpcodes(MatchTable &Table,
  ...
  if (IsDef) {
    Table << MatchTable::NamedValue(
        2, IsDead ? "static_cast<uint16_t>(RegState::Define | RegState::Dead)" : "static_cast<uint16_t>(RegState::Define)");
  } else {
    ...
    if (!I->ImplicitDefs.empty() || !I->ImplicitUses.empty()) {
      for (auto *Def : I->ImplicitDefs) {
        ...
        Table << MatchTable::Opcode("GIR_AddImplicitDef")
              ...
              << (IsDead ? MatchTable::NamedValue(2, "static_cast<unsigned>(RegState::Dead)")
