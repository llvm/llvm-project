...
void IntrinsicEmitter::EmitEnumInfo(const CodeGenIntrinsicTable &Ints,
  ...
  for (const auto &Int : Set->getIntrinsics()) {
    ...
    if (First) {
      Intrinsic::PrintIntrinsicIDEncoding(OS, TargetIndex, 0);
      First = false;
    }
  ...
  if (IntrinsicPrefix.empty()) {
    const size_t NumTargets = Ints.getAllTargets().size();
    ...
    Intrinsic::PrintIntrinsicIDEncoding(OS, NumTargets - 1, LastCount - 1);
    OS << ",\n";
    OS << "#endif\n\n";
  } else {
...
static constexpr IntrinsicLinearIndexInfo TargetLinearIndexInfos[] = {
  ...
  for (const auto &[TIdx, T] : enumerate(Ints.getAllTargets())) {
    if (!T.Enabled)
      continue;
    OS << formatv("  {{{}, {}, {}}, // {}\n", T.FirstLinearIndex,
                  T.Intrinsics.size(), TIdx, T.Name);
  }
  OS << R"(};
#endif
