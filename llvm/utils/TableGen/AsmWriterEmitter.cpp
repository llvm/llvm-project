...
private:
  ...
  void EmitOpcodeGroupDispatch(raw_ostream &O, const OpcodeGroupList &Groups,
                               bool PassSubtarget);
};
...
AsmWriterEmitter::EmitOpcodeGroupTable(raw_ostream &O) {
  ...
  // Use StringMap for O(1) insertion/lookup during accumulation, then
  // convert to OpcodeGroupList to preserve a stable group ordering.
  StringMap<std::vector<AsmWriterInst>> GroupMap;
  SmallVector<std::string, 16> GroupOrder;
  for (const AsmWriterInst &AWI : OpcodeInsts) {
    std::string Key = getGroupKey(AWI);
    if (GroupMap.try_emplace(Key).second)
      GroupOrder.push_back(Key);
    GroupMap[Key].push_back(AWI);
  }
  for (const std::string &Key : GroupOrder)
    Groups.push_back({Key, std::move(GroupMap[Key])});
