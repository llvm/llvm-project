...
static void GenerateGetLeafConstructs(const DirectiveLanguage &DirLang,
                                      raw_ostream &OS) {
  llvm::StringMap<Record *> NameToRec;
  for (Record *R : DirLang.getDirectives())
    NameToRec.insert(std::make_pair(BaseRecord(R).getName(), R));
  ...
  // Generate the locals.
  for (auto &[_, R] : NameToRec) {
    Directive Dir{R};

    std::vector<StringRef> LeafNames = Dir.getLeafConstructNames();
    if (LeafNames.empty())
      continue;

    std::string ListName = "leafConstructs_" + Dir.getFormattedName();
    OS << "  static " << DirectiveListTypeName << ' ' << ListName << " {\n";
    for (StringRef L : LeafNames) {
      Directive LeafDir{NameToRec.at(L)};
