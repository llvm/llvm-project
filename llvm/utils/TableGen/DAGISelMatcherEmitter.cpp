...
unsigned MatcherTableEmitter::EmitMatcher(const Matcher *N,
  ...
  case Matcher::CheckType: {
    ...
    if (cast<CheckTypeMatcher>(N)->getResNo() == 0) {
      if (VTBH.isSimple()) {
        ...
        if (!OmitComments)
          OS << "/*" << VTBH << "*/";
        unsigned NumBytes = EmitVBRValue(getValueTypeID(VTBH), OS);
        OS << '\n';
        return NumBytes + 1;
      }
    }
    if (VTBH.isSimple()) {
      OS << "OPC_CheckTypeRes, " << cast<CheckTypeMatcher>(N)->getResNo()
         << ", ";
      if (!OmitComments)
        OS << "/*" << getEnumName(VTBH.getSimple()) << "*/";
      unsigned NumBytes = EmitVBRValue(VTBH.getSimple().SimpleTy, OS);
      OS << '\n';
      return NumBytes + 2;
    }
  ...
  case Matcher::EmitInteger: {
    ...
    if (VTBH.isSimple()) {
      ...
      if (!OmitComments)
        OS << "/*" << VTBH << "*/";
      TypeBytes = EmitVBRValue(getValueTypeID(VTBH), OS);
      OS << ' ';
    }
  ...
  case Matcher::EmitRegister: {
    ...
    if (VTBH.isSimple()) {
      ...
      if (Reg && Reg->EnumValue > 255) {
        ...
        if (!OmitComments)
          OS << "/*" << VTBH << "*/";
        OpBytes = EmitVBRValue(getValueTypeID(VTBH), OS);
        OS << " TARGET_VAL(" << getQualifiedName(Reg->TheDef) << "),\n";
        return OpBytes + 3;
      }
      ...
      if (!OmitComments)
        OS << "/*" << VTBH << "*/";
      OpBytes = EmitVBRValue(getValueTypeID(VTBH), OS) + 1;
      OS << ' ';
    }
    if (Reg) {
  ...
  case Matcher::MorphNodeTo: {
    ...
    if (ByHwMode) {
      for (unsigned i = 0, e = EN->getNumVTs(); i != e; ++i) {
        ...
        if (!OmitComments)
          OS << "/*" << VTBH << "*/";
        NumTypeBytes += EmitVBRValue(getValueTypeID(VTBH), OS);
      }
    } else {
      for (unsigned i = 0, e = EN->getNumVTs(); i != e; ++i) {
        OS << ' ';
        if (!OmitComments)
          OS << "/*" << getEnumName(EN->getVT(i).getSimple()) << "*/";
        NumTypeBytes += EmitVBRValue(EN->getVT(i).getSimple().SimpleTy, OS);
      }
    }
