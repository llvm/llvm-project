...
def parse_commandline_args(parser):
    ...
    class FilterAction(RegexAction):
        ...
        def __call__(self, parser, namespace, values, option_string=None):
            ...
            value_list[-1] = Filter(value_list[-1].regex, is_filter_out, is_filter_all_after)
...
def do_filter(body, filters):
    ...
    return "\n".join(
        filter(lambda line: apply_filters(line, filters), lines)
    )
    ...
    def __init__(self, run_list, flags, scrubber_args, path, ginfo):
        ...
        self._filters = (
            list(
                map(
                    lambda f: Filter(
                        re.compile(f.pattern().strip('"'), f.flags()), f.is_filter_out, f.is_filter_all_after
                    ),
                    flags.filters,
                )
            )
