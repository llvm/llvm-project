...
def check_for_command(
    line, parser, args, argv, argparse_callback, autogenerated_script_name
):
    cmd_m = UTC_ARGS_CMD.match(line)
    # For lines in the autogenerated UTC header, only process the entry
    # associated with the current script. This filtering does not apply to
    # inline UTC_ARGS directives, such as --disable/--enable.
    if cmd_m and (
        UTC_ADVERT not in line or (UTC_ADVERT + autogenerated_script_name) in line
    ):
