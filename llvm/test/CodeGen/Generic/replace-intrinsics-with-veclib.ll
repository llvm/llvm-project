...
; RUN: opt -vector-library=HVML -replace-with-veclib -S < %s | FileCheck %s  --check-prefixes=COMMON,HVML
...
;
; HVML-LABEL: define {{[^@]+}}@exp_v4
; HVML-SAME: (<4 x double> [[IN:%.*]]) {
; HVML-NEXT:    [[TMP1:%.*]] = call <4 x double> @hvml_vd4_exp(<4 x double> [[IN]])
; HVML-NEXT:    ret <4 x double> [[TMP1]]
;
...
;
; HVML-LABEL: define {{[^@]+}}@exp_f32
; HVML-SAME: (<4 x float> [[IN:%.*]]) {
; HVML-NEXT:    [[TMP1:%.*]] = call <4 x float> @hvml_vs4_exp(<4 x float> [[IN]])
; HVML-NEXT:    ret <4 x float> [[TMP1]]
;
