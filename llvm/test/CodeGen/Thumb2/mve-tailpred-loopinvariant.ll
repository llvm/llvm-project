...
; This test has exit count computed as a pointer difference using ptrtoaddr.
; We need to make sure it is loop invariant.
...
while.body.preheader:                             ; preds = %entry
  %b.addr = ptrtoaddr ptr %b to i32
  %c.addr = ptrtoaddr ptr %c to i32
  %exitcount = sub i32 %b.addr, %c.addr
  %n.rnd.up = add i32 %exitcount, 15
...
vector.body:                                      ; preds = %vector.body, %while.body.preheader
  ...
  %active.lane.mask = call <16 x i1> @llvm.get.active.lane.mask.v16i1.i32(i32 %index, i32 %exitcount)
