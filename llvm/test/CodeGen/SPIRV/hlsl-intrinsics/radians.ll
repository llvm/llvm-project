; RUN: llc -verify-machineinstrs -O0 -mtriple=spirv-unknown-unknown %s -o - | FileCheck %s -DSTD=GLSL.std.450 -DOPNAME=Radians
; RUN: llc -verify-machineinstrs -O0 -mtriple=spirv64-unknown-unknown %s -o - | FileCheck %s -DSTD=OpenCL.std -DOPNAME=radians
; RUN: %if spirv-tools %{ llc -O0 -mtriple=spirv-unknown-unknown %s -o - -filetype=obj | spirv-val %}
; RUN: %if spirv-tools %{ llc -O0 -mtriple=spirv64-unknown-unknown %s -o - -filetype=obj | spirv-val %}

; CHECK-DAG: %[[#op_ext_glsl:]] = OpExtInstImport "[[STD]]"
...
; CHECK: %[[#]] = OpExtInst %[[#float_32]] %[[#op_ext_glsl]] [[OPNAME]] %[[#float_32_arg]]
...
; CHECK: %[[#]] = OpExtInst %[[#float_16]] %[[#op_ext_glsl]] [[OPNAME]] %[[#float_16_arg]]
...
; CHECK: %[[#]] = OpExtInst %[[#vec4_float_32]] %[[#op_ext_glsl]] [[OPNAME]] %[[#vec4_float_32_arg]]
...
; CHECK: %[[#]] = OpExtInst %[[#vec4_float_16]] %[[#op_ext_glsl]] [[OPNAME]] %[[#vec4_float_16_arg]]
