...
entry:
  ...
  ; CHECK: %[[#EXTRACT4:]] = OpCompositeExtract %[[#int]] %[[#SHUFFLE1]] 0
  ; CHECK: %[[#EXTRACT5:]] = OpCompositeExtract %[[#int]] %[[#SHUFFLE1]] 1
  ; CHECK: %[[#EXTRACT6:]] = OpCompositeExtract %[[#int]] %[[#SHUFFLE1]] 2
  ; CHECK: %[[#SHUFFLE2:]] = OpVectorShuffle %[[#v8i32]] %[[#BITCAST1]] %{{[a-zA-Z0-9_]+}} 1 3 5 0xFFFFFFFF 0xFFFFFFFF 0xFFFFFFFF 0xFFFFFFFF 0xFFFFFFFF
  ; CHECK: %[[#EXTRACT7:]] = OpCompositeExtract %[[#int]] %[[#SHUFFLE2]] 0
  ; CHECK: %[[#EXTRACT8:]] = OpCompositeExtract %[[#int]] %[[#SHUFFLE2]] 1
  ; CHECK: %[[#EXTRACT9:]] = OpCompositeExtract %[[#int]] %[[#SHUFFLE2]] 2
  ; CHECK: %[[#CONSTRUCT2:]] = OpCompositeConstruct %[[#v8i32]] %[[#EXTRACT4]] %[[#EXTRACT5]] %[[#EXTRACT6]] %{{[a-zA-Z0-9_]+}} %{{[a-zA-Z0-9_]+}} %{{[a-zA-Z0-9_]+}} %{{[a-zA-Z0-9_]+}} %{{[a-zA-Z0-9_]+}}
  ; CHECK: %[[#CONSTRUCT3:]] = OpCompositeConstruct %[[#v8i32]] %[[#EXTRACT7]] %[[#EXTRACT8]] %[[#EXTRACT9]] %{{[a-zA-Z0-9_]+}} %{{[a-zA-Z0-9_]+}} %{{[a-zA-Z0-9_]+}} %{{[a-zA-Z0-9_]+}} %{{[a-zA-Z0-9_]+}}
  ; CHECK: %[[#SHUFFLE2:]] = OpVectorShuffle %[[#v8i32]] %[[#CONSTRUCT2]] %[[#CONSTRUCT3]] 0 8 1 9 2 10 0xFFFFFFFF 0xFFFFFFFF
  ; CHECK: %[[#BITCAST2:]] = OpBitcast %[[#v4f64]] %[[#SHUFFLE2]]
