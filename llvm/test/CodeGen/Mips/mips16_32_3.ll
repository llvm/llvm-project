; RUN: llc  -mtriple=mipsel -mattr=mips16 -relocation-model=static -O3 < %s  | FileCheck %s -check-prefix=16
; RUN: llc  -mtriple=mipsel -mcpu=mips32 -relocation-model=static -O3 < %s  | FileCheck %s -check-prefix=32
