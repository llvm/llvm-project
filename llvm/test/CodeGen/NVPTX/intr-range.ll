...
; CHECK-LABEL: define ptx_kernel i32 @test_maxntid_product_overflow(
; CHECK-SAME: ) #[[ATTR6:[0-9]+]] {
; CHECK-NEXT:    [[TMP1:%.*]] = call range(i32 0, 1024) i32 @llvm.nvvm.read.ptx.sreg.tid.x()
; CHECK-NEXT:    [[TMP2:%.*]] = call range(i32 1, 1025) i32 @llvm.nvvm.read.ptx.sreg.ntid.x()
; CHECK-NEXT:    [[TMP3:%.*]] = call range(i32 0, 1024) i32 @llvm.nvvm.read.ptx.sreg.tid.y()
; CHECK-NEXT:    [[TMP4:%.*]] = call range(i32 1, 1025) i32 @llvm.nvvm.read.ptx.sreg.ntid.y()
; CHECK-NEXT:    [[TMP5:%.*]] = add i32 [[TMP1]], [[TMP2]]
; CHECK-NEXT:    [[TMP6:%.*]] = add i32 [[TMP5]], [[TMP3]]
; CHECK-NEXT:    [[TMP7:%.*]] = add i32 [[TMP6]], [[TMP4]]
...
; Try a maxntid product that is an exact multiple of 2^32 (65536*65536*1).
define ptx_kernel i32 @test_maxntid_product_2pow32() "nvvm.maxntid"="65536,65536,1" {
; CHECK-LABEL: define ptx_kernel i32 @test_maxntid_product_2pow32(
; CHECK-SAME: ) #[[ATTR7:[0-9]+]] {
; CHECK-NEXT:    [[TMP1:%.*]] = call range(i32 1, 1025) i32 @llvm.nvvm.read.ptx.sreg.ntid.x()
; CHECK-NEXT:    ret i32 [[TMP1]]
;
...
;.
; CHECK: attributes #[[ATTR0]] = { "nvvm.maxntid"="32,1,3" "target-cpu"="sm_20" }
; CHECK: attributes #[[ATTR1]] = { "nvvm.reqntid"="20" "target-cpu"="sm_20" }
; CHECK: attributes #[[ATTR2]] = { "nvvm.maxntid"="4" "target-cpu"="sm_20" }
; CHECK: attributes #[[ATTR3]] = { "nvvm.maxclusterrank"="8" "target-cpu"="sm_20" }
; CHECK: attributes #[[ATTR4]] = { "nvvm.cluster_dim"="4,4,1" "target-cpu"="sm_20" }
; CHECK: attributes #[[ATTR5:[0-9]+]] = { nocallback nofree nosync nounwind speculatable willreturn memory(none) "target-cpu"="sm_20" }
; CHECK: attributes #[[ATTR6]] = { "nvvm.maxntid"="641,6700417" "target-cpu"="sm_20" }
; CHECK: attributes #[[ATTR7]] = { "nvvm.maxntid"="65536,65536,1" "target-cpu"="sm_20" }
;.
