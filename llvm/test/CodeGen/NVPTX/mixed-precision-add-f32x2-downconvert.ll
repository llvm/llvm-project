...
define <2 x bfloat> @add_rn_bf16x2_f32x2(<2 x float> %a, <2 x float> %b) {
; CHECK-LABEL: add_rn_bf16x2_f32x2(
; CHECK:       {
; CHECK-NEXT:    .reg .b32 %r<4>;
; CHECK-NEXT:    .reg .b64 %rd<4>;
; CHECK-EMPTY:
; CHECK-NEXT:  // %bb.0:
; CHECK-NEXT:    ld.param::func.b64 %rd1, [add_rn_bf16x2_f32x2_param_0];
; CHECK-NEXT:    ld.param::func.b64 %rd2, [add_rn_bf16x2_f32x2_param_1];
; CHECK-NEXT:    add.rn.f32x2 %rd3, %rd1, %rd2;
; CHECK-NEXT:    mov.b64 {%r1, %r2}, %rd3;
; CHECK-NEXT:    cvt.rz.bf16x2.f32 %r3, %r2, %r1;
; CHECK-NEXT:    st.param::func.b32 [func_retval0], %r3;
; CHECK-NEXT:    ret;
  %sum = call <2 x float> @llvm.nvvm.fadd.v2f32(<2 x float> %a, <2 x float> %b, i32 1)
  %lo = extractelement <2 x float> %sum, i32 0
  %hi = extractelement <2 x float> %sum, i32 1
  %r = call <2 x bfloat> @llvm.nvvm.ff2bf16x2.rz(float %hi, float %lo, i1 false)
  ret <2 x bfloat> %r
