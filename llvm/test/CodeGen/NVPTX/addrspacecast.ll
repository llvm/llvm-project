...
; RUN: llc -O0 < %s -mtriple=nvptx64 -mcpu=sm_20 -target-abi=shortptr | FileCheck %s -check-prefixes=ALL,PTRCONV,CLS64
...
; RUN: %if ptxas %{ llc -O0 < %s -mtriple=nvptx64 -mcpu=sm_20 -target-abi=shortptr | %ptxas-verify %}
