...
; RUN: opt -mtriple=amdgpu10.10-amd-amdhsa -amdgpu-enable-uniform-intrinsic-combine=0 -O3 -S < %s | FileCheck %s -check-prefix=CURRENT-CHECK
...
; RUN: opt -mtriple=amdgpu10.10-amd-amdhsa -O3 -S < %s | FileCheck %s -check-prefix=O3-CHECK
...
; PASS-CHECK-SAME: ptr addrspace(1) [[OUT:%.*]]) {
...
; PASS-CHECK-SAME: ptr addrspace(1) [[OUT:%.*]], i32 [[MYMASK:%.*]]) {
; PASS-CHECK-NEXT:  [[ENTRY:.*]]:
; PASS-CHECK-NEXT:    br label %[[WHILE:.*]]
; PASS-CHECK:       [[WHILE]]:
; PASS-CHECK-NEXT:    [[DONE:%.*]] = phi i1 [ false, %[[ENTRY]] ], [ [[NEW_DONE:%.*]], %[[TAIL:.*]] ]
