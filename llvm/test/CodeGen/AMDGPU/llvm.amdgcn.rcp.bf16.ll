...
declare i16 @llvm.amdgcn.rcp.bf16(i16) #0

; GCN-LABEL: {{^}}rcp_bf16:
; GCN: v_rcp_bf16_e32 {{v[0-9]+}}, {{s[0-9]+}}
define amdgpu_kernel void @rcp_bf16(ptr addrspace(1) %out, i16 %src) #1 {
  %rcp = call i16 @llvm.amdgcn.rcp.bf16(i16 %src) #0
  store i16 %rcp, ptr addrspace(1) %out, align 2
  ret void
}

; GCN-LABEL: {{^}}rcp_bf16_constant_4
; GCN: v_rcp_bf16_e32 {{v[0-9]+}}, 4
define amdgpu_kernel void @rcp_bf16_constant_4(ptr addrspace(1) %out) #1 {
  %rcp = call i16 @llvm.amdgcn.rcp.bf16(i16 4) #0
  store i16 %rcp, ptr addrspace(1) %out, align 2
  ret void
}

; GCN-LABEL: {{^}}rcp_bf16_constant_100
; GCN: v_rcp_bf16_e32 {{v[0-9]+}}, 0x64
define amdgpu_kernel void @rcp_bf16_constant_100(ptr addrspace(1) %out) #1 {
  %rcp = call i16 @llvm.amdgcn.rcp.bf16(i16 100) #0
  store i16 %rcp, ptr addrspace(1) %out, align 2
  ret void
}
...
define amdgpu_kernel void @rcp_undef_bf16(ptr addrspace(1) %out) #1 {
  %rcp = call i16 @llvm.amdgcn.rcp.bf16(i16 undef)
  store i16 %rcp, ptr addrspace(1) %out, align 2
  ret void
}
