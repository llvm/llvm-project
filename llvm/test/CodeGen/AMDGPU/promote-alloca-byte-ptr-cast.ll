...
; byte-vector and a pointer of the same size. A direct bitcast between byte
; and pointer types is valid and preserves provenance, so it must not be
; routed through inttoptr/ptrtoint.
...
; CHECK-NEXT:    [[TMP2:%.*]] = bitcast <2 x b32> [[TMP0]] to ptr
