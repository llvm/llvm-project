...
; RUN: llc -mtriple=amdgcn -mcpu=gfx900 -amdgpu-enable-promote-alloca=0 < %s | FileCheck -check-prefixes=GFX900,GFX900-MUBUF %s
; RUN: llc -mtriple=amdgcn -mcpu=gfx906 -amdgpu-enable-promote-alloca=0 -mattr=+sram-ecc < %s | FileCheck --check-prefix=GFX906 %s
; RUN: llc -mtriple=amdgcn -mcpu=fiji -amdgpu-enable-promote-alloca=0 < %s | FileCheck --check-prefix=GFX803 %s
; RUN: llc -mtriple=amdgcn -mcpu=gfx900 -amdgpu-enable-promote-alloca=0 --mattr=+enable-flat-scratch < %s | FileCheck -check-prefixes=GFX900,GFX900-FLATSCR %s
