...
define void @test_ds_atomic_barrier_arrive_rtn_b64(i64 %data, ptr addrspace(3) %bar) #0 {
  ...
  ; SDAG-NEXT: {{  $}}
  ; SDAG-NEXT:   dead renamable $vgpr0_64 = DS_ATOMIC_BARRIER_ARRIVE_RTN_B64 killed renamable $vgpr2, killed renamable $vgpr0_64, 0, 0, implicit $exec :: (load store monotonic (s64) on %ir.bar, addrspace 3)
  ...
  ; GISEL-NEXT: {{  $}}
  ; GISEL-NEXT:   dead renamable $vgpr0_64 = DS_ATOMIC_BARRIER_ARRIVE_RTN_B64 killed renamable $vgpr2, killed renamable $vgpr0_64, 0, 0, implicit $exec :: (load store monotonic (i64) on %ir.bar, addrspace 3)
