...
; RUN: llc -mtriple=amdgpu12.51 < %s | FileCheck -check-prefix=GFX1250 %s
; RUN: llc -mtriple=amdgpu13.10 < %s | FileCheck -check-prefix=GFX1310 %s

; End-to-end coverage for the V_PERM_PK16 hazard. A store
; (non-safe) follows the perm, so a V_NOP must be inserted. EXEC is already
; non-zero at the perm (s_cbranch_execz is retained because V_PERM_PK16 has
; unwanted effects when EXEC is empty). Entry functions additionally get a
; defensive mitigation at the start.
;
; The hazard affects gfx1250 and gfx1251. gfx13 (gfx1310) has the same
