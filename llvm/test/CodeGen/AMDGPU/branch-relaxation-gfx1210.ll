...
; RUN: llc -mtriple=amdgcn -mcpu=gfx1210 -verify-machineinstrs -amdgpu-s-branch-bits=4 -simplifycfg-require-and-preserve-domtree=1 -amdgpu-long-branch-factor=0 < %s | FileCheck -enable-var-scope -check-prefix=GCN %s


; FIXME: We should use llvm-mc for this, but we can't even parse our own output.
;        See PR33579.
; RUN: llc -mtriple=amdgcn -mcpu=gfx1210 -verify-machineinstrs -amdgpu-s-branch-bits=4 -amdgpu-long-branch-factor=0 -o %t.o -filetype=obj -simplifycfg-require-and-preserve-domtree=1 %s
...
; each instruction is the maximum size, 12 bytes in case of gfx1210.
...
; GCN:       ; %bb.0: ; %bb
; GCN-NEXT:    s_load_b128 s[4:7], s[0:1], 0x2c
...
; GCN-NEXT:    s_wait_alu 0xfffe
; GCN-NEXT:    s_and_not1_b32 vcc_lo, exec_lo, s4
...
; GCN-NEXT:    s_wait_alu 0xfffe
; GCN-NEXT:    s_and_not1_b32 vcc_lo, exec_lo, s3
...
; GCN-NEXT:    s_wait_alu 0xfffe
; GCN-NEXT:    s_and_b32 s2, s2, s3
; GCN-NEXT:    s_wait_alu 0xfffe
; GCN-NEXT:    v_cndmask_b32_e64 v0, 0, 1, s2
; GCN-NEXT:  ; %bb.8: ; %bb14
...
; GCN-NEXT:    s_load_b64 s[2:3], s[0:1], 0x3c
; GCN-NEXT:    s_load_b64 s[0:1], s[0:1], 0x24
; GCN-NEXT:    v_mov_b32_e32 v1, 0
; GCN-NEXT:    s_wait_kmcnt 0x0
