...
; RUN: llc < %s -mtriple=amdgcn -mcpu=gfx1200 -mattr=-real-true16 -amdgpu-enable-delay-alu=0 -verify-machineinstrs | FileCheck -enable-var-scope -check-prefixes=GFX12-PACKED %s
; RUN: llc < %s -mtriple=amdgcn -mcpu=gfx1300 -mattr=+real-true16 -amdgpu-enable-delay-alu=0 -verify-machineinstrs | FileCheck -enable-var-scope -check-prefixes=GFX13-PACKED %s
; RUN: llc < %s -mtriple=amdgcn -mcpu=gfx1300 -mattr=-real-true16 -amdgpu-enable-delay-alu=0 -verify-machineinstrs | FileCheck -enable-var-scope -check-prefixes=GFX13-PACKED %s
...
; GFX13-PACKED:       ; %bb.0: ; %main_body
; GFX13-PACKED-NEXT:    tbuffer_load_d16_format_xyz v[0:1], off, s[0:3], null format:[BUF_FMT_32_FLOAT]
; GFX13-PACKED-NEXT:    s_wait_loadcnt 0x0
