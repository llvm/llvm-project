; NOTE: Assertions have been autogenerated by utils/update_llc_test_checks.py UTC_ARGS: --version 5
; RUN: llc -global-isel=0 -mtriple=amdgcn -mcpu=gfx1210 -verify-machineinstrs < %s | FileCheck -check-prefixes=GFX1210,GFX1210-SDAG %s
; RUN: llc -global-isel=1 -mtriple=amdgcn -mcpu=gfx1210 -verify-machineinstrs < %s | FileCheck -check-prefixes=GFX1210,GFX1210-GISEL %s

declare <3 x i32> @llvm.amdgcn.cvt.scalef32.sr.pk16.bf6.bf16(<16 x bfloat> %src, i32 %sr, float %scale)
declare <3 x i32> @llvm.amdgcn.cvt.scalef32.sr.pk16.bf6.f16(<16 x half> %src, i32 %sr, float %scale)
declare <3 x i32> @llvm.amdgcn.cvt.scalef32.sr.pk16.bf6.f32(<16 x float> %src, i32 %sr, float %scale)
declare <3 x i32> @llvm.amdgcn.cvt.scalef32.sr.pk16.fp6.bf16(<16 x bfloat> %src, i32 %sr, float %scale)
declare <3 x i32> @llvm.amdgcn.cvt.scalef32.sr.pk16.fp6.f16(<16 x half> %src, i32 %sr, float %scale)
declare <3 x i32> @llvm.amdgcn.cvt.scalef32.sr.pk16.fp6.f32(<16 x float> %src, i32 %sr, float %scale)

define amdgpu_ps void @test_scalef32_sr_pk16_bf6_bf16_vv(<16 x bfloat> %src, i32 %sr, float %scale, ptr addrspace(1) %out) {
; GFX1210-SDAG-LABEL: test_scalef32_sr_pk16_bf6_bf16_vv:
; GFX1210-SDAG:       ; %bb.0:
; GFX1210-SDAG-NEXT:    v_cvt_scalef32_sr_pk16_bf6_bf16 v[12:14], v[0:7], v8, v9
; GFX1210-SDAG-NEXT:    global_store_b96 v[10:11], v[12:14], off
; GFX1210-SDAG-NEXT:    s_endpgm
;
; GFX1210-GISEL-LABEL: test_scalef32_sr_pk16_bf6_bf16_vv:
; GFX1210-GISEL:       ; %bb.0:
; GFX1210-GISEL-NEXT:    v_dual_lshrrev_b32 v12, 16, v0 :: v_dual_lshrrev_b32 v13, 16, v1
; GFX1210-GISEL-NEXT:    v_dual_lshrrev_b32 v14, 16, v2 :: v_dual_lshrrev_b32 v15, 16, v3
; GFX1210-GISEL-NEXT:    v_dual_lshrrev_b32 v16, 16, v4 :: v_dual_lshrrev_b32 v17, 16, v5
; GFX1210-GISEL-NEXT:    s_delay_alu instid0(VALU_DEP_3) | instskip(SKIP_4) | instid1(VALU_DEP_4)
; GFX1210-GISEL-NEXT:    v_dual_lshlrev_b32 v12, 16, v12 :: v_dual_lshlrev_b32 v13, 16, v13
; GFX1210-GISEL-NEXT:    v_and_b32_e32 v0, 0xffff, v0
; GFX1210-GISEL-NEXT:    v_and_b32_e32 v1, 0xffff, v1
; GFX1210-GISEL-NEXT:    v_dual_lshlrev_b32 v14, 16, v14 :: v_dual_lshrrev_b32 v19, 16, v7
; GFX1210-GISEL-NEXT:    v_and_b32_e32 v2, 0xffff, v2
; GFX1210-GISEL-NEXT:    v_dual_lshrrev_b32 v18, 16, v6 :: v_dual_bitop2_b32 v0, v12, v0 bitop3:0x54
; GFX1210-GISEL-NEXT:    s_delay_alu instid0(VALU_DEP_4) | instskip(NEXT) | instid1(VALU_DEP_3)
; GFX1210-GISEL-NEXT:    v_dual_lshlrev_b32 v12, 16, v15 :: v_dual_bitop2_b32 v1, v13, v1 bitop3:0x54
; GFX1210-GISEL-NEXT:    v_dual_lshlrev_b32 v13, 16, v16 :: v_dual_bitop2_b32 v2, v14, v2 bitop3:0x54
; GFX1210-GISEL-NEXT:    v_and_b32_e32 v3, 0xffff, v3
; GFX1210-GISEL-NEXT:    v_and_b32_e32 v4, 0xffff, v4
; GFX1210-GISEL-NEXT:    v_dual_lshlrev_b32 v14, 16, v17 :: v_dual_lshlrev_b32 v15, 16, v18
; GFX1210-GISEL-NEXT:    v_and_b32_e32 v5, 0xffff, v5
; GFX1210-GISEL-NEXT:    v_and_b32_e32 v6, 0xffff, v6
; GFX1210-GISEL-NEXT:    s_delay_alu instid0(VALU_DEP_4) | instskip(SKIP_4) | instid1(VALU_DEP_4)
; GFX1210-GISEL-NEXT:    v_dual_lshlrev_b32 v16, 16, v19 :: v_dual_bitop2_b32 v4, v13, v4 bitop3:0x54
; GFX1210-GISEL-NEXT:    v_and_b32_e32 v7, 0xffff, v7
; GFX1210-GISEL-NEXT:    v_or_b32_e32 v3, v12, v3
; GFX1210-GISEL-NEXT:    v_or_b32_e32 v5, v14, v5
; GFX1210-GISEL-NEXT:    v_or_b32_e32 v6, v15, v6
; GFX1210-GISEL-NEXT:    v_or_b32_e32 v7, v16, v7
; GFX1210-GISEL-NEXT:    s_delay_alu instid0(VALU_DEP_1)
; GFX1210-GISEL-NEXT:    v_cvt_scalef32_sr_pk16_bf6_bf16 v[12:14], v[0:7], v8, v9
; GFX1210-GISEL-NEXT:    global_store_b96 v[10:11], v[12:14], off
; GFX1210-GISEL-NEXT:    s_endpgm
  %cvt = tail call <3 x i32> @llvm.amdgcn.cvt.scalef32.sr.pk16.bf6.bf16(<16 x bfloat> %src, i32 %sr, float %scale)
  store <3 x i32> %cvt, ptr addrspace(1) %out, align 8
  ret void
}

define amdgpu_ps void @test_scalef32_sr_pk16_bf6_bf16_sl(<16 x bfloat> inreg %src, i32 inreg %sr, ptr addrspace(1) %out) {
; GFX1210-SDAG-LABEL: test_scalef32_sr_pk16_bf6_bf16_sl:
; GFX1210-SDAG:       ; %bb.0:
; GFX1210-SDAG-NEXT:    v_dual_mov_b32 v2, s0 :: v_dual_mov_b32 v3, s1
; GFX1210-SDAG-NEXT:    v_dual_mov_b32 v4, s2 :: v_dual_mov_b32 v5, s3
; GFX1210-SDAG-NEXT:    v_dual_mov_b32 v6, s4 :: v_dual_mov_b32 v7, s5
; GFX1210-SDAG-NEXT:    v_dual_mov_b32 v8, s6 :: v_dual_mov_b32 v9, s7
; GFX1210-SDAG-NEXT:    s_delay_alu instid0(VALU_DEP_1)
; GFX1210-SDAG-NEXT:    v_cvt_scalef32_sr_pk16_bf6_bf16 v[10:12], v[2:9], s8, 0x42c80000
; GFX1210-SDAG-NEXT:    global_store_b96 v[0:1], v[10:12], off
; GFX1210-SDAG-NEXT:    s_endpgm
;
; GFX1210-GISEL-LABEL: test_scalef32_sr_pk16_bf6_bf16_sl:
; GFX1210-GISEL:       ; %bb.0:
; GFX1210-GISEL-NEXT:    s_lshr_b32 s9, s0, 16
; GFX1210-GISEL-NEXT:    s_lshr_b32 s10, s1, 16
; GFX1210-GISEL-NEXT:    s_lshr_b32 s11, s2, 16
; GFX1210-GISEL-NEXT:    s_lshr_b32 s12, s3, 16
; GFX1210-GISEL-NEXT:    s_lshl_b32 s9, s9, 16
; GFX1210-GISEL-NEXT:    s_and_b32 s0, s0, 0xffff
; GFX1210-GISEL-NEXT:    s_lshl_b32 s10, s10, 16
; GFX1210-GISEL-NEXT:    s_and_b32 s1, s1, 0xffff
; GFX1210-GISEL-NEXT:    s_lshr_b32 s13, s4, 16
; GFX1210-GISEL-NEXT:    s_lshr_b32 s14, s5, 16
; GFX1210-GISEL-NEXT:    s_or_b32 s0, s9, s0
; GFX1210-GISEL-NEXT:    s_or_b32 s1, s10, s1
; GFX1210-GISEL-NEXT:    s_lshl_b32 s9, s11, 16
; GFX1210-GISEL-NEXT:    s_and_b32 s2, s2, 0xffff
; GFX1210-GISEL-NEXT:    s_lshl_b32 s10, s12, 16
; GFX1210-GISEL-NEXT:    s_and_b32 s3, s3, 0xffff
; GFX1210-GISEL-NEXT:    s_lshr_b32 s15, s6, 16
; GFX1210-GISEL-NEXT:    s_lshr_b32 s16, s7, 16
; GFX1210-GISEL-NEXT:    s_or_b32 s2, s9, s2
; GFX1210-GISEL-NEXT:    s_or_b32 s3, s10, s3
; GFX1210-GISEL-NEXT:    s_lshl_b32 s9, s13, 16
; GFX1210-GISEL-NEXT:    s_and_b32 s4, s4, 0xffff
; GFX1210-GISEL-NEXT:    s_lshl_b32 s10, s14, 16
; GFX1210-GISEL-NEXT:    s_and_b32 s5, s5, 0xffff
; GFX1210-GISEL-NEXT:    s_or_b32 s4, s9, s4
; GFX1210-GISEL-NEXT:    s_or_b32 s5, s10, s5
; GFX1210-GISEL-NEXT:    s_lshl_b32 s9, s15, 16
; GFX1210-GISEL-NEXT:    s_and_b32 s6, s6, 0xffff
; GFX1210-GISEL-NEXT:    s_lshl_b32 s10, s16, 16
; GFX1210-GISEL-NEXT:    s_and_b32 s7, s7, 0xffff
; GFX1210-GISEL-NEXT:    s_or_b32 s6, s9, s6
; GFX1210-GISEL-NEXT:    s_or_b32 s7, s10, s7
; GFX1210-GISEL-NEXT:    s_delay_alu instid0(SALU_CYCLE_1) | instskip(SKIP_3) | instid1(VALU_DEP_1)
; GFX1210-GISEL-NEXT:    v_mov_b64_e32 v[8:9], s[6:7]
; GFX1210-GISEL-NEXT:    v_mov_b64_e32 v[6:7], s[4:5]
; GFX1210-GISEL-NEXT:    v_mov_b64_e32 v[4:5], s[2:3]
; GFX1210-GISEL-NEXT:    v_mov_b64_e32 v[2:3], s[0:1]
; GFX1210-GISEL-NEXT:    v_cvt_scalef32_sr_pk16_bf6_bf16 v[10:12], v[2:9], s8, 0x42c80000
; GFX1210-GISEL-NEXT:    global_store_b96 v[0:1], v[10:12], off
; GFX1210-GISEL-NEXT:    s_endpgm
  %cvt = tail call <3 x i32> @llvm.amdgcn.cvt.scalef32.sr.pk16.bf6.bf16(<16 x bfloat> %src, i32 %sr, float 100.0)
  store <3 x i32> %cvt, ptr addrspace(1) %out, align 8
  ret void
}

define amdgpu_ps void @test_scalef32_sr_pk16_bf6_f16_vv(<16 x half> %src, i32 %sr, float %scale, ptr addrspace(1) %out) {
; GFX1210-LABEL: test_scalef32_sr_pk16_bf6_f16_vv:
; GFX1210:       ; %bb.0:
; GFX1210-NEXT:    v_cvt_scalef32_sr_pk16_bf6_f16 v[12:14], v[0:7], v8, v9
; GFX1210-NEXT:    global_store_b96 v[10:11], v[12:14], off
; GFX1210-NEXT:    s_endpgm
  %cvt = tail call <3 x i32> @llvm.amdgcn.cvt.scalef32.sr.pk16.bf6.f16(<16 x half> %src, i32 %sr, float %scale)
  store <3 x i32> %cvt, ptr addrspace(1) %out, align 8
  ret void
}

define amdgpu_ps void @test_scalef32_sr_pk16_bf6_f16_sl(<16 x half> inreg %src, i32 inreg %sr, ptr addrspace(1) %out) {
; GFX1210-SDAG-LABEL: test_scalef32_sr_pk16_bf6_f16_sl:
; GFX1210-SDAG:       ; %bb.0:
; GFX1210-SDAG-NEXT:    v_dual_mov_b32 v2, s0 :: v_dual_mov_b32 v3, s1
; GFX1210-SDAG-NEXT:    v_dual_mov_b32 v4, s2 :: v_dual_mov_b32 v5, s3
; GFX1210-SDAG-NEXT:    v_dual_mov_b32 v6, s4 :: v_dual_mov_b32 v7, s5
; GFX1210-SDAG-NEXT:    v_dual_mov_b32 v8, s6 :: v_dual_mov_b32 v9, s7
; GFX1210-SDAG-NEXT:    s_delay_alu instid0(VALU_DEP_1)
; GFX1210-SDAG-NEXT:    v_cvt_scalef32_sr_pk16_bf6_f16 v[10:12], v[2:9], s8, 0x42c80000
; GFX1210-SDAG-NEXT:    global_store_b96 v[0:1], v[10:12], off
; GFX1210-SDAG-NEXT:    s_endpgm
;
; GFX1210-GISEL-LABEL: test_scalef32_sr_pk16_bf6_f16_sl:
; GFX1210-GISEL:       ; %bb.0:
; GFX1210-GISEL-NEXT:    v_mov_b64_e32 v[8:9], s[6:7]
; GFX1210-GISEL-NEXT:    v_mov_b64_e32 v[6:7], s[4:5]
; GFX1210-GISEL-NEXT:    v_mov_b64_e32 v[4:5], s[2:3]
; GFX1210-GISEL-NEXT:    v_mov_b64_e32 v[2:3], s[0:1]
; GFX1210-GISEL-NEXT:    s_delay_alu instid0(VALU_DEP_1)
; GFX1210-GISEL-NEXT:    v_cvt_scalef32_sr_pk16_bf6_f16 v[10:12], v[2:9], s8, 0x42c80000
; GFX1210-GISEL-NEXT:    global_store_b96 v[0:1], v[10:12], off
; GFX1210-GISEL-NEXT:    s_endpgm
  %cvt = tail call <3 x i32> @llvm.amdgcn.cvt.scalef32.sr.pk16.bf6.f16(<16 x half> %src, i32 %sr, float 100.0)
  store <3 x i32> %cvt, ptr addrspace(1) %out, align 8
  ret void
}

define amdgpu_ps void @test_scalef32_sr_pk16_fp6_bf16_vv(<16 x bfloat> %src, i32 %sr, float %scale, ptr addrspace(1) %out) {
; GFX1210-SDAG-LABEL: test_scalef32_sr_pk16_fp6_bf16_vv:
; GFX1210-SDAG:       ; %bb.0:
; GFX1210-SDAG-NEXT:    v_cvt_scalef32_sr_pk16_fp6_bf16 v[12:14], v[0:7], v8, v9
; GFX1210-SDAG-NEXT:    global_store_b96 v[10:11], v[12:14], off
; GFX1210-SDAG-NEXT:    s_endpgm
;
; GFX1210-GISEL-LABEL: test_scalef32_sr_pk16_fp6_bf16_vv:
; GFX1210-GISEL:       ; %bb.0:
; GFX1210-GISEL-NEXT:    v_dual_lshrrev_b32 v12, 16, v0 :: v_dual_lshrrev_b32 v13, 16, v1
; GFX1210-GISEL-NEXT:    v_dual_lshrrev_b32 v14, 16, v2 :: v_dual_lshrrev_b32 v15, 16, v3
; GFX1210-GISEL-NEXT:    v_dual_lshrrev_b32 v16, 16, v4 :: v_dual_lshrrev_b32 v17, 16, v5
; GFX1210-GISEL-NEXT:    s_delay_alu instid0(VALU_DEP_3) | instskip(SKIP_4) | instid1(VALU_DEP_4)
; GFX1210-GISEL-NEXT:    v_dual_lshlrev_b32 v12, 16, v12 :: v_dual_lshlrev_b32 v13, 16, v13
; GFX1210-GISEL-NEXT:    v_and_b32_e32 v0, 0xffff, v0
; GFX1210-GISEL-NEXT:    v_and_b32_e32 v1, 0xffff, v1
; GFX1210-GISEL-NEXT:    v_dual_lshlrev_b32 v14, 16, v14 :: v_dual_lshrrev_b32 v19, 16, v7
; GFX1210-GISEL-NEXT:    v_and_b32_e32 v2, 0xffff, v2
; GFX1210-GISEL-NEXT:    v_dual_lshrrev_b32 v18, 16, v6 :: v_dual_bitop2_b32 v0, v12, v0 bitop3:0x54
; GFX1210-GISEL-NEXT:    s_delay_alu instid0(VALU_DEP_4) | instskip(NEXT) | instid1(VALU_DEP_3)
; GFX1210-GISEL-NEXT:    v_dual_lshlrev_b32 v12, 16, v15 :: v_dual_bitop2_b32 v1, v13, v1 bitop3:0x54
; GFX1210-GISEL-NEXT:    v_dual_lshlrev_b32 v13, 16, v16 :: v_dual_bitop2_b32 v2, v14, v2 bitop3:0x54
; GFX1210-GISEL-NEXT:    v_and_b32_e32 v3, 0xffff, v3
; GFX1210-GISEL-NEXT:    v_and_b32_e32 v4, 0xffff, v4
; GFX1210-GISEL-NEXT:    v_dual_lshlrev_b32 v14, 16, v17 :: v_dual_lshlrev_b32 v15, 16, v18
; GFX1210-GISEL-NEXT:    v_and_b32_e32 v5, 0xffff, v5
; GFX1210-GISEL-NEXT:    v_and_b32_e32 v6, 0xffff, v6
; GFX1210-GISEL-NEXT:    s_delay_alu instid0(VALU_DEP_4) | instskip(SKIP_4) | instid1(VALU_DEP_4)
; GFX1210-GISEL-NEXT:    v_dual_lshlrev_b32 v16, 16, v19 :: v_dual_bitop2_b32 v4, v13, v4 bitop3:0x54
; GFX1210-GISEL-NEXT:    v_and_b32_e32 v7, 0xffff, v7
; GFX1210-GISEL-NEXT:    v_or_b32_e32 v3, v12, v3
; GFX1210-GISEL-NEXT:    v_or_b32_e32 v5, v14, v5
; GFX1210-GISEL-NEXT:    v_or_b32_e32 v6, v15, v6
; GFX1210-GISEL-NEXT:    v_or_b32_e32 v7, v16, v7
; GFX1210-GISEL-NEXT:    s_delay_alu instid0(VALU_DEP_1)
; GFX1210-GISEL-NEXT:    v_cvt_scalef32_sr_pk16_fp6_bf16 v[12:14], v[0:7], v8, v9
; GFX1210-GISEL-NEXT:    global_store_b96 v[10:11], v[12:14], off
; GFX1210-GISEL-NEXT:    s_endpgm
  %cvt = tail call <3 x i32> @llvm.amdgcn.cvt.scalef32.sr.pk16.fp6.bf16(<16 x bfloat> %src, i32 %sr, float %scale)
  store <3 x i32> %cvt, ptr addrspace(1) %out, align 8
  ret void
}

define amdgpu_ps void @test_scalef32_sr_pk16_fp6_bf16_sl(<16 x bfloat> inreg %src, i32 inreg %sr, ptr addrspace(1) %out) {
; GFX1210-SDAG-LABEL: test_scalef32_sr_pk16_fp6_bf16_sl:
; GFX1210-SDAG:       ; %bb.0:
; GFX1210-SDAG-NEXT:    v_dual_mov_b32 v2, s0 :: v_dual_mov_b32 v3, s1
; GFX1210-SDAG-NEXT:    v_dual_mov_b32 v4, s2 :: v_dual_mov_b32 v5, s3
; GFX1210-SDAG-NEXT:    v_dual_mov_b32 v6, s4 :: v_dual_mov_b32 v7, s5
; GFX1210-SDAG-NEXT:    v_dual_mov_b32 v8, s6 :: v_dual_mov_b32 v9, s7
; GFX1210-SDAG-NEXT:    s_delay_alu instid0(VALU_DEP_1)
; GFX1210-SDAG-NEXT:    v_cvt_scalef32_sr_pk16_fp6_bf16 v[10:12], v[2:9], s8, 0x42c80000
; GFX1210-SDAG-NEXT:    global_store_b96 v[0:1], v[10:12], off
; GFX1210-SDAG-NEXT:    s_endpgm
;
; GFX1210-GISEL-LABEL: test_scalef32_sr_pk16_fp6_bf16_sl:
; GFX1210-GISEL:       ; %bb.0:
; GFX1210-GISEL-NEXT:    s_lshr_b32 s9, s0, 16
; GFX1210-GISEL-NEXT:    s_lshr_b32 s10, s1, 16
; GFX1210-GISEL-NEXT:    s_lshr_b32 s11, s2, 16
; GFX1210-GISEL-NEXT:    s_lshr_b32 s12, s3, 16
; GFX1210-GISEL-NEXT:    s_lshl_b32 s9, s9, 16
; GFX1210-GISEL-NEXT:    s_and_b32 s0, s0, 0xffff
; GFX1210-GISEL-NEXT:    s_lshl_b32 s10, s10, 16
; GFX1210-GISEL-NEXT:    s_and_b32 s1, s1, 0xffff
; GFX1210-GISEL-NEXT:    s_lshr_b32 s13, s4, 16
; GFX1210-GISEL-NEXT:    s_lshr_b32 s14, s5, 16
; GFX1210-GISEL-NEXT:    s_or_b32 s0, s9, s0
; GFX1210-GISEL-NEXT:    s_or_b32 s1, s10, s1
; GFX1210-GISEL-NEXT:    s_lshl_b32 s9, s11, 16
; GFX1210-GISEL-NEXT:    s_and_b32 s2, s2, 0xffff
; GFX1210-GISEL-NEXT:    s_lshl_b32 s10, s12, 16
; GFX1210-GISEL-NEXT:    s_and_b32 s3, s3, 0xffff
; GFX1210-GISEL-NEXT:    s_lshr_b32 s15, s6, 16
; GFX1210-GISEL-NEXT:    s_lshr_b32 s16, s7, 16
; GFX1210-GISEL-NEXT:    s_or_b32 s2, s9, s2
; GFX1210-GISEL-NEXT:    s_or_b32 s3, s10, s3
; GFX1210-GISEL-NEXT:    s_lshl_b32 s9, s13, 16
; GFX1210-GISEL-NEXT:    s_and_b32 s4, s4, 0xffff
; GFX1210-GISEL-NEXT:    s_lshl_b32 s10, s14, 16
; GFX1210-GISEL-NEXT:    s_and_b32 s5, s5, 0xffff
; GFX1210-GISEL-NEXT:    s_or_b32 s4, s9, s4
; GFX1210-GISEL-NEXT:    s_or_b32 s5, s10, s5
; GFX1210-GISEL-NEXT:    s_lshl_b32 s9, s15, 16
; GFX1210-GISEL-NEXT:    s_and_b32 s6, s6, 0xffff
; GFX1210-GISEL-NEXT:    s_lshl_b32 s10, s16, 16
; GFX1210-GISEL-NEXT:    s_and_b32 s7, s7, 0xffff
; GFX1210-GISEL-NEXT:    s_or_b32 s6, s9, s6
; GFX1210-GISEL-NEXT:    s_or_b32 s7, s10, s7
; GFX1210-GISEL-NEXT:    s_delay_alu instid0(SALU_CYCLE_1) | instskip(SKIP_3) | instid1(VALU_DEP_1)
; GFX1210-GISEL-NEXT:    v_mov_b64_e32 v[8:9], s[6:7]
; GFX1210-GISEL-NEXT:    v_mov_b64_e32 v[6:7], s[4:5]
; GFX1210-GISEL-NEXT:    v_mov_b64_e32 v[4:5], s[2:3]
; GFX1210-GISEL-NEXT:    v_mov_b64_e32 v[2:3], s[0:1]
; GFX1210-GISEL-NEXT:    v_cvt_scalef32_sr_pk16_fp6_bf16 v[10:12], v[2:9], s8, 0x42c80000
; GFX1210-GISEL-NEXT:    global_store_b96 v[0:1], v[10:12], off
; GFX1210-GISEL-NEXT:    s_endpgm
  %cvt = tail call <3 x i32> @llvm.amdgcn.cvt.scalef32.sr.pk16.fp6.bf16(<16 x bfloat> %src, i32 %sr, float 100.0)
  store <3 x i32> %cvt, ptr addrspace(1) %out, align 8
  ret void
}

define amdgpu_ps void @test_scalef32_sr_pk16_fp6_f16_vv(<16 x half> %src, i32 %sr, float %scale, ptr addrspace(1) %out) {
; GFX1210-LABEL: test_scalef32_sr_pk16_fp6_f16_vv:
; GFX1210:       ; %bb.0:
; GFX1210-NEXT:    v_cvt_scalef32_sr_pk16_fp6_f16 v[12:14], v[0:7], v8, v9
; GFX1210-NEXT:    global_store_b96 v[10:11], v[12:14], off
; GFX1210-NEXT:    s_endpgm
  %cvt = tail call <3 x i32> @llvm.amdgcn.cvt.scalef32.sr.pk16.fp6.f16(<16 x half> %src, i32 %sr, float %scale)
  store <3 x i32> %cvt, ptr addrspace(1) %out, align 8
  ret void
}

define amdgpu_ps void @test_scalef32_sr_pk16_fp6_f16_sl(<16 x half> inreg %src, i32 inreg %sr, ptr addrspace(1) %out) {
; GFX1210-SDAG-LABEL: test_scalef32_sr_pk16_fp6_f16_sl:
; GFX1210-SDAG:       ; %bb.0:
; GFX1210-SDAG-NEXT:    v_dual_mov_b32 v2, s0 :: v_dual_mov_b32 v3, s1
; GFX1210-SDAG-NEXT:    v_dual_mov_b32 v4, s2 :: v_dual_mov_b32 v5, s3
; GFX1210-SDAG-NEXT:    v_dual_mov_b32 v6, s4 :: v_dual_mov_b32 v7, s5
; GFX1210-SDAG-NEXT:    v_dual_mov_b32 v8, s6 :: v_dual_mov_b32 v9, s7
; GFX1210-SDAG-NEXT:    s_delay_alu instid0(VALU_DEP_1)
; GFX1210-SDAG-NEXT:    v_cvt_scalef32_sr_pk16_fp6_f16 v[10:12], v[2:9], s8, 0x42c80000
; GFX1210-SDAG-NEXT:    global_store_b96 v[0:1], v[10:12], off
; GFX1210-SDAG-NEXT:    s_endpgm
;
; GFX1210-GISEL-LABEL: test_scalef32_sr_pk16_fp6_f16_sl:
; GFX1210-GISEL:       ; %bb.0:
; GFX1210-GISEL-NEXT:    v_mov_b64_e32 v[8:9], s[6:7]
; GFX1210-GISEL-NEXT:    v_mov_b64_e32 v[6:7], s[4:5]
; GFX1210-GISEL-NEXT:    v_mov_b64_e32 v[4:5], s[2:3]
; GFX1210-GISEL-NEXT:    v_mov_b64_e32 v[2:3], s[0:1]
; GFX1210-GISEL-NEXT:    s_delay_alu instid0(VALU_DEP_1)
; GFX1210-GISEL-NEXT:    v_cvt_scalef32_sr_pk16_fp6_f16 v[10:12], v[2:9], s8, 0x42c80000
; GFX1210-GISEL-NEXT:    global_store_b96 v[0:1], v[10:12], off
; GFX1210-GISEL-NEXT:    s_endpgm
  %cvt = tail call <3 x i32> @llvm.amdgcn.cvt.scalef32.sr.pk16.fp6.f16(<16 x half> %src, i32 %sr, float 100.0)
  store <3 x i32> %cvt, ptr addrspace(1) %out, align 8
  ret void
}

define amdgpu_ps void @test_scalef32_sr_pk16_bf6_f32_vv(<16 x float> %src, i32 %sr, float %scale, ptr addrspace(1) %out) {
; GFX1210-LABEL: test_scalef32_sr_pk16_bf6_f32_vv:
; GFX1210:       ; %bb.0:
; GFX1210-NEXT:    v_cvt_scalef32_sr_pk16_bf6_f32 v[20:22], v[0:15], v16, v17
; GFX1210-NEXT:    global_store_b96 v[18:19], v[20:22], off
; GFX1210-NEXT:    s_endpgm
  %cvt = tail call <3 x i32> @llvm.amdgcn.cvt.scalef32.sr.pk16.bf6.f32(<16 x float> %src, i32 %sr, float %scale)
  store <3 x i32> %cvt, ptr addrspace(1) %out, align 8
  ret void
}

define amdgpu_ps void @test_scalef32_sr_pk16_bf6_f32_sl(<16 x float> inreg %src, i32 inreg %sr, ptr addrspace(1) %out) {
; GFX1210-SDAG-LABEL: test_scalef32_sr_pk16_bf6_f32_sl:
; GFX1210-SDAG:       ; %bb.0:
; GFX1210-SDAG-NEXT:    v_dual_mov_b32 v2, s0 :: v_dual_mov_b32 v3, s1
; GFX1210-SDAG-NEXT:    v_dual_mov_b32 v4, s2 :: v_dual_mov_b32 v5, s3
; GFX1210-SDAG-NEXT:    v_dual_mov_b32 v6, s4 :: v_dual_mov_b32 v7, s5
; GFX1210-SDAG-NEXT:    v_dual_mov_b32 v8, s6 :: v_dual_mov_b32 v9, s7
; GFX1210-SDAG-NEXT:    v_dual_mov_b32 v10, s8 :: v_dual_mov_b32 v11, s9
; GFX1210-SDAG-NEXT:    v_dual_mov_b32 v12, s10 :: v_dual_mov_b32 v13, s11
; GFX1210-SDAG-NEXT:    v_dual_mov_b32 v14, s12 :: v_dual_mov_b32 v15, s13
; GFX1210-SDAG-NEXT:    v_dual_mov_b32 v16, s14 :: v_dual_mov_b32 v17, s15
; GFX1210-SDAG-NEXT:    s_delay_alu instid0(VALU_DEP_1)
; GFX1210-SDAG-NEXT:    v_cvt_scalef32_sr_pk16_bf6_f32 v[18:20], v[2:17], s16, 0x42c80000
; GFX1210-SDAG-NEXT:    global_store_b96 v[0:1], v[18:20], off
; GFX1210-SDAG-NEXT:    s_endpgm
;
; GFX1210-GISEL-LABEL: test_scalef32_sr_pk16_bf6_f32_sl:
; GFX1210-GISEL:       ; %bb.0:
; GFX1210-GISEL-NEXT:    v_mov_b64_e32 v[16:17], s[14:15]
; GFX1210-GISEL-NEXT:    v_mov_b64_e32 v[14:15], s[12:13]
; GFX1210-GISEL-NEXT:    v_mov_b64_e32 v[12:13], s[10:11]
; GFX1210-GISEL-NEXT:    v_mov_b64_e32 v[10:11], s[8:9]
; GFX1210-GISEL-NEXT:    v_mov_b64_e32 v[8:9], s[6:7]
; GFX1210-GISEL-NEXT:    v_mov_b64_e32 v[6:7], s[4:5]
; GFX1210-GISEL-NEXT:    v_mov_b64_e32 v[4:5], s[2:3]
; GFX1210-GISEL-NEXT:    v_mov_b64_e32 v[2:3], s[0:1]
; GFX1210-GISEL-NEXT:    s_delay_alu instid0(VALU_DEP_1)
; GFX1210-GISEL-NEXT:    v_cvt_scalef32_sr_pk16_bf6_f32 v[18:20], v[2:17], s16, 0x42c80000
; GFX1210-GISEL-NEXT:    global_store_b96 v[0:1], v[18:20], off
; GFX1210-GISEL-NEXT:    s_endpgm
  %cvt = tail call <3 x i32> @llvm.amdgcn.cvt.scalef32.sr.pk16.bf6.f32(<16 x float> %src, i32 %sr, float 100.0)
  store <3 x i32> %cvt, ptr addrspace(1) %out, align 8
  ret void
}

define amdgpu_ps void @test_scalef32_sr_pk16_fp6_f32_vv(<16 x float> %src, i32 %sr, float %scale, ptr addrspace(1) %out) {
; GFX1210-LABEL: test_scalef32_sr_pk16_fp6_f32_vv:
; GFX1210:       ; %bb.0:
; GFX1210-NEXT:    v_cvt_scalef32_sr_pk16_fp6_f32 v[20:22], v[0:15], v16, v17
; GFX1210-NEXT:    global_store_b96 v[18:19], v[20:22], off
; GFX1210-NEXT:    s_endpgm
  %cvt = tail call <3 x i32> @llvm.amdgcn.cvt.scalef32.sr.pk16.fp6.f32(<16 x float> %src, i32 %sr, float %scale)
  store <3 x i32> %cvt, ptr addrspace(1) %out, align 8
  ret void
}

define amdgpu_ps void @test_scalef32_sr_pk16_fp6_f32_sl(<16 x float> inreg %src, i32 inreg %sr, ptr addrspace(1) %out) {
; GFX1210-SDAG-LABEL: test_scalef32_sr_pk16_fp6_f32_sl:
; GFX1210-SDAG:       ; %bb.0:
; GFX1210-SDAG-NEXT:    v_dual_mov_b32 v2, s0 :: v_dual_mov_b32 v3, s1
; GFX1210-SDAG-NEXT:    v_dual_mov_b32 v4, s2 :: v_dual_mov_b32 v5, s3
; GFX1210-SDAG-NEXT:    v_dual_mov_b32 v6, s4 :: v_dual_mov_b32 v7, s5
; GFX1210-SDAG-NEXT:    v_dual_mov_b32 v8, s6 :: v_dual_mov_b32 v9, s7
; GFX1210-SDAG-NEXT:    v_dual_mov_b32 v10, s8 :: v_dual_mov_b32 v11, s9
; GFX1210-SDAG-NEXT:    v_dual_mov_b32 v12, s10 :: v_dual_mov_b32 v13, s11
; GFX1210-SDAG-NEXT:    v_dual_mov_b32 v14, s12 :: v_dual_mov_b32 v15, s13
; GFX1210-SDAG-NEXT:    v_dual_mov_b32 v16, s14 :: v_dual_mov_b32 v17, s15
; GFX1210-SDAG-NEXT:    s_delay_alu instid0(VALU_DEP_1)
; GFX1210-SDAG-NEXT:    v_cvt_scalef32_sr_pk16_fp6_f32 v[18:20], v[2:17], s16, 0x42c80000
; GFX1210-SDAG-NEXT:    global_store_b96 v[0:1], v[18:20], off
; GFX1210-SDAG-NEXT:    s_endpgm
;
; GFX1210-GISEL-LABEL: test_scalef32_sr_pk16_fp6_f32_sl:
; GFX1210-GISEL:       ; %bb.0:
; GFX1210-GISEL-NEXT:    v_mov_b64_e32 v[16:17], s[14:15]
; GFX1210-GISEL-NEXT:    v_mov_b64_e32 v[14:15], s[12:13]
; GFX1210-GISEL-NEXT:    v_mov_b64_e32 v[12:13], s[10:11]
; GFX1210-GISEL-NEXT:    v_mov_b64_e32 v[10:11], s[8:9]
; GFX1210-GISEL-NEXT:    v_mov_b64_e32 v[8:9], s[6:7]
; GFX1210-GISEL-NEXT:    v_mov_b64_e32 v[6:7], s[4:5]
; GFX1210-GISEL-NEXT:    v_mov_b64_e32 v[4:5], s[2:3]
; GFX1210-GISEL-NEXT:    v_mov_b64_e32 v[2:3], s[0:1]
; GFX1210-GISEL-NEXT:    s_delay_alu instid0(VALU_DEP_1)
; GFX1210-GISEL-NEXT:    v_cvt_scalef32_sr_pk16_fp6_f32 v[18:20], v[2:17], s16, 0x42c80000
; GFX1210-GISEL-NEXT:    global_store_b96 v[0:1], v[18:20], off
; GFX1210-GISEL-NEXT:    s_endpgm
  %cvt = tail call <3 x i32> @llvm.amdgcn.cvt.scalef32.sr.pk16.fp6.f32(<16 x float> %src, i32 %sr, float 100.0)
  store <3 x i32> %cvt, ptr addrspace(1) %out, align 8
  ret void
}
