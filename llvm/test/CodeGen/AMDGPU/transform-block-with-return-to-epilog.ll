...
; RUN: llc -mtriple=amdgpu9.00-mesa-mesa3d -stop-after=si-pre-emit-peephole -o - %s | FileCheck -check-prefix=GCN %s
...
define amdgpu_ps { <4 x float> } @test_return_to_epilog_with_optimized_kill(float %val) #0 {
  ...
  ; GCN-NEXT:   $sgpr0_64 = S_MOV_B64 $exec
  ; GCN-NEXT:   nofpexcept V_CMP_LE_F32_e32 0, killed $vgpr1, implicit-def $vcc, implicit $mode, implicit $exec
  ; GCN-NEXT:   $sgpr2_64 = S_AND_SAVEEXEC_B64 killed $vcc, implicit-def $exec, implicit-def $scc, implicit $exec
  ; GCN-NEXT:   renamable $sgpr2_64 = S_XOR_B64 $exec, killed renamable $sgpr2_64, implicit-def dead $scc
  ; GCN-NEXT:   S_CBRANCH_EXECNZ %bb.3, implicit $exec
  ; GCN-NEXT: {{  $}}
  ; GCN-NEXT: bb.1.Flow1:
  ; GCN-NEXT:   successors: %bb.6(0x40000000), %bb.2(0x40000000)
  ; GCN-NEXT:   liveins: $sgpr0_64, $sgpr2_64
  ; GCN-NEXT: {{  $}}
  ; GCN-NEXT:   $sgpr2_64 = S_ANDN2_SAVEEXEC_B64 killed $sgpr2_64, implicit-def $exec, implicit-def $scc, implicit $exec
  ; GCN-NEXT:   S_CBRANCH_EXECNZ %bb.6, implicit $exec
  ; GCN-NEXT: {{  $}}
  ; GCN-NEXT: bb.2.end:
  ; GCN-NEXT:   successors: %bb.9(0x80000000)
  ; GCN-NEXT:   liveins: $sgpr2_64
  ; GCN-NEXT: {{  $}}
  ; GCN-NEXT:   $exec = S_OR_B64 $exec, killed renamable $sgpr2_64, implicit-def $scc
  ; GCN-NEXT:   S_BRANCH %bb.9
  ; GCN-NEXT: {{  $}}
  ; GCN-NEXT: bb.3.flow.preheader:
  ; GCN-NEXT:   successors: %bb.4(0x80000000)
  ; GCN-NEXT:   liveins: $vgpr0, $sgpr0_64, $sgpr2_64
  ; GCN-NEXT: {{  $}}
  ; GCN-NEXT:   nofpexcept V_CMP_NGT_F32_e32 0, killed $vgpr0, implicit-def $vcc, implicit $mode, implicit $exec
  ; GCN-NEXT:   renamable $sgpr4_64 = S_MOV_B64 0
  ; GCN-NEXT: {{  $}}
  ; GCN-NEXT: bb.4.flow:
  ; GCN-NEXT:   successors: %bb.5(0x04000000), %bb.4(0x7c000000)
  ; GCN-NEXT:   liveins: $vcc, $sgpr0_64, $sgpr2_64, $sgpr4_64
  ; GCN-NEXT: {{  $}}
  ; GCN-NEXT:   renamable $sgpr6_64 = S_AND_B64 $exec, renamable $vcc, implicit-def $scc
  ; GCN-NEXT:   renamable $sgpr4_64 = S_OR_B64 killed renamable $sgpr6_64, killed renamable $sgpr4_64, implicit-def $scc
  ; GCN-NEXT:   $exec = S_ANDN2_B64 $exec, renamable $sgpr4_64, implicit-def $scc
  ; GCN-NEXT:   S_CBRANCH_EXECNZ %bb.4, implicit $exec
  ; GCN-NEXT: {{  $}}
  ; GCN-NEXT: bb.5.Flow:
  ; GCN-NEXT:   successors: %bb.6(0x40000000), %bb.2(0x40000000)
  ; GCN-NEXT:   liveins: $sgpr0_64, $sgpr2_64, $sgpr4_64
  ; GCN-NEXT: {{  $}}
  ; GCN-NEXT:   $exec = S_OR_B64 $exec, killed renamable $sgpr4_64, implicit-def $scc
  ; GCN-NEXT:   $sgpr2_64 = S_ANDN2_SAVEEXEC_B64 killed $sgpr2_64, implicit-def $exec, implicit-def $scc, implicit $exec
  ...
  ; GCN-NEXT:   liveins: $sgpr0_64, $sgpr2_64
  ; GCN-NEXT: {{  $}}
  ; GCN-NEXT:   dead renamable $sgpr0_64 = S_ANDN2_B64 killed renamable $sgpr0_64, $exec, implicit-def $scc
  ; GCN-NEXT:   S_CBRANCH_SCC0 %bb.8, implicit $scc
  ; GCN-NEXT: {{  $}}
  ; GCN-NEXT: bb.7.kill0:
  ; GCN-NEXT:   successors: %bb.9(0x80000000)
  ; GCN-NEXT:   liveins: $sgpr2_64, $scc
  ; GCN-NEXT: {{  $}}
