...
; RUN: llc -global-isel=0 -mtriple=amdgpu9.00-amd-amdhsa -simplify-mir -stop-after=finalize-isel -verify-machineinstrs < %s | FileCheck -check-prefixes=SDAG %s
; RUN: llc -global-isel=1 -mtriple=amdgpu9.00-amd-amdhsa -simplify-mir -stop-after=finalize-isel -verify-machineinstrs < %s | FileCheck -check-prefixes=GISEL %s
...
define float @call_changes_mode(float %x, float %y) #0 {
  ...
  ; SDAG-NEXT:   [[COPY2:%[0-9]+]]:sgpr_128 = COPY $sgpr0_3
  ; SDAG-NEXT:   $sgpr0_3 = COPY [[COPY2]]
  ; SDAG-NEXT:   $sgpr30_31 = SI_CALL killed [[SI_PC_ADD_REL_OFFSET]], @maybe_defs_mode, csr_amdgpu, implicit $sgpr0_3, implicit-def $mode
  ...
  ; GISEL-NEXT:   [[COPY2:%[0-9]+]]:sgpr_128 = COPY $sgpr0_3
  ; GISEL-NEXT:   $sgpr0_3 = COPY [[COPY2]]
  ...
  ; GISEL-NEXT:   $sgpr30_31 = noconvergent SI_CALL [[SI_PC_ADD_REL_OFFSET]], @maybe_defs_mode, csr_amdgpu, implicit $sgpr0_3
...
define void @tail_call_changes_mode() #0 {
  ...
  ; GISEL: bb.1 (%ir-block.0):
  ; GISEL-NEXT:   [[COPY:%[0-9]+]]:sgpr_128 = COPY $sgpr0_3
  ; GISEL-NEXT:   $sgpr0_3 = COPY [[COPY]]
  ; GISEL-NEXT:   [[SI_PC_ADD_REL_OFFSET:%[0-9]+]]:sreg_64 = SI_PC_ADD_REL_OFFSET target-flags(amdgpu-rel32-lo) @maybe_defs_mode, target-flags(amdgpu-rel32-hi) @maybe_defs_mode, implicit-def $scc
  ; GISEL-NEXT:   [[COPY1:%[0-9]+]]:ccr_sgpr_64 = COPY [[SI_PC_ADD_REL_OFFSET]]
  ; GISEL-NEXT:   SI_TCRETURN [[COPY1]], @maybe_defs_mode, 0, csr_amdgpu, implicit $sgpr0_3
