...
; RUN: opt -S -passes='require<profile-summary>,function(codegenprepare)' < %s | FileCheck -check-prefix=OPT %s
; RUN: llc -amdgpu-atomic-optimizer-strategy=None < %s | FileCheck -check-prefix=GCN %s
...
; OPT-SAME: ptr addrspace(1) [[OUT:%.*]], ptr addrspace(1) [[IN:%.*]]) {
; OPT-NEXT:  [[ENTRY:.*]]:
; OPT-NEXT:    [[TID:%.*]] = call i32 @llvm.amdgcn.mbcnt.lo(i32 -1, i32 0) #[[ATTR1:[0-9]+]]
...
; OPT-NEXT:    [[FADD2:%.*]] = atomicrmw fadd ptr addrspace(1) [[IN_GEP]], float 2.000000e+00 syncscope("agent") seq_cst, align 4, !atomic.ignore.denormal.mode [[META0:![0-9]+]], !amdgpu.no.fine.grained.memory [[META0]]
...
if:
  ...
  %fadd2 = atomicrmw fadd ptr addrspace(1) %in.gep, float 2.000000e+00 syncscope("agent") seq_cst, align 4, !amdgpu.no.fine.grained.memory !0, !atomic.ignore.denormal.mode !0
