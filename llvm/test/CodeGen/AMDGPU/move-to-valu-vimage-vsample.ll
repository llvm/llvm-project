...
; RUN: llc -mtriple=amdgpu11.00 -stop-after=si-fix-sgpr-copies < %s | FileCheck --check-prefix=GFX11 %s
; RUN: llc -mtriple=amdgpu12.00 -stop-after=si-fix-sgpr-copies < %s | FileCheck --check-prefix=GFX12 %s
...
define amdgpu_ps float @vimage_move_to_valu(<8 x i32> %rsrc) {
  ...
  ; GFX11-NEXT:   [[S_ANDN2_B32_:%[0-9]+]]:sreg_32_xm0_xexec = S_ANDN2_B32 [[PHI]], $exec_lo, implicit-def dead $scc
  ...
  ; GFX12-NEXT:   [[S_ANDN2_B32_:%[0-9]+]]:sreg_32_xm0_xexec = S_ANDN2_B32 [[PHI]], $exec_lo, implicit-def dead $scc
