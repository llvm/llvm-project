; RUN: opt -S -mtriple=amdgpu-unknown-unknown -passes=amdgpu-promote-alloca -disable-promote-alloca-to-vector < %s | FileCheck -check-prefix=IR %s
; RUN: llc -mtriple=amdgpu8.03 -disable-promote-alloca-to-vector < %s | FileCheck -check-prefix=ASM %s
