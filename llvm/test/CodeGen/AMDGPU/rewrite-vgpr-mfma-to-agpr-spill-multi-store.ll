...
; The accumulators are initialized from %init rather than from 0.0 on purpose.
; A constant PHI operand shares one register between all the PHIs that use it,
; in a class chosen by the constant, which changes the spilling enough that the
; pass no longer unspills anything and the test stops testing anything.

; CHECK: Split unspilled interval into {{[0-9]+}} components

define amdgpu_kernel void @multi_store_spill_slot(float %init) #0 {
entry:
  br label %do.body

do.body:
  %c_block_tile.sroa.37.0 = phi float [ %init, %entry ], [ %c_block_tile.sroa.70.0, %do.body ]
  %c_block_tile.sroa.70.0 = phi float [ %init, %entry ], [ %ext0, %do.body ]
  %c_block_tile.sroa.961.0 = phi float [ %init, %entry ], [ %ext1, %do.body ]
  %c_block_tile.sroa.994.0 = phi float [ %init, %entry ], [ %ext2, %do.body ]
  %c_block_tile.sroa.1060.0 = phi float [ %init, %entry ], [ %ext3, %do.body ]
  %c_block_tile.sroa.1093.0 = phi float [ %init, %entry ], [ %c_block_tile.sroa.1060.0, %do.body ]
  %c_block_tile.sroa.1588.0 = phi float [ %init, %entry ], [ %ext4, %do.body ]
  %c_block_tile.sroa.1687.0 = phi float [ %init, %entry ], [ %ext5, %do.body ]
  %c_block_tile.sroa.2578.0 = phi float [ %init, %entry ], [ %ext6, %do.body ]
  %c_block_tile.sroa.2611.0 = phi float [ %init, %entry ], [ %ext7, %do.body ]
  %c_block_tile.sroa.2644.0 = phi float [ %init, %entry ], [ %ext8, %do.body ]
  %c_block_tile.sroa.2677.0 = phi float [ %init, %entry ], [ %ext9, %do.body ]
  %c_block_tile.sroa.3568.0 = phi float [ %init, %entry ], [ %ext10, %do.body ]
  %c_block_tile.sroa.3634.0 = phi float [ %init, %entry ], [ %ext11, %do.body ]
  %c_block_tile.sroa.3898.0 = phi float [ %init, %entry ], [ %ext12, %do.body ]
  %c_block_tile.sroa.3931.0 = phi float [ %init, %entry ], [ %ext13, %do.body ]
  %c_block_tile.sroa.4624.0 = phi float [ %init, %entry ], [ %ext14, %do.body ]
  %c_block_tile.sroa.4657.0 = phi float [ %init, %entry ], [ %ext15, %do.body ]
  %c_block_tile.sroa.5185.0 = phi float [ %init, %entry ], [ %ext16, %do.body ]
  %c_block_tile.sroa.5218.0 = phi float [ %init, %entry ], [ %ext17, %do.body ]
  %c_block_tile.sroa.5746.0 = phi float [ %init, %entry ], [ %ext18, %do.body ]
  %c_block_tile.sroa.5779.0 = phi float [ %init, %entry ], [ %c_block_tile.sroa.5746.0, %do.body ]
  %c_block_tile.sroa.5812.0 = phi float [ %init, %entry ], [ %ext19, %do.body ]
  %c_block_tile.sroa.6373.0 = phi float [ %init, %entry ], [ %c_block_tile.sroa.6406.0, %do.body ]
  %c_block_tile.sroa.6406.0 = phi float [ %init, %entry ], [ %ext20, %do.body ]
  %c_block_tile.sroa.7297.0 = phi float [ %init, %entry ], [ %ext21, %do.body ]
  %c_block_tile.sroa.7363.0 = phi float [ %init, %entry ], [ %ext22, %do.body ]
  %c_block_tile.sroa.7396.0 = phi float [ %init, %entry ], [ %ext23, %do.body ]
  %c_block_tile.sroa.7429.0 = phi float [ %init, %entry ], [ %ext24, %do.body ]
