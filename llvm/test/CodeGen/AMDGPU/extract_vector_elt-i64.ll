; RUN: llc -mtriple=amdgcn-- < %s | FileCheck -check-prefix=GCN,BASE %s
; RUN: llc -mtriple=amdgcn-- -mcpu=tonga -mattr=-flat-for-global < %s | FileCheck -check-prefixes=GCN,FLAT %s
...
; GCN-DAG: s_cselect_b64 s[{{[0-9:]+}}], s[{{[0-9:]+}}], s[{{[0-9:]+}}]
