; RUN: llc -march=amdgcn -mcpu=tahiti -amdgpu-enable-fneg-combines -verify-machineinstrs < %s | FileCheck -check-prefix=GCN -check-prefix=SI %s
; RUN: llc -march=amdgcn -mcpu=fiji -amdgpu-enable-fneg-combines -verify-machineinstrs < %s | FileCheck -check-prefix=GCN -check-prefix=VI %s
