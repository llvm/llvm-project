; NOTE: Assertions have been autogenerated by utils/update_llc_test_checks.py UTC_ARGS: --version 5

; RUN: llc -mtriple=amdgcn -mcpu=tahiti < %s | FileCheck -check-prefix=S_SI %s
; RUN: llc -mtriple=amdgcn -mcpu=tonga < %s | FileCheck -check-prefix=S_VI %s
; RUN: llc -mtriple=amdgcn -mcpu=gfx900 < %s | FileCheck -check-prefix=S_GFX9 %s
; RUN: llc -mtriple=amdgcn -mcpu=gfx1100 < %s | FileCheck -check-prefix=S_GFX11 %s

define inreg <3 x half> @bitcast_v3bf16_to_v3f16_inreg(<3 x bfloat> inreg %a, i32 inreg %b) {
; S_SI-LABEL: bitcast_v3bf16_to_v3f16_inreg:
; S_SI:       ; %bb.0:
; S_SI-NEXT:    s_waitcnt vmcnt(0) expcnt(0) lgkmcnt(0)
; S_SI-NEXT:    s_cmp_lg_u32 s19, 0
; S_SI-NEXT:    v_mul_f32_e64 v3, 1.0, s16
; S_SI-NEXT:    v_mul_f32_e64 v4, 1.0, s17
; S_SI-NEXT:    v_mul_f32_e64 v5, 1.0, s18
; S_SI-NEXT:    s_cbranch_scc0 .LBB0_4
; S_SI-NEXT:  ; %bb.1: ; %cmp.false
; S_SI-NEXT:    v_lshrrev_b32_e32 v0, 16, v3
; S_SI-NEXT:    v_lshrrev_b32_e32 v1, 16, v4
; S_SI-NEXT:    v_lshrrev_b32_e32 v2, 16, v5
; S_SI-NEXT:    v_cvt_f32_f16_e32 v0, v0
; S_SI-NEXT:    v_cvt_f32_f16_e32 v1, v1
; S_SI-NEXT:    v_cvt_f32_f16_e32 v2, v2
; S_SI-NEXT:    s_cbranch_execnz .LBB0_3
; S_SI-NEXT:  .LBB0_2: ; %cmp.true
; S_SI-NEXT:    v_and_b32_e32 v0, 0xffff0000, v5
; S_SI-NEXT:    v_add_f32_e32 v0, 0x40c00000, v0
; S_SI-NEXT:    v_lshrrev_b32_e32 v2, 16, v0
; S_SI-NEXT:    v_and_b32_e32 v0, 0xffff0000, v4
; S_SI-NEXT:    v_add_f32_e32 v0, 0x40c00000, v0
; S_SI-NEXT:    v_lshrrev_b32_e32 v1, 16, v0
; S_SI-NEXT:    v_and_b32_e32 v0, 0xffff0000, v3
; S_SI-NEXT:    v_add_f32_e32 v0, 0x40c00000, v0
; S_SI-NEXT:    v_lshrrev_b32_e32 v0, 16, v0
; S_SI-NEXT:    v_cvt_f32_f16_e32 v0, v0
; S_SI-NEXT:    v_cvt_f32_f16_e32 v1, v1
; S_SI-NEXT:    v_cvt_f32_f16_e32 v2, v2
; S_SI-NEXT:  .LBB0_3: ; %end
; S_SI-NEXT:    s_setpc_b64 s[30:31]
; S_SI-NEXT:  .LBB0_4:
; S_SI-NEXT:    ; implicit-def: $vgpr0
; S_SI-NEXT:    ; implicit-def: $vgpr1
; S_SI-NEXT:    ; implicit-def: $vgpr2
; S_SI-NEXT:    s_branch .LBB0_2
;
; S_VI-LABEL: bitcast_v3bf16_to_v3f16_inreg:
; S_VI:       ; %bb.0:
; S_VI-NEXT:    s_waitcnt vmcnt(0) expcnt(0) lgkmcnt(0)
; S_VI-NEXT:    s_cmp_lg_u32 s18, 0
; S_VI-NEXT:    s_cbranch_scc0 .LBB0_3
; S_VI-NEXT:  ; %bb.1: ; %cmp.false
; S_VI-NEXT:    s_cbranch_execnz .LBB0_4
; S_VI-NEXT:  .LBB0_2: ; %cmp.true
; S_VI-NEXT:    s_lshl_b32 s4, s17, 16
; S_VI-NEXT:    v_mov_b32_e32 v0, 0x40c00000
; S_VI-NEXT:    v_add_f32_e32 v1, s4, v0
; S_VI-NEXT:    v_bfe_u32 v2, v1, 16, 1
; S_VI-NEXT:    v_add_u32_e32 v2, vcc, v2, v1
; S_VI-NEXT:    v_add_u32_e32 v2, vcc, 0x7fff, v2
; S_VI-NEXT:    v_or_b32_e32 v3, 0x400000, v1
; S_VI-NEXT:    v_cmp_u_f32_e32 vcc, v1, v1
; S_VI-NEXT:    s_lshl_b32 s4, s16, 16
; S_VI-NEXT:    v_cndmask_b32_e32 v1, v2, v3, vcc
; S_VI-NEXT:    v_add_f32_e32 v2, s4, v0
; S_VI-NEXT:    v_bfe_u32 v3, v2, 16, 1
; S_VI-NEXT:    v_add_u32_e32 v3, vcc, v3, v2
; S_VI-NEXT:    v_add_u32_e32 v3, vcc, 0x7fff, v3
; S_VI-NEXT:    s_and_b32 s4, s16, 0xffff0000
; S_VI-NEXT:    v_or_b32_e32 v4, 0x400000, v2
; S_VI-NEXT:    v_cmp_u_f32_e32 vcc, v2, v2
; S_VI-NEXT:    v_add_f32_e32 v0, s4, v0
; S_VI-NEXT:    v_cndmask_b32_e32 v2, v3, v4, vcc
; S_VI-NEXT:    v_bfe_u32 v3, v0, 16, 1
; S_VI-NEXT:    v_add_u32_e32 v3, vcc, v3, v0
; S_VI-NEXT:    v_add_u32_e32 v3, vcc, 0x7fff, v3
; S_VI-NEXT:    v_or_b32_e32 v4, 0x400000, v0
; S_VI-NEXT:    v_cmp_u_f32_e32 vcc, v0, v0
; S_VI-NEXT:    v_cndmask_b32_e32 v0, v3, v4, vcc
; S_VI-NEXT:    v_lshrrev_b32_e32 v0, 16, v0
; S_VI-NEXT:    v_alignbit_b32 v0, v0, v2, 16
; S_VI-NEXT:    v_mov_b32_e32 v2, 0x7fc00000
; S_VI-NEXT:    v_or_b32_sdwa v1, v1, v2 dst_sel:DWORD dst_unused:UNUSED_PAD src0_sel:WORD_1 src1_sel:DWORD
; S_VI-NEXT:    s_setpc_b64 s[30:31]
; S_VI-NEXT:  .LBB0_3:
; S_VI-NEXT:    s_branch .LBB0_2
; S_VI-NEXT:  .LBB0_4:
; S_VI-NEXT:    v_mov_b32_e32 v0, s16
; S_VI-NEXT:    v_mov_b32_e32 v1, s17
; S_VI-NEXT:    s_setpc_b64 s[30:31]
;
; S_GFX9-LABEL: bitcast_v3bf16_to_v3f16_inreg:
; S_GFX9:       ; %bb.0:
; S_GFX9-NEXT:    s_waitcnt vmcnt(0) expcnt(0) lgkmcnt(0)
; S_GFX9-NEXT:    s_cmp_lg_u32 s18, 0
; S_GFX9-NEXT:    s_cbranch_scc0 .LBB0_3
; S_GFX9-NEXT:  ; %bb.1: ; %cmp.false
; S_GFX9-NEXT:    s_cbranch_execnz .LBB0_4
; S_GFX9-NEXT:  .LBB0_2: ; %cmp.true
; S_GFX9-NEXT:    s_lshl_b32 s4, s17, 16
; S_GFX9-NEXT:    v_mov_b32_e32 v0, 0x40c00000
; S_GFX9-NEXT:    v_add_f32_e32 v1, s4, v0
; S_GFX9-NEXT:    v_bfe_u32 v2, v1, 16, 1
; S_GFX9-NEXT:    v_add_u32_e32 v2, v2, v1
; S_GFX9-NEXT:    v_add_u32_e32 v2, 0x7fff, v2
; S_GFX9-NEXT:    v_or_b32_e32 v3, 0x400000, v1
; S_GFX9-NEXT:    v_cmp_u_f32_e32 vcc, v1, v1
; S_GFX9-NEXT:    s_pack_lh_b32_b16 s4, 0, s16
; S_GFX9-NEXT:    v_cndmask_b32_e32 v1, v2, v3, vcc
; S_GFX9-NEXT:    v_add_f32_e32 v2, s4, v0
; S_GFX9-NEXT:    v_bfe_u32 v3, v2, 16, 1
; S_GFX9-NEXT:    v_add_u32_e32 v3, v3, v2
; S_GFX9-NEXT:    s_lshl_b32 s4, s16, 16
; S_GFX9-NEXT:    v_add_u32_e32 v3, 0x7fff, v3
; S_GFX9-NEXT:    v_or_b32_e32 v4, 0x400000, v2
; S_GFX9-NEXT:    v_cmp_u_f32_e32 vcc, v2, v2
; S_GFX9-NEXT:    v_add_f32_e32 v0, s4, v0
; S_GFX9-NEXT:    v_cndmask_b32_e32 v2, v3, v4, vcc
; S_GFX9-NEXT:    v_bfe_u32 v3, v0, 16, 1
; S_GFX9-NEXT:    v_add_u32_e32 v3, v3, v0
; S_GFX9-NEXT:    v_add_u32_e32 v3, 0x7fff, v3
; S_GFX9-NEXT:    v_or_b32_e32 v4, 0x400000, v0
; S_GFX9-NEXT:    v_cmp_u_f32_e32 vcc, v0, v0
; S_GFX9-NEXT:    v_cndmask_b32_e32 v0, v3, v4, vcc
; S_GFX9-NEXT:    v_mov_b32_e32 v3, 0xffff
; S_GFX9-NEXT:    v_lshrrev_b32_e32 v2, 16, v2
; S_GFX9-NEXT:    v_and_b32_sdwa v0, v3, v0 dst_sel:DWORD dst_unused:UNUSED_PAD src0_sel:DWORD src1_sel:WORD_1
; S_GFX9-NEXT:    s_movk_i32 s4, 0x7fc0
; S_GFX9-NEXT:    v_and_b32_sdwa v1, v3, v1 dst_sel:DWORD dst_unused:UNUSED_PAD src0_sel:DWORD src1_sel:WORD_1
; S_GFX9-NEXT:    v_lshl_or_b32 v0, v2, 16, v0
; S_GFX9-NEXT:    v_lshl_or_b32 v1, s4, 16, v1
; S_GFX9-NEXT:    s_setpc_b64 s[30:31]
; S_GFX9-NEXT:  .LBB0_3:
; S_GFX9-NEXT:    s_branch .LBB0_2
; S_GFX9-NEXT:  .LBB0_4:
; S_GFX9-NEXT:    v_mov_b32_e32 v0, s16
; S_GFX9-NEXT:    v_mov_b32_e32 v1, s17
; S_GFX9-NEXT:    s_setpc_b64 s[30:31]
;
; S_GFX11-LABEL: bitcast_v3bf16_to_v3f16_inreg:
; S_GFX11:       ; %bb.0:
; S_GFX11-NEXT:    s_waitcnt vmcnt(0) expcnt(0) lgkmcnt(0)
; S_GFX11-NEXT:    s_cmp_lg_u32 s2, 0
; S_GFX11-NEXT:    s_mov_b32 s2, 0
; S_GFX11-NEXT:    s_cbranch_scc0 .LBB0_3
; S_GFX11-NEXT:  ; %bb.1: ; %Flow
; S_GFX11-NEXT:    s_and_not1_b32 vcc_lo, exec_lo, s2
; S_GFX11-NEXT:    s_cbranch_vccnz .LBB0_4
; S_GFX11-NEXT:  .LBB0_2: ; %cmp.true
; S_GFX11-NEXT:    s_lshl_b32 s2, s0, 16
; S_GFX11-NEXT:    s_lshl_b32 s1, s1, 16
; S_GFX11-NEXT:    v_add_f32_e64 v0, 0x40c00000, s2
; S_GFX11-NEXT:    v_add_f32_e64 v1, 0x40c00000, s1
; S_GFX11-NEXT:    s_pack_lh_b32_b16 s0, 0, s0
; S_GFX11-NEXT:    s_delay_alu instid0(SALU_CYCLE_1) | instskip(NEXT) | instid1(VALU_DEP_3)
; S_GFX11-NEXT:    v_add_f32_e64 v2, 0x40c00000, s0
; S_GFX11-NEXT:    v_bfe_u32 v3, v0, 16, 1
; S_GFX11-NEXT:    s_delay_alu instid0(VALU_DEP_3)
; S_GFX11-NEXT:    v_bfe_u32 v4, v1, 16, 1
; S_GFX11-NEXT:    v_or_b32_e32 v7, 0x400000, v0
; S_GFX11-NEXT:    v_cmp_u_f32_e32 vcc_lo, v0, v0
; S_GFX11-NEXT:    v_bfe_u32 v5, v2, 16, 1
; S_GFX11-NEXT:    v_add_nc_u32_e32 v3, v3, v0
; S_GFX11-NEXT:    v_add_nc_u32_e32 v4, v4, v1
; S_GFX11-NEXT:    v_or_b32_e32 v8, 0x400000, v2
; S_GFX11-NEXT:    v_or_b32_e32 v6, 0x400000, v1
; S_GFX11-NEXT:    s_delay_alu instid0(VALU_DEP_4) | instskip(SKIP_2) | instid1(VALU_DEP_3)
; S_GFX11-NEXT:    v_add_nc_u32_e32 v3, 0x7fff, v3
; S_GFX11-NEXT:    v_add_nc_u32_e32 v5, v5, v2
; S_GFX11-NEXT:    v_add_nc_u32_e32 v4, 0x7fff, v4
; S_GFX11-NEXT:    v_cndmask_b32_e32 v0, v3, v7, vcc_lo
; S_GFX11-NEXT:    v_cmp_u_f32_e32 vcc_lo, v1, v1
; S_GFX11-NEXT:    s_delay_alu instid0(VALU_DEP_4) | instskip(NEXT) | instid1(VALU_DEP_3)
; S_GFX11-NEXT:    v_add_nc_u32_e32 v5, 0x7fff, v5
; S_GFX11-NEXT:    v_lshrrev_b32_e32 v0, 16, v0
; S_GFX11-NEXT:    v_cndmask_b32_e32 v1, v4, v6, vcc_lo
; S_GFX11-NEXT:    v_cmp_u_f32_e32 vcc_lo, v2, v2
; S_GFX11-NEXT:    s_delay_alu instid0(VALU_DEP_3) | instskip(NEXT) | instid1(VALU_DEP_3)
; S_GFX11-NEXT:    v_and_b32_e32 v0, 0xffff, v0
; S_GFX11-NEXT:    v_lshrrev_b32_e32 v1, 16, v1
; S_GFX11-NEXT:    s_delay_alu instid0(VALU_DEP_1) | instskip(NEXT) | instid1(VALU_DEP_1)
; S_GFX11-NEXT:    v_dual_cndmask_b32 v2, v5, v8 :: v_dual_and_b32 v1, 0xffff, v1
; S_GFX11-NEXT:    v_lshrrev_b32_e32 v2, 16, v2
; S_GFX11-NEXT:    s_delay_alu instid0(VALU_DEP_2) | instskip(NEXT) | instid1(VALU_DEP_2)
; S_GFX11-NEXT:    v_lshl_or_b32 v1, 0x7fc0, 16, v1
; S_GFX11-NEXT:    v_lshl_or_b32 v0, v2, 16, v0
; S_GFX11-NEXT:    s_setpc_b64 s[30:31]
; S_GFX11-NEXT:  .LBB0_3:
; S_GFX11-NEXT:    s_branch .LBB0_2
; S_GFX11-NEXT:  .LBB0_4:
; S_GFX11-NEXT:    v_dual_mov_b32 v0, s0 :: v_dual_mov_b32 v1, s1
; S_GFX11-NEXT:    s_setpc_b64 s[30:31]
  %cmp = icmp eq i32 %b, 0
  br i1 %cmp, label %cmp.true, label %cmp.false

cmp.true:
  %a1 = fadd <3 x bfloat> %a, splat (bfloat 0xR40C0)
  %a2 = bitcast <3 x bfloat> %a1 to <3 x half>
  br label %end

cmp.false:
  %a3 = bitcast <3 x bfloat> %a to <3 x half>
  br label %end

end:
  %phi = phi <3 x half> [ %a2, %cmp.true ], [ %a3, %cmp.false ]
  ret <3 x half> %phi
}

define inreg <3 x bfloat> @bitcast_v3f16_to_v3bf16_inreg(<3 x half> inreg %a, i32 inreg %b) {
; S_SI-LABEL: bitcast_v3f16_to_v3bf16_inreg:
; S_SI:       ; %bb.0:
; S_SI-NEXT:    s_waitcnt vmcnt(0) expcnt(0) lgkmcnt(0)
; S_SI-NEXT:    v_cvt_f16_f32_e32 v3, s16
; S_SI-NEXT:    v_cvt_f16_f32_e32 v4, s17
; S_SI-NEXT:    v_cvt_f16_f32_e32 v5, s18
; S_SI-NEXT:    s_cmp_lg_u32 s19, 0
; S_SI-NEXT:    s_cbranch_scc0 .LBB1_4
; S_SI-NEXT:  ; %bb.1: ; %cmp.false
; S_SI-NEXT:    v_lshlrev_b32_e32 v0, 16, v3
; S_SI-NEXT:    v_lshlrev_b32_e32 v1, 16, v4
; S_SI-NEXT:    v_lshlrev_b32_e32 v2, 16, v5
; S_SI-NEXT:    s_cbranch_execnz .LBB1_3
; S_SI-NEXT:  .LBB1_2: ; %cmp.true
; S_SI-NEXT:    v_cvt_f32_f16_e32 v0, v5
; S_SI-NEXT:    v_cvt_f32_f16_e32 v1, v4
; S_SI-NEXT:    v_cvt_f32_f16_e32 v2, v3
; S_SI-NEXT:    v_add_f32_e32 v0, 0x38000000, v0
; S_SI-NEXT:    v_add_f32_e32 v1, 0x38000000, v1
; S_SI-NEXT:    v_add_f32_e32 v2, 0x38000000, v2
; S_SI-NEXT:    v_cvt_f16_f32_e32 v2, v2
; S_SI-NEXT:    v_cvt_f16_f32_e32 v1, v1
; S_SI-NEXT:    v_cvt_f16_f32_e32 v3, v0
; S_SI-NEXT:    v_lshlrev_b32_e32 v0, 16, v2
; S_SI-NEXT:    v_lshlrev_b32_e32 v1, 16, v1
; S_SI-NEXT:    v_lshlrev_b32_e32 v2, 16, v3
; S_SI-NEXT:  .LBB1_3: ; %end
; S_SI-NEXT:    s_setpc_b64 s[30:31]
; S_SI-NEXT:  .LBB1_4:
; S_SI-NEXT:    ; implicit-def: $vgpr0
; S_SI-NEXT:    ; implicit-def: $vgpr1
; S_SI-NEXT:    ; implicit-def: $vgpr2
; S_SI-NEXT:    s_branch .LBB1_2
;
; S_VI-LABEL: bitcast_v3f16_to_v3bf16_inreg:
; S_VI:       ; %bb.0:
; S_VI-NEXT:    s_waitcnt vmcnt(0) expcnt(0) lgkmcnt(0)
; S_VI-NEXT:    s_cmp_lg_u32 s18, 0
; S_VI-NEXT:    s_cbranch_scc0 .LBB1_3
; S_VI-NEXT:  ; %bb.1: ; %cmp.false
; S_VI-NEXT:    s_cbranch_execnz .LBB1_4
; S_VI-NEXT:  .LBB1_2: ; %cmp.true
; S_VI-NEXT:    s_lshr_b32 s4, s16, 16
; S_VI-NEXT:    v_mov_b32_e32 v0, 0x200
; S_VI-NEXT:    v_mov_b32_e32 v3, s4
; S_VI-NEXT:    v_add_f16_e32 v1, s17, v0
; S_VI-NEXT:    v_add_f16_e32 v2, s16, v0
; S_VI-NEXT:    v_add_f16_sdwa v0, v3, v0 dst_sel:WORD_1 dst_unused:UNUSED_PAD src0_sel:DWORD src1_sel:DWORD
; S_VI-NEXT:    v_or_b32_e32 v0, v2, v0
; S_VI-NEXT:    v_or_b32_e32 v1, 0x7e000000, v1
; S_VI-NEXT:    s_setpc_b64 s[30:31]
; S_VI-NEXT:  .LBB1_3:
; S_VI-NEXT:    s_branch .LBB1_2
; S_VI-NEXT:  .LBB1_4:
; S_VI-NEXT:    v_mov_b32_e32 v0, s16
; S_VI-NEXT:    v_mov_b32_e32 v1, s17
; S_VI-NEXT:    s_setpc_b64 s[30:31]
;
; S_GFX9-LABEL: bitcast_v3f16_to_v3bf16_inreg:
; S_GFX9:       ; %bb.0:
; S_GFX9-NEXT:    s_waitcnt vmcnt(0) expcnt(0) lgkmcnt(0)
; S_GFX9-NEXT:    s_cmp_lg_u32 s18, 0
; S_GFX9-NEXT:    s_cbranch_scc0 .LBB1_3
; S_GFX9-NEXT:  ; %bb.1: ; %cmp.false
; S_GFX9-NEXT:    s_cbranch_execnz .LBB1_4
; S_GFX9-NEXT:  .LBB1_2: ; %cmp.true
; S_GFX9-NEXT:    v_mov_b32_e32 v0, 0x200
; S_GFX9-NEXT:    v_pk_add_f16 v1, s17, v0
; S_GFX9-NEXT:    v_pk_add_f16 v0, s16, v0 op_sel_hi:[1,0]
; S_GFX9-NEXT:    s_setpc_b64 s[30:31]
; S_GFX9-NEXT:  .LBB1_3:
; S_GFX9-NEXT:    s_branch .LBB1_2
; S_GFX9-NEXT:  .LBB1_4:
; S_GFX9-NEXT:    v_mov_b32_e32 v0, s16
; S_GFX9-NEXT:    v_mov_b32_e32 v1, s17
; S_GFX9-NEXT:    s_setpc_b64 s[30:31]
;
; S_GFX11-LABEL: bitcast_v3f16_to_v3bf16_inreg:
; S_GFX11:       ; %bb.0:
; S_GFX11-NEXT:    s_waitcnt vmcnt(0) expcnt(0) lgkmcnt(0)
; S_GFX11-NEXT:    s_cmp_lg_u32 s2, 0
; S_GFX11-NEXT:    s_mov_b32 s2, 0
; S_GFX11-NEXT:    s_cbranch_scc0 .LBB1_3
; S_GFX11-NEXT:  ; %bb.1: ; %Flow
; S_GFX11-NEXT:    s_and_not1_b32 vcc_lo, exec_lo, s2
; S_GFX11-NEXT:    s_cbranch_vccnz .LBB1_4
; S_GFX11-NEXT:  .LBB1_2: ; %cmp.true
; S_GFX11-NEXT:    v_pk_add_f16 v1, 0x200, s1
; S_GFX11-NEXT:    v_pk_add_f16 v0, 0x200, s0 op_sel_hi:[0,1]
; S_GFX11-NEXT:    s_setpc_b64 s[30:31]
; S_GFX11-NEXT:  .LBB1_3:
; S_GFX11-NEXT:    s_branch .LBB1_2
; S_GFX11-NEXT:  .LBB1_4:
; S_GFX11-NEXT:    v_dual_mov_b32 v0, s0 :: v_dual_mov_b32 v1, s1
; S_GFX11-NEXT:    s_setpc_b64 s[30:31]
  %cmp = icmp eq i32 %b, 0
  br i1 %cmp, label %cmp.true, label %cmp.false

cmp.true:
  %a1 = fadd <3 x half> %a, splat (half 0xH0200)
  %a2 = bitcast <3 x half> %a1 to <3 x bfloat>
  br label %end

cmp.false:
  %a3 = bitcast <3 x half> %a to <3 x bfloat>
  br label %end

end:
  %phi = phi <3 x bfloat> [ %a2, %cmp.true ], [ %a3, %cmp.false ]
  ret <3 x bfloat> %phi
}

define inreg <3 x i16> @bitcast_v3bf16_to_v3i16_inreg(<3 x bfloat> inreg %a, i32 inreg %b) {
; S_SI-LABEL: bitcast_v3bf16_to_v3i16_inreg:
; S_SI:       ; %bb.0:
; S_SI-NEXT:    s_waitcnt vmcnt(0) expcnt(0) lgkmcnt(0)
; S_SI-NEXT:    s_cmp_lg_u32 s19, 0
; S_SI-NEXT:    v_mul_f32_e64 v5, 1.0, s16
; S_SI-NEXT:    v_mul_f32_e64 v4, 1.0, s17
; S_SI-NEXT:    v_mul_f32_e64 v3, 1.0, s18
; S_SI-NEXT:    s_cbranch_scc0 .LBB2_4
; S_SI-NEXT:  ; %bb.1: ; %cmp.false
; S_SI-NEXT:    v_lshrrev_b32_e32 v0, 16, v5
; S_SI-NEXT:    v_lshrrev_b32_e32 v1, 16, v4
; S_SI-NEXT:    v_lshrrev_b32_e32 v2, 16, v3
; S_SI-NEXT:    s_cbranch_execnz .LBB2_3
; S_SI-NEXT:  .LBB2_2: ; %cmp.true
; S_SI-NEXT:    v_and_b32_e32 v1, 0xffff0000, v4
; S_SI-NEXT:    v_and_b32_e32 v0, 0xffff0000, v5
; S_SI-NEXT:    v_add_f32_e32 v1, 0x40c00000, v1
; S_SI-NEXT:    v_add_f32_e32 v0, 0x40c00000, v0
; S_SI-NEXT:    v_lshrrev_b32_e32 v2, 16, v1
; S_SI-NEXT:    v_alignbit_b32 v0, v2, v0, 16
; S_SI-NEXT:    v_and_b32_e32 v2, 0xffff0000, v3
; S_SI-NEXT:    v_add_f32_e32 v2, 0x40c00000, v2
; S_SI-NEXT:    v_and_b32_e32 v1, 0xffff0000, v1
; S_SI-NEXT:    v_lshrrev_b32_e32 v2, 16, v2
; S_SI-NEXT:    v_alignbit_b32 v1, v2, v1, 16
; S_SI-NEXT:  .LBB2_3: ; %end
; S_SI-NEXT:    s_setpc_b64 s[30:31]
; S_SI-NEXT:  .LBB2_4:
; S_SI-NEXT:    ; implicit-def: $vgpr0
; S_SI-NEXT:    ; implicit-def: $vgpr1
; S_SI-NEXT:    ; implicit-def: $vgpr2
; S_SI-NEXT:    s_branch .LBB2_2
;
; S_VI-LABEL: bitcast_v3bf16_to_v3i16_inreg:
; S_VI:       ; %bb.0:
; S_VI-NEXT:    s_waitcnt vmcnt(0) expcnt(0) lgkmcnt(0)
; S_VI-NEXT:    s_cmp_lg_u32 s18, 0
; S_VI-NEXT:    s_cbranch_scc0 .LBB2_3
; S_VI-NEXT:  ; %bb.1: ; %cmp.false
; S_VI-NEXT:    s_cbranch_execnz .LBB2_4
; S_VI-NEXT:  .LBB2_2: ; %cmp.true
; S_VI-NEXT:    s_lshl_b32 s4, s17, 16
; S_VI-NEXT:    v_mov_b32_e32 v0, 0x40c00000
; S_VI-NEXT:    v_add_f32_e32 v1, s4, v0
; S_VI-NEXT:    v_bfe_u32 v2, v1, 16, 1
; S_VI-NEXT:    v_add_u32_e32 v2, vcc, v2, v1
; S_VI-NEXT:    v_add_u32_e32 v2, vcc, 0x7fff, v2
; S_VI-NEXT:    v_or_b32_e32 v3, 0x400000, v1
; S_VI-NEXT:    v_cmp_u_f32_e32 vcc, v1, v1
; S_VI-NEXT:    s_lshl_b32 s4, s16, 16
; S_VI-NEXT:    v_cndmask_b32_e32 v1, v2, v3, vcc
; S_VI-NEXT:    v_add_f32_e32 v2, s4, v0
; S_VI-NEXT:    v_bfe_u32 v3, v2, 16, 1
; S_VI-NEXT:    v_add_u32_e32 v3, vcc, v3, v2
; S_VI-NEXT:    v_add_u32_e32 v3, vcc, 0x7fff, v3
; S_VI-NEXT:    s_and_b32 s4, s16, 0xffff0000
; S_VI-NEXT:    v_or_b32_e32 v4, 0x400000, v2
; S_VI-NEXT:    v_cmp_u_f32_e32 vcc, v2, v2
; S_VI-NEXT:    v_add_f32_e32 v0, s4, v0
; S_VI-NEXT:    v_cndmask_b32_e32 v2, v3, v4, vcc
; S_VI-NEXT:    v_bfe_u32 v3, v0, 16, 1
; S_VI-NEXT:    v_add_u32_e32 v3, vcc, v3, v0
; S_VI-NEXT:    v_add_u32_e32 v3, vcc, 0x7fff, v3
; S_VI-NEXT:    v_or_b32_e32 v4, 0x400000, v0
; S_VI-NEXT:    v_cmp_u_f32_e32 vcc, v0, v0
; S_VI-NEXT:    v_cndmask_b32_e32 v0, v3, v4, vcc
; S_VI-NEXT:    v_lshrrev_b32_e32 v0, 16, v0
; S_VI-NEXT:    v_alignbit_b32 v0, v0, v2, 16
; S_VI-NEXT:    v_mov_b32_e32 v2, 0x7fc00000
; S_VI-NEXT:    v_or_b32_sdwa v1, v1, v2 dst_sel:DWORD dst_unused:UNUSED_PAD src0_sel:WORD_1 src1_sel:DWORD
; S_VI-NEXT:    s_setpc_b64 s[30:31]
; S_VI-NEXT:  .LBB2_3:
; S_VI-NEXT:    s_branch .LBB2_2
; S_VI-NEXT:  .LBB2_4:
; S_VI-NEXT:    v_mov_b32_e32 v0, s16
; S_VI-NEXT:    v_mov_b32_e32 v1, s17
; S_VI-NEXT:    s_setpc_b64 s[30:31]
;
; S_GFX9-LABEL: bitcast_v3bf16_to_v3i16_inreg:
; S_GFX9:       ; %bb.0:
; S_GFX9-NEXT:    s_waitcnt vmcnt(0) expcnt(0) lgkmcnt(0)
; S_GFX9-NEXT:    s_cmp_lg_u32 s18, 0
; S_GFX9-NEXT:    s_cbranch_scc0 .LBB2_3
; S_GFX9-NEXT:  ; %bb.1: ; %cmp.false
; S_GFX9-NEXT:    s_cbranch_execnz .LBB2_4
; S_GFX9-NEXT:  .LBB2_2: ; %cmp.true
; S_GFX9-NEXT:    s_lshl_b32 s4, s17, 16
; S_GFX9-NEXT:    v_mov_b32_e32 v0, 0x40c00000
; S_GFX9-NEXT:    v_add_f32_e32 v1, s4, v0
; S_GFX9-NEXT:    v_bfe_u32 v2, v1, 16, 1
; S_GFX9-NEXT:    v_add_u32_e32 v2, v2, v1
; S_GFX9-NEXT:    v_add_u32_e32 v2, 0x7fff, v2
; S_GFX9-NEXT:    v_or_b32_e32 v3, 0x400000, v1
; S_GFX9-NEXT:    v_cmp_u_f32_e32 vcc, v1, v1
; S_GFX9-NEXT:    s_pack_lh_b32_b16 s4, 0, s16
; S_GFX9-NEXT:    v_cndmask_b32_e32 v1, v2, v3, vcc
; S_GFX9-NEXT:    v_add_f32_e32 v2, s4, v0
; S_GFX9-NEXT:    v_bfe_u32 v3, v2, 16, 1
; S_GFX9-NEXT:    v_add_u32_e32 v3, v3, v2
; S_GFX9-NEXT:    s_lshl_b32 s4, s16, 16
; S_GFX9-NEXT:    v_add_u32_e32 v3, 0x7fff, v3
; S_GFX9-NEXT:    v_or_b32_e32 v4, 0x400000, v2
; S_GFX9-NEXT:    v_cmp_u_f32_e32 vcc, v2, v2
; S_GFX9-NEXT:    v_add_f32_e32 v0, s4, v0
; S_GFX9-NEXT:    v_cndmask_b32_e32 v2, v3, v4, vcc
; S_GFX9-NEXT:    v_bfe_u32 v3, v0, 16, 1
; S_GFX9-NEXT:    v_add_u32_e32 v3, v3, v0
; S_GFX9-NEXT:    v_add_u32_e32 v3, 0x7fff, v3
; S_GFX9-NEXT:    v_or_b32_e32 v4, 0x400000, v0
; S_GFX9-NEXT:    v_cmp_u_f32_e32 vcc, v0, v0
; S_GFX9-NEXT:    v_cndmask_b32_e32 v0, v3, v4, vcc
; S_GFX9-NEXT:    v_lshrrev_b32_e32 v0, 16, v0
; S_GFX9-NEXT:    v_mov_b32_e32 v3, 0xffff0000
; S_GFX9-NEXT:    v_and_or_b32 v0, v2, v3, v0
; S_GFX9-NEXT:    v_mov_b32_e32 v2, 0xffff
; S_GFX9-NEXT:    s_movk_i32 s4, 0x7fc0
; S_GFX9-NEXT:    v_and_b32_sdwa v1, v2, v1 dst_sel:DWORD dst_unused:UNUSED_PAD src0_sel:DWORD src1_sel:WORD_1
; S_GFX9-NEXT:    v_lshl_or_b32 v1, s4, 16, v1
; S_GFX9-NEXT:    s_setpc_b64 s[30:31]
; S_GFX9-NEXT:  .LBB2_3:
; S_GFX9-NEXT:    s_branch .LBB2_2
; S_GFX9-NEXT:  .LBB2_4:
; S_GFX9-NEXT:    v_mov_b32_e32 v0, s16
; S_GFX9-NEXT:    v_mov_b32_e32 v1, s17
; S_GFX9-NEXT:    s_setpc_b64 s[30:31]
;
; S_GFX11-LABEL: bitcast_v3bf16_to_v3i16_inreg:
; S_GFX11:       ; %bb.0:
; S_GFX11-NEXT:    s_waitcnt vmcnt(0) expcnt(0) lgkmcnt(0)
; S_GFX11-NEXT:    s_cmp_lg_u32 s2, 0
; S_GFX11-NEXT:    s_mov_b32 s2, 0
; S_GFX11-NEXT:    s_cbranch_scc0 .LBB2_3
; S_GFX11-NEXT:  ; %bb.1: ; %Flow
; S_GFX11-NEXT:    s_and_not1_b32 vcc_lo, exec_lo, s2
; S_GFX11-NEXT:    s_cbranch_vccnz .LBB2_4
; S_GFX11-NEXT:  .LBB2_2: ; %cmp.true
; S_GFX11-NEXT:    s_lshl_b32 s2, s0, 16
; S_GFX11-NEXT:    s_lshl_b32 s1, s1, 16
; S_GFX11-NEXT:    v_add_f32_e64 v0, 0x40c00000, s2
; S_GFX11-NEXT:    v_add_f32_e64 v1, 0x40c00000, s1
; S_GFX11-NEXT:    s_pack_lh_b32_b16 s0, 0, s0
; S_GFX11-NEXT:    s_delay_alu instid0(SALU_CYCLE_1) | instskip(NEXT) | instid1(VALU_DEP_3)
; S_GFX11-NEXT:    v_add_f32_e64 v2, 0x40c00000, s0
; S_GFX11-NEXT:    v_bfe_u32 v3, v0, 16, 1
; S_GFX11-NEXT:    s_delay_alu instid0(VALU_DEP_3)
; S_GFX11-NEXT:    v_bfe_u32 v4, v1, 16, 1
; S_GFX11-NEXT:    v_or_b32_e32 v7, 0x400000, v0
; S_GFX11-NEXT:    v_cmp_u_f32_e32 vcc_lo, v0, v0
; S_GFX11-NEXT:    v_bfe_u32 v5, v2, 16, 1
; S_GFX11-NEXT:    v_add_nc_u32_e32 v3, v3, v0
; S_GFX11-NEXT:    v_add_nc_u32_e32 v4, v4, v1
; S_GFX11-NEXT:    v_or_b32_e32 v8, 0x400000, v2
; S_GFX11-NEXT:    v_or_b32_e32 v6, 0x400000, v1
; S_GFX11-NEXT:    s_delay_alu instid0(VALU_DEP_4) | instskip(SKIP_2) | instid1(VALU_DEP_3)
; S_GFX11-NEXT:    v_add_nc_u32_e32 v3, 0x7fff, v3
; S_GFX11-NEXT:    v_add_nc_u32_e32 v5, v5, v2
; S_GFX11-NEXT:    v_add_nc_u32_e32 v4, 0x7fff, v4
; S_GFX11-NEXT:    v_cndmask_b32_e32 v0, v3, v7, vcc_lo
; S_GFX11-NEXT:    v_cmp_u_f32_e32 vcc_lo, v1, v1
; S_GFX11-NEXT:    s_delay_alu instid0(VALU_DEP_4) | instskip(NEXT) | instid1(VALU_DEP_3)
; S_GFX11-NEXT:    v_add_nc_u32_e32 v5, 0x7fff, v5
; S_GFX11-NEXT:    v_lshrrev_b32_e32 v0, 16, v0
; S_GFX11-NEXT:    v_cndmask_b32_e32 v1, v4, v6, vcc_lo
; S_GFX11-NEXT:    v_cmp_u_f32_e32 vcc_lo, v2, v2
; S_GFX11-NEXT:    s_delay_alu instid0(VALU_DEP_2) | instskip(SKIP_1) | instid1(VALU_DEP_2)
; S_GFX11-NEXT:    v_lshrrev_b32_e32 v1, 16, v1
; S_GFX11-NEXT:    v_cndmask_b32_e32 v2, v5, v8, vcc_lo
; S_GFX11-NEXT:    v_lshl_or_b32 v1, 0x7fc0, 16, v1
; S_GFX11-NEXT:    s_delay_alu instid0(VALU_DEP_2)
; S_GFX11-NEXT:    v_and_or_b32 v0, 0xffff0000, v2, v0
; S_GFX11-NEXT:    s_setpc_b64 s[30:31]
; S_GFX11-NEXT:  .LBB2_3:
; S_GFX11-NEXT:    s_branch .LBB2_2
; S_GFX11-NEXT:  .LBB2_4:
; S_GFX11-NEXT:    v_dual_mov_b32 v0, s0 :: v_dual_mov_b32 v1, s1
; S_GFX11-NEXT:    s_setpc_b64 s[30:31]
  %cmp = icmp eq i32 %b, 0
  br i1 %cmp, label %cmp.true, label %cmp.false

cmp.true:
  %a1 = fadd <3 x bfloat> %a, splat (bfloat 0xR40C0)
  %a2 = bitcast <3 x bfloat> %a1 to <3 x i16>
  br label %end

cmp.false:
  %a3 = bitcast <3 x bfloat> %a to <3 x i16>
  br label %end

end:
  %phi = phi <3 x i16> [ %a2, %cmp.true ], [ %a3, %cmp.false ]
  ret <3 x i16> %phi
}

define inreg <3 x bfloat> @bitcast_v3i16_to_v3bf16_inreg(<3 x i16> inreg %a, i32 inreg %b) {
; S_SI-LABEL: bitcast_v3i16_to_v3bf16_inreg:
; S_SI:       ; %bb.0:
; S_SI-NEXT:    s_waitcnt vmcnt(0) expcnt(0) lgkmcnt(0)
; S_SI-NEXT:    s_cmp_lg_u32 s19, 0
; S_SI-NEXT:    s_cbranch_scc0 .LBB3_4
; S_SI-NEXT:  ; %bb.1: ; %cmp.false
; S_SI-NEXT:    s_lshl_b32 s6, s16, 16
; S_SI-NEXT:    s_lshl_b32 s7, s17, 16
; S_SI-NEXT:    s_lshl_b32 s8, s18, 16
; S_SI-NEXT:    s_cbranch_execnz .LBB3_3
; S_SI-NEXT:  .LBB3_2: ; %cmp.true
; S_SI-NEXT:    s_add_i32 s16, s16, 3
; S_SI-NEXT:    s_and_b32 s4, s16, 0xffff
; S_SI-NEXT:    s_lshl_b32 s5, s17, 16
; S_SI-NEXT:    s_or_b32 s4, s5, s4
; S_SI-NEXT:    s_add_i32 s4, s4, 0x30000
; S_SI-NEXT:    s_lshl_b32 s5, s18, 16
; S_SI-NEXT:    s_and_b32 s7, s4, 0xffff0000
; S_SI-NEXT:    s_lshl_b32 s6, s4, 16
; S_SI-NEXT:    s_add_i32 s8, s5, 0x30000
; S_SI-NEXT:  .LBB3_3: ; %end
; S_SI-NEXT:    v_mov_b32_e32 v0, s6
; S_SI-NEXT:    v_mov_b32_e32 v1, s7
; S_SI-NEXT:    v_mov_b32_e32 v2, s8
; S_SI-NEXT:    s_setpc_b64 s[30:31]
; S_SI-NEXT:  .LBB3_4:
; S_SI-NEXT:    ; implicit-def: $sgpr6
; S_SI-NEXT:    ; implicit-def: $sgpr7
; S_SI-NEXT:    ; implicit-def: $sgpr8
; S_SI-NEXT:    s_branch .LBB3_2
;
; S_VI-LABEL: bitcast_v3i16_to_v3bf16_inreg:
; S_VI:       ; %bb.0:
; S_VI-NEXT:    s_waitcnt vmcnt(0) expcnt(0) lgkmcnt(0)
; S_VI-NEXT:    s_cmp_lg_u32 s18, 0
; S_VI-NEXT:    s_cbranch_scc0 .LBB3_4
; S_VI-NEXT:  ; %bb.1: ; %cmp.false
; S_VI-NEXT:    s_cbranch_execnz .LBB3_3
; S_VI-NEXT:  .LBB3_2: ; %cmp.true
; S_VI-NEXT:    s_add_i32 s5, s16, 3
; S_VI-NEXT:    s_and_b32 s4, s16, 0xffff0000
; S_VI-NEXT:    s_and_b32 s5, s5, 0xffff
; S_VI-NEXT:    s_or_b32 s4, s4, s5
; S_VI-NEXT:    s_add_i32 s17, s17, 3
; S_VI-NEXT:    s_add_i32 s16, s4, 0x30000
; S_VI-NEXT:  .LBB3_3: ; %end
; S_VI-NEXT:    v_mov_b32_e32 v0, s16
; S_VI-NEXT:    v_mov_b32_e32 v1, s17
; S_VI-NEXT:    s_setpc_b64 s[30:31]
; S_VI-NEXT:  .LBB3_4:
; S_VI-NEXT:    s_branch .LBB3_2
;
; S_GFX9-LABEL: bitcast_v3i16_to_v3bf16_inreg:
; S_GFX9:       ; %bb.0:
; S_GFX9-NEXT:    s_waitcnt vmcnt(0) expcnt(0) lgkmcnt(0)
; S_GFX9-NEXT:    s_cmp_lg_u32 s18, 0
; S_GFX9-NEXT:    s_cbranch_scc0 .LBB3_3
; S_GFX9-NEXT:  ; %bb.1: ; %cmp.false
; S_GFX9-NEXT:    s_cbranch_execnz .LBB3_4
; S_GFX9-NEXT:  .LBB3_2: ; %cmp.true
; S_GFX9-NEXT:    v_pk_add_u16 v1, s17, 3
; S_GFX9-NEXT:    v_pk_add_u16 v0, s16, 3 op_sel_hi:[1,0]
; S_GFX9-NEXT:    s_setpc_b64 s[30:31]
; S_GFX9-NEXT:  .LBB3_3:
; S_GFX9-NEXT:    s_branch .LBB3_2
; S_GFX9-NEXT:  .LBB3_4:
; S_GFX9-NEXT:    v_mov_b32_e32 v0, s16
; S_GFX9-NEXT:    v_mov_b32_e32 v1, s17
; S_GFX9-NEXT:    s_setpc_b64 s[30:31]
;
; S_GFX11-LABEL: bitcast_v3i16_to_v3bf16_inreg:
; S_GFX11:       ; %bb.0:
; S_GFX11-NEXT:    s_waitcnt vmcnt(0) expcnt(0) lgkmcnt(0)
; S_GFX11-NEXT:    s_cmp_lg_u32 s2, 0
; S_GFX11-NEXT:    s_mov_b32 s2, 0
; S_GFX11-NEXT:    s_cbranch_scc0 .LBB3_3
; S_GFX11-NEXT:  ; %bb.1: ; %Flow
; S_GFX11-NEXT:    s_and_not1_b32 vcc_lo, exec_lo, s2
; S_GFX11-NEXT:    s_cbranch_vccnz .LBB3_4
; S_GFX11-NEXT:  .LBB3_2: ; %cmp.true
; S_GFX11-NEXT:    v_pk_add_u16 v1, s1, 3
; S_GFX11-NEXT:    v_pk_add_u16 v0, s0, 3 op_sel_hi:[1,0]
; S_GFX11-NEXT:    s_setpc_b64 s[30:31]
; S_GFX11-NEXT:  .LBB3_3:
; S_GFX11-NEXT:    s_branch .LBB3_2
; S_GFX11-NEXT:  .LBB3_4:
; S_GFX11-NEXT:    v_dual_mov_b32 v0, s0 :: v_dual_mov_b32 v1, s1
; S_GFX11-NEXT:    s_setpc_b64 s[30:31]
  %cmp = icmp eq i32 %b, 0
  br i1 %cmp, label %cmp.true, label %cmp.false

cmp.true:
  %a1 = add <3 x i16> %a, splat (i16 3)
  %a2 = bitcast <3 x i16> %a1 to <3 x bfloat>
  br label %end

cmp.false:
  %a3 = bitcast <3 x i16> %a to <3 x bfloat>
  br label %end

end:
  %phi = phi <3 x bfloat> [ %a2, %cmp.true ], [ %a3, %cmp.false ]
  ret <3 x bfloat> %phi
}

define inreg <3 x i16> @bitcast_v3f16_to_v3i16_inreg(<3 x half> inreg %a, i32 inreg %b) {
; S_SI-LABEL: bitcast_v3f16_to_v3i16_inreg:
; S_SI:       ; %bb.0:
; S_SI-NEXT:    s_waitcnt vmcnt(0) expcnt(0) lgkmcnt(0)
; S_SI-NEXT:    v_cvt_f16_f32_e32 v0, s16
; S_SI-NEXT:    v_cvt_f16_f32_e32 v1, s17
; S_SI-NEXT:    v_cvt_f16_f32_e32 v2, s18
; S_SI-NEXT:    s_cmp_lg_u32 s19, 0
; S_SI-NEXT:    s_cbranch_scc0 .LBB4_4
; S_SI-NEXT:  ; %bb.1: ; %cmp.false
; S_SI-NEXT:    s_cbranch_execnz .LBB4_3
; S_SI-NEXT:  .LBB4_2: ; %cmp.true
; S_SI-NEXT:    v_cvt_f32_f16_e32 v1, v1
; S_SI-NEXT:    v_cvt_f32_f16_e32 v0, v0
; S_SI-NEXT:    v_cvt_f32_f16_e32 v2, v2
; S_SI-NEXT:    v_add_f32_e32 v1, 0x38000000, v1
; S_SI-NEXT:    v_add_f32_e32 v0, 0x38000000, v0
; S_SI-NEXT:    v_cvt_f16_f32_e32 v1, v1
; S_SI-NEXT:    v_add_f32_e32 v2, 0x38000000, v2
; S_SI-NEXT:    v_cvt_f16_f32_e32 v0, v0
; S_SI-NEXT:    v_cvt_f16_f32_e32 v2, v2
; S_SI-NEXT:    v_lshlrev_b32_e32 v1, 16, v1
; S_SI-NEXT:    v_or_b32_e32 v0, v0, v1
; S_SI-NEXT:    v_alignbit_b32 v1, v2, v1, 16
; S_SI-NEXT:  .LBB4_3: ; %end
; S_SI-NEXT:    s_setpc_b64 s[30:31]
; S_SI-NEXT:  .LBB4_4:
; S_SI-NEXT:    s_branch .LBB4_2
;
; S_VI-LABEL: bitcast_v3f16_to_v3i16_inreg:
; S_VI:       ; %bb.0:
; S_VI-NEXT:    s_waitcnt vmcnt(0) expcnt(0) lgkmcnt(0)
; S_VI-NEXT:    s_cmp_lg_u32 s18, 0
; S_VI-NEXT:    s_cbranch_scc0 .LBB4_3
; S_VI-NEXT:  ; %bb.1: ; %cmp.false
; S_VI-NEXT:    s_cbranch_execnz .LBB4_4
; S_VI-NEXT:  .LBB4_2: ; %cmp.true
; S_VI-NEXT:    s_lshr_b32 s4, s16, 16
; S_VI-NEXT:    v_mov_b32_e32 v0, 0x200
; S_VI-NEXT:    v_mov_b32_e32 v3, s4
; S_VI-NEXT:    v_add_f16_e32 v1, s17, v0
; S_VI-NEXT:    v_add_f16_e32 v2, s16, v0
; S_VI-NEXT:    v_add_f16_sdwa v0, v3, v0 dst_sel:WORD_1 dst_unused:UNUSED_PAD src0_sel:DWORD src1_sel:DWORD
; S_VI-NEXT:    v_or_b32_e32 v0, v2, v0
; S_VI-NEXT:    v_or_b32_e32 v1, 0x7e000000, v1
; S_VI-NEXT:    s_setpc_b64 s[30:31]
; S_VI-NEXT:  .LBB4_3:
; S_VI-NEXT:    s_branch .LBB4_2
; S_VI-NEXT:  .LBB4_4:
; S_VI-NEXT:    v_mov_b32_e32 v0, s16
; S_VI-NEXT:    v_mov_b32_e32 v1, s17
; S_VI-NEXT:    s_setpc_b64 s[30:31]
;
; S_GFX9-LABEL: bitcast_v3f16_to_v3i16_inreg:
; S_GFX9:       ; %bb.0:
; S_GFX9-NEXT:    s_waitcnt vmcnt(0) expcnt(0) lgkmcnt(0)
; S_GFX9-NEXT:    s_cmp_lg_u32 s18, 0
; S_GFX9-NEXT:    s_cbranch_scc0 .LBB4_3
; S_GFX9-NEXT:  ; %bb.1: ; %cmp.false
; S_GFX9-NEXT:    s_cbranch_execnz .LBB4_4
; S_GFX9-NEXT:  .LBB4_2: ; %cmp.true
; S_GFX9-NEXT:    v_mov_b32_e32 v0, 0x200
; S_GFX9-NEXT:    v_pk_add_f16 v1, s17, v0
; S_GFX9-NEXT:    v_pk_add_f16 v0, s16, v0 op_sel_hi:[1,0]
; S_GFX9-NEXT:    s_setpc_b64 s[30:31]
; S_GFX9-NEXT:  .LBB4_3:
; S_GFX9-NEXT:    s_branch .LBB4_2
; S_GFX9-NEXT:  .LBB4_4:
; S_GFX9-NEXT:    v_mov_b32_e32 v0, s16
; S_GFX9-NEXT:    v_mov_b32_e32 v1, s17
; S_GFX9-NEXT:    s_setpc_b64 s[30:31]
;
; S_GFX11-LABEL: bitcast_v3f16_to_v3i16_inreg:
; S_GFX11:       ; %bb.0:
; S_GFX11-NEXT:    s_waitcnt vmcnt(0) expcnt(0) lgkmcnt(0)
; S_GFX11-NEXT:    s_cmp_lg_u32 s2, 0
; S_GFX11-NEXT:    s_mov_b32 s2, 0
; S_GFX11-NEXT:    s_cbranch_scc0 .LBB4_3
; S_GFX11-NEXT:  ; %bb.1: ; %Flow
; S_GFX11-NEXT:    s_and_not1_b32 vcc_lo, exec_lo, s2
; S_GFX11-NEXT:    s_cbranch_vccnz .LBB4_4
; S_GFX11-NEXT:  .LBB4_2: ; %cmp.true
; S_GFX11-NEXT:    v_pk_add_f16 v1, 0x200, s1
; S_GFX11-NEXT:    v_pk_add_f16 v0, 0x200, s0 op_sel_hi:[0,1]
; S_GFX11-NEXT:    s_setpc_b64 s[30:31]
; S_GFX11-NEXT:  .LBB4_3:
; S_GFX11-NEXT:    s_branch .LBB4_2
; S_GFX11-NEXT:  .LBB4_4:
; S_GFX11-NEXT:    v_dual_mov_b32 v0, s0 :: v_dual_mov_b32 v1, s1
; S_GFX11-NEXT:    s_setpc_b64 s[30:31]
  %cmp = icmp eq i32 %b, 0
  br i1 %cmp, label %cmp.true, label %cmp.false

cmp.true:
  %a1 = fadd <3 x half> %a, splat (half 0xH0200)
  %a2 = bitcast <3 x half> %a1 to <3 x i16>
  br label %end

cmp.false:
  %a3 = bitcast <3 x half> %a to <3 x i16>
  br label %end

end:
  %phi = phi <3 x i16> [ %a2, %cmp.true ], [ %a3, %cmp.false ]
  ret <3 x i16> %phi
}

define inreg <3 x half> @bitcast_v3i16_to_v3f16_inreg(<3 x i16> inreg %a, i32 inreg %b) {
; S_SI-LABEL: bitcast_v3i16_to_v3f16_inreg:
; S_SI:       ; %bb.0:
; S_SI-NEXT:    s_waitcnt vmcnt(0) expcnt(0) lgkmcnt(0)
; S_SI-NEXT:    s_cmp_lg_u32 s19, 0
; S_SI-NEXT:    s_cbranch_scc0 .LBB5_4
; S_SI-NEXT:  ; %bb.1: ; %cmp.false
; S_SI-NEXT:    v_cvt_f32_f16_e32 v0, s16
; S_SI-NEXT:    v_cvt_f32_f16_e32 v1, s17
; S_SI-NEXT:    v_cvt_f32_f16_e32 v2, s18
; S_SI-NEXT:    s_cbranch_execnz .LBB5_3
; S_SI-NEXT:  .LBB5_2: ; %cmp.true
; S_SI-NEXT:    s_add_i32 s18, s18, 3
; S_SI-NEXT:    s_add_i32 s17, s17, 3
; S_SI-NEXT:    s_add_i32 s16, s16, 3
; S_SI-NEXT:    v_cvt_f32_f16_e32 v0, s16
; S_SI-NEXT:    v_cvt_f32_f16_e32 v1, s17
; S_SI-NEXT:    v_cvt_f32_f16_e32 v2, s18
; S_SI-NEXT:  .LBB5_3: ; %end
; S_SI-NEXT:    s_setpc_b64 s[30:31]
; S_SI-NEXT:  .LBB5_4:
; S_SI-NEXT:    ; implicit-def: $vgpr0
; S_SI-NEXT:    ; implicit-def: $vgpr1
; S_SI-NEXT:    ; implicit-def: $vgpr2
; S_SI-NEXT:    s_branch .LBB5_2
;
; S_VI-LABEL: bitcast_v3i16_to_v3f16_inreg:
; S_VI:       ; %bb.0:
; S_VI-NEXT:    s_waitcnt vmcnt(0) expcnt(0) lgkmcnt(0)
; S_VI-NEXT:    s_cmp_lg_u32 s18, 0
; S_VI-NEXT:    s_cbranch_scc0 .LBB5_4
; S_VI-NEXT:  ; %bb.1: ; %cmp.false
; S_VI-NEXT:    s_cbranch_execnz .LBB5_3
; S_VI-NEXT:  .LBB5_2: ; %cmp.true
; S_VI-NEXT:    s_add_i32 s5, s16, 3
; S_VI-NEXT:    s_and_b32 s4, s16, 0xffff0000
; S_VI-NEXT:    s_and_b32 s5, s5, 0xffff
; S_VI-NEXT:    s_or_b32 s4, s4, s5
; S_VI-NEXT:    s_add_i32 s17, s17, 3
; S_VI-NEXT:    s_add_i32 s16, s4, 0x30000
; S_VI-NEXT:  .LBB5_3: ; %end
; S_VI-NEXT:    v_mov_b32_e32 v0, s16
; S_VI-NEXT:    v_mov_b32_e32 v1, s17
; S_VI-NEXT:    s_setpc_b64 s[30:31]
; S_VI-NEXT:  .LBB5_4:
; S_VI-NEXT:    s_branch .LBB5_2
;
; S_GFX9-LABEL: bitcast_v3i16_to_v3f16_inreg:
; S_GFX9:       ; %bb.0:
; S_GFX9-NEXT:    s_waitcnt vmcnt(0) expcnt(0) lgkmcnt(0)
; S_GFX9-NEXT:    s_cmp_lg_u32 s18, 0
; S_GFX9-NEXT:    s_cbranch_scc0 .LBB5_3
; S_GFX9-NEXT:  ; %bb.1: ; %cmp.false
; S_GFX9-NEXT:    s_cbranch_execnz .LBB5_4
; S_GFX9-NEXT:  .LBB5_2: ; %cmp.true
; S_GFX9-NEXT:    v_pk_add_u16 v1, s17, 3
; S_GFX9-NEXT:    v_pk_add_u16 v0, s16, 3 op_sel_hi:[1,0]
; S_GFX9-NEXT:    s_setpc_b64 s[30:31]
; S_GFX9-NEXT:  .LBB5_3:
; S_GFX9-NEXT:    s_branch .LBB5_2
; S_GFX9-NEXT:  .LBB5_4:
; S_GFX9-NEXT:    v_mov_b32_e32 v0, s16
; S_GFX9-NEXT:    v_mov_b32_e32 v1, s17
; S_GFX9-NEXT:    s_setpc_b64 s[30:31]
;
; S_GFX11-LABEL: bitcast_v3i16_to_v3f16_inreg:
; S_GFX11:       ; %bb.0:
; S_GFX11-NEXT:    s_waitcnt vmcnt(0) expcnt(0) lgkmcnt(0)
; S_GFX11-NEXT:    s_cmp_lg_u32 s2, 0
; S_GFX11-NEXT:    s_mov_b32 s2, 0
; S_GFX11-NEXT:    s_cbranch_scc0 .LBB5_3
; S_GFX11-NEXT:  ; %bb.1: ; %Flow
; S_GFX11-NEXT:    s_and_not1_b32 vcc_lo, exec_lo, s2
; S_GFX11-NEXT:    s_cbranch_vccnz .LBB5_4
; S_GFX11-NEXT:  .LBB5_2: ; %cmp.true
; S_GFX11-NEXT:    v_pk_add_u16 v1, s1, 3
; S_GFX11-NEXT:    v_pk_add_u16 v0, s0, 3 op_sel_hi:[1,0]
; S_GFX11-NEXT:    s_setpc_b64 s[30:31]
; S_GFX11-NEXT:  .LBB5_3:
; S_GFX11-NEXT:    s_branch .LBB5_2
; S_GFX11-NEXT:  .LBB5_4:
; S_GFX11-NEXT:    v_dual_mov_b32 v0, s0 :: v_dual_mov_b32 v1, s1
; S_GFX11-NEXT:    s_setpc_b64 s[30:31]
  %cmp = icmp eq i32 %b, 0
  br i1 %cmp, label %cmp.true, label %cmp.false

cmp.true:
  %a1 = add <3 x i16> %a, splat (i16 3)
  %a2 = bitcast <3 x i16> %a1 to <3 x half>
  br label %end

cmp.false:
  %a3 = bitcast <3 x i16> %a to <3 x half>
  br label %end

end:
  %phi = phi <3 x half> [ %a2, %cmp.true ], [ %a3, %cmp.false ]
  ret <3 x half> %phi
}
