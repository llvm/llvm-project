...
; CHECK-SAME: ptr [[INDIRECT:%.*]]) {
; CHECK-NEXT:    call void [[INDIRECT]]() #[[ATTR14]]
; CHECK-NEXT:    call void @use_most()
; CHECK-NEXT:    ret void
;
...
; Sanitizer tests: amdgpu-agpr-alloc=0 must not be inferred for functions with
; sanitizer attributes. Sanitizer runtime calls (e.g. __asan_malloc_impl) are
; introduced after the Attributor runs and may use AGPRs.
...
; A sanitized function with a user-defined amdgpu-agpr-alloc=0 (mimics an
; annotation from e.g. inline asm constraints). The early return after
; indicateOptimisticFixpoint() means the pre-existing attribute is trusted
; and the sanitizer check is skipped.
...
attributes #2 = { sanitize_address "amdgpu-agpr-alloc"="0" }
...
; CHECK: attributes #[[ATTR1:[0-9]+]] = { convergent nocallback nocreateundeforpoison nofree nosync nounwind willreturn memory(none) }
; CHECK: attributes #[[ATTR2]] = { "amdgpu-agpr-alloc"="56" "amdgpu-no-cluster-id-x" "amdgpu-no-cluster-id-y" "amdgpu-no-cluster-id-z" "amdgpu-no-completion-action" "amdgpu-no-default-queue" "amdgpu-no-dispatch-id" "amdgpu-no-dispatch-ptr" "amdgpu-no-flat-scratch-init" "amdgpu-no-heap-ptr" "amdgpu-no-hostcall-ptr" "amdgpu-no-implicitarg-ptr" "amdgpu-no-lds-kernel-id" "amdgpu-no-multigrid-sync-arg" "amdgpu-no-queue-ptr" "amdgpu-no-workgroup-id-x" "amdgpu-no-workgroup-id-y" "amdgpu-no-workgroup-id-z" "amdgpu-no-workitem-id-x" "amdgpu-no-workitem-id-y" "amdgpu-no-workitem-id-z" "amdgpu-no-wwm" }
; CHECK: attributes #[[ATTR3]] = { "amdgpu-no-wwm" "amdgpu-waves-per-eu"="1,1" }
; CHECK: attributes #[[ATTR4]] = { sanitize_address "amdgpu-no-wwm" }
; CHECK: attributes #[[ATTR5]] = { sanitize_memory "amdgpu-no-wwm" }
; CHECK: attributes #[[ATTR6]] = { sanitize_thread "amdgpu-no-wwm" }
; CHECK: attributes #[[ATTR7]] = { sanitize_hwaddress "amdgpu-no-wwm" }
; CHECK: attributes #[[ATTR8]] = { sanitize_address "amdgpu-agpr-alloc"="0" "amdgpu-no-wwm" }
; CHECK: attributes #[[ATTR9:[0-9]+]] = { nocallback nofree nosync nounwind speculatable willreturn memory(none) }
; CHECK: attributes #[[ATTR10:[0-9]+]] = { nocallback nofree nosync nounwind willreturn memory(argmem: readwrite) }
; CHECK: attributes #[[ATTR11:[0-9]+]] = { nocallback nofree nosync nounwind willreturn memory(read) }
; CHECK: attributes #[[ATTR12:[0-9]+]] = { nounwind }
; CHECK: attributes #[[ATTR13:[0-9]+]] = { nocallback nounwind }
; CHECK: attributes #[[ATTR14]] = { "amdgpu-agpr-alloc"="0" }
;.
; CHECK: [[META0]] = !{!"a55"}
; CHECK: [[META1]] = !{!"v55"}
; CHECK: [[META2]] = !{!"a[55:57]"}
