;RUN: llc < %s -mtriple=amdgcn -mcpu=verde -amdgpu-enable-promote-alloca=0 | FileCheck %s -check-prefix=CHECK
;RUN: llc < %s -mtriple=amdgcn -mcpu=tonga -mattr=-flat-for-global -amdgpu-enable-promote-alloca=0 | FileCheck %s -check-prefix=CHECK
