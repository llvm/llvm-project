; RUN: not llc -global-isel=0 -mtriple=amdgpu9.00 -filetype=null < %s 2>&1 | FileCheck --implicit-check-not=error %s
; RUN: not llc -global-isel=1 -mtriple=amdgpu9.00 -filetype=null < %s 2>&1 | FileCheck --implicit-check-not=error %s
