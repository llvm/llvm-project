; RUN: llc -mtriple=amdgpu9.00-amd-amdhsa -enable-misched=0 -post-RA-scheduler=0 -stress-regalloc=8 < %s | FileCheck %s -check-prefixes=GCN,MUBUF
; RUN: llc -mtriple=amdgpu9.00-amd-amdhsa -enable-misched=0 -post-RA-scheduler=0 -stress-regalloc=8 -mattr=+enable-flat-scratch < %s | FileCheck %s -check-prefixes=GCN,FLATSCR
