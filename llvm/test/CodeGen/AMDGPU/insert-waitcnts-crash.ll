...
; RUN: llc -mtriple=amdgpu10.30-amd-amdhsa -stop-after=si-insert-waitcnts < %s | FileCheck %s
...
define fastcc i32 @foo() #0 {
  ...
  ; CHECK-NEXT:   liveins: $sgpr12, $sgpr13, $sgpr14, $sgpr15, $vgpr31, $sgpr4_64, $sgpr6_64, $sgpr8_64, $sgpr10_64, $sgpr30_64
  ; CHECK-NEXT: {{  $}}
  ...
  ; CHECK-NEXT:   BUFFER_STORE_DWORD_OFFSET $vgpr40, $sgpr0_128, $sgpr33, 0, 0, 0, implicit $exec :: ("amdgpu-thread-private" store (s32) into %stack.1, addrspace 5)
  ...
  ; CHECK-NEXT:   $vgpr40 = V_WRITELANE_B32 killed $sgpr30, 0, $vgpr40, implicit-def $sgpr30_64, implicit $sgpr30_64
  ; CHECK-NEXT:   $vgpr40 = V_WRITELANE_B32 killed $sgpr31, 1, $vgpr40, implicit $sgpr30_64
  ; CHECK-NEXT:   frame-setup CFI_INSTRUCTION llvm_vector_registers $pc_reg, $vgpr40, 0, 32, $vgpr40, 1, 32
  ; CHECK-NEXT:   BUNDLE implicit-def $sgpr16_64, implicit-def $sgpr16, implicit-def $scc, implicit-def $sgpr17 {
  ; CHECK-NEXT:     $sgpr16_64 = S_GETPC_B64
  ...
  ; CHECK-NEXT:   renamable $sgpr16_64 = S_LOAD_DWORDX2_IMM killed renamable $sgpr16_64, 0, 0 :: (dereferenceable invariant load (s64) from got, addrspace 4)
  ; CHECK-NEXT:   S_WAITCNT .Lgkmcnt_0
  ; CHECK-NEXT:   dead $sgpr30_64 = SI_CALL killed renamable $sgpr16_64, @bar, csr_amdgpu, implicit killed $sgpr4_64, implicit killed $sgpr6_64, implicit killed $sgpr8_64, implicit killed $sgpr10_64, implicit killed $sgpr12, implicit killed $sgpr13, implicit killed $sgpr14, implicit killed $sgpr15, implicit killed $vgpr31, implicit $sgpr0_128
  ...
  ; CHECK-NEXT:   $sgpr30 = V_READLANE_B32 $vgpr40, 0, implicit-def $sgpr30_64
  ...
  ; CHECK-NEXT:   $vgpr40 = BUFFER_LOAD_DWORD_OFFSET $sgpr0_128, $sgpr33, 0, 0, 0, implicit $exec :: ("amdgpu-thread-private" load (s32) from %stack.1, addrspace 5)
  ; CHECK-NEXT:   $exec_lo = S_MOV_B32 killed $sgpr5
  ; CHECK-NEXT:   $sgpr33 = S_MOV_B32 killed $sgpr4
  ; CHECK-NEXT:   S_WAITCNT .Vmcnt_0
  ; CHECK-NEXT:   S_SETPC_B64_return undef $sgpr30_64, implicit undef $vgpr0
  fence acquire
