...
; RUN: opt -mtriple=amdgpu-unknown-amdhsa -S -passes=amdgpu-attributor < %s | FileCheck -check-prefixes=ATTRIBUTOR_HSA %s
...
declare void @external.func() #1

; This function gets deleted.
define internal void @defined.func() #1 {
...
define void @func_call_external() #1 {
...
define void @func_call_defined() #1 {
...
define amdgpu_kernel void @kern_call_external() #1 {
...
define amdgpu_kernel void @func_kern_defined() #1 {
...
define float @func_indirect_call(ptr %fptr) #1 {
; ATTRIBUTOR_HSA-LABEL: define {{[^@]+}}@func_indirect_call
; ATTRIBUTOR_HSA-SAME: (ptr [[FPTR:%.*]]) #[[ATTR13]] {
...
declare float @extern() #1
define float @func_extern_call() #1 {
...
define float @func_null_call(ptr %fptr) #1 {
...
define float @func_other_intrinsic_call(float %arg) #1 {
; ATTRIBUTOR_HSA-LABEL: define {{[^@]+}}@func_other_intrinsic_call
; ATTRIBUTOR_HSA-SAME: (float [[ARG:%.*]]) #[[ATTR11]] {
...
declare void @extern_func_sanitize_address() #3

define amdgpu_kernel void @kern_decl_sanitize_address() #1 {
...
declare void @enqueue_block_decl() #4
...
attributes #2 = { nounwind sanitize_address }
attributes #3 = { nounwind sanitize_address "amdgpu-no-implicitarg-ptr" }
attributes #4 = { "enqueued-block" }
