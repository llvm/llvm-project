; RUN: llc -mtriple=amdgcn -amdgpu-enable-promote-alloca=0 < %s | FileCheck -check-prefix=SI -check-prefix=FUNC %s
; RUN: llc -mtriple=amdgcn -mcpu=tonga -amdgpu-enable-promote-alloca=0 < %s | FileCheck -check-prefix=SI -check-prefix=FUNC %s
