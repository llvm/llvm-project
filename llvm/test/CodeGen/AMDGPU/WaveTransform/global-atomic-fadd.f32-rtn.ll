...
; RUN: llc -mtriple=amdgcn -mcpu=gfx90a -verify-machineinstrs -stop-after=finalize-isel -amdgpu-wave-transform-cf=1 -amdgpu-atomic-optimizer-strategy=DPP < %s | FileCheck -check-prefixes=GFX90A %s
; RUN: llc -mtriple=amdgcn -mcpu=gfx1200 -verify-machineinstrs -stop-after=finalize-isel -amdgpu-wave-transform-cf=1 -amdgpu-atomic-optimizer-strategy=DPP < %s | FileCheck -check-prefix=GFX1200 %s
; RUN: llc -mtriple=amdgcn -mcpu=gfx1200 -verify-machineinstrs -stop-after=finalize-isel -amdgpu-wave-transform-cf=1 -amdgpu-atomic-optimizer-strategy=Iterative < %s | FileCheck -check-prefix=ITERATE %s
...
define amdgpu_ps float @global_atomic_fadd_f32_rtn_atomicrmw(ptr addrspace(1) %ptr, float %data) {
  ...
  ; GFX1200-NEXT:   [[DEF1:%[0-9]+]]:sgpr_32 = IMPLICIT_DEF
  ...
  ; ITERATE-NEXT:   [[DEF:%[0-9]+]]:sgpr_32 = IMPLICIT_DEF
  ; ITERATE-NEXT:   [[DEF1:%[0-9]+]]:sgpr_32 = IMPLICIT_DEF
...
define amdgpu_ps float @global_atomic_fadd_f32_saddr_rtn_atomicrmw(ptr addrspace(1) inreg %ptr, float %data) {
  ...
  ; GFX90A-NEXT:   SI_BRCOND killed [[V_CMP_NE_U32_e64_]], %bb.4
  ...
  ; GFX90A-NEXT:   SI_BRCOND killed [[V_CMP_NE_U32_e64_1]], %bb.3
  ...
  ; GFX90A-NEXT:   [[COPY8:%[0-9]+]]:vgpr_32 = COPY %1
  ...
  ; GFX90A-NEXT: {{  $}}
  ; GFX90A-NEXT:   [[PHI:%[0-9]+]]:vgpr_32 = PHI [[DEF2]], %bb.1, [[GLOBAL_ATOMIC_ADD_F32_SADDR_RTN]], %bb.2
  ; GFX90A-NEXT:   [[V_READFIRSTLANE_B32_:%[0-9]+]]:sreg_32_xm0 = V_READFIRSTLANE_B32 [[PHI]], implicit $exec
  ; GFX90A-NEXT:   early-clobber %48:vgpr_32 = STRICT_WWM [[V_MOV_B32_dpp6]], implicit $exec
  ...
  ; GFX90A-NEXT:   [[COPY9:%[0-9]+]]:vgpr_32 = COPY [[V_READFIRSTLANE_B32_]]
  ; GFX90A-NEXT:   [[V_CNDMASK_B32_e64_2:%[0-9]+]]:vgpr_32 = V_CNDMASK_B32_e64 0, killed [[V_ADD_F32_e64_6]], 0, [[COPY9]], [[V_CMP_NE_U32_e64_2]], implicit $exec
  ; GFX90A-NEXT: {{  $}}
  ; GFX90A-NEXT: bb.4 (%ir-block.34):
  ; GFX90A-NEXT:   [[PHI1:%[0-9]+]]:vgpr_32 = PHI [[DEF]], %bb.0, [[V_CNDMASK_B32_e64_2]], %bb.3
  ...
  ; GFX1200-NEXT:   SI_BRCOND killed [[V_CMP_NE_U32_e64_]], %bb.4
  ...
  ; ITERATE-NEXT:   SI_BRCOND killed [[V_CMP_NE_U32_e64_]], %bb.4
  ...
  ; ITERATE-NEXT:   SI_BRCOND_UNIFORM killed [[COPY7]], %bb.5
