; RUN: llc -mtriple=amdgpu11.00--amdpal -amdgpu-use-amdgpu-trackers=0 -verify-misched < %s | FileCheck --check-prefixes=GFX11-PAL %s
...
; RUN: llc -mtriple=amdgpu8.02 -amdgpu-scalarize-global-loads=false -amdgpu-use-amdgpu-trackers=0 -verify-misched < %s | FileCheck --check-prefixes=TONGA %s
...
; RUN: llc -mtriple=amdgpu9.08 -amdgpu-use-amdgpu-trackers=0 -verify-misched < %s | FileCheck --check-prefixes=GFX908 %s
; RUN: llc -mtriple=amdgpu9.08 -amdgpu-use-amdgpu-trackers=1 -verify-misched < %s | FileCheck --check-prefixes=GFX908-GCNTRACKERS %s
; RUN: llc -mtriple=amdgpu6.00 -amdgpu-use-amdgpu-trackers=0 -verify-misched < %s | FileCheck --check-prefixes=GENERIC %s
