; RUN: llc -mtriple=amdgpu7.00-amd-amdhsa -enable-ipra=1 < %s | FileCheck -check-prefix=GCN %s
; RUN: llc -mtriple=amdgpu7.00-amd-amdhsa -enable-ipra=0 < %s | FileCheck -check-prefix=GCN %s
