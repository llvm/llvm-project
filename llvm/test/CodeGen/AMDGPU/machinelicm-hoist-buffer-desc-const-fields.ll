...
; The buffer descriptor mixes a loop-carried pointer with two constant fields, so
; the REG_SEQUENCE building it is not loop-invariant and pre-RA LICM cannot hoist
; it. Two-Address later splits it into per-subreg copies, of which the two
; constant ones are invariant, but that happens after pre-RA LICM has run, so
