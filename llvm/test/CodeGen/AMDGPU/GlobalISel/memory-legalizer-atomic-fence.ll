...
; RUN: llc -global-isel -stop-after=si-memory-legalizer -mtriple=amdgpu6.00-amd- < %s | FileCheck -check-prefix=GFX6 %s
; RUN: llc -global-isel -stop-after=si-memory-legalizer -mtriple=amdgpu8.03-amd- < %s | FileCheck -check-prefix=GFX8 %s
; RUN: llc -global-isel -stop-after=si-memory-legalizer -mtriple=amdgpu8.03-amd-amdhsa < %s | FileCheck -check-prefix=GFX8 %s
; RUN: llc -global-isel -stop-after=si-memory-legalizer -mtriple=amdgpu10.10-amd-amdhsa < %s | FileCheck -check-prefix=GFX10WGP %s
; RUN: llc -global-isel -stop-after=si-memory-legalizer -mtriple=amdgpu10.10-amd-amdhsa -mattr=+cumode < %s | FileCheck -check-prefix=GFX10CU %s
; RUN: llc -global-isel -stop-after=si-memory-legalizer -mtriple=amdgpu11.00-amd-amdhsa < %s | FileCheck -check-prefix=GFX11WGP %s
; RUN: llc -global-isel -stop-after=si-memory-legalizer -mtriple=amdgpu11.00-amd-amdhsa -mattr=+cumode < %s | FileCheck -check-prefix=GFX11CU %s
...
define amdgpu_kernel void @system_one_as_release() #0 {
  ...
  ; GFX10WGP: bb.0.entry:
  ; GFX10WGP-NEXT:   S_WAITCNT_soft .Vmcnt_0
  ; GFX10WGP-NEXT:   S_WAITCNT_lds_before_direct
  ; GFX10WGP-NEXT:   S_WAITCNT_VSCNT_soft undef $sgpr_null, 0
  ; GFX10WGP-NEXT:   S_ENDPGM 0
  ;
  ...
  ; GFX10CU: bb.0.entry:
  ; GFX10CU-NEXT:   S_WAITCNT_soft .Vmcnt_0
  ; GFX10CU-NEXT:   S_WAITCNT_lds_before_direct
  ; GFX10CU-NEXT:   S_WAITCNT_VSCNT_soft undef $sgpr_null, 0
  ; GFX10CU-NEXT:   S_ENDPGM 0
  ;
