...
; RUN: llc -global-isel -global-isel-abort=2 -mtriple=amdgcn-mesa-mesa3d -mcpu=tahiti -stop-after=instruction-select -o - %s | FileCheck --check-prefixes=CHECK %s
; RUN: llc -global-isel -global-isel-abort=2 -mtriple=amdgcn-mesa-mesa3d -mcpu=gfx1010 -mattr=+wavefrontsize64 -stop-after=instruction-select -o - %s | FileCheck --check-prefixes=CHECK %s
; RUN: llc -global-isel -global-isel-abort=2 -mtriple=amdgcn-mesa-mesa3d -mcpu=gfx1100 -mattr=+wavefrontsize64 -stop-after=instruction-select -o - %s | FileCheck --check-prefixes=CHECK %s
...
define amdgpu_ps <4 x float> @struct_tbuffer_load_v4f32__vgpr_rsrc__sgpr_vindex__sgpr_voffset__vgpr_soffset(ptr addrspace(8) %rsrc, i32 inreg %vindex, i32 inreg %voffset, i32 %soffset) {
  ...
  ; CHECK-NEXT:   [[TBUFFER_LOAD_FORMAT_XYZW_BOTHEN:%[0-9]+]]:vreg_128(<4 x f32>) = TBUFFER_LOAD_FORMAT_XYZW_BOTHEN [[REG_SEQUENCE1]](i64), [[REG_SEQUENCE]](<4 x s32>), [[V_READFIRSTLANE_B32_]](i32), 0, 78, 0, 0, implicit $exec :: (dereferenceable load (<4 x f32>) from %ir.rsrc, align 1, addrspace 8)
  ...
  ; CHECK-NEXT: {{  $}}
  ; CHECK-NEXT: bb.5:
  ; CHECK-NEXT:   [[COPY14:%[0-9]+]]:vgpr_32(f32) = COPY [[TBUFFER_LOAD_FORMAT_XYZW_BOTHEN]].sub0(<4 x f32>)
  ; CHECK-NEXT:   [[COPY15:%[0-9]+]]:vgpr_32(f32) = COPY [[TBUFFER_LOAD_FORMAT_XYZW_BOTHEN]].sub1(<4 x f32>)
  ; CHECK-NEXT:   [[COPY16:%[0-9]+]]:vgpr_32(f32) = COPY [[TBUFFER_LOAD_FORMAT_XYZW_BOTHEN]].sub2(<4 x f32>)
  ; CHECK-NEXT:   [[COPY17:%[0-9]+]]:vgpr_32(f32) = COPY [[TBUFFER_LOAD_FORMAT_XYZW_BOTHEN]].sub3(<4 x f32>)
