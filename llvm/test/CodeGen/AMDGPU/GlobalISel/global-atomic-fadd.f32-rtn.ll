...
; RUN: llc -global-isel -mtriple=amdgcn -mcpu=gfx90a -stop-after=instruction-select -amdgpu-atomic-optimizer-strategy=DPP < %s | FileCheck -check-prefixes=GFX90A_GFX940,GFX90A %s
; RUN: llc -global-isel -mtriple=amdgcn -mcpu=gfx940 -stop-after=instruction-select -amdgpu-atomic-optimizer-strategy=DPP < %s | FileCheck -check-prefixes=GFX90A_GFX940,GFX940 %s
...
define amdgpu_ps float @global_atomic_fadd_f32_saddr_rtn_atomicrmw(ptr addrspace(1) inreg %ptr, float %data) {
  ...
  ; GFX90A-NEXT: {{  $}}
  ; GFX90A-NEXT:   [[PHI:%[0-9]+]]:vgpr_32 = PHI %47, %bb.6, [[DEF]], %bb.2
  ...
  ; GFX90A-NEXT: {{  $}}
  ; GFX90A-NEXT:   [[PHI1:%[0-9]+]]:sreg_64_xexec = PHI [[S_MOV_B]], %bb.3, %45, %bb.5
  ; GFX90A-NEXT:   [[PHI2:%[0-9]+]]:vgpr_32 = PHI %68, %bb.5, [[S_LOAD_DWORD_IMM]], %bb.3
  ; GFX90A-NEXT:   [[V_ADD_F32_e64_6:%[0-9]+]]:vgpr_32 = nofpexcept V_ADD_F32_e64 0, [[PHI2]], 0, [[STRICT_WWM]], 0, 0, implicit $mode, implicit $exec
  ; GFX90A-NEXT:   [[REG_SEQUENCE2:%[0-9]+]]:vreg_64_align2 = REG_SEQUENCE [[V_ADD_F32_e64_6]], %subreg.sub0, [[PHI2]], %subreg.sub1
  ...
  ; GFX90A-NEXT:   [[V_CMP_EQ_U32_e64_1:%[0-9]+]]:sreg_64_xexec = V_CMP_EQ_U32_e64 [[GLOBAL_ATOMIC_CMPSWAP_SADDR_RTN]], [[PHI2]], implicit $exec
  ; GFX90A-NEXT:   [[SI_IF_BREAK:%[0-9]+]]:sreg_64_xexec = SI_IF_BREAK [[V_CMP_EQ_U32_e64_1]], [[PHI1]], implicit-def $scc
  ...
  ; GFX90A-NEXT: {{  $}}
  ; GFX90A-NEXT:   [[PHI3:%[0-9]+]]:vgpr_32 = PHI [[GLOBAL_ATOMIC_CMPSWAP_SADDR_RTN]], %bb.5
  ; GFX90A-NEXT:   [[PHI4:%[0-9]+]]:sreg_64_xexec = PHI [[SI_IF_BREAK]], %bb.5
  ; GFX90A-NEXT:   SI_END_CF [[PHI4]], implicit-def $exec, implicit-def $scc, implicit $exec
  ; GFX90A-NEXT:   S_BRANCH %bb.4
  ; GFX90A-NEXT: {{  $}}
  ; GFX90A-NEXT: bb.7.Flow2:
  ; GFX90A-NEXT:   successors: %bb.9(0x80000000)
  ; GFX90A-NEXT: {{  $}}
  ; GFX90A-NEXT:   [[PHI5:%[0-9]+]]:vgpr_32 = PHI %52, %bb.8, [[DEF]], %bb.1
  ; GFX90A-NEXT:   SI_END_CF [[SI_IF]], implicit-def $exec, implicit-def $scc, implicit $exec
  ; GFX90A-NEXT:   S_BRANCH %bb.9
  ; GFX90A-NEXT: {{  $}}
  ; GFX90A-NEXT: bb.8 (%ir-block.46):
  ; GFX90A-NEXT:   successors: %bb.7(0x80000000)
  ; GFX90A-NEXT: {{  $}}
  ; GFX90A-NEXT:   [[V_READFIRSTLANE_B32_:%[0-9]+]]:sreg_32 = V_READFIRSTLANE_B32 [[PHI]], implicit $exec
  ...
  ; GFX90A-NEXT:   [[V_ADD_F32_e64_7:%[0-9]+]]:vgpr_32 = nofpexcept V_ADD_F32_e64 0, [[COPY21]], 0, [[STRICT_WWM1]], 0, 0, implicit $mode, implicit $exec
  ; GFX90A-NEXT:   S_BRANCH %bb.7
  ; GFX90A-NEXT: {{  $}}
  ; GFX90A-NEXT: bb.9 (%ir-block.52):
  ; GFX90A-NEXT:   $vgpr0 = COPY [[PHI5]]
  ; GFX90A-NEXT:   SI_RETURN_TO_EPILOG implicit $vgpr0
  ;
