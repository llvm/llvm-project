; RUN: llc -mtriple=amdgpu6.00 -global-isel -stop-after=instruction-select -o - %s | FileCheck -check-prefixes=GCN,GFX8 %s
; RUN: llc -mtriple=amdgpu8.02 -global-isel -stop-after=instruction-select -o - %s | FileCheck -check-prefixes=GCN,GFX9 %s

; RUN: not llc -mtriple=amdgpu6.00 -global-isel < %s 2>&1 | FileCheck %s
; RUN: not llc -mtriple=amdgpu8.02 -global-isel < %s 2>&1 | FileCheck %s
...
define amdgpu_kernel void @load_zeroinit_lds_global(ptr addrspace(1) %out, i1 %p) {
  ...
  ; GCN: bb.1 (%ir-block.0):
  ; GCN:   liveins: $sgpr4_64
  ; GCN:   [[COPY:%[0-9]+]]:sreg_64 = COPY $sgpr4_64
