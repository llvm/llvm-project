...
; RUN: opt < %s -passes=amdgpu-lower-intrinsics -S -mtriple=amdgpu12.00-amd-amdhsa -codegen-opt-level=0 | FileCheck --check-prefixes=CHECK,NOOPT %s
; RUN: opt < %s -passes=amdgpu-lower-intrinsics -S -mtriple=amdgpu12.00-amd-amdhsa -codegen-opt-level=1 -mattr=+wavefrontsize32 | FileCheck --check-prefixes=CHECK,OPT-WAVE32 %s
; RUN: opt < %s -passes=amdgpu-lower-intrinsics -S -mtriple=amdgpu12.00-amd-amdhsa -codegen-opt-level=1 -mattr=+wavefrontsize64 | FileCheck --check-prefixes=CHECK,OPT-WAVE64 %s

declare void @foo(i1)

define amdgpu_kernel void @barrier() {
; NOOPT-LABEL: define amdgpu_kernel void @barrier() {
...
; NOOPT-NEXT:    ret void
;
; OPT-WAVE32-LABEL: define amdgpu_kernel void @barrier(
...
;
; OPT-WAVE64-LABEL: define amdgpu_kernel void @barrier(
; OPT-WAVE64-SAME: ) #[[ATTR0:[0-9]+]] {
; OPT-WAVE64-NEXT:    call void @llvm.amdgcn.s.barrier.signal(i32 -1)
; OPT-WAVE64-NEXT:    call void @llvm.amdgcn.s.barrier.wait(i16 -1)
; OPT-WAVE64-NEXT:    [[ISFIRST:%.*]] = call i1 @llvm.amdgcn.s.barrier.signal.isfirst(i32 -1)
