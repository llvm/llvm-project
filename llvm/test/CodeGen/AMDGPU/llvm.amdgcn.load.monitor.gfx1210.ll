; NOTE: Assertions have been autogenerated by utils/update_llc_test_checks.py
; RUN: llc -global-isel=0 -march=amdgcn -mcpu=gfx1210 -verify-machineinstrs < %s | FileCheck -check-prefix=GFX1210 %s
; RUN: llc -global-isel=1 -march=amdgcn -mcpu=gfx1210 -verify-machineinstrs < %s | FileCheck -check-prefix=GFX1210 %s

declare i32 @llvm.amdgcn.global.load.monitor.b32.i32.p1(ptr addrspace(1))
declare <2 x i32> @llvm.amdgcn.global.load.monitor.b64.v2i32.p1(ptr addrspace(1))
declare <4 x i32> @llvm.amdgcn.global.load.monitor.b128.v4i32.p1(ptr addrspace(1))
declare i32 @llvm.amdgcn.flat.load.monitor.b32.i32.p0(ptr)
declare <2 x i32> @llvm.amdgcn.flat.load.monitor.b64.v2i32.p0(ptr)
declare <4 x i32> @llvm.amdgcn.flat.load.monitor.b128.v4i32.p0(ptr)

define amdgpu_ps void @global_load_monitor_b32_vaddr(ptr addrspace(1) %addr, ptr addrspace(1) %use) {
; GFX1210-LABEL: global_load_monitor_b32_vaddr:
; GFX1210:       ; %bb.0: ; %entry
; GFX1210-NEXT:    global_load_monitor_b32 v0, v[0:1], off offset:32
; GFX1210-NEXT:    s_wait_loadcnt 0x0
; GFX1210-NEXT:    global_store_b32 v[2:3], v0, off
; GFX1210-NEXT:    s_sendmsg sendmsg(MSG_DEALLOC_VGPRS)
; GFX1210-NEXT:    s_endpgm
entry:
  %gep = getelementptr i64, ptr addrspace(1) %addr, i32 4
  %val = call i32 @llvm.amdgcn.global.load.monitor.b32.i32.p1(ptr addrspace(1) %gep)
  store i32 %val, ptr addrspace(1) %use
  ret void
}

define amdgpu_ps void @global_load_monitor_b32_saddr(ptr addrspace(1) inreg %addr, ptr addrspace(1) %use) {
; GFX1210-LABEL: global_load_monitor_b32_saddr:
; GFX1210:       ; %bb.0: ; %entry
; GFX1210-NEXT:    v_mov_b32_e32 v2, 0
; GFX1210-NEXT:    global_load_monitor_b32 v2, v2, s[0:1] offset:32
; GFX1210-NEXT:    s_wait_loadcnt 0x0
; GFX1210-NEXT:    global_store_b32 v[0:1], v2, off
; GFX1210-NEXT:    s_sendmsg sendmsg(MSG_DEALLOC_VGPRS)
; GFX1210-NEXT:    s_endpgm
entry:
  %gep = getelementptr i64, ptr addrspace(1) %addr, i32 4
  %val = call i32 @llvm.amdgcn.global.load.monitor.b32.i32.p1(ptr addrspace(1) %gep)
  store i32 %val, ptr addrspace(1) %use
  ret void
}

define amdgpu_ps void @global_load_monitor_b64_vaddr(ptr addrspace(1) %addr, ptr addrspace(1) %use) {
; GFX1210-LABEL: global_load_monitor_b64_vaddr:
; GFX1210:       ; %bb.0: ; %entry
; GFX1210-NEXT:    global_load_monitor_b64 v[0:1], v[0:1], off offset:32
; GFX1210-NEXT:    s_wait_loadcnt 0x0
; GFX1210-NEXT:    global_store_b64 v[2:3], v[0:1], off
; GFX1210-NEXT:    s_sendmsg sendmsg(MSG_DEALLOC_VGPRS)
; GFX1210-NEXT:    s_endpgm
entry:
  %gep = getelementptr i64, ptr addrspace(1) %addr, i32 4
  %val = call <2 x i32> @llvm.amdgcn.global.load.monitor.b64.v2i32.p1(ptr addrspace(1) %gep)
  store <2 x i32> %val, ptr addrspace(1) %use
  ret void
}

define amdgpu_ps void @global_load_monitor_b64_saddr(ptr addrspace(1) inreg %addr, ptr addrspace(1) %use) {
; GFX1210-LABEL: global_load_monitor_b64_saddr:
; GFX1210:       ; %bb.0: ; %entry
; GFX1210-NEXT:    v_mov_b32_e32 v2, 0
; GFX1210-NEXT:    global_load_monitor_b64 v[2:3], v2, s[0:1] offset:32
; GFX1210-NEXT:    s_wait_loadcnt 0x0
; GFX1210-NEXT:    global_store_b64 v[0:1], v[2:3], off
; GFX1210-NEXT:    s_sendmsg sendmsg(MSG_DEALLOC_VGPRS)
; GFX1210-NEXT:    s_endpgm
entry:
  %gep = getelementptr i64, ptr addrspace(1) %addr, i32 4
  %val = call <2 x i32> @llvm.amdgcn.global.load.monitor.b64.v2i32.p1(ptr addrspace(1) %gep)
  store <2 x i32> %val, ptr addrspace(1) %use
  ret void
}

define amdgpu_ps void @global_load_monitor_b128_vaddr(ptr addrspace(1) %addr, ptr addrspace(1) %use) {
; GFX1210-LABEL: global_load_monitor_b128_vaddr:
; GFX1210:       ; %bb.0: ; %entry
; GFX1210-NEXT:    global_load_monitor_b128 v[4:7], v[0:1], off offset:32
; GFX1210-NEXT:    s_wait_loadcnt 0x0
; GFX1210-NEXT:    global_store_b128 v[2:3], v[4:7], off
; GFX1210-NEXT:    s_sendmsg sendmsg(MSG_DEALLOC_VGPRS)
; GFX1210-NEXT:    s_endpgm
entry:
  %gep = getelementptr i64, ptr addrspace(1) %addr, i32 4
  %val = call <4 x i32> @llvm.amdgcn.global.load.monitor.b128.v4i32.p1(ptr addrspace(1) %gep)
  store <4 x i32> %val, ptr addrspace(1) %use
  ret void
}

define amdgpu_ps void @global_load_monitor_b128_saddr(ptr addrspace(1) inreg %addr, ptr addrspace(1) %use) {
; GFX1210-LABEL: global_load_monitor_b128_saddr:
; GFX1210:       ; %bb.0: ; %entry
; GFX1210-NEXT:    v_mov_b32_e32 v2, 0
; GFX1210-NEXT:    global_load_monitor_b128 v[2:5], v2, s[0:1] offset:32
; GFX1210-NEXT:    s_wait_loadcnt 0x0
; GFX1210-NEXT:    global_store_b128 v[0:1], v[2:5], off
; GFX1210-NEXT:    s_sendmsg sendmsg(MSG_DEALLOC_VGPRS)
; GFX1210-NEXT:    s_endpgm
entry:
  %gep = getelementptr i64, ptr addrspace(1) %addr, i32 4
  %val = call <4 x i32> @llvm.amdgcn.global.load.monitor.b128.v4i32.p1(ptr addrspace(1) %gep)
  store <4 x i32> %val, ptr addrspace(1) %use
  ret void
}

define amdgpu_ps void @flat_load_monitor_b32(ptr %addr, ptr addrspace(1) %use) {
; GFX1210-LABEL: flat_load_monitor_b32:
; GFX1210:       ; %bb.0: ; %entry
; GFX1210-NEXT:    flat_load_monitor_b32 v0, v[0:1] offset:32
; GFX1210-NEXT:    s_wait_loadcnt_dscnt 0x0
; GFX1210-NEXT:    global_store_b32 v[2:3], v0, off
; GFX1210-NEXT:    s_sendmsg sendmsg(MSG_DEALLOC_VGPRS)
; GFX1210-NEXT:    s_endpgm
entry:
  %gep = getelementptr i64, ptr addrspace(0) %addr, i32 4
  %val = call i32 @llvm.amdgcn.flat.load.monitor.b32.i32.p0(ptr addrspace(0) %gep)
  store i32 %val, ptr addrspace(1) %use
  ret void
}

define amdgpu_ps void @flat_load_monitor_b64(ptr %addr, ptr addrspace(1) %use) {
; GFX1210-LABEL: flat_load_monitor_b64:
; GFX1210:       ; %bb.0: ; %entry
; GFX1210-NEXT:    flat_load_monitor_b64 v[0:1], v[0:1] offset:32
; GFX1210-NEXT:    s_wait_loadcnt_dscnt 0x0
; GFX1210-NEXT:    global_store_b64 v[2:3], v[0:1], off
; GFX1210-NEXT:    s_sendmsg sendmsg(MSG_DEALLOC_VGPRS)
; GFX1210-NEXT:    s_endpgm
entry:
  %gep = getelementptr i64, ptr addrspace(0) %addr, i32 4
  %val = call <2 x i32> @llvm.amdgcn.flat.load.monitor.b64.v2i32.p0(ptr addrspace(0) %gep)
  store <2 x i32> %val, ptr addrspace(1) %use
  ret void
}

define amdgpu_ps void @flat_load_monitor_b128(ptr %addr, ptr addrspace(1) %use) {
; GFX1210-LABEL: flat_load_monitor_b128:
; GFX1210:       ; %bb.0: ; %entry
; GFX1210-NEXT:    flat_load_monitor_b128 v[4:7], v[0:1] offset:32
; GFX1210-NEXT:    s_wait_loadcnt_dscnt 0x0
; GFX1210-NEXT:    global_store_b128 v[2:3], v[4:7], off
; GFX1210-NEXT:    s_sendmsg sendmsg(MSG_DEALLOC_VGPRS)
; GFX1210-NEXT:    s_endpgm
entry:
  %gep = getelementptr i64, ptr addrspace(0) %addr, i32 4
  %val = call <4 x i32> @llvm.amdgcn.flat.load.monitor.b128.v4i32.p0(ptr addrspace(0) %gep)
  store <4 x i32> %val, ptr addrspace(1) %use
  ret void
}
