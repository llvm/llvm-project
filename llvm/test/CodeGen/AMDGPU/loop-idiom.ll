...
; RUN: opt -passes=loop-idiom -S < %s -mtriple=amdgpu6.00 | FileCheck --check-prefix=SI --check-prefix=FUNC %s
; RUN: opt -passes=loop-idiom -S < %s -mtriple=amdgpu8.02 | FileCheck --check-prefix=SI --check-prefix=FUNC %s
