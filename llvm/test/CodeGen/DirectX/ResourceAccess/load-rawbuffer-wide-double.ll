...
define <8 x double> @loadv8f64_byte(i32 %index) {
  ...
  ; CHECK: [[LOAD0:%.*]] = call { <4 x i32>, i1 } [[LOAD_INTRINSIC:@llvm\.dx\.resource\.load\.rawbuffer[^(]*]]([[BUFFER_TY:target\("dx\.RawBuffer", i8, 0, 0\)]] %buffer, i32 %index, i32 poison)
  ; CHECK: %[[INDEX16:.*]] = add i32 %index, 16
  ; CHECK: [[LOAD16:%.*]] = call { <4 x i32>, i1 } [[LOAD_INTRINSIC]]([[BUFFER_TY]] %buffer, i32 %[[INDEX16]], i32 poison)
  ; CHECK: %[[INDEX32:.*]] = add i32 %index, 32
  ; CHECK: [[LOAD32:%.*]] = call { <4 x i32>, i1 } [[LOAD_INTRINSIC]]([[BUFFER_TY]] %buffer, i32 %[[INDEX32]], i32 poison)
  ; CHECK: %[[INDEX48:.*]] = add i32 %[[INDEX32]], 16
  ; CHECK: [[LOAD48:%.*]] = call { <4 x i32>, i1 } [[LOAD_INTRINSIC]]([[BUFFER_TY]] %buffer, i32 %[[INDEX48]], i32 poison)
