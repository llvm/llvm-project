...
; RUN: llc < %s -wasm-disable-explicit-locals -wasm-keep-registers -fast-isel -fast-isel-abort=1 | FileCheck %s --check-prefixes=CHECK,DEFAULT
; RUN: llc < %s -mcpu=mvp -wasm-disable-explicit-locals -wasm-keep-registers -fast-isel -fast-isel-abort=1 | FileCheck %s --check-prefixes=CHECK,MVP
...
; MVP:         .functype load_i8_s_with_folded_offset (i32) -> (i32)
; MVP-NEXT:  # %bb.0:
; MVP-NEXT:    i32.load8_u $push3=, 24($0)
; MVP-NEXT:    i32.const $push0=, 24
; MVP-NEXT:    i32.shl $push1=, $pop3, $pop0
; MVP-NEXT:    i32.const $push4=, 24
; MVP-NEXT:    i32.shr_s $push2=, $pop1, $pop4
; MVP-NEXT:    # fallthrough-return
  %q = ptrtoint ptr %p to i32
  %r = add nuw i32 %q, 24
  %s = inttoptr i32 %r to ptr
  %t = load i8, ptr %s
