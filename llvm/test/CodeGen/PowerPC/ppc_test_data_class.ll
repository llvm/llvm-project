; RUN: llc   -mtriple=powerpc-ibm-aix-xcoff -mcpu=pwr9 -ppc-asm-full-reg-names < %s | FileCheck --check-prefixes=PWR9,RLW %s
; RUN: llc   -mtriple=powerpc-ibm-aixi-xcoff -mcpu=pwr10 -ppc-asm-full-reg-names < %s | FileCheck --check-prefix=PWR10 %s
; RUN: llc   -mtriple=powerpc64-ibm-aix -mcpu=pwr9 -ppc-asm-full-reg-names < %s | FileCheck --check-prefixes=PWR9,SRW %s
; RUN: llc   -mtriple=powerpc64-ibm-aix -mcpu=pwr10 -ppc-asm-full-reg-names < %s | FileCheck --check-prefix=PWR10 %s
; RUN: llc   -mtriple=powerpc64le-unknown-linux-gnu -mcpu=pwr9 -ppc-asm-full-reg-names < %s | FileCheck --check-prefixes=PWR9,SRW %s
...
; RLW-NEXT:     rlwinm r3, r3, 27, 31, 31
; SRW-NEXT:     srwi r3, r3, 5
; PWR9-NEXT:    blr
;
