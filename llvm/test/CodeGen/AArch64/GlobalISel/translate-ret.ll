...
define <8 x float> @func_v8f32(ptr %ptr) {
  ...
  ; CHECK-NEXT: {{  $}}
  ; CHECK-NEXT:   [[COPY:%[0-9]+]]:_(p0) = COPY $x0
  ; CHECK-NEXT:   [[LOAD:%[0-9]+]]:_(<8 x f32>) = G_LOAD [[COPY]](p0) :: (load (<8 x f32>) from %ir.ptr)
  ; CHECK-NEXT:   [[UV:%[0-9]+]]:_(<4 x f32>), [[UV1:%[0-9]+]]:_(<4 x f32>) = G_UNMERGE_VALUES [[LOAD]](<8 x f32>)
  ; CHECK-NEXT:   $q0 = COPY [[UV]](<4 x f32>)
  ; CHECK-NEXT:   $q1 = COPY [[UV1]](<4 x f32>)
...
define <6 x float> @func_v6f32(ptr %ptr) {
  ...
  ; CHECK-NEXT: {{  $}}
  ; CHECK-NEXT:   [[COPY:%[0-9]+]]:_(p0) = COPY $x0
  ; CHECK-NEXT:   [[LOAD:%[0-9]+]]:_(<6 x f32>) = G_LOAD [[COPY]](p0) :: (load (<6 x f32>) from %ir.ptr, align 32)
  ; CHECK-NEXT:   [[UV:%[0-9]+]]:_(f32), [[UV1:%[0-9]+]]:_(f32), [[UV2:%[0-9]+]]:_(f32), [[UV3:%[0-9]+]]:_(f32), [[UV4:%[0-9]+]]:_(f32), [[UV5:%[0-9]+]]:_(f32) = G_UNMERGE_VALUES [[LOAD]](<6 x f32>)
  ; CHECK-NEXT:   $s0 = COPY [[UV]](f32)
  ; CHECK-NEXT:   $s1 = COPY [[UV1]](f32)
  ; CHECK-NEXT:   $s2 = COPY [[UV2]](f32)
  ; CHECK-NEXT:   $s3 = COPY [[UV3]](f32)
  ; CHECK-NEXT:   $s4 = COPY [[UV4]](f32)
  ; CHECK-NEXT:   $s5 = COPY [[UV5]](f32)
...
define i128 @ABIi128(i128 %arg1) {
  ...
  ; CHECK-NEXT:   [[BITCAST:%[0-9]+]]:_(f128) = G_BITCAST [[MV]](i128)
  ; CHECK-NEXT:   [[FPTOUI:%[0-9]+]]:_(i128) = G_FPTOUI [[BITCAST]](f128)
  ; CHECK-NEXT:   [[UV:%[0-9]+]]:_(i64), [[UV1:%[0-9]+]]:_(i64) = G_UNMERGE_VALUES [[FPTOUI]](i128)
  ; CHECK-NEXT:   $x0 = COPY [[UV]](i64)
  ; CHECK-NEXT:   $x1 = COPY [[UV1]](i64)
  ; CHECK-NEXT:   RET_ReallyLR implicit $x0, implicit $x1
