...
; RUN: llc < %s -mtriple arm64e-apple-darwin     -global-isel -verify-machineinstrs -global-isel-abort=1                   | FileCheck %s --check-prefixes=CHECK,CM-DFL
; RUN: llc < %s -mtriple arm64e-apple-darwin     -global-isel -verify-machineinstrs -global-isel-abort=1 -code-model=large | FileCheck %s --check-prefixes=CHECK,CM-LRG
...
; CM-DFL-LABEL: test_global_addr_disc:
; CM-DFL:       ; %bb.0:
; CM-DFL-NEXT:  Lloh{{.*}}:
; CM-DFL-NEXT:    adrp x8, _g.ref.da.42.addr@PAGE
; CM-DFL-NEXT:  Lloh{{.*}}:
; CM-DFL-NEXT:    add x8, x8, _g.ref.da.42.addr@PAGEOFF
; CM-DFL-NEXT:    adrp x16, _g@GOTPAGE
; CM-DFL-NEXT:    ldr x16, [x16, _g@GOTPAGEOFF]
; CM-DFL-NEXT:    mov x17, x8
; CM-DFL-NEXT:    movk x17, #42, lsl #48
; CM-DFL-NEXT:    pacda x16, x17
; CM-DFL-NEXT:    mov x0, x16
; CM-DFL-NEXT:    ret
;
;
; CM-LRG-LABEL: test_global_addr_disc:
; CM-LRG:       ; %bb.0:
; CM-LRG-NEXT:  Lloh{{.*}}:
; CM-LRG-NEXT:    adrp x8, _g.ref.da.42.addr@GOTPAGE
; CM-LRG-NEXT:  Lloh{{.*}}:
; CM-LRG-NEXT:    ldr x8, [x8, _g.ref.da.42.addr@GOTPAGEOFF]
; CM-LRG-NEXT:    adrp x16, _g@GOTPAGE
; CM-LRG-NEXT:    ldr x16, [x16, _g@GOTPAGEOFF]
; CM-LRG-NEXT:    mov x17, x8
; CM-LRG-NEXT:    movk x17, #42, lsl #48
; CM-LRG-NEXT:    pacda x16, x17
; CM-LRG-NEXT:    mov x0, x16
; CM-LRG-NEXT:    ret
...
; CM-DFL-LABEL: test_global_strong_def:
; CM-DFL:       ; %bb.0:
; CM-DFL-NEXT:    adrp x16, _g_strong_def@PAGE
; CM-DFL-NEXT:    add x16, x16, _g_strong_def@PAGEOFF
; CM-DFL-NEXT:    pacdza x16
; CM-DFL-NEXT:    mov x0, x16
; CM-DFL-NEXT:    ret
;
;
; CM-LRG-LABEL: test_global_strong_def:
; CM-LRG:       ; %bb.0:
; CM-LRG-NEXT:    adrp x16, _g_strong_def@GOTPAGE
; CM-LRG-NEXT:    ldr x16, [x16, _g_strong_def@GOTPAGEOFF]
; CM-LRG-NEXT:    pacdza x16
; CM-LRG-NEXT:    mov x0, x16
; CM-LRG-NEXT:    ret
