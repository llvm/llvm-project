...
; CHECK:   [[LOAD:%[0-9]+]]:_(i32) = G_SEXTLOAD [[FRAME_INDEX]](p0) :: (invariant load (i8) from %fixed-stack.1, align 16)
; CHECK:   [[ASSERT_SEXT:%[0-9]+]]:_(i32) = G_ASSERT_SEXT [[LOAD]], 8
; CHECK:   [[TRUNC:%[0-9]+]]:_(i8) = G_TRUNC [[ASSERT_SEXT]](i32)
; CHECK:   [[FRAME_INDEX1:%[0-9]+]]:_(p0) = G_FRAME_INDEX %fixed-stack.0
; CHECK:   [[LOAD1:%[0-9]+]]:_(i32) = G_SEXTLOAD [[FRAME_INDEX1]](p0) :: (invariant load (i8) from %fixed-stack.0)
; CHECK:   [[ASSERT_SEXT1:%[0-9]+]]:_(i32) = G_ASSERT_SEXT [[LOAD1]], 8
; CHECK:   [[TRUNC1:%[0-9]+]]:_(i8) = G_TRUNC [[ASSERT_SEXT1]](i32)
; CHECK:   [[ADD:%[0-9]+]]:_(i8) = G_ADD [[TRUNC]], [[TRUNC1]]
; CHECK:   [[SEXT:%[0-9]+]]:_(i32) = G_SEXT [[ADD]](i8)
...
; CHECK: [[C42:%[0-9]+]]:_(i8) = G_CONSTANT i8 42
; CHECK: [[C12:%[0-9]+]]:_(i8) = G_CONSTANT i8 12
; CHECK: [[SP:%[0-9]+]]:_(p0) = COPY $sp
; CHECK: [[C42_OFFS:%[0-9]+]]:_(i64) = G_CONSTANT i64 0
; CHECK: [[C42_LOC:%[0-9]+]]:_(p0) = G_PTR_ADD [[SP]], [[C42_OFFS]](i64)
; CHECK: G_STORE [[C42]](i8), [[C42_LOC]](p0) :: (store (i8) into stack)
; CHECK: [[C12_OFFS:%[0-9]+]]:_(i64) = G_CONSTANT i64 1
; CHECK: [[C12_LOC:%[0-9]+]]:_(p0) = G_PTR_ADD [[SP]], [[C12_OFFS]](i64)
; CHECK: G_STORE [[C12]](i8), [[C12_LOC]](p0) :: (store (i8) into stack + 1)
...
; CHECK: [[LD1:%[0-9]+]]:_(i64) = G_LOAD %0(p0) :: (load (i64) from %ir.ptr)
; CHECK: [[CST:%[0-9]+]]:_(i64) = G_CONSTANT i64 8
; CHECK: [[GEP:%[0-9]+]]:_(p0) = nuw inbounds G_PTR_ADD %0, [[CST]](i64)
; CHECK: [[LD2:%[0-9]+]]:_(i64) = G_LOAD %3(p0) :: (load (i64) from %ir.ptr + 8)

; CHECK: [[SP:%[0-9]+]]:_(p0) = COPY $sp
; CHECK: [[OFF:%[0-9]+]]:_(i64) = G_CONSTANT i64 0
