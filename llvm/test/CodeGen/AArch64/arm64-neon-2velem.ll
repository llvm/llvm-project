...
entry:
  ...
  %lane2 = shufflevector <4 x float> %v, <4 x float> undef, <4 x i32> <i32 3, i32 3, i32 3, i32 3>
	%1 = fmul contract <4 x float> %lane2, %c
	%s = fsub contract <4 x float> %0, %1
  ret <4 x float> %s
}
...
; The multiply-subtract is only fused when the fsub carries the contract flag;
; without it the fmul stays separate.
define <4 x float> @no_fuse_dup_fsub_no_contract(<4 x float> %a, <4 x float> %b, <4 x float> %c, <4 x float> %v) {
; CHECK-SD-LABEL: no_fuse_dup_fsub_no_contract:
; CHECK-SD:       // %bb.0: // %entry
; CHECK-SD-NEXT:    fmul v2.4s, v2.4s, v3.s[3]
; CHECK-SD-NEXT:    fmla v0.4s, v1.4s, v3.s[3]
; CHECK-SD-NEXT:    fsub v0.4s, v0.4s, v2.4s
; CHECK-SD-NEXT:    ret
;
; CHECK-GI-LABEL: no_fuse_dup_fsub_no_contract:
; CHECK-GI:       // %bb.0: // %entry
; CHECK-GI-NEXT:    fmla v0.4s, v1.4s, v3.s[3]
; CHECK-GI-NEXT:    fmul v1.4s, v2.4s, v3.s[3]
...
; Likewise, the multiply-add is only fused when the fadd carries the contract
; flag.
define <4 x float> @no_fuse_dup_fadd_no_contract(<4 x float> %a, <4 x float> %b, <4 x float> %c, <4 x float> %v) {
; CHECK-SD-LABEL: no_fuse_dup_fadd_no_contract:
; CHECK-SD:       // %bb.0: // %entry
; CHECK-SD-NEXT:    fmul v2.4s, v2.4s, v3.s[3]
; CHECK-SD-NEXT:    fmla v0.4s, v1.4s, v3.s[3]
...
; CHECK-SD-NEXT:    ret
;
; CHECK-GI-LABEL: no_fuse_dup_fadd_no_contract:
; CHECK-GI:       // %bb.0: // %entry
; CHECK-GI-NEXT:    fmla v0.4s, v1.4s, v3.s[3]
; CHECK-GI-NEXT:    fmul v1.4s, v2.4s, v3.s[3]
...
; CHECK-GI-NEXT:    ret
entry:
  %lane1 = shufflevector <4 x float> %v, <4 x float> poison, <4 x i32> <i32 3, i32 3, i32 3, i32 3>
  %0 = tail call <4 x float> @llvm.fma.v4f32(<4 x float> %lane1, <4 x float> %b, <4 x float> %a)
  %lane2 = shufflevector <4 x float> %v, <4 x float> poison, <4 x i32> <i32 3, i32 3, i32 3, i32 3>
  %1 = fmul contract <4 x float> %lane2, %c
  %s = fadd <4 x float> %0, %1
  ret <4 x float> %s
}
...
; CHECK-NEXT:    ret
entry:
  %lane = shufflevector <2 x float> %v, <2 x float> poison, <2 x i32> <i32 1, i32 1>
  %0 = tail call <2 x float> @llvm.fma.v2f32(<2 x float> %lane, <2 x float> %b, <2 x float> %a)
  ret <2 x float> %0
}
