...
; i256 shift left with narrow (i32) amount
define i256 @shl_i256_narrow(i256 %a, i32 %amt) nounwind {
...
; CHECK-NEXT:    ldp x11, x10, [x9]
; CHECK-NEXT:    ldp x14, x9, [x9, #16]
; CHECK-NEXT:    lsr x15, x11, #1
; CHECK-NEXT:    lsl x16, x10, x8
; CHECK-NEXT:    lsr x10, x10, #1
; CHECK-NEXT:    lsr x17, x14, #1
; CHECK-NEXT:    lsl x14, x14, x8
; CHECK-NEXT:    lsl x9, x9, x8
; CHECK-NEXT:    lsr x15, x15, x13
; CHECK-NEXT:    lsl x0, x11, x8
; CHECK-NEXT:    lsr x8, x10, x12
; CHECK-NEXT:    lsr x10, x17, x13
; CHECK-NEXT:    orr x1, x16, x15
; CHECK-NEXT:    orr x2, x14, x8
; CHECK-NEXT:    orr x3, x9, x10
; CHECK-NEXT:    add sp, sp, #64
; CHECK-NEXT:    ret
  %ext = zext i32 %amt to i256
  %r = shl i256 %a, %ext
  ret i256 %r
}

; i256 logical shift right with narrow (i32) amount
define i256 @lshr_i256_narrow(i256 %a, i32 %amt) nounwind {
...
; CHECK-NEXT:    lsr x9, x8, #3
; CHECK-NEXT:    stp x2, x3, [sp, #16]
; CHECK-NEXT:    and x12, x8, #0x3f
; CHECK-NEXT:    stp x0, x1, [sp]
; CHECK-NEXT:    mvn w14, w4
; CHECK-NEXT:    eor x12, x12, #0x3f
; CHECK-NEXT:    and x9, x9, #0x18
; CHECK-NEXT:    stp q0, q0, [sp, #32]
; CHECK-NEXT:    add x9, x10, x9
; CHECK-NEXT:    ldp x10, x11, [x9]
; CHECK-NEXT:    ldp x13, x9, [x9, #16]
; CHECK-NEXT:    lsl x15, x11, #1
; CHECK-NEXT:    lsr x10, x10, x8
; CHECK-NEXT:    lsr x11, x11, x8
; CHECK-NEXT:    lsl x16, x13, #1
; CHECK-NEXT:    lsl x17, x9, #1
; CHECK-NEXT:    lsr x13, x13, x8
; CHECK-NEXT:    lsl x15, x15, x12
; CHECK-NEXT:    lsr x3, x9, x8
; CHECK-NEXT:    lsl x8, x16, x14
; CHECK-NEXT:    lsl x9, x17, x12
; CHECK-NEXT:    orr x0, x15, x10
; CHECK-NEXT:    orr x1, x11, x8
; CHECK-NEXT:    orr x2, x9, x13
; CHECK-NEXT:    add sp, sp, #64
; CHECK-NEXT:    ret
  %ext = zext i32 %amt to i256
  %r = lshr i256 %a, %ext
