...
; RUN: llc -verify-machineinstrs -mtriple=aarch64 -code-model=tiny -relocation-model=pic < src.ll | FileCheck %s --check-prefix=CHECK-PIC
...
; RUN: llc -verify-machineinstrs -mtriple=aarch64 -code-model=tiny -relocation-model=pic -fast-isel < src.ll | FileCheck %s --check-prefix=CHECK-PIC
...
; RUN: llc -verify-machineinstrs -mtriple=aarch64 -code-model=tiny -relocation-model=pic -global-isel < src.ll | FileCheck %s --check-prefix=CHECK-PIC-GLOBISEL
