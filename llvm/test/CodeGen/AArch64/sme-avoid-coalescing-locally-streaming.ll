...
define void @dont_coalesce_args(<2 x i64> %a) "aarch64_pstate_sm_body" nounwind {
  ...
  ; CHECK-COALESCER-BARRIER-NEXT:   [[COPY1:%[0-9]+]]:fpr128 = COPY [[COPY]]
  ; CHECK-COALESCER-BARRIER-NEXT:   [[DEF:%[0-9]+]]:zpr = IMPLICIT_DEF
  ; CHECK-COALESCER-BARRIER-NEXT:   [[INSERT_SUBREG:%[0-9]+]]:zpr = INSERT_SUBREG [[DEF]], [[COPY1]], %subreg.zsub
  ...
  ; CHECK-REGALLOC-NEXT: {{  $}}
  ; CHECK-REGALLOC-NEXT:   renamable $q0 = KILL $q0, implicit-def $z0
  ; CHECK-REGALLOC-NEXT:   STR_ZXI killed renamable $z0, %stack.0, 0 :: (store (s128) into %stack.0)
  ...
  ; CHECK-REGALLOC-NEXT:   $z0 = LDR_ZXI %stack.0, 0 :: (load (s128) from %stack.0)
...
define <2 x i64> @dont_coalesce_res() "aarch64_pstate_sm_body" nounwind {
  ...
  ; CHECK-COALESCER-BARRIER-NEXT:   $q0 = COPY [[COPY1]]
  ...
  ; CHECK-REGALLOC-NEXT:   ADJCALLSTACKUP 0, 0, implicit-def dead $sp, implicit $sp
  ; CHECK-REGALLOC-NEXT:   STR_ZXI $z0, %stack.0, 0 :: (store (s128) into %stack.0)
  ...
  ; CHECK-REGALLOC-NEXT:   renamable $z0 = LDR_ZXI %stack.0, 0 :: (load (s128) from %stack.0)
  ; CHECK-REGALLOC-NEXT:   $q0 = KILL renamable $q0, implicit killed $z0
