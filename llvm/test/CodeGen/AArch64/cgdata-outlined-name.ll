...
; RUN: llc -mtriple=arm64-apple-darwin -codegen-data-generate=true -append-content-hash-outlined-name=false -filetype=obj %t/local-two.ll -o %t_write_base
; RUN: llvm-objdump -d %t_write_base | FileCheck %s --check-prefix=BASE
; RUN: llc -mtriple=arm64-apple-darwin -codegen-data-generate=true -append-content-hash-outlined-name=true -filetype=obj %t/local-two.ll -o %t_write_suffix
...
; RUN: llc -mtriple=arm64-apple-darwin -codegen-data-use-path=%t_cgdata_base -append-content-hash-outlined-name=false -filetype=obj %t/local-one.ll -o %t_read_base
; RUN: llvm-objdump -d %t_read_base | FileCheck %s --check-prefix=BASE
; RUN: llc -mtriple=arm64-apple-darwin -codegen-data-use-path=%t_cgdata_suffix -append-content-hash-outlined-name=true -filetype=obj %t/local-one.ll -o %t_read_suffix
