...
; RUN: llc -aarch64-sve-vector-bits-min=256  < %s | FileCheck %s -check-prefixes=CHECK,VBITS_GE_256,CHECK-SVE
; RUN: llc -aarch64-sve-vector-bits-min=512  < %s | FileCheck %s -check-prefixes=CHECK,CHECK-SVE,VBITS_GE_512
; RUN: llc -aarch64-sve-vector-bits-min=2048 < %s | FileCheck %s -check-prefixes=CHECK,CHECK-SVE,VBITS_GE_512
; Run with +sve2 to check lowering to SVE2 pairwise operations.
; RUN: llc -aarch64-sve-vector-bits-min=256 -mattr=+sve2  < %s | FileCheck %s -check-prefixes=CHECK,VBITS_GE_256,CHECK-SVE2
