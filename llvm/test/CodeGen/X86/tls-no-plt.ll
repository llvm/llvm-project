; RUN: llc < %s -mtriple=i386-linux-musl -relocation-model=pic --opaque-pointers=1 | FileCheck --check-prefixes=CHECK,X86 %s
; RUN: llc < %s -mtriple=x86_64-linux-musl -relocation-model=pic --opaque-pointers=1 | FileCheck --check-prefixes=CHECK,X64 %s
