...
; CHECK-PREG:     renamable $r15, renamable $r14, renamable $r12, renamable $r13, renamable $rbx, renamable $rbp = STATEPOINT 0, 0, 0, @func, 2, 0, 2, 0, 2, 0, 2, 18, killed renamable $r15(tied-def 0), 3, killed renamable $r14(tied-def 1), 3, killed renamable $r12(tied-def 2), 3, killed renamable $r13(tied-def 3), 3, killed renamable $rbx(tied-def 4), 3, 1, 8, %stack.0, 0, 3, 1, 8, %stack.1, 0, 3, 1, 8, %stack.3, 0, 3, 1, 8, %stack.4, 0, 3, 1, 8, %stack.5, 0, 3, 1, 8, %stack.7, 0, 3, 1, 8, %stack.8, 0, 3, 1, 8, %stack.2, 0, 3, 1, 8, %stack.6, 0, 3, 1, 8, %stack.9, 0, 3, 1, 8, %stack.10, 0, 3, 1, 8, %stack.11, 0, 3, killed renamable $rbp(tied-def 5), 3, 2, 0, 2, 18, 0, 0, 3, 1, 1, 3, 2, 2, 3, 3, 3, 3, 4, 4, 3, 5, 5, 3, 6, 6, 3, 7, 7, 3, 8, 8, 3, 9, 9, 3, 10, 10, 3, 11, 11, 3, 12, 12, 3, 13, 13, 3, 14, 14, 3, 15, 15, 3, 16, 16, 3, 17, 17, 3, csr_64, implicit-def $rsp, implicit-def $ssp :: (load store (s64) on %stack.0), (load store (s64) on %stack.1), (load store (s64) on %stack.2), (load store (s64) on %stack.3), (load store (s64) on %stack.4), (load store (s64) on %stack.5), (load store (s64) on %stack.6), (load store (s64) on %stack.7), (load store (s64) on %stack.8), (load store (s64) on %stack.9), (load store (s64) on %stack.10), (load store (s64) on %stack.11)
