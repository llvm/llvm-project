...
; RUN: llc < %s --opaque-pointers=1 -mtriple=i686-unknown-unknown   -mattr=+sse,+sse2 | FileCheck %s --check-prefix=X86
; RUN: llc < %s --opaque-pointers=1 -mtriple=x86_64-unknown-unknown -mattr=+sse,+sse2 | FileCheck %s --check-prefix=X64
...
; X86-NEXT:    movl 148(%esp,%eax), %edx
; X86-NEXT:    movl 152(%esp,%eax), %ebx
; X86-NEXT:    movb %ch, %cl
; X86-NEXT:    shldl %cl, %edx, %ebx
; X86-NEXT:    shll %cl, %edx
; X86-NEXT:    notb %cl
; X86-NEXT:    movl 144(%esp,%eax), %ebp
; X86-NEXT:    movl %ebp, %edi
; X86-NEXT:    shrl %edi
; X86-NEXT:    shrl %cl, %edi
; X86-NEXT:    orl %edx, %edi
; X86-NEXT:    movl 140(%esp,%eax), %eax
; X86-NEXT:    movb %ch, %cl
...
; X86-NEXT:    movb %ch, %cl
; X86-NEXT:    movl {{[-0-9]+}}(%e{{[sb]}}p), %esi # 4-byte Reload
; X86-NEXT:    shrdl %cl, %esi, (%esp) # 4-byte Folded Spill
