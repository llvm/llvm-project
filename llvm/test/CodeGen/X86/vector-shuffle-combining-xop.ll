...
; X64-AVX:       # %bb.0:
; X64-AVX-NEXT:    vshufpd {{.*#+}} ymm0 = ymm0[0],ymm1[0],ymm0[3],ymm1[3]
; X64-AVX-NEXT:    vshufpd {{.*#+}} ymm0 = ymm0[1,1,2,3]
; X64-AVX-NEXT:    retq
;
; X64-AVX2-LABEL: demandedelts_vpermil2pd256_as_shufpd:
; X64-AVX2:       # %bb.0:
; X64-AVX2-NEXT:    vpermil2pd {{.*#+}} ymm0 = ymm1[0,0],ymm0[3],ymm1[3]
; X64-AVX2-NEXT:    retq
...
; X86:       # %bb.0:
; X86-NEXT:    vpaddq {{\.?LCPI[0-9]+_[0-9]+}}, %xmm0, %xmm0 # [185207048,252579084,50462976,117835012]
; X86-NEXT:    retl
;
; X64-LABEL: combine_vpperm_identity_bitcast:
; X64:       # %bb.0:
; X64-NEXT:    vpaddq {{\.?LCPI[0-9]+_[0-9]+}}(%rip), %xmm0, %xmm0 # [1084818905618843912,506097522914230528]
; X64-NEXT:    retq
