...
; NOADX-NEXT:    retq
  ...
  %not.ov.ext = zext i1 %not.ov to i32
  %sum = sub i32 %a, %b
  %res = sub i32 %sum, %not.ov.ext
  ret i32 %res
}
...
; NOADX:       # %bb.0:
; NOADX-NEXT:    movq %rdi, %rax
; NOADX-NEXT:    xorl %edi, %edi
; NOADX-NEXT:    cmpb %cl, %dl
; NOADX-NEXT:    setno %dil
; NOADX-NEXT:    subq %rsi, %rax
; NOADX-NEXT:    subq %rdi, %rax
; NOADX-NEXT:    retq
  %ssub = call { i8, i1 } @llvm.ssub.with.overflow.i8(i8 %c, i8 %d)
  %ov = extractvalue { i8, i1 } %ssub, 1
  %not.ov = xor i1 %ov, true
  %not.ov.ext = zext i1 %not.ov to i64
  %sum = sub i64 %a, %b
  %res = sub i64 %sum, %not.ov.ext
  ret i64 %res
}
