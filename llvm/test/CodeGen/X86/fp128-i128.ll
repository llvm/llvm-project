...
; RUN: llc < %s -O2 -verify-machineinstrs -mtriple=x86_64-linux-android -mattr=+mmx,avx2 -enable-legalize-types-checking | FileCheck %s --check-prefixes=CHECK,AVX,AVX2
; RUN: llc < %s -O2 -verify-machineinstrs -mtriple=x86_64-linux-gnu     -mattr=+mmx,avx2 -enable-legalize-types-checking | FileCheck %s --check-prefixes=CHECK,AVX,AVX2
; RUN: llc < %s -O2 -verify-machineinstrs -mtriple=x86_64-linux-android -mattr=+mmx,avx512vl -enable-legalize-types-checking | FileCheck %s --check-prefixes=CHECK,AVX,AVX512
; RUN: llc < %s -O2 -verify-machineinstrs -mtriple=x86_64-linux-gnu     -mattr=+mmx,avx512vl -enable-legalize-types-checking | FileCheck %s --check-prefixes=CHECK,AVX,AVX512
...
; AVX2-NEXT:    callq __gttf2@PLT
; AVX2-NEXT:    movl %eax, %ebp
; AVX2-NEXT:    vmovaps {{[-0-9]+}}(%r{{[sb]}}p), %xmm0 # 16-byte Reload
; AVX2-NEXT:    vmovaps %xmm0, %xmm1
; AVX2-NEXT:    callq __subtf3@PLT
; AVX2-NEXT:    testl %ebp, %ebp
; AVX2-NEXT:    jle .LBB10_1
; AVX2-NEXT:  # %bb.2: # %if.then
; AVX2-NEXT:    vmovaps {{.*#+}} xmm1 = [-0.0E+0]
; AVX2-NEXT:    vandnps %xmm0, %xmm1, %xmm3
; AVX2-NEXT:    vmovaps (%rsp), %xmm0 # 16-byte Reload
; AVX2-NEXT:    jmp .LBB10_3
; AVX2-NEXT:  .LBB10_1:
; AVX2-NEXT:    vmovaps (%rsp), %xmm3 # 16-byte Reload
; AVX2-NEXT:  .LBB10_3: # %cleanup
; AVX2-NEXT:    vmovaps {{.*#+}} xmm1 = [-0.0E+0]
; AVX2-NEXT:    vandps {{[-0-9]+}}(%r{{[sb]}}p), %xmm1, %xmm2 # 16-byte Folded Reload
; AVX2-NEXT:    vandnps %xmm0, %xmm1, %xmm0
; AVX2-NEXT:    vorps %xmm2, %xmm0, %xmm0
; AVX2-NEXT:    vmovaps %xmm3, (%rbx)
; AVX2-NEXT:    vmovaps %xmm0, 16(%rbx)
...
; AVX2-NEXT:    retq
;
; AVX512-LABEL: TestCopySign:
; AVX512:       # %bb.0: # %entry
; AVX512-NEXT:    pushq %rbp
; AVX512-NEXT:    pushq %rbx
; AVX512-NEXT:    subq $40, %rsp
; AVX512-NEXT:    movq %rdi, %rbx
; AVX512-NEXT:    vmovaps {{[0-9]+}}(%rsp), %xmm0
...
; AVX512-NEXT:    callq __gttf2@PLT
; AVX512-NEXT:    movl %eax, %ebp
; AVX512-NEXT:    vmovdqa {{[-0-9]+}}(%r{{[sb]}}p), %xmm0 # 16-byte Reload
; AVX512-NEXT:    vmovdqa %xmm0, %xmm1
; AVX512-NEXT:    callq __subtf3@PLT
; AVX512-NEXT:    testl %ebp, %ebp
; AVX512-NEXT:    jle .LBB10_1
; AVX512-NEXT:  # %bb.2: # %if.then
; AVX512-NEXT:    vmovdqa {{.*#+}} xmm1 = [-0.0E+0]
; AVX512-NEXT:    vpandn %xmm0, %xmm1, %xmm1
; AVX512-NEXT:    vmovdqa (%rsp), %xmm0 # 16-byte Reload
; AVX512-NEXT:    jmp .LBB10_3
; AVX512-NEXT:  .LBB10_1:
; AVX512-NEXT:    vmovdqa (%rsp), %xmm1 # 16-byte Reload
; AVX512-NEXT:  .LBB10_3: # %cleanup
; AVX512-NEXT:    vmovdqa {{[-0-9]+}}(%r{{[sb]}}p), %xmm2 # 16-byte Reload
; AVX512-NEXT:    vpternlogd {{.*#+}} xmm0 = xmm0 ^ (mem & (xmm0 ^ xmm2))
; AVX512-NEXT:    vmovdqa %xmm1, (%rbx)
; AVX512-NEXT:    vmovdqa %xmm0, 16(%rbx)
