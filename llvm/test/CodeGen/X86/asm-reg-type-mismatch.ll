...
target triple = "x86_64-unknown-linux-gnu"
	...
	%struct.nsISupports = type { ptr }
	%struct.nsXPTCMiniVariant = type { %"struct.nsXPTCMiniVariant::._39" }
	%"struct.nsXPTCMiniVariant::._39" = type { i64 }
	%struct.nsXPTCVariant = type { %struct.nsXPTCMiniVariant, ptr, %struct.nsXPTType, i8 }
	%struct.nsXPTType = type { %struct.XPTTypeDescriptorPrefix }

define i32 @test1(ptr %that, i32 %methodIndex, i32 %paramCount, ptr %params) nounwind {
entry:
...
entry:
  %0 = tail call i64 asm sideeffect "movq $1, $0", "={xmm7},*m,~{dirflag},~{fpsr},~{flags}"(ptr elementtype(i64) null) nounwind
  ret i64 %0
  ; CHECK: test2
	; CHECK: movq {{.*}}, %xmm7
	; CHECK: movq %xmm7, %rax
