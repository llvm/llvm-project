...
; Darwin currently crashes due to an unrelated DWARF generation bug
; RUN: not --crash llc < %s -mtriple=x86_64-apple-darwin
...
;
; NOTE: This test case also revealed an unrelated Darwin-specific crash in
;       DwarfCompileUnit::constructSubprogramScopeDIE during DWARF generation.
;
; FIXME: Investigate and fix the Darwin crash separately. The crash occurs in
;        the Assembly Printer pass when generating DWARF debug info for
;        functions with aggregate types containing nested empty structs.
