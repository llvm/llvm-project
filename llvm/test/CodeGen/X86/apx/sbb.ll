...
; RUN: llc < %s -mtriple=x86_64-unknown -mattr=+ndd,+enable-ndd-imm -verify-machineinstrs --show-mc-encoding | FileCheck %s --check-prefixes=CHECK,IMM,IMMONLY
; RUN: llc < %s -mtriple=x86_64-unknown -mattr=+ndd,+enable-ndd-mem -verify-machineinstrs --show-mc-encoding | FileCheck %s --check-prefixes=CHECK,MEM,MEMONLY
; RUN: llc < %s -mtriple=x86_64-unknown -mattr=+ndd,+enable-ndd-imm,+enable-ndd-mem -verify-machineinstrs --show-mc-encoding | FileCheck %s --check-prefixes=CHECK,IMM,MEM,BOTH
