...
entry:
  %c = alloca i8, align 1
  call void asm sideeffect inteldialect "vaddps xmm1, xmm2, $1{1to4}\0A\09vaddps xmm1, xmm2, $2\0A\09mov eax, $3\0A\09mov $0, rax", "=*m,*m,*m,*m,~{eax},~{xmm1},~{dirflag},~{fpsr},~{flags}"(ptr elementtype(i8) %c, ptr elementtype(i8) %c, ptr elementtype(i8) %c, ptr elementtype(i8) %c) #1
  ret void
}
