...
; RUN: %if x86-registered-target %{ not llc -O2 -enable-new-pm -mtriple=x86_64-linux-unknown -regalloc-enable-advisor=development < %s 2>&1 | FileCheck %s %}
; RUN: %if x86-registered-target %{ not llc -O2 -enable-new-pm -mtriple=x86_64-linux-unknown -regalloc-enable-advisor=release < %s 2>&1 | FileCheck %s %}
; RUN: %if x86-registered-target %{ llc -O2 -enable-new-pm -mtriple=x86_64-linux-unknown -regalloc-enable-advisor=default < %s 2>&1 | FileCheck %s --check-prefix=DEFAULT %}
