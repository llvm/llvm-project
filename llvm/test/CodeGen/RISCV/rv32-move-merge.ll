...
; CHECK32ZDINX-NEXT:    sw ra, 12(sp) # 4-byte Spill
; CHECK32ZDINX-NEXT:    sw s0, 8(sp) # 4-byte Spill
; CHECK32ZDINX-NEXT:    sw s1, 4(sp) # 4-byte Spill
...
; CHECK32ZDINX-NEXT:    lw ra, 12(sp) # 4-byte Reload
; CHECK32ZDINX-NEXT:    lw s0, 8(sp) # 4-byte Reload
; CHECK32ZDINX-NEXT:    lw s1, 4(sp) # 4-byte Reload
; CHECK32ZDINX-NEXT:    addi sp, sp, 16
...
; CHECK32P-NEXT:    sw ra, 12(sp) # 4-byte Spill
; CHECK32P-NEXT:    sw s0, 8(sp) # 4-byte Spill
; CHECK32P-NEXT:    sw s1, 4(sp) # 4-byte Spill
; CHECK32P-NEXT:    addd s0, a0, a2
; CHECK32P-NEXT:    call foo
; CHECK32P-NEXT:    padd.dw a0, s0, zero
; CHECK32P-NEXT:    lw ra, 12(sp) # 4-byte Reload
; CHECK32P-NEXT:    lw s0, 8(sp) # 4-byte Reload
; CHECK32P-NEXT:    lw s1, 4(sp) # 4-byte Reload
; CHECK32P-NEXT:    addi sp, sp, 16
; CHECK32P-NEXT:    ret
  %1 = add i64 %a, %b
  call void @foo()
