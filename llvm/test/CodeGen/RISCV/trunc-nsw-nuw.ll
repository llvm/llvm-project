...
; RUN: llc < %s -mtriple=riscv64 -mattr=+m | FileCheck %s --check-prefixes=CHECK,NOTOPO
; Topological DAG-combiner ordering causes visitTRUNCATE's trunc-through-binop
; fold to beat visitSIGN_EXTEND's trunc-elision fold, losing the nsw flag and
; regressing the codegen. Fixed by a subsequent commit.
; RUN: llc < %s -mtriple=riscv64 -mattr=+m -combiner-topological-sorting | FileCheck %s --check-prefixes=CHECK,TOPO

define signext i8 @trunc_nsw_add(i32 signext %x) nounwind {
; NOTOPO-LABEL: trunc_nsw_add:
; NOTOPO:       # %bb.0: # %entry
; NOTOPO-NEXT:    addiw a0, a0, 1
; NOTOPO-NEXT:    ret
;
; TOPO-LABEL: trunc_nsw_add:
; TOPO:       # %bb.0: # %entry
; TOPO-NEXT:    addi a0, a0, 1
; TOPO-NEXT:    slli a0, a0, 56
; TOPO-NEXT:    srai a0, a0, 56
; TOPO-NEXT:    ret
entry:
  %add = add nsw i32 %x, 1
