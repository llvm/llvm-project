...
; RUN: llc -mtriple=riscv64-pc-windows-msvc -verify-machineinstrs < %s | FileCheck %s --check-prefix=WIN64
...
declare void @forward_windows_sret(ptr, ptr sret(%struct.Buffer))
...
define void @windows_sret_after_this(ptr %this, ptr noalias sret(%struct.Buffer) %result) {
; WIN64-LABEL: windows_sret_after_this:
; WIN64:       # %bb.0: # %entry
; WIN64-NEXT:    tail forward_windows_sret
entry:
  tail call void @forward_windows_sret(ptr %this, ptr sret(%struct.Buffer) %result)
  ret void
}

define void @windows_sret(ptr %this, ptr noalias sret(%struct.Buffer) %result, i64 %tag) {
; WIN64-LABEL: windows_sret:
; WIN64:       # %bb.0: # %entry
; WIN64-NEXT:  mv	a0, a1
; WIN64-NEXT:	mv	a1, a2
; WIN64-NEXT:	tail	forward_sret
entry:
  tail call void @forward_sret(ptr sret(%struct.Buffer) %result, i64 %tag)
  ret void
}
