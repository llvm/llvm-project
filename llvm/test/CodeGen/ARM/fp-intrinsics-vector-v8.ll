...
; RUN: llc -mtriple=armv8a-none-eabi %s -o - | FileCheck %s
...
; CHECK-NEXT:    bx lr
  %val = call <4 x float> @llvm.experimental.constrained.nearbyint.v4f32(<4 x float> %x, metadata !"round.dynamic", metadata !"fpexcept.strict") #0
  ret <4 x float> %val
}
