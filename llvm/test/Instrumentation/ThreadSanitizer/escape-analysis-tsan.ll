...
; %p is loaded from arr[%k], which may-alias both arr[%i] (holding a pointer
; loaded from a global, possibly shared across threads) and arr[%j] (holding a
; local). The backward base-finding walk must not stop at the first may-aliasing
; store: it has to keep climbing and see the foreign source, so the access
; through %p is conservatively instrumented. Otherwise TSan would miss a real
; race when %k == %i at runtime.
define void @mayalias_foreign(i64 %i, i64 %j, i64 %k) nounwind uwtable sanitize_thread {
  %arr = alloca [4 x ptr], align 8
  %gv  = load ptr, ptr @GPtr, align 8
  %gi  = getelementptr inbounds [4 x ptr], ptr %arr, i64 0, i64 %i
  store ptr %gv, ptr %gi, align 8
  %lx  = alloca i32, align 4
  %gj  = getelementptr inbounds [4 x ptr], ptr %arr, i64 0, i64 %j
  store ptr %lx, ptr %gj, align 8
  %gk  = getelementptr inbounds [4 x ptr], ptr %arr, i64 0, i64 %k
  %p   = load ptr, ptr %gk, align 8
  store i32 1, ptr %p, align 4
  ret void
}
; CHECK-LABEL: define void @mayalias_foreign
