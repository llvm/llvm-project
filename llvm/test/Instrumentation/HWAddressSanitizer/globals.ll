; RUN: opt < %s -S -passes=hwasan -mtriple=aarch64--linux-android29 -opaque-pointers | FileCheck --check-prefixes=CHECK,CHECK29 %s
; RUN: opt < %s -S -passes=hwasan -mtriple=aarch64--linux-android30 -opaque-pointers | FileCheck --check-prefixes=CHECK,CHECK30 %s
