...
; AFTER: define void @test2_a(ptr nofree readnone captures(address) %ignore)
entry:
  %f1ptr = alloca ptr
...
; AFTER-LABEL: define void @test3(ptr nofree readonly captures(none) %src, ptr nofree writeonly captures(none) %dest, i64 %size)
  %fptr = alloca ptr
