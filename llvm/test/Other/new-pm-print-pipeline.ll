...
; RUN: opt -disable-output -disable-verify -print-pipeline-passes -passes='function(float2int,float2int<max-integer-bw=32>)' < %s | FileCheck %s --match-full-lines --check-prefixes=CHECK-40
; CHECK-40: function(float2int<max-integer-bw=64>,float2int<max-integer-bw=32>)
