...
; RUN: opt -disable-output -disable-verify -print-pipeline-passes -passes='function(callsite-splitting,callsite-splitting<duplication-threshold=10>)' < %s | FileCheck %s --match-full-lines --check-prefixes=CHECK-39
; CHECK-39: function(callsite-splitting<duplication-threshold=5>,callsite-splitting<duplication-threshold=10>)
