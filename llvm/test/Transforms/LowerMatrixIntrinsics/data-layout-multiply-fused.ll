...
; RUN: opt -passes=lower-matrix-intrinsics,instcombine -data-layout='p:64:64' -fuse-matrix-tile-size-rows=2 -fuse-matrix-tile-size-inner=2 -fuse-matrix-tile-size-columns=2 -matrix-allow-contract -force-fuse-matrix -verify-dom-info %s -S | FileCheck %s --check-prefix=PTR64
; RUN: opt -passes=lower-matrix-intrinsics,instcombine -data-layout='p:32:32' -fuse-matrix-tile-size-rows=2 -fuse-matrix-tile-size-inner=2 -fuse-matrix-tile-size-columns=2 -matrix-allow-contract -force-fuse-matrix -verify-dom-info %s -S | FileCheck %s --check-prefix=PTR32
