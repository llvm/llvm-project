...
for.body:                                         ; preds = %for.body.lr.ph, %for.inc
  %add12 = phi i64 [ %0, %for.body.lr.ph ], [ %add.reass, %for.inc ]
  %i.011 = phi i64 [ 0, %for.body.lr.ph ], [ %add3, %for.inc ]
  br i1 %cmp1, label %if.then, label %for.inc

if.then:                                          ; preds = %for.body
  %arrayidx = getelementptr inbounds nuw float, ptr %a, i64 %add12
  store float %conv, ptr %arrayidx, align 4, !tbaa !5
  br label %for.inc

for.inc:                                          ; preds = %for.body, %if.then
  %add3 = add i64 %i.011, 8
  %add.reass = add nuw i64 %i.011, %invariant.op
