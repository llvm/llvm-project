...
; CHECK-NEXT:    [[SUB:%.*]] = add i32 [[ADD9]], 32
; CHECK-NEXT:    [[DIV:%.*]] = udiv i32 [[SUB]], 32
; CHECK-NEXT:    [[MUL:%.*]] = mul i32 32, [[DIV]]
...
; CHECK-NEXT:    [[TMP1:%.*]] = getelementptr float, ptr [[ARRAYIDX]], i32 [[J_035]]
; CHECK-NEXT:    [[ARRAYIDX6:%.*]] = getelementptr float, ptr [[TMP1]], i32 0
...
; CHECK-NEXT:    [[ADD8]] = add i32 32, [[J_035]]
; CHECK-NEXT:    [[ADD:%.*]] = add i32 32, [[ADD8]]
...
; CHECK-NEXT:    [[TMP4:%.*]] = getelementptr float, ptr [[ARRAYIDX11]], i32 [[MUL]]
; CHECK-NEXT:    [[ARRAYIDX13:%.*]] = getelementptr float, ptr [[TMP4]], i32 0
...
; CHECK-NEXT:    [[AVG_VAL_1_RIPPLE_VECTORIZED_RIPPLE_REDUCTION_PARTIAL_MASKING:%.*]] = select <32 x i1> splat (i1 true), <32 x float> [[AVG_VAL_1_RIPPLE_VECTORIZED]], <32 x float> splat (float -0.000000e+00)
; CHECK-NEXT:    [[AVG_VAL_1_RIPPLE_VECTORIZED_RIPPLE_REDUCTION:%.*]] = call reassoc float @llvm.vector.reduce.fadd.v32f32(float -0.000000e+00, <32 x float> [[AVG_VAL_1_RIPPLE_VECTORIZED_RIPPLE_REDUCTION_PARTIAL_MASKING]])
; CHECK-NEXT:    [[DIV15:%.*]] = fdiv float [[AVG_VAL_1_RIPPLE_VECTORIZED_RIPPLE_REDUCTION]], [[CONV]]
; CHECK-NEXT:    [[ARRAYIDX16:%.*]] = getelementptr inbounds float, ptr [[OUT:%.*]], i32 [[I_036]]
