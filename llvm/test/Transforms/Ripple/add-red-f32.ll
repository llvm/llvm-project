...
; CHECK-NEXT:  entry:
; CHECK-NEXT:    [[CMP21:%.*]] = icmp ult i32 32, [[N:%.*]]
...
; CHECK-NEXT:    [[DIV:%.*]] = udiv i32 [[N]], 32
; CHECK-NEXT:    [[MUL:%.*]] = mul i32 32, [[DIV]]
; CHECK-NEXT:    [[CMP5:%.*]] = icmp ult i32 [[MUL]], [[N]]
...
; CHECK-NEXT:    [[TMP0:%.*]] = getelementptr float, ptr [[A:%.*]], i32 [[I_022]]
; CHECK-NEXT:    [[ARRAYIDX2:%.*]] = getelementptr float, ptr [[TMP0]], i32 0
...
; CHECK-NEXT:    [[ADD4]] = add i32 32, [[I_022]]
; CHECK-NEXT:    [[ADD:%.*]] = add i32 32, [[ADD4]]
; CHECK-NEXT:    [[CMP:%.*]] = icmp ult i32 [[ADD]], [[N]]
; CHECK-NEXT:    br i1 [[CMP]], label [[FOR_BODY]], label [[FOR_COND_CLEANUP]]
; CHECK:       if.then:
; CHECK-NEXT:    [[TMP2:%.*]] = getelementptr float, ptr [[A]], i32 [[DIV]]
; CHECK-NEXT:    [[ARRAYIDX8:%.*]] = getelementptr float, ptr [[TMP2]], i32 0
...
; CHECK:       if.end:
; CHECK-NEXT:    [[RES_1_RIPPLE_VECTORIZED:%.*]] = phi <32 x float> [ [[ADD9_RIPPLE_VECTORIZED]], [[IF_THEN]] ], [ [[RES_0_LCSSA_RIPPLE_VECTORIZED]], [[FOR_COND_CLEANUP]] ]
; CHECK-NEXT:    [[RES_1_RIPPLE_VECTORIZED_RIPPLE_REDUCTION_PARTIAL_MASKING:%.*]] = select <32 x i1> splat (i1 true), <32 x float> [[RES_1_RIPPLE_VECTORIZED]], <32 x float> splat (float -0.000000e+00)
