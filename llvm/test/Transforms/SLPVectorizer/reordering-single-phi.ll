...
; RUN: %if x86-registered-target %{ opt -S --passes=slp-vectorizer -mtriple=x86_64-unknown-linux < %s | FileCheck %s --check-prefixes=CHECK,X86-64 %}
; RUN: %if aarch64-registered-target %{ opt -S --passes=slp-vectorizer -mtriple=aarch64-unknown-linux < %s | FileCheck %s --check-prefixes=CHECK,AARCH64 %}
...
; CHECK:       [[FOR_BODY]]:
; X86-64-NEXT:    [[TMP0:%.*]] = phi float [ 0.000000e+00, %[[ENTRY]] ], [ [[TMP9:%.*]], %[[FOR_BODY]] ]
...
; CHECK-NEXT:    [[TMP2:%.*]] = add nuw nsw i64 [[INDVARS_IV]], 4
; CHECK-NEXT:    [[ARRAYIDX31:%.*]] = getelementptr inbounds [32000 x float], ptr @a, i64 0, i64 [[TMP2]]
; X86-64-NEXT:    [[TMP3:%.*]] = load float, ptr [[ARRAYIDX31]], align 4
; X86-64-NEXT:    [[TMP4:%.*]] = load <4 x float>, ptr [[ARRAYIDX]], align 4
; X86-64-NEXT:    [[TMP5:%.*]] = shufflevector <4 x float> [[TMP4]], <4 x float> poison, <4 x i32> <i32 poison, i32 0, i32 1, i32 2>
; X86-64-NEXT:    [[TMP6:%.*]] = insertelement <4 x float> poison, float [[TMP0]], i64 0
; X86-64-NEXT:    [[TMP7:%.*]] = shufflevector <4 x float> [[TMP5]], <4 x float> [[TMP6]], <4 x i32> <i32 4, i32 1, i32 2, i32 3>
; X86-64-NEXT:    [[TMP8:%.*]] = fmul fast <4 x float> [[TMP7]], [[TMP4]]
; X86-64-NEXT:    store <4 x float> [[TMP8]], ptr [[ARRAYIDX6]], align 4
; X86-64-NEXT:    [[INDVARS_IV_NEXT]] = add nuw nsw i64 [[INDVARS_IV]], 5
; X86-64-NEXT:    [[ARRAYIDX41:%.*]] = getelementptr inbounds [32000 x float], ptr @a, i64 0, i64 [[INDVARS_IV_NEXT]]
; X86-64-NEXT:    [[TMP9]] = load float, ptr [[ARRAYIDX41]], align 4
; X86-64-NEXT:    [[MUL45:%.*]] = fmul fast float [[TMP9]], [[TMP3]]
; X86-64-NEXT:    store float [[MUL45]], ptr [[ARRAYIDX31]], align 4
