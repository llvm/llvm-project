...
; RUN: %if x86-registered-target %{ opt -S -passes=slp-vectorizer -slp-threshold=-10000 -slp-min-tree-size=0 -mtriple=x86_64-unknown-unknown < %s | FileCheck %s --check-prefixes=CHECK,MINTREE %}
...
; RUN: %if aarch64-registered-target %{ opt -S -passes=slp-vectorizer -slp-threshold=-10000 -slp-min-tree-size=0 -mtriple=aarch64-unknown-linux-gnu < %s | FileCheck %s --check-prefixes=CHECK,MINTREE %}
...
; FORCED-LABEL: @simple_select_eph(
; FORCED-NEXT:    [[C0:%.*]] = extractelement <4 x i32> [[C:%.*]], i32 0
; FORCED-NEXT:    [[C1:%.*]] = extractelement <4 x i32> [[C]], i32 1
; FORCED-NEXT:    [[C2:%.*]] = extractelement <4 x i32> [[C]], i32 2
; FORCED-NEXT:    [[C3:%.*]] = extractelement <4 x i32> [[C]], i32 3
; FORCED-NEXT:    [[A0:%.*]] = extractelement <4 x float> [[A:%.*]], i32 0
; FORCED-NEXT:    [[A1:%.*]] = extractelement <4 x float> [[A]], i32 1
; FORCED-NEXT:    [[A2:%.*]] = extractelement <4 x float> [[A]], i32 2
; FORCED-NEXT:    [[A3:%.*]] = extractelement <4 x float> [[A]], i32 3
; FORCED-NEXT:    [[B0:%.*]] = extractelement <4 x float> [[B:%.*]], i32 0
; FORCED-NEXT:    [[B1:%.*]] = extractelement <4 x float> [[B]], i32 1
; FORCED-NEXT:    [[B2:%.*]] = extractelement <4 x float> [[B]], i32 2
; FORCED-NEXT:    [[B3:%.*]] = extractelement <4 x float> [[B]], i32 3
; FORCED-NEXT:    [[CMP0:%.*]] = icmp ne i32 [[C0]], 0
; FORCED-NEXT:    [[CMP1:%.*]] = icmp ne i32 [[C1]], 0
; FORCED-NEXT:    [[CMP2:%.*]] = icmp ne i32 [[C2]], 0
; FORCED-NEXT:    [[CMP3:%.*]] = icmp ne i32 [[C3]], 0
; FORCED-NEXT:    [[S0:%.*]] = select i1 [[CMP0]], float [[A0]], float [[B0]]
; FORCED-NEXT:    [[S1:%.*]] = select i1 [[CMP1]], float [[A1]], float [[B1]]
; FORCED-NEXT:    [[S2:%.*]] = select i1 [[CMP2]], float [[A2]], float [[B2]]
; FORCED-NEXT:    [[S3:%.*]] = select i1 [[CMP3]], float [[A3]], float [[B3]]
; FORCED-NEXT:    [[RA:%.*]] = insertelement <4 x float> poison, float [[S0]], i32 0
; FORCED-NEXT:    [[RB:%.*]] = insertelement <4 x float> [[RA]], float [[S1]], i32 1
; FORCED-NEXT:    [[RC:%.*]] = insertelement <4 x float> [[RB]], float [[S2]], i32 2
; FORCED-NEXT:    [[RD:%.*]] = insertelement <4 x float> [[RC]], float [[S3]], i32 3
; FORCED-NEXT:    [[Q0:%.*]] = extractelement <4 x float> [[RD]], i32 0
; FORCED-NEXT:    [[Q1:%.*]] = extractelement <4 x float> [[RD]], i32 1
; FORCED-NEXT:    [[Q2:%.*]] = extractelement <4 x float> [[RD]], i32 2
; FORCED-NEXT:    [[Q3:%.*]] = extractelement <4 x float> [[RD]], i32 3
; FORCED-NEXT:    [[Q4:%.*]] = fadd float [[Q0]], [[Q1]]
; FORCED-NEXT:    [[Q5:%.*]] = fadd float [[Q2]], [[Q3]]
; FORCED-NEXT:    [[Q6:%.*]] = fadd float [[Q4]], [[Q5]]
; FORCED-NEXT:    [[QI:%.*]] = fcmp olt float [[Q6]], [[Q5]]
; FORCED-NEXT:    call void @llvm.assume(i1 [[QI]])
; FORCED-NEXT:    ret void
;
; COST-LABEL: @simple_select_eph(
; COST-NEXT:    [[C0:%.*]] = extractelement <4 x i32> [[C:%.*]], i32 0
; COST-NEXT:    [[C1:%.*]] = extractelement <4 x i32> [[C]], i32 1
; COST-NEXT:    [[C2:%.*]] = extractelement <4 x i32> [[C]], i32 2
; COST-NEXT:    [[C3:%.*]] = extractelement <4 x i32> [[C]], i32 3
; COST-NEXT:    [[A0:%.*]] = extractelement <4 x float> [[A:%.*]], i32 0
; COST-NEXT:    [[A1:%.*]] = extractelement <4 x float> [[A]], i32 1
; COST-NEXT:    [[A2:%.*]] = extractelement <4 x float> [[A]], i32 2
; COST-NEXT:    [[A3:%.*]] = extractelement <4 x float> [[A]], i32 3
; COST-NEXT:    [[B0:%.*]] = extractelement <4 x float> [[B:%.*]], i32 0
; COST-NEXT:    [[B1:%.*]] = extractelement <4 x float> [[B]], i32 1
; COST-NEXT:    [[B2:%.*]] = extractelement <4 x float> [[B]], i32 2
; COST-NEXT:    [[B3:%.*]] = extractelement <4 x float> [[B]], i32 3
; COST-NEXT:    [[CMP0:%.*]] = icmp ne i32 [[C0]], 0
; COST-NEXT:    [[CMP1:%.*]] = icmp ne i32 [[C1]], 0
; COST-NEXT:    [[CMP2:%.*]] = icmp ne i32 [[C2]], 0
; COST-NEXT:    [[CMP3:%.*]] = icmp ne i32 [[C3]], 0
; COST-NEXT:    [[S0:%.*]] = select i1 [[CMP0]], float [[A0]], float [[B0]]
; COST-NEXT:    [[S1:%.*]] = select i1 [[CMP1]], float [[A1]], float [[B1]]
; COST-NEXT:    [[S2:%.*]] = select i1 [[CMP2]], float [[A2]], float [[B2]]
; COST-NEXT:    [[S3:%.*]] = select i1 [[CMP3]], float [[A3]], float [[B3]]
; COST-NEXT:    [[RA:%.*]] = insertelement <4 x float> poison, float [[S0]], i32 0
; COST-NEXT:    [[RB:%.*]] = insertelement <4 x float> [[RA]], float [[S1]], i32 1
; COST-NEXT:    [[RC:%.*]] = insertelement <4 x float> [[RB]], float [[S2]], i32 2
; COST-NEXT:    [[RD:%.*]] = insertelement <4 x float> [[RC]], float [[S3]], i32 3
; COST-NEXT:    [[Q0:%.*]] = extractelement <4 x float> [[RD]], i32 0
; COST-NEXT:    [[Q1:%.*]] = extractelement <4 x float> [[RD]], i32 1
; COST-NEXT:    [[Q2:%.*]] = extractelement <4 x float> [[RD]], i32 2
; COST-NEXT:    [[Q3:%.*]] = extractelement <4 x float> [[RD]], i32 3
; COST-NEXT:    [[Q4:%.*]] = fadd float [[Q0]], [[Q1]]
; COST-NEXT:    [[Q5:%.*]] = fadd float [[Q2]], [[Q3]]
; COST-NEXT:    [[Q6:%.*]] = fadd float [[Q4]], [[Q5]]
; COST-NEXT:    [[QI:%.*]] = fcmp olt float [[Q6]], [[Q5]]
; COST-NEXT:    call void @llvm.assume(i1 [[QI]])
; COST-NEXT:    ret void
;
; MINTREE-LABEL: @simple_select_eph(
; MINTREE-NEXT:    [[C0:%.*]] = extractelement <4 x i32> [[C:%.*]], i32 0
; MINTREE-NEXT:    [[C1:%.*]] = extractelement <4 x i32> [[C]], i32 1
; MINTREE-NEXT:    [[C2:%.*]] = extractelement <4 x i32> [[C]], i32 2
; MINTREE-NEXT:    [[C3:%.*]] = extractelement <4 x i32> [[C]], i32 3
; MINTREE-NEXT:    [[A0:%.*]] = extractelement <4 x float> [[A:%.*]], i32 0
; MINTREE-NEXT:    [[A1:%.*]] = extractelement <4 x float> [[A]], i32 1
; MINTREE-NEXT:    [[A2:%.*]] = extractelement <4 x float> [[A]], i32 2
; MINTREE-NEXT:    [[A3:%.*]] = extractelement <4 x float> [[A]], i32 3
; MINTREE-NEXT:    [[B0:%.*]] = extractelement <4 x float> [[B:%.*]], i32 0
; MINTREE-NEXT:    [[B1:%.*]] = extractelement <4 x float> [[B]], i32 1
; MINTREE-NEXT:    [[B2:%.*]] = extractelement <4 x float> [[B]], i32 2
; MINTREE-NEXT:    [[B3:%.*]] = extractelement <4 x float> [[B]], i32 3
; MINTREE-NEXT:    [[CMP0:%.*]] = icmp ne i32 [[C0]], 0
; MINTREE-NEXT:    [[CMP1:%.*]] = icmp ne i32 [[C1]], 0
; MINTREE-NEXT:    [[CMP2:%.*]] = icmp ne i32 [[C2]], 0
; MINTREE-NEXT:    [[CMP3:%.*]] = icmp ne i32 [[C3]], 0
; MINTREE-NEXT:    [[S0:%.*]] = select i1 [[CMP0]], float [[A0]], float [[B0]]
; MINTREE-NEXT:    [[S1:%.*]] = select i1 [[CMP1]], float [[A1]], float [[B1]]
; MINTREE-NEXT:    [[S2:%.*]] = select i1 [[CMP2]], float [[A2]], float [[B2]]
; MINTREE-NEXT:    [[S3:%.*]] = select i1 [[CMP3]], float [[A3]], float [[B3]]
; MINTREE-NEXT:    [[RA:%.*]] = insertelement <4 x float> poison, float [[S0]], i32 0
; MINTREE-NEXT:    [[RB:%.*]] = insertelement <4 x float> [[RA]], float [[S1]], i32 1
; MINTREE-NEXT:    [[RC:%.*]] = insertelement <4 x float> [[RB]], float [[S2]], i32 2
; MINTREE-NEXT:    [[RD:%.*]] = insertelement <4 x float> [[RC]], float [[S3]], i32 3
; MINTREE-NEXT:    [[Q2:%.*]] = extractelement <4 x float> [[RD]], i32 2
; MINTREE-NEXT:    [[Q3:%.*]] = extractelement <4 x float> [[RD]], i32 3
; MINTREE-NEXT:    [[Q5:%.*]] = fadd float [[Q2]], [[Q3]]
; MINTREE-NEXT:    [[TMP1:%.*]] = shufflevector <4 x float> [[RD]], <4 x float> poison, <3 x i32> <i32 0, i32 1, i32 poison>
; MINTREE-NEXT:    [[TMP2:%.*]] = insertelement <3 x float> [[TMP1]], float [[Q5]], i64 2
; MINTREE-NEXT:    [[TMP3:%.*]] = call float @llvm.vector.reduce.fadd.v3f32(float -0.000000e+00, <3 x float> [[TMP2]])
; MINTREE-NEXT:    [[QI:%.*]] = fcmp olt float [[TMP3]], [[Q5]]
; MINTREE-NEXT:    call void @llvm.assume(i1 [[QI]])
; MINTREE-NEXT:    ret void
;
  %c0 = extractelement <4 x i32> %c, i32 0
  %c1 = extractelement <4 x i32> %c, i32 1
  %c2 = extractelement <4 x i32> %c, i32 2
...
;
; MINTREE-LABEL: @simple_select_partial_vector(
; MINTREE-NEXT:    [[C0:%.*]] = extractelement <4 x i32> [[C:%.*]], i32 0
; MINTREE-NEXT:    [[C1:%.*]] = extractelement <4 x i32> [[C]], i32 1
; MINTREE-NEXT:    [[A0:%.*]] = extractelement <4 x float> [[A:%.*]], i32 0
; MINTREE-NEXT:    [[A1:%.*]] = extractelement <4 x float> [[A]], i32 1
; MINTREE-NEXT:    [[B0:%.*]] = extractelement <4 x float> [[B:%.*]], i32 0
; MINTREE-NEXT:    [[B1:%.*]] = extractelement <4 x float> [[B]], i32 1
; MINTREE-NEXT:    [[TMP1:%.*]] = insertelement <2 x i32> poison, i32 [[C0]], i32 0
; MINTREE-NEXT:    [[TMP2:%.*]] = insertelement <2 x i32> [[TMP1]], i32 [[C1]], i32 1
; MINTREE-NEXT:    [[TMP3:%.*]] = icmp ne <2 x i32> [[TMP2]], zeroinitializer
; MINTREE-NEXT:    [[TMP4:%.*]] = insertelement <2 x float> poison, float [[A0]], i32 0
; MINTREE-NEXT:    [[TMP5:%.*]] = insertelement <2 x float> [[TMP4]], float [[A1]], i32 1
; MINTREE-NEXT:    [[TMP6:%.*]] = insertelement <2 x float> poison, float [[B0]], i32 0
; MINTREE-NEXT:    [[TMP7:%.*]] = insertelement <2 x float> [[TMP6]], float [[B1]], i32 1
; MINTREE-NEXT:    [[TMP8:%.*]] = select <2 x i1> [[TMP3]], <2 x float> [[TMP5]], <2 x float> [[TMP7]]
; MINTREE-NEXT:    [[TMP9:%.*]] = extractelement <2 x float> [[TMP8]], i32 0
; MINTREE-NEXT:    [[RA:%.*]] = insertelement <4 x float> poison, float [[TMP9]], i32 0
; MINTREE-NEXT:    [[TMP10:%.*]] = extractelement <2 x float> [[TMP8]], i32 1
; MINTREE-NEXT:    [[RB:%.*]] = insertelement <4 x float> [[RA]], float [[TMP10]], i32 1
; MINTREE-NEXT:    ret <4 x float> [[RB]]
;
  %c0 = extractelement <4 x i32> %c, i32 0
  %c1 = extractelement <4 x i32> %c, i32 1
  %a0 = extractelement <4 x float> %a, i32 0
