...
; RUN: %if x86-registered-target %{ opt -S -passes=slp-vectorizer -slp-threshold=-10000 -mtriple=x86_64-unknown-unknown < %s | FileCheck %s --check-prefixes=CHECK,X86-DEFAULT %}
; RUN: %if x86-registered-target %{ opt -S -passes=slp-vectorizer -slp-threshold=0 -mtriple=x86_64-unknown-unknown < %s | FileCheck %s --check-prefixes=CHECK,X86-DEFAULT %}
; RUN: %if x86-registered-target %{ opt -S -passes=slp-vectorizer -slp-threshold=-10000 -slp-min-tree-size=0 -mtriple=x86_64-unknown-unknown < %s | FileCheck %s --check-prefixes=CHECK,X86-MIN %}
; RUN: %if aarch64-registered-target %{ opt -S -passes=slp-vectorizer -slp-threshold=-10000 -mtriple=aarch64-unknown-linux-gnu < %s | FileCheck %s --check-prefixes=CHECK,AARCH64 %}
...
; RUN: %if aarch64-registered-target %{ opt -S -passes=slp-vectorizer -slp-threshold=-10000 -slp-min-tree-size=0 -mtriple=aarch64-unknown-linux-gnu < %s | FileCheck %s --check-prefixes=CHECK,AARCH64 %}
...
; X86-DEFAULT-LABEL: @simple_select_eph(
; X86-DEFAULT-NEXT:    [[C0:%.*]] = extractelement <4 x i32> [[C:%.*]], i32 0
; X86-DEFAULT-NEXT:    [[C1:%.*]] = extractelement <4 x i32> [[C]], i32 1
; X86-DEFAULT-NEXT:    [[C2:%.*]] = extractelement <4 x i32> [[C]], i32 2
; X86-DEFAULT-NEXT:    [[C3:%.*]] = extractelement <4 x i32> [[C]], i32 3
; X86-DEFAULT-NEXT:    [[A0:%.*]] = extractelement <4 x float> [[A:%.*]], i32 0
; X86-DEFAULT-NEXT:    [[A1:%.*]] = extractelement <4 x float> [[A]], i32 1
; X86-DEFAULT-NEXT:    [[A2:%.*]] = extractelement <4 x float> [[A]], i32 2
; X86-DEFAULT-NEXT:    [[A3:%.*]] = extractelement <4 x float> [[A]], i32 3
; X86-DEFAULT-NEXT:    [[B0:%.*]] = extractelement <4 x float> [[B:%.*]], i32 0
; X86-DEFAULT-NEXT:    [[B1:%.*]] = extractelement <4 x float> [[B]], i32 1
; X86-DEFAULT-NEXT:    [[B2:%.*]] = extractelement <4 x float> [[B]], i32 2
; X86-DEFAULT-NEXT:    [[B3:%.*]] = extractelement <4 x float> [[B]], i32 3
; X86-DEFAULT-NEXT:    [[CMP0:%.*]] = icmp ne i32 [[C0]], 0
; X86-DEFAULT-NEXT:    [[CMP1:%.*]] = icmp ne i32 [[C1]], 0
; X86-DEFAULT-NEXT:    [[CMP2:%.*]] = icmp ne i32 [[C2]], 0
; X86-DEFAULT-NEXT:    [[CMP3:%.*]] = icmp ne i32 [[C3]], 0
; X86-DEFAULT-NEXT:    [[S0:%.*]] = select i1 [[CMP0]], float [[A0]], float [[B0]]
; X86-DEFAULT-NEXT:    [[S1:%.*]] = select i1 [[CMP1]], float [[A1]], float [[B1]]
; X86-DEFAULT-NEXT:    [[S2:%.*]] = select i1 [[CMP2]], float [[A2]], float [[B2]]
; X86-DEFAULT-NEXT:    [[S3:%.*]] = select i1 [[CMP3]], float [[A3]], float [[B3]]
; X86-DEFAULT-NEXT:    [[RA:%.*]] = insertelement <4 x float> poison, float [[S0]], i32 0
; X86-DEFAULT-NEXT:    [[RB:%.*]] = insertelement <4 x float> [[RA]], float [[S1]], i32 1
; X86-DEFAULT-NEXT:    [[RC:%.*]] = insertelement <4 x float> [[RB]], float [[S2]], i32 2
; X86-DEFAULT-NEXT:    [[RD:%.*]] = insertelement <4 x float> [[RC]], float [[S3]], i32 3
; X86-DEFAULT-NEXT:    [[TMP1:%.*]] = shufflevector <4 x float> [[RD]], <4 x float> poison, <2 x i32> <i32 0, i32 1>
; X86-DEFAULT-NEXT:    [[TMP2:%.*]] = call float @llvm.vector.reduce.fadd.v2f32(float -0.000000e+00, <2 x float> [[TMP1]])
; X86-DEFAULT-NEXT:    [[TMP3:%.*]] = shufflevector <4 x float> [[RD]], <4 x float> poison, <2 x i32> <i32 2, i32 3>
; X86-DEFAULT-NEXT:    [[TMP4:%.*]] = call float @llvm.vector.reduce.fadd.v2f32(float -0.000000e+00, <2 x float> [[TMP3]])
; X86-DEFAULT-NEXT:    [[Q6:%.*]] = fadd float [[TMP2]], [[TMP4]]
; X86-DEFAULT-NEXT:    [[QI:%.*]] = fcmp olt float [[Q6]], [[TMP4]]
; X86-DEFAULT-NEXT:    call void @llvm.assume(i1 [[QI]])
; X86-DEFAULT-NEXT:    ret <4 x float> undef
;
; X86-MIN-LABEL: @simple_select_eph(
; X86-MIN-NEXT:    [[C0:%.*]] = extractelement <4 x i32> [[C:%.*]], i32 0
; X86-MIN-NEXT:    [[C1:%.*]] = extractelement <4 x i32> [[C]], i32 1
; X86-MIN-NEXT:    [[C2:%.*]] = extractelement <4 x i32> [[C]], i32 2
; X86-MIN-NEXT:    [[C3:%.*]] = extractelement <4 x i32> [[C]], i32 3
; X86-MIN-NEXT:    [[A0:%.*]] = extractelement <4 x float> [[A:%.*]], i32 0
; X86-MIN-NEXT:    [[A1:%.*]] = extractelement <4 x float> [[A]], i32 1
; X86-MIN-NEXT:    [[A2:%.*]] = extractelement <4 x float> [[A]], i32 2
; X86-MIN-NEXT:    [[A3:%.*]] = extractelement <4 x float> [[A]], i32 3
; X86-MIN-NEXT:    [[B0:%.*]] = extractelement <4 x float> [[B:%.*]], i32 0
; X86-MIN-NEXT:    [[B1:%.*]] = extractelement <4 x float> [[B]], i32 1
; X86-MIN-NEXT:    [[B2:%.*]] = extractelement <4 x float> [[B]], i32 2
; X86-MIN-NEXT:    [[B3:%.*]] = extractelement <4 x float> [[B]], i32 3
; X86-MIN-NEXT:    [[CMP0:%.*]] = icmp ne i32 [[C0]], 0
; X86-MIN-NEXT:    [[CMP1:%.*]] = icmp ne i32 [[C1]], 0
; X86-MIN-NEXT:    [[CMP2:%.*]] = icmp ne i32 [[C2]], 0
; X86-MIN-NEXT:    [[CMP3:%.*]] = icmp ne i32 [[C3]], 0
; X86-MIN-NEXT:    [[S0:%.*]] = select i1 [[CMP0]], float [[A0]], float [[B0]]
; X86-MIN-NEXT:    [[S1:%.*]] = select i1 [[CMP1]], float [[A1]], float [[B1]]
; X86-MIN-NEXT:    [[S2:%.*]] = select i1 [[CMP2]], float [[A2]], float [[B2]]
; X86-MIN-NEXT:    [[S3:%.*]] = select i1 [[CMP3]], float [[A3]], float [[B3]]
; X86-MIN-NEXT:    [[RA:%.*]] = insertelement <4 x float> poison, float [[S0]], i32 0
; X86-MIN-NEXT:    [[RB:%.*]] = insertelement <4 x float> [[RA]], float [[S1]], i32 1
; X86-MIN-NEXT:    [[RC:%.*]] = insertelement <4 x float> [[RB]], float [[S2]], i32 2
; X86-MIN-NEXT:    [[RD:%.*]] = insertelement <4 x float> [[RC]], float [[S3]], i32 3
; X86-MIN-NEXT:    [[TMP1:%.*]] = shufflevector <4 x float> [[RD]], <4 x float> poison, <2 x i32> <i32 0, i32 1>
; X86-MIN-NEXT:    [[TMP2:%.*]] = call float @llvm.vector.reduce.fadd.v2f32(float -0.000000e+00, <2 x float> [[TMP1]])
; X86-MIN-NEXT:    [[TMP3:%.*]] = shufflevector <4 x float> [[RD]], <4 x float> poison, <2 x i32> <i32 2, i32 3>
; X86-MIN-NEXT:    [[TMP4:%.*]] = call float @llvm.vector.reduce.fadd.v2f32(float -0.000000e+00, <2 x float> [[TMP3]])
; X86-MIN-NEXT:    [[TMP5:%.*]] = insertelement <2 x float> poison, float [[TMP2]], i32 0
; X86-MIN-NEXT:    [[TMP6:%.*]] = insertelement <2 x float> [[TMP5]], float [[TMP4]], i32 1
; X86-MIN-NEXT:    [[TMP7:%.*]] = call float @llvm.vector.reduce.fadd.v2f32(float -0.000000e+00, <2 x float> [[TMP6]])
; X86-MIN-NEXT:    [[QI:%.*]] = fcmp olt float [[TMP7]], [[TMP4]]
; X86-MIN-NEXT:    call void @llvm.assume(i1 [[QI]])
; X86-MIN-NEXT:    ret <4 x float> undef
;
