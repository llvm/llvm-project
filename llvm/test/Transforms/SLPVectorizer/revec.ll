...
; RUN: opt -passes=slp-vectorizer -S -slp-revec -slp-max-reg-size=1024 -slp-threshold=-100 -slp-vectorize-non-power-of-2 %s | FileCheck %s
...
; CHECK-LABEL: @hor_reduction_four_points_non_pow2(
; CHECK-NEXT:  entry:
; CHECK-NEXT:    [[TMP0:%.*]] = load <12 x i32>, ptr [[A:%.*]], align 16
; CHECK-NEXT:    [[TMP1:%.*]] = shufflevector <12 x i32> [[TMP0]], <12 x i32> poison, <4 x i32> <i32 0, i32 1, i32 2, i32 3>
; CHECK-NEXT:    [[TMP2:%.*]] = shufflevector <12 x i32> [[TMP0]], <12 x i32> poison, <4 x i32> <i32 4, i32 5, i32 6, i32 7>
; CHECK-NEXT:    [[RDX_OP:%.*]] = add <4 x i32> [[TMP1]], [[TMP2]]
; CHECK-NEXT:    [[TMP3:%.*]] = shufflevector <12 x i32> [[TMP0]], <12 x i32> poison, <4 x i32> <i32 8, i32 9, i32 10, i32 11>
; CHECK-NEXT:    [[RDX_OP1:%.*]] = add <4 x i32> [[RDX_OP]], [[TMP3]]
; CHECK-NEXT:    [[OP_RDX:%.*]] = add <4 x i32> [[RDX_OP1]], <i32 0, i32 1, i32 2, i32 3>
; CHECK-NEXT:    ret <4 x i32> [[OP_RDX]]
;
entry:
  %gep1 = getelementptr <4 x i32>, ptr %a, i64 1
