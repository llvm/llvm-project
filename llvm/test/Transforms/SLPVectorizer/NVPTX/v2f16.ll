...
; RUN: opt < %s -passes=slp-vectorizer -S -mtriple=nvptx64-nvidia-cuda -mcpu=sm_90 | FileCheck %s -check-prefixes=VECTOR,BF16
; RUN: opt < %s -passes=slp-vectorizer -S -mtriple=nvptx64-nvidia-cuda -mcpu=sm_80 | FileCheck %s -check-prefixes=VECTOR,BF16
...
define i32 @mul_bf16_splat(<2 x bfloat> %x, bfloat %scale) {
; BF16-LABEL: @mul_bf16_splat(
; BF16-NEXT:    [[TMP1:%.*]] = insertelement <2 x bfloat> poison, bfloat [[SCALE:%.*]], i64 0
; BF16-NEXT:    [[TMP2:%.*]] = shufflevector <2 x bfloat> [[TMP1]], <2 x bfloat> poison, <2 x i32> zeroinitializer
; BF16-NEXT:    [[TMP3:%.*]] = fmul <2 x bfloat> [[X:%.*]], [[TMP2]]
; BF16-NEXT:    [[RESULT:%.*]] = bitcast <2 x bfloat> [[TMP3]] to i32
; BF16-NEXT:    ret i32 [[RESULT]]
;
  %x0 = extractelement <2 x bfloat> %x, i64 0
  %x1 = extractelement <2 x bfloat> %x, i64 1
  %m0 = fmul bfloat %x0, %scale
  %m1 = fmul bfloat %x1, %scale
  %r0 = insertelement <2 x bfloat> poison, bfloat %m0, i64 0
  %r1 = insertelement <2 x bfloat> %r0, bfloat %m1, i64 1
  %result = bitcast <2 x bfloat> %r1 to i32
  ret i32 %result
}
