...
; CHECK-NEXT:    [[GEP4:%.*]] = getelementptr inbounds float, ptr [[A:%.*]], i64 4
; CHECK-NEXT:    [[LD4:%.*]] = load float, ptr [[GEP4]], align 4
; CHECK-NEXT:    [[TMP1:%.*]] = load <4 x float>, ptr [[A]], align 4
; CHECK-NEXT:    [[TMP2:%.*]] = insertelement <4 x float> [[TMP1]], float [[LD4]], i32 1
; CHECK-NEXT:    [[TMP3:%.*]] = shufflevector <4 x float> [[TMP1]], <4 x float> poison, <4 x i32> <i32 1, i32 3, i32 1, i32 2>
; CHECK-NEXT:    [[TMP4:%.*]] = fmul fast <4 x float> [[TMP2]], [[TMP3]]
; CHECK-NEXT:    [[TMP5:%.*]] = shufflevector <4 x float> [[TMP4]], <4 x float> poison, <4 x i32> <i32 0, i32 2, i32 3, i32 1>
