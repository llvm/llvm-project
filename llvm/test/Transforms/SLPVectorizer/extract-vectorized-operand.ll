...
; CHECK-NEXT:  [[BB:.*]]:
; CHECK-NEXT:    [[TMP0:%.*]] = shufflevector <2 x ptr addrspace(1)> splat (ptr addrspace(1) null), <2 x ptr addrspace(1)> splat (ptr addrspace(1) null), <2 x i32> <i32 1, i32 0>
; CHECK-NEXT:    [[TMP1:%.*]] = extractelement <2 x ptr addrspace(1)> [[TMP0]], i32 0
; CHECK-NEXT:    br label %[[BB43:.*]]
; CHECK:       [[BB20:.*]]:
; CHECK-NEXT:    br label %[[BB105:.*]]
; CHECK:       [[BB43]]:
; CHECK-NEXT:    [[TMP2:%.*]] = phi <2 x ptr addrspace(1)> [ [[TMP3:%.*]], %[[BB51:.*]] ], [ splat (ptr addrspace(1) null), %[[BB]] ]
; CHECK-NEXT:    br i1 false, label %[[BB105]], label %[[BB51]]
; CHECK:       [[BB51]]:
; CHECK-NEXT:    [[TMP3]] = phi <2 x ptr addrspace(1)> [ poison, %[[BB54:.*]] ], [ splat (ptr addrspace(1) null), %[[BB43]] ]
; CHECK-NEXT:    br label %[[BB43]]
; CHECK:       [[BB54]]:
