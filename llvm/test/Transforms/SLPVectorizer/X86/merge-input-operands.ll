...
; The merged scalars s0..s3 are also read by a sibling bundle inside the same
; tree. That bundle matches the merged operand node as a whole, and its lanes
; refer to the merged sums themselves, so it must read the <4 x float> add and
; not the reuse shuffle that feeds the fmul.
define void @merged_scalars_reused_by_sibling(ptr %A, ptr %Out) {
; MERGE-LABEL: @merged_scalars_reused_by_sibling(
; MERGE-NEXT:  entry:
; MERGE-NEXT:    [[P4:%.*]] = getelementptr float, ptr [[A:%.*]], i64 4
; MERGE-NEXT:    [[TMP0:%.*]] = load <4 x float>, ptr [[A]], align 4
; MERGE-NEXT:    [[TMP1:%.*]] = load <4 x float>, ptr [[P4]], align 4
...
; MERGE-NEXT:    [[TMP4:%.*]] = shufflevector <4 x float> [[TMP2]], <4 x float> poison, <4 x i32> <i32 2, i32 3, i32 3, i32 2>
; MERGE-NEXT:    [[TMP5:%.*]] = fmul <4 x float> [[TMP3]], [[TMP4]]
; MERGE-NEXT:    [[TMP6:%.*]] = fadd <4 x float> [[TMP5]], [[TMP2]]
; MERGE-NEXT:    store <4 x float> [[TMP6]], ptr [[OUT:%.*]], align 4
; MERGE-NEXT:    ret void
;
; NOMERGE-LABEL: @merged_scalars_reused_by_sibling(
; NOMERGE-NEXT:  entry:
; NOMERGE-NEXT:    [[P2:%.*]] = getelementptr float, ptr [[A:%.*]], i64 2
; NOMERGE-NEXT:    [[P4:%.*]] = getelementptr float, ptr [[A]], i64 4
; NOMERGE-NEXT:    [[P6:%.*]] = getelementptr float, ptr [[A]], i64 6
; NOMERGE-NEXT:    [[TMP0:%.*]] = load <2 x float>, ptr [[A]], align 4
; NOMERGE-NEXT:    [[TMP1:%.*]] = load <2 x float>, ptr [[P2]], align 4
; NOMERGE-NEXT:    [[TMP2:%.*]] = load <4 x float>, ptr [[A]], align 4
; NOMERGE-NEXT:    [[TMP3:%.*]] = load <2 x float>, ptr [[P4]], align 4
; NOMERGE-NEXT:    [[TMP4:%.*]] = load <2 x float>, ptr [[P6]], align 4
; NOMERGE-NEXT:    [[TMP5:%.*]] = load <4 x float>, ptr [[P4]], align 4
; NOMERGE-NEXT:    [[TMP6:%.*]] = fadd <2 x float> [[TMP0]], [[TMP3]]
; NOMERGE-NEXT:    [[TMP7:%.*]] = shufflevector <2 x float> [[TMP6]], <2 x float> poison, <4 x i32> <i32 0, i32 1, i32 0, i32 1>
; NOMERGE-NEXT:    [[TMP8:%.*]] = fadd <2 x float> [[TMP1]], [[TMP4]]
; NOMERGE-NEXT:    [[TMP9:%.*]] = shufflevector <2 x float> [[TMP8]], <2 x float> poison, <4 x i32> <i32 0, i32 1, i32 1, i32 0>
; NOMERGE-NEXT:    [[TMP10:%.*]] = fadd <4 x float> [[TMP2]], [[TMP5]]
; NOMERGE-NEXT:    [[TMP11:%.*]] = fmul <4 x float> [[TMP7]], [[TMP9]]
; NOMERGE-NEXT:    [[TMP12:%.*]] = fadd <4 x float> [[TMP11]], [[TMP10]]
; NOMERGE-NEXT:    store <4 x float> [[TMP12]], ptr [[OUT:%.*]], align 4
; NOMERGE-NEXT:    ret void
;
entry:
  %a0 = load float, ptr %A
  ...
  store float %u0, ptr %Out
  %q1 = getelementptr float, ptr %Out, i64 1
  store float %u1, ptr %q1
  %q2 = getelementptr float, ptr %Out, i64 2
  store float %u2, ptr %q2
  %q3 = getelementptr float, ptr %Out, i64 3
  store float %u3, ptr %q3
  ret void
}

; Same, but the sibling bundle only reads part of the merged scalars (s2 and s3)
; and fills the remaining lanes with constants, so it is a partial gather rather
; than a whole-node match.
define void @merged_scalars_partially_reused(ptr %A, ptr %Out) {
; MERGE-LABEL: @merged_scalars_partially_reused(
; MERGE-NEXT:  entry:
; MERGE-NEXT:    [[P4:%.*]] = getelementptr float, ptr [[A:%.*]], i64 4
; MERGE-NEXT:    [[TMP0:%.*]] = load <4 x float>, ptr [[A]], align 4
; MERGE-NEXT:    [[TMP1:%.*]] = load <4 x float>, ptr [[P4]], align 4
...
; MERGE-NEXT:    [[TMP4:%.*]] = shufflevector <4 x float> [[TMP2]], <4 x float> poison, <4 x i32> <i32 2, i32 3, i32 3, i32 2>
; MERGE-NEXT:    [[TMP5:%.*]] = fmul <4 x float> [[TMP3]], [[TMP4]]
; MERGE-NEXT:    [[TMP6:%.*]] = shufflevector <4 x float> <float poison, float poison, float 1.000000e+00, float 2.000000e+00>, <4 x float> [[TMP2]], <4 x i32> <i32 6, i32 7, i32 2, i32 3>
; MERGE-NEXT:    [[TMP7:%.*]] = fadd <4 x float> [[TMP5]], [[TMP6]]
; MERGE-NEXT:    store <4 x float> [[TMP7]], ptr [[OUT:%.*]], align 4
; MERGE-NEXT:    ret void
;
; NOMERGE-LABEL: @merged_scalars_partially_reused(
; NOMERGE-NEXT:  entry:
; NOMERGE-NEXT:    [[P2:%.*]] = getelementptr float, ptr [[A:%.*]], i64 2
; NOMERGE-NEXT:    [[P4:%.*]] = getelementptr float, ptr [[A]], i64 4
; NOMERGE-NEXT:    [[P6:%.*]] = getelementptr float, ptr [[A]], i64 6
...
; NOMERGE-NEXT:    [[TMP7:%.*]] = shufflevector <2 x float> [[TMP6]], <2 x float> poison, <4 x i32> <i32 0, i32 1, i32 1, i32 0>
; NOMERGE-NEXT:    [[TMP8:%.*]] = fmul <4 x float> [[TMP5]], [[TMP7]]
; NOMERGE-NEXT:    [[TMP9:%.*]] = shufflevector <4 x float> [[TMP7]], <4 x float> <float poison, float poison, float 1.000000e+00, float 2.000000e+00>, <4 x i32> <i32 0, i32 1, i32 6, i32 7>
; NOMERGE-NEXT:    [[TMP10:%.*]] = fadd <4 x float> [[TMP8]], [[TMP9]]
; NOMERGE-NEXT:    store <4 x float> [[TMP10]], ptr [[OUT:%.*]], align 4
; NOMERGE-NEXT:    ret void
;
entry:
  %a0 = load float, ptr %A
  ...
  %u0 = fadd float %m0, %s2
  %u1 = fadd float %m1, %s3
  %u2 = fadd float %m2, 1.0
  %u3 = fadd float %m3, 2.0
