...
; RUN: opt < %s -slp-threshold=-10 -passes=slp-vectorizer -mattr=+sse2     -S | FileCheck %s --check-prefix=SSE
; RUN: opt < %s -slp-threshold=-10 -passes=slp-vectorizer -mattr=+avx      -S | FileCheck %s --check-prefix=AVX
; RUN: opt < %s -slp-threshold=-10 -passes=slp-vectorizer -mattr=+avx2     -S | FileCheck %s --check-prefix=AVX
; RUN: opt < %s -slp-threshold=-10 -passes=slp-vectorizer -mattr=+avx512f  -S | FileCheck %s --check-prefix=AVX
; RUN: opt < %s -slp-threshold=-10 -passes=slp-vectorizer -mattr=+avx512vl -S | FileCheck %s --check-prefix=AVX
...
; SSE-NEXT:    [[TMP5:%.*]] = extractelement <2 x i8> [[TMP2]], i64 0
; SSE-NEXT:    [[TMP6:%.*]] = zext i8 [[TMP5]] to i32
; SSE-NEXT:    [[T4:%.*]] = getelementptr inbounds i8, ptr [[PTR:%.*]], i32 [[TMP6]]
; SSE-NEXT:    [[TMP7:%.*]] = extractelement <2 x i8> [[TMP2]], i64 1
; SSE-NEXT:    [[T3:%.*]] = zext i8 [[TMP7]] to i32
...
; SSE-NEXT:    [[T4:%.*]] = getelementptr inbounds i8, ptr [[PTR:%.*]], i32 [[TMP6]]
; SSE-NEXT:    [[TMP7:%.*]] = extractelement <2 x i8> [[TMP2]], i64 1
; SSE-NEXT:    [[T3:%.*]] = sext i8 [[TMP7]] to i32
; SSE-NEXT:    [[T5:%.*]] = getelementptr inbounds i8, ptr [[PTR]], i32 [[T3]]
; SSE-NEXT:    [[T6:%.*]] = load i8, ptr [[T4]], align 1
; SSE-NEXT:    [[T7:%.*]] = load i8, ptr [[T5]], align 1
; SSE-NEXT:    [[T8:%.*]] = add i8 [[T6]], [[T7]]
