...
; Only operand 0 of the fadd/fsub is checked for a one-use fmul, so an fmul on
; operand 1 gets no scalar fusion credit. To be addressed in a follow-up.

; Both fmuls are operand 1. The chain vectorizes into a 4-wide reduction and the
; vector fmul still fuses to an fma.
...
; CHECK-NEXT:    [[TMP5:%.*]] = shufflevector <2 x double> [[TMP2]], <2 x double> poison, <4 x i32> <i32 0, i32 1, i32 poison, i32 poison>
; CHECK-NEXT:    [[TMP6:%.*]] = shufflevector <2 x double> [[TMP3]], <2 x double> [[TMP2]], <4 x i32> <i32 0, i32 1, i32 2, i32 3>
; CHECK-NEXT:    [[ADD1:%.*]] = call reassoc nsz contract double @llvm.vector.reduce.fadd.v4f64(double 0.000000e+00, <4 x double> [[TMP6]])
...
; Same on the subtrahend of an fsub, which fuses to an fnmadd.
