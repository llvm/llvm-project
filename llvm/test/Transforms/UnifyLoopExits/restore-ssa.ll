...
; CHECK-NEXT:            to label [[B:%.*]] [label [[A_TARGET_RETURN:%.*]]]
; CHECK:       B:
; CHECK-NEXT:    [[MYTMP41:%.*]] = load i32, ptr [[ARG2:%.*]], align 4
; CHECK-NEXT:    [[CMP:%.*]] = icmp slt i32 [[MYTMP41]], 0
; CHECK-NEXT:    callbr void asm "", "r,!i"(i1 [[CMP]])
; CHECK-NEXT:            to label [[A]] [label [[B_TARGET_C:%.*]]]
; CHECK:       C:
; CHECK-NEXT:    [[INC:%.*]] = add i32 [[MYTMP41_MOVED:%.*]], 1
; CHECK-NEXT:    callbr void asm "", ""()
...
define i32 @exiting-used-in-exit_switch(ptr %arg1, ptr %arg2, i1 %Pred) local_unnamed_addr align 2 {
; CHECK-LABEL: @exiting-used-in-exit_switch(
; CHECK-NEXT:  entry:
; CHECK-NEXT:    switch i1 [[PRED:%.*]], label [[A:%.*]] [
; CHECK-NEXT:    ]
; CHECK:       A:
; CHECK-NEXT:    [[MYTMP42:%.*]] = load i32, ptr [[ARG1:%.*]], align 4
; CHECK-NEXT:    [[CMP1:%.*]] = icmp slt i32 [[MYTMP42]], 0
; CHECK-NEXT:    switch i1 [[CMP1]], label [[B:%.*]] [
; CHECK-NEXT:      i1 false, label [[A_TARGET_RETURN:%.*]]
; CHECK-NEXT:    ]
; CHECK:       B:
; CHECK-NEXT:    [[MYTMP41:%.*]] = load i32, ptr [[ARG2:%.*]], align 4
; CHECK-NEXT:    [[CMP:%.*]] = icmp slt i32 [[MYTMP41]], 0
; CHECK-NEXT:    switch i1 [[CMP]], label [[A]] [
; CHECK-NEXT:      i1 false, label [[B_TARGET_C:%.*]]
; CHECK-NEXT:    ]
; CHECK:       C:
; CHECK-NEXT:    [[INC:%.*]] = add i32 [[MYTMP41_MOVED:%.*]], 1
; CHECK-NEXT:    switch i1 [[PRED]], label [[RETURN:%.*]] [
; CHECK-NEXT:    ]
; CHECK:       return:
; CHECK-NEXT:    [[PHI:%.*]] = phi i32 [ [[INC]], [[C:%.*]] ], [ [[PHI_MOVED:%.*]], [[LOOP_EXIT_GUARD:%.*]] ]
; CHECK-NEXT:    ret i32 [[PHI]]
...
;
entry:
  switch i1 %Pred, label %A []

A:
  %mytmp42 = load i32, ptr %arg1, align 4
  %cmp1 = icmp slt i32 %mytmp42, 0
  switch i1 %cmp1, label %B [ i1 0, label %return ]

B:
  %mytmp41 = load i32, ptr %arg2, align 4
  %cmp = icmp slt i32 %mytmp41, 0
  switch i1 %cmp, label %A [ i1 0, label %C ]

C:
  %inc = add i32 %mytmp41, 1
  switch i1 %Pred, label %return []
...
; CHECK-NEXT:    [[CMP:%.*]] = icmp slt i32 [[MYTMP42]], 0
; CHECK-NEXT:    callbr void asm "", "r,!i"(i1 [[CMP]])
; CHECK-NEXT:            to label [[A]] [label [[C_TARGET_D:%.*]]]
; CHECK:       D:
; CHECK-NEXT:    [[INC:%.*]] = add i32 [[MYTMP41_MOVED:%.*]], 1
; CHECK-NEXT:    callbr void asm "", ""()
; CHECK-NEXT:            to label [[RETURN:%.*]] []
...
define i32 @internal-used-in-exit_switch(ptr %arg1, ptr %arg2, i1 %Pred) local_unnamed_addr align 2 {
; CHECK-LABEL: @internal-used-in-exit_switch(
; CHECK-NEXT:  entry:
; CHECK-NEXT:    [[MYTMP42:%.*]] = load i32, ptr [[ARG1:%.*]], align 4
; CHECK-NEXT:    switch i1 [[PRED:%.*]], label [[A:%.*]] [
; CHECK-NEXT:    ]
...
; CHECK:       B:
; CHECK-NEXT:    [[MYTMP41:%.*]] = load i32, ptr [[ARG2:%.*]], align 4
; CHECK-NEXT:    switch i1 [[PRED]], label [[C:%.*]] [
; CHECK-NEXT:    ]
; CHECK:       C:
; CHECK-NEXT:    [[CMP:%.*]] = icmp slt i32 [[MYTMP42]], 0
; CHECK-NEXT:    switch i1 [[CMP]], label [[A]] [
; CHECK-NEXT:      i1 false, label [[C_TARGET_D:%.*]]
; CHECK-NEXT:    ]
; CHECK:       D:
; CHECK-NEXT:    [[INC:%.*]] = add i32 [[MYTMP41_MOVED:%.*]], 1
; CHECK-NEXT:    switch i1 [[PRED]], label [[RETURN:%.*]] [
...
B:
  %mytmp41 = load i32, ptr %arg2, align 4
  switch i1 %Pred, label %C []

C:
  %cmp = icmp slt i32 %mytmp42, 0
  switch i1 %cmp, label %A [ i1 0, label %D ]
...
define i32 @mixed-use-in-exit_switch(ptr %arg1, ptr %arg2, i1 %Pred) local_unnamed_addr align 2 {
; CHECK-LABEL: @mixed-use-in-exit_switch(
; CHECK-NEXT:  entry:
; CHECK-NEXT:    [[MYTMP42:%.*]] = load i32, ptr [[ARG1:%.*]], align 4
; CHECK-NEXT:    [[CMP2:%.*]] = icmp slt i32 [[MYTMP42]], 0
; CHECK-NEXT:    switch i1 [[CMP2]], label [[A:%.*]] [
; CHECK-NEXT:      i1 false, label [[RETURN:%.*]]
; CHECK-NEXT:    ]
; CHECK:       A:
; CHECK-NEXT:    [[MYTMP43:%.*]] = add i32 [[MYTMP42]], 1
; CHECK-NEXT:    [[CMP1:%.*]] = icmp slt i32 [[MYTMP42]], 0
...
entry:
  %mytmp42 = load i32, ptr %arg1, align 4
  %cmp2 = icmp slt i32 %mytmp42, 0
  switch i1 %cmp2, label %A [ i1 0, label %return ]
...
; CHECK-NEXT:    [[CMP1:%.*]] = icmp slt i32 [[MYTMP42]], 0
; CHECK-NEXT:    callbr void asm "", "r,!i"(i1 [[CMP1]])
; CHECK-NEXT:            to label [[B:%.*]] [label [[A_TARGET_E:%.*]]]
; CHECK:       B:
; CHECK-NEXT:    [[MYTMP41:%.*]] = load i32, ptr [[ARG2:%.*]], align 4
; CHECK-NEXT:    callbr void asm "", ""()
; CHECK-NEXT:            to label [[C:%.*]] []
...
define i32 @phi-via-external-block_switch(ptr %arg1, ptr %arg2, i1 %Pred) local_unnamed_addr align 2 {
; CHECK-LABEL: @phi-via-external-block_switch(
; CHECK-NEXT:  entry:
; CHECK-NEXT:    [[MYTMP42:%.*]] = load i32, ptr [[ARG1:%.*]], align 4
; CHECK-NEXT:    switch i1 [[PRED:%.*]], label [[A:%.*]] [
; CHECK-NEXT:    ]
...
; CHECK-NEXT:    [[CMP1:%.*]] = icmp slt i32 [[MYTMP42]], 0
; CHECK-NEXT:    switch i1 [[CMP1]], label [[B:%.*]] [
; CHECK-NEXT:      i1 false, label [[A_TARGET_E:%.*]]
; CHECK-NEXT:    ]
; CHECK:       B:
; CHECK-NEXT:    [[MYTMP41:%.*]] = load i32, ptr [[ARG2:%.*]], align 4
; CHECK-NEXT:    switch i1 [[PRED]], label [[C:%.*]] [
...
A:
  %cmp1 = icmp slt i32 %mytmp42, 0
  switch i1 %cmp1, label %B [ i1 0, label %E ]
