...
; RUN: opt < %s -passes='unify-loop-exits' -S | FileCheck %s --check-prefix=NOLOWER
...
;
; NOLOWER-LABEL: @loop_1(
; NOLOWER-NEXT:  entry:
; NOLOWER-NEXT:    br i1 [[PREDENTRY:%.*]], label [[A:%.*]], label [[G:%.*]]
; NOLOWER:       A:
; NOLOWER-NEXT:    br label [[B:%.*]]
; NOLOWER:       B:
; NOLOWER-NEXT:    switch i32 [[VALUE:%.*]], label [[B_TARGET_X:%.*]] [
; NOLOWER-NEXT:      i32 0, label [[C:%.*]]
; NOLOWER-NEXT:      i32 1, label [[D:%.*]]
; NOLOWER-NEXT:    ]
; NOLOWER:       C:
; NOLOWER-NEXT:    br label [[D]]
; NOLOWER:       D:
; NOLOWER-NEXT:    br i1 [[PREDD:%.*]], label [[A]], label [[LOOP_EXIT_GUARD:%.*]]
; NOLOWER:       X:
; NOLOWER-NEXT:    br label [[EXIT:%.*]]
; NOLOWER:       Y:
; NOLOWER-NEXT:    br label [[EXIT]]
; NOLOWER:       G:
; NOLOWER-NEXT:    br label [[EXIT]]
; NOLOWER:       exit:
; NOLOWER-NEXT:    ret void
; NOLOWER:       B.target.X:
; NOLOWER-NEXT:    br label [[LOOP_EXIT_GUARD]]
; NOLOWER:       loop.exit.guard:
; NOLOWER-NEXT:    [[GUARD_X:%.*]] = phi i1 [ true, [[B_TARGET_X]] ], [ false, [[D]] ]
; NOLOWER-NEXT:    br i1 [[GUARD_X]], label [[X:%.*]], label [[Y:%.*]]
;
entry:
