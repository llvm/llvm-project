...
; CHECK-NEXT:  [[ENTRY:.*:]]
; CHECK-NEXT:    [[TMP0:%.*]] = tail call ptr @llvm.objc.retain(ptr [[X]]) #[[ATTR0]]
; CHECK-NEXT:    call void @llvm.objc.autoreleasePoolPop(ptr poison) #[[ATTR0]]
; CHECK-NEXT:    [[TMP1:%.*]] = tail call ptr @llvm.objc.retain(ptr [[X]]) #[[ATTR0]]
; CHECK-NEXT:    call void @use_pointer(ptr [[X]])
; CHECK-NEXT:    call void @use_pointer(ptr [[X]])
; CHECK-NEXT:    call void @llvm.objc.release(ptr [[X]]) #[[ATTR0]]
...
entry:
  call ptr @llvm.objc.retain(ptr %x) nounwind
  call void @llvm.objc.autoreleasePoolPop(ptr poison)
  call ptr @llvm.objc.retain(ptr %x) nounwind
  call void @use_pointer(ptr %x)
  call void @use_pointer(ptr %x)
  call void @llvm.objc.release(ptr %x) nounwind
...
; Delete no-ops where poison can be assumed to be null.

define void @test18b() {
; CHECK-LABEL: define void @test18b() {
; CHECK-NEXT:    ret void
;
  call ptr @llvm.objc.retain(ptr poison)
  call void @llvm.objc.release(ptr poison)
  call ptr @llvm.objc.autorelease(ptr poison)
  ret void
}
...
; CHECK:       [[LPAD20]]:
; CHECK-NEXT:    [[TMP502:%.*]] = phi ptr [ poison, %[[INVOKE_CONT23]] ], [ [[SELF]], %[[IF_THEN12]] ]
...
lpad20:                                           ; preds = %invoke.cont23, %if.then12
  %tmp502 = phi ptr [ poison, %invoke.cont23 ], [ %self, %if.then12 ]
...
invoke.cont:
  ...
  %tmp31 = insertvalue { <2 x float>, <2 x float> } poison, <2 x float> %tmp30, 0
