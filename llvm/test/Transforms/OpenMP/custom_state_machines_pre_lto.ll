...
; RUN: opt --mtriple=amdgpu-amd-amdhsa -openmp-opt-disable-state-machine-rewrite -S -passes=openmp-opt < %s | FileCheck %s --check-prefix=AMDGPU2
; RUN: opt --mtriple=amdgpu-amd-amdhsa -S -passes=openmp-opt-postlink < %s | FileCheck %s --check-prefix=AMDGPU3
