...
; RUN: opt --mtriple=amdgcn-amd-amdhsa -S -passes=openmp-opt < %s | FileCheck %s --check-prefix=AMDGPU1
; RUN: opt --mtriple=nvptx64--         -S -passes=openmp-opt < %s | FileCheck %s --check-prefix=NVPTX1
; RUN: opt --mtriple=amdgcn-amd-amdhsa -openmp-opt-disable-state-machine-rewrite -S -passes=openmp-opt < %s | FileCheck %s --check-prefix=AMDGPU2
; RUN: opt --mtriple=amdgcn-amd-amdhsa -S -passes=openmp-opt-postlink < %s | FileCheck %s --check-prefix=AMDGPU3
