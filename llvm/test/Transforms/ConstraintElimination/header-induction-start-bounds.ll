...
; CHECK-LABEL: define void @gep_nuw_negative_offset(
; CHECK-SAME: ptr [[P:%.*]]) {
; CHECK-NEXT:  [[ENTRY:.*]]:
; CHECK-NEXT:    br label %[[LOOP:.*]]
; CHECK:       [[LOOP]]:
...
; Same for a non-constant offset: nuw applies to it as well.
define void @gep_nuw_variable_offset(ptr %p, i64 %off) {
; CHECK-LABEL: define void @gep_nuw_variable_offset(
...
; CHECK-NEXT:    [[PV:%.*]] = phi ptr [ [[P]], %[[ENTRY]] ], [ [[PV_NEXT:%.*]], %[[LOOP]] ]
; CHECK-NEXT:    call void @use(i1 true)
; CHECK-NEXT:    [[PV_NEXT]] = getelementptr nuw i8, ptr [[PV]], i64 [[OFF]]
; CHECK-NEXT:    [[EC:%.*]] = call i1 @cond()
; CHECK-NEXT:    br i1 [[EC]], label %[[LOOP]], label %[[EXIT:.*]]
; CHECK:       [[EXIT]]:
; CHECK-NEXT:    ret void
...
loop:
  ...
  %c = icmp uge ptr %pv, %p
  call void @use(i1 %c)
  %pv.next = getelementptr nuw i8, ptr %pv, i64 %off
  %ec = call i1 @cond()
  br i1 %ec, label %loop, label %exit
