...
;
  %r = fmul <2 x float> %x, <float +qnan, float +qnan>
  ret <2 x float> %r
}
...
;
  %r = fmul <vscale x 1 x double> %x, splat (double +qnan)
  ret <vscale x 1 x double> %r
}
