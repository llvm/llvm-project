; RUN: opt -S -passes=simplifycfg -simplifycfg-require-and-preserve-domtree=1 -mtriple=amdgpu6.00-unknown-unknown < %s | FileCheck -check-prefix=SI -check-prefix=ALL %s
; RUN: opt -S -passes=simplifycfg -simplifycfg-require-and-preserve-domtree=1 -mtriple=amdgpu8.02-unknown-unknown < %s | FileCheck -check-prefix=SI -check-prefix=ALL %s
