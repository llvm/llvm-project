...
; by verifier.
; RUN: opt --opaque-pointers=0 -passes=cg-profile -module-summary %s -o %t.bc
; RUN: opt --opaque-pointers=0 -module-summary %p/Inputs/cg_profile.ll -o %t2.bc
; RUN: llvm-lto --opaque-pointers=0 -thinlto -o %t3 %t.bc %t2.bc
; RUN: opt --opaque-pointers=0 -passes=function-import -print-imports -summary-file %t3.thinlto.bc %t.bc \
