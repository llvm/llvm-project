...
; RUN: opt -S -passes='verify,loop-vectorize,verify' -enable-vplan-native-path -force-vector-width=2 < %s | FileCheck %s

; Test that non-widenable intrinsics in the outer loop are emitted as single
; scalar calls and do not crash the VPlan native path.
