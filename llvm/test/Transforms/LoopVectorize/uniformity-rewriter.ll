...
define void @uniformityrew_load(ptr %src.a, ptr noalias %src.b, ptr noalias %dst, i64 %n) {
; CHECK-LABEL: define void @uniformityrew_load(
...
define void @uniformityrew_binop(ptr %src.a, ptr noalias %src.b, ptr noalias %dst, i64 %n) {
; CHECK-LABEL: define void @uniformityrew_binop(
; CHECK-SAME: ptr [[SRC_A:%.*]], ptr noalias [[SRC_B:%.*]], ptr noalias [[DST:%.*]], i64 [[N:%.*]]) {
; CHECK-NEXT:  [[ENTRY:.*]]:
; CHECK-NEXT:    [[TMP0:%.*]] = add i64 [[N]], 1
; CHECK-NEXT:    [[MIN_ITERS_CHECK:%.*]] = icmp ult i64 [[TMP0]], 4
...
; CHECK:       [[LOOP]]:
; CHECK-NEXT:    [[IV:%.*]] = phi i64 [ [[BC_RESUME_VAL]], %[[SCALAR_PH]] ], [ [[IV_NEXT:%.*]], %[[LOOP]] ]
; CHECK-NEXT:    [[LOAD_A:%.*]] = or i64 [[IV]], [[N]]
; CHECK-NEXT:    [[D:%.*]] = udiv i64 [[LOAD_A]], 2
; CHECK-NEXT:    [[TRUNC:%.*]] = trunc i64 [[D]] to i32
; CHECK-NEXT:    [[GEP_B:%.*]] = getelementptr i32, ptr [[SRC_B]], i32 [[TRUNC]]
; CHECK-NEXT:    [[LOAD_B:%.*]] = load i32, ptr [[GEP_B]], align 4
; CHECK-NEXT:    [[GEP_DST:%.*]] = getelementptr i32, ptr [[DST]], i64 [[IV]]
; CHECK-NEXT:    store i32 [[LOAD_B]], ptr [[GEP_DST]], align 4
; CHECK-NEXT:    [[IV_NEXT]] = add i64 [[IV]], 1
...
loop:
  %iv = phi i64 [ 0, %entry ], [ %iv.next, %loop ]
  %or.iv.3 = or i64 %iv, %n
  %d = udiv i64 %or.iv.3, 2
  %trunc = trunc i64 %d to i32
  %gep.b = getelementptr i32, ptr %src.b, i32 %trunc
  %load.b = load i32, ptr %gep.b, align 4
  %gep.dst = getelementptr i32, ptr %dst, i64 %iv
  store i32 %load.b, ptr %gep.dst
  %iv.next = add i64 %iv, 1
