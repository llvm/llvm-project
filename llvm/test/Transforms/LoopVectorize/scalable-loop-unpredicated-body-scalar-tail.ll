...
; RUN: opt -S -passes=loop-vectorize -force-vector-interleave=1 -force-vector-width="vscale x 4" < %s | FileCheck %s --check-prefix=CHECKUF1
; RUN: opt -S -passes=loop-vectorize -force-vector-interleave=2 -force-vector-width="vscale x 4" < %s | FileCheck %s --check-prefix=CHECKUF2
...
; CHECKUF2-NEXT:    [[TMP6:%.*]] = shl nuw i64 [[TMP0]], 3
