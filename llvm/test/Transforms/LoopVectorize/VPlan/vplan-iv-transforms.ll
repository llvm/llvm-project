...
; CHECK-LABEL: VPlan for loop in 'iv_expand'
; CHECK:  VPlan 'Initial VPlan for VF={8},UF>=1' {
; CHECK-NEXT:  Live-in vp<[[VP0:%[0-9]+]]> = VF
; CHECK-NEXT:  Live-in vp<[[VP1:%[0-9]+]]> = VF * UF
; CHECK-NEXT:  Live-in vp<[[VP2:%[0-9]+]]> = vector-trip-count
...
; FIXME: The nuw/nsw flags from the scalar IV are not valid for the truncated
; IV.
define void @iv_truncated_wrap_flags(ptr %dst, i64 %n) {
; CHECK-LABEL: VPlan for loop in 'iv_truncated_wrap_flags'
; CHECK:  VPlan 'Initial VPlan for VF={8},UF>=1' {
; CHECK-NEXT:  Live-in vp<[[VP0:%[0-9]+]]> = VF
; CHECK-NEXT:  Live-in vp<[[VP1:%[0-9]+]]> = VF * UF
; CHECK-NEXT:  Live-in vp<[[VP2:%[0-9]+]]> = vector-trip-count
...
; CHECK-NEXT:  ir-bb<loop>:
; CHECK-NEXT:    IR   %iv = phi i64 [ 0, %entry ], [ %iv.next, %loop ] (extra operand: vp<%bc.resume.val> from scalar.ph)
; CHECK-NEXT:    IR   %val = trunc i64 %iv to i16
; CHECK-NEXT:    IR   %ptr = getelementptr inbounds i16, ptr %dst, i64 %iv
; CHECK-NEXT:    IR   store i16 %val, ptr %ptr, align 2
; CHECK-NEXT:    IR   %iv.next = add nuw nsw i64 %iv, 1
; CHECK-NEXT:    IR   %exit.cond = icmp eq i64 %iv.next, %n
; CHECK-NEXT:  No successors
; CHECK-NEXT:  }
;
entry:
...
loop:
  %iv = phi i64 [ 0, %entry ], [ %iv.next, %loop ]
  %val = trunc i64 %iv to i16
  %ptr = getelementptr inbounds i16, ptr %dst, i64 %iv
  store i16 %val, ptr %ptr, align 2
