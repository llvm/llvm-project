...
; CHECK-NEXT:    EMIT vp<[[VP4:%[0-9]+]]> = TC > VF ? TC - VF : 0 ir<%N>, vp<[[VP1]]>
; CHECK-NEXT:  Successor(s): vector loop
; CHECK-EMPTY:
; CHECK-NEXT:  <x1> vector loop: {
...
; CHECK-EMPTY:
; CHECK-NEXT:    vector.body:
; CHECK-NEXT:      ACTIVE-LANE-MASK-PHI vp<[[VP7:%[0-9]+]]> = phi vp<%extract.entry.alm.part>, vp<%extract.next.alm.part>
; CHECK-NEXT:      WIDEN-REDUCTION-PHI ir<%accum> = phi (add) vp<[[VP3]]>, vp<[[VP11:%[0-9]+]]> (VF scaled by 1/4)
; CHECK-NEXT:      vp<[[VP8:%[0-9]+]]> = SCALAR-STEPS vp<[[VP5]]>, ir<1>, vp<[[VP0]]>
; CHECK-NEXT:      CLONE ir<%gep.a> = getelementptr ir<%a>, vp<[[VP8]]>
; CHECK-NEXT:      vp<[[VP9:%[0-9]+]]> = vector-pointer i8, ir<%gep.a>, ir<1>
; CHECK-NEXT:      WIDEN ir<%load.a> = load vp<[[VP9]]>, vp<[[VP7]]>
; CHECK-NEXT:      CLONE ir<%gep.b> = getelementptr ir<%b>, vp<[[VP8]]>
; CHECK-NEXT:      vp<[[VP10:%[0-9]+]]> = vector-pointer i8, ir<%gep.b>, ir<1>
; CHECK-NEXT:      WIDEN ir<%load.b> = load vp<[[VP10]]>, vp<[[VP7]]>
; CHECK-NEXT:      EXPRESSION vp<[[VP11]]> = ir<%accum> + partial.reduce.add (mul (ir<%load.b> zext to i32), (ir<%load.a> zext to i32), vp<[[VP7]]>)
; CHECK-NEXT:      EMIT vp<%index.next> = add vp<[[VP5]]>, vp<[[VP1]]>
; CHECK-NEXT:      EMIT vp<%active.lane.mask.next> = wide active lane mask vp<[[VP5]]>, vp<[[VP4]]>, ir<1>
; CHECK-NEXT:      EMIT vp<%extract.next.alm.part> = extract-vector-for-part vp<%active.lane.mask.next>, ir<0>
; CHECK-NEXT:      EMIT vp<[[VP12:%[0-9]+]]> = not vp<%extract.next.alm.part>
...
; CHECK-EMPTY:
; CHECK-NEXT:  middle.block:
; CHECK-NEXT:    EMIT vp<[[VP14:%[0-9]+]]> = compute-reduction-result (add) vp<[[VP11]]>
