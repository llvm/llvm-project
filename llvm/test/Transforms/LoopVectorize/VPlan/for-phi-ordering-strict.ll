...
; CHECK-NEXT:      WIDEN-REDUCTION-PHI ir<%sum> = phi (add) vp<[[VP4]]>, ir<%sum.next>
; CHECK-NEXT:      WIDEN ir<%iv.nonzero> = icmp ne ir<%iv>, ir<0>
; CHECK-NEXT:      WIDEN ir<%or.cond> = select ir<%iv.nonzero>, ir<%guard>, ir<false>
; CHECK-NEXT:      EMIT vp<[[VP6:%[0-9]+]]> = not ir<%or.cond>
; CHECK-NEXT:      WIDEN-INTRINSIC vp<[[VP7:%[0-9]+]]> = call llvm.masked.srem(ir<%iv>, ir<%divisor>, vp<[[VP6]]>)
; CHECK-NEXT:      BLEND ir<%cond> = vp<[[VP7]]> ir<0>/ir<%or.cond>
; CHECK-NEXT:      WIDEN ir<%sum.next> = add ir<%sum>, ir<%cond>
...
; CHECK-NEXT:    EMIT vp<[[VP9:%[0-9]+]]> = compute-reduction-result (add) ir<%sum.next>
; CHECK-NEXT:    EMIT vp<[[VP10:%[0-9]+]]> = extract-last-part ir<%prev.iv>
...
; CHECK-NEXT:    IR   %sum = phi i32 [ 0, %entry ], [ %sum.next, %loop.latch ] (extra operand: vp<%bc.merge.rdx> from scalar.ph)
