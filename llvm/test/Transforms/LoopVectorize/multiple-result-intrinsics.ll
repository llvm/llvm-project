; NOTE: Assertions have been autogenerated by utils/update_test_checks.py UTC_ARGS: --filter "(:|sincos|frexp|modf|extract|store|uadd|sadd|usub|ssub|umul|smul)" --version 5
...
; CHECK:    [[OVERFLOW:%.*]] = extractelement <2 x i1> [[TMP6]], i32 0
; CHECK:    [[TMP8:%.*]] = extractelement <2 x i1> [[TMP6]], i32 1
; CHECK:    store <2 x i64> [[TMP5]], ptr [[TMP9:%.*]], align 8
; CHECK:    store i1 [[OVERFLOW]], ptr [[ARRAYIDX_OVERFLOW:%.*]], align 1
; CHECK:    store i1 [[TMP8]], ptr [[TMP11:%.*]], align 1
; CHECK:  [[EXIT:.*:]]
; CHECK:  [[EXIT1:.*:]]
;
entry:
...
for.body:
  ...
  %arrayidx_overflow = getelementptr inbounds i1, ptr %out_overflow, i64 %iv
  store i1 %overflow, ptr %arrayidx_overflow, align 1
  %iv.next = add nuw nsw i64 %iv, 1
  %exitcond.not = icmp eq i64 %iv.next, 1024
  br i1 %exitcond.not, label %exit, label %for.body
