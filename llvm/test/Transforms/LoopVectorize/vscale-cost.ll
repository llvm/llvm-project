...
; REQUIRES: asserts
; RUN: opt %s -passes=loop-vectorize -force-vector-interleave=1 -force-vector-width="vscale x 4" -enable-interleaved-mem-accesses \
; RUN:   -disable-output -debug-only=loop-vectorize -S 2>&1 | FileCheck %s
