...
; The compare feeding the index select uses the smax result, so the smax has
; an in-loop user besides its phi.
define i64 @test_cmp_uses_smax_result(ptr %src, i64 %n) {
; CHECK-LABEL: define i64 @test_cmp_uses_smax_result(
; CHECK-SAME: ptr [[SRC:%.*]], i64 [[N:%.*]]) {
; CHECK-NEXT:  [[ENTRY:.*]]:
; CHECK-NEXT:    br label %[[LOOP:.*]]
; CHECK:       [[LOOP]]:
...
; CHECK-NEXT:    [[CMP:%.*]] = icmp ne i64 [[MAX_VAL_NEXT]], [[MAX_VAL]]
; CHECK-NEXT:    [[MAX_IDX_NEXT]] = select i1 [[CMP]], i64 [[IV]], i64 [[MAX_IDX]]
; CHECK-NEXT:    [[IV_NEXT]] = add nuw nsw i64 [[IV]], 1
; CHECK-NEXT:    [[EXITCOND_NOT:%.*]] = icmp eq i64 [[IV_NEXT]], [[N]]
; CHECK-NEXT:    br i1 [[EXITCOND_NOT]], label %[[EXIT:.*]], label %[[LOOP]]
...
loop:
  ...
  %l = load i64, ptr %gep
  %max.val.next = tail call i64 @llvm.smax.i64(i64 %max.val, i64 %l)
  %cmp = icmp ne i64 %max.val.next, %max.val
  %max.idx.next = select i1 %cmp, i64 %iv, i64 %max.idx
  %iv.next = add nuw nsw i64 %iv, 1
  %exitcond.not = icmp eq i64 %iv.next, %n
  br i1 %exitcond.not, label %exit, label %loop
