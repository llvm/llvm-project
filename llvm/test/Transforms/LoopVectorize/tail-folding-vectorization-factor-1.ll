...
; RUN: opt < %s -passes=loop-vectorize -force-vector-interleave=4 -tail-folding-policy=prefer-fold-tail -pass-remarks='loop-vectorize' -disable-output -S 2>&1 | FileCheck %s --check-prefix=CHECK-REMARKS
; RUN: opt < %s -passes=loop-vectorize -force-vector-interleave=4 -tail-folding-policy=prefer-fold-tail -S | FileCheck %s
