...
; CHECK-EPI-NEXT:    [[TMP7:%.*]] = insertelement <16 x float> splat (float -0.000000e+00), float [[STARTVAL]], i32 0
; CHECK-EPI-NEXT:    br label %[[VECTOR_PH:.*]]
; CHECK-EPI:       [[VECTOR_PH]]:
; CHECK-EPI-NEXT:    [[INDEX1:%.*]] = phi i32 [ 0, %[[VECTOR_PH1]] ], [ [[INDEX_NEXT1:%.*]], %[[VECTOR_PH]] ]
; CHECK-EPI-NEXT:    [[VEC_PHI:%.*]] = phi <16 x float> [ [[TMP7]], %[[VECTOR_PH1]] ], [ [[TMP6:%.*]], %[[VECTOR_PH]] ]
...
; CHECK-EPI-NEXT:    [[TMP6]] = fsub reassoc contract <16 x float> [[VEC_PHI]], [[TMP11]]
...
; CHECK-EPI-NEXT:    [[TMP12:%.*]] = call reassoc contract float @llvm.vector.reduce.fadd.v16f32(float -0.000000e+00, <16 x float> [[TMP6]])
...
; CHECK-EPI-NEXT:    [[BC_MERGE_RDX:%.*]] = phi float [ [[TMP12]], %[[VEC_EPILOG_ITER_CHECK]] ], [ [[STARTVAL]], %[[VECTOR_MAIN_LOOP_ITER_CHECK1]] ]
; CHECK-EPI-NEXT:    [[TMP14:%.*]] = insertelement <4 x float> splat (float -0.000000e+00), float [[BC_MERGE_RDX]], i32 0
; CHECK-EPI-NEXT:    br label %[[VECTOR_BODY:.*]]
; CHECK-EPI:       [[VECTOR_BODY]]:
; CHECK-EPI-NEXT:    [[INDEX:%.*]] = phi i32 [ [[VEC_EPILOG_RESUME_VAL]], %[[VEC_EPILOG_PH]] ], [ [[INDEX_NEXT:%.*]], %[[VECTOR_BODY]] ]
; CHECK-EPI-NEXT:    [[VEC_PHI3:%.*]] = phi <4 x float> [ [[TMP14]], %[[VEC_EPILOG_PH]] ], [ [[TMP15:%.*]], %[[VECTOR_BODY]] ]
...
; CHECK-EPI-NEXT:    [[TMP15]] = fsub reassoc contract <4 x float> [[VEC_PHI3]], [[TMP4]]
...
; CHECK-EPI:       [[MIDDLE_BLOCK]]:
; CHECK-EPI-NEXT:    [[TMP17:%.*]] = call reassoc contract float @llvm.vector.reduce.fadd.v4f32(float -0.000000e+00, <4 x float> [[TMP15]])
...
; CHECK-EPI-NEXT:    [[BC_MERGE_RDX7:%.*]] = phi float [ [[TMP17]], %[[MIDDLE_BLOCK]] ], [ [[TMP12]], %[[VEC_EPILOG_ITER_CHECK]] ], [ [[STARTVAL]], %[[VECTOR_MAIN_LOOP_ITER_CHECK]] ]
; CHECK-EPI-NEXT:    br label %[[LOOP:.*]]
; CHECK-EPI:       [[LOOP]]:
; CHECK-EPI-NEXT:    [[IV:%.*]] = phi i32 [ [[BC_RESUME_VAL]], %[[VEC_EPILOG_VECTOR_BODY]] ], [ [[IV_NEXT:%.*]], %[[LOOP]] ]
; CHECK-EPI-NEXT:    [[ACCUM:%.*]] = phi float [ [[BC_MERGE_RDX7]], %[[VEC_EPILOG_VECTOR_BODY]] ], [ [[SUB:%.*]], %[[LOOP]] ]
...
; CHECK-EPI:       [[EXIT]]:
; CHECK-EPI-NEXT:    [[SUB_LCSSA:%.*]] = phi float [ [[SUB]], %[[LOOP]] ], [ [[TMP12]], %[[MIDDLE_BLOCK1]] ], [ [[TMP17]], %[[MIDDLE_BLOCK]] ]
...
; CHECK-PARTIAL-RED-EPI-NEXT:    [[TMP7:%.*]] = insertelement <16 x float> splat (float -0.000000e+00), float [[STARTVAL]], i32 0
...
; CHECK-PARTIAL-RED-EPI-NEXT:    [[VEC_PHI:%.*]] = phi <16 x float> [ [[TMP7]], %[[VECTOR_PH1]] ], [ [[TMP6:%.*]], %[[VECTOR_PH]] ]
...
; CHECK-PARTIAL-RED-EPI-NEXT:    [[TMP6]] = fsub reassoc contract <16 x float> [[VEC_PHI]], [[TMP12]]
...
; CHECK-PARTIAL-RED-EPI-NEXT:    [[TMP14:%.*]] = call reassoc contract float @llvm.vector.reduce.fadd.v16f32(float -0.000000e+00, <16 x float> [[TMP6]])
...
; CHECK-PARTIAL-RED-EPI-NEXT:    [[BC_MERGE_RDX:%.*]] = phi float [ [[TMP14]], %[[VEC_EPILOG_ITER_CHECK]] ], [ [[STARTVAL]], %[[VECTOR_MAIN_LOOP_ITER_CHECK1]] ]
; CHECK-PARTIAL-RED-EPI-NEXT:    [[TMP16:%.*]] = insertelement <8 x float> splat (float -0.000000e+00), float [[BC_MERGE_RDX]], i32 0
; CHECK-PARTIAL-RED-EPI-NEXT:    br label %[[VECTOR_BODY:.*]]
; CHECK-PARTIAL-RED-EPI:       [[VECTOR_BODY]]:
; CHECK-PARTIAL-RED-EPI-NEXT:    [[INDEX:%.*]] = phi i32 [ [[VEC_EPILOG_RESUME_VAL]], %[[VEC_EPILOG_PH]] ], [ [[INDEX_NEXT:%.*]], %[[VECTOR_BODY]] ]
; CHECK-PARTIAL-RED-EPI-NEXT:    [[VEC_PHI3:%.*]] = phi <8 x float> [ [[TMP16]], %[[VEC_EPILOG_PH]] ], [ [[TMP15:%.*]], %[[VECTOR_BODY]] ]
...
; CHECK-PARTIAL-RED-EPI-NEXT:    [[TMP15]] = fsub reassoc contract <8 x float> [[VEC_PHI3]], [[TMP4]]
...
; CHECK-PARTIAL-RED-EPI:       [[MIDDLE_BLOCK]]:
; CHECK-PARTIAL-RED-EPI-NEXT:    [[TMP17:%.*]] = call reassoc contract float @llvm.vector.reduce.fadd.v8f32(float -0.000000e+00, <8 x float> [[TMP15]])
...
; CHECK-PARTIAL-RED-EPI-NEXT:    [[BC_MERGE_RDX7:%.*]] = phi float [ [[TMP17]], %[[MIDDLE_BLOCK]] ], [ [[TMP14]], %[[VEC_EPILOG_ITER_CHECK]] ], [ [[STARTVAL]], %[[VECTOR_MAIN_LOOP_ITER_CHECK]] ]
; CHECK-PARTIAL-RED-EPI-NEXT:    br label %[[LOOP:.*]]
; CHECK-PARTIAL-RED-EPI:       [[LOOP]]:
; CHECK-PARTIAL-RED-EPI-NEXT:    [[IV:%.*]] = phi i32 [ [[BC_RESUME_VAL]], %[[VEC_EPILOG_VECTOR_BODY]] ], [ [[IV_NEXT:%.*]], %[[LOOP]] ]
; CHECK-PARTIAL-RED-EPI-NEXT:    [[ACCUM:%.*]] = phi float [ [[BC_MERGE_RDX7]], %[[VEC_EPILOG_VECTOR_BODY]] ], [ [[SUB:%.*]], %[[LOOP]] ]
...
; CHECK-PARTIAL-RED-EPI:       [[EXIT]]:
; CHECK-PARTIAL-RED-EPI-NEXT:    [[SUB_LCSSA:%.*]] = phi float [ [[SUB]], %[[LOOP]] ], [ [[TMP14]], %[[MIDDLE_BLOCK1]] ], [ [[TMP17]], %[[MIDDLE_BLOCK]] ]
...
; CHECK-EPI-NEXT:    [[VEC_PHI:%.*]] = phi <8 x float> [ zeroinitializer, %[[VECTOR_PH]] ], [ [[TMP9:%.*]], %[[VECTOR_BODY]] ]
...
; CHECK-EPI-NEXT:    [[TMP10:%.*]] = fsub reassoc nsz contract <8 x float> [[VEC_PHI]], [[TMP5]]
; CHECK-EPI-NEXT:    [[TMP9]] = fsub reassoc nsz contract <8 x float> [[TMP10]], [[TMP7]]
...
; CHECK-EPI:       [[MIDDLE_BLOCK]]:
; CHECK-EPI-NEXT:    [[TMP20:%.*]] = call reassoc nsz contract float @llvm.vector.reduce.fadd.v8f32(float 0.000000e+00, <8 x float> [[TMP9]])
...
; CHECK-EPI-NEXT:    [[BC_MERGE_RDX:%.*]] = phi float [ [[TMP20]], %[[VEC_EPILOG_ITER_CHECK]] ], [ 0.000000e+00, %[[VECTOR_MAIN_LOOP_ITER_CHECK]] ]
; CHECK-EPI-NEXT:    [[TMP23:%.*]] = insertelement <4 x float> zeroinitializer, float [[BC_MERGE_RDX]], i32 0
; CHECK-EPI-NEXT:    br label %[[VEC_EPILOG_VECTOR_BODY:.*]]
; CHECK-EPI:       [[VEC_EPILOG_VECTOR_BODY]]:
; CHECK-EPI-NEXT:    [[INDEX4:%.*]] = phi i64 [ [[VEC_EPILOG_RESUME_VAL]], %[[VEC_EPILOG_PH]] ], [ [[INDEX_NEXT11:%.*]], %[[VEC_EPILOG_VECTOR_BODY]] ]
; CHECK-EPI-NEXT:    [[VEC_PHI4:%.*]] = phi <4 x float> [ [[TMP23]], %[[VEC_EPILOG_PH]] ], [ [[TMP22:%.*]], %[[VEC_EPILOG_VECTOR_BODY]] ]
...
; CHECK-EPI-NEXT:    [[TMP21:%.*]] = fsub reassoc nsz contract <4 x float> [[VEC_PHI4]], [[TMP16]]
; CHECK-EPI-NEXT:    [[TMP22]] = fsub reassoc nsz contract <4 x float> [[TMP21]], [[TMP18]]
...
; CHECK-EPI:       [[VEC_EPILOG_MIDDLE_BLOCK]]:
; CHECK-EPI-NEXT:    [[TMP24:%.*]] = call reassoc nsz contract float @llvm.vector.reduce.fadd.v4f32(float 0.000000e+00, <4 x float> [[TMP22]])
...
; CHECK-EPI-NEXT:    [[BC_MERGE_RDX9:%.*]] = phi float [ [[TMP24]], %[[VEC_EPILOG_MIDDLE_BLOCK]] ], [ [[TMP20]], %[[VEC_EPILOG_ITER_CHECK]] ], [ 0.000000e+00, %[[ITER_CHECK]] ]
; CHECK-EPI-NEXT:    br label %[[FOR_BODY:.*]]
; CHECK-EPI:       [[FOR_BODY]]:
; CHECK-EPI-NEXT:    [[IV:%.*]] = phi i64 [ [[BC_RESUME_VAL]], %[[VEC_EPILOG_SCALAR_PH]] ], [ [[IV_NEXT:%.*]], %[[FOR_BODY]] ]
; CHECK-EPI-NEXT:    [[ACCUM:%.*]] = phi float [ [[BC_MERGE_RDX9]], %[[VEC_EPILOG_SCALAR_PH]] ], [ [[SUB:%.*]], %[[FOR_BODY]] ]
...
; CHECK-EPI:       [[FOR_EXIT]]:
; CHECK-EPI-NEXT:    [[SUB_LCSSA:%.*]] = phi float [ [[SUB]], %[[FOR_BODY]] ], [ [[TMP20]], %[[MIDDLE_BLOCK]] ], [ [[TMP24]], %[[VEC_EPILOG_MIDDLE_BLOCK]] ]
; CHECK-EPI-NEXT:    ret float [[SUB_LCSSA]]
;
...
; CHECK-PARTIAL-RED-EPI-NEXT:    [[VEC_PHI:%.*]] = phi <8 x float> [ zeroinitializer, %[[VECTOR_PH]] ], [ [[TMP9:%.*]], %[[VECTOR_BODY]] ]
...
; CHECK-PARTIAL-RED-EPI-NEXT:    [[TMP11:%.*]] = fsub reassoc nsz contract <8 x float> [[VEC_PHI]], [[TMP5]]
; CHECK-PARTIAL-RED-EPI-NEXT:    [[TMP9]] = fsub reassoc nsz contract <8 x float> [[TMP11]], [[TMP7]]
...
; CHECK-PARTIAL-RED-EPI:       [[MIDDLE_BLOCK]]:
; CHECK-PARTIAL-RED-EPI-NEXT:    [[TMP10:%.*]] = call reassoc nsz contract float @llvm.vector.reduce.fadd.v8f32(float 0.000000e+00, <8 x float> [[TMP9]])
