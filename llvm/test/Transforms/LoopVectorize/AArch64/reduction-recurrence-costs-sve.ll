...
; PRED-NEXT:    [[VEC_PHI:%.*]] = phi <vscale x 8 x i16> [ zeroinitializer, %[[VECTOR_PH]] ], [ [[TMP15:%.*]], %[[VECTOR_BODY]] ]
...
; PRED-NEXT:    [[TMP16:%.*]] = udiv <vscale x 8 x i16> [[WIDE_MASKED_LOAD]], [[BROADCAST_SPLAT]]
; PRED-NEXT:    [[TMP13:%.*]] = or <vscale x 8 x i16> [[TMP16]], [[VEC_PHI]]
; PRED-NEXT:    [[TMP15]] = select <vscale x 8 x i1> [[ACTIVE_LANE_MASK]], <vscale x 8 x i16> [[TMP13]], <vscale x 8 x i16> [[VEC_PHI]]
...
; PRED-NEXT:    [[TMP17:%.*]] = extractelement <vscale x 8 x i1> [[ACTIVE_LANE_MASK_NEXT]], i32 0
; PRED-NEXT:    [[TMP18:%.*]] = xor i1 [[TMP17]], true
; PRED-NEXT:    br i1 [[TMP18]], label %[[MIDDLE_BLOCK:.*]], label %[[VECTOR_BODY]], !llvm.loop [[LOOP4:![0-9]+]]
; PRED:       [[MIDDLE_BLOCK]]:
; PRED-NEXT:    [[TMP19:%.*]] = call i16 @llvm.vector.reduce.or.nxv8i16(<vscale x 8 x i16> [[TMP15]])
; PRED-NEXT:    br label %[[EXIT:.*]]
; PRED:       [[SCALAR_PH]]:
; PRED-NEXT:    br label %[[LOOP:.*]]
; PRED:       [[LOOP]]:
