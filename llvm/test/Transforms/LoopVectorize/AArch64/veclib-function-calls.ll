...
; RUN: opt -mattr=+sve -vector-library=LIBMVEC -passes=inject-tli-mappings,loop-vectorize,simplifycfg -mcpu=neoverse-v1 -force-vector-interleave=1 -tail-folding-policy=must-fold-tail -S < %s | FileCheck %s -check-prefix=LIBMVEC-SVE
...
; RUN: opt -mattr=+sve -vector-library=sleefgnuabi -passes=inject-tli-mappings,loop-vectorize,simplifycfg -mcpu=neoverse-v1 -force-vector-interleave=1 -tail-folding-policy=must-fold-tail -S < %s | FileCheck %s -check-prefix=SLEEF-SVE
...
; RUN: opt -mattr=+sve -vector-library=ArmPL -passes=inject-tli-mappings,loop-vectorize,simplifycfg -mcpu=neoverse-v1 -force-vector-interleave=1 -tail-folding-policy=must-fold-tail -S < %s | FileCheck %s -check-prefix=ARMPL-SVE
