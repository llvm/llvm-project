...
; CHECK:       [[SCALAR_PH]]:
; CHECK-NEXT:    br i1 true, [[EXIT:label %.*]], label %[[SCALAR_PH1]]
; CHECK:       [[SCALAR_PH1]]:
;
entry:
  br label %loop
...
; CHECK-NEXT:    [[INDEX_NEXT]] = add nuw i64 [[INDEX]], 2
; CHECK-NEXT:    [[TMP9:%.*]] = icmp eq i64 [[INDEX_NEXT]], 100
; CHECK-NEXT:    br i1 [[TMP9]], label %[[SCALAR_PH:.*]], label %[[VECTOR_BODY]], !llvm.loop [[LOOP6:![0-9]+]]
; CHECK:       [[SCALAR_PH]]:
; CHECK-NEXT:    br i1 true, [[EXIT:label %.*]], label %[[SCALAR_PH1]]
; CHECK:       [[SCALAR_PH1]]:
;
...
; CHECK-NEXT:    br i1 [[TMP122]], label %[[MIDDLE_BLOCK:.*]], label %[[VECTOR_BODY]], !llvm.loop [[LOOP14:![0-9]+]]
; CHECK:       [[MIDDLE_BLOCK]]:
; CHECK-NEXT:    br i1 false, [[EXIT:label %.*]], label %[[SCALAR_PH]]
; CHECK:       [[SCALAR_PH]]:
;
