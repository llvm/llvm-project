...
; To test the vectorization, the loop body size limit has been dropped to allow for smaller test cases to be tested
; This still tests that loops are costed, and either accepted or rejected based on the cost model.
; RUN: opt -passes=loop-vectorize -S -low-trip-count-loop-body-size-limit=5 < %s | FileCheck %s
...
;
; To test the vectorization path, the limit for bodies has been lowered
; to keep the test cases small and to show the functionality of the
; option, but cost is still considered for if vectorization is beneficial.
...
define void @tc4_vf2_ic2(ptr noalias %a, ptr noalias %b) #0 {
