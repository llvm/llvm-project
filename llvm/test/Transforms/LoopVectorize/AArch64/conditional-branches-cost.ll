...
; PRED-NEXT:    [[TMP5:%.*]] = uitofp <vscale x 2 x i16> [[BROADCAST_SPLAT]] to <vscale x 2 x double>
; PRED-NEXT:    call void @llvm.masked.store.nxv2f64.p0(<vscale x 2 x double> [[TMP5]], ptr align 8 [[NEXT_GEP]], <vscale x 2 x i1> [[ACTIVE_LANE_MASK]])
; PRED-NEXT:    [[INDEX_NEXT]] = add i64 [[INDEX]], [[TMP7]]
; PRED-NEXT:    [[ACTIVE_LANE_MASK_NEXT]] = call <vscale x 2 x i1> @llvm.get.active.lane.mask.nxv2i1.i64(i64 [[INDEX_NEXT]], i64 257)
; PRED-NEXT:    [[TMP15:%.*]] = extractelement <vscale x 2 x i1> [[ACTIVE_LANE_MASK_NEXT]], i64 0
...
; DEFAULT-NEXT:    [[TMP70:%.*]] = call i64 @llvm.vscale.i64()
; DEFAULT-NEXT:    [[TMP71:%.*]] = shl nuw i64 [[TMP70]], 2
; DEFAULT-NEXT:    [[N_MOD_VF16:%.*]] = urem i64 [[TMP0]], [[TMP71]]
...
; DEFAULT-NEXT:    [[WIDE_LOAD21:%.*]] = load <vscale x 4 x i8>, ptr [[TMP72]], align 1
; DEFAULT-NEXT:    [[TMP73:%.*]] = uitofp <vscale x 4 x i8> [[WIDE_LOAD21]] to <vscale x 4 x float>
; DEFAULT-NEXT:    [[TMP74:%.*]] = getelementptr i8, ptr [[B]], i64 [[INDEX20]]
; DEFAULT-NEXT:    [[WIDE_LOAD22:%.*]] = load <vscale x 4 x i8>, ptr [[TMP74]], align 1
; DEFAULT-NEXT:    [[TMP75:%.*]] = icmp ne <vscale x 4 x i8> [[WIDE_LOAD22]], zeroinitializer
; DEFAULT-NEXT:    [[TMP76:%.*]] = xor <vscale x 4 x i8> [[WIDE_LOAD21]], splat (i8 1)
; DEFAULT-NEXT:    [[TMP77:%.*]] = call <vscale x 4 x i8> @llvm.masked.udiv.nxv4i8(<vscale x 4 x i8> [[TMP76]], <vscale x 4 x i8> [[BROADCAST_SPLAT19]], <vscale x 4 x i1> [[TMP75]])
; DEFAULT-NEXT:    [[TMP78:%.*]] = icmp ugt <vscale x 4 x i8> [[TMP77]], splat (i8 1)
; DEFAULT-NEXT:    [[TMP79:%.*]] = select <vscale x 4 x i1> [[TMP78]], <vscale x 4 x i32> zeroinitializer, <vscale x 4 x i32> splat (i32 255)
; DEFAULT-NEXT:    [[PREDPHI23:%.*]] = select <vscale x 4 x i1> [[TMP75]], <vscale x 4 x i32> [[TMP79]], <vscale x 4 x i32> zeroinitializer
; DEFAULT-NEXT:    [[TMP80:%.*]] = zext <vscale x 4 x i8> [[WIDE_LOAD21]] to <vscale x 4 x i32>
; DEFAULT-NEXT:    [[TMP81:%.*]] = sub <vscale x 4 x i32> [[PREDPHI23]], [[TMP80]]
; DEFAULT-NEXT:    [[TMP82:%.*]] = sitofp <vscale x 4 x i32> [[TMP81]] to <vscale x 4 x float>
; DEFAULT-NEXT:    [[TMP83:%.*]] = call <vscale x 4 x float> @llvm.fmuladd.nxv4f32(<vscale x 4 x float> [[TMP82]], <vscale x 4 x float> splat (float 3.000000e+00), <vscale x 4 x float> [[TMP73]])
; DEFAULT-NEXT:    [[TMP84:%.*]] = fptoui <vscale x 4 x float> [[TMP83]] to <vscale x 4 x i8>
; DEFAULT-NEXT:    [[TMP85:%.*]] = getelementptr i8, ptr [[C]], i64 [[INDEX20]]
; DEFAULT-NEXT:    store <vscale x 4 x i8> [[TMP84]], ptr [[TMP85]], align 1
; DEFAULT-NEXT:    [[INDEX_NEXT24]] = add nuw i64 [[INDEX20]], [[TMP71]]
