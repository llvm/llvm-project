...
; CHECK-NEXT:   MULACC-REDUCE ir<[[REDUCE]]> = ir<[[ACC]]> + partial.reduce.add (mul (ir<%load.b> zext to i32), (ir<%load.a> zext to i32))
...
; CHECK-NEXT:   WIDEN-CAST vp<[[EXTA:%.+]]> = zext ir<%load.a> to i32
; CHECK-NEXT:   WIDEN vp<[[MUL:%.+]]> = mul vp<[[EXTB]]>, vp<[[EXTA]]>
; CHECK-NEXT:   PARTIAL-REDUCE vp<[[REDUCE]]> = ir<%accum> + reduce.add (vp<[[MUL]]>)
