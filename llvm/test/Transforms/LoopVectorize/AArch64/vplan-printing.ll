...
; CHECK-NEXT:   BUNDLE vp<[[REDUCE]]> = ir<[[ACC]]> + partial.reduce.add (mul (ir<%load.b> zext to i32), (ir<%load.a> zext to i32))
...
; CHECK-NEXT:   vp<[[PTR_B:%.+]]> = vector-pointer ir<%gep.b>
; CHECK-NEXT:   WIDEN ir<%load.b> = load vp<[[PTR_B]]>
; CHECK-NEXT:   WIDEN-CAST ir<[[EXTB:%.+]]> = zext ir<%load.b> to i32
; CHECK-NEXT:   WIDEN-CAST ir<[[EXTA:%.+]]> = zext ir<%load.a> to i32
; CHECK-NEXT:   WIDEN ir<[[MUL:%.+]]> = mul ir<[[EXTB]]>, ir<[[EXTA]]>
; CHECK-NEXT:   PARTIAL-REDUCE ir<[[REDUCE]]> = ir<%accum> + reduce.add (ir<[[MUL]]>)
...
define i32 @print_partial_reduction_sub(ptr %a, ptr %b) {
; CHECK:      VPlan 'Initial VPlan for VF={8,16},UF>=1' {
...
; CHECK-NEXT:     WIDEN-REDUCTION-PHI ir<%accum> = phi vp<%3>, vp<%8> (VF scaled by 1/4)
...
; CHECK-NEXT:     BUNDLE vp<%8> = ir<%accum> + partial.reduce.add (sub (0, mul (ir<%load.b> zext to i32), (ir<%load.a> zext to i32)))
...
; CHECK-NEXT:   WIDEN-REDUCTION-PHI ir<%accum> = phi vp<%1>, ir<%add>.1 (VF scaled by 1/4)
...
; CHECK-NEXT:   PARTIAL-REDUCE ir<%add>.1 = ir<%accum> +  reduce.add (ir<%add>)
; CHECK-NEXT:   EMIT vp<%index.next> = add nuw vp<%index>, ir<16>
; CHECK-NEXT:   EMIT branch-on-count vp<%index.next>, ir<1024>
; CHECK-NEXT: Successor(s): middle.block, vector.body
; CHECK-EMPTY:
; CHECK-NEXT: middle.block:
; CHECK-NEXT:   EMIT vp<%5> = compute-reduction-result ir<%accum>, ir<%add>.1
; CHECK-NEXT:   EMIT vp<%cmp.n> = icmp eq ir<1024>, ir<1024>
; CHECK-NEXT:   EMIT branch-on-cond vp<%cmp.n>
; CHECK-NEXT: Successor(s): ir-bb<for.exit>, ir-bb<scalar.ph>
; CHECK-EMPTY:
; CHECK-NEXT: ir-bb<for.exit>:
; CHECK-NEXT:   IR   %add.lcssa = phi i32 [ %add, %for.body ] (extra operand: vp<%5> from middle.block)
; CHECK-NEXT: No successors
; CHECK-EMPTY:
; CHECK-NEXT: ir-bb<scalar.ph>:
; CHECK-NEXT:   EMIT-SCALAR vp<%vec.epilog.resume.val> = phi [ ir<1024>, middle.block ], [ ir<0>, ir-bb<entry> ], [ ir<0>, ir-bb<vector.main.loop.iter.check> ]
; CHECK-NEXT:   EMIT-SCALAR vp<%bc.merge.rdx> = phi [ vp<%5>, middle.block ], [ ir<0>, ir-bb<entry> ], [ ir<0>, ir-bb<vector.main.loop.iter.check> ]
; CHECK-NEXT: Successor(s): ir-bb<for.body>
; CHECK-EMPTY:
; CHECK-NEXT: ir-bb<for.body>:
