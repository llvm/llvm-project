; NOTE: Assertions have been autogenerated by utils/update_analyze_test_checks.py UTC_ARGS: --filter "Cost.of.*EXPRESSION|Cost.of.*PARTIAL" --version 6
; RUN: opt -passes=loop-vectorize -use-partial-reductions-by-default             \
...
; NEON:  Cost of 2 for VF 8: EXPRESSION vp<[[VP8:%[0-9]+]]> = ir<%accum> + partial.reduce.add (sub (0, ir<%load.a>) sext to i32)
; NEON:  Cost of 1 for VF 8: EXPRESSION vp<[[VP9:%[0-9]+]]> = vp<[[VP8]]> + partial.reduce.add (ir<%load.b> sext to i32)
;
; SVE-LABEL: 'reduce_sub_add_chain_without_mul'
; SVE:  Cost of 2 for VF 8: EXPRESSION vp<[[VP8:%[0-9]+]]> = ir<%accum> + partial.reduce.add (sub (0, ir<%load.a>) sext to i32)
; SVE:  Cost of 1 for VF 8: EXPRESSION vp<[[VP9:%[0-9]+]]> = vp<[[VP8]]> + partial.reduce.add (ir<%load.b> sext to i32)
;
; SVE2-LABEL: 'reduce_sub_add_chain_without_mul'
; SVE2:  Cost of 2 for VF 8: EXPRESSION vp<[[VP8:%[0-9]+]]> = ir<%accum> + partial.reduce.add (sub (0, ir<%load.a>) sext to i32)
; SVE2:  Cost of 1 for VF 8: EXPRESSION vp<[[VP9:%[0-9]+]]> = vp<[[VP8]]> + partial.reduce.add (ir<%load.b> sext to i32)
; SVE2:  Cost of 2 for VF vscale x 8: EXPRESSION vp<[[VP8]]> = ir<%accum> + partial.reduce.add (sub (0, ir<%load.a>) sext to i32)
; SVE2:  Cost of 1 for VF vscale x 8: EXPRESSION vp<[[VP9]]> = vp<[[VP8]]> + partial.reduce.add (ir<%load.b> sext to i32)
;
...
; I8MM:  Cost of 2 for VF 8: EXPRESSION vp<[[VP8:%[0-9]+]]> = ir<%accum> + partial.reduce.add (sub (0, ir<%load.a>) sext to i32)
; I8MM:  Cost of 1 for VF 8: EXPRESSION vp<[[VP9:%[0-9]+]]> = vp<[[VP8]]> + partial.reduce.add (ir<%load.b> sext to i32)
; I8MM:  Cost of 2 for VF vscale x 8: EXPRESSION vp<[[VP8]]> = ir<%accum> + partial.reduce.add (sub (0, ir<%load.a>) sext to i32)
; I8MM:  Cost of 1 for VF vscale x 8: EXPRESSION vp<[[VP9]]> = vp<[[VP8]]> + partial.reduce.add (ir<%load.b> sext to i32)
;
entry:
  br label %for.body
...
;; Using partial reductions by default, in place of a normal add or fadd.

define half @sum_half(ptr %h) {
;; This previously caused crashes in codegen due to the illegal type.
;; The SVE/SME runlines won't generate a plan since we force scalable
;; vectorization and the trip count is 2 (below minimum threshold), so
;; there's no cost printed for them.
; NEON-LABEL: 'sum_half'
; NEON:  Cost of 1 for VF 2: PARTIAL-REDUCE ir<%sum.next> = ir<%sum> + fast reduce.fadd (ir<%l>)
;
; SVE-LABEL: 'sum_half'
; SVE2-LABEL: 'sum_half'
; SVE2p1-LABEL: 'sum_half'
; SVE2p3-LABEL: 'sum_half'
; SME2-LABEL: 'sum_half'
; I8MM-LABEL: 'sum_half'
entry:
  br label %loop
...
loop:
  ...
  %sum = phi half [ 0.0, %entry ], [ %sum.next, %loop ]
  %p = getelementptr inbounds half, ptr %h, i64 %iv
  %l = load half, ptr %p, align 2
  %sum.next = fadd fast half %sum, %l
  %iv.next = add nuw nsw i64 %iv, 1
  %ec = icmp eq i64 %iv.next, 2
  br i1 %ec, label %exit, label %loop, !llvm.loop !39
...
!40 = !{!"llvm.loop.interleave.count", i32 1}
!41 = !{!"llvm.loop.vectorize.width", i32 2}

define float @sum_float(ptr %in, i64 %limit) {
; NEON-LABEL: 'sum_float'
; NEON:  Cost of 1 for VF 4: PARTIAL-REDUCE ir<%sum.next> = ir<%sum> + fast reduce.fadd (ir<%l>)
; NEON:  Cost of 1 for VF 4: PARTIAL-REDUCE ir<%sum.next> = ir<%sum> + fast reduce.fadd (ir<%l>)
;
; SVE-LABEL: 'sum_float'
; SVE:  Cost of 1 for VF vscale x 4: PARTIAL-REDUCE ir<%sum.next> = ir<%sum> + fast reduce.fadd (ir<%l>)
; SVE:  Cost of 1 for VF vscale x 4: PARTIAL-REDUCE ir<%sum.next> = ir<%sum> + fast reduce.fadd (ir<%l>)
;
; SVE2-LABEL: 'sum_float'
; SVE2:  Cost of 1 for VF vscale x 4: PARTIAL-REDUCE ir<%sum.next> = ir<%sum> + fast reduce.fadd (ir<%l>)
; SVE2:  Cost of 1 for VF vscale x 4: PARTIAL-REDUCE ir<%sum.next> = ir<%sum> + fast reduce.fadd (ir<%l>)
;
; SVE2p1-LABEL: 'sum_float'
; SVE2p1:  Cost of 1 for VF vscale x 4: PARTIAL-REDUCE ir<%sum.next> = ir<%sum> + fast reduce.fadd (ir<%l>)
; SVE2p1:  Cost of 1 for VF vscale x 4: PARTIAL-REDUCE ir<%sum.next> = ir<%sum> + fast reduce.fadd (ir<%l>)
;
; SVE2p3-LABEL: 'sum_float'
; SVE2p3:  Cost of 1 for VF vscale x 4: PARTIAL-REDUCE ir<%sum.next> = ir<%sum> + fast reduce.fadd (ir<%l>)
; SVE2p3:  Cost of 1 for VF vscale x 4: PARTIAL-REDUCE ir<%sum.next> = ir<%sum> + fast reduce.fadd (ir<%l>)
;
; SME2-LABEL: 'sum_float'
; SME2:  Cost of 1 for VF vscale x 4: PARTIAL-REDUCE ir<%sum.next> = ir<%sum> + fast reduce.fadd (ir<%l>)
; SME2:  Cost of 1 for VF vscale x 4: PARTIAL-REDUCE ir<%sum.next> = ir<%sum> + fast reduce.fadd (ir<%l>)
;
; I8MM-LABEL: 'sum_float'
; I8MM:  Cost of 1 for VF vscale x 4: PARTIAL-REDUCE ir<%sum.next> = ir<%sum> + fast reduce.fadd (ir<%l>)
; I8MM:  Cost of 1 for VF vscale x 4: PARTIAL-REDUCE ir<%sum.next> = ir<%sum> + fast reduce.fadd (ir<%l>)
;
entry:
  br label %loop
...
loop:
  ...
  %p = getelementptr inbounds float, ptr %in, i64 %iv
  %l = load float, ptr %p, align 4
  %sum.next = fadd fast float %sum, %l
  %iv.next = add nuw nsw i64 %iv, 1
  %ec = icmp eq i64 %iv.next, %limit
  br i1 %ec, label %exit, label %loop, !llvm.loop !42
...
!44 = !{!"llvm.loop.vectorize.width", i32 4}
