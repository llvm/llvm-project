...
; CHECK:  Cost of 6 for VF 2: REPLICATE ir<%14> = load vp<%next.gep> (!alias.scope !tmp0)
; CHECK:  Cost of 6 for VF 2: REPLICATE ir<%17> = load ir<%16> (!alias.scope !tmp0)
; CHECK:  Cost of 6 for VF 2: REPLICATE ir<%20> = load ir<%19> (!alias.scope !tmp1)
; CHECK:  Cost of 6 for VF 2: REPLICATE ir<%23> = load ir<%22> (!alias.scope !tmp1)
; CHECK:  Cost of 6 for VF 2: REPLICATE store ir<%30>, vp<%next.gep>.1 (!alias.scope !tmp2, !noalias !tmp3)
; CHECK:  Cost of 6 for VF 2: REPLICATE ir<%32> = load ir<%31> (!alias.scope !tmp0)
; CHECK:  Cost of 6 for VF 2: REPLICATE ir<%35> = load ir<%34> (!alias.scope !tmp0)
; CHECK:  Cost of 6 for VF 2: REPLICATE ir<%38> = load ir<%37> (!alias.scope !tmp1)
; CHECK:  Cost of 6 for VF 2: REPLICATE ir<%41> = load ir<%40> (!alias.scope !tmp1)
; CHECK:  Cost of 6 for VF 2: REPLICATE store ir<%48>, ir<%49> (!alias.scope !tmp2, !noalias !tmp3)
; CHECK:  Cost for VF 2: 80 (Estimated cost per lane: 40)
...
; CHECK:  Cost of 6 for VF 2: REPLICATE ir<%10> = load vp<%next.gep> (!alias.scope !tmp0)
; CHECK:  Cost of 6 for VF 2: REPLICATE ir<%13> = load ir<%12> (!alias.scope !tmp0)
; CHECK:  Cost of 6 for VF 2: REPLICATE store ir<%18>, vp<%next.gep>.1 (!alias.scope !tmp1, !noalias !tmp0)
; CHECK:  Cost of 6 for VF 2: REPLICATE ir<%20> = load ir<%19> (!alias.scope !tmp0)
; CHECK:  Cost of 6 for VF 2: REPLICATE ir<%23> = load ir<%22> (!alias.scope !tmp0)
; CHECK:  Cost of 6 for VF 2: REPLICATE store ir<%28>, ir<%29> (!alias.scope !tmp1, !noalias !tmp0)
; CHECK:  Cost for VF 2: 52 (Estimated cost per lane: 26)
; CHECK:  Cost of 18 for VF 4: INTERLEAVE-GROUP with factor 4, vp<%next.gep>
; CHECK:    ir<%10> = load from index 0
; CHECK:    ir<%20> = load from index 1
...
; CHECK:    ir<%1> = load from index 0
; CHECK:    ir<%3> = load from index 1 (!alias.scope !tmp1)
; CHECK:  Cost of 6 for VF 2: REPLICATE store ir<%conv>, ir<%arrayidx3> (!alias.scope !tmp2, !noalias !tmp3)
; CHECK:  Cost of 6 for VF 2: REPLICATE store ir<%conv9>, ir<%y11> (!alias.scope !tmp2, !noalias !tmp3)
...
; CHECK:    ir<%3> = load from index 1 (!alias.scope !tmp1)
; CHECK:  Cost of 6 for VF 2: REPLICATE store ir<%conv>, ir<%arrayidx3> (!alias.scope !tmp2, !noalias !tmp3)
; CHECK:  Cost of 6 for VF 2: REPLICATE store ir<%conv8>, ir<%y10> (!alias.scope !tmp2, !noalias !tmp3)
; CHECK:  Cost for VF 2: 46 (Estimated cost per lane: 23)
; CHECK:  Cost of 6 for VF 4: INTERLEAVE-GROUP with factor 2, ir<%arrayidx>
; CHECK:    ir<%0> = load from index 0
; CHECK:    ir<%2> = load from index 1 (!alias.scope !tmp0)
...
; CHECK:  Cost of 6 for VF 2: REPLICATE store ir<%conv>, ir<%arrayidx3> (!alias.scope !tmp2, !noalias !tmp3)
; CHECK:  Cost of 6 for VF 2: REPLICATE store ir<%conv9>, ir<%y11> (!alias.scope !tmp2, !noalias !tmp3)
; CHECK:  Cost of 6 for VF 2: REPLICATE store ir<%conv16>, ir<%z18> (!alias.scope !tmp2, !noalias !tmp3)
; CHECK:  Cost of 6 for VF 2: REPLICATE store ir<%conv23>, ir<%w25> (!alias.scope !tmp2, !noalias !tmp3)
; CHECK:  Cost for VF 2: 88 (Estimated cost per lane: 44)
; CHECK:  Cost of 36 for VF 4: INTERLEAVE-GROUP with factor 4, ir<%arrayidx>
; CHECK:    ir<%0> = load from index 0
; CHECK:    ir<%2> = load from index 1
...
; CHECK:  Cost of 6 for VF 2: REPLICATE store ir<%conv>, ir<%arrayidx3> (!alias.scope !tmp2, !noalias !tmp3)
; CHECK:  Cost of 6 for VF 2: REPLICATE store ir<%conv8>, ir<%y10> (!alias.scope !tmp2, !noalias !tmp3)
; CHECK:  Cost of 6 for VF 2: REPLICATE store ir<%conv14>, ir<%z16> (!alias.scope !tmp2, !noalias !tmp3)
; CHECK:  Cost of 6 for VF 2: REPLICATE store ir<%conv20>, ir<%w22> (!alias.scope !tmp2, !noalias !tmp3)
; CHECK:  Cost for VF 2: 88 (Estimated cost per lane: 44)
; CHECK:  Cost of 36 for VF 4: INTERLEAVE-GROUP with factor 4, ir<%arrayidx>
; CHECK:    ir<%0> = load from index 0
; CHECK:    ir<%2> = load from index 1
