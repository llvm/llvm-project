...
; REQUIRES: asserts
; RUN: opt -mattr=+simd128 -passes=loop-vectorize -debug-only=loop-vectorize,vectorutils -disable-output -print-node-address=false < %s 2>&1 | FileCheck %s
...
; CHECK:    ir<%13> = load from index 1 (!alias.scope <badref>)
; CHECK:  Cost of 11 for VF 4: INTERLEAVE-GROUP with factor 2, vp<%next.gep>.1
; CHECK:    store ir<%11> to index 0
; CHECK:    store ir<%13> to index 1 (!alias.scope <badref>, !noalias <badref>)
...
; CHECK:  Cost of 6 for VF 2: REPLICATE ir<%14> = load vp<%next.gep> (!alias.scope <badref>)
; CHECK:  Cost of 6 for VF 2: REPLICATE ir<%17> = load ir<%16> (!alias.scope <badref>)
; CHECK:  Cost of 6 for VF 2: REPLICATE ir<%20> = load ir<%19> (!alias.scope <badref>)
; CHECK:  Cost of 6 for VF 2: REPLICATE ir<%23> = load ir<%22> (!alias.scope <badref>)
; CHECK:  Cost of 6 for VF 2: REPLICATE store ir<%30>, vp<%next.gep>.1 (!alias.scope <badref>, !noalias <badref>)
; CHECK:  Cost of 6 for VF 2: REPLICATE ir<%32> = load ir<%31> (!alias.scope <badref>)
; CHECK:  Cost of 6 for VF 2: REPLICATE ir<%35> = load ir<%34> (!alias.scope <badref>)
; CHECK:  Cost of 6 for VF 2: REPLICATE ir<%38> = load ir<%37> (!alias.scope <badref>)
; CHECK:  Cost of 6 for VF 2: REPLICATE ir<%41> = load ir<%40> (!alias.scope <badref>)
; CHECK:  Cost of 6 for VF 2: REPLICATE store ir<%48>, ir<%49> (!alias.scope <badref>, !noalias <badref>)
; CHECK:  Cost for VF 2: 80 (Estimated cost per lane: 40)
...
; CHECK:    ir<%35> = load from index 3 (!alias.scope <badref>)
...
; CHECK:    ir<%41> = load from index 3 (!alias.scope <badref>)
; CHECK:  Cost of 11 for VF 4: INTERLEAVE-GROUP with factor 2, vp<%next.gep>.1
; CHECK:    store ir<%30> to index 0
; CHECK:    store ir<%48> to index 1 (!alias.scope <badref>, !noalias <badref>)
...
; CHECK:  Cost of 6 for VF 2: REPLICATE ir<%10> = load vp<%next.gep> (!alias.scope <badref>)
; CHECK:  Cost of 6 for VF 2: REPLICATE ir<%13> = load ir<%12> (!alias.scope <badref>)
; CHECK:  Cost of 6 for VF 2: REPLICATE store ir<%18>, vp<%next.gep>.1 (!alias.scope <badref>, !noalias <badref>)
; CHECK:  Cost of 6 for VF 2: REPLICATE ir<%20> = load ir<%19> (!alias.scope <badref>)
; CHECK:  Cost of 6 for VF 2: REPLICATE ir<%23> = load ir<%22> (!alias.scope <badref>)
; CHECK:  Cost of 6 for VF 2: REPLICATE store ir<%28>, ir<%29> (!alias.scope <badref>, !noalias <badref>)
; CHECK:  Cost for VF 2: 52 (Estimated cost per lane: 26)
; CHECK:  Cost of 18 for VF 4: INTERLEAVE-GROUP with factor 4, vp<%next.gep>
; CHECK:    ir<%10> = load from index 0
; CHECK:    ir<%20> = load from index 1
; CHECK:    ir<%13> = load from index 2
; CHECK:    ir<%23> = load from index 3 (!alias.scope <badref>)
; CHECK:  Cost of 11 for VF 4: INTERLEAVE-GROUP with factor 2, vp<%next.gep>.1
; CHECK:    store ir<%18> to index 0
; CHECK:    store ir<%28> to index 1 (!alias.scope <badref>, !noalias <badref>)
...
; CHECK:    store ir<%sub> to index 1 (!alias.scope <badref>, !noalias <badref>)
; CHECK:  Cost for VF 2: 29 (Estimated cost per lane: 14.5)
; CHECK:  Cost of 6 for VF 4: INTERLEAVE-GROUP with factor 2, ir<%arrayidx>
; CHECK:    ir<%0> = load from index 0
; CHECK:    ir<%2> = load from index 1 (!alias.scope <badref>)
...
; CHECK:  LV: Scalar loop costs: 18.
; CHECK:  Cost of 6 for VF 2: REPLICATE ir<%0> = load ir<%arrayidx> (!alias.scope <badref>)
; CHECK:  Cost of 6 for VF 2: REPLICATE ir<%1> = load ir<%arrayidx1> (!alias.scope <badref>)
; CHECK:  Cost of 6 for VF 2: REPLICATE ir<%2> = load ir<%y> (!alias.scope <badref>)
; CHECK:  Cost of 6 for VF 2: REPLICATE ir<%3> = load ir<%y9> (!alias.scope <badref>)
; CHECK:  Cost of 7 for VF 2: INTERLEAVE-GROUP with factor 2, ir<%arrayidx4>
; CHECK:    store ir<%mul> to index 0
; CHECK:    store ir<%mul11> to index 1 (!alias.scope <badref>, !noalias <badref>)
...
; CHECK:    ir<%1> = load from index 0
; CHECK:    ir<%3> = load from index 1 (!alias.scope <badref>)
; CHECK:  Cost of 6 for VF 2: REPLICATE store ir<%conv>, ir<%arrayidx3> (!alias.scope <badref>, !noalias <badref>)
; CHECK:  Cost of 6 for VF 2: REPLICATE store ir<%conv9>, ir<%y11> (!alias.scope <badref>, !noalias <badref>)
...
; CHECK:    store ir<%conv9> to index 1 (!alias.scope <badref>, !noalias <badref>)
...
; CHECK:    ir<%3> = load from index 1 (!alias.scope <badref>)
; CHECK:  Cost of 6 for VF 2: REPLICATE store ir<%conv>, ir<%arrayidx3> (!alias.scope <badref>, !noalias <badref>)
; CHECK:  Cost of 6 for VF 2: REPLICATE store ir<%conv8>, ir<%y10> (!alias.scope <badref>, !noalias <badref>)
; CHECK:  Cost for VF 2: 46 (Estimated cost per lane: 23)
; CHECK:  Cost of 6 for VF 4: INTERLEAVE-GROUP with factor 2, ir<%arrayidx>
; CHECK:    ir<%0> = load from index 0
; CHECK:    ir<%2> = load from index 1 (!alias.scope <badref>)
...
; CHECK:  Cost of 11 for VF 4: INTERLEAVE-GROUP with factor 2, ir<%arrayidx3>
; CHECK:    store ir<%conv> to index 0
; CHECK:    store ir<%conv8> to index 1 (!alias.scope <badref>, !noalias <badref>)
; CHECK:  Cost for VF 4: 43 (Estimated cost per lane: 10.8)
; CHECK:  LV: Selecting VF: 4.
;
entry:
...
; CHECK:    ir<%6> = load from index 3 (!alias.scope <badref>)
...
; CHECK:    ir<%7> = load from index 3 (!alias.scope <badref>)
...
; CHECK:    store ir<%mul20> to index 3 (!alias.scope <badref>, !noalias <badref>)
...
; CHECK:    store ir<%div> to index 3 (!alias.scope <badref>, !noalias <badref>)
; CHECK:  Cost for VF 2: 54 (Estimated cost per lane: 27)
...
; CHECK:  Cost of 6 for VF 2: REPLICATE ir<%2> = load ir<%y> (!alias.scope <badref>)
; CHECK:  Cost of 6 for VF 2: REPLICATE ir<%3> = load ir<%y9> (!alias.scope <badref>)
; CHECK:  Cost of 6 for VF 2: REPLICATE ir<%4> = load ir<%z> (!alias.scope <badref>)
; CHECK:  Cost of 6 for VF 2: REPLICATE ir<%5> = load ir<%z17> (!alias.scope <badref>)
; CHECK:  Cost of 6 for VF 2: REPLICATE ir<%6> = load ir<%w> (!alias.scope <badref>)
; CHECK:  Cost of 6 for VF 2: REPLICATE ir<%7> = load ir<%w25> (!alias.scope <badref>)
...
; CHECK:    store ir<%mul27> to index 3 (!alias.scope <badref>, !noalias <badref>)
...
; CHECK:  Cost of 6 for VF 2: REPLICATE ir<%3> = load ir<%y9> (!alias.scope <badref>)
; CHECK:  Cost of 6 for VF 2: REPLICATE ir<%4> = load ir<%z> (!alias.scope <badref>)
; CHECK:  Cost of 6 for VF 2: REPLICATE ir<%5> = load ir<%z16> (!alias.scope <badref>)
; CHECK:  Cost of 6 for VF 2: REPLICATE ir<%6> = load ir<%w> (!alias.scope <badref>)
; CHECK:  Cost of 6 for VF 2: REPLICATE ir<%7> = load ir<%w23> (!alias.scope <badref>)
; CHECK:  Cost of 14 for VF 2: INTERLEAVE-GROUP with factor 4, ir<%arrayidx4>
; CHECK:    store ir<%mul> to index 0
; CHECK:    store ir<%add> to index 1
; CHECK:    store ir<%div> to index 2
; CHECK:    store ir<%sub> to index 3 (!alias.scope <badref>, !noalias <badref>)
; CHECK:  Cost for VF 2: 98 (Estimated cost per lane: 49)
; CHECK:  Cost of 18 for VF 4: INTERLEAVE-GROUP with factor 4, ir<%arrayidx>
; CHECK:    ir<%0> = load from index 0
; CHECK:    ir<%2> = load from index 1
...
; CHECK:  Cost of 6 for VF 2: REPLICATE store ir<%conv>, ir<%arrayidx3> (!alias.scope <badref>, !noalias <badref>)
; CHECK:  Cost of 6 for VF 2: REPLICATE store ir<%conv9>, ir<%y11> (!alias.scope <badref>, !noalias <badref>)
; CHECK:  Cost of 6 for VF 2: REPLICATE store ir<%conv16>, ir<%z18> (!alias.scope <badref>, !noalias <badref>)
; CHECK:  Cost of 6 for VF 2: REPLICATE store ir<%conv23>, ir<%w25> (!alias.scope <badref>, !noalias <badref>)
; CHECK:  Cost for VF 2: 88 (Estimated cost per lane: 44)
; CHECK:  Cost of 36 for VF 4: INTERLEAVE-GROUP with factor 4, ir<%arrayidx>
; CHECK:    ir<%0> = load from index 0
; CHECK:    ir<%2> = load from index 1
...
; CHECK:    store ir<%conv23> to index 3 (!alias.scope <badref>, !noalias <badref>)
...
; CHECK:  Cost of 6 for VF 2: REPLICATE store ir<%conv>, ir<%arrayidx3> (!alias.scope <badref>, !noalias <badref>)
; CHECK:  Cost of 6 for VF 2: REPLICATE store ir<%conv8>, ir<%y10> (!alias.scope <badref>, !noalias <badref>)
; CHECK:  Cost of 6 for VF 2: REPLICATE store ir<%conv14>, ir<%z16> (!alias.scope <badref>, !noalias <badref>)
; CHECK:  Cost of 6 for VF 2: REPLICATE store ir<%conv20>, ir<%w22> (!alias.scope <badref>, !noalias <badref>)
; CHECK:  Cost for VF 2: 88 (Estimated cost per lane: 44)
; CHECK:  Cost of 36 for VF 4: INTERLEAVE-GROUP with factor 4, ir<%arrayidx>
; CHECK:    ir<%0> = load from index 0
; CHECK:    ir<%2> = load from index 1
...
; CHECK:    store ir<%conv20> to index 3 (!alias.scope <badref>, !noalias <badref>)
; CHECK:  Cost for VF 4: 126 (Estimated cost per lane: 31.5)
; CHECK:  LV: Selecting VF: 1.
;
entry:
