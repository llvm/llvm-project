...
; CHECK-NEXT:    [[TMP15:%.*]] = freeze i1 [[TMP13]]
; CHECK-NEXT:    [[TMP14:%.*]] = icmp eq i64 [[INDEX_NEXT]], 128
; CHECK-NEXT:    [[TMP20:%.*]] = or i1 [[TMP15]], [[TMP14]]
; CHECK-NEXT:    br i1 [[TMP20]], label %[[MIDDLE_BLOCK:.*]], label %[[VECTOR_BODY]], !llvm.loop [[LOOP0:![0-9]+]]
; CHECK:       [[MIDDLE_BLOCK]]:
; CHECK-NEXT:    [[TMP16:%.*]] = select i1 [[TMP15]], <16 x float> [[VEC_PHI]], <16 x float> [[TMP4]]
; CHECK-NEXT:    [[TMP17:%.*]] = select i1 [[TMP15]], <16 x float> [[VEC_PHI1]], <16 x float> [[TMP5]]
; CHECK-NEXT:    [[TMP18:%.*]] = select i1 [[TMP15]], <16 x float> [[VEC_PHI2]], <16 x float> [[TMP6]]
; CHECK-NEXT:    [[TMP19:%.*]] = select i1 [[TMP15]], <16 x float> [[VEC_PHI3]], <16 x float> [[TMP7]]
; CHECK-NEXT:    [[TMP22:%.*]] = select i1 [[TMP15]], i64 [[INDEX]], i64 128
...
; CHECK:       [[LOOP]]:
; CHECK-NEXT:    [[IV:%.*]] = phi i64 [ [[TMP22]], %[[SCALAR_PH]] ], [ [[IV_NEXT:%.*]], %[[LOOP]] ]
