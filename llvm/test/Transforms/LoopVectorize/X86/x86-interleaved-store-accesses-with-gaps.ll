...
; RUN: opt -mcpu=skx -S -passes='loop-vectorize,instcombine,loop-mssa(licm)' -force-vector-width=4 -force-vector-interleave=1 -enable-interleaved-mem-accesses -tail-folding-policy=must-fold-tail < %s | FileCheck %s -check-prefix=DISABLED_MASKED_STRIDED
