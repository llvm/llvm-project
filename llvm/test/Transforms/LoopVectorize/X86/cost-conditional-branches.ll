...
define i32 @cost_ashr_with_op_known_invariant_via_scev(i8 %a, ptr %p) {
...
; CHECK-NEXT:    br label [[VECTOR_BODY:%.*]]
; CHECK:       vector.body:
; CHECK-NEXT:    [[INDEX:%.*]] = phi i32 [ 0, [[LOOP_HEADER1]] ], [ [[INDEX_NEXT:%.*]], [[PRED_UREM_CONTINUE6:%.*]] ]
; CHECK-NEXT:    [[DOTCAST:%.*]] = trunc i32 [[INDEX]] to i8
; CHECK-NEXT:    [[OFFSET_IDX:%.*]] = sub i8 100, [[DOTCAST]]
...
; CHECK-NEXT:    [[TMP7:%.*]] = extractelement <4 x i32> [[PREDPHI]], i32 0
; CHECK-NEXT:    [[TMP8:%.*]] = ashr i32 [[CONV5_I]], [[TMP7]]
; CHECK-NEXT:    [[BROADCAST_SPLATINSERT7:%.*]] = insertelement <4 x i32> poison, i32 [[TMP8]], i64 0
; CHECK-NEXT:    [[BROADCAST_SPLAT8:%.*]] = shufflevector <4 x i32> [[BROADCAST_SPLATINSERT7]], <4 x i32> poison, <4 x i32> zeroinitializer
; CHECK-NEXT:    [[TMP9:%.*]] = icmp eq <4 x i32> [[BROADCAST_SPLAT8]], zeroinitializer
; CHECK-NEXT:    [[TMP10:%.*]] = shl <4 x i32> [[PREDPHI]], splat (i32 24)
; CHECK-NEXT:    [[TMP11:%.*]] = ashr exact <4 x i32> [[TMP10]], splat (i32 24)
; CHECK-NEXT:    [[TMP12:%.*]] = extractelement <4 x i1> [[TMP9]], i32 0
; CHECK-NEXT:    [[TMP13:%.*]] = select i1 [[TMP12]], <4 x i32> [[TMP11]], <4 x i32> zeroinitializer
; CHECK-NEXT:    [[PREDPHI9:%.*]] = select <4 x i1> [[TMP6]], <4 x i32> [[TMP13]], <4 x i32> zeroinitializer
; CHECK-NEXT:    [[TMP14:%.*]] = getelementptr i32, ptr [[P:%.*]], i8 [[OFFSET_IDX]]
; CHECK-NEXT:    [[TMP15:%.*]] = getelementptr i32, ptr [[TMP14]], i64 0
; CHECK-NEXT:    [[TMP16:%.*]] = getelementptr i32, ptr [[TMP15]], i64 -3
; CHECK-NEXT:    [[REVERSE:%.*]] = shufflevector <4 x i32> [[PREDPHI9]], <4 x i32> poison, <4 x i32> <i32 3, i32 2, i32 1, i32 0>
; CHECK-NEXT:    store <4 x i32> [[REVERSE]], ptr [[TMP16]], align 4
...
loop.latch:
  ...
  %gep = getelementptr i32, ptr %p, i8 %iv
  store i32 %p.2, ptr %gep
  %iv.next = add i8 %iv, -1
...
; CHECK-NEXT:    br i1 [[TMP43]], label [[MIDDLE_BLOCK:%.*]], label [[VECTOR_BODY]], !llvm.loop [[LOOP11:![0-9]+]]
; CHECK:       middle.block:
; CHECK-NEXT:    br label [[SCALAR_PH:%.*]]
; CHECK:       scalar.ph:
; CHECK-NEXT:    br label [[LOOP_HEADER:%.*]]
...
; CHECK-NEXT:    br i1 [[TMP163]], label [[MIDDLE_BLOCK:%.*]], label [[VECTOR_BODY]], !llvm.loop [[LOOP13:![0-9]+]]
; CHECK:       middle.block:
...
; CHECK-NEXT:    br i1 [[TMP208]], label [[VEC_EPILOG_MIDDLE_BLOCK:%.*]], label [[VEC_EPILOG_VECTOR_BODY]], !llvm.loop [[LOOP15:![0-9]+]]
; CHECK:       vec.epilog.middle.block:
...
; CHECK-NEXT:    br i1 [[EC]], label [[EXIT]], label [[LOOP_HEADER]], !llvm.loop [[LOOP16:![0-9]+]]
; CHECK:       exit:
