...
; RUN: opt -mtriple=amdgcn-amd-amdhsa -mcpu=gfx900 < %s -passes=loop-vectorize -S | FileCheck -check-prefix=GFX9 %s
; RUN: opt -mtriple=amdgcn-amd-amdhsa -mcpu=fiji < %s -passes=loop-vectorize -S | FileCheck -check-prefix=VI %s
; RUN: opt -mtriple=amdgcn-amd-amdhsa -mcpu=hawaii < %s -passes=loop-vectorize -S | FileCheck -check-prefix=CI %s
...
; GFX9-NEXT:    [[TMP1:%.*]] = getelementptr inbounds half, ptr addrspace(1) [[TMP0]], i64 2
...
; VI-NEXT:    [[TMP1:%.*]] = getelementptr inbounds half, ptr addrspace(1) [[TMP0]], i64 2
...
; VI-NEXT:    [[TMP5:%.*]] = call fast half @llvm.vector.reduce.fadd.v2f16(half 0.000000e+00, <2 x half> [[BIN_RDX]])
; VI-NEXT:    br label [[FOR_END:%.*]]
; VI:       for.end:
; VI-NEXT:    ret half [[TMP5]]
;
...
; CI:       for.end:
; CI-NEXT:    [[ADD_LCSSA:%.*]] = phi half [ [[ADD]], [[FOR_BODY]] ]
; CI-NEXT:    ret half [[ADD_LCSSA]]
;
entry:
  br label %for.body
