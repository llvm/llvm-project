...
; ZVE32X-NEXT:    [[WIDE_LOAD:%.*]] = load <8 x i32>, ptr [[TMP4]], align 4
...
; ZVE32X-NEXT:    [[TMP7:%.*]] = call i1 @llvm.vector.reduce.or.v8i1(<8 x i1> [[TMP2]])
; ZVE32X-NEXT:    [[TMP6:%.*]] = icmp eq i64 [[INDEX_NEXT]], 1024
; ZVE32X-NEXT:    br i1 [[TMP7]], label %[[VECTOR_EARLY_EXIT:.*]], label %[[VECTOR_BODY_INTERIM]]
; ZVE32X:       [[VECTOR_BODY_INTERIM]]:
; ZVE32X-NEXT:    br i1 [[TMP6]], label %[[MIDDLE_BLOCK:.*]], label %[[VECTOR_BODY]], !llvm.loop [[LOOP0:![0-9]+]]
; ZVE32X:       [[MIDDLE_BLOCK]]:
; ZVE32X-NEXT:    br label %[[EXIT:.*]]
; ZVE32X:       [[VECTOR_EARLY_EXIT]]:
; ZVE32X-NEXT:    [[FIRST_ACTIVE_LANE:%.*]] = call i32 @llvm.experimental.cttz.elts.i32.v8i1(<8 x i1> [[TMP1]], i1 false)
; ZVE32X-NEXT:    [[TMP5:%.*]] = extractelement <8 x i32> [[WIDE_LOAD]], i32 [[FIRST_ACTIVE_LANE]]
; ZVE32X-NEXT:    br label %[[EXIT]]
; ZVE32X:       [[EXIT]]:
