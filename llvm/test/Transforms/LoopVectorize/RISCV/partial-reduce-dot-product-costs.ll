...
; CHECK:  Cost of 5 for VF vscale x 8: EXPRESSION vp<[[VP10:%[0-9]+]]> = ir<%acc> + partial.reduce.add (mul (ir<%la> sext to i64), (ir<%lb> sext to i64), vp<%evl>)
;
entry:
  br label %loop
loop:
...
; CHECK:  Cost of 5 for VF vscale x 8: EXPRESSION vp<[[VP10:%[0-9]+]]> = ir<%acc> + partial.reduce.add (mul (ir<%la> zext to i64), (ir<%lb> zext to i64), vp<%evl>)
;
entry:
  br label %loop
loop:
...
; CHECK:  Cost of 5 for VF vscale x 16: EXPRESSION vp<[[VP10:%[0-9]+]]> = ir<%acc> + partial.reduce.add (mul (ir<%la> sext to i64), (ir<%lb> sext to i64), vp<%evl>)
;
entry:
  br label %loop
loop:
