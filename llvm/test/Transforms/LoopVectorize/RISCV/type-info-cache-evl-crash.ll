...
; This test tries to recreate the conditions for a crash that occurred when a
; recipe was erased and replaced with a new one during EVL optimization.
...
; CHECK:       [[VECTOR_PH]]:
; CHECK-NEXT:    [[BROADCAST_SPLATINSERT:%.*]] = insertelement <vscale x 32 x ptr> poison, ptr [[DSTV]], i64 0
; CHECK-NEXT:    [[BROADCAST_SPLAT:%.*]] = shufflevector <vscale x 32 x ptr> [[BROADCAST_SPLATINSERT]], <vscale x 32 x ptr> poison, <vscale x 32 x i32> zeroinitializer
; CHECK-NEXT:    br label %[[VECTOR_BODY:.*]]
; CHECK:       [[VECTOR_BODY]]:
; CHECK-NEXT:    [[EVL_BASED_IV:%.*]] = phi i64 [ 0, %[[VECTOR_PH]] ], [ [[INDEX_EVL_NEXT:%.*]], %[[VECTOR_BODY]] ]
; CHECK-NEXT:    [[AVL:%.*]] = phi i64 [ [[TMP0]], %[[VECTOR_PH]] ], [ [[AVL_NEXT:%.*]], %[[VECTOR_BODY]] ]
; CHECK-NEXT:    [[TMP11:%.*]] = call i32 @llvm.experimental.get.vector.length.i64(i64 [[AVL]], i32 32, i1 true)
; CHECK-NEXT:    [[TMP13:%.*]] = getelementptr i8, ptr [[SRC]], i64 [[EVL_BASED_IV]]
; CHECK-NEXT:    [[VP_OP_LOAD:%.*]] = call <vscale x 32 x i8> @llvm.vp.load.nxv32i8.p0(ptr align 1 [[TMP13]], <vscale x 32 x i1> splat (i1 true), i32 [[TMP11]]), !alias.scope [[META0:![0-9]+]]
; CHECK-NEXT:    [[TMP4:%.*]] = zext <vscale x 32 x i8> [[VP_OP_LOAD]] to <vscale x 32 x i32>
; CHECK-NEXT:    [[TMP9:%.*]] = ashr <vscale x 32 x i32> [[TMP4]], zeroinitializer
; CHECK-NEXT:    [[TMP6:%.*]] = icmp ult <vscale x 32 x i32> [[TMP4]], zeroinitializer
; CHECK-NEXT:    [[TMP7:%.*]] = select <vscale x 32 x i1> [[TMP6]], <vscale x 32 x i32> [[TMP9]], <vscale x 32 x i32> zeroinitializer
; CHECK-NEXT:    [[TMP8:%.*]] = trunc <vscale x 32 x i32> [[TMP7]] to <vscale x 32 x i8>
; CHECK-NEXT:    call void @llvm.vp.scatter.nxv32i8.nxv32p0(<vscale x 32 x i8> [[TMP8]], <vscale x 32 x ptr> align 1 [[BROADCAST_SPLAT]], <vscale x 32 x i1> splat (i1 true), i32 [[TMP11]]), !alias.scope [[META3:![0-9]+]], !noalias [[META0]]
; CHECK-NEXT:    call void @llvm.vp.scatter.nxv32i16.nxv32p0(<vscale x 32 x i16> zeroinitializer, <vscale x 32 x ptr> align 2 splat (ptr null), <vscale x 32 x i1> splat (i1 true), i32 [[TMP11]])
