...
; CHECK:       [[VECTOR_PH]]:
; CHECK-NEXT:    [[BROADCAST_SPLATINSERT:%.*]] = insertelement <vscale x 16 x float> poison, float [[A_0]], i64 0
; CHECK-NEXT:    [[BROADCAST_SPLAT:%.*]] = shufflevector <vscale x 16 x float> [[BROADCAST_SPLATINSERT]], <vscale x 16 x float> poison, <vscale x 16 x i32> zeroinitializer
...
; CHECK-NEXT:    [[TMP6]] = call float @llvm.vp.reduce.fadd.nxv16f32(float [[VEC_PHI]], <vscale x 16 x float> [[BROADCAST_SPLAT]], <vscale x 16 x i1> splat (i1 true), i32 [[TMP9]])
