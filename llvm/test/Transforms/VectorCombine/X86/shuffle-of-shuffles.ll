...
; inner shuffle masks are entirely poison - merged mask references no real source, fold to poison

define <4 x i32> @concat_inner_shuffles_all_poison_masks(<4 x i32> %x, <4 x i32> %y) {
; CHECK-LABEL: define <4 x i32> @concat_inner_shuffles_all_poison_masks(
; CHECK-SAME: <4 x i32> [[X:%.*]], <4 x i32> [[Y:%.*]]) #[[ATTR0]] {
; CHECK-NEXT:    ret <4 x i32> poison
;
  %a = shufflevector <4 x i32> %x, <4 x i32> poison, <4 x i32> <i32 poison, i32 poison, i32 poison, i32 poison>
  %b = shufflevector <4 x i32> %y, <4 x i32> poison, <4 x i32> <i32 poison, i32 poison, i32 poison, i32 poison>
