...
target datalayout = "e-m:e-p270:32:32-p271:32:32-p272:64:64-p1:128:128:128:128-i64:64-f80:128-n8:16:32:64-S128"
...
define <2 x float> @test_wide_index(ptr addrspace(1) %p) {
; CHECK-LABEL: define <2 x float> @test_wide_index(
; CHECK-SAME: ptr addrspace(1) [[P:%.*]]) {
; CHECK-NEXT:    [[TMP1:%.*]] = getelementptr i8, ptr addrspace(1) [[P]], i128 48
; CHECK-NEXT:    [[R:%.*]] = load <2 x float>, ptr addrspace(1) [[TMP1]], align 8
; CHECK-NEXT:    ret <2 x float> [[R]]
;
  %q = getelementptr i8, ptr addrspace(1) %p, i128 40
  %v = load <4 x float>, ptr addrspace(1) %q, align 16
  %r = shufflevector <4 x float> %v, <4 x float> poison, <2 x i32> <i32 2, i32 3>
  ret <2 x float> %r
}
