...
; RUN: llc %s -o - | FileCheck %s --check-prefixes=COMMON,DEFAULT
...
; PREINDEX-NEXT:    cbnz x2, .LBB4_1
; PREINDEX-NEXT:  // %bb.2: // %for.exit
; PREINDEX-NEXT:    ret
;
; POSTINDEX-LABEL: mixed_offsets_fixed_then_scalable:
; POSTINDEX:       // %bb.0: // %entry
; POSTINDEX-NEXT:    ptrue p0.s
; POSTINDEX-NEXT:    mov x8, xzr
; POSTINDEX-NEXT:    add x9, x0, #32
; POSTINDEX-NEXT:  .LBB4_1: // %for.body
; POSTINDEX-NEXT:    // =>This Inner Loop Header: Depth=1
; POSTINDEX-NEXT:    ldr z0, [x9]
; POSTINDEX-NEXT:    ldr z1, [x9, #4, mul vl]
; POSTINDEX-NEXT:    incb x9
; POSTINDEX-NEXT:    ld1w { z2.s }, p0/z, [x0, x8, lsl #2]
; POSTINDEX-NEXT:    add z0.s, z0.s, z1.s
; POSTINDEX-NEXT:    add z0.s, z2.s, z0.s
; POSTINDEX-NEXT:    st1w { z0.s }, p0, [x1, x8, lsl #2]
; POSTINDEX-NEXT:    incw x8
; POSTINDEX-NEXT:    cmp x2, x8
; POSTINDEX-NEXT:    b.ne .LBB4_1
...
; COMMON-NEXT:    add x9, x1, x2, lsl #2
; COMMON-NEXT:  .LBB10_1: // %vector.body
; COMMON-NEXT:    // =>This Inner Loop Header: Depth=1
; COMMON-NEXT:    ldr z0, [x1]
; COMMON-NEXT:    ldr z1, [x9]
; COMMON-NEXT:    addvl x2, x2, #-8
; COMMON-NEXT:    ldr z2, [x1, #1, mul vl]
; COMMON-NEXT:    ldr z3, [x9, #1, mul vl]
; COMMON-NEXT:    add x9, x9, x8
