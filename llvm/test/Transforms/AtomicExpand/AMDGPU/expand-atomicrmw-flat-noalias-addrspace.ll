...
; RUN: opt -S -mtriple=amdgpu7.00-amd-amdhsa -passes='require<libcall-lowering-info>,atomic-expand' %s | FileCheck -check-prefixes=ALL,GFX7 %s
; RUN: opt -S -mtriple=amdgpu9.00-amd-amdhsa -passes='require<libcall-lowering-info>,atomic-expand' %s | FileCheck -check-prefixes=ALL,GFX900 %s
...
; RUN: opt -S -mtriple=amdgpu12.00-amd-amdhsa -passes='require<libcall-lowering-info>,atomic-expand' %s | FileCheck -check-prefixes=ALL,GFX12 %s
...
; ALL-SAME: ptr [[PTR:%.*]], ptr addrspace(1) [[VALUE:%.*]]) {
; ALL-NEXT:    [[IS_PRIVATE:%.*]] = call i1 @llvm.amdgcn.is.private(ptr [[PTR]])
; ALL-NEXT:    br i1 [[IS_PRIVATE]], label %[[ATOMICRMW_PRIVATE:.*]], label %[[ATOMICRMW_GLOBAL:.*]]
; ALL:       [[ATOMICRMW_PRIVATE]]:
; ALL-NEXT:    [[TMP1:%.*]] = addrspacecast ptr [[PTR]] to ptr addrspace(5)
...
; ALL-SAME: ptr [[PTR:%.*]], ptr addrspace(3) [[VALUE:%.*]]) {
...
; GFX942-SAME: ptr [[PTR:%.*]], double [[VALUE:%.*]]) {
...
; GFX942-SAME: ptr [[PTR:%.*]], <2 x half> [[VALUE:%.*]]) {
...
; GFX12-SAME: ptr [[PTR:%.*]], <2 x half> [[VALUE:%.*]]) {
...
; GFX942-SAME: ptr [[PTR:%.*]], <2 x bfloat> [[VALUE:%.*]]) {
...
; GFX12-SAME: ptr [[PTR:%.*]], <2 x bfloat> [[VALUE:%.*]]) {
