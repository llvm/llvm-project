...
; RUN: opt -mtriple=amdgpu9.00-amd-amdhsa -S -passes='require<libcall-lowering-info>,atomic-expand' %s | FileCheck -check-prefixes=CHECK,GFX900 %s
; RUN: opt -mtriple=amdgpu9.0a-amd-amdhsa -S -passes='require<libcall-lowering-info>,atomic-expand' %s | FileCheck -check-prefixes=CHECK,GFX90A %s
; RUN: opt -mtriple=amdgpu9.42-amd-amdhsa -S -passes='require<libcall-lowering-info>,atomic-expand' %s | FileCheck -check-prefixes=CHECK,GFX942 %s
...
; GFX900-SAME: ptr addrspace(1) [[PTR:%.*]], <2 x half> [[VALUE:%.*]]) {
...
; GFX942-SAME: ptr addrspace(1) [[PTR:%.*]], <2 x half> [[VALUE:%.*]]) {
...
; GFX900-SAME: ptr addrspace(1) [[PTR:%.*]], <2 x bfloat> [[VALUE:%.*]]) {
...
;
; GFX942-LABEL: define <2 x bfloat> @test_atomicrmw_fadd_v2bf16_global_agent_align4(
...
; CHECK-SAME: ptr addrspace(1) [[PTR:%.*]], <4 x half> [[VALUE:%.*]]) {
...
; CHECK-SAME: ptr addrspace(1) [[PTR:%.*]], <4 x bfloat> [[VALUE:%.*]]) {
...
; CHECK-SAME: ptr addrspace(1) [[PTR:%.*]], <2 x float> [[VALUE:%.*]]) {
...
; CHECK-SAME: ptr addrspace(1) [[PTR:%.*]], <2 x half> [[VALUE:%.*]]) {
...
; CHECK-SAME: ptr addrspace(1) [[PTR:%.*]], <2 x bfloat> [[VALUE:%.*]]) {
