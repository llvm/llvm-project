...
; CHECK-SAME: i32 [[N:%.*]], ptr nofree readonly captures(none) [[SRC:%.*]], ptr noalias nofree writeonly captures(none) [[DST:%.*]]) local_unnamed_addr #[[ATTR0:[0-9]+]] {
; CHECK-NEXT:  entry:
...
; CHECK-NEXT:    [[TMP1:%.*]] = and i32 [[N]], 2147483646
; CHECK-NEXT:    [[UNROLL_ITER:%.*]] = zext nneg i32 [[TMP1]] to i64
...
; CHECK-NEXT:    br i1 [[LCMP_MOD_NOT]], label [[LOOP_LATCH_EPIL_PREHEADER]], label [[EXIT]]
...
; CHECK-SAME: i32 [[N:%.*]], ptr nofree readonly captures(none) [[SRC:%.*]], ptr noalias nofree writeonly captures(none) [[DST:%.*]]) local_unnamed_addr #[[ATTR0]] {
; CHECK-NEXT:  entry:
