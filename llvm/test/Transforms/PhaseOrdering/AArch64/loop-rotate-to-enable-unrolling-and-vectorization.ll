...
; CHECK-NEXT:    br label %[[COMMON_RET]]
; CHECK:       [[COMMON_RET]]:
; CHECK-NEXT:    [[COMMON_RET_OP:%.*]] = phi ptr [ [[END]], %[[ENTRY]] ], [ [[END]], %[[FOR_BODY_PROL_LOOPEXIT]] ], [ [[PTR_DEC_2_LE]], %[[COMMON_RET_LOOPEXIT_UNR_LCSSA_LOOPEXIT_SPLIT_LOOP_EXIT10]] ], [ [[PTR_DEC_1_LE]], %[[COMMON_RET_LOOPEXIT_UNR_LCSSA_LOOPEXIT_SPLIT_LOOP_EXIT12]] ], [ [[PTR_DEC_LE]], %[[COMMON_RET_LOOPEXIT_UNR_LCSSA_LOOPEXIT_SPLIT_LOOP_EXIT14]] ], [ [[END]], %[[FOR_COND_3]] ], [ [[PTR_DEC3]], %[[FOR_BODY]] ], [ [[PTR_DEC5]], %[[FOR_BODY_PROL]] ]
...
; CHECK-SAME: ptr noalias [[SCAN:%.*]], ptr noalias [[MATCH:%.*]]) local_unnamed_addr #[[ATTR1:[0-9]+]] {
; CHECK-NEXT:  [[ENTRY:.*]]:
; CHECK-NEXT:    call void @llvm.assume(i1 true) [ "dereferenceable"(ptr [[SCAN]], i64 1024) ]
; CHECK-NEXT:    call void @llvm.assume(i1 true) [ "dereferenceable"(ptr [[MATCH]], i64 1024) ]
; CHECK-NEXT:    br label %[[HEADER:.*]]
; CHECK:       [[HEADER]]:
; CHECK-NEXT:    [[LEN:%.*]] = phi i64 [ 0, %[[ENTRY]] ], [ [[INDEX_NEXT:%.*]], %[[HEADER]] ]
; CHECK-NEXT:    [[TMP0:%.*]] = phi i64 [ 15, %[[ENTRY]] ], [ [[TMP9:%.*]], %[[HEADER]] ]
...
; CHECK-NEXT:    [[DOTNOT:%.*]] = icmp ne i16 [[TMP4]], 0
; CHECK-NEXT:    [[TMP11:%.*]] = icmp eq i64 [[INDEX_NEXT]], 1024
; CHECK-NEXT:    [[TMP8:%.*]] = or i1 [[DOTNOT]], [[TMP11]]
; CHECK-NEXT:    [[TMP9]] = add i64 [[TMP0]], 16
; CHECK-NEXT:    br i1 [[TMP8]], label %[[MIDDLE_SPLIT:.*]], label %[[HEADER]], !llvm.loop [[LOOP2:![0-9]+]]
; CHECK:       [[MIDDLE_SPLIT]]:
; CHECK-NEXT:    br i1 [[DOTNOT]], label %[[EXIT:.*]], label %[[VECTOR_EARLY_EXIT:.*]]
; CHECK:       [[VECTOR_EARLY_EXIT]]:
; CHECK-NEXT:    [[TMP10:%.*]] = add nuw nsw i64 [[TMP0]], 1
; CHECK-NEXT:    br label %[[EXIT1:.*]]
; CHECK:       [[EXIT]]:
; CHECK-NEXT:    [[TMP6:%.*]] = tail call i64 @llvm.experimental.cttz.elts.i64.v16i1(<16 x i1> [[TMP3]], i1 true)
; CHECK-NEXT:    [[TMP7:%.*]] = add i64 [[LEN]], [[TMP6]]
; CHECK-NEXT:    br label %[[EXIT1]]
; CHECK:       [[EXIT1]]:
; CHECK-NEXT:    [[LEN_LCSSA:%.*]] = phi i64 [ [[TMP10]], %[[VECTOR_EARLY_EXIT]] ], [ [[TMP7]], %[[EXIT]] ]
