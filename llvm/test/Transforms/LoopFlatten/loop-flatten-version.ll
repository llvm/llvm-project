...
; CHECK-VERSION-OVER-WIDEN-NEXT:    br i1 [[CMP3]], label [[FOR_INNER_PREHEADER_PH:%.*]], label [[FOR_END:%.*]]
; CHECK-VERSION-OVER-WIDEN:       for.outer.preheader:
; CHECK-VERSION-OVER-WIDEN-NEXT:    br label [[FOR_INNER_PREHEADER:%.*]]
; CHECK-VERSION-OVER-WIDEN:       for.inner.preheader:
; CHECK-VERSION-OVER-WIDEN-NEXT:    [[I:%.*]] = phi i64 [ 0, [[FOR_INNER_PREHEADER_PH]] ], [ [[INC2:%.*]], [[FOR_OUTER:%.*]] ]
; CHECK-VERSION-OVER-WIDEN-NEXT:    br label [[FOR_INNER:%.*]]
; CHECK-VERSION-OVER-WIDEN:       for.inner:
; CHECK-VERSION-OVER-WIDEN-NEXT:    [[J:%.*]] = phi i64 [ 0, [[FOR_INNER_PREHEADER]] ], [ [[INC1:%.*]], [[FOR_INNER]] ]
...
; CHECK-VERSION-OVER-WIDEN-NEXT:    store i32 0, ptr [[ARRAYIDX]], align 4
; CHECK-VERSION-OVER-WIDEN-NEXT:    [[INC1]] = add nuw i64 [[J]], 1
; CHECK-VERSION-OVER-WIDEN-NEXT:    [[CMP2:%.*]] = icmp ult i64 [[INC1]], [[N]]
; CHECK-VERSION-OVER-WIDEN-NEXT:    br i1 [[CMP2]], label [[FOR_INNER]], label [[FOR_OUTER]]
; CHECK-VERSION-OVER-WIDEN:       for.outer:
; CHECK-VERSION-OVER-WIDEN-NEXT:    [[INC2]] = add i64 [[I]], 1
; CHECK-VERSION-OVER-WIDEN-NEXT:    [[CMP1:%.*]] = icmp ult i64 [[INC2]], [[N]]
; CHECK-VERSION-OVER-WIDEN-NEXT:    br i1 [[CMP1]], label [[FOR_INNER_PREHEADER]], label [[FOR_END_LOOPEXIT_LOOPEXIT1:%.*]]
; CHECK-VERSION-OVER-WIDEN:       for.end.loopexit:
; CHECK-VERSION-OVER-WIDEN-NEXT:    br label [[FOR_END]]
; CHECK-VERSION-OVER-WIDEN:       for.end:
...
; CHECK-VERSION-OVER-WIDEN-NEXT:    br label [[FOR_COND1_FOR_COND_CLEANUP3_CRIT_EDGE_US_LOOPEXIT:%.*]]
; CHECK-VERSION-OVER-WIDEN:       for.cond1.for.cond.cleanup3_crit_edge.us.loopexit.loopexit1:
; CHECK-VERSION-OVER-WIDEN-NEXT:    br label [[FOR_COND1_FOR_COND_CLEANUP3_CRIT_EDGE_US_LOOPEXIT]]
; CHECK-VERSION-OVER-WIDEN:       for.cond1.for.cond.cleanup3_crit_edge.us.loopexit:
; CHECK-VERSION-OVER-WIDEN-NEXT:    br label [[FOR_COND1_FOR_COND_CLEANUP3_CRIT_EDGE_US]]
; CHECK-VERSION-OVER-WIDEN:       for.cond1.for.cond.cleanup3_crit_edge.us.loopexit50:
; CHECK-VERSION-OVER-WIDEN-NEXT:    br label [[FOR_COND1_FOR_COND_CLEANUP3_CRIT_EDGE_US]]
; CHECK-VERSION-OVER-WIDEN:       for.cond1.for.cond.cleanup3_crit_edge.us:
; CHECK-VERSION-OVER-WIDEN-NEXT:    [[INC13_US]] = add nuw nsw i32 [[K_031_US]], 1
; CHECK-VERSION-OVER-WIDEN-NEXT:    [[EXITCOND52:%.*]] = icmp ne i32 [[INC13_US]], [[N]]
; CHECK-VERSION-OVER-WIDEN-NEXT:    br i1 [[EXITCOND52]], label [[FOR_COND1_PREHEADER_US]], label [[FOR_COND_CLEANUP_LOOPEXIT:%.*]]
; CHECK-VERSION-OVER-WIDEN:       for.cond5.preheader.us.us:
; CHECK-VERSION-OVER-WIDEN-NEXT:    [[I_028_US_US:%.*]] = phi i32 [ [[INC10_US_US:%.*]], [[FOR_COND5_FOR_COND_CLEANUP7_CRIT_EDGE_US_US:%.*]] ], [ 0, [[FOR_COND5_PREHEADER_US_US_PH]] ]
; CHECK-VERSION-OVER-WIDEN-NEXT:    [[MUL_US_US:%.*]] = mul nsw i32 [[I_028_US_US]], [[M]]
; CHECK-VERSION-OVER-WIDEN-NEXT:    br label [[FOR_BODY8_US_US:%.*]]
; CHECK-VERSION-OVER-WIDEN:       for.cond5.for.cond.cleanup7_crit_edge.us.us:
; CHECK-VERSION-OVER-WIDEN-NEXT:    [[INC10_US_US]] = add nuw nsw i32 [[I_028_US_US]], 1
; CHECK-VERSION-OVER-WIDEN-NEXT:    [[EXITCOND51:%.*]] = icmp ne i32 [[INC10_US_US]], [[FLATTEN_TRIPCOUNT]]
; CHECK-VERSION-OVER-WIDEN-NEXT:    br i1 [[EXITCOND51]], label [[FOR_COND5_PREHEADER_US_US]], label [[FOR_COND1_FOR_COND_CLEANUP3_CRIT_EDGE_US_LOOPEXIT_LOOPEXIT1:%.*]]
; CHECK-VERSION-OVER-WIDEN:       for.body8.us.us:
; CHECK-VERSION-OVER-WIDEN-NEXT:    [[J_026_US_US:%.*]] = phi i32 [ 0, [[FOR_COND5_PREHEADER_US_US]] ]
; CHECK-VERSION-OVER-WIDEN-NEXT:    [[ADD_US_US:%.*]] = add nsw i32 [[J_026_US_US]], [[MUL_US_US]]
; CHECK-VERSION-OVER-WIDEN-NEXT:    [[IDXPROM_US_US:%.*]] = sext i32 [[I_028_US_US]] to i64
; CHECK-VERSION-OVER-WIDEN-NEXT:    [[ARRAYIDX_US_US:%.*]] = getelementptr inbounds i32, ptr [[A]], i64 [[IDXPROM_US_US]]
; CHECK-VERSION-OVER-WIDEN-NEXT:    tail call void @f(ptr [[ARRAYIDX_US_US]])
; CHECK-VERSION-OVER-WIDEN-NEXT:    [[INC_US_US:%.*]] = add nuw nsw i32 [[J_026_US_US]], 1
; CHECK-VERSION-OVER-WIDEN-NEXT:    [[EXITCOND:%.*]] = icmp ne i32 [[INC_US_US]], [[M]]
; CHECK-VERSION-OVER-WIDEN-NEXT:    br label [[FOR_COND5_FOR_COND_CLEANUP7_CRIT_EDGE_US_US]]
; CHECK-VERSION-OVER-WIDEN:       for.cond.cleanup.loopexit:
; CHECK-VERSION-OVER-WIDEN-NEXT:    br label [[FOR_COND_CLEANUP]]
; CHECK-VERSION-OVER-WIDEN:       for.cond.cleanup:
; CHECK-VERSION-OVER-WIDEN-NEXT:    ret void
;
entry:
...
;
; CHECK-VERSION-OVER-WIDEN-LABEL: define void @overflow(
; CHECK-VERSION-OVER-WIDEN-SAME: i32 [[LIM:%.*]], ptr [[A:%.*]]) {
; CHECK-VERSION-OVER-WIDEN-NEXT:  entry:
; CHECK-VERSION-OVER-WIDEN-NEXT:    [[CMP17_NOT:%.*]] = icmp eq i32 [[LIM]], 0
; CHECK-VERSION-OVER-WIDEN-NEXT:    br i1 [[CMP17_NOT]], label [[FOR_COND_CLEANUP:%.*]], label [[FOR_COND1_PREHEADER_LVER_CHECK:%.*]]
; CHECK-VERSION-OVER-WIDEN:       for.cond1.preheader.lver.check:
; CHECK-VERSION-OVER-WIDEN-NEXT:    [[FLATTEN_MUL:%.*]] = call { i32, i1 } @llvm.umul.with.overflow.i32(i32 [[LIM]], i32 100000)
; CHECK-VERSION-OVER-WIDEN-NEXT:    [[FLATTEN_TRIPCOUNT:%.*]] = extractvalue { i32, i1 } [[FLATTEN_MUL]], 0
; CHECK-VERSION-OVER-WIDEN-NEXT:    [[FLATTEN_OVERFLOW:%.*]] = extractvalue { i32, i1 } [[FLATTEN_MUL]], 1
; CHECK-VERSION-OVER-WIDEN-NEXT:    br i1 [[FLATTEN_OVERFLOW]], label [[FOR_COND1_PREHEADER_PH_LVER_ORIG:%.*]], label [[FOR_COND1_PREHEADER_PH:%.*]]
; CHECK-VERSION-OVER-WIDEN:       for.cond1.preheader.ph.lver.orig:
; CHECK-VERSION-OVER-WIDEN-NEXT:    br label [[FOR_COND1_PREHEADER_LVER_ORIG:%.*]]
; CHECK-VERSION-OVER-WIDEN:       for.cond1.preheader.lver.orig:
; CHECK-VERSION-OVER-WIDEN-NEXT:    [[I_018_LVER_ORIG:%.*]] = phi i32 [ [[INC6_LVER_ORIG:%.*]], [[FOR_COND_CLEANUP3_LVER_ORIG:%.*]] ], [ 0, [[FOR_COND1_PREHEADER_PH_LVER_ORIG]] ]
; CHECK-VERSION-OVER-WIDEN-NEXT:    [[MUL_LVER_ORIG:%.*]] = mul i32 [[I_018_LVER_ORIG]], 100000
; CHECK-VERSION-OVER-WIDEN-NEXT:    br label [[FOR_BODY4_LVER_ORIG:%.*]]
; CHECK-VERSION-OVER-WIDEN:       for.body4.lver.orig:
; CHECK-VERSION-OVER-WIDEN-NEXT:    [[J_016_LVER_ORIG:%.*]] = phi i32 [ 0, [[FOR_COND1_PREHEADER_LVER_ORIG]] ], [ [[INC_LVER_ORIG:%.*]], [[IF_END_LVER_ORIG:%.*]] ]
; CHECK-VERSION-OVER-WIDEN-NEXT:    [[ADD_LVER_ORIG:%.*]] = add i32 [[J_016_LVER_ORIG]], [[MUL_LVER_ORIG]]
; CHECK-VERSION-OVER-WIDEN-NEXT:    [[TMP0:%.*]] = load i32, ptr @first, align 4
; CHECK-VERSION-OVER-WIDEN-NEXT:    [[TOBOOL_NOT_LVER_ORIG:%.*]] = icmp eq i32 [[TMP0]], 0
; CHECK-VERSION-OVER-WIDEN-NEXT:    br i1 [[TOBOOL_NOT_LVER_ORIG]], label [[IF_END_LVER_ORIG]], label [[IF_THEN_LVER_ORIG:%.*]]
; CHECK-VERSION-OVER-WIDEN:       if.then.lver.orig:
; CHECK-VERSION-OVER-WIDEN-NEXT:    [[ARRAYIDX_LVER_ORIG:%.*]] = getelementptr inbounds [0 x i8], ptr @a, i32 0, i32 [[ADD_LVER_ORIG]]
; CHECK-VERSION-OVER-WIDEN-NEXT:    [[TMP1:%.*]] = load i8, ptr [[ARRAYIDX_LVER_ORIG]], align 1
; CHECK-VERSION-OVER-WIDEN-NEXT:    tail call void asm sideeffect "", "r"(i8 [[TMP1]])
; CHECK-VERSION-OVER-WIDEN-NEXT:    store i32 0, ptr @first, align 4
; CHECK-VERSION-OVER-WIDEN-NEXT:    br label [[IF_END_LVER_ORIG]]
; CHECK-VERSION-OVER-WIDEN:       if.end.lver.orig:
; CHECK-VERSION-OVER-WIDEN-NEXT:    tail call void asm sideeffect "", "r"(i32 [[ADD_LVER_ORIG]])
; CHECK-VERSION-OVER-WIDEN-NEXT:    [[INC_LVER_ORIG]] = add nuw nsw i32 [[J_016_LVER_ORIG]], 1
; CHECK-VERSION-OVER-WIDEN-NEXT:    [[CMP2_LVER_ORIG:%.*]] = icmp ult i32 [[J_016_LVER_ORIG]], 99999
; CHECK-VERSION-OVER-WIDEN-NEXT:    br i1 [[CMP2_LVER_ORIG]], label [[FOR_BODY4_LVER_ORIG]], label [[FOR_COND_CLEANUP3_LVER_ORIG]]
; CHECK-VERSION-OVER-WIDEN:       for.cond.cleanup3.lver.orig:
; CHECK-VERSION-OVER-WIDEN-NEXT:    [[INC6_LVER_ORIG]] = add i32 [[I_018_LVER_ORIG]], 1
; CHECK-VERSION-OVER-WIDEN-NEXT:    [[CMP_LVER_ORIG:%.*]] = icmp ult i32 [[INC6_LVER_ORIG]], [[LIM]]
; CHECK-VERSION-OVER-WIDEN-NEXT:    br i1 [[CMP_LVER_ORIG]], label [[FOR_COND1_PREHEADER_LVER_ORIG]], label [[FOR_COND_CLEANUP_LOOPEXIT_LOOPEXIT:%.*]]
; CHECK-VERSION-OVER-WIDEN:       for.cond1.preheader.ph:
; CHECK-VERSION-OVER-WIDEN-NEXT:    br label [[FOR_COND1_PREHEADER:%.*]]
; CHECK-VERSION-OVER-WIDEN:       for.cond1.preheader:
; CHECK-VERSION-OVER-WIDEN-NEXT:    [[I_018:%.*]] = phi i32 [ [[INC6:%.*]], [[FOR_COND_CLEANUP3:%.*]] ], [ 0, [[FOR_COND1_PREHEADER_PH]] ]
; CHECK-VERSION-OVER-WIDEN-NEXT:    [[MUL:%.*]] = mul i32 [[I_018]], 100000
; CHECK-VERSION-OVER-WIDEN-NEXT:    br label [[FOR_BODY4:%.*]]
; CHECK-VERSION-OVER-WIDEN:       for.cond.cleanup.loopexit.loopexit:
; CHECK-VERSION-OVER-WIDEN-NEXT:    br label [[FOR_COND_CLEANUP_LOOPEXIT:%.*]]
; CHECK-VERSION-OVER-WIDEN:       for.cond.cleanup.loopexit.loopexit1:
; CHECK-VERSION-OVER-WIDEN-NEXT:    br label [[FOR_COND_CLEANUP_LOOPEXIT]]
; CHECK-VERSION-OVER-WIDEN:       for.cond.cleanup.loopexit:
; CHECK-VERSION-OVER-WIDEN-NEXT:    br label [[FOR_COND_CLEANUP]]
; CHECK-VERSION-OVER-WIDEN:       for.cond.cleanup:
; CHECK-VERSION-OVER-WIDEN-NEXT:    ret void
; CHECK-VERSION-OVER-WIDEN:       for.cond.cleanup3:
; CHECK-VERSION-OVER-WIDEN-NEXT:    [[INC6]] = add i32 [[I_018]], 1
; CHECK-VERSION-OVER-WIDEN-NEXT:    [[CMP:%.*]] = icmp ult i32 [[INC6]], [[FLATTEN_TRIPCOUNT]]
; CHECK-VERSION-OVER-WIDEN-NEXT:    br i1 [[CMP]], label [[FOR_COND1_PREHEADER]], label [[FOR_COND_CLEANUP_LOOPEXIT_LOOPEXIT1:%.*]]
; CHECK-VERSION-OVER-WIDEN:       for.body4:
; CHECK-VERSION-OVER-WIDEN-NEXT:    [[J_016:%.*]] = phi i32 [ 0, [[FOR_COND1_PREHEADER]] ]
; CHECK-VERSION-OVER-WIDEN-NEXT:    [[ADD:%.*]] = add i32 [[J_016]], [[MUL]]
; CHECK-VERSION-OVER-WIDEN-NEXT:    [[TMP2:%.*]] = load i32, ptr @first, align 4
; CHECK-VERSION-OVER-WIDEN-NEXT:    [[TOBOOL_NOT:%.*]] = icmp eq i32 [[TMP2]], 0
; CHECK-VERSION-OVER-WIDEN-NEXT:    br i1 [[TOBOOL_NOT]], label [[IF_END:%.*]], label [[IF_THEN:%.*]]
; CHECK-VERSION-OVER-WIDEN:       if.then:
; CHECK-VERSION-OVER-WIDEN-NEXT:    [[ARRAYIDX:%.*]] = getelementptr inbounds [0 x i8], ptr @a, i32 0, i32 [[I_018]]
; CHECK-VERSION-OVER-WIDEN-NEXT:    [[TMP3:%.*]] = load i8, ptr [[ARRAYIDX]], align 1
; CHECK-VERSION-OVER-WIDEN-NEXT:    tail call void asm sideeffect "", "r"(i8 [[TMP3]])
; CHECK-VERSION-OVER-WIDEN-NEXT:    store i32 0, ptr @first, align 4
; CHECK-VERSION-OVER-WIDEN-NEXT:    br label [[IF_END]]
; CHECK-VERSION-OVER-WIDEN:       if.end:
; CHECK-VERSION-OVER-WIDEN-NEXT:    tail call void asm sideeffect "", "r"(i32 [[I_018]])
; CHECK-VERSION-OVER-WIDEN-NEXT:    [[INC:%.*]] = add nuw nsw i32 [[J_016]], 1
; CHECK-VERSION-OVER-WIDEN-NEXT:    [[CMP2:%.*]] = icmp ult i32 [[J_016]], 99999
; CHECK-VERSION-OVER-WIDEN-NEXT:    br label [[FOR_COND_CLEANUP3]]
;
entry:
