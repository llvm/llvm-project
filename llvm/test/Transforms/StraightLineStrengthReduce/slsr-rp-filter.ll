...
; RUN: opt < %s -passes=slsr -S | FileCheck %s --check-prefixes=CHECK,FILTER
; RUN: opt < %s -passes=slsr -S -slsr-rp-filter=false | FileCheck %s --check-prefixes=CHECK,OFF

; SLSR rewrites a candidate as its basis plus a delta, which keeps the basis
; live down to the candidate. The register-pressure filter drops a block's
; rewrites when two things hold: the block has more than 16 distinct bases, and
; rewriting them all would raise the block's peak pressure by more than 15%.
;
; The basis count is what makes a block interesting, because each distinct basis
; contributes one extended live range however many candidates share it. That
; count is a property of the def-use graph, so it does not move when the block
; is reordered. The peak comparison is what tells us the extended ranges
; actually overlap; it is the only signal that can distinguish that, and it is
; why a bare count of rewrites is not enough.
;
; -slsr-rp-filter=false bypasses the filter entirely.

target datalayout = "e-i64:64-v16:16-v32:32-n16:32:64"

declare void @foo(i32)

; 17 distinct bases, and every %t<k> is consumed early while %b<k> is used after
; %u<k>, so each rewrite stretches a basis across the whole run of %u's without
; retiring anything. Peak pressure goes 20 -> 36, an 80% rise, so all 17
...
; The same 17 bases, but each %u<k> immediately follows its %t<k>, so the
; extended ranges never coexist and peak pressure only goes 20 -> 21. The
; rewrites survive even though the basis count is identical to the block above,
; which is exactly the case a rewrite count could not tell apart.
...
; Identical in shape to @many_bases_overlapping but with one family removed, so
; the block holds 16 distinct bases rather than 17. That is not more than the
; gate, so the filter never looks at the block and the rewrites stand despite
; the same relative pressure growth. This pins the gate at 16 without needing an
; option to move it.
