...
; RUN: opt -aa-pipeline=basic-aa -passes=attributor -attributor-manifest-internal  -attributor-annotate-decl-cs  -pass-remarks=attributor -disable-output < %s 2>&1 | FileCheck %s --check-prefixes=REMARKS,REMARKS-TUNIT --implicit-check-not=remark
; RUN: opt -aa-pipeline=basic-aa -passes=attributor-cgscc -attributor-manifest-internal  -attributor-annotate-decl-cs -pass-remarks=attributor -disable-output < %s 2>&1 | FileCheck %s --check-prefixes=REMARKS,REMARKS-CGSCC --implicit-check-not=remark
...
define void @load_undef_pointer() {
; CHECK: Function Attrs: mustprogress nofree norecurse nosync nounwind willreturn memory(none)
; CHECK-LABEL: define {{[^@]+}}@load_undef_pointer
; CHECK-SAME: () #[[ATTR0]] {
; CHECK-NEXT:    unreachable
;
