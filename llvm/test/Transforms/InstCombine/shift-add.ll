...
; Boundary: X is at most 31 (BitWidth - 1), so the fold is valid.
...
; negative test: X may be 32 (BitWidth), where the original shift is defined
; but the pre-shifted one would be poison.
define i32 @lshr_add_negative_no_exact_max_bitwidth(i32 range(i32 1, 33) %x) {
; CHECK-LABEL: @lshr_add_negative_no_exact_max_bitwidth(
; CHECK-NEXT:    [[A:%.*]] = add nsw i32 [[X:%.*]], -1
; CHECK-NEXT:    [[R:%.*]] = lshr i32 2, [[A]]
; CHECK-NEXT:    ret i32 [[R]]
;
...
; negative test: same boundary for shl.
define i32 @shl_add_negative_no_flags_max_bitwidth(i32 range(i32 2, 34) %x) {
; CHECK-LABEL: @shl_add_negative_no_flags_max_bitwidth(
; CHECK-NEXT:    [[A:%.*]] = add nsw i32 [[X:%.*]], -2
; CHECK-NEXT:    [[R:%.*]] = shl i32 12, [[A]]
; CHECK-NEXT:    ret i32 [[R]]
;
  %a = add nsw i32 %x, -2
