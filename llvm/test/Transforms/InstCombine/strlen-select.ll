...
define i64 @call_strlen_musttail_select(ptr %p) {
; CHECK-LABEL: define i64 @call_strlen_musttail_select(
; CHECK-SAME: ptr [[P:%.*]]) {
; CHECK-NEXT:    [[L:%.*]] = musttail call i64 @strlen(ptr nonnull @s1)
; CHECK-NEXT:    ret i64 [[L]]
;
  %s = select i1 false, ptr @s0, ptr @s1
  %l = musttail call i64 @strlen(ptr %s)
  ret i64 %l
}
