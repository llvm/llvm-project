...
; RUN: opt -S -passes=instcombine -mtriple=amdgpu9.00-amd-amdhsa %s | FileCheck -check-prefixes=GCN %s
; RUN: opt -S -passes=instcombine -mtriple=amdgpu10.10-amd-amdhsa %s | FileCheck -check-prefixes=GCN %s
; RUN: opt -S -passes=instcombine -mtriple=amdgpu11.00-amd-amdhsa %s | FileCheck -check-prefixes=GCN %s
; RUN: opt -S -passes=instcombine -mtriple=amdgpu12.00-amd-amdhsa %s | FileCheck -check-prefixes=GFX12 %s
; RUN: opt -S -passes=instcombine -mtriple=amdgpu-amd-amdhsa %s | FileCheck -check-prefixes=GFXUNKNOWN %s
