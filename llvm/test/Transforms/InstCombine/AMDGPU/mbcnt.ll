...
; RUN: opt -S -mtriple=amdgpu-amd-amdhsa -passes=instcombine < %s | FileCheck --check-prefixes=DEFAULT,CHECK %s
; RUN: opt -S -mtriple=amdgpu-amd-amdhsa -mattr=+wavefrontsize32 -passes=instcombine < %s | FileCheck --check-prefixes=WAVE32,CHECK %s
; RUN: opt -S -mtriple=amdgpu-amd-amdhsa -mattr=+wavefrontsize64 -passes=instcombine < %s | FileCheck --check-prefixes=WAVE64,CHECK %s
; RUN: opt -S -mtriple=amdgpu10.30-amd-amdhsa -passes=instcombine < %s | FileCheck --check-prefixes=WAVE32,CHECK %s
; RUN: opt -S -mtriple=amdgpu10.30-amd-amdhsa -mattr=+wavefrontsize64 -passes=instcombine < %s | FileCheck --check-prefixes=WAVE64,CHECK %s
