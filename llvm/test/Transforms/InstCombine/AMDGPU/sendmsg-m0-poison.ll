...
; RUN: opt < %s -mtriple=amdgpu -passes=instcombine -S | FileCheck %s --check-prefixes=CHECK,DEFAULT
; RUN: opt < %s -mtriple=amdgpu9.00-amd-amdhsa -passes=instcombine -S | FileCheck %s --check-prefixes=CHECK,GFX9
; RUN: opt < %s -mtriple=amdgpu10.10-amd-amdhsa -passes=instcombine -S | FileCheck %s --check-prefixes=CHECK,GFX10
; RUN: opt < %s -mtriple=amdgpu11.00-amd-amdhsa -passes=instcombine -S | FileCheck %s --check-prefixes=CHECK,GFX11
