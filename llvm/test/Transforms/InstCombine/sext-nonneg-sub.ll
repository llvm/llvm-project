...
; NEGATIVE TEST: b - smin(c, d) where b doesn't appear in smin
define i64 @neg_unrelated_smin(i32 %a, i32 %b, i32 %c) {
; CHECK-LABEL: define i64 @neg_unrelated_smin(
; CHECK-SAME: i32 [[A:%.*]], i32 [[B:%.*]], i32 [[C:%.*]]) {
; CHECK-NEXT:    [[MIN:%.*]] = call i32 @llvm.smin.i32(i32 [[A]], i32 [[C]])
; CHECK-NEXT:    [[SUB:%.*]] = sub nsw i32 [[B]], [[MIN]]
; CHECK-NEXT:    [[EXT:%.*]] = sext i32 [[SUB]] to i64
; CHECK-NEXT:    ret i64 [[EXT]]
;
  %min = call i32 @llvm.smin.i32(i32 %a, i32 %c)
  %sub = sub nsw i32 %b, %min     ; b doesn't appear in smin
  %ext = sext i32 %sub to i64
  ret i64 %ext
}
