...
; strcmp(x, y), strncmp(x, y, n), memcmp(x, y, n), and bcmp(x, y, n) are all
; commutative in the sense that swapping their pointer arguments negates the
; sign of a nonzero result without changing its magnitude, and leaves a zero
; result unchanged. When two such calls in a function have their pointer
; arguments swapped and are only ever compared against zero with an
; equality or signed-ordering predicate, the dominated call should be
; folded away in favor of the dominating one, swapping the predicate of any
; ordering comparison to compensate for the sign flip.
...
; (strcmp(x, y) < 0) and (strcmp(y, x) > 0) are equivalent: swapping the
; pointer arguments negates the sign of a nonzero result, so the dominated
; call's `sgt` comparison should be rewritten to an `slt` comparison against
; the dominating call, per CmpInst::getSwappedPredicate().
define i1 @strcmp_slt_sgt(ptr %x, ptr %y, i1 %c) {
; CHECK-LABEL: define i1 @strcmp_slt_sgt(
; CHECK-SAME: ptr [[X:%.*]], ptr [[Y:%.*]], i1 [[C:%.*]]) {
; CHECK-NEXT:    [[CALL1:%.*]] = call i32 @strcmp(ptr noundef nonnull dereferenceable(1) [[X]], ptr noundef nonnull dereferenceable(1) [[Y]])
; CHECK-NEXT:    br i1 [[C]], label %[[RHS:.*]], label %[[END:.*]]
; CHECK:       [[RHS]]:
; CHECK-NEXT:    br label %[[END]]
; CHECK:       [[END]]:
; CHECK-NEXT:    [[R:%.*]] = icmp slt i32 [[CALL1]], 0
; CHECK-NEXT:    ret i1 [[R]]
;
  %call1 = call i32 @strcmp(ptr %x, ptr %y)
  %cmp1 = icmp slt i32 %call1, 0
  br i1 %c, label %rhs, label %end

rhs:
  %call2 = call i32 @strcmp(ptr %y, ptr %x)
  %cmp2 = icmp sgt i32 %call2, 0
  br label %end

end:
  %r = phi i1 [ %cmp1, %0 ], [ %cmp2, %rhs ]
  ret i1 %r
}

; (strcmp(x, y) <= 0) and (strcmp(y, x) >= 0) are equivalent, exercising the
; sle/sge half of the swapped-predicate table.
define i1 @strcmp_sle_sge(ptr %x, ptr %y, i1 %c) {
; CHECK-LABEL: define i1 @strcmp_sle_sge(
; CHECK-SAME: ptr [[X:%.*]], ptr [[Y:%.*]], i1 [[C:%.*]]) {
; CHECK-NEXT:    [[CALL1:%.*]] = call i32 @strcmp(ptr noundef nonnull dereferenceable(1) [[X]], ptr noundef nonnull dereferenceable(1) [[Y]])
; CHECK-NEXT:    br i1 [[C]], label %[[RHS:.*]], label %[[END:.*]]
; CHECK:       [[RHS]]:
; CHECK-NEXT:    br label %[[END]]
; CHECK:       [[END]]:
; CHECK-NEXT:    [[R:%.*]] = icmp slt i32 [[CALL1]], 1
; CHECK-NEXT:    ret i1 [[R]]
;
  %call1 = call i32 @strcmp(ptr %x, ptr %y)
  %cmp1 = icmp sle i32 %call1, 0
  br i1 %c, label %rhs, label %end

rhs:
  %call2 = call i32 @strcmp(ptr %y, ptr %x)
  %cmp2 = icmp sge i32 %call2, 0
  br label %end
...
; A single call's result may be used by a mix of an equality comparison and
; a signed-ordering comparison; both are sign-agnostic (in the sense used by
; isOnlyUsedInSignAgnosticZeroComparisons) and should be rewritten together.
define i1 @strcmp_mixed_eq_and_ordering(ptr %x, ptr %y, i1 %c) {
; CHECK-LABEL: define i1 @strcmp_mixed_eq_and_ordering(
; CHECK-SAME: ptr [[X:%.*]], ptr [[Y:%.*]], i1 [[C:%.*]]) {
; CHECK-NEXT:    [[CALL1:%.*]] = call i32 @strcmp(ptr noundef nonnull dereferenceable(1) [[X]], ptr noundef nonnull dereferenceable(1) [[Y]])
; CHECK-NEXT:    [[CMP1:%.*]] = icmp eq i32 [[CALL1]], 0
; CHECK-NEXT:    br i1 [[C]], label %[[RHS:.*]], label %[[END:.*]]
; CHECK:       [[RHS]]:
; CHECK-NEXT:    [[CMP2:%.*]] = icmp sgt i32 [[CALL1]], 0
; CHECK-NEXT:    br label %[[END]]
; CHECK:       [[END]]:
; CHECK-NEXT:    [[R:%.*]] = phi i1 [ [[CMP1]], [[TMP0:%.*]] ], [ [[CMP2]], %[[RHS]] ]
; CHECK-NEXT:    ret i1 [[R]]
;
  %call1 = call i32 @strcmp(ptr %x, ptr %y)
  %cmp1 = icmp eq i32 %call1, 0
...
; Negative test: an unsigned ordering predicate against zero has no
; sign-based interpretation ("x <u 0" is simply always false, regardless of
; the sign of x), so it must not be folded via the swapped-predicate rule.
; This also regression-tests a use-after-free: when the currently-visited
; call is the one folded away, the caller must not treat that as "no change"
; and go on to further optimize the now-erased call.
define i1 @strcmp_unsigned_ordering_not_folded(ptr %x, ptr %y) {
; CHECK-LABEL: define i1 @strcmp_unsigned_ordering_not_folded(
; CHECK-SAME: ptr [[X:%.*]], ptr [[Y:%.*]]) {
; CHECK-NEXT:    [[CALL1:%.*]] = call i32 @strcmp(ptr noundef nonnull dereferenceable(1) [[X]], ptr noundef nonnull dereferenceable(1) [[Y]])
; CHECK-NEXT:    [[CMP1:%.*]] = icmp eq i32 [[CALL1]], 0
; CHECK-NEXT:    br i1 [[CMP1]], label %[[RHS:.*]], label %[[END:.*]]
