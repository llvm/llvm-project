...
; CHECK-LABEL: @nonnull_gep_inbounds_bundle_null_is_valid(
; CHECK-NEXT:    [[P2:%.*]] = getelementptr inbounds i8, ptr [[P:%.*]], i64 [[I:%.*]]
; CHECK-NEXT:    call void @llvm.assume(i1 true) [ "nonnull"(ptr [[P2]]) ]
; CHECK-NEXT:    ret void
;
  %p2 = getelementptr inbounds i8, ptr %p, i64 %i
  call void @llvm.assume(i1 true) ["nonnull"(ptr %p2)]
...
; The condition has a use other than the assume. The fact is still available to
; %q, which is not one of the assumption's ephemeral values, and the assume
; itself is preserved -- simplifying %a or %n with it would make the assumption
