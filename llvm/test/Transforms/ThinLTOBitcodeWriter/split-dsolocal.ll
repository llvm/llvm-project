...
; RUN: llvm-modextract --opaque-pointers=0 -b -n 0 -o %t0.bc %t
; RUN: llvm-modextract --opaque-pointers=0 -b -n 1 -o %t1.bc %t
; RUN: llvm-dis --opaque-pointers=0 -o - %t0.bc | FileCheck --check-prefix=M0 %s
; RUN: llvm-dis --opaque-pointers=0 -o - %t1.bc | FileCheck --check-prefix=M1 %s
