...
; RUN: opt --opaque-pointers=0 -thinlto-bc -thin-link-bitcode-file=%t2 -thinlto-split-lto-unit -o %t %s
; RUN: llvm-modextract --opaque-pointers=0 -b -n 0 -o %t0.bc %t
; RUN: llvm-modextract --opaque-pointers=0 -b -n 1 -o %t1.bc %t
; RUN: llvm-modextract --opaque-pointers=0 -b -n 0 -o %t0.thinlink.bc %t2
; RUN: llvm-modextract --opaque-pointers=0 -b -n 1 -o %t1.thinlink.bc %t2
; RUN: not llvm-modextract --opaque-pointers=0 -b -n 2 -o - %t 2>&1 | FileCheck --check-prefix=ERROR %s
...
; RUN: llvm-lto --opaque-pointers=0 -thinlto -o %t3 %t0.bc
...
; RUN: llvm-lto --opaque-pointers=0 -thinlto -o %t4 %t0.bc
