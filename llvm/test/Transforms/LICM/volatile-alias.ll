; RUN: opt --opaque-pointers=0 -basic-aa -sroa -loop-rotate %s | opt --opaque-pointers=0 -aa-pipeline=basic-aa -passes='require<aa>,require<targetir>,require<scalar-evolution>,require<opt-remark-emit>,loop-mssa(licm)' -S | FileCheck %s
; RUN: opt --opaque-pointers=0 -basic-aa -sroa -loop-rotate -licm -verify-memoryssa -S < %s | FileCheck %s
