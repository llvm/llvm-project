...
; RUN: opt -passes=loop-split-utils -loop-split-outer -loop-split-points=50 -S < %s | FileCheck %s

; Split the outer loop of a nest (-loop-split-outer; the default picks the
; innermost loop instead). Splitting it clones the whole loop body, including
; the inner loop, into the later partition. This exercises that cloning a loop
; containing a subloop does not crash and that the inner loop is duplicated
