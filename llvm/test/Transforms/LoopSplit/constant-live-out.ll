...
; A live-out whose exit value is a constant is not defined inside the loop, so
; it needs no per-partition reconstruction. The transform must not treat it as
; an escaping value (which would walk uses() on a constant and crash).

define i64 @const_live_out(ptr %a, i64 %n) {
; CHECK-LABEL: define i64 @const_live_out(
