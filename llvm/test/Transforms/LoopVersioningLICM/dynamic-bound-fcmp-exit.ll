...
; CHECK-SAME: ptr [[A:%.*]], ptr [[B:%.*]], ptr [[LIMIT:%.*]]) {
; CHECK-NEXT:  [[ENTRY:.*]]:
; CHECK-NEXT:    br label %[[LOOP:.*]]
; CHECK:       [[LOOP]]:
; CHECK-NEXT:    [[IV:%.*]] = phi i64 [ 0, %[[ENTRY]] ], [ [[IV_NEXT:%.*]], %[[LOOP]] ]
; CHECK-NEXT:    [[FIV:%.*]] = phi float [ 0.000000e+00, %[[ENTRY]] ], [ [[FIV_NEXT:%.*]], %[[LOOP]] ]
; CHECK-NEXT:    [[GEP_B:%.*]] = getelementptr inbounds i32, ptr [[B]], i64 [[IV]]
; CHECK-NEXT:    [[B_VAL:%.*]] = load i32, ptr [[GEP_B]], align 4
; CHECK-NEXT:    [[GEP_A:%.*]] = getelementptr inbounds i32, ptr [[A]], i64 [[IV]]
; CHECK-NEXT:    store i32 [[B_VAL]], ptr [[GEP_A]], align 4
; CHECK-NEXT:    [[IV_NEXT]] = add nuw nsw i64 [[IV]], 1
; CHECK-NEXT:    [[FIV_NEXT]] = fadd float [[FIV]], 1.000000e+00
; CHECK-NEXT:    [[LIM:%.*]] = load float, ptr [[LIMIT]], align 4
; CHECK-NEXT:    [[EC:%.*]] = fcmp olt float [[FIV_NEXT]], [[LIM]]
