...
; RUN: opt -mtriple=aarch64--    -mattr=+sve -passes=pre-isel-intrinsic-lowering -S < %s | FileCheck %s
; RUN: opt -mtriple=aarch64_be-- -mattr=+sve -passes=pre-isel-intrinsic-lowering -S < %s | FileCheck %s
...
;
; Both RUN lines share a prefix on purpose: the mask comes from
; llvm.get.active.lane.mask, which is lane-indexed, so the emitted IR must be
; identical on big- and little-endian targets. An endian-sensitive construct
; such as a bitcast from an integer bit pattern would not be.
...
; A bound wider than one vector register is declined on both.
