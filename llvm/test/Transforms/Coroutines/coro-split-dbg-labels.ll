...
; RUN: opt < %s -passes='cgscc(coro-split)' -enable-profcheck -S | FileCheck %s
;
; Check the generated DWARF debug info:
; REQUIRES: object-emission
; RUN: opt < %s -passes='cgscc(coro-split),coro-cleanup' -enable-profcheck \
...
; RUN:   | opt -passes='cgscc(coro-split)' -enable-profcheck -S \
...
; CHECK-LABEL:   define ptr @f() #1 !dbg !{{[0-9]+}} !prof !{{[0-9]+}} {
...
; CHECK-LABEL:   define internal void @f.destroy({{.*}}) #1 !dbg !{{[0-9]+}} !prof !{{[0-9]+}} {
; CHECK:         resume.0:        ; preds = %resume.entry
