...
; CHECK-NEXT:    [[A3:%.*]] = and <2 x i32> zeroinitializer, splat (i32 4)
; CHECK-NEXT:    [[B2:%.*]] = add <2 x i32> [[B:%.*]], splat (i32 1)
; CHECK-NEXT:    [[B3:%.*]] = and <2 x i32> [[B2]], splat (i32 8)
; CHECK-NEXT:    [[C:%.*]] = or <2 x i32> [[A3]], [[B3]]
; CHECK-NEXT:    [[D:%.*]] = ashr <2 x i32> [[C]], splat (i32 3)
...
; CHECK-NEXT:    [[X3:%.*]] = and <2 x i32> zeroinitializer, splat (i32 4)
; CHECK-NEXT:    [[Y:%.*]] = insertelement <2 x i32> undef, i32 [[B:%.*]], i32 0
; CHECK-NEXT:    [[Y2:%.*]] = insertelement <2 x i32> [[Y]], i32 [[A:%.*]], i32 1
; CHECK-NEXT:    [[Y3:%.*]] = and <2 x i32> [[Y2]], splat (i32 8)
; CHECK-NEXT:    [[Z:%.*]] = or <2 x i32> [[X3]], [[Y3]]
; CHECK-NEXT:    [[U:%.*]] = ashr <2 x i32> [[Z]], splat (i32 3)
...
; CHECK-NEXT:    [[A2:%.*]] = add <2 x i32> [[A:%.*]], splat (i32 1)
; CHECK-NEXT:    [[A3:%.*]] = and <2 x i32> [[A2]], splat (i32 2)
; CHECK-NEXT:    [[X:%.*]] = uitofp <2 x i32> [[A3]] to <2 x double>
; CHECK-NEXT:    [[Y:%.*]] = fadd <2 x double> [[X]], splat (double 1.000000e+00)
...
; CHECK-NEXT:    [[BIG_NUMBER:%.*]] = shl <2 x i8> zeroinitializer, splat (i8 1)
