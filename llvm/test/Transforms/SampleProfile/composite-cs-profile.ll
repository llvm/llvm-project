; Verify that composite CS profiles use an ordered offset table, can be loaded by
...
; RUN: llvm-profdata merge --sample --extbinary-composite-prof --extbinary \
; RUN:   %S/Inputs/indirect-call-csspgo.prof -o %t.payload.prof
; RUN: opt -S %S/Inputs/composite-cs-inline-icall.ll -passes=sample-profile \
