...
; COMBINED-NEXT:  [[ENTRY:.*:]]
; COMBINED-NEXT:    [[MASKED:%.*]] = and i32 [[N]], 1
; COMBINED-NEXT:    [[MEMSET_NOTZERO_NOT:%.*]] = icmp eq i32 [[MASKED]], 0
; COMBINED-NEXT:    br i1 [[MEMSET_NOTZERO_NOT]], label %[[BB1:.*]], label %[[BB0:.*]]
; COMBINED:       [[BB0]]:
; COMBINED-NEXT:    store i8 [[VALUE]], ptr [[DST]], align 1
; COMBINED-NEXT:    br label %[[BB1]]
...
; AIC-NEXT:    ret void
;
; COMBINED-LABEL: define void @range_0_1_assume(
; COMBINED-SAME: ptr [[DST:%.*]], i8 [[VALUE:%.*]], i64 [[N:%.*]]) {
; COMBINED-NEXT:  [[ENTRY:.*:]]
; COMBINED-NEXT:    [[IN_RANGE:%.*]] = icmp ult i64 [[N]], 2
; COMBINED-NEXT:    call void @llvm.assume(i1 [[IN_RANGE]])
; COMBINED-NEXT:    [[MEMSET_NOTZERO_NOT:%.*]] = icmp eq i64 [[N]], 0
; COMBINED-NEXT:    br i1 [[MEMSET_NOTZERO_NOT]], label %[[BB1:.*]], label %[[BB0:.*]]
; COMBINED:       [[BB0]]:
; COMBINED-NEXT:    store i8 [[VALUE]], ptr [[DST]], align 1
; COMBINED-NEXT:    br label %[[BB1]]
...
; COMBINED-SAME: ptr [[DST:%.*]], i8 [[VALUE:%.*]], i64 [[N:%.*]]) {
; COMBINED-NEXT:  [[ENTRY:.*:]]
; COMBINED-NEXT:    [[LEN:%.*]] = urem i64 [[N]], 3
