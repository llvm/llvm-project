...
; RUN:   -lowertypetests-read-summary=%s %s | FileCheck --check-prefixes=CHECK,CROSSDSO %s
; RUN: grep -v "llvm.module.flags" %s | opt -S -passes=lowertypetests -lowertypetests-summary-action=export \
; RUN:   -lowertypetests-read-summary=%s | FileCheck --check-prefixes=CHECK,NORMAL %s
