...
; NOAES-NEXT:    [[DATA_INDEXER:%.*]] = lshr i8 [[MSG]], [[IV_BITS]]
; NOAES-NEXT:    [[CRC_INDEXER_CAST:%.*]] = trunc i16 [[CRC2]] to i8
; NOAES-NEXT:    [[CRC_DATA_INDEXER:%.*]] = xor i8 [[DATA_INDEXER]], [[CRC_INDEXER_CAST]]
; NOAES-NEXT:    [[INDEXER_EXT:%.*]] = zext i8 [[CRC_DATA_INDEXER]] to i64
; NOAES-NEXT:    [[TBL_PTRADD:%.*]] = getelementptr inbounds i16, ptr @.crctable, i64 [[INDEXER_EXT]]
; NOAES-NEXT:    [[TBL_LD:%.*]] = load i16, ptr [[TBL_PTRADD]], align 2
; NOAES-NEXT:    [[CRC_LE_SHIFT:%.*]] = lshr i16 [[CRC2]], 8
; NOAES-NEXT:    [[CRC_NEXT3]] = xor i16 [[CRC_LE_SHIFT]], [[TBL_LD]]
; NOAES-NEXT:    [[IV_NEXT]] = add nuw nsw i8 [[IV]], 1
; NOAES-NEXT:    [[EXIT_COND1:%.*]] = icmp ne i8 [[IV]], 0
; NOAES-NEXT:    br i1 [[EXIT_COND1]], label %[[LOOP]], label %[[EXIT:.*]]
; NOAES:       [[EXIT]]:
; NOAES-NEXT:    [[CRC_NEXT_LCSSA:%.*]] = phi i16 [ [[CRC_NEXT3]], %[[LOOP]] ]
...
; AES-NEXT:    [[DATA_EXT:%.*]] = zext i8 [[MSG]] to i32
; AES-NEXT:    [[XOR_CRC_DATA:%.*]] = xor i32 [[CRC_EXT]], [[DATA_EXT]]
; AES-NEXT:    [[CLMUL_MU:%.*]] = call i32 @llvm.clmul.i32(i32 [[XOR_CRC_DATA]], i32 511)
; AES-NEXT:    [[QUOT_LE_MASK:%.*]] = and i32 [[CLMUL_MU]], 255
; AES-NEXT:    [[CLMUL_GP:%.*]] = call i32 @llvm.clmul.i32(i32 [[QUOT_LE_MASK]], i32 81923)
; AES-NEXT:    [[XOR_CRC_MULT:%.*]] = xor i32 [[CRC_EXT]], [[CLMUL_GP]]
; AES-NEXT:    [[CRC_LE_LSHR:%.*]] = lshr i32 [[XOR_CRC_MULT]], 8
; AES-NEXT:    [[CRC_NEXT1:%.*]] = trunc i32 [[CRC_LE_LSHR]] to i16
; AES-NEXT:    br label %[[LOOP:.*]]
; AES:       [[LOOP]]:
; AES-NEXT:    [[IV:%.*]] = phi i8 [ 0, %[[ENTRY]] ], [ [[IV_NEXT:%.*]], %[[LOOP]] ]
; AES-NEXT:    [[CRC:%.*]] = phi i16 [ [[CHECKSUM]], %[[ENTRY]] ], [ [[CRC_NEXT:%.*]], %[[LOOP]] ]
; AES-NEXT:    [[DATA:%.*]] = phi i8 [ [[MSG]], %[[ENTRY]] ], [ [[DATA_NEXT:%.*]], %[[LOOP]] ]
; AES-NEXT:    [[CRC_TRUNC:%.*]] = trunc i16 [[CRC]] to i8
; AES-NEXT:    [[XOR_DATA_CRC:%.*]] = xor i8 [[DATA]], [[CRC_TRUNC]]
; AES-NEXT:    [[AND_DATA_CRC:%.*]] = and i8 [[XOR_DATA_CRC]], 1
; AES-NEXT:    [[DATA_NEXT]] = lshr i8 [[DATA]], 1
; AES-NEXT:    [[CHECK_SB:%.*]] = icmp eq i8 [[AND_DATA_CRC]], 0
; AES-NEXT:    [[CRC_LSHR:%.*]] = lshr i16 [[CRC]], 1
; AES-NEXT:    [[XOR:%.*]] = xor i16 [[CRC_LSHR]], -24575
; AES-NEXT:    [[CRC_NEXT]] = select i1 [[CHECK_SB]], i16 [[CRC_LSHR]], i16 [[XOR]]
; AES-NEXT:    [[IV_NEXT]] = add nuw nsw i8 [[IV]], 1
; AES-NEXT:    [[EXIT_COND:%.*]] = icmp samesign ult i8 [[IV]], 7
...
; NOAES-LABEL: define i16 @crc16.le.tc8.udiv(
...
; NOAES-NEXT:    [[CRC_DATA_INDEXER:%.*]] = xor i8 [[DATA_INDEXER]], [[CRC_INDEXER_CAST]]
; NOAES-NEXT:    [[INDEXER_EXT:%.*]] = zext i8 [[CRC_DATA_INDEXER]] to i64
; NOAES-NEXT:    [[TBL_PTRADD:%.*]] = getelementptr inbounds i16, ptr @.crctable.1, i64 [[INDEXER_EXT]]
; NOAES-NEXT:    [[TBL_LD:%.*]] = load i16, ptr [[TBL_PTRADD]], align 2
; NOAES-NEXT:    [[CRC_LE_SHIFT:%.*]] = lshr i16 [[CRC2]], 8
; NOAES-NEXT:    [[CRC_NEXT3]] = xor i16 [[CRC_LE_SHIFT]], [[TBL_LD]]
; NOAES-NEXT:    [[IV_NEXT]] = add nuw nsw i8 [[IV]], 1
...
; AES-NEXT:    [[XOR_DATA_CRC:%.*]] = xor i8 [[DATA]], [[CRC_TRUNC]]
; AES-NEXT:    [[AND_DATA_CRC:%.*]] = and i8 [[XOR_DATA_CRC]], 1
; AES-NEXT:    [[DATA_NEXT]] = udiv i8 [[DATA]], 2
; AES-NEXT:    [[CHECK_SB:%.*]] = icmp eq i8 [[AND_DATA_CRC]], 0
; AES-NEXT:    [[CRC_LSHR:%.*]] = udiv i16 [[CRC]], 2
; AES-NEXT:    [[XOR:%.*]] = xor i16 [[CRC_LSHR]], -24575
; AES-NEXT:    [[CRC_NEXT]] = select i1 [[CHECK_SB]], i16 [[CRC_LSHR]], i16 [[XOR]]
; AES-NEXT:    [[IV_NEXT]] = add nuw nsw i8 [[IV]], 1
; AES-NEXT:    [[EXIT_COND:%.*]] = icmp samesign ult i8 [[IV]], 7
...
; NOAES-LABEL: define i16 @crc16.le.tc16(
; NOAES-SAME: i16 [[MSG:%.*]], i16 [[CHECKSUM:%.*]]) #[[ATTR0]] {
; NOAES-NEXT:  [[ENTRY:.*]]:
; NOAES-NEXT:    br label %[[LOOP:.*]]
; NOAES:       [[LOOP]]:
; NOAES-NEXT:    [[IV:%.*]] = phi i8 [ 0, %[[ENTRY]] ], [ [[IV_NEXT:%.*]], %[[LOOP]] ]
; NOAES-NEXT:    [[CRC2:%.*]] = phi i16 [ [[CHECKSUM]], %[[ENTRY]] ], [ [[CRC_NEXT3:%.*]], %[[LOOP]] ]
; NOAES-NEXT:    [[IV_BITS:%.*]] = shl i8 [[IV]], 3
; NOAES-NEXT:    [[IV_INDEXER:%.*]] = zext i8 [[IV_BITS]] to i16
; NOAES-NEXT:    [[DATA_INDEXER:%.*]] = lshr i16 [[MSG]], [[IV_INDEXER]]
; NOAES-NEXT:    [[CRC_DATA_INDEXER:%.*]] = xor i16 [[DATA_INDEXER]], [[CRC2]]
; NOAES-NEXT:    [[INDEXER_LO:%.*]] = trunc i16 [[CRC_DATA_INDEXER]] to i8
; NOAES-NEXT:    [[INDEXER_EXT:%.*]] = zext i8 [[INDEXER_LO]] to i64
; NOAES-NEXT:    [[TBL_PTRADD:%.*]] = getelementptr inbounds i16, ptr @.crctable.2, i64 [[INDEXER_EXT]]
; NOAES-NEXT:    [[TBL_LD:%.*]] = load i16, ptr [[TBL_PTRADD]], align 2
; NOAES-NEXT:    [[CRC_LE_SHIFT:%.*]] = lshr i16 [[CRC2]], 8
; NOAES-NEXT:    [[CRC_NEXT3]] = xor i16 [[CRC_LE_SHIFT]], [[TBL_LD]]
; NOAES-NEXT:    [[IV_NEXT]] = add nuw nsw i8 [[IV]], 1
; NOAES-NEXT:    [[EXIT_COND1:%.*]] = icmp ne i8 [[IV]], 1
; NOAES-NEXT:    br i1 [[EXIT_COND1]], label %[[LOOP]], label %[[EXIT:.*]]
; NOAES:       [[EXIT]]:
; NOAES-NEXT:    [[CRC_NEXT_LCSSA:%.*]] = phi i16 [ [[CRC_NEXT3]], %[[LOOP]] ]
; NOAES-NEXT:    ret i16 [[CRC_NEXT_LCSSA]]
...
; AES-NEXT:    [[CLMUL_MU:%.*]] = call i32 @llvm.clmul.i32(i32 [[XOR_CRC_DATA1]], i32 114687)
; AES-NEXT:    [[QUOT_LE_MASK:%.*]] = and i32 [[CLMUL_MU]], 65535
; AES-NEXT:    [[CLMUL_GP:%.*]] = call i32 @llvm.clmul.i32(i32 [[QUOT_LE_MASK]], i32 81923)
; AES-NEXT:    [[XOR_CRC_MULT:%.*]] = xor i32 [[CRC_EXT]], [[CLMUL_GP]]
...
; AES-NEXT:    [[IV:%.*]] = phi i8 [ 0, %[[ENTRY]] ], [ [[IV_NEXT:%.*]], %[[LOOP]] ]
; AES-NEXT:    [[CRC:%.*]] = phi i16 [ [[CHECKSUM]], %[[ENTRY]] ], [ [[CRC_NEXT:%.*]], %[[LOOP]] ]
; AES-NEXT:    [[DATA:%.*]] = phi i16 [ [[MSG]], %[[ENTRY]] ], [ [[DATA_NEXT:%.*]], %[[LOOP]] ]
; AES-NEXT:    [[XOR_CRC_DATA:%.*]] = xor i16 [[CRC]], [[DATA]]
; AES-NEXT:    [[AND_CRC_DATA:%.*]] = and i16 [[XOR_CRC_DATA]], 1
; AES-NEXT:    [[DATA_NEXT]] = lshr i16 [[DATA]], 1
; AES-NEXT:    [[CHECK_SB:%.*]] = icmp eq i16 [[AND_CRC_DATA]], 0
; AES-NEXT:    [[CRC_LSHR:%.*]] = lshr i16 [[CRC]], 1
; AES-NEXT:    [[CRC_XOR:%.*]] = xor i16 [[CRC_LSHR]], -24575
; AES-NEXT:    [[CRC_NEXT]] = select i1 [[CHECK_SB]], i16 [[CRC_LSHR]], i16 [[CRC_XOR]]
; AES-NEXT:    [[IV_NEXT]] = add nuw nsw i8 [[IV]], 1
; AES-NEXT:    [[EXIT_COND:%.*]] = icmp samesign ult i8 [[IV]], 15
; AES-NEXT:    br i1 [[EXIT_COND]], label %[[LOOP]], label %[[EXIT:.*]]
; AES:       [[EXIT]]:
...
; NOAES-LABEL: define i8 @crc8.le.tc16(
; NOAES-SAME: i16 [[MSG:%.*]], i8 [[CHECKSUM:%.*]]) #[[ATTR0]] {
; NOAES-NEXT:  [[ENTRY:.*]]:
; NOAES-NEXT:    br label %[[LOOP:.*]]
; NOAES:       [[LOOP]]:
; NOAES-NEXT:    [[IV:%.*]] = phi i8 [ 0, %[[ENTRY]] ], [ [[IV_NEXT:%.*]], %[[LOOP]] ]
; NOAES-NEXT:    [[CRC2:%.*]] = phi i8 [ [[CHECKSUM]], %[[ENTRY]] ], [ [[TBL_LD:%.*]], %[[LOOP]] ]
; NOAES-NEXT:    [[IV_BITS:%.*]] = shl i8 [[IV]], 3
; NOAES-NEXT:    [[IV_INDEXER:%.*]] = zext i8 [[IV_BITS]] to i16
; NOAES-NEXT:    [[DATA_INDEXER:%.*]] = lshr i16 [[MSG]], [[IV_INDEXER]]
; NOAES-NEXT:    [[CRC_INDEXER_CAST:%.*]] = zext i8 [[CRC2]] to i16
; NOAES-NEXT:    [[CRC_DATA_INDEXER:%.*]] = xor i16 [[DATA_INDEXER]], [[CRC_INDEXER_CAST]]
; NOAES-NEXT:    [[INDEXER_LO:%.*]] = trunc i16 [[CRC_DATA_INDEXER]] to i8
; NOAES-NEXT:    [[INDEXER_EXT:%.*]] = zext i8 [[INDEXER_LO]] to i64
; NOAES-NEXT:    [[TBL_PTRADD:%.*]] = getelementptr inbounds i8, ptr @.crctable.3, i64 [[INDEXER_EXT]]
; NOAES-NEXT:    [[TBL_LD]] = load i8, ptr [[TBL_PTRADD]], align 1
; NOAES-NEXT:    [[IV_NEXT]] = add nuw nsw i8 [[IV]], 1
; NOAES-NEXT:    [[EXIT_COND1:%.*]] = icmp ne i8 [[IV]], 1
; NOAES-NEXT:    br i1 [[EXIT_COND1]], label %[[LOOP]], label %[[EXIT:.*]]
; NOAES:       [[EXIT]]:
; NOAES-NEXT:    [[CRC_NEXT_LCSSA:%.*]] = phi i8 [ [[TBL_LD]], %[[LOOP]] ]
...
; AES-NEXT:    [[DATA_EXT:%.*]] = zext i16 [[MSG]] to i32
; AES-NEXT:    [[XOR_CRC_DATA1:%.*]] = xor i32 [[CRC_EXT]], [[DATA_EXT]]
; AES-NEXT:    [[CLMUL_MU:%.*]] = call i32 @llvm.clmul.i32(i32 [[XOR_CRC_DATA1]], i32 24423)
; AES-NEXT:    [[QUOT_LE_MASK:%.*]] = and i32 [[CLMUL_MU]], 65535
; AES-NEXT:    [[CLMUL_GP:%.*]] = call i32 @llvm.clmul.i32(i32 [[QUOT_LE_MASK]], i32 59)
; AES-NEXT:    [[XOR_CRC_MULT:%.*]] = xor i32 [[CRC_EXT]], [[CLMUL_GP]]
; AES-NEXT:    [[CRC_LE_LSHR:%.*]] = lshr i32 [[XOR_CRC_MULT]], 16
...
; AES:       [[LOOP]]:
; AES-NEXT:    [[IV:%.*]] = phi i8 [ 0, %[[ENTRY]] ], [ [[IV_NEXT:%.*]], %[[LOOP]] ]
; AES-NEXT:    [[CRC:%.*]] = phi i8 [ [[CHECKSUM]], %[[ENTRY]] ], [ [[CRC_NEXT:%.*]], %[[LOOP]] ]
; AES-NEXT:    [[DATA:%.*]] = phi i16 [ [[MSG]], %[[ENTRY]] ], [ [[DATA_NEXT:%.*]], %[[LOOP]] ]
; AES-NEXT:    [[DATA_TRUNC:%.*]] = trunc i16 [[DATA]] to i8
; AES-NEXT:    [[XOR_CRC_DATA:%.*]] = xor i8 [[CRC]], [[DATA_TRUNC]]
; AES-NEXT:    [[AND_CRC_DATA:%.*]] = and i8 [[XOR_CRC_DATA]], 1
; AES-NEXT:    [[DATA_NEXT]] = lshr i16 [[DATA]], 1
; AES-NEXT:    [[CHECK_SB:%.*]] = icmp eq i8 [[AND_CRC_DATA]], 0
; AES-NEXT:    [[CRC_LSHR:%.*]] = lshr i8 [[CRC]], 1
; AES-NEXT:    [[CRC_XOR:%.*]] = xor i8 [[CRC_LSHR]], 29
; AES-NEXT:    [[CRC_NEXT]] = select i1 [[CHECK_SB]], i8 [[CRC_LSHR]], i8 [[CRC_XOR]]
; AES-NEXT:    [[IV_NEXT]] = add nuw nsw i8 [[IV]], 1
; AES-NEXT:    [[EXIT_COND:%.*]] = icmp samesign ult i8 [[IV]], 15
; AES-NEXT:    br i1 [[EXIT_COND]], label %[[LOOP]], label %[[EXIT:.*]]
; AES:       [[EXIT]]:
...
; NOAES-LABEL: define i16 @crc16.be.tc8.crc.init.li(
; NOAES-SAME: i16 [[CHECKSUM:%.*]], i8 [[MSG:%.*]]) #[[ATTR0]] {
; NOAES-NEXT:  [[ENTRY:.*]]:
; NOAES-NEXT:    [[MSG_EXT:%.*]] = zext i8 [[MSG]] to i16
; NOAES-NEXT:    [[MSG_SHL:%.*]] = shl nuw i16 [[MSG_EXT]], 8
; NOAES-NEXT:    [[CRC_INIT:%.*]] = xor i16 [[MSG_SHL]], [[CHECKSUM]]
; NOAES-NEXT:    br label %[[LOOP:.*]]
; NOAES:       [[LOOP]]:
; NOAES-NEXT:    [[IV:%.*]] = phi i32 [ 0, %[[ENTRY]] ], [ [[IV_NEXT:%.*]], %[[LOOP]] ]
; NOAES-NEXT:    [[CRC2:%.*]] = phi i16 [ [[CRC_INIT]], %[[ENTRY]] ], [ [[CRC_NEXT3:%.*]], %[[LOOP]] ]
...
; NOAES-NEXT:    [[TBL_PTRADD:%.*]] = getelementptr inbounds i16, ptr @.crctable.4, i64 [[INDEXER_EXT]]
; NOAES-NEXT:    [[TBL_LD:%.*]] = load i16, ptr [[TBL_PTRADD]], align 2
; NOAES-NEXT:    [[CRC_BE_SHIFT:%.*]] = shl i16 [[CRC2]], 8
; NOAES-NEXT:    [[CRC_NEXT3]] = xor i16 [[CRC_BE_SHIFT]], [[TBL_LD]]
; NOAES-NEXT:    [[IV_NEXT]] = add nuw nsw i32 [[IV]], 1
; NOAES-NEXT:    [[EXIT_COND1:%.*]] = icmp ne i32 [[IV]], 0
; NOAES-NEXT:    br i1 [[EXIT_COND1]], label %[[LOOP]], label %[[EXIT:.*]]
; NOAES:       [[EXIT]]:
; NOAES-NEXT:    [[CRC_NEXT_LCSSA:%.*]] = phi i16 [ [[CRC_NEXT3]], %[[LOOP]] ]
; NOAES-NEXT:    ret i16 [[CRC_NEXT_LCSSA]]
...
; AES-NEXT:    [[CRC_EXT:%.*]] = zext i16 [[CRC_INIT]] to i32
; AES-NEXT:    [[CRC_BE_LSHR:%.*]] = lshr i32 [[CRC_EXT]], 8
; AES-NEXT:    [[CLMUL_MU:%.*]] = call i32 @llvm.clmul.i32(i32 [[CRC_BE_LSHR]], i32 273)
; AES-NEXT:    [[QUOT_BE_LSHR:%.*]] = lshr i32 [[CLMUL_MU]], 8
; AES-NEXT:    [[CLMUL_GP:%.*]] = call i32 @llvm.clmul.i32(i32 [[QUOT_BE_LSHR]], i32 69665)
; AES-NEXT:    [[CRC_BE_SHL:%.*]] = shl i32 [[CRC_EXT]], 8
; AES-NEXT:    [[XOR_CRC_MULT:%.*]] = xor i32 [[CRC_BE_SHL]], [[CLMUL_GP]]
; AES-NEXT:    [[CRC_NEXT1:%.*]] = trunc i32 [[XOR_CRC_MULT]] to i16
; AES-NEXT:    br label %[[LOOP:.*]]
; AES:       [[LOOP]]:
; AES-NEXT:    [[IV:%.*]] = phi i32 [ 0, %[[ENTRY]] ], [ [[IV_NEXT:%.*]], %[[LOOP]] ]
; AES-NEXT:    [[CRC:%.*]] = phi i16 [ [[CRC_INIT]], %[[ENTRY]] ], [ [[CRC_NEXT:%.*]], %[[LOOP]] ]
; AES-NEXT:    [[CRC_SHL:%.*]] = shl i16 [[CRC]], 1
; AES-NEXT:    [[CRC_XOR:%.*]] = xor i16 [[CRC_SHL]], 4129
; AES-NEXT:    [[CHECK_SB:%.*]] = icmp slt i16 [[CRC]], 0
; AES-NEXT:    [[CRC_NEXT]] = select i1 [[CHECK_SB]], i16 [[CRC_XOR]], i16 [[CRC_SHL]]
; AES-NEXT:    [[IV_NEXT]] = add nuw nsw i32 [[IV]], 1
; AES-NEXT:    [[EXIT_COND:%.*]] = icmp samesign ult i32 [[IV]], 7
; AES-NEXT:    br i1 [[EXIT_COND]], label %[[LOOP]], label %[[EXIT:.*]]
; AES:       [[EXIT]]:
; AES-NEXT:    [[CRC_NEXT_LCSSA:%.*]] = phi i16 [ [[CRC_NEXT1]], %[[LOOP]] ]
; AES-NEXT:    ret i16 [[CRC_NEXT_LCSSA]]
...
; NOAES-LABEL: define i16 @crc16.be.tc8.crc.init.arg(
; NOAES-SAME: i16 [[CRC_INIT:%.*]]) #[[ATTR0]] {
; NOAES-NEXT:  [[ENTRY:.*]]:
; NOAES-NEXT:    br label %[[LOOP:.*]]
; NOAES:       [[LOOP]]:
; NOAES-NEXT:    [[IV:%.*]] = phi i32 [ 0, %[[ENTRY]] ], [ [[IV_NEXT:%.*]], %[[LOOP]] ]
...
; NOAES-NEXT:    [[INDEXER_HI_LO_BYTE:%.*]] = trunc i16 [[INDEXER_HI]] to i8
; NOAES-NEXT:    [[INDEXER_EXT:%.*]] = zext i8 [[INDEXER_HI_LO_BYTE]] to i64
; NOAES-NEXT:    [[TBL_PTRADD:%.*]] = getelementptr inbounds i16, ptr @.crctable.5, i64 [[INDEXER_EXT]]
; NOAES-NEXT:    [[TBL_LD:%.*]] = load i16, ptr [[TBL_PTRADD]], align 2
; NOAES-NEXT:    [[CRC_BE_SHIFT:%.*]] = shl i16 [[CRC2]], 8
; NOAES-NEXT:    [[CRC_NEXT3]] = xor i16 [[CRC_BE_SHIFT]], [[TBL_LD]]
; NOAES-NEXT:    [[IV_NEXT]] = add nuw nsw i32 [[IV]], 1
...
; NOAES-LABEL: define i16 @crc16.be.tc8.crc.init.arg.flipped.sb.check(
; NOAES-SAME: i16 [[CRC_INIT:%.*]]) #[[ATTR0]] {
; NOAES-NEXT:  [[ENTRY:.*]]:
; NOAES-NEXT:    br label %[[LOOP:.*]]
; NOAES:       [[LOOP]]:
...
; NOAES-NEXT:    [[INDEXER_HI_LO_BYTE:%.*]] = trunc i16 [[INDEXER_HI]] to i8
; NOAES-NEXT:    [[INDEXER_EXT:%.*]] = zext i8 [[INDEXER_HI_LO_BYTE]] to i64
; NOAES-NEXT:    [[TBL_PTRADD:%.*]] = getelementptr inbounds i16, ptr @.crctable.6, i64 [[INDEXER_EXT]]
; NOAES-NEXT:    [[TBL_LD:%.*]] = load i16, ptr [[TBL_PTRADD]], align 2
; NOAES-NEXT:    [[CRC_BE_SHIFT:%.*]] = shl i16 [[CRC2]], 8
; NOAES-NEXT:    [[CRC_NEXT3]] = xor i16 [[CRC_BE_SHIFT]], [[TBL_LD]]
; NOAES-NEXT:    [[IV_NEXT]] = add nuw nsw i32 [[IV]], 1
...
; AES-NEXT:    [[CRC_SHL:%.*]] = shl i16 [[CRC]], 1
; AES-NEXT:    [[CRC_XOR:%.*]] = xor i16 [[CRC_SHL]], 4129
; AES-NEXT:    [[CHECK_SB:%.*]] = icmp sge i16 [[CRC]], 0
; AES-NEXT:    [[CRC_NEXT]] = select i1 [[CHECK_SB]], i16 [[CRC_SHL]], i16 [[CRC_XOR]]
; AES-NEXT:    [[IV_NEXT]] = add nuw nsw i32 [[IV]], 1
; AES-NEXT:    [[EXIT_COND:%.*]] = icmp samesign ult i32 [[IV]], 7
; AES-NEXT:    br i1 [[EXIT_COND]], label %[[LOOP]], label %[[EXIT:.*]]
; AES:       [[EXIT]]:
...
; NOAES-LABEL: define i8 @crc8.be.tc8.ptr.nested.loop(
; NOAES-SAME: ptr [[MSG:%.*]], i32 [[LOOP_LIMIT:%.*]]) #[[ATTR0]] {
; NOAES-NEXT:  [[ENTRY:.*]]:
; NOAES-NEXT:    br label %[[OUTER_LOOP:.*]]
; NOAES:       [[OUTER_LOOP]]:
; NOAES-NEXT:    [[CRC_OUTER:%.*]] = phi i8 [ 0, %[[ENTRY]] ], [ [[CRC_NEXT_LCSSA:%.*]], %[[INNER_EXIT:.*]] ]
; NOAES-NEXT:    [[OUTER_IV:%.*]] = phi i32 [ 0, %[[ENTRY]] ], [ [[OUTER_IV_NEXT:%.*]], %[[INNER_EXIT]] ]
; NOAES-NEXT:    [[OUTER_EXIT_COND:%.*]] = icmp ult i32 [[OUTER_IV]], [[LOOP_LIMIT]]
; NOAES-NEXT:    br i1 [[OUTER_EXIT_COND]], label %[[PH:.*]], label %[[EXIT:.*]]
; NOAES:       [[PH]]:
; NOAES-NEXT:    [[OUTER_IV_EXT:%.*]] = sext i32 [[OUTER_IV]] to i64
; NOAES-NEXT:    [[MSG_OUTER_IV:%.*]] = getelementptr inbounds i8, ptr [[MSG]], i64 [[OUTER_IV_EXT]]
; NOAES-NEXT:    [[MSG_LOAD:%.*]] = load i8, ptr [[MSG_OUTER_IV]], align 1
; NOAES-NEXT:    [[CRC_INIT:%.*]] = xor i8 [[MSG_LOAD]], [[CRC_OUTER]]
; NOAES-NEXT:    br label %[[INNER_LOOP:.*]]
; NOAES:       [[INNER_LOOP]]:
; NOAES-NEXT:    [[INNER_IV:%.*]] = phi i32 [ 0, %[[PH]] ], [ [[INNER_IV_NEXT:%.*]], %[[INNER_LOOP]] ]
; NOAES-NEXT:    [[CRC2:%.*]] = phi i8 [ [[CRC_INIT]], %[[PH]] ], [ [[TBL_LD:%.*]], %[[INNER_LOOP]] ]
; NOAES-NEXT:    [[INDEXER_EXT:%.*]] = zext i8 [[CRC2]] to i64
; NOAES-NEXT:    [[TBL_PTRADD:%.*]] = getelementptr inbounds i8, ptr @.crctable.7, i64 [[INDEXER_EXT]]
; NOAES-NEXT:    [[TBL_LD]] = load i8, ptr [[TBL_PTRADD]], align 1
; NOAES-NEXT:    [[INNER_IV_NEXT]] = add nuw nsw i32 [[INNER_IV]], 1
; NOAES-NEXT:    [[EXIT_COND1:%.*]] = icmp ne i32 [[INNER_IV]], 0
; NOAES-NEXT:    br i1 [[EXIT_COND1]], label %[[INNER_LOOP]], label %[[INNER_EXIT]]
; NOAES:       [[INNER_EXIT]]:
; NOAES-NEXT:    [[CRC_NEXT_LCSSA]] = phi i8 [ [[TBL_LD]], %[[INNER_LOOP]] ]
; NOAES-NEXT:    [[OUTER_IV_NEXT]] = add i32 [[OUTER_IV]], 1
; NOAES-NEXT:    br label %[[OUTER_LOOP]]
; NOAES:       [[EXIT]]:
; NOAES-NEXT:    [[CRC_OUTER_LCSSA:%.*]] = phi i8 [ [[CRC_OUTER]], %[[OUTER_LOOP]] ]
; NOAES-NEXT:    ret i8 [[CRC_OUTER_LCSSA]]
;
; AES-LABEL: define i8 @crc8.be.tc8.ptr.nested.loop(
; AES-SAME: ptr [[MSG:%.*]], i32 [[LOOP_LIMIT:%.*]]) #[[ATTR0]] {
; AES-NEXT:  [[ENTRY:.*]]:
; AES-NEXT:    br label %[[OUTER_LOOP:.*]]
; AES:       [[OUTER_LOOP]]:
; AES-NEXT:    [[CRC_OUTER:%.*]] = phi i8 [ 0, %[[ENTRY]] ], [ [[CRC_NEXT_LCSSA:%.*]], %[[INNER_EXIT:.*]] ]
; AES-NEXT:    [[OUTER_IV:%.*]] = phi i32 [ 0, %[[ENTRY]] ], [ [[OUTER_IV_NEXT:%.*]], %[[INNER_EXIT]] ]
; AES-NEXT:    [[OUTER_EXIT_COND:%.*]] = icmp ult i32 [[OUTER_IV]], [[LOOP_LIMIT]]
; AES-NEXT:    br i1 [[OUTER_EXIT_COND]], label %[[PH:.*]], label %[[EXIT:.*]]
; AES:       [[PH]]:
; AES-NEXT:    [[OUTER_IV_EXT:%.*]] = sext i32 [[OUTER_IV]] to i64
; AES-NEXT:    [[MSG_OUTER_IV:%.*]] = getelementptr inbounds i8, ptr [[MSG]], i64 [[OUTER_IV_EXT]]
; AES-NEXT:    [[MSG_LOAD:%.*]] = load i8, ptr [[MSG_OUTER_IV]], align 1
; AES-NEXT:    [[CRC_INIT:%.*]] = xor i8 [[MSG_LOAD]], [[CRC_OUTER]]
; AES-NEXT:    br label %[[INNER_LOOP:.*]]
; AES:       [[INNER_LOOP]]:
; AES-NEXT:    [[INNER_IV:%.*]] = phi i32 [ 0, %[[PH]] ], [ [[INNER_IV_NEXT:%.*]], %[[INNER_LOOP]] ]
; AES-NEXT:    [[CRC2:%.*]] = phi i8 [ [[CRC_INIT]], %[[PH]] ], [ [[TBL_LD:%.*]], %[[INNER_LOOP]] ]
; AES-NEXT:    [[INDEXER_EXT:%.*]] = zext i8 [[CRC2]] to i64
; AES-NEXT:    [[TBL_PTRADD:%.*]] = getelementptr inbounds i8, ptr @.crctable, i64 [[INDEXER_EXT]]
; AES-NEXT:    [[TBL_LD]] = load i8, ptr [[TBL_PTRADD]], align 1
; AES-NEXT:    [[INNER_IV_NEXT]] = add nuw nsw i32 [[INNER_IV]], 1
; AES-NEXT:    [[EXIT_COND1:%.*]] = icmp ne i32 [[INNER_IV]], 0
; AES-NEXT:    br i1 [[EXIT_COND1]], label %[[INNER_LOOP]], label %[[INNER_EXIT]]
; AES:       [[INNER_EXIT]]:
; AES-NEXT:    [[CRC_NEXT_LCSSA]] = phi i8 [ [[TBL_LD]], %[[INNER_LOOP]] ]
; AES-NEXT:    [[OUTER_IV_NEXT]] = add i32 [[OUTER_IV]], 1
; AES-NEXT:    br label %[[OUTER_LOOP]]
; AES:       [[EXIT]]:
; AES-NEXT:    [[CRC_OUTER_LCSSA:%.*]] = phi i8 [ [[CRC_OUTER]], %[[OUTER_LOOP]] ]
; AES-NEXT:    ret i8 [[CRC_OUTER_LCSSA]]
;
entry:
  br label %outer.loop
...
; NOAES:       [[LOOP]]:
; NOAES-NEXT:    [[IV:%.*]] = phi i8 [ 0, %[[ENTRY]] ], [ [[IV_NEXT:%.*]], %[[LOOP]] ]
; NOAES-NEXT:    [[CRC2:%.*]] = phi i32 [ [[CHECKSUM]], %[[ENTRY]] ], [ [[CRC_NEXT3:%.*]], %[[LOOP]] ]
; NOAES-NEXT:    [[IV_BITS:%.*]] = shl i8 [[IV]], 3
; NOAES-NEXT:    [[IV_INDEXER:%.*]] = zext i8 [[IV_BITS]] to i32
; NOAES-NEXT:    [[DATA_INDEXER:%.*]] = lshr i32 [[MSG]], [[IV_INDEXER]]
; NOAES-NEXT:    [[CRC_DATA_INDEXER:%.*]] = xor i32 [[DATA_INDEXER]], [[CRC2]]
; NOAES-NEXT:    [[INDEXER_LO:%.*]] = trunc i32 [[CRC_DATA_INDEXER]] to i8
; NOAES-NEXT:    [[INDEXER_EXT:%.*]] = zext i8 [[INDEXER_LO]] to i64
; NOAES-NEXT:    [[TBL_PTRADD:%.*]] = getelementptr inbounds i32, ptr @.crctable.8, i64 [[INDEXER_EXT]]
; NOAES-NEXT:    [[TBL_LD:%.*]] = load i32, ptr [[TBL_PTRADD]], align 4
; NOAES-NEXT:    [[CRC_LE_SHIFT:%.*]] = lshr i32 [[CRC2]], 8
; NOAES-NEXT:    [[CRC_NEXT3]] = xor i32 [[CRC_LE_SHIFT]], [[TBL_LD]]
; NOAES-NEXT:    [[IV_NEXT]] = add nuw nsw i8 [[IV]], 1
; NOAES-NEXT:    [[EXIT_COND1:%.*]] = icmp ne i8 [[IV]], 0
; NOAES-NEXT:    br i1 [[EXIT_COND1]], label %[[LOOP]], label %[[EXIT:.*]]
; NOAES:       [[EXIT]]:
; NOAES-NEXT:    [[CRC_NEXT_LCSSA:%.*]] = phi i32 [ [[CRC_NEXT3]], %[[LOOP]] ]
; NOAES-NEXT:    ret i32 [[CRC_NEXT_LCSSA]]
;
; AES-LABEL: define i32 @crc32.le.tc8.data32(
; AES-SAME: i32 [[CHECKSUM:%.*]], i32 [[MSG:%.*]]) #[[ATTR0]] {
; AES-NEXT:  [[ENTRY:.*]]:
; AES-NEXT:    [[CRC_EXT:%.*]] = zext i32 [[CHECKSUM]] to i64
; AES-NEXT:    [[DATA_LE_MASK:%.*]] = and i32 [[MSG]], 255
; AES-NEXT:    [[DATA_EXT:%.*]] = zext i32 [[DATA_LE_MASK]] to i64
; AES-NEXT:    [[XOR_CRC_DATA1:%.*]] = xor i64 [[CRC_EXT]], [[DATA_EXT]]
; AES-NEXT:    [[CLMUL_MU:%.*]] = call i64 @llvm.clmul.i64(i64 [[XOR_CRC_DATA1]], i64 273)
; AES-NEXT:    [[QUOT_LE_MASK:%.*]] = and i64 [[CLMUL_MU]], 255
; AES-NEXT:    [[CLMUL_GP:%.*]] = call i64 @llvm.clmul.i64(i64 [[QUOT_LE_MASK]], i64 67601)
; AES-NEXT:    [[XOR_CRC_MULT:%.*]] = xor i64 [[CRC_EXT]], [[CLMUL_GP]]
; AES-NEXT:    [[CRC_LE_LSHR:%.*]] = lshr i64 [[XOR_CRC_MULT]], 8
; AES-NEXT:    [[CRC_NEXT2:%.*]] = trunc i64 [[CRC_LE_LSHR]] to i32
; AES-NEXT:    br label %[[LOOP:.*]]
; AES:       [[LOOP]]:
; AES-NEXT:    [[CRC:%.*]] = phi i32 [ [[CHECKSUM]], %[[ENTRY]] ], [ [[CRC_NEXT:%.*]], %[[LOOP]] ]
; AES-NEXT:    [[DATA:%.*]] = phi i32 [ [[MSG]], %[[ENTRY]] ], [ [[DATA_NEXT:%.*]], %[[LOOP]] ]
; AES-NEXT:    [[IV:%.*]] = phi i8 [ 0, %[[ENTRY]] ], [ [[IV_NEXT:%.*]], %[[LOOP]] ]
; AES-NEXT:    [[XOR_CRC_DATA:%.*]] = xor i32 [[CRC]], [[DATA]]
; AES-NEXT:    [[SB_CRC_DATA:%.*]] = and i32 [[XOR_CRC_DATA]], 1
; AES-NEXT:    [[CHECK_SB:%.*]] = icmp eq i32 [[SB_CRC_DATA]], 0
; AES-NEXT:    [[CRC_LSHR:%.*]] = lshr i32 [[CRC]], 1
; AES-NEXT:    [[CRC_XOR:%.*]] = xor i32 [[CRC_LSHR]], 33800
; AES-NEXT:    [[CRC_NEXT]] = select i1 [[CHECK_SB]], i32 [[CRC_LSHR]], i32 [[CRC_XOR]]
; AES-NEXT:    [[IV_NEXT]] = add nuw nsw i8 [[IV]], 1
; AES-NEXT:    [[DATA_NEXT]] = lshr i32 [[DATA]], 1
; AES-NEXT:    [[EXIT_COND:%.*]] = icmp samesign ult i8 [[IV]], 7
; AES-NEXT:    br i1 [[EXIT_COND]], label %[[LOOP]], label %[[EXIT:.*]]
; AES:       [[EXIT]]:
...
; NOAES-LABEL: define i32 @crc.disabled.optsize(
; NOAES-SAME: i32 [[CHECKSUM:%.*]], i32 [[MSG:%.*]]) #[[ATTR1:[0-9]+]] {
; NOAES-NEXT:  [[ENTRY:.*]]:
; NOAES-NEXT:    br label %[[LOOP:.*]]
; NOAES:       [[LOOP]]:
...
;.
; NOAES: attributes #[[ATTR0]] = { "target-features"="-aes" }
; NOAES: attributes #[[ATTR1]] = { optsize "target-features"="-aes" }
;.
; AES: attributes #[[ATTR0]] = { "target-features"="+aes" }
; AES: attributes #[[ATTR1]] = { optsize "target-features"="+aes" }
; AES: attributes #[[ATTR2:[0-9]+]] = { nocallback nocreateundeforpoison nofree nosync nounwind speculatable willreturn memory(none) }
;.
