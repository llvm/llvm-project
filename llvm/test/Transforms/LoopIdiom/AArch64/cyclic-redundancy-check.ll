...
; RUN: opt -passes=loop-idiom -loop-idiom-force-crc-clmul -mtriple=aarch64 -mattr=-aes -S %s | FileCheck --check-prefixes=ALL,NOAES %s
; RUN: opt -passes=loop-idiom -loop-idiom-force-crc-clmul -mtriple=aarch64 -mattr=+aes -S %s | FileCheck --check-prefixes=ALL,AES %s
...
; AES-NEXT:    br i1 false, label %[[LOOP]], label %[[EXIT:.*]]
; AES:       [[EXIT]]:
; AES-NEXT:    [[CRC_NEXT_LCSSA:%.*]] = phi i16 [ [[CRC_NEXT3]], %[[LOOP]] ]
; AES-NEXT:    ret i16 [[CRC_NEXT_LCSSA]]
;
entry:
  br label %loop
...
; AES-NEXT:    [[CRC_SHL1:%.*]] = shl i16 [[CRC_CAST]], 8
; AES-NEXT:    [[XOR_CRC_MULT:%.*]] = xor i16 [[CRC_SHL1]], [[CLMUL_GP]]
...
; AES:       [[INNER_EXIT]]:
; AES-NEXT:    [[CRC_NEXT_LCSSA]] = phi i8 [ [[CRC_NEXT2]], %[[INNER_LOOP]] ]
...
; NOAES-LABEL: define i32 @crc32.le.tc32.optsize(
