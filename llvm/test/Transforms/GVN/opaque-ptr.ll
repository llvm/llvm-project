...
; RUN: opt < %s -passes=gvn -use-constant-int-for-fixed-length-splat -S | FileCheck %s --check-prefixes=CHECK,MDEP
; RUN: opt < %s -passes='gvn<memoryssa>' -S | FileCheck %s --check-prefixes=CHECK,MSSA
; RUN: opt < %s -passes='gvn<memoryssa>' -use-constant-int-for-fixed-length-splat -S | FileCheck %s --check-prefixes=CHECK,MSSA

declare void @use(ptr)
declare void @use.i32(i32)

define void @gep_cse(ptr %p) {
; CHECK-LABEL: define void @gep_cse(
; CHECK-SAME: ptr [[P:%.*]]) {
...
; CHECK-LABEL: define void @gep_cse_offset_canonicalization(
; CHECK-SAME: ptr [[P:%.*]], i64 [[IDX:%.*]], i64 [[IDX2:%.*]]) {
...
; CHECK-LABEL: define i32 @phi_trans(
; CHECK-SAME: i1 [[C:%.*]], ptr [[P1:%.*]], ptr [[P2:%.*]]) {
...
; CHECK-LABEL: define i32 @phi_trans_different_types(
; CHECK-SAME: i1 [[C:%.*]], ptr [[P1:%.*]], ptr [[P2:%.*]]) {
; CHECK-NEXT:    br i1 [[C]], label %[[IF:.*]], label %[[ELSE:.*]]
; CHECK:       [[IF]]:
; CHECK-NEXT:    [[GEP1:%.*]] = getelementptr i32, ptr [[P1]], i64 1
...
; MDEP-LABEL: define i32 @select_pre(
; MDEP-SAME: ptr [[PX:%.*]], ptr [[PY:%.*]]) {
; MDEP-NEXT:    [[T2:%.*]] = load i32, ptr [[PY]], align 4
; MDEP-NEXT:    [[T3:%.*]] = load i32, ptr [[PX]], align 4
; MDEP-NEXT:    [[CMP:%.*]] = icmp slt i32 [[T2]], [[T3]]
; MDEP-NEXT:    [[SELECT:%.*]] = select i1 [[CMP]], ptr [[PX]], ptr [[PY]]
; MDEP-NEXT:    [[TMP1:%.*]] = select i1 [[CMP]], i32 [[T3]], i32 [[T2]]
; MDEP-NEXT:    ret i32 [[TMP1]]
;
; MSSA-LABEL: define i32 @select_pre(
; MSSA-SAME: ptr [[PX:%.*]], ptr [[PY:%.*]]) {
; MSSA-NEXT:    [[T2:%.*]] = load i32, ptr [[PY]], align 4
; MSSA-NEXT:    [[T3:%.*]] = load i32, ptr [[PX]], align 4
; MSSA-NEXT:    [[CMP:%.*]] = icmp slt i32 [[T2]], [[T3]]
; MSSA-NEXT:    [[SELECT:%.*]] = select i1 [[CMP]], ptr [[PX]], ptr [[PY]]
; MSSA-NEXT:    [[R:%.*]] = load i32, ptr [[SELECT]], align 4
; MSSA-NEXT:    ret i32 [[R]]
;
...
; CHECK-LABEL: define i64 @select_pre_different_types(
; CHECK-SAME: ptr [[PX:%.*]], ptr [[PY:%.*]]) {
