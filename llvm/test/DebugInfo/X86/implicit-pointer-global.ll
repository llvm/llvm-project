...
;
; An implicit pointer to a global address is not supported: the value is not
; available to point at. Emitting only the global's address would describe the
; variable as living at &g, which is a different location, so drop it instead.
; This needs updating if the fix instead emits a correct implicit pointer.
;
...
; CHECK: NULL
;
; Giving up must not leave the artificial DIE behind that would have held the
; pointee value.
; CHECK-NOT: DW_TAG_dwarf_procedure

@g = global i64 0, align 8

define void @globaladdr_tag_offset() !dbg !6 {
entry:
  call void @llvm.dbg.value(metadata ptr @g, metadata !10, metadata !DIExpression(DW_OP_LLVM_implicit_pointer)), !dbg !11
...
!7 = !DIDerivedType(tag: DW_TAG_pointer_type, baseType: !13, size: 64)
