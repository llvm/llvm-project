; RUN: llc -O0 -mtriple=amdgcn-amd-amdhsa -mcpu=gfx906 -verify-machineinstrs -filetype=obj < %s | llvm-dwarfdump -debug-info - | FileCheck --check-prefixes=CHECK,CHECK-ORIG-OPS %s
; RUN: llc -O0 -mtriple=amdgcn-amd-amdhsa -mcpu=gfx906 -verify-machineinstrs -filetype=obj -emit-heterogeneous-dwarf-as-user-ops < %s | llvm-dwarfdump -debug-info - | FileCheck --check-prefixes=CHECK,CHECK-USER-OPS %s
