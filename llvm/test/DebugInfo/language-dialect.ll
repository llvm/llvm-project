...
;;   CU1 (simt.cu): simt_kernel - explicit dialect: DW_LANG_DIALECT_simt
;;   CU2 (tile.cu): tile_kernel - explicit dialect: DW_LANG_DIALECT_tile
;;
;; DW_AT_LLVM_language_dialect is only emitted when DW_LANG_DIALECT_simt or
;; DW_LANG_DIALECT_tile is explicitly specified.
...
; IR-NOT: dialect:
; IR-SAME: )
; IR: !{{[0-9]+}} = distinct !DICompileUnit({{.*}}dialect: DW_LANG_DIALECT_simt{{.*}})
; IR: !{{[0-9]+}} = distinct !DICompileUnit({{.*}}dialect: DW_LANG_DIALECT_tile{{.*}})
...
; DWARF: DW_TAG_compile_unit
;; DW_LANG_DIALECT_simt = 1
; DWARF:      .b8 1 // DW_AT_LLVM_language_dialect
; DWARF: DW_TAG_compile_unit
;; DW_LANG_DIALECT_tile = 2
...
;; CU for default the case (no dialect).
...
!1 = distinct !DICompileUnit(language: DW_LANG_C_plus_plus, file: !18, producer: "", isOptimized: false, runtimeVersion: 0, emissionKind: FullDebug, dialect: DW_LANG_DIALECT_simt)
...
;; CU for tile.
!2 = distinct !DICompileUnit(language: DW_LANG_C_plus_plus, file: !6, producer: "", isOptimized: false, runtimeVersion: 0, emissionKind: FullDebug, dialect: DW_LANG_DIALECT_tile)
