...
; CHECK-NEXT:  entry:
; CHECK-NEXT:    call void @llvm.dbg.assign(metadata ptr [[THIS:%.*]], metadata [[META26:![0-9]+]], metadata !DIExpression(), metadata [[META29:![0-9]+]], metadata ptr undef, metadata !DIExpression()), !dbg [[DBG30:![0-9]+]]
; CHECK-NEXT:    [[CALL:%.*]] = call { <2 x float>, <2 x float> } @_ZNK1c5m_fn1Ev(ptr [[THIS]]), !dbg [[DBG31:![0-9]+]]
; CHECK-NEXT:    [[TMP0:%.*]] = extractvalue { <2 x float>, <2 x float> } [[CALL]], 0, !dbg [[DBG31]]
; CHECK-NEXT:    call void @llvm.dbg.assign(metadata <2 x float> [[TMP0]], metadata [[META28:![0-9]+]], metadata !DIExpression(DW_OP_LLVM_fragment, 0, 64), metadata [[META32:![0-9]+]], metadata ptr undef, metadata !DIExpression()), !dbg [[DBG30]]
; CHECK-NEXT:    [[TMP1:%.*]] = extractvalue { <2 x float>, <2 x float> } [[CALL]], 1, !dbg [[DBG31]]
; CHECK-NEXT:    call void @llvm.dbg.assign(metadata <2 x float> [[TMP1]], metadata [[META28]], metadata !DIExpression(DW_OP_LLVM_fragment, 64, 64), metadata [[META33:![0-9]+]], metadata ptr undef, metadata !DIExpression()), !dbg [[DBG30]]
