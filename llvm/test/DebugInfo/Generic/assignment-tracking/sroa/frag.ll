...
; CHECK-LABEL: @_Z1dv(
; CHECK-NEXT:  entry:
; CHECK-NEXT:    call void @llvm.dbg.assign(metadata i1 undef, metadata [[META11:![0-9]+]], metadata !DIExpression(DW_OP_LLVM_fragment, 0, 256), metadata [[META22:![0-9]+]], metadata ptr undef, metadata !DIExpression()), !dbg [[DBG23:![0-9]+]]
; CHECK-NEXT:    [[CALL:%.*]] = call { <2 x float>, <2 x float> } @_Z3fn1v(), !dbg [[DBG24:![0-9]+]]
; CHECK-NEXT:    [[TMP0:%.*]] = extractvalue { <2 x float>, <2 x float> } [[CALL]], 0, !dbg [[DBG24]]
; CHECK-NEXT:    [[TMP1:%.*]] = extractvalue { <2 x float>, <2 x float> } [[CALL]], 1, !dbg [[DBG24]]
; CHECK-NEXT:    call void @llvm.dbg.assign(metadata <2 x float> [[TMP0]], metadata [[META11]], metadata !DIExpression(DW_OP_LLVM_fragment, 256, 64), metadata [[META25:![0-9]+]], metadata ptr undef, metadata !DIExpression()), !dbg [[DBG23]]
; CHECK-NEXT:    call void @llvm.dbg.assign(metadata <2 x float> [[TMP1]], metadata [[META11]], metadata !DIExpression(DW_OP_LLVM_fragment, 320, 64), metadata [[META26:![0-9]+]], metadata ptr undef, metadata !DIExpression()), !dbg [[DBG23]]
