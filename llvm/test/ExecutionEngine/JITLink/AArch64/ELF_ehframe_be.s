# REQUIRES: asserts
# RUN: llvm-mc -triple=aarch64_be-linux-gnu -filetype=obj -o %t %s
# RUN: llvm-jitlink -num-threads=0 -debug-only=jitlink -noexec \
# RUN:              -phony-externals %t 2>&1 | FileCheck %s
#
# Check that splitting of eh-frame sections works in a big-endian (BE8)
# object: .eh_frame records are data (big-endian here), so this exercises
# the endianness-aware reads of DWARFRecordSectionSplitter and
# EHFrameEdgeFixer.
#
