...
# RUN: llvm-mc -triple=aarch64_be-unknown-linux-gnu -position-independent \
...
# Big-endian (BE8) counterpart of ELF_relocations.s: same relocation coverage,
# plus checks that data fixups are written in target (big) endianness.
# Instructions are always little-endian encoded (the A64 ISA is
# word-invariant), so instruction fixups still patch LE-encoded words here.
...
# Check R_AARCH64_LDST*_ABS_LO12NC relocation of a local symbol
#
...
# In a big-endian object the 64-bit value must be stored big-endian.
#
...
# Check R_AARCH64_ABS32 relocation of an external symbol
#
...
external_data_addr_word:
        .word	external_data
        .size	external_data_addr_word, 4

# Check R_AARCH64_PREL32 / R_AARCH64_PREL64 relocations of local symbols.
# Delta fixups must be written in target (big) endianness, for both forward
# (positive) and backward (negative) deltas.
#
# jitlink-check: *{4}prel32_pos = (named_func - prel32_pos)[31:0]
# jitlink-check: *{8}prel64_pos = named_func - prel64_pos
# jitlink-check: *{4}prel32_neg = (main - prel32_neg)[31:0]
# jitlink-check: *{8}prel64_neg = main - prel64_neg
        .globl  prel32_pos
        .p2align  2
prel32_pos:
        .word	named_func - .
