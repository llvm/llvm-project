...
; CHECK-NEXT:    --> {%a0,+,1}<%b3> U: full-set S: full-set Exits: ((-1 * (ptrtoaddr ptr %a0 to i64)) + %a0) LoopDispositions: { %b3: Computable, %b1: Uniform }
...
; CHECK-NEXT:    --> {(1 + %a0),+,1}<%b3> U: full-set S: full-set Exits: (1 + (-1 * (ptrtoaddr ptr %a0 to i64)) + %a0) LoopDispositions: { %b3: Computable, %b1: Uniform }
; CHECK-NEXT:  Determining loop execution counts for: @f
; CHECK-NEXT:  Loop %b3: <multiple exits> backedge-taken count is (-1 * (ptrtoaddr ptr %a0 to i64))
; CHECK-NEXT:    exit count for b3: (-1 * (ptrtoaddr ptr %a0 to i64))
; CHECK-NEXT:    exit count for b4: ***COULDNOTCOMPUTE***
; CHECK-NEXT:  Loop %b3: constant max backedge-taken count is i64 -1
; CHECK-NEXT:  Loop %b3: symbolic max backedge-taken count is (-1 * (ptrtoaddr ptr %a0 to i64))
; CHECK-NEXT:    symbolic max exit count for b3: (-1 * (ptrtoaddr ptr %a0 to i64))
