...
; May-aliasing array slots as a store destination. The destination is loaded
; from arr[%k], which may-alias arr[%i] (holding the address of a global) and
; arr[%j] (holding a local). Storing %x through that destination may write into
; the global slot, so %x escapes. The backward base-finding walk must not stop
; at the first may-aliasing store (arr[%j]=%loc): it has to keep walking up to
; see arr[%i]=@GPtr, otherwise it would unsoundly report %x as non-escaping.
define void @mayalias_dest_escape(i64 %i, i64 %j, i64 %k) {
; CHECK-LABEL: Printing analysis 'Escape Analysis' for function 'mayalias_dest_escape':
; CHECK: arr escapes: no
; CHECK: x escapes: yes
; CHECK: loc escapes: no
  %arr = alloca [4 x ptr], align 8
  %x = alloca i8, align 1
  %loc = alloca i32, align 4
  %gi = getelementptr inbounds [4 x ptr], ptr %arr, i64 0, i64 %i
  store ptr @GPtr, ptr %gi, align 8
  %gj = getelementptr inbounds [4 x ptr], ptr %arr, i64 0, i64 %j
  store ptr %loc, ptr %gj, align 8
  %gk = getelementptr inbounds [4 x ptr], ptr %arr, i64 0, i64 %k
  %dst = load ptr, ptr %gk, align 8
  store ptr %x, ptr %dst, align 8
  ret void
}
...
declare noalias ptr @realloc(ptr, i64) allockind("realloc") allocsize(1) "alloc-family"="malloc"
