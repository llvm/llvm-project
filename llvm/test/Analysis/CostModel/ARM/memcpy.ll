...
; RUN: opt < %s  -passes="print<cost-model>" 2>&1 -disable-output -cost-kind=code-size | \
; RUN:    FileCheck %s --check-prefixes=COMMON,CHECK-NO-SA
; RUN: opt < %s  -passes="print<cost-model>" 2>&1 -disable-output -cost-kind=code-size -mattr=+strict-align | \
