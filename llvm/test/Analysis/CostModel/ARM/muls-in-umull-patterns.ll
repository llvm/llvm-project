...
; RUN: opt -passes="print<cost-model>" -cost-kind=all 2>&1 -disable-output -mtriple thumbv8.1m.main -mattr=+dsp  < %s | FileCheck %s
; RUN: opt -passes="print<cost-model>" -cost-kind=all 2>&1 -disable-output -mtriple thumbv8.1m.main < %s | FileCheck %s --check-prefix=CHECK-NO-DSP
