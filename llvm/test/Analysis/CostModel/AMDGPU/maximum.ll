...
; RUN: opt -passes="print<cost-model>" 2>&1 -disable-output -mtriple=amdgpu9.50-unknown-amdhsa -mattr=+half-rate-64-ops < %s | FileCheck -check-prefix=GFX950-FASTF64 %s
; RUN: opt -passes="print<cost-model>" 2>&1 -disable-output -mtriple=amdgpu9.0a-unknown-amdhsa -mattr=+half-rate-64-ops < %s | FileCheck -check-prefixes=ALL,GFX9,GFX90A-FASTF64 %s
