...
; RUN: opt -passes="print<cost-model>" 2>&1 -disable-output -mtriple=amdgpu7.00-unknown-amdhsa < %s | FileCheck -check-prefixes=ALL %s
; RUN: opt -passes="print<cost-model>" -cost-kind=code-size 2>&1 -disable-output -mtriple=amdgpu7.00-unknown-amdhsa < %s | FileCheck -check-prefixes=ALL-SIZE %s
; END.
