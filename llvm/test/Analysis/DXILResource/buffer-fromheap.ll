...
; CHECK-NEXT:   Kind: RawBuffer
  ...
  ; Duplicated initialization should resolve to the same resource.
  %srv1_dupl = tail call target("dx.RawBuffer", %struct.S, 0, 0)
      @llvm.dx.resource.handlefromheap.tdx.RawBuffer_s_struct.Ss_0_0t(i32 %idx)
...
; CHECK-NOT: Class: SRV
  ...
  %uav0 = tail call target("dx.RawBuffer", double, 1, 0)
      @llvm.dx.resource.handlefromheap.tdx.RawBuffer_f64_1_0t(i32 %add1)
...
; CHECK-NEXT:   Alignment: 0
  ...
  %uav1 = tail call target("dx.RawBuffer", <4 x float>, 1, 0)
      @llvm.dx.resource.handlefromheap.tdx.RawBuffer_v4f32_1_0t(i32 %add2)
...
; CHECK-NEXT:   Alignment: 0
  ...
  %uav3 = tail call target("dx.TypedBuffer", i32, 1, 0, 1)
      @llvm.dx.resource.handlefromheap.tdx.TypedBuffer_i32_1_0_1t(i32 10)

  ; Duplicated initialization should resolve to the same resource.
  %uav3_dupl = tail call target("dx.TypedBuffer", i32, 1, 0, 1)
      @llvm.dx.resource.handlefromheap.tdx.TypedBuffer_i32_1_0_1t(i32 10)
...
; CHECK-NOT: Class: UAV
  ...
  %cbv0 = tail call target("dx.CBuffer", %P)
      @llvm.dx.resource.handlefromheap.tdx.CBuffer_s_Pst(i32 %add4)
; CHECK: Resource [[CBV0:7]]:
; CHECK-NEXT:   HeapIndexID: {{[0-9]+}}
; CHECK-NEXT:   Globally Coherent: 0
; CHECK-NEXT:   Has Atomic64 Use: 0
; CHECK-NEXT:   Counter Direction: Unknown
...
; CHECK-NEXT:   CBuffer size: 4
  ...
  %add5 = add i32 %idx, 5
  %cbv1 = tail call target("dx.CBuffer", %Q)
      @llvm.dx.resource.handlefromheap.tdx.CBuffer_s_Qst(i32 %add5)

  ; Duplicated initialization should resolve to the same resource.
  %cbv1_dupl = tail call target("dx.CBuffer", %Q)
      @llvm.dx.resource.handlefromheap.tdx.CBuffer_s_Qst(i32 %add5)

; CHECK: Resource [[CBV1:8]]:
; CHECK-NEXT:   HeapIndexID: {{[0-9]+}}
; CHECK-NEXT:   Globally Coherent: 0
; CHECK-NEXT:   Has Atomic64 Use: 0
; CHECK-NEXT:   Counter Direction: Unknown
...
; CHECK-DAG: Call bound to [[UAV0]]:  %uav0 = tail call target("dx.RawBuffer", double, 1, 0) @llvm.dx.resource.handlefromheap.tdx.RawBuffer_f64_1_0t(i32 %add1)
; CHECK-DAG: Call bound to [[UAV1]]:  %uav1 = tail call target("dx.RawBuffer", <4 x float>, 1, 0) @llvm.dx.resource.handlefromheap.tdx.RawBuffer_v4f32_1_0t(i32 %add2)
; CHECK-DAG: Call bound to [[UAV2]]:  %uav2 = tail call target("dx.RawBuffer", <4 x float>, 1, 0) @llvm.dx.resource.handlefromheap.tdx.RawBuffer_v4f32_1_0t(i32 %add3)
; CHECK-DAG: Call bound to [[UAV3]]:  %uav3 = tail call target("dx.TypedBuffer", i32, 1, 0, 1) @llvm.dx.resource.handlefromheap.tdx.TypedBuffer_i32_1_0_1t(i32 10)
; CHECK-DAG: Call bound to [[CBV0]]:  %cbv0 = tail call target("dx.CBuffer", %P) @llvm.dx.resource.handlefromheap.tdx.CBuffer_s_Pst(i32 %add4)
; CHECK-DAG: Call bound to [[CBV1]]:  %cbv1 = tail call target("dx.CBuffer", %Q) @llvm.dx.resource.handlefromheap.tdx.CBuffer_s_Qst(i32 %add5)

; duplicate calls should map to existing resources in the list, not create new ones
; CHECK-DAG: Call bound to [[SRV1]]:  %srv1_dupl = tail call target("dx.RawBuffer", %struct.S, 0, 0) @llvm.dx.resource.handlefromheap.tdx.RawBuffer_s_struct.Ss_0_0t(i32 %idx)
; CHECK-DAG: Call bound to [[CBV1]]:  %cbv1_dupl = tail call target("dx.CBuffer", %Q) @llvm.dx.resource.handlefromheap.tdx.CBuffer_s_Qst(i32 %add5)
; CHECK-DAG: Call bound to [[UAV3]]:  %uav3_dupl = tail call target("dx.TypedBuffer", i32, 1, 0, 1) @llvm.dx.resource.handlefromheap.tdx.TypedBuffer_i32_1_0_1t(i32 10)
