...
# CHECK: :[[@LINE-1]]:{{[0-9]+}}: error: register is not supported for use with this directive
    ...
    ret
    .seh_endproc
# CHECK: :[[@LINE-1]]:{{[0-9]+}}: error: Missing .seh_endepilogue in i
...
k:
    ...
    .seh_pushreg %rsi
    .seh_splitchained
# CHECK: :[[@LINE-1]]:{{[0-9]+}}: error: can't split into a new chained region (.seh_splitchained) in the middle of a prolog in k
    .seh_startepilogue
