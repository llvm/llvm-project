// RUN: not llvm-mc -triple=amdgpu9.08 %s -filetype=null 2>&1 | FileCheck --check-prefixes=GFX908,NOT-GFX90A --implicit-check-not=error: %s
// RUN: not llvm-mc -triple=amdgpu10.10 %s -filetype=null 2>&1 | FileCheck --check-prefixes=GFX1010,NOT-GFX90A --implicit-check-not=error: %s
// RUN: llvm-mc -triple=amdgpu9.0a -show-encoding %s | FileCheck --check-prefix=GFX90A %s
...
// NOT-GFX90A: :[[@LINE+1]]:{{[0-9]+}}: error: instruction not supported on this GPU
...
// GFX908: :[[@LINE+1]]:{{[0-9]+}}: error: instruction not supported on this GPU
...
// NOT-GFX90A: :[[@LINE+1]]:{{[0-9]+}}: error: not a valid operand.
...
// GFX90A: global_atomic_add_f32  v0, v[0:1], v2, off glc ; encoding: [0x00,0x80,0x35,0xdd,0x00,0x02,0x7f,0x00]
// GFX908: :[[@LINE+2]]:{{[0-9]+}}: error: invalid operand for instruction
...
// GFX908: :[[@LINE+2]]:{{[0-9]+}}: error: operands are not valid for this GPU or mode
// GFX1010: :[[@LINE+1]]:{{[0-9]+}}: error: instruction not supported on this GPU
global_atomic_add_f32 v1, v0, v2, s[0:1] glc ; encoding: [0x00,0x80,0x35,0xdd,0x00,0x02,0x00,0x01]

// GFX908: :[[@LINE+3]]:{{[0-9]+}}: error: invalid operand for instruction
// GFX1010: :[[@LINE+2]]:{{[0-9]+}}: error: instruction not supported on this GPU
// GFX90A: global_atomic_pk_add_f16  v0, v[0:1], v2, off glc ; encoding: [0x00,0x80,0x39,0xdd,0x00,0x02,0x7f,0x00]
global_atomic_pk_add_f16 v0, v[0:1], v2, off glc
