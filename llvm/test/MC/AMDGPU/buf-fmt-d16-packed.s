// RUN: llvm-mc -triple=amdgpu8.10 -show-encoding %s | FileCheck -check-prefix=PACKED %s
// RUN: llvm-mc -triple=amdgpu9.00 -show-encoding %s | FileCheck -check-prefix=PACKED %s

// RUN: not llvm-mc -triple=amdgpu8.03 -filetype=null 2>&1 %s | FileCheck -check-prefix=UNPACKED-ERR --implicit-check-not=error: %s
