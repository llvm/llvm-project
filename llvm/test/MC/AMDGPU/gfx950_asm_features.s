...
// RUN: llvm-mc -triple=amdgpu9.50 -show-encoding %s | FileCheck --check-prefix=GFX950 --strict-whitespace %s
// RUN: not llvm-mc -triple=amdgpu9.42 %s -filetype=null 2>&1 | FileCheck --check-prefixes=NOT-GFX942 --implicit-check-not=error: %s
// RUN: not llvm-mc -triple=amdgpu9.0a %s -filetype=null 2>&1 | FileCheck --check-prefixes=NOT-GFX90a --implicit-check-not=error: %s
// RUN: not llvm-mc -triple=amdgpu10.10 %s -filetype=null 2>&1 | FileCheck --check-prefixes=NOT-GFX1010 --implicit-check-not=error: %s
