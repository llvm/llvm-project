...
// RUN: not llvm-mc -triple=amdgpu6.01 %s -filetype=null 2>&1 | FileCheck -check-prefix=SIVI --implicit-check-not=error: %s
// RUN: not llvm-mc -triple=amdgpu8.02 %s -filetype=null 2>&1 | FileCheck -check-prefix=SIVI --implicit-check-not=error: %s
