...
// RUN: not llvm-mc -triple=amdgpu9.00 %s -filetype=null 2>&1 | FileCheck -check-prefix=PREGFX11 --implicit-check-not=error: %s
// RUN: not llvm-mc -triple=amdgpu10.10 %s -filetype=null 2>&1 | FileCheck -check-prefix=PREGFX11 --implicit-check-not=error: %s
// RUN: llvm-mc -triple=amdgpu11.00 -show-encoding %s | FileCheck -check-prefix=GFX11 --implicit-check-not=error: %s
