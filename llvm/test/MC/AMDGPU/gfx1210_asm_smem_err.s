// RUN: not llvm-mc -arch=amdgcn -mcpu=gfx1210 -show-encoding %s 2>&1 | FileCheck --check-prefix=GFX1210-ERR --implicit-check-not=error: --strict-whitespace %s

s_buffer_load_i8 s5, s[4:7], s0 scale_offset
// GFX1210-ERR: :[[@LINE-1]]:{{[0-9]+}}: error: scale_offset is not supported for this instruction
// GFX1210-ERR-NEXT:{{^}}s_buffer_load_i8 s5, s[4:7], s0 scale_offset
// GFX1210-ERR-NEXT:{{^}}                                ^

s_prefetch_data s[18:19], 100, s10, 7 nv
// GFX1210-ERR: :[[@LINE-1]]:{{[0-9]+}}: error: invalid operand for instruction
// GFX1210-ERR-NEXT:{{^}}s_prefetch_data s[18:19], 100, s10, 7 nv
// GFX1210-ERR-NEXT:{{^}}                                      ^

s_prefetch_data s[18:19], 100, s10, 7 scale_offset
// GFX1210-ERR: :[[@LINE-1]]:{{[0-9]+}}: error: invalid operand for instruction
// GFX1210-ERR-NEXT:{{^}}s_prefetch_data s[18:19], 100, s10, 7 scale_offset
// GFX1210-ERR-NEXT:{{^}}                                      ^
