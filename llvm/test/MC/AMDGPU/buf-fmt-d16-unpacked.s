// RUN: llvm-mc -triple=amdgpu8.03 -show-encoding %s | FileCheck -check-prefix=UNPACKED %s
// RUN: not llvm-mc -triple=amdgpu8.10 -filetype=null 2>&1 %s | FileCheck -check-prefix=PACKED-ERR --implicit-check-not=error: %s
// RUN: not llvm-mc -triple=amdgpu9.00 -filetype=null 2>&1 %s | FileCheck -check-prefix=PACKED-ERR --implicit-check-not=error: %s
