// RUN: not llvm-mc -triple=amdgpu9.08 -show-encoding %s | FileCheck --check-prefix=GFX908 %s
// RUN: not llvm-mc -triple=amdgpu9.08 %s -filetype=null 2>&1 | FileCheck --check-prefix=GFX908-ERR --implicit-check-not=error: %s
