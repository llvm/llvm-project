// RUN: llvm-mc -triple=amdgpu9.00 -show-encoding %s | FileCheck -check-prefix=GFX9-MADMIX %s
// RUN: not llvm-mc -triple=amdgpu9.04 %s -filetype=null 2>&1 | FileCheck -check-prefix=GFX9-FMAMIX-ERR --implicit-check-not=error: %s
// RUN: not llvm-mc -triple=amdgpu9.06 %s -filetype=null 2>&1 | FileCheck -check-prefix=GFX9-FMAMIX-ERR --implicit-check-not=error: %s
