// RUN: llvm-mc -triple=amdgpu9.42 -show-encoding %s | FileCheck --check-prefix=GFX942 --strict-whitespace %s
// RUN: llvm-mc -triple=amdgpu9.50 -show-encoding %s | FileCheck --check-prefix=GFX942 --strict-whitespace %s
// RUN: not llvm-mc -triple=amdgpu9.0a %s -filetype=null 2>&1 | FileCheck --check-prefixes=NOT-GFX942,GFX90A --implicit-check-not=error: %s
// RUN: not llvm-mc -triple=amdgpu10.10 %s -filetype=null 2>&1 | FileCheck --check-prefixes=NOT-GFX942,GFX10 --implicit-check-not=error: %s
