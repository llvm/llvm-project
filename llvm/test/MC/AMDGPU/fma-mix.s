// RUN: llvm-mc -triple=amdgpu9.04 -show-encoding %s | FileCheck -check-prefix=GFX9-FMAMIX %s
// RUN: llvm-mc -triple=amdgpu9.06 -show-encoding %s | FileCheck -check-prefix=GFX9-FMAMIX %s
// RUN: not llvm-mc -triple=amdgpu9.00 %s -filetype=null 2>&1 | FileCheck -check-prefix=GFX9-MADMIX-ERR --implicit-check-not=error: %s
