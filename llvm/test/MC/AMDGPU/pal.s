// RUN: llvm-mc -triple=amdgpu7.00--amdpal -show-encoding %s | FileCheck %s --check-prefix=ASM
// RUN: llvm-mc -filetype=obj -triple=amdgpu7.00--amdpal -show-encoding %s | llvm-readobj --symbols -S --sd - | FileCheck %s --check-prefix=ELF
