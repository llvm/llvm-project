...
; RUN: opt --opaque-pointers=0 -module-summary %p/Inputs/alias_resolution.ll -o %t2.bc
; RUN: llvm-lto --opaque-pointers=0 -thinlto-action=thinlink -o %t.index.bc %t1.bc %t2.bc
; RUN: llvm-lto --opaque-pointers=0 -thinlto-action=promote -thinlto-index %t.index.bc %t2.bc -o - | llvm-dis --opaque-pointers=0 -o - | FileCheck %s --check-prefix=PROMOTE_MOD2 --check-prefix=NOTPROMOTED
; RUN: llvm-lto --opaque-pointers=0 -thinlto-action=promote -thinlto-index %t.index.bc %t1.bc -o - | llvm-dis --opaque-pointers=0 -o - | FileCheck %s --check-prefix=PROMOTE_MOD1 --check-prefix=NOTPROMOTED
