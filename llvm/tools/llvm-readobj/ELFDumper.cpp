...
static const EnumEntry<unsigned> ElfHeaderAMDGPUFlagsABIVersion01[] = {
...
static const EnumEntry<unsigned> ElfHeaderAMDGPUFlagsABIVersion2[] = {
  ...
  LLVM_READOBJ_ENUM_ENT(ELF, EF_AMDGPU_FEATURE_SRAM_ECC_DEFAULT),
  LLVM_READOBJ_ENUM_ENT(ELF, EF_AMDGPU_FEATURE_SRAM_ECC_OFF),
  LLVM_READOBJ_ENUM_ENT(ELF, EF_AMDGPU_FEATURE_SRAM_ECC_ON),
};
...
template <class ELFT> void LLVMStyle<ELFT>::printFileHeaders(const ELFO *Obj) {
  const Elf_Ehdr *E = Obj->getHeader();
  {
    ...
    if (E->e_machine == EM_MIPS)
      ...
      if (E->e_ident[EI_ABIVERSION] == 0 || E->e_ident[EI_ABIVERSION] == 1) {
        ...
        W.printFlags("Flags", E->e_flags,
                     ...
                     unsigned(ELF::EF_AMDGPU_FEATURE_SRAM_ECC));
      }
    }
    else if (E->e_machine == EM_RISCV)
