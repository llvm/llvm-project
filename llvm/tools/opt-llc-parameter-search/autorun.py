            ...
            match flag["type"]:
                case "uint" | "int" | "number":
                    ...
                    if distribution == "logarithmic":
                        ...
                        if low == 0 or high == 0:
                            print(
                                "0 is an invalid number for a log distribution",
                                file=sys.stderr,
                            )
                            print(f"Found in {name}", file=sys.stderr)
                            sys.exit(1)
                        space[name] = hp.qloguniform(
                            name, np.log(low), np.log(high), step
                        )
...
def objective(params: dict[str, Any]) -> dict[str, Any]:
    ...
    runtimes = run.replay_and_measure_kernel(
        json_path, config, output_dir, False, False  # run_bitcode
    )  # dry_run
    if runtimes:
        return {
...
def write_trials_csv(
    baseline: dict[str, Any], trial_results: list[dict[str, Any]]
) -> None:
    """
...
def main():
    ...
    parser.add_argument(
        "bc_path", help="Path to the recorded device IR bitcode (.bc) file"
    )
    parser.add_argument(
        "json_path", help="Path to the JSON file containing settings for desired flags"
    )
    parser.add_argument(
        "--arch", default="gfx90a", help="GPU Architecture (default: gfx90a)"
    )
    parser.add_argument(
        "--max-trials", default="100", help="Maximum number of trials (defualt: 100)"
    )
    parser.add_argument(
        "--no-plot",
        default=False,
    ...
    if status != 0:
        sys.exit(status)
    baseline_runtimes = run.replay_and_measure_kernel(
        kernel_json_path, baseline_config, output_dir, False, False
    )
