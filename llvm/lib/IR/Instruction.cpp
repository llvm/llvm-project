...
void Instruction::dropUBImplyingAttrsAndMetadata(bool mayTriggerUBIfPoison) {
  // !annotation metadata does not impact semantics.
  // !range, !nonnull and !align produce poison, so they are safe to speculate.
  // !noundef and various AA metadata must be dropped, as it generally produces
  // immediate undefined behavior.
  unsigned PoisonKnownIDs[] = {LLVMContext::MD_annotation,
                               LLVMContext::MD_range, LLVMContext::MD_nonnull,
                               LLVMContext::MD_align};
  unsigned SimpleKnownIDs[] = {LLVMContext::MD_annotation};
  if (mayTriggerUBIfPoison)
    dropUBImplyingAttrsAndUnknownMetadata(SimpleKnownIDs);
  else
    dropUBImplyingAttrsAndUnknownMetadata(PoisonKnownIDs);
}
