...
Expected<MappedFileRegionArena> MappedFileRegionArena::create(
  ...
  if (FileSize->Size < Capacity) {
    // Ensure the exclusive lock before resize the file. In the rare case when
    // opening a large CAS using a small requested size, a shared lock needs to
    // switch to an exclusive lock here.
