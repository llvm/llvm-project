...
void IRChangedPrinter::InstructionChangeReporter::generateIRRepresentation(
  ...
  auto PrintValueName = [this](raw_ostream &OS, const Value &V) {
    OS << (isa<GlobalValue>(V) ? "@<" : "%<") << getValueID(V) << '>';
  };
  auto PrintTypeName = [this](raw_ostream &OS, const Type &Ty) {
  ...
  for (const Function *F : Functions) {
    ...
    for (const BasicBlock &BB : *F) {
      ...
      for (const Instruction &I : BB) {
        ...
        printInstructionWithCustomNames(OS, MST, I, PrintValueName,
                                        PrintTypeName,
