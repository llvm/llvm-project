...
X86_64ABIInfo::useFirstFieldIfTransparentUnion(const Type *Ty) const {
  if (const auto *ST = dyn_cast<StructType>(Ty)) {
    if (ST->isTransparentUnion()) {
      assert(!ST->getFields().empty() &&
             "sema created an empty transparent union");
      return ST->getFields().front().FieldType;
    }
  }
  return Ty;
}
...
X86_64ABIInfo::classifyArgumentType(const Type *Ty, unsigned FreeIntRegs,
  ...
  case Integer:
    ...
    if (Hi == NoClass && ResType->isInteger()) {
      if (Ty->isInteger() && isPromotableIntegerType(cast<IntegerType>(Ty)))
...
ABIArgInfo X86_64ABIInfo::getIndirectResult(const Type *Ty,
  ...
  if (!isAggregateTypeForABI(Ty) && !isIllegalVectorType(Ty) &&
    ...
    return (Ty->isInteger() && isPromotableIntegerType(cast<IntegerType>(Ty))
