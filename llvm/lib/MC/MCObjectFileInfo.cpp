...
MCObjectFileInfo::getPseudoProbeDescSection(StringRef FuncName) const {
  ...
  if (ObjFileType == MCContext::IsELF) {
    ...
    // FIXME: For each function generated by the frontend, there's a pseudo
    // probe descriptor to record its GUID, name and hash. The pseudo probe desc
    // section should be associated with its function section. However, the
    // function might be fully inlined. It is possible that several ODR
    // functions with the same name (GUID) but different bodies (hash) are fully
    // inlined. Current implementation assumes that GUID is unique and only
    // preserves one of its descriptors. For probes in those functions, they may
    // be mapped to wrong function. Setting selection to
    // IMAGE_COMDAT_SELECT_EXACT_MATCH can prompt this situation implicitly, but
    // it cannot address this issue and will cause linker duplicate symbol
    // errors. We set selection to IMAGE_COMDAT_SELECT_ANY here as a workaround.
    // This may discard little pseudo probe descriptor sections, reducing
    // profile quality.
    ...
    // Try to associate with its function.
    MCSymbol *FuncSym = Ctx->lookupSymbol(FuncName);
    if (FuncSym &&
        FuncSym == static_cast<MCSectionCOFF &>(FuncSym->getSection())
                       .getCOMDATSymbol()) {
      COMDATSymName = FuncSym->getName();
      Selection = COFF::IMAGE_COMDAT_SELECT_ASSOCIATIVE;
    } else
      COMDATSymName = (S->getName() + "_" + FuncName).toStringRef(Storage);
