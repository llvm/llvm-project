...
bool InstCombinerImpl::SimplifyDemandedBits(Instruction *I, unsigned OpNo,
  ...
  if (auto *BC = dyn_cast<BitCastInst>(VInst)) {
    ...
    if (Src->getType()->getScalarSizeInBits() ==
        BC->getType()->getScalarSizeInBits()) {
      llvm::computeKnownBits(Src, Known, Q, Depth + 1);
      return false;
    }
  }
    ...
    if (IntrinsicInst *II = dyn_cast<IntrinsicInst>(I)) {
      ...
      case Intrinsic::ctpop: {
        ...
        if (DemandedMask == 1 && VTy->getScalarSizeInBits() % 2 == 0 &&
          ...
          return InsertNewInstWith(CallInst::Create(Ctpop, {X}),
                                   I->getIterator());
        }
        break;
      }
