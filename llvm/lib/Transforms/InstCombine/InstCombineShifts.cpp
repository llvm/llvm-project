  ...
  if (match(Op1, m_APInt(C))) {
    ...
    if (match(Op0, m_NUWMul(m_Value(X), m_APInt(MulC)))) {
      if (BitWidth > 2 && (*MulC - 1).isPowerOf2() &&
          MulC->logBase2() == ShAmtC && ShAmtC * 2 == BitWidth)
        // lshr i[2N] (mul nuw X, (2^N)+1), N --> X
        return replaceInstUsesWith(I, X);

      if (Op0->hasOneUse() && BitWidth > 2 && MulC->logBase2() == ShAmtC &&
          ShAmtC < BitWidth - 1) {
        ...
        if (M_API.isPowerOf2()) {
          ...
          // lshr (mul nuw (X, 2^N + 2^M)), N -> add nuw (X, lshr(X, N - M))
  ...
  if (match(Op1, m_APInt(ShAmtAPInt)) && ShAmtAPInt->ult(BitWidth)) {
    ...
    if (match(Op0, m_OneUse(m_NSWMul(m_Value(X), m_APInt(MulC)))) &&
      ...
      if (M_API.isPowerOf2()) {
        ...
        Value *ShiftedX = Builder.CreateAShr(X, ConstantInt::get(Ty, ShAmt - M),
                                             "", I.isExact());
