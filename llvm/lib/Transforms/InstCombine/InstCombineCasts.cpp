...
foldBitcastShuffleAShrToWideAShr(BitCastInst &BC,
  ...
  for (unsigned i = 0; i < N; ++i) {
    ...
    auto *ShiftLo =
        dyn_cast_or_null<ConstantInt>(ShiftVec->getAggregateElement(Lo));
    auto *ShiftHi =
        dyn_cast_or_null<ConstantInt>(ShiftVec->getAggregateElement(Hi));
  ...
  Constant *SplatAmt = ConstantVector::getSplat(ElementCount::getFixed(N),
