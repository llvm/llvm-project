...
// Returns a list of <MemoryLocation, bool> pairs written by I. The bool means
// whether the write is from Initializes attr.
SmallVector<std::pair<MemoryLocation, bool>, 1>
...
/// Assuming this instruction has a dead analyzable write, can we delete this
/// instruction?
...
/// Returns true if \p UseInst completely overwrites \p DefLoc (stored by
/// \p DefInst).
...
/// If we have a zero initializing memset following a call to malloc, try
/// folding it into a call to calloc.
...
/// \returns true if \p Def is a no-op store, either because it directly stores
/// back a loaded value or stores zero to a calloced object.
