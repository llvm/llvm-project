...
bool LoopVectorizationPlanner::isMoreProfitable(const VectorizationFactor &A,
  ...
  LLVM_DEBUG(
      dbgs() << "LV: Comparing VF " << A.Width << " with cost " << CostA
             << ", scalar cost " << A.ScalarCost << ", and estimated width "
             << EstimatedWidthA << " against VF " << B.Width << " with cost "
             << CostB << ", scalar cost " << B.ScalarCost
             << ", and estimated width " << EstimatedWidthB
             << " for maximum trip count " << MaxTripCount << " with tail "
             << HasTail << ": normalized costs are " << CostA * EstimatedWidthB
             << " and " << CostB * EstimatedWidthA
