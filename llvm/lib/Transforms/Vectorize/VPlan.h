...
class LLVM_ABI_FOR_TEST VPRegionBlock : public VPBlockBase {
  ...
  VPRegionBlock(VPBlockBase *Entry, VPBlockBase *Exiting, VPCanonicalIV *CanIV,
                const std::string &Name = "")
      : VPBlockBase(VPRegionBlockSC, Name), Entry(Entry), Exiting(Exiting),
        IsReplicator(false), CanIV(CanIV) {
    assert(Entry->getPredecessors().empty() && "Entry block has predecessors.");
    assert(Exiting->getSuccessors().empty() && "Exit block has successors.");
    Entry->setParent(this);
    Exiting->setParent(this);
  ...
  VPRegionBlock(VPValue *StartV, const std::string &Name = "",
                bool IsReplicator = false)
      : VPBlockBase(VPRegionBlockSC, Name), Entry(nullptr), Exiting(nullptr),
        IsReplicator(IsReplicator) {
    if (StartV)
      CanIV = new VPCanonicalIV(StartV);
  }
...
public:
  ...
  /// VPlan is destroyed.
  VPRegionBlock *createVPRegionBlock(VPValue *StartV,
                                     const std::string &Name = "") {
    auto *VPB = new VPRegionBlock(StartV, Name, false);
    CreatedBlocks.push_back(VPB);
    return VPB;
  }
