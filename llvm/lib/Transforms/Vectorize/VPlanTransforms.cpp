...
matchExtendedReductionOperand(VPWidenRecipe *UpdateR, VPValue *Op) {
  ...
  if (match(Op, m_IsA<VPWidenCastRecipe>(m_AnyExtend(m_VPValue())))) {
  ...
  if (!match(LHS, m_IsA<VPWidenCastRecipe>(m_AnyExtend(m_VPValue()))))
    return std::nullopt;
  ...
  if (match(RHS, m_IsA<VPWidenCastRecipe>(m_AnyExtend(m_VPValue()))))
