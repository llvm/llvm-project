...
VPlanPtr LoopVectorizationPlanner::tryToBuildVPlan1() {
  ...
  if (PreferControlFlow || TTI.preferControlFlowVectorization())
    RUN_VPLAN_PASS_NO_VERIFY(VPlanTransforms::optimizeConditionalVPBB, *VPlan0);
...
VPlanPtr LoopVectorizationPlanner::tryToBuildVPlan(VPlanPtr Plan,
  ...
  for (VPBasicBlock *VPBB : VPBlockUtils::blocksOnly<VPBasicBlock>(RPOT)) {
    ...
    for (VPRecipeBase &R : make_early_inc_range(
      ...
      // transformed. Also skip the `BranchOnCond` generated by control flow
      // vectorization.
      if ((&R == Term && match(&R, m_BranchOnCond(m_VPValue()))) ||
