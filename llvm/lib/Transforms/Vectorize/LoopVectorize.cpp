...
LoopVectorizationPlanner::cost(VPlan &Plan, ElementCount VF,
                               bool TailFoldedEpilogueCM) const {
  LoopVectorizationCostModel *ChosenCM =
      TailFoldedEpilogueCM ? EpilogueTailFoldedCM : &CM;
  VPCostContext CostCtx(ChosenCM->TTI, *(ChosenCM->TLI), Plan, *ChosenCM,
                        ChosenCM->CostKind, PSE, OrigLoop);
  ...
  if (EpilogueTailFoldedCM) {
    ...
    for (auto &P : EpilogueTailFoldedPlans) {
      ...
      for (unsigned I = 0; I < VFs.size(); I++) {
        ...
        InstructionCost Cost = cost(*P, VF, /*TailFoldedEpilogueCM*/ true);
        VectorizationFactor CurrentFactor(VF, Cost, ScalarCost);
