...
// When the loop being vectorized is an inner loop whose runtime memory checks
// are not loop-invariant w.r.t. an outer loop (i.e., the checks execute on
// every outer iteration), use a tighter threshold. This prevents the
// combinatorial explosion of pointer-pair checks that arises when inlining
// exposes multiple memory streams to the vectorizer's outer-loop interleaving.
...
public:
  ...
  void create(Loop *L, const LoopAccessInfo &LAI,
    ...
    // For inner loops, apply a tighter threshold. When an inner loop requires
    // many runtime checks (e.g., due to outer-loop interleaving creating a
    // combinatorial explosion of pointer-pair comparisons), the checks execute
    // on every outer-loop iteration, causing severe overhead from both the
    // check instructions themselves and the resulting register pressure.
    if (L->getParentLoop() && NumChecks > VectorizeMemoryCheckInnerLoopThreshold)
