...
Error RawMemProfReader::symbolizeAndFilterStackFrames() {
  ...
  for (auto &Entry : StackMap) {
    for (const uint64_t VAddr : Entry.getSecond()) {
      ...
      if (DI.getFrame(0).LinkageFunctionName == DILineInfo::BadString ||
      ...
      for (size_t I = 0, NumFrames = DI.getNumberOfFrames(); I < NumFrames;
        ...
        const uint64_t Guid =
            IndexedMemProfRecord::getGUID(DIFrame.LinkageFunctionName);
        ...
        // callsite frames.
        if (KeepSymbolName)
          GuidToSymbolName.insert({Guid, DIFrame.LinkageFunctionName});
