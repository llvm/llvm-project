...
// which is why the release and the revision fields do not meet. Each mask is
// also the largest value its field can hold, which is what bounds a field
// written out as part of a numeric architecture name.
static constexpr uint32_t GMDIDArchitectureShift = 22;
static constexpr uint32_t GMDIDArchitectureMask = 0x3ff;
...
static bool parseNumericArchName(StringRef Name, GMDID &ID) {
  ...
  // A field too wide for the GMDID it spells describes no GPU that could ever
  // report it, so this is not a numeric name rather than one naming no device.
  return ID.Architecture <= GMDIDArchitectureMask &&
         ID.Release <= GMDIDReleaseMask && ID.Revision <= GMDIDRevisionMask;
}
