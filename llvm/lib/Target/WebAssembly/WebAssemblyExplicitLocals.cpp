    ...
    for (MachineInstr &MI : llvm::make_early_inc_range(MBB)) {
      ...
      for (auto &Def : MI.defs()) {
        ...
        if (!MFI.isVRegStackified(OldReg)) {
          ...
          // When libcalls are emitted for thread context, the frame base vreg has
          // an implicit use in the DW_AT_frame_base debug info, so we should not
          // remove it.
          bool NeedsRegForDebug =
              MFI.isFrameBaseVirtual() &&
              OldReg == MFI.getFrameBaseVreg() &&
