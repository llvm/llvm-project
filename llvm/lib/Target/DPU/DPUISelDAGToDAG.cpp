...
void DPUDAGToDAGISel::Select(SDNode *Node) {
  ...
  LLVM_DEBUG({
    dbgs() << "DPU/DAGsel - selecting: OPCODE="
           << Node->getOperationName(CurDAG) << " ";
    Node->dump(CurDAG);
    dbgs() << "==========\n";
    Node->dumprFull(CurDAG);
    dbgs() << '\n';
  });

  {
    LLVM_DEBUG(dbgs() << "DPU/DAGsel - instruction opcode = #" << Opcode
                      << "\n");
    int NOp = Node->getNumOperands();
    LLVM_DEBUG(dbgs() << "DPU/DAGSel - instruction has " << NOp
                      << " operands\n");
    for (int OpNum = 0; OpNum < NOp; OpNum++) {
      LLVM_DEBUG({
        SDValue OpValue = Node->getOperand(OpNum);
        dbgs() << "DPU/DAGSel\toperand " << OpNum << " = ";
        OpValue->dump(CurDAG);
      });
    }
  }
  ...
  switch (Opcode) {
  case ISD::UNDEF: {
    LLVM_DEBUG(dbgs() << "DPU/DAGsel - Oups! should soon be an undef!!!\n");
    // Will be solved soon by VerifyThatSelectionMakesSense...
  } break;
  case ISD::FrameIndex: {
    ...
    LLVM_DEBUG({
      dbgs() << "DPU/DAGsel - frame index - replacing with R22\n";
      dbgs() << "\tFI=" << std::to_string(FI) << "\n";
      dbgs() << "\tTFI=";
      TFI.dump(CurDAG);
      dbgs() << "\n";
    });
    // Each frame index has its own offset from the stack pointer. Add this
    // offset to STKP to obtain the object address. Right now, we cannot get the
    // frame offset, it's not defined. But we can create an add instruction,
    // hoping that DPURegisterInfo::eliminateFrameIndex will do the
    // substitution...
  ...
  case ISD::STORE: {
    LLVM_DEBUG({
      ...
      std::string memoryVT =
          (SN->getMemoryVT().isSimple())
              ? std::to_string(SN->getMemoryVT().getSimpleVT().SimpleTy)
              : "<!simple>";
      std::string sourceVT =
          (SN->getOperand(1).getValueType().isSimple())
              ? std::to_string(
                    SN->getOperand(1).getValueType().getSimpleVT().SimpleTy)
              : "<!simple>";
      dbgs() << "DPU/DAGsel - store node\n"
             << "\tis truncating = " << std::to_string(SN->isTruncatingStore())
             << "\n"
             << "\tmemory VT     = " << memoryVT << "\n"
             << "\tinto mram     = "
             << std::to_string(IsAStoreToAddrSpace(
                    Node, (unsigned int)DPUADDR_SPACE::MRAM))
             << "\n"
             << "\tsource type   = " << sourceVT << "\n";
    });
  } break;
  ...
  default:
    // Nothing to do...
    LLVM_DEBUG(dbgs() << "regular opcode, with no DPU specific processing '"
                      << Opcode << " (" << Node->getOperationName(CurDAG) << ")"
                      << "'\n");
  }

  // Select the default instruction
  SelectCode(Node);
  assert(VerifyThatSelectionMakesSense(Node) && "DPU DAG selection failed!");
  // TODO: see how we can re-inject this:
  /*
  LLVM_DEBUG({
                 dbgs() << "DPU/DAGsel - morph result => ";
  ...
  assert(VerifyThatSelectionMakesSense(ResNode) && "DPU DAG selection failed!");
   */
}

// TODO: Keep or drop, but do something!
bool DPUDAGToDAGISel::VerifyThatSelectionMakesSense(SDNode *result) {
  // Consider that the result node is inconsistent if this node refers to itself
  // in its operands... This is typically the case of something unknown,
  // accidentally reduced to a MOVErr operation.
  if (result == nullptr)
    return true;

  for (unsigned int i = 0; i < result->getNumOperands(); i++) {
    SDValue operandValue = result->getOperand(i);

    SDNode *operandNode = operandValue.getNode();
    if (operandNode == result) {
      LLVM_DEBUG(dbgs() << "DPU/DAGsel -------------------- BUG BUG BUG "
                           "--------------------\n");
      LLVM_DEBUG(dbgs() << "NODE SELECTION FAILURE\n");
      LLVM_DEBUG({
        dbgs() << "Selected node ";
        result->dump(CurDAG);
      });
      LLVM_DEBUG({
        dbgs() << "Operand number " << std::to_string(i) << " equals ";
        operandNode->dump(CurDAG);
      });
      LLVM_DEBUG(
          dbgs() << "In other words, the instruction refers to itself!\n");
      return false;
    }
  }
  return true;
