  ...
  if (!ShiftLeft && ShiftAmt >= 8) {
    if (ArithmeticShift) {
      ...
      BuildMI(*BB, MI, dl, TII.get(AVR::ADDRdRr), Tmp)
          .addSubReg(Regs[0].first, Regs[0].second)
          .addSubReg(Regs[0].first, Regs[0].second);
  ...
  if (!ArithmeticShift && ShiftAmt >= 4) {
    ...
    for (size_t I = 0; I < Regs.size(); I++) {
      ...
      BuildMI(*BB, MI, dl, TII.get(AVR::SWAPRd), SwapReg)
          .addSubReg(Regs[Idx].first, Regs[Idx].second);
      if (I != 0) {
  ...
  while (!ShiftLeft && ShiftAmt) {
    ...
    for (size_t I = 0; I < Regs.size(); I++) {
      ...
      if (I == 0) {
        ...
        BuildMI(*BB, MI, dl, TII.get(Opc), Out).addSubReg(In, InSubreg);
      } else {
        BuildMI(*BB, MI, dl, TII.get(AVR::RORRd), Out).addSubReg(In, InSubreg);
      }
      Regs[I] = std::pair(Out, 0);
    }
    ShiftAmt--;
...
AVRTargetLowering::insertWideShift(MachineInstr &MI,
  ...
  if (Opc != ISD::SHL &&
    ...
    BuildMI(*BB, MI, dl, TII.get(AVR::REG_SEQUENCE), MI.getOperand(0).getReg())
        .addSubReg(Registers[3].first, Registers[3].second)
        .addImm(AVR::sub_lo)
        .addSubReg(Registers[2].first, Registers[2].second)
        .addImm(AVR::sub_hi);
    BuildMI(*BB, MI, dl, TII.get(AVR::REG_SEQUENCE), MI.getOperand(1).getReg())
        .addSubReg(Registers[1].first, Registers[1].second)
        .addImm(AVR::sub_lo)
        .addSubReg(Registers[0].first, Registers[0].second)
        .addImm(AVR::sub_hi);
  } else {
