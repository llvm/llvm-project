...
// basic block boundaries and by SCHED_BARRIER (llvm.amdgcn.sched.barrier).
//
// V_PK * F32 unpacking lowers each packed op to two scalar VALUs that define the
// destination register by subregs (e.g. vreg_64: undef %dst.sub0 = …; %dst.sub1 = …),
// matching how wide accumulators are updated in real kernels (full wide def, then
// per-lane subreg writes — see incoming FMHA MIR). A single V_PK full-reg def leaves
// IsSSA set on the function; multiple subreg defs require clearing IsSSA. Subreg
// indices follow the V_PK dest (sub0/sub1 for vreg_64; composite 64-bit lanes
// e.g. sub0_sub1 … sub6_sub7 on wide vectors via composeSubRegIndices). Unpacked
// sources use the same compose pattern as SIPreEmitPeephole (operand packed subreg
// composed with sub0/sub1 for the scalar lane). Undef on the first unpacked def
// only when the destination vreg has no prior def in the block and the destination
// is not also a packed source (otherwise lanes already hold defined values).
// Emit low lane, then high lane (first BuildMI before V_PK is earliest in the
// block).
//
...
//
// AMDGPUIGLPOptMaskExperimental is 4 (see AMDGPUIGLPUnpack.h). AMDGPUIGroupLP
// createIGLPStrategy() only supports IGLPStrategyID 0–3; mask 4 must not reach
// the scheduler unless that code is extended—keep kRemoveExperimentalIGLPOptPseudo
// true, or strip by other means, while experimenting.
//
// kRequireExperimentalIGLPOptForRegion
//   When true (current default): only schedule sub-regions that contain an
//   IGLP_OPT pseudo with immediate == AMDGPUIGLPOptMaskExperimental are
//   considered for V_PK unpack.
//   When false: any sub-region with at least one V_PK is a candidate; IGLP_OPT
//   is not required.
//
// kRemoveExperimentalIGLPOptPseudo
//   When true (current default): in every schedule sub-region, remove IGLP_OPT
//   pseudos whose immediate is AMDGPUIGLPOptMaskExperimental so they do not
//   reach the scheduler — including regions with no V_PK *machine* op (e.g.
//   packed math only appears as INLINEASM). When false: leave those pseudos.
//
// Flip these when you need the experimental IGLP_OPT hints to persist.
//
static constexpr bool kRequireExperimentalIGLPOptForRegion = true;
static constexpr bool kRemoveExperimentalIGLPOptPseudo = true;
// ============================================================================

namespace {

/// A schedule sub-region (between SCHED_BARRIERs) that has at least one V_PK,
/// and when kRequireExperimentalIGLPOptForRegion also an IGLP_OPT with mask
/// AMDGPUIGLPOptMaskExperimental, for unpack / cleanup.
...
/// Log why a single V_PK was not unpacked. Enable with -debug-only=amdgpu-iglp-unpack.
static void debugSkipVPKUnpack(const MachineInstr &MI, const SIInstrInfo *TII,
                               const TargetRegisterInfo *TRI, const char *Reason,
                               unsigned LoSeqIdx = 0, unsigned HiSeqIdx = 0,
                               bool LoRCValid = true, bool HiRCValid = true) {
  LLVM_DEBUG({
    dbgs() << DEBUG_TYPE << ": skip unpack (" << Reason << ")";
    if (MI.getNumOperands() > 0 && MI.getOperand(0).isReg()) {
      dbgs() << " op=" << TII->getName(MI.getOpcode())
             << " dst=" << printReg(MI.getOperand(0).getReg(), TRI,
...
static bool
findCandidateRegion(MachineBasicBlock &MBB,
                    MachineBasicBlock::iterator RegionBegin,
                    MachineBasicBlock::iterator RegionEnd,
                    const MCInstrInfo &II, CandidateRegion &Out) {
  Out = CandidateRegion{};
  bool HasExperimentalIGLP = false;
  ...
  for (MachineBasicBlock::iterator It = RegionBegin; It != RegionEnd; ++It) {
    ...
    if (MI.getOpcode() == AMDGPU::IGLP_OPT && MI.getNumOperands() >= 1 &&
        ...
        MI.getOperand(0).getImm() ==
            static_cast<int64_t>(AMDGPUIGLPOptMaskExperimental))
      HasExperimentalIGLP = true;
  ...
  if (Out.VPKInsts.empty())
    return false;
  if (kRequireExperimentalIGLPOptForRegion && !HasExperimentalIGLP)
    return false;
...
/// Erase IGLP_OPT (mask == AMDGPUIGLPOptMaskExperimental) in [Begin, End).
static bool stripExperimentalIGLPOptInRange(MachineBasicBlock::iterator Begin,
                                            MachineBasicBlock::iterator End,
                                            LiveIntervals &LIS) {
...
static bool schedRegionHasExplicitAllocatablePhysReg(
...
/// True if any packed source uses the same vreg as the destination (dst-as-src).
...
static bool tryFoldThroughSameBlockCopyDef(const MachineInstr &VPK,
  ...
  for (MachineBasicBlock::const_iterator It = MBB->begin(), E = VPK.getIterator();
       It != E; ++It) {
...
static void addOperandAndMods(MachineInstrBuilder &NewMI, unsigned SrcMods,
  ...
  if (OrigSrcReg.isVirtual() && OrigSrcReg != DstReg) {
    ...
    if (tryFoldThroughSameBlockCopyDef(VPK, OrigSrcReg, FinalSubIdx, FoldReg,
      ...
      if (Sz <= 32)
        FinalSubIdx = FoldSub;
      else if (unsigned Composed =
                   TRI->composeSubRegIndices(FoldSub, PairLane))
...
static void recomputeIntervalsAfterVirtualUnpack(
    ArrayRef<MachineInstr *> MIs, LiveIntervals &LIS,
...
static void eraseRedundantCopyDefsForRegIfUnused(Register R, MachineBasicBlock &MBB,
                                                 MachineRegisterInfo &MRI,
                                                 LiveIntervals &LIS,
                                                 SmallVectorImpl<Register> &RegsToRecompute) {
...
static bool instrIsBeforeInSameBB(const MachineInstr *A, const MachineInstr *B) {
...
static LaneBitmask laneMaskForRegOperand(const MachineOperand &MO, Register R,
  ...
  // equal a single 32-bit lane for 64-bit vreg_64 classes; OR in the paired lane
  // so full-reg reads overlap sub1 when querying sub1's mask.
...
static bool instrBlocksSinkOfSameVRegCopy(const MachineInstr &Blocker,
                                        const MachineInstr &CopyMI, Register R,
                                        LaneBitmask SMask, LaneBitmask DMask,
                                        const MachineRegisterInfo &MRI,
                                        const SIRegisterInfo &TRI) {
...
/// that has an operand on \p R touching a lane overlapping \p DMask.
static MachineInstr *findFirstDependentUseAfterSameVRegCopy(
  ...
  for (auto It = std::next(CopyMI.getIterator()); It != MBB->end(); ++It) {
    MachineInstr &MI = *It;
    for (MachineOperand &MO : MI.operands()) {
...
/// Sink COPY %R:DstSub = %R:SrcSub toward its first dependent use in the MBB.
static bool trySinkSameVRegSubregCopy(MachineInstr &CopyMI,
  ...
  MachineInstr *FirstUse = findFirstDependentUseAfterSameVRegCopy(
      CopyMI, R, DMask, MRI, TRI);
  if (!FirstUse)
    return false;

  MachineBasicBlock::iterator CopyIt = CopyMI.getIterator();
  MachineBasicBlock::iterator FirstUseIt = FirstUse->getIterator();
  MachineBasicBlock::iterator InsertPt = FirstUseIt;
...
static bool cleanupSameVRegSubregCopiesInRange(MachineBasicBlock::iterator Begin,
                                               MachineBasicBlock::iterator End,
                                               MachineRegisterInfo &MRI,
                                               LiveIntervals &LIS,
                                               const SIRegisterInfo &TRI) {
  ...
  for (MachineInstr *MI : Copies) {
    if (trySinkSameVRegSubregCopy(*MI, MRI, LIS, TRI))
      Changed = true;
  }
  return Changed;
}
...
static bool performF32Unpacking(MachineInstr &I, const SIInstrInfo *TII,
  ...
  const bool UndefOnFirstLane =
      !vpkAnySrcUsesDst(I, TII, DstReg) &&
  ...
  // high lane.
  MachineInstrBuilder Op0L =
      createUnpackedMI(I, TII, TRI, UnpackedOpcode,
                       /*IsHiBits=*/false, DstReg, LoSeqIdx, LaneSrcBase, DstSub,
...
bool AMDGPUIGLPUnpackImpl::run(MachineFunction &MF) {
  ...
  for (MachineBasicBlock &MBB : MF) {
    splitMBBBySchedBarriers(MBB, SubRegions);
        ...
        LLVM_DEBUG({
          ...
          for (auto It = Beg; It != End; ++It) {
            if (isVPKOpcode(II, It->getOpcode()))
              ++NumVPK;
            if (It->getOpcode() == AMDGPU::IGLP_OPT && It->getNumOperands() >= 1 &&
                It->getOperand(0).isImm() &&
                It->getOperand(0).getImm() ==
                    static_cast<int64_t>(AMDGPUIGLPOptMaskExperimental))
              HasIGLP4 = true;
          }
          if (NumVPK > 0 && kRequireExperimentalIGLPOptForRegion && !HasIGLP4)
            dbgs() << DEBUG_TYPE << ": skip region MBB#" << MBB.getNumber()
                   << " (" << NumVPK << " V_PK, missing IGLP_OPT imm="
                   << AMDGPUIGLPOptMaskExperimental << ")\n";
        });
        if (kRemoveExperimentalIGLPOptPseudo) {
          if (stripExperimentalIGLPOptInRange(Beg, End, *LIS))
            Changed = true;
        }
        continue;
      }
...
PreservedAnalyses
AMDGPUIGLPUnpackPass::run(MachineFunction &MF, MachineFunctionAnalysisManager &MFAM) {
