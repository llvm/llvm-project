...
static void unmergeReadAnyLane(MachineIRBuilder &B,
  ...
  if (Ty.getScalarType().isFloat()) {
    VgprSrc = B.buildBitcast({VgprRB, Ty.dropType()}, VgprSrc).getReg(0);
  }
  auto Unmerge = B.buildUnmerge({VgprRB, UnmergeTy.dropType()}, VgprSrc);
...
static Register buildReadAnyLane(MachineIRBuilder &B, Register VgprSrc,
  ...
  if (Ty.getScalarType().isFloat()) {
    auto Merge = B.buildMergeLikeInstr({SgprRB, Ty.dropType()}, SgprDstParts);
