...
AMDGPUAsmPrinter::getAmdhsaKernelDescriptor(const MachineFunction &MF,
  ...
  assert(STM.getGeneration() >= AMDGPUSubtarget::GFX10 ||
         STM.hasGFX90AInsts() || AMDGPU::isGFX1250Plus(STM) ||
...
bool AMDGPUAsmPrinter::runOnMachineFunction(MachineFunction &MF) {
  ...
  if (MFI->isEntryFunction() && AMDGPU::getWavegroupEnable(MF.getFunction())) {
    ...
    for (auto MI : ResourceUsage->WavegroupRankCalls) {
...
void AMDGPUAsmPrinter::getSIProgramInfo(SIProgramInfo &ProgInfo,
  ...
  SmallVector<const MCExpr *> MaxExprArgs = {
      ...
      CreateExpr(STM.getMinNumVGPRs(
          MaxVGPRAllocs, MFI->getDynamicVGPRBlockSize()))};
  ...
  if (STM.getFeatureBits().test(FeatureAddressableLocalMemorySize327680) ||
      STM.getFeatureBits().test(FeatureAddressableLocalMemorySize196608)) {
