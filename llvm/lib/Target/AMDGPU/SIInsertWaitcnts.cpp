...
void WaitcntBrackets::updateByEvent(WaitEventType E, MachineInstr &Inst) {
  ...
  if (T == AMDGPU::EXP_CNT) {
    ...
    // counter types.
    if (Context->isAsyncLdsDmaWrite(Inst) && T == AMDGPU::LOAD_CNT) {
...
void WaitcntBrackets::applyWaitcnt(const AMDGPU::Waitcnt &Wait) {
  applyWaitcnt(Wait, AMDGPU::LOAD_CNT);
  applyWaitcnt(Wait, AMDGPU::EXP_CNT);
  applyWaitcnt(Wait, AMDGPU::DS_CNT);
  applyWaitcnt(Wait, AMDGPU::STORE_CNT);
  applyWaitcnt(Wait, AMDGPU::SAMPLE_CNT);
  applyWaitcnt(Wait, AMDGPU::BVH_CNT);
  applyWaitcnt(Wait, AMDGPU::KM_CNT);
  applyWaitcnt(Wait, AMDGPU::X_CNT);
  applyWaitcnt(Wait, AMDGPU::VA_VDST);
  applyWaitcnt(Wait, AMDGPU::VM_VSRC);
}
...
bool WaitcntGeneratorPreGFX12::applyPreexistingWaitcnt(
  ...
  for (auto &II :
    ...
    if (Opcode == AMDGPU::S_WAITCNT) {
      ...
      if (TrySimplify)
        ScoreBrackets.simplifyWaitcnt(AMDGPU::InstCounterType::STORE_CNT, OldVSCnt);
      Wait.set(
          AMDGPU::InstCounterType::STORE_CNT,
          std::min(Wait.get(AMDGPU::InstCounterType::STORE_CNT), OldVSCnt));
