...
//
// This saves one v_cndmask per pattern.
bool AMDGPUPostLegalizerCombinerImpl::matchConditionalSubToFMA(
  ...
  auto matchSelectWithZero = [this, &MI](MachineInstr *SelMI, Register &Cond,
    ...
    // On GFX10+ (hasVOP3Literal) the literal 0xbff00000 (-1.0 hi bits) folds
    // directly into v_cndmask_b32_e64 with no extra v_mov_b32, so the transform
    // is a net win of one instruction regardless of loop context.
    // Pre-GFX10: the transform is only beneficial if constants can be shared
    // with other instructions or used inside loops, so for simplicity perform
    // the transform in loops only.
    MachineLoop *MILoop = MLI->getLoopFor(MI.getParent());
    if (!MILoop && !STI.hasVOP3Literal())
      return false;
    ...
    if (MILoop != MLI->getLoopFor(SelMI->getParent()))
      return false;
  ...
  if (MI.getOpcode() == TargetOpcode::G_FSUB) {
    ...
    if (RHSMI->getOpcode() == TargetOpcode::G_FNEG &&
        MRI.hasOneNonDBGUse(RHSMI->getOperand(0).getReg())) {
