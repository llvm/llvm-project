...
// Add every "VGPR as memory" (addrspace(13)) global reachable from the
// vector-of-pointers operand V to Out (deduplicated via Seen).
// getUnderlyingObject only looks through *scalar* pointers, so a global nested
// inside a constant - e.g. a splat or ConstantVector used as a gather/scatter
// base - is invisible to it; walk into constant aggregates/expressions to find
// such references.
static void
collectNestedVGPRMemoryGlobals(Value *V,
                               SmallPtrSetImpl<GlobalVariable *> &Seen,
  ...
  while (!Work.empty()) {
    ...
    if (Cur->getType()->isPointerTy())
      Cur = getUnderlyingObject(Cur);
    if (auto *GV = dyn_cast<GlobalVariable>(Cur)) {
      if (GV->getAddressSpace() == AMDGPUAS::VGPR && Seen.insert(GV).second)
        Out.push_back(GV);
    } else if (auto *C = dyn_cast<Constant>(Cur)) {
  ...
  for (Function &F : M) {
    ...
    for (Instruction &I : instructions(F)) {
      ...
      for (Value *Op : I.operands()) {
        Type *T = Op->getType();
        if (T->isPointerTy()) {
          // Scalar pointer: getUnderlyingObject sees through
          // constant-expression GEPs/casts, so a global referenced via e.g.
          // getelementptr(@g, off) is found.
          ...
          // Vector of pointers (e.g. a gather/scatter base): the global may be
          // nested inside a constant that getUnderlyingObject can't see
          // through.
          collectNestedVGPRMemoryGlobals(Op, Seen, Uses[&F]);
        }
        // Any other operand type can't name a global; skipping it avoids a
