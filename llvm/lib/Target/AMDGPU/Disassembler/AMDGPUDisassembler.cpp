...
DecodeStatus AMDGPUDisassembler::getInstruction(MCInst &MI, uint64_t &Size,
  ...
  do {
    ...
    if (Bytes.size() >= 8) {
      ...
      Res = tryDecodeInst(DecoderTableDPP8GFX1264, MI, QW, Address);
      if (Res && convertDPP8Inst(MI) == MCDisassembler::Success)
        break;
      MI = MCInst(); // clear
      ...
      Res = tryDecodeInst(DecoderTableDPPGFX1264, MI, QW, Address);
      if (Res) {
        if (MCII->get(MI.getOpcode()).TSFlags & SIInstrFlags::VOPC)
          convertVOPCDPPInst(MI);
        break;
    ...
    Res = tryDecodeInst(DecoderTableGFX1232, MI, DW, Address);
    if (Res) break;
  ...
  if (Res && (MI.getOpcode() == AMDGPU::V_MAC_F32_e64_vi ||
              ...
              MI.getOpcode() == AMDGPU::V_FMAC_F32_e64_gfx12 ||
              ...
              MI.getOpcode() == AMDGPU::V_FMAC_F16_e64_gfx12)) {
...
MCOperand AMDGPUDisassembler::decodeOperand_VGPR_16_F128(unsigned Val) const {
  // Undo the autogenerated shift
  return decodeVGPR_16_F128(Val >> 1);
}
...
inline MCOperand AMDGPUDisassembler::decodeVGPR_16_F128(unsigned Val) const {
  assert(Val < 256);
  return createRegOperand(AMDGPU::VGPR_16_F128RegClassID,
                          ((Val & 127) << 1) | (Val >> 7));
}
