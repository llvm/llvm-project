...
///
/// A VOPD3 component cannot encode a literal, but its src0 slot can read a
/// scalar register, so such a pair is still possible once the value is moved
/// into one. A pair is considered only when all of its non-inline immediates
/// have the same 32-bit value. This keeps the transformation pair-local and
/// prevents it from increasing instruction count or extending register
/// pressure across pairs.
///
//
//===----------------------------------------------------------------------===//
