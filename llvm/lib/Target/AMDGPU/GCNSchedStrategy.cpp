...
void GCNSchedStrategy::initCandidate(SchedCandidate &Cand, SUnit *SU,
  ...
  // Only consider AGPRs when the target has allocatable
  // AGPRs (AGPRExcessLimit is set from getNumAllocatableRegs).
  bool ConsiderAGPR = AGPRExcessLimit > 0;
  bool ShouldTrackAGPRs =
      ConsiderAGPR && !ShouldTrackVGPRs && AGPRPressure >= AGPRExcessLimit;
  ...
  if (SGPRDelta >= 0 || VGPRDelta >= 0 || (ConsiderAGPR && AGPRDelta >= 0)) {
    HasHighPressure = true;
    if (SGPRDelta > VGPRDelta && (!ConsiderAGPR || SGPRDelta > AGPRDelta)) {
      Cand.RPDelta.CriticalMax =
          PressureChange(AMDGPU::RegisterPressureSets::SReg_32);
      Cand.RPDelta.CriticalMax.setUnitInc(SGPRDelta);
    } else if (ConsiderAGPR && AGPRDelta > VGPRDelta) {
      Cand.RPDelta.CriticalMax =
