...
// +----------+-----+-----+-------+---------+------------+---------+----------+
//
// Some cases are ambiguous, like the one marked with "yes?" above: fewer
// intrinsics and fewer vaddr and fewer total transfers between SP and TX, but
// higher vdata. We start by erring on the side of converting these to
// MSAA_LOAD.
//
...
bool optimizeSection(std::list<std::list<IntrinsicInst *>> &MergeableInsts) {
  ...
  for (auto IIList : MergeableInsts) {
    ...
    unsigned NumVDataLoads = 4 / (isD16 ? 2 : 1);
    unsigned NumVAddrMsaas = 3 * NumMsaas;
    unsigned NumVDataMsaas = 4 / (isD16 ? 2 : 1);
