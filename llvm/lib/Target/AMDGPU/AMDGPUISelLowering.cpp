...
static bool extractArgAndConstant(SDValue LHS, SDValue RHS,
                                  bool isFloatingPoint, SDValue &ArgVal,
                                  SDValue &ConstVal) {
  if (isFloatingPoint) {
    if (isa<ConstantFPSDNode>(RHS)) {
      ArgVal = LHS;
      ConstVal = RHS;
      return true;
    } else if (isa<ConstantFPSDNode>(LHS)) {
      ArgVal = RHS;
      ConstVal = LHS;
      return true;
    }
...
foldCmpSelectWithSharedConstant(SDNode *N, TargetLowering::DAGCombinerInfo &DCI,
  ...
  if (!extractArgAndConstant(LHS, RHS, isFloatingPoint, ArgVal, ConstVal))
    return SDValue();
