...
//
/// \file
/// This GlobalISel pass has two modes:
/// In structurized whole-wave control-flow mode, it selects divergent i1 phis
/// as lane mask phis. Lane mask merging uses same algorithm as SDAG in
/// SILowerI1Copies. Handles all cases of temporal divergence.
///
/// In unstructured per-lane control-flow mode, it converts divergent i1 phis
/// into vreg32 phis, which uses the same algorithm as in
/// AMDGPUFinalizeISelWaveTransform.cpp, which also handles all cases of
/// temporal divergence. It also generates different pseudo-op for uniform and
/// divergent branch, which will be used and lowered by wave-tranform.
///
