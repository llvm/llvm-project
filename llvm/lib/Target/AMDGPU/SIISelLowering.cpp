...
SDValue SITargetLowering::LowerLoadStoreVGPR(SDValue Op,
  ...
  // The pointer is a byte offset into the file, and it can have an arbitrary
  // shape: an addrspace(13) global (offset in metadata), the constant
  // LowerGlobalAddress folds it to, a runtime value, or any sum of those. We
  // never reject on the shape of the address - an unknown pointer must still
  // work. Instead we treat the pointer value itself as the (runtime) index and
  // only recognize a fully constant offset as an optimization, so the common
  // case can use the cheaper constant-index lowering below. This mirrors the
  // gfx13 lowering, whose index is simply the whole pointer.
  //
  // Walk the entire ADD/PTRADD tree so that every term is summed rather than
  // rejected: constant leaves accumulate into ByteOffset (in 64 bits, since the
  // addrspace(13) pointer is only 32-bit and a folded constant such as a
  // negative GEP index would otherwise wrap and defeat the out-of-range check
  // below), and all runtime leaves accumulate into a single DynByteOffset.
  uint64_t ByteOffset = 0;
  SDValue DynByteOffset; // summed non-constant byte offset, for a runtime index
  auto AddDynTerm = [&](SDValue Term) {
    Term = DAG.getZExtOrTrunc(Term, DL, MVT::i32);
    DynByteOffset = DynByteOffset
                        ? DAG.getNode(ISD::ADD, DL, MVT::i32, DynByteOffset,
                                      Term)
                        : Term;
  };
  SmallVector<SDValue, 4> Worklist{MemOp->getBasePtr()};
  while (!Worklist.empty()) {
    SDValue V = Worklist.pop_back_val();
    if (V.getOpcode() == ISD::ADD || V.getOpcode() == ISD::PTRADD) {
    ...
    } else if (auto *GA = dyn_cast<GlobalAddressSDNode>(V)) {
      ...
      // Any other value is a runtime byte offset.
      AddDynTerm(V);
    }
  }
