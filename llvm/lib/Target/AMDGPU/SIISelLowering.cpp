...
SDValue SITargetLowering::performMinMaxCombine(SDNode *N,
  ...
  if (supportsMin3Max3(*Subtarget, Opc, VT)) {
    ...
    bool TreeCombineApplies = Op0.getOpcode() == Opc && Op0.hasOneUse() &&
                              Op1.getOpcode() == Opc && Op1.hasOneUse();
    bool TreeCombineDeferred =
        TreeCombineApplies && (IsTreeWithCombinableChildren(Op0) ||
    ...
    if (TreeCombineApplies && !TreeCombineDeferred) {
      SDLoc DL(N);
      SDValue Inner =
    ...
    if (Op0.getOpcode() == Opc && Op0.hasOneUse() && !TreeCombineDeferred) {
      SDLoc DL(N);
    ...
    if (Op1.getOpcode() == Opc && Op1.hasOneUse() && !TreeCombineDeferred) {
      SDLoc DL(N);
      return DAG.getNode(minMaxOpcToMin3Max3Opc(Opc), DL, N->getValueType(0),
