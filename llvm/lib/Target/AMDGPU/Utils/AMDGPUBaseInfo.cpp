...
#if LLPC_BUILD_NPI
  unsigned AddressableNumVGPRs = getAddressableNumVGPRs(STI, DynamicVGPRBlockSize);
#else /* LLPC_BUILD_NPI */
...
#if LLPC_BUILD_NPI
  return ST.hasFeature(AMDGPU::FeatureAddressableLocalMemorySize327680) ||
                 ST.hasFeature(AMDGPU::FeatureAddressableLocalMemorySize196608)
             ? 256
             : 128;
}
