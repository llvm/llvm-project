  ...
  switch (Opc) {
    ...
    case Hexagon::PS_vselect: {
      ...
      if (Op0.getReg() != Op2.getReg()) {
        RegState S = Op0.getReg() != Op3.getReg() ? PState & ~RegState::Kill
                                                  : PState;
...
bool HexagonInstrInfo::PredicateInstruction(
  ...
  Register PredReg;
  unsigned PredRegPos;
  RegState PredRegFlags = RegState::NoFlags;
...
bool HexagonInstrInfo::getPredReg(ArrayRef<MachineOperand> Cond,
      Register &PredReg, unsigned &PredRegPos, RegState &PredRegFlags) const {
  if (Cond.empty())
    return false;
  assert(Cond.size() == 2);
