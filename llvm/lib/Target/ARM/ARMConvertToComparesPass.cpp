...
static void copyNonDefNonPredOperands(MachineInstr &Dst,
  ...
  unsigned End =
      (PIdx == -1) ? Src.getNumExplicitOperands() : static_cast<unsigned>(PIdx);
  ...
  // Forward any implicit operands (e.g. liveness uses added by earlier passes)
  // that live beyond the explicit + predicate operands.
  for (const MachineOperand &MO : Src.implicit_operands()) {
    // Skip implicit-def of CPSR; BuildMI already added one for the CMP.
