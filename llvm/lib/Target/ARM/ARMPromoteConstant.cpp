//==- ARMPromoteConstant.cpp - Promote constant to global for ARM
//----------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file implements the ARMPromoteConstant pass which promotes constants
...
//
// Ported from AArch64PromoteConstant for ARM NEON / vector aggregates.
//
//===----------------------------------------------------------------------===//
...
#define DEBUG_TYPE "arm-promote-const"

// Stress testing mode - disable heuristics.
static cl::opt<bool> Stress("arm-stress-promote-const", cl::Hidden,
...
/// See AArch64PromoteConstant for the motivating example.
class ARMPromoteConstant : public ModulePass {
public:
  ...
  ARMPromoteConstant() : ModulePass(ID) {}

  StringRef getPassName() const override { return "ARM Promote Constant"; }
  ...
  bool runOnModule(Module &M) override {
    ...
    for (auto &MF : M) {
      const ARMSubtarget &ST = TM.getSubtarget<ARMSubtarget>(MF);
      Changed |= runOnFunction(MF, ST, PromotionCache);
    }
    return Changed;
  }
...
private:
  ...
  bool runOnFunction(Function &F, const ARMSubtarget &ST,
                     PromotionCacheTy &PromotionCache);
...
char ARMPromoteConstant::ID = 0;

INITIALIZE_PASS_BEGIN(ARMPromoteConstant, "arm-promote-const",
                      "ARM Promote Constant Pass", false, false)
INITIALIZE_PASS_DEPENDENCY(TargetPassConfig)
INITIALIZE_PASS_DEPENDENCY(DominatorTreeWrapperPass)
INITIALIZE_PASS_END(ARMPromoteConstant, "arm-promote-const",
                    "ARM Promote Constant Pass", false, false)

ModulePass *llvm::createARMPromoteConstantPass() {
  return new ARMPromoteConstant();
}
...
shouldConvert(Constant &C,
              ARMPromoteConstant::PromotionCacheTy &PromotionCache) {
  auto Converted = PromotionCache.insert(
      std::make_pair(&C, ARMPromoteConstant::PromotedConstant()));
  if (Converted.second)
    Converted.first->second.ShouldConvert = shouldConvertImpl(&C);
  return Converted.first->second.ShouldConvert;
}

Instruction *ARMPromoteConstant::findInsertionPoint(Instruction &User,
                                                    unsigned OpNo) {
...
bool ARMPromoteConstant::isDominated(Instruction *NewPt, Instruction *User,
                                     unsigned OpNo,
                                     InsertionPoints &InsertPts) {
  DominatorTree &DT =
...
bool ARMPromoteConstant::tryAndMerge(Instruction *NewPt, Instruction *User,
                                     unsigned OpNo,
                                     InsertionPoints &InsertPts) {
  DominatorTree &DT =
      getAnalysis<DominatorTreeWrapperPass>(*NewPt->getParent()->getParent())
...
void ARMPromoteConstant::computeInsertionPoint(Instruction *User, unsigned OpNo,
                                               InsertionPoints &InsertPts) {
...
static void ensurePromotedGV(Function &F, Constant &C,
                             ARMPromoteConstant::PromotedConstant &PC) {
...
void ARMPromoteConstant::insertDefinitions(Function &F,
...
void ARMPromoteConstant::promoteConstants(
...
bool ARMPromoteConstant::runOnFunction(Function &F, const ARMSubtarget &ST,
                                       PromotionCacheTy &PromotionCache) {
  if (!Stress && !ST.hasNEON() && !ST.hasMVEIntegerOps() &&
      !ST.hasMVEFloatOps())
    return false;
