...
public:
  ...
  bool cpsrLivesOut(MachineBasicBlock *MBB);
...
// cmp)
bool ARMConditionOptimizer::canAdjustCmp(MachineInstr &CmpMI) {
...
bool ARMConditionOptimizer::registersMatch(MachineInstr *FirstMI,
                                           MachineInstr *SecondMI) {
  Register FirstReg = FirstMI->getOperand(0).getReg();
  Register SecondReg = SecondMI->getOperand(0).getReg();
...
// Check if CPSR lives out to any successor block.
bool ARMConditionOptimizer::cpsrLivesOut(MachineBasicBlock *MBB) {
  for (auto *SuccBB : MBB->successors()) {
    if (SuccBB->isLiveIn(ARM::CPSR)) {
      LLVM_DEBUG(dbgs() << "CPSR live into successor "
...
static bool isCSINCInstruction(unsigned Opc) { return Opc == ARM::t2CSINC; }
...
ARMConditionOptimizer::findSuitableCompare(MachineBasicBlock *MBB) {
  ...
  // Since we may modify cmp of this MBB, make sure CPSR does not live out.
  if (cpsrLivesOut(MBB))
    return nullptr;
...
ARMConditionOptimizer::adjustCmp(MachineInstr *CmpMI, ARMCC::CondCodes Cmp) {
  ...
  // Bail out on cmn 0 (CMN with immediate 0). It is a valid instruction but
  ...
  // If we have an invalid opcode after complement, return the old cmp
  if (Opc == ARM::INSTRUCTION_LIST_END)
...
bool ARMConditionOptimizer::optimizeIntraBlock(MachineBasicBlock &MBB) {
  ...
  // Since we may modify cmps in this MBB, make sure CPSR does not live out.
  if (cpsrLivesOut(&MBB))
    return false;
...
bool ARMConditionOptimizer::optimizeCrossBlock(MachineBasicBlock &HBB) {
  ...
  LLVM_DEBUG(dbgs() << "\tcondition: " << (int)HeadCmp << '\n');
  LLVM_DEBUG(dbgs() << "\timmediate: " << HeadImm << '\n');

  LLVM_DEBUG(dbgs() << "True branch:\n");
  LLVM_DEBUG(dbgs() << "\tcondition: " << (int)TrueCmp << '\n');
