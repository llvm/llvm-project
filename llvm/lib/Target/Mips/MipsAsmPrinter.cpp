...
void MipsAsmPrinter::emitStartOfAsmFile(Module &M) {
  ...
  if (TT.isOSBinFormatELF()) {
    ...
    // updateABIInfo() derives the FP ABI from STI via setFpAbiFromPredicates(),
    // which for a module with no functions is the default subtarget's. Reset
    // it to 'any' in that case, as explained above.
    if (!HaveFpABI)
    ...
    // (-mfpxx or -mfp64) and omit it otherwise. There is no directive for an
    // unset FP ABI, so omit it in that case too.
    if (HaveFpABI && ((ABI.IsO32() && (STI.isABI_FPXX() || STI.isFP64bit())) ||
                      STI.useSoftFloat()))
