  ...
  if (!FLI.CanLowerReturn) {
    ...
    for (unsigned i = 0; i < SplitEVTs.size(); ++i) {
      ...
      if (MRI.getType(CurVReg).getSizeInBits() == TypeSize::getFixed(1) &&
        ...
        CurVReg = MIRBuilder.buildZExt(LLT::buildInteger(8), CurVReg).getReg(0);
...
void AArch64CallLowering::saveVarArgRegisters(
  ...
  const LLT s64 = LLT::buildInteger(64);
  ...
  for (auto &Arg : F.args()) {
    ...
    if (OrigArg.Ty->isIntegerTy(1)) {
      ...
      if (!Flags.isZExt() && !Flags.isSExt()) {
        ...
        Register WideReg =
            MRI.createGenericVirtualRegister(LLT::buildInteger(8));
...
bool AArch64CallLowering::lowerCall(MachineIRBuilder &MIRBuilder,
  ...
  for (auto &OrigArg : Info.OrigArgs) {
    ...
    if (OrigArg.Ty->isIntegerTy(1) && !Flags.isSExt() && !Flags.isZExt()) {
      ...
      OutArg.Regs[0] =
          MIRBuilder.buildZExt(LLT::buildInteger(8), OutArg.Regs[0]).getReg(0);
