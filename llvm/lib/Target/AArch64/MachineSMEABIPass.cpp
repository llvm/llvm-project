...
struct MachineSMEABI : public MachineFunctionPass {
  ...
  SmallVector<ZAState> assignBundleZAStates(FunctionInfo const &FnInfo);
...
/// that have incoming or outgoing edges in that bundle.
SmallVector<ZAState>
MachineSMEABI::assignBundleZAStates(FunctionInfo const &FnInfo) {
...
bool MachineSMEABI::runOnMachineFunction(MachineFunction &MF) {
  ...
  SmallVector<ZAState> BundleStates = assignBundleZAStates(FnInfo);

  EmitContext Context;
  insertStateChanges(Context, FnInfo, BundleStates);
