...
static std::string mangleVectorParameters(ArrayRef<llvm::omp::ParamAttrTy> ParamAttrs) {
  SmallString<256> Buffer;
  llvm::raw_svector_ostream Out(Buffer);
  for (const auto &ParamAttr : ParamAttrs) {
    switch (ParamAttr.Kind) {
    case llvm::omp::Linear:
      Out << 'l';
      break;
    case llvm::omp::LinearRef:
      Out << 'R';
      break;
    case llvm::omp::LinearUVal:
      Out << 'U';
      break;
    case llvm::omp::LinearVal:
      Out << 'L';
      break;
    case llvm::omp::Uniform:
      Out << 'u';
      break;
    case llvm::omp::Vector:
      Out << 'v';
      break;
    }
    if (ParamAttr.HasVarStride)
      Out << "s" << ParamAttr.StrideOrArg;
    else if (ParamAttr.Kind == llvm::omp::Linear || ParamAttr.Kind == llvm::omp::LinearRef ||
             ParamAttr.Kind == llvm::omp::LinearUVal || ParamAttr.Kind == llvm::omp::LinearVal) {
...
static unsigned evaluateCDTSize(const llvm::Function *Fn,
  ...
  if (RetTy && !RetTy->isVoidTy()) {
    ...
    for (unsigned I = 0; I < NumParams; ++I) {
      bool IsVector =
          (I < ParamAttrs.size()) ? ParamAttrs[I].Kind == llvm::omp::ParamKindTy::Vector : true;
      if (!IsVector)
        continue;
...
void OpenMPIRBuilder::emitDeclareSimdFunction(llvm::Function *Fn,
                                              const llvm::APSInt &VLENVal,
                                              ArrayRef<llvm::omp::ParamAttrTy> ParamAttrs,
                                              DeclareSimdBranch Branch) {
