...
bool TargetLowering::SimplifyDemandedVectorElts(SDValue Op,
  ...
  bool Simplified =
      SimplifyDemandedVectorElts(Op, DemandedElts, DoNotPoisonEltMask,
                                 KnownUndef, KnownZero, TLO);
  if (Simplified) {
    DCI.AddToWorklist(Op.getNode());
    DCI.CommitTargetLoweringOpt(TLO);
  }
...
bool TargetLowering::SimplifyDemandedVectorElts(
  ...
  return SimplifyDemandedVectorElts(Op, OriginalDemandedElts, DoNotPoisonEltMask,
                                    KnownUndef, KnownZero, TLO, Depth, AssumeSingleUse);
}
...
bool TargetLowering::SimplifyDemandedVectorElts(
  ...
  auto SimplifyDemandedVectorEltsBinOp = [&](SDValue Op0, SDValue Op1) {
    SDValue NewOp0 = SimplifyMultipleUseDemandedVectorElts(Op0, DemandedEltsInclDoNotPoison,
                                                           TLO.DAG, Depth + 1);
    SDValue NewOp1 = SimplifyMultipleUseDemandedVectorElts(Op1, DemandedEltsInclDoNotPoison,
                                                           TLO.DAG, Depth + 1);
    if (NewOp0 || NewOp1) {
      SDValue NewOp =
  ...
  case ISD::BITCAST: {
    ...
    if ((NumElts % NumSrcElts) == 0) {
      ...
      SrcDoNotPoisonEltMask = APIntOps::ScaleBitMask(DoNotPoisonEltMask, NumSrcElts);
      ...
      if (IsLE) {
        ...
        for (unsigned i = 0; i != NumElts; ++i)
          if (DemandedEltsInclDoNotPoison[i]) {
  ...
  case ISD::FREEZE: {
    SDValue N0 = Op.getOperand(0);
    if (TLO.DAG.isGuaranteedNotToBeUndefOrPoison(N0, DemandedEltsInclDoNotPoison,
                                                 /*PoisonOnly=*/false))
  ...
  case ISD::CONCAT_VECTORS: {
    ...
    for (unsigned i = 0; i != NumSubVecs; ++i) {
      ...
      APInt DoNotPoisonSubElts = DoNotPoisonEltMask.extractBits(NumSubElts, i * NumSubElts);
      APInt SubUndef, SubZero;
      if (SimplifyDemandedVectorElts(SubOp, SubElts, DoNotPoisonSubElts,
                                     SubUndef, SubZero, TLO,
                                     Depth + 1))
        return true;
    ...
    if (!DemandedEltsInclDoNotPoison.isAllOnes()) {
      ...
      for (unsigned i = 0; i != NumSubVecs; ++i) {
        SDValue SubOp = Op.getOperand(i);
        APInt SubElts = DemandedEltsInclDoNotPoison.extractBits(NumSubElts, i * NumSubElts);
  ...
  case ISD::INSERT_SUBVECTOR: {
    ...
    DoNoPoisonSrcElts.insertBits(APInt::getZero(NumSubElts), Idx);
  ...
  case ISD::EXTRACT_SUBVECTOR: {
    ...
    unsigned NumSrcElts = Src.getValueType().getVectorNumElements();
    APInt DemandedSrcElts = DemandedElts.zext(NumSrcElts).shl(Idx);
    APInt DoNotPoisonDemandedSrcElts = DoNotPoisonEltMask.zext(NumSrcElts).shl(Idx);
    APInt DemandedSrcEltsInclDoNotPoison = DemandedEltsInclDoNotPoison.zext(NumSrcElts).shl(Idx);

    APInt SrcUndef, SrcZero;
    if (SimplifyDemandedVectorElts(Src, DemandedSrcElts, DoNotPoisonDemandedSrcElts, SrcUndef, SrcZero, TLO,
                                   Depth + 1))
      return true;
  ...
  case ISD::INSERT_VECTOR_ELT: {
    ...
    if (CIdx && CIdx->getAPIntValue().ult(NumElts)) {
      unsigned Idx = CIdx->getZExtValue();
      // FIXME: We could perhaps optimize this better. If the element isn't
      // demanded but most not be poisoned, then we could insert UNDEF instead
      // of the orignal value. Or if we know that the source element is
      // guaranteed not to be poison then we can drop the INSERT_VECTOR_ELT
      // anyway.
      ...
      if (SimplifyDemandedVectorElts(Vec, DemandedVecElts, SrcDoNotPoisonEltMask,
  ...
  case ISD::VSELECT: {
    ...
    if (SimplifyDemandedVectorElts(LHS, DemandedLHS, DoNotPoisonEltMask,
                                   UndefLHS, ZeroLHS, TLO,
                                   Depth + 1))
      return true;
    if (SimplifyDemandedVectorElts(RHS, DemandedRHS, DoNotPoisonEltMask,
                                   UndefRHS, ZeroRHS, TLO,
                                   Depth + 1))
      return true;
  ...
  case ISD::VECTOR_SHUFFLE: {
    ...
    APInt DoNotPoisonLHS(NumElts, 0);
    APInt DoNotPoisonRHS(NumElts, 0);
    for (unsigned i = 0; i != NumElts; ++i) {
      int M = ShuffleMask[i];
      if (M < 0 || !DemandedEltsInclDoNotPoison[i])
        continue;
      assert(0 <= M && M < (int)(2 * NumElts) && "Shuffle index out of range");
      if (!DemandedElts[i]) {
        if (M < (int)NumElts)
          DoNotPoisonLHS.setBit(M);
        else
          DoNotPoisonRHS.setBit(M - NumElts);
        continue;
      }
      if (M < (int)NumElts)
    ...
    APInt UndefLHS, ZeroLHS;
    APInt UndefRHS, ZeroRHS;
    if (SimplifyDemandedVectorElts(LHS, DemandedLHS, DoNotPoisonLHS, UndefLHS, ZeroLHS, TLO,
                                   Depth + 1))
      return true;
    if (SimplifyDemandedVectorElts(RHS, DemandedRHS, DoNotPoisonRHS, UndefRHS, ZeroRHS, TLO,
                                   Depth + 1))
      return true;
  ...
  case ISD::ADD: {
    ...
    if (Op0 == Op1 && Op->isOnlyUserOf(Op0.getNode())) {
      APInt UndefLHS, ZeroLHS;
      if (SimplifyDemandedVectorElts(Op0, DemandedElts, DoNotPoisonEltMask, UndefLHS, ZeroLHS, TLO,
  ...
  case ISD::FREM: {
    ...
    APInt UndefRHS, ZeroRHS;
    if (SimplifyDemandedVectorElts(Op1, DemandedElts, DoNotPoisonEltMask, UndefRHS, ZeroRHS, TLO,
                                   Depth + 1))
      return true;
    APInt UndefLHS, ZeroLHS;
    if (SimplifyDemandedVectorElts(Op0, DemandedElts, DoNotPoisonEltMask, UndefLHS, ZeroLHS, TLO,
  ...
  case ISD::AND: {
    ...
    APInt SrcUndef, SrcZero;
    if (SimplifyDemandedVectorElts(Op1, DemandedElts, DoNotPoisonEltMask, SrcUndef, SrcZero, TLO,
                                   Depth + 1))
      return true;
    ...
    if (SimplifyDemandedVectorElts(Op0, DemandedElts0, DoNotPoisonEltMask, KnownUndef, KnownZero,
                                   TLO, Depth + 1))
      return true;
  ...
  case ISD::SIGN_EXTEND:
  case ISD::ZERO_EXTEND:
    if (SimplifyDemandedVectorElts(Op.getOperand(0), DemandedElts, DoNotPoisonEltMask, KnownUndef,
                                   KnownZero, TLO, Depth + 1))
      return true;

    if (!DemandedEltsInclDoNotPoison.isAllOnes())
      if (SDValue NewOp = SimplifyMultipleUseDemandedVectorElts(
              Op.getOperand(0), DemandedEltsInclDoNotPoison, TLO.DAG, Depth + 1))
