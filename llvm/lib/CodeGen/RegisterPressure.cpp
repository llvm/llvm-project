...
void RegisterPressure::dump(const TargetRegisterInfo *TRI) const {
  ...
  dbgs() << "Live In: ";
  for (const VRegMaskOrUnitPair &P : LiveInRegs) {
  ...
  dbgs() << '\n';
  dbgs() << "Live Out: ";
  for (const VRegMaskOrUnitPair &P : LiveOutRegs) {
    dbgs() << printVRegOrUnit(P.RegUnit, TRI);
    if (!P.LaneMask.all())
      dbgs() << ':' << PrintLaneMask(P.LaneMask);
    dbgs() << ' ';
...
void RegPressureTracker::initLiveThru(const RegPressureTracker &RPTracker) {
  ...
  for (const VRegMaskOrUnitPair &Pair : P.LiveOutRegs) {
...
static LaneBitmask getRegLanes(ArrayRef<VRegMaskOrUnitPair> RegUnits,
                               Register RegUnit) {
  auto I = llvm::find_if(RegUnits, [RegUnit](const VRegMaskOrUnitPair Other) {
...
static void addRegLanes(SmallVectorImpl<VRegMaskOrUnitPair> &RegUnits,
                        VRegMaskOrUnitPair Pair) {
  Register RegUnit = Pair.RegUnit;
...
static void setRegZero(SmallVectorImpl<VRegMaskOrUnitPair> &RegUnits,
                       Register RegUnit) {
  auto I = llvm::find_if(RegUnits, [RegUnit](const VRegMaskOrUnitPair Other) {
    return Other.RegUnit == RegUnit;
  });
...
static void removeRegLanes(SmallVectorImpl<VRegMaskOrUnitPair> &RegUnits,
                           VRegMaskOrUnitPair Pair) {
  Register RegUnit = Pair.RegUnit;
  assert(Pair.LaneMask.any());
  auto I = llvm::find_if(RegUnits, [RegUnit](const VRegMaskOrUnitPair Other) {
...
class RegisterOperandsCollector {
  ...
  void collectInstr(const MachineInstr &MI) const {
    ...
    for (const VRegMaskOrUnitPair &P : RegOpers.Defs)
  ...
  void pushReg(Register Reg,
               SmallVectorImpl<VRegMaskOrUnitPair> &RegUnits) const {
    if (Reg.isVirtual()) {
      addRegLanes(RegUnits, VRegMaskOrUnitPair(Reg, LaneBitmask::getAll()));
    } else if (MRI.isAllocatable(Reg)) {
      for (MCRegUnit Unit : TRI.regunits(Reg.asMCReg()))
        addRegLanes(RegUnits, VRegMaskOrUnitPair(Unit, LaneBitmask::getAll()));
    }
  }
  ...
  void pushRegLanes(Register Reg, unsigned SubRegIdx,
    ...
    if (Reg.isVirtual()) {
      ...
      addRegLanes(RegUnits, VRegMaskOrUnitPair(Reg, LaneMask));
    } else if (MRI.isAllocatable(Reg)) {
      for (MCRegUnit Unit : TRI.regunits(Reg.asMCReg()))
        addRegLanes(RegUnits, VRegMaskOrUnitPair(Unit, LaneBitmask::getAll()));
    }
...
void RegisterOperands::adjustLaneLiveness(const LiveIntervals &LIS,
  ...
  if (AddFlagsMI != nullptr) {
    for (const VRegMaskOrUnitPair &P : DeadDefs) {
...
void PressureDiffs::addInstruction(unsigned Idx,
  ...
  for (const VRegMaskOrUnitPair &P : RegOpers.Uses)
...
void RegPressureTracker::addLiveRegs(ArrayRef<VRegMaskOrUnitPair> Regs) {
  for (const VRegMaskOrUnitPair &P : Regs) {
...
void RegPressureTracker::discoverLiveInOrOut(
    VRegMaskOrUnitPair Pair, SmallVectorImpl<VRegMaskOrUnitPair> &LiveInOrOut) {
  assert(Pair.LaneMask.any());

  Register RegUnit = Pair.RegUnit;
  auto I =
      llvm::find_if(LiveInOrOut, [RegUnit](const VRegMaskOrUnitPair &Other) {
        return Other.RegUnit == RegUnit;
      });
...
void RegPressureTracker::discoverLiveIn(VRegMaskOrUnitPair Pair) {
  discoverLiveInOrOut(Pair, P.LiveInRegs);
}

void RegPressureTracker::discoverLiveOut(VRegMaskOrUnitPair Pair) {
  discoverLiveInOrOut(Pair, P.LiveOutRegs);
}

void RegPressureTracker::bumpDeadDefs(ArrayRef<VRegMaskOrUnitPair> DeadDefs) {
  for (const VRegMaskOrUnitPair &P : DeadDefs) {
...
void RegPressureTracker::recede(const RegisterOperands &RegOpers,
                                SmallVectorImpl<VRegMaskOrUnitPair> *LiveUses) {
  ...
  for (const VRegMaskOrUnitPair &Def : RegOpers.Defs) {
    ...
    if (LiveOut.any()) {
      discoverLiveOut(VRegMaskOrUnitPair(Reg, LiveOut));
  ...
  for (const VRegMaskOrUnitPair &Use : RegOpers.Uses) {
    ...
    if (PreviousMask.none()) {
      if (LiveUses != nullptr) {
        if (!TrackLaneMasks) {
          addRegLanes(*LiveUses, VRegMaskOrUnitPair(Reg, NewMask));
        } else {
          auto I =
              llvm::find_if(*LiveUses, [Reg](const VRegMaskOrUnitPair Other) {
          ...
          if (IsRedef) {
            ...
            removeRegLanes(*LiveUses, VRegMaskOrUnitPair(Reg, NewMask));
          } else {
            addRegLanes(*LiveUses, VRegMaskOrUnitPair(Reg, NewMask));
          }
        }
...
void RegPressureTracker::recede(SmallVectorImpl<VRegMaskOrUnitPair> *LiveUses) {
...
void RegPressureTracker::advance(const RegisterOperands &RegOpers) {
  ...
  for (const VRegMaskOrUnitPair &Use : RegOpers.Uses) {
    ...
    if (LiveIn.any()) {
      discoverLiveIn(VRegMaskOrUnitPair(Reg, LiveIn));
      increaseRegPressure(Reg, LiveMask, LiveMask | LiveIn);
      LiveRegs.insert(VRegMaskOrUnitPair(Reg, LiveIn));
    }
    // Kill liveness at last uses.
    if (RequireIntervals) {
      LaneBitmask LastUseMask = getLastUsedLanes(Reg, SlotIdx);
      if (LastUseMask.any()) {
        LiveRegs.erase(VRegMaskOrUnitPair(Reg, LastUseMask));
