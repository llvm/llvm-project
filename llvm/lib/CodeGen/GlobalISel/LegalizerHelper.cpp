...
LegalizerHelper::bitcastExtractVectorElt(MachineInstr &MI, unsigned TypeIdx,
  ...
  if (NewNumElts < OldNumElts) {
    ...
    if (DstTy.isFloat()) {
      auto Trunc = MIRBuilder.buildTrunc(DstTy.dropType(), ExtractedBits);
...
LegalizerHelper::lowerFPTOINT_SAT(MachineInstr &MI) {
  ...
  if (AreExactFloatBounds) {
    ...
    auto MaxP = MIRBuilder.buildFCmp(
        CmpInst::FCMP_ULT, SrcTy.changeElementType(LLT::integer(1)), Src, MaxC);
    auto Max = MIRBuilder.buildSelect(SrcTy, MaxP, Src, MaxC);
    // Clamp by MaxFloat from above. NaN cannot occur.
    auto MinC = MIRBuilder.buildFConstant(SrcTy, MaxFloat);
    auto MinP = MIRBuilder.buildFCmp(CmpInst::FCMP_OGT,
                                     SrcTy.changeElementType(LLT::integer(1)),
...
LegalizerHelper::lowerUnmergeValues(MachineInstr &MI) {
  ...
  for (auto &Def : MI.all_defs()) {
    ...
    if (Ty.isFloat())
      bitcastDst(MI, Ty.dropType(), Def.getOperandNo());
  }

  if (SrcTy.isFloatVector())
    bitcastSrc(MI, SrcTy.dropType(), NumDst);
