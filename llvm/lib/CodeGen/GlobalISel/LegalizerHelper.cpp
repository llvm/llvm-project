...
LegalizerHelper::bitcastExtractSubvector(MachineInstr &MI, unsigned TypeIdx,
  ...
  Idx /= AdjustAmt;
  SrcTy = LLT::vector(SrcTyEC.divideCoefficientBy(AdjustAmt),
                      LLT::integer(AdjustAmt));
...
LegalizerHelper::bitcastInsertSubvector(MachineInstr &MI, unsigned TypeIdx,
  ...
  Idx /= AdjustAmt;
  BigVecTy = LLT::vector(BigVecTyEC.divideCoefficientBy(AdjustAmt),
                         LLT::integer(AdjustAmt));
  SubVecTy = LLT::vector(SubVecTyEC.divideCoefficientBy(AdjustAmt),
                         LLT::integer(AdjustAmt));
...
LegalizerHelper::lowerBitCount(MachineInstr &MI) {
  ...
  case TargetOpcode::G_CTLZ: {
    ...
    if (isSupported({TargetOpcode::G_CTLZ_ZERO_UNDEF, {DstTy, SrcTy}})) {
      ...
      auto ICmp = MIRBuilder.buildICmp(CmpInst::ICMP_EQ,
                                       SrcTy.changeElementType(LLT::integer(1)),
                                       SrcReg, ZeroSrc);
  ...
  case TargetOpcode::G_CTTZ: {
    ...
    if (isSupported({TargetOpcode::G_CTTZ_ZERO_UNDEF, {DstTy, SrcTy}})) {
      ...
      auto ICmp = MIRBuilder.buildICmp(CmpInst::ICMP_EQ,
                                       DstTy.changeElementType(LLT::integer(1)),
                                       SrcReg, Zero);
      auto LenConst = MIRBuilder.buildConstant(DstTy, Len);
...
LegalizerHelper::LegalizeResult LegalizerHelper::lowerFPTOSI(MachineInstr &MI) {
  ...
  if (!SrcTy.getScalarType().isFloat(32) ||
      !DstTy.getScalarType().isInteger(64))
    return UnableToLegalize;
  ...
  const LLT I1 = LLT::integer(1);
  auto CmpGt =
      MIRBuilder.buildICmp(CmpInst::ICMP_SGT, I1, Exponent, ExponentLoBit);
  ...
  auto ExponentLt0 =
      MIRBuilder.buildICmp(CmpInst::ICMP_SLT, I1, Exponent, ZeroSrcTy);
...
LegalizerHelper::lowerFPTOINT_SAT(MachineInstr &MI) {
  ...
  if (AreExactFloatBounds) {
    ...
    auto MaxP = MIRBuilder.buildFCmp(
        CmpInst::FCMP_OGT, SrcTy.changeElementType(LLT::integer(1)), Src, MaxC);
    auto Max = MIRBuilder.buildSelect(SrcTy, MaxP, Src, MaxC);
    // Clamp by MaxFloat from above. NaN cannot occur.
    auto MinC = MIRBuilder.buildFConstant(SrcTy, MaxFloat);
    auto MinP = MIRBuilder.buildFCmp(CmpInst::FCMP_OLT,
                                     SrcTy.changeElementType(LLT::integer(1)),
    ...
    auto IsZero = MIRBuilder.buildFCmp(
        CmpInst::FCMP_UNO, DstTy.changeElementType(LLT::integer(1)), Src, Src);
  ...
  auto ULT = MIRBuilder.buildFCmp(CmpInst::FCMP_ULT,
                                  SrcTy.changeElementType(LLT::integer(1)), Src,
  ...
  auto OGT = MIRBuilder.buildFCmp(CmpInst::FCMP_OGT,
                                  SrcTy.changeElementType(LLT::integer(1)), Src,
...
LegalizerHelper::lowerFPTRUNC_F64_TO_F16(MachineInstr &MI) {
  ...
  auto Unmerge =
      MIRBuilder.buildUnmerge(I32, MIRBuilder.buildBitcast(I64, Src));
  ...
  auto D0_NE_SigSetHigh =
      MIRBuilder.buildICmp(CmpInst::ICMP_NE, I1, D0, SigSetHigh);
  ...
  auto CmpEGt30 = MIRBuilder.buildICmp(CmpInst::ICMP_SGT, I1, E,
                                       MIRBuilder.buildConstant(I32, 30));
  V = MIRBuilder.buildSelect(I32, CmpEGt30,
                             MIRBuilder.buildConstant(I32, 0x7c00), V);

  auto CmpEGt1039 = MIRBuilder.buildICmp(CmpInst::ICMP_EQ, I1, E,
...
LegalizerHelper::lowerFCopySign(MachineInstr &MI) {
  ...
  LLT Src0IntTy = Src0Ty.changeToInteger();
  LLT Src1IntTy = Src1Ty.changeToInteger();
  LLT DstIntTy = DstTy.changeToInteger();
...
LegalizerHelper::lowerUnmergeValues(MachineInstr &MI) {
  ...
  for (auto &Def : MI.all_defs()) {
    LLT Ty = MRI.getType(Def.getReg());
    if (Ty.isFloat())
      bitcastDst(MI, Ty.changeToInteger(), Def.getOperandNo());
  }

  if (SrcTy.isFloatVector())
    bitcastSrc(MI, SrcTy.changeToInteger(), NumDst);

  SrcReg = MI.getOperand(NumDst).getReg();
  Dst0Reg = MI.getOperand(0).getReg();
  DstTy = MRI.getType(Dst0Reg);
...
LegalizerHelper::LegalizeResult LegalizerHelper::lowerFAbs(MachineInstr &MI) {
  ...
  LLT Ty = MRI.getType(DstReg);
  LLT IntTy = Ty.changeToInteger();

  // Reset sign bit
  auto Bitcast = MIRBuilder.buildBitcast(IntTy, SrcReg);
  auto SignMax = MIRBuilder.buildConstant(
      IntTy, APInt::getSignedMaxValue(Ty.getScalarSizeInBits()));
