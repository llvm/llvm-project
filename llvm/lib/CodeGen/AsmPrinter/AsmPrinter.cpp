...
bool AsmPrinter::doInitialization(Module &M) {
  if (MFPass) {
    auto *MMIWP =
        MFPass->getAnalysisIfAvailable<MachineModuleInfoWrapperPass>();
  ...
  if (MFPass) {
    GCModuleInfo *MI = MFPass->getAnalysisIfAvailable<GCModuleInfo>();
...
void AsmPrinter::emitBBAddrMapSection(const MachineFunction &MF) {
  ...
  if (Features.hasPGOAnalysis()) {
    ...
    const MachineBlockFrequencyInfo *MBFI = Features.BBFreq ? [this]() {
      if (MFPass) {
        auto *Wrapper =
            MFPass->getAnalysisIfAvailable<LazyMachineBlockFrequencyInfoPass>();
    ...
    const MachineBranchProbabilityInfo *MBPI = Features.BrProb ? [this]() {
      if (MFPass) {
        auto *Wrapper = MFPass->getAnalysisIfAvailable<
...
void AsmPrinter::emitFunctionBody() {
  ...
  if (isVerbose()) {
    ...
    if (MFPass) {
      auto MDTWrapper =
          MFPass->getAnalysisIfAvailable<MachineDominatorTreeWrapperPass>();
    ...
    if (MFPass) {
      auto *MLIWrapper =
          MFPass->getAnalysisIfAvailable<MachineLoopInfoWrapperPass>();
...
void AsmPrinter::SetupMachineFunction(MachineFunction &MF) {
  ...
  if (MFPass)
    ORE = &MFPass->getAnalysis<MachineOptimizationRemarkEmitterPass>().getORE();
  else
