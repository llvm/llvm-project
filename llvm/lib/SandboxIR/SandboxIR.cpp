...
Value *UIToFPInst::create(Value *Src, Type *DestTy, BBIterator WhereIt,
                          BasicBlock *WhereBB, Context &Ctx,
                          const Twine &Name) {
  return CastInst::create(DestTy, Instruction::Opcode::UIToFP, Src, WhereIt,
                          WhereBB, Ctx, Name);
}
Value *UIToFPInst::create(Value *Src, Type *DestTy, Instruction *InsertBefore,
                          Context &Ctx, const Twine &Name) {
  return create(Src, DestTy, InsertBefore->getIterator(),
                InsertBefore->getParent(), Ctx, Name);
}
Value *UIToFPInst::create(Value *Src, Type *DestTy, BasicBlock *InsertAtEnd,
                          Context &Ctx, const Twine &Name) {
  return create(Src, DestTy, InsertAtEnd->end(), InsertAtEnd, Ctx, Name);
}

#ifndef NDEBUG
void UIToFPInst::dump(raw_ostream &OS) const {
  dumpCommonPrefix(OS);
  dumpCommonSuffix(OS);
}

void UIToFPInst::dump() const {
  dump(dbgs());
  dbgs() << "\n";
}
#endif // NDEBUG
