...
namespace {
    struct HighlightingRegionInfo {
        ...
        HighlightingRegionInfo(RegionInfo *RI, const Function* F, const BasicBlock* HighlightBB = nullptr, const Instruction* HighlightInst = nullptr)
...
namespace llvm {
    template <> 
    struct GraphTraits<HighlightingRegionInfo*> : public GraphTraits<FlatIt<RegionNode*>> {
        using nodes_iterator =
            df_iterator<NodeRef, df_iterator_default_set<NodeRef>, false,
            GraphTraits<FlatIt<NodeRef>>>;

        static NodeRef getEntryNode(HighlightingRegionInfo *G) {
            return GraphTraits<FlatIt<Region*>>::getEntryNode(G->RI->getTopLevelRegion());
        }
...
namespace llvm {
    ...
    template <>
    struct DOTGraphTraits<HighlightingRegionInfo*> : public DOTGraphTraits<RegionNode*> {
        ...
        static std::string getGraphName(const HighlightingRegionInfo *) { return "Region Graph"; }

        std::string getNodeLabel(RegionNode *Node, HighlightingRegionInfo *G) {
            //return Base::getNodeLabel(Node, reinterpret_cast<RegionNode *>(G->RI->getTopLevelRegion()));
            ...
            if (!Node->isSubRegion()) {
                ...
                if (isSimple())
                    return DOTGraphTraits<DOTFuncInfo *>
                    ::getSimpleNodeLabel(BB, &CFGInfo);
                else
                    return DOTGraphTraits<DOTFuncInfo *>
                    ::getCompleteNodeLabel(BB, &CFGInfo);
            }
        ...
        static std::string getNodeAttributes(RegionNode *R,  HighlightingRegionInfo *G) {
        ...
        static void printRegionCluster(const Region &R, GraphWriter<HighlightingRegionInfo *> &GW, unsigned depth = 0, const BasicBlock *HighlightBB=nullptr,const  Instruction *HighlightInst = nullptr ) {
            raw_ostream &O = GW.getOStream();
            O.indent(2 * depth) << "subgraph cluster_" << static_cast<const void*>(&R)
                << " {\n";
            O.indent(2 * (depth + 1)) << "label = \"\";\n";
        ...
        static void addCustomGraphFeatures(
            const HighlightingRegionInfo *G,
            GraphWriter<HighlightingRegionInfo *> &GW) {
            raw_ostream &O = GW.getOStream();
            O << "\tcolorscheme = \"paired12\"\n";
            printRegionCluster(*G->RI->getTopLevelRegion(), GW, 4, G->HighlightBB, G->HighlightInst);
        }
    };
}
...
struct RegionViewer: public FunctionPass {
  ...
  RegionViewer(const BasicBlock *HighlightBB, const Instruction *HighlightInst) : FunctionPass(ID),        HighlightBB(HighlightBB), HighlightInst(HighlightInst)  {
  ...
  bool runOnFunction(Function &F) override {
      ...
      ViewGraph(&Graph, "reg", false, Twine("Region Graph for '") + F.getName().str() + "' function");
...
FunctionPass *llvm::createRegionViewerPass(const BasicBlock *BB, const Instruction *Inst) {
...
void llvm::viewRegion( RegionInfo *RI) {     if (!RI) return; viewRegionInfo(RI, false); }
...
void llvm:: viewRegion(const llvm::Instruction &I) {
    return viewRegion(&I);
}
