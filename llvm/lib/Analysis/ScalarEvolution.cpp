...
bool ScalarEvolution::isImpliedCondBalancedTypes(
  ...
  // X > Y - 1 implies X >= Y, and this holds trivially when Y - 1 does not
  // overflow. When it does overflow, it must be INT_MAX, and the comparison
  // would evaluate to false anyway. Use this predicate-strictness-boundary fact
  // to sharpen results in some edge cases.
  if (ICmpInst::isRelational(FoundPred) &&
