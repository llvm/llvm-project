...
TEST(IntelGPUTargetParserTest, ParseArch) {
  ...
  // A field wider than the GMDID field it spells describes no GPU that could
  // ever report it, so it names no device however well the rest of the name
  // matches a row. The architecture holds 10 bits, the release 8, the
  // revision 6.
  ...
  // Wider than the whole GMDID, so the integer itself does not fit.
