...
TEST_F(IR2VecTestFixture, GetBBVector_FlowAware) {
  ...
  EXPECT_EQ(BBVec.size(), 2u);
  EXPECT_TRUE(BBVec.approximatelyEquals(Embedding(2, 62.8)));
}
...
TEST_F(IR2VecTestFixture, GetFunctionVector_FlowAware) {
  ...
  EXPECT_EQ(FuncVec.size(), 2u);
  // Function vector should match BB vector (only one BB): {62.8, 62.8}
  EXPECT_TRUE(FuncVec.approximatelyEquals(Embedding(2, 62.8)));
}
