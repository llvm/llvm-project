...
static constexpr unsigned MAX_TYPEIDS = 21;
...
TEST(IR2VecVocabularyTest, DummyVocabTest) {
  for (unsigned Dim = 1; Dim <= 10; ++Dim) {
    ...
    EXPECT_EQ(VocabVec.size(), MAX_OPCODES + MAX_TYPEIDS + MAX_OPERANDS);
    ...
    EXPECT_EQ(Result.size(), MAX_OPCODES + MAX_TYPEIDS + MAX_OPERANDS);
...
TEST(IR2VecVocabularyTest, StringKeyGeneration) {
  ...
  StringRef FuncArgKey =
      Vocabulary::getStringKey(MAX_OPCODES + MAX_TYPEIDS + 0);
  StringRef PtrArgKey = Vocabulary::getStringKey(MAX_OPCODES + MAX_TYPEIDS + 1);
