...
TEST(BTFDedupTest, mergedBlobDedup) {
  ...
  ASSERT_SUCCEEDED(
      Merged
          .merge(StringRef(reinterpret_cast<const char *>(Blob1.data()),
                           Blob1.size()),
                 !sys::IsBigEndianHost)
          .takeError());
  ASSERT_SUCCEEDED(
      Merged
          .merge(StringRef(reinterpret_cast<const char *>(Blob2.data()),
                           Blob2.size()),
                 !sys::IsBigEndianHost)
          .takeError());
...
TEST(BTFDedupTest, allKindsDedup) {
  ...
  for (uint32_t Id = 1; Id <= 19; ++Id) {
    ASSERT_TRUE(B.findType(Id)) << "Type " << Id << " missing after dedup";
  }
}
