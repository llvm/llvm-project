...
### Throws functions under WinCall

A function marked with the ``throws`` attribute (a herbceptions function)
keeps the regular WinCall rules for its payload: scalars in
RAX/RDX/RCX/R8, non-scalars in XMM0/XMM1, exactly as documented above.
The success/failure discriminant is not assigned a register of its own:
it is carried out-of-band in the carry flag.

- The callee sets CF before returning (CF = 1 means the error value is
  live, CF = 0 means the return payload holds an ordinary result).
- The caller reads CF immediately after the call, before any stack
  adjustment can clobber EFLAGS.
- Because the convention already returns up to four GPR words of payload,
  no extra return slot is needed for ``std::error``-style two-word values;
  the discriminant consumes none of them.
