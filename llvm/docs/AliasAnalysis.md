...
alias analyses respond to a query with a {ref}`Must, May, or No <Must, May, or No>` alias response,
indicating that two pointers always point to the same object, might point to the
same object, or are known to never point to the same object.

The LLVM [AliasAnalysis](https://llvm.org/doxygen/classllvm_1_1AliasAnalysis.html) class is the
...
The [AliasAnalysis](https://llvm.org/doxygen/classllvm_1_1AliasAnalysis.html)
...
(never access memory or only read memory)=
...
look at the {ref}`various alias analysis implementations` included with LLVM.
...
1. If you are a function-local analysis, subclass `FunctionPass`.
1. If you don't need to look at the program at all, subclass `ImmutablePass`.
...
{ref}`update notification` methods to the superclass as well if you override them,
...
[AliasSetTracker](https://llvm.org/doxygen/classllvm_1_1AliasSetTracker.html)
...
As an example user of this, the [Loop Invariant Code Motion](https://llvm.org/doxygen/structLICM.html)
pass uses `AliasSetTracker`s to calculate alias
...
{ref}`alias` method directly if possible) to get the best precision and efficiency.
...
{ref}`the clients` that are useful for monitoring and evaluating different
implementations.

(various alias analysis implementations)=
...
* Many common standard C library functions {ref}`never access memory or only read
  memory <never access memory or only read memory>`.
...
out the Alias Sets formed by the {ref}`AliasSetTracker` class.  This is useful if
