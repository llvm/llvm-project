...
usual include guards and #includes [^footnote-2], we get to the definition of our class:
...
result wrapped in an Expected value [^footnote-3] that we must check for error before
...
[^footnote-1]: Actually we use a cut-down version of KaleidoscopeJIT that makes a
