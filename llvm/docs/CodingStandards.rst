...
Given the majority of LLVM's test cases are written as lit-style tests that
are essentially code of their own (usually IR), there are some general best
practices to ensure the tests are maintainable and effective.

Autogenerate Assertions Where Relevant
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

LLVM has several scripts available for different types of tests that will
automatically generate ``FileCheck`` based assertions. These scripts include:

*   ``update_test_checks.py`` - Used for IR to IR transformations driven by ``opt``.
*   ``update_llc_test_checks.py`` - Used for IR to assembly transformations driven by ``llc``.
*   ``update_mir_test_checks.py`` - Used for IR/MIR to MIR transformations driven by ``llc``.

The list above is not exhaustive. Some other parts of LLVM have their own update
scripts. You should generally use the same scripts as surrounding tests in the folder.
These scripts should be used wherever they can be, assuming there is not a strong
reason not to. Having a standard format makes the tests to read and being able
to automatically update them with a script makes them much easier to maintain.

Minimize Check Prefixes
^^^^^^^^^^^^^^^^^^^^^^^

``FileCheck`` allows specifying different check prefixes with the ``--check-prefix``
flag. This can be useful when you want to run multiple ``FileCheck`` commands
in the same file that expect different output, and is especially useful when
some lines might be common between multiple invocations.

However, the number of check prefixes should be minimized as much as possible
to aid in readability. A check prefix should not be specified if all ``FileCheck``
invocations expect the same output.

Omit Target Triples and Datalayouts where Possible
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Depending upon the type of test that you are writing, it might be possible to
omit the target triple and data layout strings, for example in instcombine
tests. These should be omitted where possible to reduce clutter and avoid
needing a specific backend to be built for the test to run.

Use Named Values
^^^^^^^^^^^^^^^^

When writing an IR test, use appropriately named values rather than using
numbered values that are emitted by some tools. Running the test case through
the ``metarenamer`` pass can be a good start in some cases.

Strip Unnecessary Function and Parameter Attributes
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Even reduced IR test cases will usually have a lot of function and
parameter attributes that are not relevant to the behavior being tested.
Any function and parameter attributes that are not necessary to test the code
path of interest should be removed.

Avoid UB in Test Inputs
^^^^^^^^^^^^^^^^^^^^^^^

Unless a test is specifically testing how the optimizer/CodeGen deals with UB,
test cases should not have any immediate UB (e.g., branching on poison loading
from a pointer that has a poison value).

Put the Test in the Right Place
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

There are large number of testing directories within LLVM and it is important to
organize tests correctly. If your test requires a specific target triple, you
likely should put it in the target-named subfolder of the area you are interested
in testing. This will automatically handle marking the test unsupported if the
necessary backend is not built.

It is also preferrable to add tests to existing files when possible rather than
creating new files.
