(amdgpu-dwarf-extensions-for-heterogeneous-debugging)=
...
(amdgpu-dwarf-allow-location-description-on-the-dwarf-evaluation-stack)=
...
(amdgpu-dwarf-operation-to-create-vector-composite-location-descriptions)=
...
(amdgpu-dwarf-support-for-divergent-control-flow-of-simt-hardware)=
...
(amdgpu-dwarf-changes-relative-to-dwarf-version-5)=
...
The names for the new operations, attributes, and constants include "`LLVM`" and are encoded with vendor specific codes so these extensions
...
(amdgpu-dwarf-expression-evaluation-context)=
...
(amdgpu-dwarf-operation-expressions)=
...
(amdgpu-dwarf-vendor-extensions-operations)=
...
(amdgpu-dwarf-control-flow-operations)=
...
5. `DW_OP_call2, DW_OP_call4, DW_OP_call_ref`
   ...
   *Operand interpretation of* `DW_OP_call2`*,* `DW_OP_call4`*, and*
   ...
   - Otherwise, there is no effect and no changes are made to the stack.
     ...
     `DW_OP_call*` to get a location description for any non-`DW_TAG_dwarf_procedure` debugging information entries. Also, the
...
(amdgpu-dwarf-location-description-operations)=
...
(amdgpu-dwarf-location-list-expressions)=
...
(amdgpu-dwarf-full-and-partial-compilation-unit-entries)=
...
(amdgpu-dwarf-low-level-information)=
...
6. For languages that are implemented using a SIMT execution model, a
   ...
   N{sup}`th` least significant bit of the mask corresponds to the N{sup}`th` lane. If the bit is 1 the lane is active, otherwise it is
...
2. A `DW_TAG_call_site_parameter` debugger information entry may have a
   ...
   execution upon which the user is focused, if any. The resulting value V{sub}`1` is the value of the parameter at the time of the call made by the
   call site.
   ...
   referenced parameter lives during the call made by the call site. If E{sub}`2` would just be a `DW_OP_push_object_address`, then the
...
(amdgpu-dwarf-lexical-block-entries)=
...
(amdgpu-dwarf-base-type-entries)=
...
(amdgpu-dwarf-line-number-information)=
...
04. `augmentation` (sequence of UTF-8 characters)
...
06. `segment_selector_size` (ubyte)

    The size of a segment selector in this CIE and any FDEs that use it, in
    bytes.

07. `code_alignment_factor` (unsigned LEB128)
...
08. `data_alignment_factor` (signed LEB128)
...
09. `return_address_register` (unsigned LEB128)
...
(amdgpu-dwarf-call-frame-instructions)=
...
(amdgpu-dwarf-row-creation-instructions)=
...
(amdgpu-dwarf-cfa-definition-instructions)=
...
(amdgpu-dwarf-register-rule-instructions)=
...
(amdgpu-dwarf-call-frame-calling-address)=
...
(amdgpu-dwarf-32-bit-and-64-bit-dwarf-formats)=
...
(amdgpu-dwarf-references)=

## D. References

> (amdgpu-dwarf-amd)=

01. [AMD] [Advanced Micro Devices](https://www.amd.com/)

    (amdgpu-dwarf-amd-rocgdb)=

02. [AMD-ROCgdb] [AMD ROCm Debugger (ROCgdb)](https://github.com/ROCm/ROCgdb)

    (amdgpu-dwarf-amd-rocm)=

03. [AMD-ROCm] [AMD ROCm Platform](https://rocm-documentation.readthedocs.io)

    (amdgpu-dwarf-amdgpu-dwarf-loc)=

04. [AMDGPU-DWARF-LOC] [Allow Location Descriptions on the DWARF Expression Stack](https://llvm.org/docs/AMDGPUDwarfExtensionAllowLocationDescriptionOnTheDwarfExpressionStack/AMDGPUDwarfExtensionAllowLocationDescriptionOnTheDwarfExpressionStack.html)

    (amdgpu-dwarf-amdgpu-llvm)=

05. [AMDGPU-LLVM] [User Guide for AMDGPU LLVM Backend](https://llvm.org/docs/AMDGPUUsage.html)

    (amdgpu-dwarf-cuda)=

06. [CUDA] [Nvidia CUDA Language](https://docs.nvidia.com/cuda/cuda-c-programming-guide/)

    (amdgpu-dwarf-dwarf)=

07. [DWARF] [DWARF Debugging Information Format](http://dwarfstd.org/)

    (amdgpu-dwarf-elf)=

08. [ELF] [Executable and Linkable Format (ELF)](http://www.sco.com/developers/gabi/)

    (amdgpu-dwarf-gcc)=

09. [GCC] [GCC: The GNU Compiler Collection](https://www.gnu.org/software/gcc/)

    (amdgpu-dwarf-gdb)=

10. [GDB] [GDB: The GNU Project Debugger](https://www.gnu.org/software/gdb/)

    (amdgpu-dwarf-hip)=

11. [HIP] [HIP Programming Guide](https://rocm-documentation.readthedocs.io/en/latest/Programming_Guides/Programming-Guides.html#hip-programing-guide)

    (amdgpu-dwarf-hsa)=

12. [HSA] [Heterogeneous System Architecture (HSA) Foundation](http://www.hsafoundation.com/)

    (amdgpu-dwarf-llvm)=

13. [LLVM] [The LLVM Compiler Infrastructure](https://llvm.org/)

    (amdgpu-dwarf-opencl)=

14. [OpenCL] [The OpenCL Specification Version 2.0](http://www.khronos.org/registry/cl/specs/opencl-2.0.pdf)

    (amdgpu-dwarf-perforce-totalview)=

15. [Perforce-TotalView] [Perforce TotalView HPC Debugging Software](https://totalview.io/products/totalview)
