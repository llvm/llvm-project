...
support can be found at [Itanium C++ ABI: Exception Handling]. A description of the
exception frame format can be found at [Exception Frames],
with details of the DWARF 4 specification at [DWARF 4 Standard].  A description for the C++ exception
...
[Exception Frames]: http://refspecs.linuxfoundation.org/LSB_3.0.0/LSB-Core-generic/LSB-Core-generic/ehframechpt.html
[DWARF 4 Standard]: http://dwarfstd.org/Dwarf4Std.php
[Exception Handling Tables]: http://itanium-cxx-abi.github.io/cxx-abi/exceptions.pdf

### Setjmp/Longjmp Exception Handling

Setjmp/Longjmp (SJLJ) based exception handling uses LLVM intrinsics
{ref}`llvm.eh.sjlj.setjmp` and {ref}`llvm.eh.sjlj.longjmp` to handle control flow for
exception handling.
...
context. The runtime returns to the function via {ref}`llvm.eh.sjlj.longjmp`, where
...
1. A request is made to allocate exception space for an exception structure.
...
1. A call is made to the runtime to raise the exception, passing the exception
...
1. where to continue when the call succeeds normally, and

1. where to continue if the call raises an exception, either by a throw or the
...
must call the {ref}`llvm.eh.typeid.for` intrinsic to determine the index for a given
...
* `__cxa_end_catch` takes no arguments. This function:
  ...
  1. Locates the most recently caught exception and decrements its handler
     count,

  1. Removes the exception from the *caught* stack if the handler count goes to
     zero, and
...
the {ref}`resume instruction <i_resume>` if none of the conditions
match.
...
a destination address by {ref}`llvm.eh.sjlj.longjmp`. The buffer format and the
...
a buffer populated by {ref}`llvm.eh.sjlj.setjmp`. The frame pointer and stack
...
* `EH_RETURN`
  ...
  is used in GCC unwinder ([libgcc]),
  but not in LLVM unwinder ([libunwind]).
