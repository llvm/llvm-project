...
A Basic profile's content marker selects Basic mode when replayed with this
option. Use the same `llvm-profgen` build, binary and address-base options as
...
:::{option} --basic-events, --ba
Generate a flat, line-number sample profile from sampled instruction-pointer
perf events. Use this option with `--perfdata` or `--perfscript`.

With `--perfdata`, llvm-profgen invokes `perf script` with the required
options. `--perfscript` accepts user-space output generated by
`perf script --show-mmap-events --show-task-events -F pid,ip,time -G`, or
kernel output generated by `perf script --show-mmap-events -F pid,ip -G`.
Kernel input requires `--kernel` when invoking `llvm-profgen`.
:::
...
Use this option with flat, line-number branch-stack profiles.
:::
