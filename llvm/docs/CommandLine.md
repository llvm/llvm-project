...
for the option declared (of course this {ref}`can be changed <can be changed>`).
...
1. Speed: The CommandLine library is very quick and uses little resources.  The
...
1. Type Safe: As a user of CommandLine, you don't have to worry about
   remembering the type of arguments that you want (is it an int?  a string? a
   bool? an enum?) and keep casting it around.  Not only does this help prevent
   error prone constructs, it also leads to dramatically cleaner source code.

1. No subclasses required: To use CommandLine, you instantiate variables that
...
1. Globally accessible: Libraries can specify command line arguments that are
   ...
   pass to the parser.  This also makes supporting {ref}`dynamically loaded options <dynamically loaded options>`
   trivial.

1. Cleaner: CommandLine supports enum and other types directly, meaning that
...
1. Powerful: The CommandLine library supports many different types of arguments,
   from simple {ref}`boolean flags <boolean flags>` to {ref}`scalars arguments <scalars arguments>` ({ref}`strings <strings>`,
   {ref}`integers <integers>`, {ref}`enums <enums>`, {ref}`doubles <doubles>`), to {ref}`lists of arguments <lists of arguments>`.  This is
   possible because CommandLine is...

1. Extensible: It is very simple to add a new argument type to CommandLine.
   Simply specify the parser that you want to use with the command line option
   when you declare it. {ref}`Custom parsers <Custom parsers>` are no problem.

1. Labor Saving: The CommandLine library cuts down on the amount of grunt work
...
with helper functions like {ref}`cl::desc(...) <cl::desc(...)>`, so there are no positional
dependencies to remember.  The available options are discussed in detail in the
{ref}`Reference Guide <Reference Guide>`.
...
argument, the CommandLine library allows for {ref}`positional <positional>` arguments to be
...
as the input filename.  Here we use the {ref}`cl::init <cl::init>` option to specify an initial
value for the command line option, which is used if the option is not specified
(if you do not specify a {ref}`cl::init <cl::init>` modifier for an option, then the default
...
always specify an input filename, we would add the {ref}`cl::Required <cl::Required>` flag, and we
could eliminate the {ref}`cl::init <cl::init>` modifier, like this:
...
By simply adding the {ref}`cl::Required <cl::Required>` flag, the CommandLine library will
...
that the "`-q`" option is specified with the "{ref}`cl::Hidden <cl::Hidden>`" flag.  This
modifier prevents it from being shown by the standard "`-help`" output (note
that it is still shown in the "`-help-hidden`" output).

The CommandLine library uses a {ref}`different parser <different parser>` for different data types.
...
{ref}`float <float>`, {ref}`double <double>`, and {ref}`int <int>` parsers work like you would expect, using the
...
This brief example has shown you how to use the '{ref}`cl::opt <cl::opt>`' class to parse
...
{ref}`aliases <aliases>`, and {ref}`lists <lists>` of options.
...
condition, we can use the "{ref}`cl::alias <cl::alias>`" class to make the "`-q`" option an
...
alias that updates the "`Quiet`" variable (as specified by the {ref}`cl::aliasopt <cl::aliasopt>`
...
... which is much nicer!  The "{ref}`cl::alias <cl::alias>`" can be used to specify an
alternative name for any variable type, and has many uses.

(unnamed alternatives using the generic parser)=
...
your own parser, as described in the {ref}`Extension Guide <Extension Guide>`).  This parser maps
...
1. A user could specify more than one of the options at a time, for example,
   "`compiler -O3 -O2`".  The CommandLine library would not be able to catch
   this erroneous input for us.

1. We would have to test 4 different variables to see which ones are set.

1. This doesn't map to the numeric levels that we want... so we cannot easily
...
argument to the {ref}`cl::ParseCommandLineOptions <cl::ParseCommandLineOptions>` call in main.  This additional
...
categories ({ref}`cl::OptionCategory <cl::OptionCategory>` objects) and then placing our options into
these categories using the {ref}`cl::cat <cl::cat>` option attribute. For example:
...
(interpreter style options)=
(cl::ConsumeAfter)=
(this section for more information)=

#### The `cl::ConsumeAfter` modifier

The `cl::ConsumeAfter` {ref}`formatting option <formatting option>` is used to construct programs that
...
program, there must be at least one {ref}`positional argument <positional argument>` specified, there must
not be any {ref}`cl::list <cl::list>` positional arguments, and the `cl::ConsumeAfter` option
should be a {ref}`cl::list <cl::list>` option.
...
to fill in with the {ref}`cl::location <cl::location>` attribute:
...
{ref}`cl::opt <cl::opt>` template, indicating that the template should not maintain a copy of
the value itself.  In addition to this, we specify the {ref}`cl::location <cl::location>`
...
* The option name attribute (which is required for all options, except
  {ref}`positional options <positional options>`) specifies what the option name is.  This option is
...
* The **cl::value_desc** attribute specifies a string that can be used to
  fine tune the `-help` output for a command line option.  Look {ref}`here <here>` for an
  example.

(cl::init)=

* The **cl::init** attribute specifies an initial value for a {ref}`scalar <scalar>`
...
* The **cl::location** attribute where to store the value for a parsed command
  line option if using external storage.  See the section on {ref}`Internal vs External Storage <Internal vs External Storage>`
  for more information.

(cl::aliasopt)=

* The **cl::aliasopt** attribute specifies which option a {ref}`cl::alias <cl::alias>` option is
  an alias for.
...
* The **cl::values** attribute specifies the string-to-value mapping to be used
  ...
  is used most frequently with enum values, two macros are often useful:
   ...
   1. The **clEnumVal** macro is used as a nice simple way to specify a triplet
   ...
   1. The **clEnumValN** macro is used to specify macro options where the option
...
* The **cl::cat** attribute specifies the option category that the option
  belongs to. The category should be a {ref}`cl::OptionCategory <cl::OptionCategory>` object.
...
* The **cl::callback** attribute specifies a callback function that is
  ...
  as in option B implies option A.  If the option is a {ref}`cl::list <cl::list>`,
  and {ref}`cl::CommaSeparated <cl::CommaSeparated>` is also specified, the callback will fire
...
constructors for {ref}`cl::opt <cl::opt>` and {ref}`cl::list <cl::list>`.  These modifiers give you the
...
1. Controlling the number of occurrences required and allowed

1. Controlling whether or not a value must be specified

1. Controlling other formatting options

1. Miscellaneous option modifiers
...
* The **cl::NotHidden** modifier (which is the default for {ref}`cl::opt <cl::opt>` and
  {ref}`cl::list <cl::list>` options) indicates the option is to appear in both help
  listings.

(cl::Hidden)=

* The **cl::Hidden** modifier (which is the default for {ref}`cl::alias <cl::alias>` options)
...
* The **cl::Optional** modifier (which is the default for the {ref}`cl::opt <cl::opt>` and
  {ref}`cl::alias <cl::alias>` classes) indicates that your program will allow either zero or
  one occurrence of the option to be specified.

(cl::ZeroOrMore)=

* The **cl::ZeroOrMore** modifier (which is the default for the {ref}`cl::list <cl::list>`
...
* The **cl::ConsumeAfter** modifier is described in the {ref}`Positional arguments section <Positional arguments section>`.

If an option is not specified, then the value of the option is equal to the
value specified by the {ref}`cl::init <cl::init>` attribute.  If the `cl::init` attribute is
not specified, the option value is initialized with the default constructor for
the data type.

If an option is specified multiple times for an option of the {ref}`cl::opt <cl::opt>` class,
...
* The **cl::ValueOptional** modifier (which is the default for `bool` typed
  ...
  the {ref}`cl::ValueRequired <cl::ValueRequired>` modifier.

(cl::ValueRequired)=

* The **cl::ValueRequired** modifier (which is the default for all other types
  except for {ref}`unnamed alternatives using the generic parser <unnamed alternatives using the generic parser>`) specifies that a
...
* The **cl::ValueDisallowed** modifier (which is the default for {ref}`unnamed alternatives using the generic parser <unnamed alternatives using the generic parser>`)
  indicates that it is a runtime error
...
want them to.  As mentioned above, you can specify the {ref}`cl::ValueDisallowed <cl::ValueDisallowed>`
modifier to a boolean argument to restrict your command line parser.  These
options are mostly useful when {ref}`extending the library <extending the library>`.
...
* The **cl::Positional** modifier specifies that this is a positional argument
  ...
  {ref}`Positional Arguments <Positional Arguments>` section for more information.

* The **cl::ConsumeAfter** modifier specifies that this option is used to
  capture "interpreter style" arguments.  See {ref}`this section for more information <this section for more information>`.
...
only consists of one function {ref}`cl::ParseCommandLineOptions <cl::ParseCommandLineOptions>` and three main
classes: {ref}`cl::opt <cl::opt>`, {ref}`cl::list <cl::list>`, and {ref}`cl::alias <cl::alias>`.  This section describes
...
in `-help` can be modified prior to calling {ref}`cl::ParseCommandLineOptions <cl::ParseCommandLineOptions>`.
...
{ref}`additional extra text <additional extra text>` to emit when the `-help` option is invoked.
...
to contain the value parsed for the option (see {ref}`Internal vs External Storage <Internal vs External Storage>`
for more information).
...
so this option is only used when using a {ref}`custom parser <custom parser>`.
...
This class works the exact same as the {ref}`cl::opt <cl::opt>` class, except that the second
...
The {ref}`cl::aliasopt <cl::aliasopt>` attribute should be used to specify which option this is an
alias for.  Alias arguments default to being {ref}`cl::Hidden <cl::Hidden>`, and use the aliased
...
{ref}`Writing a Custom Parser <Writing a Custom Parser>` for more details on this type of library extension.

(enums)=
(cl::parser)=

* The generic `parser<t>` parser can be used to map strings values to any data
  type, through the use of the {ref}`cl::values <cl::values>` property, which specifies the
...
As {ref}`discussed previously <discussed previously>`, parsers are the portion of the CommandLine library
...
1. Specialize the {ref}`cl::parser <cl::parser>` template for your custom data type.
...
1. Write an independent class, using it explicitly from options that need it.
