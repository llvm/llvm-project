...
1. `cmpNumbers(number1, number2)` is a method that returns -1 if left is less
   than right; 0, if left and right are equal; and 1 otherwise.
2. `cmpFlags(flag1, flag2)` is a hypothetical method that compares two flags.
...
1. For two trees *T1* and *T2* we perform *depth-first-traversal* and have
   two sequences as a product: "*T1Items*" and "*T2Items*".
2. We then compare chains "*T1Items*" and "*T2Items*" in
