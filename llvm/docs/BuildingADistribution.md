...
then setting the *LLVM\_\<distribution>\_DISTRIBUTION_COMPONENTS* variable to the
...
explicitly included in the *LLVM\_\<distribution>\_DISTRIBUTION_COMPONENTS*
...
explicitly in some *LLVM\_\<distribution>\_DISTRIBUTION_COMPONENTS* list.
...
*CMAKE\_\<LANG>\_FLAGS_RELWITHDEBINFO* option for C and CXX.
...
: This variable can be set to a semicolon-separated name of LLVM library
  ...
  1. `all` - All available LLVM component libraries
  2. `Native` - The LLVM target for the Native system
  3. `AllTargetsAsmParsers` - All the included target ASM parsers libraries
  4. `AllTargetsDescs` - All the included target descriptions libraries
  5. `AllTargetsDisassemblers` - All the included target dissassemblers libraries
  6. `AllTargetsInfos` - All the included target info libraries
