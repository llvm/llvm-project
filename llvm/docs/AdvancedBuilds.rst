...
.. note::
   ...
   The detailed multi-stage PGO workflow previously documented here is no longer
   up to date. The ``stage2-instrumented-generate-profdata`` target referenced in
   older documentation is no longer generated by current LLVM builds. This section
   will be updated once a new recommended workflow is documented.
