...
in AIIR by way of AIIR attributes. Flang can leverage these AIIR attributes to
...
`SubprogramAttr` on FuncOp. This allows AIIR to generate LLVM IR metadata
...
types. Flang will generate debug metadata in the form of AIIR attributes. These
...
1. AIIR attributes are generated by reading information from AST or FIR. This
...
and `DeclareOp` to extract the source information and build the AIIR
attributes. A class will be added to handle conversion of AIIR and FIR types to
`DITypeAttr`.

Following sections provide details of how various language constructs will be
handled. In these sections, the LLVM IR metadata and AIIR attributes have been
used interchangeably. As an example, `DILocalVariableAttr` is an AIIR attribute
which gets translated to LLVM IR's `DILocalVariable`.

### Variables

#### Local Variables
  In AIIR, local variables are represented by `DILocalVariableAttr` which
...
## Missing metadata in AIIR

Some metadata types that are needed for fortran are present in LLVM IR but are
absent from AIIR. A non comprehensive list is given below.

1. `DICommonBlockAttr`
2. `DIGenericSubrangeAttr`
3. `DISubrangeAttr` in AIIR takes IntegerAttr at the moment so only works
...
- [4] https://github.com/llvm/llvm-project/blob/main/aiir/lib/Target/LLVMIR/DebugTranslation.cpp
