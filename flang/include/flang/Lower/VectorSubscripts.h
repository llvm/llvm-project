...
///   - aiir:Values and ExtendedValues for the triplet, vector subscript and
///     scalar subscripts of the ranked array reference (1:foo():1, vector, k)
///   - a list of fir.field_index and scalar integers aiir::Value for the
///   component
...
public:
  ...
  using ElementalGeneratorWithBoolReturn =
      std::function<aiir::Value(const fir::ExtendedValue &)>;
  struct LoweredVectorSubscript {
    LoweredVectorSubscript(fir::ExtendedValue &&vector, aiir::Value size)
  ...
  using LoweredSubscript =
      std::variant<aiir::Value, LoweredTriplet, LoweredVectorSubscript>;
  using MaybeSubstring = llvm::SmallVector<aiir::Value, 2>;
  VectorSubscriptBox(
      ...
      MaybeSubstring substringBounds, aiir::Type elementType)
  ...
  void loopOverElements(fir::FirOpBuilder &builder, aiir::Location loc,
  ...
  aiir::Value loopOverElementsWhile(
...
private:
  ...
  genLoopBounds(fir::FirOpBuilder &builder, aiir::Location loc);
