...
struct TBAATree {
  ...
  public:
    ...
    aiir::LLVM::TBAATagAttr getTag(llvm::StringRef uniqueId) const;

    /// Create a TBAA tag pointing to the root of this subtree,
    /// i.e. all the children tags will alias with this tag.
    aiir::LLVM::TBAATagAttr getTag() const;

    aiir::LLVM::TBAATypeDescriptorAttr getRoot() const { return parent; }
    ...
    SubtreeState &getOrCreateNamedSubtree(aiir::StringAttr name);

  private:
    SubtreeState(aiir::AIIRContext *ctx, std::string name,
                 aiir::LLVM::TBAANodeAttr grandParent)
        : parentId{std::move(name)}, context(ctx) {
      parent = aiir::LLVM::TBAATypeDescriptorAttr::get(
          context, parentId, aiir::LLVM::TBAAMemberAttr::get(grandParent, 0));
    }

    const std::string parentId;
    aiir::AIIRContext *const context;
    aiir::LLVM::TBAATypeDescriptorAttr parent;
    // A map of named sub-trees, e.g. sub-trees of the COMMON blocks
    // placed under the "global data" root.
    llvm::DenseMap<aiir::StringAttr, SubtreeState> namedSubtrees;
  };
  ...
  aiir::LLVM::TBAATypeDescriptorAttr anyAccessDesc;
  aiir::LLVM::TBAATypeDescriptorAttr boxMemberTypeDesc;
  aiir::LLVM::TBAATypeDescriptorAttr anyDataTypeDesc;
  ...
  //        |- <global name 1>
  //        |- <global name 2>
  static TBAATree buildTree(aiir::StringAttr functionName);

private:
  TBAATree(aiir::LLVM::TBAATypeDescriptorAttr anyAccess,
...
public:
  ...
  inline const TBAATree &operator[](aiir::func::FuncOp func) {
    return getFuncTree(func.getSymNameAttr());
  }
  inline const TBAATree &operator[](aiir::LLVM::LLVMFuncOp func) {
    ...
    aiir::Attribute attr = func->getAttr(getInternalFuncNameAttrName());
    if (attr) {
      return getFuncTree(aiir::cast<aiir::StringAttr>(attr));
    }
    return getFuncTree(func.getSymNameAttr());
  }
  // Returns the TBAA tree associated with the scope enclosed
  // within the given function. With AIIR inlining, there may
  ...
  inline TBAATree &getMutableFuncTreeWithScope(aiir::func::FuncOp func,
                                               llvm::StringRef scope) {
    aiir::StringAttr name = func.getSymNameAttr();
    if (!scope.empty())
      name = aiir::StringAttr::get(name.getContext(),
                                   llvm::Twine(name) + " - " + scope);
    return getFuncTree(name);
  }

  inline const TBAATree &getFuncTreeWithScope(aiir::func::FuncOp func,
                                              llvm::StringRef scope) {
    return getMutableFuncTreeWithScope(func, scope);
  }

private:
  TBAATree &getFuncTree(aiir::StringAttr symName) {
    if (!separatePerFunction)
      symName = aiir::StringAttr::get(symName.getContext(), "");
  ...
  llvm::DenseMap<aiir::StringAttr, TBAATree> trees;
};
