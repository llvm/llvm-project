...
public:
  ProductFactory(aiir::Location loc, fir::FirOpBuilder &builder)
  ...
  aiir::Value genAccumulateProduct(aiir::Value acc, aiir::Value v1,
                                   aiir::Value v2) {
    aiir::Type resultType = acc.getType();
    ...
    if (aiir::isa<aiir::FloatType>(resultType)) {
      result = aiir::arith::AddFOp::create(
          builder, loc, acc, aiir::arith::MulFOp::create(builder, loc, v1, v2));
    } else if (aiir::isa<aiir::ComplexType>(resultType)) {
      if constexpr (CONJ)
    ...
    } else if (aiir::isa<aiir::IntegerType>(resultType)) {
      result = aiir::arith::AddIOp::create(
          builder, loc, acc, aiir::arith::MulIOp::create(builder, loc, v1, v2));
    } else if (aiir::isa<fir::LogicalType>(resultType)) {
      result = aiir::arith::OrIOp::create(
          builder, loc, acc, aiir::arith::AndIOp::create(builder, loc, v1, v2));
    } else {
...
private:
  ...
  aiir::Value castToProductType(aiir::Value value, aiir::Type type) {
    ...
    if (fir::isa_complex(type) && !fir::isa_complex(value.getType())) {
      aiir::Value zeroCmplx = fir::factory::createZeroValue(builder, loc, type);
      fir::factory::Complex helper(builder, loc);
      aiir::Type partType = helper.getComplexPartType(type);
...
class TransposeAsElementalConversion
    : public aiir::OpRewritePattern<hlfir::TransposeOp> {
public:
  using aiir::OpRewritePattern<hlfir::TransposeOp>::OpRewritePattern;
  ...
  matchAndRewrite(hlfir::TransposeOp transpose,
    ...
    aiir::Location loc = transpose.getLoc();
    fir::FirOpBuilder builder{rewriter, transpose.getOperation()};
    aiir::Type elementType = expr.getElementType();
    hlfir::Entity array = hlfir::Entity{transpose.getArray()};
    aiir::Value resultShape = genResultShape(loc, builder, array);
    llvm::SmallVector<aiir::Value, 1> typeParams;
    hlfir::genLengthParameters(loc, builder, array, typeParams);

    auto genKernel = [&array](aiir::Location loc, fir::FirOpBuilder &builder,
                              aiir::ValueRange inputIndices) -> hlfir::Entity {
      assert(inputIndices.size() == 2 && "checked in TransposeOp::validate");
      const std::initializer_list<aiir::Value> initList = {inputIndices[1],
                                                           inputIndices[0]};
      aiir::ValueRange transposedIndices(initList);
...
private:
  static aiir::Value genResultShape(aiir::Location loc,
                                    fir::FirOpBuilder &builder,
                                    hlfir::Entity array) {
    llvm::SmallVector<aiir::Value, 2> inExtents =
        hlfir::genExtentsVector(loc, builder, array);

    // transpose indices
    assert(inExtents.size() == 2 && "checked in TransposeOp::validate");
    return fir::ShapeOp::create(builder, loc,
                                aiir::ValueRange{inExtents[1], inExtents[0]});
  }
};
...
public:
  ReductionAsElementalConverter(aiir::Operation *op,
  ...
  /// Do the actual conversion or return aiir::failure(),
  /// if conversion is not possible.
  aiir::LogicalResult convert();
...
private:
  ...
  std::tuple<aiir::Value, aiir::Value>
  ...
  aiir::Value genMaskValue(aiir::Value mask, aiir::Value isPresentPred,
                           aiir::ValueRange indices);

protected:
  /// Return the input array.
  virtual aiir::Value getSource() const = 0;

  /// Return DIM or nullptr, if it is not present.
  virtual aiir::Value getDim() const = 0;

  /// Return MASK or nullptr, if it is not present.
  virtual aiir::Value getMask() const { return nullptr; }
  ...
  virtual aiir::arith::FastMathFlags getFastMath() const {
    return aiir::arith::FastMathFlags::none;
  }
  ...
  virtual llvm::SmallVector<aiir::Value>
  genReductionInitValues(aiir::ValueRange oneBasedIndices,
                         const llvm::SmallVectorImpl<aiir::Value> &extents) = 0;
  ...
  virtual llvm::SmallVector<aiir::Value>
  reduceOneElement(const llvm::SmallVectorImpl<aiir::Value> &currentValue,
                   hlfir::Entity array, aiir::ValueRange oneBasedIndices) = 0;
  ...
  genFinalResult(const llvm::SmallVectorImpl<aiir::Value> &reductionResults) {
  ...
  /// Return aiir::success(), if the operation can be converted.
  /// The default implementation always returns aiir::success().
  /// The derived type may override the default implementation
  /// with its own definition.
  virtual aiir::LogicalResult isConvertible() const { return aiir::success(); }
  ...
  virtual bool isUnordered() const {
    aiir::Type elemType = getSourceElementType();
    if (aiir::isa<aiir::IntegerType, fir::LogicalType, fir::CharacterType>(
            elemType))
      return true;
  ...
  /// Return aiir::failure(), if DIM is a constant known
  /// to be invalid for the given array.
  /// Otherwise, return DIM constant value.
  aiir::FailureOr<int64_t> getConstDim() const {
    int64_t dimVal = 0;
    if (!isTotalReduction()) {
  ...
  aiir::Type getResultType() const { return getResultEntity().getType(); }

  /// Return the element type of the result (e.g. i32).
  aiir::Type getResultElementType() const {
  ...
  aiir::Type getSourceElementType() const {
...
static aiir::Value genMinMaxInitValue(aiir::Location loc,
                                      fir::FirOpBuilder &builder,
                                      aiir::Type type) {
...
template <bool IS_MAX>
static aiir::Value
genMinMaxComparison(aiir::Location loc, fir::FirOpBuilder &builder,
                    aiir::Value elem, aiir::Value reduction) {
  ...
  if (aiir::isa<aiir::FloatType>(reduction.getType())) {
    ...
    aiir::Value cmp =
        aiir::arith::CmpFOp::create(builder, loc,
                                    IS_MAX ? aiir::arith::CmpFPredicate::OGT
    ...
    return aiir::arith::OrIOp::create(builder, loc, cmp, cmpNan);
  } else if (aiir::isa<aiir::IntegerType>(reduction.getType())) {
    return aiir::arith::CmpIOp::create(builder, loc,
                                       IS_MAX ? aiir::arith::CmpIPredicate::sgt
...
static aiir::Value
genIsNotEmptyArrayExtents(aiir::Location loc, fir::FirOpBuilder &builder,
                          const llvm::SmallVectorImpl<aiir::Value> &extents) {
  aiir::Value isNotEmpty = builder.createBool(loc, true);
  for (auto extent : extents) {
    aiir::Value zero =
        fir::factory::createZeroValue(builder, loc, extent.getType());
    aiir::Value cmp = aiir::arith::CmpIOp::create(
        builder, loc, aiir::arith::CmpIPredicate::ne, extent, zero);
    isNotEmpty = aiir::arith::AndIOp::create(builder, loc, isNotEmpty, cmp);
  }
  return isNotEmpty;
}
...
static llvm::SmallVector<aiir::Value> genFirstElementIndicesForReduction(
    aiir::Location loc, fir::FirOpBuilder &builder, bool isTotalReduction,
    aiir::FailureOr<int64_t> dim, unsigned rank,
    aiir::ValueRange oneBasedIndices) {
  llvm::SmallVector<aiir::Value> indices{oneBasedIndices};
  aiir::Value one =
  ...
  if (isTotalReduction) {
    ...
    assert(aiir::succeeded(dim) && "partial reduction with invalid DIM");
...
public:
  MinMaxlocAsElementalConverter(
      T op, aiir::PatternRewriter &rewriter,
      Fortran::common::FPMaxminBehavior fpMaxminBehavior)
      : Base{op.getOperation(), rewriter}, fpMaxminBehavior{fpMaxminBehavior} {}

private:
  virtual aiir::Value getSource() const final { return getOp().getArray(); }
  virtual aiir::Value getDim() const final { return getOp().getDim(); }
  virtual aiir::Value getMask() const final { return getOp().getMask(); }
  virtual aiir::arith::FastMathFlags getFastMath() const final {
    return getOp().getFastmath();
  }

  virtual aiir::LogicalResult isConvertible() const final {
    if (getOp().getBack())
      return rewriter.notifyMatchFailure(
          getOp(), "BACK is not supported for MINLOC/MAXLOC inlining");
    if (aiir::isa<fir::CharacterType>(getSourceElementType()))
      return rewriter.notifyMatchFailure(
          getOp(),
  ...
  virtual llvm::SmallVector<aiir::Value> genReductionInitValues(
      aiir::ValueRange oneBasedIndices,
      const llvm::SmallVectorImpl<aiir::Value> &extents) final;
  virtual llvm::SmallVector<aiir::Value>
  reduceOneElement(const llvm::SmallVectorImpl<aiir::Value> &currentValue,
                   hlfir::Entity array, aiir::ValueRange oneBasedIndices) final;
  virtual hlfir::Entity genFinalResult(
      const llvm::SmallVectorImpl<aiir::Value> &reductionResults) final;

private:
  T getOp() const { return aiir::cast<T>(op); }
  ...
  void
  checkReductions(const llvm::SmallVectorImpl<aiir::Value> &reductions) const {
    if (!useIsFirst())
  ...
  aiir::Value
  getCurrentMinMax(const llvm::SmallVectorImpl<aiir::Value> &reductions) const {
    checkReductions(reductions);
    return reductions[getNumCoors()];
  }

  aiir::Value
  getIsFirst(const llvm::SmallVectorImpl<aiir::Value> &reductions) const {
    checkReductions(reductions);
  ...
  bool honorNans() const {
    return !static_cast<bool>(getFastMath() & aiir::arith::FastMathFlags::nnan);
  }
...
MinMaxlocAsElementalConverter<T>::genReductionInitValues(
  ...
  if (!useIsFirst() && honorNans()) {
    ...
    aiir::Value isNotEmpty = genIsNotEmptyArrayExtents(loc, builder, extents);

    llvm::SmallVector<aiir::Value> indices = genFirstElementIndicesForReduction(
        loc, builder, isTotalReduction(), getConstDim(), getSourceRank(),
        oneBasedIndices);

    llvm::SmallVector<aiir::Type> ifTypes(getNumCoors(),
    ...
    llvm::SmallVector<aiir::Value> results(getNumCoors(), one);
    aiir::Value minMaxFirst =
  ...
  llvm::SmallVector<aiir::Value> result(getNumCoors(), zeroCoor);

  // Initial value for the MIN/MAX value.
  aiir::Value minMaxInit =
...
MinMaxlocAsElementalConverter<T>::reduceOneElement(
    const llvm::SmallVectorImpl<aiir::Value> &currentValue, hlfir::Entity array,
    aiir::ValueRange oneBasedIndices) {
  checkReductions(currentValue);
  hlfir::Entity elementValue =
      hlfir::loadElementAt(loc, builder, array, oneBasedIndices);
  aiir::Value cmp = genMinMaxComparison<isMax>(loc, builder, elementValue,
                                               getCurrentMinMax(currentValue));
  if (useIsFirst()) {
    // If isFirst is true, then do the reduction update regardless
    // of the FP comparison.
    cmp =
        aiir::arith::OrIOp::create(builder, loc, cmp, getIsFirst(currentValue));
  }
  ...
  for (unsigned coorIdx = 0; coorIdx < getNumCoors(); ++coorIdx) {
    aiir::Value currentCoor = currentValue[coorIdx];
    aiir::Value newCoor = builder.createConvert(
        loc, currentCoor.getType(), oneBasedIndices[coorIdx + dim - 1]);
    aiir::Value update =
        aiir::arith::SelectOp::create(builder, loc, cmp, newCoor, currentCoor);
    newIndices.push_back(update);
  }

  aiir::Value newMinMax = aiir::arith::SelectOp::create(
      builder, loc, cmp, elementValue, getCurrentMinMax(currentValue));
  newIndices.push_back(newMinMax);

  if (useIsFirst()) {
    aiir::Value newIsFirst = builder.createBool(loc, false);
...
hlfir::Entity MinMaxlocAsElementalConverter<T>::genFinalResult(
    const llvm::SmallVectorImpl<aiir::Value> &reductionResults) {
  ...
  // for this.
  aiir::Value tempArray = builder.createTemporary(
      loc, fir::SequenceType::get(rank, getResultElementType()));
  for (unsigned i = 0; i < rank; ++i) {
    aiir::Value coor = reductionResults[i];
    aiir::Value idx = builder.createIntegerConstant(loc, indexType, i + 1);
    aiir::Value resultElement =
        hlfir::getElementAt(loc, builder, hlfir::Entity{tempArray}, {idx});
    hlfir::AssignOp::create(builder, loc, coor, resultElement);
  }
  aiir::Value tempExpr = hlfir::AsExprOp::create(
...
protected:
  ...
  OpT getOp() const { return aiir::cast<OpT>(op); }

  void checkReductions(const llvm::SmallVectorImpl<aiir::Value> &reductions) {
...
private:
  virtual aiir::LogicalResult isConvertible() const final {
    if (aiir::isa<fir::CharacterType>(this->getSourceElementType()))
      return this->rewriter.notifyMatchFailure(
          this->getOp(),
          "CHARACTER type is not supported for MINVAL/MAXVAL inlining");
    if (auto intType =
            aiir::dyn_cast<aiir::IntegerType>(this->getSourceElementType()))
  ...
  reduceOneElement(const llvm::SmallVectorImpl<aiir::Value> &currentValue,
                   hlfir::Entity array,
                   aiir::ValueRange oneBasedIndices) final {
    ...
    aiir::Location loc = this->loc;
    hlfir::Entity elementValue =
        hlfir::loadElementAt(loc, builder, array, oneBasedIndices);
    aiir::Value currentMinMax = getCurrentMinMax(currentValue);
    if (isUnordered()) {
      result.push_back(
      ...
      if (useIsFirst())
        cmp = aiir::arith::OrIOp::create(builder, loc, cmp,
  ...
  bool honorNans() const {
    return !aiir::arith::bitEnumContainsAny(this->getFastMath(),
  ...
  virtual bool isUnordered() const override {
    if (aiir::isa<aiir::IntegerType>(this->getSourceElementType()))
      return true;
    ...
    return fpMaxminBehavior == Fortran::common::FPMaxminBehavior::Extremum ||
           ...
           (fpMaxminBehavior == Fortran::common::FPMaxminBehavior::Portable &&
            aiir::arith::bitEnumContainsAll(
                this->getFastMath(), aiir::arith::FastMathFlags::nnan |
                                         aiir::arith::FastMathFlags::nsz));
  }

  // Generate arith.max/minsi, arith.max/minimumf or arith.max/minnumf to reduce
  // a single element.
  aiir::Value reduceOneElementUnordered(aiir::Location loc,
                                        fir::FirOpBuilder &builder,
                                        aiir::Value elementValue,
                                        aiir::Value currentMinMax) {
    assert(!useIsFirst() &&
...
MinMaxvalAsElementalConverter<T>::genReductionInitValues(
  ...
  if (!useIsFirst() && honorNans() && !isUnordered()) {
    ...
    ifOp = fir::IfOp::create(builder, loc, this->getResultElementType(),
                             ...
                             /*withElseRegion=*/true);
    builder.setInsertionPointToStart(&ifOp.getThenRegion().front());
    aiir::Value minMaxFirst = hlfir::loadElementAt(
...
public:
  SumAsElementalConverter(hlfir::SumOp op, aiir::PatternRewriter &rewriter)
      : Base{op, rewriter} {}

private:
  virtual llvm::SmallVector<aiir::Value> genReductionInitValues(
      [[maybe_unused]] aiir::ValueRange oneBasedIndices,
      [[maybe_unused]] const llvm::SmallVectorImpl<aiir::Value> &extents)
      final {
    return {
        fir::factory::createZeroValue(builder, loc, getResultElementType())};
  }
  ...
  aiir::Value genScalarAdd(aiir::Value value1, aiir::Value value2);
};
...
private:
  ...
  aiir::Value genScalarMult(aiir::Value value1, aiir::Value value2);
};
...
protected:
  ...
  virtual aiir::Value getSource() const final { return getOp().getMask(); }
  virtual aiir::Value getDim() const final { return getOp().getDim(); }

  virtual hlfir::Entity genFinalResult(
      const llvm::SmallVectorImpl<aiir::Value> &reductionResults) override {
    checkReductions(reductionResults);
    return hlfir::Entity{reductionResults[0]};
  }
};
...
public:
  AllAnyAsElementalConverter(T op, aiir::PatternRewriter &rewriter)
      : Base{op, rewriter} {}
...
private:
  ...
  reduceOneElement(const llvm::SmallVectorImpl<aiir::Value> &currentValue,
    ...
    if constexpr (isAll)
      return {aiir::arith::AndIOp::create(builder, loc, mask, currentValue[0])};
    else
      return {aiir::arith::OrIOp::create(builder, loc, mask, currentValue[0])};
  }

  virtual hlfir::Entity genFinalResult(
      const llvm::SmallVectorImpl<aiir::Value> &reductionValues) final {
...
public:
  CountAsElementalConverter(hlfir::CountOp op, aiir::PatternRewriter &rewriter)
      : Base{op, rewriter} {}
...
private:
  ...
  reduceOneElement(const llvm::SmallVectorImpl<aiir::Value> &currentValue,
    ...
    aiir::Value one =
        builder.createIntegerConstant(loc, getResultElementType(), 1);
    aiir::Value add1 =
        aiir::arith::AddIOp::create(builder, loc, currentValue[0], one);
    return {aiir::arith::SelectOp::create(builder, loc, cond, add1,
                                          currentValue[0])};
  }
};

aiir::LogicalResult ReductionAsElementalConverter::convert() {
  aiir::LogicalResult canConvert(isConvertible());

  if (aiir::failed(canConvert))
    return canConvert;

  hlfir::Entity array = hlfir::Entity{getSource()};
  bool isTotalReduce = isTotalReduction();
  auto dimVal = getConstDim();
  if (aiir::failed(dimVal))
    return dimVal;
  aiir::Value mask = getMask();
  aiir::Value resultShape, dimExtent;
  llvm::SmallVector<aiir::Value> arrayExtents;
  if (isTotalReduce)
  ...
  aiir::Value isPresentPred, maskValue;
  if (mask) {
    if (aiir::isa<fir::BaseBoxType>(mask.getType())) {
  ...
  auto genKernel = [&](aiir::Location loc, fir::FirOpBuilder &builder,
                       aiir::ValueRange inputIndices) -> hlfir::Entity {
    ...
    llvm::SmallVector<aiir::Value, 1> reductionInitValues =
        genReductionInitValues(inputIndices, extents);

    auto genBody = [&](aiir::Location loc, fir::FirOpBuilder &builder,
                       aiir::ValueRange oneBasedIndices,
                       aiir::ValueRange reductionArgs)
        -> llvm::SmallVector<aiir::Value, 1> {
      ...
      llvm::SmallVector<aiir::Value, 1> reductionValues = reductionArgs;
      llvm::SmallVector<aiir::Type, 1> reductionTypes;
      ...
      if (mask) {
        ...
        aiir::Value isUnmasked = fir::ConvertOp::create(
    ...
    llvm::SmallVector<aiir::Value, 1> reductionFinalValues =
  ...
  if (isTotalReduce) {
    hlfir::Entity result = genKernel(loc, builder, aiir::ValueRange{});
...
ReductionAsElementalConverter::genResultShapeForPartialReduction(
  ...
  llvm::SmallVector<aiir::Value> inExtents =
      hlfir::genExtentsVector(loc, builder, array);
  assert(dimVal > 0 && dimVal <= static_cast<int64_t>(inExtents.size()) &&
         "DIM must be present and a positive constant not exceeding "
         "the array's rank");

  aiir::Value dimExtent = inExtents[dimVal - 1];
  inExtents.erase(inExtents.begin() + dimVal - 1);
  return {fir::ShapeOp::create(builder, loc, inExtents), dimExtent};
}

aiir::Value SumAsElementalConverter::genScalarAdd(aiir::Value value1,
                                                  aiir::Value value2) {
  aiir::Type ty = value1.getType();
  ...
  else if (aiir::isa<aiir::ComplexType>(ty))
    return fir::AddcOp::create(builder, loc, value1, value2);
  else if (aiir::isa<aiir::IntegerType>(ty))
    return aiir::arith::AddIOp::create(builder, loc, value1, value2);

  llvm_unreachable("unsupported SUM reduction type");
}

aiir::Value ProductAsElementalConverter::genScalarMult(aiir::Value value1,
                                                       aiir::Value value2) {
  aiir::Type ty = value1.getType();
  assert(ty == value2.getType() && "reduction values' types do not match");
  if (aiir::isa<aiir::FloatType>(ty))
...
aiir::Value ReductionAsElementalConverter::genMaskValue(
    aiir::Value mask, aiir::Value isPresentPred, aiir::ValueRange indices) {
  ...
  if (isPresentPred) {
    ...
    builder.setInsertionPointToStart(&ifOp.getElseRegion().front());
    aiir::Value trueValue = builder.createBool(loc, true);
  ...
  if (maskVar.isScalar()) {
    if (aiir::isa<fir::BaseBoxType>(mask.getType())) {
      ...
      aiir::Value addr = hlfir::genVariableRawAddress(loc, builder, maskVar);
...
class ReductionConversion : public aiir::OpRewritePattern<Op> {
public:
...
/// optionally wrapped into hlfir.elemental.
template <typename Op>
class ExtremumReductionConversion : public aiir::OpRewritePattern<Op> {
public:
  using aiir::OpRewritePattern<Op>::OpRewritePattern;
  ...
  ExtremumReductionConversion(
      ...
      : aiir::OpRewritePattern<Op>{ctx}, fpMaxminBehavior{fpMaxminBehavior} {}
...
template <typename Op>
class ArrayShiftConversion : public aiir::OpRewritePattern<Op> {
public:
  ...
  matchAndRewrite(Op op, aiir::PatternRewriter &rewriter) const override {
    ...
    hlfir::ExprType expr = aiir::dyn_cast<hlfir::ExprType>(op.getType());
    assert(expr &&
    ...
    if constexpr (std::is_same_v<Op, hlfir::EOShiftOp>) {
      ...
      if (aiir::isa<fir::CharacterType>(expr.getEleTy())) {
    ...
    aiir::Type elementType = array.getFortranElementType();
...
private:
  ...
  static aiir::Value normalizeShiftValue(aiir::Location loc,
                                         fir::FirOpBuilder &builder,
                                         aiir::Value shiftVal,
                                         aiir::Value extent,
                                         aiir::Type calcType) {
    ...
    aiir::Value zero = builder.createIntegerConstant(loc, calcType, 0);
    aiir::Value one = builder.createIntegerConstant(loc, calcType, 1);
    aiir::Value isZero = aiir::arith::CmpIOp::create(
        builder, loc, aiir::arith::CmpIPredicate::eq, extent, zero);
    extent = aiir::arith::SelectOp::create(builder, loc, isZero, one, extent);
  ...
  /// to generate AIIR where we have to select between the default
  ...
  static aiir::Type getDefaultBoundaryValueType(aiir::Type elementType) {
    ...
    if (auto charTy = aiir::dyn_cast<fir::CharacterType>(elementType))
      return fir::BoxCharType::get(charTy.getContext(), charTy.getFKind());

    if (aiir::isa<fir::LogicalType>(elementType) ||
  ...
  /// of F2023.
  static aiir::Value genDefaultBoundary(aiir::Location loc,
                                        fir::FirOpBuilder &builder,
    ...
    if (aiir::isa<fir::CharacterType>(elementType)) {
      ...
      aiir::Value zeroLen = builder.createIntegerConstant(
  ...
  static aiir::Value loadEoshiftVal(aiir::Location loc,
                                    fir::FirOpBuilder &builder,
                                    hlfir::Entity entity,
                                    aiir::ValueRange indices = {}) {
    ...
    if (boundaryVal.isVariable() && boundaryValTy &&
        aiir::isa<fir::BoxCharType>(boundaryValTy))
  ...
  [[maybe_unused]] static std::pair<aiir::Value, aiir::Value>
  genScalarBoundaryForEOShift(aiir::Location loc, fir::FirOpBuilder &builder,
                              hlfir::EOShiftOp op) {
    ...
    if (!op.getBoundary()) {
      ...
      aiir::Value defaultVal = genDefaultBoundary(loc, builder, elementType);
      aiir::Value boundaryIsScalarPred = builder.createBool(loc, true);
      return {defaultVal, boundaryIsScalarPred};
    }

    hlfir::Entity boundary{op.getBoundary()};
    aiir::Type boundaryValTy = getDefaultBoundaryValueType(elementType);

    if (boundary.isScalar()) {
      if (!boundaryValTy || !boundary.mayBeOptional()) {
        // The boundary must be present.
        aiir::Value boundaryVal = loadEoshiftVal(loc, builder, boundary);
        aiir::Value boundaryIsScalarPred = builder.createBool(loc, true);
      ...
      // value.
      assert(aiir::isa<fir::BaseBoxType>(boundary.getType()));
      aiir::Value isPresentPred =
          fir::IsPresentOp::create(builder, loc, builder.getI1Type(), boundary);
      aiir::Value boundaryVal =
          builder
    ...
    if (!boundaryValTy || !boundary.mayBeOptional()) {
      // The boundary must be present
      aiir::Value boundaryIsScalarPred = builder.createBool(loc, false);
    ...
    aiir::Value trueVal = builder.createBool(loc, true);
    aiir::Value falseVal = builder.createBool(loc, false);
    aiir::Value boundaryIsScalarPred = aiir::arith::SelectOp::create(
  ...
  [[maybe_unused]] static aiir::Value selectBoundaryValue(
      aiir::Location loc, fir::FirOpBuilder &builder, hlfir::EOShiftOp op,
      aiir::Value precomputedScalarBoundary, aiir::Value boundaryIsScalarPred,
      aiir::ValueRange oneBasedIndices) {
    ...
    aiir::Type boundaryValTy = precomputedScalarBoundary.getType();
    aiir::Value newBoundaryVal =
        builder
  ...
  genElementalArrayShift(aiir::PatternRewriter &rewriter, Op op,
                         int64_t dimVal) {
    ...
    aiir::Type calcType = builder.getI64Type();
    // Set the indices arithmetic overflow flags.
    setArithOverflowFlags(op, builder);

    aiir::Value arrayShape = hlfir::genShape(loc, builder, array);
    llvm::SmallVector<aiir::Value, maxRank> arrayExtents =
        hlfir::getExplicitExtentsFromShape(arrayShape, builder);
    llvm::SmallVector<aiir::Value, 1> typeParams;
    hlfir::genLengthParameters(loc, builder, array, typeParams);
    aiir::Value shiftDimExtent =
    ...
    aiir::Value boundaryVal, boundaryIsScalarPred;
    if constexpr (std::is_same_v<Op, hlfir::EOShiftOp>)
    ...
    auto genKernel = [&](aiir::Location loc, fir::FirOpBuilder &builder,
                         aiir::ValueRange inputIndices) -> hlfir::Entity {
      llvm::SmallVector<aiir::Value, maxRank> indices{inputIndices};
      if (!shiftVal) {
        ...
        llvm::SmallVector<aiir::Value, maxRank> shiftIndices{indices};
      ...
      if constexpr (std::is_same_v<Op, hlfir::EOShiftOp>) {
        llvm::SmallVector<aiir::Value, maxRank> boundaryIndices{indices};
      ...
      if constexpr (std::is_same_v<Op, hlfir::EOShiftOp>) {
        ...
        aiir::arith::IntegerOverflowFlags savedFlags =
            builder.getIntegerOverflowFlags();
        builder.setIntegerOverflowFlags(aiir::arith::IntegerOverflowFlags::nsw);
        aiir::Value indexPlusShift =
            aiir::arith::AddIOp::create(builder, loc, index, shiftVal);
        builder.setIntegerOverflowFlags(savedFlags);
        aiir::Value one = builder.createIntegerConstant(loc, calcType, 1);
        aiir::Value cmp1 = aiir::arith::CmpIOp::create(
            builder, loc, aiir::arith::CmpIPredicate::sge, indexPlusShift, one);
        aiir::Value cmp2 = aiir::arith::CmpIOp::create(
            builder, loc, aiir::arith::CmpIPredicate::sle, indexPlusShift,
            shiftDimExtent);
        aiir::Value loadFromArray =
            aiir::arith::AndIOp::create(builder, loc, cmp1, cmp2);
        aiir::Type boundaryValTy = boundaryVal.getType();
        aiir::Value result =
            builder
        ...
        // in LLVM.
        aiir::Value wrapBound =
            aiir::arith::SubIOp::create(builder, loc, shiftDimExtent, shiftVal);
        aiir::Value adjustedShiftVal =
            aiir::arith::SubIOp::create(builder, loc, shiftVal, shiftDimExtent);
        aiir::Value index =
            builder.createConvert(loc, calcType, inputIndices[dimVal - 1]);
        aiir::Value wrapCheck = aiir::arith::CmpIOp::create(
            builder, loc, aiir::arith::CmpIPredicate::sle, index, wrapBound);
        aiir::Value actualShift = aiir::arith::SelectOp::create(
            builder, loc, wrapCheck, shiftVal, adjustedShiftVal);
        aiir::Value newIndex =
            aiir::arith::AddIOp::create(builder, loc, index, actualShift);
    ...
    hlfir::ElementalOp elementalOp = hlfir::genElementalOp(
        ...
        /*isUnordered=*/true,
        array.isPolymorphic() ? static_cast<aiir::Value>(array) : nullptr,
  ...
  /// loop idioms in LLVM.
  static aiir::Operation *genInMemArrayShift(aiir::PatternRewriter &rewriter,
    ...
    hlfir::EvaluateInMemoryOp evalOp = hlfir::EvaluateInMemoryOp::create(
        builder, loc, aiir::cast<hlfir::ExprType>(op.getType()), arrayShape);
    builder.setInsertionPointToStart(&evalOp.getBody().front());

    aiir::Value resultArray = evalOp.getMemory();
    aiir::Type arrayType = fir::dyn_cast_ptrEleTy(resultArray.getType());
    ...
    auto genDimensionShift =
        [&](aiir::Location loc, fir::FirOpBuilder &builder,
            aiir::Value shiftVal, [[maybe_unused]] aiir::Value boundary,
            bool exposeContiguity, aiir::ValueRange oneBasedIndices)
        -> llvm::SmallVector<aiir::Value, 0> {
      // Create a vector of indices (s(1), ..., s(dim-1), nullptr, s(dim+1),
      // ..., s(n)) so that we can update the dimVal index as needed.
      llvm::SmallVector<aiir::Value, maxRank> srcIndices(
      ...
      srcIndices.append(oneBasedIndices.begin() + (dimVal - 1),
                        oneBasedIndices.end());
      llvm::SmallVector<aiir::Value, maxRank> dstIndices(srcIndices);

      hlfir::Entity srcArray = array;
      if (exposeContiguity && aiir::isa<fir::BaseBoxType>(srcArray.getType())) {
        ...
        aiir::Value addr = hlfir::genVariableRawAddress(loc, builder, section);
        aiir::Value shape = hlfir::genShape(loc, builder, section);
      ...
      aiir::Value dstOffset, srcOffset;
      auto genCopy = [&](aiir::Location loc, fir::FirOpBuilder &builder,
                         aiir::ValueRange index, aiir::ValueRange reductionArgs)
          -> llvm::SmallVector<aiir::Value, 0> {
        assert(index.size() == 1 && "expected single loop");
        aiir::Value srcIndex = builder.createConvert(loc, calcType, index[0]);
        aiir::Value dstIndex = srcIndex;
        if (srcOffset)
          srcIndex =
              aiir::arith::AddIOp::create(builder, loc, srcIndex, srcOffset);
        srcIndices[dimVal - 1] = srcIndex;
        hlfir::Entity srcElementValue =
            hlfir::loadElementAt(loc, builder, srcArray, srcIndices);
        if (dstOffset)
          dstIndex =
              aiir::arith::AddIOp::create(builder, loc, dstIndex, dstOffset);
      ...
      if constexpr (std::is_same_v<Op, hlfir::CShiftOp>) {
        ...
        aiir::Value zero = builder.createIntegerConstant(loc, calcType, 0);
        aiir::Value isNegativeShift = aiir::arith::CmpIOp::create(
            builder, loc, aiir::arith::CmpIPredicate::slt, shiftVal, zero);
        aiir::Value shiftNeg =
            aiir::arith::SubIOp::create(builder, loc, zero, shiftVal);
        dstOffset = aiir::arith::SelectOp::create(builder, loc, isNegativeShift,
                                                  shiftNeg, zero);
        srcOffset = aiir::arith::SelectOp::create(builder, loc, isNegativeShift,
                                                  zero, shiftVal);
        aiir::Value extentNeg =
            aiir::arith::SubIOp::create(builder, loc, zero, shiftDimExtent);
        aiir::Value extentPlusShift =
            aiir::arith::AddIOp::create(builder, loc, shiftDimExtent, shiftVal);
        aiir::Value extentNegShiftCmp = aiir::arith::CmpIOp::create(
            builder, loc, aiir::arith::CmpIPredicate::sgt, extentNeg, shiftVal);
        aiir::Value negativeShiftBound = aiir::arith::SelectOp::create(
            builder, loc, extentNegShiftCmp, zero, extentPlusShift);
        aiir::Value extentMinusShift =
            aiir::arith::SubIOp::create(builder, loc, shiftDimExtent, shiftVal);
        aiir::Value extentShiftCmp = aiir::arith::CmpIOp::create(
            builder, loc, aiir::arith::CmpIPredicate::slt, shiftDimExtent,
            shiftVal);
        aiir::Value positiveShiftBound = aiir::arith::SelectOp::create(
            builder, loc, extentShiftCmp, zero, extentMinusShift);
        aiir::Value copyEnd = aiir::arith::SelectOp::create(
        ...
        aiir::Value initEnd =
            aiir::arith::SubIOp::create(builder, loc, shiftDimExtent, copyEnd);
        aiir::Value initOffset = aiir::arith::SelectOp::create(
            builder, loc, isNegativeShift, zero, copyEnd);
        auto genInit = [&](aiir::Location loc, fir::FirOpBuilder &builder,
                           aiir::ValueRange index,
                           aiir::ValueRange reductionArgs)
            -> llvm::SmallVector<aiir::Value, 0> {
          aiir::Value dstIndex = builder.createConvert(loc, calcType, index[0]);
          dstIndex =
              aiir::arith::AddIOp::create(builder, loc, dstIndex, initOffset);
          dstIndices[dimVal - 1] = dstIndex;
          hlfir::Entity dstElement = hlfir::getElementAt(
              loc, builder, hlfir::Entity{resultArray}, dstIndices);
    ...
    auto genShiftBody = [&](aiir::Location loc, fir::FirOpBuilder &builder,
                            aiir::ValueRange oneBasedIndices,
                            aiir::ValueRange reductionArgs)
        -> llvm::SmallVector<aiir::Value, 0> {
      ...
      aiir::Value elemSize;
      aiir::Value stride;
      if (dimVal == 1 && aiir::isa<fir::BaseBoxType>(array.getType())) {
        aiir::Type indexType = builder.getIndexType();
        elemSize =
      ...
      aiir::Value isContiguous = aiir::arith::CmpIOp::create(
          builder, loc, aiir::arith::CmpIPredicate::eq, elemSize, stride);
    ...
    llvm::SmallVector<aiir::Value, maxRank> newExtents(arrayExtents);
...
class CmpCharOpConversion : public aiir::OpRewritePattern<hlfir::CmpCharOp> {
public:
  using aiir::OpRewritePattern<hlfir::CmpCharOp>::OpRewritePattern;
  ...
  matchAndRewrite(hlfir::CmpCharOp cmp,
    ...
    auto toVariable =
        [&builder,
         &loc](aiir::Value val) -> std::pair<aiir::Value, hlfir::AssociateOp> {
      aiir::Value opnd;
      hlfir::AssociateOp associate;
      if (aiir::isa<hlfir::ExprType>(val.getType())) {
    ...
    auto charTy = aiir::cast<fir::CharacterType>(lhs.getFortranElementType());
    ...
    auto genExtractAndConvertToInt =
        [&idxAttr, &intTy, &designatorType](
            aiir::Location loc, fir::FirOpBuilder &builder,
            hlfir::Entity &charStr, aiir::Value index, aiir::Value length) {
          auto singleChr = hlfir::DesignateOp::create(
              ...
              /*substring=*/aiir::ValueRange{index, index},
              /*complexPart=*/std::nullopt,
              /*shape=*/aiir::Value{}, /*typeParams=*/aiir::ValueRange{length},
              fir::FortranVariableFlagsAttr{});
          auto chrVal = fir::LoadOp::create(builder, loc, singleChr);
          aiir::Value intVal = fir::ExtractValueOp::create(
              builder, loc, intTy, chrVal, builder.getArrayAttr(idxAttr));
          return intVal;
        };

    aiir::arith::CmpIPredicate predicate = cmp.getPredicate();
    aiir::Value oneIdx = builder.createIntegerConstant(loc, idxTy, 1);

    aiir::Value lhsLen = builder.createConvert(
        loc, idxTy, hlfir::genCharLength(loc, builder, lhs));
    aiir::Value rhsLen = builder.createConvert(
        loc, idxTy, hlfir::genCharLength(loc, builder, rhs));

    enum class GenCmp { LeftToRight, LeftToBlank, BlankToRight };

    aiir::Value zeroInt = builder.createIntegerConstant(loc, intTy, 0);
    aiir::Value oneInt = builder.createIntegerConstant(loc, intTy, 1);
    aiir::Value negOneInt = builder.createIntegerConstant(loc, intTy, -1);
    aiir::Value blankInt = builder.createIntegerConstant(loc, intTy, ' ');

    auto step = GenCmp::LeftToRight;
    auto genCmp = [&](aiir::Location loc, fir::FirOpBuilder &builder,
                      aiir::ValueRange index, aiir::ValueRange reductionArgs)
        -> llvm::SmallVector<aiir::Value, 1> {
      assert(index.size() == 1 && "expected single loop");
      assert(reductionArgs.size() == 1 && "expected single reduction value");
      aiir::Value inRes = reductionArgs[0];
      auto accEQzero = aiir::arith::CmpIOp::create(
          builder, loc, aiir::arith::CmpIPredicate::eq, inRes, zeroInt);
          ...
          builder
              ...
              .genThen([&]() {
                ...
                if (step == GenCmp::LeftToRight) {
                  ...
                  // lhsLen > rhsLen
                  offset =
                      aiir::arith::AddIOp::create(builder, loc, rhsLen, offset);
                  ...
                  // rhsLen > lhsLen
                  offset =
                      aiir::arith::AddIOp::create(builder, loc, lhsLen, offset);
                ...
                aiir::Value newVal = aiir::arith::SelectOp::create(
                    builder, loc,
                    aiir::arith::CmpIOp::create(builder, loc,
                                                aiir::arith::CmpIPredicate::ult,
                                                lhsInt, rhsInt),
                    negOneInt, inRes);
                newVal = aiir::arith::SelectOp::create(
                    builder, loc,
                    aiir::arith::CmpIOp::create(builder, loc,
                                                aiir::arith::CmpIPredicate::ugt,
                                                lhsInt, rhsInt),
                    oneInt, newVal);
    ...
    // one.
    aiir::Value minLen = aiir::arith::SelectOp::create(
        builder, loc,
        aiir::arith::CmpIOp::create(
            builder, loc, aiir::arith::CmpIPredicate::slt, lhsLen, rhsLen),
        lhsLen, rhsLen);

    llvm::SmallVector<aiir::Value, 1> loopOut =
        hlfir::genLoopNestWithReductions(loc, builder, {minLen},
                                         /*reductionInits=*/{zeroInt}, genCmp,
                                         /*isUnordered=*/false);
    aiir::Value partRes = loopOut[0];

    auto lhsLonger = aiir::arith::CmpIOp::create(
        builder, loc, aiir::arith::CmpIPredicate::sgt, lhsLen, rhsLen);
    aiir::Value tempRes =
        builder
            ...
            .genThen([&]() {
              ...
              auto lenDiff =
                  aiir::arith::SubIOp::create(builder, loc, lhsLen, rhsLen);

              llvm::SmallVector<aiir::Value, 1> output =
                  hlfir::genLoopNestWithReductions(loc, builder, {lenDiff},
                                                   /*reductionInits=*/{partRes},
                                                   genCmp,
                                                   /*isUnordered=*/false);
              aiir::Value res = output[0];
            ...
            .genElse([&]() {
              ...
              auto lenDiff =
                  aiir::arith::SubIOp::create(builder, loc, rhsLen, lhsLen);
              llvm::SmallVector<aiir::Value, 1> output =
                  hlfir::genLoopNestWithReductions(loc, builder, {lenDiff},
                                                   /*reductionInits=*/{partRes},
                                                   genCmp,
    ...
    auto finalCmpResult =
        aiir::arith::CmpIOp::create(builder, loc, predicate, tempRes, zeroInt);
    rewriter.replaceOp(cmp, finalCmpResult);
    return aiir::success();
  }
};

static std::pair<aiir::Value, hlfir::AssociateOp>
getVariable(fir::FirOpBuilder &builder, aiir::Location loc, aiir::Value val) {
...
class IndexOpConversion : public aiir::OpRewritePattern<hlfir::IndexOp> {
public:
  using aiir::OpRewritePattern<hlfir::IndexOp>::OpRewritePattern;
  ...
  matchAndRewrite(hlfir::IndexOp op,
    ...
    if (*substrLenCst == 0) {
      ...
      aiir::Value strLen = hlfir::genCharLength(loc, builder, strEntity);
      aiir::Value strEnd = aiir::arith::AddIOp::create(
      ...
      aiir::Value result =
          aiir::arith::SelectOp::create(builder, loc, back, strEnd, oneIdx);
    ...
    auto charTy = aiir::cast<fir::CharacterType>(
    ...
    auto genExtractAndConvertToInt = [&charTy, &idxTy, &oneIdx,
      ...
      auto singleChr = hlfir::DesignateOp::create(
          ...
          /*substring=*/aiir::ValueRange{index, index},
          /*complexPart=*/std::nullopt,
          /*shape=*/aiir::Value{}, /*typeParams=*/aiir::ValueRange{oneIdx},
          fir::FortranVariableFlagsAttr{});
      auto chrVal = fir::LoadOp::create(builder, loc, singleChr);
      aiir::Value intVal = fir::ExtractValueOp::create(
          builder, loc, intTy, chrVal, builder.getArrayAttr(idxAttr));
    ...
    auto genSearchBody = [&](aiir::Location loc, fir::FirOpBuilder &builder,
                             aiir::ValueRange index,
                             aiir::ValueRange reductionArgs)
        -> llvm::SmallVector<aiir::Value, 1> {
      assert(index.size() == 1 && "expected single loop");
      assert(reductionArgs.size() == 1 && "expected single reduction value");
      aiir::Value inRes = reductionArgs[0];
      auto resEQzero = aiir::arith::CmpIOp::create(
          builder, loc, aiir::arith::CmpIPredicate::eq, inRes, zeroIdx);
      ...
      aiir::Value res =
          builder
              .genIfOp(loc, {idxTy}, resEQzero,
                       /*withElseRegion=*/true)
              .genThen([&]() {
                aiir::Value idx = builder.createConvert(loc, idxTy, index[0]);
                // offset = back ? end - idx : idx;
                aiir::Value offset = aiir::arith::SelectOp::create(
                    builder, loc, back,
                    aiir::arith::SubIOp::create(builder, loc, strEnd, idx),
                    idx);

                auto haveChar =
                    genExtractAndConvertToInt(loc, builder, str, offset);
                auto charsEQ = aiir::arith::CmpIOp::create(
                    builder, loc, aiir::arith::CmpIPredicate::eq, haveChar,
                    wantChar);
                aiir::Value newVal = aiir::arith::SelectOp::create(
    ...
    llvm::SmallVector<aiir::Value, 1> loopOut =
        hlfir::genLoopNestWithReductions(loc, builder, {strLen},
                                         ...
                                         genSearchBody,
                                         /*isUnordered=*/false);
    aiir::Value result = builder.createConvert(loc, resultTy, loopOut[0]);
...
template <typename Op>
class MatmulConversion : public aiir::OpRewritePattern<Op> {
public:
  using aiir::OpRewritePattern<Op>::OpRewritePattern;

  llvm::LogicalResult
  matchAndRewrite(Op matmul, aiir::PatternRewriter &rewriter) const override {
    aiir::Location loc = matmul.getLoc();
    ...
    aiir::Value resultShape, innerProductExtent;
    ...
    if (forceMatmulAsElemental || isMatmulTranspose) {
      ...
      hlfir::ExprType resultType =
          aiir::cast<hlfir::ExprType>(matmul.getType());
    ...
    hlfir::EvaluateInMemoryOp evalOp = hlfir::EvaluateInMemoryOp::create(
        builder, loc, aiir::cast<hlfir::ExprType>(matmul.getType()),
        resultShape);
    builder.setInsertionPointToStart(&evalOp.getBody().front());
...
private:
  ...
  static std::tuple<aiir::Value, aiir::Value>
  genResultShape(aiir::Location loc, fir::FirOpBuilder &builder,
                 hlfir::Entity input1, hlfir::Entity input2) {
    llvm::SmallVector<aiir::Value, 2> input1Extents =
        hlfir::genExtentsVector(loc, builder, input1);
    llvm::SmallVector<aiir::Value, 2> input2Extents =
        hlfir::genExtentsVector(loc, builder, input2);

    llvm::SmallVector<aiir::Value, 2> newExtents;
    aiir::Value innerProduct1Extent, innerProduct2Extent;
    ...
    // optimizations.
    llvm::SmallVector<aiir::Value> innerProductExtent =
  ...
  genContiguousMatmul(aiir::Location loc, fir::FirOpBuilder &builder,
                      hlfir::Entity result, aiir::Value resultShape,
                      hlfir::Entity lhs, hlfir::Entity rhs,
                      aiir::Value innerProductExtent) {
    ...
    aiir::OpBuilder::InsertionGuard guard(builder);
    aiir::Type resultElementType = result.getFortranElementType();
    llvm::SmallVector<aiir::Value, 2> resultExtents =
        aiir::cast<fir::ShapeOp>(resultShape.getDefiningOp()).getExtents();
    ...
    // only to one.
    bool isUnordered = aiir::isa<aiir::IntegerType>(resultElementType) ||
                       aiir::isa<fir::LogicalType>(resultElementType) ||
    ...
    auto genInitBody = [&](aiir::Location loc, fir::FirOpBuilder &builder,
                           aiir::ValueRange oneBasedIndices,
                           aiir::ValueRange reductionArgs)
        -> llvm::SmallVector<aiir::Value, 0> {
    ...
    if (lhs.getRank() == 2 && rhs.getRank() == 2) {
      ...
      auto genMatrixMatrix = [&](aiir::Location loc, fir::FirOpBuilder &builder,
                                 aiir::ValueRange oneBasedIndices,
                                 aiir::ValueRange reductionArgs)
          -> llvm::SmallVector<aiir::Value, 0> {
        aiir::Value I = oneBasedIndices[0];
        aiir::Value J = oneBasedIndices[1];
        aiir::Value K = oneBasedIndices[2];
        hlfir::Entity resultElement =
    ...
    if (lhs.getRank() == 2 && rhs.getRank() == 1) {
      ...
      auto genMatrixVector = [&](aiir::Location loc, fir::FirOpBuilder &builder,
                                 aiir::ValueRange oneBasedIndices,
                                 aiir::ValueRange reductionArgs)
          -> llvm::SmallVector<aiir::Value, 0> {
        aiir::Value J = oneBasedIndices[0];
        aiir::Value K = oneBasedIndices[1];
        hlfir::Entity resultElement =
    ...
    if (lhs.getRank() == 1 && rhs.getRank() == 2) {
      ...
      auto genVectorMatrix = [&](aiir::Location loc, fir::FirOpBuilder &builder,
                                 aiir::ValueRange oneBasedIndices,
                                 aiir::ValueRange reductionArgs)
          -> llvm::SmallVector<aiir::Value, 0> {
        aiir::Value J = oneBasedIndices[0];
  ...
  genElementalMatmul(aiir::Location loc, fir::FirOpBuilder &builder,
                     hlfir::ExprType resultType, aiir::Value resultShape,
                     hlfir::Entity lhs, hlfir::Entity rhs,
                     aiir::Value innerProductExtent) {
    aiir::OpBuilder::InsertionGuard guard(builder);
    aiir::Type resultElementType = resultType.getElementType();
    auto genKernel = [&](aiir::Location loc, fir::FirOpBuilder &builder,
                         aiir::ValueRange resultIndices) -> hlfir::Entity {
      aiir::Value initValue =
          fir::factory::createZeroValue(builder, loc, resultElementType);
      // The inner product loop may be unordered if FastMathFlags::reassoc
      // transformations are allowed. The integer/logical inner product is
      ...
      auto genBody = [&](aiir::Location loc, fir::FirOpBuilder &builder,
                         ...
                         aiir::ValueRange reductionArgs)
          -> llvm::SmallVector<aiir::Value, 1> {
        llvm::SmallVector<aiir::Value, 2> lhsIndices;
        llvm::SmallVector<aiir::Value, 2> rhsIndices;
        // MATMUL:
        //   LHS(NROWS,N) * RHS(N,NCOLS) -> RESULT(NROWS,NCOLS)
        //   LHS(NROWS,N) * RHS(N) -> RESULT(NROWS)
        //   LHS(N) * RHS(N,NCOLS) -> RESULT(NCOLS)
      ...
      llvm::SmallVector<aiir::Value, 1> innerProductValue =
...
class DotProductConversion
    : public aiir::OpRewritePattern<hlfir::DotProductOp> {
public:
  using aiir::OpRewritePattern<hlfir::DotProductOp>::OpRewritePattern;
  ...
  matchAndRewrite(hlfir::DotProductOp product,
    ...
    aiir::Location loc = product.getLoc();
    ...
    aiir::Type resultElementType = product.getType();
    bool isUnordered = aiir::isa<aiir::IntegerType>(resultElementType) ||
                       aiir::isa<fir::LogicalType>(resultElementType) ||
                       static_cast<bool>(builder.getFastMathFlags() &
                                         aiir::arith::FastMathFlags::reassoc);

    aiir::Value extent = genProductExtent(loc, builder, lhs, rhs);
    ...
    llvm::SmallVector<aiir::Value, 1> result = hlfir::genLoopNestWithReductions(
        loc, builder, {extent},
...
private:
  static aiir::Value genProductExtent(aiir::Location loc,
                                      fir::FirOpBuilder &builder,
                                      hlfir::Entity input1,
                                      hlfir::Entity input2) {
    llvm::SmallVector<aiir::Value, 1> input1Extents =
        hlfir::genExtentsVector(loc, builder, input1);
    llvm::SmallVector<aiir::Value, 1> input2Extents =
        hlfir::genExtentsVector(loc, builder, input2);

    assert(input1Extents.size() == 1 && input2Extents.size() == 1 &&
           "hlfir.dot_product arguments must be vectors");
    llvm::SmallVector<aiir::Value, 1> extent =
...
class ReshapeAsElementalConversion
    : public aiir::OpRewritePattern<hlfir::ReshapeOp> {
public:
  using aiir::OpRewritePattern<hlfir::ReshapeOp>::OpRewritePattern;
  ...
  matchAndRewrite(hlfir::ReshapeOp reshape,
    ...
    auto genKernel = [&](aiir::Location loc, fir::FirOpBuilder &builder,
                         aiir::ValueRange inputIndices) -> hlfir::Entity {
      aiir::Value linearIndex =
          computeLinearIndex(loc, builder, resultExtents, inputIndices);
      fir::IfOp ifOp;
      if (pad) {
        // PAD is present. Check if this element comes from the PAD array.
        aiir::Value isInsideArray = aiir::arith::CmpIOp::create(
            builder, loc, aiir::arith::CmpIPredicate::ult, linearIndex,
            arraySize);
        ...
        llvm::SmallVector<aiir::Value, Fortran::common::maxRank> padExtents =
        ...
        aiir::Value padLinearIndex =
            aiir::arith::SubIOp::create(builder, loc, linearIndex, arraySize);
        llvm::SmallVector<aiir::Value, Fortran::common::maxRank> padIndices =
            delinearizeIndex(loc, builder, padExtents, padLinearIndex,
                             /*wrapAround=*/true);
        aiir::Value padElement =
      ...
      llvm::SmallVector<aiir::Value, Fortran::common::maxRank> arrayIndices =
    ...
    hlfir::ElementalOp elementalOp = hlfir::genElementalOp(
        loc, builder, elementType, resultShape, typeParams, genKernel,
        /*isUnordered=*/true,
        /*polymorphicMold=*/result.isPolymorphic() ? array : aiir::Value{},
...
private:
  ...
  static aiir::Value computeLinearIndex(aiir::Location loc,
                                        fir::FirOpBuilder &builder,
    ...
    aiir::Value one = builder.createIntegerConstant(loc, indexType, 1);
    aiir::Value linearIndex = zero;
    std::size_t idx = 0;
    for (auto index : llvm::reverse(indices)) {
      aiir::Value tmp = aiir::arith::SubIOp::create(
          builder, loc, builder.createConvert(loc, indexType, index), one);
      tmp = aiir::arith::AddIOp::create(builder, loc, linearIndex, tmp);
      if (idx + 1 < rank)
        tmp = aiir::arith::MulIOp::create(
            builder, loc, tmp,
  ...
  ///     in := linearIndex + 1
  ///   }
  static llvm::SmallVector<aiir::Value, Fortran::common::maxRank>
  delinearizeIndex(aiir::Location loc, fir::FirOpBuilder &builder,
                   aiir::ValueRange extents, aiir::Value linearIndex,
                   bool wrapAround) {
    ...
    for (std::size_t dim = 0; dim < extents.size(); ++dim) {
      aiir::Value extent = builder.createConvert(loc, indexType, extents[dim]);
      // Avoid the modulo for the last index, unless wrap around is allowed.
      aiir::Value currentIndex = linearIndex;
      if (dim != extents.size() - 1 || wrapAround)
        currentIndex =
            aiir::arith::RemUIOp::create(builder, loc, linearIndex, extent);
      // The result of the last division is unused, so it will be DCEd.
      linearIndex =
          aiir::arith::DivUIOp::create(builder, loc, linearIndex, extent);
      indices.push_back(
          aiir::arith::AddIOp::create(builder, loc, currentIndex, one));
    }
    return indices;
  }

  /// Return size of an array given its extents.
  static aiir::Value computeArraySize(aiir::Location loc,
                                      fir::FirOpBuilder &builder,
                                      aiir::ValueRange extents) {
    aiir::Type indexType = builder.getIndexType();
    aiir::Value size = builder.createIntegerConstant(loc, indexType, 1);
    for (auto extent : extents)
      size = aiir::arith::MulIOp::create(
...
public:
  ...
  void runOnOperation() override {
    ...
    // AIIR CSE won't be able to optimize the trivial loads of 'N' value
