...
class SimplifyIntrinsicsPass
  ...
  using FunctionTypeGeneratorTy =
      llvm::function_ref<aiir::FunctionType(fir::FirOpBuilder &)>;
  using FunctionBodyGeneratorTy =
      llvm::function_ref<void(fir::FirOpBuilder &, aiir::func::FuncOp &)>;
  using GenReductionBodyTy = llvm::function_ref<void(
      fir::FirOpBuilder &builder, aiir::func::FuncOp &funcOp, unsigned rank,
...
public:
  ...
  aiir::func::FuncOp getOrCreateFunction(fir::FirOpBuilder &builder,
                                         const aiir::StringRef &basename,
...
getSimplificationBuilder(aiir::Operation *op, const fir::KindMapping &kindMap) {
...
static aiir::FunctionType genNoneBoxType(fir::FirOpBuilder &builder,
                                         const aiir::Type &elementType) {
  aiir::Type boxType = fir::BoxType::get(builder.getNoneType());
  return aiir::FunctionType::get(builder.getContext(), {boxType},
                                 {elementType});
}

template <typename Op>
Op expectOp(aiir::Value val) {
  if (Op op = aiir::dyn_cast_or_null<Op>(val.getDefiningOp()))
    return op;
...
template <typename Op>
static aiir::Value findDefSingle(fir::ConvertOp op) {
...
static aiir::Value findDef(fir::ConvertOp op) {
  aiir::Value defOp;
...
static bool isOperandAbsent(aiir::Value val) {
  if (auto op = expectOp<fir::ConvertOp>(val)) {
    assert(op->getOperands().size() != 0);
    return aiir::isa_and_nonnull<fir::AbsentOp>(
        op->getOperand(0).getDefiningOp());
  }
  return false;
}

static bool isTrueOrNotConstant(aiir::Value val) {
  if (auto op = expectOp<aiir::arith::ConstantOp>(val)) {
    return !aiir::matchPattern(val, aiir::m_Zero());
  }
  return true;
}

static bool isZero(aiir::Value val) {
  if (auto op = expectOp<fir::ConvertOp>(val)) {
    assert(op->getOperands().size() != 0);
    if (aiir::Operation *defOp = op->getOperand(0).getDefiningOp())
      return aiir::matchPattern(defOp, aiir::m_Zero());
  }
  return false;
}

static aiir::Value findBoxDef(aiir::Value val) {
  if (auto op = expectOp<fir::ConvertOp>(val)) {
    assert(op->getOperands().size() != 0);
...
static aiir::Value findMaskDef(aiir::Value val) {
  if (auto op = expectOp<fir::ConvertOp>(val)) {
    assert(op->getOperands().size() != 0);
...
static unsigned getDimCount(aiir::Value val) {
  ...
  if (aiir::Value emboxVal = findBoxDef(val))
    if (auto boxTy = aiir::dyn_cast<fir::BoxType>(emboxVal.getType()))
...
static std::optional<aiir::Type> getArgElementType(aiir::Value val) {
  aiir::Operation *defOp;
  do {
    defOp = val.getDefiningOp();
    // Analyze only sequences of convert operations.
    if (!aiir::isa<fir::ConvertOp>(defOp))
      return std::nullopt;
    val = defOp->getOperand(0);
    // The convert operation is expected to convert from one
    // box type to another box type.
    auto boxType = aiir::cast<fir::BoxType>(val.getType());
    auto elementType = fir::unwrapSeqOrBoxedSeqType(boxType);
    if (!aiir::isa<aiir::NoneType>(elementType))
      return elementType;
  } while (true);
}

using BodyOpGeneratorTy = llvm::function_ref<aiir::Value(
    fir::FirOpBuilder &, aiir::Location, const aiir::Type &, aiir::Value,
    aiir::Value)>;
using ContinueLoopGenTy = llvm::function_ref<llvm::SmallVector<aiir::Value>(
    fir::FirOpBuilder &, aiir::Location, aiir::Value)>;
...
///    aiir constant, and controls the inital value for while loops
///    or if DoLoop is ordered/unordered.

template <typename OP, typename T, int resultIndex>
static void
genReductionLoop(fir::FirOpBuilder &builder, aiir::func::FuncOp &funcOp,
                 fir::InitValGeneratorTy initVal, ContinueLoopGenTy loopCond,
                 T unorderedOrInitialLoopCond, BodyOpGeneratorTy genBody,
                 unsigned rank, aiir::Type elementType, aiir::Location loc) {

  aiir::IndexType idxTy = builder.getIndexType();

  aiir::Block::BlockArgListType args = funcOp.front().getArguments();
  aiir::Value arg = args[0];

  aiir::Value zeroIdx = builder.createIntegerConstant(loc, idxTy, 0);

  fir::SequenceType::Shape flatShape(rank,
                                     fir::SequenceType::getUnknownExtent());
  aiir::Type arrTy = fir::SequenceType::get(flatShape, elementType);
  aiir::Type boxArrTy = fir::BoxType::get(arrTy);
  aiir::Value array = fir::ConvertOp::create(builder, loc, boxArrTy, arg);
  aiir::Type resultType = funcOp.getResultTypes()[0];
  aiir::Value init = initVal(builder, loc, resultType);

  llvm::SmallVector<aiir::Value, Fortran::common::maxRank> bounds;
  ...
  for (unsigned i = 0; i < rank; ++i) {
    ...
    auto dims = fir::BoxDimsOp::create(builder, loc, idxTy, idxTy, idxTy, array,
                                       dimIdx);
    aiir::Value len = dims.getResult(1);
    // We use C indexing here, so len-1 as loopcount
    aiir::Value loopCount = aiir::arith::SubIOp::create(builder, loc, len, one);
  ...
  // the 0 dimension.
  llvm::SmallVector<aiir::Value, Fortran::common::maxRank> indices;
  for (unsigned i = rank; 0 < i; --i) {
    aiir::Value step = one;
    aiir::Value loopCount = bounds[i - 1];
  ...
  aiir::Type eleRefTy = builder.getRefType(elementType);
  aiir::Value addr =
      fir::CoordinateOp::create(builder, loc, eleRefTy, array, indices);
  aiir::Value elem = fir::LoadOp::create(builder, loc, addr);
  aiir::Value reductionVal = genBody(builder, loc, elementType, elem, init);
  ...
  // DoLoopOp loops.
  llvm::SmallVector<aiir::Value> results = loopCond(builder, loc, reductionVal);
  ...
  for (unsigned i = 0; i < rank; ++i) {
    ...
    auto loop = aiir::cast<OP>(result->getParentOp());
  ...
  aiir::func::ReturnOp::create(builder, loc, results[resultIndex]);
}

static llvm::SmallVector<aiir::Value> nopLoopCond(fir::FirOpBuilder &builder,
                                                  aiir::Location loc,
                                                  aiir::Value reductionVal) {
  return {reductionVal};
}
...
static void genRuntimeSumBody(fir::FirOpBuilder &builder,
                              aiir::func::FuncOp &funcOp, unsigned rank,
                              aiir::Type elementType) {
  ...
  auto zero = [](fir::FirOpBuilder builder, aiir::Location loc,
                 aiir::Type elementType) {
    if (auto ty = aiir::dyn_cast<aiir::FloatType>(elementType)) {
  ...
  auto genBodyOp = [](fir::FirOpBuilder builder, aiir::Location loc,
                      aiir::Type elementType, aiir::Value elem1,
                      aiir::Value elem2) -> aiir::Value {
    if (aiir::isa<aiir::FloatType>(elementType))
      return aiir::arith::AddFOp::create(builder, loc, elem1, elem2);
    if (aiir::isa<aiir::IntegerType>(elementType))
      return aiir::arith::AddIOp::create(builder, loc, elem1, elem2);
...
static void genRuntimeMaxvalBody(fir::FirOpBuilder &builder,
                                 aiir::func::FuncOp &funcOp, unsigned rank,
                                 aiir::Type elementType) {
  auto init = [](fir::FirOpBuilder builder, aiir::Location loc,
                 aiir::Type elementType) {
    if (auto ty = aiir::dyn_cast<aiir::FloatType>(elementType)) {
      const llvm::fltSemantics &sem = ty.getFloatSemantics();
  ...
  auto genBodyOp = [](fir::FirOpBuilder builder, aiir::Location loc,
    ...
    if (aiir::isa<aiir::FloatType>(elementType)) {
      ...
      auto compare = aiir::arith::CmpFOp::create(
          builder, loc, aiir::arith::CmpFPredicate::OGT, elem1, elem2);
      return aiir::arith::SelectOp::create(builder, loc, compare, elem1, elem2);
    }
    if (aiir::isa<aiir::IntegerType>(elementType))
      return aiir::arith::MaxSIOp::create(builder, loc, elem1, elem2);
...
static void genRuntimeCountBody(fir::FirOpBuilder &builder,
  ...
  auto genBodyOp = [](fir::FirOpBuilder builder, aiir::Location loc,
    ...
    auto compare = aiir::arith::CmpIOp::create(
        builder, loc, aiir::arith::CmpIPredicate::eq, elem1, zero32);
    auto select =
        aiir::arith::SelectOp::create(builder, loc, compare, zero64, one64);
    return aiir::arith::AddIOp::create(builder, loc, select, elem2);
  };
...
static void genRuntimeAnyBody(fir::FirOpBuilder &builder,
  ...
  auto genBodyOp = [](fir::FirOpBuilder builder, aiir::Location loc,
    ...
    return aiir::arith::CmpIOp::create(
        builder, loc, aiir::arith::CmpIPredicate::ne, elem1, zero);
  };

  auto continueCond = [](fir::FirOpBuilder builder, aiir::Location loc,
                         aiir::Value reductionVal) {
    auto one1 = builder.createIntegerConstant(loc, builder.getI1Type(), 1);
    auto eor = aiir::arith::XOrIOp::create(builder, loc, reductionVal, one1);
    llvm::SmallVector<aiir::Value> results = {eor, reductionVal};
    return results;
  };

  aiir::Location loc = aiir::UnknownLoc::get(builder.getContext());
  builder.setInsertionPointToEnd(funcOp.addEntryBlock());
  aiir::Value ok = builder.createBool(loc, true);

  genReductionLoop<fir::IterWhileOp, aiir::Value, 1>(
...
static void genRuntimeAllBody(fir::FirOpBuilder &builder,
                              aiir::func::FuncOp &funcOp, unsigned rank,
                              aiir::Type elementType) {
  auto one = [](fir::FirOpBuilder builder, aiir::Location loc,
                aiir::Type elementType) {
  ...
  auto continueCond = [](fir::FirOpBuilder builder, aiir::Location loc,
                         aiir::Value reductionVal) {
    llvm::SmallVector<aiir::Value> results = {reductionVal, reductionVal};
    return results;
  };
...
static aiir::FunctionType genRuntimeMinlocType(fir::FirOpBuilder &builder,
                                               unsigned int rank) {
  aiir::Type boxType = fir::BoxType::get(builder.getNoneType());
  aiir::Type boxRefType = builder.getRefType(boxType);

  return aiir::FunctionType::get(builder.getContext(),
                                 {boxRefType, boxType, boxType}, {});
}

// Produces a loop nest for a Minloc intrinsic.
void fir::genMinMaxlocReductionLoop(
    fir::FirOpBuilder &builder, aiir::Value array,
    fir::InitValGeneratorTy initVal, fir::MinlocBodyOpGeneratorTy genBody,
    fir::AddrGeneratorTy getAddrFn, unsigned rank, aiir::Type elementType,
    aiir::Location loc, aiir::Type maskElemType, aiir::Value resultArr,
  ...
  aiir::Type resultElemType = hlfir::getFortranElementType(resultArr.getType());
  aiir::Value flagSet = builder.createIntegerConstant(loc, resultElemType, 1);
  aiir::Value zero = builder.createIntegerConstant(loc, resultElemType, 0);
  aiir::Value flagRef = builder.createTemporary(loc, resultElemType);
  fir::StoreOp::create(builder, loc, zero, flagRef);

  aiir::Value init = initVal(builder, loc, elementType);
  llvm::SmallVector<aiir::Value, Fortran::common::maxRank> bounds;
  ...
  for (unsigned i = 0; i < rank; ++i) {
    ...
    // Proceed to the outer loop.
    auto loop = aiir::cast<fir::DoLoopOp>(result->getParentOp());
  ...
  if (maskMayBeLogicalScalar) {
    if (fir::IfOp ifOp =
            aiir::dyn_cast<fir::IfOp>(builder.getBlock()->getParentOp())) {
...
static void genRuntimeMinMaxlocBody(fir::FirOpBuilder &builder,
                                    aiir::func::FuncOp &funcOp, bool isMax,
                                    unsigned rank, int maskRank,
                                    aiir::Type elementType,
                                    aiir::Type maskElemType,
                                    aiir::Type resultElemTy, bool isDim) {
  auto init = [isMax](fir::FirOpBuilder builder, aiir::Location loc,
                      aiir::Type elementType) {
    if (auto ty = aiir::dyn_cast<aiir::FloatType>(elementType)) {
      const llvm::fltSemantics &sem = ty.getFloatSemantics();
  ...
  aiir::Value mask = funcOp.front().getArgument(2);

  // Set up result array in case of early exit / 0 length array
  aiir::IndexType idxTy = builder.getIndexType();
  aiir::Type resultTy = fir::SequenceType::get(rank, resultElemTy);
  aiir::Type resultHeapTy = fir::HeapType::get(resultTy);
  aiir::Type resultBoxTy = fir::BoxType::get(resultHeapTy);

  aiir::Value returnValue = builder.createIntegerConstant(loc, resultElemTy, 0);
  aiir::Value resultArrSize = builder.createIntegerConstant(loc, idxTy, rank);

  aiir::Value resultArrInit = fir::AllocMemOp::create(builder, loc, resultTy);
  aiir::Value resultArrShape =
      fir::ShapeOp::create(builder, loc, resultArrSize);
  aiir::Value resultArr = fir::EmboxOp::create(builder, loc, resultBoxTy,
                                               resultArrInit, resultArrShape);

  aiir::Type resultRefTy = builder.getRefType(resultElemTy);

  if (maskRank > 0) {
    fir::SequenceType::Shape flatShape(rank,
                                       fir::SequenceType::getUnknownExtent());
    aiir::Type maskTy = fir::SequenceType::get(flatShape, maskElemType);
    aiir::Type boxMaskTy = fir::BoxType::get(maskTy);
    mask = fir::ConvertOp::create(builder, loc, boxMaskTy, mask);
  }

  for (unsigned int i = 0; i < rank; ++i) {
    aiir::Value index = builder.createIntegerConstant(loc, idxTy, i);
    aiir::Value resultElemAddr =
  ...
  auto genBodyOp =
      [&rank, &resultArr, isMax, &mask, &maskElemType, &maskRank](
          fir::FirOpBuilder builder, aiir::Location loc, aiir::Type elementType,
          aiir::Value array, aiir::Value flagRef, aiir::Value reduction,
          const llvm::SmallVectorImpl<aiir::Value> &indices) -> aiir::Value {
    // We are in the innermost loop: generate the reduction body.
    if (maskRank > 0) {
      aiir::Type logicalRef = builder.getRefType(maskElemType);
      aiir::Value maskAddr =
          fir::CoordinateOp::create(builder, loc, logicalRef, mask, indices);
      aiir::Value maskElem = fir::LoadOp::create(builder, loc, maskAddr);

      // fir::IfOp requires argument to be I1 - won't accept logical or any
      // other Integer.
      aiir::Type ifCompatType = builder.getI1Type();
      aiir::Value ifCompatElem =
    ...
    aiir::Value flagSet = builder.createIntegerConstant(
        loc, aiir::cast<fir::ReferenceType>(flagRef.getType()).getEleTy(), 1);
    aiir::Value isFirst = fir::LoadOp::create(builder, loc, flagRef);
    aiir::Type eleRefTy = builder.getRefType(elementType);
    aiir::Value addr =
        fir::CoordinateOp::create(builder, loc, eleRefTy, array, indices);
    aiir::Value elem = fir::LoadOp::create(builder, loc, addr);
    ...
    if (aiir::isa<aiir::FloatType>(elementType)) {
      ...
      cmp = aiir::arith::CmpFOp::create(builder, loc,
                                        isMax ? aiir::arith::CmpFPredicate::OGT
                                              : aiir::arith::CmpFPredicate::OLT,
                                        elem, reduction);

      aiir::Value cmpNan = aiir::arith::CmpFOp::create(
          builder, loc, aiir::arith::CmpFPredicate::UNE, reduction, reduction);
      aiir::Value cmpNan2 = aiir::arith::CmpFOp::create(
          builder, loc, aiir::arith::CmpFPredicate::OEQ, elem, elem);
      cmpNan = aiir::arith::AndIOp::create(builder, loc, cmpNan, cmpNan2);
      cmp = aiir::arith::OrIOp::create(builder, loc, cmp, cmpNan);
    } else if (aiir::isa<aiir::IntegerType>(elementType)) {
      cmp = aiir::arith::CmpIOp::create(builder, loc,
                                        isMax ? aiir::arith::CmpIPredicate::sgt
                                              : aiir::arith::CmpIPredicate::slt,
                                        elem, reduction);
    } else {
      llvm_unreachable("unsupported type");
    }
    ...
    isFirst = aiir::arith::XOrIOp::create(
        builder, loc, isFirst,
        builder.createIntegerConstant(loc, cmp.getType(), 1));
    cmp = aiir::arith::OrIOp::create(builder, loc, cmp, isFirst);
    fir::IfOp ifOp = fir::IfOp::create(builder, loc, elementType, cmp,
                                       /*withElseRegion*/ true);

    builder.setInsertionPointToStart(&ifOp.getThenRegion().front());
    fir::StoreOp::create(builder, loc, flagSet, flagRef);
    aiir::Type resultElemTy = hlfir::getFortranElementType(resultArr.getType());
    aiir::Type returnRefTy = builder.getRefType(resultElemTy);
    aiir::IndexType idxTy = builder.getIndexType();

    aiir::Value one = builder.createIntegerConstant(loc, resultElemTy, 1);

    for (unsigned int i = 0; i < rank; ++i) {
      aiir::Value index = builder.createIntegerConstant(loc, idxTy, i);
      aiir::Value resultElemAddr = fir::CoordinateOp::create(
          builder, loc, returnRefTy, resultArr, index);
      aiir::Value convert =
          fir::ConvertOp::create(builder, loc, resultElemTy, indices[i]);
      aiir::Value fortranIndex =
          aiir::arith::AddIOp::create(builder, loc, convert, one);
    ...
    builder.setInsertionPointAfter(ifOp);
    aiir::Value reductionVal = ifOp.getResult(0);
  ...
  if (maskRank == 0) {
    ...
    aiir::Type logical = maskElemType;
    aiir::Type logicalRefTy = builder.getRefType(logical);
    aiir::Value condAddr =
        fir::BoxAddrOp::create(builder, loc, logicalRefTy, mask);
    aiir::Value cond = fir::LoadOp::create(builder, loc, condAddr);
    aiir::Value condI1 = fir::ConvertOp::create(builder, loc, i1Type, cond);

    fir::IfOp ifOp = fir::IfOp::create(builder, loc, elementType, condI1,
                                       /*withElseRegion=*/true);

    builder.setInsertionPointToStart(&ifOp.getElseRegion().front());
    aiir::Value basicValue;
    if (aiir::isa<aiir::IntegerType>(elementType)) {
  ...
  auto getAddrFn = [](fir::FirOpBuilder builder, aiir::Location loc,
                      const aiir::Type &resultElemType, aiir::Value resultArr,
                      aiir::Value index) {
    aiir::Type resultRefTy = builder.getRefType(resultElemType);
  ...
  if (isDim) {
    ...
    aiir::Type outputArrTy = fir::SequenceType::get(resultShape, resultElemTy);
    aiir::Type outputHeapTy = fir::HeapType::get(outputArrTy);
    aiir::Type outputBoxTy = fir::BoxType::get(outputHeapTy);
    aiir::Type outputRefTy = builder.getRefType(outputBoxTy);
    aiir::Value outputArr = fir::ConvertOp::create(
...
static aiir::FunctionType genRuntimeDotType(fir::FirOpBuilder &builder,
                                            const aiir::Type &elementType) {
  aiir::Type boxType = fir::BoxType::get(builder.getNoneType());
  return aiir::FunctionType::get(builder.getContext(), {boxType, boxType},
                                 {elementType});
}
...
static void genRuntimeDotBody(fir::FirOpBuilder &builder,
                              ...
                              aiir::Type arg1ElementTy,
                              aiir::Type arg2ElementTy) {
  ...
  auto loc = aiir::UnknownLoc::get(builder.getContext());
  aiir::Type resultElementType = funcOp.getResultTypes()[0];
  builder.setInsertionPointToEnd(funcOp.addEntryBlock());

  aiir::IndexType idxTy = builder.getIndexType();

  aiir::Value zero =
      aiir::isa<aiir::FloatType>(resultElementType)
          ? builder.createRealConstant(loc, resultElementType, 0.0)
          : builder.createIntegerConstant(loc, resultElementType, 0);

  aiir::Block::BlockArgListType args = funcOp.front().getArguments();
  aiir::Value arg1 = args[0];
  aiir::Value arg2 = args[1];

  aiir::Value zeroIdx = builder.createIntegerConstant(loc, idxTy, 0);

  fir::SequenceType::Shape flatShape = {fir::SequenceType::getUnknownExtent()};
  aiir::Type arrTy1 = fir::SequenceType::get(flatShape, arg1ElementTy);
  aiir::Type boxArrTy1 = fir::BoxType::get(arrTy1);
  aiir::Value array1 = fir::ConvertOp::create(builder, loc, boxArrTy1, arg1);
  aiir::Type arrTy2 = fir::SequenceType::get(flatShape, arg2ElementTy);
  aiir::Type boxArrTy2 = fir::BoxType::get(arrTy2);
  aiir::Value array2 = fir::ConvertOp::create(builder, loc, boxArrTy2, arg2);
  ...
  aiir::Value sumVal = loop.getRegionIterArgs()[0];

  // Begin loop code
  aiir::OpBuilder::InsertPoint loopEndPt = builder.saveInsertionPoint();
  builder.setInsertionPointToStart(loop.getBody());

  aiir::Type eleRef1Ty = builder.getRefType(arg1ElementTy);
  aiir::Value index = loop.getInductionVar();
  aiir::Value addr1 =
      fir::CoordinateOp::create(builder, loc, eleRef1Ty, array1, index);
  aiir::Value elem1 = fir::LoadOp::create(builder, loc, addr1);
  // Convert to the result type.
  elem1 = fir::ConvertOp::create(builder, loc, resultElementType, elem1);

  aiir::Type eleRef2Ty = builder.getRefType(arg2ElementTy);
  aiir::Value addr2 =
      fir::CoordinateOp::create(builder, loc, eleRef2Ty, array2, index);
  aiir::Value elem2 = fir::LoadOp::create(builder, loc, addr2);
  // Convert to the result type.
  elem2 = fir::ConvertOp::create(builder, loc, resultElementType, elem2);

  if (aiir::isa<aiir::FloatType>(resultElementType))
    sumVal = aiir::arith::AddFOp::create(
        builder, loc, aiir::arith::MulFOp::create(builder, loc, elem1, elem2),
        sumVal);
  else if (aiir::isa<aiir::IntegerType>(resultElementType))
    sumVal = aiir::arith::AddIOp::create(
        builder, loc, aiir::arith::MulIOp::create(builder, loc, elem1, elem2),
        sumVal);
  else
    llvm_unreachable("unsupported type");
  ...
  aiir::Value resultVal = loop.getResult(0);
  aiir::func::ReturnOp::create(builder, loc, resultVal);
}

aiir::func::FuncOp SimplifyIntrinsicsPass::getOrCreateFunction(
    fir::FirOpBuilder &builder, const aiir::StringRef &baseName,
    FunctionTypeGeneratorTy typeGenerator,
    FunctionBodyGeneratorTy bodyGenerator) {
  ...
  std::string replacementName = aiir::Twine{baseName, "_simplified"}.str();
  // If we already have a function, just return it.
  aiir::func::FuncOp newFunc = builder.getNamedFunction(replacementName);
  aiir::FunctionType fType = typeGenerator(builder);
  if (newFunc) {
  ...
  auto inlineLinkage = aiir::LLVM::linkage::Linkage::LinkonceODR;
  auto linkage =
      aiir::LLVM::LinkageAttr::get(builder.getContext(), inlineLinkage);
...
void SimplifyIntrinsicsPass::simplifyIntOrFloatReduction(
  ...
  aiir::Operation::operand_range args = call.getArgs();

  const aiir::Value &dim = args[3];
  const aiir::Value &mask = args[4];
  ...
  aiir::Type resultType = call.getResult(0).getType();

  if (!aiir::isa<aiir::FloatType>(resultType) &&
      !aiir::isa<aiir::IntegerType>(resultType))
    return;
...
void SimplifyIntrinsicsPass::simplifyLogicalDim0Reduction(
  ...
  aiir::Value inputBox = findBoxDef(args[0]);

  aiir::Type elementType = hlfir::getFortranElementType(inputBox.getType());
  aiir::SymbolRefAttr callee = call.getCalleeAttr();
  ...
  fir::LogicalType logicalType = {
      aiir::dyn_cast<fir::LogicalType>(elementType)};
  fir::KindTy kind = logicalType.getFKind();
  aiir::Type intElementType = builder.getIntegerType(kind * 8);

  // Mangle kind into function name as it is not done by default
  std::string funcName =
      (aiir::Twine{callee.getLeafReference().getValue(), "Logical"} +
       aiir::Twine{kind} + "x" + aiir::Twine{rank})
          .str();
...
void SimplifyIntrinsicsPass::simplifyLogicalDim1Reduction(
  ...
  aiir::Operation::operand_range args = call.getArgs();
  aiir::SymbolRefAttr callee = call.getCalleeAttr();
  aiir::StringRef funcNameBase = callee.getLeafReference().getValue();
  unsigned rank = getDimCount(args[0]);
...
void SimplifyIntrinsicsPass::simplifyMinMaxlocReduction(
  ...
  aiir::Value back = args[isDim ? 7 : 6];
  if (isTrueOrNotConstant(back))
    return;

  aiir::Value mask = args[isDim ? 6 : 5];
  aiir::Value maskDef = findMaskDef(mask);
  ...
  fir::FirOpBuilder builder{getSimplificationBuilder(call, kindMap)};
  aiir::Location loc = call.getLoc();
  auto inputBox = findBoxDef(args[1]);
  aiir::Type inputType = hlfir::getFortranElementType(inputBox.getType());

  if (aiir::isa<fir::CharacterType>(inputType))
    return;

  int maskRank;
  fir::KindTy kind = 0;
  aiir::Type logicalElemType = builder.getI1Type();
  if (isOperandAbsent(mask)) {
    maskRank = -1;
  } else {
    maskRank = getDimCount(mask);
    aiir::Type maskElemTy = hlfir::getFortranElementType(maskDef.getType());
    fir::LogicalType logicalFirType = {
        aiir::dyn_cast<fir::LogicalType>(maskElemTy)};
    kind = logicalFirType.getFKind();
    // Convert fir::LogicalType to aiir::Type
    logicalElemType = logicalFirType;
  }

  aiir::Operation *outputDef = args[0].getDefiningOp();
  aiir::Value outputAlloc = outputDef->getOperand(0);
  aiir::Type outType = hlfir::getFortranElementType(outputAlloc.getType());
  ...
  std::string funcName =
      (aiir::Twine{callee.getLeafReference().getValue(), "x"} +
       aiir::Twine{rank} +
       (maskRank >= 0
            ? "_Logical" + aiir::Twine{kind} + "x" + aiir::Twine{maskRank}
            : "") +
       "_")
          .str();
...
void SimplifyIntrinsicsPass::simplifyReductionBody(
    ...
    const aiir::StringRef &funcName, aiir::Type elementType) {
  ...
  aiir::func::FuncOp newFunc =
      getOrCreateFunction(builder, funcName, typeGenerator, bodyGenerator);
  auto newCall =
      fir::CallOp::create(builder, loc, newFunc, aiir::ValueRange{args[0]});
  ...
  module.walk([&](aiir::Operation *op) {
    if (auto call = aiir::dyn_cast<fir::CallOp>(op)) {
      if (cuf::isCUDADeviceContext(op))
        return;
      if (aiir::SymbolRefAttr callee = call.getCalleeAttr()) {
        aiir::StringRef funcName = callee.getLeafReference().getValue();
        ...
        if (funcName.starts_with(RTNAME_STRING(DotProduct))) {
          ...
          LLVM_DEBUG(llvm::dbgs() << "Call operation:\n"; op->dump();
                     llvm::dbgs() << "\n");
          aiir::Operation::operand_range args = call.getArgs();
          const aiir::Value &v1 = args[0];
          const aiir::Value &v2 = args[1];
          aiir::Location loc = call.getLoc();
          ...
          aiir::Type type = call.getResult(0).getType();
          if (!aiir::isa<aiir::FloatType>(type) &&
              !aiir::isa<aiir::IntegerType>(type))
            return;
          ...
          if (!aiir::isa<aiir::FloatType, aiir::IntegerType>(*arg1Type))
            return;
          if (!aiir::isa<aiir::FloatType, aiir::IntegerType>(*arg2Type))
            return;
...
void SimplifyIntrinsicsPass::getDependentDialects(
    aiir::DialectRegistry &registry) const {
  // LLVM::LinkageAttr creation requires that LLVM dialect is loaded.
  registry.insert<aiir::LLVM::LLVMDialect>();
}
