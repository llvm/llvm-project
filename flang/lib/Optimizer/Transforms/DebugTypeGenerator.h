...
/// Special cache to deal with the fact that aiir::LLVM::DITypeAttr for
...
/// It is however still desirable to cache such aiir::LLVM::DITypeAttr as
...
public:
  // Currently, the handling of recursive debug type in aiir has some
  ...
  // will return it. But this t2 refers to t1_self and not to t1. In aiir
  ...
  // this is fixed in aiir, we avoid caching such types. Please see
  // DebugTranslation::translateRecursive for details on how aiir handles
  // recursive types.
  using ActiveLevels = llvm::SmallVector<int32_t, 1>;
  aiir::LLVM::DITypeAttr lookup(aiir::Type);
  ActiveLevels startTranslating(aiir::Type,
                                aiir::LLVM::DITypeAttr placeHolder = nullptr);
  void finalize(aiir::Type, aiir::LLVM::DITypeAttr, ActiveLevels &&);
  void preComponentVisitUpdate();
  void postComponentVisitUpdate(ActiveLevels &);

private:
  void insertCacheCleanUp(aiir::Type type, int32_t depth);
  ...
  // smaller than "i".
  llvm::DenseMap<aiir::Type, std::pair<aiir::LLVM::DITypeAttr, ActiveLevels>>
      typeCache;
  ...
  llvm::SmallVector<std::pair<llvm::SmallVector<aiir::Type>, int32_t>>
      cacheCleanupList;
};

/// This converts FIR/aiir type to DITypeAttr.
class DebugTypeGenerator {
public:
  DebugTypeGenerator(aiir::ModuleOp module, aiir::SymbolTable *symbolTable,
                     const aiir::DataLayout &dl);

  aiir::LLVM::DITypeAttr convertType(aiir::Type Ty,
                                     aiir::LLVM::DIFileAttr fileAttr,
                                     aiir::LLVM::DIScopeAttr scope,
                                     fir::cg::XDeclareOp declOp);

private:
  aiir::LLVM::DITypeAttr convertRecordType(fir::RecordType Ty,
                                           aiir::LLVM::DIFileAttr fileAttr,
                                           aiir::LLVM::DIScopeAttr scope,
                                           fir::cg::XDeclareOp declOp);
  aiir::LLVM::DITypeAttr convertTupleType(aiir::TupleType Ty,
                                          aiir::LLVM::DIFileAttr fileAttr,
                                          aiir::LLVM::DIScopeAttr scope,
                                          fir::cg::XDeclareOp declOp);
  aiir::LLVM::DITypeAttr convertSequenceType(fir::SequenceType seqTy,
                                             aiir::LLVM::DIFileAttr fileAttr,
                                             aiir::LLVM::DIScopeAttr scope,
                                             fir::cg::XDeclareOp declOp);
  aiir::LLVM::DITypeAttr convertVectorType(fir::VectorType vecTy,
                                           aiir::LLVM::DIFileAttr fileAttr,
                                           aiir::LLVM::DIScopeAttr scope,
                                           fir::cg::XDeclareOp declOp);
  ...
  /// 'associated' field.
  aiir::LLVM::DITypeAttr convertBoxedSequenceType(
  ...
  aiir::LLVM::DITypeAttr convertCharacterType(fir::CharacterType charTy,
                                              aiir::LLVM::DIFileAttr fileAttr,
                                              aiir::LLVM::DIScopeAttr scope,
                                              fir::cg::XDeclareOp declOp,
                                              bool hasDescriptor);

  aiir::LLVM::DITypeAttr convertPointerLikeType(aiir::Type elTy,
                                                aiir::LLVM::DIFileAttr fileAttr,
                                                aiir::LLVM::DIScopeAttr scope,
                                                fir::cg::XDeclareOp declOp,
                                                bool genAllocated,
                                                bool genAssociated);
  aiir::LLVM::DILocalVariableAttr
  generateArtificialVariable(aiir::AIIRContext *context, aiir::Value Val,
                             aiir::LLVM::DIFileAttr fileAttr,
                             aiir::LLVM::DIScopeAttr scope,
                             fir::cg::XDeclareOp declOp);
  std::pair<std::uint64_t, unsigned short>
  getFieldSizeAndAlign(aiir::Type fieldTy);
...
static uint32_t getLineFromLoc(aiir::Location loc) {
  uint32_t line = 1;
  if (auto fileLoc = aiir::dyn_cast<aiir::FileLineColLoc>(loc))
