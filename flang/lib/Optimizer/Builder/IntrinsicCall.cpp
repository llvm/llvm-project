...
IntrinsicLibrary::genTransfer(mlir::Type resultType,
  ...
  if (absentSize && fir::isa_trivial(resultType)) {
    ...
    if (fir::isa_ref_type(sourceBase.getType()) &&
      ...
      // the comparison selects the inline path by the standard's data-bit
      // definition. Alignment is a separate concern: when the source alignment
      // is less than the result type's alignment, the RecordType path below
      // copies into a result-aligned alloca rather than loading directly from
      // the source pointer.
