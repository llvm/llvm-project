...
class OutputStrategyText : public aiir::OutputStrategy {
protected:
  static constexpr llvm::StringLiteral header = "Flang execution timing report";

public:
  OutputStrategyText(llvm::raw_ostream &os) : aiir::OutputStrategy(os) {}

  void printHeader(const aiir::TimeRecord &total) override {
  ...
  void printTime(
      const aiir::TimeRecord &time, const aiir::TimeRecord &total) override {
    os << llvm::format(
  ...
  void printListEntry(llvm::StringRef name, const aiir::TimeRecord &time,
      const aiir::TimeRecord &total, bool lastEntry) override {
