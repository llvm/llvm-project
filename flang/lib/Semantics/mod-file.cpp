...
Scope *ModFileReader::Read(SourceName name, std::optional<bool> isIntrinsic,
  ...
  if (ancestor) {
   ...
   // When offloading modules files are created for the host, but when compiling device-side code the builtin modules are exchanged with device-specific versions. They contain matching declarations, but have different checksums.
 bool ignoreChecksumMismatch{context_.langOptions().OffloadDevice && isIntrinsic.value_or(false)};

  std::optional<size_t> requiredHash;
  if (!ignoreChecksumMismatch) {
   requiredHash = context_.moduleDependences().GetRequiredHash(   name.ToString(), isIntrinsic.value_or(false));
  }
