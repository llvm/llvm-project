...
void OmpAttributeVisitor::CheckPerfectNestAndRectangularLoop(
    ...
    for (const auto &entry : block) {
      if (const auto *doConstruct = GetDoConstructIf(entry)) {
        ...
        context_.Say(beginDir.source,
            "LOOP directive not expected in COLLAPSE loop nest"_err_en_US);
        level = 0;
      } else if (const auto &ompLoop{
                     parser::Unwrap<parser::OpenMPLoopConstruct>(entry)}) {
        if (level == 0)
          break;
        const auto &beginDir{
      ...
      //  context_.Say(beginDir.source,   "Non-perfectly nested
      //  loop"_err_en_US);
    }
    return nullptr;
  };
  ...
  while (true) {
    ...
    if (iv) {
      if (auto *symbol{ResolveOmp(*iv, flag, currScope())})
        ivs.push_back(symbol);
    }
    ...
    auto checkPerfectNest = [&, this]() {
      ...
      auto &&end{block.back()};
      if (auto &&cont{parser::Unwrap<parser::ContinueStmt>(x)}) {
        blockSize -= 1;
      }
