...
! WARNING: warning: Module file for module 'iso_fortran_env': File has a different checksum than expected for 'iso_fortran_env':

! RUN: %flang %{checksum_mismatch} -fmodule-mismatch-check=non-intrinsic %s 2>&1 | FileCheck --allow-empty --check-prefix=NO-WARN %s
