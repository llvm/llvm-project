...
! RUN:     %flang -fsyntax-only --target=x86_64-unknown-linux-gnu %s -### 2>&1 | FileCheck %s --check-prefix=DEFAULTPATH

! RUN:     %flang -fsyntax-only --target=x86_64-unknown-linux-gnu %s -cpp -DINTRINSICS_DEFAULT
! RUN: not %flang -fsyntax-only --target=x86_64-unknown-linux-gnu %s -cpp -DINTRINSICS_INPUTONE 2>&1 | FileCheck %s --check-prefix=NOINPUTONE
! RUN: not %flang -fsyntax-only --target=x86_64-unknown-linux-gnu %s -cpp -DINTRINSICS_INPUTTWO 2>&1 | FileCheck %s --check-prefix=NOINPUTTWO
! RUN:     %flang -fsyntax-only --target=x86_64-unknown-linux-gnu %s -cpp -DINTRINSICS_DEFAULT -DINTRINSICS_INPUTTWO -fintrinsic-modules-path=%S/Inputs/module-dir/
! RUN:     %flang -fsyntax-only --target=x86_64-unknown-linux-gnu %s -cpp -DINTRINSICS_INPUTONE -fintrinsic-modules-path=%S/Inputs/module-dir-one/
! RUN:     %flang -fsyntax-only --target=x86_64-unknown-linux-gnu %s -cpp -DINTRINSICS_INPUTONE -DINTRINSICS_INPUTTWO -fintrinsic-modules-path=%S/Inputs/module-dir-one/ -fintrinsic-modules-path=%S/Inputs/module-dir/
! RUN: not %flang -fsyntax-only --target=x86_64-unknown-linux-gnu %s -cpp -DINTRINSICS_INPUTONE -DINTRINSICS_INPUTTWO -fintrinsic-modules-path=%S/Inputs/module-dir/ -fintrinsic-modules-path=%S/Inputs/module-dir-one/ 2>&1 | FileCheck %s --check-prefix=WRONGINPUTONE
...
! DEFAULTPATH:      -fc1
! DEFAULTPATH-SAME: -fintrinsic-modules-path" "{{.*(\\\\|/)}}finclude{{(\\\\|/)}}flang{{(\\\\|/)}}x86_64-unknown-linux-gnu"
