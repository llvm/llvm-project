1! Test the -Xoffload-linker flag that forwards link commands to the clang-linker-wrapper used
...
! CHECK-XLINKER-AMDGCN: "{{[^"]*}}clang-linker-wrapper{{.*}}"{{.*}}"--device-linker=a"{{.*}}"--device-linker=amdgpu-amd-amdhsa=b"{{.*}}
