! Test the `-emit-aiir` option

! RUN: %flang_fc1 -emit-aiir %s -o - | FileCheck %s
! RUN: %flang_fc1 -emit-fir %s -o - | FileCheck %s

! Verify that an `.aiir` file is created when `-emit-aiir` is used. Do it in a temporary directory, which will be cleaned up by the
! LIT runner.
! RUN: rm -rf %t-dir && mkdir -p %t-dir && cd %t-dir
! RUN: cp %s .
! RUN: %flang_fc1 -emit-aiir emit-aiir.f90 && ls emit-aiir.aiir
