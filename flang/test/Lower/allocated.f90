...
! CHECK-LABEL: func.func @_QPallocated_test
...
subroutine allocated_test(scalar, array)
    ...
    ! CHECK-DAG: %[[decl0:.*]]:2 = hlfir.declare %[[arg0]] {{.*}}
    ! CHECK-DAG: %[[decl1:.*]]:2 = hlfir.declare %[[arg1]] {{.*}}
    ! CHECK: %[[scalar:.*]] = fir.load %[[decl0]]#0 : !fir.ref<!fir.box<!fir.heap<f32>>>
    ...
    ! CHECK: %[[array:.*]] = fir.load %[[decl1]]#0 : !fir.ref<!fir.box<!fir.heap<!fir.array<?xf32>>>>
