! Separate-compilation nested-merge operator declare reduction: three modules
! each declare reduction(.shared.:<type>:...) for a distinct type, an inner
! facade merges two of them under operator(.shared.), and an outer facade merges
! the inner facade with the third. A consumer uses the outer facade and reduces
! all three types with .shared.. Lowering must materialize all three imported
! reductions, including the two reachable only through the second level of merge.
!
! Accessibility is decided with the shared clause-side resolver
! (FindUserReductionSymbol), which traces the merged generic transitively through
! every branch of the nested merge, so a reduction reached only through the inner
! facade is still recognized. A single-level, first-match recovery would drop it
! and the clause side would abort with "not yet implemented".
