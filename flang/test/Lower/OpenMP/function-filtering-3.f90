...
! RUN: %flang_fc1 -fopenmp -emit-hlfir %s -o - | FileCheck --check-prefixes=AIIR-HOST,AIIR-ALL %s
...
! RUN: %if amdgpu-registered-target %{ %flang_fc1 -triple amdgcn-amd-amdhsa -fopenmp -fopenmp-is-target-device -emit-hlfir %s -o - | FileCheck --check-prefixes=AIIR-DEVICE,AIIR-ALL %s %}
! RUN: bbc -fopenmp -emit-hlfir %s -o - | FileCheck --check-prefixes=AIIR-HOST,AIIR-ALL %s
! RUN: %if amdgpu-registered-target %{ bbc -target amdgcn-amd-amdhsa -fopenmp -fopenmp-is-target-device -emit-hlfir %s -o - | FileCheck --check-prefixes=AIIR-DEVICE,AIIR-ALL %s %}
...
! AIIR-HOST: func.func @{{.*}}host_parent_procedure(
! AIIR-HOST: return
! AIIR-DEVICE-NOT: func.func {{.*}}host_parent_procedure(
...
contains
! AIIR-ALL: func.func {{.*}}@_QFhost_parent_procedurePtarget_internal_proc(
