...
subroutine pass_pointer_array(i)
  ...
  ! CHECK: %[[IS_PRESENT:.*]] = arith.cmpi ne, %[[ADDR_I64]], %c0_i64 : i64
  ! CHECK: %[[ARG:.*]]:4 = fir.if %[[IS_PRESENT]] -> (!fir.ref<!fir.array<100xf32>>, i1, i1, !fir.box<!fir.ptr<!fir.array<?xf32>>>) {
  ! CHECK:   %[[LOAD2:.*]] = fir.load %[[DECL]]#0 : !fir.ref<!fir.box<!fir.ptr<!fir.array<?xf32>>>>
  ! CHECK:   %[[COPY_IN:.*]]:3 = hlfir.copy_in %[[LOAD2]] to %[[ALLOCA]] : (!fir.box<!fir.ptr<!fir.array<?xf32>>>, !fir.ref<!fir.box<!fir.ptr<!fir.array<?xf32>>>>) -> (!fir.box<!fir.ptr<!fir.array<?xf32>>>, i1, i1)
  ...
  ! CHECK:   fir.result %[[REF]], %[[COPY_IN]]#1, %[[COPY_IN]]#2, %[[LOAD2]] : !fir.ref<!fir.array<100xf32>>, i1, i1, !fir.box<!fir.ptr<!fir.array<?xf32>>>
  ! CHECK: } else {
  ...
  ! CHECK:   fir.result %[[ABSENT]], %[[FALSE]], %[[FALSE2]], %[[ABSENT_BOX]] : !fir.ref<!fir.array<100xf32>>, i1, i1, !fir.box<!fir.ptr<!fir.array<?xf32>>>
  ! CHECK: }
  ! CHECK: fir.call @_QPtakes_opt_explicit_shape(%[[ARG]]#0) {{.*}} : (!fir.ref<!fir.array<100xf32>>) -> ()
  ! CHECK: hlfir.copy_out %[[ALLOCA]], %[[ARG]]#1, %[[ARG]]#2 to %[[ARG]]#3 : (!fir.ref<!fir.box<!fir.ptr<!fir.array<?xf32>>>>, i1, i1, !fir.box<!fir.ptr<!fir.array<?xf32>>>) -> ()
end subroutine
...
subroutine pass_pointer_array_char(c)
  ...
  ! CHECK: %[[IS_PRESENT:.*]] = arith.cmpi ne, %[[ADDR_I64]], %c0_i64 : i64
  ! CHECK: %[[ARG:.*]]:4 = fir.if %[[IS_PRESENT]] -> (!fir.boxchar<1>, i1, i1, !fir.box<!fir.ptr<!fir.array<?x!fir.char<1,?>>>>) {
  ! CHECK:   %[[LOAD2:.*]] = fir.load %[[DECL]]#0 : !fir.ref<!fir.box<!fir.ptr<!fir.array<?x!fir.char<1,?>>>>>
  ! CHECK:   %[[COPY_IN:.*]]:3 = hlfir.copy_in %[[LOAD2]] to %[[ALLOCA]] : (!fir.box<!fir.ptr<!fir.array<?x!fir.char<1,?>>>>, !fir.ref<!fir.box<!fir.ptr<!fir.array<?x!fir.char<1,?>>>>>) -> (!fir.box<!fir.ptr<!fir.array<?x!fir.char<1,?>>>>, i1, i1)
  ...
  ! CHECK:   fir.result %[[EMBOX]], %[[COPY_IN]]#1, %[[COPY_IN]]#2, %[[LOAD2]] : !fir.boxchar<1>, i1, i1, !fir.box<!fir.ptr<!fir.array<?x!fir.char<1,?>>>>
  ! CHECK: } else {
  ! CHECK:   %[[ABSENT:.*]] = fir.absent !fir.boxchar<1>
  ! CHECK:   %[[FALSE:.*]] = arith.constant false
  ! CHECK:   %[[FALSE2:.*]] = arith.constant false
  ! CHECK:   %[[ABSENT_BOX:.*]] = fir.absent !fir.box<!fir.ptr<!fir.array<?x!fir.char<1,?>>>>
  ! CHECK:   fir.result %[[ABSENT]], %[[FALSE]], %[[FALSE2]], %[[ABSENT_BOX]] : !fir.boxchar<1>, i1, i1, !fir.box<!fir.ptr<!fir.array<?x!fir.char<1,?>>>>
  ! CHECK: }
  ! CHECK: fir.call @_QPtakes_opt_explicit_shape_char(%[[ARG]]#0) {{.*}} : (!fir.boxchar<1>) -> ()
  ! CHECK: hlfir.copy_out %[[ALLOCA]], %[[ARG]]#1, %[[ARG]]#2 to %[[ARG]]#3 : (!fir.ref<!fir.box<!fir.ptr<!fir.array<?x!fir.char<1,?>>>>>, i1, i1, !fir.box<!fir.ptr<!fir.array<?x!fir.char<1,?>>>>) -> ()
end subroutine
...
subroutine pass_opt_assumed_shape(x)
  ...
  ! CHECK:   %[[COPY_IN:.*]]:3 = hlfir.copy_in %[[DECL]]#0 to %[[ALLOCA]] : (!fir.box<!fir.array<?xf32>>, !fir.ref<!fir.box<!fir.array<?xf32>>>) -> (!fir.box<!fir.array<?xf32>>, i1, i1)
  ...
  ! CHECK:   fir.result %[[REF]], %[[COPY_IN]]#1, %[[COPY_IN]]#2, %[[DECL]]#0 : !fir.ref<!fir.array<100xf32>>, i1, i1, !fir.box<!fir.array<?xf32>>
  ! CHECK: } else {
  ! CHECK:   %[[ABSENT:.*]] = fir.absent !fir.ref<!fir.array<100xf32>>
  ! CHECK:   %[[FALSE:.*]] = arith.constant false
  ! CHECK:   %[[FALSE2:.*]] = arith.constant false
  ! CHECK:   %[[ABSENT_BOX:.*]] = fir.absent !fir.box<!fir.array<?xf32>>
  ! CHECK:   fir.result %[[ABSENT]], %[[FALSE]], %[[FALSE2]], %[[ABSENT_BOX]] : !fir.ref<!fir.array<100xf32>>, i1, i1, !fir.box<!fir.array<?xf32>>
  ! CHECK: }
  ! CHECK: fir.call @_QPtakes_opt_explicit_shape(%[[ARG]]#0) {{.*}} : (!fir.ref<!fir.array<100xf32>>) -> ()
  ! CHECK: hlfir.copy_out %[[ALLOCA]], %[[ARG]]#1, %[[ARG]]#2 to %[[ARG]]#3 : (!fir.ref<!fir.box<!fir.array<?xf32>>>, i1, i1, !fir.box<!fir.array<?xf32>>) -> ()
end subroutine
...
subroutine pass_opt_assumed_shape_char(c)
  ...
  ! CHECK:   %[[COPY_IN:.*]]:3 = hlfir.copy_in %[[DECL]]#0 to %[[ALLOCA]] : (!fir.box<!fir.array<?x!fir.char<1,?>>>, !fir.ref<!fir.box<!fir.array<?x!fir.char<1,?>>>>) -> (!fir.box<!fir.array<?x!fir.char<1,?>>>, i1, i1)
  ...
  ! CHECK:   %[[EMBOX:.*]] = fir.emboxchar %[[REF]], %[[ELE_SIZE]] : (!fir.ref<!fir.char<1,?>>, index) -> !fir.boxchar<1>
  ! CHECK:   fir.result %[[EMBOX]], %[[COPY_IN]]#1, %[[COPY_IN]]#2, %[[DECL]]#0 : !fir.boxchar<1>, i1, i1, !fir.box<!fir.array<?x!fir.char<1,?>>>
  ! CHECK: } else {
  ! CHECK:   %[[ABSENT:.*]] = fir.absent !fir.boxchar<1>
  ! CHECK:   %[[FALSE:.*]] = arith.constant false
  ! CHECK:   %[[FALSE2:.*]] = arith.constant false
  ! CHECK:   %[[ABSENT_BOX:.*]] = fir.absent !fir.box<!fir.array<?x!fir.char<1,?>>>
  ! CHECK:   fir.result %[[ABSENT]], %[[FALSE]], %[[FALSE2]], %[[ABSENT_BOX]] : !fir.boxchar<1>, i1, i1, !fir.box<!fir.array<?x!fir.char<1,?>>>
  ! CHECK: }
  ! CHECK: fir.call @_QPtakes_opt_explicit_shape_char(%[[ARG]]#0) {{.*}} : (!fir.boxchar<1>) -> ()
  ! CHECK: hlfir.copy_out %[[ALLOCA]], %[[ARG]]#1, %[[ARG]]#2 to %[[ARG]]#3 : (!fir.ref<!fir.box<!fir.array<?x!fir.char<1,?>>>>, i1, i1, !fir.box<!fir.array<?x!fir.char<1,?>>>) -> ()
end subroutine
...
subroutine pass_opt_assumed_shape_to_intentin(x)
  ...
  ! CHECK:   %[[BOX_ADDR:.*]] = fir.box_addr %[[COPY_IN]]#0 : (!fir.box<!fir.array<?xf32>>) -> !fir.ref<!fir.array<?xf32>>
  ! CHECK:   %[[REF:.*]] = fir.convert %[[BOX_ADDR]] : (!fir.ref<!fir.array<?xf32>>) -> !fir.ref<!fir.array<100xf32>>
  ! CHECK:   fir.result %[[REF]], %[[COPY_IN]]#1, %[[COPY_IN]]#2 : !fir.ref<!fir.array<100xf32>>, i1, i1
  ! CHECK: } else {
  ! CHECK:   %[[ABSENT:.*]] = fir.absent !fir.ref<!fir.array<100xf32>>
  ! CHECK:   %[[FALSE:.*]] = arith.constant false
  ! CHECK:   %[[FALSE2:.*]] = arith.constant false
  ! CHECK:   fir.result %[[ABSENT]], %[[FALSE]], %[[FALSE2]] : !fir.ref<!fir.array<100xf32>>, i1, i1
  ! CHECK: }
  ! CHECK: fir.call @_QPtakes_opt_explicit_shape_intentin(%[[ARG]]#0) {{.*}} : (!fir.ref<!fir.array<100xf32>>) -> ()
  ! CHECK: hlfir.copy_out %[[ALLOCA]], %[[ARG]]#1, %[[ARG]]#2 : (!fir.ref<!fir.box<!fir.array<?xf32>>>, i1, i1) -> ()
end subroutine
