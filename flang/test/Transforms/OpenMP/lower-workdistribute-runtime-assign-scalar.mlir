...
func.func @x(%arr : !fir.ref<!fir.array<?x?xf32>>) {
    ...
    %198 = omp.map.info var_ptr(%arr : !fir.ref<!fir.array<?x?xf32>>, f32) map_clauses(implicit, tofrom) capture(ByRef) bounds(%195, %197) name("y") -> !fir.ref<!fir.array<?x?xf32>>
    %199 = omp.map.info var_ptr(%1 : !fir.ref<i32>, i32) map_clauses(implicit, exit_release_or_enter_alloc) capture(ByCopy) name("") -> !fir.ref<i32>
    %200 = omp.map.info var_ptr(%0 : !fir.ref<i32>, i32) map_clauses(implicit, exit_release_or_enter_alloc) capture(ByCopy) name("") -> !fir.ref<i32>
