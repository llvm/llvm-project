...
! The difficulty is that at the aiir::LLVM::DITypeAttr, because of the
! lack of AIIR attribute true recursion, the aiir::LLVM::DITypeAttr for
! `t0` inside `t1` is special because it is only valid when found
! in an aiir::LLVM::DITypeAttr tree under `base` (it will point to `base`
...
! point to the actual aiir::LLVM::DITypeAttr for `base` instead of
...
! aiir::LLVM::DITypeAttr tree definition. This used to not be the case,
! leading to a lot of duplicate aiir::LLVM::DITypeAttr and actual LLVM IR
