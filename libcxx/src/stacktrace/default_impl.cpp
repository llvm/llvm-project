...
int add_image(dl_phdr_info* info, size_t, void* state_v) {
  ...
  auto& imgs  = state.images_;

  auto __end = imgs.images_.begin() + imgs.count_;
  auto __it = std::lower_bound(imgs.images_.begin(), __end, info->dlpi_addr, [](_Image const& __img, uintptr_t __addr) {
    return __img.loaded_at_ < __addr;
  });
  if (__it != __end && __it->loaded_at_ == info->dlpi_addr) {
    return 0;
  }
...
void __stacktrace::_Images::refresh() {
  std::lock_guard<std::mutex> __lock(mutex_);
#    if __has_include("dlfcn.h") && __has_include("link.h")
  // Re-walk every call: unlike a one-shot snapshot, this notices images `dlopen`'d since the
  // last capture. Cheap in the common case since `add_image` skips anything already known.
