...
#if defined(__ELF__) && __has_feature(cxx_thread_local)
#  define _LIBCPP_HAZARD_POINTER_THREAD_LOCAL 1
#else
#  define _LIBCPP_HAZARD_POINTER_THREAD_LOCAL 0
#endif
...
#else
constexpr size_t __cache_line_size = hardware_destructive_interference_size;
#endif
...
class __grow_array {
  struct __round {
    __round* __prev_ = nullptr; // the previous (smaller) round; owns elements [0, __prev_->__size_)
    size_t __size_   = 0;       // > __prev_->__size_
  ...
  static _Tp* __slab(__round* __curr) noexcept {
    const uintptr_t __list_end = reinterpret_cast<uintptr_t>(__curr->__list() + __curr->__size_);
    return reinterpret_cast<_Tp*>(__ceil_to(static_cast<size_t>(__list_end), __alignment()));
  }

  // Builds a round of __capacity elements on top of __prev; returns nullptr (and updates __prev) when
  // another thread published a round meanwhile. May throw bad_alloc.
  __round* __new_round(size_t __capacity, __round*& __prev) {
    const size_t __base = __prev ? __prev->__size_ : 0;
    void* __raw =
        std::__libcpp_allocate<byte>(static_cast<__element_count>(__bytes(__capacity, __base)), __alignment());
    __round* __curr = std::construct_at(static_cast<__round*>(__raw));
    __curr->__prev_ = __prev;
    __curr->__size_ = __capacity;
    _Tp* __slab_ptr = __slab(__curr);
    _Tp** __list    = __curr->__list();
    for (size_t __i = 0; __i < __base; ++__i)
      __list[__i] = __prev->__list()[__i];
    for (size_t __i = __base; __i < __capacity; ++__i)
      __list[__i] = nullptr;
    for (size_t __i = __base; __i < __capacity; ++__i) {
      __round* __seen = __array_.load(memory_order_acquire);
      if (__seen != __prev) { // race lost early: stop building
        __prev = __seen;
        __del_round(__curr);
        return nullptr;
      }
      __list[__i] = std::construct_at(__slab_ptr + (__i - __base));
    }
    return __curr;
  }
  ...
  void __del_round(__round* __curr) noexcept {
    ...
    for (size_t __i = 0; __i < __built - __base; ++__i)
      std::destroy_at(__list[__built - 1 - __i]);
    std::__libcpp_deallocate<byte>(
        reinterpret_cast<byte*>(__curr), static_cast<__element_count>(__size), __alignment());
  }
  ...
  __round* __at_slow(size_t __index) {
    ...
    const size_t __capacity = std::__bit_ceil(__index + 1);
    while (true) {
...
public:
  ...
  __grow_array(__grow_array&&)                 = delete;
  __grow_array& operator=(const __grow_array&) = delete;
  __grow_array& operator=(__grow_array&&)      = delete;
  ~__grow_array()                              = default;
...
// costs no atomics on the domain's shared state. Reached through __tls_cache (one TLS load) where
// _LIBCPP_HAZARD_POINTER_THREAD_LOCAL, else through the pthread key (call_once fast path + __libcpp_tls_get);
// see __hazard_pointer_domain::__thread_cache. The key is always what registers the thread-exit hook.
...
// Sentinel stored by __at_thread_exit once a thread's cache has been torn down (in __tls_cache, or in the
// TLS key on the pthread-only path). While it is there, __thread_cache() reports "no cache", so a
// hazard_pointer destroyed by a destructor that runs after ours releases its record straight to the domain
// instead of touching the freed cache.
//
// With __tls_cache the sentinel stays for the rest of the thread's life. On the pthread-only path it is
// armed when the cache is torn down; POSIX clears a TSD value before each destructor round, so a later
// round sees null again, not the sentinel. We deliberately do not re-arm it there: re-arming would keep
// this thread in destructor rounds after sanitizer runtimes have already finalized it. Consequently, on
// that path a make_hazard_pointer() called from a foreign TSD destructor in a later round may build a fresh
// cache that is never evicted -- a bounded leak of that cache object plus up to __thread_cache_capacity
// records. Records are immortal, so the cache object is the actual leak.
__hazard_pointer_thread_cache __dead_thread_cache;

#if _LIBCPP_HAZARD_POINTER_THREAD_LOCAL
[[__gnu__::__tls_model__("initial-exec")]] constinit thread_local __hazard_pointer_thread_cache* __tls_cache = nullptr;
#endif
...
class __protected_set {
  ...
  bool __grow() noexcept {
    const size_t __new_capacity = __capacity_ == 0 ? 16 : __capacity_ * 2;
    uintptr_t* __new_slots = ::new (nothrow) uintptr_t[__new_capacity](); // zero-initialised
    if (__new_slots == nullptr)
      return false;
    const uintptr_t* __old      = __slots_;
    const size_t __old_capacity = __capacity_;
...
public:
  ...
  __protected_set(__protected_set&&)                 = delete;
  __protected_set& operator=(const __protected_set&) = delete;
  __protected_set& operator=(__protected_set&&)      = delete;
  ...
  bool __contains(const void* __p) const noexcept {
    if (__size_ == 0)
      return false;
    const uintptr_t __key = reinterpret_cast<uintptr_t>(__p);
    size_t __i      = __mix_pointer(__key) & __mask_;
    while (__slots_[__i] != 0) {
      if (__slots_[__i] == __key)
        return true;
...
// Fences of the reader/reclaimer protocol (see <__hazard_pointer/domain.h>)

// The fence every retire() issues before publishing its node, and the one every reclamation pass issues
// before scanning the slots. With asymmetric fences the retire side (like the reader side, inline in
// the headers) is a compiler barrier and the reclaimer side is a barrier on every CPU currently running
// a thread of this process; otherwise both are seq_cst fences.
#if _LIBCPP_HAZARD_POINTER_ASYMMETRIC_FENCE

inline void __retire_fence() noexcept { std::atomic_signal_fence(memory_order_seq_cst); }

#  if defined(__linux__)

#    if defined(SYS_membarrier)
// membarrier(2) private expedited commands (Linux 4.14). The command values are kernel ABI; spelling
// them here keeps the build independent of the UAPI headers it was configured with. The process must
// register once before using the barrier; fork() clears the registration, which shows up as EPERM.
constexpr long __membarrier_cmd_private_expedited          = 1 << 3;
constexpr long __membarrier_cmd_register_private_expedited = 1 << 4;

bool __membarrier_private_expedited() noexcept {
  if (::syscall(SYS_membarrier, __membarrier_cmd_private_expedited, 0, 0) == 0)
    return true;
  if (errno != EPERM) // ENOSYS: no membarrier(2) or seccomp; EINVAL: kernel before 4.14
    return false;
  return ::syscall(SYS_membarrier, __membarrier_cmd_register_private_expedited, 0, 0) == 0 &&
         ::syscall(SYS_membarrier, __membarrier_cmd_private_expedited, 0, 0) == 0;
}
#    else
bool __membarrier_private_expedited() noexcept { return false; }
#    endif

// Fallback without membarrier(2): taking write permission away from a resident private page makes the
// kernel interrupt every CPU that runs a thread of this process to flush its TLB, and that interrupt
// is a full barrier on the interrupted CPU. Serialised because two flips at once could coalesce.
void __mprotect_barrier() noexcept {
  static constinit __libcpp_mutex_t __lock = _LIBCPP_MUTEX_INITIALIZER;
  static void* __page                      = nullptr;
  static size_t __page_size                = 0;
  std::__libcpp_mutex_lock(std::addressof(__lock));
  if (__page == nullptr) {
    __page_size = static_cast<size_t>(::sysconf(_SC_PAGESIZE));
    void* __p   = ::mmap(nullptr, __page_size, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
    if (__p == MAP_FAILED)
      _LIBCPP_VERBOSE_ABORT("hazard_pointer: cannot map the page used as a memory barrier");
    __page = __p;
  }
  if (::mprotect(__page, __page_size, PROT_READ | PROT_WRITE) != 0)
    _LIBCPP_VERBOSE_ABORT("hazard_pointer: mprotect failed");
  *static_cast<volatile char*>(__page) = 1; // the page must be resident for the flush to go out
  if (::mprotect(__page, __page_size, PROT_READ) != 0)
    _LIBCPP_VERBOSE_ABORT("hazard_pointer: mprotect failed");
  std::__libcpp_mutex_unlock(std::addressof(__lock));
}

void __heavy_fence() noexcept {
  static atomic<bool> __no_membarrier{false};
  if (!__no_membarrier.load(memory_order_relaxed)) {
    if (__membarrier_private_expedited())
      return;
    __no_membarrier.store(true, memory_order_relaxed);
  }
  __mprotect_barrier();
}
...
void __heavy_fence() noexcept { ::FlushProcessWriteBuffers(); }

#  endif

#else

inline void __retire_fence() noexcept { std::atomic_thread_fence(memory_order_seq_cst); }
inline void __heavy_fence() noexcept { std::atomic_thread_fence(memory_order_seq_cst); }

#endif // _LIBCPP_HAZARD_POINTER_ASYMMETRIC_FENCE
...
public:
  ...
  __hazard_pointer_domain(__hazard_pointer_domain&&)                 = delete;
  __hazard_pointer_domain& operator=(const __hazard_pointer_domain&) = delete;
  __hazard_pointer_domain& operator=(__hazard_pointer_domain&&)      = delete;
  ~__hazard_pointer_domain()                                         = default;
  ...
  void __release_slot(__hazard_pointer_slot* __slot) noexcept {
    const auto __rec = static_cast<__record*>(__slot);
...
private:
  ...
  int __threshold() const noexcept {
    const int __records = __record_count_.load(memory_order_relaxed);
    const int __scaled  = __threshold_multiplier * __records;
  ...
  int __check_due_time() noexcept {
    const uintptr_t __time = __now_s();
    if (uintptr_t __due = __due_time_.load(memory_order_acquire);
        __time < __due || !__due_time_.compare_exchange_strong(
                              __due, __time + __sync_time_period_s, memory_order_acq_rel, memory_order_relaxed))
      return 0;
    const int __count = __retired_count_.exchange(0, memory_order_acq_rel);
    if (__count < 0) {
  ...
  int __extract_retired(__node* __extracted[__shard_count]) noexcept {
    ...
    for (int __s = 0; __s < __shard_count; ++__s) {
      ...
      for (const __node* __n = __extracted[__s]; __n != nullptr; __n = __n->__next_)
        ++__taken;
    }
    return __taken;
  }
  ...
  bool __load_protected(__protected_set& __set) const noexcept {
    const int __handed_out  = __record_count_.load(memory_order_relaxed);
    ...
    for (; __i < __count; ++__i) {
      if (const void* __v = __recs[__i]->__value_.load(__order); __v != nullptr && !__set.__insert(__v))
        return false;
    }
  ...
  int __match_reclaim(__node* __retired[__shard_count], const __protected_set* __set, bool& __done) noexcept {
    ...
    for (bool __more = true; __more;) { // walk the shards in lock-step (instruction-level parallelism)
      ...
      for (int __s = 0; __s < __shard_count; ++__s) {
        ...
        if (__set != nullptr ? __set->__contains(__n) : __protected_linear(__n))
  ...
  //
  // Out of line, like the other slow paths below: inlined into __hazard_pointer_retire() it would make every
  // retire() pay the prologue (six saved registers and a frame) of a pass that runs once per ~1000 calls.
  _LIBCPP_NOINLINE void __do_reclamation(int __count) noexcept {
    while (true) {
      __node* __retired[__shard_count];
      bool __done = true;
      if (const int __taken = __extract_retired(__retired)) {
        ...
        if (const int __residue = __match_reclaim(__retired, __set_ptr, __done))
...
private:
    ...
    while (true) {
      ...
      if ((__available & __lock_bit) == 0) {
        if (__cas_available(__available, __available | __lock_bit)) { // locked
          const auto __head = reinterpret_cast<__record*>(__available);
  ...
  // Pushes the chain [__head, __tail] (tail's link is overwritten) onto the available list. Out of line for
  // the same reason as __acquire_records: __hazard_pointer_release()'s fast path is a thread-cache push.
  _LIBCPP_NOINLINE void __push_available(__record* __head, __record* __tail) noexcept {
    const uintptr_t __new_head = reinterpret_cast<uintptr_t>(__head);
    _LIBCPP_ASSERT_INTERNAL((__new_head & __lock_bit) == 0, "hazard pointer record misaligned");
    while (true) {
      if (uintptr_t __available = __load_available(); (__available & __lock_bit) == 0) {
  ...
  __record* __create_record() { // may throw bad_alloc
    ...
    const int __index = __record_count_.fetch_add(1, memory_order_relaxed);
  ...
  // always 1 today; batching is kept for a future batch-acquire entry point (P3428R4). Out of line so that
  // __hazard_pointer_acquire()'s fast path (a thread-cache pop) stays a leaf function.
  _LIBCPP_NOINLINE __record* __acquire_records(uint8_t __count) {
    uint8_t __popped = 0;
  ...
  void __init_tls() noexcept {
    std::call_once(__tls_once_, [this] {
      __tls_ok_ = std::__libcpp_tls_create(
                      std::addressof(__tls_key_), std::addressof(__hazard_pointer_domain::__at_thread_exit)) == 0;
    });
  }
...
#if _LIBCPP_HAZARD_POINTER_THREAD_LOCAL
    if (__hazard_pointer_thread_cache* __p = __tls_cache; __p != nullptr) [[likely]]
      return __p == &__dead_thread_cache ? nullptr : __p;
    return __create ? __create_thread_cache() : nullptr;
#else
    __init_tls();
    if (!__tls_ok_)
      return nullptr;
  ...
  _LIBCPP_NOINLINE __hazard_pointer_thread_cache* __create_thread_cache() noexcept {
    __init_tls();
    if (!__tls_ok_)
      return nullptr;
    auto* __cache = ::new (nothrow) __hazard_pointer_thread_cache();
    if (__cache != nullptr && std::__libcpp_tls_set(__tls_key_, __cache) != 0) {
      delete __cache;
      return nullptr;
    }
#if _LIBCPP_HAZARD_POINTER_THREAD_LOCAL
...
// The TSD destructor of the thread cache: return its records to the domain, free it, and arm the sentinel.
// With __tls_cache the sentinel lives in TLS and the key is left null, so this runs once. On the pthread-only
// path it goes into the key: POSIX clears the key's value before each destructor round, so round 1 arrives
// with the cache and round 2 with the sentinel and does nothing, which leaves the key null for good.
// Deliberately no re-arming: see the note above __dead_thread_cache.
void _LIBCPP_TLS_DESTRUCTOR_CC __hazard_pointer_domain::__at_thread_exit(void* __p) {
  if (__p == nullptr || __p == static_cast<void*>(std::addressof(__dead_thread_cache)))
    return;
  const auto __cache = static_cast<__hazard_pointer_thread_cache*>(__p);
  __domain.__evict(*__cache);
  delete __cache;
#if _LIBCPP_HAZARD_POINTER_THREAD_LOCAL
  __tls_cache = std::addressof(__dead_thread_cache);
#else
  std::__libcpp_tls_set(__domain.__tls_key_, std::addressof(__dead_thread_cache));
#endif
}
