...
- P0226R1: Mathematical Special Functions for C++17 (`Github <https://github.com/llvm/llvm-project/issues/99939>`__) (only ``assoc_laguerre`` and ``hermite`` are implemented)
...
- On platforms where ``feraiseexcept`` lives in libm, libc++ now links against it, since the mathematical special
  functions use it to report the floating-point exception half of a domain or range error. ``libc++.so`` gains a
  dependency on libm there. Windows and Apple are unaffected: the symbol is in the CRT and in libSystem respectively.
