...
std::basic_ostream<_CharT, _Traits>& operator<<(std::basic_ostream<_CharT, _Traits>& os, const mutating_delimiter& d) {
  return os << d.get();
}
...
template <class _CharT, class _Traits>
std::basic_ostream<_CharT, _Traits>& operator<<(std::basic_ostream<_CharT, _Traits>& os, mutating_delimiter2& d) {
  return os << d.get();
}
...
void test(Delim&& d, Iter first, Iter last, const CharT* expected) {
  ...
  static_assert((std::is_copy_constructible<Joiner>::value ==
                 std::is_copy_constructible<typename std::decay<Delim>::type>::value),
                "");
  static_assert((std::is_move_constructible<Joiner>::value ==
                 std::is_move_constructible<typename std::decay<Delim>::type>::value),
                "");
  static_assert(
      (std::is_copy_assignable<Joiner>::value == std::is_copy_assignable<typename std::decay<Delim>::type>::value), "");
  static_assert(
      (std::is_move_assignable<Joiner>::value == std::is_move_assignable<typename std::decay<Delim>::type>::value), "");
...
int main(int, char**) {
  {
    ...
    test('X',
         cpp17_input_iterator<const char*>(chars),
         cpp17_input_iterator<const char*>(chars + 10),
         "0X1X2X3X4X5X6X7X8X9");
    test('x',
         cpp17_input_iterator<const int*>(ints),
         cpp17_input_iterator<const int*>(ints + 10),
    ...
    test('x',
         forward_iterator<const int*>(ints),
         forward_iterator<const int*>(ints + 10),
         "10x11x12x13x14x15x16x17x18x19");
    test('X',
         random_access_iterator<const char*>(chars),
         random_access_iterator<const char*>(chars + 10),
         "0X1X2X3X4X5X6X7X8X9");
    test('x',
         random_access_iterator<const int*>(ints),
...
#ifndef TEST_HAS_NO_WIDE_CHARACTERS
  {
    ...
    test(L'X',
         cpp17_input_iterator<const wchar_t*>(chars),
         cpp17_input_iterator<const wchar_t*>(chars + 10),
    ...
    test(L'X',
         forward_iterator<const wchar_t*>(chars),
         forward_iterator<const wchar_t*>(chars + 10),
         L"0X1X2X3X4X5X6X7X8X9");
    test(L'x',
         forward_iterator<const int*>(ints),
         forward_iterator<const int*>(ints + 10),
         L"10x11x12x13x14x15x16x17x18x19");
    test(L'X',
         random_access_iterator<const wchar_t*>(chars),
