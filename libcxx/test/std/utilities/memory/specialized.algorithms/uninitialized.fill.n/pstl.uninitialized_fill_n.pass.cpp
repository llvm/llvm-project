...
EXECUTION_POLICY_SFINAE_TEST(uninitialized_fill_n);

static_assert(sfinae_test_uninitialized_fill_n<int, int*, int, int>);
static_assert(!sfinae_test_uninitialized_fill_n<std::execution::parallel_policy, int*, int, int>);
...
struct TestCounted {
  ...
  void operator()(ExecutionPolicy&& policy) {
    {
      // Test uninitialized_fill_n() with a range of Counted objects.
  ...
  void operator()(ExecutionPolicy&& policy) {
    {
      ...
      std::uninitialized_fill_n(policy, Iter(data), n, 42);
      for (int i = 0; i != n; ++i) {
        assert(data[i] == 42);
      }
