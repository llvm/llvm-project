...
EXECUTION_POLICY_SFINAE_TEST(uninitialized_default_construct);

static_assert(sfinae_test_uninitialized_default_construct<int, int*, int*>);
static_assert(!sfinae_test_uninitialized_default_construct<std::execution::parallel_policy, int*, int*>);
  ...
  void operator()(ExecutionPolicy&& policy) {
    {
      ...
      runway_sample(std::size(counters) + 1, [&](size_t size) {
        ...
        std::uninitialized_default_construct(policy, Iter(pool), Iter(pool + size));
        ASSERT_SAME_TYPE(decltype(std::uninitialized_default_construct(policy, Iter(pool), Iter(pool + size))), void);
  ...
  void operator()(ExecutionPolicy&& policy) {
    {
      ...
      std::uninitialized_default_construct(policy, Iter(data), Iter(last));
