...
TEST_CONSTEXPR_CXX26 bool test() {
...

  {
    ...
    std::uninitialized_default_construct(data, last);
    for (int i = 0; i != n; ++i) {
      data[i] = -i;
      assert(data[i] == -i);
    }
...

  {
    ...
    std::uninitialized_default_construct(It(p), It(pend));
...
#if TEST_STD_VER >= 26
// Test that std::uninitialized_default_construct initializes int elements to indeterminate values,
