...
EXECUTION_POLICY_SFINAE_TEST(uninitialized_move);

static_assert(sfinae_test_uninitialized_move<int, int*, int*, int*>);
static_assert(!sfinae_test_uninitialized_move<std::execution::parallel_policy, int*, int*, int*>);
...
struct TestCustomTypes {
  ...
  void operator()(ExecutionPolicy&& policy) {
    ...
    runway_sample(n + 1, [&](size_t size) {
      ...
      auto ret = std::uninitialized_move(policy, Iter1(source.data()), Iter1(source.data() + size), Iter2(dest));
