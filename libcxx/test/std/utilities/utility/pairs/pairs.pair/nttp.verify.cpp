...
using I = test<std::pair<std::string, std::string>{}>;
// expected-error-re@-1 {{type 'std::pair<std::string, std::string>' (aka '{{(std::)?}}pair<basic_string<char>, basic_string<char>>') of non-type template parameter is not a structural type}}
} // namespace test_auto
