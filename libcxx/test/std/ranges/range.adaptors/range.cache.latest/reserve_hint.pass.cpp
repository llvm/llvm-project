...
static_assert(!HasMemberReserveHint<std::ranges::cache_latest_view<NonApproximatelySizedView>>);
static_assert(!HasMemberReserveHint<const std::ranges::cache_latest_view<NonApproximatelySizedView>>);
...
static_assert(HasMemberReserveHint<std::ranges::cache_latest_view<ApproximatelySizedView>>);
static_assert(HasMemberReserveHint<const std::ranges::cache_latest_view<ApproximatelySizedView>>);
...
static_assert(HasMemberReserveHint<std::ranges::cache_latest_view<ApproximatelySizedNotConstView>>);
static_assert(!HasMemberReserveHint<const std::ranges::cache_latest_view<ApproximatelySizedNotConstView>>);

constexpr bool test() {
  // Non-const and const are reserve_hint-able
  {
    auto view = std::views::cache_latest(ApproximatelySizedView{82});
    assert(view.reserve_hint() == 82);
    assert(std::as_const(view).reserve_hint() == 82);
  }
