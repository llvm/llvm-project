...
int main(int, char**) {
  {
    ...
    assert(std::find_first_of(
               ...
               std::equal_to<int>()) == cpp17_input_iterator<const int*>(ia + 1));
    int ic[] = {7};
    assert(std::find_first_of(
               cpp17_input_iterator<const int*>(ia),
               cpp17_input_iterator<const int*>(ia + sa),
               ...
               std::equal_to<int>()) == cpp17_input_iterator<const int*>(ia + sa));
    assert(std::find_first_of(
               cpp17_input_iterator<const int*>(ia),
               cpp17_input_iterator<const int*>(ia + sa),
               forward_iterator<const int*>(ic),
    ...
    assert(std::find_first_of(
               ...
               forward_iterator<const int*>(ic),
               forward_iterator<const int*>(ic + 1),
               std::equal_to<int>()) == cpp17_input_iterator<const int*>(ia));
  }
  {
    int a[8192];
    ...
    a[3071] = -2;
    int b[] = {-1, 999999};
    assert(std::find_first_of(std::begin(a), std::end(a), std::begin(b), std::end(b), Pred()) == std::begin(a) + 1023);
  }
  {
    int a[1073];
    ...
    for (unsigned i = 0; i < sa; i = i < 16 ? i + 1 : unsigned(3.1415 * i)) {
      a[i] = -2;
      assert(std::find_first_of(std::begin(a), std::end(a), std::begin(b), std::end(b), Pred()) == std::begin(a) + i);
      a[i] = i;
    };
  }
