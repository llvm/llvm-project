...
static void test_const_member() {
  {
    ...
    static_assert(std::is_convertible_v<time_point, std::chrono::zoned_time<duration>>);
  ...
  }
  {
    using duration   = std::chrono::nanoseconds;
    using time_point = std::chrono::sys_time<duration>;
    static_assert(std::is_convertible_v<time_point, const std::chrono::zoned_time<duration>>);
...
static void test_duration_conversion() {
  ...
  // common_type_t<duration, seconds> -> seconds
  {
    ...
    std::same_as<std::chrono::sys_seconds> decltype(auto) time = static_cast<std::chrono::sys_seconds>(zt);
    assert(time == time_point{duration{42}});
  }
  {
