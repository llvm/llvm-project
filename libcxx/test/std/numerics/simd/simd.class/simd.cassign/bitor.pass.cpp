...
template <class T>
concept has_bitor = requires(T v) { v |= v; };
...
constexpr bool test() {
  types::for_each(types::standard_integer_types{}, []<class T> {
    simd_utils::test_sizes([]<int N>(std::integral_constant<int, N>) {
      ...
      dp::simd<T, N> vec(arr); // make sure operator& is const
      const dp::simd<T, N> mask(T(15));
      std::same_as<dp::simd<T, N>&> auto&& ret = vec |= mask;
      assert(&ret == &vec);
      for (int i = 0; i != N; ++i)
        assert(ret[i] == T((i + 54) | 15));
    });
  });

  static_assert(has_bitor<dp::simd<int>>);

  types::for_each(types::vectorizable_float_types{}, []<class T> {
    simd_utils::test_sizes([]<int N>(std::integral_constant<int, N>) { static_assert(!has_bitor<dp::simd<T, N>>); });
  });
