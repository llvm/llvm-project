...
template <class T>
TEST_CONSTEXPR_CXX20 int classify(const std::complex<T>& x) {
  if (x == std::complex<T>())
    return zero;
...
void is_about(double x, double y) { assert(std::abs((x - y) / (x + y)) < 1.e-14); }

void is_about(long double x, long double y) { assert(std::abs((x - y) / (x + y)) < 1.e-14); }
