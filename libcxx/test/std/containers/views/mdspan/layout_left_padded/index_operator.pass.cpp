...
constexpr void test_operator_result(Args... args) {
  using Mapping = std::layout_left_padded<PaddingValue>::template mapping<Extents>;
...
constexpr bool test() {
  ...
  {
    using Rank0Mapping = std::layout_left_padded<4>::mapping<std::extents<int>>;
    using Rank1Mapping = std::layout_left_padded<D>::mapping<std::extents<int, D>>;
  ...
  {
    using SignedMapping   = std::layout_left_padded<D>::mapping<std::extents<int, D>>;
    using UnsignedMapping = std::layout_left_padded<D>::mapping<std::extents<unsigned, D>>;
  ...
  {
    using SignedMapping   = std::layout_left_padded<D>::mapping<std::extents<signed char, D>>;
    using UnsignedMapping = std::layout_left_padded<D>::mapping<std::extents<unsigned char, D>>;
  ...
  test_operator_result(
      std::layout_left_padded<D>::mapping<std::extents<unsigned, 5, 7>>(std::extents<unsigned, 5, 7>(), 6));
  test_operator_result(
      std::layout_left_padded<D>::mapping<std::extents<unsigned, D, 2, 3>>(std::extents<unsigned, D, 2, 3>(3), 4));

  {
    using Mapping = std::layout_left_padded<4>::mapping<std::extents<int, 2, 2>>;

    static_assert(operator_constraints<Mapping, RValueInt, RValueInt>);

    constexpr Mapping mapping(std::extents<int, 2, 2>{});
    static_assert(mapping(RValueInt{0}, RValueInt{1}) == expected_result(mapping, RValueInt{0}, RValueInt{1}));
  }
