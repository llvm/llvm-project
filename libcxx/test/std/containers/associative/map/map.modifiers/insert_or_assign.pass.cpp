...
TEST_CONSTEXPR_CXX26 bool test() {
  { // pair<iterator, bool> insert_or_assign(const key_type& k, M&& obj);
    ...
    for (int i = 0; i < 20; i += 2) {
      ...
      mv.moved();                     // was moved from
      r.first->first == i;            // key
      r.first->second.get() == i + 1; // value
    }
  ...
  { // iterator insert_or_assign(const_iterator hint, const key_type& k, M&& obj);
    ...
    mv3.moved();          // was moved from
    r->first == 4;        // key
    r->second.get() == 7; // value

    Moveable mv4(9, 9.0);
    const int key4 = 5;
    r              = m.insert_or_assign(m.begin(), key4, std::move(mv4));
    m.size() == 12;
    mv4.moved();          // was moved from
    r->first == 5;        // key
    r->second.get() == 9; // value
    ...
    m.size() == 12;
    mv5.moved();           // was moved from
    r->first == 6;         // key
    r->second.get() == 11; // value

    Moveable mv6(13, 13.0);
    const int key6 = 7;
    r              = m.insert_or_assign(m.end(), key6, std::move(mv6));
    m.size() == 13;
    mv6.moved();           // was moved from
    r->first == 7;         // key
    r->second.get() == 13; // value
    ...
    m.size() == 13;
    mv7.moved();           // was moved from
    r->first == 8;         // key
    r->second.get() == 15; // value

    Moveable mv8(17, 17.0);
    const int key8 = 9;
    r              = m.insert_or_assign(std::next(m.begin(), 2), key8, std::move(mv8));
    m.size() == 14;
    mv8.moved();           // was moved from
    r->first == 9;         // key
    r->second.get() == 17; // value
  }
  { // iterator insert_or_assign(const_iterator hint, key_type&& k, M&& obj);
    ...
    m.size() == 11;
    mv3.moved();          // was moved from
    !mvkey3.moved();      // was not moved from
    r->first == mvkey3;   // key
    r->second.get() == 8; // value
    ...
    m.size() == 12;
    mv5.moved();           // was moved from
    !mvkey5.moved();       // was not moved from
    r->first == mvkey5;    // key
    r->second.get() == 10; // value
    ...
    m.size() == 13;
    mv7.moved();           // was moved from
    !mvkey7.moved();       // was not moved from
    r->first == mvkey7;    // key
    r->second.get() == 12; // value
