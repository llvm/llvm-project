...
static TEST_ALIGN_BENCHMARK void BM_format_to_string_back_inserter(benchmark::State& state) {
...
template <class Container>
static TEST_ALIGN_BENCHMARK void BM_format_to_string_begin(benchmark::State& state) {
  using CharT = typename Container::value_type;
  size_t size = state.range(0);
  auto str    = std::basic_string<CharT>(size, CharT('*'));
...
static TEST_ALIGN_BENCHMARK void BM_format_to_string_span(benchmark::State& state) {
  size_t size = state.range(0);
  auto str    = std::basic_string<CharT>(size, CharT('*'));
...
template <class CharT>
static TEST_ALIGN_BENCHMARK void BM_format_to_string_pointer(benchmark::State& state) {
  size_t size = state.range(0);
  auto str    = std::basic_string<CharT>(size, CharT('*'));
