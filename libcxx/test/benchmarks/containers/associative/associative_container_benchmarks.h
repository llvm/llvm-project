...
void associative_container_benchmarks(std::string container) {
  ...
  bench("ctor(Self&&, const allocator_type&) (different allocs)", [=](auto& st) TEST_ALIGN_BENCHMARK {
  ...
  bench("insert(iterator, iterator) (all new keys, end)",
        [=](auto& state) TEST_ALIGN_BENCHMARK { insert_iter_iter_bench(true, state); });
  bench("insert(iterator, iterator) (all new keys, middle)",
        [=](auto& state) TEST_ALIGN_BENCHMARK { insert_iter_iter_bench(false, state); });

  bench("insert(iterator, iterator) (half new keys)", [=](auto& st) TEST_ALIGN_BENCHMARK {
    const std::size_t size = st.range(0);
    std::vector<Value> in  = make_value_types(generate_unique_keys(size));
  ...
  if constexpr (is_map_like) {
    bench("insert(iterator, iterator) (product_iterator from same type)", [=](auto& st) TEST_ALIGN_BENCHMARK {
      const std::size_t size = st.range(0);
      std::vector<Value> in  = make_value_types(generate_unique_keys(size + (size / 10)));
...
#if TEST_STD_VER >= 23
    bench("insert(iterator, iterator) (product_iterator from zip_view)", [=](auto& st) TEST_ALIGN_BENCHMARK {
      const std::size_t size = st.range(0);
  ...
  bench("erase(iterator, iterator) (erase half the container)", [=](auto& st) TEST_ALIGN_BENCHMARK {
    const std::size_t size = st.range(0);
    std::vector<Value> in  = make_value_types(generate_unique_keys(size));
    Container c(in.begin(), in.end());
