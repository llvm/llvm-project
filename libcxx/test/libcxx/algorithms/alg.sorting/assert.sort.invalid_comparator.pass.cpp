...
void check_oob_sort_read() {
  ...
  }
  {
    ...
    for (auto const& e : elements)
      copy.push_back(e.get());
    TEST_LIBCPP_ASSERT_FAILURE(std::stable_sort(copy.begin(), copy.end(), checked_predicate),
  ...
  }
  {
    ...
    TEST_LIBCPP_ASSERT_FAILURE(std::sort_heap(copy.begin(), copy.end(), checked_predicate),
                               "Your comparator is not a valid strict-weak ordering");
  }
  {
    std::vector<std::size_t*> copy;
    for (auto const& e : elements)
      copy.push_back(e.get());
    TEST_LIBCPP_ASSERT_FAILURE(std::partial_sort(copy.begin(), copy.end(), copy.end(), checked_predicate),
                               "Your comparator is not a valid strict-weak ordering");
  }
  {
    std::vector<std::size_t*> copy;
  ...
  }
  {
    ...
    for (auto const& e : elements)
      copy.push_back(e.get());
    TEST_LIBCPP_ASSERT_FAILURE(
        std::ranges::stable_sort(copy, checked_predicate), "Your comparator is not a valid strict-weak ordering");
  }
  {
    std::vector<std::size_t*> copy;
    for (auto const& e : elements)
    ...
    TEST_LIBCPP_ASSERT_FAILURE(
        std::ranges::sort_heap(copy, checked_predicate), "Your comparator is not a valid strict-weak ordering");
  }
  {
    std::vector<std::size_t*> copy;
    for (auto const& e : elements)
      copy.push_back(e.get());
    TEST_LIBCPP_ASSERT_FAILURE(std::ranges::partial_sort(copy, copy.end(), checked_predicate),
                               "Your comparator is not a valid strict-weak ordering");
  }
  {
    std::vector<std::size_t*> copy;
    for (auto const& e : elements)
      copy.push_back(e.get());
    std::vector<std::size_t*> results(copy.size(), nullptr);
    TEST_LIBCPP_ASSERT_FAILURE(std::ranges::partial_sort_copy(copy, results, checked_predicate),
                               "Your comparator is not a valid strict-weak ordering");
  }
}
...
void check_oob_nth_element_read() {
...

  {
    ...
    for (auto const& e : elements)
      copy.push_back(e.get());
    TEST_LIBCPP_ASSERT_FAILURE(
        std::nth_element(copy.begin(), copy.begin(), copy.end(), checked_predicate),
        "Would read out of bounds, does your comparator satisfy the strict-weak ordering requirement?");
  }

  {
    ...
    for (auto const& e : elements)
      copy.push_back(e.get());
    TEST_LIBCPP_ASSERT_FAILURE(
        std::ranges::nth_element(copy, copy.begin(), checked_predicate),
        "Would read out of bounds, does your comparator satisfy the strict-weak ordering requirement?");
  }
}
...
void check_irreflexive() {
  ...
  TEST_LIBCPP_ASSERT_FAILURE(
      std::ranges::sort_heap(v, std::greater_equal<int>()), "Your comparator is not a valid strict-weak ordering");
}
