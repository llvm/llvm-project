...
private:
  ...
  public:
    ...
    friend constexpr bool operator==(const __outer_iterator& __x, default_sentinel_t) {
      _LIBCPP_ASSERT_NON_NULL(__x.__parent_, "Cannot call comparison on a default-constructed iterator.");
  ...
  private:
    ...
    constexpr bool __is_done() const {
      _LIBCPP_ASSERT_NON_NULL(__i_.__parent_, "Cannot call comparison on a default-constructed iterator.");
