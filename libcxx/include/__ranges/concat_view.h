...
public:
  ...
  public:
        ...
        requires concat_is_random_access<Const, Views...>
    {
        size_t active_index = it_.index();
        if(n > 0) {
             std::visit([&](auto& active_it) {
                apply_fn_with_const_index<std::tuple_size_v<decltype(parent_->views_)>>(active_index, [&](auto index_constant) {
                    constexpr size_t I = index_constant.value;
        ...
        if(ix > iy)
        {
            ...
            [&](auto& it_x, auto& it_y) {
                it_x.apply_fn_with_const_index<std::tuple_size_v<decltype(x.parent_->views_)>>(ix, [&](auto index_constant){
                    ...
                    it_y.apply_fn_with_const_index<std::tuple_size_v<decltype(y.parent_->views_)>>(iy, [&](auto index_constant){
    ...
    friend constexpr difference_type operator-(const iterator& x, default_sentinel_t) 
        requires (sized_sentinel_for<sentinel_t<__maybe_const<Const, Views>>, 
                    iterator_t<__maybe_const<Const, Views>>> && ...) && (apply_drop_first<Const, Views...>::value)
    {
        size_t ix = x.it_.index(); 
        std::visit(
        [&](auto& it_x) {
