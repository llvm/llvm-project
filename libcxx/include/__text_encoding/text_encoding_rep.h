...
_LIBCPP_EXPORTED_FROM_ABI __text_encoding_rep __get_locale_encoding(const char* __name);
...
public:
  ...
  [[nodiscard]] _LIBCPP_HIDE_FROM_ABI constexpr __aliases_view __aliases() const noexcept {
    if (!__encoding_rep_->__name_[0])
      return __aliases_view(nullptr);
    return __aliases_view(__encoding_rep_);
  }

  _LIBCPP_HIDE_FROM_ABI friend constexpr bool
  operator==(const __text_encoding_rep& __a, const __text_encoding_rep& __b) noexcept {
    if (__a.__mib() == __id::other && __b.__mib() == __id::other)
      return __comp_name(__a.__name_, __b.__name_);

    return __a.__mib() == __b.__mib();
  }

  _LIBCPP_HIDE_FROM_ABI friend constexpr bool operator==(const __text_encoding_rep& __encoding, __id __i) noexcept {
    return __encoding.__mib() == __i;
  }

#  if _LIBCPP_HAS_LOCALIZATION
  [[nodiscard]] _LIBCPP_EXPORTED_FROM_ABI static __text_encoding_rep __environment() {
    return __get_locale_encoding("");
  }

  template <__id __i>
  [[nodiscard]] _LIBCPP_HIDE_FROM_ABI static bool __environment_is() {
    return __environment() == __i;
  }
#  else
  [[nodiscard]] _LIBCPP_HIDE_FROM_ABI static text_encoding __environment() = delete;
