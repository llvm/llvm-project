...
public:
  ...
  _LIBCPP_HIDE_FROM_ABI constexpr mapping(const extents_type& __ext, _OtherIndexType __padding) : __extents_(__ext) {
    ...
    _LIBCPP_ASSERT_VALID_ELEMENT_ACCESS(
        ...
        "layout_left_padded::mapping(extents, padding): padding must be representable as index_type.");
    _LIBCPP_ASSERT_VALID_ELEMENT_ACCESS(
        __pad > 0, "layout_left_padded::mapping(extents, padding): padding must be greater than 0.");

    if constexpr (padding_value != dynamic_extent)
      _LIBCPP_ASSERT_VALID_ELEMENT_ACCESS(
          padding_value == __pad, "layout_left_padded::mapping(extents, padding): padding must equal padding_value.");

    if constexpr (__rank_ > 1) {
      _LIBCPP_ASSERT_VALID_ELEMENT_ACCESS(
          __mdspan_detail::__least_multiple_at_least_is_representable_as<index_type>(
              static_cast<index_type>(__pad), __ext.extent(0)),
          "layout_left_padded::mapping(extents, padding): padded stride must be representable as index_type.");
      ...
      _LIBCPP_ASSERT_VALID_ELEMENT_ACCESS(
          __padded_product_is_representable(__ext, __stride_1),
          "layout_left_padded::mapping(extents, padding): required span size must be representable as index_type.");
