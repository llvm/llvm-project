...
; RUN: %ocloc compile -file %S/Inputs/branch.cl -device %inter-device -out_dir %t.ref -q
; RUN: %python %make-zebin extract %t.ref/branch_bmg.bin %t.meta
; RUN: %python %make-zebin write --kernel branch_kernel --text %t.text.bin --zeinfo %t.meta/zeinfo.yaml --notes %t.meta/note.compat.bin -o %t.bin
