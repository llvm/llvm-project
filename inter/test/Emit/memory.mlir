...
module {
  func.func @memory() attributes {xemachine.target = #xemachine.target<chip = "bmg">} {
    ...
    // CHECK-NEXT: {{.*}}opcode=send exec=1 {{.*}}sfid=gateway exdesc=0x0 desc=0x2000004 {{.*}}dst=arf0 src0=grf8 src1=arf0 len=0 eot=0
    ...
    // CHECK-NEXT: {{.*}}opcode=send exec=1 {{.*}}sfid=gateway exdesc=0x0 desc=0x2000010 {{.*}}dst=arf0 src0=grf8 src1=arf0 len=0 eot=1
