/// Check BOLT's handling of a dynamically linked PIE binary on RISC-V.
/// On RV32 BOLT currently only supports statically linked, non-PIE
/// binaries, so the input must be rejected with a clear error. On RV64
/// the same input is supported and BOLT must not emit the RV32 error.
...
// RUN: ld.lld -pie -dynamic-linker /lib/ld.so.1 \
