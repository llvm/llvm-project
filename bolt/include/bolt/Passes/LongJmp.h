...
class LongJmpPass : public BinaryFunctionPass {
  ...
  /// Exact emitted address ranges for the current layout iteration.
  DenseMap<const FunctionFragment *, std::pair<uint64_t, uint64_t>>
      FragmentAddresses;
  ...
  using LocalTrampolineTy =
      std::pair<BinaryBasicBlock *, std::unique_ptr<BinaryBasicBlock>>;
  using LocalTrampolineListTy = std::vector<LocalTrampolineTy>;

  /// State local to relaxing one function fragment. It must remain local to
  /// the invocation because functions are relaxed in parallel.
  struct LocalBranchState {
    BinaryFunction &Func;
    FunctionFragment &FF;
    ...
    /// First trampoline created for each destination basic block.
    DenseMap<const BinaryBasicBlock *, BinaryBasicBlock *> Trampolines;
  };

  bool isBranchOffsetInRange(const BinaryFunction &Func, const MCInst &Inst,
                             int64_t Offset) const;

  bool isBlockInRange(const BinaryFunction &Func, const MCInst &Inst,
                      uint64_t InstAddress, const BinaryBasicBlock &BB) const;

  void adjustBasicBlockAddress(BinaryBasicBlock &BB, uint64_t Address,
                               uint64_t Offset) const;

  /// Create a local unconditional-branch trampoline after \p BB, or at the
  /// end of the fragment if \p BB is null.
  BinaryBasicBlock *
  addLocalTrampoline(LocalBranchState &State, BinaryBasicBlock *BB,
                     const MCSymbol *TargetSymbol, BinaryBasicBlock *TargetBB,
                     uint64_t Count, uint64_t Offset = 0) const;

  /// Extract terminating unconditional branches into reusable trampoline
  /// blocks without changing the fragment size.
  void splitUnconditionalBranches(LocalBranchState &State) const;

  /// Redirect an out-of-range branch in a simple function through a local
  /// trampoline. Return true if the fragment size changes.
  bool relaxLocalBranch(LocalBranchState &State, BinaryBasicBlock &BB,
                        MCInst &Inst, uint64_t InstAddress,
                        BinaryBasicBlock &TargetBB) const;

  /// Relax an external branch in a non-simple function. Return false after
  /// reporting an error if the fragment-end trampoline is unreachable.
  bool relaxExternalBranch(
  ...
  /// Relax all short branches in one function fragment.
  bool relaxLocalFragment(BinaryFunction &Func, FunctionFragment &FF,
                          const BranchLivenessInfo *BLI,
                          LocalTrampolineListTy &FunctionTrampolines) const;

  /// Add all trampolines accumulated while relaxing \p Func to its CFG and
  /// layout.
  void commitLocalTrampolines(BinaryFunction &Func,
                              LocalTrampolineListTy &FunctionTrampolines) const;
  ...
  /// Return false after reporting an error if the layout cannot be relaxed.
  bool relaxWithClusters(BinaryContext &BC);
