...
In the example below, the first BasicBlock ends in a conditional branch, and jumps to two different BasicBlocks, each with their own authentication, and return. The return at the end of the second BasicBlock is next to the first instruction of the third BasicBlock, but they have different RA statas: the return address has already been authenticated for the return, hence it has Unsigned state. The third BasicBlock is past the pointer signing, but not the authentication. In this case, a negate-ra-state is needed after the return.
