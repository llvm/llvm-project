...
bool ScopBuilder::buildAccessMemIntrinsic(MemAccInst Inst, ScopStmt *Stmt) {
  ...
  SCEVUse LengthVal = SE.getSCEVAtScope(MemIntr->getLength(), L);
  assert(LengthVal);
  ...
  SCEVUse DestAccFunc = SE.getSCEVAtScope(DestPtrVal, L);
  assert(DestAccFunc);
  ...
  SCEVUse SrcAccFunc = SE.getSCEVAtScope(SrcPtrVal, L);
  assert(SrcAccFunc);
  // Ignore accesses to "NULL".
  // TODO: See above TODO
...
bool ScopBuilder::buildAccessCallInst(MemAccInst Inst, ScopStmt *Stmt) {
  ...
  SCEVUse AF = SE.getConstant(IntegerType::getInt64Ty(CI->getContext()), 0);
  ...
  if (ME.onlyAccessesArgPointees()) {
    ...
    for (const auto &Arg : CI->args()) {
      ...
      SCEVUse ArgSCEV = SE.getSCEVAtScope(Arg, L);
      if (ArgSCEV->isZero())
        continue;
...
static bool isDivisible(const SCEV *Expr, unsigned Size, ScalarEvolution &SE) {
  ...
  if (auto *NAryExpr = dyn_cast<SCEVNAryExpr>(Expr)) {
    for (SCEVUse OpExpr : NAryExpr->operands())
      if (!isDivisible(OpExpr, Size, SE))
        return false;
    return true;
  }

  SCEVUse SizeSCEV = SE.getConstant(Expr->getType(), Size);
  SCEVUse UDivSCEV = SE.getUDivExpr(Expr, SizeSCEV);
  SCEVUse MulSCEV = SE.getMulExpr(UDivSCEV, SizeSCEV);
  return MulSCEV == Expr;
}
...
void ScopBuilder::buildScop(Region &R, AssumptionCache &AC) {
  ...
  SCEVUse AF = SE.getConstant(IntegerType::getInt64Ty(SE.getContext()), 0);
