...
// Append the single-operand "llvm.loop.vectorize.disable" node to \p L,
// keeping any existing loop metadata intact.
static void addVectorizeDisableMetadataToLoop(Loop *L) {
  ...
  MDs.push_back(
      MDNode::get(Ctx, {MDString::get(Ctx, "llvm.loop.vectorize.disable")}));
  ...
  if (!NodeBuilder.preloadInvariantLoads()) {
    ...
    if (!(CI && CI->isZero())) {
      for (Loop *L : LI.getLoopsInPreorder()) {
        if (S.contains(L))
          addVectorizeDisableMetadataToLoop(L);
      }
    }
