...
public:
  ...
  ValidatorResult visitUDivExpr(const SCEVUDivExpr *Expr) {
    ...
    SCEVUse Dividend = Expr->getLHS();
    SCEVUse Divisor = Expr->getRHS();
  ...
  ValidatorResult visitSDivInstruction(Instruction *SDiv, const SCEV *Expr) {
    ...
    SCEVUse Dividend = SE.getSCEV(SDiv->getOperand(0));
    SCEVUse Divisor = SE.getSCEV(SDiv->getOperand(1));
  ...
  ValidatorResult visitSRemInstruction(Instruction *SRem, const SCEV *S) {
    ...
    SCEVUse DividendSCEV = SE.getSCEV(Dividend);
...
public:
  ...
  bool follow(const SCEV *S) {
    ...
    SCEVUse Dividend = SE.getSCEV(Inst->getOperand(1));
    if (!isa<SCEVConstant>(Dividend))
      return false;

    SCEVUse Divisor = SE.getSCEV(Inst->getOperand(0));
...
polly::extractConstantFactor(const SCEV *S, ScalarEvolution &SE) {
  ...
  if (auto *Add = dyn_cast<SCEVAddExpr>(S)) {
    ...
    SCEVUse NewAdd = SE.getAddExpr(LeftOvers, Add->getNoWrapFlags());
