; RUN: opt %loadPolly -polly-analyze-read-only-scalars=false -passes=polly-codegen \
; RUN:     \
; RUN:     -S < %s | FileCheck %s
; RUN: opt %loadPolly -polly-analyze-read-only-scalars=true -passes=polly-codegen \
; RUN:     \
