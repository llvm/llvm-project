; RUN: opt %loadPolly -polly-allow-nonaffine-loops '-passes=print<polly-function-scops>' -disable-output < %s 2>&1 | FileCheck %s
; RUN: opt %loadPolly -polly-allow-nonaffine-loops -passes=polly-codegen -disable-output
;
; CHECK:      Domain :=
