...
===============================================================================================================================
    ...
    Debug the binary xyz.out by invoking gdb with the plugin as shown below. Please note that plugin '<..>/ompd/__init__.py' should be used
        ...
        - The plugin uses LLVM Support to load libompd.so. After install, ompdModule.so has a RUNPATH back to <prefix>/lib so GDB can find libLLVMSupport.so or libLLVM.so. If LLVM lives in another prefix, put that lib directory on LD_LIBRARY_PATH.
