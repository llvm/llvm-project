...
// RUN:   OMP_NUM_THREADS_DEV_000000000001=10 \
...
// (c) leading zeros are a valid device id (`000000000001` is device 1);
// (d) sibling valid `_ALL=4` still applies.
...
int main(void) {
  ...
  const char *q = __kmpc_get_device_env("OMP_NUM_THREADS", 2147483646);
  if (q == NULL || strcmp(q, "4") != 0) {
    fprintf(stderr, "FAIL: large valid id expected '4', got %s\n",
            q ? q : "(null)");
    return 1;
  }
  q = __kmpc_get_device_env("OMP_NUM_THREADS", 1);
  if (q == NULL || strcmp(q, "10") != 0) {
    fprintf(stderr, "FAIL: padded device id expected '10', got %s\n",
            q ? q : "(null)");
    return 1;
  }
  printf("DONE\n");
...
// CHECK-NOT: {{^OMP: Warning #[0-9]+:.*OMP_NUM_THREADS_DEV_000000000001.*}}
