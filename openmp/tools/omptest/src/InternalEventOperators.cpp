...
bool operator==(const ImplicitTask &Expected, const ImplicitTask &Observed) {
  ...
  bool isSameActualParallelism =
      (Expected.ActualParallelism == std::numeric_limits<unsigned int>::min())
          ? true
...
bool operator==(const Target &Expected, const Target &Observed) {
  bool isSameKind = (Expected.Kind == Observed.Kind);
  bool isSameEndpoint = (Expected.Endpoint == Observed.Endpoint);
  bool isSameDeviceNum = (Expected.DeviceNum == std::numeric_limits<int>::min())
                             ? true
...
bool operator==(const TargetDataOp &Expected, const TargetDataOp &Observed) {
  ...
  bool isSameSrcAddr = (Expected.SrcAddr == std::numeric_limits<void *>::min())
                           ? true
                           : (Expected.SrcAddr == Observed.SrcAddr);
  bool isSameDstAddr = (Expected.DstAddr == std::numeric_limits<void *>::min())
                           ? true
...
bool operator==(const DeviceInitialize &Expected,
  ...
  bool isSameType = (Expected.Type == std::numeric_limits<const char *>::min())
                        ? true
...
bool operator==(const BufferRecord &Expected, const BufferRecord &Observed) {
  ...
  case ompt_callback_target_submit:
    ...
    break;
  default:
    (static_cast<bool>(false && "Encountered invalid record type")
         ? void(0)
         : __assert_fail("false && \"Encountered invalid record type\"",
                         "../src/InternalEventOperators.cpp", 285,
