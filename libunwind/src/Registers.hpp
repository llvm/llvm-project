...
#define RUN_IF_PAUTH_FEATURE_PRESENT(scratchReg, code)                \
  ...
  "cbnz " #scratchReg ", " LOCAL_LABEL_PREFIX "check_pac_code%=_" STRING(__LINE__) "\n\t" \
  "mrs  " #scratchReg ", ID_AA64ISAR2_EL1"                     "\n\t" \
  "lsr  " #scratchReg ", " #scratchReg ", #8"                  "\n\t" \
  "ands " #scratchReg ", " #scratchReg ", #15"                 "\n\t" \
  "cbnz " #scratchReg ", " LOCAL_LABEL_PREFIX "check_pac_code%=_" STRING(__LINE__) "\n\t" \
  "b " LOCAL_LABEL_PREFIX "check_pac_end%=_" STRING(__LINE__)                      "\n\t" \
  LOCAL_LABEL_PREFIX "check_pac_code%=_" STRING(__LINE__) ":"                   "\n\t" \
  code                                                         "\n\t" \
  LOCAL_LABEL_PREFIX "check_pac_end%=_" STRING(__LINE__) ":"                    "\n\t"
...
#define CHECK_SIGNING_SCHEME_FLAGS_INTEGRITY(schemeReg, schemePacReg, \
  ...
  RUN_IF_PAUTH_FEATURE_PRESENT(                                       \
    ...
    "b.eq  " LOCAL_LABEL_PREFIX "check_integrity_success_%=_" STRING(__LINE__)     "\n\t" \
    PACGA_TRAP                                                 "\n\t" \
    LOCAL_LABEL_PREFIX "check_integrity_success_%=_" STRING(__LINE__) ":"       "\n\t" \
  )

#define CHECK_SIGNING_SCHEME_FLAGS_INTEGRITY_FOR_PAUTHABI(schemeReg)         \
  "cmp   " #schemeReg ", 5"                                           "\n\t" \
  "b.eq  " LOCAL_LABEL_PREFIX "check_integrity_for_pauthabi_success_%=_" STRING(__LINE__) "\n\t" \
  PACGA_TRAP                                                          "\n\t" \
  LOCAL_LABEL_PREFIX "check_integrity_for_pauthabi_success_%=_" STRING(__LINE__) ":"   "\n\t" \

#define SIGNING_SCHEME_FLAGS_SWITCH(schemeReg, codeIf0, codeIf1, \
                                    codeIf3, codeIf5, codeIf7)   \
  "cmp " #schemeReg ", #0"                        "\n\t"         \
  "b.ne " LOCAL_LABEL_PREFIX "switch_1_%=_"   STRING(__LINE__)        "\n\t"         \
  codeIf0                                         "\n\t"         \
  "b    " LOCAL_LABEL_PREFIX "switch_end_%=_" STRING(__LINE__)        "\n\t"         \
  LOCAL_LABEL_PREFIX "switch_1_%=_" STRING(__LINE__) ":"           "\n\t"         \
  "cmp " #schemeReg ", #1"                        "\n\t"         \
  "b.ne " LOCAL_LABEL_PREFIX "switch_3_%=_" STRING(__LINE__)          "\n\t"         \
  codeIf1                                         "\n\t"         \
  "b    " LOCAL_LABEL_PREFIX "switch_end_%=_" STRING(__LINE__)        "\n\t"         \
  LOCAL_LABEL_PREFIX "switch_3_%=_" STRING(__LINE__) ":"           "\n\t"         \
  "cmp " #schemeReg ", #3"                        "\n\t"         \
  "b.ne " LOCAL_LABEL_PREFIX "switch_5_%=_"   STRING(__LINE__)        "\n\t"         \
  codeIf3                                         "\n\t"         \
  "b    " LOCAL_LABEL_PREFIX "switch_end_%=_" STRING(__LINE__)        "\n\t"         \
  LOCAL_LABEL_PREFIX "switch_5_%=_" STRING(__LINE__) ":"           "\n\t"         \
  "cmp " #schemeReg ", #5"                        "\n\t"         \
  "b.ne " LOCAL_LABEL_PREFIX "switch_7_%=_"   STRING(__LINE__)        "\n\t"         \
  codeIf5                                         "\n\t"         \
  "b    " LOCAL_LABEL_PREFIX "switch_end_%=_" STRING(__LINE__)        "\n\t"         \
  LOCAL_LABEL_PREFIX "switch_7_%=_" STRING(__LINE__) ":"           "\n\t"         \
  "cmp " #schemeReg ", #7"                        "\n\t"         \
  "b.ne " LOCAL_LABEL_PREFIX "switch_unexpected%=_" STRING(__LINE__)  "\n\t"         \
  codeIf7                                         "\n\t"         \
  "b    " LOCAL_LABEL_PREFIX "switch_end_%=_" STRING(__LINE__)        "\n\t"         \
  LOCAL_LABEL_PREFIX "switch_unexpected%=_" STRING(__LINE__) ":"   "\n\t"         \
  PACGA_TRAP                                      "\n\t"         \
  LOCAL_LABEL_PREFIX "switch_end_%=_" STRING(__LINE__) ":"         "\n\t"
...
public:
  ...
  void setRASigningScheme(uint64_t raSignState,
    ...
    asm(
        ...
        SIGNING_SCHEME_FLAGS_SWITCH(
            /*schemeReg=*/x16,
            /*codeIf0=*/"cbnz  x15, " LOCAL_LABEL_PREFIX "setscheme_unexpected%=\n\t",
            /*codeIf1=*/"cbnz  x15, " LOCAL_LABEL_PREFIX "setscheme_unexpected%=\n\t",
            /*codeIf3=*/"cbz   x15, " LOCAL_LABEL_PREFIX "setscheme_unexpected%=\n\t",
            /*codeIf5=*/"cbnz  x15, " LOCAL_LABEL_PREFIX "setscheme_unexpected%=\n\t",
            /*codeIf7=*/"cbz   x15, " LOCAL_LABEL_PREFIX "setscheme_unexpected%=\n\t"
            )
        "b " LOCAL_LABEL_PREFIX "setscheme_ok%=       \n\t"
        LOCAL_LABEL_PREFIX "setscheme_unexpected%=:\n\t"
        PACGA_TRAP               "\n\t"
        LOCAL_LABEL_PREFIX "setscheme_ok%=:        \n\t"
        "str x16, [x17, #0]       \n\t"
        RUN_IF_PAUTH_FEATURE_PRESENT(
            /*scratchReg=*/x14,
            ".arch armv8.3-a      \n\t"
...
private:
  ...
  void setZeroSigningScheme() {
    ...
    asm(// Ensure that second modifier is zero
        "ldr x16, [x17, #16]    \n\t"
        "cbz x16, " LOCAL_LABEL_PREFIX "setzero_ok%=\n\t"
        PACGA_TRAP             "\n\t"
        LOCAL_LABEL_PREFIX "setzero_ok%=:        \n\t"
  ...
  uint64_t getRASigningSchemeFlags() const {
    ...
    asm(
        ...
        RUN_IF_PAUTH_FEATURE_PRESENT(
            ...
            "pacga x17, x16, x17            \n\t"
            "cmp   x17, x15                 \n\t"
            "b.eq  " LOCAL_LABEL_PREFIX "get_ra_scheme_success%=\n\t"
            PACGA_TRAP                     "\n\t"
            LOCAL_LABEL_PREFIX "get_ra_scheme_success%=:     \n\t"
            "pacga x14, x16, x14            \n\t"
            "orr   x16, x14, x16            \n\t"
        )
        : "+r"(x16)
