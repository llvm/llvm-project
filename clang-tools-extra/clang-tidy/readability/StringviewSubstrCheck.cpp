...
void StringViewSubstrCheck::registerMatchers(MatchFinder *Finder) {
  ...
  // Match various forms of zero, including (size_t)0, static_cast<size_t>(0)
  const auto IsZero =
      expr(anyOf(ignoringParenCasts(integerLiteral(equals(0))),
...
void StringViewSubstrCheck::check(const MatchFinder::MatchResult &Result) {
  ...
  if (PrefixN) {
    ...
    const std::string PrefixText =
        tooling::fixit::getText(*PrefixN, *Result.Context).str();
  ...
  if (LengthCall && SuffixN) {
    ...
    const std::string SuffixText =
        tooling::fixit::getText(*SuffixN, *Result.Context).str();
