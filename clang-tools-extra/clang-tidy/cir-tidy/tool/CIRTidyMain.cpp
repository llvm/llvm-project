...
static cl::opt<std::string>
    Checks("checks", cl::desc(R"(Comma-separated list of globs with optional '-'
...
static cl::opt<std::string>
    WarningsAsErrors("warnings-as-errors",
                     cl::desc(R"(Upgrades warnings to errors. Same format as
'-checks'.
...
static cl::opt<bool> Fix("fix",
                         cl::desc(R"(Apply suggested fixes. Without -fix-errors
...
static cl::opt<bool>
    FixErrors("fix-errors",
              cl::desc(R"(Apply suggested fixes even if compilation
...
static cl::opt<bool>
    FixNotes("fix-notes",
             cl::desc(R"(If a warning has no fix, but a single fix can
...
.clang-tidy file, if any.
                ...
                cl::init("none"), cl::cat(CIRTidyCategory));

static cl::opt<bool>
    ListChecks("list-checks",
               cl::desc(R"(List all enabled checks and exit. Use with
...
static cl::opt<bool>
    ExplainConfig("explain-config",
                  cl::desc(R"(For each enabled check explains, where it is
...
static cl::opt<std::string>
    Config("config", cl::desc(R"(Specifies a configuration in YAML/JSON format:
  -config="{Checks: '*',
            CheckOptions: [{key: x,
                            value: y}]}"
...
static cl::opt<std::string> ConfigFile(
    "config-file",
    cl::desc(R"(Specify the path of .clang-tidy or custom config file:
...
static cl::opt<bool>
    DumpConfig("dump-config",
               cl::desc(R"(Dumps configuration in the YAML format to
...
static cl::opt<bool>
    EnableCheckProfile("enable-check-profile",
                       cl::desc(R"(Enable per-check timing profiles, and print a
report to stderr.
)"),
                       cl::init(false), cl::cat(CIRTidyCategory));

static cl::opt<std::string>
    StoreCheckProfile("store-check-profile",
                      cl::desc(R"(By default reports are printed in tabulated
format to stderr. When this option is passed,
these per-TU profiles are instead stored as JSON.
)"),
                      cl::value_desc("prefix"), cl::cat(CIRTidyCategory));
...
static cl::opt<std::string>
    ExportFixes("export-fixes",
                cl::desc(R"(YAML file to store suggested fixes in. The
stored fixes can be applied to the input source
code with cir-apply-replacements.
)"),
                cl::value_desc("filename"), cl::cat(CIRTidyCategory));

static cl::opt<bool>
    Quiet("quiet", cl::desc(R"(Run cir-tidy in quiet mode. This suppresses
...
static cl::opt<std::string>
    VfsOverlay("vfsoverlay",
               cl::desc(R"(Overlay the virtual filesystem described by file
...
static cl::opt<bool>
    UseColor("use-color",
             cl::desc(R"(Use colors in diagnostics. If not set, colors
