...
// --target=x86_64-linux -fno-delayed-template-parsing -Wno-array-compare-cxx26
// RUN: %check_clang_tidy %s misc-redundant-expression %t -- --
// --target=x86_64-linux -fno-delayed-template-parsing -Wno-array-compare-cxx26
// -DTEST_MACRO
...
int TestSimpleEquivalent(int X, int Y) {
  ...
  if (X + 1 == X + 1)
    return 1;
  // CHECK-MESSAGES: :[[@LINE-1]]:13: warning: both sides of operator are
  // equivalent
  if (X + 1 != X + 1)
    return 1;
  // CHECK-MESSAGES: :[[@LINE-1]]:13: warning: both sides of operator are
  ...
  if ((X != 1 || Y != 1) && (X != 1 || Y != 1))
    return 1;
  // CHECK-MESSAGES: :[[@LINE-1]]:26: warning: both sides of operator are
  // equivalent
  if (P.a[X - P.x] != P.a[X - P.x])
    return 1;
  // CHECK-MESSAGES: :[[@LINE-1]]:20: warning: both sides of operator are
  // equivalent
  ...
  if (+"dummy" == +"dummy")
    return 1;
  // CHECK-MESSAGES: :[[@LINE-1]]:18: warning: both sides of operator are
  // equivalent
  if (L"abc" == L"abc")
    return 1;
  // CHECK-MESSAGES: :[[@LINE-1]]:14: warning: both sides of operator are
  ...
  if (P2.a[P1.x + 2] < P2.x && P2.a[(P1.x) + (2)] < (P2.x))
    return 1;
  // CHECK-MESSAGES: :[[@LINE-1]]:29: warning: both sides of operator are
  // equivalent

  if (X && Y && X)
    return 1;
  // CHECK-MESSAGES: :[[@LINE-1]]:14: warning: operator has equivalent nested
  // operands
  if (X || (Y || X))
    return 1;
  // CHECK-MESSAGES: :[[@LINE-1]]:9: warning: operator has equivalent nested
  // operands
  if ((X ^ Y) ^ (Y ^ X))
    return 1;
  // CHECK-MESSAGES: :[[@LINE-1]]:15: warning: operator has equivalent nested
  // operands
...
int TestDefineEquivalent() {
  ...
  if ((+VAL_6) == (+VAL_6))
    return 2;
  // CHECK-MESSAGES: :[[@LINE-1]]:16: warning: both sides of operator are
  // equivalent
  if ((-VAL_6) == (-VAL_6))
    return 2;
  // CHECK-MESSAGES: :[[@LINE-1]]:16: warning: both sides of operator are
  ...
  if (0xFF <= 0xFF)
    return 0;
  // CHECK-MESSAGES: :[[@LINE-1]]:12: warning: both sides of operator are
  // equivalent
  if (042 > 042)
    return 0;
  // CHECK-MESSAGES: :[[@LINE-1]]:11: warning: both sides of operator are
  // equivalent
  ...
  TestStruct tsVar3 = {
      .mA = 3, .mB = int_val, .mC[0 ... VAL_3 - VAL_3] = int_val + 1,
      // CHECK-MESSAGES: :[[@LINE-1]]:21: warning: both sides of operator are
      // equivalent
  };

  TestStruct tsVar4 = {
      .mA = 3, .mB = int_val, .mC[0 ... 5 - 5] = int_val + 1,
      // CHECK-MESSAGES: :[[@LINE-1]]:17: warning: both sides of operator are
      // equivalent
  };

  return 1 + int_val + sizeof(tsVar1) + sizeof(tsVar2) + sizeof(tsVar3) +
         sizeof(tsVar4);
}
...
unsigned int testLoops(const unsigned int arr1[LOOP_DEFINE]) {
  ...
  // CHECK-MESSAGES: :[[@LINE-1]]:41: warning: both sides of operator are
  // equivalent
  {
  }

  for (localIndex = LOOP_DEFINE - 1; LOOP_DEFINE > LOOP_DEFINE; localIndex--)
  // CHECK-MESSAGES: :[[@LINE-1]]:50: warning: both sides of operator are
  // equivalent
  {
  }
...
int TestParamDefine() {
  ...
  ret += +getValue(1) > +getValue(1);
  // CHECK-MESSAGES: :[[@LINE-1]]:23: warning: both sides of operator are
  // equivalent
  ret += -getValue(1) > -getValue(1);
  // CHECK-MESSAGES: :[[@LINE-1]]:23: warning: both sides of operator are
  // equivalent
  ...
  ret += getValue(1) > getValue(1);
  // CHECK-MESSAGES: :[[@LINE-1]]:22: warning: both sides of operator are
  // equivalent
  ret += getValue(1) > getValue(1);
  // CHECK-MESSAGES: :[[@LINE-1]]:22: warning: both sides of operator are
  // equivalent
  ...
  // equivalent
  ret += getValue(1) > getValue(1);
  // CHECK-MESSAGES: :[[@LINE-1]]:27: warning: both sides of operator are
  // equivalent
  ret += getValue(1) > getValue(1);
  // CHECK-MESSAGES: :[[@LINE-1]]:28: warning: both sides of operator are
  // equivalent
  ret += getValue(VAL_5) > getValue(VAL_5);
  // CHECK-MESSAGES: :[[@LINE-1]]:32: warning: both sides of operator are
  // equivalent
  ret += getValue(VAL_5) > getValue(VAL_5);
  // CHECK-MESSAGES: :[[@LINE-1]]:32: warning: both sides of operator are
  // equivalent
  ...
  // CHECK-MESSAGES: :[[@LINE-1]]:42: warning: both sides of operator are
  // equivalent
  ret +=
      (getValue(VAL_1) + getValue(VAL_2)) > (getValue(VAL_1) + getValue(VAL_2));
  // CHECK-MESSAGES: :[[@LINE-1]]:44: warning: both sides of operator are
  // equivalent
  return ret;
}
...
int TestConditional(int x, int y) {
  int k = 0;
  k += (y < 0) ? x : x;
  // CHECK-MESSAGES: :[[@LINE-1]]:20: warning: 'true' and 'false' expressions
  // are equivalent
  k += (y < 0) ? x + 1 : x + 1;
...
#undef COND_OP_THIRD_MACRO
  ...
  // CHECK-MESSAGES: :[[@LINE-1]]:30: warning: 'true' and 'false' expressions
  // are equivalent
  k += (y < 0) ? sizeof(TestConditional(k, y)) : sizeof(TestConditional(k, y));
  // CHECK-MESSAGES: :[[@LINE-1]]:47: warning: 'true' and 'false' expressions
  // are equivalent No warning if the expression arguments are different.
...
MyStruct1 operator&&(const MyStruct1 &lhs, const MyStruct1 &rhs) {
  return lhs.x && rhs.x;
}
...
bool TestOverloadedOperator(MyStruct &S) {
  ...
  if (S == S)
    return true;
  // CHECK-MESSAGES: :[[@LINE-1]]:9: warning: both sides of overloaded operator
  // are equivalent
  if (S < S)
    return true;
  // CHECK-MESSAGES: :[[@LINE-1]]:9: warning: both sides of overloaded operator
...
template <typename T, typename U> void TemplateCheck() {
...
int TestArithmetic(int X, int Y) {
  if (X + 1 == X)
    return 1;
  // CHECK-MESSAGES: :[[@LINE-1]]:13: warning: logical expression is always
  // false
  if (X + 1 != X)
    return 1;
...
int TestBitwise(int X, int Y) {
  ...
  if ((X & 0xFF) == 0xF00)
    return 1;
  // CHECK-MESSAGES: :[[@LINE-1]]:18: warning: logical expression is always
  // false
  if ((X & 0xFF) != 0xF00)
    return 1;
  ...
  if ((0xFFLL & X) == 0xF00)
    return 1;
  // CHECK-MESSAGES: :[[@LINE-1]]:20: warning: logical expression is always
  // false
  if ((0xFF & X) == 0xF00ULL)
    return 1;
  // CHECK-MESSAGES: :[[@LINE-1]]:18: warning: logical expression is always
...
bool TestBitwiseUInt128(unsigned __int128 Value) {
  constexpr unsigned __int128 BigBit = static_cast<unsigned __int128>(1) << 100;

  if ((Value & BigBit) == BigBit)
    return true;

  constexpr unsigned __int128 LowMask = static_cast<unsigned __int128>(0xFF);

  if ((Value & LowMask) == BigBit)
    return true;
...
template <int N> bool TestBitwiseUInt128Template(unsigned __int128 Value) {
...
#undef CONFIG
  ...
  if (!(X != 10U) && !(X == 10))
    return 1;
  // CHECK-MESSAGES: :[[@LINE-1]]:19: warning: logical expression is always
  // false
  if (!(X != 10LL) && !(X == 10))
    return 1;
  // CHECK-MESSAGES: :[[@LINE-1]]:20: warning: logical expression is always
  ...
  if (!(X != 10ULL) && !(X == 10))
    return 1;
  // CHECK-MESSAGES: :[[@LINE-1]]:21: warning: logical expression is always
  // false
  ...
  if (X && !!X)
    return 1;
  // CHECK-MESSAGES: :[[@LINE-1]]:9: warning: equivalent expression on both
  // sides of logical operator
  if (X != 0 && X)
    return 1;
  // CHECK-MESSAGES: :[[@LINE-1]]:14: warning: equivalent expression on both
  // sides of logical operator
  if (X != 0 && !!X)
    return 1;
  // CHECK-MESSAGES: :[[@LINE-1]]:14: warning: equivalent expression on both
  ...
  if (s1 == 1 && s1 == 1)
    return true;
  // CHECK-MESSAGES: :[[@LINE-1]]:15: warning: equivalent expression on both
  // sides of logical operator
  if (s1 == 1 || s1 != 1)
    return true;
...
#define SOME_OTHER_MACRO 9
  ...
  if (X < SOME_MACRO && X > SOME_MACRO)
    return 1;
  // CHECK-MESSAGES: :[[@LINE-1]]:22: warning: logical expression is always
  // false
  if (X < SOME_MACRO && X == SOME_MACRO)
    return 1;
  // CHECK-MESSAGES: :[[@LINE-1]]:22: warning: logical expression is always
...
template <class T> int TestRelationalTemplated(int X) {
...
int TestOperatorConfusion(int X, int Y, long Z) {
  ...
  int K = !(1 | 2 | 4);
  // CHECK-MESSAGES: :[[@LINE-1]]:11: warning: ineffective logical negation
  // operator used; did you mean '~'? CHECK-FIXES: int K = ~(1 | 2 | 4);
  K = !(FLAG1 & FLAG2 & FLAG3);
  // CHECK-MESSAGES: :[[@LINE-1]]:7: warning: ineffective logical negation
  // operator CHECK-FIXES: K = ~(FLAG1 & FLAG2 & FLAG3);
  K = !(3 | 4);
  // CHECK-MESSAGES: :[[@LINE-1]]:7: warning: ineffective logical negation
  // operator CHECK-FIXES: K = ~(3 | 4);
  int NotFlags = !FLAGS;
  // CHECK-MESSAGES: :[[@LINE-1]]:18: warning: ineffective logical negation
  // operator CHECK-FIXES: int NotFlags = ~FLAGS;
  NotFlags = NOTFLAGS;
  // CHECK-MESSAGES: :[[@LINE-1]]:14: warning: ineffective logical negation
  // operator
  return !(1 | 2 | 4);
  // CHECK-MESSAGES: :[[@LINE-1]]:10: warning: ineffective logical negation
  // operator CHECK-FIXES: return ~(1 | 2 | 4);
}

template <int Shift, int Mask> int TestOperatorConfusionDependent(int Y) {
  int r1 = (Y << Shift) & 0xff;
  int r2 = (Y << 8) & Mask;
}
#undef FLAG1
...
template <class... Values> struct Bar2 {
...
void TestGH145415() {
  ...
  bool b1 =
      std::tuple_size<MonthArray>::value == std::tuple_size<ZodiacArray>::value;
