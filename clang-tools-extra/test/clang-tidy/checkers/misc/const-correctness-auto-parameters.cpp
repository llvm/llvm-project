...
void generic_lambda_param() {
  auto generic = [](auto& b) { b.const_method(); };
  // CHECK-MESSAGES: [[@LINE-1]]:3: warning: variable 'generic' of type '(lambda at {{.*}})' can be declared 'const'
  // CHECK-FIXES: auto const generic = [](auto& b) { b.const_method(); };
  Bar bar;
  generic(bar);
}
