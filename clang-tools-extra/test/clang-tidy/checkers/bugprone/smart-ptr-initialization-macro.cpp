...
void test_shared_ptr_constructor_macro1() {
  SHARED_PTR_A a(&getA());
  // CHECK-MESSAGES: :[[@LINE-1]]:18: warning: passing a raw pointer '&getA()' to std::shared_ptr<A> constructor may cause double deletion
}

void test_unique_ptr_constructor_macro1() {
  UNIQUE_PTR_A b(&getA());
  // CHECK-MESSAGES: :[[@LINE-1]]:18: warning: passing a raw pointer '&getA()' to std::unique_ptr<A> constructor may cause double deletion
}
...
void test_shared_ptr_constructor_macro3() {
  ...
  // CHECK-MESSAGES: :[[@LINE-1]]:3: warning: passing a raw pointer '&getA()' to std::shared_ptr<A> constructor may cause double deletion
}

void test_unique_ptr_constructor_macro3() {
  UNIQUE_PTR_THE_WHOLE_STATEMENT_IN_MACRO
  // CHECK-MESSAGES: :[[@LINE-1]]:3: warning: passing a raw pointer '&getA()' to std::unique_ptr<A> constructor may cause double deletion
}
...
void test_shared_ptr_constructor_macro4() {
  ...
  // CHECK-MESSAGES: :[[@LINE-1]]:22: warning: passing a raw pointer '&getA()' to std::shared_ptr<A> constructor may cause double deletion
}

void test_unique_ptr_constructor_macro5() {
  std::unique_ptr<A> COMPLICATED_SOURCE_LOCATION_FOR_B getA());
  // CHECK-MESSAGES: :[[@LINE-1]]:22: warning: passing a raw pointer '&getA()' to std::unique_ptr<A> constructor may cause double deletion
}
