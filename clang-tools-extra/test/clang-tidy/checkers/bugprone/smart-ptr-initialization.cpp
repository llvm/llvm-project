...
void test_shared_ptr_constructor() {
  ...
  // CHECK-MESSAGES: :[[@LINE-1]]:24: warning: passing a raw pointer '&getA()' to std::shared_ptr<A> constructor may cause double deletion
}

void test_unique_ptr_constructor() {
  std::unique_ptr<A> b(&getA());
  // CHECK-MESSAGES: :[[@LINE-1]]:24: warning: passing a raw pointer '&getA()' to std::unique_ptr<A> constructor may cause double deletion
}

void test_stack_variable() {
  int x = 5;
  std::unique_ptr<int> ptr(&x);
  // CHECK-MESSAGES: :[[@LINE-1]]:28: warning: passing a raw pointer '&x' to std::unique_ptr<int> constructor may cause double deletion
}
...
struct S {
  ...
  void test() {
    ...
    // CHECK-MESSAGES: :[[@LINE-1]]:30: warning: passing a raw pointer '&this->member' to std::unique_ptr<int> constructor may cause double deletion
  }
};

void test_function_return() {
  std::shared_ptr<A> sp(getAPtr());
  // CHECK-MESSAGES: :[[@LINE-1]]:25: warning: passing a raw pointer 'getAPtr()' to std::shared_ptr<A> constructor may cause double deletion
}
...
void test_shared_ptr_reset() {
  std::shared_ptr<A> a;
  a.reset(&getA());
  // CHECK-MESSAGES: :[[@LINE-1]]:11: warning: passing a raw pointer '&getA()' to std::shared_ptr<A>::reset(...) may cause double deletion
}

void test_unique_ptr_reset() {
  std::unique_ptr<A> b;
  b.reset(&getA());
  // CHECK-MESSAGES: :[[@LINE-1]]:11: warning: passing a raw pointer '&getA()' to std::unique_ptr<A>::reset(...) may cause double deletion
}
...
void test_stack_variable_reset() {
  ...
  ptr.reset(&x);
  // CHECK-MESSAGES: :[[@LINE-1]]:13: warning: passing a raw pointer '&x' to std::unique_ptr<int>::reset(...) may cause double deletion
}

void test_function_return_reset() {
  std::shared_ptr<A> sp;
  sp.reset(getAPtr());
  // CHECK-MESSAGES: :[[@LINE-1]]:12: warning: passing a raw pointer 'getAPtr()' to std::shared_ptr<A>::reset(...) may cause double deletion
}
...
void test_shared_ptr_constructor_template() {
  T a(&getA());
  // CHECK-MESSAGES: :[[@LINE-1]]:7: warning: passing a raw pointer '&getA()' to std::shared_ptr<A> constructor may cause double deletion
}
