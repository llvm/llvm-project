...
#define MAKE_LAST_N(obj, n) obj.subspan(obj.size() - n)
  ...
  auto sub_zero = s.subspan(ZERO, 3);
  // CHECK-MESSAGES: :[[@LINE-1]]:19: warning: prefer 'span::first()' over 'subspan()'
  // CHECK-FIXES: auto sub_zero = s.first(3);

  auto sub_two = s.subspan(s.size() - TWO);
  // CHECK-MESSAGES: :[[@LINE-1]]:18: warning: prefer 'span::last()' over 'subspan()'
  // CHECK-FIXES: auto sub_two = s.last(TWO);
