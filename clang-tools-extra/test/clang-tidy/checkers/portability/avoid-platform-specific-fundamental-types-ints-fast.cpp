...
// CHECK-MESSAGES: {{.*note: 'int_fast64_t' suggested for compatibility with Unix, which uses 64-bit 'long'.*|^$}}
...
// CHECK-MESSAGES: {{.*note: 'uint_fast64_t' suggested for compatibility with Unix, which uses 64-bit 'unsigned long'.*|^$}}
