...
void read_size_index(const std::vector<int> &v) {
  // CHECK-MESSAGES: :[[@LINE-1]]:46: warning: parameter 'v' can be changed to 'std::span'; it is only used for read-only access [modernize-use-span]
...
void read_data(const std::vector<int> &v) {
  // CHECK-MESSAGES: :[[@LINE-1]]:40: warning: parameter 'v' can be changed to 'std::span'; it is only used for read-only access [modernize-use-span]
  const int *p = v.data();
  (void)v.size();
}
...
void read_empty(const std::vector<int> &v) {
  // CHECK-MESSAGES: :[[@LINE-1]]:41: warning: parameter 'v' can be changed to 'std::span'; it is only used for read-only access [modernize-use-span]
  if (v.empty())
    return;
}
...
void pass_to_const_ref(const std::vector<int> &v) {
  // CHECK-MESSAGES: :[[@LINE-1]]:48: warning: parameter 'v' can be changed to 'std::span'; it is only used for read-only access [modernize-use-span]
  consumer(v);
}
