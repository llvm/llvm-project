...
// The extract/extractu/bclr/bset/insert ops have a register-operand form
// (cv.*r) and an immediate form (cv.*, position as a constant); both are
// exercised below.

// CHECK-LABEL: test_extract_r:
// CHECK: cv.extractr
int32_t test_extract_r(int32_t a0, uint16_t a1) {
  return __riscv_cv_bitmanip_extract(a0, a1);
}

// CHECK-LABEL: test_extract_i:
// CHECK: cv.extract {{.*}}, 5
int32_t test_extract_i(int32_t a0) {
  return __riscv_cv_bitmanip_extract(a0, 5);
}

// CHECK-LABEL: test_extractu_r:
// CHECK: cv.extractur
uint32_t test_extractu_r(uint32_t a0, uint16_t a1) {
...
// CHECK: cv.bclrr
uint32_t test_bclr_r(uint32_t a0, uint16_t a1) {
...
// CHECK: cv.bsetr
uint32_t test_bset_r(uint32_t a0, uint16_t a1) {
...
// CHECK: cv.insertr
uint32_t test_insert_r(uint32_t a0, uint16_t a1, uint32_t a2) {
  return __riscv_cv_bitmanip_insert(a0, a1, a2);
}

// CHECK-LABEL: test_insert_i:
// CHECK: cv.insert {{.*}}, 5
uint32_t test_insert_i(uint32_t a0, uint32_t a2) {
  return __riscv_cv_bitmanip_insert(a0, 5, a2);
}
