...
Pass 分为三条独立的 Pipeline：早期 AOT（标准优化前）、晚期 AOT（标准优化后）、JIT（运行时）。

**Pipeline 时序**:

```
Clang CodeGen → [早期 AOT] → O2/O3 标准优化 → [晚期 AOT] → 目标代码生成
                       ↓                              ↓
              提取原始 bitcode              wrapper 插桩 + period 注册
              (metadata 完整)               (优化后的最终 IR)
```
...
│  EJitRegisterBitcode                          │
└───────────────────────────────────────────────┘
输出: @__ejit_bitcode 全局变量 (metadata 完整的原始 IR)
...
│  EJitPeriodHandler                            │
└───────────────────────────────────────────────┘
输出: 可执行文件 (含 wrapper 插桩 + period 注册代码)
