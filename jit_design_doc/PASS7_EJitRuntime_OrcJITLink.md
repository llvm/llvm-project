...
```cpp
// [BiSheng] llvm/lib/ExecutionEngine/EJIT/EJitOrcEngine.h
...
EJitOrcEngine::Create(const EJitConfig& config) {
    ...
    // [BiSheng] 步骤 1: 创建嵌入式 JITLinkMemoryManager
    ...
    // [BiSheng] 步骤 2: 配置 JITTargetMachineBuilder
    auto JTMB = cantFail(JITTargetMachineBuilder::detectHost());

    // [BiSheng] 步骤 3: 构建 LLJIT
    ...
    engine->J->getIRTransformLayer().setTransform(
        [engine](orc::ThreadSafeModule TSM,
            ...
            return TSM.withModuleDo([engine](Module& M)
                -> Expected<orc::ThreadSafeModule> {
                if (!engine->ActiveCtx)
                    return std::move(TSM);
    ...
    // [BiSheng] 步骤 5: 注册 Process Symbols JITDylib
...
```cpp
// [BiSheng] llvm/lib/ExecutionEngine/EJIT/EJitJITLinkMemoryManager.h
...
public:
    ...
    // [BiSheng] 分配: 解析 LinkGraph → 分配 Code + Data 段 → 返回
    void allocate(const jitlink::JITDylib* JD,
                  jitlink::LinkGraph& G,
                  OnAllocatedFunction OnAllocated) override;

    // [BiSheng] 释放: 归还已 finalize 的分配
...
private:
    ...
    // [BiSheng] 使用 JITLink 的 BasicLayout 辅助划分段
    struct SegmentAlloc {
...
void EJitJITLinkMemoryManager::allocate(
    ...
    // [BiSheng] 步骤 1: 使用 BasicLayout 分析段布局
    BasicLayout BL(G);

    // [BiSheng] 步骤 2: 为每个 AllocGroup 分配内存
    ...
    // [BiSheng] 步骤 3: 应用布局到 LinkGraph
    BL.apply();

    // [BiSheng] 步骤 4: 创建 InFlightAlloc 并传给 callback
...
```cpp
// [BiSheng] 异步编译器: 后台线程 + 请求队列 + 隔离引擎实例
...
public:
    ...
    // [BiSheng] 停止后台线程 (等待当前编译完成)
    void stop();

    // [BiSheng] 提交异步编译请求 (非阻塞)
...
void EJitAsyncCompiler::submitRequest(CompileRequest req) {
    {
        ...
        // [BiSheng] 去重: 相同 cacheKey 已有编译在进行中，跳过
...
void EJitAsyncCompiler::workerLoop() {
    while (!stopping_.load()) {
        ...
        // [BiSheng] 在后台线程编译 (使用隔离的 Engine 实例)
        compileOne(req);
    }
...
void EJitAsyncCompiler::compileOne(const CompileRequest& req) {
    ...
    if (result.funcPtr) {
        // [BiSheng] 编译成功: 存入 Cache (内部 mutex 保护)
        cache_.put(req.cacheKey, result.funcPtr, result.codeSize);
    }
    // [BiSheng] 编译失败: 不存缓存, 后续调用 retry 或 fallback

    // [BiSheng] 编译完成 (成功或失败), 从 in-flight 集合移除
    {
        std::lock_guard<std::mutex> lock(inFlightMutex_);
        requestsInFlight_.erase(req.cacheKey);
    }
...
engine->J->getIRTransformLayer().setTransform(
    [engine](orc::ThreadSafeModule TSM, ...) -> Expected<orc::ThreadSafeModule> {
        return TSM.withModuleDo([engine](Module& M) -> Expected<orc::ThreadSafeModule> {
            if (!engine->ActiveCtx)
                return std::move(TSM);
...
private:
    ...
    // [BiSheng] period 失效索引: periodName:cellIdx → cacheKey 集合
...
```cpp
// [BiSheng] llvm/lib/ExecutionEngine/EJIT/EJitRegistrationStore.h

namespace llvm::ejit {

// [BiSheng] 全局注册数据暂存区 (进程级单例)
// 生命周期:
...
public:
    ...
    // [BiSheng] --- 写入接口 (constructor 阶段, 单线程) ---
...
```cpp
// [BiSheng] 进程级 EJIT 单例，由 ejit_init 创建，ejit_shutdown 释放
struct EJitInstance {
...
ejit_status_t ejit_init(const ejit_config_t* config) {
    ...
    auto syncEngine = cantFail(EJitOrcEngine::Create(cfg));
    ...
    if (cfg.compileMode == CompileMode::Async) {
        asyncEngine = cantFail(EJitOrcEngine::Create(cfg));
...
```cpp
// [BiSheng] ThreadSafeModule 的正确使用
// 用户线程 (同步模式):
...
```cpp
// [BiSheng] BitcodeTracker: 维护 funcName → bitcode 位置映射
...
class BitcodeTracker {
    ...
    // [BiSheng] 按需加载: 首次调用时 parseModule
...
// [BiSheng] 日志记录 (环形缓冲区, 避免 malloc 在错误路径失败)
class EJitLogger {
