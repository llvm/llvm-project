...
- 在函数入口计算 cacheKey（纯寄存器 shl|or|zext）
- 插入 `ejit_compile_or_get(cacheKey, NULL)` 调用（i64 预计算 key）
...
2. ForEach entryFunc:
     ...
     GenerateWrapperPrologue(entryFunc, dims) → 计算 cacheKey（寄存器操作）+ ejit_compile_or_get(i64 cacheKey, null) 调用
...
变换为:
    ...
    entry_jit:          ; 新 entry → Wrapper 逻辑
        ; ... dims 构建
        ; ... ejit_compile_or_get(i32 funcIdx, ...)
...
WrapperResult insertWrapperPrologue(Function* F, const DimInfo& dims,
    ...
    // Step 7: 调用 ejit_compile_or_get(i32 funcIdx, ...)
...
jit_entry:
  ...
  %jit_result = call ptr @ejit_compile_or_get(i32 <hash>, ptr null, i32 0, ptr null)
...
jit_entry:
  ...
  %jit_result = call ptr @ejit_compile_or_get(i32 <hash>, ptr %dims_ptr, i32 1, ptr null)
  %jit_is_null = icmp eq ptr %jit_result, null
  br i1 %jit_is_null, label %jit_fallback, label %jit_dispatch
...
jit_entry:
  ...
  i32 <hash>  ; FNV-1a hash of function name
  %dims_ptr = bitcast ... to ptr
  %jit_result = call ptr @ejit_compile_or_get(i32 <hash>, ptr %dims_ptr, i32 2, ptr null)
  %jit_is_null = icmp eq ptr %jit_result, null
  br i1 %jit_is_null, label %jit_fallback, label %jit_dispatch
...
// cacheKey 为 u64，无需 struct
// cacheKey = (funcIdx << 32) | d[0] | (d[1] << 8) | ...
// IR 类型: { i8*, i8 }
```
...
| cacheKey 计算 | 纯 ALU 操作 (shl/zext/or)，零栈分配 |
...
;   CHECK: call ptr @ejit_compile_or_get(i32 <hash>, ptr null, i32 0, ptr null)
;
; TEST 2: 单维度参数 — dims[1] 构建
;   CHECK: alloca [1 x { ptr, i8 }]
;   CHECK: call ptr @ejit_compile_or_get(i32 <hash>, ptr %dims_ptr, i32 1, ptr null)
;
; TEST 3: 多维度参数 — dims[2] 构建
;   CHECK: alloca [2 x { ptr, i8 }]
;   CHECK: call ptr @ejit_compile_or_get(i32 <hash>, ptr %dims_ptr, i32 2, ptr null)
;
...
| 验证项 | 方法 |
|--------|------|
| cacheKey 计算正确 | FileCheck 匹配 shl + zext + or |
...
2. **寄存器操作**: cacheKey 计算全部在寄存器中完成，无需栈分配。

3. **类型转换 (ZExt/Trunc)**: period_arr_ind 参数可能声明为 `uint8_t` / `int16_t` 等类型，ZExt 到 i64 后 OR 进 key。使用 `ZExtOrTrunc` 处理无符号/有符号参数。
