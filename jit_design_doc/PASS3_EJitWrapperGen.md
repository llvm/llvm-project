...
- 插入 `ejit_compile_or_get(funcIdx, dims, count, NULL)` 调用（funcIdx 为 FNV-1a 32-bit hash）
...
2. ForEach entryFunc:
     ...
     GenerateWrapperPrologue(entryFunc, dims) → 生成 dims 构建 + ejit_compile_or_get(i32 funcIdx, ...) 调用
