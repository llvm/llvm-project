...
- 真实 SRE 平台：定义 `EJIT_SRE_TASKPOOL_PLATFORM_QUEUE` 后，`push/pop` 路由到
  `QueueCreate/QueueWrite/QueueRead`（asm-label 到真实符号）。这些平台符号只声明、
  不定义、不提供 weak 兜底；缺符号应在链接期暴露，避免静态包 / partial-link 场景下
  本地 weak 定义遮蔽平台真实实现。SRE 符号被限制在该文件的明确区域内，不散落到业务逻辑。
...
## 10.3 可定位性 trace（默认无输出）

taskpool 关键路径埋了默认空展开的 trace 宏，便于上板时改成 `SRE_printf`：
...
- 上板时可用 `-D'EJIT_TASKPOOL_TRACE(...)=SRE_printf(__VA_ARGS__)'` 重定义。
- 参数只用**整数 / 指针 / C 字符串**，不构造 `std::string`、不使用 `raw_ostream`。
- 埋点覆盖 `compileOrGet`（enter / cacheHit / dedup / queuePush）、`runCompile`
  （begin / compiled / published）、`pollOne`（empty / dequeued）和 `freeCode`。
