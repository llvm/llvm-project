...
static bool CheckSymbolSupportsType(const Scope &scope,
  ...
  // updated global scope context was skipping those module scopes.
  // This also covers reductions declared in a module and used via USE association
  // in another module, as the reduction details are stored on the symbol in the
  // this uses user detalils, so the symbol is looked up in the module scope and then the reduction details are checked.
  if (const auto *sym{scope.FindSymbol(name)}) {
    if (const auto *useDetails = sym->detailsIf<UseDetails>()) {
        if (const auto *reductionDetails = 
                useDetails->symbol().detailsIf<UserReductionDetails>()) {
            return reductionDetails->SupportsType(type);
        }
    }
}
