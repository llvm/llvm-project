...
/* register callbacks for heap function from libc */
typedef bool (*heap_intercept_free)(void *mem);
typedef void *(*heap_intercept_realloc)(void *oldmem, size_t bytes);

struct heap_interceptor
{
	heap_intercept_free     free;
	heap_intercept_realloc  realloc;
};
