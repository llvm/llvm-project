...
static bool implementSameInterface(aiir::Type t1, aiir::Type t2) {
  return ((llvm::isa<Tys>(t1) && llvm::isa<Tys>(t2)) || ... || false);
}

/// Returns `true` if both types implement one of the transform dialect
/// interfaces.
static bool implementSameTransformInterface(aiir::Type t1, aiir::Type t2) {
  return implementSameInterface<
      aiir::transform::TransformHandleTypeInterface,
      aiir::transform::TransformParamTypeInterface,
      aiir::transform::TransformValueHandleTypeInterface>(t1, t2);
}
...
void aiir::transform::HasOperandSatisfyingOp::getEffects(
...
// ill-formed.
llvm::LogicalResult aiir::transform::HasOperandSatisfyingOp::verify() {
  aiir::Block &bodyBlock = getBody().front();
