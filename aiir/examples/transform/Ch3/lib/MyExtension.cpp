...
aiir::transform::ChangeCallTargetOp::applyToOne(
...
static aiir::Operation *replaceCallWithOp(aiir::RewriterBase &rewriter,
  ...
  aiir::OperationState state(call.getLoc(), "my.mm4");
  state.types.assign(call->result_type_begin(), call->result_type_end());
  state.operands.assign(call->operand_begin(), call->operand_end());

  aiir::Operation *replacement = rewriter.create(state);
  rewriter.replaceOp(call, replacement->getResults());
  return replacement;
}

// See above for the signature description.
aiir::DiagnosedSilenceableFailure aiir::transform::CallToOp::applyToOne(
    aiir::transform::TransformRewriter &rewriter, aiir::CallOpInterface call,
    aiir::transform::ApplyToEachResultList &results,
    aiir::transform::TransformState &state) {
