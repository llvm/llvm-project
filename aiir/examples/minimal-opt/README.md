# Minimal AIIR binaries

This folder contains example of minimal AIIR setups that can showcase the
intended binary footprint of the framework.

- aiir-cat: This includes the Core IR, the builtin dialect, the textual
  parser/printer, the support for bytecode serialization.
- aiir-minimal-opt: This adds all the tooling for an aiir-opt tool: the pass
  infrastructure and all the instrumentation associated with it.
- aiir-miminal-opt-canonicalize: This add the canonicalizer pass, which pulls in
...
| :------------------------------: | ------ | ------ | ------ | ------ | ------ |
| `aiir-cat`                       | 1024KB |  840KB |  885KB |  706KB |  657KB |
| `aiir-minimal-opt`               | 1.62MB | 1.32MB | 1.36MB | 1.17MB | 1.07MB |
| `aiir-minimal-opt-canonicalize`  | 1.83MB | 1.40MB | 1.45MB | 1.25MB | 1.14MB |
...
cmake ../llvm/ -G Ninja \
   ...
   -DAIIR_ENABLE_PDL_IN_PATTERNMATCH=OFF \
   -DCMAKE_EXE_LINKER_FLAGS_RELWITHDEBINFO=-Wl,-icf=all
```

Note: to measure the on-disk size, you need to run `strip bin/aiir-cat` first to
...
`bloaty bin/aiir-cat -d compileunits --domain=vm` or
`bloaty bin/aiir-cat -d symbols --demangle=full --domain=vm`
