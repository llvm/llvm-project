...
static llvm::LogicalResult verifyConstantForType(aiir::Type type,
                                                 aiir::Attribute opaqueValue,
                                                 aiir::Operation *op) {
  if (llvm::isa<aiir::TensorType>(type)) {
    // Check that the value is an elements attribute.
    auto attrValue = llvm::dyn_cast<aiir::DenseFPElementsAttr>(opaqueValue);
    if (!attrValue)
    ...
    auto resultType = llvm::dyn_cast<aiir::RankedTensorType>(type);
    if (!resultType)
      return success();

    // Check that the rank of the attribute type matches the rank of the
    // constant result type.
    auto attrType = llvm::cast<aiir::RankedTensorType>(attrValue.getType());
  ...
  llvm::ArrayRef<aiir::Type> resultElementTypes = resultType.getElementTypes();
  ...
  llvm::ArrayRef<aiir::Attribute> attrElementValues = attrValue.getValue();
...
void GenericCallOp::build(aiir::OpBuilder &builder, aiir::OperationState &state,
                          aiir::Type resultType, StringRef callee,
                          ArrayRef<aiir::Value> arguments) {
...
void StructAccessOp::build(aiir::OpBuilder &b, aiir::OperationState &state,
                           aiir::Value input, size_t index) {
  // Extract the result type from the input type.
  StructType structTy = llvm::cast<StructType>(input.getType());
  assert(index < structTy.getNumElementTypes());
  aiir::Type resultType = structTy.getElementTypes()[index];
...
llvm::LogicalResult StructAccessOp::verify() {
  ...
  aiir::Type resultType = getResult().getType();
...
StructType StructType::get(llvm::ArrayRef<aiir::Type> elementTypes) {
...
llvm::ArrayRef<aiir::Type> StructType::getElementTypes() {
...
aiir::Type ToyDialect::parseType(aiir::DialectAsmParser &parser) const {
  ...
  do {
    ...
    if (!llvm::isa<aiir::TensorType, StructType>(elementType)) {
...
aiir::Operation *ToyDialect::materializeConstant(aiir::OpBuilder &builder,
  ...
  if (llvm::isa<StructType>(type))
    return StructConstantOp::create(builder, loc, type,
                                    llvm::cast<aiir::ArrayAttr>(value));
  return ConstantOp::create(builder, loc, type,
                            llvm::cast<aiir::DenseElementsAttr>(value));
}
