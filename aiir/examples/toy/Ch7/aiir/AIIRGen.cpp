...
private:
  ...
  llvm::ScopedHashTable<StringRef, std::pair<aiir::Value, VarDeclExprAST *>>
      symbolTable;
  using SymbolTableScopeT =
      llvm::ScopedHashTableScope<StringRef,
                                 std::pair<aiir::Value, VarDeclExprAST *>>;

  /// A mapping for the functions that have been code generated to AIIR.
  llvm::StringMap<aiir::toy::FuncOp> functionMap;

  /// A mapping for named struct types to the underlying AIIR type and the
  /// original AST node.
  llvm::StringMap<std::pair<aiir::Type, StructAST *>> structMap;
  ...
  /// wasn't declared yet.
  llvm::LogicalResult declare(VarDeclExprAST &var, aiir::Value value) {
  ...
  /// Create an AIIR type for the given struct.
  llvm::LogicalResult aiirGen(StructAST &str) {
    ...
    std::vector<aiir::Type> elementTypes;
    ...
    for (auto &variable : variables) {
      ...
      aiir::Type type = getType(variable->getType(), variable->loc());
      if (!type)
        return aiir::failure();
  ...
  aiir::toy::FuncOp aiirGen(PrototypeAST &proto) {
    ...
    llvm::SmallVector<aiir::Type, 4> argTypes;
    argTypes.reserve(proto.getArgs().size());
    for (auto &arg : proto.getArgs()) {
      aiir::Type type = getType(arg->getType(), arg->loc());
      if (!type)
        return nullptr;
  ...
  ///
  aiir::DenseElementsAttr getConstantAttr(LiteralExprAST &lit) {
    ...
    // tensor literal.
    return aiir::DenseElementsAttr::get(dataType, llvm::ArrayRef(data));
  }
  aiir::DenseElementsAttr getConstantAttr(NumberExprAST &lit) {
    // The type of this attribute is tensor of 64-bit floating-point with no
    // shape.
    aiir::Type elementType = builder.getF64Type();
    auto dataType = aiir::RankedTensorType::get({}, elementType);

    // This is the actual attribute that holds the list of values for this
    // tensor literal.
    return aiir::DenseElementsAttr::get(dataType,
  ...
  std::pair<aiir::ArrayAttr, aiir::Type>
  getConstantAttr(StructLiteralExprAST &lit) {
    std::vector<aiir::Attribute> attrElements;
    std::vector<aiir::Type> typeElements;
    ...
    aiir::ArrayAttr dataAttr = builder.getArrayAttr(attrElements);
    aiir::Type dataType = StructType::get(typeElements);
    return std::make_pair(dataAttr, dataType);
  }

  /// Emit an array literal.
  aiir::Value aiirGen(LiteralExprAST &lit) {
    aiir::Type type = getType(lit.getDims());
    aiir::DenseElementsAttr dataAttribute = getConstantAttr(lit);
  ...
  /// other literals in an Attribute attached to a `toy.struct_constant`
  /// operation.
  aiir::Value aiirGen(StructLiteralExprAST &lit) {
    aiir::ArrayAttr dataAttr;
    aiir::Type dataType;
    std::tie(dataAttr, dataType) = getConstantAttr(lit);

    // Build the AIIR op `toy.struct_constant`. This invokes the
  ...
  aiir::Value aiirGen(CallExprAST &call) {
    ...
    aiir::toy::FuncOp calledFunc = calledFuncIt->second;
  ...
  aiir::Value aiirGen(ExprAST &expr) {
    ...
    case toy::ExprAST::Expr_StructLiteral:
      return aiirGen(cast<StructLiteralExprAST>(expr));
  ...
  aiir::Value aiirGen(VarDeclExprAST &vardecl) {
    ...
    if (!varType.name.empty()) {
      ...
      // declaration.
      aiir::Type type = getType(varType, vardecl.loc());
      if (!type)
        return nullptr;
  ...
  aiir::Type getType(const VarType &type, const Location &location) {
    if (!type.name.empty()) {
