################################################################################
# Python modules
# AIIR's Python modules are both directly used by the core project and are
...
# Function: declare_aiir_python_sources
...
function(declare_aiir_python_sources name)
  ...
  set_target_properties(${name} PROPERTIES
    ...
    EXPORT_PROPERTIES "aiir_python_SOURCES_TYPE;aiir_python_DEPENDS"
    aiir_python_SOURCES_TYPE pure
    aiir_python_DEPENDS ""
  )
  ...
  if(ARG_ADD_TO_PARENT)
    set_property(TARGET ${ARG_ADD_TO_PARENT} APPEND PROPERTY aiir_python_DEPENDS ${name})
  endif()

  # Install.
  set_property(GLOBAL APPEND PROPERTY AIIR_EXPORTS ${name})
  if(NOT LLVM_INSTALL_TOOLCHAIN_ONLY)
    _aiir_python_install_sources(
...
# Function: aiir_generate_type_stubs
...
#     and the core bindings extension module (e.g., something like StandalonePythonModules.extension._aiir.dso).
...
function(aiir_generate_type_stubs)
  ...
  if(AIIR_NB_STUBGEN)
    set(NB_STUBGEN "${AIIR_NB_STUBGEN}")
  elseif(AIIR_NB_STUBGEN_VERSION)
    set(_stubgen_path "${AIIR_BINARY_DIR}/stubgen/${AIIR_NB_STUBGEN_VERSION}/stubgen.py")
    if(NOT EXISTS "${_stubgen_path}")
      message(STATUS "Downloading stubgen.py from nanobind ${AIIR_NB_STUBGEN_VERSION}...")
      file(MAKE_DIRECTORY "${AIIR_BINARY_DIR}/stubgen/${AIIR_NB_STUBGEN_VERSION}" RESULT _created_dir)
      ...
      file(DOWNLOAD
        "https://raw.githubusercontent.com/wjakob/nanobind/${AIIR_NB_STUBGEN_VERSION}/src/stubgen.py"
        "${_stubgen_path}"
  ...
  elseif(EXISTS ${nanobind_SOURCE_DIR}/stubgen.py)
    ...
    message(FATAL_ERROR "aiir_generate_type_stubs(): could not locate 'stubgen.py'!")
  endif()
  ...
  # If PYTHONPATH is set and points to the build location of the python package then when stubgen runs, _aiir will get
  # imported twice and bad things will happen (e.g., Assertion `!instance && “PyGlobals already constructed”’).
  # This happens because aiir is a namespace package and the importer/loader can't distinguish between
  # aiir._aiir_libs._aiir and _aiir_libs._aiir imported from CWD.
  # So try to filter out any entries in PYTHONPATH that end in "AIIR_BINDINGS_PYTHON_INSTALL_PREFIX/.."
  # (e.g., python_packages/aiir_core/).
  set(_pythonpath "$ENV{PYTHONPATH}")
  cmake_path(CONVERT "${AIIR_BINDINGS_PYTHON_INSTALL_PREFIX}/.." TO_NATIVE_PATH_LIST _install_prefix NORMALIZE)
  if(WIN32)
    set(_path_sep ";")
    set(_trailing_sep "\\")
  else()
...
# Function: declare_aiir_python_extension
...
#   MODULE_NAME: Local import name of the module (i.e. "_aiir").
#   ADD_TO_PARENT: Same as for declare_aiir_python_sources.
...
function(declare_aiir_python_extension name)
  cmake_parse_arguments(ARG
  ...
  set_target_properties(${name} PROPERTIES
    # Yes: Leading-lowercase property names are load bearing and the recommended
    # way to do this: https://gitlab.kitware.com/cmake/cmake/-/issues/19261
    EXPORT_PROPERTIES "aiir_python_SOURCES_TYPE;aiir_python_EXTENSION_MODULE_NAME;aiir_python_EMBED_CAPI_LINK_LIBS;aiir_python_DEPENDS"
    aiir_python_SOURCES_TYPE "${SOURCES_TYPE}"
    aiir_python_EXTENSION_MODULE_NAME "${ARG_MODULE_NAME}"
    aiir_python_EMBED_CAPI_LINK_LIBS "${ARG_EMBED_CAPI_LINK_LIBS}"
    aiir_python_DEPENDS ""
  )
...
function(_aiir_python_install_sources name source_root_dir destination)
  foreach(source_relative_path ${ARGN})
    ...
    install(
      ...
      COMPONENT aiir-python-sources
    )
  endforeach()
  get_target_export_arg(${name} AIIR export_to_aiirtargets
    UMBRELLA aiir-python-sources)
  install(TARGETS ${name}
    COMPONENT aiir-python-sources
    ${export_to_aiirtargets}
  )
endfunction()
...
function(build_nanobind_lib)
  cmake_parse_arguments(ARG
    ""
    "INSTALL_COMPONENT;INSTALL_DESTINATION;OUTPUT_DIRECTORY;AIIR_BINDINGS_PYTHON_NB_DOMAIN"
    ""
    ${ARGN})
  ...
  if(NB_ABI MATCHES "[0-9]t")
    ...
    # name includes "-abi3" (e.g., "nanobind-abi3-aiir").
    if(AIIR_ENABLE_PYTHON_STABLE_ABI)
      set(_abi3 "-abi3")
    else()
      set(_abi3 "")
    endif()
  ...
  set(NB_LIBRARY_TARGET_NAME "nanobind${_ft}${_abi3}-${ARG_AIIR_BINDINGS_PYTHON_NB_DOMAIN}")
  ...
  aiir_python_setup_extension_rpath(${NB_LIBRARY_TARGET_NAME})
...
# Function: add_aiir_python_modules
# Adds python modules to a project, building them from a list of declared
# source groupings (see declare_aiir_python_sources and
# declare_aiir_python_extension). One of these must be called for each
...
#   AIIR_BINDINGS_PYTHON_NB_DOMAIN: nanobind (and AIIR) domain within which
...
#     extension depend on (see aiir_python_add_common_capi_library).
function(add_aiir_python_modules name)
  cmake_parse_arguments(ARG
    ""
    "ROOT_PREFIX;INSTALL_PREFIX;AIIR_BINDINGS_PYTHON_NB_DOMAIN"
    "COMMON_CAPI_LINK_LIBS;DECLARED_SOURCES"
    ${ARGN})

  # TODO(max): do the same for AIIR_PYTHON_PACKAGE_PREFIX?
  if((NOT ARG_AIIR_BINDINGS_PYTHON_NB_DOMAIN) AND AIIR_BINDINGS_PYTHON_NB_DOMAIN)
    set(ARG_AIIR_BINDINGS_PYTHON_NB_DOMAIN ${AIIR_BINDINGS_PYTHON_NB_DOMAIN})
  endif()
  if((NOT ARG_AIIR_BINDINGS_PYTHON_NB_DOMAIN) OR ("${ARG_AIIR_BINDINGS_PYTHON_NB_DOMAIN}" STREQUAL ""))
    message(WARNING "AIIR_BINDINGS_PYTHON_NB_DOMAIN CMake var is not set - setting to a default `aiir`.\
      It is highly recommend to set this to something unique so that your project's Python bindings do not collide with\
      others'. You also pass explicitly to `add_aiir_python_modules`.\
      See https://github.com/llvm/llvm-project/pull/171775 for more information.")
    set(ARG_AIIR_BINDINGS_PYTHON_NB_DOMAIN "aiir")
  endif()

  # Collect up all the transitive extension targets.
  _flatten_aiir_python_targets(_flat_targets ${ARG_DECLARED_SOURCES})
  ...
  foreach(sources_target ${_flat_targets})
    get_target_property(_source_type ${sources_target} aiir_python_SOURCES_TYPE)
  ...
  if(_needs_build_nanobind_lib)
    build_nanobind_lib(
      ...
      INSTALL_DESTINATION "${ARG_INSTALL_PREFIX}/_aiir_libs"
      OUTPUT_DIRECTORY "${ARG_ROOT_PREFIX}/_aiir_libs"
      AIIR_BINDINGS_PYTHON_NB_DOMAIN ${ARG_AIIR_BINDINGS_PYTHON_NB_DOMAIN}
    )
  endif()
  ...
  function(_process_target modules_target sources_target support_libs)
    ...
    if(_source_type STREQUAL "pure")
      ...
      add_aiir_python_sources_target(${_pure_sources_target}
    ...
    elseif(_source_type STREQUAL "extension")
      ...
      get_target_property(_module_name ${sources_target} aiir_python_EXTENSION_MODULE_NAME)
      # Transform relative source to based on root dir.
      set(_extension_target "${modules_target}.extension.${_module_name}.dso")
      add_aiir_python_extension(${_extension_target} "${_module_name}" ${NB_LIBRARY_TARGET_NAME}
        INSTALL_COMPONENT ${modules_target}
        INSTALL_DIR "${ARG_INSTALL_PREFIX}/_aiir_libs"
        OUTPUT_DIRECTORY "${ARG_ROOT_PREFIX}/_aiir_libs"
        AIIR_BINDINGS_PYTHON_NB_DOMAIN ${ARG_AIIR_BINDINGS_PYTHON_NB_DOMAIN}
        LINK_LIBS PRIVATE
          ${sources_target}
      ...
      aiir_python_setup_extension_rpath(${_extension_target})
  ...
  foreach(sources_target ${_flat_targets})
    ...
    if(_source_type STREQUAL "support")
      ...
      add_aiir_python_extension(${_extension_target} "${_module_name}" ${NB_LIBRARY_TARGET_NAME}
        ...
        # for the aiir-python bindings. Do not add new dependencies on the C++ LLVM/AIIR
      ...
      aiir_python_setup_extension_rpath(${_extension_target})
      list(APPEND _aiir_python_support_libs "${_extension_target}")
    endif()
  endforeach()

  # Build extensions.
  foreach(sources_target ${_flat_targets})
    _process_target(${name} ${sources_target} "${_aiir_python_support_libs}")
  endforeach()
...
# Function: declare_aiir_dialect_python_bindings
...
#
# Arguments:
#   ROOT_DIR: Same as for declare_aiir_python_sources().
#   ADD_TO_PARENT: Same as for declare_aiir_python_sources(). Unique names
...
#   SOURCES: Same as declare_aiir_python_sources().
#   SOURCES_GLOB: Same as declare_aiir_python_sources().
...
function(declare_aiir_dialect_python_bindings)
  ...
  declare_aiir_python_sources(${_dialect_target}
  ...
  if(ARG_TD_FILE)
    ...
    aiir_tablegen("${dialect_filename}"
    ...
    if(ARG_GEN_ENUM_BINDINGS OR ARG_GEN_ENUM_BINDINGS_TD_FILE)
      ...
      aiir_tablegen(${enum_filename} -gen-python-enum-bindings -bind-dialect=${ARG_DIALECT_NAME})
      list(APPEND _sources ${enum_filename})
    endif()

    # Generated.
    declare_aiir_python_sources("${_dialect_target}.ops_gen"
...
# Function: declare_aiir_dialect_extension_python_bindings
...
#     declare_aiir_dialect_python_bindings is satisfied. Use at your risk.
function(declare_aiir_dialect_extension_python_bindings)
  cmake_parse_arguments(ARG
    "GEN_ENUM_BINDINGS"
  ...
  declare_aiir_python_sources(${_extension_target}
    ROOT_DIR "${ARG_ROOT_DIR}"
    ADD_TO_PARENT "${ARG_ADD_TO_PARENT}"
    SOURCES "${ARG_SOURCES}"
    SOURCES_GLOB "${ARG_SOURCES_GLOB}"
  ...
  if(ARG_TD_FILE)
    ...
    set(LLVM_TARGET_DEFINITIONS ${td_file})
    aiir_tablegen("${output_filename}" -gen-python-op-bindings
    ...
    declare_aiir_python_sources("${_extension_target}.ops_gen"
      ROOT_DIR "${CMAKE_CURRENT_BINARY_DIR}"
...
# Function: aiir_python_setup_extension_rpath
# Sets RPATH properties on a target, assuming that it is being output to
# an _aiir_libs directory with all other libraries. For static linkage,
...
function(aiir_python_setup_extension_rpath target)
  cmake_parse_arguments(ARG
    ""
...
# Function: add_aiir_python_common_capi_library
...
#     target name passed to add_aiir_python_modules().
...
#     library. Typically, this will be the common _aiir_libs directory where
#     all extensions are emitted.
#   RELATIVE_INSTALL_ROOT: See aiir_python_setup_extension_rpath().
...
function(add_aiir_python_common_capi_library name)
  cmake_parse_arguments(ARG
    ""
  ...
  _flatten_aiir_python_targets(_all_source_targets ${ARG_DECLARED_SOURCES})
  foreach(t ${_all_source_targets})
    get_target_property(_local_embed_libs ${t} aiir_python_EMBED_CAPI_LINK_LIBS)
  ...
  _flatten_aiir_python_targets(_flat_header_targets ${ARG_DECLARED_HEADERS})
  set(_header_sources_target "${name}.sources")
  add_aiir_python_sources_target(${_header_sources_target}
  ...
  aiir_python_setup_extension_rpath(${name}
...
function(_flatten_aiir_python_targets output_var)
  set(_flattened)
  foreach(t ${ARGN})
    get_target_property(_source_type ${t} aiir_python_SOURCES_TYPE)
    get_target_property(_depends ${t} aiir_python_DEPENDS)
    if(_source_type)
      list(APPEND _flattened "${t}")
      if(_depends)
        _flatten_aiir_python_targets(_local_flattened ${_depends})
...
# Function: add_aiir_python_sources_target
...
function(add_aiir_python_sources_target name)
  cmake_parse_arguments(ARG
  ""
  ...
  if(ARG_UNPARSED_ARGUMENTS)
    message(FATAL_ERROR "Unhandled arguments to add_aiir_python_sources_target(${name}, ... : ${ARG_UNPARSED_ARGUMENTS}")
  endif()
...
################################################################################
function(add_aiir_python_extension libname extname nb_library_target_name)
  cmake_parse_arguments(ARG
  "_PRIVATE_SUPPORT_LIB"
  "INSTALL_COMPONENT;INSTALL_DIR;OUTPUT_DIRECTORY;AIIR_BINDINGS_PYTHON_NB_DOMAIN"
  "SOURCES;LINK_LIBS"
  ${ARGN})
  if(ARG_UNPARSED_ARGUMENTS)
    message(FATAL_ERROR "Unhandled arguments to add_aiir_python_extension(${libname}, ... : ${ARG_UNPARSED_ARGUMENTS}")
  endif()
  ...
  if(ARG__PRIVATE_SUPPORT_LIB)
    ...
    target_compile_definitions(${libname}
      PRIVATE
      AIIR_BINDINGS_PYTHON_DOMAIN=${AIIR_BINDINGS_PYTHON_NB_DOMAIN}
    )
  endif()
  target_link_libraries(${libname} PRIVATE ${nb_library_target_name})
  target_compile_definitions(${libname}
    PRIVATE
    AIIR_BINDINGS_PYTHON_DOMAIN=${ARG_AIIR_BINDINGS_PYTHON_NB_DOMAIN}
  )
  if(APPLE)
  ...
  if (NOT AIIR_DISABLE_CONFIGURE_PYTHON_DEV_PACKAGES
      AND (LLVM_COMPILER_IS_GCC_COMPATIBLE OR CLANG_CL))
    # Avoid some warnings from upstream nanobind.
    # If a superproject set AIIR_DISABLE_CONFIGURE_PYTHON_DEV_PACKAGES, let
  ...
  if(APPLE)
    ...
    if(NOT AIIR_ENABLE_PYTHON_STABLE_ABI)
  ...
  if(WIN32)
    ...
    # Since aiir uses nanobind, we can leverage their workaround by never directly
    # pyconfig.h or python.h and instead relying on the nanobind headers to include the
    # necessary python headers. This results in aiir always linking against the
