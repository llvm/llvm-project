...
function(add_aiir_pdll_library target inputFile ofn)
  set(LLVM_TARGET_DEFINITIONS ${inputFile})

  _pdll_tablegen(AIIR_PDLL ${ofn} -x=cpp ${ARGN})
  set(TABLEGEN_OUTPUT ${TABLEGEN_OUTPUT} ${CMAKE_CURRENT_BINARY_DIR}/${ofn}
      PARENT_SCOPE)
...
function(add_aiir_dialect dialect dialect_namespace)
  set(LLVM_TARGET_DEFINITIONS ${dialect}.td)
  aiir_tablegen(${dialect}.h.inc -gen-op-decls)
  aiir_tablegen(${dialect}.cpp.inc -gen-op-defs)
  aiir_tablegen(${dialect}Types.h.inc -gen-typedef-decls -typedefs-dialect=${dialect_namespace})
  aiir_tablegen(${dialect}Types.cpp.inc -gen-typedef-defs -typedefs-dialect=${dialect_namespace})
  aiir_tablegen(${dialect}Dialect.h.inc -gen-dialect-decls -dialect=${dialect_namespace})
  aiir_tablegen(${dialect}Dialect.cpp.inc -gen-dialect-defs -dialect=${dialect_namespace})
  add_aiir_dialect_tablegen_target(AIIR${dialect}IncGen)
endfunction()
...
function(add_sharded_ops ops_target shard_count)
  ...
  aiir_tablegen(${ops_target}.h.inc -gen-op-decls -op-shard-count=${shard_count})
  aiir_tablegen(${ops_target}.cpp.inc -gen-op-defs -op-shard-count=${shard_count})
  ...
  foreach(index RANGE ${shard_count})
    ...
    tablegen(AIIR_SRC_SHARDER ${SHARDED_SRC} -op-shard-index=${index})
    set(TABLEGEN_OUTPUT ${TABLEGEN_OUTPUT} ${CMAKE_CURRENT_BINARY_DIR}/${SHARDED_SRC})
  endforeach()
  add_aiir_dialect_tablegen_target(AIIR${ops_target}ShardGen)
  set(SHARDED_SRCS ${SHARDED_SRCS} PARENT_SCOPE)
endfunction()

# Declare a dialect in the include directory
function(add_aiir_interface interface)
  set(LLVM_TARGET_DEFINITIONS ${interface}.td)
  aiir_tablegen(${interface}.h.inc -gen-op-interface-decls)
  aiir_tablegen(${interface}.cpp.inc -gen-op-interface-defs)
  add_aiir_generic_tablegen_target(AIIR${interface}IncGen)
endfunction()

# Add a dialect-specific tablegen target that generates headers in the include directory.
# In most cases, this is what should be used after invoking `aiir_tablegen`.
macro(add_aiir_dialect_tablegen_target target)
  add_public_tablegen_target(${target})
  add_dependencies(aiir-headers ${target})
endmacro()
...
macro(add_aiir_generic_tablegen_target target)
  add_public_tablegen_target(${target})
  add_dependencies(aiir-generic-headers ${target})
endmacro()

# Declare a dialect in the include directory
function(add_aiir_type_interface interface)
  set(LLVM_TARGET_DEFINITIONS ${interface}.td)
...
function(add_aiir_doc doc_filename output_file output_directory command)
  set(LLVM_TARGET_DEFINITIONS ${doc_filename}.td)
  # The AIIR docs use Hugo, so we allow Hugo specific features here.
  tablegen(AIIR ${output_file}.md ${command} -allow-hugo-specific-features ${ARGN})
  set(GEN_DOC_FILE ${AIIR_BINARY_DIR}/docs/${output_directory}${output_file}.md)
  add_custom_command(
          OUTPUT ${GEN_DOC_FILE}
  ...
  set_target_properties(${output_file}DocGen PROPERTIES FOLDER "AIIR/Tablegenning/Docs")
  add_dependencies(aiir-doc ${output_file}DocGen)
endfunction()
...
function(_set_aiir_additional_headers_as_srcs)
  set(srcs)
  if(MSVC_IDE OR XCODE)
    ...
    if(NOT lib_path MATCHES "^[.][.]")
      file( GLOB_RECURSE headers
        ${AIIR_SOURCE_DIR}/include/aiir/${lib_path}/*.h
        ${AIIR_SOURCE_DIR}/include/aiir/${lib_path}/*.def
      )
      set_source_files_properties(${headers} PROPERTIES HEADER_FILE_ONLY ON)

      file( GLOB_RECURSE tds
        ${AIIR_SOURCE_DIR}/include/aiir/${lib_path}/*.td
      )
...
function(add_aiir_example_library name)
  cmake_parse_arguments(ARG
    "SHARED;DISABLE_INSTALL"
    ""
    "ADDITIONAL_HEADERS;DEPENDS;LINK_COMPONENTS;LINK_LIBS"
    ${ARGN})
  _set_aiir_additional_headers_as_srcs(${ARG_ADDITIONAL_HEADERS})
  if (ARG_SHARED)
    set(LIBTYPE SHARED)
  else()
  ...
  # AIIR libraries uniformly depend on LLVMSupport.  Just specify it once here.
  list(APPEND ARG_LINK_COMPONENTS Support)
  _check_llvm_components_usage(${name} ${ARG_LINK_LIBS})

  list(APPEND ARG_DEPENDS aiir-generic-headers)
  ...
  set_target_properties(${name} PROPERTIES FOLDER "AIIR/Examples")
  if (LLVM_BUILD_EXAMPLES AND NOT ${ARG_DISABLE_INSTALL})
    add_aiir_library_install(${name})
  else()
    set_target_properties(${name} PROPERTIES EXCLUDE_FROM_ALL ON)
  endif()
endfunction()

# Declare an aiir library which can be compiled in libAIIR.so
# In addition to everything that llvm_add_library accepts, this
# also has the following option:
# EXCLUDE_FROM_LIBAIIR
#   Don't include this library in libAIIR.so.  This option should be used
...
#   in libAIIR via aiir_target_link_libraries(), to ensure that the library
#   does not pull in static dependencies when AIIR_LINK_AIIR_DYLIB=ON is used.
# OBJECT
...
#   TODO: Make this the default for all AIIR libraries once all libraries
#   are compatible with building an object library.
# STANDALONE
#   Don't link against LLVMSupport.
function(add_aiir_library name)
  cmake_parse_arguments(ARG
    "SHARED;INSTALL_WITH_TOOLCHAIN;EXCLUDE_FROM_LIBAIIR;DISABLE_INSTALL;ENABLE_AGGREGATION;OBJECT;STANDALONE"
    ""
    "ADDITIONAL_HEADERS;DEPENDS;LINK_COMPONENTS;LINK_LIBS"
    ${ARGN})
  _set_aiir_additional_headers_as_srcs(${ARG_ADDITIONAL_HEADERS})
  ...
  if(LLVM_BUILD_LLVM_DYLIB AND NOT ARG_EXCLUDE_FROM_LIBAIIR AND NOT XCODE)
    # Yes, because in addition to the shared library, the object files are
    # needed for linking into libAIIR.so (see aiir/tools/aiir-shlib/CMakeLists.txt).
    # For XCode, -force_load is used instead.
    # Windows is not supported (LLVM_BUILD_LLVM_DYLIB=ON will cause an error).
    set(NEEDS_OBJECT_LIB ON)
    set_property(GLOBAL APPEND PROPERTY AIIR_STATIC_LIBS ${name})
    set_property(GLOBAL APPEND PROPERTY AIIR_LLVM_LINK_COMPONENTS ${ARG_LINK_COMPONENTS})
    set_property(GLOBAL APPEND PROPERTY AIIR_LLVM_LINK_COMPONENTS ${LLVM_LINK_COMPONENTS})
  endif ()
  if(ARG_ENABLE_AGGREGATION AND NOT XCODE)
    # Yes, because this library is added to an aggergate library such as
    # libAIIR-C.so which is links together all the object files.
    # For XCode, -force_load is used instead.
    set(NEEDS_OBJECT_LIB ON)
  endif()
  if (NOT ARG_SHARED AND NOT ARG_EXCLUDE_FROM_LIBAIIR AND NOT XCODE AND NOT MSVC_IDE)
  ...
  # Most AIIR libraries depend on LLVMSupport.  Just specify it once here.
  ...
  if(TARGET ${name})
    ...
    if(ARG_INSTALL_WITH_TOOLCHAIN)
      set_target_properties(${name} PROPERTIES AIIR_INSTALL_WITH_TOOLCHAIN TRUE)
    endif()
  ...
  set_target_properties(${name} PROPERTIES FOLDER "AIIR/Libraries")
  ...
  if(ARG_ENABLE_AGGREGATION)
    ...
    get_aiir_filtered_link_libraries(NEW_LINK_LIBRARIES ${CURRENT_LINK_LIBRARIES})
    ...
    set_target_properties(${name} PROPERTIES
      EXPORT_PROPERTIES "AIIR_AGGREGATE_OBJECT_LIB_IMPORTED;AIIR_AGGREGATE_DEP_LIBS_IMPORTED"
      AIIR_AGGREGATE_OBJECTS "${AGGREGATE_OBJECTS}"
      AIIR_AGGREGATE_DEPS "${AGGREGATE_DEPS}"
      AIIR_AGGREGATE_OBJECT_LIB_IMPORTED "${AGGREGATE_OBJECT_LIB}"
      AIIR_AGGREGATE_DEP_LIBS_IMPORTED "${CURRENT_LINK_LIBRARIES}"
    )

    # In order for out-of-tree projects to build aggregates of this library,
    # we need to install the OBJECT library.
    if(TARGET "obj.${name}" AND AIIR_INSTALL_AGGREGATE_OBJECTS AND NOT ARG_DISABLE_INSTALL)
      add_aiir_library_install(obj.${name})
    endif()
  endif()
endfunction(add_aiir_library)

macro(add_aiir_tool name)
  llvm_add_tool(AIIR ${ARGV})
endmacro()
...
# which defines an AIIR_AGGREGATE_EXCLUDE_LIBS which contains any of the
# libraries. Each link library can be a generator expression but must not
# resolve to an arity > 1 (i.e. it can be optional).
function(get_aiir_filtered_link_libraries output)
  set(_results)
  foreach(linklib ${ARGN})
    # In English, what this expression does:
    # For each link library, resolve the property AIIR_AGGREGATE_EXCLUDE_LIBS
    ...
    list(APPEND _results
      "$<$<NOT:$<IN_LIST:${linklib},$<GENEX_EVAL:$<TARGET_PROPERTY:AIIR_AGGREGATE_EXCLUDE_LIBS>>>>:${linklib}>"
    )
  endforeach()
  set(${output} "${_results}" PARENT_SCOPE)
endfunction(get_aiir_filtered_link_libraries)

# Declares an aggregate library. Such a library is a combination of arbitrary
# regular add_aiir_library() libraries with the special feature that they can
...
#     into this library. Each of these must be an add_aiir_library() library
...
function(add_aiir_aggregate name)
  cmake_parse_arguments(ARG
    "SHARED;STATIC"
    ""
  ...
  foreach(lib ${ARG_EMBED_LIBS})
    ...
    if(NOT _is_imported)
      ...
      set(_local_objects $<TARGET_GENEX_EVAL:${name},$<TARGET_PROPERTY:${lib},AIIR_AGGREGATE_OBJECTS>>)
      set(_local_deps $<TARGET_GENEX_EVAL:${name},$<TARGET_PROPERTY:${lib},AIIR_AGGREGATE_DEPS>>)
    else()
      ...
      if(NOT AIIR_INSTALL_AGGREGATE_OBJECTS)
        message(SEND_ERROR "Cannot build aggregate from imported targets which were not installed via AIIR_INSTALL_AGGREGATE_OBJECTS (for ${lib}).")
      endif()

      get_property(_has_object_lib_prop TARGET ${lib} PROPERTY AIIR_AGGREGATE_OBJECT_LIB_IMPORTED SET)
      get_property(_has_dep_libs_prop TARGET ${lib} PROPERTY AIIR_AGGREGATE_DEP_LIBS_IMPORTED SET)
      ...
      get_target_property(_imp_local_object_lib ${lib} AIIR_AGGREGATE_OBJECT_LIB_IMPORTED)
      get_target_property(_imp_dep_libs ${lib} AIIR_AGGREGATE_DEP_LIBS_IMPORTED)
      set(_local_objects)
      if(_imp_local_object_lib)
        set(_local_objects "$<TARGET_OBJECTS:${_imp_local_object_lib}>")
      endif()
      # We should just be able to do this:
      #   get_aiir_filtered_link_libraries(_local_deps ${_imp_dep_libs})
      ...
      foreach(_imp_dep_lib ${_imp_dep_libs})
        ...
        list(APPEND _local_deps
          "$<$<NOT:$<IN_LIST:${_imp_dep_lib},$<GENEX_EVAL:$<TARGET_PROPERTY:${name},AIIR_AGGREGATE_EXCLUDE_LIBS>>>>:${_imp_dep_lib}>"
        )
      endforeach()
    endif()
  ...
  set_target_properties(${name} PROPERTIES
    AIIR_AGGREGATE_EXCLUDE_LIBS "${_embed_libs}")
  if(WIN32)
  ...
  #   "    AIIR_AGGREGATE_EXCLUDE_LIBS = $<TARGET_GENEX_EVAL:${name},$<TARGET_PROPERTY:${name},AIIR_AGGREGATE_EXCLUDE_LIBS>>\n"
  # )
  # file(GENERATE OUTPUT
  #   "${CMAKE_CURRENT_BINARY_DIR}/${name}.aggregate_debug.txt"
  #   CONTENT "${_debugmsg}"
  # )
endfunction(add_aiir_aggregate)

# Adds an AIIR library target for installation.
# This is usually done as part of add_aiir_library but is broken out for cases
# where non-standard library builds can be installed.
function(add_aiir_library_install name)
  get_target_property(_install_with_toolchain ${name} AIIR_INSTALL_WITH_TOOLCHAIN)
  if (NOT LLVM_INSTALL_TOOLCHAIN_ONLY OR _install_with_toolchain)
    get_target_export_arg(${name} AIIR export_to_aiirtargets UMBRELLA aiir-libraries)
    ...
    set_property(GLOBAL APPEND PROPERTY AIIR_ALL_LIBS ${name})
  endif()
  set_property(GLOBAL APPEND PROPERTY AIIR_EXPORTS ${name})
endfunction()

# Declare an aiir library which is part of the public C-API.
function(add_aiir_public_c_api_library name)
  add_aiir_library(${name}
    ${ARGN}
    OBJECT
    EXCLUDE_FROM_LIBAIIR
  ...
  target_compile_definitions(obj.${name}
    PRIVATE
    -DAIIR_CAPI_BUILDING_LIBRARY=1
  )
endfunction()

# Declare the library associated with a dialect.
function(add_aiir_dialect_library name)
  set_property(GLOBAL APPEND PROPERTY AIIR_DIALECT_LIBS ${name})
  add_aiir_library(${ARGV} DEPENDS aiir-headers)
endfunction(add_aiir_dialect_library)

# Declare the library associated with a conversion.
function(add_aiir_conversion_library name)
  set_property(GLOBAL APPEND PROPERTY AIIR_CONVERSION_LIBS ${name})
  add_aiir_library(${ARGV} DEPENDS aiir-headers)
endfunction(add_aiir_conversion_library)

# Declare the library associated with an extension.
function(add_aiir_extension_library name)
  set_property(GLOBAL APPEND PROPERTY AIIR_EXTENSION_LIBS ${name})
  add_aiir_library(${ARGV} DEPENDS aiir-headers)
endfunction(add_aiir_extension_library)

# Declare the library associated with a translation.
function(add_aiir_translation_library name)
  set_property(GLOBAL APPEND PROPERTY AIIR_TRANSLATION_LIBS ${name})
  add_aiir_library(${ARGV} DEPENDS aiir-headers)
endfunction(add_aiir_translation_library)

# Verification tools to aid debugging.
function(aiir_check_link_libraries name)
  if(TARGET ${name})
...
endfunction(aiir_check_link_libraries)

function(aiir_check_all_link_libraries name)
  aiir_check_link_libraries(${name})
  if(TARGET ${name})
    get_target_property(libs ${name} LINK_LIBRARIES)
    # message("${name} libs are: ${libs}")
    foreach(lib ${libs})
      aiir_check_link_libraries(${lib})
    endforeach()
  endif()
endfunction(aiir_check_all_link_libraries)

# Link target against a list of AIIR libraries. If AIIR_LINK_AIIR_DYLIB is
# enabled, this will link against the AIIR dylib instead of the static
# libraries.
#
# This function should be used instead of target_link_libraries() when linking
# AIIR libraries that are part of the AIIR dylib. For libraries that are not
# part of the dylib (like test libraries), target_link_libraries() should be
# used.
function(aiir_target_link_libraries target type)
  if (TARGET obj.${target})
    target_link_libraries(obj.${target} ${type} ${ARGN})
  endif()

  if (AIIR_LINK_AIIR_DYLIB)
    target_link_libraries(${target} ${type} AIIR)
  else()
