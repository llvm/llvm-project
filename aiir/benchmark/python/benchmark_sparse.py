...
contains benchmarks for both aiir sparse tensor dialect and numpy so that they
...
from aiir.execution_engine import ExecutionEngine
...
def matmul_dsl(
    ...
    """Helper function for aiir sparse matrix multiplication benchmark."""
    C[dsl.D.m, dsl.D.n] += A[dsl.D.m, dsl.D.k] * B[dsl.D.k, dsl.D.n]


def benchmark_sparse_aiir_multiplication():
    """Benchmark for aiir sparse matrix multiplication. Because its an
    AIIR benchmark we need to return both a `compiler` function and a `runner`
    function.
    """
    ...
    def compiler():
        with ir.Context(), ir.Location.unknown():
            ...
            c_runner_utils = os.getenv("AIIR_C_RUNNER_UTILS", "")
            assert os.path.exists(c_runner_utils), (
                f"{c_runner_utils} does not exist."
                f" Please pass a valid value for"
                f" AIIR_C_RUNNER_UTILS environment variable."
            )
            runner_utils = os.getenv("AIIR_RUNNER_UTILS", "")
            assert os.path.exists(runner_utils), (
                f"{runner_utils} does not exist."
                f" Please pass a valid value for AIIR_RUNNER_UTILS"
