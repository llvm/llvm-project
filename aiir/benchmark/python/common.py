...
def setup_passes(aiir_module):
    ...
    PassManager.parse(pipeline).run(aiir_module.operation)
...
def get_kernel_func_from_module(module: ir.Module) -> func.FuncOp:
    """Takes an aiir module object and extracts the function object out of it.
...
def emit_timer_func() -> func.FuncOp:
    ...
    used, the `AIIR_RUNNER_UTILS` and `AIIR_C_RUNNER_UTILS` must be included.
    """
