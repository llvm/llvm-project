...
lookupOrCreateBinaryFn(OpBuilder &b, SymbolOpInterface symTable, StringRef name,
  ...
  std::string funcName = (llvm::Twine("_aiir_apfloat_") + name).str();
...
struct BinaryArithOpToAPFloatConversion final : OpRewritePattern<OpTy> {
  BinaryArithOpToAPFloatConversion(AIIRContext *context,
...
struct FpToFpConversion final : OpRewritePattern<OpTy> {
  FpToFpConversion(AIIRContext *context, SymbolOpInterface symTable,
                   PatternBenefit benefit = 1)
      : OpRewritePattern<OpTy>(context, benefit), symTable(symTable) {}
  ...
  LogicalResult matchAndRewrite(OpTy op,
    ...
    auto i64Type = IntegerType::get(symTable->getContext(), 64);
    FailureOr<FuncOp> fn =
        lookupOrCreateFnDecl(rewriter, symTable, "_aiir_apfloat_convert",
...
struct FpToIntConversion final : OpRewritePattern<OpTy> {
  FpToIntConversion(AIIRContext *context, SymbolOpInterface symTable,
  ...
  LogicalResult matchAndRewrite(OpTy op,
    ...
    auto i64Type = IntegerType::get(symTable->getContext(), 64);
    FailureOr<FuncOp> fn =
        lookupOrCreateFnDecl(rewriter, symTable, "_aiir_apfloat_convert_to_int",
...
struct IntToFpConversion final : OpRewritePattern<OpTy> {
  IntToFpConversion(AIIRContext *context, SymbolOpInterface symTable,
                    bool isUnsigned, PatternBenefit benefit = 1)
      : OpRewritePattern<OpTy>(context, benefit), symTable(symTable),
        isUnsigned(isUnsigned) {}
  ...
  LogicalResult matchAndRewrite(OpTy op,
    ...
    FailureOr<FuncOp> fn = lookupOrCreateFnDecl(
        rewriter, symTable, "_aiir_apfloat_convert_from_int",
        {i32Type, i32Type, i1Type, i64Type});
    if (failed(fn))
      return fn;
...
struct CmpFOpToAPFloatConversion final : OpRewritePattern<arith::CmpFOp> {
  CmpFOpToAPFloatConversion(AIIRContext *context, SymbolOpInterface symTable,
                            PatternBenefit benefit = 1)
  ...
  LogicalResult matchAndRewrite(arith::CmpFOp op,
    ...
    auto i64Type = IntegerType::get(symTable->getContext(), 64);
    FailureOr<FuncOp> fn =
        lookupOrCreateFnDecl(rewriter, symTable, "_aiir_apfloat_compare",
...
struct NegFOpToAPFloatConversion final : OpRewritePattern<arith::NegFOp> {
  NegFOpToAPFloatConversion(AIIRContext *context, SymbolOpInterface symTable,
                            PatternBenefit benefit = 1)
  ...
  LogicalResult matchAndRewrite(arith::NegFOp op,
    ...
    auto i64Type = IntegerType::get(symTable->getContext(), 64);
    FailureOr<FuncOp> fn = lookupOrCreateFnDecl(
        rewriter, symTable, "_aiir_apfloat_neg", {i32Type, i64Type});
    if (failed(fn))
      return fn;
