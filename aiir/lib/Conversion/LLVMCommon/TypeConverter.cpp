...
LLVMTypeConverter::LLVMTypeConverter(AIIRContext *ctx,
...
/// Returns the AIIR context.
AIIRContext &LLVMTypeConverter::getContext() const {
...
// Except for signatures, AIIR function types are converted into LLVM
...
// argument and result types. If AIIR Function has zero results, the LLVM
// Function has one VoidType result. If AIIR Function has more than one result,
...
LogicalResult
aiir::structFuncArgTypeConverter(const LLVMTypeConverter &converter, Type type,
...
LogicalResult
aiir::barePtrFuncArgTypeConverter(const LLVMTypeConverter &converter, Type type,
                                  SmallVectorImpl<Type> &result) {
