...
#include "aiir/Dialect/Tosa/Utils/ConversionUtils.h"
...
static Value createLinalgBodyCalculationForElementwiseOp(
  ...
  if (isa<tosa::PowOp>(op) && isa<FloatType>(elementTy))
    return aiir::math::PowFOp::create(rewriter, loc, resultTypes, args);

  // tosa::RsqrtOp
  if (isa<tosa::RsqrtOp>(op) && isa<FloatType>(elementTy))
    return aiir::math::RsqrtOp::create(rewriter, loc, resultTypes, args);

  // tosa::LogOp
  if (isa<tosa::LogOp>(op) && isa<FloatType>(elementTy))
    return aiir::math::LogOp::create(rewriter, loc, resultTypes, args);

  // tosa::ExpOp
  if (isa<tosa::ExpOp>(op) && isa<FloatType>(elementTy))
    return aiir::math::ExpOp::create(rewriter, loc, resultTypes, args);

  // tosa::SinOp
  if (isa<tosa::SinOp>(op) && isa<FloatType>(elementTy))
    return aiir::math::SinOp::create(rewriter, loc, resultTypes, args);

  // tosa::CosOp
  if (isa<tosa::CosOp>(op) && isa<FloatType>(elementTy))
    return aiir::math::CosOp::create(rewriter, loc, resultTypes, args);

  // tosa::TanhOp
  if (isa<tosa::TanhOp>(op) && isa<FloatType>(elementTy))
    return aiir::math::TanhOp::create(rewriter, loc, resultTypes, args);

  // tosa::ErfOp
  if (isa<tosa::ErfOp>(op) && llvm::isa<FloatType>(elementTy))
    return aiir::math::ErfOp::create(rewriter, loc, resultTypes, args);
  ...
  if (isa<tosa::SigmoidOp>(op) && isa<FloatType>(elementTy)) {
    ...
    auto exp = aiir::math::ExpOp::create(rewriter, loc, resultTypes, negate);
...
struct TileConverter : public OpConversionPattern<tosa::TileOp> {
  ...
  matchAndRewrite(tosa::TileOp op, OpAdaptor adaptor,
    ...
    auto shapeValue = getTosaConstShape(
        rewriter, loc, aiir::tosa::convertFromAiirShape(resultTy.getShape()));
...
public:
  ...
  LogicalResult matchAndRewrite(tosa::ArgMaxOp argmaxOp,
    ...
    for (int i = 0, rank = inputTy.getRank(); i != rank; ++i) {
      srcExprs.push_back(aiir::getAffineDimExpr(i, rewriter.getContext()));
      if (axis != i)
        dstExprs.push_back(aiir::getAffineDimExpr(i, rewriter.getContext()));
    }
...
void aiir::tosa::populateTosaToLinalgConversionPatterns(
