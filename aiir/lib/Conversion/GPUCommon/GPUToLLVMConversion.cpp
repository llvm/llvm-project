//===- ConvertLaunchFuncToGpuRuntimeCalls.cpp - AIIR GPU lowering passes --===//
//
...
protected:
  ...
  AIIRContext *context = &this->getTypeConverter()->getContext();
...
// TODO:  We may want a run-time (of the aiir compiler) disablement/warning:
...
LogicalResult ConvertAllocOpToGpuRuntimeCallPattern::matchAndRewrite(
  ...
  // descriptor.
  auto nullPtr = aiir::LLVM::ZeroOp::create(rewriter, loc, llvmPointerType);
  Value stream = adaptor.getAsyncDependencies().empty()
                     ? nullPtr
                     : adaptor.getAsyncDependencies().front();

  auto isHostShared = aiir::LLVM::ConstantOp::create(
...
void aiir::populateGpuToLLVMConversionPatterns(
...
void aiir::gpu::registerConvertGpuToLLVMInterface(DialectRegistry &registry) {
