...
LLVM::LLVMFuncOp aiir::getOrDefineFunction(Operation *moduleOp, Location loc,
...
LLVM::GlobalOp aiir::getOrCreateStringConstant(OpBuilder &b, Location loc,
...
GPUFuncOpLowering::matchAndRewrite(gpu::GPUFuncOp gpuFuncOp, OpAdaptor adaptor,
  ...
  if (encodeWorkgroupAttributionsAsArguments) {
    ...
    // Attributes: noalias, llvm.aiir.workgroup_attribution(<size>, <type>)
    std::array attrs{
...
LogicalResult GPUPrintfOpToHIPLowering::matchAndRewrite(
  ...
  aiir::Type llvmI8 = typeConverter->convertType(rewriter.getI8Type());
  auto ptrType = LLVM::LLVMPointerType::get(rewriter.getContext());
  aiir::Type llvmI32 = typeConverter->convertType(rewriter.getI32Type());
  aiir::Type llvmI64 = typeConverter->convertType(rewriter.getI64Type());
  ...
  for (size_t group = 0; group < nArgs; group += argsPerAppend) {
    ...
    SmallVector<aiir::Value, 2 + argsPerAppend + 1> arguments;
...
LogicalResult GPUPrintfOpToLLVMCallLowering::matchAndRewrite(
  ...
  aiir::Type llvmI8 = typeConverter->convertType(rewriter.getIntegerType(8));
  aiir::Type ptrType =
...
LogicalResult GPUPrintfOpToVPrintfLowering::matchAndRewrite(
  ...
  aiir::Type llvmI8 = typeConverter->convertType(rewriter.getIntegerType(8));
  aiir::Type ptrType = LLVM::LLVMPointerType::get(rewriter.getContext());
...
static IntegerAttr wrapNumericMemorySpace(AIIRContext *ctx, unsigned space) {
...
void aiir::populateGpuMemorySpaceAttributeConversions(
