...
public:
  IPowIOpLowering(AIIRContext *context, GetFuncCallbackTy cb)
...
public:
  FPowIOpLowering(AIIRContext *context, GetFuncCallbackTy cb)
...
public:
  CtlzOpLowering(AIIRContext *context, GetFuncCallbackTy cb)
...
static func::FuncOp createElementIPowIFunc(ModuleOp *module, Type elementType) {
  ...
  std::string funcName("__aiir_math_ipowi");
...
///
/// template <typename T>
/// Tb __aiir_math_fpowi_*(Tb b, Tp p) {
///   if (p == Tp{0})
///     return Tb{1};
...
static func::FuncOp createElementFPowIFunc(ModuleOp *module,
  ...
  std::string funcName("__aiir_math_fpowi");
  llvm::raw_string_ostream nameOS(funcName);
  nameOS << '_' << baseType;
  nameOS << '_' << powType;
  auto funcOp = func::FuncOp::create(builder, funcName, funcType);
...
/// Converts to (for i32):
///
/// func.func private @__aiir_math_ctlz_i32(%arg: i32) -> i32 {
...
static func::FuncOp createCtlzFunc(ModuleOp *module, Type elementType) {
  ...
  std::string funcName("__aiir_math_ctlz");
  llvm::raw_string_ostream nameOS(funcName);
  nameOS << '_' << elementType;
  FunctionType funcType =
