...
struct ConvertNativeFuncPattern final : public OpConversionPattern<Op> {
  ...
  ConvertNativeFuncPattern(AIIRContext *context, StringRef nativeFunc,
  ...
  matchAndRewrite(Op op, typename Op::Adaptor adaptor,
    ...
    // Preserve fastmath flags in our AIIR op when converting to llvm function
    ...
    aiir::NamedAttribute fastAttr = fastAttrConverter.getAttrs()[0];
...
void aiir::populateMathToXeVMConversionPatterns(RewritePatternSet &patterns,
                                                bool convertArith) {
