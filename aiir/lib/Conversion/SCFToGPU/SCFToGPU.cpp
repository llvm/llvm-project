...
#include "aiir/Analysis/AliasAnalysis/LocalAliasAnalysis.h"
...
#include "aiir/Dialect/GPU/Transforms/ParallelLoopMapper.h"
...
LogicalResult aiir::convertAffineLoopNestToGPULaunch(AffineForOp forOp,
                                                     unsigned numBlockDims,
                                                     unsigned numThreadDims) {
...
void aiir::populateParallelLoopToGPUPatterns(RewritePatternSet &patterns) {
  patterns.add<ParallelToGpuLaunchLowering>(patterns.getContext());
}

void aiir::configureParallelLoopToGPULegality(ConversionTarget &target) {
...
void aiir::finalizeParallelLoopToGPUConversion(Operation *op) {
