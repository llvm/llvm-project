...
void aiir::registerConvertMemRefToEmitCInterface(DialectRegistry &registry) {
...
Type convertMemRefType(MemRefType opTy, const TypeConverter *typeConverter) {
  Type resultTy;
  if (opTy.getRank() == 0) {
    resultTy = typeConverter->convertType(aiir::getElementTypeOrSelf(opTy));
  } else {
...
createPointerFromEmitcArray(Location loc, OpBuilder &builder,
  ...
  llvm::SmallVector<aiir::Value> indices(arrayType.getRank(), zeroIndex);
...
void aiir::populateMemRefToEmitCTypeConversion(TypeConverter &typeConverter) {
...
void aiir::populateMemRefToEmitCConversionPatterns(
