...
/// the results.
Value aiir::lowerAffineUpperBound(AffineForOp op, OpBuilder &builder) {
...
/// the results.
Value aiir::lowerAffineLowerBound(AffineForOp op, OpBuilder &builder) {
...
public:
  ...
  LogicalResult matchAndRewrite(AffineParallelOp op,
    ...
    for (unsigned i = 0, end = reductions.size(); i < end; i++) {
      // For each of the reduction operations get the respective aiir::Value.
      std::optional<arith::AtomicRMWKind> reductionOp =
          arith::symbolizeAtomicRMWKind(
...
void aiir::populateAffineToStdConversionPatterns(RewritePatternSet &patterns) {
  // clang-format off
  patterns.add<
      AffineApplyLowering,
...
void aiir::populateAffineToVectorConversionPatterns(
