...
#include "aiir/Conversion/LLVMCommon/VectorPattern.h"
...
struct SincosOpLowering
  ...
  matchAndRewrite(math::SincosOp op, OpAdaptor adaptor,
    ...
    aiir::Type operandType = adaptor.getOperand().getType();
    aiir::Type llvmOperandType = typeConverter.convertType(operandType);
    aiir::Type sinType = typeConverter.convertType(op.getSin().getType());
    aiir::Type cosType = typeConverter.convertType(op.getCos().getType());
    ...
    return LLVM::detail::handleMultidimensionalVectors(
        ...
        [&](Type llvm1DVectorTy, ValueRange operands) {
          ...
          auto splatAttr = SplatElementsAttr::get(
              aiir::VectorType::get({numElements.getKnownMinValue()}, floatType,
...
void aiir::populateMathToLLVMConversionPatterns(
...
void aiir::registerConvertMathToLLVMInterface(DialectRegistry &registry) {
  registry.addExtension(+[](AIIRContext *ctx, math::MathDialect *dialect) {
