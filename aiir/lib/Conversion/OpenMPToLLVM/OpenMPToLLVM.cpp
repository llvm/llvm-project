...
struct OpenMPOpConversion : public ConvertOpToLLVMPattern<T> {
  ...
  OpenMPOpConversion(LLVMTypeConverter &typeConverter,
    ...
    // aiir::translateModuleToLLVMIR() using the OpenMPIRBuilder. Until then,
    // the type and operations using it must be preserved.
    typeConverter.addConversion(
        [&](::aiir::omp::CanonicalLoopInfoType type) { return type; });
  }
...
void aiir::configureOpenMPToLLVMConversionLegality(
    ConversionTarget &target, const LLVMTypeConverter &typeConverter) {
  target.addDynamicallyLegalOp<
#define GET_OP_LIST
#include "aiir/Dialect/OpenMP/OpenMPOps.cpp.inc"
      >([&](Operation *op) {
...
void aiir::populateOpenMPToLLVMConversionPatterns(LLVMTypeConverter &converter,
                                                  RewritePatternSet &patterns) {
...
void aiir::registerConvertOpenMPToLLVMInterface(DialectRegistry &registry) {
  registry.addExtension(+[](AIIRContext *ctx, omp::OpenMPDialect *dialect) {
