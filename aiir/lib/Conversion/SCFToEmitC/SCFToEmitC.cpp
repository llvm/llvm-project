...
void aiir::registerConvertSCFToEmitCInterface(DialectRegistry &registry) {
  registry.addExtension(+[](AIIRContext *ctx, scf::SCFDialect *dialect) {
...
struct WhileLowering : public OpConversionPattern<WhileOp> {
  ...
  matchAndRewrite(WhileOp whileOp, OpAdaptor adaptor,
    ...
    AIIRContext *context = loc.getContext();
...
private:
  ...
  LogicalResult lowerDoWhile(WhileOp whileOp, ArrayRef<Value> loopVars,
                             ArrayRef<Value> resultVars, AIIRContext *context,
                             ConversionPatternRewriter &rewriter,
                             Location loc) const {
...
void aiir::populateSCFToEmitCConversionPatterns(RewritePatternSet &patterns,
