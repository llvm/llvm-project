//===- ConvertToLLVMPass.cpp - AIIR LLVM Conversion -----------------------===//
//
...
public:
  ConvertToLLVMPassInterface(AIIRContext *context,
...
public:
  AIIR_DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID(LoadDependentDialectExtension)

  LoadDependentDialectExtension() : DialectExtensionBase(/*dialectNames=*/{}) {}

  void apply(AIIRContext *context,
...
public:
  ...
  LogicalResult initialize(AIIRContext *context) final {
...
ConvertToLLVMPassInterface::ConvertToLLVMPassInterface(
    AIIRContext *context, ArrayRef<std::string> filterDialects,
    bool allowPatternRollback)
...
void aiir::registerConvertToLLVMDependentDialectLoading(
    DialectRegistry &registry) {
  registry.addExtensions<LoadDependentDialectExtension>();
}
