...
#include "aiir/Dialect/NVGPU/Utils/MMAUtils.h"
...
static bool isFirstResultLastMapDimension(AffineMap permutationMap) {
  AIIRContext *ctx = permutationMap.getContext();
...
static bool transferReadSupportsMMAMatrixType(vector::TransferReadOp readOp) {
  ...
  AIIRContext *ctx = readOp.getContext();
...
transferWriteSupportsMMAMatrixType(vector::TransferWriteOp writeOp) {
  ...
  AIIRContext *ctx = writeOp.getContext();
  AffineExpr innerDim = getAffineDimExpr(permutationMap.getNumDims() - 1, ctx);
...
static SetVector<Operation *> getOpToConvert(aiir::Operation *op,
                                             bool useNvGpu) {
...
static FailureOr<bool> isTransposed(vector::TransferReadOp op) {
  aiir::AffineMap map = op.getPermutationMap();
  ...
  aiir::AffineExpr dM = map.getResult(0);
  aiir::AffineExpr dN = map.getResult(1);
...
void aiir::populatePrepareVectorToMMAPatterns(RewritePatternSet &patterns,
                                              bool useNvGpu) {
  if (!useNvGpu) {
...
LogicalResult aiir::convertVectorToMMAOps(RewriterBase &rewriter,
                                          Operation *rootOp) {
...
LogicalResult aiir::convertVectorToNVVMCompatibleMMASync(RewriterBase &rewriter,
                                                         Operation *rootOp) {
...
std::unique_ptr<Pass> aiir::createConvertVectorToGPUPass(bool useNvGpu) {
