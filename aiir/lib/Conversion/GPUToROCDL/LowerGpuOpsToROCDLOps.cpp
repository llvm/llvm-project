//===- LowerGpuOpsToROCDLOps.cpp - AIIR GPU to ROCDL lowering passes ------===//
//
...
static Value getKnownOrOcklDim(RewriterBase &rewriter,
  ...
  AIIRContext *context = contextOp->getContext();
...
void aiir::configureGpuToROCDLConversionLegality(ConversionTarget &target) {
...
void aiir::populateGpuToROCDLConversionPatterns(
    const LLVMTypeConverter &converter, RewritePatternSet &patterns,
    aiir::gpu::amd::Runtime runtime, amdgpu::Chipset chipset) {
  using gpu::index_lowering::IndexKind;
  using gpu::index_lowering::IntrType;
  using aiir::gpu::amd::Runtime;
  auto *rocdlDialect =
