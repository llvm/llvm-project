...
#include "aiir/Dialect/EmitC/Transforms/TypeConversions.h"
...
void aiir::registerConvertArithToEmitCInterface(DialectRegistry &registry) {
...
public:
  FtoICastOpConversion(const TypeConverter &typeConverter, AIIRContext *context)
...
public:
  ItoFCastOpConversion(const TypeConverter &typeConverter, AIIRContext *context)
      : OpConversionPattern<CastOp>(typeConverter, context) {}
...
public:
  FpCastOpConversion(const TypeConverter &typeConverter, AIIRContext *context)
      : OpConversionPattern<CastOp>(typeConverter, context) {}
...
void aiir::populateArithToEmitCPatterns(TypeConverter &typeConverter,
                                        RewritePatternSet &patterns) {
  AIIRContext *ctx = patterns.getContext();

  aiir::populateEmitCSizeTTypeConversions(typeConverter);
