...
/// Converts the scaled MFMA/WMMA operands, `scalesA` and `scalesB`, from AIIR
...
static void wmmaPushInputOperand(
    ...
    Value aiirInput, SmallVectorImpl<Value> &operands,
  ...
  auto aiirInputType = cast<VectorType>(aiirInput.getType());
  bool isInputInteger = aiirInputType.getElementType().isInteger();
  if (isInputInteger) {
...
static std::optional<uint32_t> smallFloatTypeToFormatCode(Type aiirElemType) {
  return llvm::TypeSwitch<Type, std::optional<uint32_t>>(aiirElemType)
  ...
  matchAndRewrite(DPPOp DppOp, DPPOp::Adaptor adaptor,
    ...
    auto convertOperand = [&](Value operand, Type operandType) {
      if (operandType.getIntOrFloatBitWidth() <= 16) {
        ...
        auto llvmVecType = typeConverter->convertType(aiir::VectorType::get(
...
static Value setValueAtOffset(ConversionPatternRewriter &rewriter, Location loc,
  ...
  if (matchPattern(accumulator, aiir::m_Zero()))
    return value;
...
struct GlobalPrefetchOpLowering
  ...
  matchAndRewrite(GlobalPrefetchOp op, GlobalPrefetchOpAdaptor adaptor,
    ...
    rewriter.replaceOpWithNewOp<ROCDL::GlobalPrefetchOp>(
        op, prefetchPtr, immArgAttr, aiir::ArrayAttr{}, aiir::ArrayAttr{},
        aiir::ArrayAttr{});
    return success();
  }
...
struct ConvertAMDGPUToROCDLPass
  ...
  void runOnOperation() override {
    ...
    target.addIllegalDialect<::aiir::amdgpu::AMDGPUDialect>();
    target.addLegalDialect<::aiir::LLVM::LLVMDialect>();
    target.addLegalDialect<::aiir::ROCDL::ROCDLDialect>();
...
void aiir::amdgpu::populateCommonGPUTypeAndAttributeConversions(
...
void aiir::populateAMDGPUTypeAndAttributeConversions(
    TypeConverter &typeConverter) {
  typeConverter.addTypeAttributeConversion(
      [](BaseMemRefType type, amdgpu::AddressSpaceAttr as)
          -> TypeConverter::AttributeConversionResult {
        AIIRContext *ctx = as.getContext();
...
void aiir::populateAMDGPUToROCDLConversionPatterns(LLVMTypeConverter &converter,
