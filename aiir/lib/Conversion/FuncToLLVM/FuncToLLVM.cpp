...
//===----------------------------------------------------------------------===//
//
// This file implements a pass to convert AIIR Func and builtin dialects
...
#include "aiir/Dialect/LLVMIR/FunctionCallUtils.h"
...
lowerFuncAttributes(FunctionOpInterface func) {
  AIIRContext *ctx = func->getContext();
...
static void wrapForExternalCallers(OpBuilder &rewriter, Location loc,
  ...
  auto wrapperFuncOp = LLVM::LLVMFuncOp::create(
      rewriter, loc, llvm::formatv("_aiir_ciface_{0}", funcOp.getName()).str(),
...
static void wrapExternalFunction(OpBuilder &builder, Location loc,
  ...
  auto wrapperFunc = LLVM::LLVMFuncOp::create(
      builder, loc, llvm::formatv("_aiir_ciface_{0}", funcOp.getName()).str(),
...
FailureOr<LLVM::LLVMFuncOp> aiir::convertFuncOpToLLVMFuncOp(
...
void aiir::populateFuncToLLVMFuncOpConversionPattern(
    const LLVMTypeConverter &converter, RewritePatternSet &patterns,
    SymbolTableCollection *symbolTables) {
  patterns.add<FuncOpConversion>(converter, symbolTables);
}

void aiir::populateFuncToLLVMConversionPatterns(
    const LLVMTypeConverter &converter, RewritePatternSet &patterns,
    SymbolTableCollection *symbolTables) {
...
void aiir::registerConvertFuncToLLVMInterface(DialectRegistry &registry) {
  registry.addExtension(+[](AIIRContext *ctx, func::FuncDialect *dialect) {
