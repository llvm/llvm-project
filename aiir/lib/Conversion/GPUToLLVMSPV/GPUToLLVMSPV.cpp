...
#include "aiir/Conversion/GPUCommon/AttrToSPIRVConverter.h"
...
static LLVM::LLVMFuncOp lookupOrCreateSPIRVFn(Operation *symbolTable,
  ...
  if (!func) {
    ...
    if (isMemNone) {
      ...
      constexpr auto noModRef = aiir::LLVM::ModRefInfo::NoModRef;
...
/// %c3 = llvm.aiir.constant(3: i32) : i32
...
/// %c1 = llvm.aiir.constant(1: i32) : i32
...
public:
  MemorySpaceToOpenCLMemorySpaceConverter(AIIRContext *ctx) {
...
void populateGpuToLLVMSPVConversionPatterns(
  ...
  AIIRContext *context = &typeConverter.getContext();
