  ...
  matchAndRewrite(math::SincosOp op, OpAdaptor adaptor,
    ...
    if (isa<Float32Type>(computeType)) {
      ...
      const bool useApprox =
          aiir::arith::bitEnumContainsAny(flag, arith::FastMathFlags::afn);
    ...
    Value sinPtr, cosPtr;
    {
      ...
      auto *scope =
          op->getParentWithTrait<aiir::OpTrait::AutomaticAllocationScope>();
...
void aiir::populateLibDeviceConversionPatterns(
