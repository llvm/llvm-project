...
/// Creates `llvm.aiir.constant` with all bits set for the given type.
...
/// Creates `llvm.aiir.constant` with a floating-point scalar or vector value.
...
public:
  ...
  matchAndRewrite(spirv::ExecutionModeOp op, OpAdaptor adaptor,
    ...
    // Create `llvm.aiir.global` with initializer region containing one block.
...
/// Converts `spirv.GlobalVariable` to `llvm.aiir.global`. Note that SPIR-V
/// global returns a pointer, whereas in LLVM dialect the global holds an actual
/// value. This difference is handled by `spirv.aiir.addressof` and
/// `llvm.aiir.addressof`ops that both return a pointer.
...
public:
  ...
  matchAndRewrite(spirv::LoopOp loopOp, OpAdaptor adaptor,
    ...
    // `spirv.aiir.loop` with empty region is redundant and should be erased.
    ...
    // Split the current block after `spirv.aiir.loop`. The remaining ops will
    // be used in `endBlock`.
...
/// Converts `spirv.aiir.selection` with `spirv.BranchConditional` in its header
...
public:
  ...
  matchAndRewrite(spirv::SelectionOp op, OpAdaptor adaptor,
    ...
    // `spirv.aiir.selection` should have at least two blocks: one selection
    ...
    // Split the current block after `spirv.aiir.selection`. The remaining ops
...
void aiir::populateSPIRVToLLVMTypeConversion(LLVMTypeConverter &typeConverter,
                                             spirv::ClientAPI clientAPI) {
...
void aiir::populateSPIRVToLLVMConversionPatterns(
...
void aiir::populateSPIRVToLLVMFunctionConversionPatterns(
    const LLVMTypeConverter &typeConverter, RewritePatternSet &patterns) {
  patterns.add<FuncConversionPattern>(patterns.getContext(), typeConverter);
}

void aiir::populateSPIRVToLLVMModuleConversionPatterns(
    const LLVMTypeConverter &typeConverter, RewritePatternSet &patterns) {
...
void aiir::encodeBindAttribute(ModuleOp module) {
