//===- CRunnerUtils.cpp - Utils for AIIR execution ------------------------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file implements basic functions to manipulate structured AIIR types at
...
#include "aiir/ExecutionEngine/Msan.h"
...
extern "C" void memrefCopy(int64_t elemSize, UnrankedMemRefType<char> *srcArg,
  ...
  AIIR_MSAN_MEMORY_IS_INITIALIZED(src.sizes, rank * sizeof(int64_t));
...
extern "C" void *aiirAlloc(uint64_t size) { return malloc(size); }

extern "C" void *aiirAlignedAlloc(uint64_t alignment, uint64_t size) {
#ifdef _WIN32
...
extern "C" void aiirFree(void *ptr) { free(ptr); }

extern "C" void aiirAlignedFree(void *ptr) {
#ifdef _WIN32
  _aligned_free(ptr);
#else
  free(ptr);
...
extern "C" void _aiir_ciface_shuffle(StridedMemRefType<uint64_t, 1> *mref,
                                     void *g) {
  assert(mref);
...
#define IMPL_STDSORT(VNAME, V)                                                 \
  extern "C" void _aiir_ciface_stdSort##VNAME(uint64_t n,                      \
