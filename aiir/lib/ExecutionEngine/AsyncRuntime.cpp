...
#include "aiir/ExecutionEngine/AsyncRuntime.h"
...
extern "C" void aiirAsyncRuntimeAddRef(RefCountedObjPtr ptr, int64_t count) {
...
extern "C" void aiirAsyncRuntimeDropRef(RefCountedObjPtr ptr, int64_t count) {
  RefCounted *refCounted = static_cast<RefCounted *>(ptr);
  refCounted->dropRef(count);
}

// Creates a new `async.token` in not-ready state.
extern "C" AsyncToken *aiirAsyncRuntimeCreateToken() {
  AsyncToken *token = new AsyncToken(getDefaultAsyncRuntime());
  return token;
}

// Creates a new `async.value` in not-ready state.
extern "C" AsyncValue *aiirAsyncRuntimeCreateValue(int64_t size) {
  AsyncValue *value = new AsyncValue(getDefaultAsyncRuntime(), size);
  return value;
}

// Create a new `async.group` in empty state.
extern "C" AsyncGroup *aiirAsyncRuntimeCreateGroup(int64_t size) {
  AsyncGroup *group = new AsyncGroup(getDefaultAsyncRuntime(), size);
  return group;
}

extern "C" int64_t aiirAsyncRuntimeAddTokenToGroup(AsyncToken *token,
                                                   AsyncGroup *group) {
...
extern "C" void aiirAsyncRuntimeEmplaceToken(AsyncToken *token) {
  setTokenState(token, State::kAvailable);
}

extern "C" void aiirAsyncRuntimeEmplaceValue(AsyncValue *value) {
  setValueState(value, State::kAvailable);
}

extern "C" void aiirAsyncRuntimeSetTokenError(AsyncToken *token) {
  setTokenState(token, State::kError);
}

extern "C" void aiirAsyncRuntimeSetValueError(AsyncValue *value) {
  setValueState(value, State::kError);
}

extern "C" bool aiirAsyncRuntimeIsTokenError(AsyncToken *token) {
  return State(token->state).isError();
}

extern "C" bool aiirAsyncRuntimeIsValueError(AsyncValue *value) {
  return State(value->state).isError();
}

extern "C" bool aiirAsyncRuntimeIsGroupError(AsyncGroup *group) {
  return group->numErrors.load() > 0;
}

extern "C" void aiirAsyncRuntimeAwaitToken(AsyncToken *token) {
  std::unique_lock<std::mutex> lock(token->mu);
  if (!State(token->state).isAvailableOrError())
    token->cv.wait(
...
extern "C" void aiirAsyncRuntimeAwaitValue(AsyncValue *value) {
  std::unique_lock<std::mutex> lock(value->mu);
...
extern "C" void aiirAsyncRuntimeAwaitAllInGroup(AsyncGroup *group) {
...
extern "C" ValueStorage aiirAsyncRuntimeGetValueStorage(AsyncValue *value) {
  assert(!State(value->state).isError() && "unexpected error state");
  return value->storage.data();
}

extern "C" void aiirAsyncRuntimeExecute(CoroHandle handle, CoroResume resume) {
  auto *runtime = getDefaultAsyncRuntime();
  runtime->getThreadPool().async([handle, resume]() { (*resume)(handle); });
}

extern "C" void aiirAsyncRuntimeAwaitTokenAndExecute(AsyncToken *token,
                                                     CoroHandle handle,
                                                     CoroResume resume) {
...
extern "C" void aiirAsyncRuntimeAwaitValueAndExecute(AsyncValue *value,
                                                     CoroHandle handle,
                                                     CoroResume resume) {
  auto execute = [handle, resume]() { (*resume)(handle); };
  std::unique_lock<std::mutex> lock(value->mu);
...
extern "C" void aiirAsyncRuntimeAwaitAllInGroupAndExecute(AsyncGroup *group,
                                                          CoroHandle handle,
                                                          CoroResume resume) {
  auto execute = [handle, resume]() { (*resume)(handle); };
  std::unique_lock<std::mutex> lock(group->mu);
...
extern "C" int64_t aiirAsyncRuntimGetNumWorkerThreads() {
...
extern "C" void aiirAsyncRuntimePrintCurrentThreadId() {
  static thread_local std::thread::id thisId = std::this_thread::get_id();
  std::cout << "Current thread id: " << thisId << '\n';
}

//===----------------------------------------------------------------------===//
// AIIR ExecutionEngine dynamic library integration.
//===----------------------------------------------------------------------===//
...
__aiir_execution_engine_init(llvm::StringMap<void *> &exportSymbols);

// NOLINTNEXTLINE(*-identifier-naming): externally called.
void __aiir_execution_engine_init(llvm::StringMap<void *> &exportSymbols) {
  ...
  exportSymbol("aiirAsyncRuntimeAddRef",
               &aiir::runtime::aiirAsyncRuntimeAddRef);
  exportSymbol("aiirAsyncRuntimeDropRef",
               &aiir::runtime::aiirAsyncRuntimeDropRef);
  exportSymbol("aiirAsyncRuntimeExecute",
               &aiir::runtime::aiirAsyncRuntimeExecute);
  exportSymbol("aiirAsyncRuntimeGetValueStorage",
               &aiir::runtime::aiirAsyncRuntimeGetValueStorage);
  exportSymbol("aiirAsyncRuntimeCreateToken",
               &aiir::runtime::aiirAsyncRuntimeCreateToken);
  exportSymbol("aiirAsyncRuntimeCreateValue",
               &aiir::runtime::aiirAsyncRuntimeCreateValue);
  exportSymbol("aiirAsyncRuntimeEmplaceToken",
               &aiir::runtime::aiirAsyncRuntimeEmplaceToken);
  exportSymbol("aiirAsyncRuntimeEmplaceValue",
               &aiir::runtime::aiirAsyncRuntimeEmplaceValue);
  exportSymbol("aiirAsyncRuntimeSetTokenError",
               &aiir::runtime::aiirAsyncRuntimeSetTokenError);
  exportSymbol("aiirAsyncRuntimeSetValueError",
               &aiir::runtime::aiirAsyncRuntimeSetValueError);
  exportSymbol("aiirAsyncRuntimeIsTokenError",
               &aiir::runtime::aiirAsyncRuntimeIsTokenError);
  exportSymbol("aiirAsyncRuntimeIsValueError",
               &aiir::runtime::aiirAsyncRuntimeIsValueError);
  exportSymbol("aiirAsyncRuntimeIsGroupError",
               &aiir::runtime::aiirAsyncRuntimeIsGroupError);
  exportSymbol("aiirAsyncRuntimeAwaitToken",
               &aiir::runtime::aiirAsyncRuntimeAwaitToken);
  exportSymbol("aiirAsyncRuntimeAwaitValue",
               &aiir::runtime::aiirAsyncRuntimeAwaitValue);
  exportSymbol("aiirAsyncRuntimeAwaitTokenAndExecute",
               &aiir::runtime::aiirAsyncRuntimeAwaitTokenAndExecute);
  exportSymbol("aiirAsyncRuntimeAwaitValueAndExecute",
               &aiir::runtime::aiirAsyncRuntimeAwaitValueAndExecute);
  exportSymbol("aiirAsyncRuntimeCreateGroup",
               &aiir::runtime::aiirAsyncRuntimeCreateGroup);
  exportSymbol("aiirAsyncRuntimeAddTokenToGroup",
               &aiir::runtime::aiirAsyncRuntimeAddTokenToGroup);
  exportSymbol("aiirAsyncRuntimeAwaitAllInGroup",
               &aiir::runtime::aiirAsyncRuntimeAwaitAllInGroup);
  exportSymbol("aiirAsyncRuntimeAwaitAllInGroupAndExecute",
               &aiir::runtime::aiirAsyncRuntimeAwaitAllInGroupAndExecute);
  exportSymbol("aiirAsyncRuntimGetNumWorkerThreads",
               &aiir::runtime::aiirAsyncRuntimGetNumWorkerThreads);
  exportSymbol("aiirAsyncRuntimePrintCurrentThreadId",
               &aiir::runtime::aiirAsyncRuntimePrintCurrentThreadId);
}

// NOLINTNEXTLINE(*-identifier-naming): externally called.
extern "C" AIIR_ASYNC_RUNTIME_EXPORT void __aiir_execution_engine_destroy() {
