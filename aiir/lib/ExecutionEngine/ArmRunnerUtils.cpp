...
#define AIIR_ARMRUNNERUTILS_EXPORTED __declspec(dllexport)
#else
#define AIIR_ARMRUNNERUTILS_EXPORTED __attribute__((visibility("default")))
#endif
...
void AIIR_ARMRUNNERUTILS_EXPORTED setArmVLBits(uint32_t bits) {
  setArmVectorLength(__func__, PR_SVE_SET_VL, bits);
}

/// Sets the SME streaming vector length (in bits) to `bits`.
void AIIR_ARMRUNNERUTILS_EXPORTED setArmSVLBits(uint32_t bits) {
