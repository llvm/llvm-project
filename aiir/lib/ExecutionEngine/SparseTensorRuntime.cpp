...
// manipulating sparse tensors from AIIR.  More specifically, it provides
// C-API wrappers so that AIIR-generated code can call into the C++ runtime
// support library.  The functionality provided in this library is meant
// to simplify benchmarking, testing, and debugging of AIIR code operating
// on sparse tensors.  However, the provided functionality is **not**
// part of core AIIR itself.
//
...
//     ordering used by AIIR compiler-generated code (SparseTensorStorage).
//
...
//
// (I) Methods operating on AIIR buffers (memrefs) to interact with sparse
//     tensors. These methods should be used exclusively by AIIR
...
//      to interact with AIIR compiler-generated code.
//
...
#include "aiir/ExecutionEngine/SparseTensorRuntime.h"

#ifdef AIIR_CRUNNERUTILS_DEFINE_FUNCTIONS

#include "aiir/ExecutionEngine/SparseTensor/ArithmeticUtils.h"
#include "aiir/ExecutionEngine/SparseTensor/COO.h"
...
/// because the `_aiir_ciface_` calling convention migrates any returned
...
// Assume index_type is in fact uint64_t, so that _aiir_ciface_newSparseTensor
...
void *_aiir_ciface_newSparseTensor( // NOLINT
...
#define IMPL_SPARSEVALUES(VNAME, V)                                            \
  void _aiir_ciface_sparseValues##VNAME(StridedMemRefType<V, 1> *ref,          \
                                        void *tensor) {                        \
    assert(ref &&tensor);                                                      \
    std::vector<V> *v;                                                         \
...
AIIR_SPARSETENSOR_FOREVERY_V(IMPL_SPARSEVALUES)
#undef IMPL_SPARSEVALUES

#define IMPL_GETOVERHEAD(NAME, TYPE, LIB)                                      \
  void _aiir_ciface_##NAME(StridedMemRefType<TYPE, 1> *ref, void *tensor,      \
                           index_type lvl) {                                   \
    assert(ref &&tensor);                                                      \
    std::vector<TYPE> *v;                                                      \
...
AIIR_SPARSETENSOR_FOREVERY_O(IMPL_SPARSECOORDINATESBUFFER)
#undef IMPL_SPARSECOORDINATESBUFFER

#undef IMPL_GETOVERHEAD

#define IMPL_LEXINSERT(VNAME, V)                                               \
  void _aiir_ciface_lexInsert##VNAME(                                          \
...
AIIR_SPARSETENSOR_FOREVERY_V(IMPL_LEXINSERT)
#undef IMPL_LEXINSERT

#define IMPL_EXPINSERT(VNAME, V)                                               \
  void _aiir_ciface_expInsert##VNAME(                                          \
      void *t, StridedMemRefType<index_type, 1> *lvlCoordsRef,                 \
...
AIIR_SPARSETENSOR_FOREVERY_V(IMPL_EXPINSERT)
#undef IMPL_EXPINSERT

void *_aiir_ciface_createCheckedSparseTensorReader(
...
void _aiir_ciface_getSparseTensorReaderDimSizes(
...
#define IMPL_GETNEXT(VNAME, V, CNAME, C)                                       \
  bool _aiir_ciface_getSparseTensorReaderReadToBuffers##CNAME##VNAME(          \
...
AIIR_SPARSETENSOR_FOREVERY_V_O(IMPL_GETNEXT)
#undef IMPL_GETNEXT

void _aiir_ciface_outSparseTensorWriterMetaData(
...
#define IMPL_OUTNEXT(VNAME, V)                                                 \
  void _aiir_ciface_outSparseTensorWriterNext##VNAME(                          \
...
AIIR_SPARSETENSOR_FOREVERY_V(IMPL_OUTNEXT)
#undef IMPL_OUTNEXT
...
#endif // AIIR_CRUNNERUTILS_DEFINE_FUNCTIONS
