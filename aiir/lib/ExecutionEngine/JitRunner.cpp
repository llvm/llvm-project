//===- jit-runner.cpp - AIIR CPU Execution Driver Library -----------------===//
//
...
#include "aiir/ExecutionEngine/JitRunner.h"
...
struct CompileAndExecuteConfig {
  ...
  /// A custom function that is passed to ExecutionEngine. It processes AIIR
...
static OwningOpRef<Operation *> parseAIIRInput(StringRef inputFilename,
...
compileAndExecute(Options &options, Operation *module, StringRef entryPoint,
  ...
  auto expectedEngine =
      aiir::ExecutionEngine::create(module, engineOptions, std::move(tm));
  if (!expectedEngine)
...
int aiir::JitRunnerMain(int argc, char **argv, const DialectRegistry &registry,
  ...
  Options options;
  llvm::cl::ParseCommandLineOptions(argc, argv, "AIIR CPU execution driver\n");
  ...
  auto m = parseAIIRInput(options.inputFilename, !options.noImplicitModule,
                          &context);
  if (!m) {
  ...
  if (config.aiirTransformer)
    if (failed(config.aiirTransformer(m.get(), runnerOptions)))
      return EXIT_FAILURE;
  ...
  if (optLevel) {
    compileAndExecuteConfig.transformer = aiir::makeOptimizingTransformer(
