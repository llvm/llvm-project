//===- CodeGenHelpers.cpp - AIIR op definitions generator ---------------===//
//
...
#include "aiir/TableGen/Pattern.h"
...
std::string aiir::tblgen::buildErrorStreamingString(
...
static ::llvm::LogicalResult {0}(
    ::aiir::Operation *op, ::aiir::Type type, ::llvm::StringRef valueKind,
    unsigned valueIndex) {
  if (!({1})) {
...
static ::llvm::LogicalResult {0}(
    ::aiir::Attribute attr, ::llvm::StringRef attrName, llvm::function_ref<::aiir::InFlightDiagnostic()> emitError) {{
  if (attr && !({1}))
...
static ::llvm::LogicalResult {0}(
    ::aiir::Operation *op, ::aiir::Attribute attr, ::llvm::StringRef attrName) {{
...
static const char *const propConstraintCode = R"(
  static ::llvm::LogicalResult {0}(
      {3} prop, ::llvm::StringRef propName, llvm::function_ref<::aiir::InFlightDiagnostic()> emitError) {{
    if (!({1}))
  ...
  static ::llvm::LogicalResult {0}(
      ::aiir::Operation *op, {3} prop, ::llvm::StringRef propName) {{
...
static ::llvm::LogicalResult {0}(
    ::aiir::Operation *op, ::aiir::Block *successor,
...
static ::llvm::LogicalResult {0}(
    ::aiir::Operation *op, ::aiir::Region &region, ::llvm::StringRef regionName,
...
/// {3}: "::aiir::Type type" or "::aiirAttribute attr" or "propType prop".
/// Can be "T prop" for generic property constraints.
static const char *const patternConstraintCode = R"(
static ::llvm::LogicalResult {0}(
    ::aiir::PatternRewriter &rewriter, ::aiir::Operation *op, {3},
    ::llvm::StringRef failureStr) {
  if (!({1})) {
    return rewriter.notifyMatchFailure(op, [&](::aiir::Diagnostic &diag) {
      diag << failureStr << ": {2}";
    });
  }
  return ::aiir::success();
...
std::string StaticVerifierFunctionEmitter::getUniqueName(StringRef kind,
                                                         unsigned index) {
  return ("__aiir_ods_local_" + kind + "_constraint_" + uniqueOutputLabel +
          Twine(index))
      .str();
}
...
std::string aiir::tblgen::escapeString(StringRef value) {
  std::string ret;
