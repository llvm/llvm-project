...
#define DEBUG_TYPE "aiir-tblgen-pattern"
...
std::string SymbolInfoMap::SymbolInfo::getVarTypeStr(StringRef name) const {
  ...
  case Kind::Operand: {
    ...
    // operands).
    return "::aiir::Operation::operand_range";
  }
  case Kind::Value: {
    return "::aiir::Value";
  }
...
std::string SymbolInfoMap::SymbolInfo::getValueAndRangeUse(
  ...
  case Kind::Operand: {
    ...
    if (operand->isOptional()) {
      auto repl = formatv(
          fmt, formatv("({0}.empty() ? ::aiir::Value() : *{0}.begin())", name));
...
Pattern::getLocation(bool forSourceOutput) const {
  ...
  for (auto loc : def.getLoc()) {
    ...
    // making AIIR builds substantially less reproducable. As a compromise, we
