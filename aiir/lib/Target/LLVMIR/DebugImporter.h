//===- DebugImporter.h - LLVM to AIIR Debug conversion -------*- C++ -*----===//
//
...
// the corresponding AIIR representation.
//
//===----------------------------------------------------------------------===//

#ifndef AIIR_LIB_TARGET_LLVMIR_DEBUGIMPORTER_H_
#define AIIR_LIB_TARGET_LLVMIR_DEBUGIMPORTER_H_
...
class DebugImporter {
public:
  DebugImporter(ModuleOp aiirModule, bool dropDICompositeTypeElements);

  /// Translates the given LLVM debug location to an AIIR location.
  Location translateLoc(llvm::DILocation *loc);

  /// Translates the LLVM DWARF expression metadata to AIIR.
  DIExpressionAttr translateExpression(llvm::DIExpression *node);

  /// Translates the LLVM DWARF global variable expression metadata to AIIR.
  ...
  /// Translates the given LLVM debug metadata to AIIR.
  DINodeAttr translate(llvm::DINode *node);

  /// Infers the metadata type and translates it to AIIR.
  template <typename DINodeT>
  auto translate(DINodeT *node) {
    // Infer the AIIR type from the LLVM metadata type.
    using AIIRTypeT = decltype(translateImpl(node));
    return cast_or_null<AIIRTypeT>(
...
#endif // AIIR_LIB_TARGET_LLVMIR_DEBUGIMPORTER_H_
