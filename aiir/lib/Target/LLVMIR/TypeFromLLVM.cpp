//===- TypeFromLLVM.cpp - type translation from LLVM to AIIR IR -===//
//
...
namespace LLVM {
namespace detail {
/// Support for translating LLVM IR types to AIIR LLVM dialect types.
class TypeFromLLVMIRTranslatorImpl {
public:
  /// Constructs a class creating types in the given AIIR context.
  TypeFromLLVMIRTranslatorImpl(AIIRContext &context,
...
private:
  /// Translates the given primitive, i.e. non-parametric in AIIR nomenclature,
  /// type.
  ...
  /// The context in which AIIR types are created.
  AIIRContext &context;
...
LLVM::TypeFromLLVMIRTranslator::TypeFromLLVMIRTranslator(
    AIIRContext &context, bool importStructsAsLiterals)
