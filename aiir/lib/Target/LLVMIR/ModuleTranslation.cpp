//===- ModuleTranslation.cpp - AIIR to LLVM conversion --------------------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file implements the translation between an AIIR LLVM dialect module and
...
/// Create an LLVM IR constant of `llvmType` from the AIIR attribute `attr`.
...
llvm::Constant *aiir::LLVM::detail::getLLVMConstant(
  ...
  // AIIR might have a different size than the index type in the LLVM module.
...
ModuleTranslation::ModuleTranslation(Operation *module,
    ...
    : aiirModule(module), llvmModule(std::move(llvmModule)),
      debugTranslation(
  ...
  assert(satisfiesLLVMModule(aiirModule) &&
         "aiirModule should honor LLVM's module semantics.");
}
...
void aiir::LLVM::detail::connectPHINodes(Region &region,
...
llvm::CallInst *aiir::LLVM::detail::createIntrinsicCall(
...
/// Given a single AIIR operation, create the corresponding LLVM IR operation
/// using the `builder`.
...
/// to define values corresponding to the AIIR block arguments.  These nodes
...
/// Attempts to translate an AIIR attribute identified by `key`, optionally with
...
convertAIIRAttributeToLLVM(Location loc, llvm::LLVMContext &ctx, StringRef key,
...
/// Converts the AIIR attributes listed in the given array attribute into LLVM
...
convertAIIRAttributesToLLVM(Location loc, llvm::LLVMContext &ctx,
  ...
  for (Attribute attr : arrayAttr) {
    if (auto stringAttr = dyn_cast<StringAttr>(attr)) {
      FailureOr<llvm::Attribute> llvmAttr =
          convertAIIRAttributeToLLVM(loc, ctx, stringAttr.getValue());
      if (failed(llvmAttr))
        return failure();
    ...
    FailureOr<llvm::Attribute> llvmAttr = convertAIIRAttributeToLLVM(
...
LogicalResult ModuleTranslation::convertGlobalsAndAliases() {
  ...
  for (auto op : getModuleBody(aiirModule).getOps<LLVM::GlobalOp>()) {
    ...
    // external to have initializers. If AIIR does not provide an initializer,
    // default to undef.
    ...
    if (std::optional<aiir::SymbolRefAttr> comdat = op.getComdat()) {
    ...
    FailureOr<llvm::AttrBuilder> convertedTargetSpecificAttrs =
        convertAIIRAttributesToLLVM(op.getLoc(), var->getContext(),
  ...
  for (auto op : getModuleBody(aiirModule).getOps<LLVM::AliasOp>()) {
    llvm::Type *type = convertType(op.getType());
    llvm::Constant *cst = nullptr;
  ...
  // Convert llvm.aiir.global_ctors and dtors.
  for (Operation &op : getModuleBody(aiirModule)) {
  ...
  for (auto op : getModuleBody(aiirModule).getOps<LLVM::AliasOp>()) {
    ...
    for (aiir::Operation &op : initializer.without_terminator()) {
      if (failed(convertOperation(op, builder)))
...
LogicalResult ModuleTranslation::convertOneFunction(LLVMFuncOp func) {
  ...
  for (auto [aiirArg, llvmArg] :
       llvm::zip(func.getArguments(), llvmFunc->args()))
    mapValue(aiirArg, &llvmArg);
...
ModuleTranslation::convertNoBuiltin(llvm::LLVMContext &ctx, aiir::Attribute a) {
...
LogicalResult ModuleTranslation::convertFunctionSignatures() {
  ...
  for (auto function : getModuleBody(aiirModule).getOps<LLVMFuncOp>()) {
    ...
    FailureOr<llvm::AttrBuilder> convertedPassthroughAttrs =
        convertAIIRAttributesToLLVM(function.getLoc(), llvmFunc->getContext(),
    ...
    if (std::optional<aiir::SymbolRefAttr> comdat = function.getComdat()) {
      auto selectorOp = cast<ComdatSelectorOp>(
...
LogicalResult ModuleTranslation::convertIFuncs() {
  for (auto op : getModuleBody(aiirModule).getOps<IFuncOp>()) {
...
LogicalResult ModuleTranslation::convertComdats() {
  for (auto comdatOp : getModuleBody(aiirModule).getOps<ComdatOp>()) {
...
void ModuleTranslation::setTBAAMetadata(AliasAnalysisOpInterface op,
  ...
  // future, but for the time being just ignore the metadata if AIIR operation
...
LogicalResult ModuleTranslation::createTBAAMetadata() {
  ...
  aiirModule->walk([&](AliasAnalysisOpInterface analysisOpInterface) {
...
LogicalResult ModuleTranslation::createIdentMetadata() {
  if (auto attr = aiirModule->getAttrOfType<StringAttr>(
...
LogicalResult ModuleTranslation::createDependentLibrariesMetadata() {
  if (auto dependentLibrariesAttr = aiirModule->getDiscardableAttr(
...
aiir::translateModuleToLLVMIR(Operation *module, llvm::LLVMContext &llvmContext,
  ...
  // Now that all AIIR blocks are resolved into LLVM ones, patch block address
  // constants to point to the correct blocks.
  if (failed(translator.convertUnresolvedBlockAddress()))
    return nullptr;

  // Add the necessary debug info module flags, if they were not encoded in AIIR
  // beforehand.
