//===- ModuleImport.cpp - LLVM to AIIR conversion ---------------*- C++ -*-===//
//
...
#include "aiir/Target/LLVMIR/DataLayoutImporter.h"
...
#include "aiir/Dialect/LLVMIR/LLVMConversionEnumsFromLLVM.inc"
...
static constexpr StringRef getNamelessGlobalPrefix() {
  return "aiir.llvm.nameless_global";
}
...
/// Converts the LLVM instructions that have a generated AIIR builder. Using a
...
/// import methods. As a result, both the intrinsic and the instruction AIIR
...
static LogicalResult convertInstructionImpl(OpBuilder &odsBuilder,
  ...
  // Convert all instructions that provide an AIIR builder.
...
#include "aiir/Dialect/LLVMIR/LLVMOpFromLLVMIRConversions.inc"
...
ModuleImport::ModuleImport(ModuleOp aiirModule,
                           std::unique_ptr<llvm::Module> llvmModule,
                           bool emitExpensiveWarnings,
                           bool importEmptyDICompositeTypes,
                           bool preferUnregisteredIntrinsics,
                           bool importStructsAsLiterals)
    : builder(aiirModule->getContext()), context(aiirModule->getContext()),
      aiirModule(aiirModule), llvmModule(std::move(llvmModule)),
      iface(aiirModule->getContext()),
      typeTranslator(*aiirModule->getContext(), importStructsAsLiterals),
      debugImporter(std::make_unique<DebugImporter>(
          aiirModule, importEmptyDICompositeTypes)),
...
ComdatOp ModuleImport::getGlobalComdatOp() {
  ...
  builder.setInsertionPointToEnd(aiirModule.getBody());
  globalComdatOp =
      ComdatOp::create(builder, aiirModule.getLoc(), getGlobalComdatOpName());
  globalInsertionOp = globalComdatOp;
  return globalComdatOp;
}

LogicalResult ModuleImport::processTBAAMetadata(const llvm::MDNode *node) {
  Location loc = aiirModule.getLoc();
...
static Attribute convertCGProfileModuleFlagValue(ModuleOp aiirModule,
  ...
  for (unsigned i = 0; i < mdTuple->getNumOperands(); i++) {
    ...
    // FlatSymbolRefAttr::get(aiirModule->getContext(), llvmFn->getName());
    cgProfile.push_back(ModuleFlagCGProfileEntryAttr::get(
        aiirModule->getContext(),
        fromFn ? FlatSymbolRefAttr::get(aiirModule->getContext(),
                                        fromFn->getName())
               : nullptr,
        toFn ? FlatSymbolRefAttr::get(aiirModule->getContext(), toFn->getName())
             : nullptr,
        count));
  }
  return ArrayAttr::get(aiirModule->getContext(), cgProfile);
}

/// Extract a two element `MDTuple` from a `MDOperand`. Emit a warning in case
/// something else is found.
static llvm::MDTuple *getTwoElementMDTuple(ModuleOp aiirModule,
...
static llvm::ConstantAsMetadata *getConstantMDFromKeyValueTuple(
    ModuleOp aiirModule, const llvm::Module *llvmModule,
    const llvm::MDOperand &md, StringRef matchKey, bool optional = false) {
  llvm::MDTuple *tupleEntry = getTwoElementMDTuple(aiirModule, llvmModule, md);
  if (!tupleEntry)
    return nullptr;
...
convertInt64FromKeyValueTuple(ModuleOp aiirModule,
                              const llvm::Module *llvmModule,
                              const llvm::MDOperand &md, StringRef matchKey) {
  llvm::ConstantAsMetadata *valMD =
      getConstantMDFromKeyValueTuple(aiirModule, llvmModule, md, matchKey);
  if (!valMD)
    return failure();
...
convertProfileSummaryFormat(ModuleOp aiirModule, const llvm::Module *llvmModule,
                            const llvm::MDOperand &formatMD) {
  auto *tupleEntry = getTwoElementMDTuple(aiirModule, llvmModule, formatMD);
  if (!tupleEntry)
    return std::nullopt;
...
convertProfileSummaryDetailed(ModuleOp aiirModule,
                              const llvm::Module *llvmModule,
                              const llvm::MDOperand &summaryMD) {
  auto *tupleEntry = getTwoElementMDTuple(aiirModule, llvmModule, summaryMD);
  if (!tupleEntry)
    return failure();
...
static Attribute
convertProfileSummaryModuleFlagValue(ModuleOp aiirModule,
                                     const llvm::Module *llvmModule,
                                     llvm::MDTuple *mdTuple) {
  ...
  auto getOptIntValue =
    ...
    if (!getConstantMDFromKeyValueTuple(aiirModule, llvmModule, md, matchKey,
                                        /*optional=*/true))
      return FailureOr<std::optional<uint64_t>>(std::nullopt);
    if (checkOptionalPosition(md, matchKey).failed())
      return failure();
    FailureOr<uint64_t> val =
        convertInt64FromKeyValueTuple(aiirModule, llvmModule, md, matchKey);
    if (failed(val))
      return failure();
    return val;
  };
  ...
  auto getOptDoubleValue = [&](const llvm::MDOperand &md,
    ...
    auto *valMD = getConstantMDFromKeyValueTuple(aiirModule, llvmModule, md,
    ...
    if (auto *cstFP = dyn_cast<llvm::ConstantFP>(valMD->getValue())) {
      if (checkOptionalPosition(md, matchKey).failed())
        return failure();
      return FloatAttr::get(Float64Type::get(aiirModule.getContext()),
                            cstFP->getValueAPF());
    }
    emitWarning(aiirModule.getLoc())
  ...
  std::optional<ProfileSummaryFormatKind> format = convertProfileSummaryFormat(
      aiirModule, llvmModule, mdTuple->getOperand(summayIdx++));
  if (!format.has_value())
    return nullptr;

  FailureOr<uint64_t> totalCount = convertInt64FromKeyValueTuple(
      aiirModule, llvmModule, mdTuple->getOperand(summayIdx++), "TotalCount");
  if (failed(totalCount))
    return nullptr;

  FailureOr<uint64_t> maxCount = convertInt64FromKeyValueTuple(
      aiirModule, llvmModule, mdTuple->getOperand(summayIdx++), "MaxCount");
  if (failed(maxCount))
    return nullptr;
  ...
  FailureOr<uint64_t> numCounts = convertInt64FromKeyValueTuple(
      aiirModule, llvmModule, mdTuple->getOperand(summayIdx++), "NumCounts");
  if (failed(numCounts))
    return nullptr;

  FailureOr<uint64_t> numFunctions = convertInt64FromKeyValueTuple(
      aiirModule, llvmModule, mdTuple->getOperand(summayIdx++), "NumFunctions");
  ...
  FailureOr<SmallVector<ModuleFlagProfileSummaryDetailedAttr>> detailed =
      convertProfileSummaryDetailed(aiirModule, llvmModule,
  ...
  return ModuleFlagProfileSummaryAttr::get(
      aiirModule->getContext(), *format, *totalCount, *maxCount,
...
static Attribute
convertModuleFlagValueFromMDTuple(ModuleOp aiirModule,
                                  const llvm::Module *llvmModule, StringRef key,
                                  llvm::MDTuple *mdTuple) {
  if (key == LLVMDialect::getModuleFlagKeyCGProfileName())
    return convertCGProfileModuleFlagValue(aiirModule, mdTuple);
  if (key == LLVMDialect::getModuleFlagKeyProfileSummaryName())
    return convertProfileSummaryModuleFlagValue(aiirModule, llvmModule,
                                                mdTuple);
  return nullptr;
}
...
LogicalResult ModuleImport::convertModuleFlagsMetadata() {
  ...
  for (const auto [behavior, key, val] : llvmModuleFlags) {
    ...
    if (auto *constInt = llvm::mdconst::dyn_extract<llvm::ConstantInt>(val)) {
      ...
      valAttr = convertModuleFlagValueFromMDTuple(aiirModule, llvmModule.get(),
  ...
  if (!moduleFlags.empty())
    LLVM::ModuleFlagsOp::create(builder, aiirModule.getLoc(),
...
LogicalResult ModuleImport::convertLinkerOptionsMetadata() {
  for (const llvm::NamedMDNode &named : llvmModule->named_metadata()) {
    ...
    for (const llvm::MDNode *node : named.operands()) {
      ...
      LLVM::LinkerOptionsOp::create(builder, aiirModule.getLoc(),
...
LogicalResult ModuleImport::convertDependentLibrariesMetadata() {
  for (const llvm::NamedMDNode &named : llvmModule->named_metadata()) {
    ...
    if (!libraries.empty())
      aiirModule->setAttr(LLVM::LLVMDialect::getDependentLibrariesAttrName(),
      ...
      if (auto *md = dyn_cast<llvm::MDNode>(named.getOperand(0)))
        if (md->getNumOperands() == 1)
          if (auto *mdStr = dyn_cast<llvm::MDString>(md->getOperand(0)))
            aiirModule->setAttr(LLVMDialect::getIdentAttrName(),
  ...
  for (const llvm::NamedMDNode &nmd : llvmModule->named_metadata()) {
    ...
    if (nmd.getNumOperands() == 1)
      if (auto *md = dyn_cast<llvm::MDNode>(nmd.getOperand(0)))
        if (md->getNumOperands() == 1)
          if (auto *mdStr = dyn_cast<llvm::MDString>(md->getOperand(0)))
            aiirModule->setAttr(LLVMDialect::getCommandlineAttrName(),
                                builder.getStringAttr(mdStr->getString()));
  }
  return success();
}
...
void ModuleImport::processComdat(const llvm::Comdat *comdat) {
  ...
  auto selectorOp = ComdatSelectorOp::create(
      builder, aiirModule.getLoc(), comdat->getName(),
...
LogicalResult ModuleImport::convertDataLayout() {
  ...
  aiirModule->setAttr(DLTIDialect::kDataLayoutAttrName,
...
void ModuleImport::convertModuleLevelAsm() {
  ...
  llvm::SmallVector<aiir::Attribute> asmArrayAttr;

  for (llvm::StringRef line : llvm::split(asmStr, '\n'))
    if (!line.empty())
      asmArrayAttr.push_back(builder.getStringAttr(line));

  aiirModule->setAttr(LLVM::LLVMDialect::getModuleLevelAsmAttrName(),
...
LogicalResult ModuleImport::convertAlias(llvm::GlobalAlias *alias) {
  ...
  AliasOp aliasOp = AliasOp::create(builder, aiirModule.getLoc(), type,
...
LogicalResult ModuleImport::convertIFunc(llvm::GlobalIFunc *ifunc) {
  ...
  IFuncOp::create(builder, aiirModule.getLoc(), ifunc->getName(), type,
...
/// Converts LLVM string, integer, and enum attributes into AIIR attributes,
/// skipping those in `attributesToSkip` and emitting a warning at `loc` for
/// any other unsupported attributes.
static ArrayAttr convertLLVMAttributesToAIIR(
    Location loc, AIIRContext *context, llvm::AttributeSet attributes,
    ArrayRef<StringLiteral> attributesToSkip = {},
    ArrayRef<StringLiteral> attributePrefixesToSkip = {}) {
  SmallVector<Attribute> aiirAttributes;
  ...
  return ArrayAttr::get(context, aiirAttributes);
}

/// Converts LLVM attributes from `globalVar` into AIIR attributes and adds them
/// to `globalOp` as target-specific attributes.
static void processTargetSpecificAttrs(llvm::GlobalVariable *globalVar,
                                       GlobalOp globalOp) {
  ArrayAttr targetSpecificAttrs = convertLLVMAttributesToAIIR(
...
LogicalResult ModuleImport::convertGlobal(llvm::GlobalVariable *globalVar) {
  ...
  // Workaround to support LLVM's nameless globals. AIIR, in contrast to LLVM,
  ...
  GlobalOp globalOp = GlobalOp::create(
      builder, aiirModule.getLoc(), type, globalVar->isConstant(),
...
ModuleImport::convertGlobalCtorsAndDtors(llvm::GlobalVariable *globalVar) {
  ...
  if (globalVar->getName() == getGlobalCtorsVarName()) {
    globalInsertionOp = LLVM::GlobalCtorsOp::create(
        builder, aiirModule.getLoc(), builder.getArrayAttr(funcs),
...
FailureOr<Value> ModuleImport::convertConstant(llvm::Constant *constant) {
  ...
  if (auto *constTargetNone = dyn_cast<llvm::ConstantTargetNone>(constant)) {
    ...
    // Create llvm.aiir.zero operation to represent zero-initialization of
...
LogicalResult ModuleImport::convertIntrinsicArguments(
  ...
  assert(immArgPositions.size() == immArgAttrNames.size() &&
         "LLVM `immArgPositions` and AIIR `immArgAttrNames` should have equal "
         "length");
  ...
  for (llvm::Value *value : operands) {
    if (!value)
      continue;
    auto aiirValue = convertValue(value);
    if (failed(aiirValue))
      return failure();
    valuesOut.push_back(*aiirValue);
  }
  ...
  if (requiresOpBundles) {
    ...
    for (const llvm::OperandBundleUse &bundle : opBundles) {
      ...
      for (const llvm::Use &opBundleOperand : bundle.Inputs) {
        auto operandAiirValue = convertValue(opBundleOperand.get());
        if (failed(operandAiirValue))
          return failure();
        valuesOut.push_back(*operandAiirValue);
      }
    }
...
ModuleImport::convertCallOperands(llvm::CallBase *callInst,
  ...
  // that are not considered indirect calls by LLVM. However, in AIIR, they are
...
ModuleImport::convertAsmInlineOperandAttrs(const llvm::CallBase &llvmCall) {
  ...
  for (const llvm::InlineAsm::ConstraintInfo &ci : ia->ParseConstraints()) {
    ...
    if (ci.isIndirect) {
      if (llvm::Type *paramEltType = llvmCall.getParamElementType(argIdx)) {
        SmallVector<aiir::NamedAttribute> attrs;
        attrs.push_back(builder.getNamedAttr(
            aiir::LLVM::InlineAsmOp::getElementTypeAttrName(),
            aiir::TypeAttr::get(convertType(paramEltType))));
  ...
  return hasIndirect ? ArrayAttr::get(aiirModule->getContext(), opAttrs)
                     : nullptr;
}

LogicalResult ModuleImport::convertInstruction(llvm::Instruction *inst) {
  // Convert all instructions that do not provide an AIIR builder.
  Location loc = translateLoc(inst->getDebugLoc());
  ...
  if (inst->getOpcode() == llvm::Instruction::GetElementPtr) {
    ...
    // approach is that many unused `llvm.aiir.constant` would be emitted
    // at first place.
  ...
  // Convert all instructions that have an aiirBuilder.
...
LogicalResult ModuleImport::processInstruction(llvm::Instruction *inst) {
  ...
  // Convert LLVM intrinsics calls to AIIR intrinsics.
  ...
  // Convert all remaining LLVM instructions to AIIR operations.
...
// List of LLVM IR attributes that map to an explicit attribute on the AIIR
// LLVMFuncOp.
...
// List of LLVM IR attributes that are handled by prefix to map onto an AIIR
// LLVMFuncOp.
...
static void convertNoBuiltinAttrs(AIIRContext *ctx,
...
template <typename OpTy>
static void convertAllocsizeAttr(AIIRContext *ctx,
...
/// Converts LLVM attributes from `func` into AIIR attributes and adds them
...
static void processPassthroughAttrs(llvm::Function *func, LLVMFuncOp funcOp) {
  ...
  ArrayAttr passthroughAttr = convertLLVMAttributesToAIIR(
...
ModuleImport::convertArgOrResultAttrSet(llvm::AttributeSet llvmAttrSet) {
  ...
  for (auto [llvmKind, aiirName] : getAttrKindToNameMapping()) {
    ...
    if (llvmAttr.hasKindAsEnum() &&
      ...
      if (llvm::capturesNothing(llvmAttr.getCaptureInfo()))
        paramAttrs.push_back(
            builder.getNamedAttr(aiirName, builder.getUnitAttr()));
      continue;
    }

    Attribute aiirAttr;
    if (llvmAttr.isTypeAttribute())
      aiirAttr = TypeAttr::get(convertType(llvmAttr.getValueAsType()));
    else if (llvmAttr.isIntAttribute())
      aiirAttr = builder.getI64IntegerAttr(llvmAttr.getValueAsInt());
    else if (llvmAttr.isEnumAttribute())
      aiirAttr = builder.getUnitAttr();
    else if (llvmAttr.isConstantRangeAttribute()) {
      const llvm::ConstantRange &value = llvmAttr.getValueAsConstantRange();
      aiirAttr = builder.getAttr<LLVM::ConstantRangeAttr>(value.getLower(),
                                                          value.getUpper());
    } else {
      llvm_unreachable("unexpected parameter attribute kind");
    }
    paramAttrs.push_back(builder.getNamedAttr(aiirName, aiirAttr));
  }
...
LogicalResult ModuleImport::processFunction(llvm::Function *func) {
  ...
  // defines, which the import does not support. Given that AIIR lacks block
  ...
  // operands defined in a dominating block have a valid mapping to an AIIR
...
static LogicalResult setDebugIntrinsicBuilderInsertionPoint(
    aiir::OpBuilder &builder, DominanceInfo &domInfo, Value argOperand) {
...
OwningOpRef<ModuleOp> aiir::translateLLVMIRToModule(
