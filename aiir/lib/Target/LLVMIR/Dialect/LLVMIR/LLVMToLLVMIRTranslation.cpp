...
using aiir::LLVM::detail::getLLVMConstant;

#include "aiir/Dialect/LLVMIR/LLVMConversionEnumsToLLVM.inc"
...
static llvm::FastMathFlags getFastmathFlags(FastmathFlagsInterface &op) {
  ...
  ::aiir::LLVM::FastmathFlags fmfAiir = op.getFastmathAttr().getValue();
  for (auto it : handlers)
    if (bitEnumContainsAll(fmfAiir, it.first))
      (ret.*(it.second))(true);
  return ret;
}
...
getOverloadedDeclaration(CallIntrinsicOp op, llvm::Intrinsic::ID id,
  ...
  if (llvm::Intrinsic::matchIntrinsicSignature(ft, tableRef,
    ...
    return aiir::emitError(op.getLoc(), "call intrinsic signature ")
...
convertCallLLVMIntrinsicOp(CallIntrinsicOp op, llvm::IRBuilderBase &builder,
  ...
  if (!id)
    return aiir::emitError(op.getLoc(), "could not find LLVM intrinsic: ")
           << op.getIntrinAttr();
  ...
  if (intrinType != fn->getReturnType()) {
    return aiir::emitError(op.getLoc(), "intrinsic call returns ")
  ...
  if (!fn->getFunctionType()->isVarArg() &&
    ...
    return aiir::emitError(op.getLoc(), "intrinsic call has ")
  ...
  for (unsigned i = 0, e = fn->arg_size(); i != e; ++i) {
    ...
    if (actual != expected) {
      return aiir::emitError(op.getLoc(), "intrinsic call operand #")
...
/// Recursively converts an AIIR metadata attribute to an LLVM metadata node.
...
lookupNonFunctionSymbolCallee(FlatSymbolRefAttr attr, aiir::Type calleeFuncType,
                              Operation &opInst,
                              LLVM::ModuleTranslation &moduleTranslation) {
...
#include "aiir/Dialect/LLVMIR/LLVMConversions.inc"
#include "aiir/Dialect/LLVMIR/LLVMIntrinsicConversions.inc"
  ...
  // operation and store it in the AIIR-to-LLVM value mapping.  This does not
  ...
  // by the operation and store it in the AIIR-to-LLVM value mapping.
...
void aiir::registerLLVMDialectTranslation(DialectRegistry &registry) {
  registry.insert<LLVM::LLVMDialect>();
  registry.addExtension(+[](AIIRContext *ctx, LLVM::LLVMDialect *dialect) {
    dialect->addInterfaces<LLVMDialectLLVMIRTranslationInterface>();
  });
}

void aiir::registerLLVMDialectTranslation(AIIRContext &context) {
