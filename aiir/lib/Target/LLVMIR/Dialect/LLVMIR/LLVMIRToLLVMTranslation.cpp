...
//===----------------------------------------------------------------------===//
//
// This file implements a translation between LLVM IR and the AIIR LLVM dialect.
//
//===----------------------------------------------------------------------===//
...
/// Converts the LLVM intrinsic to an AIIR LLVM dialect operation if a
...
#include "aiir/Dialect/LLVMIR/LLVMIntrinsicFromLLVMIRConversions.inc"
  }

  return failure();
}

/// Returns the list of LLVM IR metadata kinds that are convertible to AIIR LLVM
/// dialect attributes.
...
/// Converts the given profiling metadata `node` to an AIIR profiling attribute
...
static LogicalResult setProfilingAttr(OpBuilder &builder, llvm::MDNode *node,
  ...
  if (auto *mdString = dyn_cast<llvm::MDString>(node->getOperand(1))) {
    if (mdString->getString() != llvm::MDProfLabels::ExpectedBranchWeights)
      return failure();
    // The AIIR WeightedBranchOpInterface does not support the
  ...
  if (auto iface = dyn_cast<WeightedBranchOpInterface>(op)) {
    ...
    // in PGO modes. AIIR WeightedBranchOpInterface does not allow this,
...
/// into corresponding AIIR attributes and set them on the given operation as a
...
static LogicalResult setMmraAttr(llvm::MDNode *node, Operation *op,
  ...
  if (llvm::MMRAMetadata::isTagMD(node)) {
    ...
    aiirMmra = ArrayAttr::get(ctx, tags);
  }
  op->setAttr(LLVMDialect::getMmraAttrName(), aiirMmra);
  return success();
}

/// Converts the given loop metadata node to an AIIR loop annotation attribute
/// and attaches it to the imported operation if the translation succeeds.
/// Returns failure otherwise.
...
/// Converts the provided metadata node `node` to an AIIR DenseI32ArrayAttr if
/// possible.
...
/// Convert an `MDNode` to an AIIR `IntegerAttr` if possible.
...
/// Converts the given intel required subgroup size metadata node to an AIIR
/// attribute and attaches it to the imported operation if the translation
...
public:
  ...
  /// Returns the list of LLVM IR metadata kinds that are convertible to AIIR
...
void aiir::registerLLVMDialectImport(DialectRegistry &registry) {
...
void aiir::registerLLVMDialectImport(AIIRContext &context) {
