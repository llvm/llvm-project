...
//===----------------------------------------------------------------------===//
//
// This file implements a translation between the AIIR VCIX dialect and
// LLVM IR.
//
//===----------------------------------------------------------------------===//
...
static llvm::Value *createVL(llvm::IRBuilderBase &builder, llvm::Value *vl,
  ...
  return aiir::LLVM::detail::getLLVMConstant(
      xlen,
...
public:
  ...
  convertOperation(Operation *op, llvm::IRBuilderBase &builder,
    ...
    Operation &opInst = *op;
#include "aiir/Dialect/LLVMIR/VCIXConversions.inc"
...
void aiir::registerVCIXDialectTranslation(DialectRegistry &registry) {
  registry.insert<vcix::VCIXDialect>();
  registry.addExtension(+[](AIIRContext *ctx, vcix::VCIXDialect *dialect) {
    dialect->addInterfaces<VCIXDialectLLVMIRTranslationInterface>();
  });
}

void aiir::registerVCIXDialectTranslation(AIIRContext &context) {
