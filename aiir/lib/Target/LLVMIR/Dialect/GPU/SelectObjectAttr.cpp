...
SelectObjectAttrImpl::getSelectedObject(gpu::BinaryOp op) const {
  ...
  if (Attribute target =
    ...
    if (auto indexAttr = aiir::dyn_cast<IntegerAttr>(target)) {
      index = indexAttr.getInt();
    } else {
      for (auto [i, attr] : llvm::enumerate(objects)) {
        auto obj = aiir::dyn_cast<gpu::ObjectAttr>(attr);
  ...
  return aiir::dyn_cast<gpu::ObjectAttr>(objects[index]);
}
...
LogicalResult SelectObjectAttrImpl::embedBinary(
  ...
  auto op = aiir::dyn_cast<gpu::BinaryOp>(operation);
  if (!op) {
...
public:
  LaunchKernel(Module &module, IRBuilderBase &builder,
               aiir::LLVM::ModuleTranslation &moduleTranslation);
  ...
  Value *createKernelArgArray(aiir::gpu::LaunchFuncOp op);

  // Create the full kernel launch.
  llvm::LogicalResult createKernelLaunch(aiir::gpu::LaunchFuncOp op,
                                         aiir::gpu::ObjectAttr object);
...
LogicalResult SelectObjectAttrImpl::launchKernel(
  ...
  auto launchFuncOp = aiir::dyn_cast<gpu::LaunchFuncOp>(launchFuncOperation);
  if (!launchFuncOp) {
    launchFuncOperation->emitError("operation must be a GPU launch func Op.");
    return failure();
  }

  auto binOp = aiir::dyn_cast<gpu::BinaryOp>(binaryOperation);
  if (!binOp) {
...
// return %array
llvm::Value *
llvm::LaunchKernel::createKernelArgArray(aiir::gpu::LaunchFuncOp op) {
...
llvm::LogicalResult
llvm::LaunchKernel::createKernelLaunch(aiir::gpu::LaunchFuncOp op,
                                       aiir::gpu::ObjectAttr object) {
  auto llvmValue = [&](aiir::Value value) -> Value * {
  ...
  // Get grid dimensions.
  aiir::gpu::KernelDim3 grid = op.getGridSizeOperandValues();
  Value *gx = llvmValue(grid.x), *gy = llvmValue(grid.y),
        *gz = llvmValue(grid.z);

  // Get block dimensions.
  aiir::gpu::KernelDim3 block = op.getBlockSizeOperandValues();
  ...
  if (aiir::Value dynSz = op.getDynamicSharedMemorySize())
  ...
  if (aiir::Value asyncObject = op.getAsyncObject()) {
  ...
  if (op.hasClusterSize()) {
    aiir::gpu::KernelDim3 cluster = op.getClusterSizeOperandValues();
...
void aiir::gpu::registerOffloadingLLVMTranslationInterfaceExternalModels(
