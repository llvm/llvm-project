...
//===----------------------------------------------------------------------===//
//
// This file implements a translation between the AIIR OpenMP dialect and LLVM
// IR.
//
//===----------------------------------------------------------------------===//
...
public:
  AIIR_DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID(OpenMPAllocaStackFrame)
...
public:
  AIIR_DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID(OpenMPLoopInfoStackFrame)
...
///
/// Its purpose is to serve as the glue between AIIR failures represented as
...
public:
  ...
  void registerType(LLVM::ModuleTranslation &moduleTranslation,
    ...
    linearVarTypes.push_back(moduleTranslation.convertType(
        aiir::cast<aiir::TypeAttr>(ty).getValue()));
  }
...
static llvm::Expected<llvm::BasicBlock *> convertOmpOpRegions(
  ...
  for (Block *bb : blocks) {
    ...
    // also consistent with AIIR's idiom of handling special region terminators
...
/// Convert ProcBindKind from AIIR-generated enum to LLVM enum.
...
/// A util to collect info needed to convert delayed privatizers from AIIR to
/// LLVM.
struct PrivateVarsInfo {
  ...
  PrivateVarsInfo(OP op)
    ...
    aiirVars.reserve(blockArgs.size());
    llvmVars.reserve(blockArgs.size());
    collectPrivatizationDecls<OP>(op);

    for (aiir::Value privateVar : op.getPrivateVars())
      aiirVars.push_back(privateVar);
  }

  MutableArrayRef<BlockArgument> blockArgs;
  SmallVector<aiir::Value> aiirVars;
  ...
  if (region.hasOneBlock()) {
    ...
    if (potentialTerminator && potentialTerminator->isTerminator()) {
      ...
      if (block->empty()) {
        ...
        // omp.yield(%0 : i32)
        // because the llvm.aiir.constant (AIIR op) isn't converted into any
        // llvm op
...
mapInitializationArgs(T loop, LLVM::ModuleTranslation &moduleTranslation,
  ...
  aiir::omp::DeclareReductionOp &reduction = reductionDecls[i];
  Region &initializerRegion = reduction.getInitializerRegion();
  Block &entry = initializerRegion.front();

  aiir::Value aiirSource = loop.getReductionVars()[i];
  llvm::Value *llvmSource = moduleTranslation.lookupValue(aiirSource);
  ...
  if (!isa<LLVM::LLVMPointerType>(
      ...
      isa<LLVM::LLVMPointerType>(aiirSource.getType())) {
    origVal =
...
static void collectReductionInfo(
  ...
  for (unsigned i = 0; i < numReductions; ++i) {
    ...
    aiir::Type allocatedType;
    reductionDecls[i].getAllocRegion().walk([&](aiir::Operation *op) {
      if (auto alloca = aiir::dyn_cast<LLVM::AllocaOp>(op)) {
...
/// Version of initPrivateVar which looks up the nonPrivateVar from aiirPrivVar.
static llvm::Expected<llvm::Value *> initPrivateVar(
    llvm::IRBuilderBase &builder, LLVM::ModuleTranslation &moduleTranslation,
    omp::PrivateClauseOp &privDecl, Value aiirPrivVar, BlockArgument &blockArg,
    llvm::Value *llvmPrivateVar, llvm::BasicBlock *privInitBlock,
    llvm::DenseMap<Value, Value> *mappedPrivateVars = nullptr) {
  return initPrivateVar(
      builder, moduleTranslation, privDecl,
      findAssociatedValue(aiirPrivVar, builder, moduleTranslation,
                          mappedPrivateVars),
...
initPrivateVars(llvm::IRBuilderBase &builder,
  ...
  for (auto [idx, zip] : llvm::enumerate(llvm::zip_equal(
           privateVarsInfo.privatizers, privateVarsInfo.aiirVars,
           privateVarsInfo.blockArgs, privateVarsInfo.llvmVars))) {
    auto [privDecl, aiirPrivVar, blockArg, llvmPrivateVar] = zip;
    llvm::Expected<llvm::Value *> privVarOrErr = initPrivateVar(
        builder, moduleTranslation, privDecl, aiirPrivVar, blockArg,
...
allocatePrivateVars(llvm::IRBuilderBase &builder,
  ...
  for (auto [privDecl, aiirPrivVar, blockArg] :
       llvm::zip_equal(privateVarsInfo.privatizers, privateVarsInfo.aiirVars,
...
static bool opIsInSingleThread(aiir::Operation *op) {
  for (aiir::Operation *parent = op->getParentOp(); parent != nullptr;
       parent = parent->getParentOp()) {
    if (aiir::isa<omp::SingleOp, omp::CriticalOp>(parent))
      return true;
    ...
    //   }
    // }
    if (aiir::isa<omp::ParallelOp>(parent))
      return false;
  }
  return false;
}
...
static LogicalResult copyFirstPrivateVars(
    aiir::Operation *op, llvm::IRBuilderBase &builder,
    LLVM::ModuleTranslation &moduleTranslation,
    SmallVectorImpl<aiir::Value> &aiirPrivateVars,
    ArrayRef<llvm::Value *> llvmPrivateVars,
    SmallVectorImpl<omp::PrivateClauseOp> &privateDecls, bool insertBarrier,
    llvm::DenseMap<Value, Value> *mappedPrivateVars = nullptr) {
  llvm::SmallVector<llvm::Value *> moldVars(aiirPrivateVars.size());
  llvm::transform(aiirPrivateVars, moldVars.begin(), [&](aiir::Value aiirVar) {
    // map copyRegion rhs arg
    llvm::Value *moldVar = findAssociatedValue(
        aiirVar, builder, moduleTranslation, mappedPrivateVars);
    assert(moldVar);
    return moldVar;
  });
...
cleanupPrivateVars(llvm::IRBuilderBase &builder,
  ...
  if (failed(inlineOmpRegionCleanup(
    ...
    return aiir::emitError(loc, "failed to inline `dealloc` region of an "
                                "`omp.private` op in");
...
static bool constructIsCancellable(Operation *op) {
  ...
  return op
      ->walk([](Operation *child) {
        if (aiir::isa<omp::CancelOp, omp::CancellationPointOp>(child))
          return WalkResult::interrupt();
...
buildDependData(std::optional<ArrayAttr> dependKinds, OperandRange dependVars,
  ...
  for (auto dep : llvm::zip(dependVars, dependKinds->getValue())) {
    ...
    switch (
        cast<aiir::omp::ClauseTaskDependAttr>(std::get<1>(dep)).getValue()) {
    case aiir::omp::ClauseTaskDepend::taskdependin:
    ...
    case aiir::omp::ClauseTaskDepend::taskdependout:
    case aiir::omp::ClauseTaskDepend::taskdependinout:
      type = llvm::omp::RTLDependenceKindTy::DepInOut;
      break;
    case aiir::omp::ClauseTaskDepend::taskdependmutexinoutset:
      type = llvm::omp::RTLDependenceKindTy::DepMutexInOutSet;
      break;
    case aiir::omp::ClauseTaskDepend::taskdependinoutset:
...
static void pushCancelFinalizationCB(
    ...
    aiir::Operation *op, llvm::omp::Directive cancelDirective) {
...
/// aiir::omp::IteratorOp for lowering to LLVM IR.
///
...
private:
  ...
  llvm::Value *lookUpAsI64(aiir::Value val, const LLVM::ModuleTranslation &mt,
                           llvm::IRBuilderBase &builder) {
...
public:
  IteratorInfo(aiir::omp::IteratorOp itersOp,
...
void TaskContextStructManager::generateTaskContextStruct() {
  ...
  for (omp::PrivateClauseOp &privOp : privateDecls) {
    ...
    if (!privOp.readsFromMold())
      continue;
    Type aiirType = privOp.getType();
    privateVarTypes.push_back(moduleTranslation.convertType(aiirType));
  }
...
static void fillAffinityLocators(Operation::operand_range affinityVars,
  ...
  for (auto [i, affinityVar] : llvm::enumerate(affinityVars)) {
    auto entryOp = affinityVar.getDefiningOp<aiir::omp::AffinityEntryOp>();
...
convertIteratorRegion(llvm::Value *linearIV, IteratorInfo &iterInfo,
                      aiir::Block &iteratorRegionBlock,
                      llvm::IRBuilderBase &builder,
                      LLVM::ModuleTranslation &moduleTranslation) {
  ...
  if (aiir::failed(moduleTranslation.convertBlock(iteratorRegionBlock,
...
static aiir::LogicalResult
fillAffinityIteratorLoop(aiir::omp::IteratorOp itersOp,
                         llvm::IRBuilderBase &builder,
                         aiir::LLVM::ModuleTranslation &moduleTranslation,
                         llvm::Value *affinityList, IteratorInfo &iterInfo) {
  aiir::Region &itersRegion = itersOp.getRegion();
  aiir::Block &iteratorRegionBlock = itersRegion.front();
  ...
  auto bodyGen = [&](llvm::OpenMPIRBuilder::InsertPointTy bodyIP,
    ...
    auto yield =
        aiir::dyn_cast<aiir::omp::YieldOp>(iteratorRegionBlock.getTerminator());
    assert(yield && yield.getResults().size() == 1 &&
           "expect omp.yield in iterator region to have one result");
    auto entryOp =
        yield.getResults()[0].getDefiningOp<aiir::omp::AffinityEntryOp>();
...
static aiir::LogicalResult
buildAffinityData(aiir::omp::TaskOp &taskOp, llvm::IRBuilderBase &builder,
                  aiir::LLVM::ModuleTranslation &moduleTranslation,
...
convertOmpTaskOp(omp::TaskOp taskOp, llvm::IRBuilderBase &builder,
  ...
  for (auto [privDecl, aiirPrivVar, blockArg, llvmPrivateVarAlloc] :
       llvm::zip_equal(privateVarsInfo.privatizers, privateVarsInfo.aiirVars,
                       privateVarsInfo.blockArgs,
    ...
    llvm::Expected<llvm::Value *> privateVarOrErr =
        initPrivateVar(builder, moduleTranslation, privDecl, aiirPrivVar,
    ...
    if ((privateVarOrErr.get() != llvmPrivateVarAlloc) &&
        !aiir::isa<LLVM::LLVMPointerType>(blockArg.getType())) {
  ...
  if (failed(copyFirstPrivateVars(
          taskOp, builder, moduleTranslation, privateVarsInfo.aiirVars,
  ...
  auto bodyCB = [&](InsertPointTy allocaIP,
    ...
    for (auto [i, zip] : llvm::enumerate(llvm::zip_equal(
             ...
             privateVarsInfo.aiirVars))) {
      auto [blockArg, privDecl, aiirPrivVar] = zip;
    ...
    for (auto [blockArg, llvmPrivateVar, privateDecl] :
      ...
      if (!aiir::isa<LLVM::LLVMPointerType>(blockArg.getType())) {
...
convertOmpTaskloopOp(Operation &opInst, llvm::IRBuilderBase &builder,
  ...
  for (auto [i, zip] : llvm::enumerate(llvm::zip_equal(
    ...
    auto [privDecl, aiirPrivVar, blockArg, llvmPrivateVarAlloc] = zip;
  ...
  setInsertPointForPossiblyEmptyBlock(builder, copyBlock);
  if (failed(copyFirstPrivateVars(
          taskloopOp, builder, moduleTranslation, privateVarsInfo.aiirVars,
          taskStructMgr.getLLVMPrivateVarGEPs(), privateVarsInfo.privatizers,
  ...
  int sched = 0; // default
  aiir::Value grainsizeVal = taskloopOp.getGrainsize();
  aiir::Value numTasksVal = taskloopOp.getNumTasks();
...
convertOmpWsloop(Operation &opInst, llvm::IRBuilderBase &builder,
  ...
  if (failed(copyFirstPrivateVars(
          wsloopOp, builder, moduleTranslation, privateVarsInfo.aiirVars,
  ...
  if (!wsloopOp.getLinearVars().empty()) {
    ...
    for (aiir::Attribute linearVarType : linearVarTypes)
    ...
    for (auto [idx, linearVar] : llvm::enumerate(wsloopOp.getLinearVars()))
      linearClauseProcessor.createLinearVar(
          ...
          idx);
    for (aiir::Value linearStep : wsloopOp.getLinearStepVars())
  ...
  omp::TargetOp targetOp = wsloopOp->getParentOfType<aiir::omp::TargetOp>();
  if (targetOp) {
...
convertOmpParallel(omp::ParallelOp opInst, llvm::IRBuilderBase &builder,
  ...
  auto bodyGenCB = [&](InsertPointTy allocaIP,
    ...
    if (failed(copyFirstPrivateVars(
            opInst, builder, moduleTranslation, privateVarsInfo.aiirVars,
            privateVarsInfo.llvmVars, privateVarsInfo.privatizers,
...
convertOmpSimd(Operation &opInst, llvm::IRBuilderBase &builder,
  ...
  if (!simdOp.getLinearVars().empty()) {
    ...
    for (auto [idx, linearVar] : llvm::enumerate(simdOp.getLinearVars())) {
      ...
      for (auto [aiirPrivVar, llvmPrivateVar] : llvm::zip_equal(
               privateVarsInfo.aiirVars, privateVarsInfo.llvmVars)) {
        // If the linear variable is implicit, reuse the already
        // existing llvm::Value
        if (linearVar == aiirPrivVar) {
          isImplicit = true;
    ...
    for (aiir::Value linearStep : simdOp.getLinearStepVars())
      linearClauseProcessor.initLinearStep(moduleTranslation, linearStep);
  }
  ...
  aiir::OperandRange operands = simdOp.getAlignedVars();
...
convertOmpCanonicalLoopOp(omp::CanonicalLoopOp op, llvm::IRBuilderBase &builder,
  ...
  llvm::Expected<llvm::CanonicalLoopInfo *> llvmOrError =
      ompBuilder->createCanonicalLoop(
          ...
          [&](llvm::OpenMPIRBuilder::InsertPointTy ip, llvm::Value *llvmIV) {
            // Register the mapping of AIIR induction variable to LLVM-IR
            // induction variable
  ...
  // Register the mapping of AIIR loop to LLVM-IR OpenMPIRBuilder loop
...
static LogicalResult applyTile(omp::TileOp op, llvm::IRBuilderBase &builder,
  ...
  if (!op.getGeneratees().empty()) {
    for (auto [aiirLoop, genLoop] :
         zip_equal(op.getGeneratees(), generatedLoops))
      moduleTranslation.mapOmpLoop(aiirLoop, genLoop);
  }
...
convertOmpAtomicUpdate(omp::AtomicUpdateOp &opInst,
  ...
  if (innerOpList.size() == 2) {
    ...
    aiir::Operation &innerOp = *opInst.getRegion().front().begin();
    ...
    aiirExpr = (isXBinopExpr ? innerOp.getOperand(1) : innerOp.getOperand(0));
    llvmExpr = moduleTranslation.lookupValue(aiirExpr);
  } else {
...
convertOmpAtomicCapture(omp::AtomicCaptureOp atomicCaptureOp,
  ...
  if (atomicWriteOp) {
    ...
    aiirExpr = atomicWriteOp.getExpr();
  } else {
    ...
    if (innerOpList.size() == 2) {
      aiir::Operation &innerOp = *atomicUpdateOp.getRegion().front().begin();
      if (!llvm::is_contained(innerOp.getOperands(),
  ...
  llvm::Value *llvmExpr = moduleTranslation.lookupValue(aiirExpr);
  llvm::Value *llvmX =
...
convertToDeviceClauseKind(aiir::omp::DeclareTargetDeviceType deviceClause) {
  switch (deviceClause) {
  case aiir::omp::DeclareTargetDeviceType::host:
    return llvm::OffloadEntriesInfoManager::OMPTargetDeviceClauseHost;
    break;
  case aiir::omp::DeclareTargetDeviceType::nohost:
    return llvm::OffloadEntriesInfoManager::OMPTargetDeviceClauseNoHost;
    break;
  case aiir::omp::DeclareTargetDeviceType::any:
...
convertToCaptureClauseKind(
  ...
  case aiir::omp::DeclareTargetCaptureClause::to:
    return llvm::OffloadEntriesInfoManager::OMPTargetGlobalVarEntryTo;
  case aiir::omp::DeclareTargetCaptureClause::link:
    return llvm::OffloadEntriesInfoManager::OMPTargetGlobalVarEntryLink;
  case aiir::omp::DeclareTargetCaptureClause::enter:
    return llvm::OffloadEntriesInfoManager::OMPTargetGlobalVarEntryEnter;
  case aiir::omp::DeclareTargetCaptureClause::none:
...
static Operation *getGlobalOpFromValue(Value value) {
  ...
  if (auto addressOfOp = dyn_cast_if_present<LLVM::AddressOfOp>(op)) {
    auto modOp = addressOfOp->getParentOfType<aiir::ModuleOp>();
...
getDeclareTargetRefPtrSuffix(LLVM::GlobalOp globalOp,
  ...
  if (globalOp.getVisibility() == aiir::SymbolTable::Visibility::Private) {
...
struct MapInfoData : MapInfosTy {
  ...
  llvm::SmallVector<aiir::Operation *, 4> MapClause;
...
// CGOpenMPRuntime.cpp.
static llvm::Value *getSizeInBytes(DataLayout &dl, const aiir::Type &type,
                                   Operation *clauseOp,
  ...
  if (auto memberClause =
          aiir::dyn_cast_if_present<aiir::omp::MapInfoOp>(clauseOp)) {
    ...
    if (!memberClause.getBounds().empty()) {
      ...
      for (auto bounds : memberClause.getBounds()) {
        if (auto boundOp = aiir::dyn_cast_if_present<aiir::omp::MapBoundsOp>(
...
// Convert the AIIR map flag set to the runtime map flag set for embedding
...
convertClauseMapFlags(omp::ClauseMapFlags aiirFlags) {
  auto mapTypeToBool = [&aiirFlags](omp::ClauseMapFlags flag) {
    return (aiirFlags & flag) == flag;
  };
  const bool hasExplicitMap =
      (aiirFlags & ~omp::ClauseMapFlags::is_device_ptr) !=
  ...
  llvm::sort(
      indices.begin(), indices.end(), [&](const size_t a, const size_t b) {
        ...
        for (auto it : llvm::zip(memberIndicesA, memberIndicesB)) {
          int64_t aIndex = aiir::cast<IntegerAttr>(std::get<0>(it)).getInt();
          int64_t bIndex = aiir::cast<IntegerAttr>(std::get<1>(it)).getInt();
...
static bool checkIfPointerMap(omp::MapInfoOp mapOp) {
  ...
  // as a pointer when we lower it to LLVM-IR even if at the AIIR level it has
...
static void genMapInfos(llvm::IRBuilderBase &builder,
  ...
  // AIIR value to LLVM value mapping, however, we only wish to do this
...
convertOmpTargetData(Operation *op, llvm::IRBuilderBase &builder,
  ...
  auto getDeviceID = [&](aiir::Value dev) -> llvm::Value * {
  ...
  LogicalResult result =
      llvm::TypeSwitch<Operation *, LogicalResult>(op)
          .Case([&](omp::TargetDataOp dataOp) {
            ...
            if (aiir::Value devId = dataOp.getDevice())
          ...
          .Case([&](omp::TargetEnterDataOp enterDataOp) -> LogicalResult {
            ...
            if (aiir::Value devId = enterDataOp.getDevice())
              deviceID = getDeviceID(devId);
          ...
          .Case([&](omp::TargetExitDataOp exitDataOp) -> LogicalResult {
            ...
            if (aiir::Value devId = exitDataOp.getDevice())
              deviceID = getDeviceID(devId);
          ...
          .Case([&](omp::TargetUpdateOp updateDataOp) -> LogicalResult {
            ...
            if (aiir::Value devId = updateDataOp.getDevice())
              deviceID = getDeviceID(devId);
...
convertOmpDistribute(Operation &opInst, llvm::IRBuilderBase &builder,
  ...
  auto bodyGenCB = [&](InsertPointTy allocaIP,
    ...
    if (failed(copyFirstPrivateVars(
            distributeOp, builder, moduleTranslation, privVarsInfo.aiirVars,
...
static LogicalResult
convertFlagsAttr(Operation *op, aiir::omp::FlagsAttr attribute,
                 LLVM::ModuleTranslation &moduleTranslation) {
  if (!cast<aiir::ModuleOp>(op))
    return failure();
...
/// If the given \p value is defined by an \c llvm.aiir.constant operation and
...
static uint64_t getTypeByteSize(aiir::Type type, const DataLayout &dl) {
...
static uint64_t getReductionDataSize(OpTy &op) {
  if (op.getNumReductionVars() > 0) {
    ...
    auto structType = aiir::LLVM::LLVMStructType::getLiteral(
...
initTargetRuntimeAttrs(llvm::IRBuilderBase &builder,
  ...
  if (aiir::Value devId = targetOp.getDevice()) {
...
convertOmpTarget(Operation &opInst, llvm::IRBuilderBase &builder,
  ...
  // get the correct debug locations. For outlined functions, the normal AIIR op
  ...
  auto bodyCB = [&](InsertPointTy allocaIP, InsertPointTy codeGenIP)
    ...
    if (failed(copyFirstPrivateVars(
            targetOp, builder, moduleTranslation, privateVarsInfo.aiirVars,
            privateVarsInfo.llvmVars, privateVarsInfo.privatizers,
  ...
  // except if they are constants. In any case, map the AIIR block argument to
...
static LogicalResult
convertDeclareTargetAttr(Operation *op, aiir::omp::DeclareTargetAttr attribute,
                         LLVM::ModuleTranslation &moduleTranslation) {
  // Amend omp.declare_target by deleting the IR of the outlined functions
  // created for target regions. They cannot be filtered out from AIIR earlier
  ...
  // those that aren't are removed earlier as an AIIR transformation pass.
  ...
  if (LLVM::GlobalOp gOp = dyn_cast<LLVM::GlobalOp>(op)) {
    ...
    if (auto *gVal = llvmModule->getNamedValue(gOp.getSymName())) {
      ...
      bool isExternallyVisible =
          gOp.getVisibility() != aiir::SymbolTable::Visibility::Private;
      ...
      // unused for AIIR at the moment, required in Clang for book
      // keeping
      std::vector<llvm::GlobalVariable *> generatedRefs;

      std::vector<llvm::Triple> targetTriple;
      auto targetTripleAttr = dyn_cast_or_null<aiir::StringAttr>(
          op->getParentOfType<aiir::ModuleOp>()->getAttr(
      ...
      if (ompBuilder->Config.isTargetDevice() &&
          (attribute.getCaptureClause().getValue() !=
               aiir::omp::DeclareTargetCaptureClause::to ||
           ompBuilder->Config.hasRequiresUnifiedSharedMemory())) {
...
public:
  ...
  /// Given an OpenMP AIIR attribute, create the corresponding LLVM-IR,
...
static bool isHostDeviceOp(Operation *op) {
  ...
  if (auto parentFn = op->getParentOfType<LLVM::LLVMFuncOp>()) {
    if (auto declareTargetIface =
            llvm::dyn_cast<aiir::omp::DeclareTargetInterface>(
                parentFn.getOperation()))
      if (declareTargetIface.isDeclareTarget() &&
          declareTargetIface.getDeclareTargetDeviceType() !=
              aiir::omp::DeclareTargetDeviceType::host)
        return false;
...
convertTargetAllocMemOp(Operation &opInst, llvm::IRBuilderBase &builder,
  ...
  aiir::Value deviceNum = allocMemOp.getDevice();
...
convertTargetFreeMemOp(Operation &opInst, llvm::IRBuilderBase &builder,
  ...
  // Get the corresponding device value in llvm
  aiir::Value deviceNum = freeMemOp.getDevice();
  llvm::Value *llvmDeviceNum = moduleTranslation.lookupValue(deviceNum);
  // Get the corresponding heapref value in llvm
  aiir::Value heapref = freeMemOp.getHeapref();
...
/// Given an OpenMP AIIR operation, create the corresponding LLVM IR (including
...
void aiir::registerOpenMPDialectTranslation(DialectRegistry &registry) {
...
void aiir::registerOpenMPDialectTranslation(AIIRContext &context) {
