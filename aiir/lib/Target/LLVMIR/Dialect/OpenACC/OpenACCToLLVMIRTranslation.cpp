...
//===----------------------------------------------------------------------===//
//
// This file implements a translation between the AIIR OpenACC dialect and LLVM
// IR.
//
//===----------------------------------------------------------------------===//
...
#include "aiir/Target/LLVMIR/Dialect/OpenMPCommon.h"
...
static llvm::Value *createSourceLocationInfo(OpenACCIRBuilder &builder,
  ...
  uint32_t strLen;
  llvm::Constant *locStr = aiir::LLVM::createSourceLocStrFromLocation(
...
processOperands(llvm::IRBuilderBase &builder,
  ...
  for (Value data : operands) {
    ...
    llvm::Constant *mapName =
        aiir::LLVM::createMappingInformation(data.getLoc(), *accBuilder);
...
processDataOperands(llvm::IRBuilderBase &builder,
  ...
  llvm::SmallVector<aiir::Value> create, copyin;
  for (aiir::Value dataOp : op.getDataClauseOperands()) {
    if (auto createOp = dataOp.getDefiningOp<acc::CreateOp>()) {
      create.push_back(createOp.getVarPtr());
    } else if (auto copyinOp = aiir::dyn_cast_or_null<acc::CopyinOp>(
...
processDataOperands(llvm::IRBuilderBase &builder,
  ...
  llvm::SmallVector<aiir::Value> deleteOperands, copyoutOperands;
  for (aiir::Value dataOp : op.getDataClauseOperands()) {
    if (auto devicePtrOp = aiir::dyn_cast_or_null<acc::GetDevicePtrOp>(
            dataOp.getDefiningOp())) {
      for (auto &u : devicePtrOp.getAccPtr().getUses()) {
        if (aiir::dyn_cast_or_null<acc::DeleteOp>(u.getOwner()))
          deleteOperands.push_back(devicePtrOp.getVarPtr());
        else if (aiir::dyn_cast_or_null<acc::CopyoutOp>(u.getOwner()))
...
processDataOperands(llvm::IRBuilderBase &builder,
  ...
  llvm::SmallVector<aiir::Value> from, to;
  for (aiir::Value dataOp : op.getDataClauseOperands()) {
    if (auto getDevicePtrOp = aiir::dyn_cast_or_null<acc::GetDevicePtrOp>(
            dataOp.getDefiningOp())) {
      from.push_back(getDevicePtrOp.getVarPtr());
    } else if (auto updateDeviceOp =
                   aiir::dyn_cast_or_null<acc::UpdateDeviceOp>(
                       dataOp.getDefiningOp())) {
...
static LogicalResult convertDataOp(acc::DataOp &op,
  ...
  llvm::SmallVector<aiir::Value> copyin, copyout, create, present,
      deleteOperands;
  for (aiir::Value dataOp : op.getDataClauseOperands()) {
    if (auto devicePtrOp = aiir::dyn_cast_or_null<acc::GetDevicePtrOp>(
            dataOp.getDefiningOp())) {
      ...
      // available.
      copyin.push_back(copyinOp.getVarPtr());
    } else if (auto createOp = aiir::dyn_cast_or_null<acc::CreateOp>(
                   dataOp.getDefiningOp())) {
      // TODO create zero currenlty handled as create. Update when extension
      // available.
      create.push_back(createOp.getVarPtr());
    } else if (auto presentOp = aiir::dyn_cast_or_null<acc::PresentOp>(
                   dataOp.getDefiningOp())) {
...
/// Given an OpenACC AIIR operation, create the corresponding LLVM IR
...
void aiir::registerOpenACCDialectTranslation(DialectRegistry &registry) {
  registry.insert<acc::OpenACCDialect>();
  registry.addExtension(+[](AIIRContext *ctx, acc::OpenACCDialect *dialect) {
    dialect->addInterfaces<OpenACCDialectLLVMIRTranslationInterface>();
  });
}

void aiir::registerOpenACCDialectTranslation(AIIRContext &context) {
