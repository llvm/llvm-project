...
//===----------------------------------------------------------------------===//
//
// This file implements a translation between the AIIR ROCDL dialect and
// LLVM IR.
//
//===----------------------------------------------------------------------===//
...
public:
  ...
  convertOperation(Operation *op, llvm::IRBuilderBase &builder,
    ...
    Operation &opInst = *op;
#include "aiir/Dialect/LLVMIR/ROCDLConversions.inc"
  ...
  amendOperation(Operation *op, ArrayRef<llvm::Instruction *> instructions,
    ...
    if (dialect->getKernelAttrHelper().getName() == attribute.getName()) {
      ...
      // AIIR's GPU kernel APIs all assume and produce uniformly-sized
...
void aiir::registerROCDLDialectTranslation(DialectRegistry &registry) {
  registry.insert<ROCDL::ROCDLDialect>();
  registry.addExtension(+[](AIIRContext *ctx, ROCDL::ROCDLDialect *dialect) {
    dialect->addInterfaces<ROCDLDialectLLVMIRTranslationInterface>();
  });
}

void aiir::registerROCDLDialectTranslation(AIIRContext &context) {
