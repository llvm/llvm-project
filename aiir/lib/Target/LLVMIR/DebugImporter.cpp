//===- DebugImporter.cpp - LLVM to AIIR Debug conversion ------------------===//
//
...
DebugImporter::DebugImporter(ModuleOp aiirModule,
                             bool dropDICompositeTypeElements)
    : cache([&](llvm::DINode *node) { return createRecSelf(node); }),
      context(aiirModule.getContext()), aiirModule(aiirModule),
...
DebugImporter::translateImpl(llvm::DIGlobalVariable *node) {
  // Names of DIGlobalVariables can be empty. AIIR models them as null, instead
...
DISubprogramAttr DebugImporter::translateImpl(llvm::DISubprogram *node) {
  ...
  // We currently only support `string` values for annotations on the AIIR side.
