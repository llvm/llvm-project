//===- TranslateRegistration.cpp - hooks to aiir-translate ----------------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file implements a translation from SPIR-V binary module to AIIR SPIR-V
// ModuleOp.
//
//===----------------------------------------------------------------------===//
...
deserializeModule(const llvm::MemoryBuffer *input, AIIRContext *context,
...
void registerFromSPIRVTranslation() {
  static llvm::cl::opt<bool> enableControlFlowStructurization(
      ...
      llvm::cl::desc(
          "Enable control flow structurization into `spirv.aiir.selection` and "
          "`spirv.aiir.loop`. This may need to be disabled to support "
          "deserialization of early exits (see #138688)"),
      llvm::cl::init(true));

  TranslateToAIIRRegistration fromBinary(
      "deserialize-spirv", "deserializes the SPIR-V module",
      [](llvm::SourceMgr &sourceMgr, AIIRContext *context) {
...
void registerToSPIRVTranslation() {
  ...
  TranslateFromAIIRRegistration toBinary(
...
static LogicalResult roundTripModule(spirv::ModuleOp module, bool emitDebugInfo,
  ...
  AIIRContext deserializationContext(context->getDialectRegistry());
...
void registerTestRoundtripSPIRV() {
  TranslateFromAIIRRegistration roundtrip(
