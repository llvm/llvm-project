//===- Deserializer.cpp - AIIR SPIR-V Deserializer ------------------------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file defines the SPIR-V binary to AIIR SPIR-V module deserializer.
//
//===----------------------------------------------------------------------===//
...
LogicalResult spirv::Deserializer::processDecoration(ArrayRef<uint32_t> words) {
  ...
  case spirv::Decoration::LinkageAttributes: {
    ...
    auto linkageTypeAttr = opBuilder.getAttr<::aiir::spirv::LinkageTypeAttr>(
        static_cast<::aiir::spirv::LinkageType>(words[wordIndex++]));
    auto linkageAttr = opBuilder.getAttr<::aiir::spirv::LinkageAttributesAttr>(
...
spirv::Deserializer::processFunctionEnd(ArrayRef<uint32_t> operands) {
  ...
  // Put all structured control flow in spirv.aiir.selection/spirv.aiir.loop
  // ops.
      ...
      if (allMemberDecorations.count(memberIndex)) {
        for (auto &memberDecoration : allMemberDecorations[memberIndex]) {
          ...
          if (memberDecoration.first == spirv::Decoration::Offset) {
            ...
            auto intType = aiir::IntegerType::get(context, 32);
...
Block *spirv::Deserializer::getOrCreateBlock(uint32_t id) {
  ...
  // spirv.aiir.selection or spirv.aiir.loop or function). Create it into the
...
/// spirv.aiir.selection/spirv.aiir.loop op.
...
public:
  ...
  ///
  /// This method will create an spirv.aiir.loop op in the `mergeBlock` and move
  /// all blocks in the structured loop into the spirv.aiir.loop's region. All
...
private:
  /// Creates a new spirv.aiir.selection op at the beginning of the
  /// `mergeBlock`.
  spirv::SelectionOp createSelectionOp(uint32_t selectionControl);

  /// Creates a new spirv.aiir.loop op at the beginning of the `mergeBlock`.
...
LogicalResult ControlFlowStructurizer::structurize() {
  ...
  if (!valuesToYield.empty()) {
    ...
    // Update `aiir.merge` with values to be yield.
    ...
    // AIIR does not allow changing the number of results of an operation, so
