//===- Deserializer.h - AIIR SPIR-V Deserializer ----------------*- C++ -*-===//
//
...
//===----------------------------------------------------------------------===//
//
// This file declares the SPIR-V binary to AIIR SPIR-V module deserializer.
//
//===----------------------------------------------------------------------===//

#ifndef AIIR_TARGET_SPIRV_DESERIALIZER_H
#define AIIR_TARGET_SPIRV_DESERIALIZER_H

#include "aiir/Dialect/SPIRV/IR/SPIRVEnums.h"
#include "aiir/Dialect/SPIRV/IR/SPIRVOps.h"
#include "aiir/IR/Builders.h"
#include "aiir/Target/SPIRV/Deserialization.h"
...
/// spirv.aiir.selection/spirv.aiir.loop later.
struct BlockMergeInfo {
  Block *mergeBlock;
  Block *continueBlock; // nullptr for spirv.aiir.selection
  Location loc;
...
public:
  ...
  explicit Deserializer(ArrayRef<uint32_t> binary, AIIRContext *context,
...
private:
  ...
  // we use spirv.aiir.selection and spirv.aiir.loop to group structured control
  // flow. The deserializer need to turn structured control flow marked with
  // merge instructions into using spirv.aiir.selection/spirv.aiir.loop ops.
  //
  ...
  //
  // 1. Deserialize all basic blocks in a function and create AIIR blocks for
  ...
  //    spirv.aiir.selection/spirv.aiir.loop's region. Structured control flow
  //    guarantees that we enter and exit in structured ways and the construct
  //    is nestable.
  // 3. Put the new spirv.aiir.selection/spirv.aiir.loop op at the beginning of
  // the
  //    old merge block and redirect all branches to the old header block to the
  //    old merge block (which contains the spirv.aiir.selection/spirv.aiir.loop
  //    op now).
  ...
  /// spirv.aiir.selection/spirv.aiir.loop op. This method iterates until all
  ...
  /// (`spirv.aiir.addressof` and `spirv.aiir.referenceof`) to turn an symbol
...
private:
  ...
  /// AIIRContext to create SPIR-V ModuleOp into.
  AIIRContext *context;
...
#endif // AIIR_TARGET_SPIRV_DESERIALIZER_H
