//===- SerializeOps.cpp - AIIR SPIR-V Serialization (Ops) -----------------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file defines the serialization methods for AIIR SPIR-V module ops.
//
//===----------------------------------------------------------------------===//
...
LogicalResult Serializer::processFuncOp(spirv::FuncOp op) {
  ...
  // ::aiir::spirv::Decoration attributes.

  for (auto attr : op->getAttrs()) {
    // Only generate OpDecorate op for spirv::Decoration attributes.
    auto isValidDecoration = aiir::spirv::symbolizeEnum<spirv::Decoration>(
...
LogicalResult Serializer::processSelectionOp(spirv::SelectionOp selectionOp) {
  ...
  // values yielded (with `aiir.merge`) from inside the region. The selection op
  ...
  // This SelectionOp is in some AIIR block with preceding and following ops. In
  ...
  // contains a spirv.aiir.merge op, itself. But we need to have an OpLabel
...
LogicalResult Serializer::processLoopOp(spirv::LoopOp loopOp) {
  ...
  // satisfying AIIR region's structural requirement.
  ...
  // Before we do anything replace results of the selection operation with
  // values yielded (with `aiir.merge`) from inside the region.
  auto mergeOp = cast<spirv::MergeOp>(mergeBlock->back());
  ...
  // This LoopOp is in some AIIR block with preceding and following ops. In the
  ...
  // LoopOp's entry block is just there for satisfying AIIR's structural
  ...
  // a spirv.aiir.merge op, itself. But we need to have an OpLabel instruction
...
LogicalResult Serializer::processSwitchOp(spirv::SwitchOp switchOp) {
  ...
  std::optional<aiir::DenseIntElementsAttr> literals = switchOp.getLiterals();
