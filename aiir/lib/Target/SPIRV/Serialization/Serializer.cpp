//===- Serializer.cpp - AIIR SPIR-V Serializer ----------------------------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file defines the AIIR SPIR-V module to SPIR-V binary serializer.
//
//===----------------------------------------------------------------------===//
...
static Block *getPhiIncomingBlock(Block *block) {
  ...
  // spirv.aiir.loop, we jump to this spirv.aiir.loop from its enclosing block.
...
Serializer::Serializer(spirv::ModuleOp module,
    ...
    : module(module), aiirBuilder(module.getContext()), options(options) {}
...
Serializer::processTypeImpl(Location loc, Type type, uint32_t &typeID,
  ...
  // such module fails validation. Indeed at AIIR level the two types are
...
LogicalResult Serializer::prepareBasicType(
  ...
  if (auto arrayType = dyn_cast<spirv::ArrayType>(type)) {
    ...
    if (auto elementCountID = prepareConstantInt(
            loc, aiirBuilder.getI32IntegerAttr(arrayType.getNumElements()))) {
  ...
  if (auto tensorArmType = dyn_cast<TensorArmType>(type)) {
    ...
    if (tensorArmType.hasRank()) {
      ...
      rank = dims.size();
      rankID = prepareConstantInt(loc, aiirBuilder.getI32IntegerAttr(rank));
      if (rankID == 0) {
        return failure();
      }
      ...
      if (rank > 0 && shaped) {
        ...
        if (rank == 1) {
          ...
          shapeID = prepareDenseElementsConstant(
              loc, shapeType,
              aiirBuilder.getI32TensorAttr(SmallVector<int32_t>(dims)), 0,
              index);
        } else {
          shapeID = prepareArrayConstant(
              loc, shapeType,
              aiirBuilder.getI32ArrayAttr(SmallVector<int32_t>(dims)));
        }
        if (shapeID == 0) {
          return failure();
        }
...
LogicalResult Serializer::emitPhiForBlockArguments(Block *block) {
  ...
  for (Block *aiirPredecessor : block->getPredecessors()) {
    auto *terminator = aiirPredecessor->getTerminator();
    LLVM_DEBUG(llvm::dbgs() << "  aiir predecessor ");
    LLVM_DEBUG(printBlock(aiirPredecessor, llvm::dbgs()));
    LLVM_DEBUG(llvm::dbgs() << "    terminator: " << *terminator << "\n");
    // The predecessor here is the immediate one according to AIIR's IR
    ...
    // spirv.aiir.selection/spirv.aiir.loop op in the AIIR predecessor block,
    // the branch op jumping to the OpPhi's block then resides in the previous
    // structured control flow op's merge block.
    Block *spirvPredecessor = getPhiIncomingBlock(aiirPredecessor);
