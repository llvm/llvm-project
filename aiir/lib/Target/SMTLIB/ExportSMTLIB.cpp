...
#include "aiir/Dialect/SMT/IR/SMTVisitors.h"
#include "aiir/Support/IndentedOstream.h"
#include "aiir/Target/SMTLIB/Namespace.h"
...
using ValueMap = llvm::ScopedHashTable<aiir::Value, std::string>;
...
struct TypeVisitor : public smt::SMTTypeVisitor<TypeVisitor, void,
                                                aiir::raw_indented_ostream &> {
  TypeVisitor(const SMTEmissionOptions &options) : options(options) {}

  void visitSMTType(BoolType type, aiir::raw_indented_ostream &stream) {
    stream << "Bool";
  }

  void visitSMTType(IntType type, aiir::raw_indented_ostream &stream) {
    stream << "Int";
  }

  void visitSMTType(BitVectorType type, aiir::raw_indented_ostream &stream) {
    stream << "(_ BitVec " << type.getWidth() << ")";
  }

  void visitSMTType(ArrayType type, aiir::raw_indented_ostream &stream) {
    stream << "(Array ";
  ...
  void visitSMTType(SMTFuncType type, aiir::raw_indented_ostream &stream) {
    stream << "(";
  ...
  void visitSMTType(SortType type, aiir::raw_indented_ostream &stream) {
...
struct VisitorInfo {
  VisitorInfo(aiir::raw_indented_ostream &stream, ValueMap &valueMap)
  ...
  // Stream to print to.
  aiir::raw_indented_ostream &stream;
...
struct ExpressionVisitor
  ...
  LogicalResult dispatchSMTOpVisitor(Operation *op, VisitorInfo &info) {
    ...
    if (res.hasOneUse() && options.inlineSingleUseValues) {
      ...
      aiir::raw_indented_ostream indentedStream(sstream);
...
struct StatementVisitor
    : public smt::SMTOpVisitor<StatementVisitor, LogicalResult,
                               aiir::raw_indented_ostream &, ValueMap &> {
  using smt::SMTOpVisitor<StatementVisitor, LogicalResult,
                          aiir::raw_indented_ostream &, ValueMap &>::visitSMTOp;

  StatementVisitor(const SMTEmissionOptions &options, Namespace &names)
      : options(options), typeVisitor(options), names(names),
        exprVisitor(options, names) {}

  LogicalResult visitSMTOp(BVConstantOp op, aiir::raw_indented_ostream &stream,
                           ValueMap &valueMap) {
  ...
  LogicalResult visitSMTOp(IntConstantOp op, aiir::raw_indented_ostream &stream,
                           ValueMap &valueMap) {
    SmallString<16> str;
  ...
  LogicalResult visitSMTOp(DeclareFunOp op, aiir::raw_indented_ostream &stream,
                           ValueMap &valueMap) {
    StringRef name =
  ...
  LogicalResult visitSMTOp(AssertOp op, aiir::raw_indented_ostream &stream,
                           ValueMap &valueMap) {
  ...
  LogicalResult visitSMTOp(ResetOp op, aiir::raw_indented_ostream &stream,
                           ValueMap &valueMap) {
    stream << "(reset)\n";
    return success();
  }

  LogicalResult visitSMTOp(PushOp op, aiir::raw_indented_ostream &stream,
                           ValueMap &valueMap) {
    stream << "(push " << op.getCount() << ")\n";
    return success();
  }

  LogicalResult visitSMTOp(PopOp op, aiir::raw_indented_ostream &stream,
                           ValueMap &valueMap) {
    stream << "(pop " << op.getCount() << ")\n";
    return success();
  }

  LogicalResult visitSMTOp(CheckOp op, aiir::raw_indented_ostream &stream,
                           ValueMap &valueMap) {
  ...
  LogicalResult visitSMTOp(SetLogicOp op, aiir::raw_indented_ostream &stream,
                           ValueMap &valueMap) {
...
LogicalResult smt::exportSMTLIB(Operation *module, llvm::raw_ostream &os,
  ...
  aiir::raw_indented_ostream ios(os);
...
void smt::registerExportSMTLIBTranslation() {
  ...
  static aiir::TranslateFromAIIRRegistration toSMTLIB(
      ...
      [](aiir::DialectRegistry &registry) {
        ...
        registry.insert<aiir::func::FuncDialect, arith::ArithDialect,
                        smt::SMTDialect>();
      });
}
