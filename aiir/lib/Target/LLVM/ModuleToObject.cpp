...
LogicalResult ModuleToObject::optimizeModule(llvm::Module &module,
  ...
  if (error) {
    InFlightDiagnostic aiirError = getOperation().emitError();
    llvm::handleAllErrors(
        std::move(error), [&aiirError](const llvm::ErrorInfoBase &ei) {
          aiirError << "Could not optimize LLVM IR: " << ei.message() << "\n";
        });
    return aiirError;
  }
  return success();
