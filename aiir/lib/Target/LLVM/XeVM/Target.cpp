//===- Target.cpp - AIIR LLVM XeVM target compilation -----------*- C++ -*-===//
//
...
#include "aiir/Target/LLVM/XeVM/Utils.h"
...
void aiir::xevm::registerXeVMTargetInterfaceExternalModels(
    DialectRegistry &registry) {
  registry.addExtension(+[](AIIRContext *ctx, XeVMDialect *dialect) {
...
// - L0 runtime consumes IL and is external to AIIR codebase (rt wrappers).
// - `ocloc` tool can be "queried" from within AIIR.
FailureOr<SmallVector<char, 0>> SerializeGPUModuleBase::compileToBinary(
  ...
  std::string basename = llvm::formatv(
      "aiir-{0}-{1}-{2}", getGPUModuleOp().getNameAttr().getValue(),
