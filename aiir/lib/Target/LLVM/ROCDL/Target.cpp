//===- Target.cpp - AIIR LLVM ROCDL target compilation ----------*- C++ -*-===//
//
...
void aiir::ROCDL::registerROCDLTargetInterfaceExternalModels(
...
StringRef aiir::ROCDL::getROCMPath() {
...
aiir::ROCDL::assembleIsa(StringRef isa, StringRef targetTriple, StringRef chip,
                         StringRef features,
...
FailureOr<SmallVector<char, 0>>
aiir::ROCDL::linkObjectCode(ArrayRef<char> objectCode, StringRef lldPath,
                            function_ref<InFlightDiagnostic()> emitError) {
...
ROCDLTargetAttrImpl::serializeToObject(
  ...
  if (!module)
    return std::nullopt;
  if (!aiir::isa<gpu::GPUModuleOp>(module)) {
