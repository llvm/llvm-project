//===- Target.cpp - AIIR LLVM NVVM target compilation -----------*- C++ -*-===//
//
...
#include "aiir/Target/LLVM/NVVM/Utils.h"
...
extern "C" const unsigned char _aiir_embedded_libdevice[];
extern "C" const unsigned _aiir_embedded_libdevice_size;
...
void aiir::NVVM::registerNVVMTargetInterfaceExternalModels(
...
StringRef aiir::NVVM::getCUDAToolkitPath() {
...
LogicalResult SerializeGPUModuleBase::appendStandardLibs() {
  ...
  DialectResourceBlobManager::BlobEntry *blob =
      resourceManager.getBlobManager().lookup("_aiir_embedded_libdevice");
  if (blob) {
  ...
  auto unmanagedBlob = UnmanagedAsmResourceBlob::allocateInferAlign(
      ArrayRef<char>{(const char *)_aiir_embedded_libdevice,
                     _aiir_embedded_libdevice_size});
  librariesToLink.push_back(DenseResourceElementsAttr::get(
      type, resourceManager.insert("_aiir_embedded_libdevice",
...
static void setOptionalCommandlineArguments(NVVMTargetAttr target,
  ...
  std::optional<aiir::NamedAttribute> cmdOptions = target.getCmdOptions();
...
NVPTXSerializer::compileToBinary(StringRef ptxCode) {
  ...
  // Base name for all temp files: aiir-<module name>-<target triple>-<chip>.
  std::string basename =
      llvm::formatv("aiir-{0}-{1}-{2}", getOperation().getNameAttr().getValue(),
...
#endif // AIIR_ENABLE_NVPTXCOMPILER
