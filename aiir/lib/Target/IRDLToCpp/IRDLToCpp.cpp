...
static void generateOpGetterDeclarations(irdl::detail::dictionary &dict,
  ...
  for (size_t i = 0, end = opStrings.opOperandNames.size(); i < end; ++i) {
    ...
    opGetters += llvm::formatv("::aiir::Value get{0}() { return "
  ...
  for (size_t i = 0, end = opStrings.opResultNames.size(); i < end; ++i) {
    ...
    resGetters += llvm::formatv(
        R"(::aiir::Value get{0}() { return ::llvm::cast<::aiir::Value>(getStructuredResults({1}).front()); }
  )",
        op, i);
  }
  ...
  for (size_t i = 0, end = opStrings.opRegionNames.size(); i < end; ++i) {
    ...
    regionAdaptorGetters += llvm::formatv(
        R"(::aiir::Region &get{0}() { return *getRegions()[{1}]; }
  )",
        op, i);
    regionGetters += llvm::formatv(
        R"(::aiir::Region &get{0}() { return (*this)->getRegion({1}); }
  )",
        op, i);
  }
...
static void generateOpBuilderDeclarations(irdl::detail::dictionary &dict,
  ...
  stream << llvm::formatv(
      R"(static void build(::aiir::OpBuilder &opBuilder, ::aiir::OperationState &opState, {0} {1} ::llvm::ArrayRef<::aiir::NamedAttribute> attributes = {{});)",
      resultParams, operandParams);
  stream << "\n";
  stream << llvm::formatv(
      R"(static {0} create(::aiir::OpBuilder &opBuilder, ::aiir::Location location, {1} {2} ::llvm::ArrayRef<::aiir::NamedAttribute> attributes = {{});)",
      opStrings.opCppName, resultParams, operandParams);
  stream << "\n";
  stream << llvm::formatv(
      R"(static {0} create(::aiir::ImplicitLocOpBuilder &opBuilder, {1} {2} ::llvm::ArrayRef<::aiir::NamedAttribute> attributes = {{});)",
      opStrings.opCppName, resultParams, operandParams);
  stream << "\n";
...
static SmallVector<std::string> generateTraits(irdl::OperationOp op,
  ...
  if (!strings.opRegionNames.empty()) {
    cppTraitNames.push_back(
        llvm::formatv("::aiir::OpTrait::NRegions<{0}>::Impl",
                      strings.opRegionNames.size())
            .str());

    // Requires verifyInvariantsImpl is implemented on the op
    cppTraitNames.emplace_back("::aiir::OpTrait::OpInvariants");
  }
...
static void generateRegionConstraintVerifiers(
  ...
  for (size_t i = 0; i < strings.opRegionNames.size(); ++i) {
    ...
    std::string helperFnName =
        llvm::formatv("__aiir_irdl_local_region_constraint_{0}_{1}",
    ...
    verifierHelpers.push_back(llvm::formatv(
        R"(static ::llvm::LogicalResult {0}(::aiir::Operation *op, ::aiir::Region &region, ::llvm::StringRef regionName, unsigned regionIndex) {{
    if (!({1})) {{
  ...
  if (::aiir::failed({0}(*this, (*this)->getRegion({1}), "{2}", {1})))
    return ::aiir::failure();)",
...
static std::string generateOpDefinition(irdl::detail::dictionary &dict,
                                        irdl::OperationOp op) {
  static const auto perOpDefTemplate = aiir::irdl::detail::Template{
#include "Templates/PerOperationDef.txt"
  ...
  auto resultTypes = llvm::join(
      llvm::map_range(opStrings.opResultNames,
                      [](StringRef attr) -> std::string {
                        return llvm::formatv("::aiir::Type {0}, ", attr);
                      }),
      "");
  auto operandTypes = llvm::join(
      llvm::map_range(opStrings.opOperandNames,
                      [](StringRef attr) -> std::string {
                        return llvm::formatv("::aiir::Value {0}, ", attr);
                      }),
      "");
  auto operandAdder =
      llvm::join(llvm::map_range(opStrings.opOperandNames,
  ...
  const auto buildDefinition = llvm::formatv(
      R"(
void {0}::build(::aiir::OpBuilder &opBuilder, ::aiir::OperationState &opState, {1} {2} ::llvm::ArrayRef<::aiir::NamedAttribute> attributes) {{
{3}
{4}
}

{0} {0}::create(::aiir::OpBuilder &opBuilder, ::aiir::Location location, {1} {2} ::llvm::ArrayRef<::aiir::NamedAttribute> attributes) {{
  ::aiir::OperationState __state__(location, getOperationName());
...
{0} {0}::create(::aiir::ImplicitLocOpBuilder &opBuilder, {1} {2} ::llvm::ArrayRef<::aiir::NamedAttribute> attributes) {{
...
static LogicalResult generateLib(irdl::DialectOp dialect, raw_ostream &output,
  ...
  static const auto typeHeaderDefTemplate = aiir::irdl::detail::Template{
#include "Templates/TypeHeaderDef.txt"
  };
  static const auto typeDefTemplate = aiir::irdl::detail::Template{
#include "Templates/TypeDef.txt"
  };
  static const auto dialectDefTemplate = aiir::irdl::detail::Template{
#include "Templates/DialectDef.txt"
  ...
  dict["TYPE_PARSER"] = llvm::formatv(
      R"(static ::aiir::OptionalParseResult generatedTypeParser(::aiir::AsmParser &parser, ::llvm::StringRef *mnemonic, ::aiir::Type &value) {
  return ::aiir::AsmParser::KeywordSwitch<::aiir::OptionalParseResult>(parser)
    {0}
  ...
  dict["TYPE_PRINTER"] = llvm::formatv(
      R"(static ::llvm::LogicalResult generatedTypePrinter(::aiir::Type def, ::aiir::AsmPrinter &printer) {
  return ::llvm::TypeSwitch<::aiir::Type, ::llvm::LogicalResult>(def)
    {0}
    .Default([](auto) {{ return ::aiir::failure(); });
})",
  ...
  dict["TYPE_DEFINES"] =
      join(map_range(typeNames,
                     [&](StringRef name) -> std::string {
                       return formatv("AIIR_DEFINE_EXPLICIT_TYPE_ID({1}::{0})",
                                      name, dialectStrings.namespacePath);
                     }),
           "\n");
...
static LogicalResult verifySupported(irdl::DialectOp dialect) {
  ...
  dialect.walk([&](aiir::Operation *op) {
    res =
        llvm::TypeSwitch<Operation *, LogicalResult>(op)
            ...
            .Default([](aiir::Operation *op) -> LogicalResult {
              return op->emitError("IRDL C++ translation does not yet support "
                                   "translation of ")
...
#include "Templates/TypeDef.txt"
  ...
  for (auto dialect : dialects) {
    ...
    SmallVector<SmallString<8>> namespaceAbsolutePath{{"aiir"}, dialectName};
    std::string namespaceOpen;
    std::string namespaceClose;
    std::string namespacePath;
