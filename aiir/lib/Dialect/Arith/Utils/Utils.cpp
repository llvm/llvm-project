...
aiir::inferExpandShapeOutputShape(OpBuilder &b, Location loc,
...
detail::op_matcher<arith::ConstantIndexOp> aiir::matchConstantIndex() {
  return detail::op_matcher<arith::ConstantIndexOp>();
}

llvm::SmallBitVector aiir::getPositionsOfShapeOne(unsigned rank,
...
Value aiir::getValueOrCreateConstantIntOp(OpBuilder &b, Location loc,
                                          OpFoldResult ofr) {
...
Value aiir::getValueOrCreateConstantIndexOp(OpBuilder &b, Location loc,
                                            OpFoldResult ofr) {
  if (auto value = dyn_cast_if_present<Value>(ofr))
    return value;
  auto attr = cast<IntegerAttr>(cast<Attribute>(ofr));
...
Value aiir::getValueOrCreateCastToIndexLike(OpBuilder &b, Location loc,
...
static Value convertScalarToComplexDtype(ImplicitLocOpBuilder &b, Value operand,
  ...
  if (isa<FloatType>(operand.getType())) {
    ...
    Value zero = aiir::arith::ConstantFloatOp::create(
        b, toFpTy, aiir::APFloat(toFpTy.getFloatSemantics(), 0));
...
Value aiir::convertScalarToDtype(OpBuilder &b, Location loc, Value operand,
...
SmallVector<Value>
aiir::getValueOrCreateConstantIndexOp(OpBuilder &b, Location loc,
...
Value aiir::createScalarOrSplatConstant(OpBuilder &builder, Location loc,
...
Type aiir::getType(OpFoldResult ofr) {
  if (auto value = dyn_cast_if_present<Value>(ofr))
...
FloatType parseFloatType(AIIRContext *ctx, StringRef name) {
  ...
  return dyn_cast_or_null<FloatType>(aiir::parseType(name, ctx));
}
