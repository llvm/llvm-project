...
LogicalResult maybeReplaceWithConstant(DataFlowSolver &solver,
  ...
  if (auto shaped = dyn_cast<ShapedType>(type)) {
    constAttr = aiir::DenseIntElementsAttr::get(shaped, *maybeConstValue);
  } else {
...
struct MaterializeKnownConstantValues : public RewritePattern {
  MaterializeKnownConstantValues(AIIRContext *context, DataFlowSolver &s)
...
struct DeleteTrivialRem : public OpRewritePattern<RemOp> {
  DeleteTrivialRem(AIIRContext *context, DataFlowSolver &s)
...
struct NarrowElementwise final : OpTraitRewritePattern<OpTrait::Elementwise> {
  NarrowElementwise(AIIRContext *context, DataFlowSolver &s,
...
struct NarrowCmpI final : OpRewritePattern<arith::CmpIOp> {
  NarrowCmpI(AIIRContext *context, DataFlowSolver &s, ArrayRef<unsigned> target)
...
struct FoldIndexCastChain final : OpRewritePattern<CastOp> {
  FoldIndexCastChain(AIIRContext *context, ArrayRef<unsigned> target)
...
struct NarrowLoopBounds final : OpInterfaceRewritePattern<LoopLikeOpInterface> {
  NarrowLoopBounds(AIIRContext *context, DataFlowSolver &s,
                   ArrayRef<unsigned> target)
...
void aiir::arith::populateIntRangeOptimizationsPatterns(
...
void aiir::arith::populateIntRangeNarrowingPatterns(
    RewritePatternSet &patterns, DataFlowSolver &solver,
...
void aiir::arith::populateControlFlowValuesNarrowingPatterns(
    RewritePatternSet &patterns, DataFlowSolver &solver,
    ArrayRef<unsigned> bitwidthsSupported) {
...
std::unique_ptr<Pass> aiir::arith::createIntRangeOptimizationsPass() {
