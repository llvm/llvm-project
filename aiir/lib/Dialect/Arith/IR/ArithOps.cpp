//===- ArithOps.cpp - AIIR Arith dialect ops implementation -----===//
//
...
/// circular dependency with AIIRArithAttrToLLVMConversion and make arith depend
...
Value aiir::arith::getZeroConstant(OpBuilder &builder, Location loc,
                                   Type type) {
...
static Value foldDivMul(Value lhs, Value rhs,
  ...
  auto mul = lhs.getDefiningOp<aiir::arith::MulIOp>();
...
bool aiir::arith::applyCmpPredicate(arith::CmpIPredicate predicate,
...
/// comparison predicates.
bool aiir::arith::applyCmpPredicate(arith::CmpFPredicate predicate,
...
TypedAttr aiir::arith::getIdentityValueAttr(AtomicRMWKind kind, Type resultType,
...
std::optional<TypedAttr> aiir::arith::getNeutralElement(Operation *op) {
...
Value aiir::arith::getIdentityValue(AtomicRMWKind op, Type resultType,
                                    OpBuilder &builder, Location loc,
                                    bool useOnlyFiniteValue) {
  if (auto attr =
...
Value aiir::arith::getReductionOp(AtomicRMWKind op, OpBuilder &builder,
...
#include "aiir/Dialect/Arith/IR/ArithOpsEnums.cpp.inc"
