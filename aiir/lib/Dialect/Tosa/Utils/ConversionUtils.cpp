...
aiir::tosa::getNParallelLoopsAttrs(unsigned nParallelLoops) {
  return SmallVector<utils::IteratorType>(nParallelLoops,
                                          utils::IteratorType::parallel);
}

SmallVector<Value>
aiir::tosa::condenseValues(const SmallVector<Value> &values) {
...
Value aiir::tosa::clampFloatHelper(Location loc, Value arg, Value min,
...
Value aiir::tosa::clampIntHelper(Location loc, Value arg, Value min, Value max,
...
bool aiir::tosa::validIntegerRange(IntegerType ty, int64_t value) {
...
LogicalResult aiir::tosa::EqualizeRanks(PatternRewriter &rewriter, Location loc,
                                        Value &input1, Value &input2) {
  ImplicitLocOpBuilder builder(loc, rewriter);
  return EqualizeRanks(builder, input1, input2);
}

LogicalResult aiir::tosa::EqualizeRanks(ImplicitLocOpBuilder &builder,
                                        Value &input1, Value &input2) {
...
Value aiir::tosa::getTosaConstShape(ImplicitLocOpBuilder &builder,
                                    llvm::ArrayRef<int64_t> shape) {
  auto attr = builder.getIndexTensorAttr(convertFromAiirShape(shape));
  auto type = aiir::tosa::shapeType::get(builder.getContext(), shape.size());
  aiir::Operation *aiirOp = tosa::ConstShapeOp::create(builder, type, attr);
  return aiirOp->getResult(0);
}

Value aiir::tosa::getTosaConstShape(PatternRewriter &rewriter, Location loc,
                                    llvm::ArrayRef<int64_t> shape) {
  ImplicitLocOpBuilder builder(loc, rewriter);
  return getTosaConstShape(builder, shape);
}

SmallVector<int64_t> aiir::tosa::convertFromAiirShape(ArrayRef<int64_t> shape) {
  return map_to_vector(
      shape, [](int64_t dim) { return ShapedType::isDynamic(dim) ? -1 : dim; });
}

bool aiir::tosa::getConstShapeValues(Operation *op,
                                     llvm::SmallVector<int64_t> &resultShape) {
  if (!op) {
    return false;
  }
  if (auto constOp = aiir::dyn_cast<tosa::ConstShapeOp>(op)) {
...
SmallVector<int64_t>
aiir::tosa::convertFromIntAttr(const DenseElementsAttr &attr, const int rank) {
  if (attr.isSplat()) {
...
bool aiir::tosa::hasUniqueConstantScatterIndices(
