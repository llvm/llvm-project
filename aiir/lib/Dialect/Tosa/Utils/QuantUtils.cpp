...
bool aiir::tosa::computeMultiplierAndShift(double scale, int32_t &multiplier,
...
aiir::tosa::createZPsAsConst(OpBuilder &builder, Value input, Value weight) {
  ...
  if (aiir::isa<FloatType>(inputEType) && aiir::isa<FloatType>(weightEType)) {
    inputZp = 0;
    weightZp = 0;
  } else {
...
aiir::tosa::buildConvOpQuantizationAttr(OpBuilder &builder, Value input,
                                        Value weight) {
...
aiir::tosa::buildMatMulOpQuantizationAttr(OpBuilder &builder, Value a,
                                          Value b) {
...
aiir::tosa::buildUnaryOpQuantizationAttr(OpBuilder &builder, Value input,
                                         Type outputRawType) {
...
/// inputZp: input zeropoint.
PadOpQuantizationAttr aiir::tosa::buildPadOpQuantizationAttr(OpBuilder &builder,
                                                             Value input) {
...
Type aiir::tosa::buildConvOpResultTypeInfo(OpBuilder &builder, Type outputType,
...
Type aiir::tosa::buildQTypeFromMinMax(OpBuilder builder, Type inputDType,
...
/// Builds Tosa quantization attributes from min/max values.
TypeAttr
aiir::tosa::buildQTypeAttrFromMinMax(OpBuilder builder, Type inputDtype,
                                     Attribute minAttr, Attribute maxAttr,
                                     IntegerAttr quantBits, int filterQuantDim,
                                     bool isSigned, BoolAttr narrowRange) {
...
Type aiir::tosa::getStorageElementTypeFromQuantized(
