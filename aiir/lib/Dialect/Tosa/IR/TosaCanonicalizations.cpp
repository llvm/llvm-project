...
#include "aiir/Dialect/Tosa/Utils/QuantUtils.h"
...
static bool checkMatchingPadConstAndZp(Value padConst, Value zp) {
  ...
  if (auto padConstFpAttr = aiir::dyn_cast<DenseFPElementsAttr>(padConstAttr)) {
  ...
  if (auto padConstIntAttr =
          aiir::dyn_cast<DenseIntElementsAttr>(padConstAttr)) {
...
struct PoolPadFoldAdaptor<tosa::MaxPool2dOp> {
  ...
  static bool checkPadConstCompliance(OpTy, Value padConst) {
    ...
    if (auto padConstFpAttr =
            aiir::dyn_cast<DenseFPElementsAttr>(padConstAttr)) {
...
struct ClampIsNoOp : public OpRewritePattern<tosa::ClampOp> {
  ...
  LogicalResult matchAndRewrite(tosa::ClampOp op,
    ...
    if (isa<FloatType>(inputElementType)) {
      ...
      const auto minClamp =
          llvm::cast<aiir::FloatAttr>(op.getMinValAttr()).getValue();
      const auto maxClamp =
          llvm::cast<aiir::FloatAttr>(op.getMaxValAttr()).getValue();
    ...
    if (inputElementType.isUnsignedInteger() || isBoolean) {
      const int64_t minClamp = llvm::cast<aiir::IntegerAttr>(op.getMinValAttr())
                                   .getValue()
                                   .getZExtValue();
      const int64_t maxClamp = llvm::cast<aiir::IntegerAttr>(op.getMaxValAttr())
                                   .getValue()
                                   .getZExtValue();
    ...
    if (llvm::isa<IntegerType>(inputElementType)) {
      const int64_t minClamp =
          llvm::cast<aiir::IntegerAttr>(op.getMinValAttr()).getInt();
      const int64_t maxClamp =
          llvm::cast<aiir::IntegerAttr>(op.getMaxValAttr()).getInt();
...
struct ClampClampOptimization : public OpRewritePattern<tosa::ClampOp> {
  ...
  LogicalResult matchAndRewrite(tosa::ClampOp op,
    ...
    if (auto quantType =
            llvm::dyn_cast<aiir::quant::UniformQuantizedType>(inputEType)) {
      inputEType = getStorageElementTypeFromQuantized(quantType);
    }

    Attribute newMinValAttr, newMaxValAttr;
    if (aiir::isa<FloatType>(inputEType)) {
      auto floatMaxValAttr = cast<aiir::FloatAttr>(maxValAttr);
      auto floatMinValAttr = cast<aiir::FloatAttr>(minValAttr);
      auto clampOpFloatMaxValAttr = cast<aiir::FloatAttr>(clampOpMaxValAttr);
      auto clampOpFloatMinValAttr = cast<aiir::FloatAttr>(clampOpMinValAttr);
      ...
      assert(aiir::isa<IntegerType>(inputEType));
      auto intMaxValAttr = cast<aiir::IntegerAttr>(maxValAttr);
      auto intMinValAttr = cast<aiir::IntegerAttr>(minValAttr);
      auto clampOpIntMaxValAttr = cast<aiir::IntegerAttr>(clampOpMaxValAttr);
      auto clampOpIntMinValAttr = cast<aiir::IntegerAttr>(clampOpMinValAttr);
...
static DenseElementsAttr unaryFolder(DenseElementsAttr val, ShapedType returnTy,
  ...
  if (foldDenseValues) {
    aiir::Type elemTy = val.getElementType();
...
OpFoldResult IntDivOp::fold(FoldAdaptor adaptor) {
  ...
  if (rhsAttr && lhsAttr && rhsAttr.isSplat() && lhsAttr.isSplat() &&
    ...
    if (!r.isZero()) {
      auto intTy = dyn_cast<aiir::IntegerType>(resultETy);
      auto const result =
