//===- TosaOps.cpp - AIIR Dialect for TOSA --------------------------------===//
//
...
#include "aiir/Dialect/Tosa/IR/TosaOpsDialect.cpp.inc"
...
#include "aiir/Dialect/Tosa/IR/TosaAvailability.cpp.inc"
#include "aiir/Dialect/Tosa/IR/TosaEnums.cpp.inc"
#include "aiir/Dialect/Tosa/IR/TosaInterfaces.cpp.inc"
#include "aiir/Dialect/Tosa/IR/TosaOpAvailabilityImpl.inc"

namespace {
#include "aiir/Dialect/Tosa/IR/TosaDialectBytecode.cpp.inc"
...
static SmallVector<int64_t> convertToAiirShape(ArrayRef<int64_t> shape) {
...
RankedTensorType aiir::tosa::getVariableType(tosa::VariableOp variableOp) {
  Type elementType = variableOp.getType();
  DenseIntElementsAttr varShapeAttr = variableOp.getVarShape();
  auto shape = convertToAiirShape(to_vector(varShapeAttr.getValues<int64_t>()));
...
#include "aiir/Dialect/Tosa/IR/TosaOpsTypesBase.cpp.inc"
      >();
  addOperations<
#define GET_OP_LIST
...
#include "aiir/Dialect/Tosa/IR/TosaAttributes.cpp.inc"
      >();
...
ParseResult getShapeAndElementType(OpAsmParser &parser, Type parsedType,
  ...
  if (auto shapedType = dyn_cast<ShapedType>(parsedType)) {
    ...
    varShapeAttr = builder.getIndexTensorAttr(convertFromAiirShape(shape));
    return success();
  }
  return parser.emitError(parser.getCurrentLocation())
...
ParseResult aiir::tosa::parseVariableOpTypeOrInitialValue(
...
void aiir::tosa::printVariableOpTypeOrInitialValue(
  ...
  if (!dyn_cast_or_null<TypedAttr>(initialValueAttr)) {
    auto shape =
        convertToAiirShape(to_vector(varShapeAttr.getValues<int64_t>()));
...
static Type getStorageElementTypeOrSelf(Type type) {
  ...
  if (auto quantType = llvm::dyn_cast<aiir::quant::QuantizedType>(srcType))
...
static LogicalResult verifyRescaleValueAndZpTypes(Operation *op, Value val,
  ...
  bool bothInts =
      aiir::isa<IntegerType>(eType) && aiir::isa<IntegerType>(eZpType);
  bool sameBitWidth =
...
Value aiir::tosa::createPadConstTensor(OpBuilder &builder, Location loc,
                                       Value src, int32_t val) {
  const auto srcType = getElementTypeOrSelf(src);
  const auto srcElemType = getStorageElementTypeOrSelf(src);
  const auto padConstType = aiir::RankedTensorType::get({1}, srcType);
  const auto padConstEType = aiir::RankedTensorType::get({1}, srcElemType);
...
unsigned aiir::tosa::getBitWidth(Type type) {
...
static LogicalResult verifyConvOp(T op) {
  ...
  if (auto quantType = llvm::dyn_cast<aiir::quant::QuantizedType>(inputEType))
    inputEType = getStorageElementTypeFromQuantized(quantType);

  if (auto quantType = llvm::dyn_cast<aiir::quant::QuantizedType>(weightEType))
    weightEType = getStorageElementTypeFromQuantized(quantType);

  if (auto quantType = llvm::dyn_cast<aiir::quant::QuantizedType>(biasEType))
    biasEType = getStorageElementTypeFromQuantized(quantType);

  if (auto quantType = llvm::dyn_cast<aiir::quant::QuantizedType>(resultEType))
...
LogicalResult tosa::ConstOp::verify() {
  ...
  if (auto result = llvm::dyn_cast<aiir::quant::QuantizedType>(
...
static LogicalResult verifySameElementTypes(T op, Type aType, Type bType,
  ...
  auto aQuantType =
      llvm::dyn_cast<aiir::quant::UniformQuantizedType>(aElementType);
  auto bQuantType =
      llvm::dyn_cast<aiir::quant::UniformQuantizedType>(bElementType);
...
LogicalResult tosa::ClampOp::verify() {
  ...
  if (auto quantType =
          llvm::dyn_cast<aiir::quant::UniformQuantizedType>(inputETy)) {
    inputETy = getStorageElementTypeFromQuantized(quantType);
  }
  aiir::Type outputETy =
      llvm::cast<ShapedType>(getOutput().getType()).getElementType();
  if (auto quantType =
          llvm::dyn_cast<aiir::quant::UniformQuantizedType>(outputETy)) {
  ...
  if (inputETy.isInteger(dataTypeBitWidth)) {
    ...
    // datatype
    auto intMaxValAttr = aiir::dyn_cast<aiir::IntegerAttr>(maxValAttr);
    auto intMinValAttr = aiir::dyn_cast<aiir::IntegerAttr>(minValAttr);
    ...
    // input's datatype
    auto floatMaxValAttr = aiir::dyn_cast<aiir::FloatAttr>(maxValAttr);
    auto floatMinValAttr = aiir::dyn_cast<aiir::FloatAttr>(minValAttr);
...
static void buildVariableOp(OpBuilder &builder, OperationState &result,
  ...
  ArrayRef<int64_t> shape = shapedType.getShape();
  auto varShapeAttr = builder.getIndexTensorAttr(convertFromAiirShape(shape));
...
LogicalResult tosa::SliceOp::inferReturnTypeComponents(
  ...
  if (inputShape.hasRank()) {
    ...
    outputShape = convertToAiirShape(size);
  }
  inferredReturnShapes.push_back(ShapedTypeComponents(outputShape));
  return success();
}
...
LogicalResult tosa::MulOp::verify() {
  ...
  if (aHasRank && bHasRank) {
    ...
    if (!aiir::OpTrait::util::getBroadcastedShape(
...
LogicalResult tosa::TileOp::inferReturnTypeComponents(
  ...
  multiples = convertToAiirShape(multiples);
...
LogicalResult tosa::ReshapeOp::inferReturnTypeComponents(
  ...
  newShapeValue = convertToAiirShape(newShapeValue);
...
LogicalResult RescaleOp::verify() {
  ...
  if (!aiir::isa<IntegerType>(inputElementType)) {
  ...
  if (!aiir::isa<IntegerType>(outputElementType)) {
...
LogicalResult IfOp::verify() {
  ...
  // AIIR will verify the absence of the terminator for us if otherwise.
...
std::optional<Value> aiir::tosa::createZeroPointTensor(OpBuilder &builder,
                                                       Location loc,
                                                       Type srcElemType,
                                                       int64_t zp) {
  srcElemType = getStorageElementTypeOrSelf(srcElemType);
  auto zpType = aiir::RankedTensorType::get({1}, srcElemType);
...
bool aiir::tosa::isa_tosa_shape_type(aiir::Type t) {
  return aiir::isa<tosa::shapeType>(t);
}

LogicalResult
aiir::tosa::shapeType::verify(function_ref<InFlightDiagnostic()> emitError,
                              int rank) {
  if (rank < 0)
...
LogicalResult OpTrait::tosa::verifyTosaResolvableShapeOperands(Operation *op) {
  for (auto v : op->getOperands()) {
    if (aiir::isa<::aiir::tosa::shapeType>(v.getType())) {
...
OpTrait::tosa::verifyTosaShapeOperatorWithSameRanks(Operation *op) {
  ...
  auto getRank = [](const Type type) {
    return aiir::cast<aiir::tosa::shapeType>(type).getRank();
  };
