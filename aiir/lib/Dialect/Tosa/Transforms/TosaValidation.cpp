...
private:
  ...
  static void getMaxNestedDepth(Operation *op, int32_t &depth) {
    if (isa<aiir::func::FuncOp>(op) || isa<ModuleOp>(op))
      return;
  ...
  SmallVector<
      ...
      constCheckers;
  DenseMap<StringAttr, aiir::Type> variablesMap;
...
LogicalResult TosaValidation::levelCheckSize(Operation *op,
  ...
  if (ShapedType type = dyn_cast<ShapedType>(typeToCheck)) {
    ...
    auto shape = type.getShape();
    for (auto dim : shape) {
      const bool dimIsDynamic = aiir::ShapedType::isDynamic(dim);
...
inline bool CompatibleTypes(const aiir::Type &type,
                            const aiir::Type &declaredType) {
...
LogicalResult TosaValidation::CheckVariable(Operation *op) {
  if (auto variableOp = dyn_cast<aiir::tosa::VariableOp>(op)) {
    aiir::StringAttr nameAttr = variableOp.getNameAttr();
...
LogicalResult TosaValidation::CheckVariableReadOrWrite(Operation *op) {
  if (isa<aiir::tosa::VariableReadOp>(op) ||
      isa<aiir::tosa::VariableWriteOp>(op)) {
    aiir::StringAttr nameAttr = cast<aiir::StringAttr>(op->getAttr("name"));
    if (!variablesMap.count(nameAttr))
