...
llvm::APInt calculateReducedValue(const aiir::ElementsAttr &oldTensorAttr,
...
struct ReduceConstantOptimization : public OpRewritePattern<OperationType> {
  ...
  ReduceConstantOptimization(AIIRContext *context,
  ...
  LogicalResult matchAndRewrite(OperationType op,
    ...
    auto denseAttr =
        aiir::DenseElementsAttr::get(rankedTensorType, newReducedTensor);
...
void aiir::tosa::populateTosaConstantReduction(AIIRContext *ctx,
...
void aiir::tosa::populateTosaFoldConstantTransposePatterns(
    AIIRContext *ctx, RewritePatternSet &patterns) {
  patterns.add<TosaFoldConstantTranspose>(ctx);
}

void aiir::tosa::populateTosaFoldConstantReciprocalPatterns(
    AIIRContext *ctx, RewritePatternSet &patterns) {
