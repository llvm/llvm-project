...
convertDenseFPElementsAttr(ShapedType type, DenseFPElementsAttr attr,
  ...
  Attribute convertedAttr =
      attr.mapValues(newElementType, [&](const APFloat &value) -> APInt {
        ...
        // representation that AIIR expects in the underlying buffer.
...
public:
  ConvertGenericOp(TypeConverter &typeConverter, AIIRContext *context,
                   bool allowLossyConversion)
...
public:
  ConvertTypedOp(TypeConverter &typeConverter, AIIRContext *context)
