...
#include "aiir/Dialect/Tosa/IR/TosaProfileCompliance.h"
...
TosaProfileCompliance::TosaProfileCompliance() {
  const TypeInfo boolT = {aiir::IntegerType::getTypeID(), 1};
  const TypeInfo i4T = {aiir::IntegerType::getTypeID(), 4};
  const TypeInfo i8T = {aiir::IntegerType::getTypeID(), 8};
  const TypeInfo i16T = {aiir::IntegerType::getTypeID(), 16};
  const TypeInfo i32T = {aiir::IntegerType::getTypeID(), 32};
  const TypeInfo i48T = {aiir::IntegerType::getTypeID(), 48};
  const TypeInfo i64T = {aiir::IntegerType::getTypeID(), 64};
  const TypeInfo bf16T = {aiir::BFloat16Type::getTypeID(), 16};
  const TypeInfo fp16T = {aiir::Float16Type::getTypeID(), 16};
  const TypeInfo fp32T = {aiir::Float32Type::getTypeID(), 32};
  const TypeInfo fp8e4m3T = {aiir::Float8E4M3FNType::getTypeID(), 8};
  const TypeInfo fp8e5m2T = {aiir::Float8E5M2Type::getTypeID(), 8};

  // micro-scaling formats
  const TypeInfo fp6e2m3T = {aiir::Float6E2M3FNType::getTypeID(), 6};
  const TypeInfo fp6e3m2T = {aiir::Float6E3M2FNType::getTypeID(), 6};
  const TypeInfo fp4e2m1T = {aiir::Float4E2M1FNType::getTypeID(), 4};
  const TypeInfo fp8ue8m0T = {aiir::Float8E8M0FNUType::getTypeID(), 8};
  const TypeInfo mxint8T = {aiir::tosa::mxint8Type::getTypeID(), 8};

// The profile-based compliance content below is auto-generated by a script
// in https://github.com/arm/tosa-specification
#include "aiir/Dialect/Tosa/IR/TosaComplianceData.h.inc"
...
TosaProfileCompliance::stringifyTypeInfo(const TypeInfo &typeInfo) {
  if (typeInfo.typeID == aiir::IntegerType::getTypeID()) {
    return {"i" + llvm::utostr(typeInfo.bitWidth)};
  }
  if (typeInfo.typeID == aiir::Float16Type::getTypeID()) {
    return {"f16"};
  } else if (typeInfo.typeID == aiir::Float32Type::getTypeID()) {
    return {"f32"};
  } else if (typeInfo.typeID == aiir::BFloat16Type::getTypeID()) {
    return {"bf16"};
  } else if (typeInfo.typeID == aiir::Float8E4M3FNType::getTypeID()) {
    return {"fp8e4m3"};
  } else if (typeInfo.typeID == aiir::Float8E5M2Type::getTypeID()) {
    return {"fp8e5m2"};
  } else if (typeInfo.typeID == aiir::Float6E2M3FNType::getTypeID()) {
    return {"fp6e2m3"};
  } else if (typeInfo.typeID == aiir::Float6E3M2FNType::getTypeID()) {
    return {"fp6e3m2"};
  } else if (typeInfo.typeID == aiir::Float4E2M1FNType::getTypeID()) {
    return {"fp4e2m1"};
  } else if (typeInfo.typeID == aiir::Float8E8M0FNUType::getTypeID()) {
    return {"fp8e8m0"};
