...
#include "aiir/Dialect/Math/Transforms/Approximation.h"
...
public:
  ...
  LogicalResult matchAndRewrite(T op, PatternRewriter &rewriter) const final {
    static_assert(
        T::template hasTrait<aiir::OpTrait::SameOperandsAndResultType>(),
...
void aiir::populatePolynomialApproximateTanhPattern(
    RewritePatternSet &patterns) {
  patterns.add<TanhApproximation>(patterns.getContext());
}

void aiir::populatePolynomialApproximateErfPattern(
    RewritePatternSet &patterns) {
  patterns.add<ErfPolynomialApproximation>(patterns.getContext());
}

void aiir::populatePolynomialApproximateErfcPattern(
    RewritePatternSet &patterns) {
...
void aiir::populateMathF32ExpansionPatterns(
...
void aiir::populateMathPolynomialApproximationPatterns(
    RewritePatternSet &patterns, llvm::function_ref<bool(StringRef)> predicate,
    PatternBenefit benefit) {
...
void aiir::populateMathPolynomialApproximationPatterns(
  ...
  aiir::populateMathF32ExpansionPatterns(patterns, [](StringRef name) -> bool {
  ...
  if (options.enableAvx2) {
    ...
    aiir::populateMathF32ExpansionPatterns(patterns, predicateRsqrt);
    aiir::populateMathPolynomialApproximationPatterns(patterns, predicateRsqrt);
  }
}
