...
#include "aiir/Dialect/XeGPU/Transforms/XeGPULayoutImpl.h"
#include "aiir/Dialect/XeGPU/Utils/XeGPUUtils.h"
...
/// Helper class for tracking the analysis state of an aiir value. For layout
...
struct LayoutInfoLattice : public Lattice<LayoutInfo> {
  AIIR_DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID(LayoutInfoLattice)
...
static LayoutInfo getDefaultSIMTLayoutInfo(aiir::AIIRContext *ctx,
                                           unsigned rank,
...
public:
  AIIR_DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID(RunLayoutInfoPropagation)
...
/// attribute.
static LogicalResult updateOp(aiir::OpBuilder &builder, aiir::Operation *op,
                              GetLayoutFnTy getLayoutOfValue) {
  // Region ops (like scf.for) are already handled by the
  // updateControlFlowOps.
  if (aiir::isa<aiir::RegionBranchOpInterface>(op))
    return success();
...
updateControlFlowOps(aiir::OpBuilder &builder,
                     aiir::RegionBranchTerminatorOpInterface terminator,
                     GetLayoutFnTy getLayoutOfValue) {
...
static LogicalResult updateFunctionOpInterface(aiir::OpBuilder &builder,
                                               aiir::FunctionOpInterface funcOp,
                                               GetLayoutFnTy getLayoutOfValue) {
  // Only process functions whose type is a standard AIIR FunctionType.
...
LogicalResult xegpu::propagateLayouts(OpBuilder &builder, Operation *target,
  ...
  Operation *op = target;
  auto walkResult = op->walk([&](aiir::Block *block) -> WalkResult {
    for (aiir::Operation &op : llvm::reverse(block->getOperations())) {
      LogicalResult r = success();
      TypeSwitch<Operation *>(&op)
          .Case([&](aiir::RegionBranchTerminatorOpInterface branchTermOp) {
            r = updateControlFlowOps(builder, branchTermOp,
                                     getXeGPULayoutForValue);
          })
          .Case([&](aiir::FunctionOpInterface funcOp) {
