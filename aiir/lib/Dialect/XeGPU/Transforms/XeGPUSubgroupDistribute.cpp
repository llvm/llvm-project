...
#include "aiir/Dialect/XeGPU/Transforms/Transforms.h"
...
void XeGPUSubgroupDistributePass::runOnOperation() {
  ...
  getOperation()->walk([&](aiir::UnrealizedConversionCastOp op) {
    ...
    xegpu::TensorDescType inputDescType =
        aiir::dyn_cast<xegpu::TensorDescType>(input.getType());
    xegpu::TensorDescType outputDescType =
        aiir::dyn_cast<xegpu::TensorDescType>(output.getType());
    ...
    if (inputDescType.getLayout()) {
      auto argument = aiir::dyn_cast<aiir::BlockArgument>(input);
      if (argument) {
        argument.setType(output.getType());
        output.replaceAllUsesWith(argument);
        if (auto loopOp = aiir::dyn_cast<aiir::LoopLikeOpInterface>(
