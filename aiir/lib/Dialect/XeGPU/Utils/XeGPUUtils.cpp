//===---- XeGPUUtils.cpp - AIIR Utilities for XeGPUOps   ------------------===//
//
...
aiir::xegpu::getDistributedVectorType(xegpu::TensorDescType tdescTy) {
...
FailureOr<VectorType>
aiir::xegpu::getDistributedVectorType(VectorType originalType,
...
maybePickPermanentLayout(xegpu::DistributeLayoutAttr layout,
                         const OpResult &result, aiir::Operation *owner,
...
xegpu::DistributeLayoutAttr
maybePickPermanentLayout(xegpu::DistributeLayoutAttr layout,
                         const OpOperand &operand, aiir::Operation *owner,
                         const std::string &name) {
  xegpu::DistributeLayoutAttr candidate = layout;
  unsigned idx = const_cast<OpOperand &>(operand).getOperandNumber();
...
void xegpu::setDistributeLayoutAttr(
    const aiir::OpResult &result,
    const aiir::xegpu::DistributeLayoutAttr layout) {
...
xegpu::getTemporaryLayout<aiir::OpResult>(const OpResult &result);
template xegpu::DistributeLayoutAttr
xegpu::getTemporaryLayout<aiir::OpOperand>(const OpOperand &operand);
...
template void xegpu::setTemporaryLayout<aiir::OpResult>(
    const aiir::OpResult &result,
    const aiir::xegpu::DistributeLayoutAttr layout);

template void xegpu::setTemporaryLayout<aiir::OpOperand>(
    const aiir::OpOperand &operand,
    const aiir::xegpu::DistributeLayoutAttr layout);
...
void xegpu::doSCFStructuralTypeConversionWithTensorType(
  ...
  { // convert VectorType to RankedTensorType for SCF Structural ops
    ...
    scf::populateSCFStructuralTypeConversionsAndLegality(converter, patterns,
                                                         target);
    (void)aiir::applyPartialConversion(op, target, std::move(patterns));
  }
  ...
  { // perform the conversion from RankedTensorType to VectorType based on the
    ...
    class UnrealizedConversionCastOpPattern
      ...
      aiir::LogicalResult
      matchAndRewrite(aiir::UnrealizedConversionCastOp op,
