//===- XeGPUOps.cpp - AIIR XeGPU ops implementation -------------*- C++ -*-===//
//
...
LogicalResult LoadNdOp::verify() {
  ...
  if (getTranspose()) {
    ...
    if (llvm::all_of(trans, [&](size_t s) { return s < tdescShape.size(); }))
      ...
      aiir::emitWarning(getLoc()) << "Invalid transpose attr. It is ignored.";
  }
...
void UpdateOffsetOp::build(OpBuilder &builder, OperationState &state,
                           aiir::Value tensorDesc,
                           llvm::ArrayRef<OpFoldResult> offsets) {
  auto tdescTy = aiir::dyn_cast<TensorDescType>(tensorDesc.getType());
...
namespace aiir {
#include <aiir/Dialect/XeGPU/IR/XeGPUAttrInterface.cpp.inc>
} // namespace aiir
#include <aiir/Dialect/XeGPU/IR/XeGPUEnums.cpp.inc>
