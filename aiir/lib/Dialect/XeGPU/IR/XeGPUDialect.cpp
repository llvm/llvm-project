//===- XeGPUDialect.cpp - AIIR XeGPU dialect implementation -----*- C++ -*-===//
//
...
#include "aiir/Dialect/XeGPU/uArch/IntelGpuXe2.h"
...
#include <aiir/Dialect/XeGPU/IR/XeGPUTypes.cpp.inc>
      >();
  addOperations<
#define GET_OP_LIST
#include <aiir/Dialect/XeGPU/IR/XeGPU.cpp.inc>
      >();
  addAttributes<
#define GET_ATTRDEF_LIST
#include <aiir/Dialect/XeGPU/IR/XeGPUAttrs.cpp.inc>
      >();
}
#define GET_OP_INTERFACE_CLASSES
#include "aiir/Dialect/XeGPU/IR/XeGPUOpInterface.cpp.inc"
...
//===----------------------------------------------------------------------===//
BlockTensorDescAttr BlockTensorDescAttr::get(aiir::AIIRContext *context,
...
ScatterTensorDescAttr::get(aiir::AIIRContext *context,
...
//===----------------------------------------------------------------------===//
LogicalResult
LayoutAttr::verify(llvm::function_ref<aiir::InFlightDiagnostic()> emitError,
...
LogicalResult
RangeAttr::verify(llvm::function_ref<aiir::InFlightDiagnostic()> emitError,
...
aiir::Type TensorDescType::parse(AsmParser &parser) {
  llvm::SmallVector<int64_t> shape;
  aiir::Type elementType;
  aiir::FailureOr<aiir::Attribute> encoding;
  aiir::FailureOr<aiir::Attribute> layout;
  ...
  if (aiir::failed(parser.parseDimensionList(shape))) {
    parser.emitError(shapeLoc, "failed to parse parameter 'shape'");
    return {};
  }

  auto elemTypeLoc = parser.getCurrentLocation();
  if (aiir::failed(parser.parseType(elementType))) {
  ...
  while (aiir::succeeded(parser.parseOptionalComma())) {
    ...
    if (aiir::succeeded(res)) {
      if (aiir::isa<LayoutAttr>(attr)) {
        layout = attr;
        continue;
      }
      if (aiir::isa<BlockTensorDescAttr, ScatterTensorDescAttr>(attr)) {
        encoding = attr;
        continue;
      }
    }
  ...
  AIIRContext *ctxt = parser.getContext();
  return TensorDescType::getChecked(
      ...
      layout.value_or(aiir::Attribute()));
}
...
void TensorDescType::print(AsmPrinter &printer) const {
  ...
  for (int64_t dim : shape) {
    if (aiir::ShapedType::isDynamic(dim))
      printer << '?';
    else
      printer << dim;
    printer << 'x';
...
TensorDescType TensorDescType::get(llvm::ArrayRef<int64_t> shape,
                                   aiir::Type elementType, int array_length,
                                   bool boundary_check,
                                   MemorySpace memory_space,
                                   aiir::Attribute layout) {
...
TensorDescType TensorDescType::get(llvm::ArrayRef<int64_t> shape,
                                   aiir::Type elementType, int chunk_size,
                                   MemorySpace memory_space,
                                   aiir::Attribute layout) {
  auto *context = elementType.getContext();
...
TensorDescType::verify(llvm::function_ref<InFlightDiagnostic()> emitError,
                       llvm::ArrayRef<int64_t> shape, aiir::Type elementType,
                       aiir::Attribute encoding, aiir::Attribute layout) {
  size_t rank = shape.size();

  if (rank == 0)
    return emitError() << "expected non-zero rank tensor";

  auto blockAttr = aiir::dyn_cast_if_present<BlockTensorDescAttr>(encoding);
  if (blockAttr) {
  ...
  int chunkAlignmentFactor =
      bitWidth < xegpu::uArch::generalPackedFormatBitSize
          ...
          : 1;
  auto scatterAttr = aiir::dyn_cast_if_present<ScatterTensorDescAttr>(encoding);
  if (scatterAttr) {
...
// XeGPU_MemDescType
//===----------------------------------------------------------------------===//
aiir::Type MemDescType::parse(AsmParser &parser) {
  llvm::SmallVector<int64_t> shape;
  aiir::Type elementType;
  aiir::FailureOr<MemLayoutAttr> layout;
  ...
  auto shapeLoc = parser.getCurrentLocation();
  if (aiir::failed(parser.parseDimensionList(shape, false, true))) {
    parser.emitError(shapeLoc, "failed to parse parameter 'shape'");
    return {};
  }
...
#include <aiir/Dialect/XeGPU/IR/XeGPUDialect.cpp.inc>
