...
LogicalResult MLProgramPipelineGlobals::buildGlobalMap(ModuleOp module) {
  ...
  auto res = module->walk([&](Operation *op) {
    if (auto caller = aiir::dyn_cast<CallOpInterface>(op)) {
      auto callable = caller.getCallableForCallee();
      // For now we do not know how to handle Value based tracing, so fail.
      if (aiir::isa<Value>(callable)) {
        return WalkResult::interrupt();
      }

      auto symbol = aiir::dyn_cast<SymbolRefAttr>(callable);
...
void MLProgramPipelineGlobals::processBlock(
  ...
  for (auto &op : block) {
    ...
    // known load.
    if (auto load = aiir::dyn_cast<GlobalLoadOp>(op)) {
    ...
    if (auto store = aiir::dyn_cast<GlobalStoreOp>(op)) {
    ...
    if (auto call = aiir::dyn_cast<CallOpInterface>(op)) {
      auto loadSymbols =
