...
#include "aiir/Dialect/Linalg/Transforms/Hoisting.h"
#include "aiir/Dialect/Linalg/Transforms/Transforms.h"
#include "aiir/Dialect/Linalg/Utils/Utils.h"
#include "aiir/Dialect/SCF/Transforms/TileUsingInterface.h"
...
transform::FuseIntoContainingOp::apply(transform::TransformRewriter &rewriter,
  ...
  if (std::empty(producerOps)) {
    results.set(cast<OpResult>(getFusedOp()), SmallVector<aiir::Operation *>{});
  ...
  while (!remainingProducers.empty()) {
    ...
    if (failed(nextProducer)) {
      auto diag = aiir::emitSilenceableFailure(getLoc())
...
transform::MatchOp::apply(transform::TransformRewriter &rewriter,
  ...
  auto matchFun = [&](Operation *op) {
    ...
    if (getFilterOperandTypes().has_value()) {
      aiir::ArrayAttr types = getFilterOperandTypes().value();
      auto operandTypes = op->getOperandTypes();

      if (types.size() == 1) {
        // All the operands must must be equal to the specified type
        auto typeattr =
            dyn_cast<aiir::TypeAttr>(getFilterOperandTypes().value()[0]);
        Type t = cast<::aiir::Type>(typeattr.getValue());
        ...
        for (auto [attr, operandType] :
          ...
          auto typeattr = cast<aiir::TypeAttr>(attr);
          Type type = cast<::aiir::Type>(typeattr.getValue());
...
transform::PromoteOp::applyToOne(transform::TransformRewriter &rewriter,
  ...
  if (getMapping().has_value()) {
    ...
    auto addressSpace = cast<aiir::gpu::GPUMemorySpaceMappingAttr>(mapping[0]);

    if (addressSpace.getAddressSpace() ==
        aiir::gpu::GPUDialect::getWorkgroupAddressSpace()) {
      promotionOptions =
          promotionOptions
              ...
              .setUseFullTileBuffers({false, false});
               ...
               aiir::gpu::GPUDialect::getPrivateAddressSpace()) {
      promotionOptions =
          promotionOptions
...
SplitOp::apply(transform::TransformRewriter &rewriter,
  ...
  if (isMultiwaySplit && !llvm::hasSingleElement(payload)) {
    return aiir::emitSilenceableFailure(getLoc())
...
struct VectorizationPattern : public RewritePattern {
  explicit VectorizationPattern(AIIRContext *context,
...
transform::HoistRedundantVectorBroadcastsOp::applyToOne(
    transform::TransformRewriter &rewriter, aiir::Operation *target,
    transform::ApplyToEachResultList &results,
    transform::TransformState &state) {
  rewriter.setInsertionPoint(target);
...
doit(RewriterBase &rewriter, OpTy target,
  ...
  if (isa<aiir::ParallelCombiningOpInterface>(target.getOperation()))
...
#include "aiir/Dialect/Linalg/TransformOps/LinalgTransformOpsEnums.cpp.inc"
