...
#include "aiir/Dialect/Utils/VerificationUtils.h"
...
public:
  FoldFillWithPack(AIIRContext *context)
...
void GenericOp::build(
  ...
  build(builder, result, resultTensorTypes, inputs, outputs,
        ...
        builder.getArrayAttr(llvm::map_to_vector(
            iteratorTypes,
            [&](utils::IteratorType iter) -> aiir::Attribute {
...
static void printShortForm(OpAsmPrinter &p, Operation *payloadOp) {
  ...
  for (const auto &attr : payloadOp->getAttrs()) {
    auto fastAttr =
        llvm::dyn_cast<aiir::arith::FastMathFlagsAttr>(attr.getValue());
    if (fastAttr && fastAttr.getValue() == aiir::arith::FastMathFlags::none) {
...
void TransposeOp::build(::aiir::OpBuilder &builder,
                        ::aiir::OperationState &result, Value input, Value init,
...
struct SwapTransposeWithBroadcast : OpRewritePattern<linalg::TransposeOp> {
  ...
  LogicalResult matchAndRewrite(linalg::TransposeOp transposeOp,
    ...
    AIIRContext *ctx = transposeOp.getContext();
    SmallVector<OpFoldResult> dims;
    auto broadcastInputTy =
        aiir::cast<RankedTensorType>(broadcastInput.getType());
...
void BroadcastOp::build(::aiir::OpBuilder &builder,
                        ::aiir::OperationState &result, Value input, Value init,
...
#include "aiir/Dialect/Linalg/IR/LinalgNamedStructuredOps.yamlgen.cpp.inc"
...
AffineMap aiir::linalg::extractOrIdentityMap(std::optional<AffineMap> maybeMap,
                                             unsigned rank,
                                             AIIRContext *context) {
  if (maybeMap)
    return *maybeMap;
...
aiir::linalg::makeAffineDimExprs(unsigned num, unsigned &startIdx,
                                 AIIRContext *context) {
...
SmallVector<AffineExpr, 4> aiir::linalg::concat(ArrayRef<AffineExpr> a,
...
std::string aiir::linalg::generateLibraryCallName(Operation *op) {
...
SoftmaxOp::getTiledImplementation(OpBuilder &builder,
  ...
  Operation *tiledOp =
      aiir::clone(builder, getOperation(), resultTypes, tiledOperands);
...
WinogradInputTransformOp::getTiledImplementation(OpBuilder &builder,
  ...
  Location loc = getLoc();
  AIIRContext *context = builder.getContext();
...
SmallVector<AffineMap> MatmulOp::getDefaultIndexingMaps(AIIRContext *context) {
  AffineExpr d0, d1, d2;
...
//===----------------------------------------------------------------------===//
SmallVector<AffineMap>
BatchMatmulOp::getDefaultIndexingMaps(AIIRContext *context) {
  AffineExpr d0, d1, d2, d3;
  SmallVector<AffineMap> indexingMaps;
  bindDims(context, d0, d1, d2, d3);
...
ParseResult ElementwiseOp::parse(OpAsmParser &parser, OperationState &result) {
  ...
  Attribute attr;
  aiir::linalg::ElementwiseKind elemwiseKindVal;
...
SmallVector<AffineMap>
BatchReduceMatmulOp::getDefaultIndexingMaps(AIIRContext *context) {
  AffineExpr d0, d1, d2, d3;
  SmallVector<AffineMap> indexingMaps;
  bindDims(context, d0, d1, d2, d3);
  indexingMaps.push_back(AffineMap::get(4, 0, {d0, d1, d3}, context));
