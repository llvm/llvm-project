...
#include "aiir/Dialect/Tensor/Utils/Utils.h"
#include "aiir/IR/PatternMatch.h"

using namespace aiir;
using namespace aiir::linalg;

FailureOr<SplitReductionResult> aiir::linalg::splitReduction(
...
FailureOr<SplitReductionResult> aiir::linalg::splitReductionByScaling(
    RewriterBase &b, LinalgOp op,
    const ControlSplitReductionFn &controlSplitReductionFn, bool useAlloc) {
  OpBuilder::InsertionGuard guard(b);
  b.setInsertionPoint(op);
...
struct LinalgSplitReduction : public OpInterfaceRewritePattern<LinalgOp> {
  ...
  LinalgSplitReduction(AIIRContext *context,
