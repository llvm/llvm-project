...
SmallVector<Value> aiir::linalg::peelLoop(RewriterBase &rewriter,
                                          Operation *op) {
...
/// where relevant.
void aiir::linalg::peelLoops(RewriterBase &rewriter,
...
LinalgTilingOptions &
aiir::linalg::LinalgTilingOptions::setTileSizes(ArrayRef<int64_t> ts) {
...
LogicalResult aiir::linalg::CopyVectorizationPattern::matchAndRewrite(
...
dropDimsAndCompress(AffineExpr expr, ArrayRef<unsigned> dimsToDrop,
                    unsigned newNumDims, AIIRContext *ctx) {
...
struct DownscaleSizeOneWindowedConvolution final
  ...
  DownscaleSizeOneWindowedConvolution(AIIRContext *context,
