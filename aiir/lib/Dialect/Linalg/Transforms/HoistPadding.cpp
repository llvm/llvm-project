...
using namespace aiir::linalg::detail;
...
static Value buildLoopIterationCount(RewriterBase &rewriter, scf::ForOp outer,
                                     scf::ForOp forOp) {
  AIIRContext *ctx = forOp->getContext();
...
/// `outermostEnclosingForOp`.
/// The loop nest is built just before `outermostEnclosingForOp`.
FailureOr<PackingResult> aiir::linalg::detail::buildPackingLoopNest(
...
FailureOr<Value> aiir::linalg::hoistPaddingOnTensors(
