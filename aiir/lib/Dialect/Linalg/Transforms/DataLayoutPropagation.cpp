...
public:
  BubbleUpPackOpThroughGenericOpPattern(AIIRContext *context,
...
public:
  BubbleUpPackThroughPadOp(AIIRContext *context, ControlPropagationFn fun)
      : OpRewritePattern<linalg::PackOp>(context), controlFn(std::move(fun)) {}
...
class BubbleUpPackOpThroughReshapeOp final
    : public OpRewritePattern<linalg::PackOp> {
public:
  BubbleUpPackOpThroughReshapeOp(AIIRContext *context, ControlPropagationFn fun)
      : OpRewritePattern<linalg::PackOp>(context), controlFn(std::move(fun)) {}
...
public:
  PushDownUnPackOpThroughReshapeOp(AIIRContext *context,
                                   ControlPropagationFn fun)
...
public:
  PushDownUnPackOpThroughGenericOp(AIIRContext *context,
                                   ControlPropagationFn fun,
                                   bool poisonPaddingOk)
...
struct PushDownUnPackThroughPadOp : public OpRewritePattern<tensor::PadOp> {
  PushDownUnPackThroughPadOp(AIIRContext *context, ControlPropagationFn fun)
...
pushDownExtractSliceOpThroughGenericOp(RewriterBase &rewriter,
  ...
  AIIRContext *ctx = genericOp.getContext();
...
public:
  PushDownExtractSliceOpThroughGenericOp(AIIRContext *context,
                                         ControlPropagationFn fun)
      : OpRewritePattern<GenericOp>(context), controlFn(std::move(fun)) {}
...
void aiir::linalg::populateDataLayoutPropagationPatterns(
...
void aiir::linalg::populateExtractSliceSinkingPatterns(
    RewritePatternSet &patterns,
    const ControlPropagationFn &controlPackUnPackPropagation) {
