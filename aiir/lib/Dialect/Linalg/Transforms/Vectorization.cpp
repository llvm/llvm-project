...
VectorizationState::maskOperation(RewriterBase &rewriter, Operation *opToMask,
  ...
  auto maskOp = cast<vector::MaskOp>(
      aiir::vector::maskOperation(rewriter, opToMask, mask));
...
aiir::linalg::getCombinerOpKind(Operation *combinerOp) {
  using ::aiir::vector::CombiningKind;
...
vectorizeTensorExtract(RewriterBase &rewriter, VectorizationState &state,
  ...
  if (memAccessKind == VectorMemoryAccessKind::ScalarBroadcast) {
    ...
    if (dstRank > 0) {
      ...
      readOrMaskedReadOp =
          aiir::vector::maskOperation(rewriter, transferReadOp, allTrue);
    }
  ...
  while (rankDiff > 0) {
    permutationMap = permutationMap.insertResult(
        aiir::getAffineConstantExpr(0, rewriter.getContext()), 0);
    rankDiff--;
  }
...
vectorizeAsLinalgGeneric(RewriterBase &rewriter, VectorizationState &state,
  ...
  aiir::getUsedValuesDefinedAbove(linalgOp->getRegion(0), valuesSet);
...
createWriteOrMaskedWrite(OpBuilder &builder, Location loc, Value vecToStore,
  ...
  return aiir::vector::maskOperation(builder, write, maskForWrite);
}
...
vectorizeAsLinalgContraction(RewriterBase &rewriter, VectorizationState &state,
  ...
  for (OpOperand &opOperand : linalgOp->getOpOperands()) {
    ...
    Value read = aiir::vector::createReadOrMaskedRead(
...
LogicalResult aiir::linalg::vectorizeOpPrecondition(
...
bool aiir::linalg::hasVectorizationImpl(Operation *op) {
  return isa<linalg::LinalgOp, tensor::PadOp, linalg::PackOp, linalg::UnPackOp,
             tensor::InsertSliceOp>(op);
}

FailureOr<VectorizationResult> aiir::linalg::vectorize(
...
LogicalResult aiir::linalg::vectorizeCopy(RewriterBase &rewriter,
...
void aiir::linalg::populatePadOpVectorizationPatterns(
...
public:
  ...
  FailureOr<Operation *> depthwiseConv(uint64_t channelDimVecSize,
    ...
    auto maybeMaskXferOp = [&](ArrayRef<int64_t> maskShape,
      ...
      return aiir::vector::maskOperation(rewriter, opToMask, maskOp);
    };
...
void aiir::linalg::populateConvolutionVectorizationPatterns(
