...
/// Emits the AIIR for the scalar part of the generic op by:
...
public:
  LinalgRewritePattern(AIIRContext *context)
...
struct FoldAffineOp : public RewritePattern {
  FoldAffineOp(AIIRContext *context)
...
static void lowerLinalgToLoopsImpl(Operation *enclosingOp) {
  AIIRContext *context = enclosingOp->getContext();
...
aiir::linalg::linalgOpToAffineLoops(RewriterBase &rewriter, LinalgOp linalgOp) {
  return linalgOpToLoopsImpl<affine::AffineForOp>(rewriter, linalgOp);
}

/// Emits a loop nest of `scf.for` with the proper body for `linalgOp`.
FailureOr<LinalgLoops> aiir::linalg::linalgOpToLoops(RewriterBase &rewriter,
                                                     LinalgOp linalgOp) {
...
FailureOr<LinalgLoops>
aiir::linalg::linalgOpToParallelLoops(RewriterBase &rewriter,
                                      LinalgOp linalgOp) {
