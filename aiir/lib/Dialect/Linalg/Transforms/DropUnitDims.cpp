...
static UnitExtentReplacementInfo dropUnitExtentFromOperandMetadata(
    AIIRContext *context, IndexingMapOpInterface op, OpOperand *opOperand,
...
struct DropUnitDims : public OpRewritePattern<GenericOp> {
  DropUnitDims(AIIRContext *context, ControlDropUnitDims options = {},
...
struct DropPadUnitDims : public OpRewritePattern<tensor::PadOp> {
  DropPadUnitDims(AIIRContext *context, ControlDropUnitDims options = {},
                  PatternBenefit benefit = 1)
      : OpRewritePattern(context, benefit), options(std::move(options)) {}
...
/// broadcasting.
void aiir::linalg::populateFoldUnitExtentDimsPatterns(
...
void aiir::linalg::populateFoldUnitExtentDimsCanonicalizationPatterns(
    RewritePatternSet &patterns, linalg::ControlDropUnitDims &options) {
  auto *context = patterns.getContext();
...
void aiir::linalg::populateMoveInitOperandsToInputPattern(
...
void aiir::linalg::populateContractionOpRankReducingPatterns(
    RewritePatternSet &patterns) {
