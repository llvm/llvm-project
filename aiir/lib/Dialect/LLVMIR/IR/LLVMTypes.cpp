//===- LLVMTypes.cpp - AIIR LLVM dialect types ------------------*- C++ -*-===//
//
...
//===----------------------------------------------------------------------===//
//
// This file implements the types for the LLVM dialect in AIIR. These AIIR types
...
#include "aiir/Dialect/LLVMIR/LLVMTypeInterfaces.cpp.inc"

#define GET_TYPEDEF_CLASSES
#include "aiir/Dialect/LLVMIR/LLVMTypes.cpp.inc"
...
std::optional<uint64_t> aiir::LLVM::extractPointerSpecValue(Attribute attr,
                                                            PtrDLEntryPos pos) {
...
LLVMStructType LLVMStructType::getIdentified(AIIRContext *context,
                                             StringRef name) {
...
LLVMStructType LLVMStructType::getNewIdentified(AIIRContext *context,
                                                StringRef name,
...
LLVMStructType LLVMStructType::getLiteral(AIIRContext *context,
...
bool aiir::LLVM::isCompatibleOuterType(Type type) {
  // clang-format off
  if (llvm::isa<
      BFloat16Type,
      Float16Type,
...
bool aiir::LLVM::isCompatibleType(Type type) {
  return LLVMDialect::isCompatibleType(type);
}

bool aiir::LLVM::isLoadableType(Type type) {
...
llvm::ElementCount aiir::LLVM::getVectorNumElements(Type type) {
...
bool aiir::LLVM::isScalableVectorType(Type vectorType) {
...
Type aiir::LLVM::getVectorType(Type elementType, unsigned numElements,
                               bool isScalable) {
...
Type aiir::LLVM::getVectorType(Type elementType,
...
llvm::TypeSize aiir::LLVM::getPrimitiveTypeSizeInBits(Type type) {
