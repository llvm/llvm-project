//===- LLVMTypeSyntax.cpp - Parsing/printing for AIIR LLVM Dialect types --===//
//
...
static void dispatchPrint(AsmPrinter &printer, Type type) {
  if (isCompatibleType(type) &&
    ...
    return aiir::LLVM::detail::printType(type, printer);
...
void aiir::LLVM::detail::printType(Type type, AsmPrinter &printer) {
  if (!type) {
    printer << "<<NULL-TYPE>>";
    return;
  }
...
Type aiir::LLVM::detail::parseType(DialectAsmParser &parser) {
