...
//===----------------------------------------------------------------------===//
//
// This file defines the attribute details for the LLVM IR dialect in AIIR.
//
//===----------------------------------------------------------------------===//
...
#include "aiir/Dialect/LLVMIR/LLVMAttrInterfaces.cpp.inc"
#include "aiir/Dialect/LLVMIR/LLVMOpsEnums.cpp.inc"
#define GET_ATTRDEF_CLASSES
#include "aiir/Dialect/LLVMIR/LLVMOpsAttrDefs.cpp.inc"
...
MemoryEffectsAttr MemoryEffectsAttr::get(AIIRContext *context,
...
DIExpressionAttr DIExpressionAttr::get(AIIRContext *context) {
...
TargetFeaturesAttr TargetFeaturesAttr::get(AIIRContext *context,
...
TargetFeaturesAttr
TargetFeaturesAttr::getChecked(function_ref<InFlightDiagnostic()> emitError,
                               AIIRContext *context, StringRef targetFeatures) {
  SmallVector<StringRef> features;
  targetFeatures.split(features, ',', /*MaxSplit=*/-1,
                       /*KeepEmpty=*/false);
...
FailureOr<::aiir::Attribute> TargetAttr::query(DataLayoutEntryKey key) {
