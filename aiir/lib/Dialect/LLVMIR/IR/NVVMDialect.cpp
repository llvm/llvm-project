...
#include "aiir/Dialect/LLVMIR/NVVMOpsDialect.cpp.inc"
#include "aiir/Dialect/LLVMIR/NVVMOpsEnums.cpp.inc"
...
static bool isPtrInAddrSpace(aiir::Value ptr, NVVMMemorySpace targetAS) {
  auto ptrTy = llvm::cast<LLVM::LLVMPointerType>(ptr.getType());
  return ptrTy.getAddressSpace() == static_cast<unsigned>(targetAS);
}

static bool isPtrInGenericSpace(aiir::Value ptr) {
  return isPtrInAddrSpace(ptr, NVVMMemorySpace::Generic);
}

static bool isPtrInSharedCTASpace(aiir::Value ptr) {
  return isPtrInAddrSpace(ptr, NVVMMemorySpace::Shared);
}

static bool isPtrInSharedClusterSpace(aiir::Value ptr) {
...
LogicalResult ConvertF32x2ToF6x2Op::verify() {
  ...
  if (!llvm::isa<aiir::Float6E2M3FNType, aiir::Float6E3M2FNType>(getDstTy())) {
    return emitOpError("Only ")
           << aiir::Float6E2M3FNType::get(ctx) << " and "
           << aiir::Float6E3M2FNType::get(ctx)
...
LogicalResult ConvertF32x2ToF8x2Op::verify() {
  ...
  return llvm::TypeSwitch<aiir::Type, LogicalResult>(getDstTy())
      .Case<aiir::Float8E4M3FNType, aiir::Float8E5M2Type>(
          [&](aiir::Type) -> LogicalResult {
            if (!isRoundingModeRN) {
              return emitOpError("Only RN rounding mode is supported for "
                                 "conversions from f32x2 to ")
                     << aiir::Float8E4M3FNType::get(ctx) << " and "
                     << aiir::Float8E5M2Type::get(ctx) << " types";
            }
            if (!isSatFinite) {
      ...
      .Default([&](aiir::Type) {
        return emitOpError("Only ")
               << aiir::Float8E4M3FNType::get(ctx) << ", "
               << aiir::Float8E5M2Type::get(ctx) << ", and "
               << aiir::Float8E8M0FNUType::get(ctx)
               << " types are "
...
LogicalResult ConvertF16x2ToF8x2Op::verify() {
  ...
  if (!llvm::isa<aiir::Float8E4M3FNType, aiir::Float8E5M2Type>(getDstTy())) {
    return emitOpError("Only ")
           << aiir::Float8E4M3FNType::get(ctx) << " and "
           << aiir::Float8E5M2Type::get(ctx)
...
LogicalResult ConvertBF16x2ToF8x2Op::verify() {
  ...
  if (!llvm::isa<aiir::Float8E8M0FNUType>(getDstTy()))
    return emitOpError("Only ") << aiir::Float8E8M0FNUType::get(getContext())
...
LogicalResult ConvertF32x2ToF4x2Op::verify() {
  ...
  if (!llvm::isa<aiir::Float4E2M1FNType>(getDstTy()))
    return emitOpError("Only ")
           << aiir::Float4E2M1FNType::get(ctx)
...
LogicalResult ConvertF32x4ToF4x4Op::verify() {
  ...
  if (!llvm::isa<aiir::Float4E2M1FNType>(getDstTy()))
    return emitOpError("Only ") << aiir::Float4E2M1FNType::get(ctx)
                                << " type is supported for conversions from "
                                   "f32x4 to f4x4.";
...
std::optional<aiir::NVVM::MMATypes>
...
MMATypes MmaOp::accumPtxType() {
  std::optional<aiir::NVVM::MMATypes> val = inferOperandMMAType(
...
MMATypes MmaOp::resultPtxType() {
  std::optional<aiir::NVVM::MMATypes> val =
...
void MmaOp::print(OpAsmPrinter &p) {
  ...
  SmallVector<StringRef, 4> ignoreAttrNames{
      aiir::NVVM::MmaOp::getOperandSegmentSizeAttr()};
...
MMATypes MmaSpOp::accumPtxType() {
  std::optional<aiir::NVVM::MMATypes> val = MmaOp::inferOperandMMAType(
      getODSOperands(2).getTypes().front(), /*isAccumulator=*/true);
  assert(val.has_value() && "accumulator PTX type should always be inferrable");
  return val.value();
}
...
MmaSpOp::getIntrinsicIDAndArgs(Operation &op, LLVM::ModuleTranslation &mt,
  ...
  for (aiir::Value v : thisOp.getOperands())
...
void MmaSpOp::print(OpAsmPrinter &p) {
  ...
  SmallVector<StringRef, 4> ignoreAttrNames{
      aiir::NVVM::MmaSpOp::getOperandSegmentSizeAttr()};
...
static void
inferAndSetMultiplicandTypes(AIIRContext *ctx, NamedAttrList &attrs,
...
static void addInferredMultiplicandTypes(
    AIIRContext *ctx, OperationState &result, ValueRange operandA,
    ValueRange operandB,
    std::optional<std::array<MMATypes, 2>> multiplicandPtxTypes) {
  if (multiplicandPtxTypes) {
    result.addAttribute("multiplicandAPtxType",
...
void MmaBlockScaleOp::print(OpAsmPrinter &p) {
  ...
  std::array<MMAOperandFragment, 3> frags{
      ...
      MMAOperandFragment("C", "")};
  SmallVector<StringRef, 4> ignoreAttrNames{
      aiir::NVVM::MmaBlockScaleOp::getOperandSegmentSizeAttr()};
...
void MmaSpBlockScaleOp::print(OpAsmPrinter &p) {
  ...
  std::array<MMAOperandFragment, 3> frags{
      ...
      MMAOperandFragment("C", "")};
  SmallVector<StringRef, 4> ignoreAttrNames{
      aiir::NVVM::MmaSpBlockScaleOp::getOperandSegmentSizeAttr()};
  ...
  if (returnStructType) {
    ...
    if (!predicateType.isInteger(1))
      return verifyTypeError("second element in the returned struct",
                             aiir::IntegerType::get(getContext(), 1),
                             predicateType);
  } else {
    if (getReturnValueAndIsValid())
      return emitOpError("expected return type to be a two-element struct");
...
std::pair<aiir::Type, unsigned> NVVM::inferMMAType(NVVM::MMATypes type,
...
LogicalResult NVVM::WgmmaMmaAsyncOp::verify() {
  ...
  if ((typeA != WGMMATypes::f16 && typeA != WGMMATypes::bf16) &&
      (getLayoutA() == aiir::NVVM::MMALayout::col ||
       getLayoutB() == aiir::NVVM::MMALayout::row)) {
    return emitOpError()
...
bool NVVM::WgmmaMmaAsyncOp::getAsmValues(
    ...
    llvm::SmallVectorImpl<std::pair<aiir::Value, aiir::NVVM::PTXRegisterMod>>
        &asmValues) {
  bool isF16 = getTypeA() == WGMMATypes::f16 || getTypeA() == WGMMATypes::bf16;
  if (getResults())
    asmValues.push_back({getResults(), aiir::NVVM::PTXRegisterMod::Write});
  if (getInouts())
    asmValues.push_back({getInouts(), aiir::NVVM::PTXRegisterMod::ReadWrite});
  asmValues.push_back({getDescriptorA(), aiir::NVVM::PTXRegisterMod::Read});
  asmValues.push_back({getDescriptorB(), aiir::NVVM::PTXRegisterMod::Read});
  asmValues.push_back({makeConstantI32(rewriter, static_cast<int>(getScaleD())),
                       aiir::NVVM::PTXRegisterMod::Read});
  if (getTypeD() != WGMMATypes::s32) {
    asmValues.push_back(
...
static LogicalResult verifyAddSubFOp(OpType op) {
  aiir::NVVM::FPRoundingMode rndMode = op.getRnd();
  aiir::NVVM::SaturationMode satMode = op.getSat();
  bool isFTZ = op.getFtz();

  aiir::Type opType = op.getRes().getType();
  aiir::Type opBaseType = isa<VectorType>(opType)
...
LogicalResult NVVM::FmaOp::verify() {
  ...
  aiir::NVVM::FPRoundingMode rndMode = getRnd();
  aiir::NVVM::SaturationMode satMode = getSat();
  bool isFTZ = getFtz();
  bool isRelu = getRelu();
  bool hasOOB = getOob();
...
aiir::NVVM::IDArgPair NVVM::BarrierOp::getIntrinsicIDAndArgs(
    Operation &op, LLVM::ModuleTranslation &mt, llvm::IRBuilderBase &builder) {
...
aiir::NVVM::IDArgPair MBarrierInitOp::getIntrinsicIDAndArgs(
    Operation &op, LLVM::ModuleTranslation &mt, llvm::IRBuilderBase &builder) {
...
aiir::NVVM::IDArgPair MBarrierInvalOp::getIntrinsicIDAndArgs(
    Operation &op, LLVM::ModuleTranslation &mt, llvm::IRBuilderBase &builder) {
...
aiir::NVVM::IDArgPair MBarrierExpectTxOp::getIntrinsicIDAndArgs(
    Operation &op, LLVM::ModuleTranslation &mt, llvm::IRBuilderBase &builder) {
...
aiir::NVVM::IDArgPair MBarrierCompleteTxOp::getIntrinsicIDAndArgs(
    Operation &op, LLVM::ModuleTranslation &mt, llvm::IRBuilderBase &builder) {
...
aiir::NVVM::IDArgPair MBarrierArriveOp::getIntrinsicIDAndArgs(
    Operation &op, LLVM::ModuleTranslation &mt, llvm::IRBuilderBase &builder) {
...
aiir::NVVM::IDArgPair MBarrierArriveDropOp::getIntrinsicIDAndArgs(
    Operation &op, LLVM::ModuleTranslation &mt, llvm::IRBuilderBase &builder) {
...
bool MBarrierArriveExpectTxOp::getAsmValues(
  ...
  for (auto val : getOperands())
    asmValues.push_back({val, aiir::NVVM::PTXRegisterMod::Read});

  return false;
}

aiir::NVVM::IDArgPair MBarrierArriveExpectTxOp::getIntrinsicIDAndArgs(
    Operation &op, LLVM::ModuleTranslation &mt, llvm::IRBuilderBase &builder) {
...
aiir::NVVM::IDArgPair MBarrierArriveDropExpectTxOp::getIntrinsicIDAndArgs(
    Operation &op, LLVM::ModuleTranslation &mt, llvm::IRBuilderBase &builder) {
...
aiir::NVVM::IDArgPair MBarrierArriveNocompleteOp::getIntrinsicIDAndArgs(
    Operation &op, LLVM::ModuleTranslation &mt, llvm::IRBuilderBase &builder) {
...
aiir::NVVM::IDArgPair MBarrierArriveDropNocompleteOp::getIntrinsicIDAndArgs(
    Operation &op, LLVM::ModuleTranslation &mt, llvm::IRBuilderBase &builder) {
...
aiir::NVVM::IDArgPair MBarrierTestWaitOp::getIntrinsicIDAndArgs(
    Operation &op, LLVM::ModuleTranslation &mt, llvm::IRBuilderBase &builder) {
...
aiir::NVVM::IDArgPair MBarrierTryWaitOp::getIntrinsicIDAndArgs(
    Operation &op, LLVM::ModuleTranslation &mt, llvm::IRBuilderBase &builder) {
...
aiir::NVVM::IDArgPair CpAsyncMBarrierArriveOp::getIntrinsicIDAndArgs(
    Operation &op, LLVM::ModuleTranslation &mt, llvm::IRBuilderBase &builder) {
...
aiir::NVVM::IDArgPair CpAsyncBulkPrefetchOp::getIntrinsicIDAndArgs(
    Operation &op, LLVM::ModuleTranslation &mt, llvm::IRBuilderBase &builder) {
  ...
  aiir::Value cacheHint = thisOp.getL2CacheHint();
...
aiir::NVVM::IDArgPair CpAsyncBulkGlobalToSharedClusterOp::getIntrinsicIDAndArgs(
    Operation &op, LLVM::ModuleTranslation &mt, llvm::IRBuilderBase &builder) {
  ...
  aiir::Value multicastMask = thisOp.getMulticastMask();
...
aiir::NVVM::IDArgPair CpAsyncBulkSharedCTAToGlobalOp::getIntrinsicIDAndArgs(
    Operation &op, LLVM::ModuleTranslation &mt, llvm::IRBuilderBase &builder) {
  ...
  if (aiir::Value byteMask = thisOp.getByteMask()) {
...
CpAsyncBulkTensorGlobalToSharedClusterOp::getIntrinsicIDAndArgs(
  ...
  for (aiir::Value v : thisOp.getCoordinates())
    args.push_back(mt.lookupValue(v));
  for (aiir::Value v : thisOp.getIm2colOffsets())
    args.push_back(mt.lookupValue(v));

  // MulticastMask, if available
  aiir::Value mcMask = thisOp.getMulticastMask();
...
aiir::NVVM::IDArgPair CpAsyncBulkTensorPrefetchOp::getIntrinsicIDAndArgs(
    Operation &op, LLVM::ModuleTranslation &mt, llvm::IRBuilderBase &builder) {
...
llvm::Intrinsic::ID ConvertF32x2ToF6x2Op::getIntrinsicID(aiir::Type dstTy,
                                                         bool hasRelu) {
  return llvm::TypeSwitch<aiir::Type, llvm::Intrinsic::ID>(dstTy)
      .Case([&](aiir::Float6E2M3FNType) {
        return GET_F32x2_TO_F6x2_ID(e2m3x2, hasRelu);
      })
      .Case([&](aiir::Float6E3M2FNType) {
...
llvm::Intrinsic::ID
ConvertF32x2ToF8x2Op::getIntrinsicID(aiir::Type dstTy, NVVM::FPRoundingMode rnd,
                                     NVVM::SaturationMode sat, bool hasRelu) {
  bool hasSatFinite = (sat == NVVM::SaturationMode::SATFINITE);
...
llvm::Intrinsic::ID ConvertF16x2ToF8x2Op::getIntrinsicID(aiir::Type dstTy,
                                                         bool hasRelu) {
  return llvm::TypeSwitch<aiir::Type, llvm::Intrinsic::ID>(dstTy)
      .Case([&](aiir::Float8E4M3FNType) {
...
NVVM::IDArgPair ConvertF8x2ToF16x2Op::getIntrinsicIDAndArgs(
  ...
  llvm::Intrinsic::ID intId =
      llvm::TypeSwitch<aiir::Type, llvm::Intrinsic::ID>(curOp.getSrcType())
...
static void nvvmInferResultRanges(Operation *op, Value result,
                                  ArrayRef<::aiir::ConstantIntRanges> argRanges,
...
bool NVVM::InlinePtxOp::getAsmValues(
  ...
  for (auto arg : getReadWriteArgs())
    asmValues.push_back({arg, aiir::NVVM::PTXRegisterMod::ReadWrite});
  for (auto arg : getResults())
    asmValues.push_back({arg, aiir::NVVM::PTXRegisterMod::Write});
  for (auto arg : getReadOnlyArgs())
    asmValues.push_back({arg, aiir::NVVM::PTXRegisterMod::Read});
  if (getPredicate())
    asmValues.push_back({getPredicate(), aiir::NVVM::PTXRegisterMod::Read});
...
aiir::NVVM::IDArgPair TensormapReplaceOp::getIntrinsicIDAndArgs(
    Operation &op, LLVM::ModuleTranslation &mt, llvm::IRBuilderBase &builder) {
  ...
  if (auto attr = thisOp.getNewValueAttr()) {
    auto val =
        llvm::TypeSwitch<aiir::Attribute, unsigned>(*attr)
...
static LogicalResult
verifyTcgen05MMAOp(bool isATensor, aiir::Value disableOutputLane,
                   NVVM::CTAGroupKind ctaGroup, bool hasAShift,
                   NVVM::Tcgen05MMACollectorOp collectorOp, Location loc) {

  if (disableOutputLane) {
    aiir::VectorType disableOutputLaneType =
        cast<aiir::VectorType>(disableOutputLane.getType());
...
aiir::NVVM::IDArgPair Tcgen05MMASparseOp::getIntrinsicIDAndArgs(
    Operation &op, LLVM::ModuleTranslation &mt, llvm::IRBuilderBase &builder) {
...
aiir::NVVM::IDArgPair Tcgen05MMABlockScaleOp::getIntrinsicIDAndArgs(
    Operation &op, LLVM::ModuleTranslation &mt, llvm::IRBuilderBase &builder) {
...
aiir::NVVM::IDArgPair Tcgen05MMASparseBlockScaleOp::getIntrinsicIDAndArgs(
    Operation &op, LLVM::ModuleTranslation &mt, llvm::IRBuilderBase &builder) {
...
aiir::NVVM::IDArgPair Tcgen05MMAWsOp::getIntrinsicIDAndArgs(
    Operation &op, LLVM::ModuleTranslation &mt, llvm::IRBuilderBase &builder) {
  ...
  aiir::Value ZeroColMask = thisOp.getZeroColMask();
...
aiir::NVVM::IDArgPair Tcgen05MMAWsSparseOp::getIntrinsicIDAndArgs(
    Operation &op, LLVM::ModuleTranslation &mt, llvm::IRBuilderBase &builder) {
...
aiir::NVVM::IDArgPair NVVM::Tcgen05LdRedOp::getIntrinsicIDAndArgs(
    Operation &op, LLVM::ModuleTranslation &mt, llvm::IRBuilderBase &builder) {
  auto thisOp = cast<NVVM::Tcgen05LdRedOp>(op);
  llvm::SmallVector<llvm::Value *> args;

  aiir::VectorType VecResTy =
      cast<aiir::VectorType>(thisOp.getData().getType());
...
void NVVMDialect::initialize() {
  addOperations<
#define GET_OP_LIST
#include "aiir/Dialect/LLVMIR/NVVMOps.cpp.inc"
      >();
  addAttributes<
#define GET_ATTRDEF_LIST
#include "aiir/Dialect/LLVMIR/NVVMOpsAttributes.cpp.inc"
      >();
...
NVVMTargetAttr::verify(function_ref<InFlightDiagnostic()> emitError,
  ...
  if (files && !llvm::all_of(files, [](::aiir::Attribute attr) {
        return aiir::isa_and_nonnull<StringAttr>(attr);
      })) {
