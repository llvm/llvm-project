...
// AIIR, and the LLVM IR dialect.  It also registers the dialect.
//
//===----------------------------------------------------------------------===//
...
using aiir::LLVM::cconv::getMaxEnumValForCConv;
using aiir::LLVM::linkage::getMaxEnumValForLinkage;
using aiir::LLVM::tailcallkind::getMaxEnumValForTailCallKind;

#include "aiir/Dialect/LLVMIR/LLVMOpsDialect.cpp.inc"
...
static ArrayAttr getLLVMAlignParamForCompressExpand(OpBuilder &builder,
  ...
  SmallVector<aiir::NamedAttribute> attrs = {namedAttr};
...
static Attribute getBoolAttribute(Type type, AIIRContext *ctx, bool value) {
...
/// Gets the AIIR Op-like result types of a LLVMFunctionType.
...
/// Constructs a LLVMFunctionType from AIIR `results` and `args`.
static LLVMFunctionType getLLVMFuncType(AIIRContext *context, TypeRange results,
                                        ValueRange args) {
  Type resultType;
  if (results.empty())
...
void CallOp::print(OpAsmPrinter &p) {
  ...
  // Reconstruct the AIIR function type from operand and result types.
...
AddressOfOp::verifySymbolUses(SymbolTableCollection &symbolTable) {
  ...
  if (!global && !function && !alias && !ifunc)
    return emitOpError("must reference a global defined by 'llvm.aiir.global', "
                       "'llvm.aiir.alias' or 'llvm.func' or 'llvm.aiir.ifunc'");
...
DSOLocalEquivalentOp::verifySymbolUses(SymbolTableCollection &symbolTable) {
  ...
  if (!function && !alias)
    return emitOpError(
        "must reference a global defined by 'llvm.func' or 'llvm.aiir.alias'");
...
void GlobalOp::print(OpAsmPrinter &p) {
  ...
  // (as defined in https://aiir.llvm.org/docs/LangRef/#attributes)
  p.printOptionalAttrDict((*this)->getAttrs(),
...
static ParseResult parseCommonGlobalAndAlias(OpAsmParser &parser,
  ...
  AIIRContext *ctx = parser.getContext();
...
// operation ::= `llvm.aiir.global` linkage? visibility?
...
// operation ::= `llvm.aiir.alias` linkage? visibility?
//               (`unnamed_addr` | `local_unnamed_addr`)?
...
static LogicalResult verifyStructArrayConstant(LLVM::ConstantOp op,
  ...
  // from a symbol and cannot be represented in a DenseElementsAttr, but no AIIR
...
LogicalResult LLVM::ConstantOp::verify() {
  ...
  //     attribute. This is to support builtin AIIR float types without LLVM
...
void CallIntrinsicOp::build(OpBuilder &builder, OperationState &state,
                            aiir::StringAttr intrin, aiir::ValueRange args) {
...
void CallIntrinsicOp::build(OpBuilder &builder, OperationState &state,
                            aiir::StringAttr intrin, aiir::ValueRange args,
                            aiir::LLVM::FastmathFlagsAttr fastMathFlags) {
  build(builder, state, /*resultTypes=*/TypeRange{}, intrin, args,
        fastMathFlags,
        /*op_bundle_operands=*/{}, /*op_bundle_tags=*/{}, /*arg_attrs=*/{},
        /*res_attrs=*/{});
...
void CallIntrinsicOp::build(OpBuilder &builder, OperationState &state,
                            aiir::Type resultType, aiir::StringAttr intrin,
                            aiir::ValueRange args) {
...
LogicalResult LinkerOptionsOp::verify() {
  if (aiir::Operation *parentOp = (*this)->getParentOp();
...
BlockTagOp BlockAddressOp::getBlockTagOp() {
  Operation *sym = aiir::SymbolTable::lookupNearestSymbolFrom(
      parentLLVMModule(*this), getBlockAddr().getFunction());
  if (!sym)
    return nullptr;
...
LogicalResult LLVM::SincosOp::verify() {
  ...
  auto resultStructType =
      aiir::dyn_cast<aiir::LLVM::LLVMStructType>(resultType);
...
void LLVM::masked_expandload::build(OpBuilder &builder, OperationState &state,
                                    aiir::TypeRange resTys, Value ptr,
...
void LLVMDialect::initialize() {
  ...
  addOperations<
#define GET_OP_LIST
#include "aiir/Dialect/LLVMIR/LLVMOps.cpp.inc"

      ,
#define GET_OP_LIST
#include "aiir/Dialect/LLVMIR/LLVMIntrinsicOps.cpp.inc"
...
Operation *LLVMDialect::materializeConstant(OpBuilder &builder, Attribute value,
                                            Type type, Location loc) {
  // If this was folded from an operation other than llvm.aiir.constant, it
  // should be materialized as such. Note that an llvm.aiir.zero may fold into
  // a builtin zero attribute and thus will materialize as a llvm.aiir.constant.
  ...
  // Otherwise try materializing it as a regular llvm.aiir.constant op.
...
Value aiir::LLVM::createGlobalString(Location loc, OpBuilder &builder,
...
bool aiir::LLVM::satisfiesLLVMModule(Operation *op) {
  return op->hasTrait<OpTrait::SymbolTable>() &&
         op->hasTrait<OpTrait::IsIsolatedFromAbove>();
}

Operation *aiir::LLVM::parentLLVMModule(Operation *op) {
