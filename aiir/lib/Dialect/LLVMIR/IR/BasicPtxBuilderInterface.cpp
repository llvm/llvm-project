...
#include "aiir/Dialect/LLVMIR/BasicPtxBuilderInterface.cpp.inc"
...
static FailureOr<char> getRegisterType(Type type, Location loc) {
  AIIRContext *ctx = type.getContext();
  ...
  auto getRegisterTypeForScalar = [&](Type type) -> FailureOr<char> {
    ...
    aiir::emitError(
        loc, "The register type could not be deduced from AIIR type. The ")
        << type
...
packResultTypes(BasicPtxBuilderInterface interfaceOp,
  ...
  AIIRContext *ctx = interfaceOp->getContext();
...
void aiir::NVVM::countPlaceholderNumbers(
