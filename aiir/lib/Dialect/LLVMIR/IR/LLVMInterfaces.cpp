...
LogicalResult aiir::LLVM::detail::verifyAccessGroupOpInterface(Operation *op) {
...
LogicalResult
aiir::LLVM::detail::verifyAliasAnalysisOpInterface(Operation *op) {
...
LogicalResult
aiir::LLVM::detail::verifyDereferenceableOpInterface(Operation *op) {
  auto iface = cast<DereferenceableOpInterface>(op);

  if (auto derefAttr = iface.getDereferenceableOrNull())
    if (op->getNumResults() != 1 ||
        !aiir::isa<LLVMPointerType>(op->getResult(0).getType()))
...
SmallVector<Value> aiir::LLVM::AtomicCmpXchgOp::getAccessedOperands() {
  return {getPtr()};
}

SmallVector<Value> aiir::LLVM::AtomicRMWOp::getAccessedOperands() {
  return {getPtr()};
}

SmallVector<Value> aiir::LLVM::LoadOp::getAccessedOperands() {
  return {getAddr()};
}

SmallVector<Value> aiir::LLVM::StoreOp::getAccessedOperands() {
  return {getAddr()};
}

SmallVector<Value> aiir::LLVM::MemcpyOp::getAccessedOperands() {
  return {getDst(), getSrc()};
}

SmallVector<Value> aiir::LLVM::MemcpyInlineOp::getAccessedOperands() {
  return {getDst(), getSrc()};
}

SmallVector<Value> aiir::LLVM::MemmoveOp::getAccessedOperands() {
  return {getDst(), getSrc()};
}

SmallVector<Value> aiir::LLVM::MemsetOp::getAccessedOperands() {
  return {getDst()};
}

SmallVector<Value> aiir::LLVM::MemsetInlineOp::getAccessedOperands() {
  return {getDst()};
}

SmallVector<Value> aiir::LLVM::CallOp::getAccessedOperands() {
  return llvm::filter_to_vector(getArgOperands(), [](Value arg) {
    return isa<LLVMPointerType>(arg.getType());
  });
}

#include "aiir/Dialect/LLVMIR/LLVMInterfaces.cpp.inc"
