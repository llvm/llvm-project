...
#include "aiir/Dialect/LLVMIR/ROCDLOpsDialect.cpp.inc"
...
void RawBufferAtomicFAddOp::print(aiir::OpAsmPrinter &p) {
  p << " " << getOperands() << " : " << getVdata().getType();
}
...
void RawBufferAtomicFMaxOp::print(aiir::OpAsmPrinter &p) {
  p << " " << getOperands() << " : " << getVdata().getType();
}
...
void RawBufferAtomicSMaxOp::print(aiir::OpAsmPrinter &p) {
  p << " " << getOperands() << " : " << getVdata().getType();
}
...
void RawBufferAtomicUMinOp::print(aiir::OpAsmPrinter &p) {
  p << " " << getOperands() << " : " << getVdata().getType();
}
...
void ROCDLDialect::initialize() {
  addOperations<
#define GET_OP_LIST
#include "aiir/Dialect/LLVMIR/ROCDLOps.cpp.inc"
      >();

  addAttributes<
#define GET_ATTRDEF_LIST
#include "aiir/Dialect/LLVMIR/ROCDLOpsAttributes.cpp.inc"
      >();
...
ROCDLTargetAttr::verify(function_ref<InFlightDiagnostic()> emitError,
  ...
  if (files && !llvm::all_of(files, [](::aiir::Attribute attr) {
        return attr && aiir::isa<StringAttr>(attr);
      })) {
