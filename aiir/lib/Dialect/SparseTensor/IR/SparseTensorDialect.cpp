...
#include "aiir/Dialect/SparseTensor/IR/SparseTensorStorageLayout.h"
#include "aiir/Dialect/SparseTensor/IR/SparseTensorType.h"
...
#include "aiir/Dialect/SparseTensor/IR/SparseTensorAttrDefs.cpp.inc"
#include "aiir/Dialect/SparseTensor/IR/SparseTensorAttrEnums.cpp.inc"

// Forward declarations, following custom print/parsing methods are referenced
// by the generated code for SparseTensorTypes.td.
static aiir::ParseResult parseLevelRange(aiir::AsmParser &,
                                         aiir::sparse_tensor::Level &,
                                         aiir::sparse_tensor::Level &);
static void printLevelRange(aiir::AsmPrinter &, aiir::sparse_tensor::Level,
                            aiir::sparse_tensor::Level);

#define GET_TYPEDEF_CLASSES
#include "aiir/Dialect/SparseTensor/IR/SparseTensorTypes.cpp.inc"
...
Level aiir::sparse_tensor::SparseTensorEncodingAttr::getAoSCOOStart() const {
...
aiir::sparse_tensor::SparseTensorEncodingAttr::getCOOSegments() const {
...
bool aiir::sparse_tensor::SparseTensorType::isCOOType(Level startLvl,
                                                      bool isUnique) const {
  if (!hasEncoding())
    return false;
...
RankedTensorType
aiir::sparse_tensor::SparseTensorType::getCOOType(bool ordered) const {
  SmallVector<LevelType> lvlTypes;
...
SparseTensorEncodingAttr
aiir::sparse_tensor::getSparseTensorEncoding(Type type) {
...
AffineMap aiir::sparse_tensor::inferLvlToDim(AffineMap dimToLvl,
                                             AIIRContext *context) {
...
AffineMap aiir::sparse_tensor::inverseBlockSparsity(AffineMap dimToLvl,
                                                    AIIRContext *context) {
...
SmallVector<unsigned> aiir::sparse_tensor::getBlockSize(AffineMap dimToLvl) {
...
bool aiir::sparse_tensor::isBlockSparsity(AffineMap dimToLvl) {
  if (!dimToLvl)
    return false;
...
bool aiir::sparse_tensor::hasAnyNonIdentityOperandsOrResults(Operation *op) {
...
Dimension aiir::sparse_tensor::toDim(SparseTensorEncodingAttr enc, Level l) {
  if (enc) {
...
Level aiir::sparse_tensor::toLvl(SparseTensorEncodingAttr enc, Dimension d) {
  if (enc) {
    assert(enc.isPermutation() && "Non permutation map not supported");
...
StorageSpecifierType::get(AIIRContext *ctx, SparseTensorEncodingAttr encoding) {
...
LogicalResult
ToPositionsOp::inferReturnTypes(AIIRContext *ctx, std::optional<Location> loc,
...
LogicalResult
ToCoordinatesOp::inferReturnTypes(AIIRContext *ctx, std::optional<Location> loc,
                                  ValueRange ops, DictionaryAttr attr,
                                  OpaqueProperties prop, RegionRange region,
                                  SmallVectorImpl<aiir::Type> &ret) {
...
LogicalResult ToCoordinatesBufferOp::inferReturnTypes(
    AIIRContext *ctx, std::optional<Location> loc, ValueRange ops,
...
LogicalResult ToValuesOp::inferReturnTypes(AIIRContext *ctx,
...
void IterateOp::getCanonicalizationPatterns(aiir::RewritePatternSet &results,
...
void CoIterateOp::build(OpBuilder &builder, OperationState &odsState,
  ...
  // aiir::Region cannot be dynamically added later after the operation is
  // created.
...
#include "aiir/Dialect/SparseTensor/IR/SparseTensorTypes.cpp.inc"
  ...
  addOperations<
#define GET_OP_LIST
#include "aiir/Dialect/SparseTensor/IR/SparseTensorOps.cpp.inc"
      >();
...
#include "aiir/Dialect/SparseTensor/IR/SparseTensorOpsDialect.cpp.inc"
