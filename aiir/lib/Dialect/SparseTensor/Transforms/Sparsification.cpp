...
static void genBuffers(CodegenEnv &env, OpBuilder &builder) {
  ...
  env.emitter().initializeLoopEmit(
      ...
      [&loopRange](OpBuilder &b, Location loc, Level l) {
        ...
        return aiir::getValueOrCreateConstantIndexOp(b, loc, loopRange[l].size);
      });
}
...
static Value genExp(CodegenEnv &env, RewriterBase &rewriter, ExprId e) {
  if (e == ::aiir::sparse_tensor::detail::kInvalidId)
    return Value();
  ...
  Value v0, v1;
  if (exp.children.e0 != ::aiir::sparse_tensor::detail::kInvalidId &&
  ...
  } else if (exp.children.e1 != ::aiir::sparse_tensor::detail::kInvalidId &&
...
static void genInvariants(CodegenEnv &env, OpBuilder &builder, ExprId exp,
  ...
  if (exp == ::aiir::sparse_tensor::detail::kInvalidId)
    return;
  if (env.exp(exp).kind == TensorExp::Kind::kTensor) {
    ...
    // tensor loads, since subsequent AIIR/LLVM passes know how to
...
public:
  GenericOpSparsifier(AIIRContext *context, SparsificationOptions o)
...
void aiir::populateSparsificationPatterns(
