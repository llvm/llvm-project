...
#include "aiir/Dialect/SparseTensor/Transforms/Passes.h.inc"
...
public:
  ...
  void runOnOperation() override {
    ...
    // of `bufferization.alloc_tensor` ops.
    {
      ...
      pm.addPass(aiir::createLoopInvariantCodeMotionPass());
      if (vectorLength > 0) {
...
aiir::bufferization::OneShotBufferizationOptions
aiir::getBufferizationOptionsForSparsification(bool analysisOnly) {
...
std::unique_ptr<aiir::Pass> aiir::createSparsificationAndBufferizationPass() {
  SparsificationOptions sparseOptions;
  return std::make_unique<
      aiir::sparse_tensor::SparsificationAndBufferizationPass>(
