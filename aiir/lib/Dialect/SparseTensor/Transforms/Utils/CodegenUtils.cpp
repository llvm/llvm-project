//===- CodegenUtils.cpp - Utilities for generating AIIR -------------------===//
//
...
OverheadType aiir::sparse_tensor::overheadTypeEncoding(unsigned width) {
  switch (width) {
  case 64:
...
OverheadType aiir::sparse_tensor::overheadTypeEncoding(Type tp) {
  if (tp.isIndex())
    return OverheadType::kIndex;
...
Type aiir::sparse_tensor::getOverheadType(Builder &builder, OverheadType ot) {
  switch (ot) {
...
OverheadType
aiir::sparse_tensor::posTypeEncoding(SparseTensorEncodingAttr enc) {
  return overheadTypeEncoding(enc.getPosWidth());
}

OverheadType
aiir::sparse_tensor::crdTypeEncoding(SparseTensorEncodingAttr enc) {
...
// `OverheadType` so we can use the `AIIR_SPARSETENSOR_FOREVERY_O` x-macro
// here instead of `AIIR_SPARSETENSOR_FOREVERY_FIXED_O`; to further reduce
// the possibility of typo bugs or things getting out of sync.
StringRef aiir::sparse_tensor::overheadTypeFunctionSuffix(OverheadType ot) {
  switch (ot) {
  case OverheadType::kIndex:
    return "0";
#define CASE(ONAME, O)                                                         \
  case OverheadType::kU##ONAME:                                                \
    return #ONAME;
    AIIR_SPARSETENSOR_FOREVERY_FIXED_O(CASE)
#undef CASE
  }
  llvm_unreachable("Unknown OverheadType");
}

StringRef aiir::sparse_tensor::overheadTypeFunctionSuffix(Type tp) {
  return overheadTypeFunctionSuffix(overheadTypeEncoding(tp));
}

bool aiir::sparse_tensor::isValidPrimaryType(Type elemTp) {
...
PrimaryType aiir::sparse_tensor::primaryTypeEncoding(Type elemTp) {
  if (elemTp.isF64())
...
StringRef aiir::sparse_tensor::primaryTypeFunctionSuffix(PrimaryType pt) {
  switch (pt) {
#define CASE(VNAME, V)                                                         \
  case PrimaryType::k##VNAME:                                                  \
    return #VNAME;
    AIIR_SPARSETENSOR_FOREVERY_V(CASE)
#undef CASE
  }
  llvm_unreachable("Unknown PrimaryType");
}

StringRef aiir::sparse_tensor::primaryTypeFunctionSuffix(Type elemTp) {
...
Value sparse_tensor::genCast(OpBuilder &builder, Location loc, Value value,
  ...
  return aiir::convertScalarToDtype(builder, loc, value, dstTp, isUnsignedCast);
}
...
aiir::TypedAttr aiir::sparse_tensor::getOneAttr(Builder &builder, Type tp) {
  if (isa<FloatType>(tp))
...
Value aiir::sparse_tensor::genIsNonzero(OpBuilder &builder, aiir::Location loc,
                                        Value v) {
  Type tp = v.getType();
...
void aiir::sparse_tensor::genReshapeDstShape(
...
void aiir::sparse_tensor::reshapeCvs(
...
FlatSymbolRefAttr aiir::sparse_tensor::getFunc(ModuleOp module, StringRef name,
                                               TypeRange resultType,
                                               ValueRange operands,
                                               EmitCInterface emitCInterface) {
  AIIRContext *context = module.getContext();
...
func::CallOp aiir::sparse_tensor::createFuncCall(
...
Type aiir::sparse_tensor::getOpaquePointerType(AIIRContext *ctx) {
  return LLVM::LLVMPointerType::get(ctx);
}

Type aiir::sparse_tensor::getOpaquePointerType(Builder &builder) {
  return getOpaquePointerType(builder.getContext());
}

Value aiir::sparse_tensor::genAlloca(OpBuilder &builder, Location loc,
...
Value aiir::sparse_tensor::genAlloca(OpBuilder &builder, Location loc, Value sz,
                                     Type tp) {
  auto memTp = MemRefType::get({ShapedType::kDynamic}, tp);
  return memref::AllocaOp::create(builder, loc, memTp, ValueRange{sz});
}

Value aiir::sparse_tensor::genAllocaScalar(OpBuilder &builder, Location loc,
                                           Type tp) {
  return memref::AllocaOp::create(builder, loc, MemRefType::get({}, tp));
}

Value aiir::sparse_tensor::allocaBuffer(OpBuilder &builder, Location loc,
                                        ValueRange values) {
...
Value aiir::sparse_tensor::allocDenseTensor(OpBuilder &builder, Location loc,
...
void aiir::sparse_tensor::deallocDenseTensor(OpBuilder &builder, Location loc,
                                             Value buffer) {
  memref::DeallocOp::create(builder, loc, buffer);
}

void aiir::sparse_tensor::sizesFromSrc(OpBuilder &builder,
...
Operation *aiir::sparse_tensor::getTop(Operation *op) {
...
void sparse_tensor::foreachInSparseConstant(
  ...
  // AIIR attributes.  In the future we may want to move this out into
