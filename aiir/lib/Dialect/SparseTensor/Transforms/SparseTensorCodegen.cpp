...
public:
  ...
  std::string getMangledFuncName() {
    ...
    for (Level l = 0; l < lvlRank; l++) {
      std::string lvlType = toAIIRString(stt.getLvlType(l));
...
public:
  ...
  SparseTensorAllocConverter(const TypeConverter &typeConverter,
                             AIIRContext *context, bool enableInit)
...
public:
  ...
  SparseTensorDeallocConverter(const TypeConverter &typeConverter,
                               AIIRContext *context, bool createDeallocs)
      : OpConversionPattern(typeConverter, context),
...
void aiir::populateSparseTensorCodegenPatterns(
