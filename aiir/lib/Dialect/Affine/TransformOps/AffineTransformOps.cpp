...
struct SimplifyAffineMinMaxOp : public OpRewritePattern<OpTy> {
  ...
  SimplifyAffineMinMaxOp(AIIRContext *ctx,
...
SimplifyMinMaxAffineOpsOp::apply(transform::TransformRewriter &rewriter,
  ...
  bool modified = false;
  if (failed(aiir::affine::simplifyAffineMinMaxOps(rewriter, targets,
                                                   &modified))) {
...
public:
  AIIR_DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID(AffineTransformDialectExtension)
...
#define GET_OP_LIST
#include "aiir/Dialect/Affine/TransformOps/AffineTransformOps.cpp.inc"
        >();
  }
};
} // namespace
...
void aiir::affine::registerTransformDialectExtension(
