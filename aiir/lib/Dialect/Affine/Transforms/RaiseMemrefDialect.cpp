  ...
  if (llvm::isa_and_nonnull<arith::AddIOp>(definingOp) ||
    ...
    if (lhsE && rhsE) {
      ...
      if (isa<arith::AddIOp>(definingOp)) {
        kind = aiir::AffineExprKind::Add;
      } else {
        kind = aiir::AffineExprKind::Mul;
...
computeAffineMapAndArgs(AIIRContext *ctx, ValueRange indices, AffineMap &map,
...
aiir::affine::createRaiseMemrefToAffine() {
