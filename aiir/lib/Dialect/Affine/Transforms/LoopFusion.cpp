...
static bool isEscapingMemref(Value memref, Block *block) {
  ...
  if (auto viewOp = dyn_cast<aiir::ViewLikeOpInterface>(defOp))
  ...
  if (!hasSingleEffect<aiir::MemoryEffects::Allocate>(defOp, memref))
    return true;
...
static Value createPrivateMemRef(AffineForOp forOp,
  ...
  for (unsigned j = 0, e = lbs[0].getNumSymbols(); j < e; ++j)
    replacements.push_back(aiir::getAffineDimExpr(j, forOp.getContext()));
...
std::unique_ptr<Pass> aiir::affine::createLoopFusionPass(
