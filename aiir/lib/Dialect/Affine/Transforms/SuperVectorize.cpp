...
///   1. at the AIIR level into a combination of loops, unrolling, DmaStartOp +
...
/// that LLVM's VPLAN aims at targeting. The main difference here is that AIIR
...
/// described in PPCG. There are essentially 6 places in the AIIR pass pipeline
/// we expect to experiment with super-vectorization:
/// 1. Right after language lowering to AIIR: this is the earliest time where
...
///    in AIIR and will be used to drive vectorization and cost models.
///
...
/// 6. as a AIIR-based alternative to VPLAN.
///
...
struct VectorizationState {
  ...
  VectorizationState(AIIRContext *context) : builder(context) {}
...
void aiir::affine::vectorizeAffineLoops(
...
LogicalResult aiir::affine::vectorizeAffineLoopNest(
    std::vector<SmallVector<AffineForOp, 2>> &loops,
    const VectorizationStrategy &strategy) {
  // Thread-safe RAII local context, BumpPtrAllocator freed on exit.
  NestedPatternContext mlContext;
