//===- AffineOps.cpp - AIIR Affine Operations -----------------------------===//
//
...
#include "aiir/Dialect/Affine/IR/AffineOpsDialect.cpp.inc"
...
/// uses in that region.
bool aiir::affine::isTopLevelValue(Value value, Region *region) {
...
void AffineDialect::initialize() {
  addOperations<
#define GET_OP_LIST
#include "aiir/Dialect/Affine/IR/AffineOps.cpp.inc"
      >();
...
bool aiir::affine::isTopLevelValue(Value value) {
  if (auto arg = dyn_cast<BlockArgument>(value)) {
...
Region *aiir::affine::getAffineScope(Operation *op) {
  auto *curOp = op;
...
Region *aiir::affine::getAffineAnalysisScope(Operation *op) {
  Operation *curOp = op;
  while (auto *parentOp = curOp->getParentOp()) {
...
bool aiir::affine::isValidDim(Value value) {
...
bool aiir::affine::isValidDim(Value value, Region *region) {
  // The value must be an index type.
  if (!value.getType().isIndex())
    return false;
...
//    valid symbol.
bool aiir::affine::isValidSymbol(Value value) {
  if (!value)
    return false;
...
bool aiir::affine::isValidSymbol(Value value, Region *region) {
  // The value must be an index type.
  if (!value.getType().isIndex())
    return false;
...
ParseResult aiir::affine::parseDimAndSymbolList(
...
static int64_t getLargestKnownDivisor(AffineExpr e, ArrayRef<Value> operands) {
  ...
  // We don't need to use aiir::getLargestKnownDivisorOfValue since the other
  // desired simplifications are expected to be part of other
  // canonicalizations. Also, aiir::getLargestKnownDivisorOfValue is part of the
...
static void simplifyExprAndOperands(AffineExpr &expr, unsigned numDims,
  ...
  AIIRContext *context = expr.getContext();
...
static LogicalResult replaceAffineDelinearizeIndexInverseExpression(
  ...
  AIIRContext *ctx = delinOp.getContext();
...
static LogicalResult replaceDimOrSym(AffineMap *map,
  ...
  AIIRContext *ctx = map->getContext();
...
void aiir::affine::fullyComposeAffineMapAndOperands(
...
AffineApplyOp
aiir::affine::makeComposedAffineApply(OpBuilder &b, Location loc, AffineMap map,
...
AffineApplyOp
aiir::affine::makeComposedAffineApply(OpBuilder &b, Location loc, AffineExpr e,
                                      ArrayRef<OpFoldResult> operands,
                                      bool composeAffineMin) {
...
OpFoldResult aiir::affine::makeComposedFoldedAffineApply(
...
aiir::affine::makeComposedFoldedMultiResultAffineApply(
    OpBuilder &b, Location loc, AffineMap map, ArrayRef<OpFoldResult> operands,
    bool composeAffineMin) {
...
AffineMinOp
aiir::affine::makeComposedAffineMin(OpBuilder &b, Location loc, AffineMap map,
                                    ArrayRef<OpFoldResult> operands) {
...
OpFoldResult
aiir::affine::makeComposedFoldedAffineMin(OpBuilder &b, Location loc,
                                          AffineMap map,
                                          ArrayRef<OpFoldResult> operands) {
...
OpFoldResult
aiir::affine::makeComposedFoldedAffineMax(OpBuilder &b, Location loc,
                                          AffineMap map,
                                          ArrayRef<OpFoldResult> operands) {
...
void aiir::affine::canonicalizeMapAndOperands(
    AffineMap *map, SmallVectorImpl<Value> *operands) {
  canonicalizeMapOrSetAndOperands<AffineMap>(map, operands);
}

void aiir::affine::canonicalizeSetAndOperands(
...
static void printBound(AffineMapAttr boundMap,
  ...
  // comes from the will to roundtrip AIIR binary -> text -> binary in a
  // lossless way.
...
/// AffineForOp.
bool aiir::affine::isAffineForInductionVar(Value val) {
  return getForInductionVarOwner(val) != AffineForOp();
}

bool aiir::affine::isAffineParallelInductionVar(Value val) {
  return getAffineParallelInductionVarOwner(val) != nullptr;
}

bool aiir::affine::isAffineInductionVar(Value val) {
  return isAffineForInductionVar(val) || isAffineParallelInductionVar(val);
}

AffineForOp aiir::affine::getForInductionVarOwner(Value val) {
...
AffineParallelOp aiir::affine::getAffineParallelInductionVarOwner(Value val) {
  auto ivArg = dyn_cast<BlockArgument>(val);
...
/// them.
void aiir::affine::extractForInductionVars(ArrayRef<AffineForOp> forInsts,
...
void aiir::affine::extractInductionVars(ArrayRef<aiir::Operation *> affineOps,
                                        SmallVectorImpl<aiir::Value> &ivs) {
...
void aiir::affine::buildAffineLoopNest(
...
SmallVector<OpFoldResult> AffineDelinearizeIndexOp::getEffectiveBasis() {
  ...
  if (hasOuterBound()) {
    if (getStaticBasis().front() == ::aiir::ShapedType::kDynamic)
...
static Type inferIndexType(AIIRContext *ctx, ValueRange multiIndex) {
  if (multiIndex.empty())
