...
void aiir::affine::getAffineForIVs(Operation &op,
...
void aiir::affine::getEnclosingAffineOps(Operation &op,
...
std::optional<int64_t> MemRefRegion::getConstantBoundingSizeAndShape(
  ...
  AIIRContext *context = memref.getContext();
...
aiir::affine::getMemRefIntOrFloatEltSizeInBytes(MemRefType memRefType) {
...
aiir::affine::getIntOrFloatMemRefSizeInBytes(MemRefType memRefType) {
...
LogicalResult aiir::affine::boundCheckLoadOrStoreOp(LoadOrStoreOp loadOrStoreOp,
                                                    bool emitError) {
...
aiir::affine::boundCheckLoadOrStoreOp(AffineReadOpInterface loadOp,
                                      bool emitError);
template LogicalResult
aiir::affine::boundCheckLoadOrStoreOp(AffineWriteOpInterface storeOp,
                                      bool emitError);
...
unsigned aiir::affine::getInnermostCommonLoopDepth(
...
aiir::affine::computeSliceUnion(ArrayRef<Operation *> opsA,
...
bool aiir::affine::buildSliceTripCountMap(
...
uint64_t aiir::affine::getSliceIterationCount(
...
void aiir::affine::getComputationSliceState(
...
// solution.
AffineForOp aiir::affine::insertBackwardComputationSlice(
...
unsigned aiir::affine::getNestingDepth(Operation *op) {
  Operation *currOp = op;
  unsigned depth = 0;
...
void aiir::affine::getAffineIVs(Operation &op, SmallVectorImpl<Value> &ivs) {
  auto *currOp = op.getParentOp();
  AffineForOp currAffineForOp;
...
unsigned aiir::affine::getNumCommonSurroundingLoops(Operation &a,
                                                    Operation &b) {
...
std::optional<int64_t> aiir::affine::getMemoryFootprintBytes(AffineForOp forOp,
                                                             int memorySpace) {
...
bool aiir::affine::isLoopParallelAndContainsReduction(AffineForOp forOp) {
...
/// at 'forOp'.
void aiir::affine::getSequentialLoops(
...
IntegerSet aiir::affine::simplifyIntegerSet(IntegerSet set) {
...
//      0 |     0 |       1 |  -1 |             0 |    -1 |              >= 0
//
FailureOr<AffineValueMap> aiir::affine::simplifyConstrainedMinMaxOp(
...
Block *aiir::affine::findInnermostCommonBlockInScope(Operation *a,
                                                     Operation *b) {
