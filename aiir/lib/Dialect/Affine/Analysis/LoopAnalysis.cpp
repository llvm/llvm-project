...
/// A directed graph to model relationships between AIIR Operations.
...
private:
  /// A node of a directed graph between AIIR Operations to model various
...
/// count expressions.
void aiir::affine::getTripCountMapAndOperands(
    AffineForOp forOp, AffineMap *tripCountMap,
    SmallVectorImpl<Value> *tripCountOperands) {
  AIIRContext *context = forOp.getContext();
...
/// cases.
std::optional<uint64_t> aiir::affine::getConstantTripCount(AffineForOp forOp) {
...
uint64_t aiir::affine::getLargestDivisorOfTripCount(AffineForOp forOp) {
  SmallVector<Value, 4> operands;
  AffineMap map;
  getTripCountMapAndOperands(forOp, &map, &operands);
...
bool aiir::affine::isInvariantAccess(LoadOrStoreOp memOp, AffineForOp forOp) {
...
template bool aiir::affine::isInvariantAccess(AffineReadOpInterface,
                                              AffineForOp);
template bool aiir::affine::isInvariantAccess(AffineWriteOpInterface,
                                              AffineForOp);
template bool aiir::affine::isInvariantAccess(AffineLoadOp, AffineForOp);
template bool aiir::affine::isInvariantAccess(AffineStoreOp, AffineForOp);

DenseSet<Value> aiir::affine::getInvariantAccesses(Value iv,
...
template <typename LoadOrStoreOp>
bool aiir::affine::isContiguousAccess(Value iv, LoadOrStoreOp memoryOp,
                                      int *memRefDim) {
...
template bool aiir::affine::isContiguousAccess(Value iv,
...
bool aiir::affine::isVectorizableLoopBody(
...
// the support.
bool aiir::affine::isOpwiseShiftValid(AffineForOp forOp,
...
bool aiir::affine::isTilingValid(ArrayRef<AffineForOp> loops) {
...
bool aiir::affine::hasCyclicDependence(AffineForOp root) {
