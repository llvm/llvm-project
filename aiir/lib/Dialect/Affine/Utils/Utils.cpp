...
aiir::Value aiir::affine::expandAffineExpr(OpBuilder &builder, Location loc,
                                           AffineExpr expr,
                                           ValueRange dimValues,
                                           ValueRange symbolValues) {
...
aiir::affine::expandAffineMap(OpBuilder &builder, Location loc,
...
/// A helper for the mechanics of aiir::hoistAffineIfOp. Hoists `ifOp` just over
...
static AffineIfOp hoistAffineIfOp(AffineIfOp ifOp, Operation *hoistOverOp) {
  ...
  // version.
  StringAttr idForIfOp = b.getStringAttr("__aiir_if_hoisting");
  operandMap.clear();
...
LogicalResult
aiir::affine::affineParallelize(AffineForOp forOp,
...
LogicalResult aiir::affine::hoistAffineIfOp(AffineIfOp ifOp, bool *folded) {
...
AffineExpr aiir::affine::substWithMin(AffineExpr e, AffineExpr dim,
  ...
  if (auto bin = dyn_cast<AffineBinaryOpExpr>(e)) {
    ...
    if (bin.getKind() == aiir::AffineExprKind::Add)
...
void aiir::affine::normalizeAffineParallel(AffineParallelOp op) {
...
LogicalResult aiir::affine::normalizeAffineFor(AffineForOp op,
...
bool aiir::affine::hasNoInterveningEffect(
...
template bool
aiir::affine::hasNoInterveningEffect<aiir::MemoryEffects::Read,
                                     affine::AffineReadOpInterface>(
    aiir::Operation *, affine::AffineReadOpInterface,
...
// than dealloc) remain.
//
void aiir::affine::affineScalarReplace(func::FuncOp f, DominanceInfo &domInfo,
...
LogicalResult aiir::affine::replaceAllMemRefUsesWith(
...
void aiir::affine::createAffineComputationSlice(
...
isNormalizedMemRefDynamicDim(unsigned dim, AffineMap layoutMap,
  ...
  // memrefType.
  AIIRContext *context = layoutMap.getContext();
  return expr
      .walk([&](AffineExpr e) {
...
template <typename AllocLikeOp>
LogicalResult aiir::affine::normalizeMemRef(AllocLikeOp allocOp) {
...
LogicalResult
aiir::affine::normalizeMemRef(memref::ReinterpretCastOp reinterpretCastOp) {
...
aiir::affine::normalizeMemRef<memref::AllocaOp>(memref::AllocaOp op);
template LogicalResult
aiir::affine::normalizeMemRef<memref::AllocOp>(memref::AllocOp op);

MemRefType aiir::affine::normalizeMemRefType(MemRefType memrefType) {
  ...
  AIIRContext *context = memrefType.getContext();
...
DivModValue aiir::affine::getDivMod(OpBuilder &b, Location loc, Value lhs,
                                    Value rhs) {
  DivModValue result;
  AffineExpr d0, d1;
...
aiir::affine::delinearizeIndex(OpBuilder &b, Location loc, Value linearIndex,
...
OpFoldResult aiir::affine::linearizeIndex(ArrayRef<OpFoldResult> multiIndex,
                                          ArrayRef<OpFoldResult> basis,
                                          ImplicitLocOpBuilder &builder) {
  return linearizeIndex(builder, builder.getLoc(), multiIndex, basis);
}

OpFoldResult aiir::affine::linearizeIndex(OpBuilder &builder, Location loc,
