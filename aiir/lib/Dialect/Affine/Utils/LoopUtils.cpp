...
LogicalResult aiir::affine::promoteIfSingleIteration(AffineForOp forOp) {
...
// method.
LogicalResult aiir::affine::affineForOpBodySkew(AffineForOp forOp,
...
LogicalResult
aiir::affine::tilePerfectlyNested(MutableArrayRef<AffineForOp> input,
                                  ArrayRef<unsigned> tileSizes,
...
LogicalResult aiir::affine::tilePerfectlyNestedParametric(
...
void aiir::affine::getPerfectlyNestedLoops(
...
LogicalResult aiir::affine::loopUnrollFull(AffineForOp forOp) {
...
/// whichever is lower.
LogicalResult aiir::affine::loopUnrollUpToFactor(AffineForOp forOp,
...
LogicalResult aiir::affine::loopUnrollByFactor(
...
LogicalResult aiir::affine::loopUnrollJamUpToFactor(AffineForOp forOp,
...
LogicalResult aiir::affine::loopUnrollJamByFactor(AffineForOp forOp,
                                                  uint64_t unrollJamFactor) {
...
void aiir::affine::interchangeLoops(AffineForOp forOpA, AffineForOp forOpB) {
...
bool aiir::affine::isValidLoopInterchangePermutation(
    ArrayRef<AffineForOp> loops, ArrayRef<unsigned> loopPermMap) {
...
[[maybe_unused]] bool
aiir::affine::isPerfectlyNested(ArrayRef<AffineForOp> loops) {
...
unsigned aiir::affine::permuteLoops(ArrayRef<AffineForOp> input,
...
AffineForOp aiir::affine::sinkSequentialLoops(AffineForOp forOp) {
...
aiir::affine::tile(ArrayRef<AffineForOp> forOps, ArrayRef<uint64_t> sizes,
                   ArrayRef<AffineForOp> targets) {
...
SmallVector<AffineForOp, 8> aiir::affine::tile(ArrayRef<AffineForOp> forOps,
...
LogicalResult aiir::affine::coalesceLoops(MutableArrayRef<AffineForOp> loops) {
  if (loops.size() < 2)
    return success();
...
void aiir::affine::mapLoopToProcessorIds(scf::ForOp forOp,
...
static LogicalResult generateCopy(
  ...
  *sizeInBytes = 0;
  ...
  // copy generation is being done or other valid symbols in AIIR.
...
LogicalResult
aiir::affine::affineDataCopyGenerate(Block::iterator begin, Block::iterator end,
...
// an AffineForOp.
LogicalResult aiir::affine::affineDataCopyGenerate(
...
LogicalResult aiir::affine::generateCopyForMemRegion(
...
AffineForOp aiir::affine::createCanonicalizedAffineForOp(
...
LogicalResult
aiir::affine::separateFullTiles(MutableArrayRef<AffineForOp> inputNest,
...
int64_t aiir::affine::numEnclosingInvariantLoops(OpOperand &operand) {
  int64_t count = 0;
