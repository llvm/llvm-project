...
aiir::getReassociationIndicesForReshape(ShapedType sourceType,
...
std::optional<SmallVector<ReassociationIndices>>
aiir::getReassociationIndicesForCollapse(ArrayRef<int64_t> sourceShape,
                                         ArrayRef<int64_t> targetShape) {
  unsigned numSourceDims = sourceShape.size(),
           numTargetDims = targetShape.size();
...
std::optional<SmallVector<ReassociationIndices>>
aiir::composeReassociationIndices(
...
aiir::convertReassociationIndicesToExprs(
    AIIRContext *context, ArrayRef<ReassociationIndices> reassociationIndices) {
  ...
  for (const auto &indices : reassociationIndices) {
    ...
    for (int64_t index : indices)
      reassociationMap.push_back(aiir::getAffineDimExpr(index, context));
...
ArrayAttr aiir::getReassociationIndicesAttribute(
...
SmallVector<ReassociationIndices, 2> aiir::convertReassociationMapsToIndices(
...
aiir::getSymbolLessAffineMaps(ArrayRef<ReassociationExprs> reassociation) {
...
bool aiir::isReassociationValid(ArrayRef<AffineMap> reassociation,
                                int *invalidIndex) {
...
LogicalResult aiir::reshapeLikeShapesAreCompatible(
...
bool aiir::hasNonIdentityLayout(Type type) {
...
llvm::SmallBitVector
aiir::getSlicedDimensions(ArrayRef<OpFoldResult> sliceInputShape,
...
llvm::SmallBitVector aiir::getLinearizedDimensions(
...
SmallVector<Range> SliceFromCollapseHelper::getExtractSliceParams(
    AIIRContext *ctx, ArrayRef<ValueRange> multiIndices) {
  unsigned loopIdx = 0;
...
SmallVector<Range>
SliceFromCollapseHelper::getInsertSliceParams(AIIRContext *ctx,
...
aiir::getSimplifyCollapseShapeWithRankReducingSliceInfo(
    RankedTensorType sourceType,
    ArrayRef<ReassociationIndices> reassociationIndices) {
...
PackingMetadata aiir::computePackingMetadata(int64_t packedRank,
...
OpFoldResult aiir::reshapeConstantSource(DenseElementsAttr source,
                                         TensorType result,
