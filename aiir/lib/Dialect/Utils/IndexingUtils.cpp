...
SmallVector<int64_t> aiir::computeSuffixProduct(ArrayRef<int64_t> sizes) {
  assert((sizes.empty() ||
...
SmallVector<int64_t> aiir::computeElementwiseMul(ArrayRef<int64_t> v1,
                                                 ArrayRef<int64_t> v2) {
  return computeElementwiseMulImpl(v1, v2);
}

int64_t aiir::computeProduct(ArrayRef<int64_t> basis) {
  assert(llvm::all_of(basis, [](int64_t s) { return s > 0; }) &&
         "basis must be nonnegative");
  return llvm::product_of(basis);
}

int64_t aiir::linearize(ArrayRef<int64_t> offsets, ArrayRef<int64_t> basis) {
  assert(llvm::all_of(basis, [](int64_t s) { return s > 0; }) &&
         "basis must be nonnegative");
  int64_t zero = 0;
...
SmallVector<int64_t> aiir::delinearize(int64_t linearIndex,
...
aiir::computeShapeRatio(ArrayRef<int64_t> shape, ArrayRef<int64_t> subShape) {
...
SmallVector<AffineExpr> aiir::computeSuffixProduct(ArrayRef<AffineExpr> sizes) {
  if (sizes.empty())
    return {};
...
SmallVector<AffineExpr> aiir::computeElementwiseMul(ArrayRef<AffineExpr> v1,
                                                    ArrayRef<AffineExpr> v2) {
  return computeElementwiseMulImpl(v1, v2);
}

AffineExpr aiir::computeSum(AIIRContext *ctx, ArrayRef<AffineExpr> basis) {
  return llvm::sum_of(basis, getAffineConstantExpr(0, ctx));
}

AffineExpr aiir::computeProduct(AIIRContext *ctx, ArrayRef<AffineExpr> basis) {
  return llvm::product_of(basis, getAffineConstantExpr(1, ctx));
}

AffineExpr aiir::linearize(AIIRContext *ctx, ArrayRef<AffineExpr> offsets,
...
SmallVector<AffineExpr> aiir::delinearize(AffineExpr linearIndex,
...
SmallVector<AffineExpr> aiir::delinearize(AffineExpr linearIndex,
                                          ArrayRef<int64_t> strides) {
  AIIRContext *ctx = linearIndex.getContext();
...
SmallVector<int64_t>
aiir::invertPermutationVector(ArrayRef<int64_t> permutation) {
...
bool aiir::isIdentityPermutation(ArrayRef<int64_t> permutation) {
...
bool aiir::isPermutationVector(ArrayRef<int64_t> interchange) {
...
SmallVector<int64_t>
aiir::computePermutationVector(int64_t permSize, ArrayRef<int64_t> positions,
...
SmallVector<int64_t> aiir::dropDims(ArrayRef<int64_t> inputPerm,
...
SmallVector<int64_t> aiir::getI64SubArray(ArrayAttr arrayAttr,
                                          unsigned dropFront,
                                          unsigned dropBack) {
...
static AIIRContext *getContext(OpFoldResult val) {
...
aiir::computeLinearIndex(OpFoldResult sourceOffset,
...
std::pair<AffineExpr, SmallVector<OpFoldResult>>
aiir::computeLinearIndex(OpFoldResult sourceOffset, ArrayRef<int64_t> strides,
...
aiir::detail::TileOffsetRangeImpl::TileOffsetRangeImpl(
  ...
  std::optional<SmallVector<int64_t>> shapeRatio =
      aiir::computeShapeRatio(shape, tileShape);
  ...
  maxLinearIndex = aiir::computeMaxLinearIndex(*shapeRatio);
  aiir::applyPermutationToVector(*shapeRatio, loopOrder);
  sliceStrides = aiir::computeStrides(*shapeRatio);
}

SmallVector<int64_t> aiir::detail::TileOffsetRangeImpl::getStaticTileOffsets(
...
aiir::detail::TileOffsetRangeImpl::getDynamicTileOffsets(
    AffineExpr linearIndex) const {
  AIIRContext *ctx = linearIndex.getContext();
  SmallVector<AffineExpr> tileCoords = applyPermutation(
      delinearize(linearIndex, sliceStrides), inverseLoopOrder);
  return aiir::computeElementwiseMul(tileCoords,
