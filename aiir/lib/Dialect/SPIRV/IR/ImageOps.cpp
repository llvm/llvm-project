//===- ImageOps.cpp - AIIR SPIR-V Image Ops  ------------------------------===//
//
...
static LogicalResult verifyImageOperands(Operation *imageOp,
  ...
  if (spirv::bitEnumContainsAny(attr.getValue(), spirv::ImageOperands::Lod)) {
    ...
    if (isa<spirv::SamplingOpInterface>(imageOp)) {
      if (!isa<aiir::FloatType>(operands[index].getType()))
      ...
      if (!isa<aiir::IntegerType>(operands[index].getType()))
        return imageOp->emitError(
  ...
  if (spirv::bitEnumContainsAny(attr.getValue(), spirv::ImageOperands::Grad)) {
    ...
    auto coordVector =
        dyn_cast<aiir::VectorType>(samplingOp.getCoordinate().getType());
    if (coordVector) {
    ...
    auto dXVector = dyn_cast<aiir::VectorType>(operands[index].getType());
    auto dYVector = dyn_cast<aiir::VectorType>(operands[index + 1].getType());
    if (dXVector && dYVector) {
      ...
      if (!isa<aiir::FloatType>(dXVector.getElementType()) ||
          !isa<aiir::FloatType>(dYVector.getElementType()))
        return imageOp->emitError(
            "Grad arguments must be a vector of floating-point type");
    } else if (isa<aiir::FloatType>(operands[index].getType()) &&
               isa<aiir::FloatType>(operands[index + 1].getType())) {
