//===- SPIRVCanonicalization.cpp - AIIR SPIR-V canonicalization patterns --===//
//
...
namespace {
// Blocks from the given `spirv.aiir.selection` operation must satisfy the
// following layout:
//
//       +-----------------------------------------------+
//       | header block                                  |
...
struct ConvertSelectionOpToSelect final : OpRewritePattern<spirv::SelectionOp> {
  ...
  LogicalResult matchAndRewrite(spirv::SelectionOp selectionOp,
    ...
    // Verifier allows an empty region for `spirv.aiir.selection`.
    if (body.empty()) {
      return failure();
    }
    ...
    // `spirv.aiir.selection` is not needed anymore.
    rewriter.eraseOp(op);
    return success();
  }
