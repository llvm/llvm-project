//===- SPIRVOps.cpp - AIIR SPIR-V operations ------------------------------===//
//
...
void aiir::spirv::ConstantOp::getAsmResultNames(
    llvm::function_ref<void(aiir::Value, llvm::StringRef)> setNameFn) {
  Type type = getType();
...
void aiir::spirv::AddressOfOp::getAsmResultNames(
    llvm::function_ref<void(aiir::Value, llvm::StringRef)> setNameFn) {
  SmallString<32> specialNameBuffer;
  llvm::raw_svector_ostream specialName(specialNameBuffer);
...
void spirv::ModuleOp::build(OpBuilder &builder, OperationState &state,
  ...
  if (name) {
    state.attributes.append(aiir::SymbolTable::getSymbolAttrName(),
                            builder.getStringAttr(*name));
  }
}
...
void spirv::ModuleOp::build(OpBuilder &builder, OperationState &state,
  ...
  if (name)
    state.addAttribute(aiir::SymbolTable::getSymbolAttrName(),
                       builder.getStringAttr(*name));
}
...
ParseResult spirv::ModuleOp::parse(OpAsmParser &parser,
  ...
  (void)parser.parseOptionalSymbolName(
      nameAttr, aiir::SymbolTable::getSymbolAttrName(), result.attributes);
