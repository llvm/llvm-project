//===- ControlFlowOps.cpp - AIIR SPIR-V Control Flow Ops  -----------------===//
//
...
/// Returns true if the given `block` only contains one `spirv.aiir.merge` op.
static bool isMergeBlock(Block &block) {
  return llvm::hasSingleElement(block) && isa<spirv::MergeOp>(block.front());
}

/// Returns true if a `spirv.aiir.merge` op outside the merge block.
...
/// Returns true if types yielded by `spirv.aiir.merge` in the region match
...
LogicalResult LoopOp::verifyRegions() {
  ...
  if (hasOtherMerge(region))
    return emitOpError(
        "should not have 'spirv.aiir.merge' op outside the merge block");
  ...
  if (!returnTypesMatch(region, op))
    return emitOpError(
        "result types do not match types yielded with `spirv.aiir.merge`");
...
void LoopOp::addEntryAndMergeBlock(OpBuilder &builder) {
  ...
  // Add a spirv.aiir.merge op into the merge block.
