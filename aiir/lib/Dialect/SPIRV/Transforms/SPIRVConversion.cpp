...
#include "aiir/Dialect/SPIRV/IR/TargetAndABI.h"
...
#define DEBUG_TYPE "aiir-spirv-conversion"
...
static std::optional<SmallVector<int64_t>> getTargetShape(VectorType vecType) {
  ...
  std::optional<SmallVector<int64_t>> targetShape = SmallVector<int64_t>(
      1, aiir::spirv::getComputeVectorSize(vecType.getShape().back()));
  if (!targetShape) {
...
static spirv::ScalarType getIndexType(AIIRContext *ctx,
...
Value aiir::spirv::getBuiltinVariableValue(Operation *op,
                                           spirv::BuiltIn builtin,
                                           Type integerType, OpBuilder &builder,
                                           StringRef prefix, StringRef suffix) {
...
Value aiir::spirv::linearizeIndex(ValueRange indices, ArrayRef<int64_t> strides,
...
Value aiir::spirv::getVulkanElementPtr(const SPIRVTypeConverter &typeConverter,
...
Value aiir::spirv::getOpenCLElementPtr(const SPIRVTypeConverter &typeConverter,
                                       MemRefType baseType, Value basePtr,
                                       ValueRange indices, Location loc,
                                       OpBuilder &builder) {
  // Get base and offset of the MemRefType and verify they are static.
...
Value aiir::spirv::getElementPtr(const SPIRVTypeConverter &typeConverter,
                                 MemRefType baseType, Value basePtr,
                                 ValueRange indices, Location loc,
                                 OpBuilder &builder) {
...
int aiir::spirv::getComputeVectorSize(int64_t size) {
  for (int i : {4, 3, 2}) {
    if (size % i == 0)
      return i;
  }
...
SmallVector<int64_t>
aiir::spirv::getNativeVectorShapeImpl(vector::ReductionOp op) {
  VectorType srcVectorType = op.getSourceVectorType();
  assert(srcVectorType.getRank() == 1); // Guaranteed by semantics
  int64_t vectorSize =
      aiir::spirv::getComputeVectorSize(srcVectorType.getDimSize(0));
  return {vectorSize};
}

SmallVector<int64_t>
aiir::spirv::getNativeVectorShapeImpl(vector::TransposeOp op) {
  VectorType vectorType = op.getResultVectorType();
  SmallVector<int64_t> nativeSize(vectorType.getRank(), 1);
  nativeSize.back() =
      aiir::spirv::getComputeVectorSize(vectorType.getShape().back());
  return nativeSize;
}

std::optional<SmallVector<int64_t>>
aiir::spirv::getNativeVectorShape(Operation *op) {
  if (OpTrait::hasElementwiseMappableTraits(op) && op->getNumResults() == 1) {
    if (auto vecType = dyn_cast<VectorType>(op->getResultTypes()[0])) {
      SmallVector<int64_t> nativeSize(vecType.getRank(), 1);
      nativeSize.back() =
          aiir::spirv::getComputeVectorSize(vecType.getShape().back());
      return nativeSize;
    }
  }
...
LogicalResult aiir::spirv::unrollVectorsInSignatures(Operation *op) {
...
LogicalResult aiir::spirv::unrollVectorsInFuncBodies(Operation *op) {
  AIIRContext *context = op->getContext();
  ...
  // Unroll vectors in function bodies to native vector size.
  {
    ...
    auto options = vector::UnrollVectorOptions().setNativeShapeFn(
        [](auto op) { return aiir::spirv::getNativeVectorShape(op); });
...
AIIRContext *SPIRVTypeConverter::getContext() const {
...
void aiir::populateBuiltinFuncToSPIRVPatterns(
    const SPIRVTypeConverter &typeConverter, RewritePatternSet &patterns) {
  patterns.add<FuncOpConversion>(typeConverter, patterns.getContext());
}

void aiir::populateFuncOpVectorRewritePatterns(RewritePatternSet &patterns) {
  patterns.add<FuncOpVectorUnroll>(patterns.getContext());
}

void aiir::populateReturnOpVectorRewritePatterns(RewritePatternSet &patterns) {
