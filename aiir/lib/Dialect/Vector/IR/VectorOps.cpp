//===- VectorOps.cpp - AIIR Vector Dialect Operations ---------------------===//
//
...
#include "aiir/Dialect/Vector/IR/VectorDialect.cpp.inc"
// Pull in all enum type and utility function definitions.
#include "aiir/Dialect/Vector/IR/VectorEnums.cpp.inc"
...
/// arguments.
void aiir::vector::buildTerminatedBody(OpBuilder &builder, Location loc) {
...
AffineMap aiir::vector::getTransferMinorIdentityMap(ShapedType shapedType,
                                                    VectorType vectorType) {
...
bool aiir::vector::checkSameValueRAW(vector::TransferWriteOp defWrite,
                                     vector::TransferReadOp read) {
...
bool aiir::vector::checkSameValueWAW(vector::TransferWriteOp write,
...
bool aiir::vector::isDisjointTransferIndices(
...
bool aiir::vector::isDisjointTransferSet(VectorTransferOpInterface transferA,
...
#include "aiir/Dialect/Vector/IR/VectorAttributes.cpp.inc"
      >();

  addOperations<
#define GET_OP_LIST
#include "aiir/Dialect/Vector/IR/VectorOps.cpp.inc"
      >();
...
Value aiir::vector::getVectorReductionOp(arith::AtomicRMWKind op,
...
getDimMap(ArrayRef<AffineMap> indexingMaps, ArrayAttr iteratorTypes,
          IteratorType targetIteratorType, AIIRContext *context) {
...
LogicalResult
ExtractOp::inferReturnTypes(AIIRContext *, std::optional<Location>,
...
static Value extractInsertFoldConstantOp(OpType op, AdaptorType adaptor,
  ...
  for (unsigned i = 0, e = staticPosition.size(); i < e; ++i) {
    ...
    if (auto attr = aiir::dyn_cast_if_present<IntegerAttr>(positionAttr)) {
...
static Attribute foldPoisonIndexInsertExtractOp(AIIRContext *context,
...
LogicalResult
ToElementsOp::inferReturnTypes(AIIRContext *ctx, std::optional<Location> loc,
...
BroadcastableToResult aiir::vector::isBroadcastableTo(
...
LogicalResult
ShuffleOp::inferReturnTypes(AIIRContext *, std::optional<Location> loc,
...
VectorType aiir::vector::inferTransferOpMaskType(VectorType vecType,
                                                 AffineMap permMap) {
...
TransferReadOp::bubbleDownCasts(OpBuilder &builder) {
  if (!hasPureBufferSemantics())
    return failure();
  return aiir::detail::bubbleDownInPlaceMemorySpaceCastImpl(getBaseMutable(),
                                                            getResult());
}
...
public:
  using Base::Base;
  FoldTransposeBroadcast(AIIRContext *context, PatternBenefit benefit = 1)
...
void aiir::vector::MaskOp::print(OpAsmPrinter &p) {
  p << " " << getMask();
  if (getPassthru())
    p << ", " << getPassthru();
...
void aiir::vector::populateVectorToVectorCanonicalizationPatterns(
...
Value aiir::vector::makeArithReduction(OpBuilder &b, Location loc,
...
struct StepCompareFolder : public OpRewritePattern<StepOp> {
  ...
  LogicalResult matchAndRewrite(StepOp stepOp,
    ...
    for (OpOperand &use : stepOp.getResult().getUses()) {
      ...
      Value splat = aiir::arith::ConstantOp::create(rewriter, cmpiOp.getLoc(),
                                                    type, boolAttr);
...
/// as masked operation.
void aiir::vector::createMaskOpRegion(OpBuilder &builder,
                                      Operation *maskableOp) {
...
Operation *aiir::vector::maskOperation(OpBuilder &builder,
...
/// target instructions.
Value aiir::vector::selectPassthru(OpBuilder &builder, Value mask,
