//===- VectorUtils.cpp - AIIR Utilities for VectorOps   ------------------===//
//
...
#include "aiir/Dialect/Vector/Utils/VectorUtils.h"
...
/// the type of `source`.
Value aiir::vector::createOrFoldDimOp(OpBuilder &b, Location loc, Value source,
                                      int64_t dim) {
...
aiir::vector::isTranspose2DSlice(vector::TransposeOp op) {
...
AffineMap aiir::makePermutationMap(
...
Value vector::createReadOrMaskedRead(OpBuilder &builder, Location loc,
  ...
  return aiir::vector::maskOperation(builder, transferReadOp, mask)
      ->getResult(0);
}
