...
struct LinearizeConstantLike final
  ...
  LinearizeConstantLike(const TypeConverter &typeConverter,
                        AIIRContext *context, PatternBenefit benefit = 1)
...
struct LinearizeVectorExtractStridedSlice final
    : public aiir::OpConversionPattern<aiir::vector::ExtractStridedSliceOp> {
  using Base::Base;
...
struct LinearizeVectorInsertStridedSlice final
    : public aiir::OpConversionPattern<aiir::vector::InsertStridedSliceOp> {
  using Base::Base;
...
struct LinearizeVectorCreateMask final
  ...
  matchAndRewrite(vector::CreateMaskOp createMaskOp, OpAdaptor adaptor,
    ...
    auto isNonZero = rewriter.createOrFold<aiir::arith::CmpIOp>(
        loc, aiir::arith::CmpIPredicate::sgt, firstOperand, zero);
    auto isNonZeroIndex = rewriter.createOrFold<aiir::arith::IndexCastOp>(
        loc, rewriter.getIndexType(), isNonZero);
    auto secondOperand = adaptor.getOperands().back();
    auto maskSize = rewriter.createOrFold<aiir::arith::AndIOp>(
        loc, rewriter.getIndexType(), isNonZeroIndex, secondOperand);

    auto newMask =
        aiir::vector::CreateMaskOp::create(rewriter, loc, dstTy, maskSize);
...
struct LinearizeVectorLoad final : public OpConversionPattern<vector::LoadOp> {
  using Base::Base;
  LinearizeVectorLoad(const TypeConverter &typeConverter, AIIRContext *context,
                      PatternBenefit benefit = 1)
      : OpConversionPattern(typeConverter, context, benefit) {}
...
struct LinearizeVectorStore final
  ...
  using Base::Base;
  LinearizeVectorStore(const TypeConverter &typeConverter, AIIRContext *context,
                       PatternBenefit benefit = 1)
      : OpConversionPattern(typeConverter, context, benefit) {}
...
void aiir::vector::populateForVectorLinearize(TypeConverter &typeConverter,
...
void aiir::vector::populateVectorLinearizeBasePatterns(
...
void aiir::vector::populateVectorLinearizeShuffleLikeOpsPatterns(
    const TypeConverter &typeConverter, const ConversionTarget &target,
    RewritePatternSet &patterns) {
