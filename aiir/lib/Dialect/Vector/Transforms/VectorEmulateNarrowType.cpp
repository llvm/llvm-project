...
// in:
//  * "vector-emulate-narrow-type-unaligned-non-atomic.aiir".
//
...
struct ConvertVectorStore final : OpConversionPattern<vector::StoreOp> {
  ...
  ConvertVectorStore(AIIRContext *context, bool disableAtomicRMW,
                     bool assumeAligned)
  ...
  matchAndRewrite(vector::StoreOp op, OpAdaptor adaptor,
    ...
    //
    // Input AIIR: vector.store %val, %dest[%c2] : memref<12xi2>, vector<7xi2>
    //
...
static LogicalResult commonConversionPrecondition(PatternRewriter &rewriter,
  ...
  // to really work with subbyte elements across the AIIR/LLVM boundary.
...
struct RewriteExtOfBitCast : OpRewritePattern<ExtOpType> {
  ...
  RewriteExtOfBitCast(AIIRContext *context, PatternBenefit benefit)
...
struct RewriteVectorTranspose : OpRewritePattern<vector::TransposeOp> {
  ...
  RewriteVectorTranspose(AIIRContext *context, PatternBenefit benefit)
