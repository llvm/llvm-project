...
public:
  ...
  explicit InnerOuterDimReductionConversion(
      AIIRContext *context, vector::VectorMultiReductionLowering options,
      PatternBenefit benefit = 1)
      : aiir::OpRewritePattern<vector::MultiDimReductionOp>(context, benefit),
  ...
  LogicalResult matchAndRewrite(vector::MultiDimReductionOp multiReductionOp,
    ...
    newMultiRedOp =
        aiir::vector::maskOperation(rewriter, newMultiRedOp, transposedMask);
...
public:
  ...
  explicit FlattenMultiReduction(AIIRContext *context,
  ...
  LogicalResult matchAndRewrite(vector::MultiDimReductionOp multiReductionOp,
    ...
    newMultiDimRedOp =
        aiir::vector::maskOperation(rewriter, newMultiDimRedOp, newVectorMask);
  ...
  matchAndRewriteMaskableOp(vector::MultiDimReductionOp multiReductionOp,
    ...
    for (int64_t i = 0; i < outerDim; ++i) {
      ...
      if (mask) {
        ...
        reductionOp = aiir::vector::maskOperation(rewriter, reductionOp, m);
      }
...
struct OneDimMultiReductionToReduction
  ...
  matchAndRewriteMaskableOp(vector::MultiDimReductionOp multiReductionOp,
    ...
    if (mask)
      reductionOp = aiir::vector::maskOperation(rewriter, reductionOp, mask);
...
struct LowerVectorMultiReductionPass
  ...
  void runOnOperation() override {
    ...
    aiir::vector::populateVectorMultiReductionReorderPatterns(
        patterns, this->loweringStrategy);
    if (failed(applyPatternsGreedily(op, std::move(patterns))))
      signalPassFailure();

    RewritePatternSet flatteningPatterns(context);
    aiir::vector::populateVectorMultiReductionFlatteningPatterns(
    ...
    aiir::vector::populateVectorMultiReductionUnrollingPatterns(
...
void aiir::vector::populateVectorMultiReductionReorderPatterns(
...
void aiir::vector::populateVectorMultiReductionFlatteningPatterns(
    RewritePatternSet &patterns, VectorMultiReductionLowering options,
    PatternBenefit benefit) {
  patterns.add<FlattenMultiReduction>(patterns.getContext(), options, benefit);
}

void aiir::vector::populateVectorMultiReductionUnrollingPatterns(
    RewritePatternSet &patterns, VectorMultiReductionLowering options,
    PatternBenefit benefit) {
