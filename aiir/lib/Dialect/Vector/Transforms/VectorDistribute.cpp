...
struct WarpOpToScfIfPattern : public WarpDistributionPattern {
  WarpOpToScfIfPattern(AIIRContext *context,
...
getInnerRegionEscapingValues(WarpExecuteOnLane0Op warpOp, Region &innerRegion,
  ...
  aiir::visitUsedValuesDefinedAbove(innerRegion, [&](OpOperand *operand) {
...
struct WarpOpTransferWrite : public WarpDistributionPattern {
  WarpOpTransferWrite(AIIRContext *ctx, DistributionMapFn fn,
  ...
  LogicalResult tryDistributeOp(RewriterBase &rewriter,
    ...
    if (map.getNumResults() > 1) {
      delinearized = aiir::affine::AffineDelinearizeIndexOp::create(
...
struct WarpOpTransferRead : public WarpDistributionPattern {
  ...
  LogicalResult matchAndRewrite(WarpExecuteOnLane0Op warpOp,
    ...
    if (read.getMask()) {
      ...
      // by shape information on the warp op, and thus requires materializing
      // the permutation in IR.
      if (!aiir::compressUnusedDims(read.getPermutationMap()).isIdentity())
        return rewriter.notifyMatchFailure(
...
struct WarpOpExtractScalar : public WarpDistributionPattern {
  WarpOpExtractScalar(AIIRContext *ctx, WarpShuffleFromIdxFn fn,
                      PatternBenefit b = 1)
...
struct WarpOpScfIfOp : public WarpDistributionPattern {
  WarpOpScfIfOp(AIIRContext *ctx, DistributionMapFn fn, PatternBenefit b = 1)
      : WarpDistributionPattern(ctx, b), distributionMapFn(std::move(fn)) {}
  LogicalResult matchAndRewrite(WarpExecuteOnLane0Op warpOp,
                                PatternRewriter &rewriter) const override {
    ...
    auto encloseRegionInWarpOp =
        [&](Block *oldIfBranch, Block *newIfBranch,
          ...
          aiir::vector::moveScalarUniformCode(innerWarp);
        };
...
struct WarpOpScfForOp : public WarpDistributionPattern {
  ...
  WarpOpScfForOp(AIIRContext *ctx, DistributionMapFn fn, PatternBenefit b = 1)
      : WarpDistributionPattern(ctx, b), distributionMapFn(std::move(fn)) {}
  LogicalResult matchAndRewrite(WarpExecuteOnLane0Op warpOp,
                                PatternRewriter &rewriter) const override {
    gpu::YieldOp warpOpYield = warpOp.getTerminator();
...
struct WarpOpReduction : public WarpDistributionPattern {
  WarpOpReduction(AIIRContext *context,
...
void aiir::vector::populateWarpExecuteOnLane0OpToScfForPattern(
...
void aiir::vector::populateDistributeTransferWriteOpPatterns(
...
void aiir::vector::populatePropagateWarpVectorDistributionPatterns(
    RewritePatternSet &patterns, const DistributionMapFn &distributionMapFn,
...
void aiir::vector::populateDistributeReduction(
    RewritePatternSet &patterns,
...
void aiir::vector::moveScalarUniformCode(WarpExecuteOnLane0Op warpOp) {
