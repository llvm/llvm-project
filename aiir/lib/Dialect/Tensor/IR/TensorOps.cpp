...
#include "aiir/Dialect/Linalg/IR/RelayoutOpInterface.h"
...
#include "aiir/Interfaces/Utils/InferIntRangeCommon.h"
...
bool aiir::tensor::preservesStaticInformation(Type source, Type target) {
...
/// ```
bool aiir::tensor::canFoldIntoConsumerOp(CastOp castOp) {
  if (!castOp)
    return false;
...
bool aiir::tensor::canFoldIntoProducerOp(CastOp castOp) {
  if (!castOp)
    return false;
...
bool aiir::tensor::hasFoldableTensorCastOperand(Operation *op) {
...
SmallVector<Value> aiir::tensor::getUpdatedOperandsAfterCastOpFolding(
...
/// that can be folded.
LogicalResult aiir::tensor::foldTensorCast(Operation *op) {
  bool folded = false;
...
void aiir::tensor::populateFoldCollapseExtractPatterns(
...
ExtractSliceOp::rankReduceIfNeeded(OpBuilder &b, Location loc, Value value,
  ...
  auto maybeRankReductionMask =
      aiir::computeRankReductionMask(sourceShape, desiredShape);
...
public:
  ...
  ConstantOpExtractSliceFolder(AIIRContext *context,
...
void aiir::tensor::populateFoldConstantExtractSlicePatterns(
    RewritePatternSet &patterns,
...
Value aiir::tensor::createCanonicalRankReducingExtractSliceOp(
...
Value aiir::tensor::createCanonicalRankReducingInsertSliceOp(OpBuilder &b,
                                                             Location loc,
                                                             Value tensor,
                                                             Value dest) {
