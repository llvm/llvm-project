...
PadOp aiir::tensor::createPadHighOp(RankedTensorType resType, Value source,
...
SmallVector<Value> aiir::tensor::createDynamicDimValues(OpBuilder &b,
                                                        Location loc,
                                                        Value rankedTensor) {
...
aiir::tensor::computeTransposedType(RankedTensorType rankedTensorType,
...
CollapseShapeOp
aiir::tensor::dropGivenUnitDims(OpBuilder &b, Location loc, Value src,
...
bool aiir::tensor::isCastLikeInsertSliceOp(InsertSliceOp op) {
...
bool aiir::tensor::isCastLikeExtractSliceOp(ExtractSliceOp op) {
  llvm::SmallBitVector droppedDims = op.getDroppedDims();
  int64_t resultDim = 0;
