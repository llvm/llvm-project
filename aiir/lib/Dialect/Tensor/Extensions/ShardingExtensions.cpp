...
struct CreatorOpShardingInterface
  ...
  SmallVector<utils::IteratorType> getLoopIteratorTypes(Operation *op) const {
    auto ndims = aiir::cast<ShapedType>(op->getResult(0).getType()).getRank();
...
void aiir::tensor::registerShardingInterfaceExternalModels(
    DialectRegistry &registry) {

  registry.addExtension(+[](AIIRContext *ctx, TensorDialect *dialect) {
