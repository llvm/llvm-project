//===- Shape.cpp - AIIR Shape Operations ----------------------------------===//
//
...
#include "aiir/Dialect/Shape/IR/ShapeOpsDialect.cpp.inc"

namespace {
#include "ShapeCanonicalization.inc"
} // namespace

RankedTensorType shape::getExtentTensorType(AIIRContext *ctx, int64_t rank) {
...
void ShapeDialect::initialize() {
  addOperations<
#define GET_OP_LIST
#include "aiir/Dialect/Shape/IR/ShapeOps.cpp.inc"
      >();
  addTypes<
#define GET_TYPEDEF_LIST
#include "aiir/Dialect/Shape/IR/ShapeOpsTypes.cpp.inc"
      >();
...
LogicalResult aiir::shape::AddOp::inferReturnTypes(
    AIIRContext *context, std::optional<Location> location,
...
bool aiir::shape::AddOp::isCompatibleReturnTypes(TypeRange l, TypeRange r) {
  // SizeType is compatible with IndexType.
  return eachHasOnlyOneOfTypes<SizeType, IndexType>(l, r);
}

OpFoldResult aiir::shape::AddOp::fold(FoldAdaptor adaptor) {
  // add(x, 0) -> x
...
LogicalResult aiir::shape::ConstShapeOp::inferReturnTypes(
    AIIRContext *context, std::optional<Location> location,
...
bool aiir::shape::ConstShapeOp::isCompatibleReturnTypes(TypeRange l,
                                                        TypeRange r) {
...
void CstrBroadcastableOp::getCanonicalizationPatterns(
    RewritePatternSet &patterns, AIIRContext *context) {
...
LogicalResult aiir::shape::DimOp::inferReturnTypes(
    AIIRContext *context, std::optional<Location> location,
...
bool aiir::shape::DimOp::isCompatibleReturnTypes(TypeRange l, TypeRange r) {
  return eachHasOnlyOneOfTypes<SizeType, IndexType>(l, r);
}
...
LogicalResult aiir::shape::DivOp::inferReturnTypes(
    AIIRContext *context, std::optional<Location> location,
...
bool aiir::shape::DivOp::isCompatibleReturnTypes(TypeRange l, TypeRange r) {
  // SizeType is compatible with IndexType.
  return eachHasOnlyOneOfTypes<SizeType, IndexType>(l, r);
}
...
void FunctionLibraryOp::build(OpBuilder &builder, OperationState &result,
  ...
  result.attributes.push_back(builder.getNamedAttr(
      ::aiir::SymbolTable::getSymbolAttrName(), builder.getStringAttr(name)));
}
...
ParseResult FunctionLibraryOp::parse(OpAsmParser &parser,
  ...
  // Parse the op name.
  StringAttr nameAttr;
  if (parser.parseSymbolName(nameAttr, ::aiir::SymbolTable::getSymbolAttrName(),
                             result.attributes))
    return failure();
...
void FunctionLibraryOp::print(OpAsmPrinter &p) {
  ...
  p.printOptionalAttrDictWithKeyword(
      (*this)->getAttrs(), {aiir::SymbolTable::getSymbolAttrName(), "mapping"});
  p << ' ';
...
LogicalResult aiir::shape::GetExtentOp::inferReturnTypes(
    AIIRContext *context, std::optional<Location> location,
...
bool aiir::shape::GetExtentOp::isCompatibleReturnTypes(TypeRange l,
                                                       TypeRange r) {
  // SizeType is compatible with IndexType.
  return eachHasOnlyOneOfTypes<SizeType, IndexType>(l, r);
}
...
LogicalResult aiir::shape::MeetOp::inferReturnTypes(
    AIIRContext *context, std::optional<Location> location,
...
bool aiir::shape::MeetOp::isCompatibleReturnTypes(TypeRange l, TypeRange r) {
  if (l.size() != 1 || r.size() != 1)
    return false;
  if (l == r)
    return true;
...
LogicalResult aiir::shape::RankOp::inferReturnTypes(
    AIIRContext *context, std::optional<Location> location,
...
bool aiir::shape::RankOp::isCompatibleReturnTypes(TypeRange l, TypeRange r) {
  // SizeType is compatible with IndexType.
  return eachHasOnlyOneOfTypes<SizeType, IndexType>(l, r);
}
...
LogicalResult aiir::shape::NumElementsOp::inferReturnTypes(
    AIIRContext *context, std::optional<Location> location,
...
bool aiir::shape::NumElementsOp::isCompatibleReturnTypes(TypeRange l,
                                                         TypeRange r) {
  // SizeType is compatible with IndexType.
  return eachHasOnlyOneOfTypes<SizeType, IndexType>(l, r);
}
...
LogicalResult aiir::shape::MaxOp::inferReturnTypes(
    AIIRContext *context, std::optional<Location> location,
...
bool aiir::shape::MaxOp::isCompatibleReturnTypes(TypeRange l, TypeRange r) {
  if (l.size() != 1 || r.size() != 1)
    return false;
...
LogicalResult aiir::shape::MinOp::inferReturnTypes(
    AIIRContext *context, std::optional<Location> location,
...
bool aiir::shape::MinOp::isCompatibleReturnTypes(TypeRange l, TypeRange r) {
  if (l.size() != 1 || r.size() != 1)
    return false;
  if (llvm::isa<ShapeType>(l.front()) && llvm::isa<ShapeType>(r.front()))
    return true;
...
LogicalResult aiir::shape::MulOp::inferReturnTypes(
    AIIRContext *context, std::optional<Location> location,
...
bool aiir::shape::MulOp::isCompatibleReturnTypes(TypeRange l, TypeRange r) {
  // SizeType is compatible with IndexType.
  return eachHasOnlyOneOfTypes<SizeType, IndexType>(l, r);
}
...
LogicalResult aiir::shape::ShapeOfOp::inferReturnTypes(
    AIIRContext *context, std::optional<Location> location,
...
bool aiir::shape::ShapeOfOp::isCompatibleReturnTypes(TypeRange l, TypeRange r) {
  if (l.size() != 1 || r.size() != 1)
    return false;
  if (l == r)
    return true;
