//===- OpenMPDialect.cpp - AIIR Dialect for OpenMP implementation ---------===//
//
...
#include "aiir/Dialect/OpenMP/OpenMPOpsDialect.cpp.inc"
#include "aiir/Dialect/OpenMP/OpenMPOpsEnums.cpp.inc"
#include "aiir/Dialect/OpenMP/OpenMPOpsInterfaces.cpp.inc"
#include "aiir/Dialect/OpenMP/OpenMPTypeInterfaces.cpp.inc"

using namespace aiir;
using namespace aiir::omp;

static ArrayAttr makeArrayAttr(AIIRContext *context,
                               llvm::ArrayRef<Attribute> attrs) {
  return attrs.empty() ? nullptr : ArrayAttr::get(context, attrs);
}

static DenseBoolArrayAttr
makeDenseBoolArrayAttr(AIIRContext *ctx, const ArrayRef<bool> boolArray) {
  return boolArray.empty() ? nullptr : DenseBoolArrayAttr::get(ctx, boolArray);
}

static DenseI64ArrayAttr
makeDenseI64ArrayAttr(AIIRContext *ctx, const ArrayRef<int64_t> intArray) {
...
static std::string generateLoopNestingName(StringRef prefix,
  ...
  while (o) {
    ...
    // ambiguity.
    if (!parent || parent->hasTrait<aiir::OpTrait::IsIsolatedFromAbove>())
      break;
...
#include "aiir/Dialect/OpenMP/OpenMPOpsAttributes.cpp.inc"
      >();
  addTypes<
#define GET_TYPEDEF_LIST
#include "aiir/Dialect/OpenMP/OpenMPOpsTypes.cpp.inc"
      >();
  ...
  aiir::ModuleOp::attachInterface<aiir::omp::OffloadModuleDefaultModel>(
      *getContext());
  ...
  // that Fortran (or other languages that lower to AIIR) translates too
  aiir::LLVM::GlobalOp::attachInterface<
      aiir::omp::DeclareTargetDefaultModel<aiir::LLVM::GlobalOp>>(
      *getContext());
  aiir::LLVM::LLVMFuncOp::attachInterface<
      aiir::omp::DeclareTargetDefaultModel<aiir::LLVM::LLVMFuncOp>>(
      *getContext());
  aiir::func::FuncOp::attachInterface<
      aiir::omp::DeclareTargetDefaultModel<aiir::func::FuncOp>>(*getContext());
}
...
parseScheduleClause(OpAsmParser &parser, ClauseScheduleKindAttr &scheduleAttr,
  ...
  if (parser.parseKeyword(&keyword))
    return failure();
  std::optional<aiir::omp::ClauseScheduleKind> schedule =
...
static void printGrainsizeClause(OpAsmPrinter &p, Operation *op,
                                 ...
                                 Value grainsize, aiir::Type grainsizeType) {
...
static void printNumTasksClause(OpAsmPrinter &p, Operation *op,
                                ...
                                Value numTasks, aiir::Type numTasksType) {
...
static ParseResult parseClauseWithRegionArgs(
  ...
  if (needsBarrier) {
    if (parser.parseOptionalKeyword(getPrivateNeedsBarrierSpelling())
            .succeeded())
      *needsBarrier = aiir::UnitAttr::get(parser.getContext());
  }
  ...
  if (!mapIndicesVec.empty())
    *mapIndices =
        aiir::DenseI64ArrayAttr::get(parser.getContext(), mapIndicesVec);
...
static void printClauseWithRegionArgs(
    OpAsmPrinter &p, AIIRContext *ctx, StringRef clauseName,
  ...
  if (!byref) {
    aiir::SmallVector<bool> values(operands.size(), false);
...
static void printBlockArgClause(OpAsmPrinter &p, AIIRContext *ctx,
...
static void
printBlockArgClause(OpAsmPrinter &p, AIIRContext *ctx, StringRef clauseName,
...
static void printBlockArgRegion(OpAsmPrinter &p, Operation *op, Region &region,
  ...
  auto iface = llvm::cast<aiir::omp::BlockArgOpenMPOpInterface>(op);
...
static ParseResult parseSplitIteratedList(
  ...
  return parser.parseCommaSeparatedList([&]() -> ParseResult {
    ...
    if (llvm::isa<aiir::omp::IteratedType>(ty)) {
...
verifyCopyprivateVarList(Operation *op, OperandRange copyprivateVars,
  ...
  for (auto copyprivateVarAndSym :
    ...
    std::optional<std::variant<aiir::func::FuncOp, aiir::LLVM::LLVMFuncOp>>
        funcOp;
    if (aiir::func::FuncOp aiirFuncOp =
            SymbolTable::lookupNearestSymbolFrom<aiir::func::FuncOp>(op,
                                                                     symbolRef))
      funcOp = aiirFuncOp;
    else if (aiir::LLVM::LLVMFuncOp llvmFuncOp =
                 SymbolTable::lookupNearestSymbolFrom<aiir::LLVM::LLVMFuncOp>(
                     op, symbolRef))
      funcOp = llvmFuncOp;
...
static void printDependVarList(OpAsmPrinter &p, Operation *op,
  ...
  for (unsigned i = 0, e = dependKinds->size(); i < e; ++i) {
    ...
    p << stringifyClauseTaskDepend(
             llvm::cast<aiir::omp::ClauseTaskDependAttr>((*dependKinds)[i])
                 .getValue())
...
static ParseResult parseMapClause(OpAsmParser &parser,
  ...
  mapType =
      parser.getBuilder().getAttr<aiir::omp::ClauseMapFlagsAttr>(mapTypeBits);
...
static void printCaptureType(OpAsmPrinter &p, Operation *op,
  ...
  if (mapCaptureType.getValue() == aiir::omp::VariableCaptureKind::ByRef)
    typeCap << "ByRef";
  if (mapCaptureType.getValue() == aiir::omp::VariableCaptureKind::ByCopy)
    typeCap << "ByCopy";
  if (mapCaptureType.getValue() == aiir::omp::VariableCaptureKind::VLAType)
    typeCap << "VLAType";
  if (mapCaptureType.getValue() == aiir::omp::VariableCaptureKind::This)
    typeCap << "This";
  p << typeCapStr;
}
...
static ParseResult parseCaptureType(OpAsmParser &parser,
  ...
  if (mapCaptureKey == "This")
    mapCaptureType = aiir::omp::VariableCaptureKindAttr::get(
        parser.getContext(), aiir::omp::VariableCaptureKind::This);
  if (mapCaptureKey == "ByRef")
    mapCaptureType = aiir::omp::VariableCaptureKindAttr::get(
        parser.getContext(), aiir::omp::VariableCaptureKind::ByRef);
  if (mapCaptureKey == "ByCopy")
    mapCaptureType = aiir::omp::VariableCaptureKindAttr::get(
        parser.getContext(), aiir::omp::VariableCaptureKind::ByCopy);
  if (mapCaptureKey == "VLAType")
    mapCaptureType = aiir::omp::VariableCaptureKindAttr::get(
        parser.getContext(), aiir::omp::VariableCaptureKind::VLAType);

  return success();
}

static LogicalResult verifyMapClause(Operation *op, OperandRange mapVars) {
  llvm::DenseSet<aiir::TypedValue<aiir::omp::PointerLikeType>> updateToVars;
  llvm::DenseSet<aiir::TypedValue<aiir::omp::PointerLikeType>> updateFromVars;

  for (auto mapOp : mapVars) {
    if (!mapOp.getDefiningOp())
      return emitError(op->getLoc(), "missing map operation");

    if (auto mapInfoOp = mapOp.getDefiningOp<aiir::omp::MapInfoOp>()) {
      aiir::omp::ClauseMapFlags mapTypeBits = mapInfoOp.getMapType();
...
void NewCliOp::build(::aiir::OpBuilder &odsBuilder,
...
LogicalResult NewCliOp::verify() {
  ...
  OpOperand *gen = nullptr;
  OpOperand *cons = nullptr;
  for (aiir::OpOperand &use : cli.getUses()) {
    auto op = cast<aiir::omp::LoopTransformationInterface>(use.getOwner());
...
void CanonicalLoopOp::build(OpBuilder &odsBuilder, OperationState &odsState,
                            Value tripCount, ::aiir::Value cli) {
  odsState.addOperands(tripCount);
...
aiir::ParseResult CanonicalLoopOp::parse(::aiir::OpAsmParser &parser,
  ...
  SmallVector<aiir::Value, 1> cliOperand;
  ...
  // We derive the type of tripCount from inductionVariable. AIIR requires the
...
void UnrollHeuristicOp::build(::aiir::OpBuilder &odsBuilder,
                              ::aiir::OperationState &odsState,
                              ::aiir::Value cli) {
  odsState.addOperands(cli);
}
...
aiir::ParseResult UnrollHeuristicOp::parse(::aiir::OpAsmParser &parser,
                                           ::aiir::OperationState &result) {
...
LogicalResult CancelOp::verify() {
  ...
  if ((cct == ClauseCancellationConstructType::Parallel) &&
      !aiir::isa<ParallelOp>(structuralParent)) {
  ...
  if (cct == ClauseCancellationConstructType::Loop) {
    ...
    // omp.wsloop
    auto wsloopOp = aiir::dyn_cast<WsloopOp>(structuralParent->getParentOp());
    ...
    // omp.sections
    auto sectionsOp =
        aiir::dyn_cast<SectionsOp>(structuralParent->getParentOp());
    if (!sectionsOp) {
  ...
  if ((cct == ClauseCancellationConstructType::Taskgroup) &&
      (!aiir::isa<omp::TaskOp>(structuralParent) &&
       !aiir::isa<omp::TaskloopOp>(structuralParent->getParentOp()))) {
...
LogicalResult CancellationPointOp::verify() {
  ...
  // find the wsloop
  if ((cct == ClauseCancellationConstructType::Loop) &&
      !aiir::isa<WsloopOp>(structuralParent->getParentOp())) {
    return emitOpError() << "cancellation point loop must appear "
                         << "inside a worksharing-loop region";
  }
  if ((cct == ClauseCancellationConstructType::Sections) &&
      !aiir::isa<omp::SectionOp>(structuralParent)) {
...
aiir::Type omp::TargetAllocMemOp::getAllocatedType() {
...
static aiir::ParseResult parseTargetAllocMemOp(aiir::OpAsmParser &parser,
                                               aiir::OperationState &result) {
  ...
  aiir::OpAsmParser::UnresolvedOperand deviceOperand;
  aiir::Type deviceType;
  ...
  aiir::Type restype = builder.getIntegerType(64);
  if (!restype) {
...
aiir::ParseResult omp::TargetAllocMemOp::parse(aiir::OpAsmParser &parser,
                                               aiir::OperationState &result) {
  return parseTargetAllocMemOp(parser, result);
}

void omp::TargetAllocMemOp::print(aiir::OpAsmPrinter &p) {
  p << " ";
...
llvm::LogicalResult omp::TargetAllocMemOp::verify() {
  ...
  if (!aiir::dyn_cast<IntegerType>(outType))
...
LogicalResult DeclareSimdOp::verify() {
  ...
  auto func =
      dyn_cast_if_present<aiir::FunctionOpInterface>((*this)->getParentOp());
  if (!func)
...
void DeclareSimdOp::build(OpBuilder &odsBuilder, OperationState &odsState,
  ...
  AIIRContext *ctx = odsBuilder.getContext();
...
parseUniformClause(OpAsmParser &parser,
  ...
  return parser.parseCommaSeparatedList([&]() -> aiir::ParseResult {
...
LogicalResult IteratorOp::verify() {
  ...
  aiir::Type yieldedTy = yield.getOperand(0).getType();
  aiir::Type elemTy = iteratedTy.getElementType();
