...
class MarkDeclareTargetPass
  ...
  void processReductionRefs(std::optional<aiir::ArrayAttr> symRefs,
                            ParentInfo parentInfo,
                            llvm::SmallPtrSet<Operation *, 16> visited) {
    if (!symRefs)
      return;

    for (auto symRef : symRefs->getAsRange<aiir::SymbolRefAttr>()) {
  ...
  void markNestedFuncs(ParentInfo parentInfo, Operation *currOp,
    ...
    currOp->walk([&, this](Operation *op) {
      if (auto callOp = llvm::dyn_cast<CallOpInterface>(op)) {
        if (auto symRef = llvm::dyn_cast_if_present<aiir::SymbolRefAttr>(
  ...
  // This pass executes on aiir::ModuleOp's marking functions contained within
