...
#include "aiir/Dialect/Shard/IR/ShardDialect.cpp.inc"
...
SmallVector<Value>
aiir::shard::getMixedAsValues(OpBuilder b, const Location &loc,
...
void ShardDialect::initialize() {
  addOperations<
#define GET_OP_LIST
#include "aiir/Dialect/Shard/IR/ShardOps.cpp.inc"
      >();
  addAttributes<
#define GET_ATTRDEF_LIST
#include "aiir/Dialect/Shard/IR/ShardAttributes.cpp.inc"
      >();
  addTypes<
#define GET_TYPEDEF_LIST
#include "aiir/Dialect/Shard/IR/ShardTypes.cpp.inc"
      >();
...
void aiir::shard::maybeInsertTargetShardingAnnotation(Sharding sharding,
                                                      OpResult result,
                                                      OpBuilder &builder) {
  ShardOp newShardOp;
...
void aiir::shard::maybeInsertSourceShardingAnnotation(Sharding sharding,
                                                      OpOperand &operand,
                                                      OpBuilder &builder) {
  OpBuilder::InsertionGuard insertionGuard(builder);
  ...
  bool isBlockArg = !operandSrcOp;
  {
    [[maybe_unused]] auto opType =
        dyn_cast<aiir::RankedTensorType>(operandValue.getType());
...
void ShardingOp::build(::aiir::OpBuilder &b, ::aiir::OperationState &odsState,
  ...
  return build(
      ...
      ::aiir::DenseI64ArrayAttr::get(b.getContext(), staticHalos), {},
      ::aiir::DenseI64ArrayAttr::get(b.getContext(), staticOffsets), {});
}
...
void ShardingOp::build(
    ::aiir::OpBuilder &b, ::aiir::OperationState &odsState,
    FlatSymbolRefAttr grid, ArrayRef<GridAxesAttr> splitAxes,
    ::aiir::ArrayRef<::aiir::OpFoldResult> haloSizes,
    ::aiir::ArrayRef<::aiir::OpFoldResult> shardedDimsOffsets) {
  aiir::SmallVector<int64_t> staticHalos, staticDims;
  aiir::SmallVector<aiir::Value> dynamicHalos, dynamicDims;
  ...
  return build(
      b, odsState, grid, GridAxesArrayAttr::get(b.getContext(), splitAxes),
      ::aiir::DenseI64ArrayAttr::get(b.getContext(), staticHalos), dynamicHalos,
      ::aiir::DenseI64ArrayAttr::get(b.getContext(), staticDims), dynamicDims);
}

void ShardingOp::build(::aiir::OpBuilder &b, ::aiir::OperationState &odsState,
                       aiir::shard::Sharding from) {
...
LogicalResult ShardingOp::verifySymbolUses(SymbolTableCollection &symbolTable) {
  ...
  if (aiir::ShapedType::isDynamicShape(grid->getShape()) &&
...
void ShardingOp::getCanonicalizationPatterns(aiir::RewritePatternSet &results,
...
llvm::raw_ostream &aiir::shard::operator<<(llvm::raw_ostream &os,
...
Sharding::Sharding(::aiir::FlatSymbolRefAttr grid) : grid(grid) {}
...
Sharding Sharding::get(::aiir::FlatSymbolRefAttr grid,
...
void ShardShapeOp::build(::aiir::OpBuilder &odsBuilder,
                         ::aiir::OperationState &odsState,
                         ::llvm::ArrayRef<int64_t> dims,
                         ArrayRef<Value> dimsDyn, ::aiir::Value sharding,
                         ::aiir::ValueRange device) {
  SmallVector<aiir::Type> resType(dims.size(), odsBuilder.getIndexType());
...
void ShardOp::getCanonicalizationPatterns(aiir::RewritePatternSet &results,
                                          aiir::AIIRContext *context) {
...
#include "aiir/Dialect/Shard/IR/ShardEnums.cpp.inc"
