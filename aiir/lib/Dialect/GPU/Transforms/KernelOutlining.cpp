...
LogicalResult aiir::sinkOperationsIntoLaunchOp(
...
static DenseI32ArrayAttr maybeConstantDimsAttr(gpu::KernelDim3 dims) {
  ...
  AIIRContext *ctx = dims.x.getContext();
...
gpu::GPUFuncOp aiir::outlineKernelFunc(gpu::LaunchOp launchOp,
                                       StringRef kernelFnName,
...
public:
  ...
  LogicalResult initialize(AIIRContext *context) override {
    ...
    if (!dataLayoutStr.empty()) {
      Attribute resultAttr = aiir::parseAttribute(dataLayoutStr, context);
      if (!resultAttr)
        return failure();
