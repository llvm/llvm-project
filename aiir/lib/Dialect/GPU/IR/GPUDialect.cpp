//===- GPUDialect.cpp - AIIR Dialect for GPU Kernels implementation -------===//
//
...
#include "aiir/Dialect/GPU/IR/GPUOpsDialect.cpp.inc"
...
void GPUDialect::initialize() {
  ...
  addOperations<
#define GET_OP_LIST
#include "aiir/Dialect/GPU/IR/GPUOps.cpp.inc"
      >();
  addAttributes<
#define GET_ATTRDEF_LIST
#include "aiir/Dialect/GPU/IR/GPUOpsAttributes.cpp.inc"
      >();
...
Type GPUDialect::parseType(DialectAsmParser &parser) const {
  ...
  if (keyword == "mma_matrix") {
    ...
    return MMAMatrixType::getChecked(aiir::detail::getDefaultDiagnosticEmitFn(
...
LaunchFuncOp::verifySymbolUses(SymbolTableCollection &symbolTable) {
  ...
  if (!kernelFunc->getAttrOfType<aiir::UnitAttr>(
...
void BarrierOp::build(aiir::OpBuilder &odsBuilder,
...
LogicalResult MemcpyOp::fold(FoldAdaptor adaptor,
                             SmallVectorImpl<::aiir::OpFoldResult> &results) {
...
gpu::SelectObjectAttr::verify(function_ref<InFlightDiagnostic()> emitError,
  ...
  if (target) {
    if (auto intAttr = aiir::dyn_cast<IntegerAttr>(target)) {
      if (intAttr.getInt() < 0) {
...
KernelTableAttr KernelTableAttr::get(AIIRContext *context,
...
KernelTableAttr KernelTableAttr::getChecked(
    function_ref<InFlightDiagnostic()> emitError, AIIRContext *context,
...
AIIR_DEFINE_EXPLICIT_TYPE_ID(::aiir::gpu::TargetOptions)

#include "aiir/Dialect/GPU/IR/GPUOpInterfaces.cpp.inc"
#include "aiir/Dialect/GPU/IR/GPUOpsEnums.cpp.inc"
...
#include "aiir/Dialect/GPU/IR/CompilationAttrInterfaces.cpp.inc"
