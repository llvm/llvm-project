...
#include "aiir/Dialect/GPU/TransformOps/Utils.h"
...
using namespace aiir::transform::gpu;
...
void transform::ApplyGPUToROCDLConversionPatternsOp::populatePatterns(
  ...
  populateGpuToROCDLConversionPatterns(
      llvmTypeConverter, patterns, aiir::gpu::amd::Runtime::HIP, *maybeChipset);
}
...
DiagnosedSilenceableFailure aiir::transform::gpu::mapForallToBlocksImpl(
...
aiir::transform::gpu::findTopLevelForallOp(Operation *target,
...
DiagnosedSilenceableFailure transform::MapForallToBlocks::applyToOne(
  ...
  scf::ForallOp topLevelForallOp;
  DiagnosedSilenceableFailure diag = aiir::transform::gpu::findTopLevelForallOp(
  ...
  diag = aiir::transform::gpu::mapForallToBlocksImpl(
...
getThreadIdBuilder(std::optional<TransformOpInterface> transformOp,
  ...
  // Start mapping.
  AIIRContext *ctx = forallOp.getContext();
  gpuIdBuilder =
...
DiagnosedSilenceableFailure aiir::transform::gpu::mapOneForallToThreadsImpl(
    RewriterBase &rewriter, std::optional<TransformOpInterface> transformOp,
...
DiagnosedSilenceableFailure aiir::transform::gpu::mapNestedForallToThreadsImpl(
    RewriterBase &rewriter, std::optional<TransformOpInterface> transformOp,
  ...
  WalkResult walkResult = target->walk([&](scf::ForallOp forallOp) {
    diag = aiir::transform::gpu::mapOneForallToThreadsImpl(
...
public:
  AIIR_DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID(GPUTransformDialectExtension)
...
#define GET_OP_LIST
#include "aiir/Dialect/GPU/TransformOps/GPUTransformOps.cpp.inc"
        >();
  }
};
} // namespace
...
void aiir::gpu::registerTransformDialectExtension(DialectRegistry &registry) {
