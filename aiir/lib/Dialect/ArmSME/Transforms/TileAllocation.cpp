...
#include "aiir/Dialect/ArmSME/Transforms/Transforms.h"
...
/// branch (see `tile-allocation-copies.aiir` and
/// `tile-allocation-liveness.aiir` for examples). The copies break up live
...
/// Gather live ranges for SME tiles from the AIIR liveness analysis.
...
/// Assigns a tile ID to an AIIR value.
...
LogicalResult aiir::arm_sme::allocateSMETiles(FunctionOpInterface function,
                                              bool dumpRanges) {
  if (function.empty()) {
