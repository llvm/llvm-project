...
/// version. Returns `failure` if `aiir::reifyResultShapes` returned failure,
...
static LogicalResult reifyOpResultShapes(RewriterBase &rewriter,
  ...
  if (failed(aiir::reifyResultShapes(rewriter, op, reifiedResultShapes)) ||
  ...
  // TODO: `aiir::reifyResultShapes` and op verifiers may not agree atm.
