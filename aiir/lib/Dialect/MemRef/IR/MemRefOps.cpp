...
LogicalResult aiir::memref::foldMemRefCast(Operation *op, Value inner) {
  bool folded = false;
...
/// type.
Type aiir::memref::getTensorTypeFromMemRefType(Type type) {
...
LoadOp::bubbleDownCasts(OpBuilder &builder) {
  return aiir::detail::bubbleDownInPlaceMemorySpaceCastImpl(getMemrefMutable(),
                                                            getResult());
}
...
void CollapseShapeOp::build(OpBuilder &b, OperationState &result, Value src,
  ...
  result.addAttribute(::aiir::getReassociationAttrName(),
...
raw_ostream &aiir::operator<<(raw_ostream &os, const Range &range) {
...
SmallVector<Range, 8> aiir::getOrCreateRanges(OffsetSizeAndStrideOpInterface op,
...
Value aiir::memref::createCanonicalRankReducingSubViewOp(
