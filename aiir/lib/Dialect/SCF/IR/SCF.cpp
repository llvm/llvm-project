...
#include "aiir/Dialect/SCF/IR/SCFOpsDialect.cpp.inc"
...
void SCFDialect::initialize() {
  addOperations<
#define GET_OP_LIST
#include "aiir/Dialect/SCF/IR/SCFOps.cpp.inc"
      >();
...
void aiir::scf::buildTerminatedBody(OpBuilder &builder, Location loc) {
...
std::optional<llvm::APSInt> aiir::scf::computeUbMinusLb(Value lb, Value ub,
                                                        bool isSigned) {
  llvm::APSInt diff;
...
void ExecuteRegionOp::getCanonicalizationPatterns(RewritePatternSet &results,
  ...
  populateRegionBranchOpInterfaceInliningPattern(
      results, ExecuteRegionOp::getOperationName(),
      aiir::detail::defaultReplBuilderFn, [](Operation *op) {
...
ForOp aiir::scf::getForInductionVarOwner(Value val) {
...
void aiir::scf::promote(RewriterBase &rewriter, scf::ForallOp forallOp) {
  OpBuilder::InsertionGuard g(rewriter);
...
LoopNest aiir::scf::buildLoopNest(
...
SmallVector<Value>
aiir::scf::replaceAndCastForOpIterArg(RewriterBase &rewriter, scf::ForOp forOp,
...
void ForallOp::build(
    aiir::OpBuilder &b, aiir::OperationState &result,
...
ForallOp aiir::scf::getForallOpThreadIndexOwner(Value val) {
...
bool aiir::scf::insideMutuallyExclusiveBranches(Operation *a, Operation *b) {
...
LogicalResult
IfOp::inferReturnTypes(AIIRContext *ctx, std::optional<Location> loc,
...
struct ConditionPropagation : public OpRewritePattern<IfOp> {
  ...
  LogicalResult matchAndRewrite(IfOp op,
    ...
    bool changed = false;
    aiir::Type i1Ty = rewriter.getI1Type();
...
ParallelOp aiir::scf::getParallelForInductionVarOwner(Value val) {
  auto ivArg = llvm::dyn_cast<BlockArgument>(val);
  if (!ivArg)
    return ParallelOp();
  assert(ivArg.getOwner() && "unlinked block argument");
...
void WhileOp::build(::aiir::OpBuilder &odsBuilder,
                    ::aiir::OperationState &odsState, TypeRange resultTypes,
