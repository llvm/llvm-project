...
SmallVector<scf::ForOp> aiir::replaceLoopNestWithNewYields(
  ...
  // https://discourse.llvm.org/t/rfc-update-to-aiir-developer-policy-on-recursion/62235)
...
FailureOr<func::FuncOp> aiir::outlineSingleBlockRegion(RewriterBase &rewriter,
                                                       Location loc,
                                                       Region &region,
                                                       StringRef funcName,
                                                       func::CallOp *callOp) {
...
LogicalResult aiir::outlineIfOp(RewriterBase &b, scf::IfOp ifOp,
...
bool aiir::getInnermostParallelLoops(Operation *rootOp,
...
void aiir::generateUnrolledLoop(
...
FailureOr<UnrolledLoopInfo> aiir::loopUnrollByFactor(
...
LogicalResult aiir::loopUnrollFull(scf::ForOp forOp) {
  IRRewriter rewriter(forOp.getContext());
...
LogicalResult aiir::loopUnrollJamByFactor(scf::ForOp forOp,
...
Range aiir::emitNormalizedLoopBounds(RewriterBase &rewriter, Location loc,
...
void aiir::denormalizeInductionVariable(RewriterBase &rewriter, Location loc,
...
LogicalResult aiir::coalesceLoops(RewriterBase &rewriter,
...
LogicalResult aiir::coalesceLoops(MutableArrayRef<scf::ForOp> loops) {
  if (loops.empty()) {
    return failure();
  }
...
LogicalResult aiir::coalescePerfectlyNestedSCFForLoops(scf::ForOp op) {
...
void aiir::collapseParallelLoops(
...
SmallVector<Loops, 8> aiir::tile(ArrayRef<scf::ForOp> forOps,
...
Loops aiir::tile(ArrayRef<scf::ForOp> forOps, ArrayRef<Value> sizes,
                 scf::ForOp target) {
...
Loops aiir::tilePerfectlyNested(scf::ForOp rootForOp, ArrayRef<Value> sizes) {
...
void aiir::getPerfectlyNestedLoops(SmallVectorImpl<scf::ForOp> &nestedLoops,
                                   scf::ForOp root) {
  getPerfectlyNestedLoopsImpl(nestedLoops, root);
}

TileLoops aiir::extractFixedOuterLoops(scf::ForOp rootForOp,
...
scf::ForallOp aiir::fuseIndependentSiblingForallLoops(scf::ForallOp target,
                                                      scf::ForallOp source,
...
scf::ForOp aiir::fuseIndependentSiblingForLoops(scf::ForOp target,
                                                scf::ForOp source,
                                                RewriterBase &rewriter) {
...
FailureOr<scf::ForallOp> aiir::normalizeForallOp(RewriterBase &rewriter,
...
bool aiir::isPerfectlyNestedForLoops(
...
aiir::getConstLoopBounds(aiir::LoopLikeOpInterface loopOp) {
...
aiir::getConstLoopTripCounts(aiir::LoopLikeOpInterface loopOp) {
  std::optional<SmallVector<OpFoldResult>> loBnds = loopOp.getLoopLowerBounds();
  std::optional<SmallVector<OpFoldResult>> upBnds = loopOp.getLoopUpperBounds();
  std::optional<SmallVector<OpFoldResult>> steps = loopOp.getLoopSteps();
  if (!loBnds || !upBnds || !steps)
...
FailureOr<scf::ParallelOp> aiir::parallelLoopUnrollByFactors(
