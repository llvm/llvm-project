...
static bool haveNoDataDependenciesExceptSameIndex(
  ...
  auto checkLoadInWalkHasNoIncompatibleDataDeps = [&](Operation *loadOp) {
    ...
    if (!memOpInterf &&
        !loadOp->hasTrait<aiir::OpTrait::HasRecursiveMemoryEffects>())
      return WalkResult::advance();
    ...
    SmallVector<aiir::Operation *> &writeOps = writeOpsIt->second;
...
void aiir::scf::naivelyFuseParallelOps(
...
std::unique_ptr<Pass> aiir::createParallelLoopFusionPass() {
