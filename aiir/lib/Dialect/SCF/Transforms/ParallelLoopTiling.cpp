...
aiir::scf::tileParallelLoop(ParallelOp op, ArrayRef<int64_t> tileSizes,
                            bool noMinMaxBounds) {
  OpBuilder b(op);
...
struct ParallelLoopTiling
  ...
  void runOnOperation() override {
    for (auto tileSize : tileSizes)
      if (tileSize == 0) {
        aiir::emitError(aiir::UnknownLoc::get(&Pass::getContext()),
...
std::unique_ptr<Pass>
aiir::createParallelLoopTilingPass(ArrayRef<int64_t> tileSizes,
                                   bool noMinMaxBounds) {
