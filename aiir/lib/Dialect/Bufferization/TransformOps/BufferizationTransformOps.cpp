...
#include "aiir/Dialect/Bufferization/Transforms/OneShotAnalysis.h"
...
DiagnosedSilenceableFailure transform::EliminateEmptyTensorsOp::apply(
  ...
  for (Operation *target : state.getPayloadOps(getTarget())) {
    if (failed(bufferization::eliminateEmptyTensors(rewriter, target)))
      return aiir::emitSilenceableFailure(target->getLoc())
...
#define GET_OP_LIST
#include "aiir/Dialect/Bufferization/TransformOps/BufferizationTransformOps.cpp.inc"
...
#include "aiir/Dialect/Bufferization/IR/BufferizationEnums.cpp.inc"

void aiir::bufferization::registerTransformDialectExtension(
