...
using namespace aiir::bufferization::func_ext;
...
static LogicalResult getFuncOpsOrderedByCalls(
  ...
  for (aiir::Region &region : moduleOp->getRegions()) {
...
LogicalResult
aiir::bufferization::analyzeModuleOp(Operation *moduleOp,
...
void aiir::bufferization::removeBufferizationAttributesInModule(
    Operation *moduleOp) {
  for (aiir::Region &region : moduleOp->getRegions()) {
    for (aiir::Block &block : region.getBlocks()) {
      for (func::FuncOp funcOp : block.getOps<func::FuncOp>()) {
...
LogicalResult aiir::bufferization::bufferizeModuleOp(
...
LogicalResult aiir::bufferization::runOneShotModuleBufferize(
    Operation *moduleOp, const OneShotBufferizationOptions &options,
    BufferizationState &state, BufferizationStatistics *statistics) {
  assert(options.bufferizeFunctionBoundaries &&
         "expected that function boundary bufferization is activated");
