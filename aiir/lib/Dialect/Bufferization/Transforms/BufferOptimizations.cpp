...
static bool defaultIsSmallAlloc(Value alloc, unsigned maximumSizeInBytes,
  ...
  if (!elemType.isIntOrFloat() &&
    ...
    return false;
  unsigned bitwidth = aiir::DataLayout::closest(alloc.getDefiningOp())
...
void aiir::bufferization::hoistBuffersFromLoops(Operation *op) {
  BufferAllocationHoisting<BufferAllocationLoopHoistingState> optimizer(op);
  optimizer.hoist();
}

std::unique_ptr<Pass> aiir::bufferization::createPromoteBuffersToStackPass(
    std::function<bool(Value)> isSmallAlloc) {
