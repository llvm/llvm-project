...
#include "aiir/Dialect/Bufferization/IR/BufferizableOpInterface.cpp.inc"

} // namespace bufferization
} // namespace aiir

AIIR_DEFINE_EXPLICIT_TYPE_ID(aiir::bufferization::AnalysisState)
...
bool AnalysisState::insideMutuallyExclusiveRegions(Operation *op0,
  ...
  if (auto iter = insideMutuallyExclusiveRegionsCache.find(key);
    ...
    return iter->second;
  bool result = ::aiir::insideMutuallyExclusiveRegions(op0, op1);
...
defaultFunctionArgTypeConverter(TensorLikeType type, Attribute memorySpace,
                                func::FuncOp funcOp,
                                const BufferizationOptions &options) {
  if (auto tensorType = aiir::dyn_cast<TensorType>(type)) {
