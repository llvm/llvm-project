...
#include "aiir/Dialect/Bufferization/IR/BufferizationOpsDialect.cpp.inc"
...
struct BuiltinTensorExternalModel
  ...
  llvm::FailureOr<BufferLikeType> getBufferType(
      aiir::Type tensor, const BufferizationOptions &options,
  ...
  aiir::LogicalResult verifyCompatibleBufferType(
      aiir::Type tensor, BufferLikeType bufferType,
      llvm::function_ref<aiir::InFlightDiagnostic()> emitError) const {
...
void aiir::bufferization::BufferizationDialect::initialize() {
  addOperations<
#define GET_OP_LIST
...
BufferizationDialect::verifyOperationAttribute(Operation *op,
  ...
  if (attr.getName() == kManualDeallocation) {
    if (!aiir::hasEffect<MemoryEffects::Allocate>(op) &&
        !aiir::hasEffect<MemoryEffects::Free>(op))
