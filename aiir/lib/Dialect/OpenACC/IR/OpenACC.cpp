//===- OpenACC.cpp - OpenACC AIIR Operations ------------------------------===//
//
...
#include "aiir/Dialect/OpenACC/OpenACCOpsDialect.cpp.inc"
#include "aiir/Dialect/OpenACC/OpenACCOpsEnums.cpp.inc"
#include "aiir/Dialect/OpenACC/OpenACCOpsInterfaces.cpp.inc"
#include "aiir/Dialect/OpenACC/OpenACCTypeInterfaces.cpp.inc"
#include "aiir/Dialect/OpenACCMPCommon/Interfaces/OpenACCMPOpsInterfaces.cpp.inc"
...
struct MemRefPointerLikeModel
  ...
  aiir::Value genAllocate(Type pointer, OpBuilder &builder, Location loc,
  ...
  aiir::Value genLoad(Type pointer, OpBuilder &builder, Location loc,
...
/// list is empty).
aiir::ArrayAttr addDeviceTypeAffectedOperandHelper(
    AIIRContext *context, aiir::ArrayAttr existingDeviceTypes,
    llvm::ArrayRef<acc::DeviceType> newDeviceTypes) {
  llvm::SmallVector<aiir::Attribute> deviceTypes;
  ...
  return aiir::ArrayAttr::get(context, deviceTypes);
}
...
aiir::ArrayAttr addDeviceTypeAffectedOperandHelper(
    AIIRContext *context, aiir::ArrayAttr existingDeviceTypes,
    llvm::ArrayRef<acc::DeviceType> newDeviceTypes, aiir::ValueRange arguments,
    aiir::MutableOperandRange argCollection,
...
void OpenACCDialect::initialize() {
  addOperations<
#define GET_OP_LIST
#include "aiir/Dialect/OpenACC/OpenACCOps.cpp.inc"
      >();
  addAttributes<
#define GET_ATTRDEF_LIST
#include "aiir/Dialect/OpenACC/OpenACCOpsAttributes.cpp.inc"
      >();
  addTypes<
#define GET_TYPEDEF_LIST
#include "aiir/Dialect/OpenACC/OpenACCOpsTypes.cpp.inc"
      >();
...
static bool hasDeviceTypeValues(std::optional<aiir::ArrayAttr> arrayAttr) {
  return arrayAttr && *arrayAttr && arrayAttr->size() > 0;
}

static bool hasDeviceType(std::optional<aiir::ArrayAttr> arrayAttr,
                          aiir::acc::DeviceType deviceType) {
  if (!hasDeviceTypeValues(arrayAttr))
    return false;

  for (auto attr : *arrayAttr) {
    auto deviceTypeAttr = aiir::dyn_cast<aiir::acc::DeviceTypeAttr>(attr);
...
static void printDeviceTypes(aiir::OpAsmPrinter &p,
                             std::optional<aiir::ArrayAttr> deviceTypes) {
  if (!hasDeviceTypeValues(deviceTypes))
    return;

  p << "[";
  llvm::interleaveComma(*deviceTypes, p,
                        [&](aiir::Attribute attr) { p << attr; });
  p << "]";
}
...
static aiir::Operation::operand_range
getValuesFromSegments(std::optional<aiir::ArrayAttr> arrayAttr,
                      aiir::Operation::operand_range range,
...
static aiir::Value
getWaitDevnumValue(std::optional<aiir::ArrayAttr> deviceTypeAttr,
                   aiir::Operation::operand_range operands,
                   std::optional<llvm::ArrayRef<int32_t>> segments,
                   std::optional<aiir::ArrayAttr> hasWaitDevnum,
                   aiir::acc::DeviceType deviceType) {
...
static aiir::Operation::operand_range
getWaitValuesWithoutDevnum(std::optional<aiir::ArrayAttr> deviceTypeAttr,
                           aiir::Operation::operand_range operands,
                           std::optional<llvm::ArrayRef<int32_t>> segments,
                           std::optional<aiir::ArrayAttr> hasWaitDevnum,
                           aiir::acc::DeviceType deviceType) {
  ...
  if (auto pos = findSegment(*deviceTypeAttr, deviceType)) {
    if (hasWaitDevnum && *hasWaitDevnum) {
      auto boolAttr = aiir::dyn_cast<aiir::BoolAttr>((*hasWaitDevnum)[*pos]);
...
static LogicalResult checkVarAndVarType(Op op) {
  ...
  if (!aiir::isa<aiir::acc::PointerLikeType>(op.getVar().getType()) &&
      !aiir::isa<aiir::acc::MappableType>(op.getVar().getType()))
...
static LogicalResult checkRecipe(OpT op, llvm::StringRef operandName) {
  ...
  // at this time.
  if (aiir::acc::isMappableType(op.getVar().getType()) &&
      !std::is_same_v<OpT, acc::ReductionOp>)
    return success();

  aiir::SymbolRefAttr operandRecipe = op.getRecipeAttr();
  if (!operandRecipe)
...
static ParseResult parseVar(aiir::OpAsmParser &parser,
...
static void printVar(aiir::OpAsmPrinter &p, aiir::Operation *op,
                     aiir::Value var) {
  if (aiir::isa<aiir::acc::PointerLikeType>(var.getType()))
    p << "varPtr(";
  else
    p << "var(";
  p.printOperand(var);
...
static ParseResult parseAccVar(aiir::OpAsmParser &parser,
                               OpAsmParser::UnresolvedOperand &var,
                               aiir::Type &accVarType) {
...
static void printAccVar(aiir::OpAsmPrinter &p, aiir::Operation *op,
                        aiir::Value accVar, aiir::Type accVarType) {
  if (aiir::isa<aiir::acc::PointerLikeType>(accVar.getType()))
    p << "accPtr(";
  else
    p << "accVar(";
...
static ParseResult parseVarPtrType(aiir::OpAsmParser &parser,
                                   aiir::Type &varPtrType,
                                   aiir::TypeAttr &varTypeAttr) {
  ...
  if (succeeded(parser.parseOptionalKeyword("varType"))) {
    ...
    if (failed(parser.parseType(varType)))
      return failure();
    varTypeAttr = aiir::TypeAttr::get(varType);
    if (failed(parser.parseRParen()))
      return failure();
  } else {
    ...
    if (auto ptrTy = dyn_cast<acc::PointerLikeType>(varPtrType)) {
      ...
      varTypeAttr = aiir::TypeAttr::get(elementType ? elementType : varPtrType);
    } else {
      varTypeAttr = aiir::TypeAttr::get(varPtrType);
    }
  }
...
static void printVarPtrType(aiir::OpAsmPrinter &p, aiir::Operation *op,
                            aiir::Type varPtrType, aiir::TypeAttr varTypeAttr) {
  p.printType(varPtrType);
  p << ")";

  // Print the `varType` only if it differs from the element type of
  // `varPtr`'s type.
  aiir::Type varType = varTypeAttr.getValue();
  aiir::Type typeToCheckAgainst =
      aiir::isa<aiir::acc::PointerLikeType>(varPtrType)
          ? aiir::cast<aiir::acc::PointerLikeType>(varPtrType).getElementType()
          : varPtrType;
...
static ParseResult parseRecipeSym(aiir::OpAsmParser &parser,
                                  aiir::SymbolRefAttr &recipeAttr) {
  if (failed(parser.parseAttribute(recipeAttr)))
    return failure();
  return success();
}

static void printRecipeSym(aiir::OpAsmPrinter &p, aiir::Operation *op,
                           aiir::SymbolRefAttr recipeAttr) {
  p << recipeAttr;
}
...
static bool isEnclosedIntoComputeOp(aiir::Operation *op) {
...
static void addResultEffect(
    SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>
        &effects,
    Value result) {
  effects.emplace_back(EffectTy::get(), aiir::cast<aiir::OpResult>(result));
}
...
static LogicalResult checkDataOperands(Op op,
                                       const aiir::ValueRange &operands) {
  for (aiir::Value operand : operands)
    if (!aiir::isa<acc::AttachOp, acc::CopyinOp, acc::CopyoutOp, acc::CreateOp,
...
template <typename OpT, typename RecipeOpT>
static LogicalResult checkPrivateOperands(aiir::Operation *accConstructOp,
                                          const aiir::ValueRange &operands,
                                          llvm::StringRef operandName) {
  llvm::DenseSet<Value> set;
  for (aiir::Value operand : operands) {
    if (!aiir::isa<OpT>(operand.getDefiningOp()))
...
LogicalResult acc::ParallelOp::verify() {
  if (failed(checkPrivateOperands<aiir::acc::PrivateOp,
                                  aiir::acc::PrivateRecipeOp>(
          *this, getPrivateOperands(), "private")))
    return failure();
  if (failed(checkPrivateOperands<aiir::acc::FirstprivateOp,
                                  aiir::acc::FirstprivateRecipeOp>(
          *this, getFirstprivateOperands(), "firstprivate")))
    return failure();
  if (failed(checkPrivateOperands<aiir::acc::ReductionOp,
...
static aiir::Value
getValueInDeviceTypeSegment(std::optional<aiir::ArrayAttr> arrayAttr,
                            aiir::Operation::operand_range range,
                            aiir::acc::DeviceType deviceType) {
  if (!arrayAttr)
    return {};
...
bool acc::ParallelOp::hasAsyncOnly() {
  return hasAsyncOnly(aiir::acc::DeviceType::None);
}

bool acc::ParallelOp::hasAsyncOnly(aiir::acc::DeviceType deviceType) {
  return hasDeviceType(getAsyncOnly(), deviceType);
}

aiir::Value acc::ParallelOp::getAsyncValue() {
  return getAsyncValue(aiir::acc::DeviceType::None);
}

aiir::Value acc::ParallelOp::getAsyncValue(aiir::acc::DeviceType deviceType) {
  return getValueInDeviceTypeSegment(getAsyncOperandsDeviceType(),
                                     getAsyncOperands(), deviceType);
}

aiir::Value acc::ParallelOp::getNumWorkersValue() {
  return getNumWorkersValue(aiir::acc::DeviceType::None);
}

aiir::Value
acc::ParallelOp::getNumWorkersValue(aiir::acc::DeviceType deviceType) {
  return getValueInDeviceTypeSegment(getNumWorkersDeviceType(), getNumWorkers(),
                                     deviceType);
}

aiir::Value acc::ParallelOp::getVectorLengthValue() {
  return getVectorLengthValue(aiir::acc::DeviceType::None);
}

aiir::Value
acc::ParallelOp::getVectorLengthValue(aiir::acc::DeviceType deviceType) {
  return getValueInDeviceTypeSegment(getVectorLengthDeviceType(),
                                     getVectorLength(), deviceType);
}

aiir::Operation::operand_range ParallelOp::getNumGangsValues() {
  return getNumGangsValues(aiir::acc::DeviceType::None);
}

aiir::Operation::operand_range
ParallelOp::getNumGangsValues(aiir::acc::DeviceType deviceType) {
...
bool acc::ParallelOp::hasWaitOnly() {
  return hasWaitOnly(aiir::acc::DeviceType::None);
}

bool acc::ParallelOp::hasWaitOnly(aiir::acc::DeviceType deviceType) {
  return hasDeviceType(getWaitOnly(), deviceType);
}

aiir::Operation::operand_range ParallelOp::getWaitValues() {
  return getWaitValues(aiir::acc::DeviceType::None);
}

aiir::Operation::operand_range
ParallelOp::getWaitValues(aiir::acc::DeviceType deviceType) {
...
aiir::Value ParallelOp::getWaitDevnum() {
  return getWaitDevnum(aiir::acc::DeviceType::None);
}

aiir::Value ParallelOp::getWaitDevnum(aiir::acc::DeviceType deviceType) {
  return getWaitDevnumValue(getWaitOperandsDeviceType(), getWaitOperands(),
                            getWaitOperandsSegments(), getHasWaitDevnum(),
                            deviceType);
}

void ParallelOp::build(aiir::OpBuilder &odsBuilder,
                       aiir::OperationState &odsState,
                       aiir::ValueRange numGangs, aiir::ValueRange numWorkers,
                       aiir::ValueRange vectorLength,
                       aiir::ValueRange asyncOperands,
                       aiir::ValueRange waitOperands, aiir::Value ifCond,
                       aiir::Value selfCond, aiir::ValueRange reductionOperands,
                       aiir::ValueRange gangPrivateOperands,
                       aiir::ValueRange gangFirstPrivateOperands,
                       aiir::ValueRange dataClauseOperands) {
  ParallelOp::build(
...
void acc::ParallelOp::addNumWorkersOperand(
    AIIRContext *context, aiir::Value newValue,
...
void acc::ParallelOp::addAsyncOnly(
    AIIRContext *context, llvm::ArrayRef<DeviceType> effectiveDeviceTypes) {
...
void acc::ParallelOp::addNumGangsOperands(
    AIIRContext *context, aiir::ValueRange newValues,
    llvm::ArrayRef<DeviceType> effectiveDeviceTypes) {
  llvm::SmallVector<int32_t> segments;
...
void acc::ParallelOp::addWaitOperands(
    AIIRContext *context, bool hasDevnum, aiir::ValueRange newValues,
    llvm::ArrayRef<DeviceType> effectiveDeviceTypes) {
  ...
  llvm::SmallVector<aiir::Attribute> hasDevnums;
  ...
  hasDevnums.insert(
      ...
      aiir::BoolAttr::get(context, hasDevnum));
  setHasWaitDevnumAttr(aiir::ArrayAttr::get(context, hasDevnums));
}

void acc::ParallelOp::addPrivatization(AIIRContext *context,
                                       aiir::acc::PrivateOp op,
                                       aiir::acc::PrivateRecipeOp recipe) {
  op.setRecipeAttr(aiir::SymbolRefAttr::get(context, recipe.getSymName()));
  getPrivateOperandsMutable().append(op.getResult());
}

void acc::ParallelOp::addFirstPrivatization(
    AIIRContext *context, aiir::acc::FirstprivateOp op,
    aiir::acc::FirstprivateRecipeOp recipe) {
  op.setRecipeAttr(aiir::SymbolRefAttr::get(context, recipe.getSymName()));
  getFirstprivateOperandsMutable().append(op.getResult());
}

void acc::ParallelOp::addReduction(AIIRContext *context,
                                   aiir::acc::ReductionOp op,
                                   aiir::acc::ReductionRecipeOp recipe) {
  op.setRecipeAttr(aiir::SymbolRefAttr::get(context, recipe.getSymName()));
...
static ParseResult parseNumGangs(
    ...
    llvm::SmallVectorImpl<aiir::OpAsmParser::UnresolvedOperand> &operands,
    llvm::SmallVectorImpl<Type> &types, aiir::ArrayAttr &deviceTypes,
    aiir::DenseI32ArrayAttr &segments) {
  ...
  do {
    ...
    if (succeeded(parser.parseOptionalLSquare())) {
      ...
      attributes.push_back(aiir::acc::DeviceTypeAttr::get(
          parser.getContext(), aiir::acc::DeviceType::None));
    }
...
static void printSingleDeviceType(aiir::OpAsmPrinter &p, aiir::Attribute attr) {
  auto deviceTypeAttr = aiir::dyn_cast<aiir::acc::DeviceTypeAttr>(attr);
  if (deviceTypeAttr.getValue() != aiir::acc::DeviceType::None)
    p << " [" << attr << "]";
}

static void printNumGangs(aiir::OpAsmPrinter &p, aiir::Operation *op,
                          aiir::OperandRange operands, aiir::TypeRange types,
                          std::optional<aiir::ArrayAttr> deviceTypes,
                          std::optional<aiir::DenseI32ArrayAttr> segments) {
  unsigned opIdx = 0;
...
static void printDeviceTypeOperandsWithSegment(
    aiir::OpAsmPrinter &p, aiir::Operation *op, aiir::OperandRange operands,
    aiir::TypeRange types, std::optional<aiir::ArrayAttr> deviceTypes,
    std::optional<aiir::DenseI32ArrayAttr> segments) {
  unsigned opIdx = 0;
  llvm::interleaveComma(llvm::enumerate(*deviceTypes), p, [&](auto it) {
    p << "{";
...
static ParseResult parseWaitClause(
    ...
    llvm::SmallVectorImpl<aiir::OpAsmParser::UnresolvedOperand> &operands,
    llvm::SmallVectorImpl<Type> &types, aiir::ArrayAttr &deviceTypes,
    aiir::DenseI32ArrayAttr &segments, aiir::ArrayAttr &hasDevNum,
    aiir::ArrayAttr &keywordOnly) {
  llvm::SmallVector<aiir::Attribute> deviceTypeAttrs, keywordAttrs, devnum;
  llvm::SmallVector<int32_t> seg;
  ...
  if (failed(parser.parseOptionalLParen())) {
    keywordAttrs.push_back(aiir::acc::DeviceTypeAttr::get(
        parser.getContext(), aiir::acc::DeviceType::None));
  ...
  do {
    ...
    if (succeeded(parser.parseOptionalLSquare())) {
      ...
      deviceTypeAttrs.push_back(aiir::acc::DeviceTypeAttr::get(
          parser.getContext(), aiir::acc::DeviceType::None));
    }
  } while (succeeded(parser.parseOptionalComma()));
...
static bool hasOnlyDeviceTypeNone(std::optional<aiir::ArrayAttr> attrs) {
  if (!hasDeviceTypeValues(attrs))
    return false;
  if (attrs->size() != 1)
    return false;
  if (auto deviceTypeAttr =
          aiir::dyn_cast<aiir::acc::DeviceTypeAttr>((*attrs)[0]))
    return deviceTypeAttr.getValue() == aiir::acc::DeviceType::None;
  return false;
}

static void printWaitClause(aiir::OpAsmPrinter &p, aiir::Operation *op,
                            aiir::OperandRange operands, aiir::TypeRange types,
                            std::optional<aiir::ArrayAttr> deviceTypes,
                            std::optional<aiir::DenseI32ArrayAttr> segments,
                            std::optional<aiir::ArrayAttr> hasDevNum,
                            std::optional<aiir::ArrayAttr> keywordOnly) {
  ...
  if (hasDeviceTypeValues(deviceTypes)) {
    ...
    llvm::interleaveComma(llvm::enumerate(*deviceTypes), p, [&](auto it) {
      p << "{";
      auto boolAttr = aiir::dyn_cast<aiir::BoolAttr>((*hasDevNum)[it.index()]);
...
static void
printDeviceTypeOperands(aiir::OpAsmPrinter &p, aiir::Operation *op,
                        aiir::OperandRange operands, aiir::TypeRange types,
                        std::optional<aiir::ArrayAttr> deviceTypes) {
  if (!hasDeviceTypeValues(deviceTypes))
    return;
...
static ParseResult parseDeviceTypeOperandsWithKeywordOnly(
    ...
    llvm::SmallVectorImpl<aiir::OpAsmParser::UnresolvedOperand> &operands,
    llvm::SmallVectorImpl<Type> &types, aiir::ArrayAttr &deviceTypes,
    aiir::ArrayAttr &keywordOnlyDeviceType) {

  llvm::SmallVector<aiir::Attribute> keywordOnlyDeviceTypeAttributes;
  bool needCommaBeforeOperands = false;

  if (failed(parser.parseOptionalLParen())) {
    // Keyword only
    keywordOnlyDeviceTypeAttributes.push_back(aiir::acc::DeviceTypeAttr::get(
        parser.getContext(), aiir::acc::DeviceType::None));
...
static void printDeviceTypeOperandsWithKeywordOnly(
    aiir::OpAsmPrinter &p, aiir::Operation *op, aiir::OperandRange operands,
    aiir::TypeRange types, std::optional<aiir::ArrayAttr> deviceTypes,
    std::optional<aiir::ArrayAttr> keywordOnlyDeviceTypes) {
...
static ParseResult parseOperandWithKeywordOnly(
    ...
    aiir::Type &operandType, aiir::UnitAttr &attr) {
  // Keyword only
  if (failed(parser.parseOptionalLParen())) {
    attr = aiir::UnitAttr::get(parser.getContext());
    return success();
  }
...
static void printOperandWithKeywordOnly(aiir::OpAsmPrinter &p,
                                        aiir::Operation *op,
                                        std::optional<aiir::Value> operand,
...
static ParseResult parseOperandsWithKeywordOnly(
    aiir::OpAsmParser &parser,
    llvm::SmallVectorImpl<aiir::OpAsmParser::UnresolvedOperand> &operands,
    llvm::SmallVectorImpl<Type> &types, aiir::UnitAttr &attr) {
  // Keyword only
  if (failed(parser.parseOptionalLParen())) {
    attr = aiir::UnitAttr::get(parser.getContext());
    return success();
...
static void printOperandsWithKeywordOnly(aiir::OpAsmPrinter &p,
                                         aiir::Operation *op,
                                         aiir::OperandRange operands,
                                         aiir::TypeRange types,
                                         aiir::UnitAttr attr) {
  if (attr)
    return;
...
static ParseResult
parseCombinedConstructsLoop(aiir::OpAsmParser &parser,
                            aiir::acc::CombinedConstructsTypeAttr &attr) {
  if (succeeded(parser.parseOptionalKeyword("kernels"))) {
    attr = aiir::acc::CombinedConstructsTypeAttr::get(
        parser.getContext(), aiir::acc::CombinedConstructsType::KernelsLoop);
  } else if (succeeded(parser.parseOptionalKeyword("parallel"))) {
    attr = aiir::acc::CombinedConstructsTypeAttr::get(
        parser.getContext(), aiir::acc::CombinedConstructsType::ParallelLoop);
  } else if (succeeded(parser.parseOptionalKeyword("serial"))) {
    attr = aiir::acc::CombinedConstructsTypeAttr::get(
        parser.getContext(), aiir::acc::CombinedConstructsType::SerialLoop);
  } else {
...
static void
printCombinedConstructsLoop(aiir::OpAsmPrinter &p, aiir::Operation *op,
                            aiir::acc::CombinedConstructsTypeAttr attr) {
  if (attr) {
    switch (attr.getValue()) {
    case aiir::acc::CombinedConstructsType::KernelsLoop:
      p << "kernels";
      break;
    case aiir::acc::CombinedConstructsType::ParallelLoop:
      p << "parallel";
      break;
    case aiir::acc::CombinedConstructsType::SerialLoop:
      p << "serial";
      break;
    };
  }
...
bool acc::SerialOp::hasAsyncOnly(aiir::acc::DeviceType deviceType) {
  return hasDeviceType(getAsyncOnly(), deviceType);
}

aiir::Value acc::SerialOp::getAsyncValue() {
  return getAsyncValue(aiir::acc::DeviceType::None);
}

aiir::Value acc::SerialOp::getAsyncValue(aiir::acc::DeviceType deviceType) {
  return getValueInDeviceTypeSegment(getAsyncOperandsDeviceType(),
                                     getAsyncOperands(), deviceType);
}
...
bool acc::SerialOp::hasWaitOnly(aiir::acc::DeviceType deviceType) {
  return hasDeviceType(getWaitOnly(), deviceType);
}

aiir::Operation::operand_range SerialOp::getWaitValues() {
  return getWaitValues(aiir::acc::DeviceType::None);
}

aiir::Operation::operand_range
SerialOp::getWaitValues(aiir::acc::DeviceType deviceType) {
  return getWaitValuesWithoutDevnum(
      getWaitOperandsDeviceType(), getWaitOperands(), getWaitOperandsSegments(),
      getHasWaitDevnum(), deviceType);
}

aiir::Value SerialOp::getWaitDevnum() {
  return getWaitDevnum(aiir::acc::DeviceType::None);
}

aiir::Value SerialOp::getWaitDevnum(aiir::acc::DeviceType deviceType) {
  return getWaitDevnumValue(getWaitOperandsDeviceType(), getWaitOperands(),
                            getWaitOperandsSegments(), getHasWaitDevnum(),
                            deviceType);
}
...
void acc::SerialOp::addPrivatization(AIIRContext *context,
                                     aiir::acc::PrivateOp op,
                                     aiir::acc::PrivateRecipeOp recipe) {
  op.setRecipeAttr(aiir::SymbolRefAttr::get(context, recipe.getSymName()));
  getPrivateOperandsMutable().append(op.getResult());
...
void acc::SerialOp::addReduction(AIIRContext *context,
                                 aiir::acc::ReductionOp op,
                                 aiir::acc::ReductionRecipeOp recipe) {
  op.setRecipeAttr(aiir::SymbolRefAttr::get(context, recipe.getSymName()));
  getReductionOperandsMutable().append(op.getResult());
...
bool acc::KernelsOp::hasAsyncOnly(aiir::acc::DeviceType deviceType) {
  return hasDeviceType(getAsyncOnly(), deviceType);
}

aiir::Value acc::KernelsOp::getAsyncValue() {
  return getAsyncValue(aiir::acc::DeviceType::None);
}

aiir::Value acc::KernelsOp::getAsyncValue(aiir::acc::DeviceType deviceType) {
  return getValueInDeviceTypeSegment(getAsyncOperandsDeviceType(),
                                     getAsyncOperands(), deviceType);
}

aiir::Value acc::KernelsOp::getNumWorkersValue() {
  return getNumWorkersValue(aiir::acc::DeviceType::None);
}

aiir::Value
acc::KernelsOp::getNumWorkersValue(aiir::acc::DeviceType deviceType) {
  return getValueInDeviceTypeSegment(getNumWorkersDeviceType(), getNumWorkers(),
                                     deviceType);
}

aiir::Value acc::KernelsOp::getVectorLengthValue() {
  return getVectorLengthValue(aiir::acc::DeviceType::None);
}

aiir::Value
acc::KernelsOp::getVectorLengthValue(aiir::acc::DeviceType deviceType) {
  return getValueInDeviceTypeSegment(getVectorLengthDeviceType(),
                                     getVectorLength(), deviceType);
}

aiir::Operation::operand_range KernelsOp::getNumGangsValues() {
  return getNumGangsValues(aiir::acc::DeviceType::None);
}

aiir::Operation::operand_range
KernelsOp::getNumGangsValues(aiir::acc::DeviceType deviceType) {
  return getValuesFromSegments(getNumGangsDeviceType(), getNumGangs(),
                               getNumGangsSegments(), deviceType);
}
...
bool acc::KernelsOp::hasWaitOnly(aiir::acc::DeviceType deviceType) {
  return hasDeviceType(getWaitOnly(), deviceType);
}

aiir::Operation::operand_range KernelsOp::getWaitValues() {
  return getWaitValues(aiir::acc::DeviceType::None);
}

aiir::Operation::operand_range
KernelsOp::getWaitValues(aiir::acc::DeviceType deviceType) {
  return getWaitValuesWithoutDevnum(
      getWaitOperandsDeviceType(), getWaitOperands(), getWaitOperandsSegments(),
      getHasWaitDevnum(), deviceType);
}

aiir::Value KernelsOp::getWaitDevnum() {
  return getWaitDevnum(aiir::acc::DeviceType::None);
}

aiir::Value KernelsOp::getWaitDevnum(aiir::acc::DeviceType deviceType) {
  return getWaitDevnumValue(getWaitOperandsDeviceType(), getWaitOperands(),
                            getWaitOperandsSegments(), getHasWaitDevnum(),
                            deviceType);
}
...
void acc::KernelsOp::addPrivatization(AIIRContext *context,
                                      aiir::acc::PrivateOp op,
                                      aiir::acc::PrivateRecipeOp recipe) {
  op.setRecipeAttr(aiir::SymbolRefAttr::get(context, recipe.getSymName()));
  getPrivateOperandsMutable().append(op.getResult());
...
void acc::KernelsOp::addReduction(AIIRContext *context,
                                  aiir::acc::ReductionOp op,
                                  aiir::acc::ReductionRecipeOp recipe) {
  op.setRecipeAttr(aiir::SymbolRefAttr::get(context, recipe.getSymName()));
  getReductionOperandsMutable().append(op.getResult());
...
LogicalResult acc::HostDataOp::verify() {
  ...
  llvm::SmallPtrSet<aiir::Value, 4> seenVars;
  for (aiir::Value operand : getDataClauseOperands()) {
    auto useDeviceOp =
        aiir::dyn_cast<acc::UseDeviceOp>(operand.getDefiningOp());
    if (!useDeviceOp)
...
static ParseResult parseGangClause(
    OpAsmParser &parser,
    llvm::SmallVectorImpl<aiir::OpAsmParser::UnresolvedOperand> &gangOperands,
    llvm::SmallVectorImpl<Type> &gangOperandsType, aiir::ArrayAttr &gangArgType,
    aiir::ArrayAttr &deviceType, aiir::DenseI32ArrayAttr &segments,
    aiir::ArrayAttr &gangOnlyDeviceType) {
  llvm::SmallVector<GangArgTypeAttr> gangArgTypeAttributes;
  llvm::SmallVector<aiir::Attribute> deviceTypeAttributes;
  llvm::SmallVector<aiir::Attribute> gangOnlyDeviceTypeAttributes;
  llvm::SmallVector<int32_t> seg;
  ...
  if (failed(parser.parseOptionalLParen())) {
    // Gang only keyword
    gangOnlyDeviceTypeAttributes.push_back(aiir::acc::DeviceTypeAttr::get(
        parser.getContext(), aiir::acc::DeviceType::None));
    gangOnlyDeviceType =
  ...
  auto argNum = aiir::acc::GangArgTypeAttr::get(parser.getContext(),
                                                aiir::acc::GangArgType::Num);
  auto argDim = aiir::acc::GangArgTypeAttr::get(parser.getContext(),
                                                aiir::acc::GangArgType::Dim);
  auto argStatic = aiir::acc::GangArgTypeAttr::get(
      parser.getContext(), aiir::acc::GangArgType::Static);
  ...
  do {
    ...
    if (succeeded(parser.parseOptionalLSquare())) {
      ...
      deviceTypeAttributes.push_back(aiir::acc::DeviceTypeAttr::get(
          parser.getContext(), aiir::acc::DeviceType::None));
    }
  ...
  llvm::SmallVector<aiir::Attribute> arrayAttr(gangArgTypeAttributes.begin(),
  ...
  llvm::SmallVector<aiir::Attribute> gangOnlyAttr(
...
void printGangClause(OpAsmPrinter &p, Operation *op,
                     aiir::OperandRange operands, aiir::TypeRange types,
                     std::optional<aiir::ArrayAttr> gangArgTypes,
                     std::optional<aiir::ArrayAttr> deviceTypes,
                     std::optional<aiir::DenseI32ArrayAttr> segments,
                     std::optional<aiir::ArrayAttr> gangOnlyDeviceTypes) {
  ...
  if (hasDeviceTypeValues(deviceTypes)) {
    ...
    llvm::interleaveComma(llvm::enumerate(*deviceTypes), p, [&](auto it) {
      ...
      llvm::interleaveComma(
          llvm::seq<int32_t>(0, (*segments)[it.index()]), p, [&](auto it) {
            auto gangArgTypeAttr = aiir::dyn_cast<aiir::acc::GangArgTypeAttr>(
                (*gangArgTypes)[opIdx]);
            if (gangArgTypeAttr.getValue() == aiir::acc::GangArgType::Num)
              p << LoopOp::getGangNumKeyword();
            else if (gangArgTypeAttr.getValue() == aiir::acc::GangArgType::Dim)
              p << LoopOp::getGangDimKeyword();
            else if (gangArgTypeAttr.getValue() ==
                     aiir::acc::GangArgType::Static)
...
bool hasDuplicateDeviceTypes(
    std::optional<aiir::ArrayAttr> segments,
    llvm::SmallSet<aiir::acc::DeviceType, 3> &deviceTypes) {
  if (!segments)
    return false;
  for (auto attr : *segments) {
    auto deviceTypeAttr = aiir::dyn_cast<aiir::acc::DeviceTypeAttr>(attr);
...
static std::optional<aiir::acc::DeviceType>
checkDeviceTypes(aiir::ArrayAttr deviceTypes) {
  llvm::SmallSet<aiir::acc::DeviceType, 3> crtDeviceTypes;
  if (!deviceTypes)
    return std::nullopt;
  for (auto attr : deviceTypes) {
    auto deviceTypeAttr =
        aiir::dyn_cast_or_null<aiir::acc::DeviceTypeAttr>(attr);
    if (!deviceTypeAttr)
      return aiir::acc::DeviceType::None;
...
LogicalResult acc::LoopOp::verify() {
  ...
  auto hasDeviceNone = [](aiir::acc::DeviceTypeAttr attr) -> bool {
    return attr.getValue() == aiir::acc::DeviceType::None;
  };
  bool hasDefaultSeq =
      getSeqAttr()
          ? llvm::any_of(getSeqAttr().getAsRange<aiir::acc::DeviceTypeAttr>(),
                         hasDeviceNone)
          : false;
  bool hasDefaultIndependent =
      getIndependentAttr()
          ? llvm::any_of(
                getIndependentAttr().getAsRange<aiir::acc::DeviceTypeAttr>(),
                hasDeviceNone)
          : false;
  bool hasDefaultAuto =
      getAuto_Attr()
          ? llvm::any_of(getAuto_Attr().getAsRange<aiir::acc::DeviceTypeAttr>(),
                         hasDeviceNone)
          : false;
  ...
  if (getSeqAttr()) {
    for (auto attr : getSeqAttr()) {
      ...
      if (hasVector(deviceTypeAttr.getValue()) ||
          ...
          getGangValue(aiir::acc::GangArgType::Num,
                       deviceTypeAttr.getValue()) ||
          getGangValue(aiir::acc::GangArgType::Dim,
                       deviceTypeAttr.getValue()) ||
          getGangValue(aiir::acc::GangArgType::Static,
                       deviceTypeAttr.getValue()))
  ...
  if (getUnstructured()) {
    ...
    if (getCollapseAttr()) {
      for (auto collapseEntry : getCollapseAttr()) {
        auto intAttr = aiir::dyn_cast<IntegerAttr>(collapseEntry);
    ...
    // level.
    aiir::Operation *expectedParent = this->getOperation();
    bool foundSibling = false;
    getRegion().walk<WalkOrder::PreOrder>([&](aiir::Operation *op) {
      if (aiir::isa<aiir::LoopLikeOpInterface>(op)) {
        // This effectively checks that we are not looking at a sibling loop.
        if (op->getParentOfType<aiir::LoopLikeOpInterface>() !=
            expectedParent) {
          foundSibling = true;
...
bool LoopOp::hasAuto() { return hasAuto(aiir::acc::DeviceType::None); }

bool LoopOp::hasAuto(aiir::acc::DeviceType deviceType) {
  return hasDeviceType(getAuto_(), deviceType);
}

bool LoopOp::hasIndependent() {
  return hasIndependent(aiir::acc::DeviceType::None);
}

bool LoopOp::hasIndependent(aiir::acc::DeviceType deviceType) {
  return hasDeviceType(getIndependent(), deviceType);
}

bool LoopOp::hasSeq() { return hasSeq(aiir::acc::DeviceType::None); }

bool LoopOp::hasSeq(aiir::acc::DeviceType deviceType) {
  return hasDeviceType(getSeq(), deviceType);
}

aiir::Value LoopOp::getVectorValue() {
  return getVectorValue(aiir::acc::DeviceType::None);
}

aiir::Value LoopOp::getVectorValue(aiir::acc::DeviceType deviceType) {
  return getValueInDeviceTypeSegment(getVectorOperandsDeviceType(),
                                     getVectorOperands(), deviceType);
}

bool LoopOp::hasVector() { return hasVector(aiir::acc::DeviceType::None); }

bool LoopOp::hasVector(aiir::acc::DeviceType deviceType) {
  return hasDeviceType(getVector(), deviceType);
}

aiir::Value LoopOp::getWorkerValue() {
  return getWorkerValue(aiir::acc::DeviceType::None);
}

aiir::Value LoopOp::getWorkerValue(aiir::acc::DeviceType deviceType) {
  return getValueInDeviceTypeSegment(getWorkerNumOperandsDeviceType(),
                                     getWorkerNumOperands(), deviceType);
}

bool LoopOp::hasWorker() { return hasWorker(aiir::acc::DeviceType::None); }

bool LoopOp::hasWorker(aiir::acc::DeviceType deviceType) {
  return hasDeviceType(getWorker(), deviceType);
}

aiir::Operation::operand_range LoopOp::getTileValues() {
  return getTileValues(aiir::acc::DeviceType::None);
}

aiir::Operation::operand_range
LoopOp::getTileValues(aiir::acc::DeviceType deviceType) {
...
std::optional<int64_t> LoopOp::getCollapseValue() {
  return getCollapseValue(aiir::acc::DeviceType::None);
}

std::optional<int64_t>
LoopOp::getCollapseValue(aiir::acc::DeviceType deviceType) {
  if (!getCollapseAttr())
    return std::nullopt;
  if (auto pos = findSegment(getCollapseDeviceTypeAttr(), deviceType)) {
    auto intAttr =
        aiir::dyn_cast<IntegerAttr>(getCollapseAttr().getValue()[*pos]);
    return intAttr.getValue().getZExtValue();
  }
  return std::nullopt;
}

aiir::Value LoopOp::getGangValue(aiir::acc::GangArgType gangArgType) {
  return getGangValue(gangArgType, aiir::acc::DeviceType::None);
}
...
aiir::Value LoopOp::getGangValue(aiir::acc::GangArgType gangArgType,
  ...
  if (auto pos = findSegment(*getGangOperandsDeviceType(), deviceType)) {
    ...
    aiir::Operation::operand_range values =
        getGangOperands()
...
bool LoopOp::hasGang() { return hasGang(aiir::acc::DeviceType::None); }

bool LoopOp::hasGang(aiir::acc::DeviceType deviceType) {
  return hasDeviceType(getGang(), deviceType);
}

llvm::SmallVector<aiir::Region *> acc::LoopOp::getLoopRegions() {
  return {&getRegion()};
}
...
void acc::LoopOp::addSeq(AIIRContext *context,
                         llvm::ArrayRef<DeviceType> effectiveDeviceTypes) {
...
void acc::LoopOp::addAuto(AIIRContext *context,
                          llvm::ArrayRef<DeviceType> effectiveDeviceTypes) {
...
void acc::LoopOp::setCollapseForDeviceTypes(
    AIIRContext *context, llvm::ArrayRef<DeviceType> effectiveDeviceTypes,
    llvm::APInt value) {
  llvm::SmallVector<aiir::Attribute> newValues;
  llvm::SmallVector<aiir::Attribute> newDeviceTypes;
  ...
  if (effectiveDeviceTypes.empty()) {
    ...
    newValues.push_back(
        aiir::IntegerAttr::get(aiir::IntegerType::get(context, 64), value));
...
void acc::LoopOp::addGangOperands(
    AIIRContext *context, llvm::ArrayRef<DeviceType> effectiveDeviceTypes,
    llvm::ArrayRef<GangArgType> argTypes, aiir::ValueRange values) {
  llvm::SmallVector<int32_t> segments;
  ...
  if (numAdded > 0) {
    llvm::SmallVector<aiir::Attribute> gangTypes;
    ...
    for (auto i : llvm::index_range(0u, numAdded)) {
      llvm::transform(argTypes, std::back_inserter(gangTypes),
                      [=](aiir::acc::GangArgType gangTy) {
                        return aiir::acc::GangArgTypeAttr::get(context, gangTy);
                      });
      (void)i;
    }

    setGangOperandsArgTypeAttr(aiir::ArrayAttr::get(context, gangTypes));
  }
}

void acc::LoopOp::addPrivatization(AIIRContext *context,
                                   aiir::acc::PrivateOp op,
                                   aiir::acc::PrivateRecipeOp recipe) {
  op.setRecipeAttr(aiir::SymbolRefAttr::get(context, recipe.getSymName()));
  getPrivateOperandsMutable().append(op.getResult());
...
void acc::LoopOp::addReduction(AIIRContext *context, aiir::acc::ReductionOp op,
                               aiir::acc::ReductionRecipeOp recipe) {
  op.setRecipeAttr(aiir::SymbolRefAttr::get(context, recipe.getSymName()));
  getReductionOperandsMutable().append(op.getResult());
}
...
LogicalResult acc::DataOp::verify() {
  ...
  for (aiir::Value operand : getDataClauseOperands())
    if (isa<BlockArgument>(operand) ||
        !aiir::isa<acc::AttachOp, acc::CopyinOp, acc::CopyoutOp, acc::CreateOp,
                   acc::DeleteOp, acc::DetachOp, acc::DevicePtrOp,
                   acc::GetDevicePtrOp, acc::NoCreateOp, acc::PresentOp>(
            operand.getDefiningOp()))
...
bool acc::DataOp::hasAsyncOnly(aiir::acc::DeviceType deviceType) {
  return hasDeviceType(getAsyncOnly(), deviceType);
}

aiir::Value DataOp::getAsyncValue() {
  return getAsyncValue(aiir::acc::DeviceType::None);
}

aiir::Value DataOp::getAsyncValue(aiir::acc::DeviceType deviceType) {
  return getValueInDeviceTypeSegment(getAsyncOperandsDeviceType(),
                                     getAsyncOperands(), deviceType);
}

bool DataOp::hasWaitOnly() { return hasWaitOnly(aiir::acc::DeviceType::None); }

bool DataOp::hasWaitOnly(aiir::acc::DeviceType deviceType) {
  return hasDeviceType(getWaitOnly(), deviceType);
}

aiir::Operation::operand_range DataOp::getWaitValues() {
  return getWaitValues(aiir::acc::DeviceType::None);
}

aiir::Operation::operand_range
DataOp::getWaitValues(aiir::acc::DeviceType deviceType) {
  return getWaitValuesWithoutDevnum(
      getWaitOperandsDeviceType(), getWaitOperands(), getWaitOperandsSegments(),
      getHasWaitDevnum(), deviceType);
}

aiir::Value DataOp::getWaitDevnum() {
  return getWaitDevnum(aiir::acc::DeviceType::None);
}

aiir::Value DataOp::getWaitDevnum(aiir::acc::DeviceType deviceType) {
  return getWaitDevnumValue(getWaitOperandsDeviceType(), getWaitOperands(),
                            getWaitOperandsSegments(), getHasWaitDevnum(),
                            deviceType);
}
...
void acc::DataOp::addWaitOnly(AIIRContext *context,
                              llvm::ArrayRef<DeviceType> effectiveDeviceTypes) {
  setWaitOnlyAttr(addDeviceTypeAffectedOperandHelper(context, getWaitOnlyAttr(),
                                                     effectiveDeviceTypes));
}
...
void ExitDataOp::addAsyncOnly(AIIRContext *context,
                              llvm::ArrayRef<DeviceType> effectiveDeviceTypes) {
  assert(effectiveDeviceTypes.empty());
  assert(!getAsyncAttr());
  assert(!getAsyncOperand());

  setAsyncAttr(aiir::UnitAttr::get(context));
}
...
void ExitDataOp::addWaitOnly(AIIRContext *context,
                             llvm::ArrayRef<DeviceType> effectiveDeviceTypes) {
  assert(effectiveDeviceTypes.empty());
  assert(!getWaitAttr());
  ...
  setWaitAttr(aiir::UnitAttr::get(context));
}
...
LogicalResult acc::EnterDataOp::verify() {
  ...
  for (aiir::Value operand : getDataClauseOperands())
    if (!aiir::isa<acc::AttachOp, acc::CreateOp, acc::CopyinOp>(
            operand.getDefiningOp()))
      return emitError("expect data entry operation as defining op");
...
void EnterDataOp::addWaitOnly(AIIRContext *context,
                              llvm::ArrayRef<DeviceType> effectiveDeviceTypes) {
  assert(effectiveDeviceTypes.empty());
  assert(!getWaitAttr());
  assert(getWaitOperands().empty());
...
template <typename Op>
static LogicalResult
checkDeclareOperands(Op &op, const aiir::ValueRange &operands,
  ...
  for (aiir::Value operand : operands) {
    if (isa<BlockArgument>(operand) ||
        !aiir::isa<acc::CopyinOp, acc::CopyoutOp, acc::CreateOp,
    ...
    aiir::Value var{getVar(operand.getDefiningOp())};
    assert(var && "declare operands can only be data entry operations which "
                  "must have var");
    (void)var;
    std::optional<aiir::acc::DataClause> dataClauseOptional{
...
static ParseResult parseBindName(OpAsmParser &parser,
                                 aiir::ArrayAttr &bindIdName,
                                 aiir::ArrayAttr &bindStrName,
                                 aiir::ArrayAttr &deviceIdTypes,
                                 aiir::ArrayAttr &deviceStrTypes) {
  llvm::SmallVector<aiir::Attribute> bindIdNameAttrs;
  llvm::SmallVector<aiir::Attribute> bindStrNameAttrs;
  llvm::SmallVector<aiir::Attribute> deviceIdTypeAttrs;
  llvm::SmallVector<aiir::Attribute> deviceStrTypeAttrs;

  if (failed(parser.parseCommaSeparatedList([&]() {
        aiir::Attribute newAttr;
        bool isSymbolRefAttr;
        auto parseResult = parser.parseAttribute(newAttr);
        if (auto symbolRefAttr = dyn_cast<aiir::SymbolRefAttr>(newAttr)) {
          bindIdNameAttrs.push_back(symbolRefAttr);
          isSymbolRefAttr = true;
        } else if (auto stringAttr = dyn_cast<aiir::StringAttr>(newAttr)) {
        ...
        if (failed(parser.parseOptionalLSquare())) {
          if (isSymbolRefAttr) {
            deviceIdTypeAttrs.push_back(aiir::acc::DeviceTypeAttr::get(
                parser.getContext(), aiir::acc::DeviceType::None));
          } else {
            deviceStrTypeAttrs.push_back(aiir::acc::DeviceTypeAttr::get(
                parser.getContext(), aiir::acc::DeviceType::None));
          }
        } else {
          if (isSymbolRefAttr) {
...
static void printBindName(aiir::OpAsmPrinter &p, aiir::Operation *op,
                          std::optional<aiir::ArrayAttr> bindIdName,
                          std::optional<aiir::ArrayAttr> bindStrName,
                          std::optional<aiir::ArrayAttr> deviceIdTypes,
                          std::optional<aiir::ArrayAttr> deviceStrTypes) {
  // Create combined vectors for all bind names and device types
  llvm::SmallVector<aiir::Attribute> allBindNames;
  llvm::SmallVector<aiir::Attribute> allDeviceTypes;
...
static ParseResult parseRoutineGangClause(OpAsmParser &parser,
                                          aiir::ArrayAttr &gang,
                                          aiir::ArrayAttr &gangDim,
                                          aiir::ArrayAttr &gangDimDeviceTypes) {

  llvm::SmallVector<aiir::Attribute> gangAttrs, gangDimAttrs,
      gangDimDeviceTypeAttrs;
  bool needCommaBeforeOperands = false;

  // Gang keyword only
  if (failed(parser.parseOptionalLParen())) {
    gangAttrs.push_back(aiir::acc::DeviceTypeAttr::get(
        parser.getContext(), aiir::acc::DeviceType::None));
  ...
  if (failed(parser.parseCommaSeparatedList([&]() {
        ...
        if (succeeded(parser.parseOptionalLSquare())) {
          ...
          gangDimDeviceTypeAttrs.push_back(aiir::acc::DeviceTypeAttr::get(
              parser.getContext(), aiir::acc::DeviceType::None));
        }
        return success();
      })))
...
void printRoutineGangClause(OpAsmPrinter &p, Operation *op,
                            std::optional<aiir::ArrayAttr> gang,
                            std::optional<aiir::ArrayAttr> gangDim,
                            std::optional<aiir::ArrayAttr> gangDimDeviceTypes) {

  if (!hasDeviceTypeValues(gangDimDeviceTypes) && hasDeviceTypeValues(gang) &&
      gang->size() == 1) {
    auto deviceTypeAttr = aiir::dyn_cast<aiir::acc::DeviceTypeAttr>((*gang)[0]);
    if (deviceTypeAttr.getValue() == aiir::acc::DeviceType::None)
      return;
  }
...
static void
printDeviceTypeArrayAttr(aiir::OpAsmPrinter &p, aiir::Operation *op,
                         std::optional<aiir::ArrayAttr> deviceTypes) {

  if (hasDeviceTypeValues(deviceTypes) && deviceTypes->size() == 1) {
    auto deviceTypeAttr =
        aiir::dyn_cast<aiir::acc::DeviceTypeAttr>((*deviceTypes)[0]);
    if (deviceTypeAttr.getValue() == aiir::acc::DeviceType::None)
      return;
  }
  ...
  p << "([";
  llvm::interleaveComma(*deviceTypes, p, [&](aiir::Attribute attr) {
    auto dTypeAttr = aiir::dyn_cast<aiir::acc::DeviceTypeAttr>(attr);
    p << dTypeAttr;
  });
  p << "])";
}

bool RoutineOp::hasWorker() { return hasWorker(aiir::acc::DeviceType::None); }

bool RoutineOp::hasWorker(aiir::acc::DeviceType deviceType) {
  return hasDeviceType(getWorker(), deviceType);
}

bool RoutineOp::hasVector() { return hasVector(aiir::acc::DeviceType::None); }

bool RoutineOp::hasVector(aiir::acc::DeviceType deviceType) {
  return hasDeviceType(getVector(), deviceType);
}

bool RoutineOp::hasSeq() { return hasSeq(aiir::acc::DeviceType::None); }

bool RoutineOp::hasSeq(aiir::acc::DeviceType deviceType) {
  return hasDeviceType(getSeq(), deviceType);
}

std::optional<std::variant<aiir::SymbolRefAttr, aiir::StringAttr>>
RoutineOp::getBindNameValue() {
  return getBindNameValue(aiir::acc::DeviceType::None);
}

std::optional<std::variant<aiir::SymbolRefAttr, aiir::StringAttr>>
RoutineOp::getBindNameValue(aiir::acc::DeviceType deviceType) {
  if (hasDeviceTypeValues(getBindIdNameDeviceType())) {
    if (auto pos = findSegment(*getBindIdNameDeviceType(), deviceType)) {
      ...
      auto symbolRefAttr = dyn_cast<aiir::SymbolRefAttr>(attr);
  ...
  if (hasDeviceTypeValues(getBindStrNameDeviceType())) {
    if (auto pos = findSegment(*getBindStrNameDeviceType(), deviceType)) {
      ...
      auto stringAttr = dyn_cast<aiir::StringAttr>(attr);
...
bool RoutineOp::hasGang() { return hasGang(aiir::acc::DeviceType::None); }

bool RoutineOp::hasGang(aiir::acc::DeviceType deviceType) {
  return hasDeviceType(getGang(), deviceType);
}

std::optional<int64_t> RoutineOp::getGangDimValue() {
  return getGangDimValue(aiir::acc::DeviceType::None);
}

std::optional<int64_t>
RoutineOp::getGangDimValue(aiir::acc::DeviceType deviceType) {
  if (!hasDeviceTypeValues(getGangDimDeviceType()))
    return std::nullopt;
  if (auto pos = findSegment(*getGangDimDeviceType(), deviceType)) {
    auto intAttr = aiir::dyn_cast<aiir::IntegerAttr>((*getGangDim())[*pos]);
    return intAttr.getInt();
  }
  return std::nullopt;
}

void RoutineOp::addSeq(AIIRContext *context,
                       llvm::ArrayRef<DeviceType> effectiveDeviceTypes) {
  setSeqAttr(addDeviceTypeAffectedOperandHelper(context, getSeqAttr(),
                                                effectiveDeviceTypes));
}

void RoutineOp::addVector(AIIRContext *context,
                          llvm::ArrayRef<DeviceType> effectiveDeviceTypes) {
  setVectorAttr(addDeviceTypeAffectedOperandHelper(context, getVectorAttr(),
                                                   effectiveDeviceTypes));
}

void RoutineOp::addWorker(AIIRContext *context,
                          llvm::ArrayRef<DeviceType> effectiveDeviceTypes) {
  setWorkerAttr(addDeviceTypeAffectedOperandHelper(context, getWorkerAttr(),
                                                   effectiveDeviceTypes));
}

void RoutineOp::addGang(AIIRContext *context,
                        llvm::ArrayRef<DeviceType> effectiveDeviceTypes) {
  setGangAttr(addDeviceTypeAffectedOperandHelper(context, getGangAttr(),
                                                 effectiveDeviceTypes));
}
...
void RoutineOp::addGang(AIIRContext *context,
                        llvm::ArrayRef<DeviceType> effectiveDeviceTypes,
                        uint64_t val) {
  llvm::SmallVector<aiir::Attribute> dimValues;
  llvm::SmallVector<aiir::Attribute> deviceTypes;
  ...
  if (effectiveDeviceTypes.empty()) {
    dimValues.push_back(
        aiir::IntegerAttr::get(aiir::IntegerType::get(context, 64), val));
    deviceTypes.push_back(
        acc::DeviceTypeAttr::get(context, acc::DeviceType::None));
  } else {
    for (DeviceType dt : effectiveDeviceTypes) {
  ...
  setGangDimAttr(aiir::ArrayAttr::get(context, dimValues));
  setGangDimDeviceTypeAttr(aiir::ArrayAttr::get(context, deviceTypes));
}

void RoutineOp::addBindStrName(AIIRContext *context,
                               llvm::ArrayRef<DeviceType> effectiveDeviceTypes,
                               aiir::StringAttr val) {
  ...
  llvm::SmallVector<aiir::Attribute> vals;
  if (getBindStrNameAttr())
    llvm::copy(getBindStrNameAttr(), std::back_inserter(vals));
  for (unsigned i = 0; i < after - before; ++i)
    vals.push_back(val);

  setBindStrNameAttr(aiir::ArrayAttr::get(context, vals));
}

void RoutineOp::addBindIDName(AIIRContext *context,
                              llvm::ArrayRef<DeviceType> effectiveDeviceTypes,
                              aiir::SymbolRefAttr val) {
  unsigned before =
  ...
  setBindIdNameAttr(aiir::ArrayAttr::get(context, vals));
}
...
void acc::InitOp::addDeviceType(AIIRContext *context,
                                aiir::acc::DeviceType deviceType) {
  llvm::SmallVector<aiir::Attribute> deviceTypes;
  ...
  setDeviceTypesAttr(aiir::ArrayAttr::get(context, deviceTypes));
}
...
void acc::ShutdownOp::addDeviceType(AIIRContext *context,
                                    aiir::acc::DeviceType deviceType) {
  llvm::SmallVector<aiir::Attribute> deviceTypes;
  if (getDeviceTypesAttr())
    llvm::copy(getDeviceTypesAttr(), std::back_inserter(deviceTypes));
...
LogicalResult acc::UpdateOp::verify() {
  ...
  for (aiir::Value operand : getDataClauseOperands())
    if (!aiir::isa<acc::UpdateDeviceOp, acc::UpdateHostOp, acc::GetDevicePtrOp>(
            operand.getDefiningOp()))
      return emitError("expect data entry/exit operation or acc.getdeviceptr "
                       "as defining op");
...
bool UpdateOp::hasAsyncOnly(aiir::acc::DeviceType deviceType) {
  return hasDeviceType(getAsyncOnly(), deviceType);
}

aiir::Value UpdateOp::getAsyncValue() {
  return getAsyncValue(aiir::acc::DeviceType::None);
}

aiir::Value UpdateOp::getAsyncValue(aiir::acc::DeviceType deviceType) {
...
bool UpdateOp::hasWaitOnly(aiir::acc::DeviceType deviceType) {
  return hasDeviceType(getWaitOnly(), deviceType);
}

aiir::Operation::operand_range UpdateOp::getWaitValues() {
  return getWaitValues(aiir::acc::DeviceType::None);
}

aiir::Operation::operand_range
UpdateOp::getWaitValues(aiir::acc::DeviceType deviceType) {
  return getWaitValuesWithoutDevnum(
      getWaitOperandsDeviceType(), getWaitOperands(), getWaitOperandsSegments(),
      getHasWaitDevnum(), deviceType);
}

aiir::Value UpdateOp::getWaitDevnum() {
  return getWaitDevnum(aiir::acc::DeviceType::None);
}

aiir::Value UpdateOp::getWaitDevnum(aiir::acc::DeviceType deviceType) {
  return getWaitDevnumValue(getWaitOperandsDeviceType(), getWaitOperands(),
                            getWaitOperandsSegments(), getHasWaitDevnum(),
                            deviceType);
}

void UpdateOp::addAsyncOnly(AIIRContext *context,
                            llvm::ArrayRef<DeviceType> effectiveDeviceTypes) {
  setAsyncOnlyAttr(addDeviceTypeAffectedOperandHelper(
      context, getAsyncOnlyAttr(), effectiveDeviceTypes));
}
...
void UpdateOp::addWaitOnly(AIIRContext *context,
                           llvm::ArrayRef<DeviceType> effectiveDeviceTypes) {
  setWaitOnlyAttr(addDeviceTypeAffectedOperandHelper(context, getWaitOnlyAttr(),
                                                     effectiveDeviceTypes));
}
...
aiir::acc::getVarPtr(aiir::Operation *accDataClauseOp) {
  auto varPtr{llvm::TypeSwitch<aiir::Operation *,
                               aiir::TypedValue<aiir::acc::PointerLikeType>>(
                  accDataClauseOp)
                  .Case<ACC_DATA_ENTRY_OPS>(
                      [&](auto entry) { return entry.getVarPtr(); })
                  .Case<aiir::acc::CopyoutOp, aiir::acc::UpdateHostOp>(
                      [&](auto exit) { return exit.getVarPtr(); })
                  .Default([&](aiir::Operation *) {
                    return aiir::TypedValue<aiir::acc::PointerLikeType>();
                  })};
  return varPtr;
}

aiir::Value aiir::acc::getVar(aiir::Operation *accDataClauseOp) {
  auto varPtr{
      llvm::TypeSwitch<aiir::Operation *, aiir::Value>(accDataClauseOp)
          .Case<ACC_DATA_ENTRY_OPS>([&](auto entry) { return entry.getVar(); })
          .Default([&](aiir::Operation *) { return aiir::Value(); })};
  return varPtr;
}

aiir::Type aiir::acc::getVarType(aiir::Operation *accDataClauseOp) {
  auto varType{llvm::TypeSwitch<aiir::Operation *, aiir::Type>(accDataClauseOp)
                   .Case<ACC_DATA_ENTRY_OPS>(
                       [&](auto entry) { return entry.getVarType(); })
                   .Case<aiir::acc::CopyoutOp, aiir::acc::UpdateHostOp>(
                       [&](auto exit) { return exit.getVarType(); })
                   .Default([&](aiir::Operation *) { return aiir::Type(); })};
  return varType;
}

aiir::TypedValue<aiir::acc::PointerLikeType>
aiir::acc::getAccPtr(aiir::Operation *accDataClauseOp) {
  auto accPtr{llvm::TypeSwitch<aiir::Operation *,
                               aiir::TypedValue<aiir::acc::PointerLikeType>>(
                  accDataClauseOp)
...
aiir::Value aiir::acc::getAccVar(aiir::Operation *accDataClauseOp) {
  auto accPtr{llvm::TypeSwitch<aiir::Operation *, aiir::Value>(accDataClauseOp)
                  .Case<ACC_DATA_ENTRY_OPS, ACC_DATA_EXIT_OPS>(
...
aiir::Value aiir::acc::getVarPtrPtr(aiir::Operation *accDataClauseOp) {
  auto varPtrPtr{
      llvm::TypeSwitch<aiir::Operation *, aiir::Value>(accDataClauseOp)
          .Case<ACC_DATA_ENTRY_OPS>(
...
aiir::acc::getBounds(aiir::Operation *accDataClauseOp) {
  aiir::SmallVector<aiir::Value> bounds{
      llvm::TypeSwitch<aiir::Operation *, aiir::SmallVector<aiir::Value>>(
          accDataClauseOp)
          .Case<ACC_DATA_ENTRY_OPS, ACC_DATA_EXIT_OPS>([&](auto dataClause) {
            return aiir::SmallVector<aiir::Value>(
                dataClause.getBounds().begin(), dataClause.getBounds().end());
          })
          .Default([&](aiir::Operation *) {
            return aiir::SmallVector<aiir::Value, 0>();
          })};
  return bounds;
}

aiir::SmallVector<aiir::Value>
aiir::acc::getAsyncOperands(aiir::Operation *accDataClauseOp) {
  return llvm::TypeSwitch<aiir::Operation *, aiir::SmallVector<aiir::Value>>(
             accDataClauseOp)
      .Case<ACC_DATA_ENTRY_OPS, ACC_DATA_EXIT_OPS>([&](auto dataClause) {
        return aiir::SmallVector<aiir::Value>(
...
aiir::ArrayAttr
aiir::acc::getAsyncOperandsDeviceType(aiir::Operation *accDataClauseOp) {
  return llvm::TypeSwitch<aiir::Operation *, aiir::ArrayAttr>(accDataClauseOp)
      .Case<ACC_DATA_ENTRY_OPS, ACC_DATA_EXIT_OPS>([&](auto dataClause) {
        return dataClause.getAsyncOperandsDeviceTypeAttr();
      })
      .Default([&](aiir::Operation *) { return aiir::ArrayAttr{}; });
}

aiir::ArrayAttr aiir::acc::getAsyncOnly(aiir::Operation *accDataClauseOp) {
  return llvm::TypeSwitch<aiir::Operation *, aiir::ArrayAttr>(accDataClauseOp)
      .Case<ACC_DATA_ENTRY_OPS, ACC_DATA_EXIT_OPS>(
...
std::optional<llvm::StringRef> aiir::acc::getVarName(aiir::Operation *accOp) {
  auto name{
      llvm::TypeSwitch<aiir::Operation *, std::optional<llvm::StringRef>>(accOp)
          .Case<ACC_DATA_ENTRY_OPS>([&](auto entry) { return entry.getName(); })
          .Default([&](aiir::Operation *) -> std::optional<llvm::StringRef> {
            return {};
          })};
  return name;
}
...
aiir::acc::getDataClause(aiir::Operation *accDataEntryOp) {
  auto dataClause{
      llvm::TypeSwitch<aiir::Operation *, std::optional<aiir::acc::DataClause>>(
          accDataEntryOp)
          .Case<ACC_DATA_ENTRY_OPS>(
              [&](auto entry) { return entry.getDataClause(); })
          .Default([&](aiir::Operation *) { return std::nullopt; })};
  return dataClause;
}

bool aiir::acc::getImplicitFlag(aiir::Operation *accDataEntryOp) {
  auto implicit{llvm::TypeSwitch<aiir::Operation *, bool>(accDataEntryOp)
                    .Case<ACC_DATA_ENTRY_OPS>(
...
aiir::ValueRange aiir::acc::getDataOperands(aiir::Operation *accOp) {
  auto dataOperands{
      llvm::TypeSwitch<aiir::Operation *, aiir::ValueRange>(accOp)
          .Case<ACC_COMPUTE_AND_DATA_CONSTRUCT_OPS>(
              [&](auto entry) { return entry.getDataClauseOperands(); })
          .Default([&](aiir::Operation *) { return aiir::ValueRange(); })};
  return dataOperands;
}

aiir::MutableOperandRange
aiir::acc::getMutableDataOperands(aiir::Operation *accOp) {
  auto dataOperands{
      llvm::TypeSwitch<aiir::Operation *, aiir::MutableOperandRange>(accOp)
          .Case<ACC_COMPUTE_AND_DATA_CONSTRUCT_OPS>(
              [&](auto entry) { return entry.getDataClauseOperandsMutable(); })
          .Default([&](aiir::Operation *) { return nullptr; })};
  return dataOperands;
}

aiir::SymbolRefAttr aiir::acc::getRecipe(aiir::Operation *accOp) {
  auto recipe{
      llvm::TypeSwitch<aiir::Operation *, aiir::SymbolRefAttr>(accOp)
          .Case<ACC_DATA_ENTRY_OPS>(
              [&](auto entry) { return entry.getRecipeAttr(); })
          .Default([&](aiir::Operation *) { return aiir::SymbolRefAttr{}; })};
  return recipe;
}
