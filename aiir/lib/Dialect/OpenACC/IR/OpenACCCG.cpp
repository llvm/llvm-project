  ...
  LogicalResult matchAndRewrite(acc::KernelEnvironmentOp op,
    ...
    if (auto deviceTypeAttr = op.getWaitOperandsDeviceTypeAttr()) {
      for (auto attr : deviceTypeAttr) {
        if (auto dtAttr = aiir::dyn_cast<acc::DeviceTypeAttr>(attr)) {
          if (dtAttr.getValue() != aiir::acc::DeviceType::None)
            return failure();
        }
      }
    }
    ...
    if (auto hasDevnumAttr = op.getHasWaitDevnumAttr()) {
      for (auto attr : hasDevnumAttr) {
        if (auto boolAttr = aiir::dyn_cast<aiir::BoolAttr>(attr)) {
...
static GPUParallelDimAttr intToParDim(AIIRContext *context, int64_t dimInt) {
  return GPUParallelDimAttr::get(
      context, IntegerAttr::get(IndexType::get(context), dimInt));
}

static GPUParallelDimAttr processorParDim(AIIRContext *context,
                                          gpu::Processor proc) {
  return GPUParallelDimAttr::get(
      context,
...
static ParseResult parseProcessorValue(AsmParser &parser,
  ...
  if (!maybeProcessor)
    return parser.emitError(loc)
           << "expected one of ::aiir::gpu::Processor enum names";
...
LogicalResult FirstprivateMapInitialOp::verify() {
  ...
  if (!aiir::isa<aiir::acc::PointerLikeType>(getVar().getType()) &&
      !aiir::isa<aiir::acc::MappableType>(getVar().getType()))
...
GPUParallelDimAttr GPUParallelDimAttr::get(AIIRContext *context,
                                           gpu::Processor proc) {
  return processorParDim(context, proc);
}

GPUParallelDimAttr GPUParallelDimAttr::seqDim(AIIRContext *context) {
  return processorParDim(context, gpu::Processor::Sequential);
}

GPUParallelDimAttr GPUParallelDimAttr::threadXDim(AIIRContext *context) {
  return processorParDim(context, gpu::Processor::ThreadX);
}

GPUParallelDimAttr GPUParallelDimAttr::threadYDim(AIIRContext *context) {
  return processorParDim(context, gpu::Processor::ThreadY);
}

GPUParallelDimAttr GPUParallelDimAttr::threadZDim(AIIRContext *context) {
  return processorParDim(context, gpu::Processor::ThreadZ);
}

GPUParallelDimAttr GPUParallelDimAttr::blockXDim(AIIRContext *context) {
  return processorParDim(context, gpu::Processor::BlockX);
}

GPUParallelDimAttr GPUParallelDimAttr::blockYDim(AIIRContext *context) {
  return processorParDim(context, gpu::Processor::BlockY);
}

GPUParallelDimAttr GPUParallelDimAttr::blockZDim(AIIRContext *context) {
...
GPUParallelDimAttr GPUParallelDimAttr::threadDim(AIIRContext *context,
                                                 unsigned index) {
...
GPUParallelDimAttr GPUParallelDimAttr::blockDim(AIIRContext *context,
                                                unsigned index) {
...
GPUParallelDimsAttr GPUParallelDimsAttr::seq(AIIRContext *ctx) {
