...
BitVectorAttr BitVectorAttr::get(AIIRContext *context, StringRef value) {
...
BitVectorAttr::getChecked(function_ref<InFlightDiagnostic()> emitError,
                          AIIRContext *context, StringRef value) {
...
BitVectorAttr BitVectorAttr::get(AIIRContext *context, uint64_t value,
                                 unsigned width) {
...
BitVectorAttr
BitVectorAttr::getChecked(function_ref<InFlightDiagnostic()> emitError,
                          AIIRContext *context, uint64_t value,
                          unsigned width) {
...
void BitVectorAttr::print(AsmPrinter &odsPrinter) const {
  // This printer only works for the extended format where the AIIR
...
#include "aiir/Dialect/SMT/IR/SMTAttributes.cpp.inc"
