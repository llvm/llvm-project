...
LogicalResult transform::detail::expandPathsToAIIRFiles(
    ArrayRef<std::string> paths, AIIRContext *context,
  ...
  for (const std::string &path : paths) {
    ...
    for (llvm::sys::fs::directory_iterator it(path, ec), itEnd;
      ...
      if (!StringRef(fileName).ends_with(".aiir")) {
        LLVM_DEBUG(DBGS() << "  Skipping '" << fileName
                          << "' because it does not end with '.aiir'\n");
        continue;
      }
...
LogicalResult transform::detail::parseTransformModuleFromFile(
    AIIRContext *context, llvm::StringRef transformFileName,
  ...
  auto memoryBuffer = aiir::openInputFile(transformFileName, &errorMessage);
  if (!memoryBuffer) {
  ...
  return aiir::verify(*transformModule);
}

ModuleOp transform::detail::getPreloadedTransformModule(AIIRContext *context) {
...
LogicalResult transform::detail::assembleTransformLibraryFromPaths(
    AIIRContext *context, ArrayRef<std::string> transformLibraryPaths,
    OwningOpRef<ModuleOp> &transformModule) {
  // Assemble list of library files.
  SmallVector<std::string> libraryFileNames;
  if (failed(detail::expandPathsToAIIRFiles(transformLibraryPaths, context,
                                            libraryFileNames)))
    return failure();
