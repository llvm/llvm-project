...
LogicalResult transform::smt::ConstrainParamsOp::verify() {
  auto yieldTerminator =
      dyn_cast<aiir::smt::YieldOp>(getRegion().front().back());
  if (!yieldTerminator)
    return emitOpError() << "expected '"
                         << aiir::smt::YieldOp::getOperationName()
                         << "' as terminator";

  auto checkTypes = [](size_t idx, Type smtType, StringRef smtDesc,
                       Type paramType, StringRef paramDesc,
                       auto *atOp) -> InFlightDiagnostic {
    if (!isa<aiir::smt::BoolType, aiir::smt::IntType, aiir::smt::BitVectorType>(
            smtType))
    ...
    if (isa<aiir::smt::IntType>(smtType)) {
    ...
    } else if (isa<aiir::smt::BoolType>(smtType)) {
    ...
    } else if (auto bvSmtType = dyn_cast<aiir::smt::BitVectorType>(smtType)) {
      auto wrappedIntType = dyn_cast<IntegerType>(typeWrappedByParam);
  ...
  for (auto &op : getBody().getOps()) {
    if (!isa<aiir::smt::SMTDialect>(op.getDialect()))
      return emitOpError(
