...
#include "aiir/Dialect/Transform/PDLExtension/PDLExtensionOps.h"
...
AIIR_DEFINE_EXPLICIT_TYPE_ID(aiir::transform::PDLMatchHooks)

#define GET_OP_CLASSES
#include "aiir/Dialect/Transform/PDLExtension/PDLExtensionOps.cpp.inc"
...
public:
  AIIR_DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID(PatternApplicatorExtension)
...
LogicalResult transform::WithPDLPatternsOp::verify() {
  ...
  for (Operation &op : body->getOperations()) {
    ...
    if (op.hasTrait<::aiir::transform::PossibleTopLevelTransformOpTrait>()) {
      if (topLevelOp) {
