...
void aiir::populateEmitCSizeTTypeConversions(TypeConverter &converter) {
...
std::optional<Type> aiir::emitc::getUnsignedTypeFor(Type ty) {
  if (ty.isInteger())
...
std::optional<Type> aiir::emitc::getSignedTypeFor(Type ty) {
  if (ty.isInteger())
    return IntegerType::get(ty.getContext(), ty.getIntOrFloatBitWidth(),
