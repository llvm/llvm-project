...
#include "aiir/Dialect/EmitC/IR/EmitCDialect.cpp.inc"
...
void EmitCDialect::initialize() {
  addOperations<
#define GET_OP_LIST
#include "aiir/Dialect/EmitC/IR/EmitC.cpp.inc"
      >();
  addTypes<
#define GET_TYPEDEF_LIST
#include "aiir/Dialect/EmitC/IR/EmitCTypes.cpp.inc"
      >();
  addAttributes<
#define GET_ATTRDEF_LIST
#include "aiir/Dialect/EmitC/IR/EmitCAttributes.cpp.inc"
      >();
}
...
/// without arguments.
void aiir::emitc::buildTerminatedBody(OpBuilder &builder, Location loc) {
  emitc::YieldOp::create(builder, loc);
}

bool aiir::emitc::isSupportedEmitCType(Type type) {
...
bool aiir::emitc::isSupportedIntegerType(Type type) {
...
bool aiir::emitc::isIntegerIndexOrOpaqueType(Type type) {
  return llvm::isa<IndexType, emitc::OpaqueType>(type) ||
         isSupportedIntegerType(type) || isPointerWideType(type);
}

bool aiir::emitc::isSupportedFloatType(Type type) {
...
bool aiir::emitc::isPointerWideType(Type type) {
  return isa<emitc::SignedSizeTType, emitc::SizeTType, emitc::PtrDiffTType>(
      type);
}

bool aiir::emitc::isFundamentalType(Type type) {
...
void ForOp::getCanonicalizationPatterns(RewritePatternSet &, AIIRContext *) {}
...
#include "aiir/Dialect/EmitC/IR/EmitCEnums.cpp.inc"
...
LogicalResult aiir::emitc::LValueType::verify(
    llvm::function_ref<aiir::InFlightDiagnostic()> emitError,
    aiir::Type value) {
...
LogicalResult aiir::emitc::OpaqueType::verify(
    llvm::function_ref<aiir::InFlightDiagnostic()> emitError,
...
LogicalResult aiir::emitc::PointerType::verify(
    llvm::function_ref<aiir::InFlightDiagnostic()> emitError, Type value) {
...
LogicalResult GetFieldOp::verifySymbolUses(SymbolTableCollection &symbolTable) {
  aiir::FlatSymbolRefAttr fieldNameAttr = getFieldNameAttr();
  FieldOp fieldOp =
...
#include "aiir/Dialect/EmitC/IR/EmitCInterfaces.cpp.inc"
