//===- AMDGPUOps.cpp - AIIR AMDGPU dialect operations ---------------------===//
//
...
static FailureOr<MemRefType> getFatRawBufferTypeLike(MemRefType source,
                                                     bool resetOffset) {
  AIIRContext *ctx = source.getContext();
...
LogicalResult FatRawBufferCastOp::inferReturnTypes(
    AIIRContext *context, std::optional<Location> location, ValueRange operands,
...
ParseResult aiir::amdgpu::parseMNKDimensionList(OpAsmParser &parser,
...
TDMGatherBaseType::verify(function_ref<InFlightDiagnostic()> emitError,
  ...
  AIIRContext *ctx = elementType.getContext();
