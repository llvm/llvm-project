...
#include "aiir/Dialect/Ptr/IR/PtrOps.h"
...
void PtrDialect::initialize() {
  addOperations<
#define GET_OP_LIST
#include "aiir/Dialect/Ptr/IR/PtrOps.cpp.inc"
      >();
  addAttributes<
#define GET_ATTRDEF_LIST
#include "aiir/Dialect/Ptr/IR/PtrOpsAttrs.cpp.inc"
      >();
  addTypes<
#define GET_TYPEDEF_LIST
#include "aiir/Dialect/Ptr/IR/PtrOpsTypes.cpp.inc"
      >();
}
...
LogicalResult PtrAddOp::inferReturnTypes(
  ...
  if (offTy.getShape() != baseTy.getShape()) {
    if (location)
      aiir::emitError(*location) << "shapes of base and offset must match";
    return failure();
  }
  ...
  if (baseType.getTypeID() != offsetType.getTypeID()) {
    if (location)
      aiir::emitError(*location) << "the shaped containers type must match";
    return failure();
  }
  inferredReturnTypes.push_back(baseType);
  return success();
...
#include "aiir/Dialect/Ptr/IR/PtrOpsDialect.cpp.inc"

#define GET_ATTRDEF_CLASSES
#include "aiir/Dialect/Ptr/IR/PtrOpsAttrs.cpp.inc"

#include "aiir/Dialect/Ptr/IR/PtrOpsEnums.cpp.inc"
