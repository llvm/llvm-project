...
static Value collapseInnerDims(OpBuilder &builder, aiir::Location loc,
                               Value input) {
...
getSrcIndxValue(OpBuilder &rewriter, Location loc, Value operand,
  ...
  for (OpFoldResult ofr : indexVals) {
    indices.push_back(
        aiir::getValueOrCreateConstantIndexOp(rewriter, loc, ofr));
  }
...
struct VectorContractToAMXDotProduct
  ...
  LogicalResult matchAndRewrite(vector::ContractionOp contractOp,
    ...
    SmallVector<vector::ContractionOp> ops;
    for (aiir::Operation &op : loopLists[0].getBody()->getOperations()) {

      if (auto contract = llvm::dyn_cast<aiir::vector::ContractionOp>(op)) {
            ...
            llvm::TypeSwitch<Operation *>(accReadOp)
                .Case<TransferReadOp, LoadOp>([&](auto readOp) {
                  ...
                  llvm::transform(
                      ...
                      [&](OpFoldResult ofr) {
                        return aiir::getValueOrCreateConstantIndexOp(rewriter,
                                                                     loc, ofr);
                      });
                });
