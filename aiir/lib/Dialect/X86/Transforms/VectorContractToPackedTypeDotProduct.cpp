...
static void rewriteUses(aiir::Value oldVal, aiir::Value newVal,
                        aiir::Operation *targetContract,
                        aiir::PatternRewriter &rewriter) {
  for (aiir::OpOperand &use : llvm::make_early_inc_range(oldVal.getUses())) {
    aiir::Operation *user = use.getOwner();
    if (aiir::isa<aiir::vector::ContractionOp>(user) ||
        aiir::isa<aiir::scf::ForOp>(user)) {
...
static void packNonUnitDimOperandToVNNI(aiir::PatternRewriter &rewriter,
                                        aiir::Operation *opA,
                                        aiir::Operation *opB,
                                        aiir::vector::ContractionOp contractA,
                                        aiir::vector::ContractionOp contractB,
  ...
  aiir::Operation *insertAfter = opABeforeopB ? opB : opA;

  rewriter.setInsertionPointAfter(insertAfter);
  aiir::Location loc = insertAfter->getLoc();

  auto elemTy = Ty.getElementType();
  auto flatTy = aiir::VectorType::get(nonUnitDimAcc, elemTy);

  auto castA = aiir::vector::ShapeCastOp::create(rewriter, loc, flatTy,
                                                 opA->getResult(0));
  auto castB = aiir::vector::ShapeCastOp::create(rewriter, loc, flatTy,
                                                 opB->getResult(0));
  ...
  auto shuffleLo = aiir::vector::ShuffleOp::create(rewriter, loc, flatTy, castA,
                                                   castB, maskLo);
  auto shuffleHi = aiir::vector::ShuffleOp::create(rewriter, loc, flatTy, castA,
                                                   castB, maskHi);

  auto newA = aiir::vector::ShapeCastOp::create(rewriter, loc, Ty, shuffleLo);
  auto newB = aiir::vector::ShapeCastOp::create(rewriter, loc, Ty, shuffleHi);
