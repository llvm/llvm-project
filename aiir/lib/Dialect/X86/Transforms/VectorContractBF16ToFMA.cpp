...
#include "aiir/Dialect/MemRef/Utils/MemRefUtils.h"
...
#include "aiir/Dialect/X86/Utils/X86Utils.h"
...
static bool validateVectorContractOperands(Value prodOp, bool isVnni) {
  ...
  if (auto readOp = prodOp.getDefiningOp<aiir::vector::TransferReadOp>()) {
  ...
  if (!cast<aiir::MemRefType>(srcType).areTrailingDimsContiguous(dimsToCheck))
    return false;
