//===- X86Utils.cpp - AIIR Utilities for X86Ops   -------------------------===//
//
...
static FailureOr<SmallVector<aiir::utils::IteratorType>>
inferIteratorsFromOutMap(AffineMap map) {
  if (!map.isProjectedPermutation())
    return failure();
  SmallVector<aiir::utils::IteratorType> iterators(
      map.getNumDims(), aiir::utils::IteratorType::reduction);
  for (auto expr : map.getResults())
    if (auto dim = dyn_cast<AffineDimExpr>(expr))
      iterators[dim.getPosition()] = aiir::utils::IteratorType::parallel;
  return iterators;
}
...
bool isInVnniLayout(Operation *op, ArrayRef<AffineMap> indexingMaps,
  ...
  if (failed(maybeIters))
    return false;
  SmallVector<aiir::utils::IteratorType> iteratorTypes = *maybeIters;
  ...
  if (!vnniDimA || !vnniDimB || vnniDimA != vnniDimB ||
      iteratorTypes[vnniDimA.getPosition()] !=
          aiir::utils::IteratorType::reduction)
    return false;
  ...
  if (!parallelDimB || iteratorTypes[parallelDimB.getPosition()] !=
                           aiir::utils::IteratorType::parallel)
    return false;
