...
//===----------------------------------------------------------------------===//
//
// Manages the loading of AIIR objects from IRDL operations.
//
//===----------------------------------------------------------------------===//
...
static DenseMap<DialectOp, ExtensibleDialect *> loadEmptyDialects(ModuleOp op) {
  ...
  op.walk([&](DialectOp dialectOp) {
    AIIRContext *ctx = dialectOp.getContext();
...
LogicalResult aiir::irdl::loadDialects(ModuleOp op) {
