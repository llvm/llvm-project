...
LogicalResult RegionConstraint::verify(aiir::Region &region,
                                       ConstraintVerifier &constraintContext) {
  const auto emitError = [parentOp = region.getParentOp()](aiir::Location loc) {
    return [loc, parentOp] {
      InFlightDiagnostic diag = aiir::emitError(loc);
  ...
  if (argumentConstraints.has_value()) {
    ...
    if (actualArgs.size() != argumentConstraints->size()) {
      const aiir::Location firstArgLoc =
    ...
    for (auto [arg, constraint] : llvm::zip(actualArgs, *argumentConstraints)) {
      aiir::Attribute type = TypeAttr::get(arg.getType());
