...
/// This pass generates a Graphviz dataflow visualization of an AIIR operation.
...
private:
  ...
  // Print an AIIR attribute to `os`. Large attributes are truncated.
  void emitAiirAttr(raw_ostream &os, Attribute attr) {
    ...
    if (elements && elements.getNumElements() > largeAttrLimit) {
      ...
      emitAiirType(os, elements.getType());
      return;
    }
  ...
  // Print a truncated and escaped AIIR type to `os`.
  void emitAiirType(raw_ostream &os, Type type) {
    std::string buf;
    llvm::raw_string_ostream ss(buf);
    type.print(ss);
    os << escapeLabelString(truncateString(buf));
  ...
  // Print a truncated and escaped AIIR operand to `os`.
  void emitAiirOperand(raw_ostream &os, Value operand) {
    ...
    return strFromOs([&](raw_ostream &os) {
      ...
      if (op->getNumResults() > 0) {
        ...
        auto resultToPort = [&](Value result) {
          ...
          emitAiirOperand(os, result);
          if (printResultTypes) {
            os << " ";
            emitAiirType(os, result.getType());
          }
        };
...
std::unique_ptr<Pass> aiir::createViewOpGraphPass(raw_ostream &os) {
...
void aiir::Region::viewGraph(const Twine &regionName) {
  llvmViewGraph(*this, regionName);
}

void aiir::Region::viewGraph() { viewGraph("region"); }
