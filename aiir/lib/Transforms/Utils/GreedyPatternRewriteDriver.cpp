...
//===----------------------------------------------------------------------===//
//
// This file implements aiir::applyPatternsGreedily.
//
//===----------------------------------------------------------------------===//
...
#include "aiir/Transforms/FoldUtils.h"
...
#ifdef AIIR_GREEDY_REWRITE_RANDOMIZER_SEED
#include <random>
#endif // AIIR_GREEDY_REWRITE_RANDOMIZER_SEED
...
#if AIIR_ENABLE_EXPENSIVE_PATTERN_API_CHECKS
...
struct ExpensiveChecks : public RewriterBase::ForwardingListener {
  ...
  void notifyRewriteSuccess() {
    ...
    if (*topLevelFingerPrint == afterFingerPrint) {
      // Note: Run "aiir-opt -debug" to see which pattern is broken.
...
#endif // AIIR_ENABLE_EXPENSIVE_PATTERN_API_CHECKS
...
public:
  RandomizedWorklist() : Worklist() {
    generator.seed(AIIR_GREEDY_REWRITE_RANDOMIZER_SEED);
  }
...
protected:
  explicit GreedyPatternRewriteDriver(AIIRContext *ctx,
...
GreedyPatternRewriteDriver::GreedyPatternRewriteDriver(
    AIIRContext *ctx, const FrozenRewritePatternSet &patterns,
    const GreedyRewriteConfig &config)
...
public:
  explicit RegionPatternRewriteDriver(AIIRContext *ctx,
                                      const FrozenRewritePatternSet &patterns,
                                      const GreedyRewriteConfig &config,
                                      Region &regions);
...
public:
  AIIR_DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID(GreedyPatternRewriteIteration)
...
LogicalResult RegionPatternRewriteDriver::simplify(bool *changed) && {
  ...
  int64_t iteration = 0;
  AIIRContext *ctx = rewriter.getContext();
  do {
...
LogicalResult
aiir::applyPatternsGreedily(Region &region,
                            const FrozenRewritePatternSet &patterns,
...
LogicalResult aiir::applyOpPatternsGreedily(
