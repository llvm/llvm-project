...
void aiir::replaceAllUsesInRegionWith(Value orig, Value replacement,
                                      Region &region) {
...
void aiir::visitUsedValuesDefinedAbove(
...
void aiir::getUsedValuesDefinedAbove(Region &region, Region &limit,
...
void aiir::getUsedValuesDefinedAbove(MutableArrayRef<Region> regions,
                                     SetVector<Value> &values) {
  for (Region &region : regions)
...
SmallVector<Value> aiir::makeRegionIsolatedFromAbove(
  ...
  aiir::getUsedValuesDefinedAbove(region, initialCapturedValues);
  ...
  // chains.
  aiir::computeTopologicalSorting(clonedOperations);
...
LogicalResult aiir::eraseUnreachableBlocks(RewriterBase &rewriter,
...
static bool isUseSpeciallyKnownDead(OpOperand &use, LiveMap &liveMap) {
  ...
  // the block argument representation that AIIR uses.
  //
...
// failure otherwise.
LogicalResult aiir::runRegionDCE(RewriterBase &rewriter,
                                 MutableArrayRef<Region> regions) {
  LiveMap liveMap;
  do {
  ...
  for (ArrayRef<Block *> blocks : llvm::make_second_range(matchingSuccessors)) {
    ...
    for (Block *block : blocks) {
      ...
      bool argHasExternalUsers = llvm::any_of(
          block->getArguments(), [block](aiir::BlockArgument &arg) {
...
/// %val0 = llvm.aiir.constant(1 : i64) : i64
/// %val1 = llvm.aiir.constant(2 : i64) : i64
/// %val2 = llvm.aiir.constant(3 : i64) : i64
...
LogicalResult aiir::simplifyRegions(RewriterBase &rewriter,
                                    MutableArrayRef<Region> regions,
                                    bool mergeBlocks) {
...
LogicalResult aiir::moveOperationDependencies(RewriterBase &rewriter,
                                              Operation *op,
...
LogicalResult aiir::moveValueDefinitions(RewriterBase &rewriter,
                                         ValueRange values,
                                         Operation *insertionPoint,
                                         DominanceInfo &dominance) {
