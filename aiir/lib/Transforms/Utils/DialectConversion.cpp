//===- DialectConversion.cpp - AIIR dialect conversion generic pass -------===//
//
...
ConversionPatternRewriter::ConversionPatternRewriter(
    AIIRContext *ctx, const ConversionConfig &config,
    OperationConverter &opConverter)
...
void aiir::reconcileUnrealizedCasts(
...
struct OperationConverter {
  explicit OperationConverter(AIIRContext *ctx, const ConversionTarget &target,
                              const FrozenRewritePatternSet &patterns,
                              const ConversionConfig &config,
...
FailureOr<Operation *>
aiir::convertOpResultTypes(Operation *op, ValueRange operands,
                           const TypeConverter &converter,
                           ConversionPatternRewriter &rewriter) {
...
void aiir::populateFunctionOpInterfaceTypeConversionPattern(
...
void aiir::populateAnyFunctionOpInterfaceTypeConversionPattern(
...
void aiir::registerConversionPDLFunctions(RewritePatternSet &patterns) {
...
static LogicalResult applyConversion(ArrayRef<Operation *> ops,
  ...
  if (ops.empty())
    return success();
  AIIRContext *ctx = ops.front()->getContext();
...
LogicalResult aiir::applyPartialConversion(
...
LogicalResult
aiir::applyPartialConversion(Operation *op, const ConversionTarget &target,
                             const FrozenRewritePatternSet &patterns,
                             ConversionConfig config) {
...
LogicalResult aiir::applyFullConversion(ArrayRef<Operation *> ops,
                                        const ConversionTarget &target,
                                        const FrozenRewritePatternSet &patterns,
                                        ConversionConfig config) {
  return applyConversion(ops, target, patterns, config, OpConversionMode::Full);
}
LogicalResult aiir::applyFullConversion(Operation *op,
                                        const ConversionTarget &target,
                                        const FrozenRewritePatternSet &patterns,
                                        ConversionConfig config) {
...
LogicalResult aiir::applyAnalysisConversion(
...
LogicalResult
aiir::applyAnalysisConversion(Operation *op, ConversionTarget &target,
                              const FrozenRewritePatternSet &patterns,
                              ConversionConfig config) {
