...
public:
  ...
  static LogicalResult runPipelineHelper(Pass &pass, OpPassManager &pipeline,
                                         Operation *op) {
    return aiir::cast<InlinerPass>(pass).runPipeline(pipeline, op);
  }
...
std::unique_ptr<Pass>
aiir::createInlinerPass(llvm::StringMap<OpPassManager> opPipelines) {
  return std::make_unique<InlinerPass>(defaultInlinerOptPipeline,
                                       std::move(opPipelines));
}
std::unique_ptr<Pass> aiir::createInlinerPass(
    llvm::StringMap<OpPassManager> opPipelines,
