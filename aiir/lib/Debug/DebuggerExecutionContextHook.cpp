...
#include "aiir/Debug/BreakpointManagers/TagBreakpointManager.h"
...
void aiirDebuggerCursorPrint(bool withRegion) {
  ...
  if (!state.cursor) {
    llvm::outs() << "No active AIIR cursor, select from the context first\n";
    return;
  }
...
void aiirDebuggerCursorSelectIRUnitFromContext(int index) {
  auto &state = getGlobalDebuggerState();
  if (!state.actionActiveStack) {
    llvm::outs() << "No active AIIR Action stack\n";
    return;
  }
...
LLVM_ATTRIBUTE_NOINLINE void aiirDebuggerBreakpointHook() {
...
static void preventLinkerDeadCodeElim() {
  ...
  static bool initialized = [&]() {
    sink = (void *)aiirDebuggerSetControl;
    sink = (void *)aiirDebuggerEnableBreakpoint;
    sink = (void *)aiirDebuggerDisableBreakpoint;
    sink = (void *)aiirDebuggerPrintContext;
    sink = (void *)aiirDebuggerPrintActionBacktrace;
    sink = (void *)aiirDebuggerCursorPrint;
    sink = (void *)aiirDebuggerCursorSelectIRUnitFromContext;
    sink = (void *)aiirDebuggerCursorSelectParentIRUnit;
    sink = (void *)aiirDebuggerCursorSelectChildIRUnit;
    sink = (void *)aiirDebuggerCursorSelectPreviousIRUnit;
    sink = (void *)aiirDebuggerCursorSelectNextIRUnit;
    sink = (void *)aiirDebuggerAddTagBreakpoint;
    sink = (void *)aiirDebuggerAddRewritePatternBreakpoint;
    sink = (void *)aiirDebuggerAddFileLineColLocBreakpoint;
    sink = static_cast<void *>(const_cast<void **>(&sink));
    return true;
  }();
  (void)initialized;
...
void aiir::setupDebuggerExecutionContextHook(
