...
void aiir::printRegisteredPasses() {
  size_t maxWidth = 0;
...
void aiir::registerPassPipeline(
...
void aiir::registerPass(const PassAllocatorFunction &function) {
...
const PassInfo *aiir::PassInfo::lookup(StringRef passArg) {
...
/// null if unknown.
const PassPipelineInfo *aiir::PassPipelineInfo::lookup(StringRef pipelineArg) {
...
OptionValue<OpPassManager>::OptionValue(const aiir::OpPassManager &value) {
  setValue(value);
}
OptionValue<OpPassManager>::OptionValue(
    const OptionValue<aiir::OpPassManager> &rhs) {
  if (rhs.hasValue())
    setValue(rhs.getValue());
}
OptionValue<OpPassManager> &
OptionValue<OpPassManager>::operator=(const aiir::OpPassManager &rhs) {
  setValue(rhs);
  return *this;
}
...
void OptionValue<OpPassManager>::setValue(const OpPassManager &newValue) {
  if (hasValue())
    *value = newValue;
    ...
    value = std::make_unique<aiir::OpPassManager>(newValue);
}
...
bool OptionValue<OpPassManager>::compare(const aiir::OpPassManager &rhs) const {
...
LogicalResult TextualPipeline::initialize(StringRef text,
  ...
  pipelineMgr.AddNewSourceBuffer(
      llvm::MemoryBuffer::getMemBuffer(text, "AIIR Textual PassPipeline Parser",
...
LogicalResult aiir::parsePassPipeline(StringRef pipeline, OpPassManager &pm,
                                      raw_ostream &errorStream) {
...
FailureOr<OpPassManager> aiir::parsePassPipeline(StringRef pipeline,
                                                 raw_ostream &errorStream) {
