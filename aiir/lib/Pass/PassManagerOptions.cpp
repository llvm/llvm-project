...
struct PassManagerOptions {
  ...
  llvm::cl::opt<std::string> reproducerFile{
      "aiir-pass-pipeline-crash-reproducer",
      llvm::cl::desc("Generate a .aiir reproducer file at the given output path"
                     " if the pass manager crashes or fails")};
  llvm::cl::opt<bool> localReproducer{
      "aiir-pass-pipeline-local-reproducer",
  ...
  // IR Printing
  //===--------------------------------------------------------------------===//
  PassNameCLParser printBefore{"aiir-print-ir-before",
                               "Print IR before specified passes"};
  PassNameCLParser printAfter{"aiir-print-ir-after",
                              "Print IR after specified passes"};
  llvm::cl::opt<bool> printBeforeAll{
      "aiir-print-ir-before-all", llvm::cl::desc("Print IR before each pass"),
      llvm::cl::init(false)};
  llvm::cl::opt<bool> printAfterAll{"aiir-print-ir-after-all",
...
void aiir::registerPassManagerCLOptions() {
  // Make sure that the options struct has been constructed.
  *options;
}

LogicalResult aiir::applyPassManagerCLOptions(PassManager &pm) {
...
void aiir::applyDefaultTimingPassManagerCLOptions(PassManager &pm) {
