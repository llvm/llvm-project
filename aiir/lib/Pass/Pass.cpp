...
AIIR_DEFINE_EXPLICIT_TYPE_ID(::aiir::PassExecutionAction)
...
struct OpPassManagerImpl {
  ...
  /// passes, etc.
  LogicalResult finalizePassList(AIIRContext *ctx);

  /// Return the operation name of this pass manager.
  std::optional<OperationName> getOpName(AIIRContext &context) {
  ...
  /// operation type.
  bool canScheduleOn(AIIRContext &context, OperationName opName);
...
LogicalResult OpPassManagerImpl::finalizePassList(AIIRContext *ctx) {
...
bool OpPassManagerImpl::canScheduleOn(AIIRContext &context,
...
OpPassManager::getOpName(AIIRContext &context) const {
...
void printAsTextualPipeline(
  ...
  llvm::interleave(
      passes,
      [&](aiir::Pass &pass) { pass.printAsTextualPipeline(os, pretty); },
      [&]() {
        os << ",";
        if (pretty)
          os << "\n";
...
LogicalResult OpPassManager::initialize(AIIRContext *context,
...
LogicalResult OpToOpPassAdaptor::tryMergeInto(AIIRContext *ctx,
...
PassManager::PassManager(AIIRContext *ctx, StringRef operationName,
                         Nesting nesting)
