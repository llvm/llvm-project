...
using namespace aiir::remark::detail;
...
void Remark::print(llvm::raw_ostream &os, bool printLocation) const {
  ...
  if (printLocation) {
    if (auto flc = aiir::dyn_cast<aiir::FileLineColLoc>(getLocation())) {
...
llvm::remarks::Remark Remark::generateRemark() const {
  ...
  // AIIR does not use passes; instead, it has categories and sub-categories.
...
llvm::LogicalResult RemarkEngine::initialize(
    std::unique_ptr<AIIRRemarkStreamerBase> streamer,
...
buildFilter(const aiir::remark::RemarkCategories &cats,
...
llvm::LogicalResult aiir::remark::enableOptimizationRemarks(
    AIIRContext &ctx, std::unique_ptr<detail::AIIRRemarkStreamerBase> streamer,
