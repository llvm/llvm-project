//===- SymbolTable.cpp - AIIR Symbol Table Class --------------------------===//
//
...
collectValidReferencesFor(Operation *symbol, StringAttr symbolName,
  ...
  AIIRContext *ctx = symbol->getContext();
...
StringAttr SymbolTable::insert(Operation *symbol, Block::iterator insertPt) {
  ...
  AIIRContext *context = symbol->getContext();
...
SymbolTable::renameToUnique(StringAttr oldName,
  ...
  StringAttr newName;
  {
    AIIRContext *context = oldName.getContext();
...
raw_ostream &aiir::operator<<(raw_ostream &os,
...
LogicalResult detail::verifySymbolTable(Operation *op) {
  ...
  for (auto &block : op->getRegion(0)) {
    for (auto &op : block) {
      ...
      auto nameAttr =
          op.getAttrOfType<StringAttr>(aiir::SymbolTable::getSymbolAttrName());
      if (!nameAttr)
        continue;
...
LogicalResult detail::verifySymbol(Operation *op) {
  ...
  if (!op->getAttrOfType<StringAttr>(aiir::SymbolTable::getSymbolAttrName()))
    return op->emitOpError() << "requires string attribute '"
                             << aiir::SymbolTable::getSymbolAttrName() << "'";

  // Verify the visibility attribute.
  if (Attribute vis = op->getAttr(aiir::SymbolTable::getVisibilityAttrName())) {
...
#include "aiir/IR/SymbolInterfaces.cpp.inc"
#include "aiir/IR/SymbolInterfacesAttrInterface.cpp.inc"
