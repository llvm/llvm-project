//===- AffineExpr.cpp - AIIR Affine Expr Classes --------------------------===//
//
...
AIIRContext *AffineExpr::getContext() const { return expr->context; }
...
WalkRetTy aiir::AffineExpr::walk(AffineExpr e,
...
template void aiir::AffineExpr::walk(AffineExpr e,
                                     function_ref<void(AffineExpr)> callback);
template WalkResult
aiir::AffineExpr::walk(AffineExpr e,
                       function_ref<WalkResult(AffineExpr)> callback);

// Dispatch affine expression construction based on kind.
AffineExpr aiir::getAffineBinaryOpExpr(AffineExprKind kind, AffineExpr lhs,
                                       AffineExpr rhs) {
...
bool AffineExpr::isFunctionOfDim(unsigned position) const {
  if (getKind() == AffineExprKind::DimId) {
    return *this == aiir::getAffineDimExpr(position, getContext());
  }
...
bool AffineExpr::isFunctionOfSymbol(unsigned position) const {
  if (getKind() == AffineExprKind::SymbolId) {
    return *this == aiir::getAffineSymbolExpr(position, getContext());
  }
  if (auto expr = llvm::dyn_cast<AffineBinaryOpExpr>(*this)) {
...
AffineExpr aiir::getAffineDimExpr(unsigned position, AIIRContext *context) {
...
AffineExpr aiir::getAffineSymbolExpr(unsigned position, AIIRContext *context) {
...
AffineExpr aiir::getAffineConstantExpr(int64_t constant, AIIRContext *context) {
...
aiir::getAffineConstantExprs(ArrayRef<int64_t> constants,
                             AIIRContext *context) {
...
raw_ostream &aiir::operator<<(raw_ostream &os, AffineExpr expr) {
  expr.print(os);
  return os;
}
...
AffineExpr aiir::getAffineExprFromFlatForm(ArrayRef<int64_t> flatExprs,
                                           unsigned numDims,
                                           unsigned numSymbols,
...
AffineExpr aiir::simplifyAffineExpr(AffineExpr expr, unsigned numDims,
                                    unsigned numSymbols) {
...
std::optional<int64_t> aiir::getBoundForAffineExpr(
