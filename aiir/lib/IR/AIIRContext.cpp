//===- AIIRContext.cpp - AIIR Type Classes --------------------------------===//
//
...
#define DEBUG_TYPE "aiircontext"

using namespace aiir;
using namespace aiir::detail;

//===----------------------------------------------------------------------===//
// AIIRContext CommandLine Options
//===----------------------------------------------------------------------===//

namespace {
/// This struct contains command line options that can be used to initialize
/// various bits of an AIIRContext. This uses a struct wrapper to avoid the need
/// for global command line options.
struct AIIRContextOptions {
  llvm::cl::opt<bool> disableThreading{
      "aiir-disable-threading",
      llvm::cl::desc("Disable multi-threading within AIIR, overrides any "
                     "further call to AIIRContext::enableMultiThreading()")};

  llvm::cl::opt<bool> printOpOnDiagnostic{
      "aiir-print-op-on-diagnostic",
  ...
  llvm::cl::opt<bool> printStackTraceOnDiagnostic{
      "aiir-print-stacktrace-on-diagnostic",
      llvm::cl::desc("When a diagnostic is emitted, also print the stack trace "
                     "as an attached note")};
};
} // namespace

static llvm::ManagedStatic<AIIRContextOptions> clOptions;
...
/// various flags within the AIIRContext. These flags are used when constructing
/// an AIIR context for initialization.
void aiir::registerAIIRContextCLOptions() {
...
namespace aiir {
/// This is the implementation of the AIIRContext class, using the pImpl idiom.
/// This class is completely private to this file, so everything is public.
class AIIRContextImpl {
public:
  //===--------------------------------------------------------------------===//
  // Remark
  //===--------------------------------------------------------------------===//
  ...
  /// Enable support for multi-threading within AIIR.
  ...
  /// This points to the ThreadPool used when processing AIIR tasks in parallel.
  ...
  /// The AIIRContext owns the objects. These need to be declared after the
...
public:
  AIIRContextImpl(bool threadingIsEnabled)
...
AIIRContext::AIIRContext(Threading setting)
    : AIIRContext(DialectRegistry(), setting) {}

AIIRContext::AIIRContext(const DialectRegistry &registry, Threading setting)
    : impl(new AIIRContextImpl(setting == Threading::ENABLED &&
...
void AIIRContext::registerActionHandler(HandlerTy handler) {
  getImpl().actionHandler = std::move(handler);
}

/// Dispatch the provided action to the handler if any, or just execute it.
void AIIRContext::executeActionInternal(function_ref<void()> actionFn,
                                        const tracing::Action &action) {
  assert(getImpl().actionHandler);
  getImpl().actionHandler(actionFn, action);
}

bool AIIRContext::hasActionHandler() { return (bool)getImpl().actionHandler; }
...
DiagnosticEngine &AIIRContext::getDiagEngine() { return getImpl().diagEngine; }

//===----------------------------------------------------------------------===//
// Remark Handlers
//===----------------------------------------------------------------------===//

void AIIRContext::setRemarkEngine(
    std::unique_ptr<remark::detail::RemarkEngine> engine) {
  getImpl().remarkEngine = std::move(engine);
}

remark::detail::RemarkEngine *AIIRContext::getRemarkEngine() {
...
void AIIRContext::appendDialectRegistry(const DialectRegistry &registry) {
  if (registry.isSubsetOf(impl->dialectsRegistry))
    return;

  assert(impl->multiThreadedExecutionContext == 0 &&
         "appending to the AIIRContext dialect registry while in a "
...
const DialectRegistry &AIIRContext::getDialectRegistry() {
  return impl->dialectsRegistry;
}

/// Return information about all registered IR dialects.
std::vector<Dialect *> AIIRContext::getLoadedDialects() {
...
std::vector<StringRef> AIIRContext::getAvailableDialects() {
...
/// then return nullptr.
Dialect *AIIRContext::getLoadedDialect(StringRef name) {
...
Dialect *AIIRContext::getOrLoadDialect(StringRef name) {
...
Dialect *
AIIRContext::getOrLoadDialect(StringRef dialectNamespace, TypeID dialectID,
...
bool AIIRContext::isDialectLoading(StringRef dialectNamespace) {
...
DynamicDialect *AIIRContext::getOrLoadDynamicDialect(
...
void AIIRContext::loadAllAvailableDialects() {
  for (StringRef name : getAvailableDialects())
    getOrLoadDialect(name);
}

llvm::hash_code AIIRContext::getRegistryHash() {
...
bool AIIRContext::allowsUnregisteredDialects() {
  return impl->allowUnregisteredDialects;
}

void AIIRContext::allowUnregisteredDialects(bool allowing) {
  assert(impl->multiThreadedExecutionContext == 0 &&
         "changing AIIRContext `allow-unregistered-dialects` configuration "
         "while in a multi-threaded execution context");
  impl->allowUnregisteredDialects = allowing;
}

/// Return true if multi-threading is enabled by the context.
bool AIIRContext::isMultithreadingEnabled() {
  return impl->threadingIsEnabled && llvm::llvm_is_multithreaded();
}

/// Set the flag specifying if multi-threading is disabled by the context.
void AIIRContext::disableMultithreading(bool disable) {
  // This API can be overridden by the global debugging flag
  // --aiir-disable-threading
  if (isThreadingGloballyDisabled())
    return;
  assert(impl->multiThreadedExecutionContext == 0 &&
         "changing AIIRContext `disable-threading` configuration while "
...
void AIIRContext::setThreadPool(llvm::ThreadPoolInterface &pool) {
...
unsigned AIIRContext::getNumThreads() {
...
llvm::ThreadPoolInterface &AIIRContext::getThreadPool() {
  assert(isMultithreadingEnabled() &&
...
void AIIRContext::enterMultiThreadedExecution() {
#ifndef NDEBUG
  ++impl->multiThreadedExecutionContext;
#endif
}
void AIIRContext::exitMultiThreadedExecution() {
#ifndef NDEBUG
  --impl->multiThreadedExecutionContext;
#endif
}
...
/// Operation::emit.
bool AIIRContext::shouldPrintOpOnDiagnostic() {
...
void AIIRContext::printOpOnDiagnostic(bool enable) {
  assert(impl->multiThreadedExecutionContext == 0 &&
         "changing AIIRContext `print-op-on-diagnostic` configuration while in "
...
/// emitted.
bool AIIRContext::shouldPrintStackTraceOnDiagnostic() {
  return impl->printStackTraceOnDiagnostic;
}

/// Set the flag specifying if we should attach the current stacktrace when
/// emitting diagnostics.
void AIIRContext::printStackTraceOnDiagnostic(bool enable) {
  assert(impl->multiThreadedExecutionContext == 0 &&
         "changing AIIRContext `print-stacktrace-on-diagnostic` configuration "
         "while in a multi-threaded execution context");
  impl->printStackTraceOnDiagnostic = enable;
}

/// Return information about all registered operations.
ArrayRef<RegisteredOperationName> AIIRContext::getRegisteredOperations() {
...
AIIRContext::getRegisteredOperationsByDialect(StringRef dialectName) {
  auto *lowerBound =
...
bool AIIRContext::isOperationRegistered(StringRef name) {
...
const AbstractAttribute &AbstractAttribute::lookup(TypeID typeID,
  ...
  if (!abstract)
    llvm::report_fatal_error("Trying to create an Attribute that was not "
                             "registered in this AIIRContext.");
  return *abstract;
}
...
AbstractAttribute::lookup(StringRef name, AIIRContext *context) {
  AIIRContextImpl &impl = context->getImpl();
...
OperationName::OperationName(StringRef name, AIIRContext *context) {
  AIIRContextImpl &ctxImpl = context->getImpl();
...
RegisteredOperationName::lookup(TypeID typeID, AIIRContext *ctx) {
...
std::optional<RegisteredOperationName>
RegisteredOperationName::lookup(StringRef name, AIIRContext *ctx) {
  auto &impl = ctx->getImpl();
...
void RegisteredOperationName::insert(
  ...
  AIIRContext *ctx = impl->getDialect()->getContext();
...
const AbstractType &AbstractType::lookup(TypeID typeID, AIIRContext *context) {
  const AbstractType *type = lookupMutable(typeID, context);
  if (!type)
    llvm::report_fatal_error(
        "Trying to create a Type that was not registered in this AIIRContext.");
  return *type;
}

AbstractType *AbstractType::lookupMutable(TypeID typeID, AIIRContext *context) {
  auto &impl = context->getImpl();
  return impl.registeredTypes.lookup(typeID);
}

std::optional<std::reference_wrapper<const AbstractType>>
AbstractType::lookup(StringRef name, AIIRContext *context) {
  AIIRContextImpl &impl = context->getImpl();
...
StorageUniquer &AIIRContext::getTypeUniquer() { return getImpl().typeUniquer; }

BFloat16Type BFloat16Type::get(AIIRContext *context) {
  return context->getImpl().bf16Ty;
}
Float16Type Float16Type::get(AIIRContext *context) {
  return context->getImpl().f16Ty;
}
FloatTF32Type FloatTF32Type::get(AIIRContext *context) {
  return context->getImpl().tf32Ty;
}
Float32Type Float32Type::get(AIIRContext *context) {
  return context->getImpl().f32Ty;
}
Float64Type Float64Type::get(AIIRContext *context) {
  return context->getImpl().f64Ty;
}
Float80Type Float80Type::get(AIIRContext *context) {
  return context->getImpl().f80Ty;
}
Float128Type Float128Type::get(AIIRContext *context) {
  return context->getImpl().f128Ty;
}

/// Get an instance of the IndexType.
IndexType IndexType::get(AIIRContext *context) {
...
IntegerType IntegerType::get(AIIRContext *context, unsigned width,
                             IntegerType::SignednessSemantics signedness) {
...
IntegerType::getChecked(function_ref<InFlightDiagnostic()> emitError,
                        AIIRContext *context, unsigned width,
...
NoneType NoneType::get(AIIRContext *context) {
...
StorageUniquer &AIIRContext::getAttributeUniquer() {
...
BoolAttr BoolAttr::get(AIIRContext *context, bool value) {
  return value ? context->getImpl().trueAttr : context->getImpl().falseAttr;
}

UnitAttr UnitAttr::get(AIIRContext *context) {
  return context->getImpl().unitAttr;
}

UnknownLoc UnknownLoc::get(AIIRContext *context) {
  return context->getImpl().unknownLocAttr;
}

DistinctAttrStorage *
detail::DistinctAttributeUniquer::allocateStorage(AIIRContext *context,
                                                  Attribute referencedAttr) {
  return context->getImpl().distinctAttributeAllocator.allocate(referencedAttr);
}

/// Return empty dictionary.
DictionaryAttr DictionaryAttr::getEmpty(AIIRContext *context) {
  return context->getImpl().emptyDictionaryAttr;
}

void StringAttrStorage::initialize(AIIRContext *context) {
...
StringAttr StringAttr::get(AIIRContext *context) {
...
StorageUniquer &AIIRContext::getAffineUniquer() {
...
AffineMap AffineMap::get(AIIRContext *context) {
...
aiir::detail::getDefaultDiagnosticEmitFn(AIIRContext *ctx) {
  return [ctx] { return emitError(UnknownLoc::get(ctx)); };
}
llvm::unique_function<InFlightDiagnostic()>
aiir::detail::getDefaultDiagnosticEmitFn(const Location &loc) {
