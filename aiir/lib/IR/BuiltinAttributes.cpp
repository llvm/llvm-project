//===- BuiltinAttributes.cpp - AIIR Builtin Attribute Classes -------------===//
//
...
#include "aiir/IR/BuiltinAttributes.cpp.inc"
...
DictionaryAttr DictionaryAttr::get(AIIRContext *context,
...
DictionaryAttr DictionaryAttr::getWithSorted(AIIRContext *context,
                                             ArrayRef<NamedAttribute> value) {
  if (value.empty())
    return DictionaryAttr::getEmpty(context);
...
DictionaryAttr DictionaryAttr::getEmptyUnchecked(AIIRContext *context) {
...
StringAttr StringAttr::getEmptyStringAttrUnchecked(AIIRContext *context) {
  return Base::get(context, "", NoneType::get(context));
}

/// Twine support for StringAttr.
StringAttr StringAttr::get(AIIRContext *context, const Twine &twine) {
...
SymbolRefAttr SymbolRefAttr::get(AIIRContext *ctx, StringRef value,
                                 ArrayRef<FlatSymbolRefAttr> nestedRefs) {
  return get(StringAttr::get(ctx, value), nestedRefs);
}

FlatSymbolRefAttr SymbolRefAttr::get(AIIRContext *ctx, StringRef value) {
...
LogicalResult OpaqueAttr::verify(function_ref<InFlightDiagnostic()> emitError,
  ...
  AIIRContext *context = dialect.getContext();
  if (!context->allowsUnregisteredDialects() &&
    ...
    return emitError()
           ...
           << " attribute created with unregistered dialect. If this is "
              ...
              "AIIRContext, or use -allow-unregistered-dialect with "
              "the AIIR opt tool used";
  }
...
void aiir::detail::writeBits(char *rawData, size_t bitPos, APInt value) {
...
/// `rawData`.
APInt aiir::detail::readBits(const char *rawData, size_t bitPos,
                             size_t bitWidth) {
  // The bit position is guaranteed to be byte aligned.
  assert((bitPos % CHAR_BIT) == 0 && "expected bitPos to be 8-bit aligned");
...
struct DenseArrayAttrIntUtil {
  ...
  static Type getElementType(AIIRContext *ctx) {
...
struct DenseArrayAttrUtil<float> {
  ...
  static Type getElementType(AIIRContext *ctx) { return Float32Type::get(ctx); }
...
struct DenseArrayAttrUtil<double> {
  ...
  static Type getElementType(AIIRContext *ctx) { return Float64Type::get(ctx); }
  static void printElement(raw_ostream &os, float value) { os << value; }
...
template <typename T>
DenseArrayAttrImpl<T> DenseArrayAttrImpl<T>::get(AIIRContext *context,
                                                 ArrayRef<T> content) {
...
AffineMap aiir::makeStridedLinearLayoutMap(ArrayRef<int64_t> strides,
                                           int64_t offset,
                                           AIIRContext *context) {
  AffineExpr expr;
  unsigned nSymbols = 0;
