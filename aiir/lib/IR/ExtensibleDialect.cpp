...
std::unique_ptr<DynamicOpDefinition> DynamicOpDefinition::get(
  ...
  auto getCanonicalizationPatternsFn = [](RewritePatternSet &, AIIRContext *) {
  };
...
public:
  ...
  AIIR_DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID(IsExtensibleDialect)
};
} // namespace

ExtensibleDialect::ExtensibleDialect(StringRef name, AIIRContext *ctx,
                                     TypeID typeID)
...
public:
  ...
  AIIR_DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID(IsDynamicDialect)
};
} // namespace

DynamicDialect::DynamicDialect(StringRef name, AIIRContext *ctx)
    : SelfOwningTypeID(),
