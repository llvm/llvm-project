//===- AffineMap.cpp - AIIR Affine Map Classes ----------------------------===//
//
...
AffineMap AffineMap::getConstantMap(int64_t val, AIIRContext *context) {
...
AffineMap AffineMap::getFilteredIdentityMap(
    AIIRContext *ctx, unsigned numDims,
...
AffineMap AffineMap::getMultiDimIdentityMap(unsigned numDims,
  ...
  for (unsigned i = 0; i < numDims; ++i)
    dimExprs.push_back(aiir::getAffineDimExpr(i, context));
  return get(/*dimCount=*/numDims, /*symbolCount=*/0, dimExprs, context);
}

AIIRContext *AffineMap::getContext() const { return map->context; }
...
AffineMap aiir::simplifyAffineMap(AffineMap map) {
  SmallVector<AffineExpr, 8> exprs;
...
AffineMap aiir::removeDuplicateExprs(AffineMap map) {
...
AffineMap aiir::inversePermutation(AffineMap map) {
  if (map.isEmpty())
    return map;
...
AffineMap aiir::inverseAndBroadcastProjectedPermutation(AffineMap map) {
  assert(map.isProjectedPermutation(/*allowZeroInResults=*/true));
  AIIRContext *context = map.getContext();
  AffineExpr zero = aiir::getAffineConstantExpr(0, context);
...
AffineMap aiir::concatAffineMaps(ArrayRef<AffineMap> maps,
                                 AIIRContext *context) {
  if (maps.empty())
...
AffineMap aiir::projectDims(AffineMap map,
...
AffineMap aiir::projectSymbols(AffineMap map,
...
AffineMap aiir::getProjectedMap(AffineMap map,
                                const llvm::SmallBitVector &projectedDimensions,
                                bool compressDimsFlag,
                                bool compressSymbolsFlag) {
...
llvm::SmallBitVector aiir::getUnusedDimsBitVector(ArrayRef<AffineMap> maps) {
...
llvm::SmallBitVector aiir::getUnusedSymbolsBitVector(ArrayRef<AffineMap> maps) {
...
AffineMap
aiir::expandDimsToRank(AffineMap map, int64_t rank,
                       const llvm::SmallBitVector &projectedDimensions) {
