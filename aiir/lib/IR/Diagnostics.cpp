//===- Diagnostics.cpp - AIIR Diagnostics ---------------------------------===//
//
...
emitDiag(Location location, DiagnosticSeverity severity, const Twine &message) {
  AIIRContext *ctx = location->getContext();
...
InFlightDiagnostic aiir::emitError(Location loc) { return emitError(loc, {}); }
InFlightDiagnostic aiir::emitError(Location loc, const Twine &message) {
  return emitDiag(loc, DiagnosticSeverity::Error, message);
}

/// Emit a warning message using this location.
InFlightDiagnostic aiir::emitWarning(Location loc) {
  return emitWarning(loc, {});
}
InFlightDiagnostic aiir::emitWarning(Location loc, const Twine &message) {
  return emitDiag(loc, DiagnosticSeverity::Warning, message);
}

/// Emit a remark message using this location.
InFlightDiagnostic aiir::emitRemark(Location loc) {
  return emitRemark(loc, {});
}
InFlightDiagnostic aiir::emitRemark(Location loc, const Twine &message) {
...
SourceMgrDiagnosticHandler::SourceMgrDiagnosticHandler(
    llvm::SourceMgr &mgr, AIIRContext *ctx, raw_ostream &os,
...
SourceMgrDiagnosticHandler::SourceMgrDiagnosticHandler(
    llvm::SourceMgr &mgr, AIIRContext *ctx, ShouldShowLocFn &&shouldShowLocFn)
...
SourceMgrDiagnosticVerifierHandler::SourceMgrDiagnosticVerifierHandler(
    llvm::SourceMgr &srcMgr, AIIRContext *ctx, raw_ostream &out, Level level)
...
SourceMgrDiagnosticVerifierHandler::SourceMgrDiagnosticVerifierHandler(
    llvm::SourceMgr &srcMgr, AIIRContext *ctx, Level level)
...
void SourceMgrDiagnosticVerifierHandler::registerInContext(AIIRContext *ctx) {
...
struct ParallelDiagnosticHandlerImpl : public llvm::PrettyStackTraceEntry {
  ...
  ParallelDiagnosticHandlerImpl(AIIRContext *ctx) : context(ctx) {
...
ParallelDiagnosticHandler::ParallelDiagnosticHandler(AIIRContext *ctx)
