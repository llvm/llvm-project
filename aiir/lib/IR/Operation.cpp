...
InFlightDiagnostic Operation::emitError(const Twine &message) {
  InFlightDiagnostic diag = aiir::emitError(getLoc(), message);
...
InFlightDiagnostic Operation::emitWarning(const Twine &message) {
  InFlightDiagnostic diag = aiir::emitWarning(getLoc(), message);
  if (getContext()->shouldPrintOpOnDiagnostic())
...
InFlightDiagnostic Operation::emitRemark(const Twine &message) {
  InFlightDiagnostic diag = aiir::emitRemark(getLoc(), message);
  if (getContext()->shouldPrintOpOnDiagnostic())
    diag.attachNote(getLoc()) << "see current operation: " << *this;
  return diag;
}
...
auto llvm::ilist_detail::SpecificNodeAccess<
    llvm::ilist_detail::compute_node_options<::aiir::Operation>::type>::
...
void llvm::ilist_traits<::aiir::Operation>::deleteNode(Operation *op) {
  op->destroy();
}

Block *llvm::ilist_traits<::aiir::Operation>::getContainingBlock() {
...
void llvm::ilist_traits<::aiir::Operation>::addNodeToList(Operation *op) {
...
void llvm::ilist_traits<::aiir::Operation>::removeNodeFromList(Operation *op) {
...
void llvm::ilist_traits<::aiir::Operation>::transferNodesFromList(
...
void OpState::genericPrintProperties(OpAsmPrinter &p, Attribute properties,
  ...
  if (!properties)
    return;
  auto dictAttr = dyn_cast_or_null<::aiir::DictionaryAttr>(properties);
