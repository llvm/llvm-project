//===- Location.cpp - AIIR Location Classes -------------------------------===//
//
...
struct FileLineColRangeAttrStorage final
  ...
  construct(::aiir::AttributeStorageAllocator &allocator, KeyTy &&tblgenKey) {
...
#include "aiir/IR/BuiltinLocationAttributes.cpp.inc"
...
FileLineColLoc FileLineColLoc::get(AIIRContext *context, StringRef fileName,
...
bool aiir::isStrictFileLineColLoc(Location loc) {
  if (auto range = aiir::dyn_cast<FileLineColRange>(loc))
