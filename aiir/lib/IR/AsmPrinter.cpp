//===- AsmPrinter.cpp - AIIR Assembly Printer Implementation --------------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file implements the AIIR AsmPrinter class, which is used to implement
...
#define DEBUG_TYPE "aiir-asm-printer"
...
AIIRContext *AsmParser::getContext() const { return getBuilder().getContext(); }
...
#include "aiir/IR/OpAsmAttrInterface.cpp.inc"
#include "aiir/IR/OpAsmOpInterface.cpp.inc"
#include "aiir/IR/OpAsmTypeInterface.cpp.inc"
...
struct AsmPrinterOptions {
  llvm::cl::opt<int64_t> printElementsAttrWithHexIfLarger{
      "aiir-print-elementsattrs-with-hex-if-larger",
      llvm::cl::desc(
  ...
  llvm::cl::opt<unsigned> elideElementsAttrIfLarger{
      "aiir-elide-elementsattrs-if-larger",
  ...
  llvm::cl::opt<bool> useNameLocAsPrefix{
      "aiir-use-nameloc-as-prefix", llvm::cl::init(false),
...
void aiir::registerAsmPrinterCLOptions() {
...
public:
  ...
  /// custom dialects. Usage in AIIR dialects is disallowed.
  void printNewline() {
    os << newLine;
...
private:
  ...
  void printGenericOp(Operation *op, bool printOpName = true) override {
    ...
    // --aiir-very-unsafe-disable-verifier-on-parsing), so guard against them.
...
StringRef aiir::toString(AsmResourceEntryKind kind) {
  switch (kind) {
...
public:
  ...
  explicit AsmStateImpl(AIIRContext *ctx, const OpPrintingFlags &printerFlags,
                        AsmState::LocationMap *locationMap)
...
AsmState::AsmState(AIIRContext *ctx, const OpPrintingFlags &printerFlags,
                   LocationMap *locationMap, FallbackAsmResourceMap *map)
...
private:
  /// This class represents a resource builder implementation for the AIIR
...
raw_ostream &aiir::operator<<(raw_ostream &os, Value value) {
...
raw_ostream &aiir::operator<<(raw_ostream &os, Block &block) {
  block.print(os);
  return os;
}
