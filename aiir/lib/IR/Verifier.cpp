//===- Verifier.cpp - AIIR Verifier Implementation ------------------------===//
//
...
// attributes, etc.  AIIR supports transformations moving IR through locally
...
LogicalResult OperationVerifier::verifyOnEntrance(Block &block) {
  ...
  AIIRContext *blockCtx = parentOp ? parentOp->getContext() : nullptr;
    ...
    if (blockCtx) {
      ...
      if (arg.getLoc().getContext() != blockCtx)
        return emitError(parentOp->getLoc(), "block argument #")
               << idx
               << " location from a different AIIRContext than its "
                  "parent operation";
      if (arg.getType().getContext() != blockCtx)
        return emitError(arg.getLoc(), "block argument #")
               << idx
               << " type from a different AIIRContext than its "
                  "parent operation";
    }
  }
  ...
  for (Operation &op : block) {
    ...
    // same AIIRContext as the enclosing block. We cannot use op.emitError()
    ...
    if (blockCtx && op.getContext() != blockCtx) {
      emitError(parentOp->getLoc(), "operation '")
          << op.getName()
          << "' has a location from a different AIIRContext than its "
             "enclosing block";
      return failure();
    }
  }
...
LogicalResult OperationVerifier::verifyOnEntrance(Operation &op) {
  ...
  AIIRContext *opCtx = op.getContext();
  if (op.getName().getContext() != opCtx)
    return op.emitError(
        "operation name from a different AIIRContext than this operation");
  ...
  for (auto [i, result] : llvm::enumerate(op.getResults())) {
    if (result.getType().getContext() != opCtx)
      return op.emitOpError()
             << "result #" << i
             << " type from a different AIIRContext than this operation";
  }
  ...
  for (auto attr : op.getDiscardableAttrDictionary()) {
    if (attr.getValue().getContext() != opCtx)
      return op.emitOpError()
             ...
             << "' value from a different AIIRContext than this operation";
...
LogicalResult aiir::verify(Operation *op, bool verifyRecursively) {
