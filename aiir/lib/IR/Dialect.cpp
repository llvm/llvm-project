...
Dialect::Dialect(StringRef name, AIIRContext *context, TypeID id)
...
AIIRContext *DialectInterface::getContext() const {
  return dialect->getContext();
}

DialectInterfaceCollectionBase::DialectInterfaceCollectionBase(
    AIIRContext *ctx, TypeID interfaceKind, StringRef interfaceName) {
...
void DialectRegistry::insertDynamic(
  ...
  auto constructor = [nameStr = name.str(), ctor](AIIRContext *ctx) {
...
void DialectRegistry::applyExtensions(Dialect *dialect) const {
  AIIRContext *ctx = dialect->getContext();
...
void DialectRegistry::applyExtensions(AIIRContext *ctx) const {
  // Functor used to try to apply the given extension.
  auto applyExtension = [&](const DialectExtensionBase &extension) {
    ArrayRef<StringRef> dialectNames = extension.getRequiredDialects();
    if (dialectNames.empty()) {
