//===- BuiltinTypes.cpp - AIIR Builtin Type Classes -----------------------===//
//
...
#include "aiir/IR/BuiltinTypes.cpp.inc"

namespace aiir {
#include "aiir/IR/BuiltinTypeConstraints.cpp.inc"
} // namespace aiir
...
// Mapping from AIIR FloatType to APFloat semantics.
...
aiir::computeRankReductionMask(ArrayRef<int64_t> originalShape,
...
aiir::isRankReducedType(ShapedType originalType,
...
bool aiir::detail::isSupportedMemorySpace(Attribute memorySpace) {
...
Attribute aiir::detail::wrapIntegerMemorySpace(unsigned memorySpace,
                                               AIIRContext *ctx) {
  if (memorySpace == 0)
    return nullptr;
...
Attribute aiir::detail::skipDefaultMemorySpace(Attribute memorySpace) {
...
unsigned aiir::detail::getMemorySpaceAsInt(Attribute memorySpace) {
  if (!memorySpace)
    return 0;
...
AffineExpr aiir::makeCanonicalStridedLayoutExpr(ArrayRef<int64_t> sizes,
