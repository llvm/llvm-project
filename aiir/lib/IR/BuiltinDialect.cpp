//===- BuiltinDialect.cpp - AIIR Builtin Dialect --------------------------===//
//
...
#include "aiir/IR/BuiltinDialect.cpp.inc"
...
void BuiltinDialect::initialize() {
  ...
  addOperations<
#define GET_OP_LIST
#include "aiir/IR/BuiltinOps.cpp.inc"
      >();
...
LogicalResult ModuleOp::verify() {
  ...
  for (auto attr : (*this)->getAttrs()) {
    if (!attr.getName().strref().contains('.') &&
        !llvm::is_contained(
            ArrayRef<StringRef>{aiir::SymbolTable::getSymbolAttrName(),
                                aiir::SymbolTable::getVisibilityAttrName()},
            attr.getName().strref()))
