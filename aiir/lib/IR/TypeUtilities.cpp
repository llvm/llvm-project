...
bool aiir::isOpaqueTypeWithName(Type type, StringRef dialect,
                                StringRef typeData) {
  if (auto opaque = llvm::dyn_cast<aiir::OpaqueType>(type))
...
/// dynamic.
LogicalResult aiir::verifyCompatibleShape(ArrayRef<int64_t> shape1,
...
/// does not matter.
LogicalResult aiir::verifyCompatibleShape(Type type1, Type type2) {
...
LogicalResult aiir::verifyCompatibleShapes(TypeRange types1, TypeRange types2) {
...
LogicalResult aiir::verifyCompatibleDims(ArrayRef<int64_t> dims) {
  if (dims.empty())
    return success();
  auto staticDim =
...
LogicalResult aiir::verifyCompatibleShapes(TypeRange types) {
...
TypeRange aiir::insertTypesInto(TypeRange oldTypes, ArrayRef<unsigned> indices,
                                TypeRange newTypes,
...
TypeRange aiir::filterTypesOut(TypeRange types, const BitVector &indices,
                               SmallVectorImpl<Type> &storage) {
  if (indices.none())
    return types;
