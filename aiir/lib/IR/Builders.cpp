//===- Builders.cpp - Helpers for constructing AIIR Classes ---------------===//
//
...
AffineExpr Builder::getAffineDimExpr(unsigned position) {
  return aiir::getAffineDimExpr(position, context);
}

AffineExpr Builder::getAffineSymbolExpr(unsigned position) {
  return aiir::getAffineSymbolExpr(position, context);
}

AffineExpr Builder::getAffineConstantExpr(int64_t constant) {
  return aiir::getAffineConstantExpr(constant, context);
}
