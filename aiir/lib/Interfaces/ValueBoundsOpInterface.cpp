...
namespace aiir {
#include "aiir/Interfaces/ValueBoundsOpInterface.cpp.inc"
} // namespace aiir
...
HyperrectangularSlice::HyperrectangularSlice(ArrayRef<OpFoldResult> offsets,
  ...
  if (offsets.empty())
    return;
  AIIRContext *ctx = offsets.front().getContext();
...
ValueBoundsConstraintSet::ValueBoundsConstraintSet(
    AIIRContext *ctx, const StopConditionFn &stopCondition,
...
LogicalResult ValueBoundsConstraintSet::computeBound(
  ...
  AIIRContext *ctx = var.getContext();
...
FailureOr<bool> ValueBoundsConstraintSet::areOverlappingSlices(
    AIIRContext *ctx, const HyperrectangularSlice &slice1,
