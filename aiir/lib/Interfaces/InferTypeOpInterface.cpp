...
namespace aiir {
#include "aiir/Interfaces/InferTypeOpInterface.cpp.inc"
} // namespace aiir

LogicalResult
aiir::reifyResultShapes(OpBuilder &b, Operation *op,
...
aiir::reifyShapeOfResult(OpBuilder &b, Operation *op, int resultIndex) {
  auto reifiableOp = dyn_cast<ReifyRankedShapedTypeOpInterface>(op);
  if (!reifiableOp)
    return failure();
...
FailureOr<OpFoldResult> aiir::reifyDimOfResult(OpBuilder &b, Operation *op,
...
LogicalResult aiir::detail::inferReturnTensorTypes(
...
LogicalResult aiir::detail::verifyInferredResultTypes(Operation *op) {
...
void aiir::detail::reportFatalInferReturnTypesError(OperationState &state) {
  std::string buffer;
