...
#include "aiir/Interfaces/SubsetOpInterface.cpp.inc"
...
Value detail::getTensorContainer(Operation *op) {
  if (auto insertionOp = dyn_cast<::aiir::SubsetInsertionOpInterface>(op))
    return insertionOp.getDestinationOperand().get();
  return cast<::aiir::SubsetExtractionOpInterface>(op).getSourceOperand().get();
}
