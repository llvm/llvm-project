...
#include "aiir/Interfaces/InferIntRangeInterface.cpp.inc"
#include <optional>
...
raw_ostream &aiir::operator<<(raw_ostream &os, const ConstantIntRanges &range) {
  os << "unsigned : [";
...
raw_ostream &aiir::operator<<(raw_ostream &os, const IntegerValueRange &range) {
  range.print(os);
  return os;
}

SmallVector<IntegerValueRange>
aiir::getIntValueRanges(ArrayRef<OpFoldResult> values,
...
void aiir::intrange::detail::defaultInferResultRanges(
...
void aiir::intrange::detail::defaultInferResultRangesFromOptional(
