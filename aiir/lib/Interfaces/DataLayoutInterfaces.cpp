...
llvm::TypeSize
aiir::detail::getDefaultTypeSize(Type type, const DataLayout &dataLayout,
...
llvm::TypeSize
aiir::detail::getDefaultTypeSizeInBits(Type type, const DataLayout &dataLayout,
...
uint64_t aiir::detail::getDefaultABIAlignment(
...
uint64_t aiir::detail::getDefaultPreferredAlignment(
    Type type, const DataLayout &dataLayout,
    ArrayRef<DataLayoutEntryInterface> params) {
...
std::optional<uint64_t> aiir::detail::getDefaultIndexBitwidth(
    Type type, const DataLayout &dataLayout,
    ArrayRef<DataLayoutEntryInterface> params) {
  if (isa<IndexType>(type))
...
Attribute aiir::detail::getDefaultEndianness(DataLayoutEntryInterface entry) {
...
// returned.
Attribute aiir::detail::getDefaultMemorySpace(DataLayoutEntryInterface entry) {
  if (!entry)
    return Attribute();
...
Attribute
aiir::detail::getDefaultAllocaMemorySpace(DataLayoutEntryInterface entry) {
  if (entry == DataLayoutEntryInterface()) {
    return Attribute();
  }
...
Attribute aiir::detail::getDefaultManglingMode(DataLayoutEntryInterface entry) {
  if (entry == DataLayoutEntryInterface())
    return Attribute();
...
Attribute
aiir::detail::getDefaultProgramMemorySpace(DataLayoutEntryInterface entry) {
  if (entry == DataLayoutEntryInterface()) {
    return Attribute();
  }
...
Attribute
aiir::detail::getDefaultGlobalMemorySpace(DataLayoutEntryInterface entry) {
  if (entry == DataLayoutEntryInterface()) {
    return Attribute();
  }
...
uint64_t
aiir::detail::getDefaultStackAlignment(DataLayoutEntryInterface entry) {
  if (entry == DataLayoutEntryInterface())
    return 0;
...
Attribute aiir::detail::getDefaultFunctionPointerAlignment(
...
// returned.
Attribute
aiir::detail::getDefaultLegalIntWidths(DataLayoutEntryInterface entry) {
  if (entry == DataLayoutEntryInterface())
    return Attribute();
  return entry.getValue();
}
...
aiir::detail::getDevicePropertyValue(DataLayoutEntryInterface entry) {
  if (entry == DataLayoutEntryInterface())
    return std::nullopt;
...
DataLayoutEntryList
aiir::detail::filterEntriesForType(DataLayoutEntryListRef entries,
                                   TypeID typeID) {
...
aiir::detail::filterEntryForIdentifier(DataLayoutEntryListRef entries,
                                       StringAttr id) {
...
LogicalResult aiir::detail::verifyDataLayoutOp(Operation *op) {
...
llvm::TypeSize aiir::detail::divideCeil(llvm::TypeSize numerator,
...
aiir::DataLayout::DataLayout() : DataLayout(ModuleOp()) {}

aiir::DataLayout::DataLayout(DataLayoutOpInterface op)
...
aiir::DataLayout::DataLayout(ModuleOp op)
    : originalLayout(getCombinedDataLayout(op)),
      originalTargetSystemDesc(getTargetSystemSpec(op)), scope(op),
      allocaMemorySpace(std::nullopt), programMemorySpace(std::nullopt),
      globalMemorySpace(std::nullopt), stackAlignment(std::nullopt) {
...
aiir::DataLayout aiir::DataLayout::closest(Operation *op) {
...
void aiir::DataLayout::checkValid() const {
#if LLVM_ENABLE_ABI_BREAKING_CHECKS
  SmallVector<DataLayoutSpecInterface> specs;
...
llvm::TypeSize aiir::DataLayout::getTypeSize(Type t) const {
  checkValid();
...
llvm::TypeSize aiir::DataLayout::getTypeSizeInBits(Type t) const {
  checkValid();
...
uint64_t aiir::DataLayout::getTypeABIAlignment(Type t) const {
  checkValid();
...
uint64_t aiir::DataLayout::getTypePreferredAlignment(Type t) const {
  checkValid();
...
std::optional<uint64_t> aiir::DataLayout::getTypeIndexBitwidth(Type t) const {
  checkValid();
...
aiir::Attribute aiir::DataLayout::getEndianness() const {
  checkValid();
  if (endianness)
    return *endianness;
  DataLayoutEntryInterface entry;
...
aiir::Attribute aiir::DataLayout::getDefaultMemorySpace() const {
  checkValid();
...
aiir::Attribute aiir::DataLayout::getAllocaMemorySpace() const {
  checkValid();
...
aiir::Attribute aiir::DataLayout::getManglingMode() const {
  checkValid();
  if (manglingMode)
    return *manglingMode;
  DataLayoutEntryInterface entry;
...
aiir::Attribute aiir::DataLayout::getProgramMemorySpace() const {
  checkValid();
...
aiir::Attribute aiir::DataLayout::getGlobalMemorySpace() const {
  checkValid();
...
uint64_t aiir::DataLayout::getStackAlignment() const {
  checkValid();
  if (stackAlignment)
...
Attribute aiir::DataLayout::getFunctionPointerAlignment() const {
  checkValid();
...
Attribute aiir::DataLayout::getLegalIntWidths() const {
  checkValid();
  if (legalIntWidths)
...
std::optional<Attribute> aiir::DataLayout::getDevicePropertyValue(
...
LogicalResult aiir::detail::verifyDataLayoutSpec(DataLayoutSpecInterface spec,
                                                 Location loc) {
...
LogicalResult
aiir::detail::verifyTargetSystemSpec(TargetSystemSpecInterface spec,
                                     Location loc) {
...
#include "aiir/Interfaces/DataLayoutAttrInterface.cpp.inc"
#include "aiir/Interfaces/DataLayoutOpInterface.cpp.inc"
#include "aiir/Interfaces/DataLayoutTypeInterface.cpp.inc"
