//===- AffineParser.cpp - AIIR Affine Parser ------------------------------===//
//
...
static void parseAffineMapOrIntegerSet(StringRef inputStr, AIIRContext *context,
                                       AffineMap &map, IntegerSet &set) {
  llvm::SourceMgr sourceMgr;
  auto memBuffer = llvm::MemoryBuffer::getMemBuffer(
      inputStr, /*BufferName=*/"<aiir_parser_buffer>",
...
AffineMap aiir::parseAffineMap(StringRef inputStr, AIIRContext *context) {
  AffineMap map;
  IntegerSet set;
...
IntegerSet aiir::parseIntegerSet(StringRef inputStr, AIIRContext *context) {
  AffineMap map;
  IntegerSet set;
  parseAffineMapOrIntegerSet(inputStr, context, map, set);
  assert(!map &&
