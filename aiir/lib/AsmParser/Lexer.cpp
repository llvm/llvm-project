//===- Lexer.cpp - AIIR Lexer Implementation ------------------------------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file implements the lexer for the AIIR textual form.
//
//===----------------------------------------------------------------------===//
...
Lexer::Lexer(const llvm::SourceMgr &sourceMgr, AIIRContext *context,
...
Token Lexer::emitError(const char *loc, const Twine &message) {
  aiir::emitError(getEncodedSourceLocation(SMLoc::getFromPointer(loc)),
                  message);
