//===- Parser.cpp - AIIR Parser Implementation ----------------------------===//
//
...
InFlightDiagnostic Parser::emitError(SMLoc loc, const Twine &message) {
  auto diag = aiir::emitError(getEncodedSourceLocation(loc), message);
...
AIIR_DECLARE_EXPLICIT_SELF_OWNING_TYPE_ID(OperationParser::DeferredLocInfo *)
AIIR_DEFINE_EXPLICIT_SELF_OWNING_TYPE_ID(OperationParser::DeferredLocInfo *)
...
ParseResult OperationParser::parseGenericOperationAfterOpName(
  ...
  if (!parsedFnType) {
    ...
    if (!fnType)
      return aiir::emitError(typeLoc, "expected function type");
  ...
  if (operandTypes.size() != parsedOperandUseInfo->size()) {
    ...
    return aiir::emitError(typeLoc, "expected ")
...
Operation *OperationParser::parseGenericOperation() {
  ...
  if (!properties && !result.getRawProperties()) {
    ...
    if (info) {
      if (failed(info->verifyInherentAttrs(result.attributes, [&]() {
            return aiir::emitError(srcLocation) << "'" << name << "' op ";
          })))
        return nullptr;
    }
  }
  ...
  if (properties) {
    auto emitError = [&]() {
      return aiir::emitError(srcLocation, "invalid properties ")
...
OperationParser::parseCustomOperation(ArrayRef<ResultRecord> resultIDs) {
  ...
  if (auto opInfo = opNameInfo->getRegisteredInfo()) {
    ...
    if (!dialect) {
      ...
      note << "; for more info on dialect registration see "
              "https://aiir.llvm.org/getting_started/Faq/"
  ...
  // debugging.
  llvm::PrettyStackTraceFormat fmt("AIIR Parser: custom op parser '%s'",
...
/// This class represents an implementation of a resource entry for the AIIR
/// textual format.
...
LogicalResult
aiir::parseAsmSourceFile(const llvm::SourceMgr &sourceMgr, Block *block,
