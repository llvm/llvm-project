...
llvm::Expected<DialectPlugin> DialectPlugin::load(const std::string &filename) {
  ...
  // aiirGetDialectPluginInfo should be resolved to the definition from the
  // plugin we are currently loading.
  intptr_t getDetailsFn =
      (intptr_t)library.getAddressOfSymbol("aiirGetDialectPluginInfo");
  ...
  plugin.info =
      reinterpret_cast<decltype(aiirGetDialectPluginInfo) *>(getDetailsFn)();
