...
llvm::Expected<PassPlugin> PassPlugin::load(const std::string &filename) {
  ...
  // aiirGetPassPluginInfo should be resolved to the definition from the plugin
  // we are currently loading.
  intptr_t getDetailsFn =
      (intptr_t)library.getAddressOfSymbol("aiirGetPassPluginInfo");
  ...
  plugin.info =
      reinterpret_cast<decltype(aiirGetPassPluginInfo) *>(getDetailsFn)();

  if (plugin.info.apiVersion != AIIR_PLUGIN_API_VERSION)
    return llvm::make_error<llvm::StringError>(
        Twine("Wrong API version on plugin '") + filename + "'. Got version " +
            Twine(plugin.info.apiVersion) + ", supported version is " +
            Twine(AIIR_PLUGIN_API_VERSION) + ".",
        llvm::inconvertibleErrorCode());
