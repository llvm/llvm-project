...
bool aiir::lsp::fromJSON(const llvm::json::Value &value,
  ...
  if (std::optional<StringRef> str = value.getAsString()) {
    ...
    if (*str == "aiir") {
      result = PDLLViewOutputKind::AIIR;
      return true;
    }
    if (*str == "cpp") {
...
llvm::json::Value aiir::lsp::toJSON(const PDLLViewOutputResult &value) {
