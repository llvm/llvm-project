...
#include "aiir/Tools/PDLL/AST/Nodes.h"
#include "aiir/Tools/PDLL/AST/Types.h"
#include "aiir/Tools/PDLL/CodeGen/CPPGen.h"
#include "aiir/Tools/PDLL/CodeGen/AIIRGen.h"
#include "aiir/Tools/PDLL/ODS/Constraint.h"
#include "aiir/Tools/PDLL/ODS/Context.h"
#include "aiir/Tools/PDLL/ODS/Dialect.h"
#include "aiir/Tools/PDLL/ODS/Operation.h"
#include "aiir/Tools/PDLL/Parser/CodeComplete.h"
#include "aiir/Tools/PDLL/Parser/Parser.h"
#include "aiir/Tools/lsp-server-support/CompilationDatabase.h"
#include "aiir/Tools/lsp-server-support/SourceMgrUtils.h"
...
/// Convert the given AIIR diagnostic to the LSP form.
...
public:
  ...
  void codeCompleteConstraintName(ast::Type currentType,
    ...
    auto addCoreConstraint = [&](StringRef constraint, StringRef aiirType,
      ...
      item.documentation = llvm::lsp::MarkupContent{
          llvm::lsp::MarkupKind::Markdown,
          ("A single entity core constraint of type `" + aiirType + "`").str()};
      item.sortText = "0";
    ...
    // inferred.
    if (!currentType) {
      addCoreConstraint("Attr", "aiir::Attribute");
      addCoreConstraint("Op", "aiir::Operation *");
      addCoreConstraint("Value", "aiir::Value");
      addCoreConstraint("ValueRange", "aiir::ValueRange");
      addCoreConstraint("Type", "aiir::Type");
      addCoreConstraint("TypeRange", "aiir::TypeRange");
    }
    if (allowInlineTypeConstraints) {
      /// Attr<Type>.
      if (!currentType || isa<ast::AttributeType>(currentType))
        addCoreConstraint("Attr<type>", "aiir::Attribute", "Attr<$1>");
      /// Value<Type>.
      if (!currentType || isa<ast::ValueType>(currentType))
        addCoreConstraint("Value<type>", "aiir::Value", "Value<$1>");
      /// ValueRange<TypeRange>.
      if (!currentType || isa<ast::ValueRangeType>(currentType))
        addCoreConstraint("ValueRange<type>", "aiir::ValueRange",
                          "ValueRange<$1>");
    }
...
void PDLDocument::getPDLLViewOutput(raw_ostream &os,
  ...
  // Generate the AIIR for the ast module. We also capture diagnostics here to
  // show to the user, which may be useful if PDLL isn't capturing constraints
  // expected by PDL.
  AIIRContext aiirContext;
  SourceMgrDiagnosticHandler diagHandler(sourceMgr, &aiirContext, os);
  OwningOpRef<ModuleOp> pdlModule =
      codegenPDLLToAIIR(&aiirContext, astContext, sourceMgr, **astModule);
  if (!pdlModule)
    return;
  if (kind == lsp::PDLLViewOutputKind::AIIR) {
