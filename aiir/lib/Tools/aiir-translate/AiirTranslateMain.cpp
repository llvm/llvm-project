//===- AiirTranslateMain.cpp - AIIR Translation entry point ---------------===//
//
...
public:
  ErrorDiagnosticFilter(AIIRContext *ctx) : ScopedDiagnosticHandler(ctx) {
...
LogicalResult aiir::aiirTranslateMain(int argc, char **argv,
  ...
  // Processes the memory buffer with a new AIIRContext.
