//===- LSPServer.cpp - AIIR Language Server -------------------------------===//
//
...
#include "LSPServer.h"
#include "AIIRServer.h"
#include "Protocol.h"
#include "llvm/Support/LSP/Logging.h"
#include "llvm/Support/LSP/Transport.h"
#include <optional>

#define DEBUG_TYPE "aiir-lsp-server"
...
using llvm::lsp::AIIRConvertBytecodeParams;
...
namespace {
struct LSPServer {
  LSPServer(AIIRServer &server) : server(server) {}
  ...
  void onConvertFromBytecode(const AIIRConvertBytecodeParams &params,
                             Callback<AIIRConvertBytecodeResult> reply);
  void onConvertToBytecode(const AIIRConvertBytecodeParams &params,
                           Callback<AIIRConvertBytecodeResult> reply);
...
void LSPServer::onInitialize(const InitializeParams &params,
  ...
  llvm::json::Object result{
      {{"serverInfo",
        llvm::json::Object{{"name", "aiir-lsp-server"}, {"version", "0.0.0"}}},
...
void LSPServer::onConvertFromBytecode(
    const AIIRConvertBytecodeParams &params,
    Callback<AIIRConvertBytecodeResult> reply) {
  reply(server.convertFromBytecode(params.uri));
}

void LSPServer::onConvertToBytecode(const AIIRConvertBytecodeParams &params,
                                    Callback<AIIRConvertBytecodeResult> reply) {
...
LogicalResult lsp::runAiirLSPServer(AIIRServer &server,
  ...
  // Bytecode
  messageHandler.method("aiir/convertFromBytecode", &lspServer,
                        &LSPServer::onConvertFromBytecode);
  messageHandler.method("aiir/convertToBytecode", &lspServer,
