//===- AiirLspServerMain.cpp - AIIR Language Server main ------------------===//
//
...
LogicalResult aiir::AiirLspServerMain(int argc, char **argv,
  ...
  llvm::cl::ParseCommandLineOptions(argc, argv, "AIIR LSP Language Server");
  ...
  // Register the additionally supported URI schemes for the AIIR server.
  URIForFile::registerSupportedScheme("aiir.bytecode-aiir");

  // Configure the servers and start the main language server.
  AIIRServer server(registry_fn);
  return runAiirLSPServer(server, transport);
}

llvm::LogicalResult aiir::AiirLspServerMain(int argc, char **argv,
                                            DialectRegistry &registry) {
  auto registry_fn =
      [&registry](const lsp::URIForFile &uri) -> DialectRegistry & {
    return registry;
  };
  return AiirLspServerMain(argc, argv, registry_fn);
}
