//===- AIIRServer.cpp - AIIR Generic Language Server ----------------------===//
//
...
#include "aiir/AsmParser/CodeComplete.h"
...
/// Returns a language server location from the given AIIR file location.
...
/// Returns a language server location from the given AIIR location, or
...
/// Collect all of the locations from the given AIIR location that are not
...
namespace {
/// This class represents all of the information pertaining to a specific AIIR
/// document.
struct AIIRDocument {
  AIIRDocument(AIIRContext &context, const lsp::URIForFile &uri,
               StringRef contents, std::vector<lsp::Diagnostic> &diagnostics);
  AIIRDocument(const AIIRDocument &) = delete;
  AIIRDocument &operator=(const AIIRDocument &) = delete;
  ...
  llvm::Expected<lsp::AIIRConvertBytecodeResult> convertToBytecode();
...
AIIRDocument::AIIRDocument(AIIRContext &context, const lsp::URIForFile &uri,
                           StringRef contents,
...
//===----------------------------------------------------------------------===//
// AIIRDocument: Definitions and References
//===----------------------------------------------------------------------===//

void AIIRDocument::getLocationsOf(const lsp::URIForFile &uri,
...
void AIIRDocument::findReferencesOf(const lsp::URIForFile &uri,
...
std::optional<lsp::Hover>
AIIRDocument::findHover(const lsp::URIForFile &uri,
                        const lsp::Position &hoverPos) {
...
std::optional<lsp::Hover> AIIRDocument::buildHoverForOperation(
...
lsp::Hover AIIRDocument::buildHoverForOperationResult(SMRange hoverRange,
                                                      Operation *op,
                                                      unsigned resultStart,
                                                      unsigned resultEnd,
                                                      SMLoc posLoc) {
...
lsp::Hover
AIIRDocument::buildHoverForBlock(SMRange hoverRange,
                                 const AsmParserState::BlockDefinition &block) {
  lsp::Hover hover(lsp::Range(sourceMgr, hoverRange));
  llvm::raw_string_ostream os(hover.contents.value);
...
lsp::Hover AIIRDocument::buildHoverForBlockArgument(
...
lsp::Hover AIIRDocument::buildHoverForAttributeAlias(
    SMRange hoverRange, const AsmParserState::AttributeAliasDefinition &attr) {
  lsp::Hover hover(lsp::Range(sourceMgr, hoverRange));
  llvm::raw_string_ostream os(hover.contents.value);
...
lsp::Hover AIIRDocument::buildHoverForTypeAlias(
...
//===----------------------------------------------------------------------===//
// AIIRDocument: Document Symbols
//===----------------------------------------------------------------------===//

void AIIRDocument::findDocumentSymbols(
    std::vector<lsp::DocumentSymbol> &symbols) {
...
lsp::CompletionList
AIIRDocument::getCodeCompletion(const lsp::URIForFile &uri,
                                const lsp::Position &completePos,
                                const DialectRegistry &registry) {
  ...
  AIIRContext tmpContext(registry, AIIRContext::Threading::DISABLED);
...
//===----------------------------------------------------------------------===//
// AIIRDocument: Code Action
//===----------------------------------------------------------------------===//

void AIIRDocument::getCodeActionForDiagnostic(
...
//===----------------------------------------------------------------------===//
// AIIRDocument: Bytecode
//===----------------------------------------------------------------------===//

llvm::Expected<lsp::AIIRConvertBytecodeResult>
AIIRDocument::convertToBytecode() {
  ...
  lsp::AIIRConvertBytecodeResult result;
  {
...
namespace {
/// This class represents a single chunk of an AIIR text file.
struct AIIRTextFileChunk {
  AIIRTextFileChunk(AIIRContext &context, uint64_t lineOffset,
...
namespace {
/// This class represents a text file containing one or more AIIR documents.
class AIIRTextFile {
public:
  AIIRTextFile(const lsp::URIForFile &uri, StringRef fileContents,
...
private:
  /// Find the AIIR document that contains the given position, and update the
  /// position to be anchored at the start of the found chunk instead of the
  /// beginning of the file.
  AIIRTextFileChunk &getChunkFor(lsp::Position &pos);
  ...
  std::vector<std::unique_ptr<AIIRTextFileChunk>> chunks;
};
} // namespace

AIIRTextFile::AIIRTextFile(const lsp::URIForFile &uri, StringRef fileContents,
                           int64_t version, lsp::DialectRegistryFn registryFn,
                           std::vector<lsp::Diagnostic> &diagnostics)
    : context(registryFn(uri), AIIRContext::Threading::DISABLED),
      contents(fileContents.str()), version(version) {
  context.allowUnregisteredDialects();

  // Split the file into separate AIIR documents.
  SmallVector<StringRef, 8> subContents;
  StringRef(contents).split(subContents, kDefaultSplitMarker);
  chunks.emplace_back(std::make_unique<AIIRTextFileChunk>(
  ...
  for (StringRef docContents : llvm::drop_begin(subContents)) {
    ...
    auto chunk = std::make_unique<AIIRTextFileChunk>(context, lineOffset, uri,
...
void AIIRTextFile::getLocationsOf(const lsp::URIForFile &uri,
                                  lsp::Position defPos,
                                  std::vector<lsp::Location> &locations) {
  AIIRTextFileChunk &chunk = getChunkFor(defPos);
...
void AIIRTextFile::findReferencesOf(const lsp::URIForFile &uri,
                                    lsp::Position pos,
                                    std::vector<lsp::Location> &references) {
  AIIRTextFileChunk &chunk = getChunkFor(pos);
...
std::optional<lsp::Hover> AIIRTextFile::findHover(const lsp::URIForFile &uri,
                                                  lsp::Position hoverPos) {
  AIIRTextFileChunk &chunk = getChunkFor(hoverPos);
...
void AIIRTextFile::findDocumentSymbols(
    std::vector<lsp::DocumentSymbol> &symbols) {
  if (chunks.size() == 1)
  ...
  for (unsigned i = 0, e = chunks.size(); i < e; ++i) {
    AIIRTextFileChunk &chunk = *chunks[i];
...
lsp::CompletionList AIIRTextFile::getCodeCompletion(const lsp::URIForFile &uri,
                                                    lsp::Position completePos) {
  AIIRTextFileChunk &chunk = getChunkFor(completePos);
...
void AIIRTextFile::getCodeActions(const lsp::URIForFile &uri,
                                  const lsp::Range &pos,
                                  const lsp::CodeActionContext &context,
                                  std::vector<lsp::CodeAction> &actions) {
  ...
  for (auto &diag : context.diagnostics) {
    ...
    AIIRTextFileChunk &chunk = getChunkFor(diagPos);
...
llvm::Expected<lsp::AIIRConvertBytecodeResult>
AIIRTextFile::convertToBytecode() {
...
AIIRTextFileChunk &AIIRTextFile::getChunkFor(lsp::Position &pos) {
  if (chunks.size() == 1)
    return *chunks.front();
  ...
  AIIRTextFileChunk &chunk = it == chunks.end() ? *chunks.back() : **(--it);
...
struct lsp::AIIRServer::Impl {
  ...
  llvm::StringMap<std::unique_ptr<AIIRTextFile>> files;
};
...
lsp::AIIRServer::AIIRServer(lsp::DialectRegistryFn registryFn)
    : impl(std::make_unique<Impl>(registryFn)) {}
lsp::AIIRServer::~AIIRServer() = default;

void lsp::AIIRServer::addOrUpdateDocument(
    const URIForFile &uri, StringRef contents, int64_t version,
    std::vector<llvm::lsp::Diagnostic> &diagnostics) {
  impl->files[uri.file()] = std::make_unique<AIIRTextFile>(
      uri, contents, version, impl->registryFn, diagnostics);
}

std::optional<int64_t> lsp::AIIRServer::removeDocument(const URIForFile &uri) {
...
void lsp::AIIRServer::getLocationsOf(
...
void lsp::AIIRServer::findReferencesOf(
...
std::optional<lsp::Hover> lsp::AIIRServer::findHover(const URIForFile &uri,
...
void lsp::AIIRServer::findDocumentSymbols(
...
lsp::CompletionList
lsp::AIIRServer::getCodeCompletion(const URIForFile &uri,
...
void lsp::AIIRServer::getCodeActions(const URIForFile &uri, const Range &pos,
...
llvm::Expected<lsp::AIIRConvertBytecodeResult>
lsp::AIIRServer::convertFromBytecode(const URIForFile &uri) {
  AIIRContext tempContext(impl->registryFn(uri));
  ...
  ScopedDiagnosticHandler diagHandler(
      &tempContext,
      [&](aiir::Diagnostic &diag) { errorMsg += diag.str() + "\n"; });
...
llvm::Expected<lsp::AIIRConvertBytecodeResult>
lsp::AIIRServer::convertToBytecode(const URIForFile &uri) {
  auto fileIt = impl->files.find(uri.file());
  if (fileIt == impl->files.end()) {
    return llvm::make_error<llvm::lsp::LSPError>(
