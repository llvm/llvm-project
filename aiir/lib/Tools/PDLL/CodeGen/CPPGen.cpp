...
void CodeGen::generate(const ast::Module &astModule, ModuleOp module) {
  ...
  os << "template <typename... ConfigsT>\n"
        ...
        "::aiir::RewritePatternSet &patterns, ConfigsT &&...configs) {\n";
...
struct {0} : ::aiir::PDLPatternModule {{
  template <typename... ConfigsT>
  {0}(::aiir::AIIRContext *context, ConfigsT &&...configs)
    : ::aiir::PDLPatternModule(::aiir::parseSourceString<::aiir::ModuleOp>(
)";
  os << llvm::formatv(patternClassStartStr, patternName);

  os << "R\"aiir(";
  pattern->print(os, OpPrintingFlags().enableDebugInfo());
  os << "\n    )aiir\", context), std::forward<ConfigsT>(configs)...) {\n";
...
StringRef CodeGen::getNativeTypeName(ast::Type type) {
  return llvm::TypeSwitch<ast::Type, StringRef>(type)
      .Case([&](ast::AttributeType) { return "::aiir::Attribute"; })
      .Case([&](ast::OperationType opType) -> StringRef {
        ...
        return "::aiir::Operation *";
      })
      .Case([&](ast::TypeType) { return "::aiir::Type"; })
      .Case([&](ast::ValueType) { return "::aiir::Value"; })
      .Case([&](ast::TypeRangeType) { return "::aiir::TypeRange"; })
      .Case([&](ast::ValueRangeType) { return "::aiir::ValueRange"; });
}
...
void CodeGen::generateConstraintOrRewrite(const ast::CallableDecl *decl,
  ...
  os << " " << name << "PDLFn(::aiir::PatternRewriter &rewriter";
...
void aiir::pdll::codegenPDLLToCPP(const ast::Module &astModule, ModuleOp module,
                                  raw_ostream &os) {
  CodeGen codegen(os);
