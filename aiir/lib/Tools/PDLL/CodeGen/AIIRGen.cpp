...
class CodeGen {
public:
  CodeGen(AIIRContext *aiirContext, const ast::Context &context,
          const llvm::SourceMgr &sourceMgr)
      : builder(aiirContext), odsContext(context.getODSContext()),
        sourceMgr(sourceMgr) {
    // Make sure that the PDL dialect is loaded.
    aiirContext->loadDialect<pdl::PDLDialect>();
  }

  OwningOpRef<ModuleOp> generate(const ast::Module &module);

private:
  /// Generate an AIIR location from the given source location.
  Location genLoc(llvm::SMLoc loc);
  Location genLoc(llvm::SMRange loc) { return genLoc(loc.Start); }

  /// Generate an AIIR type from the given source type.
  Type genType(ast::Type type);

  /// Generate AIIR for the given AST node.
  ...
  /// Generate the set of AIIR values defined for the given variable decl, and
  ...
  /// to the AIIR values of the variable.
  ...
  /// The AIIR builder used for building the resultant IR.
  OpBuilder builder;

  /// A map from variable declarations to the AIIR equivalent.
  using VariableMapTy =
...
OwningOpRef<ModuleOp> CodeGen::generate(const ast::Module &module) {
  OwningOpRef<ModuleOp> aiirModule =
...
void CodeGen::genImpl(const ast::UserConstraintDecl *decl) {
  ...
  // constraint declarations doesn't require any AIIR to be generated, only uses
  // of it do.
}

void CodeGen::genImpl(const ast::UserRewriteDecl *decl) {
  // All PDLL rewrites get inlined when called, and the main native
  // rewrite declarations doesn't require any AIIR to be generated, only uses
  // of it do.
}
...
Value CodeGen::genNonInitializerVar(const ast::VariableDecl *varDecl,
  ...
  Type aiirType = genType(type);
  if (isa<ast::ValueType>(type))
    return pdl::OperandOp::create(builder, loc, aiirType, getTypeConstraint());
  if (isa<ast::TypeType>(type))
    return pdl::TypeOp::create(builder, loc, aiirType, /*type=*/TypeAttr());
  ...
  if (ast::RangeType rangeTy = dyn_cast<ast::RangeType>(type)) {
    ...
    if (isa<ast::ValueType>(eleTy))
      return pdl::OperandsOp::create(builder, loc, aiirType,
                                     getTypeConstraint());
    if (isa<ast::TypeType>(eleTy))
      return pdl::TypesOp::create(builder, loc, aiirType,
                                  /*types=*/ArrayAttr());
  }
...
Value CodeGen::genExprImpl(const ast::AttributeExpr *expr) {
  ...
  assert(attr && "invalid AIIR attribute data");
...
Value CodeGen::genExprImpl(const ast::MemberAccessExpr *expr) {
  ...
  if (ast::OperationType opType = dyn_cast<ast::OperationType>(parentType)) {
    if (isa<ast::AllResultsMemberAccessExpr>(expr)) {
      Type aiirType = genType(expr->getType());
      if (isa<pdl::ValueType>(aiirType))
        return pdl::ResultOp::create(builder, loc, aiirType, parentExprs[0],
                                     builder.getI32IntegerAttr(0));
      return pdl::ResultsOp::create(builder, loc, aiirType, parentExprs[0]);
    }
...
Value CodeGen::genExprImpl(const ast::TypeExpr *expr) {
  ...
  assert(type && "invalid AIIR type data");
...
OwningOpRef<ModuleOp> aiir::pdll::codegenPDLLToAIIR(
    AIIRContext *aiirContext, const ast::Context &context,
    const llvm::SourceMgr &sourceMgr, const ast::Module &module) {
  CodeGen codegen(aiirContext, context, sourceMgr);
  OwningOpRef<ModuleOp> aiirModule = codegen.generate(module);
  if (failed(verify(*aiirModule)))
    return nullptr;
  return aiirModule;
}
