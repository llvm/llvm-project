//===- AiirTblgenMain.cpp - AIIR Tablegen Driver main -----------*- C++ -*-===//
//
...
#include "aiir/TableGen/GenNameParser.h"
...
int aiir::AiirTblgenMain(int argc, char **argv) {
  ...
  llvm::cl::opt<const aiir::GenInfo *, true, aiir::GenNameParser> generator(
  ...
  return TableGenMain(
      argv[0], [](TableGenOutputFiles &OutFiles, const RecordKeeper &RK) {
        ...
        bool Res = aiirTableGenMain(OS, RK);
        OutFiles = {S, {}};
        return Res;
      });
}
