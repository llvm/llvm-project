//===- BytecodeReader.cpp - AIIR Bytecode Reader --------------------------===//
//
...
#include "aiir/Bytecode/Encoding.h"
...
#define DEBUG_TYPE "aiir-bytecode-reader"
...
struct BytecodeDialect {
  ...
  LogicalResult load(const DialectReader &reader, AIIRContext *ctx);
...
LogicalResult ResourceSectionReader::initialize(
  ...
  AIIRContext *ctx = fileLoc->getContext();
...
public:
  ...
  AIIRContext *getContext() const override { return getLoc().getContext(); }
...
LogicalResult AttrTypeReader::parseAsmEntry(T &result, EncodingReader &reader,
  ...
  // Invoke the AIIR assembly parser to parse the entry text.
  size_t numRead = 0;
  AIIRContext *context = fileLoc->getContext();
  if constexpr (std::is_same_v<T, Type>)
    result =
...
/// This class is used to read a bytecode buffer and translate it into AIIR.
class aiir::BytecodeReader::Impl {
...
private:
  ...
  AIIRContext *getContext() const { return config.getContext(); }
  ...
  if (!loadedDialect && !ctx->allowsUnregisteredDialects()) {
    return reader.emitError("dialect '")
           ...
           << "' is unknown. If this is intended, please call "
              "allowUnregisteredDialects() on the AIIRContext, or use "
              "-allow-unregistered-dialect with the AIIR tool used.";
  }
...
LogicalResult BytecodeReader::Impl::processUseLists(Operation *topLevelOp) {
  ...
  topLevelOp->walk<aiir::WalkOrder::PreOrder>(
...
bool aiir::isBytecode(llvm::MemoryBufferRef buffer) {
...
readBytecodeFileImpl(llvm::MemoryBufferRef buffer, Block *block,
  ...
  if (!isBytecode(buffer)) {
    return emitError(sourceFileLoc,
                     "input buffer is not an AIIR bytecode file");
  }
...
LogicalResult aiir::readBytecodeFile(llvm::MemoryBufferRef buffer, Block *block,
                                     const ParserConfig &config) {
  return readBytecodeFileImpl(buffer, block, config, /*bufferOwnerRef=*/{});
}
LogicalResult
aiir::readBytecodeFile(const std::shared_ptr<llvm::SourceMgr> &sourceMgr,
