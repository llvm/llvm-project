//===- IRInterfaces.cpp - AIIR IR interfaces pybind -----------------------===//
//
...
/// of AiirVlaues. Returns an empty std::vector if the list is empty.
std::vector<AiirValue> wrapOperands(std::optional<nb::sequence> operandList) {
  ...
  aiirOperands.reserve(nb::len(*operandList));
  for (size_t i = 0, e = nb::len(*operandList); i < e; ++i) {
    ...
    try {
      ...
      if (!val)
        throw nb::cast_error();
      aiirOperands.push_back(val->get());
      continue;
...
/// AiirRegion. Returns an empty std::vector if the list is empty.
std::vector<AiirRegion>
wrapRegions(std::optional<std::vector<PyRegion>> regions) {
  std::vector<AiirRegion> aiirRegions;

  if (regions) {
    aiirRegions.reserve(regions->size());
    for (PyRegion &region : *regions) {
      aiirRegions.push_back(region);
    }
  }
...
public:
  ...
  constexpr static GetTypeIDFunctionTy getInterfaceID =
      &aiirInferTypeOpInterfaceTypeID;
  ...
  static void appendResultsCallback(intptr_t nTypes, AiirType *types,
                                    void *userData) {
    ...
    for (intptr_t i = 0; i < nTypes; ++i) {
      data->inferredTypes.emplace_back(data->pyAiirContext.getRef(), types[i]);
    }
  }
  ...
  inferReturnTypes(std::optional<nb::sequence> operandList,
    ...
    std::vector<AiirValue> aiirOperands = wrapOperands(std::move(operandList));
    std::vector<AiirRegion> aiirRegions = wrapRegions(std::move(regions));

    std::vector<PyType> inferredTypes;
    PyAiirContext &pyContext = context.resolve();
    AppendResultsCallbackData data{inferredTypes, pyContext};
    AiirStringRef opNameRef =
        aiirStringRefCreate(getOpName().data(), getOpName().length());
    AiirAttribute attributeDict =
        attributes ? attributes->get() : aiirAttributeGetNull();

    AiirLogicalResult result = aiirInferTypeOpInterfaceInferReturnTypes(
        opNameRef, pyContext.get(), location.resolve(), aiirOperands.size(),
        aiirOperands.data(), attributeDict, properties, aiirRegions.size(),
        aiirRegions.data(), &appendResultsCallback, &data);
...
public:
  PyShapedTypeComponents(AiirType elementType) : elementType(elementType) {}
  PyShapedTypeComponents(nb::list shape, AiirType elementType)
...
public:
  ...
  constexpr static GetTypeIDFunctionTy getInterfaceID =
      &aiirInferShapedTypeOpInterfaceTypeID;
  ...
  static void appendResultsCallback(bool hasRank, intptr_t rank,
                                    ...
                                    AiirAttribute attribute, void *userData) {
    auto *data = static_cast<AppendResultsCallbackData *>(userData);
    if (!hasRank) {
  ...
  std::vector<PyShapedTypeComponents> inferReturnTypeComponents(
      ...
      std::optional<PyAttribute> attributes, void *properties,
      std::optional<std::vector<PyRegion>> regions,
      DefaultingPyAiirContext context, DefaultingPyLocation location) {
    std::vector<AiirValue> aiirOperands = wrapOperands(std::move(operandList));
    std::vector<AiirRegion> aiirRegions = wrapRegions(std::move(regions));
    ...
    AiirLogicalResult result = aiirInferShapedTypeOpInterfaceInferReturnTypes(
        opNameRef, pyContext.get(), location.resolve(), aiirOperands.size(),
        aiirOperands.data(), attributeDict, properties, aiirRegions.size(),
        aiirRegions.data(), &appendResultsCallback, &data);
...
public:
  ...
  constexpr static GetTypeIDFunctionTy getInterfaceID =
      &aiirMemoryEffectsOpInterfaceTypeID;
  ...
  static void attach(nb::object &target, const std::string &opName,
    ...
    callbacks.getEffects = [](AiirOperation op,
    ...
    aiirMemoryEffectsOpInterfaceAttachFallbackModel(
