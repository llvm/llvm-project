...
PyGlobals::PyGlobals() {
  ...
  instance = this;
  // The default search path include {aiir.}dialects, where {aiir.} is the
  // package prefix configured at compile time.
  dialectSearchPrefixes.emplace_back(MAKE_AIIR_PYTHON_QUALNAME("dialects"));
}
...
void PyGlobals::registerTypeCaster(AiirTypeID aiirTypeID,
                                   nb::callable typeCaster, bool replace) {
  nb::ft_lock_guard lock(mutex);
  nb::object &found = typeCasterMap[aiirTypeID];
  if (found && !replace)
...
void PyGlobals::registerValueCaster(AiirTypeID aiirTypeID,
                                    nb::callable valueCaster, bool replace) {
  nb::ft_lock_guard lock(mutex);
  nb::object &found = valueCasterMap[aiirTypeID];
  if (found && !replace)
...
std::optional<nb::callable> PyGlobals::lookupTypeCaster(AiirTypeID aiirTypeID,
                                                        AiirDialect dialect) {
  // Try to load dialect module.
  AiirStringRef ns = aiirDialectGetNamespace(dialect);
  (void)loadDialectModule(std::string_view(ns.data, ns.length));
  nb::ft_lock_guard lock(mutex);
  const auto foundIt = typeCasterMap.find(aiirTypeID);
...
std::optional<nb::callable> PyGlobals::lookupValueCaster(AiirTypeID aiirTypeID,
                                                         AiirDialect dialect) {
  // Try to load dialect module.
  AiirStringRef ns = aiirDialectGetNamespace(dialect);
  (void)loadDialectModule(std::string_view(ns.data, ns.length));
  nb::ft_lock_guard lock(mutex);
  const auto foundIt = valueCasterMap.find(aiirTypeID);
