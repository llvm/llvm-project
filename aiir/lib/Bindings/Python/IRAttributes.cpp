...
and the buffer contents must be bit-castable to the AIIR internal
representation:
...
Args:
  ...
  signless: If inferring an appropriate AIIR type, use signless types for
    integers (defaults True).
  type: Skips inference of the AIIR element type and uses this instead. The
    storage size must be consistent with the actual contents of the buffer.
  shape: Overrides the shape of the buffer when constructing the AIIR
...
Raises:
  ValueError: If the type of the buffer or array cannot be matched to an AIIR
...
void PyAffineMapAttribute::bindDerived(ClassTy &c) {
  c.def_static(
      ...
      [](PyAffineMap &affineMap) {
        AiirAttribute attr = aiirAffineMapAttrGet(affineMap.get());
  ...
  c.def_prop_ro(
      ...
      [](PyAffineMapAttribute &self) {
        return PyAffineMap(self.getContext(), aiirAffineMapAttrGetValue(self));
      },
...
void PyIntegerSetAttribute::bindDerived(ClassTy &c) {
  c.def_static(
      ...
      [](PyIntegerSet &integerSet) {
        AiirAttribute attr = aiirIntegerSetAttrGet(integerSet.get());
...
AiirAttribute PyArrayAttribute::getItem(intptr_t i) const {
  return aiirArrayAttrGetElement(*this, i);
}
...
void PyArrayAttribute::bindDerived(ClassTy &c) {
  c.def_static(
      ...
      [](nb::typed<nb::sequence, PyAttribute> attributes,
        ...
        std::vector<AiirAttribute> aiirAttributes;
        aiirAttributes.reserve(nb::len(attributes));
        for (auto attribute : attributes) {
          aiirAttributes.push_back(pyTryCast<PyAttribute>(attribute));
        }
        AiirAttribute attr = aiirArrayAttrGet(
            context->get(), aiirAttributes.size(), aiirAttributes.data());
  ...
  c.def("__getitem__",
        [](PyArrayAttribute &arr,
          ...
          if (i >= aiirArrayAttrGetNumElements(arr))
      ...
      .def("__len__",
           [](const PyArrayAttribute &arr) {
             return aiirArrayAttrGetNumElements(arr);
           })
  ...
  c.def("__add__", [](PyArrayAttribute arr,
    ...
    std::vector<AiirAttribute> attributes;
    intptr_t numOldElements = aiirArrayAttrGetNumElements(arr);
    ...
    AiirAttribute arrayAttr = aiirArrayAttrGet(
...
void PyFloatAttribute::bindDerived(ClassTy &c) {
  c.def_static(
      ...
      [](PyType &type, double value, DefaultingPyLocation loc) {
        ...
        AiirAttribute attr = aiirFloatAttrDoubleGetChecked(loc, type, value);
        if (aiirAttributeIsNull(attr))
          throw AIIRError("Invalid attribute", errors.take());
  ...
  c.def_static(
      "get_unchecked",
      [](PyType &type, double value, DefaultingPyAiirContext context) {
        PyAiirContext::ErrorCapture errors(context->getRef());
        AiirAttribute attr =
            aiirFloatAttrDoubleGet(context.get()->get(), type, value);
        if (aiirAttributeIsNull(attr))
          throw AIIRError("Invalid attribute", errors.take());
        return PyFloatAttribute(type.getContext(), attr);
      },
  ...
  c.def_static(
      "get_f32",
      [](double value, DefaultingPyAiirContext context) {
        AiirAttribute attr = aiirFloatAttrDoubleGet(
            context->get(), aiirF32TypeGet(context->get()), value);
  ...
  c.def_static(
      ...
      [](double value, DefaultingPyAiirContext context) {
        AiirAttribute attr = aiirFloatAttrDoubleGet(
            context->get(), aiirF64TypeGet(context->get()), value);
        return PyFloatAttribute(context->getRef(), attr);
      },
      nb::arg("value"), nb::arg("context") = nb::none(),
      "Gets an uniqued float point attribute associated to a f64 type");
  c.def_prop_ro("value", aiirFloatAttrGetValueDouble,
                "Returns the value of the float attribute");
  c.def("__float__", aiirFloatAttrGetValueDouble,
...
void PyIntegerAttribute::bindDerived(ClassTy &c) {
  c.def_static(
      ...
      [](PyType &type, nb::object value) {
        ...
        if (aiirTypeIsAIndex(type)) {
          int64_t intValue = nb::cast<int64_t>(value);
          AiirAttribute attr = aiirIntegerAttrGet(type, intValue);
          return PyIntegerAttribute(type.getContext(), attr);
        }

        // Get the bit width of the integer type.
        unsigned bitWidth = aiirIntegerTypeGetWidth(type);
        ...
        if (aiirIntegerTypeIsSigned(type)) {
        ...
        AiirAttribute attr =
            aiirIntegerAttrGetFromWords(type, numWords, words.data());
        return PyIntegerAttribute(type.getContext(), attr);
      },
  ...
  c.def_prop_ro_static("static_typeid", [](nb::object & /*class*/) {
    return PyTypeID(aiirIntegerAttrGetTypeID());
  });
}

nb::int_ PyIntegerAttribute::toPyInt(PyIntegerAttribute &self) {
  AiirType type = aiirAttributeGetType(self);
  unsigned bitWidth = aiirIntegerAttrGetValueBitWidth(self);

  // For integers that fit in 64 bits, use the fast path.
  if (bitWidth <= 64) {
    if (aiirTypeIsAIndex(type) || aiirIntegerTypeIsSignless(type))
      return nb::int_(aiirIntegerAttrGetValueInt(self));
    if (aiirIntegerTypeIsSigned(type))
      return nb::int_(aiirIntegerAttrGetValueSInt(self));
    return nb::int_(aiirIntegerAttrGetValueUInt(self));
  }

  // For larger integers, reconstruct the value from raw words.
  unsigned numWords = aiirIntegerAttrGetValueNumWords(self);
  std::vector<uint64_t> words(numWords);
  aiirIntegerAttrGetValueWords(self, words.data());
...
void PyBoolAttribute::bindDerived(ClassTy &c) {
  c.def_static(
      "get",
      [](bool value, DefaultingPyAiirContext context) {
        AiirAttribute attr = aiirBoolAttrGet(context->get(), value);
        return PyBoolAttribute(context->getRef(), attr);
      },
      nb::arg("value"), nb::arg("context") = nb::none(),
      "Gets an uniqued bool attribute");
  c.def_prop_ro("value", aiirBoolAttrGetValue,
                "Returns the value of the bool attribute");
  c.def("__bool__", aiirBoolAttrGetValue,
...
PySymbolRefAttribute::fromList(const std::vector<std::string> &symbols,
  ...
  if (symbols.empty())
    throw std::runtime_error("SymbolRefAttr must be composed of at least "
                             "one symbol.");
  AiirStringRef rootSymbol = toAiirStringRef(symbols[0]);
  std::vector<AiirAttribute> referenceAttrs;
  for (size_t i = 1; i < symbols.size(); ++i) {
    referenceAttrs.push_back(
        aiirFlatSymbolRefAttrGet(context.get(), toAiirStringRef(symbols[i])));
  }
  return PySymbolRefAttribute(context.getRef(),
                              aiirSymbolRefAttrGet(context.get(), rootSymbol,
                                                   referenceAttrs.size(),
                                                   referenceAttrs.data()));
}
...
void PySymbolRefAttribute::bindDerived(ClassTy &c) {
  ...
  c.def_prop_ro(
      ...
      [](PySymbolRefAttribute &self) {
        intptr_t numNested = aiirSymbolRefAttrGetNumNestedReferences(self);
        std::vector<AiirStringRef> symbols;
        symbols.reserve(numNested + 1);
        symbols.push_back(aiirSymbolRefAttrGetRootReference(self));
        for (intptr_t i = 0; i < numNested; ++i) {
          symbols.push_back(aiirSymbolRefAttrGetRootReference(
              aiirSymbolRefAttrGetNestedReference(self, i)));
        }
        return symbols;
      },
...
void PyFlatSymbolRefAttribute::bindDerived(ClassTy &c) {
  c.def_static(
      ...
      [](const std::string &value, DefaultingPyAiirContext context) {
        AiirAttribute attr =
            aiirFlatSymbolRefAttrGet(context->get(), toAiirStringRef(value));
  ...
  c.def_prop_ro(
      ...
      [](PyFlatSymbolRefAttribute &self) {
        AiirStringRef stringRef = aiirFlatSymbolRefAttrGetValue(self);
...
void PyOpaqueAttribute::bindDerived(ClassTy &c) {
  c.def_static(
      ...
      [](const std::string &dialectNamespace, const nb_buffer &buffer,
         PyType &type, DefaultingPyAiirContext context) {
        const nb_buffer_info bufferInfo = buffer.request();
        intptr_t bufferSize = bufferInfo.size;
        AiirAttribute attr = aiirOpaqueAttrGet(
            context->get(), toAiirStringRef(dialectNamespace), bufferSize,
  ...
  c.def_prop_ro(
      ...
      [](PyOpaqueAttribute &self) {
        AiirStringRef stringRef = aiirOpaqueAttrGetDialectNamespace(self);
        return nb::str(stringRef.data, stringRef.length);
      },
  ...
  c.def_prop_ro(
      "data",
      [](PyOpaqueAttribute &self) {
        AiirStringRef stringRef = aiirOpaqueAttrGetData(self);
...
PyDenseElementsAttribute PyDenseElementsAttribute::getFromList(
  ...
  AiirType shapedType;
  if (explicitType) {
    if ((!aiirTypeIsAShaped(*explicitType) ||
         !aiirShapedTypeHasStaticShape(*explicitType))) {
    ...
    shapedType = aiirRankedTensorTypeGet(
        shape.size(), shape.data(),
        aiirAttributeGetType(pyTryCast<PyAttribute>(attributes[0])),
        aiirAttributeGetNull());
  }

  std::vector<AiirAttribute> aiirAttributes;
  aiirAttributes.reserve(numAttributes);
  for (const nb::handle &attribute : attributes) {
    AiirAttribute aiirAttribute = pyTryCast<PyAttribute>(attribute);
    AiirType attrType = aiirAttributeGetType(aiirAttribute);
    aiirAttributes.push_back(aiirAttribute);

    if (!aiirTypeEqual(aiirShapedTypeGetElementType(shapedType), attrType)) {
      std::string message = nanobind::detail::join(
  ...
  AiirAttribute elements = aiirDenseElementsAttrGet(
      shapedType, aiirAttributes.size(), aiirAttributes.data());
...
PyDenseElementsAttribute PyDenseElementsAttribute::getFromBuffer(
  ...
  AiirContext context = contextWrapper->get();
  AiirAttribute attr = getAttributeFromBuffer(
  ...
  if (aiirAttributeIsNull(attr)) {
    throw std::invalid_argument(
        ...
        "AIIR expected layout and is a bug.");
  }
...
PyDenseElementsAttribute::getSplat(const PyType &shapedType,
  ...
  auto contextWrapper =
      PyAiirContext::forContext(aiirTypeGetContext(shapedType));
  if (!aiirAttributeIsAInteger(elementAttr) &&
      !aiirAttributeIsAFloat(elementAttr)) {
  ...
  if (!aiirTypeIsAShaped(shapedType) ||
      !aiirShapedTypeHasStaticShape(shapedType)) {
    std::string message =
        "Expected a static ShapedType for the shaped_type parameter: ";
    message.append(nb::cast<std::string>(nb::repr(nb::cast(shapedType))));
    throw nb::value_error(message.c_str());
  }
  AiirType shapedElementType = aiirShapedTypeGetElementType(shapedType);
  AiirType attrType = aiirAttributeGetType(elementAttr);
  if (!aiirTypeEqual(shapedElementType, attrType)) {
    std::string message =
  ...
  AiirAttribute elements =
      aiirDenseElementsAttrSplatGet(shapedType, elementAttr);
  return PyDenseElementsAttribute(contextWrapper->getRef(), elements);
}

intptr_t PyDenseElementsAttribute::dunderLen() const {
  return aiirElementsAttrGetNumElements(*this);
}

std::unique_ptr<nb_buffer_info> PyDenseElementsAttribute::accessBuffer() {
  AiirType shapedType = aiirAttributeGetType(*this);
  AiirType elementType = aiirShapedTypeGetElementType(shapedType);
  std::string format;

  if (aiirTypeIsAF32(elementType)) {
    // f32
    return bufferInfo<float>(shapedType);
  }
  if (aiirTypeIsAF64(elementType)) {
    // f64
    return bufferInfo<double>(shapedType);
  }
  if (aiirTypeIsAF16(elementType)) {
    // f16
    return bufferInfo<uint16_t>(shapedType, "e");
  }
  if (aiirTypeIsAIndex(elementType)) {
    // Same as IndexType::kInternalStorageBitWidth
    return bufferInfo<int64_t>(shapedType);
  }
  if (aiirTypeIsAInteger(elementType) &&
      aiirIntegerTypeGetWidth(elementType) == 32) {
    if (aiirIntegerTypeIsSignless(elementType) ||
        aiirIntegerTypeIsSigned(elementType)) {
      // i32
      return bufferInfo<int32_t>(shapedType);
    }
    if (aiirIntegerTypeIsUnsigned(elementType)) {
      // unsigned i32
      return bufferInfo<uint32_t>(shapedType);
    }
  } else if (aiirTypeIsAInteger(elementType) &&
             aiirIntegerTypeGetWidth(elementType) == 64) {
    if (aiirIntegerTypeIsSignless(elementType) ||
        aiirIntegerTypeIsSigned(elementType)) {
      // i64
      return bufferInfo<int64_t>(shapedType);
    ...
    if (aiirIntegerTypeIsUnsigned(elementType)) {
      ...
      return bufferInfo<uint64_t>(shapedType);
             ...
             aiirIntegerTypeGetWidth(elementType) == 8) {
    if (aiirIntegerTypeIsSignless(elementType) ||
        aiirIntegerTypeIsSigned(elementType)) {
      // i8
    ...
    if (aiirIntegerTypeIsUnsigned(elementType)) {
      ...
      return bufferInfo<uint8_t>(shapedType);
             ...
             aiirIntegerTypeGetWidth(elementType) == 16) {
    if (aiirIntegerTypeIsSignless(elementType) ||
        aiirIntegerTypeIsSigned(elementType)) {
      // i16
    ...
    if (aiirIntegerTypeIsUnsigned(elementType)) {
      ...
      return bufferInfo<uint16_t>(shapedType);
             ...
             aiirIntegerTypeGetWidth(elementType) == 1) {
    // i1 / bool
...
void PyDenseElementsAttribute::bindDerived(ClassTy &c) {
  ...
  c.def_prop_ro("is_splat",
                [](PyDenseElementsAttribute &self) -> bool {
                  return aiirDenseElementsAttrIsSplat(self);
                })
      .def("get_splat_value",
           [](PyDenseElementsAttribute &self)
               -> nb::typed<nb::object, PyAttribute> {
             if (!aiirDenseElementsAttrIsSplat(self))
               throw nb::value_error(
                   "get_splat_value called on a non-splat attribute");
             return PyAttribute(self.getContext(),
                                aiirDenseElementsAttrGetSplatValue(self))
                 .maybeDownCast();
           });
}
...
AiirType PyDenseElementsAttribute::getShapedType(
    std::optional<AiirType> bulkLoadElementType,
  ...
  if (aiirTypeIsAShaped(*bulkLoadElementType)) {
    if (explicitShape) {
  ...
  AiirAttribute encodingAttr = aiirAttributeGetNull();
  return aiirRankedTensorTypeGet(shape.size(), shape.data(),
                                 *bulkLoadElementType, encodingAttr);
}

AiirAttribute PyDenseElementsAttribute::getAttributeFromBuffer(
  ...
  if (explicitType) {
    ...
    if (format == "f") {
      ...
      bulkLoadElementType = aiirF32TypeGet(context);
    } else if (format == "d") {
      // f64
      assert(view.itemsize == 8 && "mismatched array itemsize");
      bulkLoadElementType = aiirF64TypeGet(context);
    } else if (format == "e") {
      // f16
      assert(view.itemsize == 2 && "mismatched array itemsize");
      bulkLoadElementType = aiirF16TypeGet(context);
    } else if (format == "?") {
      // i1
      bulkLoadElementType = aiirIntegerTypeGet(context, 1);
    } else if (isSignedIntegerFormat(format)) {
      if (view.itemsize == 4) {
        // i32
        bulkLoadElementType = signless ? aiirIntegerTypeGet(context, 32)
                                       : aiirIntegerTypeSignedGet(context, 32);
      } else if (view.itemsize == 8) {
        // i64
        bulkLoadElementType = signless ? aiirIntegerTypeGet(context, 64)
                                       : aiirIntegerTypeSignedGet(context, 64);
      } else if (view.itemsize == 1) {
        // i8
        bulkLoadElementType = signless ? aiirIntegerTypeGet(context, 8)
                                       : aiirIntegerTypeSignedGet(context, 8);
      } else if (view.itemsize == 2) {
        // i16
        bulkLoadElementType = signless ? aiirIntegerTypeGet(context, 16)
                                       : aiirIntegerTypeSignedGet(context, 16);
      }
    } else if (isUnsignedIntegerFormat(format)) {
      if (view.itemsize == 4) {
        // unsigned i32
        bulkLoadElementType = signless
                                  ? aiirIntegerTypeGet(context, 32)
                                  : aiirIntegerTypeUnsignedGet(context, 32);
      } else if (view.itemsize == 8) {
        // unsigned i64
        bulkLoadElementType = signless
                                  ? aiirIntegerTypeGet(context, 64)
                                  : aiirIntegerTypeUnsignedGet(context, 64);
      } else if (view.itemsize == 1) {
        // i8
        bulkLoadElementType = signless ? aiirIntegerTypeGet(context, 8)
                                       : aiirIntegerTypeUnsignedGet(context, 8);
      } else if (view.itemsize == 2) {
        // i16
        bulkLoadElementType = signless
                                  ? aiirIntegerTypeGet(context, 16)
                                  : aiirIntegerTypeUnsignedGet(context, 16);
      }
    }
  ...
  AiirType type = getShapedType(bulkLoadElementType, explicitShape, view);
  return aiirDenseElementsAttrRawBufferGet(type, view.len, view.buf);
}
...
nb::int_ PyDenseIntElementsAttribute::dunderGetItem(intptr_t pos) const {
  ...
  AiirType type = aiirAttributeGetType(*this);
  type = aiirShapedTypeGetElementType(type);
  // Index type can also appear as a DenseIntElementsAttr and therefore can be
  // casted to integer.
  assert(aiirTypeIsAInteger(type) ||
         aiirTypeIsAIndex(type) && "expected integer/index element type in "
  ...
  if (aiirTypeIsAIndex(type)) {
    return nb::int_(aiirDenseElementsAttrGetIndexValue(*this, pos));
  }
  unsigned width = aiirIntegerTypeGetWidth(type);
  bool isUnsigned = aiirIntegerTypeIsUnsigned(type);
  if (isUnsigned) {
    if (width == 1) {
      return nb::int_(int(aiirDenseElementsAttrGetBoolValue(*this, pos)));
    }
    if (width == 8) {
      return nb::int_(aiirDenseElementsAttrGetUInt8Value(*this, pos));
    }
    if (width == 16) {
      return nb::int_(aiirDenseElementsAttrGetUInt16Value(*this, pos));
    }
    if (width == 32) {
      return nb::int_(aiirDenseElementsAttrGetUInt32Value(*this, pos));
    }
    if (width == 64) {
      return nb::int_(aiirDenseElementsAttrGetUInt64Value(*this, pos));
    }
  } else {
    if (width == 1) {
      return nb::int_(int(aiirDenseElementsAttrGetBoolValue(*this, pos)));
    ...
    if (width == 8) {
      return nb::int_(aiirDenseElementsAttrGetInt8Value(*this, pos));
    }
    if (width == 16) {
      return nb::int_(aiirDenseElementsAttrGetInt16Value(*this, pos));
    }
    if (width == 32) {
      return nb::int_(aiirDenseElementsAttrGetInt32Value(*this, pos));
    }
    if (width == 64) {
      return nb::int_(aiirDenseElementsAttrGetInt64Value(*this, pos));
    }
  }
...
PyDenseResourceElementsAttribute::getFromBuffer(
  ...
  AiirAttribute attr = aiirUnmanagedDenseResourceElementsAttrGet(
      type, toAiirStringRef(name), view->buf, rawBufferSize, inferredAlignment,
...
intptr_t PyDictAttribute::dunderLen() const {
  return aiirDictionaryAttrGetNumElements(*this);
}

bool PyDictAttribute::dunderContains(const std::string &name) const {
  return !aiirAttributeIsNull(
      aiirDictionaryAttrGetElementByName(*this, toAiirStringRef(name)));
}
...
void PyDictAttribute::bindDerived(ClassTy &c) {
  ...
  c.def_static(
      ...
      [](const nb::typed<nb::dict, nb::str, PyAttribute> &attributes,
        ...
        aiirNamedAttributes.reserve(attributes.size());
        for (std::pair<nb::handle, nb::handle> it : attributes) {
          auto &aiirAttr = nb::cast<PyAttribute &>(it.second);
          auto name = nb::cast<std::string>(it.first);
          aiirNamedAttributes.push_back(aiirNamedAttributeGet(
              aiirIdentifierGet(aiirAttributeGetContext(aiirAttr),
                                toAiirStringRef(name)),
              aiirAttr));
        }
        AiirAttribute attr =
            aiirDictionaryAttrGet(context->get(), aiirNamedAttributes.size(),
  ...
  c.def("__getitem__",
        [](PyDictAttribute &self,
          ...
          AiirAttribute attr =
              aiirDictionaryAttrGetElementByName(self, toAiirStringRef(name));
          if (aiirAttributeIsNull(attr))
  ...
  c.def("__getitem__", [](PyDictAttribute &self, intptr_t index) {
    ...
    AiirNamedAttribute namedAttr = aiirDictionaryAttrGetElement(self, index);
...
nb::float_ PyDenseFPElementsAttribute::dunderGetItem(intptr_t pos) const {
  ...
  // TODO: consider caching the type properties in the constructor to avoid
  // querying them on each element access.
  if (aiirTypeIsAF32(type)) {
    return nb::float_(aiirDenseElementsAttrGetFloatValue(*this, pos));
  }
  if (aiirTypeIsAF64(type)) {
    return nb::float_(aiirDenseElementsAttrGetDoubleValue(*this, pos));
  }
...
void PyTypeAttribute::bindDerived(ClassTy &c) {
  c.def_static(
      "get",
      [](const PyType &value, DefaultingPyAiirContext context) {
        AiirAttribute attr = aiirTypeAttrGet(value.get());
  ...
  c.def_prop_ro(
      "value", [](PyTypeAttribute &self) -> nb::typed<nb::object, PyType> {
        return PyType(self.getContext(), aiirTypeAttrGetValue(self.get()))
            .maybeDownCast();
      });
}
...
void PyStridedLayoutAttribute::bindDerived(ClassTy &c) {
  c.def_static(
      ...
      [](int64_t offset, const std::vector<int64_t> &strides,
        ...
        AiirAttribute attr = aiirStridedLayoutAttrGet(
  ...
  c.def_static(
      "get_fully_dynamic",
      [](int64_t rank, DefaultingPyAiirContext ctx) {
        auto dynamic = aiirShapedTypeGetDynamicStrideOrOffset();
  ...
  c.def_prop_ro(
      ...
      [](PyStridedLayoutAttribute &self) {
        return aiirStridedLayoutAttrGetOffset(self);
      },
      "Returns the value of the float point attribute");
  c.def_prop_ro(
      "strides",
      [](PyStridedLayoutAttribute &self) {
        intptr_t size = aiirStridedLayoutAttrGetNumStrides(self);
        std::vector<int64_t> strides(size);
        for (intptr_t i = 0; i < size; i++) {
          strides[i] = aiirStridedLayoutAttrGetStride(self, i);
        }
        return strides;
      },
      "Returns the value of the float point attribute");
...
void PyStringAttribute::bindDerived(ClassTy &c) {
  c.def_static(
      ...
      [](const std::string &value, DefaultingPyAiirContext context) {
        AiirAttribute attr =
            aiirStringAttrGet(context->get(), toAiirStringRef(value));
  ...
  c.def_static(
      "get",
      [](const nb::bytes &value, DefaultingPyAiirContext context) {
        AiirAttribute attr =
            aiirStringAttrGet(context->get(), toAiirStringRef(value));
        return PyStringAttribute(context->getRef(), attr);
      },
  ...
  c.def_static(
      ...
      [](PyType &type, const std::string &value) {
        AiirAttribute attr =
            aiirStringAttrTypedGet(type, toAiirStringRef(value));
  ...
  c.def_prop_ro(
      ...
      [](PyStringAttribute &self) {
        AiirStringRef stringRef = aiirStringAttrGetValue(self);
        return nb::str(stringRef.data, stringRef.length);
      },
...
static AiirDynamicAttrDefinition
getDynamicAttrDef(const std::string &fullAttrName,
  ...
  AiirDynamicAttrDefinition attrDef = aiirExtensibleDialectLookupAttrDefinition(
      dialect, toAiirStringRef(attrName));
...
void PyDynamicAttribute::bindDerived(ClassTy &c) {
  c.def_static(
      ...
      [](const std::string &fullAttrName, const std::vector<PyAttribute> &attrs,
        ...
        AiirDynamicAttrDefinition attrDef =
            getDynamicAttrDef(fullAttrName, context);
        AiirAttribute attr =
            aiirDynamicAttrGet(attrDef, aiirAttrs.data(), aiirAttrs.size());
  ...
  c.def_prop_ro(
      ...
      [](PyDynamicAttribute &self) {
        size_t numParams = aiirDynamicAttrGetNumParams(self);
        std::vector<PyAttribute> params;
        params.reserve(numParams);
        for (size_t i = 0; i < numParams; ++i)
          params.emplace_back(self.getContext(),
                              aiirDynamicAttrGetParam(self, i));
        return params;
      },
      "Returns the parameters of the dynamic attribute as a list of "
      "attributes.");
  c.def_prop_ro("attr_name", [](PyDynamicAttribute &self) {
    AiirDynamicAttrDefinition attrDef = aiirDynamicAttrGetAttrDef(self);
    AiirStringRef name = aiirDynamicAttrDefinitionGetName(attrDef);
    AiirDialect dialect = aiirDynamicAttrDefinitionGetDialect(attrDef);
  ...
  c.def_static(
      "lookup_typeid",
      [](const std::string &fullAttrName, DefaultingPyAiirContext context) {
        AiirDynamicAttrDefinition attrDef =
            getDynamicAttrDef(fullAttrName, context);
        return PyTypeID(aiirDynamicAttrDefinitionGetTypeID(attrDef));
      },
...
void populateIRAttributes(nb::module_ &m) {
  ...
  PyGlobals::get().registerTypeCaster(aiirDenseTypedElementsAttrGetTypeID(),
