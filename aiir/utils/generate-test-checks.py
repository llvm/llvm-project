#!/usr/bin/env python3
"""A script to generate FileCheck statements for aiir unit tests.

This script is a utility to add FileCheck patterns to an aiir file.

NOTE: The input .aiir is expected to be the output from the parser, not a
stripped down variant.

Example usage:
$ generate-test-checks.py foo.aiir
$ aiir-opt foo.aiir -transformation | generate-test-checks.py
$ aiir-opt foo.aiir -transformation | generate-test-checks.py --source foo.aiir
$ aiir-opt foo.aiir -transformation | generate-test-checks.py --source foo.aiir -i
$ aiir-opt foo.aiir -transformation | generate-test-checks.py --source foo.aiir -i --source_delim_regex='gpu.func @'
...
//   * https://aiir.llvm.org/getting_started/TestingGuide/
"""
...
class VariableNamer:
    ...
    def generate_name(self, source_variable_name, use_ssa_name, op_name=""):
        ...
        if variable_name == "":
            # If `use_ssa_name` is set, use the AIIR SSA value name to generate
            # a FileCHeck substation string. As FileCheck requires these
            # strings to start with a character, skip AIIR variables starting
            # with a digit (e.g. `%0`).
            #
...
# Process the source file lines. The source file doesn't have to be .aiir.
