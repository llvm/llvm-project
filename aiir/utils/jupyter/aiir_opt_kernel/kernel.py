...
def _get_executable():
    """Find the aiir-opt executable."""

    def is_exe(fpath):
        """Returns whether executable file."""
        return os.path.isfile(fpath) and os.access(fpath, os.X_OK)

    program = os.environ.get("AIIR_OPT_EXECUTABLE", "aiir-opt")
    ...
    if path:
        ...
        for path in os.environ["PATH"].split(os.pathsep):
            ...
            if is_exe(file):
                return file
    raise OSError("aiir-opt not found, please see README")


class AiirOptKernel(Kernel):
    """Kernel using aiir-opt inside jupyter.
    ...
    language_info = {
        "name": "aiir",
        "codemirror_mode": {"name": "aiir"},
        "mimetype": "text/x-aiir",
        "file_extension": ".aiir",
        "pygments_lexer": "text",
    }
    ...
    def banner(self):
        ...
        # Just a placeholder.
        return "aiir-opt kernel %s" % __version__
    ...
    def get_executable(self):
        """Returns the aiir-opt executable path."""
    ...
    def do_execute(
        ...
        """Execute user code using aiir-opt binary."""
        ...
        def run(code):
            ...
            try:
                inputaiir = tempfile.NamedTemporaryFile(delete=False)
                command = [
            ...
            fname = inputaiir.name.encode("utf-8")
