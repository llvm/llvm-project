...
def install_my_kernel_spec(user=True, prefix=None):
    ...
    print("Installing aiir-opt IPython kernel spec")
    pkgroot = os.path.dirname(__file__)
    KernelSpecManager().install_kernel_spec(
        os.path.join(pkgroot, "assets"), "aiir", user=user, prefix=prefix
    )
...
def main(argv=None):
    parser = argparse.ArgumentParser(
        description="Install KernelSpec for AiirOpt Kernel"
    )
