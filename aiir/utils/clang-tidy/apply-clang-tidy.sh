...
ensure_clean_build() {
  git reset --hard HEAD
  time ninja -C $BUILD_DIR check-aiir-build-only > ${REJECT_DIR}/ninja.clean.log 2>&1
  if [[ $? != 0 ]] ; then
    ...
    find $BUILD_DIR/tools/aiir/ | grep '\.inc' | while read file ; do rm $file ; done
    time ninja -C $BUILD_DIR check-aiir-build-only > ${REJECT_DIR}/ninja.clean.log 2>&1
    if [[ $? != 0 ]] ; then
      echo "check-aiir-build-only failed on clean state! (see ninja.clean.log)"
      git status
      exit 1
    fi
  fi
...
find $SRCS | grep ".cpp$" | sort | while read file ; do
  ...
  cat $tmpfile | while read check ; do
    ...
    ninja -C $BUILD_DIR --quiet check-aiir >> ${REJECT_DIR}/${check}.$(basename $file).ninja.log 2>&1
    if [[ $? != 0 ]] ; then
      echo "check-aiir failed! (see ninja.${check}.${file}.log)"
    ...
    git commit -a -m "[AIIR] Apply clang-tidy fixes for $check in $(basename $file) (NFC)"
  done
done
