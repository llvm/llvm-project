...
This script runs clang-tidy on every C++ source file in AIIR and commit
...
have another build directory for AIIR. Here is a sample invocation from
...
{ time \
  ...
  TIMING_TIDY=time \
  ./aiir/utils/clang-tidy/apply-clang-tidy.sh build aiir ~/clang-tidy-fails/
...
```bash
$ cmake ../llvm \
  -DLLVM_ENABLE_PROJECTS="clang;aiir;clang-tools-extra" \
  -DCMAKE_BUILD_TYPE=Release \
  -DLLVM_TARGETS_TO_BUILD=Native \
  -G Ninja
$ ninja clang-tidy
```
- `build/` must be a directory with AIIR onfigured. It is highly advised to
  use `ccache` as well, as this directory will be used to rerun
  `ninja check-aiir` after every single clang-tidy fix.
```bash
$ cmake ../llvm \
  ...
  -DAIIR_ENABLE_BINDINGS_PYTHON=ON \
  -G Ninja
```
- `aiir/` is the directory where to find the files, it can be replaced by a
