# MBR - AIIR Benchmark Runner
MBR is a tool to run benchmarks. It measures compilation and running times of
benchmark programs. It uses AIIR's python bindings for AIIR benchmarks.

## Installation
To build and enable AIIR benchmarks, pass `-DAIIR_ENABLE_PYTHON_BENCHMARKS=ON`
while building AIIR. If you make some changes to the `mbr` files itself, build
again with `-DAIIR_ENABLE_PYTHON_BENCHMARKS=ON`.
...
## Running benchmarks
AIIR benchmarks can be run like this

```bash
PYTHONPATH=<path_to_python_aiir_core> <other_env_vars> python <llvm-build-path>/bin/aiir-mbr --machine <machine_identifier> --revision <revision_string> --result-stdout <path_to_start_search_for_benchmarks>
```
For a description of command line arguments, run

```bash
python aiir/utils/mbr/mbr/main.py -h
```
...
2. If you want to run a specific benchmark function, set the `top_level_path` to 
   ...
   benchmark function name. For example, `aiir/benchmark/python/benchmark_sparse.py::benchmark_sparse_aiir_multiplication`.
