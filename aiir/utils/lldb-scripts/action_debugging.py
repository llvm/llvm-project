...
class AiirDebug:
    """AIIR debugger commands
    This is the class that hooks into LLDB and registers the `aiir` command.
    ...
    def __init__(self, debugger, unused):
        super().__init__()
        self.create_options()
        self.help_string = AiirDebug.parser.format_help()

    @classmethod
    def create_options(cls):
        if AiirDebug.parser:
            return AiirDebug.parser
        ...
        AiirDebug.parser = argparse.ArgumentParser(
            prog=cls.lldb_command, usage=usage, description=description, add_help=False
        )
        AiirDebug.subparsers = AiirDebug.parser.add_subparsers(dest="command")
        return AiirDebug.parser

    def get_short_help(self):
        return "AIIR debugger commands"
    ...
    def __call__(self, debugger, command, exe_ctx, result):
        ...
        try:
            args = AiirDebug.parser.parse_args(command_args)
        except:
...
class SetControl:
    ...
    # pass to AIIR and the help string.
    commands = {
    ...
    @classmethod
    def register_aiir_subparser(cls):
        for cmd, (cmdInt, help) in cls.commands.items():
            parser = AiirDebug.subparsers.add_parser(
                cmd,
                help=help,
            )
    ...
    def process_options(cls, options, debugger, command, exe_ctx, result):
        ...
        result = frame.EvaluateExpression(
            "((bool (*)(int))aiirDebuggerSetControl)(%d)" % (cmdInt[0]),
            exprOptions,
        )
...
class PrintContext:
    @classmethod
    def register_aiir_subparser(cls):
        cls.parser = AiirDebug.subparsers.add_parser(
    ...
    def process_options(cls, options, debugger, command, exe_ctx, result):
        ...
        if not frame.IsValid():
            ...
            return
        result = frame.EvaluateExpression(
            "((bool (*)())&aiirDebuggerPrintContext)()", exprOptions
        )
        if not result.error.Success():
...
class Backtrace:
    ...
    def process_options(cls, options, debugger, command, exe_ctx, result):
        ...
        result = frame.EvaluateExpression(
            "((bool(*)(bool))aiirDebuggerPrintActionBacktrace)(%d)" % (options.context),
            exprOptions,
        )
        if not result.error.Success():
...
class PrintCursor:
    ...
    def process_options(cls, options, debugger, command, exe_ctx, result):
        ...
        result = frame.EvaluateExpression(
            "((bool(*)(bool))aiirDebuggerCursorPrint)(%d)" % (options.print_region),
            exprOptions,
        )
        if not result.error.Success():
...
class SelectCursorFromContext:
    ...
    def process_options(cls, options, debugger, command, exe_ctx, result):
        ...
        result = frame.EvaluateExpression(
            "((bool(*)(int))aiirDebuggerCursorSelectIRUnitFromContext)(%d)"
            % options.index,
            exprOptions,
        )
        if not result.error.Success():
...
class CursorSelectParent:
    ...
    def process_options(cls, options, debugger, command, exe_ctx, result):
        ...
        result = frame.EvaluateExpression(
            "((bool(*)())aiirDebuggerCursorSelectParentIRUnit)()",
            exprOptions,
        )
        if not result.error.Success():
            print("Error manipulating cursor: %s" % (result.error))
...
class SelectCursorChild:
    ...
    def process_options(cls, options, debugger, command, exe_ctx, result):
        ...
        result = frame.EvaluateExpression(
            "((bool(*)(int))aiirDebuggerCursorSelectChildIRUnit)(%d)" % options.index,
            exprOptions,
        )
        if not result.error.Success():
            print("Error manipulating cursor: %s" % (result.error))
...
class CursorSelecPrevious:
    ...
    def process_options(cls, options, debugger, command, exe_ctx, result):
        ...
        result = frame.EvaluateExpression(
            "((bool(*)())aiirDebuggerCursorSelectPreviousIRUnit)()",
            exprOptions,
        )
        if not result.error.Success():
            print("Error manipulating cursor: %s" % (result.error))
...
class CursorSelecNext:
    ...
    def process_options(cls, options, debugger, command, exe_ctx, result):
        ...
        result = frame.EvaluateExpression(
            "((bool(*)())aiirDebuggerCursorSelectNextIRUnit)()",
            exprOptions,
        )
        if not result.error.Success():
            print("Error manipulating cursor: %s" % (result.error))
...
class Breakpoint:
    ...
    def enable(self, frame=None):
        ...
        cmd = (
            "((bool (*)(void *))aiirDebuggerEnableBreakpoint)((void *)%s)" % self.handle
        )
    ...
    def disable(self, frame=None):
        ...
        # LLDB can't signal any error with the expression evaluation (at least I don't know how).
        cmd = (
            "((bool (*)(void *)) aiirDebuggerDisableBreakpoint)((void *)%s)"
            % self.handle
        )
        result = frame.EvaluateExpression(cmd, exprOptions)
        if not result.error.Success():
...
class TagBreakpoint(Breakpoint):
    ...
    def register_aiir_subparser(cls):
        cls.parser = AiirDebug.subparsers.add_parser(
            cls.aiir_subcommand, help="add a breakpoint on actions' tag matching"
        )
        cls.parser.set_defaults(func=cls.process_options)
    ...
    def install(self, frame):
        result = frame.EvaluateExpression(
            '((void *(*)(const char *))aiirDebuggerAddTagBreakpoint)("%s")'
            % (self.tag),
            exprOptions,
        )
        if not result.error.Success():
...
class FileLineBreakpoint(Breakpoint):
    ...
    def install(self, frame):
        result = frame.EvaluateExpression(
            '((void *(*)(const char *, int, int))aiirDebuggerAddFileLineColLocBreakpoint)("%s", %d, %d)'
...
def __lldb_init_module(debugger, dict):
    ...
    if debugger.GetNumTargets() > 1:
        print(
            "Multiple targets (%s) loaded, attaching AIIR debugging to %s"
            % (debugger.GetNumTargets(), target)
        )

    # Register all classes that have a register_lldb_command method
    module_name = __name__
    parser = AiirDebug.create_options()
    AiirDebug.__doc__ = parser.format_help()

    # Add the AIIR entry point to LLDB as a command.
    command = "command script add -o -c %s.%s %s" % (
        module_name,
        AiirDebug.__name__,
        AiirDebug.lldb_command,
    )
    ...
    on_breackpoint = target.BreakpointCreateByName("aiirDebuggerBreakpointHook")

    print(
        'The "{0}" command has been installed for target `{1}`, type "help {0}" or "{0} '
        '--help" for detailed help.'.format(AiirDebug.lldb_command, target)
    )
    for _name, cls in inspect.getmembers(sys.modules[module_name]):
        if inspect.isclass(cls) and getattr(cls, "register_aiir_subparser", None):
            cls.register_aiir_subparser()
