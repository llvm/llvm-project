"""
LLDB Formatters for AIIR data types.

Load into LLDB with 'command script import /path/to/aiirDataFormatters.py'
"""
...
builtin_attr_type_mnemonics = {
    "aiir::AffineMapAttr": '"affine_map<...>"',
    "aiir::ArrayAttr": '"[...]"',
    "aiir::DenseArray": '"array<...>"',
    "aiir::DenseResourceElementsAttr": '"dense_resource<...>"',
    "aiir::DictionaryAttr": '"{...}"',
    "aiir::IntegerAttr": '"float"',
    "aiir::IntegerAttr": '"integer"',
    "aiir::IntegerSetAttr": '"affine_set<...>"',
    "aiir::SparseElementsAttr": '"sparse<...>"',
    "aiir::StringAttr": '""...""',
    "aiir::StridedLayout": '"strided_layout"',
    "aiir::UnitAttr": '"unit"',
    "aiir::CallSiteLoc": '"loc(callsite(...))"',
    "aiir::FusedLoc": '"loc(fused<...>[...])"',
    "aiir::UnknownLoc": '"loc(unknown)"',
    "aiir::Float4E2M1FNType": '"f4E2M1FN"',
    "aiir::Float6E2M3FNType": '"f6E2M3FN"',
    "aiir::Float6E3M2FNType": '"f6E3M2FN"',
    "aiir::Float8E5M2Type": '"f8E5M2"',
    "aiir::Float8E4M3Type": '"f8E4M3"',
    "aiir::Float8E4M3FNType": '"f8E4M3FN"',
    "aiir::Float8E5M2FNUZType": '"f8E5M2FNUZ"',
    "aiir::Float8E4M3FNUZType": '"f8E4M3FNUZ"',
    "aiir::Float8E4M3B11FNUZType": '"f8E4M3B11FNUZ"',
    "aiir::Float8E3M4Type": '"f8E3M4"',
    "aiir::Float8E8M0FNUType": '"f8E8M0FNU"',
    "aiir::BFloat16Type": '"bf16"',
    "aiir::Float16Type": '"f16"',
    "aiir::FloatTF32Type": '"tf32"',
    "aiir::Float32Type": '"f32"',
    "aiir::Float64Type": '"f64"',
    "aiir::Float80Type": '"f80"',
    "aiir::Float128Type": '"f128"',
    "aiir::FunctionType": '"(...) -> (...)"',
    "aiir::IndexType": '"index"',
    "aiir::IntegerType": '"iN"',
    "aiir::NoneType": '"none"',
    "aiir::TupleType": '"tuple<...>"',
    "aiir::MemRefType": '"memref<...>"',
    "aiir::UnrankedMemRef": '"memref<...>"',
    "aiir::UnrankedTensorType": '"tensor<...>"',
    "aiir::RankedTensorType": '"tensor<...>"',
    "aiir::VectorType": '"vector<...>"',
}
...
class ComputedTypeIDMap:
    ...
    def __init__(self, target: lldb.SBTarget, internal_dict: dict):
        ...
        for type_id in type_ids:
            ...
            match = re.search("^aiir::detail::TypeIDResolver<(.*), void>::id$", name)
            if not match:
                continue
...
def is_derived_attribute_or_type(sbtype: lldb.SBType, internal_dict):
    ...
    return base_name.startswith(("aiir::Attribute::AttrBase", "aiir::Type::TypeBase"))
...
def is_attribute_or_type(sbtype: lldb.SBType, internal_dict):
    ...
    if num_bases == 0:
        return typeName in ["aiir::Attribute", "aiir::Type", "aiir::Location"]

    # Check the easy cases of AttrBase/TypeBase.
    if typeName.startswith(("aiir::Attribute::AttrBase", "aiir::Type::TypeBase")):
        return True
...
class AttrTypeSynthProvider:
    ...
    def __init__(self, valobj: lldb.SBValue, internal_dict):
        ...
        if self.valobj.GetTypeName() == "aiir::Location":
        ...
        elif type_name.startswith("aiir::Dense"):
            self.mnemonic = "dense<...>"
        else:
...
def is_op(sbtype: lldb.SBType, internal_dict):
    ...
    if sbtype.GetNumberOfDirectBaseClasses() == 0:
        return typeName == "aiir::OpState"
    if typeName == "aiir::Operation" or typeName.startswith("aiir::Op<"):
        return True
...
class OperationSynthProvider:
    ...
    def update(self):
        ...
        if "aiir::Operation" not in self.valobj.GetTypeName():
...
class ValueSynthProvider:
    ...
    def get_child_at_index(self, index):
        ...
        if self.kind == 7:
            derived_impl_str = f"((aiir::detail::BlockArgumentImpl *){impl_ptr_str})"
        elif self.kind == 6:
            derived_impl_str = f"((aiir::detail::OutOfLineOpResult *){impl_ptr_str})"
        else:
            derived_impl_str = f"((aiir::detail::InlineOpResult *){impl_ptr_str})"
...
def __lldb_init_module(debugger: lldb.SBDebugger, internal_dict):
    cat: lldb.SBTypeCategory = debugger.CreateCategory("aiir")
    cat.SetEnabled(True)

    # Attributes and Types
    cat.AddTypeSummary(
        lldb.SBTypeNameSpecifier(
            "aiirDataFormatters.is_attribute_or_type", lldb.eFormatterMatchCallback
        ),
        lldb.SBTypeSummary.CreateWithFunctionName(
            "aiirDataFormatters.AttrTypeSummaryProvider"
        ),
    )
    cat.AddTypeSynthetic(
        ...
        lldb.SBTypeSynthetic.CreateWithClassName(
            "aiirDataFormatters.AttrTypeSynthProvider"
        ),
    )

    # Operation
    cat.AddTypeSynthetic(
        lldb.SBTypeNameSpecifier("aiir::Block", lldb.eFormatterMatchExact),
        lldb.SBTypeSynthetic.CreateWithClassName(
            "aiirDataFormatters.BlockSynthProvider"
        ),
    )

    # NamedAttribute
    cat.AddTypeSummary(
        lldb.SBTypeNameSpecifier("aiir::NamedAttribute", lldb.eFormatterMatchExact),
        lldb.SBTypeSummary.CreateWithSummaryString("${var.name%S} = ${var.value%S}"),
    )

    # OperationName
    cat.AddTypeSummary(
        lldb.SBTypeNameSpecifier("aiir::OperationName", lldb.eFormatterMatchExact),
    ...
    cat.AddTypeSummary(
        lldb.SBTypeNameSpecifier(
            "aiirDataFormatters.is_op", lldb.eFormatterMatchCallback
        ),
        lldb.SBTypeSummary.CreateWithFunctionName(
            "aiirDataFormatters.OperationSummaryProvider"
        ),
    )
    cat.AddTypeSynthetic(
        lldb.SBTypeNameSpecifier(
        ...
        lldb.SBTypeSynthetic.CreateWithClassName(
            "aiirDataFormatters.OperationSynthProvider"
        ),
    )
    ...
    def add_direct_range_summary_and_synth(name):
        ...
        cat.AddTypeSynthetic(
            lldb.SBTypeNameSpecifier(name, lldb.eFormatterMatchExact),
            lldb.SBTypeSynthetic.CreateWithClassName(
                "aiirDataFormatters.DirectRangeSynthProvider"
            ),
        )
    ...
    def add_indirect_range_summary_and_synth(name):
        ...
        cat.AddTypeSynthetic(
            lldb.SBTypeNameSpecifier(name, lldb.eFormatterMatchExact),
            lldb.SBTypeSynthetic.CreateWithClassName(
                "aiirDataFormatters.InDirectRangeSynthProvider"
            ),
        )
    ...
    def add_iplist_range_summary_and_synth(name):
        ...
        cat.AddTypeSynthetic(
            lldb.SBTypeNameSpecifier(name, lldb.eFormatterMatchExact),
            lldb.SBTypeSynthetic.CreateWithClassName(
                "aiirDataFormatters.IPListRangeSynthProvider"
            ),
        )

    add_direct_range_summary_and_synth("aiir::Operation::operand_range")
    add_direct_range_summary_and_synth("aiir::OperandRange")
    add_direct_range_summary_and_synth("aiir::Operation::result_range")
    add_direct_range_summary_and_synth("aiir::ResultRange")
    add_direct_range_summary_and_synth("aiir::SuccessorRange")
    add_indirect_range_summary_and_synth("aiir::ValueRange")
    add_indirect_range_summary_and_synth("aiir::TypeRange")
    add_iplist_range_summary_and_synth("aiir::Block::OpListType")
    add_iplist_range_summary_and_synth("aiir::Region::BlockListType")
    ...
    def add_value_summary_and_synth(name):
        cat.AddTypeSummary(
            lldb.SBTypeNameSpecifier(name, lldb.eFormatterMatchExact),
            lldb.SBTypeSummary.CreateWithFunctionName(
                "aiirDataFormatters.ValueSummaryProvider"
            ),
        )
        cat.AddTypeSynthetic(
            lldb.SBTypeNameSpecifier(name, lldb.eFormatterMatchExact),
            lldb.SBTypeSynthetic.CreateWithClassName(
                "aiirDataFormatters.ValueSynthProvider"
            ),
        )

    add_value_summary_and_synth("aiir::BlockArgument")
    add_value_summary_and_synth("aiir::Value")
    add_value_summary_and_synth("aiir::OpResult")
    add_value_summary_and_synth("aiir::detail::OpResultImpl")
