//===- aiir-rewrite.cpp - AIIR Rewrite Driver -----------------------------===//
//
...
//===----------------------------------------------------------------------===//
//
// Main entry function for aiir-rewrite.
//
//===----------------------------------------------------------------------===//

#include "aiir/AsmParser/AsmParser.h"
#include "aiir/AsmParser/AsmParserState.h"
...
public:
  ...
  AIIRContext *getContext() { return &context; }
...
std::unique_ptr<RewritePad> RewritePad::init(StringRef inputFilename,
  ...
  // Set up the AIIR context and error handling.
...
using RewriterFunction = std::function<aiir::LogicalResult(
    aiir::RewritePad &rewriteState, llvm::raw_ostream &os)>;

/// Structure to group information about a rewriter (argument to invoke via
/// aiir-tblgen, description, and rewriter function).
class RewriterInfo {
public:
  ...
  LogicalResult invoke(aiir::RewritePad &rewriteState, raw_ostream &os) const {
    assert(rewriter && "Cannot call rewriter with null rewriter");
    return rewriter(rewriteState, os);
  }

  /// Returns the command line option that may be passed to 'aiir-rewrite' to
...
private:
  // The argument with which to invoke the rewriter via aiir-tblgen.
  StringRef arg;
...
static aiir::RewriterRegistration rewriteSimpleRename("simple-rename",
...
static aiir::RewriterRegistration
...
int main(int argc, char **argv) {
  ...
  llvm::cl::opt<const aiir::RewriterInfo *, false, aiir::RewriterNameParser>
      rewriter("", llvm::cl::desc("Rewriter to run"));

  std::string helpHeader = "aiir-rewrite";
  ...
  if (!rewriter) {
    ...
    return aiir::asMainReturnCode(aiir::failure());
  }
  ...
  return aiir::asMainReturnCode(result);
}
