//===- RewriterGen.cpp - AIIR pattern rewriter generator ------------------===//
//
...
#define DEBUG_TYPE "aiir-tblgen-rewritergen"

namespace llvm {
template <>
struct format_provider<aiir::tblgen::Pattern::IdentifierLine> {
  static void format(const aiir::tblgen::Pattern::IdentifierLine &v,
...
public:
  ...
  // Emits the aiir::RewritePattern struct named `rewriteName`.
...
void PatternEmitter::emitStaticMatcher(DagNode tree, std::string funcName) {
  os << formatv(
      "static ::llvm::LogicalResult {0}(::aiir::PatternRewriter &rewriter, "
      "::aiir::Operation *op0, ::llvm::SmallVector<::aiir::Operation "
      "*, 4> &tblgen_ops",
      funcName);
...
void PatternEmitter::emitStaticMatchCall(DagNode tree, StringRef opName) {
  ...
  os << formatv("if(::aiir::failed({0}(rewriter, {1}, tblgen_ops", funcName,
                opName);
  ...
  os << "))) {\n";
  os.scope().os << "return ::aiir::failure();\n";
  os << "}\n";
}
...
void PatternEmitter::emitStaticVerifierCall(StringRef funcName,
  ...
  os << formatv("if(::aiir::failed({0}(rewriter, {1}, {2}, {3}))) {{\n",
...
void PatternEmitter::emitNativeCodeMatch(DagNode tree, StringRef opName,
  ...
  emitMatchCheck(opName, formatv("!::aiir::failed({0})", nativeCodeCall),
                 formatv("\"{0} return ::aiir::failure\"", nativeCodeCall));
...
void PatternEmitter::emitEitherOperandMatch(DagNode tree, DagNode eitherArgTree,
  ...
  os << formatv(
      "auto {0} = [&](::aiir::OperandRange v0, ::aiir::OperandRange v1) {{\n",
      lambda);
  ...
  os << formatv("if(::aiir::failed({0}(eitherOperand0, eitherOperand1)) && "
                "::aiir::failed({0}(eitherOperand1, "
                "eitherOperand0)))\n",
                lambda);
  os.indent() << "return ::aiir::failure();\n";
...
void PatternEmitter::emitAttributeMatch(DagNode tree, StringRef castedName,
  ...
  if (attr.hasDefaultValue()) {
    ...
    // should just capture a aiir::Attribute() to signal the missing state.
  ...
  if (!matcher.isUnspecified()) {
    ...
    if (attr.isOptional()) {
      ...
      if (!StringRef(matcher.getConditionTemplate()).contains("!$_self") &&
        ...
        os << "if (!tblgen_attr) return ::aiir::failure();\n";
      }
    }
    emitStaticVerifierCall(
...
void PatternEmitter::emit(StringRef rewriteName) {
  ...
  os << formatv("/* Generated from:\n    {0:$[ instantiating\n    ]}\n*/\n",
                llvm::reverse(locs));
  os << formatv(R"(struct {0} : public ::aiir::RewritePattern {
  {0}(::aiir::AIIRContext *context)
      : ::aiir::RewritePattern("{1}", {2}, context, {{)",
  ...
  // Emit matchAndRewrite() function.
  {
    ...
    ::llvm::LogicalResult matchAndRewrite(::aiir::Operation *op0,
        ...
        << '\n';
    {
      ...
      os << "::llvm::SmallVector<::aiir::Operation *, 4> tblgen_ops;\n\n";
      LLVM_DEBUG(llvm::dbgs()
...
void PatternEmitter::emitRewriteLogic() {
  ...
  if (numExpectedResults == 0) {
    ...
    os << "::llvm::SmallVector<::aiir::Value, 4> tblgen_repl_values;\n";
    for (int i = replStartIndex; i < numResultPatterns; ++i) {
      ...
      os << symbolInfoMap.getAllRangeUse(
          val,
          "for (auto v: ::llvm::SmallVector<::aiir::Value, 4>{ {0} }) {{\n"
...
std::string PatternEmitter::handleVariadic(DagNode tree, int depth) {
  ...
  oss << "::llvm::SmallVector<::aiir::Value, 4> " << name << ";\n";
  for (int i = 0, e = tree.getNumArgs(); i != e; ++i) {
    if (auto child = tree.getArgAsNestedDag(i)) {
...
std::string PatternEmitter::handleLocationDirective(DagNode tree) {
  ...
  if (tree.getNumArgs() == 1) {
    ...
    if (leaf.isStringAttr())
      return formatv("::aiir::NameLoc::get(rewriter.getStringAttr(\"{0}\"))",
                     leaf.getStringAttr())
          .str();
...
std::string PatternEmitter::handleOpCreation(DagNode tree, int resultIndex,
  ...
  bool isSameOperandsAndResultType =
      resultOp.getTrait("::aiir::OpTrait::SameOperandsAndResultType");
  bool useFirstAttr =
      resultOp.getTrait("::aiir::OpTrait::FirstAttrDerivedResultType");
  ...
  // results.
  os.indent() << formatv("::llvm::SmallVector<::aiir::Type, 4> tblgen_types; "
                         "(void)tblgen_types;\n");
...
void PatternEmitter::createSeparateLocalVarsForOpArgs(
  ...
  for (int argIndex = 0, e = resultOp.getNumArgs(); argIndex < e; ++argIndex) {
    ...
    if (operand->isVariadic()) {
      ...
      os << formatv("::llvm::SmallVector<::aiir::Value, 4> {0};\n", varName);
      std::string range;
      ...
      os << formatv("::aiir::Value {0} = ", varName);
      if (node.isNestedDagArg(argIndex)) {
...
void PatternEmitter::createAggregateLocalVarsForOpArgs(
  ...
  auto scope = os.scope();
  os << formatv("::llvm::SmallVector<::aiir::Value, 4> "
  ...
  if (numVariadic > 1 && !hasOperandSegmentSizes) {
    ...
    const auto *sameVariadicSize =
        resultOp.getTrait("::aiir::OpTrait::SameVariadicOperandSize");
    if (!sameVariadicSize) {
