//===- OpInterfacesGen.cpp - AIIR op interface utility generator ----------===//
//
...
using aiir::tblgen::Interface;
using aiir::tblgen::InterfaceMethod;
using aiir::tblgen::OpInterface;
...
void InterfaceGenerator::emitConceptDecl(const Interface &interface) {
  ...
  if (!baseInterfaces.empty()) {
    ...
    // base class concepts.
    os << "\n    void initializeInterfaceConcept(::aiir::detail::InterfaceMap "
          "&interfaceMap) {\n";
...
void InterfaceGenerator::emitInterfaceTraitDecl(const Interface &interface) {
  ...
  os << llvm::formatv("  template <typename {3}>\n"
                      "  struct {0}Trait : public ::aiir::{2}<{0},"
                      " detail::{1}>::Trait<{3}> {{\n",
  ...
  if (auto verify = interface.getVerify()) {
    ...
    os << llvm::formatv(
              "    static ::llvm::LogicalResult {0}(::aiir::Operation *op) ",
...
void InterfaceGenerator::emitInterfaceDecl(const Interface &interface) {
  ...
  os << llvm::formatv("class {0} : public ::aiir::{3}<{1}, detail::{2}> {\n"
                      "public:\n"
                      "  using ::aiir::{3}<{1}, detail::{2}>::{3};\n",
...
static void emitInterfaceDoc(const Record &interfaceDef, raw_ostream &os) {
  ...
  if (auto description = interface.getDescription())
    aiir::tblgen::emitDescription(*description, os);
...
bool InterfaceGenerator::emitInterfaceDocs() {
  os << "<!-- Autogenerated by aiir-tblgen; don't manually edit -->\n";
...
struct InterfaceGenRegistration {
  ...
  aiir::GenRegistration genDecls, genDefs, genDocs;
};
} // namespace
