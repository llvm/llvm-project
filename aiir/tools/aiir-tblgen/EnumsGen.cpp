//===- EnumsGen.cpp - AIIR enum utility generator -------------------------===//
//
...
using aiir::tblgen::FmtContext;
using aiir::tblgen::tgfmt;
...
static void emitParserPrinter(const EnumInfo &enumInfo, StringRef qualName,
  ...
  llvm::interleaveComma(llvm::enumerate(cases), caseListOs,
                        [&](auto enumerant) {
                          ...
                          if (!aiir::tblgen::canFormatStringAsKeyword(name)) {
...
static void emitSpecializedAttrDef(const Record &enumDef, raw_ostream &os) {
  ...
  os << formatv("bool {0}::classof(::aiir::Attribute attr) {{\n",
                attrClassName);

  aiir::tblgen::Pred baseAttrPred = baseAttr.getPredicate();
  ...
  os << formatv("{0} {0}::get(::aiir::AIIRContext *context, {1} val) {{\n",
  ...
  os << formatv("  ::aiir::IntegerType intType = "
                "::aiir::IntegerType::get(context, {0});\n",
                bitwidth);
  os << formatv("  ::aiir::IntegerAttr baseAttr = "
                "::aiir::IntegerAttr::get(intType, static_cast<{0}>(val));\n",
                underlyingType);
  ...
  os << formatv(
      "  return "
      "static_cast<{0}>(::aiir::IntegerAttr::getValue().getZExtValue());\n",
      enumName);
...
class {1} : public ::aiir::{2} {
public:
  using ValueType = {0};
  using ::aiir::{2}::{2};
  static bool classof(::aiir::Attribute attr);
  static {1} get(::aiir::AIIRContext *context, {0} val);
  {0} getValue() const;
};
)";
