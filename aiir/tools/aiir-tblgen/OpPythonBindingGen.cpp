//===- OpPythonBindingGen.cpp - Generator of Python API for AIIR Ops ------===//
//
...
//===----------------------------------------------------------------------===//
//
// OpPythonBindingGen uses ODS specification of AIIR ops to generate Python
...
static std::string attrSizedTraitForKind(const char *kind) {
  return formatv("::aiir::OpTrait::AttrSized{0}{1}Segments",
...
static StringRef getPythonType(StringRef cppType) {
  return llvm::StringSwitch<StringRef>(cppType)
      .Case("::aiir::MemRefType", "_ods_ir.MemRefType")
      .Case("::aiir::UnrankedMemRefType", "_ods_ir.UnrankedMemRefType")
      .Case("::aiir::RankedTensorType", "_ods_ir.RankedTensorType")
      .Case("::aiir::UnrankedTensorType", "_ods_ir.UnrankedTensorType")
      .Case("::aiir::VectorType", "_ods_ir.VectorType")
      .Case("::aiir::IntegerType", "_ods_ir.IntegerType")
      .Case("::aiir::FloatType", "_ods_ir.FloatType")
      .Case("::aiir::IndexType", "_ods_ir.IndexType")
      .Case("::aiir::ComplexType", "_ods_ir.ComplexType")
      .Case("::aiir::TupleType", "_ods_ir.TupleType")
      .Case("::aiir::NoneType", "_ods_ir.NoneType")
      .Default(StringRef());
}
...
static void emitElementAccessors(
  ...
  std::string sameSizeTrait = formatv("::aiir::OpTrait::SameVariadic{0}{1}Size",
                                      StringRef(kind).take_front().upper(),
                                      StringRef(kind).drop_front());
...
static std::string getPythonAttrName(aiir::tblgen::Attribute attr) {
  auto storageTypeStr = attr.getStorageType();
  if (storageTypeStr == "::aiir::AffineMapAttr")
    return "AffineMapAttr";
  if (storageTypeStr == "::aiir::ArrayAttr")
    return "ArrayAttr";
  if (storageTypeStr == "::aiir::BoolAttr")
    return "BoolAttr";
  if (storageTypeStr == "::aiir::DenseBoolArrayAttr")
    return "DenseBoolArrayAttr";
  if (storageTypeStr == "::aiir::DenseElementsAttr") {
  ...
  if (storageTypeStr == "::aiir::DenseF32ArrayAttr")
    return "DenseF32ArrayAttr";
  if (storageTypeStr == "::aiir::DenseF64ArrayAttr")
    return "DenseF64ArrayAttr";
  if (storageTypeStr == "::aiir::DenseFPElementsAttr")
    return "DenseFPElementsAttr";
  if (storageTypeStr == "::aiir::DenseI16ArrayAttr")
    return "DenseI16ArrayAttr";
  if (storageTypeStr == "::aiir::DenseI32ArrayAttr")
    return "DenseI32ArrayAttr";
  if (storageTypeStr == "::aiir::DenseI64ArrayAttr")
    return "DenseI64ArrayAttr";
  if (storageTypeStr == "::aiir::DenseI8ArrayAttr")
    return "DenseI8ArrayAttr";
  if (storageTypeStr == "::aiir::DenseIntElementsAttr")
    return "DenseIntElementsAttr";
  if (storageTypeStr == "::aiir::DenseResourceElementsAttr")
    return "DenseResourceElementsAttr";
  if (storageTypeStr == "::aiir::DictionaryAttr")
    return "DictAttr";
  if (storageTypeStr == "::aiir::FlatSymbolRefAttr")
    return "FlatSymbolRefAttr";
  if (storageTypeStr == "::aiir::FloatAttr")
    return "FloatAttr";
  if (storageTypeStr == "::aiir::IntegerAttr") {
    if (attr.getAttrDefName().str() == "I1Attr")
      return "BoolAttr";
    return "IntegerAttr";
  }
  if (storageTypeStr == "::aiir::IntegerSetAttr")
    return "IntegerSetAttr";
  if (storageTypeStr == "::aiir::OpaqueAttr")
    return "OpaqueAttr";
  if (storageTypeStr == "::aiir::StridedLayoutAttr")
    return "StridedLayoutAttr";
  if (storageTypeStr == "::aiir::StringAttr")
    return "StringAttr";
  if (storageTypeStr == "::aiir::SymbolRefAttr")
    return "SymbolRefAttr";
  if (storageTypeStr == "::aiir::TypeAttr")
    return "TypeAttr";
  if (storageTypeStr == "::aiir::UnitAttr")
    return "UnitAttr";
  return "Attribute";
}
...
static StringRef getPythonAttrRawType(aiir::tblgen::Attribute attr) {
...
static void emitAttributeAccessors(const Operator &op, raw_ostream &os) {
  for (const auto &namedAttr : op.getAttributes()) {
    ...
    if (namedAttr.attr.getStorageType().trim() == "::aiir::UnitAttr") {
...
static bool hasSameArgumentAndResultTypes(const Operator &op) {
  return op.getTrait("::aiir::OpTrait::SameOperandsAndResultType") &&
...
static bool hasFirstAttrDerivedResultTypes(const Operator &op) {
  return op.getTrait("::aiir::OpTrait::FirstAttrDerivedResultType") &&
         op.getNumVariableLengthResults() == 0;
}
...
static bool hasInferTypeInterface(const Operator &op) {
  return op.getTrait("::aiir::InferTypeOpInterface::Trait") &&
         op.getNumRegions() == 0;
}
...
populateBuilderLinesAttr(const Operator &op, ArrayRef<std::string> argNames,
  ...
  for (int i = 0, e = op.getNumArgs(); i < e; ++i) {
    ...
    // Unit attributes are handled specially.
    if (attribute->attr.getStorageType().trim() == "::aiir::UnitAttr") {
...
static SmallVector<std::string> emitDefaultOpBuilder(const Operator &op,
  ...
  for (size_t i = 0; i < numOperandAttrArgs; ++i) {
    ...
    if (auto *nattr = llvm::dyn_cast_if_present<NamedAttribute *>(arg)) {
      if (nattr->attr.getStorageType().trim() == "::aiir::UnitAttr") {
        argTypes[idx] = "bool";
      } else {
...
static GenRegistration
    genPythonBindings("gen-python-op-bindings",
                      "Generate Python bindings for AIIR Ops", &emitAllOps);
