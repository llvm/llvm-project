//===- OpFormatGen.cpp - AIIR operation asm format generator --------------===//
//
...
#define DEBUG_TYPE "aiir-tblgen-opformatgen"
...
struct OperationFormat {
  ...
  OperationFormat(const Operator &op, bool hasProperties)
    ...
    hasSingleBlockTrait = op.getTrait("::aiir::OpTrait::SingleBlock");
  }
...
const char *const optionalAttrParserCode = R"(
  ::aiir::OptionalParseResult parseResult{0}Attr =
...
const char *const enumAttrParserCode = R"(
  {
    ...
    ::aiir::NamedAttrList attrStorage;
    auto loc = parser.getCurrentLocation();
    if (parser.parseOptionalKeyword(&attrStr, {4})) {
      ::aiir::StringAttr attrVal;
      ::aiir::OptionalParseResult parseResult =
...
const char *const propertyParserCode = R"(
  ...
  auto {0}PropParseResult = [&](auto& propStorage) -> ::aiir::ParseResult {{
    {2}
    return ::aiir::success();
  }({1}.{0});
...
const char *const optionalPropertyParserCode = R"(
  auto {0}PropParseResult = [&](auto& propStorage) -> ::aiir::OptionalParseResult {{
    {2}
    return ::aiir::success();
  }({1}.{0});
...
const char *const optionalOperandParserCode = R"(
  {
    {0}OperandsLoc = parser.getCurrentLocation();
    ::aiir::OpAsmParser::UnresolvedOperand operand;
    ::aiir::OptionalParseResult parseResult =
...
const char *const functionalTypeParserCode = R"(
  ::aiir::FunctionType {0}__{1}_functionType;
...
const char *const inferReturnTypesParserCode = R"(
  ::llvm::SmallVector<::aiir::Type> inferredReturnTypes;
  if (::aiir::failed({0}::inferReturnTypes(parser.getContext(),
...
static const char *regionListParserCode = R"(
  {
    std::unique_ptr<::aiir::Region> region;
    ...
    if (firstRegionResult.has_value()) {
      ...
      while (succeeded(parser.parseOptionalComma())) {
        region = std::make_unique<::aiir::Region>();
...
static void genElementParserStorage(FormatElement *element, const Operator &op,
  ...
  if (auto *optional = dyn_cast<OptionalElement>(element)) {
    ...
    body << "  ::llvm::SmallVector<::aiir::OpAsmParser::UnresolvedOperand, 4> "
            "allOperands;\n";

  } else if (isa<RegionsDirective>(element)) {
    body << "  ::llvm::SmallVector<std::unique_ptr<::aiir::Region>, 2> "
            "fullRegions;\n";

  } else if (isa<SuccessorsDirective>(element)) {
    body << "  ::llvm::SmallVector<::aiir::Block *, 2> fullSuccessors;\n";
    ...
    if (operand->getVar()->isVariableLength()) {
      body
          << "  ::llvm::SmallVector<::aiir::OpAsmParser::UnresolvedOperand, 4> "
          << name << "Operands;\n";
      ...
      body << "  ::aiir::OpAsmParser::UnresolvedOperand " << name
           << "RawOperand{};\n"
           << "  ::llvm::ArrayRef<::aiir::OpAsmParser::UnresolvedOperand> "
    ...
    if (region->getVar()->isVariadic()) {
      body << formatv(
          "  ::llvm::SmallVector<std::unique_ptr<::aiir::Region>, 2> "
          "{0}Regions;\n",
          name);
    } else {
      body << formatv("  std::unique_ptr<::aiir::Region> {0}Region = "
                      "std::make_unique<::aiir::Region>();\n",
                      name);
    }
    ...
    if (successor->getVar()->isVariadic()) {
      body << formatv("  ::llvm::SmallVector<::aiir::Block *, 2> "
                      "{0}Successors;\n",
                      name);
    } else {
      body << formatv("  ::aiir::Block *{0}Successor = nullptr;\n", name);
    }
    ...
    if (lengthKind != ArgumentLengthKind::Single)
      body << "  ::llvm::SmallVector<::aiir::Type, 1> " << name << "Types;\n";
    else
      body
          << formatv("  ::aiir::Type {0}RawType{{};\n", name)
          << formatv(
                 "  ::llvm::ArrayRef<::aiir::Type> {0}Types(&{0}RawType, 1);\n",
                 name);
  } else if (auto *dir = dyn_cast<FunctionalTypeDirective>(element)) {
    ArgumentLengthKind ignored;
    body << "  ::llvm::ArrayRef<::aiir::Type> "
...
static void genCustomDirectiveParser(CustomDirective *dir, MethodBody &body,
  ...
  for (FormatElement *param : dir->getElements()) {
    if (auto *operand = dyn_cast<OperandVariable>(param)) {
      ...
      if (var->isOptional()) {
        body << formatv(
            "    ::std::optional<::aiir::OpAsmParser::UnresolvedOperand> "
            "{0}Operand;\n",
            var->name);
      } else if (var->isVariadicOfVariadic()) {
        body << formatv("    "
                        "::llvm::SmallVector<::llvm::SmallVector<::aiir::"
      ...
      if (lengthKind == ArgumentLengthKind::Optional) {
        body << formatv("    ::aiir::Type {0}Type;\n", listName);
      } else if (lengthKind == ArgumentLengthKind::VariadicOfVariadic) {
        body << formatv(
            "    ::llvm::SmallVector<llvm::SmallVector<::aiir::Type>> "
            "{0}TypeGroups;\n",
            listName);
      }
    } else if (auto *dir = dyn_cast<RefDirective>(param)) {
      ...
      if (auto *operand = dyn_cast<OperandVariable>(input)) {
        ...
        if (lengthKind == ArgumentLengthKind::Optional) {
          body << formatv("    ::aiir::Type {0}Type = {0}Types.empty() ? "
                          "::aiir::Type() : {0}Types[0];\n",
                          listName);
        }
      }
  ...
  if (isOptional) {
    body << "    if (!odsResult.has_value()) return {};\n"
         << "    if (::aiir::failed(*odsResult)) return ::aiir::failure();\n";
  } else {
    body << "    if (odsResult) return ::aiir::failure();\n";
  }
...
static void genParsedAttrPropertiesSetter(OperationFormat &fmt, Operator &op,
  ...
  paramList.emplace_back("::aiir::Attribute", "attr");
  paramList.emplace_back("::llvm::function_ref<::aiir::InFlightDiagnostic()>",
                         "emitError");
  ...
  body << R"decl(
::aiir::DictionaryAttr dict = ::llvm::dyn_cast<::aiir::DictionaryAttr>(attr);
if (!dict) {
...
::aiir::DenseSet<::aiir::StringAttr> usedKeys;
::aiir::AIIRContext *ctx = dict.getContext();
(void)ctx;
)decl";
...
auto setFromAttr = [] (auto &propStorage, ::aiir::Attribute propAttr,
         ::llvm::function_ref<::aiir::InFlightDiagnostic()> emitError) -> ::aiir::LogicalResult {{
  {0};
};
auto {1}AttrName = ::aiir::StringAttr::get(ctx, "{1}");
...
if (attr && ::aiir::failed(setFromAttr(prop.{1}, attr, emitError)))
  return ::aiir::failure();
)decl";
...
auto {0}AttrName = ::aiir::StringAttr::get(ctx, "{0}");
...
if (attr || /*isRequired=*/{1}) {{
  ...
  body << R"decl(
for (::aiir::NamedAttribute attr : dict) {
...
void OperationFormat::genParser(Operator &op, OpClass &opClass) {
  SmallVector<MethodParameter> paramList;
  paramList.emplace_back("::aiir::OpAsmParser &", "parser");
  paramList.emplace_back("::aiir::OperationState &", "result");

  auto *method = opClass.addStaticMethod("::aiir::ParseResult", "parse",
                                         std::move(paramList));
...
void OperationFormat::genElementParser(FormatElement *element, MethodBody &body,
  ...
  if (auto *optional = dyn_cast<OptionalElement>(element)) {
    ...
    if (auto *attrVar = dyn_cast<AttributeVariable>(firstElement)) {
      ...
      body << "  if (::aiir::succeeded(parser.parseOptional";
      ...
      body << "  if (auto optResult = [&]() -> ::aiir::OptionalParseResult {\n";
      genCustomDirectiveParser(custom, body, useProperties, opCppClassName,
                               /*isOptional=*/true);
      body << "    return ::aiir::success();\n"
           << "  }(); optResult.has_value() && ::aiir::failed(*optResult)) {\n"
           << "    return ::aiir::failure();\n"
    ...
    if (lengthKind == ArgumentLengthKind::VariadicOfVariadic) {
      ...
      TypeSwitch<FormatElement *>(dir->getArg())
          ...
          .Default([&](auto operand) {
            body << formatv(false, parserCode, "::aiir::Type", listName);
          });
    }
  } else if (auto *dir = dyn_cast<FunctionalTypeDirective>(element)) {
    ArgumentLengthKind ignored;
...
void OperationFormat::genParserTypeResolution(Operator &op, MethodBody &body) {
  ...
  for (TypeResolution &resolver :
    ...
    body << "  for (::aiir::Type type : " << variable->name << "Types) {\n"
         << "    (void)type;\n"
         << "    if (!("
  ...
  if (!buildableTypes.empty()) {
    ...
    for (auto &it : buildableTypes)
      body << "  ::aiir::Type odsBuildableType" << it.second << " = "
...
void OperationFormat::genParserOperandTypeResolution(
  ...
  if (allOperandTypes) {
    ...
    if (op.getNumOperands() > 1) {
      body << "::llvm::concat<const ::aiir::OpAsmParser::UnresolvedOperand>(";
  ...
  if (allOperands) {
    ...
    if (op.getNumOperands() > 1) {
      body << "::llvm::concat<const ::aiir::Type>(";
...
void OperationFormat::genParserVariadicSegmentResolution(Operator &op,
  ...
  if (!allOperands) {
    if (op.getTrait("::aiir::OpTrait::AttrSizedOperandSegments")) {
  ...
  if (!allResultTypes &&
      op.getTrait("::aiir::OpTrait::AttrSizedResultSegments")) {
...
static void genNonDefaultValueCheck(MethodBody &body, const Operator &op,
  ...
  if (attr.hasDefaultValue()) {
    FmtContext fctx;
    fctx.withBuilder("::aiir::OpBuilder((*this)->getContext())");
    body << getter << "Attr() != "
...
static void genVariadicSegmentElision(OperationFormat &fmt, Operator &op,
  ...
  if (!fmt.allOperands &&
      op.getTrait("::aiir::OpTrait::AttrSizedOperandSegments"))
    ...
    if (!attr.isDerivedAttr() && attr.hasDefaultValue()) {
      ...
      std::string defaultValue =
          std::string(tgfmt(attr.getConstBuilderTemplate(), &fctx,
                            tgfmt(attr.getDefaultValue(), &fctx)));
      body << "  {\n";
      body << "     ::aiir::Builder odsBuilder(getContext());\n";
      body << "     ::aiir::Attribute attr = " << op.getGetterName(name)
           << "Attr();\n";
...
static void genCustomDirectiveParameterPrinter(FormatElement *element,
  ...
  if (auto *attr = dyn_cast<AttributeVariable>(element)) {
    ...
    if (var->isVariadic())
      ...
      body << formatv("({0}() ? {0}().getType() : ::aiir::Type())", name);
    else
      body << name << "().getType()";

  } else if (auto *string = dyn_cast<StringElement>(element)) {
    FmtContext ctx;
    ctx.withBuilder("::aiir::Builder(getContext())");
...
static void genVariadicRegionPrinter(const Twine &regionListName,
  ...
  body << "    llvm::interleaveComma(" << regionListName
       << ", _odsPrinter, [&](::aiir::Region &region) {\n      ";
...
static MethodBody &genTypeOperandPrinter(FormatElement *arg, const Operator &op,
  ...
  if (var->isOptional())
    return body << formatv(
               "({0}() ? ::llvm::ArrayRef<::aiir::Type>({0}().getType()) : "
               "::llvm::ArrayRef<::aiir::Type>())",
               op.getGetterName(var->name));
  if (useArrayRef)
    return body << "::llvm::ArrayRef<::aiir::Type>("
    ...
    if (auto *operand = dyn_cast<OperandVariable>(dir->getArg())) {
      if (operand->getVar()->isVariadicOfVariadic()) {
        body << formatv(
            ...
            "[&](::aiir::TypeRange types) {{ _odsPrinter << \"(\" << "
            "types << \")\"; });\n",
            op.getGetterName(operand->getVar()->name));
        return;
      }
...
void OperationFormat::genPrinter(Operator &op, OpClass &opClass) {
  auto *method = opClass.addMethod(
      "void", "print",
      MethodParameter("::aiir::OpAsmPrinter &", "_odsPrinter"));
  auto &body = method->body();
    ...
    if (def.isSubClassOf("AllTypesMatch")) {
      ...
      if (auto name = def.getValueAsOptionalString("cppInterfaceName")) {
        if (*name == "InferTypeOpInterface" &&
            def.getValueAsString("cppNamespace") == "::aiir")
  ...
  auto isBase = [](FormatElement *el) {
    ...
    return !attr->getTypeBuilder() &&
           (attr->shouldBeQualified() ||
            attr->getVar()->attr.getStorageType() == "::aiir::Attribute");
  };
...
void OpFormatParser::handleShapedTypeMatchesElementCountAndTypesConstraint(
  ...
  if (ConstArgument arg = findSeenArg(shapedArg)) {
    variableTyResolver[elementsArg] = {
        arg, "::llvm::SmallVector<::aiir::Type>(::llvm::cast<::aiir::"
             "ShapedType>($_self).getNumElements(), "
             "::llvm::cast<::aiir::ShapedType>($_self).getElementType())"};
  }
...
void aiir::tblgen::generateOpFormat(const Operator &constOp, OpClass &opClass,
                                    bool hasProperties) {
