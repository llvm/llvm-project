...
void Generator::emitParseDispatch(StringRef kind,
  ...
  aiir::raw_indented_ostream os(output);
  char const *head =
      R"(static {0} read{0}(AIIRContext* context, DialectBytecodeReader &reader))";
...
void Generator::emitParse(StringRef kind, const Record &x) {
  ...
  char const *head =
      R"(static {0} read{1}(AIIRContext* context, DialectBytecodeReader &reader) )";
  aiir::raw_indented_ostream os(output);
  std::string returnType = getCType(&x);
  os << formatv(head,
                kind == "attribute" ? "::aiir::Attribute" : "::aiir::Type",
                x.getName());
...
static void printParseConditional(aiir::raw_indented_ostream &ios,
                                  ArrayRef<const Init *> args,
