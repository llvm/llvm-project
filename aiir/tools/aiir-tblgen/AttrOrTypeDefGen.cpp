//===- AttrOrTypeDefGen.cpp - AIIR AttrOrType definitions generator -------===//
//
...
#define DEBUG_TYPE "aiir-tblgen-attrortypedefgen"
...
void DefGen::createParentWithTraits() {
  ParentClass defParent(strfmt("::aiir::{0}::{1}Base", valueType, defType));
  ...
  defParent.addTemplateParam(storageCls
                                 ...
                                 : strfmt("::aiir::{0}Storage", valueType));
  ...
  std::string opAsmInterfaceTraitName =
      strfmt("::aiir::OpAsm{0}Interface::Trait", defType);
  if (def.genMnemonicAlias() &&
...
void DefGen::emitVerifierDecl() {
  defCls.declareStaticMethod(
      ...
      getBuilderParams({{"::llvm::function_ref<::aiir::InFlightDiagnostic()>",
                         "emitError"}}));
}
...
void DefGen::emitInvariantsVerifierImpl() {
  ...
  verifier->body() << "return ::aiir::success();";
}
...
void DefGen::emitInvariantsVerifier(bool hasImpl, bool hasCustomVerifier) {
  ...
  auto emitVerifierCall = [&](StringRef name) {
    verifier->body() << strfmt("if (::aiir::failed({0}(", name);
    llvm::interleaveComma(
        llvm::map_range(builderParams,
                        [](auto &param) { return param.getName(); }),
        verifier->body());
    verifier->body() << ")))\n";
    verifier->body() << "  return ::aiir::failure();\n";
  };
...
void DefGen::emitParserPrinter() {
  ...
  parserParams.emplace_back("::aiir::AsmParser &", "odsParser");
  if (isa<AttrDef>(&def))
    parserParams.emplace_back("::aiir::Type", "odsType");
  auto *parser = defCls.addMethod(strfmt("::aiir::{0}", valueType), "parse",
  ...
  Method *printer =
      defCls.addMethod("void", "print", props,
                       MethodParameter("::aiir::AsmPrinter &", "odsPrinter"));
...
void DefGen::emitDefaultBuilder() {
  Method *m = defCls.addStaticMethod(
      ...
      getBuilderParams({{"::aiir::AIIRContext *", "context"}}));
...
getCustomBuilderParams(std::initializer_list<MethodParameter> prefix,
  ...
  if (!builder.hasInferredContextParameter())
    builderParams.emplace_back("::aiir::AIIRContext *", "context");
  for (auto &param : params) {
    builderParams.emplace_back(param.getCppType(), *param.getName(),
...
void DefGen::emitMnemonicAliasMethod() {
  ...
  Method *m = defCls.addMethod<Method::Const>("::aiir::OpAsmAliasResult",
...
void DefGen::emitConstruct() {
  Method *construct = storageCls->addMethod(
      ...
      def.hasStorageCustomConstructor() ? Method::StaticDeclaration
                                        : Method::StaticInline,
      MethodParameter(strfmt("::aiir::{0}StorageAllocator &", valueType),
                      "allocator"),
...
void DefGen::emitStorageClass() {
  ...
  storageCls->addParent(strfmt("::aiir::{0}Storage", valueType));
  // Add the constructor.
...
bool DefGenerator::emitDecls(StringRef selectedDialect) {
  ...
  for (const AttrOrTypeDef &def : defs)
    if (!def.getDialect().getCppNamespace().empty())
      os << "AIIR_DECLARE_EXPLICIT_TYPE_ID("
...
::aiir::Attribute {0}::parseAttribute(::aiir::DialectAsmParser &parser,
...
void {0}::printAttribute(::aiir::Attribute attr,
                         ::aiir::DialectAsmPrinter &printer) const {{
  if (::aiir::succeeded(generatedAttributePrinter(attr, printer)))
    return;
  {2}
}
)";
...
static const char *const dialectDynamicAttrPrinterDispatch = R"(
  if (::aiir::succeeded(printIfDynamicAttr(attr, printer)))
    return;
)";
...
::aiir::Type {0}::parseType(::aiir::DialectAsmParser &parser) const {{
  ::llvm::SMLoc typeLoc = parser.getCurrentLocation();
...
void {0}::printType(::aiir::Type type,
                    ::aiir::DialectAsmPrinter &printer) const {{
  if (::aiir::succeeded(generatedTypePrinter(type, printer)))
    return;
  {2}
}
)";
...
static const char *const dialectDynamicTypePrinterDispatch = R"(
  if (::aiir::succeeded(printIfDynamicType(type, printer)))
    return;
)";
...
void DefGenerator::emitParsePrintDispatch(ArrayRef<AttrOrTypeDef> defs) {
  ...
  // Declare the parser.
  SmallVector<MethodParameter> params = {{"::aiir::AsmParser &", "parser"},
                                         {"::llvm::StringRef *", "mnemonic"}};
  if (isAttrGenerator)
    params.emplace_back("::aiir::Type", "type");
  params.emplace_back(strfmt("::aiir::{0} &", valueType), "value");
  Method parse("::aiir::OptionalParseResult",
  ...
  // printer.
  printer.body() << "  return ::llvm::TypeSwitch<::aiir::" << valueType
  ...
  parse.body() << "    .Default([&](llvm::StringRef keyword, llvm::SMLoc) {\n"
                  ...
                  "      return std::nullopt;\n"
                  "    });";
  printer.body() << "    .Default([](auto) { return ::aiir::failure(); });";
...
bool DefGenerator::emitDefs(StringRef selectedDialect) {
  ...
  for (const AttrOrTypeDef &def : defs) {
    ...
    if (!def.getDialect().getCppNamespace().empty())
      os << "AIIR_DEFINE_EXPLICIT_TYPE_ID("
         << def.getDialect().getCppNamespace() << "::" << def.getCppClassName()
         << ")\n";
  }
...
static void emitTypeConstraintDecls(const RecordKeeper &records,
                                    raw_ostream &os) {
  emitConstraintDecls(getAllCppTypeConstraints(records), os, "::aiir::Type",
                      "type");
}
...
static void emitTypeConstraintDefs(const RecordKeeper &records,
                                   raw_ostream &os) {
  emitConstraintDefs(getAllCppTypeConstraints(records), os, "::aiir::Type",
                     "type");
}

static void emitAttrConstraintDefs(const RecordKeeper &records,
                                   raw_ostream &os) {
  emitConstraintDefs(getAllCppAttrConstraints(records), os, "::aiir::Attribute",
                     "attr");
}
