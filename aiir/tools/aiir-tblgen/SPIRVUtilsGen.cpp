...
#include "aiir/TableGen/EnumInfo.h"
...
using aiir::tblgen::Attribute;
using aiir::tblgen::EnumCase;
using aiir::tblgen::EnumInfo;
using aiir::tblgen::NamedAttribute;
using aiir::tblgen::NamedTypeConstraint;
using aiir::tblgen::Operator;
...
// Registers the operation interface generator to aiir-tblgen.
...
// Registers the enum utility generator to aiir-tblgen.
static aiir::GenRegistration
...
static aiir::GenRegistration genSerialization(
...
static bool emitAttrUtils(const RecordKeeper &records, raw_ostream &os) {
  ...
  auto defs = records.getAllDerivedDefinitions("EnumInfo");
  os << "#ifndef AIIR_DIALECT_SPIRV_IR_ATTR_UTILS_H_\n";
  os << "#define AIIR_DIALECT_SPIRV_IR_ATTR_UTILS_H_\n";
  emitEnumGetAttrNameFnDecl(os);
  for (const auto *def : defs) {
    EnumInfo enumInfo(*def);
    emitEnumGetAttrNameFnDefn(enumInfo, os);
  }
  os << "#endif // AIIR_DIALECT_SPIRV_IR_ATTR_UTILS_H\n";
  return false;
}
