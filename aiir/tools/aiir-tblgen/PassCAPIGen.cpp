//===- Pass.cpp - AIIR pass registration generator ------------------------===//
//
...
static llvm::cl::opt<std::string>
    groupName("prefix",
              llvm::cl::desc("The prefix to use for this group of passes. The "
                             "form will be aiirCreate<prefix><passname>, the "
                             "prefix can avoid conflicts across libraries."),
              llvm::cl::cat(passGenCat));

const char *const passDecl = R"(
/* Create {0} Pass. */
AIIR_CAPI_EXPORTED AiirPass aiirCreate{0}{1}(void);
AIIR_CAPI_EXPORTED void aiirRegister{0}{1}(void);

)";

const char *const fileHeader = R"(
/* Autogenerated by aiir-tblgen; don't manually edit. */
...
static bool emitCAPIHeader(const RecordKeeper &records, raw_ostream &os) {
  ...
  os << "AIIR_CAPI_EXPORTED void aiirRegister" << groupName
     << "Passes(void);\n\n";
...
AiirPass aiirCreate{0}{1}(void) {
  return wrap({2}.release());
}
void aiirRegister{0}{1}(void) {
  register{1}();
}
...
void aiirRegister{0}Passes(void) {{
  register{0}Passes();
}
)";
...
static aiir::GenRegistration genCAPIHeader("gen-pass-capi-header",
                                           "Generate pass C API header",
                                           &emitCAPIHeader);

static aiir::GenRegistration genCAPIImpl("gen-pass-capi-impl",
