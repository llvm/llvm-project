//===- OpDocGen.cpp - AIIR operation documentation generator --------------===//
//
...
void aiir::tblgen::emitSummary(StringRef summary, raw_ostream &os) {
  if (summary.empty())
    return;
...
void aiir::tblgen::emitDescription(StringRef description, raw_ostream &os) {
...
void aiir::tblgen::emitDescriptionComment(StringRef description,
...
static void emitOpDoc(const Operator &op, raw_ostream &os) {
  ...
  if (op.hasDescription())
    aiir::tblgen::emitDescription(op.getDescription(), os);
  ...
  if (op.getNumAttributes() != 0) {
    ...
    os << "<table>\n";
    // Header.
    os << "<tr><th>Attribute</th><th>AIIR Type</th><th>Description</th></tr>\n";
...
static void emitSourceLink(StringRef inputFilename, raw_ostream &os) {
  size_t pathBegin = inputFilename.find("aiir/include/aiir/");
  if (pathBegin == StringRef::npos)
    return;

  StringRef inputFromAiirInclude = inputFilename.substr(pathBegin);

  os << "\n[source](https://github.com/llvm/llvm-project/blob/main/"
     << inputFromAiirInclude << ")\n";
}
...
static void emitAttrOrTypeDefDoc(const AttrOrTypeDef &def, raw_ostream &os) {
  ...
  if (def.hasDescription()) {
    aiir::tblgen::emitDescription(def.getDescription(), os);
  }
...
/// `aiir::tblgen::findDialectToGenerate`.
