...
public:
  ...
  {0}Base() : {1}(::aiir::TypeID::get<DerivedT>()) {{}
  ...
  static bool classof(const ::aiir::Pass *pass) {{
    return pass->getTypeID() == ::aiir::TypeID::get<DerivedT>();
  }

  /// A clone method to create a copy of this pass.
  std::unique_ptr<::aiir::Pass> clonePass() const override {{
  ...
  /// library.
  AIIR_DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID({0}Base<DerivedT>)
...
static void emitPassStatisticDecls(const Pass &pass, raw_ostream &os) {
  for (const PassStatistic &stat : pass.getStatistics()) {
    os << formatv(
        "  ::aiir::Pass::Statistic {0}{{this, \"{1}\", R\"PS({2})PS\"};\n",
