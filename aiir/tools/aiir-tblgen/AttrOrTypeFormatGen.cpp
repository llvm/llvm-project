//===- AttrOrTypeFormatGen.cpp - AIIR attribute and type format generator -===//
//
...
#include "aiir/TableGen/AttrOrTypeDef.h"
...
static const char *const defaultParameterParser =
    "::aiir::FieldParser<$0>::parse($_parser)";
...
void DefFormat::genParser(MethodBody &os) {
  ...
  os.indent();
  os << "::aiir::Builder odsBuilder(odsParser.getContext());\n";
  ...
  for (const AttrOrTypeParameter &param : params) {
    os << formatv("::aiir::FailureOr<{0}> _result_{1};\n",
  ...
  for (const AttrOrTypeParameter &param : params) {
    if (param.isOptional())
      continue;
    os << formatv("assert(::aiir::succeeded(_result_{0}));\n", param.getName());
  }
  ...
  if (auto *defInit = dyn_cast<llvm::DefInit>(param.getDef())) {
    ...
    if (dialectValue) {
      if (auto *dialectInit =
        ...
        if (name != "BuiltinDialect" || cppNamespace != "::aiir") {
...
void DefFormat::genParamsParser(ParamsDirective *el, FmtContext &ctx,
  ...
  auto betweenFn = [&](IteratorT it) {
    ...
    if (el->isOptional()) {
      os << formatv("if (::aiir::succeeded(_result_{0}) && !({1})) {{\n",
                    el->getName(),
...
void DefFormat::genCustomParser(CustomDirective *el, FmtContext &ctx,
  ...
  for (FormatElement *arg : el->getElements()) {
    ...
    if (auto *param = dyn_cast<ParameterElement>(arg))
      os << "::aiir::detail::unwrapForCustomParse(_result_" << param->getName()
         << ")";
  ...
  if (isOptional) {
    ...
    os << "if (::aiir::failed(*odsCustomResult)) return ::aiir::failure();\n";
  } else {
    os << "if (::aiir::failed(odsCustomResult)) return {};\n";
  }
  for (FormatElement *arg : el->getElements()) {
    if (auto *param = dyn_cast<ParameterElement>(arg)) {
      if (param->isOptional())
        continue;
      os << formatv("if (::aiir::failed(_result_{0})) {{\n", param->getName());
      os.indent() << tgfmt("$_parser.emitError(odsCustomLoc, ", &ctx)
                  << "\"custom parser failed to parse parameter '"
                  << param->getName() << "'\");\n";
      os << "return " << (isOptional ? "::aiir::failure()" : "{}") << ";\n";
      os.unindent() << "}\n";
    }
  }
...
void DefFormat::genOptionalGroupParser(OptionalElement *el, FmtContext &ctx,
  ...
  const auto guardOn = [&](auto params) {
    ...
    llvm::interleave(
        params, os,
        [&](ParameterElement *el) {
          os << formatv("(::aiir::succeeded(_result_{0}) && *_result_{0})",
                        el->getName());
        },
        " || ");
    os << ")) {\n";
  ...
  if (auto *literal = dyn_cast<LiteralElement>(first)) {
    ...
    os << "if (auto result = [&]() -> ::aiir::OptionalParseResult {\n";
    os.indent();
    genCustomParser(custom, ctx, os, /*isOptional=*/true);
    os << "return ::aiir::success();\n";
    os.unindent();
    os << "}(); result.has_value() && ::aiir::failed(*result)) {\n";
    os.indent();
    os << "return {};\n";
    os.unindent();
...
void DefFormat::genPrinter(MethodBody &os) {
  ...
  ctx.withBuilder("odsBuilder");
  os.indent();
  os << "::aiir::Builder odsBuilder(getContext());\n";
...
void aiir::tblgen::generateAttrOrTypeFormat(const AttrOrTypeDef &def,
                                            MethodBody &parser,
                                            MethodBody &printer) {
  llvm::SourceMgr mgr;
