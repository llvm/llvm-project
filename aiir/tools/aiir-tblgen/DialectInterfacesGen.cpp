//===- DialectInterfacesGen.cpp - AIIR dialect interface utility generator ===//
//
...
using aiir::tblgen::DialectInterface;
...
static void emitConstructor(const DialectInterface &interface,
  ...
  ios.indent(2);
  ios << llvm::formatv("{0}(::aiir::Dialect *dialect) : Base(dialect) {{}\n",
                       interface.getName());
}
...
void DialectInterfaceGenerator::emitInterfaceDecl(
  ...
  os << llvm::formatv(
      "class {0} : public ::aiir::DialectInterface::Base<{0}> {{\n"
      "public:\n",
      interface.getName());
...
static aiir::GenRegistration genDecls(
