//===- LLVMIRConversionGen.cpp - AIIR LLVM IR builder generator -----------===//
//
...
#include "aiir/TableGen/Argument.h"
...
// AIIR and LLVM IR operand order may differ and only for the latter the
...
using ConditionFn = aiir::function_ref<llvm::Twine(const Record &record)>;

// Emit a conditional call to the AIIR builder of the LLVM dialect operation to
...
static LogicalResult emitOneAIIRBuilder(const Record &record, raw_ostream &os,
                                        ConditionFn conditionFn) {
  auto op = tblgen::Operator(record);

  if (!record.getValue("aiirBuilder"))
    return emitError(record, "expected 'aiirBuilder' field");

  // Return early if there is no builder specified.
  StringRef builderStrRef = record.getValueAsString("aiirBuilder");
  if (builderStrRef.empty())
    return success();
  ...
  // operands used by the builder to AIIR values and returns failure if one of
...
// Emit all intrinsic AIIR builders. Returns false on success because of the
// generator registration requirements.
static bool emitIntrAIIRBuilders(const RecordKeeper &records, raw_ostream &os) {
  ...
  for (const Record *def :
    ...
    if (failed(emitOneAIIRBuilder(*def, os, emitIntrCond)))
      return true;
  }
  return false;
}
...
// generator registration requirements.
static bool emitOpAIIRBuilders(const RecordKeeper &records, raw_ostream &os) {
  ...
  for (const Record *def : records.getAllDerivedDefinitions("LLVM_OpBase")) {
    if (failed(emitOneAIIRBuilder(*def, os, emitOpcodeCond)))
      return true;
  }
  return false;
}
...
// switch-based logic to convert from the AIIR LLVM dialect enum attribute case
...
// containing switch-based logic to convert from the LLVM API enumerant to AIIR
...
// to AIIR LLVM dialect enum attribute (Enum).
...
// Emits conversion functions between AIIR enum attribute case and corresponding
...
// matching AIIR LLVM dialect intrinsic operation.
...
static aiir::GenRegistration genOpFromLLVMIRConversions(
    "gen-op-from-llvmir-conversions",
    "Generate conversions of operations from LLVM IR", emitOpAIIRBuilders);

static aiir::GenRegistration genIntrFromLLVMIRConversions(
    "gen-intr-from-llvmir-conversions",
    "Generate conversions of intrinsics from LLVM IR", emitIntrAIIRBuilders);
...
static aiir::GenRegistration genConvertibleLLVMIRIntrinsics(
