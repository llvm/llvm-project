//===- aiir-linalg-ods-yaml-gen.cpp - Linalg ODS generation from yaml  ----===//
//
...
// in AIIR) is deemed to have systemic value.
//
//===----------------------------------------------------------------------===//
...
struct ScalarTraits<SerializedAffineMap> {
  ...
  static StringRef input(StringRef scalar, void *rawYamlContext,
    ...
    if (auto attr = dyn_cast_or_null<AffineMapAttr>(
            aiir::parseAttribute(scalar, yamlContext->aiirContext)))
...
public:
  GenerationContext(AIIRContext *context, raw_ostream *odsOut,
                    raw_ostream *defnOut)
...
static std::string generateCppExpression(SerializedAffineMap self,
  ...
  static const char exprFormat[] =
      R"FMT(llvm::cast<AffineMapAttr>(aiir::parseAttribute("{0}", {1})).getValue())FMT";
...
def {0} : LinalgStructuredBase_Op<"{1}", !listconcat([AttrSizedOperandSegments],
  /*extraInterfaces=*/[{2}])> {
    ...
    let extraClassDeclaration = structuredOpsBaseDecls # [{{
      ...
      ::aiir::MutableOperandRange getDpsInitsMutable() {{
...
static SmallVector<AffineExpr> getSymbolBindings({0} self) {
  AIIRContext *context = self.getContext();
...
int main(int argc, char **argv) {
  ...
  std::unique_ptr<llvm::MemoryBuffer> file =
      aiir::openInputFile(inputFilename, &errorMessage);
  if (!file) {
  ...
  AIIRContext aiirContext;
  LinalgYAMLContext yamlContext{&aiirContext};
  ...
  // Generate.
  GenerationContext genContext(&aiirContext,
  ...
  for (auto &opConfig : opConfigs) {
    ...
    genContext.setLoc(NameLoc::get(
        StringAttr::get(&aiirContext, opConfig.metadata->cppClassName)));
