//===- aiir-pdll.cpp - AIIR PDLL frontend -----------------------*- C++ -*-===//
//
...
processBuffer(raw_ostream &os, std::unique_ptr<llvm::MemoryBuffer> chunkBuffer,
  ...
  OwningOpRef<ModuleOp> pdlModule =
      codegenPDLLToAIIR(&aiirContext, astContext, sourceMgr, **module);
  if (!pdlModule)
    return failure();

  if (outputType == OutputType::AIIR) {
...
int main(int argc, char **argv) {
  ...
  llvm::cl::opt<enum OutputType> outputType(
      ...
      llvm::cl::values(clEnumValN(OutputType::AST, "ast",
                       ...
                       clEnumValN(OutputType::AIIR, "aiir",
                                  "generate the PDL AIIR for the input file"),
  ...
  llvm::cl::list<std::string> macroNames(
      "D",
      llvm::cl::desc("Name of the macro to be defined -- ignored by aiir-pdll"),
