//===- aiir-runner.cpp - AIIR CPU Execution Driver ------------------------===//
//
...
//===----------------------------------------------------------------------===//
//
// Main entry point to a command line utility that executes an AIIR file on the
// CPU by  translating AIIR to LLVM IR before JIT-compiling and executing the
// latter.
//
//===----------------------------------------------------------------------===//
...
static llvm::cl::opt<bool> linkNestedModules(
    "link-nested-modules",
    llvm::cl::desc("Link two nested AIIR modules into a single LLVM IR module. "
                   "Useful if both the host and device code can be run on the "
                   "same CPU, as in SPIR-V CPU Runner tests."));

/// A utility function that builds llvm::Module from two nested AIIR modules.
///
/// module @main {
///   module @kernel {
///     // Some ops
...
convertAIIRModule(Operation *op, llvm::LLVMContext &context) {
...
int main(int argc, char **argv) {
  ...
  aiir::JitRunnerConfig jitRunnerConfig;
  jitRunnerConfig.llvmModuleBuilder = convertAIIRModule;
  return aiir::JitRunnerMain(argc, argv, registry, jitRunnerConfig);
}
