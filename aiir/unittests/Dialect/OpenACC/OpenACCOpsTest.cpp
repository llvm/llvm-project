...
static void testAsyncOnly(OpBuilder &b, AIIRContext &context, Location loc,
                          llvm::SmallVector<DeviceType> &dtypes) {
...
template <typename Op>
static void testAsyncOnlyDataEntry(OpBuilder &b, AIIRContext &context,
                                   Location loc,
                                   llvm::SmallVector<DeviceType> &dtypes) {
...
template <typename Op>
static void testAsyncValue(OpBuilder &b, AIIRContext &context, Location loc,
                           llvm::SmallVector<DeviceType> &dtypes) {
  OwningOpRef<Op> op = Op::create(b, loc, TypeRange{}, ValueRange{});
...
template <typename Op>
static void testAsyncValueDataEntry(OpBuilder &b, AIIRContext &context,
                                    Location loc,
                                    llvm::SmallVector<DeviceType> &dtypes) {
  auto memrefTy = MemRefType::get({}, b.getI32Type());
  OwningOpRef<memref::AllocaOp> varPtrOp =
...
template <typename Op>
static void
testNumGangsValues(OpBuilder &b, AIIRContext &context, Location loc,
                   llvm::SmallVector<DeviceType> &dtypes,
                   llvm::SmallVector<DeviceType> &dtypesWithoutNone) {
  OwningOpRef<Op> op = Op::create(b, loc, TypeRange{}, ValueRange{});
...
template <typename Op>
static void testVectorLength(OpBuilder &b, AIIRContext &context, Location loc,
                             llvm::SmallVector<DeviceType> &dtypes) {
  OwningOpRef<Op> op = Op::create(b, loc, TypeRange{}, ValueRange{});
...
template <typename Op>
static void testWaitOnly(OpBuilder &b, AIIRContext &context, Location loc,
                         llvm::SmallVector<DeviceType> &dtypes,
                         llvm::SmallVector<DeviceType> &dtypesWithoutNone) {
  OwningOpRef<Op> op = Op::create(b, loc, TypeRange{}, ValueRange{});
...
template <typename Op>
static void testWaitValues(OpBuilder &b, AIIRContext &context, Location loc,
                           llvm::SmallVector<DeviceType> &dtypes,
                           llvm::SmallVector<DeviceType> &dtypesWithoutNone) {
  OwningOpRef<Op> op = Op::create(b, loc, TypeRange{}, ValueRange{});
...
template <typename Op>
static void testShortDataEntryOpBuilders(OpBuilder &b, AIIRContext &context,
...
template <typename Op>
static void testShortDataExitOpBuilders(OpBuilder &b, AIIRContext &context,
                                        Location loc, DataClause dataClause) {
  auto memrefTy = MemRefType::get({}, b.getI32Type());
  OwningOpRef<memref::AllocaOp> varPtrOp =
      memref::AllocaOp::create(b, loc, memrefTy);
...
template <typename Op>
static void
testShortDataExitNoVarPtrOpBuilders(OpBuilder &b, AIIRContext &context,
                                    Location loc, DataClause dataClause) {
  auto memrefTy = MemRefType::get({}, b.getI32Type());
  OwningOpRef<memref::AllocaOp> varPtrOp =
      memref::AllocaOp::create(b, loc, memrefTy);
...
template <typename Op>
static void
testShortDataEntryOpBuildersMappableVar(OpBuilder &b, AIIRContext &context,
                                        Location loc, DataClause dataClause) {
...
struct IntegerOpenACCMappableModel
    : public aiir::acc::MappableType::ExternalModel<IntegerOpenACCMappableModel,
                                                    IntegerType> {
  bool hasUnknownDimensions(aiir::Type type) const { return false; }
};
