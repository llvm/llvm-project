...
#include "aiir/ExecutionEngine/MemRefUtils.h"
#include "aiir/ExecutionEngine/RunnerUtils.h"
...
static LogicalResult lowerToLLVMDialect(ModuleOp module) {
  ...
  pm.addPass(aiir::createFinalizeMemRefToLLVMConversionPass());
  pm.addNestedPass<func::FuncOp>(aiir::createArithToLLVMConversionPass());
  pm.addPass(aiir::createConvertFuncToLLVMPass());
  pm.addPass(aiir::createReconcileUnrealizedCastsPass());
  return pm.run(module);
}

TEST(AIIRExecutionEngine, SKIP_WITHOUT_JIT(AddInteger)) {
#ifdef __s390__
  std::string moduleStr = R"aiir(
...
TEST(AIIRExecutionEngine, SKIP_WITHOUT_JIT(SubtractFloat)) {
  std::string moduleStr = R"aiir(
...
TEST(NativeMemRefJit, MAYBE_JITCallback) {
  ...
  jit->registerSymbols([&](llvm::orc::MangleAndInterner interner) {
    ...
    symbolMap[interner("_aiir_ciface_callback")] = {
...
TEST(AIIRExecutionEngine, SKIP_WITHOUT_JIT(CallbackInGlobalCtor)) {
  ...
  jit->registerSymbols([&](llvm::orc::MangleAndInterner interner) {
    llvm::orc::SymbolMap symbolMap;
    symbolMap[interner("_aiir_ciface_init_callback")] = {
