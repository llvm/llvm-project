...
struct TestProperties {
  ...
  AIIR_DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID(TestProperties)
};
...
setPropertiesFromAttribute(TestProperties &prop, Attribute attr,
  ...
  auto label = dict.getAs<aiir::StringAttr>("label");
  if (!label) {
...
static Attribute getPropertiesAsAttribute(AIIRContext *ctx,
...
public:
  // Begin boilerplate
  AIIR_DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID(OpWithProperties)
  using Op::Op;
  ...
  static std::optional<aiir::Attribute> getInherentAttr(AIIRContext *context,
  ...
  static void populateInherentAttrs(AIIRContext *context,
                                    const Properties &prop,
                                    NamedAttrList &attrs) {}
  static LogicalResult
...
constexpr StringLiteral aiirSrc = R"aiir(
...
TEST(OpPropertiesTest, Properties) {
  ...
  OwningOpRef<Operation *> op = parseSourceString(aiirSrc, config);
...
TEST(OpPropertiesTest, FailedProperties) {
  ...
  owningOp = parseSourceString(aiirSrc, config);
