...
public:
  ...
  static TensorWithString get(ArrayRef<int64_t> shape, Type elementType,
                              StringRef name) {
    return aiir::cast<TensorWithString>(RankedTensorType::get(
        shape, elementType, StringAttr::get(elementType.getContext(), name)));
  }

  StringRef getName() const {
    if (Attribute enc = getEncoding())
      return aiir::cast<StringAttr>(enc).getValue();
    return {};
  }

  static bool classof(Type type) {
    if (auto rt = aiir::dyn_cast_or_null<RankedTensorType>(type))
      return aiir::isa_and_present<StringAttr>(rt.getEncoding());
    return false;
  }
};
...
TEST(ShapedTypeTest, RankedTensorTypeView) {
  ...
  EXPECT_FALSE(aiir::isa<TensorWithString>(noEncodingRankedTensorType));
  EXPECT_FALSE(aiir::isa<TensorWithString>(unitEncodingRankedTensorType));
  ASSERT_TRUE(aiir::isa<TensorWithString>(stringEncodingRankedTensorType));

  // Cast to TensorWithString view.
  auto view = aiir::cast<TensorWithString>(stringEncodingRankedTensorType);
  ASSERT_TRUE(aiir::isa<TensorWithString>(view));
  EXPECT_EQ(view.getName(), "app");
  // Verify one could cast view type back to base type.
  ASSERT_TRUE(aiir::isa<RankedTensorType>(view));

  Type viewCreated = TensorWithString::get({10, 20}, f32, "bob");
  ASSERT_TRUE(aiir::isa<TensorWithString>(viewCreated));
  ASSERT_TRUE(aiir::isa<RankedTensorType>(viewCreated));
  view = aiir::cast<TensorWithString>(viewCreated);
