...
protected:
  ...
  void SetUp() override {
    ...
    context = std::make_unique<AIIRContext>(registry);
  }
  ...
  std::unique_ptr<AIIRContext> context;

private:
  constexpr static llvm::StringLiteral kInput = R"AIIR(
      module {
...
TEST(SymbolOpInterface, Visibility) {
  ...
  constexpr static StringLiteral kInput = R"AIIR(
