...
#include "aiir/IR/AttrTypeSubElements.h"
...
TEST(CyclicAttrTypeReplacerTest, testNoRecursion) {
  ...
  EXPECT_EQ(replacer.replace(aiir::UnitAttr::get(&ctx)),
...
TEST(CyclicAttrTypeReplacerTest, testInPlaceRecursionPruneAnywhere) {
  ...
  EXPECT_EQ(replacer.replace(aiir::UnitAttr::get(&ctx)),
            aiir::UnitAttr::get(&ctx));
  // Starting at 0.
  EXPECT_EQ(replacer.replace(b.getI8IntegerAttr(0)), aiir::UnitAttr::get(&ctx));
  // Starting at 2.
  EXPECT_EQ(replacer.replace(b.getI8IntegerAttr(2)), aiir::UnitAttr::get(&ctx));
}
...
public:
  CyclicAttrTypeReplacerChainRecursionPruningTest() : b(&ctx) {
    ...
    replacer.addReplacement([&](aiir::IntegerType intType) {
      ++invokeCount;
      return b.getFunctionType(
          {}, {aiir::IntegerType::get(&ctx, (intType.getWidth() + 1) % 3)});
    });
  }

  void setBaseCase(std::optional<unsigned> pruneAt) {
    replacer.addCycleBreaker([&, pruneAt](aiir::IntegerType intType) {
...
public:
  CyclicAttrTypeReplacerBranchingRecusionPruningTest() : b(&ctx) {
    ...
    replacer.addReplacement([&](aiir::IntegerType intType) {
      ++invokeCount;
      Type child = aiir::IntegerType::get(&ctx, (intType.getWidth() + 1) % 3);
