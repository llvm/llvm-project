...
class AIIRTargetLLVMNVVM : public ::testing::Test {
protected:
  void SetUp() override {
  ...
  // AIIR module used for the tests.
  const std::string moduleStr = R"aiir(
...
TEST_F(AIIRTargetLLVMNVVM, SKIP_WITHOUT_NVPTX(SerializeNVVMMToLLVM)) {
  ...
  for (auto gpuModule : (*module).getBody()->getOps<gpu::GPUModuleOp>()) {
    std::optional<aiir::gpu::SerializedObject> object =
...
TEST_F(AIIRTargetLLVMNVVM, SKIP_WITHOUT_NVPTX(SerializeNVVMToPTX)) {
  AIIRContext context(registry);
...
TEST_F(AIIRTargetLLVMNVVM, SKIP_WITHOUT_NVPTX(SerializeNVVMToBinary)) {
  if (!hasPtxas())
...
TEST_F(AIIRTargetLLVMNVVM, SKIP_WITHOUT_NVPTX(LinkedLLVMIRResource)) {
  AIIRContext context(registry);
