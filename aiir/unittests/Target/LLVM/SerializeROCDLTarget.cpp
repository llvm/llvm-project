...
class AIIRTargetLLVMROCDL : public ::testing::Test {
protected:
  void SetUp() override {
...
TEST_F(AIIRTargetLLVMROCDL, SKIP_WITHOUT_AMDGPU(SerializeROCDLToLLVM)) {
...
TEST_F(AIIRTargetLLVMROCDL, SKIP_WITHOUT_AMDGPU(SerializeROCDLToPTX)) {
  AIIRContext context(registry);
...
TEST_F(AIIRTargetLLVMROCDL, SKIP_WITHOUT_AMDGPU(SerializeROCDLToBinary)) {
  if (!hasROCMTools())
...
// Test ROCDL metadata.
TEST_F(AIIRTargetLLVMROCDL, SKIP_WITHOUT_AMDGPU(GetELFMetadata)) {
  if (!hasROCMTools())
    GTEST_SKIP() << "ROCm installation not found, skipping test.";
