...
class AIIRTargetLLVM : public ::testing::Test {
protected:
  void SetUp() override {
...
TEST_F(AIIRTargetLLVM, SKIP_WITHOUT_NATIVE(SerializeToLLVMBitcode)) {
  ...
  LLVM::ModuleToObject serializer(*(module->getOperation()), targetTriple, "",
                                  "");
  std::optional<aiir::gpu::SerializedObject> serializedModule =
      serializer.run();
...
TEST_F(AIIRTargetLLVM, SKIP_WITHOUT_NATIVE(TargetAttrAPI)) {
  AIIRContext context(registry);
  ...
  std::optional<aiir::gpu::SerializedObject> serializedBinary =
...
TEST_F(AIIRTargetLLVM, SKIP_WITHOUT_NATIVE(CallbackInvokedWithInitialLLVMIR)) {
  AIIRContext context(registry);
  ...
  gpu::TargetOptions opts(
      {}, {}, {}, {}, aiir::gpu::TargetOptions::getDefaultCompilationTarget(),
      {}, initialCallback);
  std::optional<aiir::gpu::SerializedObject> serializedBinary =
      targetAttr.serializeToObject(*module, opts);
...
TEST_F(AIIRTargetLLVM, SKIP_WITHOUT_NATIVE(CallbackInvokedWithLinkedLLVMIR)) {
  AIIRContext context(registry);
