...
public:
  // Begin boilerplate.
  AIIR_DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID(OpWithoutProperties)
  using Op::Op;
...
public:
  AIIR_DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID(TestOpPropertiesDialect)
  static constexpr StringLiteral getDialectNamespace() {
    return StringLiteral("test_op_properties");
  }
  explicit TestOpPropertiesDialect(AIIRContext *context)
...
constexpr StringLiteral withoutPropertiesAttrsSrc = R"aiir(
...
TEST(Bytecode, DeepCallSiteLoc) {
  ...
  Location baseLoc = FileLineColLoc::get(&context, "test.aiir", 1, 1);
