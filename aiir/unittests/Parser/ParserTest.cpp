...
namespace {
TEST(AIIRParser, ParseInvalidIR) {
...
TEST(AIIRParser, ParseAtEnd) {
  std::string firstModuleStr = R"aiir(
    "test.first"() : () -> ()
  )aiir";
  std::string secondModuleStr = R"aiir(
    "test.second"() : () -> ()
  )aiir";
...
TEST(AIIRParser, AsmParserLocations) {
  std::string moduleStr = R"aiir(
func.func @foo() -> !llvm.array<2 x f32> {
  %0 = llvm.aiir.undef : !llvm.array<2 x f32>
  ...
  auto memBuffer =
      llvm::MemoryBuffer::getMemBuffer(moduleStr, "AsmParserTest.aiir",
