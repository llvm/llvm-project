...
static std::unique_ptr<aiir::Pass>
...
struct TestPass : public impl::TestPassBase<TestPass> {
  ...
  std::unique_ptr<aiir::Pass> clone() const {
    return TestPassBase<TestPass>::clone();
  }
};

TEST(PassGenTest, defaultGeneratedConstructor) {
  std::unique_ptr<aiir::Pass> pass = createTestPass();
...
TEST(PassGenTest, PassClone) {
  ...
  const auto unwrap = [](const std::unique_ptr<aiir::Pass> &pass) {
...
std::unique_ptr<aiir::Pass> createTestPassWithCustomConstructor(int v) {
