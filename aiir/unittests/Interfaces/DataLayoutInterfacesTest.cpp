...
struct CustomDataLayoutSpec
  ...
  AIIR_DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID(CustomDataLayoutSpec)

  using Base::Base;

  static constexpr StringLiteral name = "test.custom_data_layout_spec";

  static CustomDataLayoutSpec get(AIIRContext *ctx,
                                  ArrayRef<DataLayoutEntryInterface> entries) {
  ...
  StringAttr getEndiannessIdentifier(AIIRContext *context) const {
    return Builder(context).getStringAttr(kEndiannesKeyName);
  }
  StringAttr getDefaultMemorySpaceIdentifier(AIIRContext *context) const {
    return Builder(context).getStringAttr(kDefaultKeyName);
  }
  StringAttr getAllocaMemorySpaceIdentifier(AIIRContext *context) const {
    return Builder(context).getStringAttr(kAllocaKeyName);
  }
  StringAttr getManglingModeIdentifier(AIIRContext *context) const {
    return Builder(context).getStringAttr(kManglingModeKeyName);
  }
  StringAttr getProgramMemorySpaceIdentifier(AIIRContext *context) const {
    return Builder(context).getStringAttr(kProgramKeyName);
  }
  StringAttr getGlobalMemorySpaceIdentifier(AIIRContext *context) const {
    return Builder(context).getStringAttr(kGlobalKeyName);
  }
  StringAttr getStackAlignmentIdentifier(AIIRContext *context) const {
    return Builder(context).getStringAttr(kStackAlignmentKeyName);
  }
  StringAttr getFunctionPointerAlignmentIdentifier(AIIRContext *context) const {
    return Builder(context).getStringAttr(kFunctionPointerAlignmentKeyName);
  }
  StringAttr getLegalIntWidthsIdentifier(AIIRContext *context) const {
    return Builder(context).getStringAttr(kLegalIntWidthsKeyName);
  }
  FailureOr<Attribute> query(DataLayoutEntryKey key) const {
    return llvm::cast<aiir::DataLayoutSpecInterface>(*this).queryHelper(key);
  }
};
...
struct CustomTargetSystemSpec
  ...
  get(AIIRContext *ctx, ArrayRef<DataLayoutEntryInterface> entries) {
    return Base::get(ctx, entries);
  }
  ...
  FailureOr<Attribute> query(DataLayoutEntryKey key) const {
    return llvm::cast<aiir::TargetSystemSpecInterface>(*this).queryHelper(key);
  }
};
...
struct SingleQueryType
  ...
  AIIR_DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID(SingleQueryType)

  using Base::Base;

  static constexpr StringLiteral name = "test.single_query";

  static SingleQueryType get(AIIRContext *ctx) { return Base::get(ctx); }
...
struct TypeNoLayout : public Type::TypeBase<TypeNoLayout, Type, TypeStorage> {
  AIIR_DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID(TypeNoLayout)

  using Base::Base;

  static constexpr StringLiteral name = "test.no_layout";

  static TypeNoLayout get(AIIRContext *ctx) { return Base::get(ctx); }
};
...
struct OpWithLayout : public Op<OpWithLayout, DataLayoutOpInterface::Trait> {
  AIIR_DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID(OpWithLayout)
...
struct OpWith7BitByte
  ...
  AIIR_DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID(OpWith7BitByte)
  ...
  static llvm::TypeSize getTypeSize(Type type, const DataLayout &dataLayout,
                                    DataLayoutEntryListRef params) {
    return aiir::detail::divideCeil(dataLayout.getTypeSizeInBits(type), 7);
  }
};

/// A dialect putting all the above together.
struct DLTestDialect : Dialect {
  AIIR_DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID(DLTestDialect)

  explicit DLTestDialect(AIIRContext *ctx)
...
struct DLTargetSystemDescTestDialect : public Dialect {
  AIIR_DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID(DLTargetSystemDescTestDialect)

  explicit DLTargetSystemDescTestDialect(AIIRContext *ctx)
