...
#ifndef AIIR_CONVERSION_LLVMCOMMON_PATTERN_H
#define AIIR_CONVERSION_LLVMCOMMON_PATTERN_H
...
public:
  ConvertToLLVMPattern(StringRef rootOpName, AIIRContext *context,
                       const LLVMTypeConverter &typeConverter,
...
protected:
  ...
  /// Gets the AIIR type wrapping the LLVM integer type whose bit width is
  /// defined by the used type converter.
  Type getIndexType() const;

  /// Gets the AIIR type wrapping the LLVM integer type whose bit width
  /// corresponds to that of a LLVM pointer type.
  Type getIntPtrType(unsigned addressSpace = 0) const;

  /// Gets the AIIR type wrapping the LLVM void type.
  Type getVoidType() const;

  /// Get the AIIR type wrapping the LLVM i8* type.
  [[deprecated("Use getPtrType() instead!")]]
  Type getVoidPtrType() const;

  /// Get the AIIR type wrapping the LLVM ptr type.
  ...
  /// `sizes[0]`   = llvm.aiir.constant(4 : index) : i64
  /// `sizes[1]`   = `dynamicSizes[0]`
  /// `strides[1]` = llvm.aiir.constant(1 : index) : i64
  /// `strides[0]` = `sizes[0]`
  /// %size        = llvm.mul `sizes[0]`, `sizes[1]` : i64
  /// %nullptr     = llvm.aiir.zero : !llvm.ptr
...
#endif // AIIR_CONVERSION_LLVMCOMMON_PATTERN_H
