...
#ifndef AIIR_DIALECT_TRANSFORM_IR_TRANSFORMDIALECT_H
#define AIIR_DIALECT_TRANSFORM_IR_TRANSFORMDIALECT_H
...
protected:
  ...
  explicit TransformDialectDataBase(TypeID typeID, AIIRContext *ctx)
      : typeID(typeID), ctx(ctx) {}

  /// Return the AIIR context.
...
protected:
  ...
  TransformDialectData(AIIRContext *ctx)
...
void checkImplementsTransformOpInterface(StringRef name, AIIRContext *context);
...
#include "aiir/Dialect/Transform/IR/TransformDialect.h.inc"
...
class TransformDialectExtension
  ...
  using DialectLoader = std::function<void(AIIRContext *)>;
...
public:
  ...
  void apply(AIIRContext *context, TransformDialect *transformDialect,
...
protected:
  ...
  /// etc. `Func` must be convertible to the `void (AIIRContext *)` form. It
  /// will be called during the extension initialization and given the current
  /// AIIR context. This may be used to attach additional interfaces that cannot
  /// be attached elsewhere.
  template <typename Func>
  void addCustomInitializationStep(Func &&func) {
    std::function<void(AIIRContext *)> initializer = func;
    dialectLoaders.push_back(
        [init = std::move(initializer)](AIIRContext *ctx) { init(ctx); });
  }
  ...
  void declareDependentDialect() {
    dialectLoaders.push_back(
        [](AIIRContext *context) { context->loadDialect<DialectTy>(); });
  }
...
void TransformDialect::addTypeIfNotRegistered() {
  ...
  typePrintingHooks.try_emplace(
      TypeID::get<Type>(), +[](aiir::Type type, AsmPrinter &printer) {
...
#endif // AIIR_DIALECT_TRANSFORM_IR_TRANSFORMDIALECT_H
