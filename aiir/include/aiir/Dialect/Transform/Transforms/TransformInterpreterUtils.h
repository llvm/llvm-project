...
#ifndef AIIR_DIALECT_TRANSFORM_TRANSFORMS_TRANSFORMINTERPRETERUTILS_H
#define AIIR_DIALECT_TRANSFORM_TRANSFORMS_TRANSFORMINTERPRETERUTILS_H
...
/// Expands the given list of `paths` to a list of `.aiir` files.
///
/// Each entry in `paths` may either be a regular file, in which case it ends up
/// in the result list, or a directory, in which case all (regular) `.aiir`
/// files in that directory are added. Any other file types lead to a failure.
LogicalResult expandPathsToAIIRFiles(ArrayRef<std::string> paths,
                                     AIIRContext *context,
                                     SmallVectorImpl<std::string> &fileNames);

/// Utility to parse and verify the content of a `transformFileName` AIIR file
/// containing a transform dialect specification.
LogicalResult
parseTransformModuleFromFile(AIIRContext *context,
                             llvm::StringRef transformFileName,
                             OwningOpRef<ModuleOp> &transformModule);

/// Utility to parse, verify, aggregate and link the content of all aiir files
/// nested under `transformLibraryPaths` and containing transform dialect
/// specifications.
LogicalResult
assembleTransformLibraryFromPaths(AIIRContext *context,
...
ModuleOp getPreloadedTransformModule(AIIRContext *context);
...
#endif // AIIR_DIALECT_TRANSFORM_TRANSFORMS_TRANSFORMINTERPRETERUTILS_H
