...
#ifndef AIIR_DIALECT_OPENACC_ANALYSIS_OPENACCSUPPORT_H
#define AIIR_DIALECT_OPENACC_ANALYSIS_OPENACCSUPPORT_H
...
/// It follows the Concept-Model pattern used throughout AIIR (e.g., in
...
struct OpenACCSupportTraits {
  ...
  public:
    ...
    /// Get the variable name for a given AIIR value.
    ...
    virtual bool isValidValueUse(Value v, aiir::Region &region) = 0;
...
public:
  ...
  ///
  /// \param v The AIIR value to get the variable name for.
  ...
  /// \param var The AIIR value to get the recipe name for. Can be null if
  ...
  /// Check if a value use is legal in an OpenACC region.
  ///
  /// \param v The AIIR value to check for legality.
  /// \param region The AIIR region in which the legality is checked.
...
#endif // AIIR_DIALECT_OPENACC_ANALYSIS_OPENACCSUPPORT_H
