...
#ifndef AIIR_DIALECT_OPENACC_OPENACCPARMAPPING_H_
#define AIIR_DIALECT_OPENACC_OPENACCPARMAPPING_H_
...
/// the mapping functions (e.g., aiir::acc::GPUParallelDimAttr for GPU targets).
///
...
public:
  ...
  virtual ParDimAttrT map(AIIRContext *ctx, ParLevel level) const = 0;

  /// Convenience methods for specific parallelism levels.
  ParDimAttrT gangDim(AIIRContext *ctx, ParLevel level) const {
  ...
  ParDimAttrT workerDim(AIIRContext *ctx) const {
    return map(ctx, ParLevel::worker);
  }
  ParDimAttrT vectorDim(AIIRContext *ctx) const {
    return map(ctx, ParLevel::vector);
  }
  ParDimAttrT seqDim(AIIRContext *ctx) const { return map(ctx, ParLevel::seq); }
...
class DefaultACCToGPUMappingPolicy
    : public ACCParMappingPolicy<aiir::acc::GPUParallelDimAttr> {
public:
  aiir::acc::GPUParallelDimAttr map(AIIRContext *ctx,
                                    ParLevel level) const override {
    switch (level) {
    case ParLevel::gang_dim1:
      return aiir::acc::GPUParallelDimAttr::blockXDim(ctx);
    case ParLevel::gang_dim2:
      return aiir::acc::GPUParallelDimAttr::blockYDim(ctx);
    case ParLevel::gang_dim3:
      return aiir::acc::GPUParallelDimAttr::blockZDim(ctx);
    case ParLevel::worker:
      return aiir::acc::GPUParallelDimAttr::threadYDim(ctx);
    case ParLevel::vector:
      return aiir::acc::GPUParallelDimAttr::threadXDim(ctx);
    case ParLevel::seq:
      return aiir::acc::GPUParallelDimAttr::seqDim(ctx);
    }
    llvm_unreachable("Unknown ParLevel");
  }

  bool isVector(aiir::acc::GPUParallelDimAttr attr) const override {
    return attr.isThreadX();
  }

  bool isWorker(aiir::acc::GPUParallelDimAttr attr) const override {
    return attr.isThreadY();
  }

  bool isGang(aiir::acc::GPUParallelDimAttr attr) const override {
    return attr.isAnyBlock();
  }

  bool isSeq(aiir::acc::GPUParallelDimAttr attr) const override {
    return attr.isSeq();
  }
};
...
using ACCToGPUMappingPolicy =
    ACCParMappingPolicy<aiir::acc::GPUParallelDimAttr>;

} // namespace acc
} // namespace aiir

#endif // AIIR_DIALECT_OPENACC_OPENACCPARMAPPING_H_
