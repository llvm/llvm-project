...
TosaOp createOpAndInferShape(ImplicitLocOpBuilder &builder, Type resultTy,
  ...
  inferredKnowledge.dtype = aiir::cast<ShapedType>(resultTy).getElementType();
  ...
  Type newTy =
      newKnowledge.hasRank
          ? Type{aiir::RankedTensorType::get(llvm::ArrayRef(newKnowledge.sizes),
                                             newKnowledge.dtype)}
          : Type{aiir::UnrankedTensorType::get(newKnowledge.dtype)};
...
TosaOp CreateOpAndInferShape(ImplicitLocOpBuilder &builder, Type resultTy,
                             Args &&...args) {
  if (TosaOp::template hasTrait<::aiir::OpTrait::SameOperandsAndResultRank>()) {
...
SmallVector<int64_t> convertFromAiirShape(ArrayRef<int64_t> shape);
