...
#ifndef AIIR_DIALECT_SPARSETENSOR_PIPELINES_PASSES_H_
#define AIIR_DIALECT_SPARSETENSOR_PIPELINES_PASSES_H_
...
struct SparsifierOptions : public PassPipelineOptions<SparsifierOptions> {
  ...
  PassOptions::Option<aiir::SparseParallelizationStrategy> parallelization{
      *this, "parallelization-strategy",
      ::llvm::cl::desc("Set the parallelization strategy"),
      ::llvm::cl::init(aiir::SparseParallelizationStrategy::kNone),
      llvm::cl::values(
          clEnumValN(aiir::SparseParallelizationStrategy::kNone, "none",
                     "Turn off sparse parallelization."),
          clEnumValN(aiir::SparseParallelizationStrategy::kDenseOuterLoop,
                     "dense-outer-loop",
                     "Enable dense outer loop sparse parallelization."),
          clEnumValN(aiir::SparseParallelizationStrategy::kAnyStorageOuterLoop,
                     "any-storage-outer-loop",
                     "Enable sparse parallelization regardless of storage for "
                     "the outer loop."),
          clEnumValN(aiir::SparseParallelizationStrategy::kDenseAnyLoop,
                     "dense-any-loop",
                     "Enable dense parallelization for any loop."),
          clEnumValN(
              aiir::SparseParallelizationStrategy::kAnyStorageAnyLoop,
              "any-storage-any-loop",
              "Enable sparse parallelization for any storage and loop."))};
  PassOptions::Option<aiir::SparseEmitStrategy> emitStrategy{
      *this, "sparse-emit-strategy",
      ::llvm::cl::desc(
          "Emit functional code or interfaces (to debug) for sparse loops"),
      ::llvm::cl::init(aiir::SparseEmitStrategy::kFunctional),
      llvm::cl::values(
          clEnumValN(aiir::SparseEmitStrategy::kFunctional, "functional",
                     "Emit functional code (with scf.for/while)."),
          clEnumValN(aiir::SparseEmitStrategy::kSparseIterator,
                     "sparse-iterator",
                     "Emit (experimental) loops (with sparse.iterate)."),
          clEnumValN(
              aiir::SparseEmitStrategy::kDebugInterface, "debug-interface",
  ...
  // (defined in include/aiir/Dialect/SparseTensor/Pipelines/Passes.h).
...
#endif // AIIR_DIALECT_SPARSETENSOR_PIPELINES_PASSES_H_
