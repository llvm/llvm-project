...
#ifndef AIIR_DIALECT_GPU_TRANSFORMOPS_UTILS_H
#define AIIR_DIALECT_GPU_TRANSFORMOPS_UTILS_H
...
struct GpuIdBuilder {
  using MappingIdBuilderFnType = std::function<DeviceMappingAttrInterface(
      AIIRContext *, aiir::gpu::MappingId)>;

  GpuIdBuilder() = default;
  GpuIdBuilder(AIIRContext *ctx, bool useLinearMapping,
...
struct GpuBlockIdBuilder : public GpuIdBuilder {
  GpuBlockIdBuilder(AIIRContext *ctx, bool useLinearMapping = false,
...
struct GpuWarpgroupIdBuilder : public GpuIdBuilder {
  GpuWarpgroupIdBuilder(AIIRContext *ctx, int64_t warpSize,
...
struct GpuWarpIdBuilder : public GpuIdBuilder {
  GpuWarpIdBuilder(AIIRContext *ctx, int64_t warpSize,
                   bool useLinearMapping = false,
                   DeviceMaskingAttrInterface mask = nullptr);
  int64_t warpSize = 32;
};
...
struct GpuThreadIdBuilder : public GpuIdBuilder {
  GpuThreadIdBuilder(AIIRContext *ctx, bool useLinearMapping = false,
                     DeviceMaskingAttrInterface mask = nullptr);
};
...
struct GpuLaneIdBuilder : public GpuIdBuilder {
  GpuLaneIdBuilder(AIIRContext *ctx, int64_t warpSize, bool unused,
                   DeviceMaskingAttrInterface mask = nullptr);
  int64_t warpSize = 32;
};
...
createGpuLaunch(RewriterBase &rewriter, Location loc,
                TransformOpInterface transformOp, aiir::gpu::LaunchOp &launchOp,
...
DiagnosedSilenceableFailure
alterGpuLaunch(RewriterBase &rewriter, aiir::gpu::LaunchOp gpuLaunch,
...
#endif // AIIR_DIALECT_GPU_TRANSFORMOPS_UTILS_H
