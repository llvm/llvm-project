//===- MemRefUtils.h - Memref helpers to invoke AIIR JIT code ---*- C++ -*-===//
//
...
//===----------------------------------------------------------------------===//
//
// Utils for AIIR ABI interfacing with frameworks.
//
// The templated free functions below make it possible to allocate dense
// contiguous buffers with shapes that interoperate properly with the AIIR
// codegen ABI.
//
//===----------------------------------------------------------------------===//
...
#ifndef AIIR_EXECUTIONENGINE_MEMREFUTILS_H_
#define AIIR_EXECUTIONENGINE_MEMREFUTILS_H_
...
/// Build a `StridedMemRefDescriptor<T, N>` that matches the AIIR ABI.
/// This is an implementation detail that is kept in sync with AIIR codegen
...
/// Build a `StridedMemRefDescriptor<T, 0>` that matches the AIIR ABI.
/// This is an implementation detail that is kept in sync with AIIR codegen
/// conventions.  Additionally takes a `shapeAlloc` array which
/// is used instead of `shape` to allocate "more aligned" data and compute the
/// corresponding strides.
...
#endif // AIIR_EXECUTIONENGINE_MEMREFUTILS_H_
