//===- ExecutionEngine.h - AIIR Execution engine and utils -----*- C++ -*--===//
//
...
//===----------------------------------------------------------------------===//
//
// This file provides a JIT-backed execution engine for AIIR modules.
//
//===----------------------------------------------------------------------===//

#ifndef AIIR_EXECUTIONENGINE_EXECUTIONENGINE_H_
#define AIIR_EXECUTIONENGINE_EXECUTIONENGINE_H_
...
struct ExecutionEngineOptions {
  ...
  /// module from the given AIIR IR. Otherwise, a default
...
/// JIT-backed execution engine for AIIR. Assumes the IR can be converted to
/// LLVM IR. For each function, creates a wrapper function with the fixed
/// interface
///
///     void _aiir_funcName(void **)
///
...
public:
  ...
  static constexpr const char *const kLibraryDestroyFnName =
      "__aiir_execution_engine_destroy";
  ...
  /// Creates an execution engine for the given AIIR IR. If TargetMachine is
  ...
  llvm::Error invoke(StringRef funcName, Args... args) {
    const std::string adapterName =
        std::string("_aiir_ciface_") + funcName.str();
...
#endif // AIIR_EXECUTIONENGINE_EXECUTIONENGINE_H_
