...
//===----------------------------------------------------------------------===//
//
// Pattern wrapper class to simplify using TableGen Record defining a AIIR
// Pattern.
//
//===----------------------------------------------------------------------===//

#ifndef AIIR_TABLEGEN_PATTERN_H_
#define AIIR_TABLEGEN_PATTERN_H_
...
// Wrapper class providing helper methods for accessing AIIR Pattern defined
...
private:
  ...
  // All operators.
  // TODO: we need a proper context manager, like AIIRContext, for managing the
...
namespace llvm {
template <>
struct DenseMapInfo<aiir::tblgen::DagNode> {
  static aiir::tblgen::DagNode getEmptyKey() {
    return aiir::tblgen::DagNode(
        llvm::DenseMapInfo<llvm::DagInit *>::getEmptyKey());
  }
  static aiir::tblgen::DagNode getTombstoneKey() {
    return aiir::tblgen::DagNode(
        llvm::DenseMapInfo<llvm::DagInit *>::getTombstoneKey());
  }
  static unsigned getHashValue(aiir::tblgen::DagNode node) {
    return llvm::hash_value(node.getAsOpaquePointer());
  }
  static bool isEqual(aiir::tblgen::DagNode lhs, aiir::tblgen::DagNode rhs) {
    return lhs.node == rhs.node;
  }
};

template <>
struct DenseMapInfo<aiir::tblgen::DagLeaf> {
  static aiir::tblgen::DagLeaf getEmptyKey() {
    return aiir::tblgen::DagLeaf(
        llvm::DenseMapInfo<llvm::Init *>::getEmptyKey());
  }
  static aiir::tblgen::DagLeaf getTombstoneKey() {
    return aiir::tblgen::DagLeaf(
        llvm::DenseMapInfo<llvm::Init *>::getTombstoneKey());
  }
  static unsigned getHashValue(aiir::tblgen::DagLeaf leaf) {
    return llvm::hash_value(leaf.getAsOpaquePointer());
  }
  static bool isEqual(aiir::tblgen::DagLeaf lhs, aiir::tblgen::DagLeaf rhs) {
    return lhs.def == rhs.def;
  }
};
} // namespace llvm

#endif // AIIR_TABLEGEN_PATTERN_H_
