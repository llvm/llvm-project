...
#ifndef AIIR_PASS_PASSMANAGER_H
#define AIIR_PASS_PASSMANAGER_H
...
public:
  ...
  std::optional<OperationName> getOpName(AIIRContext &context) const;
...
private:
  ...
  LogicalResult initialize(AIIRContext *context, unsigned newInitGeneration);
...
public:
  ...
  static PassManager on(AIIRContext *ctx, Nesting nesting = Nesting::Explicit) {
  ...
  /// of a crash or a pass failure. `outputFile` is a .aiir filename used to
  ...
  /// and symbol name (if present). The output AIIR files are prefixed using an
