...
#ifndef AIIR_PASS_PASSOPTIONS_H_
#define AIIR_PASS_PASSOPTIONS_H_
...
public:
  ...
  bool parse(Option &opt, StringRef argName, StringRef arg,
    ...
    return failed(aiir::detail::pass_options::parseCommaSeparatedList(
  ...
  static void print(raw_ostream &os, const VectorT &vector) {
    llvm::interleave(
        ...
        [&](const ElementT &value) {
          aiir::detail::pass_options::printOptionValue<
...
template <>
struct OptionValue<aiir::OpPassManager> final : GenericOptionValue {
  using WrapperType = aiir::OpPassManager;

  OptionValue();
  OptionValue(const OptionValue<aiir::OpPassManager> &rhs);
  OptionValue(const aiir::OpPassManager &value);
  OptionValue<aiir::OpPassManager> &operator=(const aiir::OpPassManager &rhs);
  ~OptionValue();
  ...
  aiir::OpPassManager &getValue() const {
    assert(hasValue() && "invalid option value");
    return *value;
  }

  /// Set the value of the option.
  void setValue(const aiir::OpPassManager &newValue);
  void setValue(StringRef pipelineStr);

  /// Compare the option with the provided value.
  bool compare(const aiir::OpPassManager &rhs) const;
  bool compare(const GenericOptionValue &rhs) const override {
    const auto &rhsOV =
        static_cast<const OptionValue<aiir::OpPassManager> &>(rhs);
    if (!rhsOV.hasValue())
      return false;
...
private:
  ...
  std::unique_ptr<aiir::OpPassManager> value;
};
...
extern template class basic_parser<aiir::OpPassManager>;

template <>
class parser<aiir::OpPassManager> : public basic_parser<aiir::OpPassManager> {
public:
  ...
  struct ParsedPassManager {
    ...
    ~ParsedPassManager();
    operator const aiir::OpPassManager &() const {
  ...
  using OptVal = OptionValue<aiir::OpPassManager>;
  ...
  static void print(raw_ostream &os, const aiir::OpPassManager &value);

  // Overload in subclass to provide a better default value.
  StringRef getValueName() const override { return "pass-manager"; }

  void printOptionDiff(const Option &opt, aiir::OpPassManager &pm,
...
#endif // AIIR_PASS_PASSOPTIONS_H_
