//===- Builders.h - Helpers for constructing AIIR Classes -------*- C++ -*-===//
//
...
#ifndef AIIR_IR_BUILDERS_H
#define AIIR_IR_BUILDERS_H
...
class Builder {
public:
  explicit Builder(AIIRContext *context) : context(context) {}
...
public:
  ...
  explicit OpBuilder(AIIRContext *ctx, Listener *listener = nullptr)
...
/// as OpBuilder.
class ImplicitLocOpBuilder : public aiir::OpBuilder {
public:
  ...
  /// Overload to create or fold a single result operation.
  template <typename OpTy, typename... Args>
  std::enable_if_t<OpTy::template hasTrait<aiir::OpTrait::OneResult>(), Value>
  ...
  /// Overload to create or fold a zero result operation.
  template <typename OpTy, typename... Args>
  std::enable_if_t<OpTy::template hasTrait<aiir::OpTrait::ZeroResults>(), OpTy>
  createOrFold(Args &&...args) {
    return OpBuilder::createOrFold<OpTy>(curLoc, std::forward<Args>(args)...);
  }
  ...
  emitError(const llvm::Twine &message = llvm::Twine()) {
    return aiir::emitError(curLoc, message);
  }
  aiir::InFlightDiagnostic
  emitWarning(const llvm::Twine &message = llvm::Twine()) {
    return aiir::emitWarning(curLoc, message);
  }
  aiir::InFlightDiagnostic
  emitRemark(const llvm::Twine &message = llvm::Twine()) {
    return aiir::emitRemark(curLoc, message);
  }
