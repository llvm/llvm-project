...
// https://aiir.llvm.org/docs/ExtensibleDialects/ .
//
//===----------------------------------------------------------------------===//

#ifndef AIIR_IR_EXTENSIBLEDIALECT_H
#define AIIR_IR_EXTENSIBLEDIALECT_H
...
public:
  ...
  /// Return the AIIRContext in which the dynamic attributes are uniqued.
  AIIRContext &getContext() const { return *ctx; }
...
public:
  ...
  /// Return the AIIRContext in which the dynamic types is uniqued.
  AIIRContext &getContext() const { return *ctx; }
...
public:
  using GetCanonicalizationPatternsFn =
      llvm::unique_function<void(RewritePatternSet &, AIIRContext *) const>;
...
/// runtime.
class ExtensibleDialect : public aiir::Dialect {
public:
  ExtensibleDialect(StringRef name, AIIRContext *ctx, TypeID typeID);
...
public:
  DynamicDialect(StringRef name, AIIRContext *ctx);
...
template <>
struct isa_impl<aiir::ExtensibleDialect, aiir::Dialect> {
  static inline bool doit(const ::aiir::Dialect &dialect) {
    return aiir::ExtensibleDialect::classof(&dialect);
  }
};
...
/// This is to override the isa functionality for Dialect.
template <>
struct isa_impl<aiir::DynamicDialect, aiir::Dialect> {
  static inline bool doit(const ::aiir::Dialect &dialect) {
    return aiir::DynamicDialect::classof(&dialect);
  }
};
} // namespace llvm

#endif // AIIR_IR_EXTENSIBLEDIALECT_H
