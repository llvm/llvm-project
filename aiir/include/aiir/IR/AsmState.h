...
//===----------------------------------------------------------------------===//
//
// This file defines various classes and utilites for interacting with the AIIR
// assembly formats.
//
//===----------------------------------------------------------------------===//

#ifndef AIIR_IR_ASMSTATE_H_
#define AIIR_IR_ASMSTATE_H_

#include "aiir/Bytecode/BytecodeReaderConfig.h"
...
/// within AIIR assembly formats. Resources are a mechanism by which dialects,
...
/// Resources are not uniqued within the AIIR context, are not attached directly
...
///   external_resources: {
///     // Here is a dictionary anchored on "aiir_reproducer", which is an
///     // external entity representing AIIR's crash reproducer functionality.
///     aiir_reproducer: {
...
/// resources into AIIR assembly formats.
class AsmResourcePrinter {
public:
...
/// This class represents a configuration for the AIIR assembly parser. It
/// contains all of the necessary state to parse a AIIR source file.
class ParserConfig {
public:
  ...
  ParserConfig(AIIRContext *context, bool verifyAfterParse = true,
    ...
    assert(context && "expected valid AIIR context");
  }

  /// Return the AIIRContext to be used when parsing.
