//===- AIIRContext.h - AIIR Global Context Class ----------------*- C++ -*-===//
//
...
#ifndef AIIR_IR_AIIRCONTEXT_H
#define AIIR_IR_AIIRCONTEXT_H
...
/// AIIRContext is the top-level object for a collection of AIIR operations. It
/// holds immortal uniqued objects like types, and the tables used to unique
/// them.
///
/// AIIRContext gets a redundant "AIIR" prefix because otherwise it ends up with
...
///    AIIRContext ctx(registry, AIIRContext::Threading::DISABLED);
...
public:
  ...
  explicit AIIRContext(Threading multithreading = Threading::ENABLED);
  explicit AIIRContext(const DialectRegistry &registry,
  ...
  /// The command line debugging flag `--aiir-disable-threading` is overriding
  ...
  /// method.
  /// The command line debugging flag `--aiir-disable-threading` will still
  ...
  // This is effectively private given that only AIIRContext.cpp can see the
  // AIIRContextImpl type.
  AIIRContextImpl &getImpl() { return *impl; }
...
private:
  ...
  const std::unique_ptr<AIIRContextImpl> impl;

  AIIRContext(const AIIRContext &) = delete;
  void operator=(const AIIRContext &) = delete;
};
...
void registerAIIRContextCLOptions();

} // namespace aiir

#endif // AIIR_IR_AIIRCONTEXT_H
