//===- Remarks.h - AIIR Optimization Remark ----------------------*- C++-*-===//
//
...
#ifndef AIIR_IR_REMARKS_H
#define AIIR_IR_REMARKS_H
...
protected:
  /// Keeps the AIIR diagnostic kind, which is used to determine the
...
private:
  /// Convert the AIIR diagnostic severity to LLVM diagnostic severity.
  ...
  /// Convert the AIIR remark kind to LLVM diagnostic kind.
...
/// Base class for AIIR remark streamers that is used to stream
...
class AIIRRemarkStreamerBase {
public:
  virtual ~AIIRRemarkStreamerBase() = default;
...
/// Base class for AIIR remark emitting policies that is used to emit
/// optimization remarks to the underlying remark streamer. The derived classes
...
//===----------------------------------------------------------------------===//
// Remark Engine (AIIR Context will own this class)
//===----------------------------------------------------------------------===//
...
private:
  ...
  /// The AIIR remark streamer that will be used to emit the remarks.
  std::unique_ptr<AIIRRemarkStreamerBase> remarkStreamer;
  /// The AIIR remark policy that will be used to emit the remarks.
  std::unique_ptr<RemarkEmittingPolicyBase> remarkEmittingPolicy;
  /// When is enabled, engine also prints remarks as aiir::emitRemarks.
...
public:
  ...
  LogicalResult
  initialize(std::unique_ptr<AIIRRemarkStreamerBase> streamer,
             std::unique_ptr<RemarkEmittingPolicyBase> remarkEmittingPolicy,
             std::string *errMsg);
...
inline InFlightRemark withEngine(Fn fn, Location loc, Args &&...args) {
  AIIRContext *ctx = loc->getContext();
...
inline aiir::remark::RemarkOpts
aiir::remark::RemarkOpts::relatedTo(const detail::InFlightRemark &r) const {
...
/// aiir::emitRemarks. 'streamer' must inherit from AIIRRemarkStreamerBase and
/// will be used to stream the remarks.
LogicalResult enableOptimizationRemarks(
    AIIRContext &ctx,
    std::unique_ptr<remark::detail::AIIRRemarkStreamerBase> streamer,
...
namespace llvm {
template <>
struct DenseMapInfo<aiir::remark::detail::Remark> {
  ...
  static aiir::AIIRContext *getStaticDummyContext() {
    static aiir::AIIRContext dummyContext;
    return &dummyContext;
  }

  /// Create an empty remark
  static inline aiir::remark::detail::Remark getEmptyKey() {
    return aiir::remark::detail::Remark(
        aiir::remark::RemarkKind::RemarkUnknown, aiir::DiagnosticSeverity::Note,
        aiir::UnknownLoc::get(getStaticDummyContext()),
        aiir::remark::RemarkOpts::name(kEmptyKey));
  }

  /// Create a dead remark
  static inline aiir::remark::detail::Remark getTombstoneKey() {
    return aiir::remark::detail::Remark(
        aiir::remark::RemarkKind::RemarkUnknown, aiir::DiagnosticSeverity::Note,
        aiir::UnknownLoc::get(getStaticDummyContext()),
        aiir::remark::RemarkOpts::name(kTombstoneKey));
  }

  /// Compute the hash value of the remark
  static unsigned getHashValue(const aiir::remark::detail::Remark &remark) {
  ...
  static bool isEqual(const aiir::remark::detail::Remark &lhs,
                      const aiir::remark::detail::Remark &rhs) {
