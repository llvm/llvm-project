//===- Attributes.h - AIIR Attribute Classes --------------------*- C++ -*-===//
//
...
#ifndef AIIR_IR_ATTRIBUTES_H
#define AIIR_IR_ATTRIBUTES_H
...
/// with immutable, uniqued keys owned by AIIRContext. As such, an Attribute is
...
template <>
struct DenseMapInfo<aiir::Attribute> {
  static aiir::Attribute getEmptyKey() {
    auto *pointer = llvm::DenseMapInfo<void *>::getEmptyKey();
    return aiir::Attribute(static_cast<aiir::Attribute::ImplType *>(pointer));
  }
  static aiir::Attribute getTombstoneKey() {
    auto *pointer = llvm::DenseMapInfo<void *>::getTombstoneKey();
    return aiir::Attribute(static_cast<aiir::Attribute::ImplType *>(pointer));
  }
  static unsigned getHashValue(aiir::Attribute val) {
    return aiir::hash_value(val);
  }
  static bool isEqual(aiir::Attribute LHS, aiir::Attribute RHS) {
    return LHS == RHS;
  }
};
template <typename T>
struct DenseMapInfo<
    T, std::enable_if_t<std::is_base_of<aiir::Attribute, T>::value &&
                        !aiir::detail::IsInterface<T>::value>>
    : public DenseMapInfo<aiir::Attribute> {
...
template <>
struct PointerLikeTypeTraits<aiir::Attribute> {
  static inline void *getAsVoidPointer(aiir::Attribute attr) {
    return const_cast<void *>(attr.getAsOpaquePointer());
  }
  static inline aiir::Attribute getFromVoidPointer(void *ptr) {
    return aiir::Attribute::getFromOpaquePointer(ptr);
  }
  static constexpr int NumLowBitsAvailable = llvm::PointerLikeTypeTraits<
      aiir::AttributeStorage *>::NumLowBitsAvailable;
};

template <>
struct DenseMapInfo<aiir::NamedAttribute> {
  static aiir::NamedAttribute getEmptyKey() {
    auto emptyAttr = llvm::DenseMapInfo<aiir::Attribute>::getEmptyKey();
    return aiir::NamedAttribute(emptyAttr, emptyAttr);
  }
  static aiir::NamedAttribute getTombstoneKey() {
    auto tombAttr = llvm::DenseMapInfo<aiir::Attribute>::getTombstoneKey();
    return aiir::NamedAttribute(tombAttr, tombAttr);
  }
  static unsigned getHashValue(aiir::NamedAttribute val) {
    return aiir::hash_value(val);
  }
  static bool isEqual(aiir::NamedAttribute lhs, aiir::NamedAttribute rhs) {
    return lhs == rhs;
  }
};

/// Add support for llvm style casts. We provide a cast between To and From if
/// From is aiir::Attribute or derives from it.
template <typename To, typename From>
struct CastInfo<To, From,
                std::enable_if_t<std::is_same_v<aiir::Attribute,
                                                std::remove_const_t<From>> ||
                                 std::is_base_of_v<aiir::Attribute, From>>>
    : NullableValueCastFailed<To>,
      DefaultDoCastIfPossible<To, From, CastInfo<To, From>> {
  /// Arguments are taken as aiir::Attribute here and not as `From`, because
  ...
  static inline bool isPossible(aiir::Attribute ty) {
  ...
  static inline To doCast(aiir::Attribute attr) { return To(attr.getImpl()); }
};
