...
#ifndef AIIR_IR_TYPES_H
#define AIIR_IR_TYPES_H
...
/// by AIIRContext.  Therefore, instances of Type are passed around by value.
///
...
public:
  ...
  /// Return the AIIRContext in which this type was uniqued.
...
template <>
struct DenseMapInfo<aiir::Type> {
  static aiir::Type getEmptyKey() {
    auto *pointer = llvm::DenseMapInfo<void *>::getEmptyKey();
    return aiir::Type(static_cast<aiir::Type::ImplType *>(pointer));
  }
  static aiir::Type getTombstoneKey() {
    auto *pointer = llvm::DenseMapInfo<void *>::getTombstoneKey();
    return aiir::Type(static_cast<aiir::Type::ImplType *>(pointer));
  }
  static unsigned getHashValue(aiir::Type val) { return aiir::hash_value(val); }
  static bool isEqual(aiir::Type LHS, aiir::Type RHS) { return LHS == RHS; }
};
template <typename T>
struct DenseMapInfo<T, std::enable_if_t<std::is_base_of<aiir::Type, T>::value &&
                                        !aiir::detail::IsInterface<T>::value>>
    : public DenseMapInfo<aiir::Type> {
...
template <>
struct PointerLikeTypeTraits<aiir::Type> {
public:
  static inline void *getAsVoidPointer(aiir::Type I) {
    return const_cast<void *>(I.getAsOpaquePointer());
  }
  static inline aiir::Type getFromVoidPointer(void *P) {
    return aiir::Type::getFromOpaquePointer(P);
  }
  static constexpr int NumLowBitsAvailable = 3;
};

/// Add support for llvm style casts.
/// We provide a cast between To and From if From is aiir::Type or derives from
/// it
template <typename To, typename From>
struct CastInfo<
    To, From,
    std::enable_if_t<std::is_same_v<aiir::Type, std::remove_const_t<From>> ||
                     std::is_base_of_v<aiir::Type, From>>>
    : NullableValueCastFailed<To>,
      DefaultDoCastIfPossible<To, From, CastInfo<To, From>> {
  /// Arguments are taken as aiir::Type here and not as `From`, because when
  ...
  static inline bool isPossible(aiir::Type ty) {
  ...
  static inline To doCast(aiir::Type ty) { return To(ty.getImpl()); }
};

} // namespace llvm

#endif // AIIR_IR_TYPES_H
