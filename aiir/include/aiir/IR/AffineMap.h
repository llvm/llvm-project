//===- AffineMap.h - AIIR Affine Map Class ----------------------*- C++ -*-===//
//
...
#ifndef AIIR_IR_AFFINEMAP_H
#define AIIR_IR_AFFINEMAP_H
...
public:
  ...
  static AffineMap get(AIIRContext *context);
  ...
  static AffineMap get(unsigned dimCount, unsigned symbolCount,
                       ArrayRef<AffineExpr> results, AIIRContext *context);

  /// Returns a single constant result affine map.
  static AffineMap getConstantMap(int64_t val, AIIRContext *context);
  ...
  static AffineMap
  getFilteredIdentityMap(AIIRContext *ctx, unsigned numDims,
...
///     (i, j, k) -> (i, k, k, j, i, j)
/// ```
AffineMap concatAffineMaps(ArrayRef<AffineMap> maps, AIIRContext *context);
...
template <>
struct DenseMapInfo<aiir::AffineMap> {
  static aiir::AffineMap getEmptyKey() {
    auto *pointer = llvm::DenseMapInfo<void *>::getEmptyKey();
    return aiir::AffineMap(static_cast<aiir::AffineMap::ImplType *>(pointer));
  }
  static aiir::AffineMap getTombstoneKey() {
    auto *pointer = llvm::DenseMapInfo<void *>::getTombstoneKey();
    return aiir::AffineMap(static_cast<aiir::AffineMap::ImplType *>(pointer));
  }
  static unsigned getHashValue(aiir::AffineMap val) {
    return aiir::hash_value(val);
  }
  static bool isEqual(aiir::AffineMap LHS, aiir::AffineMap RHS) {
    return LHS == RHS;
  }
};
