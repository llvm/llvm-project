//===- Threading.h - AIIR Threading Utilities -------------------*- C++ -*-===//
//
...
//===----------------------------------------------------------------------===//
//
// This file defines various utilies for multithreaded processing within AIIR.
...
#ifndef AIIR_IR_THREADING_H
#define AIIR_IR_THREADING_H
...
LogicalResult failableParallelForEach(AIIRContext *context, IteratorT begin,
...
LogicalResult failableParallelForEach(AIIRContext *context, RangeT &&range,
                                      FuncT &&func) {
...
LogicalResult failableParallelForEachN(AIIRContext *context, size_t begin,
...
/// sequentially.
template <typename IteratorT, typename FuncT>
void parallelForEach(AIIRContext *context, IteratorT begin, IteratorT end,
                     FuncT &&func) {
...
/// sequentially.
template <typename RangeT, typename FuncT>
void parallelForEach(AIIRContext *context, RangeT &&range, FuncT &&func) {
...
/// sequentially.
template <typename FuncT>
void parallelFor(AIIRContext *context, size_t begin, size_t end, FuncT &&func) {
  parallelForEach(context, llvm::seq(begin, end), std::forward<FuncT>(func));
}

} // namespace aiir

#endif // AIIR_IR_THREADING_H
