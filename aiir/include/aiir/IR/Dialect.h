...
#ifndef AIIR_IR_DIALECT_H
#define AIIR_IR_DIALECT_H
...
/// Dialects are groups of AIIR operations, types and attributes, as well as
...
/// printing, etc.
///
/// Instances of the dialect object are loaded in a specific AIIRContext.
///
class Dialect {
public:
  ...
  AIIRContext *getContext() const { return context; }
...
protected:
  ...
  Dialect(StringRef name, AIIRContext *context, TypeID id);
...
template <typename T>
struct isa_impl<T, ::aiir::Dialect,
                std::enable_if_t<std::is_base_of<::aiir::Dialect, T>::value>> {
  static inline bool doit(const ::aiir::Dialect &dialect) {
    return aiir::TypeID::get<T>() == dialect.getTypeID();
  }
};
template <typename T>
struct isa_impl<
    T, ::aiir::Dialect,
    std::enable_if_t<std::is_base_of<::aiir::DialectInterface, T>::value>> {
  static inline bool doit(const ::aiir::Dialect &dialect) {
    return const_cast<::aiir::Dialect &>(dialect).getRegisteredInterface<T>();
  }
};
template <typename T>
struct cast_retty_impl<T, ::aiir::Dialect *> {
  using ret_type = T *;
};
template <typename T>
struct cast_retty_impl<T, ::aiir::Dialect> {
...
template <typename T>
struct cast_convert_val<T, ::aiir::Dialect, ::aiir::Dialect> {
  template <typename To>
  static std::enable_if_t<std::is_base_of<::aiir::Dialect, To>::value, To &>
  doitImpl(::aiir::Dialect &dialect) {
    return static_cast<To &>(dialect);
  }
  template <typename To>
  static std::enable_if_t<std::is_base_of<::aiir::DialectInterface, To>::value,
                          To &>
  doitImpl(::aiir::Dialect &dialect) {
    return *dialect.getRegisteredInterface<To>();
  }

  static auto &doit(::aiir::Dialect &dialect) { return doitImpl<T>(dialect); }
};
template <class T>
struct cast_convert_val<T, ::aiir::Dialect *, ::aiir::Dialect *> {
  static auto doit(::aiir::Dialect *dialect) {
    return &cast_convert_val<T, ::aiir::Dialect, ::aiir::Dialect>::doit(
        *dialect);
  }
};
