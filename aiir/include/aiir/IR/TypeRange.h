...
#ifndef AIIR_IR_TYPERANGE_H
#define AIIR_IR_TYPERANGE_H
...
template <>
struct DenseMapInfo<aiir::TypeRange> {
  static aiir::TypeRange getEmptyKey() {
    return aiir::TypeRange(getEmptyKeyPointer(), 0);
  }

  static aiir::TypeRange getTombstoneKey() {
    return aiir::TypeRange(getTombstoneKeyPointer(), 0);
  }

  static unsigned getHashValue(aiir::TypeRange val) { return hash_value(val); }

  static bool isEqual(aiir::TypeRange lhs, aiir::TypeRange rhs) {
    if (isEmptyKey(rhs))
...
private:
  static const aiir::Type *getEmptyKeyPointer() {
    return DenseMapInfo<aiir::Type *>::getEmptyKey();
  }

  static const aiir::Type *getTombstoneKeyPointer() {
    return DenseMapInfo<aiir::Type *>::getTombstoneKey();
  }

  static bool isEmptyKey(aiir::TypeRange range) {
    if (const auto *type =
            llvm::dyn_cast_if_present<const aiir::Type *>(range.getBase()))
      return type == getEmptyKeyPointer();
    return false;
  }

  static bool isTombstoneKey(aiir::TypeRange range) {
    if (const auto *type =
            llvm::dyn_cast_if_present<const aiir::Type *>(range.getBase()))
