//===- Diagnostics.h - AIIR Diagnostics -------------------------*- C++ -*-===//
//
...
#ifndef AIIR_IR_DIAGNOSTICS_H
#define AIIR_IR_DIAGNOSTICS_H
...
/// instead interfaced with via an AIIRContext instance.
class DiagnosticEngine {
public:
  ~DiagnosticEngine();

  // Diagnostic handler registration and use. AIIR supports the ability for the
  ...
  //
  // Tools using AIIR are encouraged to register error handlers and define a
  ...
  /// The handler type for AIIR diagnostics. This function takes a diagnostic as
...
public:
  explicit ScopedDiagnosticHandler(AIIRContext *ctx) : handlerID(0), ctx(ctx) {}
  template <typename FuncTy>
  ScopedDiagnosticHandler(AIIRContext *ctx, FuncTy &&handler)
      : handlerID(0), ctx(ctx) {
...
public:
  ...
  SourceMgrDiagnosticHandler(llvm::SourceMgr &mgr, AIIRContext *ctx,
                             raw_ostream &os,
...
public:
  ...
  SourceMgrDiagnosticVerifierHandler(llvm::SourceMgr &srcMgr, AIIRContext *ctx,
                                     raw_ostream &out,
  ...
  void registerInContext(AIIRContext *ctx);
...
public:
  ParallelDiagnosticHandler(AIIRContext *ctx);
  ~ParallelDiagnosticHandler();
