...
#ifndef AIIR_IR_ATTRTYPESUBELEMENTS_H
#define AIIR_IR_ATTRTYPESUBELEMENTS_H

#include "aiir/IR/AIIRContext.h"
#include "aiir/IR/Visitors.h"
#include "aiir/Support/CyclicReplacerCache.h"
...
/// values.
template <typename T, typename... Ts>
auto constructSubElementReplacement(AIIRContext *ctx, Ts &&...params) {
  ...
  } else if constexpr (llvm::is_detected<has_get_method, T, AIIRContext *,
                                         Ts...>::value) {
...
auto replaceImmediateSubElementsImpl(T derived, ArrayRef<Attribute> &replAttrs,
  ...
  if constexpr (llvm::is_detected<has_get_as_key, ImplT>::value) {
    ...
    if constexpr (!has_sub_attr_or_type_v<decltype(key)>) {
      ...
      auto buildReplacement = [&](auto newKey, AIIRContext *ctx) {
      ...
      AIIRContext *ctx = derived.getContext();
...
#endif // AIIR_IR_ATTRTYPESUBELEMENTS_H
