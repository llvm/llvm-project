//===- AffineExpr.h - AIIR Affine Expr Class --------------------*- C++ -*-===//
//
...
#ifndef AIIR_IR_AFFINEEXPR_H
#define AIIR_IR_AFFINEEXPR_H
...
AffineExpr getAffineDimExpr(unsigned position, AIIRContext *context);
AffineExpr getAffineSymbolExpr(unsigned position, AIIRContext *context);
AffineExpr getAffineConstantExpr(int64_t constant, AIIRContext *context);
...
namespace detail {
template <int N>
void bindDims(AIIRContext *ctx) {}

template <int N, typename AffineExprTy, typename... AffineExprTy2>
void bindDims(AIIRContext *ctx, AffineExprTy &e, AffineExprTy2 &...exprs) {
  e = getAffineDimExpr(N, ctx);
  bindDims<N + 1, AffineExprTy2 &...>(ctx, exprs...);
}

template <int N>
void bindSymbols(AIIRContext *ctx) {}

template <int N, typename AffineExprTy, typename... AffineExprTy2>
void bindSymbols(AIIRContext *ctx, AffineExprTy &e, AffineExprTy2 &...exprs) {
...
void bindDims(AIIRContext *ctx, AffineExprTy &...exprs) {
  detail::bindDims<0>(ctx, exprs...);
}

template <typename AffineExprTy>
void bindDimsList(AIIRContext *ctx, MutableArrayRef<AffineExprTy> exprs) {
  int idx = 0;
...
///   [0 .. sizeof...(exprs)]
template <typename... AffineExprTy>
void bindSymbols(AIIRContext *ctx, AffineExprTy &...exprs) {
  detail::bindSymbols<0>(ctx, exprs...);
}

template <typename AffineExprTy>
void bindSymbolsList(AIIRContext *ctx, MutableArrayRef<AffineExprTy> exprs) {
  int idx = 0;
  for (AffineExprTy &e : exprs)
...
template <>
struct DenseMapInfo<aiir::AffineExpr> {
  static aiir::AffineExpr getEmptyKey() {
    auto *pointer = llvm::DenseMapInfo<void *>::getEmptyKey();
    return aiir::AffineExpr(static_cast<aiir::AffineExpr::ImplType *>(pointer));
  }
  static aiir::AffineExpr getTombstoneKey() {
    auto *pointer = llvm::DenseMapInfo<void *>::getTombstoneKey();
    return aiir::AffineExpr(static_cast<aiir::AffineExpr::ImplType *>(pointer));
  }
  static unsigned getHashValue(aiir::AffineExpr val) {
    return aiir::hash_value(val);
  }
  static bool isEqual(aiir::AffineExpr LHS, aiir::AffineExpr RHS) {
    return LHS == RHS;
  }
};

/// Add support for llvm style casts. We provide a cast between To and From if
/// From is aiir::AffineExpr or derives from it.
template <typename To, typename From>
struct CastInfo<To, From,
                std::enable_if_t<std::is_same_v<aiir::AffineExpr,
                                                std::remove_const_t<From>> ||
                                 std::is_base_of_v<aiir::AffineExpr, From>>>
    : NullableValueCastFailed<To>,
      DefaultDoCastIfPossible<To, From, CastInfo<To, From>> {

  static inline bool isPossible(aiir::AffineExpr expr) {
    /// Return a constant true instead of a dynamic true when casting to self or
    /// up the hierarchy.
    if constexpr (std::is_base_of_v<To, From>) {
      return true;
    } else {
      if constexpr (std::is_same_v<To, ::aiir::AffineBinaryOpExpr>)
        return expr.getKind() <= ::aiir::AffineExprKind::LAST_AFFINE_BINARY_OP;
      if constexpr (std::is_same_v<To, ::aiir::AffineDimExpr>)
        return expr.getKind() == ::aiir::AffineExprKind::DimId;
      if constexpr (std::is_same_v<To, ::aiir::AffineSymbolExpr>)
        return expr.getKind() == ::aiir::AffineExprKind::SymbolId;
      if constexpr (std::is_same_v<To, ::aiir::AffineConstantExpr>)
        return expr.getKind() == ::aiir::AffineExprKind::Constant;
    }
  }
  static inline To doCast(aiir::AffineExpr expr) { return To(expr.getImpl()); }
};

} // namespace llvm

#endif // AIIR_IR_AFFINEEXPR_H
