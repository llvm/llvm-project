...
#ifndef AIIR_IR_DIALECTREGISTRY_H
#define AIIR_IR_DIALECTREGISTRY_H
...
using DialectAllocatorFunction = std::function<Dialect *(AIIRContext *)>;
using DialectAllocatorFunctionRef = function_ref<Dialect *(AIIRContext *)>;
using DynamicDialectPopulationFunction =
    std::function<void(AIIRContext *, DynamicDialect *)>;
...
public:
  ...
  virtual void apply(AIIRContext *context,
...
public:
  ...
  virtual void apply(AIIRContext *context, DialectsT *...dialects) const = 0;
...
public:
  ...
  void insert() {
    insert(TypeID::get<ConcreteDialect>(),
           ...
           static_cast<DialectAllocatorFunction>(([](AIIRContext *ctx) {
  ...
  /// calling AIIRContext::getOrLoadDialect in order for the context to take
  ...
  /// expected to call this directly.
  void applyExtensions(AIIRContext *ctx) const;
  ...
  ///  registry.addExtension(+[](AIIRContext *ctx, MyDialect *dialect) {
  ///  ... })
  ///
  /// is equivalent to:
  ///  registry.addExtension<MyDialect>(
  ///     [](AIIRContext *ctx, MyDialect *dialect){ ... }
  ///  )
  template <typename... DialectsT>
  bool addExtension(void (*extensionFn)(AIIRContext *, DialectsT *...)) {
    using ExtensionFnT = void (*)(AIIRContext *, DialectsT *...);
    ...
    struct Extension : public DialectExtension<Extension, DialectsT...> {
      ...
      void apply(AIIRContext *context, DialectsT *...dialects) const final {
...
#endif // AIIR_IR_DIALECTREGISTRY_H
