...
// must be derived from StateStackFrameBase and implement AIIR TypeID.
//
//===----------------------------------------------------------------------===//

#ifndef AIIR_SUPPORT_STACKFRAME_H
#define AIIR_SUPPORT_STACKFRAME_H
...
public:
  ...
  T *getStackTop() {
    T *top = nullptr;
    stackWalk<T>([&](T &frame) -> aiir::WalkResult {
      top = &frame;
...
namespace llvm {
template <typename T>
struct isa_impl<T, ::aiir::StateStackFrame> {
  static inline bool doit(const ::aiir::StateStackFrame &frame) {
    return frame.getTypeID() == ::aiir::TypeID::get<T>();
  }
};
} // namespace llvm

#endif // AIIR_SUPPORT_STACKFRAME_H
