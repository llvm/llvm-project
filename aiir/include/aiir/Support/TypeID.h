...
#ifndef AIIR_SUPPORT_TYPEID_H
#define AIIR_SUPPORT_TYPEID_H
...
///
///  * Explicitly via `AIIR_DECLARE_EXPLICIT_TYPE_ID` and
///    `AIIR_DEFINE_EXPLICIT_TYPE_ID`
///
...
///
///        // Foo.h
///        AIIR_DECLARE_EXPLICIT_TYPE_ID(Foo);
///
///        // Foo.cpp
///        AIIR_DEFINE_EXPLICIT_TYPE_ID(Foo);
///
///  * Explicitly via `AIIR_DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID`
...
///       class Foo {
///       public:
///         AIIR_DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID(Foo)
///       };
///       } // namespace
///
...
///     by specifying `AIIR_USE_FALLBACK_TYPE_IDS`.
///
class TypeID {
...
#define AIIR_DECLARE_EXPLICIT_SELF_OWNING_TYPE_ID(CLASS_NAME)                  \
  namespace aiir {                                                             \
  namespace detail {                                                           \
  template <>                                                                  \
...
#define AIIR_DEFINE_EXPLICIT_SELF_OWNING_TYPE_ID(CLASS_NAME)                   \
  namespace aiir {                                                             \
  namespace detail {                                                           \
...
#define AIIR_DECLARE_EXPLICIT_FALLBACK_TYPE_ID(CLASS_NAME)                     \
  namespace aiir {                                                             \
  namespace detail {                                                           \
  template <>                                                                  \
...
#define AIIR_DEFINE_EXPLICIT_FALLBACK_TYPE_ID(CLASS_NAME)


#ifndef AIIR_USE_FALLBACK_TYPE_IDS
#define AIIR_USE_FALLBACK_TYPE_IDS false
#endif

#if AIIR_USE_FALLBACK_TYPE_IDS
#define AIIR_DECLARE_EXPLICIT_TYPE_ID(CLASS_NAME)                              \
  AIIR_DECLARE_EXPLICIT_FALLBACK_TYPE_ID(CLASS_NAME)
#define AIIR_DEFINE_EXPLICIT_TYPE_ID(CLASS_NAME)                               \
  AIIR_DEFINE_EXPLICIT_FALLBACK_TYPE_ID(CLASS_NAME)
#else
#define AIIR_DECLARE_EXPLICIT_TYPE_ID(CLASS_NAME)                              \
  AIIR_DECLARE_EXPLICIT_SELF_OWNING_TYPE_ID(CLASS_NAME)
#define AIIR_DEFINE_EXPLICIT_TYPE_ID(CLASS_NAME)                               \
  AIIR_DEFINE_EXPLICIT_SELF_OWNING_TYPE_ID(CLASS_NAME)
#endif /* AIIR_USE_FALLBACK_TYPE_IDS */
...
#define AIIR_DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID(CLASS_NAME)               \
  static ::aiir::TypeID resolveTypeID() {                                      \
    static ::aiir::SelfOwningTypeID id;                                        \
    return id;                                                                 \
  }                                                                            \
  static_assert(                                                               \
      ::aiir::detail::InlineTypeIDResolver::has_resolve_typeid<                \
          CLASS_NAME>::value,                                                  \
      "`AIIR_DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID` must be placed in a "    \
...
AIIR_DECLARE_EXPLICIT_SELF_OWNING_TYPE_ID(void)

namespace llvm {
template <>
struct DenseMapInfo<aiir::TypeID> {
  static inline aiir::TypeID getEmptyKey() {
    void *pointer = llvm::DenseMapInfo<void *>::getEmptyKey();
    return aiir::TypeID::getFromOpaquePointer(pointer);
  }
  static inline aiir::TypeID getTombstoneKey() {
    void *pointer = llvm::DenseMapInfo<void *>::getTombstoneKey();
    return aiir::TypeID::getFromOpaquePointer(pointer);
  }
  static unsigned getHashValue(aiir::TypeID val) {
    return aiir::hash_value(val);
  }
  static bool isEqual(aiir::TypeID lhs, aiir::TypeID rhs) { return lhs == rhs; }
};

/// We align TypeID::Storage by 8, so allow LLVM to steal the low bits.
template <>
struct PointerLikeTypeTraits<aiir::TypeID> {
  static inline void *getAsVoidPointer(aiir::TypeID info) {
    return const_cast<void *>(info.getAsOpaquePointer());
  }
  static inline aiir::TypeID getFromVoidPointer(void *ptr) {
    return aiir::TypeID::getFromOpaquePointer(ptr);
  }
