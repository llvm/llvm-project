...
#ifndef AIIR_TARGET_LLVMIR_LLVMIMPORTINTERFACE_H
#define AIIR_TARGET_LLVMIR_LLVMIMPORTINTERFACE_H
...
#include "aiir/Target/LLVMIR/LLVMImportDialectInterface.h.inc"
...
public:
  ...
  /// Converts the LLVM intrinsic to an AIIR operation if a conversion exists.
  /// Returns failure otherwise.
  LogicalResult convertIntrinsic(OpBuilder &builder, llvm::CallInst *inst,
                                 LLVM::ModuleImport &moduleImport) const;

  /// Returns true if the given LLVM IR intrinsic is convertible to an AIIR
  /// operation.
  bool isConvertibleIntrinsic(llvm::Intrinsic::ID id) {
    return intrinsicToDialect.count(id);
  }

  /// Converts the LLVM instruction to an AIIR operation if a conversion exists.
  /// Returns failure otherwise.
  ...
  /// Returns true if the given LLVM IR instruction is convertible to an AIIR
  /// operation.
  ...
  /// to one or more AIIR dialect attributes exists and succeeds. Returns
  ...
  /// Returns true if the given LLVM IR metadata is convertible to an AIIR
  /// attribute.
...
#endif // AIIR_TARGET_LLVMIR_LLVMIMPORTINTERFACE_H
