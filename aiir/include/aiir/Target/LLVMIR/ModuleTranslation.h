//===- ModuleTranslation.h - AIIR to LLVM conversion ------------*- C++ -*-===//
//
...
#ifndef AIIR_TARGET_LLVMIR_MODULETRANSLATION_H
#define AIIR_TARGET_LLVMIR_MODULETRANSLATION_H
...
class ModuleTranslation {
  ...
  aiir::translateModuleToLLVMIR(Operation *, llvm::LLVMContext &, StringRef,
                                bool);
...
public:
  ...
  /// Stores the mapping between an AIIR value and its LLVM IR counterpart.
  void mapValue(Value aiir, llvm::Value *llvm) { mapValue(aiir) = llvm; }
  ...
  /// Finds an LLVM IR value corresponding to the given AIIR value.
  ...
  /// Stores the mapping between an AIIR block and LLVM IR basic block.
  void mapBlock(Block *aiir, llvm::BasicBlock *llvm) {
    auto result = blockMapping.try_emplace(aiir, llvm);
    (void)result;
    assert(result.second && "attempting to map a block that is already mapped");
  }

  /// Finds an LLVM IR basic block that corresponds to the given AIIR block.
  llvm::BasicBlock *lookupBlock(Block *block) const {
    return blockMapping.lookup(block);
  }

  /// Find the LLVM-IR loop that represents an AIIR loop.
  llvm::CanonicalLoopInfo *lookupOMPLoop(omp::NewCliOp aiir) const {
    llvm::CanonicalLoopInfo *result = loopMapping.lookup(aiir);
    assert(result && "attempt to get non-existing loop");
    return result;
  }

  /// Find the LLVM-IR loop that represents an AIIR loop.
  llvm::CanonicalLoopInfo *lookupOMPLoop(Value aiir) const {
    return lookupOMPLoop(aiir.getDefiningOp<omp::NewCliOp>());
  }

  /// Mark an OpenMP loop as having been consumed.
  void invalidateOmpLoop(omp::NewCliOp aiir) { loopMapping.erase(aiir); }

  /// Mark an OpenMP loop as having been consumed.
  void invalidateOmpLoop(Value aiir) {
    invalidateOmpLoop(aiir.getDefiningOp<omp::NewCliOp>());
  }

  /// Map an AIIR OpenMP dialect CanonicalLoopInfo to its lowered LLVM-IR
  /// OpenMPIRBuilder CanonicalLoopInfo
  void mapOmpLoop(omp::NewCliOp aiir, llvm::CanonicalLoopInfo *llvm) {
    assert(llvm && "argument must be non-null");
    llvm::CanonicalLoopInfo *&cur = loopMapping[aiir];
  ...
  /// Map an AIIR OpenMP dialect CanonicalLoopInfo to its lowered LLVM-IR
  /// OpenMPIRBuilder CanonicalLoopInfo
  void mapOmpLoop(Value aiir, llvm::CanonicalLoopInfo *llvm) {
    mapOmpLoop(aiir.getDefiningOp<omp::NewCliOp>(), llvm);
  }

  /// Stores the mapping between an AIIR operation with successors and a
  /// corresponding LLVM IR instruction.
  void mapBranch(Operation *aiir, llvm::Instruction *llvm) {
    auto result = branchMapping.try_emplace(aiir, llvm);
    (void)result;
    assert(result.second &&
           "attempting to map a branch that is already mapped");
  }

  /// Finds an LLVM IR instruction that corresponds to the given AIIR operation
  /// with successors.
  llvm::Instruction *lookupBranch(Operation *op) const {
    return branchMapping.lookup(op);
  }

  /// Stores a mapping between an AIIR call operation and a corresponding LLVM
  /// call instruction.
  void mapCall(Operation *aiir, llvm::CallInst *llvm) {
    auto result = callMapping.try_emplace(aiir, llvm);
    (void)result;
    assert(result.second && "attempting to map a call that is already mapped");
  }

  /// Finds an LLVM call instruction that corresponds to the given AIIR call
  /// operation.
  ...
  /// Returns the LLVM metadata corresponding to a aiir LLVM dialect alias scope
  /// attribute. Creates the metadata node if it has not been converted before.
  llvm::MDNode *getOrCreateAliasScope(AliasScopeAttr aliasScopeAttr);

  /// Returns the LLVM metadata corresponding to an array of aiir LLVM dialect
  ...
  /// Converts the type from AIIR LLVM dialect to LLVM.
  llvm::Type *convertType(Type type);

  /// Returns the AIIR context of the module being translated.
  AIIRContext &getContext() { return *aiirModule->getContext(); }

  /// Returns the LLVM context in which the IR is being constructed.
  llvm::LLVMContext &getLLVMContext() const { return llvmModule->getContext(); }

  /// Finds an LLVM IR global value that corresponds to the given AIIR operation
  ...
  /// associated argument attributes in AIIR and should be skipped during
  /// attribute mapping.
  LogicalResult
...
private:
  ...
  ///
  /// - Create named global variables that correspond to llvm.aiir.global
  /// definitions, similarly Convert llvm.global_ctors and global_dtors ops.
  /// - Create global alias that correspond to llvm.aiir.alias.
  ...
  /// Returns the LLVM metadata corresponding to the given aiir LLVM dialect
  /// TBAATagAttr.
  ...
  FailureOr<llvm::AttrBuilder> convertParameterAttrs(aiir::Location loc,
  ...
  /// Mappings between llvm.aiir.global definitions and corresponding globals.
  DenseMap<Operation *, llvm::GlobalValue *> globalsMapping;

  /// Mappings between llvm.aiir.alias definitions and corresponding global
  /// aliases.
  DenseMap<Operation *, llvm::GlobalValue *> aliasesMapping;

  /// Mappings between llvm.aiir.ifunc definitions and corresponding global
  /// ifuncs.
  ...
  /// List of not yet consumed AIIR loop handles (represented by an omp.new_cli
  ...
  /// A mapping between AIIR LLVM dialect terminators and LLVM IR terminators
  ...
  /// A mapping between AIIR LLVM dialect call operations and LLVM IR call
...
/// resolves the overloads, and maps mixed AIIR value and attribute arguments to
/// LLVM values.
...
#endif // AIIR_TARGET_LLVMIR_MODULETRANSLATION_H
