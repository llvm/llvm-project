//===- DataLayoutImporter.h - LLVM to AIIR data layout conversion -*- C++ -*-=//
//
...
#ifndef AIIR_TARGET_LLVMIR_DATALAYOUTIMPORTER_H
#define AIIR_TARGET_LLVMIR_DATALAYOUTIMPORTER_H
...
/// Returns a supported AIIR floating point type of the given bit width or
/// null if the bit width is not supported.
FloatType getFloatType(AIIRContext *context, unsigned width);

/// Helper class that translates an LLVM data layout string to an AIIR data
...
public:
  DataLayoutImporter(AIIRContext *context, StringRef dataLayoutStr)
  ...
  /// Returns the AIIR data layout specification translated from the LLVM
  /// data layout.
...
private:
  /// Translate the LLVM data layout string to an AIIR data layout
  /// specification.
...
#endif // AIIR_TARGET_LLVMIR_DATALAYOUTIMPORTER_H
