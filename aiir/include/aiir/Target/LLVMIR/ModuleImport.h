//===- ModuleImport.h - LLVM to AIIR conversion -----------------*- C++ -*-===//
//
...
#ifndef AIIR_TARGET_LLVMIR_MODULEIMPORT_H
#define AIIR_TARGET_LLVMIR_MODULEIMPORT_H

#include "aiir/Dialect/LLVMIR/LLVMDialect.h"
#include "aiir/IR/BuiltinOps.h"
#include "aiir/Target/LLVMIR/Import.h"
#include "aiir/Target/LLVMIR/LLVMImportInterface.h"
...
/// and functions from an LLVM module into an AIIR module. It holds mappings
...
/// insertion point since LLVM immediate values translate to AIIR operations
/// that are introduced at the beginning of the region.
class ModuleImport {
public:
  ModuleImport(ModuleOp aiirModule, std::unique_ptr<llvm::Module> llvmModule,
  ...
  /// Converts all functions of the LLVM module to AIIR functions.
  LogicalResult convertFunctions();

  /// Converts all comdat selectors of the LLVM module to AIIR comdat
  /// operations.
  LogicalResult convertComdats();

  /// Converts all global variables of the LLVM module to AIIR global variables.
  LogicalResult convertGlobals();

  /// Converts all aliases of the LLVM module to AIIR variables.
  LogicalResult convertAliases();

  /// Converts all ifuncs of the LLVM module to AIIR variables.
  LogicalResult convertIFuncs();

  /// Converts the data layout of the LLVM module to an AIIR data layout
  /// specification.
  LogicalResult convertDataLayout();

  /// Converts target triple of the LLVM module to an AIIR target triple
  /// specification.
  void convertTargetTriple();

  /// Converts the module level asm of the LLVM module to an AIIR module
  /// level asm specification.
  void convertModuleLevelAsm();

  /// Stores the mapping between an LLVM value and its AIIR counterpart.
  void mapValue(llvm::Value *llvm, Value aiir) { mapValue(llvm) = aiir; }

  /// Provides write-once access to store the AIIR value corresponding to the
  /// given LLVM value.
  Value &mapValue(llvm::Value *value) {
    Value &aiir = valueMapping[value];
    assert(aiir == nullptr &&
           "attempting to map a value that is already mapped");
    return aiir;
  }

  /// Returns the AIIR value mapped to the given LLVM value.
  Value lookupValue(llvm::Value *value) { return valueMapping.lookup(value); }

  /// Stores a mapping between an LLVM instruction and the imported AIIR
  ...
  void mapNoResultOp(llvm::Instruction *llvm, Operation *aiir) {
    mapNoResultOp(llvm) = aiir;
  }

  /// Provides write-once access to store the AIIR operation corresponding to
  ...
  Operation *&mapNoResultOp(llvm::Instruction *inst) {
    Operation *&aiir = noResultOpMapping[inst];
  ...
  /// Returns the AIIR operation mapped to the given LLVM instruction. Queries
  ...
  /// Stores the mapping between an LLVM block and its AIIR counterpart.
  void mapBlock(llvm::BasicBlock *llvm, Block *aiir) {
    auto result = blockMapping.try_emplace(llvm, aiir);
    (void)result;
    assert(result.second && "attempting to map a block that is already mapped");
  }

  /// Returns the AIIR block mapped to the given LLVM block.
  Block *lookupBlock(llvm::BasicBlock *block) const {
    return blockMapping.lookup(block);
  }

  /// Converts an LLVM value to an AIIR value, or returns failure if the
  /// conversion fails. Uses the `convertConstant` method to translate constant
  /// LLVM values.
  FailureOr<Value> convertValue(llvm::Value *value);

  /// Converts an LLVM metadata value to an AIIR value, or returns failure if
  /// the conversion fails. Uses the `convertConstant` method to translate
  /// constant LLVM values.
  FailureOr<Value> convertMetadataValue(llvm::Value *value);

  /// Converts a range of LLVM values to a range of AIIR values using the
  ...
  /// Converts the type from LLVM to AIIR LLVM dialect.
  ...
  /// Returns the AIIR attribute mapped to the given LLVM TBAA
  /// metadata `node`.
  ...
  /// Converts the LLVM values for an intrinsic to mixed AIIR values and
  ...
  /// corresponding AIIR attribute names.
  LogicalResult
...
private:
  ...
  /// Converts an LLVM global variable into an AIIR LLVM dialect global
  ...
  /// Converts an LLVM global alias variable into an AIIR LLVM dialect alias
  /// operation if a conversion exists. Otherwise, returns failure.
  LogicalResult convertAlias(llvm::GlobalAlias *alias);
  // Converts an LLVM global ifunc into an AIIR LLVM dialect ifunc operation.
  ...
  /// function containing the intrinsics has been fully converted to AIIR.
  LogicalResult processDebugIntrinsics();
  /// Converts all debug records in `dbgRecords`. Assumes that the
  /// function containing the record has been fully converted to AIIR.
  ...
  /// AIIR attributes to be utilized in `llvm.inline_asm`.
  ArrayAttr convertAsmInlineOperandAttrs(const llvm::CallBase &llvmCall);
  /// Converts an LLVM intrinsic to an AIIR LLVM dialect operation if an AIIR
  /// counterpart exists. Otherwise, returns failure.
  LogicalResult convertIntrinsic(llvm::CallInst *inst);
  /// Converts an LLVM instruction to an AIIR LLVM dialect operation if an AIIR
  /// counterpart exists. Otherwise, returns failure.
  ...
  /// corresponding AIIR LLVM dialect attribute dictionary.
  ...
  /// Converts an LLVM constant to an AIIR value, or returns failure if the
  /// conversion fails. The AIIR value may be produced by a ConstantOp,
  ...
  /// Converts an LLVM constant and its transitive constant dependencies to AIIR
  ...
  /// Converts all LLVM alias scopes and domains starting from `node` to AIIR
  ...
  /// Converts the given LLVM comdat struct to an AIIR comdat selector operation
  /// and stores a mapping from the struct to the symbol pointing to the
  /// translated operation.
  void processComdat(const llvm::Comdat *comdat);
  /// Returns a symbol name for a nameless global. AIIR, in contrast to LLVM,
...
#endif // AIIR_TARGET_LLVMIR_MODULEIMPORT_H
