...
#ifndef AIIR_SUPPORT_NAMESPACE_H
#define AIIR_SUPPORT_NAMESPACE_H

#include "aiir/IR/BuiltinOps.h"
#include "aiir/Target/SMTLIB/SymCache.h"
...
public:
  ...
  void add(aiir::ModuleOp module) {
    assert(module->getNumRegions() == 1);
    for (auto &op : module.getBody(0)->getOperations())
      if (auto symbol = op.getAttrOfType<aiir::StringAttr>(
  ...
  void add(SymbolCache &symCache) {
    for (auto &&[attr, _] : symCache)
      if (auto strAttr = dyn_cast<aiir::StringAttr>(attr))
...
#endif // AIIR_SUPPORT_NAMESPACE_H
