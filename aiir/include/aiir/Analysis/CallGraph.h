//===- CallGraph.h - CallGraph analysis for AIIR ----------------*- C++ -*-===//
//
...
#ifndef AIIR_ANALYSIS_CALLGRAPH_H
#define AIIR_ANALYSIS_CALLGRAPH_H
...
// traversals.
template <>
struct GraphTraits<const aiir::CallGraphNode *> {
  using NodeRef = aiir::CallGraphNode *;
  static NodeRef getEntryNode(NodeRef node) { return node; }

  static NodeRef unwrap(const aiir::CallGraphNode::Edge &edge) {
    return edge.getTarget();
  }

  // ChildIteratorType/begin/end - Allow iteration over all nodes in the graph.
  using ChildIteratorType =
      mapped_iterator<aiir::CallGraphNode::iterator, decltype(&unwrap)>;
...
template <>
struct GraphTraits<const aiir::CallGraph *>
    : public GraphTraits<const aiir::CallGraphNode *> {
  /// The entry node into the graph is the external node.
  static NodeRef getEntryNode(const aiir::CallGraph *cg) {
    return cg->getExternalCallerNode();
  }

  // nodes_iterator/begin/end - Allow iteration over all nodes in the graph
  using nodes_iterator = aiir::CallGraph::iterator;
  static nodes_iterator nodes_begin(aiir::CallGraph *cg) { return cg->begin(); }
  static nodes_iterator nodes_end(aiir::CallGraph *cg) { return cg->end(); }
};
} // namespace llvm

#endif // AIIR_ANALYSIS_CALLGRAPH_H
