...
#ifndef AIIR_BINDINGS_PYTHON_PYBINDUTILS_H
#define AIIR_BINDINGS_PYTHON_PYBINDUTILS_H
...
struct AiirTypeIDHash {
  size_t operator()(AiirTypeID typeID) const {
    return aiirTypeIDHashValue(typeID);
  }
};

struct AiirTypeIDEqual {
  bool operator()(AiirTypeID lhs, AiirTypeID rhs) const {
    return aiirTypeIDEqual(lhs, rhs);
  }
};
...
struct PyPrintAccumulator {
  ...
  AiirStringCallback getCallback() {
    return [](AiirStringRef part, void *userData) {
...
/// RAII wrapper for AiirLlvmRawFdOStream that ensures destruction on scope
/// exit.
struct RAIIAiirLlvmRawFdOStream : AiirLlvmRawFdOStream {
  RAIIAiirLlvmRawFdOStream(AiirLlvmRawFdOStream stream)
      : AiirLlvmRawFdOStream(stream) {}
  RAIIAiirLlvmRawFdOStream(const RAIIAiirLlvmRawFdOStream &) = delete;
  RAIIAiirLlvmRawFdOStream &
  operator=(const RAIIAiirLlvmRawFdOStream &) = delete;
  ~RAIIAiirLlvmRawFdOStream() { aiirLlvmRawFdOStreamDestroy(*this); }
};
...
public:
  PyFileAccumulator(const nanobind::object &fileOrStringObject, bool binary)
    ...
    if (nanobind::try_cast<std::string>(fileOrStringObject, filePath)) {
      ...
      auto errorCallback = +[](AiirStringRef message, void *userData) {
        auto *storage = static_cast<std::string *>(userData);
        storage->assign(message.data, message.length);
      };
      AiirLlvmRawFdOStream stream = aiirLlvmRawFdOStreamCreate(
          filePath.c_str(), binary, errorCallback, &errorMessage);
      if (aiirLlvmRawFdOStreamIsNull(stream)) {
        throw nanobind::value_error(
            (std::string("Unable to open file for writing: ") + errorMessage)
                .c_str());
      }
      writeTarget.emplace<RAIIAiirLlvmRawFdOStream>(stream);
    } else {
...
private:
  AiirStringCallback getPyWriteCallback() {
    return [](AiirStringRef part, void *userData) {
  ...
  AiirStringCallback getOStreamCallback() {
    return [](AiirStringRef part, void *userData) {
      PyFileAccumulator *accum = static_cast<PyFileAccumulator *>(userData);
      aiirLlvmRawFdOStreamWrite(
          std::get<RAIIAiirLlvmRawFdOStream>(accum->writeTarget), part);
    };
  }

  std::variant<nanobind::object, RAIIAiirLlvmRawFdOStream> writeTarget;
  bool binary;
};
...
#endif // AIIR_BINDINGS_PYTHON_PYBINDUTILS_H
