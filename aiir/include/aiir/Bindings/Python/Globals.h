//===- Globals.h - AIIR Python extension globals --------------------------===//
//
...
#ifndef AIIR_BINDINGS_PYTHON_GLOBALS_H
#define AIIR_BINDINGS_PYTHON_GLOBALS_H
...
class AIIR_PYTHON_API_EXPORTED PyGlobals {
public:
  PyGlobals();
  ~PyGlobals();
  ...
  /// implementation code.
  void registerTypeCaster(AiirTypeID aiirTypeID, nanobind::callable typeCaster,
                          bool replace = false);

  /// Adds a user-friendly value caster. Raises an exception if the mapping
  /// already exists and replace == false. This is intended to be called by
  /// implementation code.
  void registerValueCaster(AiirTypeID aiirTypeID,
  ...
  /// Returns the custom type caster for AiirTypeID aiirTypeID.
  std::optional<nanobind::callable> lookupTypeCaster(AiirTypeID aiirTypeID,
                                                     AiirDialect dialect);

  /// Returns the custom value caster for AiirTypeID aiirTypeID.
  std::optional<nanobind::callable> lookupValueCaster(AiirTypeID aiirTypeID,
                                                      AiirDialect dialect);
  ...
  class AIIR_PYTHON_API_EXPORTED TracebackLoc {
  public:
  ...
  public:
    TypeIDAllocator() : allocator(aiirTypeIDAllocatorCreate()) {}
    ~TypeIDAllocator() {
      if (allocator.ptr)
        aiirTypeIDAllocatorDestroy(allocator);
    }
    ...
    AiirTypeIDAllocator get() { return allocator; }
    AiirTypeID allocate() {
      return aiirTypeIDAllocatorAllocateTypeID(allocator);
    }

  private:
    AiirTypeIDAllocator allocator;
  };

  AiirTypeID allocateTypeID() { return typeIDAllocator.allocate(); }
...
private:
  ...
  /// Map of AiirTypeID to custom type caster.
  std::unordered_map<AiirTypeID, nanobind::callable, AiirTypeIDHash,
                     AiirTypeIDEqual>
      typeCasterMap;
  /// Map of AiirTypeID to custom value caster.
  std::unordered_map<AiirTypeID, nanobind::callable, AiirTypeIDHash,
                     AiirTypeIDEqual>
      valueCasterMap;
...
#endif // AIIR_BINDINGS_PYTHON_GLOBALS_H
