...
Sample usage:
  ...
  # source tree).
  python -m aiir.dialects.linalg.opdsl.dump_oplib .ops.core_named_ops
...
def main(args):
    ...
    for module_name in args.modules:
        modules.append(
            importlib.import_module(module_name, package="aiir.dialects.linalg.opdsl")
        )
    for i, file_path in enumerate(args.file or []):
        modules.append(load_module_from_file(f"_aiir_eval_oplib{i}", file_path))
    for m in modules:
