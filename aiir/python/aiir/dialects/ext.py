...
def construct_instance(origin, args):
    ...
    # `origin.get` is to construct an instance of AIIR type or attribute.
    return origin.get(
        *(
            (
...
def infer_type_impl(type_) -> Callable[[], ir.Type]:
    ...
    We use callables so that AIIR contexts are not required
...
class Dialect(ir.Dialect):
    ...
    def load(
        ...
        *,
        ...
        if hasattr(cls, "_aiir_module") and not reload:
            if cls._aiir_module.context is not ir.Context.current:
                raise RuntimeError(
        ...
        cls._aiir_module = cls._emit_module()
        pm = PassManager()
        pm.add("canonicalize, cse")
        pm.run(cls._aiir_module.operation)

        irdl.load_dialects(cls._aiir_module)
