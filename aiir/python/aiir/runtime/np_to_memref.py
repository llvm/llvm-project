...
class C128(ctypes.Structure):
    """A ctype representation for AIIR's Double Complex."""

    _fields_ = [("real", ctypes.c_double), ("imag", ctypes.c_double)]


class C64(ctypes.Structure):
    """A ctype representation for AIIR's Float Complex."""

    _fields_ = [("real", ctypes.c_float), ("imag", ctypes.c_float)]


class F16(ctypes.Structure):
    """A ctype representation for AIIR's Float16."""

    _fields_ = [("f16", ctypes.c_int16)]


class BF16(ctypes.Structure):
    """A ctype representation for AIIR's BFloat16."""

    _fields_ = [("bf16", ctypes.c_int16)]


class F8E5M2(ctypes.Structure):
    """A ctype representation for AIIR's Float8E5M2."""

    _fields_ = [("f8E5M2", ctypes.c_int8)]


class F8E3M4(ctypes.Structure):
    """A ctype representation for AIIR's Float8E3M4."""

    _fields_ = [("f8E3M4", ctypes.c_int8)]


class F8E4M3(ctypes.Structure):
    """A ctype representation for AIIR's Float8E4M3."""
...
def get_ranked_memref_descriptor(nparray):
    ...
    # Numpy uses byte quantities to express strides, AIIR OTOH uses the
