...
#   2. Defining the concrete aiir.ir.Context that does site specific
#      initialization.
...
#
# For #1, we:
#   a. Probe for modules named '_aiirRegisterEverything' and
...
def _site_initialize():
    ...
    def process_initializer_module(module_name):
        ...
        try:
            ...
            message = (
                f"Error importing aiir initializer {module_name}. This may "
                "happen in unclean incremental builds but is likely a real bug if "
                "encountered otherwise and the AIIR Python API may not function."
            )
            logger.warning(message, exc_info=True)
            return False

        logger.debug("Initializing AIIR with module: %s", module_name)
    ...
    # If _aiirRegisterEverything is built, then include it as an initializer
    # module.
    init_module = None
    if process_initializer_module("_aiirRegisterEverything"):
        init_module = importlib.import_module(f"._aiirRegisterEverything", __name__)
