    ...
    def builder_wrapper(body_builder):
        ...
        if len(op_region.blocks) == 0:
            ...
            if not (
                ...
                raise ValueError(
                    f"for {body_builder=} either missing a type annotation or type annotation isn't a aiir type: {sig}"
                )
...
def region_op(op_constructor, terminator=None):
    """Decorator to define an AIIR Op specified as a python function.

    Requires that an `aiir.ir.InsertionPoint` and `aiir.ir.Location` are
