...
from aiir.dialects import nvgpu
from aiir.dialects import nvvm
from aiir.dialects import llvm
from aiir.dialects import builtin
from aiir.dialects import scf
from aiir.dialects import vector
...
AIIR_DYNAMIC = -9223372036854775808
...
class TmaDescriptorBuilder:
    ...
    def __init__(self, swizzle, l2promo, oob, interleave, tma_box_shape, memref_ty):
        self.swizzle = swizzle  # aiir.nvgpu.TensorMapSwizzleKind
        self.l2promo = l2promo  # aiir.nvgpu.TensorMapL2PromoKind
        self.oob = oob  # aiir.nvgpu.TensorMapOOBKind
        self.interleave = interleave  # aiir.nvgpu.TensorMapInterleaveKind
...
def generate_matmul_ws(
    ...
    a_elem_ty = get_aiir_ty(input_type)
    b_elem_ty = get_aiir_ty(input_type)
    c_elem_ty = get_aiir_ty(output_type)
        ...
        with ir.InsertionPoint(fop.add_entry_block()):
            ...
            with ir.InsertionPoint(launch_op.body.blocks[0]):
                ...
                dynamic_smem = gpu.dynamic_shared_memory(
                    ir.MemRefType.get((AIIR_DYNAMIC,), T.i8(), memory_space=smem_space)
                )
                ticks = c(10000000)
