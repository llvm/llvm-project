...
def generate_matmul(
    ...
    with matmulBuilder.ir.Context() as ctx, matmulBuilder.ir.Location.unknown():
        if use_warp_specialization:
            aiir_nvgpu_module = matmulBuilder.generate_matmul_ws(
                input_type,
                output_type,
                M,
                N,
            ...
            aiir_nvgpu_module = matmulBuilder.generate_matmul_multistage(
                input_type,
                output_type,
                M,
                N,
        ...
        aiir_nvgpu_module.operation.verify()

        # Save generated IR
        if saveIR:
            # print(aiir_nvgpu_module)
            original_stdout = sys.stdout
            with open("gemm.aiir", "w") as f:
                sys.stdout = f
                print(aiir_nvgpu_module)
        ...
        # Compile
        engine = compiler.compile_and_jit(aiir_nvgpu_module)
        return engine
...
def matmul(
    ...
    # Launch the AIIR generated kernel
