...
from aiir.dialects import bufferization
...
class TypeConverter:
    """Converter between NumPy types and AIIR types."""
    ...
    def __init__(self, context: ir.Context):
        ...
        #
        # Note 2: we must construct the AIIR types in the same context as the
    ...
    def dtype_to_irtype(self, dtype: np.dtype) -> ir.Type:
        """Returns the AIIR equivalent of a NumPy dtype."""
        try:
    ...
    def sctype_to_irtype(self, sctype) -> ir.Type:
        """Returns the AIIR equivalent of a NumPy scalar type."""
    ...
    def irtype_to_dtype(self, tp: ir.Type) -> np.dtype:
        """Returns the NumPy dtype equivalent of an AIIR type."""
        return np.dtype(self.irtype_to_sctype(tp))

    def irtype_to_sctype(self, tp: ir.Type):
        """Returns the NumPy scalar-type equivalent of an AIIR type."""
...
def main():
    """
    USAGE: python3 test_stress.py [raw_module.aiir [compiled_module.aiir]]

    The environment variable SUPPORT_LIB must be set to point to the
    libaiir_c_runner_utils shared library.  There are two optional
