# RUN: env SUPPORT_LIB=%aiir_cuda_runtime \
# RUN: sh -c 'if [[ "%aiir_run_cuda_sm90_tests" == "1" ]]; \
# RUN: then %PYTHON %s | FileCheck %s; \
# RUN: else export AIIR_NVDSL_PRINT_IR=1; \
...
#   1. Build AIIR function with arguments
#   2. Build AIIR GPU kernel
#   3. Print from a GPU thread
#   4. Pass arguments, JIT compile and run the AIIR function
#
# ===----------------------------------------------------------------------===//


from aiir.dialects import gpu
from tools.nvdsl import *


# 1. The decorator generates a AIIR func.func.
# Everything inside the Python function becomes the body of the func.
# The decorator also translates `alpha` to an `index` type.
@NVDSL.aiir_func
def main(alpha):
    # 2. The decorator generates a AIIR gpu.launch.
    ...
    @NVDSL.aiir_gpu_launch(grid=(1, 1, 1), block=(4, 1, 1))
    def kernel():
...
alpha = 100
# 4. The `aiir_func` decorator JIT compiles the IR and executes the AIIR function.
main(alpha)
