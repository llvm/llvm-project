...
from aiir import ir
from aiir.dialects import arith, func, gpu, memref, nvgpu, scf, nvvm
...
DUMP_ONLY = os.getenv("AIIR_NVDSL_PRINT_IR") == "1"
...
def get_aiir_func_obj_ty(inputArgs):
    args = []
...
def get_aiir_ty(arg):
    ...
    if isinstance(arg, bool):
        ...
        dtype = get_aiir_ty_from_np(arg.dtype)
...
class NVDSL:
    @staticmethod
    def aiir_gpu_launch(grid=(1, 1, 1), block=(1, 1, 1), smem=0):
        def decorator(func):
    ...
    def aiir_func(funcBody):
        ...
        def wrapper(*args, **kwargs):
            ...
            def saveIR(module):
                ...
                with open("nvdsl.aiir", "w") as f:
                    sys.stdout = f
                    print(module)
                    sys.stdout = original_stdout

            def _binary_op(lhs, rhs, op: str, predAtt="") -> "ArithValue":
                """Generate AIIR's Arith dialects binary operations."""
                rhs = const(rhs)
            ...
            class ArithValue(ir.Value):
                """Overloads operators for AIIR's Arith dialects binary operations."""
            ...
            # Generate AIIR Context and start generating IR
            with ir.Context(), ir.Location.unknown():
                types = []
                for arg in args:
                    types.append(get_aiir_ty(arg))
            ...
            # Convert input arguments to AIIR arguments
            newArgs = get_aiir_func_obj_ty(args)
