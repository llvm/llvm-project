...
#  3. Pass numpy arrays to AIIR as memref arguments
#  4. Verify AIIR program with reference computation in python
#
# ===----------------------------------------------------------------------===//


from aiir import ir
from aiir.dialects import gpu, memref
...
@NVDSL.aiir_func
def saxpy(x, y, alpha):
    # 1. Use AIIR GPU dialect to allocate and copy memory
    ...
    @NVDSL.aiir_gpu_launch(grid=(M, 1, 1), block=(N, 1, 1))
    def saxpy_kernel():
...
if os.getenv("AIIR_NVDSL_PRINT_IR") != "1":
    # 4. Verify AIIR with reference computation
