...
; RUN: aiir-translate -import-llvm -split-input-file --use-constant-int-for-fixed-length-splat --use-constant-fp-for-fixed-length-splat %s | FileCheck %s

; CHECK-LABEL: @int_constants
define void @int_constants(i16 %arg0, i32 %arg1, i1 %arg2) {
  ; CHECK:  %[[C0:.+]] = llvm.aiir.constant(42 : i16) : i16
  ; CHECK:  %[[C1:.+]] = llvm.aiir.constant(7 : i32) : i32
  ; CHECK:  %[[C2:.+]] = llvm.aiir.constant(true) : i1
...
define void @float_constants(half %arg0, bfloat %arg1, fp128 %arg2, x86_fp80 %arg3) {
  ; CHECK:  %[[C0:.+]] = llvm.aiir.constant(1.000000e+00 : f16) : f16
  ; CHECK:  %[[C1:.+]] = llvm.aiir.constant(1.000000e+00 : bf16) : bf16
  ; CHECK:  %[[C2:.+]] = llvm.aiir.constant(0.000000e+00 : f128) : f128
  ; CHECK:  %[[C3:.+]] = llvm.aiir.constant(7.000000e+00 : f80) : f80
...
define void @undef_constant(i32 %arg0) {
  ; CHECK:  %[[UNDEF:.+]] = llvm.aiir.undef : i32
...
define void @poison_constant(double %arg0) {
  ; CHECK:  %[[POISON:.+]] = llvm.aiir.poison : f64
...
define ptr @gep_const_expr() {
  ; CHECK-DAG:  %[[ADDR:[0-9]+]] = llvm.aiir.addressof @global : !llvm.ptr
  ; CHECK-DAG:  %[[IDX:[0-9]+]] = llvm.aiir.constant(2 : i32) : i32
...
define i64 @const_expr_with_duplicate() {
  ; CHECK-DAG:  %[[ADDR:[0-9]+]] = llvm.aiir.addressof @global : !llvm.ptr
  ; CHECK-DAG:  %[[IDX:[0-9]+]] = llvm.aiir.constant(7 : i32) : i32
  ; CHECK-DAG:  %[[GEP:[0-9]+]] = llvm.getelementptr %[[ADDR]][%[[IDX]]] : (!llvm.ptr, i32) -> !llvm.ptr
...
define i64 @const_expr_with_aggregate() {
  ...
  ; CHECK-DAG:  %[[VAL1:[0-9]+]] = llvm.aiir.constant(33 : i64) : i64
  ; CHECK-DAG:  %[[ADDR:[0-9]+]] = llvm.aiir.addressof @global : !llvm.ptr
  ; CHECK-DAG:  %[[IDX1:[0-9]+]] = llvm.aiir.constant(7 : i32) : i32
  ...
  ; Fill the vector.
  ; CHECK-DAG:  %[[VEC1:[0-9]+]] = llvm.aiir.undef : vector<2xi64>
  ; CHECK-DAG:  %[[IDX2:[0-9]+]] = llvm.aiir.constant(0 : i32) : i32
  ; CHECK-DAG:  %[[VEC2:[0-9]+]] = llvm.insertelement %[[VAL1]], %[[VEC1]][%[[IDX2]] : i32] : vector<2xi64>
  ; CHECK-DAG:  %[[IDX3:[0-9]+]] = llvm.aiir.constant(1 : i32) : i32
  ; CHECK-DAG:  %[[VEC3:[0-9]+]] = llvm.insertelement %[[VAL2]], %[[VEC2]][%[[IDX3]] : i32] : vector<2xi64>
  ; CHECK-DAG:  %[[IDX4:[0-9]+]] = llvm.aiir.constant(42 : i32) : i32
...
define i32 @function_address_before_def() {
  %1 = alloca ptr
  ; CHECK:  %[[FUN:.*]] = llvm.aiir.addressof @callee : !llvm.ptr
...
; CHECK-DAG:  %[[C0:.+]] = llvm.aiir.constant(9 : i32) : i32
; CHECK-DAG:  %[[C1:.+]] = llvm.aiir.constant(4 : i8) : i8
; CHECK-DAG:  %[[C2:.+]] = llvm.aiir.constant(8 : i16) : i16
; CHECK-DAG:  %[[C3:.+]] = llvm.aiir.constant(7 : i32) : i32
; CHECK-DAG:  %[[ROOT:.+]] = llvm.aiir.undef : !llvm.struct<"simple_agg_type", (i32, i8, i16, i32)>
...
; CHECK-DAG:  %[[C1:.+]] = llvm.aiir.constant(1 : i32) : i32
; CHECK-DAG:  %[[C2:.+]] = llvm.aiir.constant(2 : i8) : i8
; CHECK-DAG:  %[[C3:.+]] = llvm.aiir.constant(3 : i16) : i16
; CHECK-DAG:  %[[C4:.+]] = llvm.aiir.constant(4 : i32) : i32
; CHECK-DAG:  %[[NESTED:.+]] = llvm.aiir.undef : !llvm.struct<"simple_agg_type", (i32, i8, i16, i32)>
...
; CHECK-DAG:  %[[NULL:.+]] = llvm.aiir.zero : !llvm.ptr
; CHECK-DAG:  %[[ROOT:.+]] = llvm.aiir.undef : !llvm.struct<"nested_agg_type", (struct<"simple_agg_type", (i32, i8, i16, i32)>, ptr)>
...
; CHECK-DAG:  %[[VEC:.+]] = llvm.aiir.zero : vector<2x!llvm.ptr>
...
define i64 @const_exprs_with_duplicate() {
  ; CHECK: %[[ADDR:.+]] = llvm.aiir.addressof @global : !llvm.ptr
...
; CHECK-LABEL: @cyclic
; CHECK:  %[[ADDR:.+]] = llvm.aiir.addressof @cyclic
...
; CHECK: llvm.aiir.global external constant @const()
; CHECK:   %[[ADDR:.+]] = llvm.aiir.addressof @const : !llvm.ptr
...
; CHECK: llvm.aiir.global external @global_zero_array() {addr_space = 0 : i32} : !llvm.array<0 x ptr> {
; CHECK:   %[[ZERO:.+]] = llvm.aiir.zero : !llvm.array<0 x ptr>
; CHECK:   llvm.return %[[ZERO]] : !llvm.array<0 x ptr>

; CHECK-LABEL: @load_zero_array
define [0 x ptr] @load_zero_array() {
  ; CHECK: %[[ADDR:.+]] = llvm.aiir.addressof @global_zero_array : !llvm.ptr
...
; CHECK: llvm.aiir.global external @global_zero_struct() {addr_space = 0 : i32} : !llvm.struct<()> {
; CHECK:   %[[ZERO:.+]] = llvm.aiir.zero : !llvm.struct<()>
...
; CHECK: llvm.aiir.global external @global_array_with_elements({{.*}}) {addr_space = 0 : i32} : !llvm.array<3 x i32>
...
; CHECK: llvm.aiir.global external @vector_splat_int(dense<7> : vector<2xi64>) {addr_space = 0 : i32} : vector<2xi64>
; CHECK: llvm.aiir.global external @vector_splat_float(dense<7.000000e+00> : vector<2xf32>) {addr_space = 0 : i32} : vector<2xf32>
