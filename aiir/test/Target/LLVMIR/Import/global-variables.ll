...
; CHECK:  llvm.aiir.global external @global_struct
...
; CHECK:  llvm.aiir.global external @global_float
; CHECK-SAME:  {addr_space = 0 : i32, alignment = 8 : i64} : f64
@global_float = external global double, align 8

; CHECK:  llvm.aiir.global internal constant @address_before
; CHECK:  = llvm.aiir.addressof @global_int : !llvm.ptr
@address_before = internal constant ptr @global_int

; CHECK:  llvm.aiir.global external @global_int
; CHECK-SAME:  {addr_space = 0 : i32, alignment = 8 : i64} : i32
@global_int = external global i32, align 8

; CHECK:  llvm.aiir.global internal constant @address_after
; CHECK:  = llvm.aiir.addressof @global_int : !llvm.ptr
@address_after = internal constant ptr @global_int

; CHECK:  llvm.aiir.global internal @global_string("hello world")
@global_string = internal global [11 x i8] c"hello world"

; CHECK:  llvm.aiir.global external @global_vector
; CHECK-SAME:  {addr_space = 0 : i32} : vector<8xi32>
@global_vector = external global <8 x i32>

; CHECK: llvm.aiir.global internal constant @global_gep_const_expr
; CHECK-SAME:  {addr_space = 0 : i32, dso_local} : !llvm.ptr {
; CHECK-DAG:  %[[ADDR:[0-9]+]] = llvm.aiir.addressof @global_int : !llvm.ptr
...
; CHECK-LABEL: llvm.aiir.global external constant @reference
; CHECK-NEXT: %[[ADDR:.*]] = llvm.aiir.addressof @simple
...
; CHECK-LABEL: llvm.aiir.global external @recursive
; CHECK: %[[ADDR:.*]] = llvm.aiir.addressof @recursive
...
; CHECK:  llvm.aiir.global external @dso_local_var
...
; CHECK:  llvm.aiir.global external thread_local @thread_local_var
...
; CHECK:  llvm.aiir.global external @addr_space_var
...
; CHECK:  llvm.aiir.global private constant @no_unnamed_addr
; CHECK-SAME:  (42 : i64) {addr_space = 0 : i32, dso_local} : i64
@no_unnamed_addr = private constant i64 42

; CHECK:  llvm.aiir.global private local_unnamed_addr constant @local_unnamed_addr
; CHECK-SAME:  (42 : i64) {addr_space = 0 : i32, dso_local} : i64
@local_unnamed_addr = private local_unnamed_addr constant i64 42

; CHECK:  llvm.aiir.global private unnamed_addr constant @unnamed_addr
; CHECK-SAME:  (42 : i64) {addr_space = 0 : i32, dso_local} : i64
...
; CHECK:  llvm.aiir.global internal constant @sectionvar("hello world")
...
; CHECK:  llvm.aiir.global internal constant @vector_constant
...
; CHECK:  llvm.aiir.global internal constant @array_constant
...
; CHECK{LITERAL}: llvm.aiir.global internal constant @nested_array_constant(dense<[[1, 2], [3, 4]]> : tensor<2x2xi32>)
...
; CHECK{LITERAL}: llvm.aiir.global internal constant @nested_array_constant3(dense<[[[1, 2], [3, 4]]]> : tensor<1x2x2xi32>)
...
; CHECK{LITERAL}: llvm.aiir.global internal constant @nested_array_vector(dense<[[[1, 2], [3, 4]]]> : vector<1x2x2xi32>)
...
; CHECK:  llvm.aiir.global internal constant @vector_constant_zero
...
; CHECK:  llvm.aiir.global internal constant @array_constant_zero
...
; CHECK: llvm.aiir.global internal constant @nested_array_constant3_zero
; CHECK-SAME:  (dense<0> : tensor<1x2x2xi32>)
; CHECK-SAME:  {addr_space = 0 : i32, dso_local} : !llvm.array<1 x array<2 x array<2 x i32>>>
@nested_array_constant3_zero = internal constant [1 x [2 x [2 x i32]]] zeroinitializer

; CHECK: llvm.aiir.global internal constant @nested_array_vector_zero
; CHECK-SAME:  (dense<0> : vector<1x2x2xi32>)
; CHECK-SAME:  {addr_space = 0 : i32, dso_local} : !llvm.array<1 x array<2 x vector<2xi32>>>
@nested_array_vector_zero = internal constant [1 x [2 x <2 x i32>]] zeroinitializer

; CHECK{LITERAL}: llvm.aiir.global internal constant @nested_bool_array_constant(dense<[[true, false]]> : tensor<1x2xi1>)
...
; CHECK: llvm.aiir.global internal constant @quad_float_constant
; CHECK-SAME:  dense<[
...
; CHECK: llvm.aiir.global internal constant @quad_float_splat_constant
...
; CHECK: llvm.aiir.global_ctors ctors = [@foo, @bar], priorities = [0 : i32, 42 : i32], data = [#llvm.zero, #llvm.zero]
; CHECK: llvm.aiir.global_dtors dtors = [@foo], priorities = [0 : i32], data = [#llvm.zero]
...
; CHECK: llvm.aiir.global_ctors ctors = [], priorities = [], data = []
@llvm.global_ctors = appending global [0 x { i32, ptr, ptr }] zeroinitializer

; CHECK: llvm.aiir.global_dtors dtors = [], priorities = [], data = []
@llvm.global_dtors = appending global [0 x { i32, ptr, ptr }] zeroinitializer

; // -----

; llvm.aiir.global_dtors dtors = [@foo], priorities = [0 : i32], data = [@foo]
...
; CHECK: llvm.aiir.global external hidden constant @hidden("string")
@hidden = hidden constant [6 x i8] c"string"

; CHECK: llvm.aiir.global external protected constant @protected(42 : i64)
...
; CHECK-DAG: llvm.aiir.global external @foo() {addr_space = 0 : i32, alignment = 8 : i64, dbg_exprs = [#[[EXPR0]]]} : i32
; CHECK-DAG: llvm.aiir.global external @bar() {addr_space = 0 : i32, alignment = 8 : i64, dbg_exprs = [#[[EXPR1]]]} : i32
...
; CHECK:  llvm.aiir.global private unnamed_addr constant @aiir.llvm.nameless_global_0("0\00")
; We skip over @aiir.llvm.nameless_global.1 and 2 because they exist
; CHECK:  llvm.aiir.global private unnamed_addr constant @aiir.llvm.nameless_global_3("1\00")
;
; CHECK:  llvm.aiir.global internal constant @zero() {addr_space = 0 : i32, dso_local} : !llvm.ptr {
; CHECK:    llvm.aiir.addressof @aiir.llvm.nameless_global_0 : !llvm.ptr
; CHECK:  llvm.aiir.global internal constant @one() {addr_space = 0 : i32, dso_local} : !llvm.ptr {
; CHECK:    llvm.aiir.addressof @aiir.llvm.nameless_global_3 : !llvm.ptr

; CHECK: llvm.aiir.global external constant @".str.1"() {addr_space = 0 : i32, dbg_exprs = [#[[GLOBAL_VAR_EXPR]]]}
...
@"aiir.llvm.nameless_global_1" = external constant [10 x i8], !dbg !0
declare void @"aiir.llvm.nameless_global_2"()
...
; CHECK:  llvm.aiir.global internal constant @reference()
; CHECK:    llvm.aiir.addressof @aiir.llvm.nameless_global_0 : !llvm.ptr
...
; CHECK-LABEL: llvm.aiir.global external @target_specific_attrs_only
...
; CHECK-LABEL: llvm.aiir.global external @target_specific_attrs_combined
