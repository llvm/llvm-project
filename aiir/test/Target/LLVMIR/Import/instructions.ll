...
define void @integer_arith(i32 %arg1, i32 %arg2, i64 %arg3, i64 %arg4) {
  ; CHECK:  %[[C1:[0-9]+]] = llvm.aiir.constant(-7 : i32) : i32
  ; CHECK:  %[[C2:[0-9]+]] = llvm.aiir.constant(42 : i32) : i32
...
define void @fp_arith(float %arg1, float %arg2, double %arg3, double %arg4) {
  ; CHECK:  %[[C1:[0-9]+]] = llvm.aiir.constant(3.030000e+01 : f32) : f32
  ; CHECK:  %[[C2:[0-9]+]] = llvm.aiir.constant(3.030000e+01 : f64) : f64
...
define ptr @pointer_casts(ptr %arg1, i64 %arg2, <3 x ptr> %arg3) {
  ; CHECK:  %[[NULL:[0-9]+]] = llvm.aiir.zero : !llvm.ptr
...
define float @insert_extract_value_struct({{i32},{float, double}}* %ptr) {
  ; CHECK:  %[[C0:.+]] = llvm.aiir.constant(2.000000e+00 : f64)
...
define void @insert_extract_value_array([4 x [4 x i8]] %arg1) {
  ; CHECK:  %[[C0:.+]] = llvm.aiir.constant(0 : i8)
...
define ptr @alloca(i64 %size) {
  ; CHECK:  %[[C1:[0-9]+]] = llvm.aiir.constant(1 : i32) : i32
...
define void @indirect_call(ptr addrspace(42) %fn) {
  ; CHECK:  %[[C0:[0-9]+]] = llvm.aiir.constant(0 : i16) : i16
...
define void @gep_static_idx(ptr %ptr) {
  ; CHECK: %[[IDX:.+]] = llvm.aiir.constant(7 : i32)
...
define void @gep_dynamic_idx(ptr %ptr, i32 %idx) {
  ; CHECK: %[[C0:.+]] = llvm.aiir.constant(0 : i32)
...
define void @freeze(i32 %arg1) {
  ; CHECK:  %[[UNDEF:[0-9]+]] = llvm.aiir.undef : i64
  ; CHECK:  %[[POISON:[0-9]+]] = llvm.aiir.poison : i16
...
define void @incompatible_call_and_callee_types() {
  ; CHECK: %[[CST:.*]] = llvm.aiir.constant(0 : i64) : i64
  ; CHECK: %[[TARGET:.*]] = llvm.aiir.addressof @callee : !llvm.ptr
