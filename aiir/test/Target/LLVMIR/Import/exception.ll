...
define i32 @invokeLandingpad() personality ptr @__gxx_personality_v0 {
  ; CHECK: %[[a1:[0-9]+]] = llvm.aiir.addressof @_ZTIii : !llvm.ptr
...
entry:
  ; CHECK: %[[c0:.*]] = llvm.aiir.constant(0 : i32) : i32
...
entry:
  ; CHECK: %[[c0:.*]] = llvm.aiir.constant(0 : i32) : i32
  ; CHECK: %[[c1:.*]] = llvm.aiir.constant(1 : i32) : i32
  ; CHECK: %[[c2:.*]] = llvm.aiir.constant(2 : i32) : i32
  ; CHECK: %[[c20:.*]] = llvm.aiir.constant(20 : i32) : i32
  ; CHECK: llvm.cond_br %[[cond]], ^[[bb1:.*]], ^[[bb2:.*]]
  br i1 %cond, label %call, label %nocall
; CHECK: ^[[bb1]]:
call:
...
entry:
  ; CHECK:    %[[null:.*]] = llvm.aiir.zero : !llvm.ptr
  ; CHECK:    %[[c1:.*]] = llvm.aiir.constant(0 : i32) : i32
  ; CHECK:    %[[struct:.*]] = llvm.aiir.zero : !llvm.struct<(ptr, i32)>
...
entry:
  ; CHECK: %[[ZERO1:.+]] = llvm.aiir.zero : !llvm.array<0 x ptr>
  ; CHECK: %[[ZERO2:.+]] = llvm.aiir.{{(zero|constant)}}{{.*}}: !llvm.array<0 x i32>
  invoke void @foo(ptr null) to label %normal unwind label %lpad
