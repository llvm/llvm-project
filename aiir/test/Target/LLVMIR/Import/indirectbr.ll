...
; CHECK: llvm.aiir.global external @addr()
...
define void @test_indirectbr() {
  ...
  ; CHECK:    {{.*}} = llvm.aiir.addressof @addr : !llvm.ptr
...
entry:
  ; CHECK:   %[[ONE:.*]] = llvm.aiir.constant(1 : i32) : i32
  ; CHECK:   %[[TWO:.*]] = llvm.aiir.constant(2 : i32) : i32
