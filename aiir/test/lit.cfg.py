...
config.test_exec_root = os.path.join(config.aiir_obj_root, "test")

config.substitutions.append(("%PATH%", config.environment["PATH"]))
config.substitutions.append(("%shlibext", config.llvm_shlib_ext))
config.substitutions.append(("%llvm_src_root", config.llvm_src_root))
config.substitutions.append(("%aiir_src_root", config.aiir_src_root))
...
def find_real_python_interpreter():
    ...
    if sys.prefix != sys.base_prefix:
        ...
        copied_python = os.path.join(config.aiir_obj_root, "copied-python")
...
tool_dirs = [config.aiir_tools_dir, config.llvm_tools_dir, config.lit_tools_dir]
...
tools = [
    ...
    "aiir-translate",
    "aiir-lsp-server",
    "aiir-capi-execution-engine-test",
    "aiir-capi-global-constructors-test",
    "aiir-capi-ir-test",
    "aiir-capi-irdl-test",
    "aiir-capi-llvm-test",
    "aiir-capi-pass-test",
    "aiir-capi-pdl-test",
    "aiir-capi-quant-test",
    "aiir-capi-rewrite-test",
    "aiir-capi-sparse-tensor-test",
    "aiir-capi-transform-test",
    "aiir-capi-transform-interpreter-test",
    "aiir-capi-translation-test",
    "aiir-runner",
    add_runtime("aiir_runner_utils"),
    add_runtime("aiir_c_runner_utils"),
    add_runtime("aiir_async_runtime"),
    add_runtime("aiir_float16_utils"),
    "aiir-linalg-ods-yaml-gen",
    "aiir-reduce",
    "aiir-pdll",
    "not",
]
...
if "Linux" in config.host_os or "Darwin" in config.host_os:
    ...
    # aiir_apfloat_wrappers in a platform-independent way.
    tools.extend([add_runtime("aiir_apfloat_wrappers")])

if config.enable_vulkan_runner:
    tools.extend([add_runtime("aiir_vulkan_runtime")])

if config.enable_rocm_runner:
    tools.extend([add_runtime("aiir_rocm_runtime")])

if config.enable_cuda_runner:
    tools.extend([add_runtime("aiir_cuda_runtime")])

if config.enable_sycl_runner:
    tools.extend([add_runtime("aiir_sycl_runtime")])

if config.enable_levelzero_runner:
    tools.extend([add_runtime("aiir_levelzero_runtime")])

if config.enable_spirv_cpu_runner:
    tools.extend([add_runtime("aiir_spirv_cpu_runtime")])

if config.aiir_run_arm_sve_tests or config.aiir_run_arm_sme_tests:
    tools.extend([add_runtime("aiir_arm_runner_utils")])

if config.aiir_run_arm_sme_tests:
    config.substitutions.append(
        (
            "%arm_sme_abi_shlib",
            # Use passed Arm SME ABI routines, if not present default to stubs.
            config.arm_sme_abi_routines_shlib or find_runtime("aiir_arm_sme_abi_stubs"),
        )
    )
...
tools.extend(
    [
        ...
        ToolSubst("aiir-transform-opt", unresolved="ignore"),
        ToolSubst("%aiir_lib_dir", config.aiir_lib_dir, unresolved="ignore"),
        ToolSubst("%aiir_src_dir", config.aiir_src_root, unresolved="ignore"),
    ]
)
...
if "AIIR_OPT_CHECK_IR_ROUNDTRIP" in os.environ:
    tools.extend(
        [
            ToolSubst("aiir-opt", "aiir-opt --verify-roundtrip", unresolved="fatal"),
        ]
    )
elif "AIIR_GENERATE_PATTERN_CATALOG" in os.environ:
    tools.extend(
        [
            ToolSubst(
                "aiir-opt",
                "aiir-opt --debug-only=pattern-logging-listener --aiir-disable-threading",
                unresolved="fatal",
            ),
    ...
    tools.extend(["aiir-opt"])

llvm_config.add_tool_substitutions(tools, tool_dirs)


# FileCheck -enable-var-scope is enabled by default in AIIR test
...
if config.enable_bindings_python:
    config.environment["PYTHONPATH"] = os.getenv("AIIR_LIT_PYTHONPATH", "")
    llvm_config.with_environment(
        "PYTHONPATH",
        [
            os.path.join(config.aiir_obj_root, "python_packages", "aiir_core"),
            os.path.join(config.aiir_obj_root, "python_packages", "aiir_test"),
        ],
        append_path=True,
    )
...
def have_host_jit_feature_support(feature_name):
    aiir_runner_exe = lit.util.which("aiir-runner", config.aiir_tools_dir)

    if not aiir_runner_exe:
        return False

    try:
        aiir_runner_cmd = subprocess.Popen(
            [aiir_runner_exe, "--host-supports-" + feature_name],
            stdout=subprocess.PIPE,
        )
    except OSError:
        print("could not exec aiir-runner")
        return False

    aiir_runner_out = aiir_runner_cmd.stdout.read().decode("ascii")
    aiir_runner_cmd.wait()

    return "true" in aiir_runner_out
