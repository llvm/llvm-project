...
struct TestBytecodeRoundtripPass
  ...
  AIIR_DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID(TestBytecodeRoundtripPass)
  ...
  aiir::Pass::Option<test::TestDialectVersion, TestDialectVersionParser>
...
private:
  ...
  void runTest0(Operation *op) {
    auto newCtx = std::make_shared<AIIRContext>();
  ...
  void runTest1(Operation *op) {
    auto *builtin = op->getContext()->getLoadedDialect<aiir::BuiltinDialect>();
