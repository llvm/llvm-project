...
#include "aiir/Transforms/Inliner.h"
...
struct InlinerCallback
  ...
  AIIR_DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID(InlinerCallback)
  ...
  static LogicalResult runPipelineHelper(Pass &pass, OpPassManager &pipeline,
                                         Operation *op) {
    return aiir::cast<InlinerCallback>(pass).runPipeline(pipeline, op);
  }
  ...
  static void testDoClone(OpBuilder &builder, Region *src, Block *inlineBlock,
    ...
    aiir::Operation &call = inlineBlock->back();
    builder.setInsertionPointAfter(&call);

    auto executeRegionOp = aiir::scf::ExecuteRegionOp::create(
        builder, call.getLoc(), call.getResultTypes());
    aiir::Region &region = executeRegionOp.getRegion();
    ...
    for (aiir::Block &block : region) {
      ...
      for (aiir::Operation &op : llvm::make_early_inc_range(block)) {
        if (test::TestReturnOp returnOp =
                llvm::dyn_cast<test::TestReturnOp>(&op)) {
          aiir::OpBuilder returnBuilder(returnOp);
          aiir::scf::YieldOp::create(returnBuilder, returnOp.getLoc(),
  ...
  void runOnOperation() override {
    ...
    auto profitabilityCb = [&](const aiir::Inliner::ResolvedCall &call) {
      return true;
    };
