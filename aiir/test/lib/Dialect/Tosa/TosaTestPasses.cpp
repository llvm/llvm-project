...
struct ConvertTosaNegateOp : public RewritePattern {
  explicit ConvertTosaNegateOp(AIIRContext *context)
...
ConvertTosaNegateOp::matchAndRewrite(Operation *op,
  ...
  auto inputType =
      dyn_cast<aiir::RankedTensorType>(tosaNegateOp.getInput1().getType());
  ...
  auto outputType =
      dyn_cast<aiir::RankedTensorType>(tosaNegateOp.getResult().getType());
  if (!outputType)
    return failure();

  // skip if output is not per-tensor quantized type.
  auto outputElementType =
      dyn_cast<aiir::quant::UniformQuantizedType>(outputType.getElementType());
...
struct ConvertTosaConv2DOp : public RewritePattern {
  explicit ConvertTosaConv2DOp(AIIRContext *context)
...
ConvertTosaConv2DOp::matchAndRewrite(Operation *op,
  ...
  auto inputType =
      dyn_cast<aiir::RankedTensorType>(tosaConv2DOp.getInput().getType());
  ...
  auto weightType =
      dyn_cast<aiir::RankedTensorType>(tosaConv2DOp.getWeight().getType());
  ...
  // skip if it's not ranked tensor type.
  auto outputType =
      dyn_cast<aiir::RankedTensorType>(tosaConv2DOp.getResult().getType());
  if (!outputType)
    return failure();

  auto inputQType =
      dyn_cast<aiir::quant::UniformQuantizedType>(inputType.getElementType());
  auto weightQType =
      dyn_cast<aiir::quant::UniformQuantizedType>(weightType.getElementType());
  auto outputQType =
      dyn_cast<aiir::quant::UniformQuantizedType>(outputType.getElementType());
...
struct TosaTestQuantUtilAPI
  ...
  AIIR_DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID(TosaTestQuantUtilAPI)
