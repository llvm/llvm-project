//===- TestTypes.cpp - AIIR Test Dialect Types ------------------*- C++ -*-===//
//
...
// The functions don't need to be in the header file, but need to be in the aiir
...
TestTensorType::getBufferType(
    const ::aiir::bufferization::BufferizationOptions &,
    ::llvm::function_ref<::aiir::InFlightDiagnostic()>) {
  return cast<bufferization::BufferLikeType>(
      TestMemrefType::get(getContext(), getShape(), getElementType(), nullptr));
}

::aiir::LogicalResult TestTensorType::verifyCompatibleBufferType(
    ::aiir::bufferization::BufferLikeType bufferType,
...
Type TestTypeNewlineAndIndentType::parse(::aiir::AsmParser &parser) {
...
void TestTypeNewlineAndIndentType::print(::aiir::AsmPrinter &printer) const {
  printer << "<";
