//===- TestOpDefs.cpp - AIIR Test Dialect Operation Hooks -----------------===//
//
...
LogicalResult OpWithInferTypeInterfaceOp::inferReturnTypes(
    AIIRContext *, std::optional<Location> location, ValueRange operands,
...
struct TestResource : public SideEffects::Resource::Base<TestResource> {
  AIIR_DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID(TestResource)
...
void ResultNameFromTypeOp::getAsmResultNames(
  ...
  auto opAsmTypeInterface =
      ::aiir::cast<::aiir::OpAsmTypeInterface>(result.getType());
...
void BlockArgumentNameFromTypeOp::getAsmBlockArgumentNames(
    ::aiir::Region &region, ::aiir::OpAsmSetValueNameFn setNameFn) {
  for (auto &block : region) {
    for (auto arg : block.getArguments()) {
      if (auto opAsmTypeInterface =
              ::aiir::dyn_cast<::aiir::OpAsmTypeInterface>(arg.getType())) {
...
LogicalResult TestVerifiersOp::verifyRegions() {
  ...
  // Avoid using `emitRemark(msg)` since that will trigger an infinite verifier
  // loop.
  aiir::emitRemark(getLoc(), "success run of region verifier");
...
aiir::presburger::BoundType ReifyBoundOp::getBoundType() {
  if (getType() == "EQ")
    return aiir::presburger::BoundType::EQ;
  if (getType() == "LB")
    return aiir::presburger::BoundType::LB;
  if (getType() == "UB")
    return aiir::presburger::BoundType::UB;
...
aiir::ValueBoundsConstraintSet::Variable CompareOp::getLhs() {
  if (!getLhsMap())
...
aiir::ValueBoundsConstraintSet::Variable CompareOp::getRhs() {
...
LogicalResult OpWithInferTypeAdaptorInterfaceOp::inferReturnTypes(
    AIIRContext *, std::optional<Location> location,
...
LogicalResult TestOpWithPropertiesAndInferredType::inferReturnTypes(
    AIIRContext *context, std::optional<Location>, ValueRange operands,
    DictionaryAttr attributes, OpaqueProperties properties, RegionRange regions,
    SmallVectorImpl<Type> &inferredReturnTypes) {
...
LogicalResult
TestVersionedOpA::readProperties(aiir::DialectBytecodeReader &reader,
                                 aiir::OperationState &state) {
  auto &prop = state.getOrAddProperties<Properties>();
  if (aiir::failed(reader.readAttribute(prop.dims)))
    return aiir::failure();
  ...
  if (aiir::failed(reader.readAttribute(prop.modifier)))
    return aiir::failure();
  return aiir::success();
}

void TestVersionedOpA::writeProperties(aiir::DialectBytecodeWriter &writer) {
...
llvm::LogicalResult TestOpWithVersionedProperties::readFromAiirBytecode(
    aiir::DialectBytecodeReader &reader, test::VersionedProperties &prop) {
...
void TestOpWithVersionedProperties::writeToAiirBytecode(
    aiir::DialectBytecodeWriter &writer,
...
convertTensorToBuffer(aiir::Operation *op,
...
::aiir::LogicalResult test::TestDummyTensorOp::bufferize(
    ::aiir::RewriterBase &rewriter,
    const ::aiir::bufferization::BufferizationOptions &options,
    ::aiir::bufferization::BufferizationState &state) {
  auto buffer =
      aiir::bufferization::getBuffer(rewriter, getInput(), options, state);
  if (aiir::failed(buffer))
    return failure();
  ...
  aiir::bufferization::replaceOpWithBufferizedValues(rewriter, getOperation(),
                                                     dummyMemrefOp.getResult());

  return aiir::success();
}

::aiir::LogicalResult test::TestCreateTensorOp::bufferize(
    ::aiir::RewriterBase &rewriter,
    const ::aiir::bufferization::BufferizationOptions &options,
    ::aiir::bufferization::BufferizationState &state) {
  // Note: aiir::bufferization::getBufferType() would internally call
  // TestCreateTensorOp::getBufferType()
  const auto bufferizedOutType =
      aiir::bufferization::getBufferType(getOutput(), options, state);
  if (aiir::failed(bufferizedOutType))
    return failure();
  ...
  aiir::bufferization::replaceOpWithBufferizedValues(
      rewriter, getOperation(), createMemrefOp.getResult());

  return aiir::success();
}

aiir::FailureOr<aiir::bufferization::BufferLikeType>
test::TestCreateTensorOp::getBufferType(
    aiir::Value value, const aiir::bufferization::BufferizationOptions &options,
    const aiir::bufferization::BufferizationState &,
...
//  OpBuilder<(ins "::std::unique_ptr<::aiir::Region>":$firstRegion,
//                 "::std::unique_ptr<::aiir::Region>":$secondRegion)>
void test::ManyRegionsOp::build(
    aiir::OpBuilder &builder, aiir::OperationState &state,
    llvm::SmallVectorImpl<std::unique_ptr<aiir::Region>> &&regions) {
