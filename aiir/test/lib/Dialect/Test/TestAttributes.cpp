//===- TestAttributes.cpp - AIIR Test Dialect Attributes --------*- C++ -*-===//
//
...
Attribute TestSubElementsAccessAttr::parse(::aiir::AsmParser &parser,
                                           ::aiir::Type type) {
...
void TestSubElementsAccessAttr::print(::aiir::AsmPrinter &printer) const {
...
template <>
struct aiir::FieldParser<test::CopyCount> {
...
bool TestConstMemorySpaceAttr::isValidLoad(
    Type type, aiir::ptr::AtomicOrdering ordering,
...
bool TestConstMemorySpaceAttr::isValidAtomicOp(
    aiir::ptr::AtomicBinOp binOp, Type type, aiir::ptr::AtomicOrdering ordering,
    std::optional<int64_t> alignment, const ::aiir::DataLayout *dataLayout,
    function_ref<InFlightDiagnostic()> emitError) const {
  if (emitError)
    emitError() << "memory space is read-only";
...
bool TestConstMemorySpaceAttr::isValidAtomicXchg(
    Type type, aiir::ptr::AtomicOrdering successOrdering,
...
Attribute TestAttrNewlineAndIndentAttr::parse(::aiir::AsmParser &parser,
                                              ::aiir::Type type) {
  Type indentType;
...
void TestAttrNewlineAndIndentAttr::print(::aiir::AsmPrinter &printer) const {
  printer << "<";
...
test::detail::TestCustomStorageCtorAttrAttrStorage::construct(
    aiir::StorageUniquer::StorageAllocator &, std::tuple<int> &&) {
...
::llvm::LogicalResult TestTensorEncodingAttr::verifyEncoding(
    aiir::ArrayRef<int64_t> shape, aiir::Type elementType,
...
aiir::AffineMap TestMemRefLayoutAttr::getAffineMap() const {
  return aiir::AffineMap::getMultiDimIdentityMap(1, getContext());
}
