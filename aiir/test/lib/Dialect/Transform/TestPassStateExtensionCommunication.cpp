...
// conducted between a regular aiir pass and transform ops through the
...
struct TestPassStateExtensionCommunication
  ...
  StringRef getDescription() const final {
    return "test state communciation between a aiir pass and transform ops";
  }
  ...
  void runOnOperation() override {
    ...
    auto stateInitializer =
        [&opCollection](aiir::transform::TransformState &state) -> void {
    ...
    auto stateExporter =
        [&opCollection](
            aiir::transform::TransformState &state) -> LogicalResult {
      TransformStateInitializerExtension *ext =
          state.getExtension<TransformStateInitializerExtension>();
      if (!ext) {
    ...
    for (auto op : module.getBody()->getOps<transform::TransformOpInterface>())
      if (failed(transform::applyTransforms(
              module, op, {}, aiir::transform::TransformOptions(), false,
