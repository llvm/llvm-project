...
from aiir.ir import *
from aiir.dialects.builtin import ModuleOp
from aiir.dialects import arith, func, scf, shape
from aiir.dialects._ods_common import _cext
...
def testTraverseOpRegionBlockIterators():
  ...
  """,
    ...
    # CHECK: class RegionSequence(collections.abc.Sequence[aiir._aiir_libs._aiir.ir.Region])
    ...
    # CHECK:     Block iter: <aiir.{{.+}}.BlockIterator
    # CHECK: Operation iter: <aiir.{{.+}}.OperationIterator
...
def testKnownOpView():
    ...
    """
        ...
        # CHECK: <aiir.dialects._arith_ops_gen.AddFOp object
        print(repr(addf))
        # CHECK: "custom.f32"()
        print(addf.lhs)
        ...
        # CHECK: <aiir.dialects.arith.ConstantOp object
        print(repr(constant))
...
def testCapsuleConversions():
    ...
    with Location.unknown(ctx):
        ...
        assert '"aiir.ir.Operation._CAPIPtr"' in repr(m_capsule)
