...
assert aiir.ir.Context._get_live_count() == 0

# Create first context.
print("CREATE C1")
c1 = aiir.ir.Context()
assert aiir.ir.Context._get_live_count() == 1
c1_repr = repr(c1)
print("C1 = ", c1_repr)
...
print("CREATE C3")
c3 = aiir.ir.Context()
assert aiir.ir.Context._get_live_count() == 2
c3_repr = repr(c3)
print("C3 =", c3)
...
assert '"aiir.ir.Context._CAPIPtr"' in repr(c4_capsule)
c5 = aiir.ir.Context._CAPICreate(c4_capsule)
assert c4 is c5
c4 = None
c5 = None
gc.collect()
...
tp = aiir.ir.ThreadPool()
assert tp.get_max_concurrency() > 0
c5 = aiir.ir.Context()
c5.set_thread_pool(tp)
assert c5.get_num_threads() == tp.get_max_concurrency()
assert c5._aiir_thread_pool_ptr() == tp._aiir_thread_pool_ptr()
c6 = aiir.ir.Context()
c6.set_thread_pool(tp)
assert c6.get_num_threads() == tp.get_max_concurrency()
assert c6._aiir_thread_pool_ptr() == tp._aiir_thread_pool_ptr()
c7 = aiir.ir.Context(thread_pool=tp)
assert c7.get_num_threads() == tp.get_max_concurrency()
assert c7._aiir_thread_pool_ptr() == tp._aiir_thread_pool_ptr()
assert aiir.ir.Context._get_live_count() == 3
c5 = None
c6 = None
c7 = None
gc.collect()
