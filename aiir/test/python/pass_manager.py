...
from aiir.dialects.func import FuncOp
from aiir.dialects.builtin import ModuleOp


# Log everything to stderr and flush so that we have a unified stream to match
# errors/info emitted by AIIR to stderr.
def log(*args):
...
def testCapsule():
    with Context():
        ...
        assert '"aiir.passmanager.PassManager._CAPIPtr"' in repr(pm_capsule)
        pm._testing_release()
...
def testParseFail():
    with Context():
        try:
            ...
            #      CHECK: ValueError exception: AIIR Textual PassPipeline Parser:1:1: error:
