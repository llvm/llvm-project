...
import aiir.dialects.python_test as test
import aiir.dialects.tensor as tensor
import aiir.dialects.arith as arith

from aiir._aiir_libs._aiirPythonTestNanobind import (
    TestAttr,
    TestType,
    TestTensorValue,
...
def testCustomAttribute():
    with Context() as ctx, Location.unknown():
        ...
        try:
            ...
            assert (
                "__init__(self, cast_from_attr: aiir._aiir_libs._aiir.ir.Attribute) -> None"
                in str(e)
            )
            assert (
                "Invoked with types: aiir._aiir_libs._aiirPythonTestNanobind.TestAttr, int"
                in str(e)
            )
        else:
            raise
...
def testCustomType():
    with Context() as ctx:
        ...
        try:
            ...
            assert (
                "__init__(self, cast_from_type: aiir._aiir_libs._aiir.ir.Type) -> None"
                in str(e)
            )
            assert (
                "Invoked with types: aiir._aiir_libs._aiirPythonTestNanobind.TestType, int"
                in str(e)
            )
        else:
            raise
