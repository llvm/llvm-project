...
def testArrayTypeOps():
    ...
    c_42 = llvm.aiir_constant(IntegerAttr.get(i32, 42))
    inserted = llvm.insertvalue(undef, c_42, [0])
    llvm.extractvalue(i32, inserted, [0])

    # CHECK: %[[UNDEF:.*]] = llvm.aiir.undef : !llvm.array<4 x i32>
    # CHECK: %[[C42:.*]] = llvm.aiir.constant(42 : i32) : i32
...
def testSmoke():
    ...
    # CHECK: %0 = llvm.aiir.undef : !llvm.struct<(f32, f32, f32, f32, f32, f32, f32, f32, f32, f32, f32, f32, f32, f32, f32, f32)>
...
def testConstant():
    i32 = IntegerType.get_signless(32)
    c_128 = llvm.aiir_constant(IntegerAttr.get(i32, 128))
    # CHECK: %{{.*}} = llvm.aiir.constant(128 : i32) : i32
    print(c_128.owner)
...
def testIntrinsics():
    ...
    ptr = llvm.PointerType.get()
    c_128 = llvm.aiir_constant(IntegerAttr.get(i32, 128))
    # CHECK: %[[CST128:.*]] = llvm.aiir.constant(128 : i32) : i32
    print(c_128.owner)
    ...
    c_0 = llvm.aiir_constant(IntegerAttr.get(IntegerType.get_signless(8), 0))
    # CHECK: %[[CST0:.+]] = llvm.aiir.constant(0 : i8) : i8
    print(c_0.owner)
