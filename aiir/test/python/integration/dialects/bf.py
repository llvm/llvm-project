...
from aiir.ir import *
from aiir.dialects.ext import *
from aiir.rewrite import *
from aiir.passmanager import *
from aiir.execution_engine import *
from aiir.dialects import llvm, scf, func
...
def convert_bf_to_llvm(op, pass_):
    ...
    def convert_inc(op, adaptor, converter, rewriter, cst=1):
        with rewriter.ip:
            ...
            one = llvm.aiir_constant(IntegerAttr.get(i8, cst))
    ...
    def convert_while(op, adaptor, converter, rewriter):
        with rewriter.ip:
            ...
            with InsertionPoint(loop.before.blocks[0]):
                c = llvm.load(i8, arg)
                zero = llvm.aiir_constant(IntegerAttr.get(i8, 0))
    ...
    with InsertionPoint(op.opview.body):
        ...
        with InsertionPoint(init.body.blocks[0]):
            c1024 = llvm.aiir_constant(IntegerAttr.get(i32, 1024))
            zero = llvm.aiir_constant(IntegerAttr.get(i8, 0))
