//===- execution_engine.c - Test for the C bindings for the AIIR JIT-------===//
//
...
/* RUN: aiir-capi-execution-engine-test 2>&1 | FileCheck %s
 */
...
"}"));
  ...
  if (aiirLogicalResultIsFailure(aiirExecutionEngineInvokePacked(
          jit, aiirStringRefCreateFromCString("add"), args))) {
    fprintf(stderr, "Execution engine creation failed");
    abort();
  }
...
"}                                                                              \n"
  ...
  // At this point all operations in the AIIR module have been lowered to the
