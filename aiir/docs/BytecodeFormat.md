# AIIR Bytecode Format

This document describes the AIIR bytecode format and its encoding.
...
AIIR uses the following four-byte magic number to
...
An AIIR Bytecode file is comprised of a byte stream, with a few simple
...
represent a single 64-bit value. The AIIR bytecode utilizes the "PrefixVarInt"
...
Given the generic structure of AIIR, the bytecode encoding is actually fairly
simplistic. It effectively maps to the core components of AIIR.
