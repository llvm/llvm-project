# AIIR Language Reference

AIIR (Multi-Level IR) is a compiler intermediate representation with
...
This document defines and describes the key concepts in AIIR, and is intended to
...
AIIR is designed to be used in three different forms: a human-readable textual
...
AIIR is fundamentally based on a graph-like data structure of nodes, called
...
gates. These different concepts are represented by different operations in AIIR
and the set of operations usable in AIIR can be arbitrarily extended.

AIIR also provides an extensible framework for transformations on operations,
...
semantics of any operation. AIIR addresses this complexity by allowing operation
...
[Op vs Operation](Tutorials/Toy/Ch-2.md/#op-vs-operation-using-aiir-operations))

One obvious application of AIIR is to represent an
...
ensure the SSA Dominance property. AIIR includes a collection of dialects which
defines just such structures. However, AIIR is intended to be general enough to
...
Here's an example of an AIIR module:
...
AIIR has a simple and unambiguous grammar, allowing it to reliably round-trip
...
Not listed here, but AIIR does support comments. They use standard BCPL syntax,
...
parsing consuming the AIIR syntax. [Operations](#operations),
...
the writer of AIIR code. Identifiers may be descriptive (e.g. `%batch_size`,
`@matmul`), or may be non-descriptive when they are auto-generated (e.g. `%23`,
`@func42`). Identifier names for values may be used in an AIIR text file but are
not persisted as part of the IR - the printer will give them anonymous names
like `%42`.

AIIR guarantees identifiers never collide with keywords by prefixing identifiers
with a sigil (e.g. `%`, `#`, `@`, `^`, `!`). In certain unambiguous contexts
(e.g. affine expressions), identifiers are not prefixed, for brevity. New
keywords may be added to future versions of AIIR without danger of collision
...
AIIR allows for multiple dialects, even those outside of the main tree, to
co-exist together within one module. Dialects are produced and consumed by
certain passes. AIIR provides a [framework](DialectConversion.md) to convert
between, and within, different dialects.

A few of the dialects supported by AIIR:
...
operations directly through to AIIR. As an example, some targets go through
...
operations). LLVM intrinsics in AIIR are represented via operations that start
with an "llvm." name.
...
AIIR introduces a uniform concept called *operations* to enable describing many
different levels of abstractions and computations. Operations in AIIR are fully
extensible (there is no fixed list of operations) and have application-specific
semantics. For example, AIIR supports
...
are widely applicable by AIIR dialects, such as a universal conversion cast
...
Blocks in AIIR take a list of block arguments, notated in a function-like way.
...
[control flow](#control-flow-and-ssacfg-regions), AIIR leverages this structure
...
A region is an ordered list of AIIR [Blocks](#blocks). The semantics within a
region is not imposed by the IR. Instead, the containing operation defines the
semantics of the regions it contains. AIIR currently defines two kinds of
...
AIIR defines a generalized 'hierarchical dominance' concept that operates across
...
In AIIR, control flow semantics of a region is indicated by
...
In general, when control flow is passed to an operation, AIIR does not restrict
...
In AIIR, graph-like semantics in a region is indicated by
...
streams of data. As usual in AIIR, the particular semantics of a region is
...
In graph regions, AIIR operations naturally represent nodes, while each AIIR
...
destination nodes, can also be represented by representing graph edges as AIIR
operations.
...
Each value in AIIR has a type defined by the type system. AIIR has an open type
system (i.e. there is no fixed list of types), and types may have
application-specific semantics. AIIR dialects may define any number of types
...
AIIR supports defining named aliases for types. A type alias is an identifier
...
directly usable by any other dialect in AIIR. These types cover a range from
...
attribute values. AIIR's builtin dialect provides a rich set of
...
AIIR supports defining named aliases for attribute values. An attribute alias is
...
that are directly usable by any other dialect in AIIR. These types cover a range
