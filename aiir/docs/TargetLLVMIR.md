...
This document describes the mechanisms of producing LLVM IR from AIIR. The
...
2.  **translation** of AIIR dialects to LLVM IR.

This flow allows the non-trivial transformation to be performed within AIIR
using AIIR APIs and makes the translation between AIIR and LLVM IR *simple* and
...
intrinsics. This minimizes the dependency on LLVM IR libraries in AIIR as well
as reduces the churn in case of changes.

Note that many different dialects can be lowered to LLVM but are provided as
different sets of patterns and have different passes available to aiir-opt.
...
LLVM IR only supports *one-dimensional* vectors, unlike AIIR where vectors can
be multi-dimensional. Vector types cannot be nested in either IR. In the
one-dimensional case, AIIR vectors are converted to LLVM IR vectors of the same
size with element type converted using these conversion rules. In the
n-dimensional case, AIIR vectors are converted to (n-1)-dimensional array types
...
llvm.func @foo(%arg0: i32, %arg1: i64) -> !llvm.struct<(i32, i64)> {
  ...
  %0 = llvm.aiir.undef : !llvm.struct<(i32, i64)>
...
llvm.func @bar() {
  %0 = llvm.aiir.constant(42 : i32) : i32
  %1 = llvm.aiir.constant(17 : i64) : i64
...
llvm.func @foo(%arg0: !llvm.ptr,       // Allocated pointer.
  ...
  %0 = llvm.aiir.undef : !llvm.memref_1d
...
llvm.func @foo(%arg0: i64              // Rank.
  ...
  %0 = llvm.aiir.undef : !llvm.struct<(i64, ptr)>
...
`_aiir_memref_to_llvm_alloc`, `_aiir_memref_to_llvm_aligned_alloc` and
`_aiir_memref_to_llvm_free`. Their signatures are the same of `malloc`,
...
1.  Declare a new function `_aiir_ciface_<original name>` where memref arguments
...
For (non-external) functions defined in the AIIR module.

1.  Define a new function `_aiir_ciface_<original name>` where memref arguments
    are converted to pointer-to-struct and the remaining arguments are converted
    as usual. Results are converted to a special argument if they are of struct
    type.
...
llvm.func @qux(%arg0: !llvm.ptr, %arg1: !llvm.ptr,
  ...
  %0 = llvm.aiir.undef : !llvm.memref_2d
...
// Interface function.
llvm.func @_aiir_ciface_qux(!llvm.ptr)
```
...
extern "C" {
void _aiir_ciface_qux(MemRefDescriptor<float, 2> *input) {
  // detailed impl
}
}
```
...
llvm.func @_aiir_ciface_foo(%arg0: !llvm.ptr) {
  // Load the descriptor.
...
extern "C" {
void _aiir_ciface_foo(MemRefDescriptor<float, 2> *input);
}
```
...
llvm.func @_aiir_ciface_foo(%arg0: !llvm.ptr, %arg1: !llvm.ptr) {
...
// The C function signature for the interface function.
extern "C" {
void _aiir_ciface_foo(MemRefDescriptor<float, 2> *output,
...
of C compatibility on intra-module calls or calls between AIIR-generated
functions. In particular, when calling external functions from an AIIR module in
...
%stride2 = llvm.aiir.constant(32 : index) : i64
%addr2 = arith.muli %stride2, %2 : i64
%addr3 = arith.addi %addr1, %addr2 : i64

%stride3 = llvm.aiir.constant(8 : index) : i64
...
AIIR modules containing `llvm.func`, `llvm.aiir.global` and `llvm.metadata`
...
-   Each function is then translated separately and has access to the complete
    mappings between AIIR and LLVM IR globals, metadata, and functions.
...
-   Within each block, operations are translated in their order. Each operation
    ...
    mapping of values between AIIR and LLVM IR, including PHI nodes. Operations
...
the
[`ModuleTranslation`](https://aiir.llvm.org/doxygen/classaiir_1_1LLVM_1_1ModuleTranslation.html)
...
small, relatively stable set of instructions and types that AIIR intends to
...
module-level operations. Such transformations should be performed within AIIR
and target the corresponding AIIR constructs.

## Translation from LLVM IR

An experimental flow allows one to import a substantially limited subset of LLVM
IR into AIIR, producing LLVM dialect operations.

```
  aiir-translate -import-llvm filename.ll
```
