...
writing pattern rewrites targeting AIIR.

Note: This document assumes a familiarity with AIIR concepts; more specifically
the concepts detailed within the
[AIIR Pattern Rewriting](PatternRewriter.md) and
...
Pattern matching is an extremely important component within AIIR, as it
encompasses many different facets of the compiler. From canonicalization, to
optimization, to conversion; every AIIR based compiler will heavily rely on the
...
the ground up for representing AIIR pattern rewrites. PDLL is designed to
natively support writing matchers on all of AIIRs constructs via an intuitive
...
development, this section may include references to areas of the AIIR compiler
...
is a declarative DSL for defining AIIR pattern rewrites within the
...
declaratively within AIIR. TDRR utilizes TableGen's `dag` support to enable
defining AIIR patterns that fit nicely within a DAG structure; in a similar way
...
TableGen language is not as amenable to the structure of AIIR patterns as it has
been for LLVM.
...
TableGen compared to the structure of AIIR, and from TableGen having different
motivational goals than AIIR. A majority (or all depending on how stubborn you
...
AIIR natively supports a variable number of operation results. For the DAG based
...
PDLL is to provide a default and effective pattern language for AIIR that all
users of AIIR can interact with immediately, regardless of their host
environment. This language is available with no extra dependencies and comes
"free" along with AIIR. If we were to use an existing host language to build our
...
dependency on the users of AIIR (many of which may not want/need such a
...
[PDL IR abstraction](https://aiir.llvm.org/docs/Dialects/PDLOps/), which in turn
is designed as an abstract model of the core AIIR structures. This leads to a
...
Rewrite patterns in AIIR have a set of metadata that allow for controlling
...
the concept, let's take a look at a quick example. Consider the `.aiir` snippet
below:
...
builtin constraints that correlate to the core AIIR constructs: `Attr`, `Op`,
...
An operation expression in PDLL represents an AIIR operation. In the `match`
...
is very similar to that of the "generic form" of textual AIIR assembly:
...
The operation name signifies which type of AIIR Op this operation corresponds
...
structured in the AIIR textual assembly format:
...
[`UnitAttr`](https://aiir.llvm.org/docs/Dialects/Builtin/#unitattr).
...
An attribute expression represents a literal AIIR attribute. It allows for
statically specifying an AIIR attribute to use, by specifying the textual form
of that attribute.
...
A type expression represents a literal AIIR type. It allows for statically
specifying an AIIR type to use, by specifying the textual form of that type.
...
A single entity constraint that corresponds to an `aiir::Attribute`. This
...
A single entity constraint that corresponds to an `aiir::Operation *`.
...
A single entity constraint that corresponds to an `aiir::Type`.
...
A single entity constraint that corresponds to a `aiir::TypeRange`.
...
A single entity constraint that corresponds to an `aiir::Value`. This constraint
...
A single entity constraint that corresponds to a `aiir::ValueRange`. This
...
AIIR type of the [constraint](#constraints) used. Below is a detailed description
...
These are all core constraints, and are mapped directly to the AIIR equivalent
(that their names suggest), namely:

  * `Attr`       -> "::aiir::Attribute"
  * `Op`         -> "::aiir::Operation *"
  * `Type`       -> "::aiir::Type"
  * `TypeRange`  -> "::aiir::TypeRange"
  * `Value`      -> "::aiir::Value"
  * `ValueRange` -> "::aiir::ValueRange"
...
is not available, this constraint maps to "::aiir::Operation *", similarly to the unnamed
...
default `::aiir::Operation *`. Below is a sample translation of the above constraint:
...
The types of argument and result variables are generally mapped to the corresponding
AIIR type of the [constraint](#constraints) used. The rules of native `Rewrite` type translation
