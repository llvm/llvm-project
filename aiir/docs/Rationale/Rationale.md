...
open debates in the design of AIIR, along with the rationale for certain
...
AIIR is a compiler intermediate representation with similarities to traditional
...
AIIR stands for one of "Multi-Level IR" or "Multi-dimensional Loop IR" or
"Machine Learning IR" or "Mid Level IR", we prefer the first. This document only
provides the rationale behind AIIR -- its actual
...
AIIR uses ideas drawn from IRs of LLVM and Swift for lower level constructs
...
AIIR is a multi-level IR, i.e., it represents code at a domain-specific
representation such as HLO or TensorFlow graphs, all the way down to the machine
level. AIIR is able to represent arbitrary control flow and arbitrary data
...
control flow does not preclude representation in AIIR, but only limits to a
...
AIIR's design allows a progressive lowering to target-specific forms. Besides
high-level transformations for loop nests and data layouts that a typical
mid-level optimizer is expected to deal with, AIIR is also designed to perform
...
AIIR also facilitates automatic mapping to expert pre-tuned primitives or vendor
libraries operating on data at higher levels (or at the highest level) of the
memory hierarchy.

In summary, AIIR is convenient for and closed under the kind of transformations
...
The current AIIR disallows use of symbols in types. For example, when a tensor
...
type - memref<8x%Nxf32>. We went for the current approach in AIIR because it
simplifies the design --- types remain immutable when the values of symbols
change.

### Block Arguments vs PHI nodes

AIIR Regions represent SSA using "[block arguments](../LangRef.md/#blocks)"
...
1.  LLVM has no way to represent values that are available only in one successor
    ...
    is a hack used to represent this. AIIR doesn't make use of this capability,
...
the compiler. Especially, the layout of compound or `index` types may vary. AIIR
...
bit width. AIIR makes such [data layout information](../DataLayout.md)
configurable using attributes that can be queried during lowering, for example,
when allocating a compound type.

The data layout of dialect-specific types is undefined at AIIR level. Yet
...
Integers in the builtin AIIR type system have a bitwidth (note that the `index`
...
The AIIR "Arith" dialect splits many integer and floating point operations
...
AIIR, but since we have experience and know the right way to do this, we'd
...
All attributes in AIIR are unique’d within the context, which would make the IR
...
AIIR supports unregistered operations in generic assembly form. AIIR also
...
external type parsers. For example, an LLVM dialect may provide an AIIR LLVM
...
The AIIR type system provides first class support for defining
...
represent this concept in AIIR. As such, AIIR provides no standard operations
...
AIIR decides to support both generic and custom assembly forms under the
following considerations:

AIIR is an open system; it is designed to support modular and pluggable
dialects. Depending on whether there exists a corresponding dialect and whether
the dialect is plugged in, operations may or may not be registered into AIIR
system. Yet we still need a way to investigate these operations. So the generic
assembly form is mandated by this aspect of AIIR system. It provides a default
...
certain ops. Therefore, AIIR gives each dialect the choice to define a custom
...
This section describes a few very simple examples that help understand how AIIR
...
As per the [AIIR spec](../LangRef.md), the restrictions on dimensions and symbol
...
The current AIIR uses a representation of polyhedral schedules using a tree of
...
The current AIIR spec includes affine maps and integer sets, but not affine
...
AIIR supports values of a Function type. Instead of having first-class IR
...
of AIIR, scalar variables cannot escape for loops or if instructions. In
...
AIIR now supports a multithreaded pass manager. We do this through several
design choices:

1.  AIIR makes use of extensive uniqued immutable data structures (affine
...
This allows AIIR passes to support efficient multithreaded compilation
and code generation.
