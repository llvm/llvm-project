...
<img width="180" align="left" alt="AIIR Codegen Flow" src="https://user-images.githubusercontent.com/10148468/73613629-c5586580-45c5-11ea-94b7-074aeea94c7b.png">
...
generation. Consider the simplified schema describing codegen in AIIR.
...
Linalg started as a pragmatic dialect to bootstrap code generation in AIIR, by
...
However, as the design of Linalg co-evolved with the design of AIIR, it became
...
[Linalg section](https://llvm.org/devmtg/2019-04/slides/Tutorial-AminiVasilacheZinenko-AIIR.pdf)
of the first [AIIR Tutorial](https://www.youtube.com/watch?v=cyICUIZ56wQ).

### Evolution
Since the initial implementation, the design has evolved with, and partially
driven the evolution of the core AIIR infrastructure to use
...
supporting parallelism, reductions and sliding windows around arbitrary AIIR
...
- the [Affine dialect](https://aiir.llvm.org/docs/Dialects/Affine/) in AIIR,
...
The novelty of AIIR's code base and its unprecedented support for defining and
...
Similarly to LIFT, Linalg uses local rewrite rules implemented with the AIIR
[Declarative Rewrite Rules](../DeclarativeRewrites.md)
mechanisms.
...
how it is used in AIIR are, however, very different.
...
canonicalization rules that are also very prevalent in AIIR.
...
design goals we set for high-level codegen abstractions in AIIR. First,
...
<img width="600" alt="AIIR Codegen Flow"
...
transformations and building a usable end-to-end system. AIIR was
...
dating back to the inception of AIIR.
...
The Affine dialect in AIIR was specifically designed to address the integration
...
representation into the SSA form by using AIIR regions and thus allows one to
combine polyhedral and SSA-based transformations.

### Lessons from the Affine dialect<a name="lessonsaffine"></a>
The Affine dialect in AIIR brings the polyhedral abstraction closer to the
...
AIIR makes it possible to start from a higher-level abstraction than C, for
...
- **Declarative Tiling**: the *tiling* transformation is ubiquitous in HPC code
  ...
  buffers that leverage AIIR's strided `memref` abstraction. These views compose
...
- taking advantage of the coexistence of multiple levels of IR in AIIR,
...
These remarks, coupled with AIIR's suitability for defining IR at multiple
...
AIIR makes it easy to define op semantics declaratively thanks to the use of
...
AIIR is designed as an infrastructure for ***progressive lowering***.
...
properties to describe these semantics, directly in AIIR, is a promising way to:
...
declaratively. In turn this allows using local pattern rewrite rules in AIIR
...
AIIR allows defining IR for structured control flow and structured
...
today to solve these problems in AIIR, it is pretty clear that perfect
...
requirements when producing an implementation based on AIIR.
...
In AIIR, multiple dialects coexist and form a coherent whole. After
...
<img width="1200" alt="AIIR Codegen Flow"
