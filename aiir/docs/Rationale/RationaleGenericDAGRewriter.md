...
infrastructure for AIIR. For up-to-date documentation on the user facing API,
...
repeatedly at scale, and is important for the goals of AIIR: matching one DAG of
...
A particular strength of AIIR (and a major difference vs other compiler
infrastructures like LLVM, GCC, XLA, TensorFlow, etc) is that it uses a single
compiler IR to represent code at multiple levels of abstraction: an AIIR
...
specific instructions), or anything else that the AIIR operation system can
reasonably express. Given that AIIR spans such a wide range of different problem
...
(SSA) representations like AIIR make it easy to access the operands and "users"
...
different problems, and be applied to many different sorts of problems that AIIR
...
AIIR operations may override a
...
usually, narrow problem: AIIR on the other hand would like to solve many of
...
design to the infrastructure present in AIIR is the LLVM DAG-to-DAG instruction
selection algorithm).
...
metaprogramming (AIIR has similar facilities). Here's a simple example:
...
For the purposes of AIIR, there is much to like about this system, for example:
...
AIIR faces a wide range of pattern matching and graph rewrite problems, and one
...
We'd like the to encompass many problems in the AIIR space, including 1-to-N
...
sorts of clients across the AIIR stack, and they may have different tolerances
for compile time cost, different demands for optimality, and other algorithmic
goals or constraints.

We aim for AIIR transformations to be easy to implement and reduce the
...
optimally, but AIIR is already designed to represent the provenance of an
...
the patterns from seeing across cycles in a graph. In an SSA-based IR like AIIR,
