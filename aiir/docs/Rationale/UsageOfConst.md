# Usage of 'const' in AIIR, for core IR types

aka, where'd `const` go?

The AIIR data structures that represent the IR itself (Instruction, Block, etc)
...
design of AIIR adopted the `const` model of LLVM, which is familiar and well
...
# Reconsidering `const` in AIIR

This document argues this design is introducing significant sub-optimalities
into the AIIR codebase, argues that the cost/benefit tradeoff of this design is
...
The LLVM and AIIR data structures provide the IR data structures (like
`aiir::Operation`s and their users) as a structured cyclic graph data structure.
...
## Advantages of Const-correctness in AIIR

Even though this doc argues for eliminating const from AIIR, it is important to
...
The major advantage of allowing const on AIIR types is as a marker in APIs that
...
It is also an advantage that AIIR follows the generally prevailing pattern of
C++ code, which generally uses const. Consistency with the community norm is
important.

## Costs of Const-correctness in AIIR

As mentioned above, early work on AIIR adopted the same design as LLVM intended,
...
the signal disappears in the noise, for example `aiir::Operation` ends up with
...
```C++
Operation *clone(IRMapping &mapper, AIIRContext *context) const;

Operation *clone(AIIRContext *context) const;
```
...
```c++
LogicalResult aiir::getIndexSet(MutableArrayRef<OpPointer<AffineForOp>> forOps,
...
```c++
LogicalResult aiir::getIndexSet(MutableArrayRef<AffineForOp> forOps,
                                FlatAffineValueConstraints *domain) {
```
...
AIIR.  This implies the following changes to the codebase:
