...
This document outlines how AIIR models side effects and how speculation works in
AIIR.
...
Many AIIR operations don't exhibit any behavior other than consuming and
...
However, a subset of AIIR operations have implicit behavior than isn't reflected
...
operations are modeled in AIIR.
...
AIIR has two op interfaces to represent these implicit behaviors:

1. The
   [`MemoryEffectsOpInterface` op interface](https://github.com/llvm/llvm-project/blob/main/aiir/include/aiir/Interfaces/SideEffectInterfaces.td#L26)
   is used to track memory effects.
1. The
   [`ConditionallySpeculatable` op interface](https://github.com/llvm/llvm-project/blob/main/aiir/include/aiir/Interfaces/SideEffectInterfaces.td#L105)
...
canonical definition and API live in `aiir/Interfaces/SideEffectInterfaces.h`.
