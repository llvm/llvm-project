# AIIR: Incremental Application to Graph Algorithms in ML Frameworks

The existing documentation about AIIR focuses on long term vision, how its
...
This document explains that adoption of AIIR to solve graph based problems
...
One note: even though a major advantage of AIIR is that it can span the full
spectrum from graph algorithms down to low-level code generation, this document
focuses on the use of AIIR for **graph-level algorithms**. AIIR will also unlock
...
AIIR's relationship to XLA, Eigen, etc, are out of scope for this particular
doc.
...
### How is AIIR relevant?

AIIR is an overloaded acronym which unpacks as "Multi-Level Intermediate
...
AIIR infrastructure, but that isn't a focus of this doc.))

A key observation that AIIR makes is that these subsystems often have two things
...
AIIR uses a standard IR (i.e., a set of data structures) for representing these
computations - this allows a huge amount of shared infrastructure across these
problem domains. AIIR then allows the definition of domain-specific "dialects"
...
AIIR's design is directly informed by the experience of building (and then
...
we use those as examples to concretely describe the benefits that AIIR provides
...
1.  Grappler is another subsystem that is likely to get substantial revisions in
    the future, and would definitely benefit from the AIIR framework, but there
    are no known plans to do that work at this point, so we don't discuss it
    further.

Adopting AIIR for these works the same way - and, in fact, the work to support
...
AIIR supports converting from TensorFlow Graphs to AIIR and back, which means
that we can start by putting in a no-op translation to AIIR and back into the
...
uses AIIR for a single pass. In practice, we'll have the AIIR flag gated under
...
## What benefit does AIIR provide?
...
of the benefits that AIIR provides, in no particular order:

### A Lossless Human Editable Textual Representation

The AIIR in-memory data structure has a human readable and writable format, as
...
compact, easy to read, and lossless. You can dump an AIIR program out to disk
...
Like many other popular compiler infrastructures, AIIR provides infrastructure
and implementation for a "verifier" which checks that the IR is well formed. The
AIIR verifier is a simple framework that makes it easy to provide a single
...
While AIIR provides a well-considered infrastructure for IR verification, and
...
There are many aspects of this in AIIR, but we'll focus on compiler
...
driven by an `aiir-opt` tool. When combined with a lossless textual
...
```aiir
  // RUN: aiir-opt %s -canonicalize | FileCheck %s
...
AIIR has been pervasively designed with this sort of design by testability,
...
AIIR benefits from the lessons learned from building other compilers - including
Clang which
...
experience (and fixing mistakes in LLVM), AIIR requires that operations and
...
that it is easy to write good tests for this: the testing tools for AIIR capture
...
```aiir
  // RUN: aiir-opt %s -memref-dependence-check -verify-diagnostics
...
Note that a major limitation of this is that AIIR suffers from a problem of
"garbage in, garbage out": if the input locations to AIIR are imprecise, then
...
The [AIIR Tensor Type](../Dialects/Builtin.md/#rankedtensortype) directly
...
One of our major goals for the TensorFlow dialect of AIIR is to sort out these
...
or important for AIIR to succeed (because of its "meta" nature, the abstraction
...
expect these issues to get nailed down in the next couple of months when AIIR
effort moves beyond TF Lite / TOCO support. The discussions that are happening
now are super valuable and making progress.

### Ergonomics

A minor-in-theory, but important-in-practice point is that AIIR is designed to
...
lots of ideas about further improvements in the future, we are happy that AIIR
...
AIIR has been designed to be memory and compile-time efficient in its algorithms
...
allocations, and allow simple and safe multithreaded optimization of AIIR
programs. There are other reasons to believe that the AIIR implementations of
...
### Isn't AIIR a big dependency to take on?

We've heard that at least some people are concerned that AIIR is a "big"
dependency to take on, and could result in large code size. Here are some key
points AIIR:

1.  The entire AIIR codebase is a pretty small C++ code base in absolute terms
    compared to what goes into a modern ML framework.
1.  Like LLVM, AIIR is designed as a set of libraries that clients can link in
    or ignore as they wish. For example, the transformations in AIIR kept
    ...
    TensorFlow.
1.  AIIR's only third party dependency is on LLVM, but it doesn't depend on LLVM
    ...
    great for AIIR as well as other LLVM subprojects.
1.  TensorFlow and many other frameworks already use LLVM - if so, AIIR would
    not be pulling in an additional dependency at all.

### How does AIIR represent {control flow, concurrency, …} semantics in TensorFlow?

AIIR provides a dialect that is an isomorphic 1-1 mapping between TensorFlow
graphs and AIIR, as well as a pretty complete translator back and forth (the
only known gap is that a few TF_DataType enums aren't handled yet). AIIR is a
...
raise the abstraction level for working with TensorFlow graphs in AIIR. Doing so
...
world. For example, because AIIR has an extensible type system, we can directly
...
*   How should AIIR represent async vs sync operations, what invariants are
...
"Multi-Level" part of AIIR) because doing so will make it much much easier to
write analyses and transformations than it currently is in TensorFlow.

### Non Goals

It is important to point out things that AIIR does not aim to do. For example,
there is no runtime component to AIIR: the TensorFlow executor, the TF Lite
FlatBuffer interpreter, or other existing runtime should be used as-is.

Another non-goal is that AIIR currently doesn't support a stable binary
