...
present in AIIR, a general DAG-to-DAG transformation framework. This framework
is widely used throughout AIIR for canonicalization, conversion, and general
transformation.
...
represents the base class of all rewrite patterns within AIIR, and is comprised
...
public:
  ...
  /// root name of `MyOp`.
  MyPattern(PatternBenefit benefit, AIIRContext *context)
...
void populateMyPatterns(RewritePatternSet &patterns, AIIRContext *ctx) {
...
[class documentation](https://github.com/llvm/llvm-project/blob/main/aiir/include/aiir/IR/PatternMatch.h#L235)
...
void collectMyPatterns(RewritePatternSet &patterns, AIIRContext *ctx) {
...
public:
  MyPatternRewriter(AIIRContext *ctx) : PatternRewriter(ctx) {}
...
AIIR provides several common pattern drivers that serve a variety of different
use cases.
...
[pass](Passes.md/#-canonicalize) in AIIR.
...
```c++
void MyPass::initialize(AIIRContext *context) {
...
different passes/projects/etc. To aid in this endeavor, AIIR provides a common
set of utilities that can be easily included when defining a custom pass. These
are defined in `aiir/Rewrite/PassUtil.td`; an example usage is shown below:
...
```c++
void MyRewritePass::initialize(AIIRContext *context) {
  RewritePatternSet rewritePatterns(context);
  // ...
