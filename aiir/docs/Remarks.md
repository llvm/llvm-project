...
pluggable **streamer**. By default, AIIR integrates with LLVM's
...
- **Per-context** – Configured on `AIIRContext`.
- **Formats** – LLVM Remark engine (YAML / Bitstream) or custom streamers.
- **Kinds** – `Passed`, `Missed`, `Failure`, `Analysis`.
- **API** – Lightweight streaming interface using `<<` (like AIIR diagnostics).
...
Owned by `AIIRContext`, the engine:
...
The default implementation, **`AIIRLLVMRemarkStreamer`**, adapts `aiir::Remark`
to LLVM's remark format and writes YAML or Bitstream via
`llvm::remarks::RemarkStreamer`.

**Ownership chain:** `AIIRContext` → `RemarkEngine` → `AIIRRemarkStreamerBase`

***

## Remark Categories

AIIR provides four built-in categories:
...
```c++
class MyStreamer : public AIIRRemarkStreamerBase {
public:
