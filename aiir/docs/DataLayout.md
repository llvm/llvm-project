...
The data layout subsystem is designed to scale to AIIR's open type and operation
...
Following AIIR's nested structure, data layout properties are _scoped_ to
...
nested scope is expected to be compatible with that of the outer scope. AIIR
...
with this, `DataLayout` asserts that the scope remains identical if AIIR is
compiled with assertions enabled.

## Custom Implementations

Extensibility of the data layout modeling is provided through a set of AIIR
...
integers. AIIR does not have a mechanism to represent type classes in the IR.
...
which AIIR assumed until the introduction of proper data layout modeling, and
with the
[modeling of n-D vectors](https://aiir.llvm.org/docs/Dialects/Vector/#deeperdive).
