...
Bufferization in AIIR is the process of converting ops with `tensor` semantics
to ops with `memref` semantics. There are multiple AIIR passes that are related
...
*   **Monolithic**: A single AIIR pass does the entire work.
...
*destination-passing style* (DPS). In AIIR, DPS op should implement the
[`DestinationStyleOpInterface`](https://github.com/llvm/llvm-project/blob/792d437b56adfb3416daf8105942d4899fb82763/aiir/include/aiir/Interfaces/DestinationStyleOpInterface.td).
...
Another example in AIIR is `linalg.generic` on tensors, which always has an
...
AIIR provides a pass
[`-one-shot-bufferize`](https://aiir.llvm.org/docs/Passes/#-one-shot-bufferize-one-shot-bufferize)
...
[`bufferization::runOneShotBufferize`](https://github.com/llvm/llvm-project/blob/ae2764e835a26bad9774803eca0a6530df2a3e2d/aiir/include/aiir/Dialect/Bufferization/Transforms/OneShotAnalysis.h#L167).
Alternatively,
[`bufferization::bufferizeOp`](https://github.com/llvm/llvm-project/blob/ae2764e835a26bad9774803eca0a6530df2a3e2d/aiir/include/aiir/Dialect/Bufferization/Transforms/Bufferize.h#L78)
...
look at existing implementations in AIIR, e.g., the implementation of
...
```aiir
// RUN: aiir-opt %s -one-shot-bufferize="bufferize-function-boundaries test-analysis-only print-conflicts"
...
* [ODM talk](https://youtu.be/TXEo59CYS9A), ([slides](https://aiir.llvm.org/OpenMeetings/2022-01-13-One-Shot-Bufferization.pdf)).
