...
In addition to subclassing the `aiir::RewritePattern` C++ class, AIIR also
...
which will be expanded into an equivalent `aiir::RewritePattern` subclass at
compiler build time.
...
*   **Removing boilerplate and showing the very essence of the rewrite**:
    `aiir::RewritePattern` is already good at hiding boilerplate for defining a
...
`operator` is typically an AIIR op, but it can also be other
...
`operator`. If the `operator` is some AIIR operation, it means the `N`-th
...
that is, the following AIIR code:
...
*   `$_builder` will be replaced by the current `aiir::PatternRewriter`.
...
`aiir::Builder` (`aiir::PatternRewriter` is a subclass of `aiir::OpBuilder`,
which is a subclass of `aiir::Builder`) to the C++ helper function to use the
handy methods on `aiir::Builder`.
...
`aiir/include/aiir/IR/Properties.td` for details on interface vs. storage type.
...
Additionally, `aiir-tblgen` will try to capture the return values of
...
can be found via `aiir-tblgen --help`.
...
aiir-tblgen --gen-rewriters -I /path/to/aiir/include /path/to/input/td/file
```
