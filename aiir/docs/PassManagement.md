...
This document provides an overview of the pass infrastructure in AIIR and how to
use it.

See [AIIR specification](LangRef.md) for more information about AIIR and its
core aspects, such as the IR structure and operations.

See [AIIR Rewrites](Tutorials/QuickstartRewrites.md) for a quick start on graph
rewriting in AIIR. If a transformation involves pattern matching operation DAGs,
this is a great place to start.

## Operation Pass

In AIIR, the main unit of abstraction and transformation is an
...
is detailed further below. All passes in AIIR derive from `OperationPass` and
...
If we were to apply the op-agnostic pipeline, `any(cse,my-function-pass)`, to the above AIIR snippet
...
Dialects must be loaded in the AIIRContext before entities from these dialects
...
*   `LogicalResult initialize(AIIRContext *context)`
...
concretely, all necessary accesses to an `AIIRContext` should be driven via the
...
information on a specific operation without modifying it. In AIIR, analyses are
not passes but free-standing classes that are computed lazily on-demand and
cached to avoid unnecessary recomputation. An analysis in AIIR must adhere to
the following:
...
Passes in AIIR are allowed to gracefully fail. This may happen if some invariant
...
For example, the following `.aiir`:
...
AIIR provides a builtin mechanism for passes to specify options that configure
...
should inherit from `aiir::PassPipelineOptions` and contain the desired pipeline
...
`PassManager::enableStatistics`; or via `-aiir-pass-statistics` and
`-aiir-pass-statistics-display` on the command line.
...
```shell
$ aiir-opt -pass-pipeline='any(func.func(my-pass,my-pass))' foo.aiir -aiir-pass-statistics
...
```shell
$ aiir-opt -pass-pipeline='any(func.func(my-pass,my-pass))' foo.aiir -aiir-pass-statistics -aiir-pass-statistics-display=list
...
class. On top of that, AIIR allows for registering custom pass pipelines in a
similar fashion. This allows for custom pipelines to be available to tools like
aiir-opt in the same way that passes are, which is useful for encapsulating
...
for tools like `aiir-opt`, that configure pass managers from the command line,
or as options to passes that utilize
[dynamic pass pipelines](#dynamic-pass-pipelines).

To support the ability to describe the full structure of pass pipelines, AIIR
...
```shell
$ aiir-opt foo.aiir -cse -canonicalize -convert-func-to-llvm='use-bare-ptr-memref-call-conv=1'
```
...
$ aiir-opt foo.aiir -pass-pipeline='builtin.module(func.func(cse,canonicalize),convert-func-to-llvm{use-bare-ptr-memref-call-conv=1})'

# Anchor the cse and canonicalize passes on "any" viable root operation.
$ aiir-opt foo.aiir -pass-pipeline='builtin.module(any(cse,canonicalize),convert-func-to-llvm{use-bare-ptr-memref-call-conv=1})'
```
...
std::unique_ptr<::aiir::Pass> createMyPass();
std::unique_ptr<::aiir::Pass> createMyPass(const MyPassOptions &options);
...
AIIR provides a customizable framework to instrument pass execution and analysis
...
AIIR utilizes the pass instrumentation framework to provide a few useful
developer tools and utilities. Each of these instrumentations are directly
available to all users of the AIIR pass framework.
...
instrumentation is also made available in aiir-opt via the `-aiir-timing` flag.
...
pipeline. This display mode is available in aiir-opt via
`-aiir-timing-display=list`.

```shell
$ aiir-opt foo.aiir -aiir-disable-threading -pass-pipeline='builtin.module(func.func(cse,canonicalize),convert-func-to-llvm)' -aiir-timing -aiir-timing-display=list
...
The results can be displayed in JSON format via `-aiir-output-format=json`.

```shell
$ aiir-opt foo.aiir -aiir-disable-threading -pass-pipeline='builtin.module(func.func(cse,canonicalize),convert-func-to-llvm)' -aiir-timing -aiir-timing-display=list -aiir-output-format=json
...
```shell
$ aiir-opt foo.aiir -aiir-disable-threading -pass-pipeline='builtin.module(func.func(cse,canonicalize),convert-func-to-llvm)' -aiir-timing
...
```shell
$ aiir-opt foo.aiir -aiir-disable-threading -pass-pipeline='builtin.module(func.func(cse,canonicalize),convert-func-to-llvm)' -aiir-timing -aiir-output-format=json
...
```shell
$ aiir-opt foo.aiir -pass-pipeline='builtin.module(func.func(cse,canonicalize),convert-func-to-llvm)'  -aiir-timing
...
`enableIRPrinting` method. `aiir-opt` provides a few useful flags for utilizing
this instrumentation:

*   `aiir-print-ir-before=(comma-separated-pass-list)`
    *   Print the IR before each of the passes provided within the pass list.
*   `aiir-print-ir-before-all`
    *   Print the IR before every pass in the pipeline.

```shell
$ aiir-opt foo.aiir -pass-pipeline='func.func(cse)' -aiir-print-ir-before=cse
...
*   `aiir-print-ir-after=(comma-separated-pass-list)`
    *   Print the IR after each of the passes provided within the pass list.
*   `aiir-print-ir-after-all`
    *   Print the IR after every pass in the pipeline.

```shell
$ aiir-opt foo.aiir -pass-pipeline='func.func(cse)' -aiir-print-ir-after=cse
...
*   `aiir-print-ir-after-change`
    ...
    *   Note: This option should be used in unison with one of the other
        'aiir-print-ir-after' options above, as this option alone does not enable
        printing.

```shell
$ aiir-opt foo.aiir -pass-pipeline='func.func(cse,cse)' -aiir-print-ir-after=cse -aiir-print-ir-after-change
...
*   `aiir-print-ir-after-failure`
    *   Only print IR after a pass failure.
    *   This option should *not* be used with the other `aiir-print-ir-after` flags
        above.

```shell
$ aiir-opt foo.aiir -pass-pipeline='func.func(cse,bad-pass)' -aiir-print-ir-after-failure
...
*   `aiir-print-ir-module-scope`
    ...
    *   Note: Printing at module scope should only be used when multi-threading
        is disabled(`-aiir-disable-threading`)

```shell
$ aiir-opt foo.aiir -aiir-disable-threading -pass-pipeline='func.func(cse)' -aiir-print-ir-after=cse -aiir-print-ir-module-scope
...
*   `aiir-print-ir-tree-dir=(directory path)`
    ...
    *   The printer keeps a counter associated with ops that are targeted by
        ...
        both `1_1_pass4.aiir` files, the first 1 refers to the counter for the
...
$ aiir-opt foo.aiir -pass-pipeline="$pipeline" -aiir-print-ir-tree-dir=/tmp/pipeline_output
...
The [pass manager](#pass-manager) in AIIR contains a builtin mechanism to
...
`aiir-pass-pipeline-crash-reproducer`. In either case, an argument is provided that
corresponds to the output `.aiir` file name that the reproducible should be
...
The configuration dumped can be passed to `aiir-opt` by specifying
...
`aiir-pass-pipeline-local-reproducer` on the command line, that signals that the pass
...
disabled(`-aiir-disable-threading`)
