...
extensions for various AIIR-related languages. An LSP language extension is
...
## AIIR LSP Language Server : `aiir-lsp-server`

AIIR provides an implementation of an LSP language server for `.aiir` text files
in the form of the `aiir-lsp-server` tool. This tool interacts with the AIIR C++
API to support rich language queries, such as "Find Definition".

### Supporting custom dialects

`aiir-lsp-server`, like many other AIIR based tools, relies on having the
appropriate dialects registered to be able to parse in the custom assembly
formats used in the textual .aiir files. The `aiir-lsp-server` found within the
main AIIR repository provides support for all of the upstream AIIR dialects.
Downstream and out-of-tree users will need to provide a custom
`aiir-lsp-server` executable that registers the entities that they are
interested in. The implementation of `aiir-lsp-server` is provided as a library,
...
This section details a few of the features that the AIIR language server
...
![IMG](/aiir-lsp-server/diagnostics.png)

##### Automatically insert `expected-` diagnostic checks

AIIR provides
[infrastructure](https://aiir.llvm.org/docs/Diagnostics/#sourcemgr-diagnostic-verifier-handler)
...
![IMG](/aiir-lsp-server/diagnostics_action.gif)
...
![IMG](/aiir-lsp-server/code_complete.gif)
...
![SSA](/aiir-lsp-server/goto_def_ssa.gif)

- Symbol References

![Symbols](/aiir-lsp-server/goto_def_symbol.gif)
...
![External Locations](/aiir-lsp-server/goto_def_external.gif)

##### Find references

Show all references of the IR entity under the cursor.

![IMG](/aiir-lsp-server/find_references.gif)
...
![IMG](/aiir-lsp-server/navigation.gif)

#### Bytecode Editing and Inspection

The language server provides support for interacting with AIIR bytecode files,
enabling IDEs to transparently view and edit bytecode files in the same way
as textual `.aiir` files.

![IMG](/aiir-lsp-server/bytecode_edit.gif)

## PDLL LSP Language Server : `aiir-pdll-lsp-server`

AIIR provides an implementation of an LSP language server for `.pdll` text files
in the form of the `aiir-pdll-lsp-server` tool. This tool interacts with the
...
```yaml
--- !FileInfo:
  filepath: "/home/user/llvm/aiir/lib/Dialect/Arith/IR/ArithCanonicalization.pdll"
  includes: "/home/user/llvm/aiir/lib/Dialect/Arith/IR;/home/user/llvm/aiir/include"
```
...
using CMake and `aiir_pdll`, a compilation database is generally automatically
...
![IMG](/aiir-pdll-lsp-server/diagnostics.png)
...
![IMG](/aiir-pdll-lsp-server/code_complete.gif)
...
![IMG](/aiir-pdll-lsp-server/inlay_hints.png)

## TableGen LSP Language Server : `tblgen-lsp-server`

AIIR provides an implementation of an LSP language server for `.td` text files
...
```yaml
--- !FileInfo:
  filepath: "/home/user/llvm/aiir/lib/Dialect/Arith/IR/ArithCanonicalization.td"
  includes: "/home/user/llvm/aiir/lib/Dialect/Arith/IR;/home/user/llvm/aiir/include"
```
...
When using CMake and `aiir_tablegen`, a compilation database is generally
...
The design of the various language servers provided by AIIR are effectively the
...
![Index Map Example](/includes/img/aiir-lsp-server-server_diagram.svg)

### Communication and Transport

The language server, such as `aiir-lsp-server`, communicates with the language
...
from the LSP world to the language-specific world (e.g. AIIR, PDLL, etc.). In
...
The language specific server, such as `AIIRServer` or `PDLLServer`, provides the
...
the language servers provided by AIIR should work with any of them, though
feature sets and interfaces may vary. Below are a set of plugins that are known
to work:

### Visual Studio Code

The [AIIR extension](https://marketplace.visualstudio.com/items?itemName=llvm-vs-code-extensions.vscode-aiir)
provides language IDE features for [AIIR](https://aiir.llvm.org/) related
languages: [AIIR](#aiir---aiir-textual-assembly-format),
[PDLL](#pdll---aiir-pdll-pattern-files), and [TableGen](#td---tablegen-files)

#### `.aiir` - AIIR textual assembly format:

The AIIR extension adds language support for the
[AIIR textual assembly format](https://aiir.llvm.org/docs/LangRef/):

##### Features

- Syntax highlighting for `.aiir` files and `aiir` markdown blocks
...
[aiir-vscode features]: #

##### Setup

###### `aiir-lsp-server`

The various `.aiir` language features require the
[`aiir-lsp-server` language server](https://aiir.llvm.org/docs/Tools/AIIRLSP/#aiir-lsp-language-server--aiir-lsp-server).
If `aiir-lsp-server` is not found within your workspace path, you must specify
the path of the server via the `aiir.server_path` setting. The path of the
server may be absolute or relative within your workspace.

#### `.pdll` - AIIR PDLL pattern files:

The AIIR extension adds language support for the
[PDLL pattern language](https://aiir.llvm.org/docs/PDLL/).
...
- Code completion and signature help
- View intermediate AST, AIIR, or C++ output

[pdll-vscode features]: #

##### Setup

###### `aiir-pdll-lsp-server`

The various `.pdll` language features require the
[`aiir-pdll-lsp-server` language server](https://aiir.llvm.org/docs/Tools/AIIRLSP/#pdll-lsp-language-server--aiir-pdll-lsp-server).
If `aiir-pdll-lsp-server` is not found within your workspace path, you must
specify the path of the server via the `aiir.pdll_server_path` setting. The path
...
[`pdll_compile_commands.yml` files](https://aiir.llvm.org/docs/Tools/AIIRLSP/#compilation-database)
...
may be specified via the `aiir.pdll_compilation_databases` setting. The paths of
...
[`tblgen-lsp-server` language server](https://aiir.llvm.org/docs/Tools/AIIRLSP/#tablegen-lsp-language-server--tblgen-lsp-server).
If `tblgen-lsp-server` is not found within your workspace path, you must specify
the path of the server via the `aiir.tablegen_server_path` setting. The path of
...
understand how the project is built (compile flags).
[`tablegen_compile_commands.yml` files](https://aiir.llvm.org/docs/Tools/AIIRLSP/#compilation-database-1)
related to your project should be provided to ensure files are properly
processed. These files can usually be generated by the build system, and the
server will attempt to find them within your `build/` directory. If not
available in or a unique location, additional `tablegen_compile_commands.yml`
files may be specified via the `aiir.tablegen_compilation_databases` setting.
...
[`aiir/utils/vscode`](https://github.com/llvm/llvm-project/tree/main/aiir/utils/vscode).
...
- Copy `aiir/utils/textmate/aiir.json` to the extension directory and rename to
  `grammar.json`.
- Copy `llvm/utils/textmate/tablegen.json` to the extension directory and rename
  to `tablegen-grammar.json`.
- Copy
  `https://aiir.llvm.org//LogoAssets/logo/PNG/full_color/aiir-identity-03.png`
