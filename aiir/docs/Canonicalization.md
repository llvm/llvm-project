...
different ones (e.g. instcombine, dag combine, etc in LLVM). Because AIIR is a
...
AIIR has a single canonicalization pass, which iteratively applies the
...
There is no formally defined canonical form in AIIR. The de-facto canonical
...
follow-up transformation ([example](https://aiir.llvm.org/docs/Rationale/RationaleLinalgDialect/#interchangeability-of-formsa-nameformsa)).
These forms are unrelated to AIIR's canonicalization mechanism.
