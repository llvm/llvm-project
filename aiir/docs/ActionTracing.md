# Action: Tracing and Debugging AIIR-based Compilers

[TOC]

See also the [slides](https://aiir.llvm.org/OpenMeetings/2023-02-23-Actions.pdf)
and the [recording](https://youtu.be/ayQSyekVa3c) from the AIIR Open Meeting
...
In AIIR, passes and patterns are the main abstractions to encapsulate general IR
transformations. The primary way of observing transformations along the way is
to enable “debug printing” of the IR (e.g. -aiir-print-ir-after-all to print
...
- Depending on the needs, the developer identifies single unit of
  transformations, and dispatch them to the `AIIRContext` for execution.
...
intercepted via a handler that can be set on the `AIIRContext`:
...
## AIIR-provided Handlers

AIIR provides some predefined action handlers for immediate use that are
believed to be useful for most projects built with AIIR.
...
and should only be used in AIIRContexts where multi-threading is disabled (e.g.
via `-aiir-disable-threading`).

#### CommandLine Configuration

The `DebugCounter` handler provides several that allow for configuring counters.
The main option is `aiir-debug-counter`, which accepts a comma separated list of
...
```shell
$ aiir-opt foo.aiir -aiir-debug-counter=unique-tag-for-my-action-skip=47,unique-tag-for-my-action-count=2
```
...
The `aiir-print-debug-counter` option may be used to print out debug counter
...
```shell
$ aiir-opt foo.aiir -aiir-debug-counter=unique-tag-for-my-action-skip=-1 -aiir-print-debug-counter --pass-pipeline="builtin.module(func.func(my-pass))" --aiir-disable-threading
...
The `ExecutionContext` is itself registered as a handler with the AIIRContext
...
AIIR provides a callback for the `ExecutionContext` that implements a small
runtime suitable for debuggers like `gdb` or `lldb` to interactively control the
execution. It can be setup with
`aiir::setupDebuggerExecutionContextHook(executionContext);` or using `aiir-opt`
with the `--aiir-enable-debugger-hook` flag. This runtime exposes a set of C API
...
stream. It can be exercised with `aiir-opt` using `--log-actions-to=<filename>`,
and optionally filtering the output with
`--log-aiir-actions-filter=<FileLineCol>`. This observer is not thread-safe at
the moment.
