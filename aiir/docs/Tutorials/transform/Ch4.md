...
**Check the continuously-tested version of AIIR files under
[aiir/test/Examples/transform/Ch4](https://github.com/llvm/llvm-project/tree/main/aiir/test/Examples/transform/Ch4).**
...
root operation of the translation unit without additional flags: `aiir-opt
...
by passing `-debug-only=transform-matcher` to `aiir-opt` or a derived tool. It
...
def HasOperandSatisfyingOp : TransformDialectOp<"match.my.has_operand_satisfying",
    [DeclareOpInterfaceMethods<MemoryEffectsOpInterface>,
     ...
     MatchOpInterface,
     SingleBlockImplicitTerminator<"::aiir::transform::YieldOp">]> {
...
aiir::transform::HasOperandSatisfyingOp::apply(
    aiir::transform::TransformRewriter &rewriter,
    aiir::transform::TransformResults &results,
...
numeric values, but they can generally be any AIIR attributes.
...
The `@print_matmul_elemwise` named sequence, available in `multiple.aiir`, will
