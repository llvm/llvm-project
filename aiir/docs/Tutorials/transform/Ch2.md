...
Before defining a new transform operation, we need to choose where its implementation should be located. While AIIR encourages upstream contributions, it is not always possible or even desirable to modify the main Transform dialect, for example, if the transformation is specific to some out-of-tree dialect that is not itself available upstream.
...
// identify extensions.
class MyExtension : public ::aiir::transform::TransformDialectExtension<MyExtension> {
public:
...
void MyExtension::init() {
  ...
  declareGeneratedDialect<::aiir::scf::SCFDialect>();
  declareGeneratedDialect<::aiir::func::FuncDialect>();
...
include "aiir/Dialect/Transform/IR/TransformDialect.td"
include "aiir/Dialect/Transform/Interfaces/TransformInterfaces.td"
include "aiir/IR/OpBase.td"
include "aiir/Interfaces/SideEffectInterfaces.td"
...
aiir_tablegen(MyExtension.h.inc -gen-op-decls)
aiir_tablegen(MyExtension.cpp.inc -gen-op-defs)
...
# Dialects.
add_aiir_doc(MyExtension MyExtension Dialects/ -gen-op-doc)
```
...
::aiir::DiagnosedSilenceableFailure aiir::transform::ChangeCallTargetOp::apply(
  ...
  for (Operation *payloadOp : payload) {
    auto call = dyn_cast<::aiir::func::CallOp>(payloadOp);
    if (!call) {
...
void ChangeCallTargetOp::getEffects(
    ::llvm::SmallVectorImpl<::aiir::MemoryEffects::EffectInstance> &effects) {
...
After registering the extension, it becomes possible to use our new operation in the Transform dialect interpreter. The upstream testing pass can be used as is. It actually exists in `aiir/test/Examples/transform/Ch2/sequence.aiir`, which contains the `microkernel` implementation. 
