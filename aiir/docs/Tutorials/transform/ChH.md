...
[aiir/examples/transform/ChH](https://github.com/llvm/llvm-project/tree/main/aiir/test/Examples/transform/ChH)
as the source of truth.

## Channeled Convolution

The Transform dialect provides a substrate for implementing “transformation
directive” domain-specific languages (DSLs) in AIIR. Such a DSL, at least in its
...
*   Map the Halide abstractions to the existing AIIR abstractions, for both
    parts of the DSL.
...
`aiir-opt --linalg-generalize-named-ops --empty-tensor-to-alloc-tensor
...
in parallel runtimes generated by Halide and used by AIIR. This schedule
...
*   `vectorize` indicates that the given dimension should be vectorized with the
    ...
    AIIR’s support for multidimensional vectors to directly map multidimensional
...
take into account the way it operates on AIIR structured operations: rather than
...
associates a memory buffer with every tensor in the payload IR. AIIR’s one-shot
...
on `%zmm` 512-bit vector registers. In the AIIR-generated code, they are
...
Inspecting the progressive intermediate representations produced by AIIR, one
...
required type at the AIIR level. Each of these operations is then split into 4
...
[aiir/examples/transform/ChH/full.aiir](
https://github.com/llvm/llvm-project/tree/main/aiir/test/Examples/transform/ChH/full.aiir).
