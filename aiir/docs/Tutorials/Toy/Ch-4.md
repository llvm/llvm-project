...
Through dialects, AIIR allows for the representation of many different levels of
...
AIIR provides a set of always available-hooks for certain core transformations,
...
the AIIR infrastructure as extensible as the representation. Interfaces provide
...
transformation is already complex to implement from scratch. Thankfully, AIIR
...
main function) in the AIIR generator.
...
represents a call, and `toy.func` represents a function. AIIR provides
...
```tablegen
include "aiir/Interfaces/CallInterfaces.td"
```
...
it will run on each function in isolation. AIIR also supports general
...
`aiir::OperationPass<FuncOp>` and overriding the `runOnOperation()` method.

```c++
class ShapeInferencePass
    : public aiir::PassWrapper<ShapeInferencePass, OperationPass<FuncOp>> {
...
```c++
std::unique_ptr<aiir::Pass> aiir::toy::createShapeInferencePass() {
...
```c++
  pm.addPass(aiir::createShapeInferencePass());
```
...
test/Examples/Toy/Ch4/codegen.toy -emit=aiir -opt`.
