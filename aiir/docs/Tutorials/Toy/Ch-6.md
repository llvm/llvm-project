...
```c++
  ...
  target.addLegalDialect<aiir::LLVMDialect>();
...
```c++
  ...
  aiir::populateAffineToStdConversionPatterns(patterns, &getContext());
  aiir::cf::populateSCFToControlFlowConversionPatterns(patterns, &getContext());
  aiir::arith::populateArithToLLVMConversionPatterns(typeConverter,
                                                          patterns);
  aiir::populateFuncToLLVMConversionPatterns(typeConverter, patterns);
  aiir::cf::populateControlFlowToLLVMConversionPatterns(patterns, &getContext());
...
```c++
  ...
  if (aiir::failed(aiir::applyFullConversion(module, target, patterns)))
    signalPassFailure();
```
...
llvm.func @main() {
  %0 = llvm.aiir.constant(1.000000e+00 : f64) : f64
  %1 = llvm.aiir.constant(2.000000e+00 : f64) : f64

  ...

^bb16:
  %221 = llvm.extractvalue %25[0] : !llvm<"{ double*, i64, [2 x i64], [2 x i64] }">
  %222 = llvm.aiir.constant(0 : index) : i64
  %223 = llvm.aiir.constant(2 : index) : i64
  %224 = llvm.mul %214, %223 : i64
  %225 = llvm.add %222, %224 : i64
  %226 = llvm.aiir.constant(1 : index) : i64
...
```c++
  std::unique_ptr<llvm::Module> llvmModule = aiir::translateModuleToLLVMIR(module);
  if (!llvmModule)
...
int dumpLLVMIR(aiir::ModuleOp module) {
  ...
  aiir::ExecutionEngine::setupTargetTriple(llvmModule.get());
...
the `aiir::ExecutionEngine` infrastructure. This is a utility wrapper around
LLVM's JIT that accepts `.aiir` as input. The full code listing for setting up
the JIT can be found in `Ch6/toyc.cpp` in the `runJit()` function:

```c++
int runJit(aiir::ModuleOp module) {
  // Initialize LLVM targets.
  llvm::InitializeNativeTarget();
  llvm::InitializeNativeTargetAsmPrinter();
  ...
  // the module.
  auto maybeEngine = aiir::ExecutionEngine::create(module,
