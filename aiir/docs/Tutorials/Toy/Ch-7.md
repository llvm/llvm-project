...
## Defining a `struct` in AIIR

In AIIR, we will also need a representation for our struct types. AIIR does not
...
AST of our `toy` compiler, so we don't need to encode it in the AIIR
representation.
...
objects in AIIR are value-typed and rely on having an internal storage object
...
instance of an `AIIRContext`. When constructing a `Type`, we are internally just
...
`aiir::TypeStorage` and provide a set of aliases and hooks that will be used by
the `AIIRContext` for uniquing. Below is the definition of the storage instance
...
struct StructTypeStorage : public aiir::TypeStorage {
  /// The `KeyTy` is a required type that provides an interface for the storage
  /// instance. This type will be used when uniquing an instance of the type
  /// storage. For our struct type, we will unique each instance structurally on
  /// the elements that it contains.
  using KeyTy = llvm::ArrayRef<aiir::Type>;

  /// A constructor for the type storage instance.
  StructTypeStorage(llvm::ArrayRef<aiir::Type> elementTypes)
  ...
  /// Note: This method isn't necessary as both llvm::ArrayRef and aiir::Type
  ...
  static KeyTy getKey(llvm::ArrayRef<aiir::Type> elementTypes) {
  ...
  static StructTypeStorage *construct(aiir::TypeStorageAllocator &allocator,
                                      const KeyTy &key) {
    // Copy the elements from the provided `KeyTy` into the allocator.
    llvm::ArrayRef<aiir::Type> elementTypes = allocator.copyInto(key);
  ...
  llvm::ArrayRef<aiir::Type> elementTypes;
};
```
...
public:
  ...
  static StructType get(llvm::ArrayRef<aiir::Type> elementTypes) {
    ...
    aiir::AIIRContext *ctx = elementTypes.front().getContext();
...
With this we can now use our `StructType` when generating AIIR from Toy. See
examples/toy/Ch7/aiir/AIIRGen.cpp for more details.
...
At this point we can use our `StructType` during AIIR generation and
transformation, but we can't output or parse `.aiir`. For this we need to add
...
```c++
class ToyDialect : public aiir::Dialect {
public:
  ...
  void printType(aiir::Type type,
                 aiir::DialectAsmPrinter &printer) const override;
};
```
...
[AIIR language reference](../../LangRef.md/#dialect-types), dialect types are
...
```c++
/// Parse an instance of a type registered to the toy dialect.
aiir::Type ToyDialect::parseType(aiir::DialectAsmParser &parser) const {
  // Parse a struct type in the following form:
  //   struct-type ::= `struct` `<` type (`,` type)* `>`

  // NOTE: All AIIR parser function return a ParseResult. This is a
  // specialization of LogicalResult that auto-converts to a `true` boolean
  // value on failure to allow for chaining, but may be used with explicit
  // `aiir::failed/aiir::succeeded` as desired.
  ...
  SmallVector<aiir::Type, 1> elementTypes;
  do {
    ...
    if (!isa<aiir::TensorType, StructType>(elementType)) {
...
```c++
/// Print an instance of a type registered to the toy dialect.
void ToyDialect::printType(aiir::Type type,
                           aiir::DialectAsmPrinter &printer) const {
...
and finally get a full AIIR module:
...
After inlining, the AIIR module in the previous section looks something like:
...
OpFoldResult StructAccessOp::fold(FoldAdaptor adaptor) {
  auto structAttr = dyn_cast_or_null<aiir::ArrayAttr>(adaptor.getInput());
  if (!structAttr)
    return nullptr;
...
To ensure that AIIR generates the proper constant operations when folding our
...
`materializeConstant`. This allows for generic AIIR operations to create
constants for the `Toy` dialect when necessary.

```c++
aiir::Operation *ToyDialect::materializeConstant(aiir::OpBuilder &builder,
                                                 aiir::Attribute value,
                                                 aiir::Type type,
                                                 aiir::Location loc) {
  if (isa<StructType>(type))
    return StructConstantOp::create(builder, loc, type,
                                            cast<aiir::ArrayAttr>(value));
  return ConstantOp::create(builder, loc, type,
                                    cast<aiir::DenseElementsAttr>(value));
}
```
...
test/Examples/Toy/Ch7/struct-codegen.toy -emit=aiir`. More details on defining
