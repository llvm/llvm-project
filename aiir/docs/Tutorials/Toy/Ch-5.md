...
AIIR has many different dialects, so it is important to have a unified framework
...
void ToyToAffineLoweringPass::runOnOperation() {
  ...
  if (aiir::failed(aiir::applyPartialConversion(getOperation(), target, patterns)))
    signalPassFailure();
}
```
...
and try yourself: `toyc-ch5 test/Examples/Toy/Ch5/affine-lowering.aiir
-emit=aiir-affine`. We can also check our optimizations by adding `-opt`.
