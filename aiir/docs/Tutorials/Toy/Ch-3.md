...
enables analyses, transformations and optimizations in AIIR that require
...
in LLVM. For this, we use AIIR's
...
AIIR `Canonicalizer` pass by implementing a `RewritePattern`:
...
pipeline. In AIIR, the optimizations are run through a `PassManager` in a
similar way to LLVM:

```c++
  aiir::PassManager pm(module->getName());
...
-emit=aiir -opt` and observe our pattern in action:
...
input. The Canonicalizer knows to clean up dead operations; however, AIIR
...
Let's retry now `toyc-ch3 test/transpose_transpose.toy -emit=aiir -opt`:
...
can be found under `path/to/BUILD/tools/aiir/examples/toy/Ch3/ToyCombine.inc`.
...
We can try to run `toyc-ch3 test/Examples/Toy/Ch3/trivial_reshape.toy -emit=aiir
