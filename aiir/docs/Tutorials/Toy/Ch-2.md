...
infrastructure to support the need for these analyses and transformation. AIIR
addresses this issue by being designed for extensibility. As such, there are few
pre-defined instructions (*operations* in AIIR terminology) or types.

## Interfacing with AIIR

[Language Reference](../../LangRef.md)

AIIR is designed to be a completely extensible infrastructure; there is no
closed set of attributes (think: constant metadata), operations, or types. AIIR
...
In AIIR, [`Operations`](../../LangRef.md/#operations) are the core unit of
...
Here is the AIIR assembly for the Toy `transpose` operations:

```aiir
%t_tensor = "toy.transpose"(%tensor) {inplace = true} : (tensor<2x3xf64>) -> tensor<3x2xf64> loc("example/file/path":12:1)
```

Let's break down the anatomy of this AIIR operation:
...
the set of operations in AIIR is extensible. Operations are modeled
...
In AIIR, every operation has a mandatory source location associated with it.
Contrary to LLVM, where debug info locations are metadata and can be dropped, in
AIIR, the location is a core requirement, and APIs depend on and manipulate it.
...
It's worth noting that the aiir-opt tool - a tool for testing
compiler passes - does not include locations in the output by default. The
`-aiir-print-debuginfo` flag specifies to include locations. (Run `aiir-opt
--help` for more options.)

### Opaque API

AIIR is designed to allow all IR elements, such as attributes, operations, and
types, to be customized. At the same time, IR elements can always be reduced to
the above fundamental concepts. This allows AIIR to parse, represent, and
...
`.aiir` file and round-trip through *aiir-opt* without registering any `toy`
related dialect:
...
In the cases of unregistered attributes, operations, and types, AIIR will
enforce some structural constraints (e.g. dominance, etc.), but otherwise they
are completely opaque. For instance, AIIR has little information about whether
...
section, we will register our dialect and operations with AIIR, plug into the
verifier, and add nicer APIs to manipulate our operations.

## Defining a Toy Dialect

To effectively interface with AIIR, we will define a new Toy dialect. This
...
/// aiir::Dialect and registers custom attributes, operations, and types. It can
...
public:
  explicit ToyDialect(aiir::AIIRContext *ctx);
...
This is the C++ definition of a dialect, but AIIR also supports defining
...
To see what this generates, we can run the `aiir-tblgen` command with the
`gen-dialect-decls` action like so:

```shell
${build_root}/bin/aiir-tblgen -gen-dialect-decls ${aiir_src_root}/examples/toy/Ch2/include/toy/Ops.td -I ${aiir_src_root}/include/
```

After the dialect has been defined, it can now be loaded into an AIIRContext:

```c++
  context.loadDialect<ToyDialect>();
```

By default, an `AIIRContext` only loads the
...
`aiir::Op` class which also takes some optional [*traits*](../../Traits) to
...
```c++
class ConstantOp : public aiir::Op<
                     /// `aiir::Op` is a CRTP class, meaning that we provide the
                     /// derived class as a template parameter.
                     ConstantOp,
                     /// The ConstantOp takes zero input operands.
                     aiir::OpTrait::ZeroOperands,
                     /// The ConstantOp returns a single result.
                     aiir::OpTrait::OneResult,
                     /// We also provide a utility `getType` accessor that
                     /// returns the TensorType of the single result.
                     aiir::OpTrait::OneTypedResult<TensorType>::Impl> {

 public:
  /// Inherit the constructors from the base Op class.
  using Op::Op;

  /// Provide the unique name for this operation. AIIR will use this to register
  ...
  aiir::DenseElementsAttr getValue();
  ...
  ///   aiir::OpBuilder::create<ConstantOp>(...)
  /// This method populates the given `state` that AIIR uses to create
  /// operations. This state is a collection of all of the discrete elements
  /// that an operation may contain.
  /// Build a constant with the given return type and `value` attribute.
  static void build(aiir::OpBuilder &builder, aiir::OperationState &state,
                    aiir::Type result, aiir::DenseElementsAttr value);
...
### Op vs Operation: Using AIIR Operations

Now that we have defined an operation, we will want to access and transform it.
In AIIR, there are two main classes related to operations: `Operation` and `Op`.
...
value* is a common idiom in AIIR and applies similarly to attributes, types,
etc). Given a generic `Operation*` instance, we can always get a specific `Op`
instance using LLVM's casting infrastructure:

```c++
void processConstantOp(aiir::Operation *operation) {
  ...
  aiir::Operation *internalOperation = op.getOperation();
...
will be expanded into an equivalent `aiir::Op` C++ template specialization at
compile time. Using the ODS framework is the desired way for defining operations
in AIIR given the simplicity, conciseness, and general stability in the face of
C++ API changes.
...
TableGen looks like. Simply run the `aiir-tblgen` command with the
`gen-op-decls` or the `gen-op-defs` action like so:

```shell
${build_root}/bin/aiir-tblgen -gen-op-defs ${aiir_src_root}/examples/toy/Ch2/include/toy/Ops.td -I ${aiir_src_root}/include/
```
...
def ConstantOp : Toy_Op<"constant"> {
  ...
  // the `state` that AIIR uses to create operations, i.e. these are used when
...
`toy.transpose` at the beginning of this chapter. AIIR allows for operations to
...
void PrintOp::print(aiir::OpAsmPrinter &printer) {
...
aiir::ParseResult PrintOp::parse(aiir::OpAsmParser &parser,
                                 aiir::OperationState &result) {
  // Parse the input operand, the attribute dictionary, and the type of the
  // input.
  aiir::OpAsmParser::UnresolvedOperand inputOperand;
  aiir::Type inputType;
...
the above example: `toyc-ch2 test/Examples/Toy/Ch2/codegen.toy -emit=aiir
-aiir-print-debuginfo`. We can also check our RoundTrip: `toyc-ch2
test/Examples/Toy/Ch2/codegen.toy -emit=aiir -aiir-print-debuginfo 2>
codegen.aiir` followed by `toyc-ch2 codegen.aiir -emit=aiir`. You should also
use `aiir-tblgen` on the final definition file and study the generated C++ code.

At this point, AIIR knows about our Toy dialect and operations. In the
