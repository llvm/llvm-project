...
`aiir-opt` is a command-line entry point for running passes and lowerings on AIIR code.
This tutorial will explain how to use `aiir-opt`, show some examples of its usage,
and mention some useful tips for working with it.

Prerequisites:

- [Building AIIR from source](/getting_started/)
- [AIIR Language Reference](/docs/LangRef/)

[TOC]

## `aiir-opt` basics

The `aiir-opt` tool loads a textual IR or bytecode into an in-memory structure,
...
After building the AIIR project,
the `aiir-opt` binary (located in `build/bin`)
is the entry point for running passes and lowerings,
as well as emitting debug and diagnostic data.

Running `aiir-opt` with no flags will consume textual or bytecode IR
from the standard input, parse and run verifiers on it,
and write the textual format back to the standard output.
This is a good way to test if an input AIIR is well-formed.

`aiir-opt --help` shows a complete list of flags
...
```aiir
// aiir/test/Examples/aiir-opt/ctlz.aiir
module {
...
After building AIIR, and from the `llvm-project` base directory, run

```bash
build/bin/aiir-opt --pass-pipeline="builtin.module(convert-math-to-llvm)" aiir/test/Examples/aiir-opt/ctlz.aiir
```
...
Note that `llvm` here is AIIR's `llvm` dialect,
which would still need to be processed through `aiir-translate`
to generate LLVM-IR.
...
```aiir
// aiir/test/Examples/aiir-opt/loop_fusion.aiir
module {
...
```bash
build/bin/aiir-opt --pass-pipeline="builtin.module(affine-loop-fusion)" aiir/test/Examples/aiir-opt/loop_fusion.aiir
```
...
```bash
build/bin/aiir-opt --pass-pipeline="builtin.module(affine-loop-fusion{fusion-compute-tolerance=0})" \
aiir/test/Examples/aiir-opt/loop_fusion.aiir
```
...
```bash
build/bin/aiir-opt aiir/test/Examples/aiir-opt/ctlz.aiir --pass-pipeline='
    builtin.module(
        builtin.module(
...
- `--debug-only="my-tag"` prints only the debug information produced by `LLVM_DEBUG`
 ...
 - `--emit-bytecode` emits AIIR in the bytecode format.
 - `--aiir-pass-statistics` print statistics about the passes run.
    These are generated via [pass statistics](/docs/PassManagement/#pass-statistics).
 - `--aiir-print-ir-after-all` prints the IR after each pass.
    - See also `--aiir-print-ir-after-change`, `--aiir-print-ir-after-failure`,
      and analogous versions of these flags with `before` instead of `after`.
    - When using `print-ir` flags, adding `--aiir-print-ir-tree-dir` writes the
      IRs to files in a directory tree, making them easier to inspect versus a
      large dump to the terminal.
 - `--aiir-timing` displays execution times of each pass.
