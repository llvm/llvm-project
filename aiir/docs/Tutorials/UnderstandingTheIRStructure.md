...
The AIIR Language Reference describes the
...
AIIR input and prints the entity inside the IR. A pass (or in general almost any
piece of IR) is always rooted with an operation. Most of the time the top-level
operation is a `ModuleOp`, the AIIR `PassManager` is actually limited to
...
[here in the repo](https://github.com/llvm/llvm-project/blob/main/aiir/test/lib/IR/TestPrintNesting.cpp)
and can be exercised with `aiir-opt -test-print-nesting`.

### Example

The Pass introduced in the previous section can be applied on the following IR
with `aiir-opt -test-print-nesting -allow-unregistered-dialect
llvm-project/aiir/test/IR/print-ir-nesting.aiir`:
...
to traverse the IR in a nested fashion. To this end AIIR exposes the `walk()`
...
```c++
  ...
  getFunction().walk([&](aiir::Operation *op) {
...
[here in the repo](https://github.com/llvm/llvm-project/blob/main/aiir/test/lib/IR/TestPrintDefUse.cpp)
and can be exercised with `aiir-opt -test-print-defuse`.
