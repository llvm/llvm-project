# Writing DataFlow Analyses in AIIR

Writing dataflow analyses in AIIR, or well any compiler, can often seem quite
...
constructs, of which AIIR has many (Block-based branches, Region-based branches,
CallGraph, etc), and it isn't always clear how best to go about performing the
propagation. To help writing these types of analyses in AIIR, this document
...
writing a simple forward dataflow analysis in AIIR. Let's say for this analysis
...
For our analysis in AIIR, we will need to define a class representing the value
...
struct MetadataLatticeValue {
  ...
  static MetadataLatticeValue getPessimisticValueState(AIIRContext *context) {
...
public:
  ForwardDataFlowAnalysis(AIIRContext *context);
