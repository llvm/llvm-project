# Quickstart tutorial to adding AIIR graph rewrite
...
See [AIIR specification](../LangRef.md) for more information about AIIR, the
...
An operation in AIIR is specified using a definition in
...
*   The operation name. This name is a unique identifier of the operation within
    AIIR. Most operations are within a dialect, so for example one could have
...
There are multiple forms of graph rewrite that can be performed in AIIR. One of
...
static Value createTFLLeakyRelu(PatternRewriter &rewriter, Operation *op,
  ...
  return aiir::TFL::LeakyReluOp::create(rewriter,
...
The file containing the patterns need to be processed using `aiir-tblgen`
...
aiir_tablegen(<name-of-the-generated-inc-file> -gen-rewriters)
...
struct ConvertTFLeakyRelu : public RewritePattern {
  ConvertTFLeakyRelu(AIIRContext *context)
...
AIIR uses [lit](https://llvm.org/docs/CommandGuide/lit.html) (LLVM Integrated
...
```aiir
// RUN: aiir-opt -tfl-legalize-tf %s | FileCheck %s
...
The RUN command at the top results in running the `aiir-opt` binary (which is
...
`aiir-opt` invoked with `-split-input-file` flag. This is especially useful for
error testing.
