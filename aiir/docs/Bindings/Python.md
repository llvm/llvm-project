...
*   Installation of Python dependencies as specified in
    `aiir/python/requirements.txt`

### CMake variables

*   **`AIIR_ENABLE_BINDINGS_PYTHON`**`:BOOL`
...
*   **`AIIR_BINDINGS_PYTHON_NB_DOMAIN`**: `STRING`

    nanobind (and AIIR) domain within which extensions will be compiled. 
    ...
    Can also be passed explicitly to `add_aiir_python_modules`.
...
which python
python -m venv ~/.venv/aiirdev
source ~/.venv/aiirdev/bin/activate
```
...
```shell
uv venv ~/.venv/aiirdev --seed -p 3.12
source ~/.venv/aiirdev/bin/activate
```
...
python -m pip install -r aiir/python/requirements.txt

# In a uv-generated virtual environment, you can instead run:
uv pip install -r aiir/python/requirements.txt

# Now run your build command with `cmake`, `ninja`, et al.

# Run aiir tests. For example, to run Python bindings tests only using ninja:
ninja check-aiir-python
```

For interactive use, it is sufficient to add the
`tools/aiir/python_packages/aiir_core/` directory in your `build/` directory to
the `PYTHONPATH`. Typically:

```shell
export PYTHONPATH=$(cd build && pwd)/tools/aiir/python_packages/aiir_core
```
...
`python_packages/` (i.e. `python_packages/aiir_core`). Official distributions
...
There are likely two primary use cases for the AIIR Python bindings:

1.  Support users who expect that an installed version of LLVM/AIIR will yield
    the ability to `import aiir` and use the API in a pure way out of the box.
...
Generally, the C++ codebase namespaces most things into the `aiir` namespace.
...
of functional units in AIIR.

Examples:

*   `aiir.ir`
*   `aiir.passes` (`pass` is a reserved word :( )
*   `aiir.dialect`
*   `aiir.execution_engine` (aside from namespacing, it is important that
...
LLVM/AIIR is a non-trivial Python-native project that is likely to co-exist with
...
(`_aiir`), while a small set of Python code is provided in
`aiir/_cext_loader.py` and siblings which loads and re-exports it. This split
...
*   `PyContext` (`aiir.ir.Context`)
*   `PyModule` (`aiir.ir.Module`)
*   `PyOperation` (`aiir.ir.Operation`) - but with caveats
...
*   `PyLocation` (`loc: aiir.ir.Location = None`)
*   `PyInsertionPoint` (`ip: aiir.ir.InsertionPoint = None`)
*   `PyAiirContext` (`context: aiir.ir.Context = None`)
...
thread context manager (i.e. `DefaultingPyAiirContext` or
...
**Note**: Multiple `PyOperation` objects (i.e., the Python objects themselves) can alias a single `aiir::Operation`.
This means, for example, if you have `py_op1` and `py_op2` which wrap the same `aiir::Operation op`
and you somehow transform `op` (e.g., you run a pass on `op`) then walking the AIIR AST via either/or `py_op1`, `py_op2`
will reflect the same AIIR AST. This is perfectly safe and supported. What is not supported is invalidating any
...
`AiirRegion`/`AiirBlock` is considered benign and these objects are not interned
...
repeated arguments such as AIIR contexts, operation insertion points and
...
An AIIR context is a top-level entity that owns attributes and types and is
referenced from virtually all IR constructs. Contexts also provide thread safety
at the C++ level. In Python bindings, the AIIR context is also a Python context
manager, one can write:

```python
from aiir.ir import Context, Module

with Context() as ctx:
  # IR construction using `ctx` as context.

  # For example, parsing an AIIR module from string requires the context.
...
Note that it is possible to manually specify the AIIR context both inside and
...
When constructing an AIIR operation, two pieces of information are required:
...
-   a *location* that contains user-understandable information about the source
    ...
    always be provided as it carries a reference to the AIIR context.
...
```python
from aiir.ir import Context, InsertionPoint, Location, Module, Operation

with Context() as ctx:
  module = Module.create()

  # Prepare for inserting operations into the body of the module and indicate
  # that these operations originate in the "f.aiir" file at the given line and
  # column.
  with InsertionPoint(module.body), Location.file("f.aiir", line=42, col=1):
    ...
    Operation(<...>, loc=Location.file("g.aiir", line=1, col=10))
...
```python
from aiir.ir import Attribute, Type
from aiir.<dialect> import ConcreteAttr, ConcreteType
...
AIIR has two kinds of values based on their defining object: block arguments and
...
```python
from aiir.ir import BlockArgument, OpResult, Value
...
AIIR interfaces are a mechanism to interact with the IR without needing to know
...
attributes and types, the AIIR context may be set up by a surrounding context
manager.

```python
from aiir.ir import Context, InferTypeOpInterface
...
```python
from aiir.ir import Block, Context, Operation
...
```python
from aiir.ir import Context, F32Type, FloatAttr

# Attribute and types require access to an AIIR context, either directly or
...
```python
from aiir.ir import Context, IntegerAttr, IntegerType
...
```python
from aiir.ir import ArrayAttr, Context, DictAttr, UnitAttr
...
In general, for the core parts of AIIR, the Python bindings should be largely
...
Tests should be added in the `aiir/test/python` directory and should
...
```python
# RUN: %PYTHON %s | aiir-opt -split-input-file | FileCheck
...
The AIIR Python bindings integrate with the tablegen-based ODS system for
providing user-friendly wrappers around AIIR dialects and operations. There are
multiple parts to this integration, outlined below. Most details have been
elided: refer to the build rules and Python sources under `aiir.dialects` for
...
invoking `aiir-tblgen` on a Python-bindings specific tablegen wrapper that
...
include "aiir/Dialect/Func/IR/FuncOps.td"

#endif // PYTHON_BINDINGS_FUNC_OPS
```

In the main repository, building the wrapper is done via the CMake function
`declare_aiir_dialect_python_bindings`, which invokes:

```
aiir-tblgen -gen-python-op-bindings -bind-dialect={DIALECT_NAMESPACE} \
...
main repository, this search path is hard-coded to include the `aiir.dialects`
...
_cext.globals.append_dialect_search_prefix("myproject.aiir.dialects")
```
...
*   A `_Dialect` class (extending `aiir.ir.Dialect`) with a `DIALECT_NAMESPACE`
    attribute.
*   An `{OpName}` class for each operation (extending `aiir.ir.OpView`).
...
*   One argument for each declared result:
    *   For single-valued results: Each will accept an `aiir.ir.Type`.
    *   For variadic results: Each will accept a `List[aiir.ir.Type]`.
*   One argument for each declared operand or attribute:
    *   For single-valued operands: Each will accept an `aiir.ir.Value`.
    *   For variadic operands: Each will accept a `List[aiir.ir.Value]`.
    *   For attributes, it will accept an `aiir.ir.Attribute`.
*   Trailing usage-specific, optional keyword arguments:
    *   `loc`: An explicit `aiir.ir.Location` to use. Defaults to the location
        bound to the thread (i.e. `with Location.unknown():`) or an error if
        none is bound nor specified.
    *   `ip`: An explicit `aiir.ir.InsertionPoint` to use. Default to the
...
Thus, a natural extension is a builder that accepts a AIIR type and a Python value and instantiates the appropriate `TypedAttr`:

```python
from typing import Union

from aiir.ir import Type, IntegerAttr, FloatAttr
from aiir.dialects._arith_ops_gen import _Dialect, ConstantOp
from aiir.dialects._ods_common import _cext
...
3. in order to register `ConstantOpExt` as the preferred `OpView` that is returned by `aiir.ir.Operation.opview` (see [Operations, Regions and Blocks](#operations-regions-and-blocks))
...
```python
from aiir.dialects._scf_ops_gen import _Dialect, ForOp
from aiir.dialects._ods_common import _cext
...
Python bindings are designed to support AIIR’s open dialect ecosystem. A dialect
can be exposed to Python as a submodule of `aiir.dialects` and interoperate with
...
`aiir.ir.Operation` class with operation-specific builder functions and
properties. Therefore, there is no need to implement a separate C API for them.
For operations defined in ODS, `aiir-tblgen -gen-python-op-bindings
...
definition and use it as source for the `aiir-tblgen` call.
Such `.td` files reside in
[`python/aiir/dialects/`](https://github.com/llvm/llvm-project/tree/main/aiir/python/aiir/dialects).
The results of `aiir-tblgen` are expected to produce a file named
`_<dialect-namespace>_ops_gen.py` by convention. The generated operation classes
can be extended as described above. AIIR provides [CMake
functions](https://github.com/llvm/llvm-project/blob/main/aiir/cmake/modules/AddAIIRPython.cmake)
to automate the production of such files. Finally, a
`python/aiir/dialects/<dialect-namespace>.py` or a
`python/aiir/dialects/<dialect-namespace>/__init__.py` file must be created and
filled with `import`s from the generated files to enable `import
aiir.dialects.<dialect-namespace>` in Python.


### Attributes and Types

Dialect attributes and types are provided in Python as subclasses of the
`aiir.ir.Attribute` and `aiir.ir.Type` classes, respectively. Bindings APIs for
  ...
  static void bindDerived(ClassTy &c) {
    c.def_static("get",
      [](DefaultingPyAiirContext context) {
        return PyTestType(context->getRef(), aiirPythonTestTestTypeGet(context.get()->get()));
      },
...
public:
  ...
  static void bindDerived(ClassTy &c) {
    c.def_static(
      "get",
      [](DefaultingPyAiirContext context) {
        return PyTestAttr(context->getRef(), aiirPythonTestTestAttributeGet(context.get()->get()));
      },
      nb::arg("context").none() = nb::none());
  }
};
...
See [`aiir/test/python/lib/PythonTestModuleNanobind.cpp`] for more examples.

**Note**: if you are defining such types/attributes in a downstream project, it is critical you define 
`AIIR_BINDINGS_PYTHON_NB_DOMAIN` (which is used to determine `AIIR_BINDINGS_PYTHON_DOMAIN`) such that it is unique to
...
“Python extension” library placed in `python/aiir/_aiir_libs` that will be
loaded by Python at runtime. AIIR provides [CMake
functions](https://github.com/llvm/llvm-project/blob/main/aiir/cmake/modules/AddAIIRPython.cmake)
to automate the production of such libraries. This library should be `import`ed
from the main dialect file, i.e. `python/aiir/dialects/<dialect-namespace>.py`
or `python/aiir/dialects/<dialect-namespace>/__init__.py`, to ensure the types
...
declaratively using Tablegen, `aiir-tblgen -gen-pass-capi-header` and
`-aiir-tblgen -gen-pass-capi-impl` automate the generation of C API. The
...
`python/aiir/dialects/<dialect-namespace>.py` or
`python/aiir/dialects/<dialect-namespace>/__init__.py`, or from a separate
`passes` submodule to be put in
`python/aiir/dialects/<dialect-namespace>/passes.py` if it is undesirable to
...
[`include/aiir/Bindings/Python/PybindAdaptors.h`](https://github.com/llvm/llvm-project/blob/main/aiir/include/aiir/Bindings/Python/PybindAdaptors.h),
or nanobind and
[`include/aiir/Bindings/Python/NanobindAdaptors.h`](https://github.com/llvm/llvm-project/blob/main/aiir/include/aiir/Bindings/Python/NanobindAdaptors.h)
utilities to connect to the rest of Python API. The bindings can be located in a
separate module or in the same module as attributes and types, and
loaded along with the dialect.


## Extending AIIR in Python

The AIIR Python bindings provide support for defining custom components in Python,
mainly including dialects, passes, and rewrite patterns.
The following sections outline how each of these can be implemented.

### Dialects

The `aiir.dialects.ext` module provides support for defining Python-defined dialects.
...
The dialect can then be loaded into AIIR by calling `MyDialect.load()` within a valid `Context`.
...
# The code below requires an available AIIR context and location.
...
converts an AIIR module containing `irdl.dialect` ops into AIIR dialects.
...
In such case, the callable is wrapped as an `aiir::Pass` internally and
...
of `aiir.rewrite.RewritePatternSet` in Python.
...
The `aiir.rewrite.PDLModule` class accepts a module containing `pdl.pattern` ops,
...
AIIR Python bindings are free-threading compatible with exceptions (discussed below) in the following sense: it is safe to work in multiple threads with **independent** contexts. Below we show an example code of safe usage:
...
- Usage of `aiir.dialects.transform.interpreter` is unsafe.
- Usage of `aiir.dialects.gpu` and `gpu-module-to-binary` is unsafe.
