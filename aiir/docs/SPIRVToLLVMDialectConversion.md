...
```shell
aiir-opt -convert-spirv-to-llvm <filename.aiir>
```
...
Examples of SPIR-V struct conversion are: ```aiir !spirv.struct<i8, i32> =>
...
```aiir
                            %mask = llvm.aiir.constant(-1 : i32) : i32
...
*   `offset` and `count` are always scalar. This means that we can have the
    ...
    ```aiir
    // Broadcasting offset
    %offset0 = llvm.aiir.undef : vector<2xi8>
    %zero = llvm.aiir.constant(0 : i32) : i32
    %offset1 = llvm.insertelement %offset, %offset0[%zero : i32] : vector<2xi8>
    %one = llvm.aiir.constant(1 : i32) : i32
...
```aiir
// Create mask
// %minus_one = llvm.aiir.constant(-1 : i32) : i32
...
// %size    = llvm.aiir.constant(32 : i32) : i32
...
*   In contrast, `spirv.ExecutionMode` may contain important information about the
    ...
    llvm.aiir.global external constant @{{.*}}() : !llvm.struct<(i32)> {
      %0   = llvm.aiir.undef : !llvm.struct<(i32)>
      %1   = llvm.aiir.constant(31 : i32) : i32
...
```aiir
                                  %mask = llvm.aiir.constant(-1 : i1) : i1
...
%i   = ...
%var = ...
%0   = llvm.aiir.constant(0 : i32) : i32
...
*   **Aligned**: alignment is passed on to LLVM op builder, for example: `aiir
...
*   **Nontemporal**: set `nontemporal` flag, for example: `aiir // %res =
    llvm.load %ptr {nontemporal} : !llvm.ptr %res = spirv.Load "Function"
    %ptr ["Nontemporal"] : f32`

*   **Volatile**: mark the op as `volatile`, for example: `aiir // %res =
...
#### `spirv.GlobalVariable` and `spirv.aiir.addressof`

`spirv.GlobalVariable` is modelled with `llvm.aiir.global` op. However, there is a
...
`spirv.aiir.addressof` and `llvm.aiir.addressof` ops that both return a pointer
...
spirv.module Logical GLSL450 {
  ...
  spirv.func @func() -> () "None" {
    %0 = spirv.aiir.addressof @struct : !spirv.ptr<!spirv.struct<f32, !spirv.array<10xf32>>, Private>
    spirv.Return
  }
}
...
// Converted result
module {
  llvm.aiir.global private @struct() : !llvm.struct<packed (f32, [10 x f32])>
  llvm.func @func() {
    %0 = llvm.aiir.addressof @struct : !llvm.ptr
    llvm.return
  }
}
```
...
Currently `llvm.aiir.global`s are created with `private` linkage for `Private`
...
```aiir
spirv.GlobalVariable @var : !spirv.ptr<f32, Input>    =>    llvm.aiir.global external constant @var() : f32
```
...
// Conversion of VariableOp without initialization
                                                               %size = llvm.aiir.constant(1 : i32) : i32
...
// Conversion of VariableOp with initialization
                                                               %c    = llvm.aiir.constant(0 : i64) : i64
%c   = spirv.Constant 0 : i64                                    %size = llvm.aiir.constant(1 : i32) : i32
...
`spirv.Constant` is mapped to `llvm.aiir.constant`. This is a straightforward
...
```aiir
// %0 = llvm.aiir.constant(0 : i8) : i8
%0 = spirv.Constant  0 : i8

// %1 = llvm.aiir.constant(dense<[2, 3, 4]> : vector<3xi32>) : vector<3xi32>
...
### `spirv.aiir.selection` and `spirv.aiir.loop`

Control flow within `spirv.aiir.selection` and `spirv.aiir.loop` is lowered directly
...
%cond = spirv.Constant true                               %cond = llvm.aiir.constant(true) : i1
...
^merge:                                               ^merge:
  spirv.aiir.merge                                            llvm.br ^continue
}
...
^merge:                                               ^merge:
  spirv.aiir.merge                                            llvm.br ^remaining
}
...
```aiir
                                           %one  = llvm.aiir.constant(1.0 : f32) : f32
...
```aiir
                                     %two   = llvm.aiir.constant(2.0: f32) : f32
                                     %2xArg = llvm.fmul %two, %arg : f32
                                     %exp   = "llvm.intr.exp"(%2xArg) : (f32) -> f32
%res = spirv.Tanh %arg : f32     =>    %one   = llvm.aiir.constant(1.0 : f32) : f32
...
The `aiir-runner` has support for executing a `gpu` dialect kernel on the
...
`-DAIIR_ENABLE_SPIRV_CPU_RUNNER=1`
...
llvm.func @main() {
  ...
  %addr = llvm.aiir.addressof @__spv__foo_arg_descriptor_set0_binding0 : ...
...
```aiir
module @__spv__foo {
  llvm.aiir.global @__spv__foo_arg_descriptor_set0_binding0 : ...
  llvm.func @__spv__foo_bar() {
    // Kernel code.
  }
}
...
[VulkanLayoutUtils]: https://github.com/llvm/llvm-project/blob/main/aiir/include/aiir/Dialect/SPIRV/LayoutUtils.h
