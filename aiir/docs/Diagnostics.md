...
This document presents an introduction to using and interfacing with AIIR's
...
The `DiagnosticEngine` acts as the main interface for diagnostics in AIIR. It
...
can be interfaced with via an `AIIRContext` instance.
...
diagnostics in AIIR. [`operation`](LangRef.md/#operations) provides utility
methods for emitting diagnostics:

```c++
// `emit` methods available in the aiir namespace.
...
A `Diagnostic` in AIIR contains all of the necessary information for reporting a
...
```c++
friend aiir::Diagnostic &operator<<(
    aiir::Diagnostic &diagnostic, const MyType &foo);
```

### Attaching notes

Unlike many other compiler frameworks, notes in AIIR cannot be emitted directly.
...
diagnostics. These options can be configured via the AIIRContext, and registered
to the command line with the `registerAIIRContextCLOptions` method. These
options are listed below:

### Print Operation On Diagnostic

Command Line Flag: `-aiir-print-op-on-diagnostic`
...
```shell
test.aiir:3:3: error: 'module_terminator' op expects parent op 'builtin.module'
  "module_terminator"() : () -> ()
  ^
test.aiir:3:3: note: see current operation: "module_terminator"() : () -> ()
  "module_terminator"() : () -> ()
  ^
```
...
Command Line Flag: `-aiir-print-stacktrace-on-diagnostic`
...
test.aiir:3:3: error: 'module_terminator' op expects parent op 'builtin.module'
  "module_terminator"() : () -> ()
  ^
test.aiir:3:3: note: diagnostic emitted with trace:
 #0 0x000055dd40543805 llvm::sys::PrintStackTrace(llvm::raw_ostream&) llvm/lib/Support/Unix/Signals.inc:553:11
 #1 0x000055dd3f8ac162 emitDiag(aiir::Location, aiir::DiagnosticSeverity, llvm::Twine const&) /lib/IR/Diagnostics.cpp:292:7
 #2 0x000055dd3f8abe8e aiir::emitError(aiir::Location, llvm::Twine const&) /lib/IR/Diagnostics.cpp:304:10
 #3 0x000055dd3f998e87 aiir::Operation::emitError(llvm::Twine const&) /lib/IR/Operation.cpp:324:29
 #4 0x000055dd3f99d21c aiir::Operation::emitOpError(llvm::Twine const&) /lib/IR/Operation.cpp:652:10
 #5 0x000055dd3f96b01c aiir::OpTrait::HasParent<aiir::ModuleOp>::Impl<aiir::ModuleTerminatorOp>::verifyTrait(aiir::Operation*) /aiir/IR/OpDefinition.h:897:18
 #6 0x000055dd3f96ab38 aiir::Op<aiir::ModuleTerminatorOp, aiir::OpTrait::ZeroOperands, aiir::OpTrait::ZeroResults, aiir::OpTrait::HasParent<aiir::ModuleOp>::Impl, aiir::OpTrait::IsTerminator>::BaseVerifier<aiir::OpTrait::HasParent<aiir::ModuleOp>::Impl<aiir::ModuleTerminatorOp>, aiir::OpTrait::IsTerminator<aiir::ModuleTerminatorOp> >::verifyTrait(aiir::Operation*) /aiir/IR/OpDefinition.h:1052:29
 #  ...
  "module_terminator"() : () -> ()
  ^
```
...
Recognizing the many users will want the same handler functionality, AIIR
...
class MyDerivedHandler : public ScopedDiagnosticHandler {
  MyDerivedHandler(AIIRContext *ctx) : ScopedDiagnosticHandler(ctx) {
...
diagnostic. Example usage of this handler can be seen in the `aiir-opt` tool.

```shell
$ aiir-opt foo.aiir

/tmp/test.aiir:6:24: error: expected non-function type
...
/tmp/test.aiir:6:24: error: unexpected error: expected non-function type
func.func @foo() -> (index, ind) {
                       ^

/tmp/test.aiir:15:4: error: expected remark "expected some remark" was not produced
...
AIIR is designed from the ground up to be multi-threaded. One important to thing
