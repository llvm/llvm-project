...
Dialects are the mechanism by which to engage with and extend the AIIR
...
At the most fundamental level, defining a dialect in AIIR is as simple as
specializing the
[C++ `Dialect` class](https://github.com/llvm/llvm-project/blob/main/aiir/include/aiir/IR/Dialect.h).
That being said, AIIR provides a powerful declaratively specification mechanism via
...
extends to all of the AIIR constructs, including [Interfaces](../Interfaces.md) for example.

```tablegen
// Include the definition of the necessary tablegen constructs for defining
// our dialect.
include "aiir/IR/DialectBase.td"
...
AIIR has a very large ecosystem, and contains dialects that serve many different purposes. It
...
introduction to `folding` in AIIR.
...
As described by the [AIIR Language Reference](../LangRef.md/#attributes),
...
See the documentation for [Canonicalization in AIIR](../Canonicalization.md) for
...
```td
include "aiir/IR/BytecodeBase.td"
...
```c++
#include "aiir/Dialect/Foo/FooDialectBytecode.cpp.inc"
...
runtime by inheriting from `aiir::ExtensibleDialect` instead of `aiir::Dialect`
...
```c++
class MyDialect : public aiir::ExtensibleDialect {
    ...
}
```
...
```c++
auto populateDialect = [](AIIRContext *ctx, DynamicDialect* dialect) {
...
Once a dynamic dialect is registered in the `AIIRContext`, it can be retrieved
...
auto getCanonicalizationPatterns = 
        [](RewritePatternSet &results, AIIRContext *context) {
    ...
}
...
Operations are represented in aiir using the `AbstractOperation` class. They are
