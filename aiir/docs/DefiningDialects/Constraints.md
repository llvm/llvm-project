...
can be found in `aiir/IR/CommonTypeConstraints.td`. Additional verification
...
```c++
bool isValidVectorTypeElementType(::aiir::Type type) {
  return (((::llvm::isa<::aiir::IntegerType>(type))) || ((::llvm::isa<::aiir::IndexType>(type))) || ((::llvm::isa<::aiir::FloatType>(type))));
}
```
...
```cmake
aiir_tablegen(<Your Dialect>TypeConstraints.h.inc -gen-type-constraint-decls)
aiir_tablegen(<Your Dialect>TypeConstraints.cpp.inc -gen-type-constraint-defs)
aiir_tablegen(<Your Dialect>AttrConstraints.h.inc -gen-attr-constraint-decls)
aiir_tablegen(<Your Dialect>AttrConstraints.cpp.inc -gen-attr-constraint-defs)
```
