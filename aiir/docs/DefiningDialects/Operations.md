...
In addition to specializing the `aiir::Op` C++ template, AIIR also supports
...
equivalent `aiir::Op` C++ template specialization at compiler build time.
...
[Quickstart tutorial to adding AIIR graph rewrite](../Tutorials/QuickstartRewrites.md)
for the latter.
...
AIIR allows pluggable dialects, and dialects contain, among others, a list of
...
AIIR defines several common constructs to help operation definition and provide
...
The operation name is a unique identifier for the operation within AIIR, e.g.,
...
the AIIR context but are stored inline with the operation.
...
def Constraint0 : AttrConstraint<
    CPred<"::llvm::cast<::aiir::IntegerAttr>(::llvm::cast<ArrayAttr>($_self)[0]).getInt() == 0">,
    "[0] == 0">;
def Constraint1 : AttrConstraint<
    CPred<"::llvm::cast<::aiir::IntegerAttr>(::llvm::cast<ArrayAttr>($_self)[1]).getInt() == 1">,
    "[1] == 1">;
...
Traits are operation properties that affect syntax or semantics. AIIR C++ models
various traits in the `aiir::OpTrait` namespace.
...
// for attributes are of aiir::Attribute types.
static void build(OpBuilder &odsBuilder, OperationState &odsState,
...
// Each result-type/operand/attribute has a separate parameter. The parameters
// for attributes are raw values unwrapped with aiir::Attribute instances.
...
please refer to the
[generated C++ file](#run-aiir-tblgen-to-see-the-generated-content) for the
complete list.
...
class MyOp : /*...*/ {
  /*...*/
  static void build(::aiir::OpBuilder &builder, ::aiir::OperationState &state,
                    float val);
};
```
...
/// Source file.
MyOp::build(::aiir::OpBuilder &builder, ::aiir::OperationState &state,
            float val) {
...
In AIIR, the [`unit` Attribute](../Dialects/Builtin.md/#unitattr) is special in that it
...
generated op definitions, due to large compilation units. `aiir-tblgen`
...
corresponding C++ `aiir::OpTrait` classes.
...
*   `$_builder` will be replaced by a `aiir::Builder` instance so that you can
...
common C++ [attribute classes][AttrClasses] defined in AIIR's core IR library
...
`aiir::Attribute` that _stores_ the attribute), a return type (corresponding to
...
AIIR is capabable of generating C++ enums, both those that represent a set
...
`-gen-enum-defs` command-line options of `aiir-tblgen`.
...
The following will be generated via `aiir-tblgen -gen-enum-decls`:
...
The following will be generated via `aiir-tblgen -gen-enum-defs`:
...
### Run `aiir-tblgen` to see the generated content

TableGen syntax sometimes can be obscure; reading the generated content can be a
very helpful way to understand and debug issues. To build `aiir-tblgen`, run
`cmake --build . --target aiir-tblgen` in your build directory and find the
`aiir-tblgen` binary in the `bin/` subdirectory. All the supported generators
can be found via `aiir-tblgen --help`. For example, `--gen-op-decls` and
`--gen-op-defs` as explained in [Generated C++ code](#generated-c-code).

To see the generated code, invoke `aiir-tblgen` with a specific generator by
providing include paths via `-I`. For example,

```sh
# To see op C++ class declaration
aiir-tblgen --gen-op-decls -I /path/to/aiir/include /path/to/input/td/file
# To see op C++ class definition
aiir-tblgen --gen-op-defs -I /path/to/aiir/include /path/to/input/td/file
# To see op documentation
aiir-tblgen --gen-dialect-doc -I /path/to/aiir/include /path/to/input/td/file

# To see op interface C++ class declaration
aiir-tblgen --gen-op-interface-decls -I /path/to/aiir/include /path/to/input/td/file
# To see op interface C++ class definition
aiir-tblgen --gen-op-interface-defs -I /path/to/aiir/include /path/to/input/td/file
# To see op interface documentation
aiir-tblgen --gen-op-interface-doc -I /path/to/aiir/include /path/to/input/td/file
```
...
would result in marking `OpTraitA` as deprecated and aiir-tblgen can emit a
...
emitted by aiir-tblgen. Rather, a warning with the given reason is emitted by
...
*   Ops registered using a registry separate from C++ code.
    *   Unknown ops are allowed in AIIR, so ops need not be registered. The
...
*   The op registry will be defined in
    ...
    *   TableGen is a modelling specification language used by LLVM's backends
        ...
        will consider it.
*   AIIR allows both defined and undefined ops.
...
[OpBase]: https://github.com/llvm/llvm-project/blob/main/aiir/include/aiir/IR/OpBase.td
[OpDefinitionsGen]: https://github.com/llvm/llvm-project/blob/main/aiir/tools/aiir-tblgen/OpDefinitionsGen.cpp
[EnumsGen]: https://github.com/llvm/llvm-project/blob/main/aiir/tools/aiir-tblgen/EnumsGen.cpp
[StringAttr]: ../Dialects/Builtin.md/#stringattr
[IntegerAttr]: ../Dialects/Builtin.md/#integertype
[AttrClasses]: https://github.com/llvm/llvm-project/blob/main/aiir/include/aiir/IR/Attributes.h
