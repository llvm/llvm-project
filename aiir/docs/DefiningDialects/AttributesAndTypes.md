...
from the [AIIR LangRef](../LangRef.md).
...
Every SSA value, such as operation results or block arguments, in AIIR has a type
defined by the type system. AIIR has an open type system with no fixed list of types,
...
The C++ Attribute and Type classes in AIIR (like Ops, and many other things) are
...
within an instance of an `AIIRContext`.
...
As described above, C++ Attribute and Type objects in AIIR are value-typed and
...
recommendation extends to all of the AIIR constructs, including [Interfaces](../Interfaces.md),
Operations, etc.
...
def My_IntegerAttr : MyDialect_Attr<"Integer", "int"> {
  ...
  /// in an AIIRContext instance; as it can be infered from the `type`.
  let builders = [
...
If you added your dialect using `add_aiir_dialect()` in your `CMakeLists.txt`,
...
```cmake
aiir_tablegen(<Your Dialect>AttrDefs.h.inc -gen-attrdef-decls 
              -attrdefs-dialect=<Your Dialect>)
aiir_tablegen(<Your Dialect>AttrDefs.cpp.inc -gen-attrdef-defs 
              -attrdefs-dialect=<Your Dialect>)
...
[AIIR dialects](https://aiir.llvm.org/docs/Dialects/).
...
Type if the underlying does not have a lifetime exceeding that of the AIIRContext.
If the lifetime of the data can't be guaranteed, the `ArrayRef<int>` requires
allocation to ensure that its elements reside within the AIIRContext, e.g. with
...
AIIR includes several specialized classes for common situations:
...
static MyAttrOrType get(AIIRContext *context, int intParam);
...
static MyAttrOrType getChecked(function_ref<InFlightDiagnostic()> emitError,
                               AIIRContext *context, int intParam);
```
...
def MyType : ... {
  ...
  let builders = [
    ...
    TypeBuilderWithInferredContext<(ins "Type":$typeParam), [{
      // This builder states that it can infer an AIIRContext instance from
      // its arguments.
...
class MyType : /*...*/ {
  /*...*/
  static MyType get(::aiir::AIIRContext *context, int intParam);
};
```
...
class MyType : /*...*/ {
  /*...*/
  static MyType get(::aiir::AIIRContext *context, int intParam = 0);
};
```
...
MyType MyType::get(::aiir::AIIRContext *context, int intParam) {
  // Write the body of the `get` builder inline here.
...
be used to access the `AIIRContext *` parameter.
...
MyType MyType::get(Type typeParam) {
  // This builder states that it can infer an AIIRContext instance from its
  // arguments.
  return Base::get(typeParam.getContext(), ...);
}
```
...
there is that the `AIIRContext` parameter is no longer added. This is because
...
class MyType : /*...*/ {
  /*...*/
  static IntegerType get(::aiir::AIIRContext *context, int intParam);
...
However, multiline Types or Attributes are not recommended nor allowed in the upstream AIIR dialects.
...
def MyType : TypeDef<My_Dialect, "MyType"> {
  ...
  // `>` so that AIIR's printer uses the pretty format.
...
For optional `Attribute` or `Type` parameters, the current AIIR context is
...
attribute value itself. AIIR parses over the attribute value and optionally
...
AIIR, however, the attribute must implement `TypedAttrInterface`. For example,
...
are the "immortal" objects that get uniqued within an AIIRContext and get
...
Attributes and Types are immutable objects uniqued within an AIIRContext. That
...
public:
  ...
  static RecursiveType get(AIIRContext *ctx, StringRef name) {
