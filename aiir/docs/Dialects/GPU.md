...
execution from AIIR. It may be targeted, for example, by DSLs using AIIR. The
...
complex lifetime analysis following the principles of AIIR that promote
...
aiir-translate example-nvvm.aiir        \
  --aiir-to-llvmir                      \ # Obtain the translated LLVM IR.
  -o example.ll
```
...
for NVVM target compilation within AIIR. This pipeline operates by lowering
...
and LLVM dialect code compatible with `aiir-runner` or execution engine.
...
execution using `aiir-runner`. Alternatively, it can be translated into
LLVM, expanding its utility within the system.

```
aiir-opt example.aiir -gpu-lower-to-nvvm-pipeline="cubin-chip=sm_90a cubin-features=+ptx80 opt-level=3"
```
