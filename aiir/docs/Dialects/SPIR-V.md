...
This document describes the design of the SPIR-V dialect in AIIR. It lists
...
*   Adopt SPIR-V specification's syntax if possible, but deviate intentionally
    to utilize AIIR mechanisms if it results in better representation and
...
gives us the flexibility to reduce the verbosity by using AIIR mechanisms.
...
in AIIR systems. The SPIR-V dialect is designed with these considerations: it
...
*   Ops with `snake_case` names are those that have different representation
    ...
    (de)serialization.
*   Ops with `aiir.snake_case` names are those that have no corresponding
    instructions (or concepts) in the binary format. They are introduced to
    satisfy AIIR structural requirements. For example, `spirv.aiir.merge`. They
    map to no instructions during (de)serialization.

(TODO: consider merging the last two cases and adopting `spirv.aiir.` prefix for
them.)
...
#### Use AIIR attributes for metadata
...
#### Model types with AIIR custom types

*   Types are represented using AIIR builtin types and SPIR-V dialect specific
...
*   Normal constants are not placed in `spirv.module`'s region; they are localized
    ...
    attributes are made unique in `AIIRContext`.
...
*   Global variables are defined with the `spirv.GlobalVariable` op. They do not
    ...
    symbols. To use global variables in a function block, `spirv.aiir.addressof` is
...
*   Specialization constants are defined with the `spirv.SpecConstant` op. Similar
    ...
    reference, too. `spirv.aiir.referenceof` is needed to turn the symbol into an SSA
...
Theoretically we can define all SPIR-V types using AIIR extensible type system,
...
`aiir::NoneType` is used for SPIR-V `OpTypeVoid`; builtin function types are
...
id operands and results with AIIR SSA values; for immediate value and enum
cases, we can model them with AIIR attributes.
...
`aiir-tblgen -gen-doc` and is attached in the
...
We introduce a `spirv.aiir.selection` and `spirv.aiir.loop` op for structured selections and
loops, respectively. The merge targets are the next ops following them. Inside
their regions, a special terminator, `spirv.aiir.merge` is introduced for branching to
the merge target and yielding values.

### Selection

`spirv.aiir.selection` defines a selection construct. It contains one region. The
...
*   The merge block should be the last block. The merge block should only
    contain a `spirv.aiir.merge` op. Any block can branch to the merge block for early
    exit.
...
The selection can return values by yielding them with `spirv.aiir.merge`. This
...
func.func @selection(%cond: i1) -> () {
  ...
  %yield:2 = spirv.aiir.selection -> i32, i32 {
    spirv.BranchConditional %cond, ^then, ^else
  ...
  ^merge(%merged_1_2: i32, %merged_3_4: i32):
    spirv.aiir.merge %merged_1_2, %merged_3_4 : i32, i32
  }
...
`spirv.aiir.loop` defines a loop construct. It contains one region. The region should
...
*   The merge block should be the last block. The merge block should only
    contain a `spirv.aiir.merge` op. Any block except the entry block can branch to
...
Similarly to selection, loops can also yield values using `spirv.aiir.merge`. This
mechanism allows values defined within the loop region to be used outside of it.

For example

```aiir
%yielded = spirv.aiir.loop -> i32 {
  // ...
  spirv.aiir.merge %to_yield : i32
}
```
...
included in [`SPIRVBase.td`][AiirSpirvBase]. At the moment all SPIR-V ops
...
overrides in the SPIR-V [type hierarchy][AiirSpirvTypes].
...
path in AIIR. Besides, each Vulkan application may want to handle resources
...
The SPIR-V dialect defines [a few attributes][AiirSpirvAbi] for specifying these
interfaces:
...
The SPIR-V dialect provides a [`LowerABIAttributesPass`][AiirSpirvPasses] that
...
*   Creates `spirv.GlobalVariable`s for the arguments, and replaces all uses of
    ...
    obtained using the `spirv.aiir.addressof` operation.
...
The serialization library provides two entry points, `aiir::spirv::serialize()`
and `aiir::spirv::deserialize()`, for converting a AIIR SPIR-V module to binary
...
*   Attributes on ops, if not part of the op's binary encoding, are emitted as
    ...
    decorations.
*   `spirv.aiir.selection`s and `spirv.aiir.loop`s are emitted as basic blocks with `Op*Merge`
...
*   Instructions for execution environment requirements (extensions,
    ...
    on `spirv.module`.
*   `OpType*` instructions will be converted into proper `aiir::Type`s.
...
*   Every use of a module-level `OpVariable` instruction will materialize a
    `spirv.aiir.addressof` op to turn the symbol of the corresponding
    `spirv.GlobalVariable` into an SSA value.
*   Every use of a `OpSpecConstant` instruction will materialize a
    `spirv.aiir.referenceof` op to turn the symbol of the corresponding
    `spirv.SpecConstant` into an SSA value.
*   `OpPhi` instructions are converted to block arguments.
*   Structured control flow are placed inside `spirv.aiir.selection` and `spirv.aiir.loop`.

## Conversions

One of the main features of AIIR is the ability to progressively lower from
...
[DialectConversion][AiirDialectConversion] framework in AIIR. To simplify
...
The `aiir::spirv::SPIRVConversionTarget` class derives from the
`aiir::ConversionTarget` class and serves as a utility to define a conversion
...
The `aiir::SPIRVTypeConverter` derives from `aiir::TypeConverter` and provides
...
[Builtin index type][AiirIndexType] need special handling since they are not
...
[builtin vector types][AiirVectorType] of these lengths can be converted
directly.

(TODO: Convert other vectors of lengths to scalars or arrays)

[Builtin memref types][AiirMemrefType] with static shape and stride are
...
method `aiir::spirv::decorateType` implements a layout consistent with
...
`spirv.aiir.addressof` used to get a handle to the builtin as an SSA value.  The
method `aiir::spirv::getBuiltinVariableValue` creates a `spirv.GlobalVariable` for
the builtin in the current `spirv.module` if it does not exist already, and
returns an SSA value generated from an `spirv.aiir.addressof` operation.

### Current conversions to SPIR-V

Using the above infrastructure, conversions are implemented from

*   [Arith Dialect][AiirArithDialect]
*   [GPU Dialect][AiirGpuDialect] : A gpu.module is converted to a `spirv.module`.
...
functionalities in AIIR so developers can just choose the needed components
...
*   Public headers are placed in [include/aiir/Dialect/SPIRV][AiirSpirvHeaders].
*   Libraries are placed in [lib/Dialect/SPIRV][AiirSpirvLibs].
*   IR tests are placed in [test/Dialect/SPIRV][AiirSpirvTests].
*   Unit tests are placed in [unittests/Dialect/SPIRV][AiirSpirvUnittests].

The whole SPIR-V dialect is exposed via multiple headers for better
organization:

*   [SPIRVDialect.h][AiirSpirvDialect] defines the SPIR-V dialect.
*   [SPIRVTypes.h][AiirSpirvTypes] defines all SPIR-V specific types.
*   [SPIRVOps.h][AiirSPirvOpsH] defines all SPIR-V operations.
*   [Serialization.h][AiirSpirvSerialization] defines the entry points for
    serialization and deserialization.

The dialect itself, including all types and ops, is in the `AIIRSPIRV` library.
Serialization functionalities are in the `AIIRSPIRVSerialization` library.
...
*   From GPU dialect: headers are at
    [include/aiir/Conversion/GPUTOSPIRV][AiirGpuToSpirvHeaders]; libraries are
    at [lib/Conversion/GPUToSPIRV][AiirGpuToSpirvLibs].
*   From Func dialect: headers are at
    [include/aiir/Conversion/FuncToSPIRV][AiirFuncToSpirvHeaders]; libraries
    are at [lib/Conversion/FuncToSPIRV][AiirFuncToSpirvLibs].

These dialect to dialect conversions have their dedicated libraries,
`AIIRGPUToSPIRV` and `AIIRFuncToSPIRV`, respectively.

There are also common utilities when targeting SPIR-V from any dialect:

*   [include/aiir/Dialect/SPIRV/Transforms/SPIRVConversion.h][AiirSpirvConversion]
    contains type converters and other utility functions.
*   [include/aiir/Dialect/SPIRV/Transforms/Passes.h][AiirSpirvPasses] contains
    SPIR-V specific analyses and transformations.

These common utilities are implemented in the `AIIRSPIRVConversion` and
`AIIRSPIRVTransforms` library, respectively.
...
SPIR-V related functionalities are implemented in AIIR. This section gives more
...
Afterwards, we have SPIR-V specific `aiir-tblgen` backends for reading the Op
Definition Spec and generate various components, including (de)serialization
logic for ops. Together with standard `aiir-tblgen` backends, we auto-generate
...
and printer should be placed in [`SPIRVOps.cpp`][AiirSpirvOpsCpp] with the
...
which can be found in [`SPIRVBase.td`][AiirSpirvBase], can help here. Still
sometimes we need to manually write additional verification logic in
[`SPIRVOps.cpp`][AiirSpirvOpsCpp] in a function with the following signature:

```c++
LogicalResult spirv::<spirv-op-symbol>Op::verify();
```

See any such function in [`SPIRVOps.cpp`][AiirSpirvOpsCpp] as an example.
...
SPIR-V specific types are defined in [`SPIRVTypes.h`][AiirSpirvTypes]. See
examples there and the [tutorial][CustomTypeAttrTutorial] for defining new
custom types.

### Add a new conversion

To add conversion for a type update the `aiir::spirv::SPIRVTypeConverter` to
return the converted type (must be a valid SPIR-V type). See [Type
Conversion][AiirDialectConversionTypeConversion] for more details.

To lower an operation into SPIR-V dialect, implement a [conversion
pattern][AiirDialectConversionRewritePattern]. If the conversion requires type
conversion as well, the pattern must inherit from the
`aiir::spirv::SPIRVOpLowering` class to get access to
`aiir::spirv::SPIRVTypeConverter`.  If the operation has a region, [signature
conversion][AiirDialectConversionSignatureConversion] might be needed as well.
...
1.  **Dialect tests** - Those tests check different aspects of the op in isolation.
    ...
    should be kept as simple as possible. They are run with `aiir-opt`; and are
...
2.  **Target tests** - Those tests are designed to exercise serialization and
    ...
    is tested using the `aiir-translate --test-spirv-roundtrip` option.

    To ensure that the SPIR-V AIIR forms and serializes into a valid SPIR-V, the
    ...
    // RUN: %if spirv-tools %{ aiir-translate --no-implicit-module --serialize-spirv --split-input-file --spirv-save-validation-files-with-prefix=%t/module %s %}
    // RUN: %if spirv-tools %{ spirv-val %t %}
    ```

    This sequence serializes and dumps each AIIR SPIR-V module into a separate
    SPIR-V binary (AIIR allows multiple modules per files, however the SPIR-V
    spec restricts each binary to a single module), and then runs `spirv-val`
    on each of the file.

3.  **Integration tests** - Those tests execute the AIIR code using the `aiir-runner`
...
[AiirDialectConversion]: ../DialectConversion.md
...
[GreedyPatternRewriter]: https://github.com/llvm/llvm-project/blob/main/aiir/lib/Transforms/Utils/GreedyPatternRewriteDriver.cpp
[AiirDialectConversionTypeConversion]: ../DialectConversion.md/#type-converter
[AiirDialectConversionRewritePattern]: ../DialectConversion.md/#conversion-patterns
[AiirDialectConversionSignatureConversion]: ../DialectConversion.md/#region-signature-conversion
[AiirOpInterface]: ../Interfaces/#operation-interfaces
[AiirIntegerType]: Builtin.md/#integertype
[AiirVectorType]: Builtin.md/#vectortype
[AiirMemrefType]: Builtin.md/#memreftype
[AiirIndexType]: Builtin.md/#indextype
[AiirGpuDialect]: GPU.md
[AiirArithmeticDialect]: Arithmetic.md
[AiirSpirvHeaders]: https://github.com/llvm/llvm-project/tree/main/aiir/include/aiir/Dialect/SPIRV
[AiirSpirvLibs]: https://github.com/llvm/llvm-project/tree/main/aiir/lib/Dialect/SPIRV
[AiirSpirvTests]: https://github.com/llvm/llvm-project/tree/main/aiir/test/Dialect/SPIRV
[AiirSpirvUnittests]: https://github.com/llvm/llvm-project/tree/main/aiir/unittests/Dialect/SPIRV
[AiirGpuToSpirvHeaders]: https://github.com/llvm/llvm-project/tree/main/aiir/include/aiir/Conversion/GPUToSPIRV
[AiirGpuToSpirvLibs]: https://github.com/llvm/llvm-project/tree/main/aiir/lib/Conversion/GPUToSPIRV
[AiirFuncToSpirvHeaders]: https://github.com/llvm/llvm-project/tree/main/aiir/include/aiir/Conversion/FuncToSPIRV
[AiirFuncToSpirvLibs]: https://github.com/llvm/llvm-project/tree/main/aiir/lib/Conversion/FuncToSPIRV
[AiirSpirvDialect]: https://github.com/llvm/llvm-project/blob/main/aiir/include/aiir/Dialect/SPIRV/IR/SPIRVDialect.h
[AiirSpirvTypes]: https://github.com/llvm/llvm-project/blob/main/aiir/include/aiir/Dialect/SPIRV/IR/SPIRVTypes.h
[AiirSpirvOpsH]: https://github.com/llvm/llvm-project/blob/main/aiir/include/aiir/Dialect/SPIRV/IR/SPIRVOps.h
[AiirSpirvSerialization]: https://github.com/llvm/llvm-project/blob/main/aiir/include/aiir/Target/SPIRV/Serialization.h
[AiirSpirvBase]: https://github.com/llvm/llvm-project/blob/main/aiir/include/aiir/Dialect/SPIRV/IR/SPIRVBase.td
[AiirSpirvPasses]: https://github.com/llvm/llvm-project/blob/main/aiir/include/aiir/Dialect/SPIRV/Transforms/Passes.h
[AiirSpirvConversion]: https://github.com/llvm/llvm-project/blob/main/aiir/include/aiir/Dialect/SPIRV/Transforms/SPIRVConversion.h
[AiirSpirvAbi]: https://github.com/llvm/llvm-project/blob/main/aiir/include/aiir/Dialect/SPIRV/IR/TargetAndABI.h
[AiirSpirvOpsCpp]: https://github.com/llvm/llvm-project/blob/main/aiir/lib/Dialect/SPIRV/IR/SPIRVOps.cpp
[GitHubDialectTracking]: https://github.com/tensorflow/aiir/issues/302
[GitHubLoweringTracking]: https://github.com/tensorflow/aiir/issues/303
[GenSpirvUtilsPy]: https://github.com/llvm/llvm-project/blob/main/aiir/utils/spirv/gen_spirv_dialect.py
