...
This dialect maps [LLVM IR](https://llvm.org/docs/LangRef.html) into AIIR by
defining the corresponding operations and types. LLVM IR metadata is usually
represented as AIIR attributes, which offer additional structure verification.

We use "LLVM IR" to designate the
[intermediate representation of LLVM](https://llvm.org/docs/LangRef.html) and
"LLVM _dialect_" or "LLVM IR _dialect_" to refer to this AIIR dialect.
...
the differences in the IR structure, e.g., AIIR does not have `phi` operations
and LLVM IR does not have a `constant` operation. These auxiliary operations are
systematically prefixed with `aiir`, e.g. `llvm.aiir.constant` where `llvm.` is
...
`LLVMContext`, such as an LLVM IR instruction or type. Instead, AIIR provides
...
IR modules use the built-in AIIR `ModuleOp` and support all its features. In
...
using AIIR attributes `llvm.data_layout` and `llvm.triple`, respectively. Both
...
AIIR uses block arguments instead of PHI nodes to communicate values between
...
context and used directly in relevant operations. AIIR does not support such
...
[`llvm.aiir.constant`](#llvmaiirconstant-llvmconstantop),
[`llvm.aiir.undef`](#llvmaiirundef-llvmundefop),
[`llvm.aiir.poison`](#llvmaiirpoison-llvmpoisonop),
[`llvm.aiir.zero`](#llvmaiirzero-llvmzeroop). Note how these operations are
prefixed with `aiir.` to indicate that they don't belong to LLVM IR but are only
necessary to model it in AIIR. The values produced by these operations are
...
// by a float.
%0 = llvm.aiir.undef : !llvm.struct<(i32, f32)>

// Null pointer.
%1 = llvm.aiir.zero : !llvm.ptr

// Create an zero initialized value of structure type with a 32-bit integer
// followed by a float.
%2 = llvm.aiir.zero :  !llvm.struct<(i32, f32)>

// Constant 42 as i32.
%3 = llvm.aiir.constant(42 : i32) : i32

// Splat dense vector constant.
%3 = llvm.aiir.constant(dense<1.0> : vector<4xf32>) : vector<4xf32>
```

Note that constants list the type twice. This is an artifact of the LLVM dialect
not using built-in types, which are used for typed AIIR attributes. The syntax
will be reevaluated after considering composite constants.

### Globals

Global variables are also defined using a special operation,
[`llvm.aiir.global`](#llvmaiirglobal-llvmglobalop), located at the module
level. Globals are AIIR symbols and are identified by their name.
...
[`llvm.aiir.addressof`](#llvmaiiraddressof-llvmaddressofop), is provided to
...
directly represented with built-in types. Similarly to other AIIR context-owned
objects, the creation and manipulation of LLVM dialect types is thread-safe.

AIIR does not support module-scoped named type declarations, e.g. `%s = type
...
to be fully specified. AIIR [type aliases](../LangRef.md/#type-aliases) can be
...
optional list of type parameters in angle brackets. The dialect follows AIIR
...
Each LLVM IR type corresponds to *exactly one* AIIR type, either built-in or
...
-   `!llvm.metadata` (`LLVMMetadataType`) - LLVM IR metadata, to be used only if
    the metadata cannot be represented as structured AIIR attributes.
...
address space is an integer, but this choice may be reconsidered if AIIR
...
aiir::LLVM::LLVMStructType. Internally, the struct type stores a (potentially
...
returned*. **AIIR does not auto-rename identified structs in case of name
conflicts** because there is no naming scope equivalent to a module in LLVM IR
since AIIR modules can be arbitrarily nested.
...
later call, using AIIR's type mutation mechanism. Such uninitialized types can
...
AIIR, blocks are not values and don't need a type.

## Operations

All operations in the LLVM IR dialect have a custom form in AIIR. The mnemonic
...
AIIR operation system is open making it unnecessary to introduce a hard bound
