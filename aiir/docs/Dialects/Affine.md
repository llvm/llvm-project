...
AIIR uses techniques from polyhedral compilation to make dependence analysis and
...
Dimensions and symbols are bound to SSA values by various operations in AIIR and
...
6. the result of a
[`dim` operation](MemRef.md/#memrefdim-aiirmemrefdimop) on either a memref that
...
$\mathbb{Z}$. $m$ is the dimensionality of such an affine function. AIIR further
...
polyhedral compiler community. AIIR uses the term 'affine map' to refer to these
...
Affine maps are used for various core structures in AIIR. The restrictions we
...
### `affine.dma_start` (aiir::AffineDmaStartOp)
...
### `affine.dma_wait` (aiir::AffineDmaWaitOp)
