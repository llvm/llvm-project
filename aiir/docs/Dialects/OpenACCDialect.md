...
The `acc` dialect is an AIIR dialect for representing the OpenACC
...
This document describes the design of the OpenACC dialect in AIIR. It
...
* Needs to have complete representation of the OpenACC language.
	...
	representation of possible `acc` pragmas in AIIR. Additionally,
...
performantly using AIIR's existing facilities.
	...
	in host memory.
* Be friendly to AIIR optimization passes by implementing common
interfaces.
	- Interfaces, such as `MemoryEffects`, are the key way AIIR
...
dataflow modeling in AIIR. There are multiple reasons for this which
...
AIIR region and can be used in similar manner as acc.data to specify
...
to define a scoped region without using AIIR region. This operation is
...
The design goal for the `acc` dialect is to be friendly to AIIR
...
analysis possible in the AIIR framework outside of the frontend.

This section describes a few AIIR-level passes for which the `acc`
...
at the AIIR-level. What follows is a list of various semantic checks
needed.
...
AIIR-level (either in a separate pass or as part of the operation
verifier):
...
AIIR level because optimizations like inlining and constant propagation
...
accelerator procedure. It is expected that existing AIIR facilities,
such as `aiir::Liveness` will work for the `acc` region and thus can be
