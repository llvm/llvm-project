...
Operations in this AIIR dialect generally correspond to a single OpenMP
...
generated for the operation, whereas the latter is used to represent it in AIIR
text form.
...
clauses usually define which arguments the corresponding AIIR operation takes,
...
[OpenMPOpBase.td](https://github.com/llvm/llvm-project/blob/main/aiir/include/aiir/Dialect/OpenMP/OpenMPOpBase.td).

### Adding a Clause

OpenMP clause definitions are located in
[OpenMPClauses.td](https://github.com/llvm/llvm-project/blob/main/aiir/include/aiir/Dialect/OpenMP/OpenMPClauses.td).
...
[OpenMPOps.td](https://github.com/llvm/llvm-project/blob/main/aiir/include/aiir/Dialect/OpenMP/OpenMPOps.td),
...
As a result of the implicit way in which fundamental properties of AIIR
...
[OpenMPClauseOperands.h](https://github.com/llvm/llvm-project/blob/main/aiir/include/aiir/Dialect/OpenMP/OpenMPClauseOperands.h).
...
argument's type:
  - Values are represented with `aiir::Value`, except for `Variadic`, which
  makes it an `llvm::SmallVector<aiir::Value>`.
  ...
  array-like) attribute type, in which case the more generic `aiir::Attribute`
  ...
  specific `aiir::Attribute` subclass) will be used instead.
...
In their AIIR representation, certain OpenMP clauses introduce a mapping between
values defined outside the operation they are applied to and entry block
arguments for the region of that AIIR operation. This enables, for example, the
introduction of private copies of the same underlying variable defined outside
the AIIR operation the clause is attached to. Currently, clauses with this
...
into account by those creating the AIIR representation, since it is a per-leaf
...
region, no AIIR values defined outside of the op itself can be used. This is
...
Additionally, the runtime library targeted by the AIIR to LLVM IR translation of
...
These host-evaluated values in AIIR would need to be placed outside of the
