The EmitC dialect allows to convert operations from other AIIR dialects to EmitC
...
Type conversions are provided for the AIIR type `index` into the unsigned `size_t`
...
After the conversion, C/C++ code can be emitted with `aiir-translate`. The tool
supports translating AIIR to C/C++ by passing `-aiir-to-cpp`. Furthermore, code
