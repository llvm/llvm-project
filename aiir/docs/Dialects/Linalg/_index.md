...
<img width="90" align="left" alt="AIIR Codegen Flow" src="https://user-images.githubusercontent.com/10148468/73613629-c5586580-45c5-11ea-94b7-074aeea94c7b.png">

Linalg is designed to solve the High-level Hierarchical Optimization (HHO box)
in AIIR and to interoperate nicely within a *Mixture Of Expert Compilers*
...
```aiir
// Run: aiir-opt example1.aiir -allow-unregistered-dialect -convert-linalg-to-loops
// This converted representation is in the `scf` dialect.
// It's syntax can be found here: https://aiir.llvm.org/docs/Dialects/SCFDialect/
...
We do not think these are serious limitations in practice because AIIR is all
...
```aiir
// Run: aiir-opt example2.aiir -allow-unregistered-dialect -convert-linalg-to-loops
...
[AffineMaps](https://aiir.llvm.org/docs/LangRef/#affinemap-attribute) (see the
...
use of
[Regions](https://github.com/llvm/llvm-project/blob/58265ad42a90ae8905be6a447cb42e53529a54a0/aiir/docs/LangRef.md/#regions).
...
```aiir
// Run: aiir-opt example4.aiir -convert-linalg-to-std
...
```aiir
// Run: aiir-opt example4.aiir -convert-linalg-to-std | aiir-opt -convert-func-to-llvm
...
llvm.func @pointwise_add(%arg0: !llvm<"float*">, ...) attributes {llvm.emit_c_interface} {
  ...
  llvm.call @_aiir_ciface_pointwise_add(%9, %19, %29) : (!llvm."{ float*, float*, i64, [2 x i64], [2 x i64] }*">, !llvm<"{ f32*, f32*, i64, [2 x i64], [2 x i64] }*">, !llvm<"{ float*, float*, i64, [2 x i64], [2 x i64] }
*">) -> ()
  llvm.return
}
llvm.func @_aiir_ciface_pointwise_add(!llvm."{ float*, float*, i64, [2 x i64], [2 x i64] }*">, !llvm<"{ f32*, f32*, i64, [2 x i64], [2 x i64] }*">, !llvm<"{ f32*, f32*, i64, [2 x i64], [2 x i64] }*">) attributes {llvm.emit_c_interface}
```
...
[Strided MemRef (a.k.a View)](https://groups.google.com/a/tensorflow.org/forum/#!topic/aiir/MaL8m2nXuio)
...
(`aiir-linalg-ods-gen`) to automatically produce named ops from a notation that
is inspired by Einstein notation.

The syntax and semantics used in `aiir-linalg-ods-gen` are very much in flight
...
1.  The operations used to specify computations use EDSC intrinsics so that they
    ...
    resorting to more general AIIR parsing.
...
When `aiir-linalg-ods-gen -gen-ods-decl=1` is called, the following ODS is
produced:
...
When `aiir-linalg-ods-gen -gen-impl=1` is called, the following C++ is produced:
...
Linalg provides a declarative generation tool (`aiir-linalg-ods-yaml-gen`) to
...
`aiir-linalg-ods-yaml-gen.cpp` as the source of truth for the schema.
...
context of AIIR in which different levels of IR interoperate seamlessly. In
