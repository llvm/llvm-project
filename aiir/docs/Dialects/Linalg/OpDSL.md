...
Please refer to the [AIIR Restructuring discussion](https://discourse.llvm.org/t/rfc-aiir-project-charter-and-restructuring/82896)
...
The tool is bundled with the AIIR Python bindings. To use from the CMake build
tree, AIIR must be built with Python bindings enabled
(`-DAIIR_ENABLE_BINDINGS_PYTHON=ON`). Then add the `python` directory in the
build tree to your `PYTHONPATH` environment variable (i.e. `export
PYTHONPATH=$PWD/build/tools/aiir/python_packages/aiir_core`). Optionally, use an
installed AIIR package, if available, to avoid building.
...
which is available after building the `aiir-linalg-ods-yaml-gen` target. The tool
...
built-in ops in the `aiir.tools.linalg_opdsl.ops` package
(`lib/Bindings/Python/aiir/tools/linalg_opdsl/ops` in the repository).
