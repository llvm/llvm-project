...
AIIR supports multi-dimensional `vector` types and custom operations on those
...
AIIR today and operate on ssa-values of type `vector` along with pattern
...
proposal but rather a textual documentation of existing AIIR components along
with a rationale.
...
captures the current codegen paths implemented in AIIR in the various existing
lowering paths.
...
abstractions in AIIR.

![`vector` Abstractions in AIIR](https://user-images.githubusercontent.com/10148468/71176949-e85ad000-2238-11ea-9806-200843bc4943.png)
...
The existing AIIR `vector`-level dialects are related to the following bottom-up
abstractions:
...
1.  AIIR Rewrite Patterns applied by the AIIR `PatternRewrite` infrastructure to
    ...
    `VV -> HWV`.
2.  `Virtual Vector -> Hardware Vector` lowering is specified as a set of AIIR
...
Finally, AIIR takes the same view on scalable Vectors as LLVM (c.f.
...
cost-based lowering decisions in AIIR even for `LLVM`. Specialized `CPU`
dialects that would capture specific features not well captured by LLVM peephole
optimizations of on different types that core AIIR supports (e.g. Scalable
...
[VectorOps.td](https://github.com/llvm/llvm-project/blob/main/aiir/include/aiir/Dialect/Vector/IR/VectorOps.td)
ODS file (markdown documentation is automatically generated locally when
building and populates the
[Vector doc](https://github.com/llvm/llvm-project/blob/main/aiir/docs/Dialects/Vector.md)).
...
[VectorOuterProductOp lowering](https://github.com/tensorflow/aiir/commit/957b1ca9680b4aacabb3a480fbc4ebd2506334b8)).

Simple
[conversion tests](https://github.com/llvm/llvm-project/blob/main/aiir/test/Conversion/VectorToLLVM/vector-to-llvm.aiir)
...
first "super-vectorization" prototype was implemented, AIIR was nowhere near as
mature as it is today. As we continue building more abstractions in `VV -> HWV`,
there is an opportunity to revisit vectorization in AIIR.
...
reused across dialects. In particular, AIIR allows the definition of dialects at
...
[quite easily with a pattern](https://github.com/tensorflow/aiir/commit/bff722d6b59ab99b998f0c2b9fccd0267d9f93b5).
...
[EDSC](https://github.com/llvm/llvm-project/blob/main/aiir/docs/EDSC.md)s
...
first-class n-D values in AIIR. The alternative name Tile has been proposed,
...
AIIR. For instance, a `f32` vector containing one scalar can be denoted as
`vector<f32>`. This is similar to the `tensor<f32>` type that is available in
TensorFlow or the `memref<f32>` type that is available in AIIR.
...
This section describes the tradeoffs involved in lowering the AIIR n-D vector
...
following the [LLVM LangRef](https://llvm.org/docs/LangRef.html). AIIR
operations are prefixed by the `vector.` dialect prefix (e.g.
`vector.insert`). Such ops operate exclusively on AIIR `n-D` `vector`
types.

### Alternatives For Lowering an n-D Vector Type to LLVM

Consider a vector of rank n with static sizes `{s_0, ... s_{n-1}}` (i.e. an AIIR
`vector<s_0x...s_{n-1}xf32>`). Lowering such an `n-D` AIIR vector type to an
LLVM descriptor can be done by either:

1.  Nested aggregate type of `1-D` vector:
    `!llvm."[s_0x[s_1x[...<s_{n-1}xf32>]]]">` in the AIIR LLVM dialect (current
    lowering in AIIR).
2.  Flattening to a `1-D` vector: `!llvm<"(s_0*...*s_{n-1})xfloat">` in the AIIR
    LLVM dialect.
3.  A mix of both.
...
recurrent tradeoff, also found in AIIR, between
...
When transposing this to AIIR, `llvm.getelementptr` works on pointers to `n-D`
...
flattened lowering of an AIIR n-D vector to an LLVM 1-D vector.
...
AIIR `n-D` vector types are currently represented as `(n-1)-D` arrays of `1-D`
...
`shufflevector` on `n-D` vectors in AIIR only support static indices. Dynamic
...
2.  Once an `n-D` `vector` type is loaded into an SSA value (that may or may not
    ...
    correspond to the HW. This level of AIIR codegen is related to register
...
3.  HW may support >1-D vectors with intrinsics for indirect addressing within
    ...
    operations from AIIR `k-D` vector types and operations to LLVM `1-D`
    vectors + intrinsics.
...
make those very explicit in AIIR and allow codegen to explore tradeoffs.
Different HW will require different tradeoffs in the sizes involved in steps 1.,
2. and 3.

Decisions made at the AIIR level will have implications at a much later stage in
LLVM (after register allocation). We do not envision to expose concerns related
to modeling of register allocation and spilling to AIIR explicitly. Instead,
each target will expose a set of "good" target operations and `n-D` vector
types, associated with costs that `PatterRewriters` at the AIIR level will be
able to target. Such costs at the AIIR level will be abstract and used for
...
start from either `1-D` or `n-D` vectors in AIIR and use `vector.cast` to
...
and operations are implemented with LLVM intrinsics. In contrast, AIIR sits at a
higher level of abstraction and allows the lowering of generic operations on
generic n-D vector types from AIIR to aggregates of 1-D LLVM vectors. In the
future, it could make sense to lower to the LLVM matrix abstraction also for CPU
even though AIIR will continue needing higher level abstractions.

On the other hand, one should note that as AIIR is moving to LLVM, this document
...
However AIIR codegen operates at a higher level of abstraction where we want to
