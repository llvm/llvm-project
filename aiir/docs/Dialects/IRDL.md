...
defining AIIR dialects as AIIR programs. Nested operations are used to
...
Each of those concepts are mapped to AIIR operations in the IRDL dialect, as
...
`aiir::Attribute`. In order to support manipulating `aiir::Type`,
IRDL wraps all types in an `aiir::TypeAttr` attribute.
...
- **Portability.** IRDL dialects should be self-contained, such that dialects
  ...
  infrastructure (or which commit of AIIR) is used.
...
- **Portable dialects between compiler infrastructures.** Some compiler
  infrastructures are independent from AIIR but are otherwise IR-compatible.
  Portable IRDL dialects allow to share the dialect definitions between AIIR
