...
class FullFeaturedHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def serve_interface(self):
        ...
        try:
            ...
            with open('/home/john/LAT5150DRVMIL/03-web-interface/clean_ui_v3.html', 'rb') as f:
                self.wfile.write(f.read())
        except:
            # Fallback to v2
            try:
                with open('/home/john/LAT5150DRVMIL/03-web-interface/military_terminal_v2.html', 'rb') as f:
                    self.wfile.write(f.read())
            except:
                # Legacy fallback
                try:
                    with open('/home/john/military_terminal.html', 'rb') as f:
                        self.wfile.write(f.read())
    ...
    def serve_status(self):
        self.send_json({
            "kernel": "/home/john/linux-6.16.9/arch/x86/boot/bzImage",
            "kernel_exists": os.path.exists("/home/john/linux-6.16.9/arch/x86/boot/bzImage"),
            "mode5": "STANDARD (safe)",
    ...
    def serve_commands(self):
        self.send_text("""cd /home/john/linux-6.16.9
...
sudo reboot""")
    ...
    def serve_handoff(self):
        self.serve_file('/home/john/COMPLETE_MILITARY_SPEC_HANDOFF.md')

    def serve_npu(self):
        self.serve_file('/home/john/NPU_MODULES_COMPLETE.md')
    ...
    def list_files(self):
        ...
        query = parse_qs(self.path.split('?', 1)[1] if '?' in self.path else '')
        path = query.get('path', ['/home/john'])[0]
    ...
    def show_logs(self):
        ...
        logs = {
            "kernel_build": "/home/john/kernel-build-apt-secure.log",
    ...
    def run_npu_modules(self):
        ...
        npu_dir = "/home/john/livecd-gen/npu_modules/bin"
    ...
    def kernel_status(self):
        ...
        # Check kernel file
        kernel_path = "/home/john/linux-6.16.9/arch/x86/boot/bzImage"
        status['kernel_built'] = os.path.exists(kernel_path)
        if status['kernel_built']:
            status['kernel_size'] = os.path.getsize(kernel_path)

        # Check DSMIL driver
        dsmil_path = "/home/john/linux-6.16.9/drivers/platform/x86/dell-milspec/dsmil-core.c"
    ...
    def rag_stats(self):
        ...
        try:
            result = subprocess.run(
                ['python3', '/home/john/rag_system.py', 'stats'],
                capture_output=True,
                text=True,
                timeout=10
            )
    ...
    def rag_search(self):
        ...
        try:
            result = subprocess.run(
                ['python3', '/home/john/rag_system.py', 'search', q],
                capture_output=True,
                text=True,
                timeout=30
            )
    ...
    def rag_ingest(self):
        ...
        try:
            result = subprocess.run(
                ['python3', '/home/john/rag_system.py', 'ingest-folder', path],
                capture_output=True,
                text=True,
                timeout=300
            )
    ...
    def archive_vx(self):
        ...
        try:
            result = subprocess.run(
                ['python3', '/home/john/web_archiver.py', 'vxunderground', topic],
                capture_output=True,
                text=True,
                timeout=60
            )
    ...
    def archive_arxiv(self):
        ...
        try:
            result = subprocess.run(
                ['python3', '/home/john/web_archiver.py', 'arxiv', arxiv_id],
                capture_output=True,
                text=True,
                timeout=120
            )
    ...
    def smart_collect(self):
        ...
        try:
            result = subprocess.run(
                ['python3', '/home/john/smart_paper_collector.py', 'collect', topic, max_size],
                capture_output=True,
                text=True,
    ...
    def github_auth_status(self):
        ...
        try:
            result = subprocess.run(
                ['python3', '/home/john/github_auth.py', 'status'],
                capture_output=True,
                text=True,
                timeout=15
            )
    ...
    def github_operation(self):
        ...
        if operation == 'clone':
            ...
            try:
                # Clone to /home/john/github_repos/
                repo_dir = '/home/john/github_repos'
    ...
    def ai_chat(self):
        """AI chat endpoint with SMART ROUTING"""
        query = parse_qs(self.path.split('?', 1)[1] if '?' in self.path else '')
        ...
        try:
            ...
            import sys
            sys.path.insert(0, '/home/john/LAT5150DRVMIL/02-ai-engine')
    ...
    def rag_add_file(self):
        ...
        try:
            import sys
            sys.path.insert(0, '/home/john/LAT5150DRVMIL/04-integrations')
...
Uploads saved to: /home/john/uploads/

Server ready!
""")

with socketserver.TCPServer(("", PORT), FullFeaturedHandler) as httpd:
    try:
        httpd.serve_forever()
