...
impl SmiController {
    ...
    unsafe fn inb_safe(&self, port: u16) -> DsmilResult<u8> {
        ...
        match port {
            DELL_LEGACY_IO_BASE | DELL_LEGACY_IO_DATA | SMI_STATUS_PORT | SMI_CMD_PORT => (),
            _ => return Err(DsmilError::InvalidDevice),
        }

        let value = rust_inb(port);

        if port == SMI_STATUS_PORT && value == 0xFF {
            return Err(DsmilError::HardwareFault);
        }

        Ok(value)
    }

    /// Perform safe port I/O read (32-bit)
    unsafe fn inl_safe(&self, port: u16) -> DsmilResult<u32> {
        extern "C" {
            fn rust_inl(port: u16) -> u32;
        }
    ...
    fn safe_delay_ms(&self, ms: u32) -> DsmilResult<()> {
        ...
        unsafe {
            // Use microsecond delays for precision while avoiding overflow
            let duration = Duration::from_millis(ms as u64);
            let micros = core::cmp::min(duration.as_micros(), u32::MAX as u128) as u32;
            rust_udelay(micros);
            ...
            match self.wait_for_completion(self.timeout_ms) {
                Ok(status) => {
                    ...
                    if request.command == SMI_CMD_TOKEN_READ {
                        unsafe {
                            request.data = self.inl_safe(DELL_LEGACY_IO_DATA)?;
                        }
                    }
...
static SMI_CONTROLLER: GlobalCell<Option<SmiController>> = GlobalCell::new(None);

/// Initialize global SMI controller
pub fn init_smi_controller(enabled: bool) -> DsmilResult<()> {
    unsafe {
        let slot = &mut *SMI_CONTROLLER.get();
        if slot.is_some() {
            return Err(DsmilError::Busy);
        }
        *slot = Some(SmiController::new(enabled));
    }
    Ok(())
}
...
pub fn get_smi_controller() -> Option<&'static mut SmiController> {
    unsafe { (&mut *SMI_CONTROLLER.get()).as_mut() }
}
