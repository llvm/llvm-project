...
impl MmioRegion {
    const MAX_RESIDENT_CHUNKS: usize = 8;
    ...
    pub fn cleanup_unused(&mut self) {
        ...
        if mapped <= Self::MAX_RESIDENT_CHUNKS {
            return;
        }

        let mut to_release = mapped - Self::MAX_RESIDENT_CHUNKS;
        for idx in (0..self.chunk_count).rev() {
