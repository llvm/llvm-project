...
pub struct EmergencyStopController {
    ...
    pub(crate) safety_limits: SecuritySafetyLimits,
}
...
impl EmergencyStopController {
    ...
    pub async fn trigger_emergency_stop(
        ...
        if state.emergency_active {
            log::warn!(
                "Emergency stop already active since {:?}",
                state.triggered_at
            );
            return Ok(());
        }
    ...
    async fn execute_emergency_response(
        ...
        state
            .active_responses
            .push("System isolation initiated".to_string());

        // Verify quarantine integrity
        if self
            .safety_limits
            .quarantine_validation
            .immediate_stop_on_violation
        {
            let quarantine_intact = self.verify_quarantine_integrity().await?;
            if quarantine_intact {
                state
                    .active_responses
                    .push("Quarantine integrity verified".to_string());
            } else {
                state.active_responses.push(
                    "CRITICAL: Quarantine integrity compromised".to_string(),
                );
                log::error!("CRITICAL: Quarantine integrity compromised during emergency stop");
            }
        }

        // Stop all active chaos tests (engine will consult emergency stop)
        state
            .active_responses
            .push("All chaos tests signalled to terminate".to_string());

        // Run system diagnostics
        state
            .active_responses
            .push("System diagnostics initiated".to_string());
    ...
    async fn send_emergency_notifications(
        ...
        for endpoint in &self.safety_limits.emergency_config.notification_endpoints {
            log::warn!(
                "Emergency notification sent to {}: {} (triggered by: {})",
                endpoint,
                reason,
                triggered_by
            );
    ...
    /// Verify system safety (used before clearing emergency stop)
    ...
    /// Verify quarantine integrity
    pub async fn verify_quarantine_integrity(&self) -> Result<bool, SecurityChaosError> {
        log::info!(
            "Verifying quarantine integrity for {} devices",
            self.safety_limits
                .quarantine_validation
                .quarantine_devices
                .len()
...
pub struct ChaosTestSafetyValidator {
    pub(crate) safety_limits: SecuritySafetyLimits,
    pub(crate) emergency_controller: Arc<EmergencyStopController>,
...
impl ChaosTestSafetyValidator {
    ...
    /// Validate test scenario safety *before* execution
    pub async fn validate_test_safety(
        ...
        // Scenario-specific soft safety hints (no hard errors unless absolutely required)
        match scenario {
            SecurityChaosScenario::BruteForceAttack {
                ...
                if total_attempts > 10_000 {
                    log::warn!(
                        "Brute force test has very high attempt count: {}",
                        total_attempts
                    );
                }
            }
            SecurityChaosScenario::APTSimulation { attack_stages, .. } => {
                if attack_stages.len() > 20 {
                    log::warn!(
                        "APT simulation has many stages ({}); consider tightening limits",
                        attack_stages.len()
                    );
                }
            }
    ...
    pub async fn monitor_test_safety(
        ...
        while start_time.elapsed() < duration {
            ...
            // Check resource limits (soft guard; will log + mark violation)
            let resources_ok = self.check_resource_limits().await?;
            if !resources_ok {
                log::warn!("Resource limits exceeded during test {}", test_id);
            }
    ...
    async fn report_safety_violation(
        ...
        match violation {
            SafetyViolation::QuarantineAccess { .. } => {
                self.emergency_controller
                    .trigger_emergency_stop("Quarantine access violation", "Safety Validator")
                    .await?;
            }
            SafetyViolation::CriticalSystemFailure { .. } => {
                self.emergency_controller
                    .trigger_emergency_stop("Critical system failure", "Safety Validator")
                    .await?;
            }
            _ => {
    ...
    /// Get target device from scenario (if any)
    fn get_target_device(&self, scenario: &SecurityChaosScenario) -> Option<u32> {
        match scenario {
            ...
            SecurityChaosScenario::EdrEvasion { host_id, .. } => Some(*host_id),
            _ => None,
        }
    }

    /// Check system resource limits (soft guard)
