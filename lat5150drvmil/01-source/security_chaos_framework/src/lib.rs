...
pub enum StealthLevel {
    /// Rapid execution, higher detection risk
    Low,
    /// Moderate delays between stages
    Medium,
    /// Long delays, maximum stealth
    High,
}
...
pub struct APTStage {
    ...
    /// Probability the stage will be detected (0.0–1.0)
    pub detection_probability: f32,
    /// Probability the stage will succeed (0.0–1.0)
...
pub struct SecurityEffectivenessScore {
    ...
    /// Representative/average response time
...
pub struct SecurityChaosResult {
    ...
    /// Wall-clock duration of the test (serialized as seconds)
...
pub struct SecurityChaosEngine {
    pub test_scenarios: Vec<SecurityChaosScenario>,
    pub system_monitor: SystemSecurityMonitor,
    pub safety_constraints: SecuritySafetyLimits,
    pub active_tests: Arc<Mutex<HashMap<String, ActiveChaosTest>>>,
    pub emergency_stop: Arc<Mutex<bool>>,
}
...
impl SecurityChaosEngine {
    ...
    /// Execute a single controlled security chaos test
    ...
    /// Reset emergency stop state (after manual verification)
