...
impl SystemSecurityMonitor {
    ...
    pub async fn validate_system_readiness(&self) -> Result<bool, SecurityChaosError> {
        ...
        if matches!(health.overall_status, SystemHealth::Critical | SystemHealth::Failure) {
            log::error!(
                "System health is {:?} - chaos testing not safe",
        ...
        if health.cpu_usage > self.safety_thresholds.max_system_load {
            log::warn!(
                "System CPU usage too high for chaos testing: {:.1}%",
        ...
        if health.memory_usage > (1.0 - self.safety_thresholds.min_available_memory) {
            log::warn!(
                "System memory usage too high for chaos testing: {:.1}%",
    ...
    pub async fn monitor_during_test(
        &self,
        duration: Duration,
    ) -> Result<Vec<SecurityMetrics>, SecurityChaosError> {
        let mut metrics_history = Vec::new();
        let start_time = Instant::now();

        log::info!(
            "Starting security monitoring for {} seconds",
            duration.as_secs()
        );
        ...
        log::info!(
            "Security monitoring completed, collected {} metric snapshots",
            metrics_history.len()
        );
        Ok(metrics_history)
    }
    ...
    async fn check_safety_violations(
        ...
        if metrics.failed_authentications > self.safety_thresholds.max_failed_authentications {
            log::error!(
                "Safety violation: Too many failed authentications ({})",
        ...
        if metrics.access_violations > self.safety_thresholds.max_access_violations {
            log::error!(
                "Safety violation: Too many access violations ({})",
                metrics.access_violations
            );
            return Ok(true);
        }
        ...
        if metrics.quarantine_violations > 0 {
            log::error!(
                "CRITICAL safety violation: Quarantine violations detected ({})",
                metrics.quarantine_violations
            );
            return Ok(true);
        }
...
impl SecurityChaosEngine {
    /// Start continuous system monitoring (returns a handle for correlation)
    pub async fn start_continuous_monitoring(
        &self,
    ) -> Result<MonitorHandle, SecurityChaosError> {
        ...
        log::info!(
            "Started continuous monitoring with handle {}",
            handle.handle_id
        );
        Ok(handle)
    }
    ...
    ) -> Result<SecurityMetrics, SecurityChaosError> {
        log::info!("Stopping monitoring for handle {}", handle.handle_id);

        let monitor = SystemSecurityMonitor::new()?;
        let final_metrics = monitor.collect_current_metrics().await?;
    ...
    pub async fn validate_safety_constraints(
        ...
        // Check system health
        let system_ready = monitor.validate_system_readiness().await?;
        if !system_ready {
            return Err(SecurityChaosError::SecurityViolation(
                "System health not sufficient for chaos testing".to_string(),
            ));
        }

        // Check for active high-risk operations
        let active_operations = monitor.get_active_operations().await?;
        for operation in &active_operations {
            if operation.risk_level >= RiskLevel::High {
                return Err(SecurityChaosError::SecurityViolation(
                    "High-risk operation active - chaos testing blocked".to_string(),
                ));
            }
        }
        ...
        match scenario {
            SecurityChaosScenario::AuthorizationBypass { risk_level, .. } => {
                if *risk_level >= "Critical".to_string() {
                    // risk_level is a String in your current enum variant
                    return Err(SecurityChaosError::SecurityViolation(
                        "AuthorizationBypass scenario risk level too high".to_string(),
                    ));
                }
            }
            ...
            SecurityChaosScenario::APTSimulation { attack_stages, .. } => {
                if attack_stages.len() > 10 {
                    log::warn!(
                        "APT simulation has many stages ({}); ensuring proper pacing",
                        attack_stages.len()
                    );
                }
            }
    ...
    /// Analyze chaos test results into a structured outcome
    pub fn analyze_chaos_results(
        ...
        let security_effectiveness =
            self.calculate_security_effectiveness(&baseline_metrics, &final_metrics, &execution_result);
        ...
        let recommendations =
            self.generate_security_recommendations(&execution_result, &vulnerabilities_found);

        // Assess compliance impact
        let compliance_impact =
            self.assess_compliance_impact(&execution_result, &vulnerabilities_found);

        // Determine overall test success
        let test_success =
            self.determine_test_success(&execution_result, &security_effectiveness);
        ...
        let result = SecurityChaosResult {
            ...
            emergency_stops_triggered: 0, // TODO: wire from safety/emergency controller
            test_success,
        };

        log::info!(
            "Chaos test analysis complete: success={}, vulnerabilities={}, recommendations={}",
            result.test_success,
    ...
    fn calculate_security_effectiveness(
        &self,
        _baseline: &SecurityMetrics,
        _final_metrics: &SecurityMetrics,
        execution_result: &ChaosExecutionResult,
    ) -> SecurityEffectivenessScore {
        let detection_rate = match execution_result {
            ChaosExecutionResult::BruteForceResult {
                lockout_triggered,
            ...
            ChaosExecutionResult::PrivilegeEscalationResult {
        ...
        let response_time_ms = match execution_result {
            ChaosExecutionResult::PrivilegeEscalationResult { detection_time_ms, .. } => {
                *detection_time_ms
            }
            ChaosExecutionResult::AuthorizationBypassResult { detection_time_ms, .. } => {
                *detection_time_ms
            }
            ChaosExecutionResult::HardwareTamperResult { detection_time_ms, .. } => {
                *detection_time_ms
            }
            _ => 500, // Default response time
        };

        let mitigation_effectiveness = detection_rate;
    ...
    fn identify_vulnerabilities(
        ...
        match execution_result {
            ChaosExecutionResult::BruteForceResult {
                ...
                if !lockout_triggered && !rate_limiting_effective {
                    vulnerabilities.push(SecurityVulnerability {
                        ...
                        category: "Authentication".to_string(),
                        description:
                            "No rate limiting or account lockout protection in place".to_string(),
                        affected_components: vec!["Authentication System".to_string()],
            ...
            ChaosExecutionResult::AuthorizationBypassResult {
                bypass_successful, ..
            } => {
                if *bypass_successful {
                    vulnerabilities.push(SecurityVulnerability {
                        ...
                        mitigation_recommendations: vec![
                            ...
                            "Add authorization logging and alerting".to_string(),
                        ],
                        discovered_at: Utc::now(),
                    });
                }
    ...
    fn generate_security_recommendations(
            ...
            ChaosExecutionResult::BruteForceResult {
                ...
                if !lockout_triggered {
                    recommendations.push(SecurityRecommendation {
                        ...
                        description:
                            "Add automatic account lockout after repeated failed authentication attempts".to_string(),
                        implementation_effort: "Low".to_string(),
                        expected_impact:
                            "Significantly reduces brute force attack effectiveness".to_string(),
                ...
                if !rate_limiting_effective {
                    recommendations.push(SecurityRecommendation {
                        ...
                        description:
                            "Add progressive delays and rate limiting for authentication attempts"
                                .to_string(),
                ...
                if *total_stages > 0 {
                    ...
                    if detection_rate < 0.8 {
                        recommendations.push(SecurityRecommendation {
                            ...
                            description:
                                "Improve detection coverage and fidelity for advanced persistent threats"
                                    .to_string(),
                            implementation_effort: "High".to_string(),
                            expected_impact:
                                "Better detection of sophisticated multi-stage attacks".to_string(),
        ...
        recommendations.push(SecurityRecommendation {
            ...
            description:
                "Schedule regular security chaos testing campaigns to validate and tune defenses"
                    .to_string(),
            implementation_effort: "Medium".to_string(),
    ...
    fn assess_compliance_impact(
        &self,
        _execution_result: &ChaosExecutionResult,
        vulnerabilities: &[SecurityVulnerability],
    ) -> ComplianceImpactAssessment {
        let overall_risk = if vulnerabilities
            .iter()
            .any(|v| v.severity >= RiskLevel::Critical)
        {
            RiskLevel::High
        } else if vulnerabilities
            .iter()
            .any(|v| v.severity >= RiskLevel::High)
        {
            RiskLevel::Medium
        } else {
            RiskLevel::Low
        ...
        ComplianceImpactAssessment {
            fips_compliance_impact:
                "Minimal impact; encryption and authentication controls validated".to_string(),
            common_criteria_impact:
                "Testing provides evidence for security functional requirements".to_string(),
            nato_stanag_impact: "Chaos testing supports assurance and readiness objectives"
                .to_string(),
    ...
    fn determine_test_success(
        ...
        match execution_result {
            ...
            ChaosExecutionResult::HardwareTamperResult { tamper_detected, .. } => *tamper_detected,
            ChaosExecutionResult::APTResult { .. } => {
                security_effectiveness.detection_rate >= 0.7
            }
            _ => security_effectiveness.overall_score >= 0.8,
        }
    }

    /// Generate comprehensive test report (log-based)
    pub async fn generate_chaos_report(
        &self,
        result: &SecurityChaosResult,
    ) -> Result<(), SecurityChaosError> {
        log::info!(
            "Generating comprehensive chaos test report for test {}",
            result.test_id
        );
        ...
        log::info!(
            "Security Effectiveness Score: {:.2}",
        ...
        log::info!(
            "Compliance Risk: {}",
            result.compliance_impact.overall_compliance_risk
        );
