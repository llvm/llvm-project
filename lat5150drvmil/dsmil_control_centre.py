...
# Add paths - control center in root, imports from 02-ai-engine
...
class DSMILControlCentre:
    ...
    def __init__(self):
        ...
        logger.info("DSMIL Control Centre initialized")

    def show_startup_banner(self):
        """Display startup banner with system information"""
        print("\n" + "=" * 70)
        print("╔══════════════════════════════════════════════════════════════════╗")
        print("║     DSMIL CONTROL CENTRE - 104-DEVICE TACTICAL EDITION          ║")
        print("║                    LAT5150DRVMIL v2.0.0                          ║")
        print("╚══════════════════════════════════════════════════════════════════╝")
        print("=" * 70)
        print()

        # Show driver status and backend
        backend = None
        if check_driver_loaded():
            driver = DSMILDriverInterface()
            if driver.open():
                version = driver.get_version()
                print(f"✓ Driver Loaded: v{version}")
                driver.close()
            else:
                print("✓ Driver Detected: /dev/dsmil0")

            # Best-effort backend detection via sysfs
            from dsmil_driver_interface import get_backend_name
            backend = get_backend_name()
            if backend:
                print(f"✓ SMBIOS Backend: {backend}")
                if backend.startswith("simulated"):
                    print("⚠ Simulation mode: no real firmware token access")
        else:
            print("⚠️  Driver Not Loaded (install with: sudo dpkg -i packaging/dsmil-complete*.deb)")

        # Show device database stats
        stats = get_statistics_extended()
        print(f"✓ Device Database: {stats['total_devices']} devices ({stats['safe']} safe, {stats['quarantined']} quarantined)")

        # Show system status if available
        status = self.adapter.get_system_status()
        if status:
            print(f"✓ System Status: {status.device_count} devices ready, {status.thermal_celsius}°C")

        print()
        print("Ready for operation. Use menu to begin device discovery and activation.")
        print("=" * 70)
    ...
    def show_main_menu(self):
        ...
        while True:
            ...
            if status:
                print(f"[Status] Devices: {len(self.discovered_devices)} discovered, " +
                      f"{len(self.activated_devices)} active | " +
                      f"Temp: {status.thermal_celsius}°C")
                print()
...
def main():
    ...
    parser.add_argument('--check-only', action='store_true',
                       help="Check dependencies and exit (for testing)")

    args = parser.parse_args()

    try:
        # Check-only mode for entrypoint validation
        if args.check_only:
            print("✓ DSMIL Control Centre dependencies OK")
            return 0
        ...
        try:
            ...
            print(f"✗ Failed to initialize DSMIL Control Centre")
            print(f"  Missing dependency: {e}")
            print(f"\nTo install dependencies:")
            print(f"  ./lat5150.sh install")
            return 1
        ...
        if args.auto_discover:
            try:
                ...
                if not args.non_interactive:
                    control_centre.show_startup_banner()
                    control_centre.show_main_menu()
            except KeyboardInterrupt:
                print("\n\n✓ User cancelled operation")
                return 0
            except Exception as e:
                print(f"\n✗ Discovery/activation failed: {e}")
                logger.error(f"Discovery/activation failed: {e}", exc_info=True)
                return 1
        else:
            # Interactive menu
            try:
                ...
                print("\n\n✓ User exited control centre")
                return 0
            except Exception as e:
                print(f"\n✗ Control centre error: {e}")
                logger.error(f"Control centre error: {e}", exc_info=True)
                return 1
