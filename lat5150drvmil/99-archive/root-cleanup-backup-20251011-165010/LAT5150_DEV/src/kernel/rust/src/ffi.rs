...
use crate::{
    DsmilError, DsmilResult, DeviceState, GroupState, TokenPosition,
    DsmilDevice, DsmilGroup, DeviceRegistry, get_device_registry,
    smi::{SmiController, get_smi_controller, init_smi_controller, cleanup_smi_controller},
    memory::{MmioRegion, MemoryPool, discover_dsmil_region},
};
...
fn to_c_error_with_value<T>(result: DsmilResult<T>) -> (i32, T)
where
    T: Default,
{
    match result {
        Ok(value) => (0, value),
        Err(e) => (CDsmilError::from(e) as i32, T::default()),
    }
}
...
pub extern "C" fn rust_dsmil_set_device_state(
    ...
    let state = match new_state {
        ...
        4 => DeviceState::Error,
        5 => DeviceState::Locked,
        _ => return CDsmilError::InvalidDevice as i32,
    };
