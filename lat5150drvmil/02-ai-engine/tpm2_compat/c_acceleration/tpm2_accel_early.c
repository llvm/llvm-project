...
// External Dell SMBIOS function declarations (would be imported)
extern int dell_smbios_call(void *buffer);
...
static int tmp2_accel_validate_security(void);
...
static int tpm2_accel_validate_dell_tokens(void)
{
    ...
    pr_info(DRIVER_NAME ": Dell token validation complete (%llu tokens authorized)\n",
            __builtin_popcountll(sec->authorized_tokens));
    return 0;
}
...
static int tpm2_accel_init_chardev(void)
{
    ...
    // Create device class
    bridge->chardev.class = class_create(THIS_MODULE, DRIVER_NAME);
...
static int tpm2_accel_init_shared_memory(void)
{
    ...
    // Allocate DMA-coherent shared memory
    bridge->shared_mem.virt_addr = dma_alloc_coherent(bridge->chardev.device,
                                                     TPM2_ACCEL_SHARED_MEM_SIZE,
                                                     &bridge->shared_mem.phys_addr,
                                                     GFP_KERNEL);
...
static int tpm2_accel_init_monitoring(void)
{
    struct tmp2_accel_monitor *mon = &tpm2_accel_monitor;
...
static void tpm2_accel_cleanup_hardware(void)
{
    ...
    if (tpm2_accel_hw.me.present && tpm2_accel_hw.me.base) {
        ...
        if (tpm2_accel_hw.me.pdev) {
            ...
            pci_dev_put(tmp2_accel_hw.me.pdev);
        }
    }
...
static void tpm2_accel_cleanup_monitoring(void)
{
    ...
    del_timer_sync(&mon->monitor_timer);
