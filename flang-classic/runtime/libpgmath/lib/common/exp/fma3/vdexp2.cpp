...
__m128d __pgm_exp_d_vec128_slowpath(__m128d const a, __m128i const i, __m128d const t,  __m128d const z)
{
    ...
    __m128d a_plus_inf = _mm_add_pd(a, INF); // check if a is too big           
    __m128d zero_inf_blend = _mm_blendv_pd(a_plus_inf, ZERO, lt_zero_mask);     

    __m128d accurate_scale_mask = (__m128d)_mm_cmp_pd(abs_lt, UPPERBOUND_2, _CMP_LT_OS); 
...
__m128d __fvd_exp_fma3(__m128d const a)
{
    ...
    // calculate mantissa
    //fast mul rint
    __m128d t = _mm_sub_pd (_mm_fmadd_pd(a, L2E, DBL2INT_CVT), DBL2INT_CVT);
    __m128d m = _mm_fmadd_pd (t, NEG_LN2_HI, a);
    m = _mm_fmadd_pd (t, NEG_LN2_LO, m);

    // evaluate highest 8 terms of polynomial with estrin, then switch to horner
    __m128d z10 = _mm_fmadd_pd(EXP_POLY_11, m, EXP_POLY_10);
    __m128d z8  = _mm_fmadd_pd(EXP_POLY_9, m, EXP_POLY_8);
    __m128d z6  = _mm_fmadd_pd(EXP_POLY_7, m, EXP_POLY_6);
    __m128d z4  = _mm_fmadd_pd(EXP_POLY_5, m, EXP_POLY_4);

    __m128d m2 = _mm_mul_pd(m, m);
    z8 = _mm_fmadd_pd(z10, m2, z8);
    z4 = _mm_fmadd_pd(z6, m2, z4); 
    
    __m128d m4 = _mm_mul_pd(m2, m2);
    z4 = _mm_fmadd_pd(z8, m4, z4);

    t = _mm_fmadd_pd(z4, m, EXP_POLY_3);
    t = _mm_fmadd_pd(t, m, EXP_POLY_2);
    t = _mm_fmadd_pd(t, m, EXP_POLY_1);
    t = _mm_fmadd_pd(t, m, EXP_POLY_0);
...
//    int exp_slowmask = _mm_movemask_pd(_mm_cmp_pd(abs_a, UPPERBOUND_1, _CMP_GE_OS));
