...
/* mmulLOG1.c --F90  MATMUL intrinsics for logical*1 type */

#include "stdioInterf.h"
#include "fioMacros.h"

void ENTF90(MATMUL_LOG1, matmul_log1)(char *dest_addr, char *s1_addr,
                                      ...
                                      F90_Desc *s1_desc, F90_Desc *s2_desc)
{
  ...
  s1_base = (__LOG1_T *)s1_addr + F90_LBASE_G(s1_desc) +
            s1_d1_lb * s1_d1_lstride + s1_d2_lb * s1_d2_lstride - 1;
  s2_base = (__LOG1_T *)s2_addr + F90_LBASE_G(s2_desc) +
            s2_d1_lb * s2_d1_lstride + s2_d2_lb * s2_d2_lstride - 1;
  dest_base = (__LOG1_T *)dest_addr + F90_LBASE_G(dest_desc) +
  ...
  if (s1_rank == 2) {
    ...
    for (k = 0; k < k_extent; k++) {
      ...
      for (m = 0; m < m_extent; m++) {
        ...
        d_elem_p = dest_base + d_d1_base + d_d2_offset;
        for (n = 0; n < n_extent; n++) {
          if ((*s1_elem_p & GET_DIST_MASK_LOG1) &&
              (*s2_elem_p & GET_DIST_MASK_LOG1)) {
            *d_elem_p = GET_DIST_TRUE_LOG1;
          }
