...
/** \file
 * \brief F90  MATMUL intrinsics for COMPLEX*8 type
 */
...
void ENTF90(MATMUL_CPLX8, matmul_cplx8)(char *dest_addr, char *s1_addr,
                                        ...
                                        F90_Desc *s1_desc, F90_Desc *s2_desc)
{
  ...
  s1_base = (__CPLX8_T *)s1_addr + F90_LBASE_G(s1_desc) +
            s1_d1_lb * s1_d1_lstride + s1_d2_lb * s1_d2_lstride - 1;
  s2_base = (__CPLX8_T *)s2_addr + F90_LBASE_G(s2_desc) +
            s2_d1_lb * s2_d1_lstride + s2_d2_lb * s2_d2_lstride - 1;
  dest_base = (__CPLX8_T *)dest_addr + F90_LBASE_G(dest_desc) +
  ...
  if ((s1_d1_sstride == 1) && (s2_d1_sstride == 1) && (d_d1_sstride == 1) &&
    ...
    if (s2_rank == 1) {
      F90_MATMUL(cplx8_str1_mxv)( dest_base + d_d1_soffset*d_d1_lstride +
      ...
      F90_MATMUL(cplx8_str1_vxm)( dest_base + d_d1_soffset*d_d1_lstride +
                                            d_d2_soffset*d_d2_lstride,
                                     s1_base + s1_d1_soffset * s1_d1_lstride,
                                     s2_base + s2_d2_soffset * s2_d2_lstride,
                                     &k_extent,&m_extent,
                                     &s2_d2_lstride, &d_d1_lstride);

    } else {
      F90_MATMUL(cplx8_str1)(dest_base + d_d1_soffset*d_d1_lstride +
                                            d_d2_soffset*d_d2_lstride,
                                     s1_base + s1_d1_soffset * s1_d1_lstride,
                                     s2_base + s2_d2_soffset * s2_d2_lstride,
                                     &k_extent,&m_extent,&n_extent,
                                     &s1_d2_lstride,&s2_d2_lstride,&d_d2_lstride,
                                     &d_d1_lstride);
    }
  } else if (s1_rank == 2) {
    ...
    for (k = 0; k < k_extent; k++) {
      ...
      for (m = 0; m < m_extent; m++) {
        ...
        for (n = 0; n < n_extent; n++) {
          d_elem_p->r +=
              s1_elem_p->r * s2_elem_p->r - s1_elem_p->i * s2_elem_p->i;
          d_elem_p->i +=
              s1_elem_p->r * s2_elem_p->i + s1_elem_p->i * s2_elem_p->r;
    ...
    for (k = 0; k < k_extent; k++) {
      ...
      rslt_tmp.i = 0;
      for (m = 0; m < m_extent; m++) {
        rslt_tmp.r += s1_elem_p->r * s2_elem_p->r - s1_elem_p->i * s2_elem_p->i;
        rslt_tmp.i += s1_elem_p->r * s2_elem_p->i + s1_elem_p->i * s2_elem_p->r;
