...
! one dimension out of two
	program p
	 ...
	 eopc=eoshift(eoshift(src,j,9,dim=2), 1, dim=2)	! eoshift, positive, constant
	 eopv=eoshift(eoshift(src,j,9,dim=2), j, dim=2)	! eoshift, positive, variable
	 eomc=eoshift(eoshift(src,j,9,dim=2),-1, dim=2)	! eoshift, negative, constant
	 eomv=eoshift(eoshift(src,j,9,dim=2), k, dim=2)	! eoshift, negative, variable
	  cpc= cshift(eoshift(src,j,9,dim=2), 1, dim=2)	!  cshift, positive, constant
	  cpv= cshift(eoshift(src,j,9,dim=2), j, dim=2)	!  cshift, positive, variable
	  cmc= cshift(eoshift(src,j,9,dim=2),-1, dim=2)	!  cshift, negative, constant
	  cmv= cshift(eoshift(src,j,9,dim=2), k, dim=2)	!  cshift, negative, variable
	 if( doprint )then
	  print 10, ' src',src
	  print 10, 'eopc',eopc
	  print 10, 'eopv',eopv
