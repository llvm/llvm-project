...
module my_container
  ...
  type container
   ...
   contains
     ...
     procedure,pass(this) :: assign_to_i 
  ...
  character c
  contains
  procedure,pass(this) :: assign_to_container2
  generic :: assignment(=) => assign_to_container2
  end type container2
...
contains
  ...
  subroutine assign_to_container2(this,src)
  class(container),intent(in) :: src
  class(container2),intent(inout) :: this
  this%container = src
  end subroutine assign_to_container2
  ...
  type(container) function add_int(first,second) RESULT(ttt)
  class(container),intent(in) :: second
...
USE CHECK_MOD
  ...
  rslt(7) = c2%extract(1.0) .eq. 11.0
  rslt(8) = c2%extract(1) .eq. 11
