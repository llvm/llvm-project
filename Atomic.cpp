...
void Fortran::lower::omp::lowerAtomic(
  ...
  if (auto *cond = get(analysis.cond)) {
    ...
    if (construct.IsCapture()) {
      ...
      builder, loc, hint, nullptr);
     hint = nullptr;
     // DO NOT clear memOrder here
    auto captureMemOrder = memOrder;
    ...
    *get(analysis.op0.assign), hint,
    makeValidForAction(captureMemOrder, action0, analysis.None, version),
    preAt, atomicAt, postAt);
   // The second operation in an atomic capture may require a different
  // memory ordering than the first.
    mlir::Operation *secondOp = genAtomicOperation(
    converter, semaCtx, loc, stmtCtx, analysis.op1.what, atomAddr, atom,
    *get(analysis.op1.assign), hint,
    makeValidForAction(captureMemOrder, action1, analysis.None, version),
    preAt, atomicAt, postAt);
