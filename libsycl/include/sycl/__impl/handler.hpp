...
private:
  ...
  template <int Dims> void prepareParallelForRange(range<Dims> numWorkItems) {
    MKernelRangeStorage[0] = numWorkItems[0];
    MKernelRangeStorage[1] = Dims > 1 ? numWorkItems[1] : 1;
    MKernelRangeStorage[2] = Dims > 2 ? numWorkItems[2] : 1;
    MKernelRange = detail::UnifiedRangeView(MKernelRangeStorage.data(), nullptr,
                                            nullptr, static_cast<size_t>(Dims));
  }
  ...
  std::array<size_t, 3> MKernelRangeStorage{{1, 1, 1}};
