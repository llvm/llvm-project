; REQUIRES: aarch64
;; Herbception ("throws"/"fails{E}") ODR checking during full LTO on AArch64:
;; the herbception specifier is not part of the mangled name, so translation
;; units that disagree about it produce identical symbol names with
;; incompatible ABIs. LTO must diagnose this.

; RUN: llvm-as %p/Inputs/herbception-odr-aarch64-throws.ll -o %tthrows.o
; RUN: llvm-as %p/Inputs/herbception-odr-aarch64-other-payload.ll -o %tother.o
; RUN: llvm-as %p/Inputs/herbception-odr-aarch64-plain-decl.ll -o %tdecl.o
