...
static uint64_t computeFileOffset(Ctx &ctx, OutputSection *os, uint64_t off,
  ...
  // A zero-sized section following a NOBITS section in the same PT_LOAD does
  // not occupy file space, so its file offset does not need to account for the
  // size of the preceding NOBITS section.
  if (os->size == 0 && prev && prev->ptLoad == os->ptLoad &&
      prev->type == SHT_NOBITS)
    return off;
...
template <class ELFT> void Writer<ELFT>::assignFileOffsets() {
  ...
  for (OutputSection *sec : ctx.outputSections) {
    if (!(sec->flags & SHF_ALLOC))
      continue;
    off = computeFileOffset(ctx, sec, off, prev);
    sec->offset = off;
