...
RelExpr Xtensa::getRelExpr(RelType type, const Symbol &s,
  ...
  switch (type) {
  case R_XTENSA_32:
    // GNU R_XTENSA_32 is partial_inplace: the assembler stores the addend
    // in the relocated word and leaves r_addend as 0. LLD cannot apply that
    // encoding correctly alongside LLVM objects, which keep the addend in
    // r_addend. Use R_XTENSA_32_ABS for the LLVM encoding.
    Err(ctx) << getErrorLoc(ctx, loc)
             << "GNU-style R_XTENSA_32 relocation found which cannot be "
                "handled properly";
    return R_NONE;
