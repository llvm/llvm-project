...
BUILD_DIR="${1:-build_release_x86}"
OUTPUT="${2:-ejit_test/lipo/ejit.o}"

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
WORK_DIR="$SCRIPT_DIR/.lipo_work_x86"
...
python3 "$SCRIPT_DIR/lipo.py" gc-merge \
  --input="$SCRIPT_DIR/libejit_lipo_x86.a" \
  --build-dir="$BUILD_DIR"
...
python3 "$SCRIPT_DIR/lipo.py" merge \
  --input="$SCRIPT_DIR/libejit_lipo_x86_gc.a" \
  --build-dir="$BUILD_DIR" \
  --output="$OUTPUT"
