...
static void print_record_ompt(ompt_record_ompt_t *rec) {
  ...
  printf("rec=%p type=%2d time=%lu thread_id=%lu target_id=%lu\n", rec,
         rec->type, rec->time, rec->thread_id, rec->target_id);

  switch (rec->type) {
  case ompt_callback_target:
  case ompt_callback_target_emi: {
    ompt_record_target_t target_rec = rec->record.target;
    printf("\tRecord Target task: target_id=0x%lx kind=%d endpoint=%d "
           "device=%d task_id=%lu codeptr=%p\n",
           target_rec.target_id, target_rec.kind, target_rec.endpoint,
           target_rec.device_num, target_rec.task_id, target_rec.codeptr_ra);
    break;
  }
  ...
  case ompt_callback_target_data_op_emi: {
    ...
    printf("\t  Record Target data op: target_id=0x%lx host_op_id=0x%lx "
           "optype=%d src_addr=%p src_device=%d "
           "dest_addr=%p dest_device=%d bytes=%lu end_time=%lu duration=%lu ns "
           "codeptr=%p\n",
           rec->target_id, target_data_op_rec.host_op_id,
           target_data_op_rec.optype, target_data_op_rec.src_addr,
           ...
           target_data_op_rec.end_time, target_data_op_rec.end_time - rec->time,
  ...
  case ompt_callback_target_submit_emi: {
    ...
    printf("\t  Record Target kernel:  target_id=0x%lx host_op_id=0x%lx "
           "requested_num_teams=%u granted_num_teams=%u "
           "end_time=%lu duration=%lu ns\n",
           rec->target_id, target_kernel_rec.host_op_id,
