...
int32_t DeviceTy::launchKernel(void *TgtEntryPtr, void **TgtVarsPtr,
  ...
  LaunchArgs.KernelLaunchInfo.ReductionDataSize =
      KernelLaunchInfo.ReductionDataSize;
  LaunchArgs.KernelLaunchInfo.MaxNumThreads = KernelLaunchInfo.MaxNumThreads;
  ...
  if (!StrictThreads || !StrictBlocks) {
    ...
    if (!StrictThreads)
      EffectiveNumThreads = getEffectiveNumThreads(
          GenericDevice, EffectiveNumThreads, KernelLaunchInfo);

    if (!StrictBlocks)
      LaunchArgs.UserNumBlocks[0] = getEffectiveNumBlocks(
          GenericDevice, LaunchArgs.UserNumBlocks[0], LaunchArgs.Tripcount,
          EffectiveNumThreads, StrictThreads, ThreadLimitFromUser,
          KernelLaunchInfo);
