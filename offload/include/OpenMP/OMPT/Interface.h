...
public:
  TracerInterfaceRAII(FunctionPairTy Callbacks, AsyncInfoTy &AsyncInfo,
                      plugin::GenericProfilerTy *ProfilerPtr,
                      int TracedDeviceId, ompt_callbacks_t EventType,
                      ArgsTy... Args)
      : Arguments(Args...), beginFunction(std::get<0>(Callbacks)) {
    plugin::GenericProfilerTy &Prof =
        ProfilerPtr ? *ProfilerPtr : plugin::getNoOpProfiler();
...
#else
// Consumers that only include this header still need the statement guard to
// expand to nothing when OMPT is not built.
