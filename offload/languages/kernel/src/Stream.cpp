...
ol_result_t StreamTy::waitOnAndTrackDependencyEvents(
    SmallVectorImpl<ol_event_handle_t> &Events) {
  if (Events.empty())
    return OL_SUCCESS;

  ol_result_t WaitResult = olWaitEvents(Queue, Events.data(), Events.size());
  ...
  DependencyEvents.append(Events.begin(), Events.end());
