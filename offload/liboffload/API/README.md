...
```
This generates `detail/OffloadPrintGeneric.inc`, which contains print operator
definitions for various API objects, including function parameters.

The public `OffloadPrintRaw.hpp` and `OffloadPrintStd.hpp` headers select
`llvm::raw_ostream` and `std::ostream`, respectively, before including the
generated implementation. Users must deliberately include the header for the
stream type they use.
