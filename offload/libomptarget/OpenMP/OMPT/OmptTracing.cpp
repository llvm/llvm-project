...
void llvm::omp::target::ompt::setBufferManagementFns(
  ...
  if (BufferMgrItr != getBufferMgmt().end()) {
    REPORT()
        << "Buffer request and complete functions already exist for device  "
        << DeviceId << "ignoring ...";
    return;
  }
...
Interface::startTargetDataSubmitTrace(int64_t SrcDeviceId, void *SrcPtrBegin,
  ...
  ODBG(debug::ODT_Tool) << "OMPT-Async: Returning data trace record buf ptr "
                        << DataPtr;
  return DataPtr;
}
...
ompt_record_ompt_t *Interface::stopTargetDataMovementTraceAsync(
  ...
  // The trace record has been created, mark it ready for delivery to the tool
  TRM->setTRStatus(DataPtr, OmptTracingBufferMgr::TR_ready);
  ODBG(debug::ODT_Tool) << "OMPT-Async: Completed target_data trace record "
                        << DataPtr;
  return DataPtr;
}
...
ompt_record_ompt_t *Interface::startTargetSubmitTrace(int64_t DeviceId,
  ...
  ODBG(debug::ODT_Tool) << "OMPT-Async: Returning kernel trace record buf ptr "
                        << DataPtr;
  return DataPtr;
}
...
Interface::stopTargetSubmitTraceAsync(ompt_record_ompt_t *DataPtr,
  ...
  // Ready Record
  TRM->setTRStatus(DataPtr, OmptTracingBufferMgr::TR_ready);
  ODBG(debug::ODT_Tool) << "OMPT-Async: Completed trace record buf ptr "
                        << DataPtr;
  return DataPtr;
}
...
int libomptarget_ompt_start_trace(int DeviceId,
  ...
  if (!PM) {
    REPORT() << "Failed to start trace for DeviceId=" << DeviceId
...
int libomptarget_ompt_flush_trace(int DeviceId) {
  if (!PM) {
    REPORT() << "Failed to flush trace for DeviceId=" << DeviceId
             << " (invalid plugin manager)";
    // Indicate failure
    return 0;
  }
...
int libomptarget_ompt_stop_trace(int DeviceId) {
  if (!PM) {
    REPORT() << "Failed to stop trace for DeviceId=" << DeviceId
             << " (invalid plugin manager)";
    // Indicate failure
    return 0;
  }
...
int libomptarget_ompt_advance_buffer_cursor(ompt_device_t *Device,
  ...
  if (!PM) {
    REPORT() << "Failed to advance buffer cursor for Device=" << Device
             << " (invalid plugin manager)";
    // Indicate failure
    return false;
  }
  ...
  *NextPos = (ompt_buffer_cursor_t)(TraceRecord + TRSize);
  ODBG(llvm::omp::target::debug::ODT_Tool)
      << "Advanced buffer pointer by " << TRSize << " bytes to "
      << TraceRecord + TRSize;
  return true;
}
