...
void X86_64ABIInfo::classify(QualType Ty, uint64_t OffsetBase, Class &Lo,
  ...
  if (const RecordType *RT = Ty->getAsCanonical<RecordType>()) {
    ...
    for (RecordDecl::field_iterator i = RD->field_begin(), e = RD->field_end();
      ...
      // alignment. A non-zero-width unnamed bit-field, however, is real storage
      // and its eightbyte(s) must be classified INTEGER, matching GCC. Skipping
      // it would leave an eightbyte NO_CLASS even though record lowering merged
