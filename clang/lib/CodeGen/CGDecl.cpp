...
void CodeGenFunction::AddPointerMetodataForRestrict(RawAddress AllocaAddr,
  ...
  while (const PointerType *PT = CurTy->getAs<PointerType>()) {
    if (RestrictSize >= RestrictCodes.size())
      RestrictCodes.push_back(0);
    RestrictSize++;
    CurTy = PT->getPointeeType();
  }

  unsigned RestictNesting = 1, Code = 0;
  while (const PointerType *PT = Ty->getAs<PointerType>()) {
    if (Ty.isRestrictQualified()) {
      assert(RestrictSize >= RestictNesting);
      auto &RestrictCodesElem = RestrictCodes[RestrictSize - RestictNesting];
      RestrictCodesElem++;
  ...
  std::reverse(RestrictMDs.begin(), RestrictMDs.end());
