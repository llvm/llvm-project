...
llvm::Value *emitAMDGCNDsBpermute(CodeGenFunction &CGF, const CallExpr *Call,
  ...
  if (Index->getType()->isPointerTy())
    Index = Builder.CreatePtrToInt(
        Index, Builder.getIntNTy(DL.getPointerSizeInBits(
  ...
  for (unsigned I = 0; I < NumWords; ++I) {
    ...
    if (I > 0)
      Word = Builder.CreateLShr(
          Word, llvm::ConstantInt::get(Word->getType(), I * 32));
    ...
    if (I > 0)
      Extended = Builder.CreateShl(
          Extended, llvm::ConstantInt::get(Extended->getType(), I * 32));
