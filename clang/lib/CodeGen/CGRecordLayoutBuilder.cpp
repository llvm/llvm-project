...
CGRecordLowering::accumulateBitFields(bool isNonVirtualBaseType,
  ...
  struct BitFieldSpan {
    ...
    uint64_t Size;
    // True if a zero-width bitfield or non-bitfield follows this span (prevents
  ...
  SmallVector<BitFieldSpan, 4> Spans;
  {
    ...
    for (; Field != FieldEnd && Field->isBitField(); ++Field) {
      ...
      if (SpanBegin == FieldEnd) {
        ...
        // be part of the same span. This can include zero-length bitfields,
        // should the target not align them to character boundaries. Such
        // non-alignment is at variance with the standards, which require
        // zero-length bitfields be a barrier between access units. But of
        // course we can't achieve that in the middle of a character.
        assert(BitOffset == Context.toBits(SpanOffset) + SpanSize &&
               "Concatenating non-contiguous bitfields");
      } else {
        // Bitfield potentially begins a new span. This includes
  ...
  for (unsigned I = 0, Size = Spans.size(); I != Size;) {
    ...
    emitAccessUnit(BeginOffset, BestEndOffset, BestClipped, Spans[I].Begin,
                   Spans[BestEnd - 1].End);
    I = BestEnd;
  }
