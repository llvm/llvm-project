...
parseCrossTUIndex(StringRef IndexPath) {
  ...
  while (std::getline(ExternalMapFile, Line)) {
    ...
    if (Pos > 0 && Pos != std::string::npos) {
      ...
      Result[LookupName] = std::string(FileName);
    } else
      return llvm::make_error<IndexError>(
...
CrossTranslationUnitContext::getCrossTUDefinition(const FunctionDecl *FD,
                                                  ...
                                                  StringRef IndexName,
                                                  bool DisplayCTUProgress) {
  return getCrossTUDefinitionImpl(FD, CrossTUDir, IndexName, DisplayCTUProgress);
}
...
CrossTranslationUnitContext::getCrossTUDefinition(const VarDecl *VD,
                                                  ...
                                                  StringRef IndexName,
                                                  bool DisplayCTUProgress) {
  return getCrossTUDefinitionImpl(VD, CrossTUDir, IndexName, DisplayCTUProgress);
}
...
CrossTranslationUnitContext::ASTFileLoader::ASTFileLoader(
    CompilerInstance &CI, StringRef CTUDir)
    : CI(CI), CTUDir(CTUDir) {}
...
CrossTranslationUnitContext::ASTFileLoader::load(StringRef Identifier) {
    ...
    return ASTUnit::LoadFromASTFile(
            std::string(AbsPath.str()), CI.getPCHContainerOperations()->getRawReader(),
...
CrossTranslationUnitContext::ASTUnitStorage::ASTUnitStorage(CompilerInstance &CI)
    : LoadGuard(CI.getAnalyzerOpts()->CTUImportThreshold) {

    AnalyzerOptionsRef Opts = CI.getAnalyzerOpts();
    if (Opts->CTUOnDemandParsing)
        Loader = std::make_unique<ASTOnDemandLoader>(CI, Opts->CTUOnDemandParsingDatabase);
    else
...
llvm::Error CrossTranslationUnitContext::ASTOnDemandLoader::lazyInitCompileCommands(){
...
CrossTranslationUnitContext::ASTOnDemandLoader::ASTOnDemandLoader(CompilerInstance &CI,
                                                                  StringRef OnDemandParsingDatabase): CI(CI), OnDemandParsingDatabase(OnDemandParsingDatabase) {}
