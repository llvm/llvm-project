  ...
  if (VD->hasLocalStorage()) {
    ...
    if (Info.CurrentCall->Callee &&
      ...
      if (auto *PVD = dyn_cast<ParmVarDecl>(VD)) {
        VD = Frame->Arguments.getOrigParam(PVD);
        Version = Frame->Arguments.Version;
        Frame = Info.getCallFrameAndDepth(Frame->Arguments.CallIndex).first;
      } else {
