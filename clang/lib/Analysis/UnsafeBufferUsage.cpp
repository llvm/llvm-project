...
static bool isSafeStringViewTwoParamConstruct(const CXXConstructExpr &Node,
  ...
  if (const auto *SL = dyn_cast<StringLiteral>(Arg0)) {
    if (auto ArgSize = Arg1->getIntegerConstantExpr(Ctx)) {
      if (llvm::APSInt::compareValues(llvm::APSInt::getUnsigned(SL->getLength()),
                                      *ArgSize) >= 0)
        return true;
  ...
  // Pattern 4: string_view(it, it) - Only safe if it's .begin() and .end() of the SAME object
  ...
  if (Obj0 && Obj1) {
    ...
    // If both are references to variables, they MUST point to the same declaration.
    if (DRE0 && DRE1) {
      if (DRE0->getDecl()->getCanonicalDecl() == DRE1->getDecl()->getCanonicalDecl())
        return true;
    }
