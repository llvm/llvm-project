...
mlir::Value CIRToLLVMAtomicFetchOpLowering::buildMinMaxPostOp(
  ...
  if (isInt) {
    ...
    if (op.getBinop() == cir::AtomicFetchKind::Max)
      pred = mlir::LLVM::FCmpPredicate::ugt;
    else
      pred = mlir::LLVM::FCmpPredicate::ult;
    cmp = mlir::LLVM::FCmpOp::create(
        rewriter, loc,
        mlir::LLVM::FCmpPredicateAttr::get(rewriter.getContext(), pred), rmwVal,
        adaptor.getVal());
  }
