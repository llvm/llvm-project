...
RValue AtomicInfo::convertAtomicTempToRValue(Address addr, SourceLocation loc,
  ...
  if (lvalue.isSimple()) {
    if (evaluationKind == TEK_Aggregate) {
      cgf.cgm.errorNYI(
          loc, "AtomicInfo::convertAtomicTempToRValue: aggregate lvalue");
      return RValue::get(nullptr);
    }

    // Drill into the padding structure if we have one.
    if (hasPadding()) {
      cgf.cgm.errorNYI(loc, "AtomicInfo::convertAtomicTempToRValue: padding");
      return RValue::get(nullptr);
    }
  ...
  cgf.cgm.errorNYI(loc,
                   "AtomicInfo::convertAtomicTempToRValue: non-simple lvalue");
  return RValue::get(nullptr);
}
...
RValue AtomicInfo::convertToValueOrAtomic(mlir::Value intVal,
  ...
  if (asValue && getEvaluationKind() == TEK_Aggregate) {
    cgf.cgm.errorNYI(loc, "convertToValueOrAtomic: aggregate result");
    return RValue::get(nullptr);
  } else {
