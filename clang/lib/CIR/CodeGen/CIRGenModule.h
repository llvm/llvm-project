...
public:
  ...
  /// If \p field is an empty field that isn't laid out in the CIR record
  /// (e.g. a [[no_unique_address]] empty member), returns its concrete byte
  /// offset within \p destClass. Such fields have no CIR field index, so a
  /// pointer-to-data-member to them is represented by an explicit offset
  /// rather than a field-index path.  Returns std::nullopt otherwise.
  std::optional<uint64_t>
  getEmptyFieldMemberPointerOffset(const CXXRecordDecl *destClass,
