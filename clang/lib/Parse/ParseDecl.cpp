...
void Parser::ParseStructDeclaration(
  ...
  // declarator in this declaration, so remember where this declaration's entries
  // start before the specifier list is parsed. Indices, not iterators: the side
  // list is a SmallVector and only ever grows within a record body.
...
bool Parser::ProcessLateParsedTypeAttrCallback(LateParsedAttribute *LA,
  ...
  // late-attribute list ConvertDeclSpecToType walks once per declarator, so this
  // callback runs N times for `IP __counted_by(n) a, b;`. Building a fresh
  // (deliberately un-uniqued) node each time would leave every node but the last
  // orphaned with a null count, so reuse the node instead. This matches the
  // eager path, where getCountAttributedType uniques on the count expression and
  // all declarators likewise share one node.
