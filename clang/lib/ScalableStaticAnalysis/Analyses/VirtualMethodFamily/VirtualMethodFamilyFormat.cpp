...
static Object serializeVirtualMethodFamilyAnalysisResult(
    ...
    JSONFormat::EntityIdToJSONFn IdToJSON) {
  Array FamilyDataArr;
  for (const auto &[ParamId, FamilyId] : R.RetAndParamData) {
    Object Item;
    Item[KeyParamId] = IdToJSON(ParamId);
    Item[KeyFamilyId] = IdToJSON(FamilyId);
...
deserializeVirtualMethodFamilyAnalysisResult(
  ...
  for (const auto &V : *FamilyDataArr) {
    ...
    if (!Item)
      return makeSawButExpectedError(V, "an object {pid, fid}");
    const Object *ParamObj = Item->getObject(KeyParamId);
    const Object *FamilyObj = Item->getObject(KeyFamilyId);
    if (!ParamObj || !FamilyObj)
      return makeSawButExpectedError(*Item,
                                     "an object with fields {'%s', '%s'}",
                                     KeyParamId.data(), KeyFamilyId.data());
    ...
    Result->RetAndParamData.insert({*ParamId, *FamilyId});
  }
  return std::move(Result);
}
