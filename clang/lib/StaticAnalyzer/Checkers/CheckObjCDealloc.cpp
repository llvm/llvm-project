...
void ObjCDeallocChecker::diagnoseMissingReleases(CheckerContext &C) const {
  ...
  for (auto *IvarSymbol : *OldUnreleased) {
    ...
    auto BR = llvm::make_unique<PathSensitiveBugReport>(*MissingReleaseBugType,
                                                       OS.str(), ErrNode);
...
bool ObjCDeallocChecker::diagnoseExtraRelease(SymbolRef ReleasedValue,
  ...
  auto BR = llvm::make_unique<PathSensitiveBugReport>(*ExtraReleaseBugType,
                                                     OS.str(), ErrNode);
...
bool ObjCDeallocChecker::diagnoseMistakenDealloc(SymbolRef DeallocedValue,
  ...
  auto BR = llvm::make_unique<PathSensitiveBugReport>(*MistakenDeallocBugType,
                                                     OS.str(), ErrNode);
  BR->addRange(M.getOriginExpr()->getSourceRange());
