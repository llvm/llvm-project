...
static void addPGOAndCoverageFlags(const ToolChain &TC, Compilation &C,
  ...
  if (Args.hasFlag(options::OPT_fmcdc_coverage, options::OPT_fno_mcdc_coverage,
    ...
    if (!Args.hasFlag(options::OPT_fcoverage_mapping,
                      options::OPT_fno_coverage_mapping, false))
      D.Diag(clang::diag::err_drv_argument_only_allowed_with)
          << "-fcoverage-mcdc" << "-fcoverage-mapping";
  ...
  if (Args.hasArg(options::OPT_fprofile_exclude_files_EQ)) {
    ...
    if (!Args.hasArg(options::OPT_coverage))
      D.Diag(clang::diag::err_drv_argument_only_allowed_with)
          << "-fprofile-exclude-files=" << "--coverage";
  ...
  if (Args.hasArg(options::OPT_fprofile_filter_files_EQ)) {
    ...
    if (!Args.hasArg(options::OPT_coverage))
      D.Diag(clang::diag::err_drv_argument_only_allowed_with)
          << "-fprofile-filter-files=" << "--coverage";
  ...
  if (Args.hasFlag(options::OPT_fjmc, options::OPT_fno_jmc, false)) {
    if (TC.getTriple().isOSBinFormatELF() ||
      ...
      if (DebugInfoKind >= llvm::codegenoptions::DebugInfoConstructor)
        ...
        D.Diag(clang::diag::warn_drv_jmc_requires_debuginfo) << "-fjmc" << "-g";
    } else {
...
void Clang::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  if (LastPICDataRelArg) {
    if (LastPICDataRelArg->getOption().matches(
      ...
      if (!PICLevel)
        D.Diag(diag::err_drv_argument_only_allowed_with)
            << "-mno-pic-data-is-text-relative" << "-fpic/-fpie";
    }
    if (!Triple.isSystemZ())
      D.Diag(diag::err_drv_unsupported_opt_for_target)
  ...
  if (VirtualFunctionElimination) {
    ...
    if (LTOMode != LTOK_Full)
      D.Diag(diag::err_drv_argument_only_allowed_with)
          << "-fvirtual-function-elimination" << "-flto=full";
  ...
  if (VirtualFunctionElimination && !WholeProgramVTables) {
    D.Diag(diag::err_drv_argument_not_allowed_with)
        << "-fno-whole-program-vtables" << "-fvirtual-function-elimination";
  }
  ...
  if (SanitizeArgs.needsLTO() && !SplitLTOUnit)
    D.Diag(diag::err_drv_argument_not_allowed_with)
        << "-fno-split-lto-unit" << "-fsanitize=cfi";
  if (SplitLTOUnit)
...
void Clang::AddClangCLArgs(const ArgList &Args, types::ID InputType,
  ...
  if (Args.hasArg(options::OPT__SLASH_kernel)) {
    ...
    if (Args.hasFlag(options::OPT__SLASH_GR, options::OPT__SLASH_GR_, false))
      D.Diag(diag::err_drv_argument_not_allowed_with) << "/GR" << "/kernel";
  }
