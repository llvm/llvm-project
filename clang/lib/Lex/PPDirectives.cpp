...
Preprocessor::ImportAction Preprocessor::HandleHeaderIncludeOrImport(
  ...
  if (P2978::managerCompiler) {
    const auto &Result = P2978::managerCompiler->findResponse(
        Filename, IsImportDecl ? P2978::FileType::HEADER_UNIT
                               : P2978::FileType::HEADER_FILE);
    if (!Result) {
      Diag(FilenameLoc, getDiagnostics().getCustomDiagID(
    ...
    if (Result->type == P2978::FileType::HEADER_UNIT) {
      ...
      IsImportDecl = true;
      SuggestedModule = {HeaderUnit, ModuleMap::NormalHeader};
    }
  } else {
  ...
  // or when useIPC is set as header-unit is already loaded.
  ...
  const bool CheckIncludePathPortability =
      !(P2978::managerCompiler && IsImportDecl) && !IsMapped &&
