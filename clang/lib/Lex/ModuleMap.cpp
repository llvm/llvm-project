...
public:
  ...
  bool loadModuleMapFile(const modulemap::ModuleMapFile &MMF);
};
...
bool ModuleMapLoader::loadModuleMapFile(const modulemap::ModuleMapFile &MMF) {
  for (const auto &Decl : MMF.Decls) {
    std::visit(
        llvm::makeVisitor(
            [&](const modulemap::ModuleDecl &MD) { handleModuleDecl(MD); },
...
bool ModuleMap::loadModuleMapFile(FileEntryRef File, bool IsSystem,
  ...
  if (MMF) {
    ...
    Result = Loader.loadModuleMapFile(*MMF);
  }
