...
BuiltinTypeMethodBuilder &BuiltinTypeMethodBuilder::concat(V Vec, S Scalar,
                                                           QualType ResultTy) {
  if constexpr (std::is_same_v<V, PlaceHolder>) {
    assert(Vec != PlaceHolder::LastStmt &&
           "concat does not support LastStmt for vector operand");
  }
  ASTContext &AST = DeclBuilder.SemaRef.getASTContext();
...
BuiltinTypeDeclBuilder &
BuiltinTypeDeclBuilder::addNestedRecord(StringRef Name,
  ...
  NestedRecord->setAccess(AccessSpecifier::AS_public);
...
CXXRecordDecl *BuiltinTypeDeclBuilder::addMipsSliceType(ResourceDimension Dim,
  ...
  addNestedRecord("mips_slice_type", MipSliceRecord);
...
CXXRecordDecl *BuiltinTypeDeclBuilder::addMipsType(ResourceDimension Dim,
  ...
  addNestedRecord("mips_type", MipRecord);
