...
public:
  ...
  NestedNameSpecifier *transform(NestedNameSpecifier *NNS, bool &OutChanged) {
    ...
    case NestedNameSpecifier::TypeSpec: {
      ...
      if (!Changed)
        return NNS;
      OutChanged = true;
      return NestedNameSpecifier::Create(SemaRef.Context, Prefix, T);
    }
    }
    llvm_unreachable("Unknown NestedNameSpecifier Kind");
  }
  ...
  TemplateName transform(TemplateName TN, bool &OutChanged) {
    ...
    case TemplateName::DependentTemplate: {
      ...
      if (!Changed)
        return TN;
      return build(SemaRef.Context.getDependentTemplateName(
          {NNS, DTN->getName(), DTN->hasTemplateKeyword()}));
    }
    ...
    case TemplateName::SubstTemplateTemplateParm: {
      ...
      TemplateName RTN;
      if (STN->getFinal()) {
        RTN = transform(STN->getReplacement(), Changed);
        if (!Changed)
          return TN;
      } else {
        ...
        if (!Arg)
          return TN;
        RTN = Arg->getAsTemplate();
      }
      return build(SemaRef.Context.getSubstTemplateTemplateParm(
          RTN, STN->getAssociatedDecl(), STN->getIndex(), STN->getPackIndex(),
          /*Final=*/true));
    }
  ...
  QualType transform(QualType TT, bool &OutChanged) {
    ...
    case Type::DependentTemplateSpecialization: {
      ...
      NestedNameSpecifier *NNS = transform(S.getQualifier(), Changed);
      if (!Changed)
        return TT;
      return build(SemaRef.Context.getDependentTemplateSpecializationType(
          T->getKeyword(), {NNS, S.getName(), S.hasTemplateKeyword()},
          SpecArgs));
    }
    ...
    case Type::SubstTemplateTypeParm: {
      ...
      QualType RT;
      if (T->getFinal()) {
        RT = transform(T->getReplacementType(), Changed);
        if (!Changed)
          return TT;
      } else {
        const TemplateArgument *Arg = Names->getArgument(
            T->getAssociatedDecl(), T->getIndex(), T->getPackIndex());
        if (!Arg)
          return TT;
        RT = Arg->getAsType();
      }
      return build(SemaRef.Context.getSubstTemplateTypeParmType(
          RT, T->getAssociatedDecl(), T->getIndex(), T->getPackIndex(),
          /*Final=*/true));
    }
