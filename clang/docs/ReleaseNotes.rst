...
- HIPSPV toolchain: `--offload=spirv{32,64}` option is deprecated and
  will be removed when the new offload driver becomes default. The
  replacement for the option is
  `--offload=spirv{32,64}-unknown-chipstar` when using the new offload
  driver (`--offload-new-driver`).
