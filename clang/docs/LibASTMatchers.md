...
in the AST of a translation unit, you can call [recordDecl()](LibASTMatchersReference.html#recordDecl0Anchor). To narrow the match down,
for example to find all class or union declarations with the name "`Foo`",
insert a [hasName](LibASTMatchersReference.html#hasName0Anchor) matcher: the
...
accept multiple inner matchers use an implicit [allOf()](LibASTMatchersReference.html#allOf0Anchor). This allows further narrowing
...
2. Look at the [AST Matcher Reference](LibASTMatchersReference.html) for
...
retrieved in the [match callback](https://clang.llvm.org/doxygen/classclang_1_1ast__matchers_1_1MatchFinder_1_1MatchCallback.html).
...
values and different parameter counts. See [ASTMatchersMacros.h](https://clang.llvm.org/doxygen/ASTMatchersMacros_8h.html).
