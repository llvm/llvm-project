...
- Forward-declare `int __llvm_profile_check_compatibility(const char
  *ProfileData, uint64_t ProfileSize)` and call it to verify that the raw
  profile in `ProfileData` (of size `ProfileSize` bytes) was generated by the
  same binary and structurally matches the in-process counters and bitmaps. This
  function returns 0 on success, and a non-zero value otherwise.
- Forward-declare `int __llvm_profile_merge_from_buffer(const char *ProfileData,
  uint64_t ProfileSize)` and call it to merge the raw profile in `ProfileData`
  into the in-process counters and bitmaps. **The caller is expected to have
  verified compatibility beforehand.** This function returns 0 on success, and a
  non-zero value if the profile data is invalid or corrupted.
