...
int pre0(int pre_a, int b_post) {
  ...
  // CHECK: MacroExpansion,File 0, [[@LINE+1]]:11 -> [[@LINE+1]]:14 = #0 (Expanded file = 1)
  return (PRE(a)
          && b_post);
...
int pre1(int pre_a, int b_post) {
  ...
  // CHECK: MacroExpansion,File 0, [[@LINE+2]]:11 -> [[@LINE+2]]:14 = #0 (Expanded file = 1)
...
int post0(int pre_a, int b_post) {
  ...
  return (pre_a
          || POST(b));
  // CHECK: MacroExpansion,File 0, [[@LINE-1]]:14 -> [[@LINE-1]]:18 = #1 (Expanded file = 1)
...
int post1(int pre_a, int b_post) {
  ...
  // CHECK: MacroExpansion,File 0, [[@LINE+2]]:14 -> [[@LINE+2]]:18 = 0 (Expanded file = 1)
  return (pre_a
          || POST(bar));
