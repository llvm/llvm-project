// RUN: %clang_cc1 -cl-std=CL2.0 -O0 -emit-llvm -o - -triple amdgpu < %s | FileCheck %s --check-prefixes=COMMON,AMDGPU
...
// RUN: %clang_cc1 -cl-std=CL2.0 -O0 -debug-info-kind=limited -gno-column-info -emit-llvm -o - -triple amdgpu < %s | FileCheck %s --check-prefixes=CHECK-DEBUG
