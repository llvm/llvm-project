// This file tests the coro_inplace_task attribute semantics. 
...
Task<int> elidable() {
  ...
  // CHECK: call void @_Z6calleev(ptr dead_on_unwind writable sret(%struct.Task) align 8 %[[TASK_OBJ]]), !annotation ![[MUST_ELIDE:.+]]
...
Task<int> nonelidable() {
  ...
  // CHECK: call void @_Z6calleev(ptr dead_on_unwind writable sret(%struct.Task) align 8
  // CHECK-NOT: !annotation ![[MUST_ELIDE]]
  co_await t;
  co_await std::move(t);
...
// CHECK: ![[MUST_ELIDE]] = !{!"coro_must_elide"}
