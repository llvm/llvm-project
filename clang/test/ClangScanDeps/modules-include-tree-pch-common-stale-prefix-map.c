...
// Clean: scan the PCH.
// RUN: clang-scan-deps -format experimental-include-tree-full -cas-path %t/cas -o %t/deps_pch_clean.json -prefix-map=%t=/^tmp -- \
...
// Clean: scan the TU.
// RUN: clang-scan-deps -format experimental-include-tree-full -cas-path %t/cas -o %t/deps_tu_clean.json -prefix-map=%t=/^tmp -- \
...
// RUN: echo "#define MOD_COMMON_MACRO_NEW 1" >> %t/mod_common.h

// Incremental: scan the PCH.
// RUN: clang-scan-deps -format experimental-include-tree-full -cas-path %t/cas -o %t/deps_pch_incremental.json -prefix-map=%t=/^tmp -- \
// RUN:     %clang -x c-header %t/prefix.h -o %t/prefix.h.pch -F %t \
// RUN:     -fmodules -fimplicit-module-maps -fmodules-cache-path=%t/cache
...
// RUN: clang-scan-deps -format experimental-include-tree-full -cas-path %t/cas -o %t/deps_tu_incremental.json -prefix-map=%t=/^tmp -- \
// RUN:     %clang -c %t/tu.c -o %t/tu.o -include %t/prefix.h -F %t \
// RUN:     -fmodules -fimplicit-module-maps -fmodules-cache-path=%t/cache
