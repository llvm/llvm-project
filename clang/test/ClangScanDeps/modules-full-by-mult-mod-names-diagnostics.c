...
//--- module.modulemap
module root{header "root.h"} module root2{header "root2.h"} //--- root.h
    ...
    [{
      "file" : "",
      "directory" : "DIR",
      "command" : "clang -nostdinc -fmodules -fmodules-cache-path=DIR/cache -I "
                  "DIR -x c"
    }]
...
// RUN:   %t/error.txt > %t/result.json
    ...
    [{
      "file" : "",
      "directory" : "DIR",
      "command" : "clang -cc1 -nostdsysteminc -nobuiltininc -fmodules "
                  "-fimplicit-module-maps -fmodules-cache-path=DIR/cache -I "
                  "DIR -x c"
    }]
...
// RUN:   %t/error.cc1.txt > %t/result.cc1.json
    ...
    // RUN: cat %t/result.cc1.json | sed 's:\\\\\?:/:g' | FileCheck -DPREFIX=%/t
    // %s

    // ERROR: Diagnostics while scanning dependencies for
    // 'modA,root,modB,modC,root2': ERROR-NEXT: module-include.input:1:1: fatal
    // error: module 'modA' not found ERROR-NEXT: module-include.input:1:3:
    // fatal error: module 'modB' not found ERROR-NEXT:
    // module-include.input:1:4: fatal error: module 'modC' not found CHECK: {
    // CHECK-NEXT:   "modules": [
    // CHECK-NEXT:     {
    // CHECK:            "file-deps": [
