...
// RUN: not %clang -x c++ -std=c++11 -include %t.h -fsyntax-only -Werror %s 2> %t.cpp11log
// RUN: FileCheck %s --check-prefix=CHECK-OPT-DIFF < %t.cpp11log

// RUN: not %clang -include %t.h -fsyntax-only -Werror %s 2> %t.missinglog2
...
// CHECK-OPT-DIFF: error: {{.*}} was disabled in AST file{{.*}} but is currently enabled
