...
void column_major_load(float *p1) {
  ...
  // expected-error@-1 {{matrix column major load is disabled. Pass -fmatrix-memory-layout=column-major to enable it}}
}

void column_major_store(sx5x10_t *m1, float *p1) {
  __builtin_matrix_column_major_store(*m1, p1, 1);
  // expected-error@-1 {{matrix column major store is disabled. Pass -fmatrix-memory-layout=column-major to enable it}}
}
