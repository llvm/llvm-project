...
// -verify=ignored -fsyntax-only %s RUN: %clang_cc1 -triple amdgpu
// -verify=ignored -fsyntax-only %s RUN: %clang_cc1 -triple r600 -verify=ignored
// -fsyntax-only %s RUN: %clang_cc1 -triple spirv-linux-vulkan-library
// -verify=ignored -fsyntax-only %s RUN: %clang_cc1 -triple
// spirv32-unknown-unknown -verify=ignored -fsyntax-only %s RUN: %clang_cc1
// -triple spirv64-unknown-unknown -verify=ignored -fsyntax-only %s

// RUN: %clang_cc1 -triple x86_64 -aux-triple nvptx64 -x cuda -verify=x86_64
// -fsyntax-only %s RUN: %clang_cc1 -triple nvptx64 -aux-triple x86_64 -x cuda
// -fcuda-is-device -verify=nvptx64-x86_64 -fsyntax-only %s RUN: %clang_cc1
// -triple aarch64 -aux-triple nvptx64 -x cuda -verify=unsupported -fsyntax-only
// %s RUN: %clang_cc1 -triple nvptx64 -aux-triple aarch64 -x cuda
// -fcuda-is-device -verify=nvptx64-unsupported -fsyntax-only %s

#if (defined(__loongarch__) || defined(__x86_64__)) && !__has_attribute(model)
#error "Should support model attribute"
#endif

int a __attribute((model(
    "tiny"))); // unsupported-warning {{unknown attribute 'model' ignored}} \
...
int b __attribute((model(
    "small"))); // unsupported-warning {{unknown attribute 'model' ignored}} \
                                       // loongarch64-error {{code model 'small' is not supported on this target}} \
                                       // nvptx64-unsupported-warning {{unknown attribute 'model' ignored}}
int c __attribute((model(
    "normal"))); // unsupported-warning {{unknown attribute 'model' ignored}} \
...
int d __attribute((model(
    "kernel"))); // unsupported-warning {{unknown attribute 'model' ignored}} \
...
int e __attribute((model(
    "medium"))); // unsupported-warning {{unknown attribute 'model' ignored}} \
...
int f __attribute((model(
    "large"))); // unsupported-warning {{unknown attribute 'model' ignored}} \
                                       // loongarch64-error {{code model 'large' is not supported on this target}} \
                                       // nvptx64-unsupported-warning {{unknown attribute 'model' ignored}}
int g __attribute((model(
    "extreme"))); // unsupported-warning {{unknown attribute 'model' ignored}} \
...
void __attribute((model("extreme"))) h() {
} // unsupported-warning {{unknown attribute 'model' ignored}} \
