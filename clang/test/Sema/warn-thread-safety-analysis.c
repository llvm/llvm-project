// RUN: %clang_cc1 -fsyntax-only -verify -Wthread-safety -Wthread-safety-pointer
// -Wthread-safety-beta %s RUN: %clang_cc1 -fsyntax-only -verify -Wthread-safety
...
struct Foo {
  ...
  struct Bar {
    struct Mutex *other_mu ACQUIRED_AFTER(
        mu_); // Note: referencing the parent structure is convenient here, but
              // this should probably be disallowed if the child structure is
              // re-used outside of the parent.
...
struct A {
  struct Lock lock;
  union {
    int b __attribute__((guarded_by(
        lock))); // Note: referencing the parent structure is convenient here,
                 // but this should probably be disallowed if the child is
                 // re-used outside of the parent.
  };
};
...
void mutex_shared_unlock(struct Mutex *mu)
    __attribute__((release_shared_capability(mu)));
void mutex_exclusive_unlock(struct Mutex *mu)
...
int Foo_func3(int i) LOCKS_EXCLUDED(mu1, mu2) { return i; }

static int Bar_fun1(int i) EXCLUSIVE_LOCKS_REQUIRED(mu1) { return i; }

void set_value(int *a, int value) EXCLUSIVE_LOCKS_REQUIRED(foo_.mu_) {
  *a = value;
}

int get_value(int *p) SHARED_LOCKS_REQUIRED(foo_.mu_) { return *p; }
...
struct LateParsing {
  ...
  struct Mutex *a_mutex_defined_early ACQUIRED_BEFORE(a_mutex_defined_late);
...
int main(void) {
  ...
  Foo_fun1(
      1); // expected-warning{{calling function 'Foo_fun1' requires holding mutex 'mu2'}} \
  ...
  mutex_shared_lock(
      &mu1); // expected-warning{{acquiring mutex 'mu1' that is already held}} \
  ...
  mutex_exclusive_lock(&mu1);
  Foo_func3(4); // expected-warning{{cannot call function 'Foo_func3' while
                // mutex 'mu1' is held}}
  mutex_unlock(&mu1);
  ...
  set_value(
      &a_,
      0); // expected-warning{{calling function 'set_value' requires holding
          // mutex 'foo_.mu_' exclusively}} expected-warning@-1{{passing pointer
          // to variable 'a_' requires holding mutex 'foo_.mu_'}}
  get_value(b_); // expected-warning{{calling function 'get_value' requires
                 // holding mutex 'foo_.mu_'}} expected-warning@-1{{passing
                 // pointer 'b_' requires holding mutex 'foo_.mu_'}}
  ...
  *b_ = 0; // expected-warning{{writing the value pointed to by 'b_' requires
           // holding mutex 'foo_.mu_' exclusively}}
  __WRITE_ONCE(*b_, 0); // expected-warning{{writing the value pointed to by
                        // 'b_' requires holding mutex 'foo_.mu_' exclusively}}
  (void)(*b_ == 0); // expected-warning{{reading the value pointed to by 'b_'
                    // requires holding mutex 'foo_.mu_'}}
  (void)(__READ_ONCE(*b_) ==
         0); // expected-warning{{reading the value pointed to by 'b_' requires
             // holding mutex 'foo_.mu_'}}
  c_ = 0; // expected-warning{{writing variable 'c_' requires holding any mutex
          // exclusively}}
  ...
  *b_ = 0;
  ...
  mutex_exclusive_lock(&mu1); // expected-note {{mutex acquired here}}
  mutex_shared_unlock(&mu1);  // expected-warning {{releasing mutex 'mu1' using
                              // shared access, expected exclusive access}}
                              // expected-note@-1{{mutex released here}}
  mutex_exclusive_unlock(
      &mu1); // expected-warning {{releasing mutex 'mu1' that was not held}}

  mutex_shared_lock(&mu1); // expected-note {{mutex acquired here}}
  mutex_exclusive_unlock(
      &mu1); // expected-warning {{releasing mutex 'mu1' using exclusive access,
             // expected shared access}} expected-note@-1{{mutex released here}}
  mutex_shared_unlock(
      &mu1); // expected-warning {{releasing mutex 'mu1' that was not held}}
  ...
  }
  {
    ...
    mutex_exclusive_lock(
        &mu1); // With basic alias analysis lock through mu1 also works.
  }
  {
    int i = 0;
    mutex_exclusive_lock(&mu1);
  ...
  for (int i = (mutex_exclusive_lock(foo_.mu_), 0),
           *priv __attribute__((cleanup(unlock_scope_type_erased))) =
               (int *)(__UINTPTR_TYPE__)(foo_.mu_);
       !i; i++) {
    a_ = 42;
  }
  ...
  foo_.a_value = 0; // expected-warning {{writing variable 'a_value' requires
                    // holding mutex 'foo_.mu_' exclusively}}
  *foo_.a_ptr = 1; // expected-warning {{writing the value pointed to by 'a_ptr'
                   // requires holding mutex 'foo_.bar.other_mu' exclusively}}
  foo_.b_value = 0; // expected-warning {{writing variable 'b_value' requires
                    // holding mutex 'foo_.task->mu' exclusively}}
  *foo_.b_ptr = 1; // expected-warning {{writing the value pointed to by 'b_ptr'
                   // requires holding mutex 'foo_.task->mu' exclusively}}

  mutex_exclusive_lock(foo_.bar.other_mu);
  *foo_.a_ptr = 1;
  mutex_exclusive_lock(
      foo_.bar.third_mu); // expected-warning{{mutex 'third_mu' must be acquired
                          // before 'other_mu'}}
...
#ifdef LATE_PARSING
  late_parsing.a_value_defined_before =
      1; // expected-warning{{writing variable 'a_value_defined_before' requires
         // holding mutex 'late_parsing.a_mutex_defined_late' exclusively}}
  late_parsing.a_ptr_defined_before = 0;
  set_value(
      &late_parsing.a_value_defined_before,
      0); // expected-warning{{calling function 'set_value' requires holding
          // mutex 'foo_.mu_' exclusively}} expected-warning@-1{{passing pointer
          // to variable 'a_value_defined_before' requires holding mutex
          // 'late_parsing.a_mutex_defined_late'}}
  mutex_exclusive_lock(late_parsing.a_mutex_defined_late);
  mutex_exclusive_lock(
      late_parsing
          .a_mutex_defined_early); // expected-warning{{mutex
                                   // 'a_mutex_defined_early' must be acquired
                                   // before 'a_mutex_defined_late'}}
  mutex_exclusive_unlock(late_parsing.a_mutex_defined_early);
  mutex_exclusive_unlock(late_parsing.a_mutex_defined_late);
  mutex_exclusive_lock(late_parsing.a_mutex_defined_late);
  mutex_exclusive_lock(
      late_parsing.a_mutex_defined_very_late); // expected-warning{{mutex
                                               // 'a_mutex_defined_very_late'
...
void mutex_init(struct Mutex *mu)
    ASSERT_EXCLUSIVE_LOCK((struct ReentrantMutex *)mu);
...
void test_fp_ops_fail(struct FPOps *ops) {
  ops->a = 42; // expected-warning {{writing variable 'a' requires holding mutex
               // '&FPOps::mu' exclusively}}
...
void visit_all_unlocked_fp(visit_fn visit EXCLUSIVE_LOCKS_REQUIRED(mu1),
                           int n) {
...
void test_fp_param(int n) {
  ...
  visit_all_locked_fp(
      visit_cb, n); // expected-warning {{calling function 'visit_all_locked_fp'
                    // requires holding mutex 'mu1' exclusively}}
}

// Acquire/release on a function pointer parameter likewise describe the
// pointee, so calling the enclosing function neither acquires nor releases mu1.
...
void test_bdev_ops_fail(struct BDevOps *ops, struct BDev *bdev) {
  ops->unlock(bdev); // expected-warning {{releasing mutex 'bdev->lock' that was
                     // not held}}
}
...
void test_trylock_conditional(void) {
  if (({
        int do_work = !!(!mutex_exclusive_trylock(&mu1));
...
void run(void) __attribute__((
    guarded_by(mu1),
    guarded_by(mu1))); // expected-warning 2{{only applies to non-static data
...
int value_with_no_open_brace
    __attribute__((guarded_by)); // expected-error{{'guarded_by' attribute takes
                                 // at least 1 argument}}
int *ptr_with_no_open_brace
    __attribute__((pt_guarded_by)); // expected-error{{'pt_guarded_by' attribute
                                    // takes at least 1 argument}}

int value_with_no_open_brace_on_acquire_after
    __attribute__((acquired_after)); // expected-error{{'acquired_after'
...
int value_with_bad_expr GUARDED_BY(
    bad_expr); // expected-error{{use of undeclared identifier 'bad_expr'}}
int *ptr_with_bad_expr PT_GUARDED_BY(
    bad_expr); // expected-error{{use of undeclared identifier 'bad_expr'}}

int value_with_bad_expr_on_acquire_after __attribute__((
    acquired_after(other_bad_expr))); //  expected-error{{use of undeclared
                                      //  identifier 'other_bad_expr'}}
int value_with_bad_expr_on_acquire_before __attribute__((
    acquired_before(other_bad_expr))); //  expected-error{{use of undeclared
                                       //  identifier 'other_bad_expr'}}
