...
// RUN: %clang_cc1 -std=c2x -fsyntax-only -Wpre-c2x-compat-pedantic -verify=c23,expected %s
...
restrict T2 t2;         // pedantic-warning {{'restrict' qualifier on an array of pointers is a C23 extension}} \
                        // c23-warning {{'restrict' qualifier on an array of pointers is incompatible with C standards before C23}}
