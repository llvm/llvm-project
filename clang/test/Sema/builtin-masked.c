...
void test_masked_load(v8i *pf, v8b mask, v2b mask2, v2b thru) {
  ...
  (void)__builtin_masked_load(mask2, pf); // expected-error {{all arguments must have the same number of elements}}
...
void test_masked_store(v8i *pf, v8f *pf2, v8b mask, v2b mask2) {
  ...
  __builtin_masked_store(mask2, *pf, pf); // expected-error {{all arguments must have the same number of elements}}
...
void test_masked_expand_load(v8i *pf, v8b mask, v2b mask2, v2b thru) {
  ...
  (void)__builtin_masked_expand_load(mask2, pf); // expected-error {{all arguments must have the same number of elements}}
...
void test_masked_compress_store(v8i *pf, v8f *pf2, v8b mask, v2b mask2) {
  ...
  __builtin_masked_compress_store(mask2, *pf, pf); // expected-error {{all arguments must have the same number of elements}}
