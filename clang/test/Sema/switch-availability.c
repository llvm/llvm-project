...
void testSwitchTwo(enum SwitchTwo st) {
  switch (st) {} // expected-warning{{enumeration values 'Ed', 'Vim', and 'Emacs' not handled in switch}} expected-note{{add missing switch cases}}
}
...
int testSwitchFour(enum SwitchFour e) {
  switch (e) { // expected-warning{{enumeration value 'Blue' not handled in switch}} expected-note{{add missing switch cases}}
  case Red:   return 1;
  case Green: return 2;
  }
