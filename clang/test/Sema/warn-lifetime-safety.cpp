...
MyObj* simple_return_stack_address() {
  MyObj s;      
  MyObj* p = &s; // expected-warning {{address of stack memory associated with local variable 's' returned}}
...
const MyObj* conditional_assign_unconditional_return(const MyObj& safe, bool c) {
  ...
  const MyObj* p = &safe;
  if (c) {
    p = &s;       // expected-warning {{address of stack memory associated with local variable 's' returned}}
  }     
  return p;      // expected-note {{returned here}}
}
...
View reassign_safe_to_local(const MyObj& safe) {
  MyObj local;
  View p = safe;
  p = local;    // expected-warning {{address of stack memory associated with local variable 'local' returned}}
  return p;     // expected-note {{returned here}}
}

View pointer_chain_to_local() {
  MyObj local;
  View p1 = local;     // expected-warning {{address of stack memory associated with local variable 'local' returned}}
  View p2 = p1; 
...
View multiple_assign_multiple_return(const MyObj& safe, bool c1, bool c2) {
  ...
  View p;
  if (c1) {
    p = local1;       // expected-warning {{address of stack memory associated with local variable 'local1' returned}}
    return p;         // expected-note {{returned here}}
  }
  else if (c2) {
    p = local2;       // expected-warning {{address of stack memory associated with local variable 'local2' returned}}
    return p;         // expected-note {{returned here}}
  }
  p = safe;
  return p;
...
View direct_return_of_local() {
  MyObj stack;      
  return stack;     // expected-warning {{address of stack memory associated with local variable 'stack' returned}}
                    // expected-note@-1 {{returned here}}
}

MyObj& reference_return_of_local() {
  MyObj stack;      
  return stack;     // expected-warning {{reference to stack memory associated with local variable 'stack' returned}}
                    // expected-note@-1 {{returned here}}
}
...
int* trivial_int_uar() {
  int *a;
  int b = 1;
  a = &b;          // expected-warning {{address of stack memory associated with local variable 'b' returned}}
...
TriviallyDestructedClass* trivial_class_uar () {
  TriviallyDestructedClass *ptr;
  TriviallyDestructedClass s;
  ptr = &s;       // expected-warning {{address of stack memory associated with local variable 's' returned}}
  return ptr;     // expected-note {{returned here}}
}

const int& return_parameter(int a) { 
  return a; // expected-warning {{reference to stack memory associated with parameter 'a' returned}}
            // expected-note@-1 {{returned here}}
}
...
const int& return_reference_to_parameter(int a) {
    const int &b = a;   // expected-warning {{reference to stack memory associated with parameter 'a' returned}}
...
MyObj*& return_ref_to_local_ptr_pointing_to_local() {
  MyObj local;
  MyObj* p = &local; // expected-warning {{reference to stack memory associated with local variable 'local' returned}}
  return p;          // expected-note {{returned here}} \
                     // expected-warning {{reference to stack memory associated with local variable 'p' returned}} \
                     // expected-note {{returned here}}
}

const int& reference_via_conditional(int a, int b, bool cond) {
    const int &c = (cond ? ((a)) : (b));  // expected-warning {{reference to stack memory associated with parameter 'b' returned}} expected-warning {{reference to stack memory associated with parameter 'a' returned}}
    return c;                             // expected-note 2 {{returned here}}
}
const int* return_pointer_to_parameter_via_reference(int a, int b, bool cond) {
    const int &c = cond ? a : b;  // expected-warning {{address of stack memory associated with parameter 'b' returned}} expected-warning {{address of stack memory associated with parameter 'a' returned}}
    const int* d = &c;
    return d;                     // expected-note 2 {{returned here}}
}

const int& return_pointer_to_parameter_via_reference_1(int a) {
    const int* d = &a; // expected-warning {{reference to stack memory associated with parameter 'a' returned}}
...
int* const& test_ref_to_ptr() {
  PtrHolder a;
  int *const &ref = a.getRef();  // expected-warning {{reference to stack memory associated with local variable 'a' returned}}
...
int** test_ternary_double_ptr(bool cond) {
  int a = 1, b = 2;
  int* pa = &a;  // expected-warning {{address of stack memory associated with local variable 'a' returned}}
  int* pb = &b;  // expected-warning {{address of stack memory associated with local variable 'b' returned}}
  int** result = cond ? &pa : &pb;  // expected-warning {{address of stack memory associated with local variable 'pb' returned}} expected-warning {{address of stack memory associated with local variable 'pa' returned}}
...
View uar_before_uaf(const MyObj& safe, bool c) {
  ...
  {
    MyObj local_obj; 
    p = local_obj;  // expected-warning {{address of stack memory associated with local variable 'local_obj' returned}}
    if (c) {
      return p;     // expected-note {{returned here}}
    }
  }
...
View lifetimebound_return_of_local() {
  MyObj stack;
  return Identity(stack); // expected-warning {{address of stack memory associated with local variable 'stack' returned}}
                          // expected-note@-1 {{returned here}}
}

const MyObj& lifetimebound_return_ref_to_local() {
  MyObj stack;
  return IdentityRef(stack); // expected-warning {{reference to stack memory associated with local variable 'stack' returned}}
                             // expected-note@-1 {{returned here}}
}

View lifetimebound_return_by_value_param(MyObj stack_param) {
  return Identity(stack_param); // expected-warning {{address of stack memory associated with parameter 'stack_param' returned}}
                                // expected-note@-1 {{returned here}}
}

View lifetimebound_return_by_value_multiple_param(int cond, MyObj a, MyObj b, MyObj c) {
  if (cond == 1) 
    return Identity(a); // expected-warning {{address of stack memory associated with parameter 'a' returned}}
                        // expected-note@-1 {{returned here}}
  if (cond == 2) 
    return Identity(b); // expected-warning {{address of stack memory associated with parameter 'b' returned}}
                        // expected-note@-1 {{returned here}}
  return Identity(c); // expected-warning {{address of stack memory associated with parameter 'c' returned}}
                      // expected-note@-1 {{returned here}}
}

template<class T>
View lifetimebound_return_by_value_param_template(T t) {
  return Identity(t); // expected-warning {{address of stack memory associated with parameter 't' returned}}
                      // expected-note@-1 {{returned here}}
}
...
View range_based_for_use_after_return() {
  MyObjStorage s;
  for (const MyObj &o : s) { // expected-warning {{address of stack memory associated with local variable 's' returned}}
    return o;  // expected-note {{returned here}}
  }
  return *s.begin();  // expected-warning {{address of stack memory associated with local variable 's' returned}}
                      // expected-note@-1 {{returned here}}
}
...
MyObj& test_user_defined_deref_uar() {
  MyObj obj;
  SmartPtr<MyObj> smart_ptr(&obj);
  return *smart_ptr;  // expected-warning {{reference to stack memory associated with local variable 'smart_ptr' returned}}
                      // expected-note@-1 {{returned here}}
}
...
const MyObj& call_max_with_obj() {
  MyObj oa, ob;
  return  MaxT(oa,    // expected-warning {{reference to stack memory associated with local variable 'oa' returned}}
                      // expected-note@-1 2 {{returned here}}
               ob);   // expected-warning {{reference to stack memory associated with local variable 'ob' returned}}
                    
}

MyObj* call_max_with_obj_error() {
  MyObj oa, ob;
  return  &MaxT(oa,   // expected-warning {{address of stack memory associated with local variable 'oa' returned}}
                      // expected-note@-1 2 {{returned here}}
                ob);  // expected-warning {{address of stack memory associated with local variable 'ob' returned}}
}

const MyObj* call_max_with_ref_obj_error() {
  MyObj oa, ob;
  const MyObj& refa = oa;     // expected-warning {{address of stack memory associated with local variable 'oa' returned}}
  const MyObj& refb = ob;     // expected-warning {{address of stack memory associated with local variable 'ob' returned}}
  return  &MaxT(refa, refb);  // expected-note 2 {{returned here}}
}
const MyObj& call_max_with_ref_obj_return_ref_error() {
  MyObj oa, ob;
  const MyObj& refa = oa;     // expected-warning {{reference to stack memory associated with local variable 'oa' returned}}
  const MyObj& refb = ob;     // expected-warning {{reference to stack memory associated with local variable 'ob' returned}}
  return  MaxT(refa, refb);   // expected-note 2 {{returned here}}
}
...
const View& call_max_with_view_with_error() {
  View va, vb;
  return MaxT(va,   // expected-warning {{reference to stack memory associated with local variable 'va' returned}}
                    // expected-note@-1 2 {{returned here}}
              vb);  // expected-warning {{reference to stack memory associated with local variable 'vb' returned}}
}
...
int** return_2p() {
  int a = 1;
  int* b = &a;  // expected-warning {{address of stack memory associated with local variable 'a' returned}}
  int** c = &b; // expected-warning {{address of stack memory associated with local variable 'b' returned}}
  return c;     // expected-note 2 {{returned here}}
}
...
int*** return_3p() {
  ...
  int* b = &a;    // expected-warning {{address of stack memory associated with local variable 'a' returned}}
  int** c = &b;   // expected-warning {{address of stack memory associated with local variable 'b' returned}}
  int*** d = &c;  // expected-warning {{address of stack memory associated with local variable 'c' returned}}
  return d;       // expected-note 3 {{returned here}}
}

View** return_view_p() {
  MyObj a;
  View b = a;     // expected-warning {{address of stack memory associated with local variable 'a' returned}}
  View* c = &b;   // expected-warning {{address of stack memory associated with local variable 'b' returned}}
  View** d = &c;  // expected-warning {{address of stack memory associated with local variable 'c' returned}}
  return d;       // expected-note 3 {{returned here}}
}
...
const MyObj& testDeref(MyObj obj) {
  View v = obj; // expected-warning {{reference to stack memory associated with parameter 'obj' returned}}
  return *v;    // expected-note {{returned here}}
}
const MyObj* testDerefAddr(MyObj obj) {
  View v = obj; // expected-warning {{address of stack memory associated with parameter 'obj' returned}}
  return &*v;   // expected-note {{returned here}}
}
...
const MyObj& ContainerMyObjReturnRef(Container<MyObj> c) {
  for (const MyObj& x : c) {  // expected-warning {{reference to stack memory associated with parameter 'c' returned}}
...
View ContainerMyObjReturnView(Container<MyObj> c) {
  for (const MyObj& x : c) {  // expected-warning {{address of stack memory associated with parameter 'c' returned}}
    return x;                 // expected-note {{returned here}}
  }
  for (View x : c) {  // expected-warning {{address of stack memory associated with parameter 'c' returned}}
    return x;         // expected-note {{returned here}}
  }
  return Global;
}
...
std::string_view refMemberReturnView3(RefMember a) { return a.str; } // expected-warning {{address of stack memory associated with parameter 'a' returned}} expected-note {{returned here}}
std::string& refMemberReturnRef1(RefMember a) { return a.str_ref; }
std::string& refMemberReturnRef2(RefMember a) { return *a.str_ptr; }
std::string& refMemberReturnRef3(RefMember a) { return a.str; } // expected-warning {{reference to stack memory associated with parameter 'a' returned}} expected-note {{returned here}}
std::string_view refViewMemberReturnView1(RefMember a) { return a.view; }
std::string_view& refViewMemberReturnView2(RefMember a) { return a.view; } // expected-warning {{reference to stack memory associated with parameter 'a' returned}} expected-note {{returned here}}
...
View test1(std::string a) {
  ...
  return View(a); // expected-warning {{address of stack memory associated with parameter 'a' returned}} expected-note {{returned here}}
}

View test2(std::string a) {
  View b = View(a); // expected-warning {{address of stack memory associated with parameter 'a' returned}}
  return b;         // expected-note {{returned here}}
}

View test3(std::string a) {
  const View& b = View(a);  // expected-warning {{address of stack memory associated with parameter 'a' returned}}
  return b;                 // expected-note {{returned here}}
}
} // namespace non_trivial_views
...
auto return_ref_capture() {
  int local = 1;
  auto lambda = [&local]() { return local; }; // expected-warning {{address of stack memory associated with local variable 'local' returned}}
...
auto capture_pointer_by_ref() {
  MyObj obj;
  MyObj* p = &obj;
  auto lambda = [&p]() { return p; }; // expected-warning {{address of stack memory associated with local variable 'p' returned}}
  return lambda; // expected-note {{returned here}}
}
...
auto capture_multiple() {
  int a, b;
  auto lambda = [
    &a,  // expected-warning {{address of stack memory associated with local variable 'a' returned}}
    &b   // expected-warning {{address of stack memory associated with local variable 'b' returned}}
  ]() { return a + b; };
...
auto capture_raw_pointer_by_value() {
  int x;
  int* p = &x; // expected-warning {{address of stack memory associated with local variable 'x' returned}}
...
auto capture_lambda() {
  int x;
  auto inner = [&x]() { return x; }; // expected-warning {{address of stack memory associated with local variable 'x' returned}}
...
auto implicit_ref_capture() {
  int local = 1;
  auto lambda = [&]() { return local; }; // expected-warning {{address of stack memory associated with local variable 'local' returned}}
  return lambda; // expected-note {{returned here}}
}
...
auto implicit_ref_capture_multiple() {
  ...
  auto lambda = [&]() { return local + local2; }; // expected-warning {{address of stack memory associated with local variable 'local2' returned}} expected-warning {{address of stack memory associated with local variable 'local' returned}}
  return lambda; // expected-note 2 {{returned here}}
}
...
auto* pointer_to_lambda_outlives() {
  ...
  return &lambda; // expected-warning {{address of stack memory associated with local variable 'lambda' returned}} \
                  // expected-note {{returned here}}
}
...
auto capture_multilevel_pointer() {
  int x;
  int *p = &x; // expected-warning {{address of stack memory associated with local variable 'x' returned}}
  int **q = &p; // expected-warning {{address of stack memory associated with local variable 'p' returned}}
  int ***r = &q; // expected-warning {{address of stack memory associated with local variable 'q' returned}}
...
int* element_use_after_return() {
  int a[10]{};
  int* p = &a[0]; // expected-warning {{address of stack memory associated with local variable 'a' returned}}
  return p;       // expected-note {{returned here}}
}
...
int* return_decayed_array() {
  int a[10]{};
  int *p = a; // expected-warning {{address of stack memory associated with local variable 'a' returned}}
  return p;   // expected-note {{returned here}}
}
...
S copy_propagation() {
  std::string str{"abc"};
  S a = getS(str); // expected-warning {{address of stack memory associated with local variable 'str' returned}}
  S b = a;
  return b; // expected-note {{returned here}}
}
...
S multiple_lifetimebound_params() {
  std::string str{"abc"};
  S s = getS2(str, std::string("temp")); // expected-warning {{address of stack memory associated with local variable 'str' returned}} \
...
DefaultedOuter nested_defaulted_outer_with_user_defined_inner() {
  std::string str{"abc"};
  DefaultedOuter o = getDefaultedOuter(str); // expected-warning {{address of stack memory associated with local variable 'str' returned}}
...
std::span<std::string_view> owner_to_pointer_via_gsl_construction() {
  ...
  return views; // expected-warning {{address of stack memory associated with local variable 'views' returned}} \
                // expected-note {{returned here}}
}
...
std::string_view return_dangling_view_through_owner() {
  ...
  S* s = ups.get(); // expected-warning {{address of stack memory associated with local variable 'ups' returned}}
...
int *constexpr_live_false(int *num) {
  int local = 0;
  return kFalse ? num : f(&local); // expected-warning {{address of stack memory associated with local variable 'local' returned}} // expected-note {{returned here}}
}

int *constexpr_live_nested(int *num) {
  int local = 0;
  return kTrue ? (kFalse ? num : f(&local)) : num; // expected-warning {{address of stack memory associated with local variable 'local' returned}} // expected-note {{returned here}}
}
...
std::function<void()> direct_return() {
  int x;
  return [&x]() { (void)x; }; // expected-warning {{address of stack memory associated with local variable 'x' returned}} \
                              // expected-note {{returned here}}
}

std::function<void()> copy_function() {
  int x;
  std::function<void()> f = [&x]() { (void)x; }; // expected-warning {{address of stack memory associated with local variable 'x' returned}}
...
std::function<void()> reassign_safe_then_unsafe() {
  ...
  f = [&local]() { (void)local; }; // expected-warning {{address of stack memory associated with local variable 'local' returned}}
