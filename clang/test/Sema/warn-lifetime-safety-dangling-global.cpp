...
void inlined() {
  int local;
  global = &local; // expected-warning {{local variable 'local' escapes to global or static storage and will dangle}}
  global_backup = global; 
  global = nullptr;
}
...
void dangling_static_field() {
  int local;
  ObjWithStaticField::static_field = &local; // expected-warning {{local variable 'local' escapes to global or static storage and will dangle}}
}