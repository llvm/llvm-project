...
void inlined() {
  int local;
  global = &local; // expected-warning {{stack memory associated with local variable 'local' escapes to a global variable 'global_backup' which will dangle}}
  global_backup = global; 
  global = nullptr;
}
...
void store_local_in_global() {
  int local;
  global = &local; // expected-warning {{stack memory associated with local variable 'local' escapes to a global variable 'global' which will dangle}}
}
...
void dangling_static_field() {
  int local;
  ObjWithStaticField::static_field = &local; // expected-warning {{stack memory associated with local variable 'local' escapes to a static variable 'static_field' which will dangle}}
}