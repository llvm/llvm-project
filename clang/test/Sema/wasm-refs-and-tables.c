...
__externref_t func(__externref_t ref) {
  ...
  _Alignof(ref);                 // expected-error {{invalid application of 'alignof' to sizeless type '__externref __externref_t'}}
  ...
  table == 1;                     // expected-error {{invalid operands to binary expression ('__externref __externref_t[0]' and 'int')}}
  1 >= table;                     // expected-error {{invalid operands to binary expression ('int' and '__externref __externref_t[0]')}}
  table == other_table;           // expected-error {{invalid operands to binary expression ('__externref __externref_t[0]' and '__externref __externref_t[0]')}}
  table !=- table;                // expected-error {{invalid argument type '__externref __externref_t *' to unary expression}}
  !table;                         // expected-error {{invalid argument type '__externref __externref_t *' to unary expression}}
  1 && table;                     // expected-error {{invalid operands to binary expression ('int' and '__externref __externref_t[0]')}}
  table || 1;                     // expected-error {{invalid operands to binary expression ('__externref __externref_t[0]' and 'int')}}
