...
// expected-error@-1 {{expected a comma after 'strip' in '__ptrauth' options argument}}
int *__ptrauth(VALID_DATA_KEY, 1, 65535, "strip"
                                         " isa-pointer") invalid23;
// expected-error@-2 {{expected a comma after 'strip' in '__ptrauth' options argument}}
