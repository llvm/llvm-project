...
// rejected. https://github.com/llvm/llvm-project/issues/199319
...
_Static_assert(__builtin_offsetof(struct MyStruct, ptrs[(uint8_t)127]) ==
                   127 * sizeof(void *),
               "offsetof with uint8_t index 127 should be correct");

_Static_assert(__builtin_offsetof(struct MyStruct, ptrs[(uint8_t)128]) ==
                   128 * sizeof(void *),
               "offsetof with uint8_t index 128 should be correctly "
               "zero-extended, not sign-extended");

_Static_assert(__builtin_offsetof(struct MyStruct, ptrs[(uint8_t)255]) ==
                   255 * sizeof(void *),
...
_Static_assert(
    __builtin_offsetof(struct BigStruct, data[(uint16_t)32768]) == 32768,
...
_Static_assert(__builtin_offsetof(struct NegIdxStruct,
                                  x[(__uint128_t)0x8000000000000000]) == 0,
               ""); // expected-error {{not an integral constant expression}}
...
_Static_assert(__builtin_offsetof(struct ShortArray,
                                  data[(uint64_t)4611686018427387904ULL]) == 0,
               ""); // expected-error {{not an integral constant expression}}
                    // expected-note {{overflow in offsetof}}
