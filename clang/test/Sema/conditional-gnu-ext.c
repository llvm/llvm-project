...
void foo(void) {
  ...
  *((double *)0 ? : (void *)(int *)0) = 0;
  // expected-error@-1 {{incomplete type 'void' is not assignable}}
  // expected-warning@-2 {{ISO C does not allow indirection on operand of type 'void *'}}
  *((double *)0 ? : (void *)(double *)0) = 0;
  // expected-error@-1 {{incomplete type 'void' is not assignable}}
  // expected-warning@-2 {{ISO C does not allow indirection on operand of type 'void *'}}
  *((double *)0 ? : (int *)(void *)0) = 0;
  // expected-error@-1 {{incomplete type 'void' is not assignable}}
  // expected-warning@-2 {{pointer type mismatch ('double *' and 'int *')}}
  // expected-warning@-3 {{ISO C does not allow indirection on operand of type 'void *'}}
...
int f0(int a) {
  return f1() ? : nil;
  // expected-warning@-1 {{pointer/integer type mismatch in conditional expression ('int' and 'void *')}}
  // expected-error@-2 {{incompatible pointer to integer conversion returning 'void *' from a function with result type 'int'}}
}
