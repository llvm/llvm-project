...
// -Wno-constant-conversion -Wno-div-by-zero %s RUN: %clang_cc1 -std=c23 -verify
// -triple x86_64 -pedantic -Wno-conversion -Wno-constant-conversion
// -Wno-div-by-zero -fexperimental-new-constant-interpreter %s

// Check that constexpr only applies to variables.
constexpr void f0() {
} // expected-error {{'constexpr' can only be used in variable declarations}}
constexpr const int f1() {
  return 0;
} // expected-error {{'constexpr' can only be used in variable declarations}}
...
constexpr signed auto Long =
    1L; // expected-error {{illegal storage class on file-scoped variable}}
...
void f3(constexpr register int
            P1) { // expected-error {{function parameter cannot be constexpr}}
  constexpr register int V9 = 0;
  constexpr register auto V10 = 0.0;
}

constexpr thread_local int V11 =
    38; // expected-error {{cannot combine with previous '_Thread_local'
        // declaration specifier}}
constexpr static thread_local double V12 =
    38; // expected-error {{cannot combine with previous '_Thread_local'
        // declaration specifier}}
constexpr extern thread_local char
    V13; // expected-error {{cannot combine with previous '_Thread_local'
         // declaration specifier}}
// expected-error@-1 {{cannot combine with previous 'constexpr' declaration
// specifier}} expected-error@-2 {{constexpr variable declaration must be a
// definition}}
constexpr thread_local short V14 =
    38; // expected-error {{cannot combine with previous '_Thread_local'
        // declaration specifier}}
...
constexpr int *restrict V18 = 0; // expected-error {{constexpr variable cannot
                                 // have type 'int *const restrict'}}

constexpr extern char Oops =
    1; // expected-error {{cannot combine with previous 'constexpr' declaration specifier}} \
...
// expected-error@-1 {{constexpr variable cannot have type 'const TheA[3]' (aka
// 'const _Atomic(int)[3]')}}
constexpr TheV V20[3] = {};
// expected-error@-1 {{constexpr variable cannot have type 'const TheV[3]' (aka
// 'const volatile short[3]')}}
constexpr TheR V21[3] = {};
// expected-error@-1 {{constexpr variable cannot have type 'const TheR[3]' (aka
...
// expected-error@-1 {{constexpr variable cannot have type 'TheA' (aka
// '_Atomic(int)')}}
constexpr struct HasV V23[2] = {};
// expected-error@-1 {{constexpr variable cannot have type 'TheV' (aka 'volatile
// short')}}
constexpr struct HasR V24[2] = {};
// expected-error@-1 {{constexpr variable cannot have type 'TheR' (aka 'float
// *restrict')}}
...
// constant expression.
constexpr int V29;
// expected-error@-1 {{constexpr variable 'V29' must be initialized by a
// constant expression}}
...
// expected-error@-1 {{constexpr variable 'V30' must be initialized by a
// constant expression}}
...
// expected-error@-1 {{constexpr variable 'V32' must be initialized by a
// constant expression}}
...
// expected-error@-1 {{constexpr variable 'V37' must be initialized by a
// constant expression}}
constexpr int V38 = 3;
constexpr int V39 = V38 / V38;
constexpr int V40 = V38 / 2;
constexpr int V41 = V38 / 0;
// expected-error@-1 {{constexpr variable 'V41' must be initialized by a
// constant expression}} expected-note@-2 {{division by zero}}
constexpr int V42 = V38 & 0;

constexpr struct S5 V43 = {randomFoo()};
// expected-error@-1 {{constexpr variable 'V43' must be initialized by a
// constant expression}}
constexpr struct S5 V44 = {0};
constexpr struct S5 V45 = {V38 / 0};
// expected-error@-1 {{constexpr variable 'V45' must be initialized by a
// constant expression}} expected-note@-2 {{division by zero}}

constexpr float V46[3] = {randomFoo()};
// expected-error@-1 {{constexpr variable 'V46' must be initialized by a
// constant expression}}
constexpr struct S5 V47[3] = {randomFoo()};
// expected-error@-1 {{constexpr variable 'V47' must be initialized by a
// constant expression}}

const static int V48 = V38;
constexpr static int V49 = V48;
// expected-error@-1 {{constexpr variable 'V49' must be initialized by a
// constant expression}}

void f4(const int P1) {
  constexpr int V = P1;
  // expected-error@-1 {{constexpr variable 'V' must be initialized by a
  // constant expression}}

  constexpr int V1 = 12;
  constexpr const int *V2 = &V1;
  // expected-error@-1 {{constexpr variable 'V2' must be initialized by a
  // constant expression}}
}
...
// exactly representable in type 'unsigned char'}}

void f5() {
  constexpr char V50 = 300;
  // expected-error@-1 {{constexpr initializer evaluates to 300 which is not
  // exactly representable in type 'const char'}}
  constexpr float V51 = 1.0 / 3.0;
  // expected-error@-1 {{constexpr initializer evaluates to 3.333333e-01 which
  // is not exactly representable in type 'const float'}}
  constexpr float V52 = 0.7;
  // expected-error@-1 {{constexpr initializer evaluates to 7.000000e-01 which
  // is not exactly representable in type 'const float'}}
  constexpr float V53 = 1.0f / 3.0f;
  constexpr float V54 = 432000000000;
  // expected-error@-1 {{constexpr initializer evaluates to 432000000000 which
  // is not exactly representable in type 'const float'}}
  constexpr unsigned char V55[] = {
      "\xAF",
      // expected-error@-1 {{constexpr initializer evaluates to -81 which is not
      // exactly representable in type 'const unsigned char'}}
  };
  ...
  // expected-error@-1 {{constexpr initializer evaluates to 299 which is not
  // exactly representable in type 'unsigned char'}}
  ...
  // expected-error@-1 {{constexpr initializer evaluates to 2.941176e-01 which
  // is not exactly representable in type 'const float'}}
  ...
  // expected-error@-1 {{constexpr initializer evaluates to 3.000000e-01 which
  // is not exactly representable in type 'float'}} expected-error@-2
  // {{constexpr initializer evaluates to -1 which is not exactly representable
  ...
  // expected-error@-1 {{constexpr initializer evaluates to 123456789 which is
  // not exactly representable in type 'float'}}

  constexpr int V71 = 0.3;
  // expected-error@-1 {{constexpr initializer for type 'const int' is of type
  // 'double'}}
  constexpr int V72 = V59;
  // expected-error@-1 {{constexpr initializer for type 'const int' is of type
  // 'const double'}}
  constexpr struct S6 V73 = {V59};
  // expected-error@-1 {{constexpr initializer for type 'unsigned char' is of
  // type 'const double'}}

  constexpr float V74 = 1;
  constexpr float V75 = V59;
  constexpr unsigned int V76[3] = {0.5};
  // expected-error@-1 {{constexpr initializer for type 'const unsigned int' is
  // of type 'double'}}

  constexpr _Complex float V77 = 0;
  constexpr float V78 = V77;
  // expected-error@-1 {{constexpr initializer for type 'const float' is of type
...
constexpr char string[] = "test"
                          "ing this out\xFF";
constexpr unsigned char ustring[] = "test"
                                    "ing this out\xFF";
// expected-error@-1 {{constexpr initializer evaluates to -1 which is not
// exactly representable in type 'const unsigned char'}}
constexpr char u8string[] = u8"test"
                            u8"ing this out\xFF";
// expected-error@-1 {{constexpr initializer evaluates to 255 which is not
// exactly representable in type 'const char'}}
constexpr unsigned char u8ustring[] = u8"test"
                                      u8"ing this out\xFF";
constexpr unsigned short uustring[] = u"test"
                                      u"ing this out\xFF";
constexpr unsigned int Ustring[] = U"test"
                                   U"ing this out\xFF";
constexpr unsigned char Arr2[6][6] = {
    {"ek\xFF"}, {"ek\xFF"}
    // expected-error@-1 2{{constexpr initializer evaluates to -1 which is not
    // exactly representable in type 'const unsigned char'}}
};
...
// expected-error@-1 {{constexpr initializer evaluates to -12 which is not
// exactly representable in type 'const unsigned int'}}
...
void f7(int n, int array[n]) {
  constexpr typeof(array) foo =
      0; // Accepted because array is a pointer type, not a VLA type
  int(*(*fp)(int n))[n];
  constexpr typeof(fp) bar =
      0; // expected-error {{constexpr variable cannot have type 'const typeof
         // (fp)' (aka 'int (*(*const)(int))[n]')}}
}
...
void infsNaNs() {
  ...
  constexpr float fl7 =
      DBL_SNAN; // expected-error {{constexpr initializer evaluates to nan which
                // is not exactly representable in type 'const float'}}
  constexpr float fl8 =
      LD_SNAN; // expected-error {{constexpr initializer evaluates to nan which
               // is not exactly representable in type 'const float'}}
  constexpr float fl9 =
      WIDE_NAN; // expected-error {{incompatible pointer types passing 'int[2]' to parameter of type 'const char *'}} \
  ...
  constexpr float fl10 =
      UTF32_NAN; // expected-error {{incompatible pointer types passing 'unsigned int[2]' to parameter of type 'const char *'}} \
                                    // expected-error {{constexpr variable 'fl10' must be initialized by a constant expression}} \
                                  // expected-note 0+ {{this conversion is not allowed in a constant expression}}
  constexpr float fl11 =
      UTF16_NAN; // expected-error {{incompatible pointer types passing 'unsigned short[2]' to parameter of type 'const char *'}} \
                                   // expected-error {{constexpr variable 'fl11' must be initialized by a constant expression}} \
                                  // expected-note 0+ {{this conversion is not allowed in a constant expression}}
  constexpr float fl12 =
      UTF8_NAN; // expected-warning {{passing 'unsigned char[2]' to parameter of type 'const char *' converts between pointers to integer types where one is of the unique plain 'char' type and the other is not}} \
  ...
  constexpr double db4 =
      FLT_SNAN; // expected-error {{constexpr initializer evaluates to nan which
                // is not exactly representable in type 'const double'}}
  constexpr double db5 =
      LD_SNAN; // expected-error {{constexpr initializer evaluates to nan which
               // is not exactly representable in type 'const double'}}
  constexpr double db6 = INF;
}

constexpr struct S9 s9 =
...
// FIXME-expected-error@-1 {{constexpr initializer evaluates to 255 which is not
// exactly representable in 'long long' bit-field with width 8}} See: GH#101299
...
void constevalif() {
  if consteval
    (300) {} // expected-error {{expected '(' after 'if'}}
}
...
void ghissue109095() {
  ...
  _Static_assert(
      i ==
      c[0]); // expected-error {{static assertion expression is not an integral constant expression}}\
...
bool issue155507(v2int16_t a, v2int16_t b) {
  return __builtin_bit_cast(unsigned char,
                            __builtin_convertvector(a == b, __vbool2)) == 0b11;
}
...
void gh173847_test() {
  constexpr double d_const =
      gh173847_double(2.0); // expected-error {{constexpr variable 'd_const'
                            // must be initialized by a constant expression}}
  constexpr long double ld_const = gh173847_long_double(
      2.0L); // expected-error {{constexpr variable 'ld_const' must be
             // initialized by a constant expression}}
}

int gh173605(int x) {
  static constexpr int c =
      c; // expected-error {{constexpr variable 'c' must be initialized by a constant expression}} \
                              // expected-note {{read of object outside its lifetime is not allowed in a constant expression}} \
                              // expected-note {{declared here}}
  static int justincase = justincase; // expected-error {{initializer element is
                                      // not a compile-time constant}}
  return x;
}

struct designated_init_A {
  int a;
  int b;
  int c : 2;
  double d;
...
struct designated_init_B {
  struct designated_init_A a;
  int y;
};
constexpr struct designated_init_A designated_init_a = {1, 2, 3, 4.0, 5,
                                                        6, 7, 8, 9};
constexpr struct designated_init_B designated_init_b = {
    designated_init_a, .a.a = 10, .a.c = 11, .a.d = 12.0,
    .a.e[1] = 13,      .y = 14}; // expected-warning 4 {{initializer partially
                                 // overrides prior initialization of this
                                 // subobject}} // expected-note 4 {{previous
                                 // initialization}}
static_assert(designated_init_b.a.a == 10);
static_assert(designated_init_b.a.b == 2);
static_assert(designated_init_b.a.c == -1);
static_assert(
    designated_init_b.a.d ==
    12.0); // expected-warning {{folding it to a constant is a GNU extension}}
static_assert(
    designated_init_b.a.e[0] ==
    5); // expected-warning {{folding it to a constant is a GNU extension}}
static_assert(
    designated_init_b.a.e[1] ==
    13); // expected-warning {{folding it to a constant is a GNU extension}}
...
const struct S2 s2[2] = {
    {{"foo"}, 1},
    [0].L[2] =
        'x'}; // expected-warning {{initializer partially overrides prior initialization of this subobject}} \
                                                        // expected-note {{previous initialization is here}}
static_assert(
    s2[0].L[2] ==
    'x'); // expected-warning {{folding it to a constant is a GNU extension}}
