...
void simple_case_gsl() {
  ...
  }             // expected-note {{is destroyed here}}
...
void multiple_pointers() {
  ...
  }             // expected-note 3 {{is destroyed here}}
  (void)*p;     // expected-note {{later used here}}
  (void)*q;     // expected-note {{later used here}}
...
void potential_together(bool cond) {
  ...
  }                     // expected-note 2 {{is destroyed here}}
...
void loop_with_break(bool cond) {
  ...
  for (int i = 0; i < 10; ++i) {
    if (cond) {
      ...
      break;     // expected-note {{is destroyed here}}
    }           
  } 
  (void)*p;     // expected-note {{later used here}}
}
...
void conditional_operator_nested(bool cond) {
  ...
  }  // expected-note 4 {{is destroyed here}}
...
void var_initializer() {
  View v = non_trivially_destructed_temporary() // expected-warning {{temporary object does not live long enough}} \
               ...
               .getView(); // expected-note {{is destroyed here}}
  v.use(); // expected-note {{later used here}}
}
...
void range_based_for_not_reference() {
  ...
  {
    MyObjStorage s;
    for (MyObj o : s) { // expected-note {{is destroyed here}}
...
void test_temporary() {
  ...
  const std::string& y = identity(S().x()); // expected-warning {{temporary object does not live long enough}} expected-note {{is destroyed here}} \
                                            // expected-note {{result of call to 'x' aliases the storage of temporary object}} \
...
void test_lifetime_extension_ok() {
  const S& x = S();
  (void)x;
  const S& y = identity(S()); // expected-warning {{temporary object does not live long enough}} expected-note {{is destroyed here}} \
                              // expected-note {{result of call to 'identity' aliases the storage of temporary object}}
  (void)y; // expected-note {{later used here}}
}

const std::string& test_return() {
  const std::string& x = S().x(); // expected-warning {{temporary object does not live long enough}} expected-note {{is destroyed here}} \
                                  // expected-note {{result of call to 'x' aliases the storage of temporary object}}
...
void test() {
  ...
  const MyObj* pTMA = mtf.memberA(MyObj()); // expected-warning {{temporary object does not live long enough}} // expected-note {{is destroyed here}} \
                                            // expected-note {{result of call to 'memberA' aliases the storage of temporary object}}
  const MyObj* pTMB = mtf.memberB(MyObj()); // tu-warning {{temporary object does not live long enough}} // tu-note {{is destroyed here}} \
                                            // tu-note {{result of call to 'memberB' aliases the storage of temporary object}}
  const MyObj* pTMC = mtf.memberC(MyObj()); // expected-warning {{temporary object does not live long enough}} // expected-note {{is destroyed here}} \
...
void indexing_with_static_operator() {
  ...
  S& x = S()("1", // expected-note 2 {{expression aliases the storage of temporary object}}
             ...
             3);  // expected-warning {{temporary object does not live long enough}} expected-note 2 {{is destroyed here}}
...
struct T {
  ...
  void bar();
  void foo() {
    v = std::string("tmp"); // expected-warning {{temporary object does not live long enough}} expected-note {{is destroyed here}}
    bar();                  // expected-note {{later used here}}
  }
};
