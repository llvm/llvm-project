...
char *volatile_ptr(char *volatile base, unsigned long offset) {
  // CHECK:                           define{{.*}} ptr @volatile_ptr(ptr noundef %[[BASE:.*]], i64 noundef %[[OFFSET:.*]])
  // CHECK-NEXT:                      [[ENTRY:.*]]:
  // CHECK-NEXT:                        %[[BASE_ADDR:.*]] = alloca ptr, align 8
  // CHECK-NEXT:                        %[[OFFSET_ADDR:.*]] = alloca i64, align 8
  // CHECK-NEXT:                        store volatile ptr %[[BASE]], ptr %[[BASE_ADDR]], align 8
  ...
  // CHECK-SANITIZE-NEXT:               %[[BASE_RELOADED_INT:.*]] = ptrtoaddr ptr %[[BASE_RELOADED]] to i64, !nosanitize
