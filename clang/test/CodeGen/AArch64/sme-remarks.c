...
void test_za_merge_paths(int a) __arm_inout("za") {
  ...
  if (a != 0)
    // expected-sdag-remark@+2 {{call from 'test_za_merge_paths' to 'unknown callee' sets up a lazy save for ZA}}
    ...
    private_za_callee_b();
  // expected-sdag-remark@+3 {{call from 'test_za_merge_paths' to 'unknown callee' sets up a lazy save for ZA}}
...
void test_lazy_save_multiple_paths(int a) __arm_inout("za") {
  ...
  if (a != 0)
    // expected-sdag-remark@+2 {{call from 'test_lazy_save_multiple_paths' to 'unknown callee' sets up a lazy save for ZA}}
    // expected-remark@+1 {{call to 'private_za_callee_a' requires ZA save}}
    private_za_callee_a();
  else {
    // expected-sdag-remark@+2 {{call from 'test_lazy_save_multiple_paths' to 'unknown callee' sets up a lazy save for ZA}}
    // expected-remark@+1 {{call to 'private_za_callee_b' requires ZA save}}
    private_za_callee_b();
    // expected-sdag-remark@+3 {{call from 'test_lazy_save_multiple_paths' to 'unknown callee' sets up a lazy save for ZA}}
