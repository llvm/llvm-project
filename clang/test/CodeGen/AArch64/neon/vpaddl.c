...
// RUN:                   %clang_cc1_cg_arm64_neon -disable-O0-optnone           -emit-llvm -o - %s | opt -S -passes=mem2reg,sroa | FileCheck %s --check-prefixes=LLVM
// RUN: %if cir-enabled %{%clang_cc1_cg_arm64_neon -disable-O0-optnone -fclangir -emit-llvm -o - %s | opt -S -passes=mem2reg,sroa | FileCheck %s --check-prefixes=LLVM %}
// RUN: %if cir-enabled %{%clang_cc1_cg_arm64_neon -disable-O0-optnone -fclangir -emit-cir  -o - %s |                               FileCheck %s --check-prefixes=CIR %}
