...
// %s --check-prefixes=ALL,LLVM RUN: %if cir-enabled %{%clang_cc1_cg_arm64_neon
// -target-feature +fullfp16 -fclangir -emit-llvm %s -disable-O0-optnone | opt
// -S -passes=mem2reg,sroa | FileCheck %s --check-prefixes=ALL,LLVM %} RUN: %if
// cir-enabled %{%clang_cc1_cg_arm64_neon -target-feature +fullfp16 -fclangir
// -emit-cir  %s -disable-O0-optnone |                               FileCheck
...
float16x4_t test_vfma_f16(float16x4_t a, float16x4_t b, float16x4_t c) {
  ...
  // half> {{.*}} [[C:%.*]]) {{.*}} { LLVM:      [[A_I:%.*]] = bitcast <4 x
  // half> [[A]] to <4 x i16> LLVM-NEXT: [[B_I:%.*]] = bitcast <4 x half> [[B]]
  // to <4 x i16> LLVM-NEXT: [[C_I:%.*]] = bitcast <4 x half> [[C]] to <4 x i16>
  ...
  // [[B_CAST]], <4 x half> [[C_CAST]], <4 x half> [[A_CAST]]) LLVM-NEXT: ret <4
  // x half> [[FMA]]
...
float16x8_t test_vfmaq_f16(float16x8_t a, float16x8_t b, float16x8_t c) {
  ...
  // half> {{.*}} [[C:%.*]]) {{.*}} { LLVM:      [[A_I:%.*]] = bitcast <8 x
  // half> [[A]] to <8 x i16> LLVM-NEXT: [[B_I:%.*]] = bitcast <8 x half> [[B]]
  // to <8 x i16> LLVM-NEXT: [[C_I:%.*]] = bitcast <8 x half> [[C]] to <8 x i16>
  ...
  // [[B_CAST]], <8 x half> [[C_CAST]], <8 x half> [[A_CAST]]) LLVM-NEXT: ret <8
  // x half> [[FMA]]
...
float16x4_t test_vfms_f16(float16x4_t a, float16x4_t b, float16x4_t c) {
  ...
  // LLVM-SAME: <4 x half> {{.*}} [[A:%.*]], <4 x half> {{.*}} [[B:%.*]], <4 x
  // half> {{.*}} [[C:%.*]]) {{.*}} { LLVM:      [[FNEG:%.*]] = fneg <4 x half>
  // [[B]] LLVM-NEXT: [[A_I:%.*]] = bitcast <4 x half> [[A]] to <4 x i16>
...
float16x8_t test_vfmsq_f16(float16x8_t a, float16x8_t b, float16x8_t c) {
  ...
  // LLVM-SAME: <8 x half> {{.*}} [[A:%.*]], <8 x half> {{.*}} [[B:%.*]], <8 x
  // half> {{.*}} [[C:%.*]]) {{.*}} { LLVM:      [[FNEG:%.*]] = fneg <8 x half>
  // [[B]] LLVM-NEXT: [[A_I:%.*]] = bitcast <8 x half> [[A]] to <8 x i16>
...
float16x4_t test_vfma_lane_f16(float16x4_t a, float16x4_t b, float16x4_t c) {
  ...
  // #cir.int<3> : !s32i] : !cir.vector<4 x !cir.f16> CIR:
  // cir.call_llvm_intrinsic "fma" %{{.*}}, [[LANE]], %{{.*}} : (!cir.vector<4 x
  ...
  // {{.*}}, <4 x i32> <i32 3, i32 3, i32 3, i32 3> LLVM:      [[FMA:%.*]] =
  // call <4 x half> @llvm.fma.v4f16(<4 x half> [[B_CAST]], <4 x half> [[LANE]],
  // <4 x half> [[A_CAST]]) LLVM:      ret <4 x half> [[FMA]]
...
float16x8_t test_vfmaq_lane_f16(float16x8_t a, float16x8_t b, float16x4_t c) {
  ...
  // : !s32i, #cir.int<3> : !s32i] : !cir.vector<8 x !cir.f16> CIR:
  // cir.call_llvm_intrinsic "fma" %{{.*}}, [[LANE]], %{{.*}} : (!cir.vector<8 x
  ...
  // half> {{.*}} [[C:%.*]]) {{.*}} { LLVM:      [[A_I:%.*]] = bitcast <8 x
  // half> [[A]] to <8 x i16> LLVM-NEXT: [[B_I:%.*]] = bitcast <8 x half> [[B]]
  // to <8 x i16> LLVM-NEXT: [[C_I:%.*]] = bitcast <4 x half> [[C]] to <4 x i16>
  // LLVM-NEXT: [[A_BYTES:%.*]] = bitcast <8 x i16> [[A_I]] to <16 x i8>
  // LLVM-NEXT: [[B_BYTES:%.*]] = bitcast <8 x i16> [[B_I]] to <16 x i8>
  // LLVM-NEXT: [[C_BYTES:%.*]] = bitcast <4 x i16> [[C_I]] to <8 x i8>
  // LLVM-DAG:  [[C_CAST:%.*]] = bitcast <8 x i8> [[C_BYTES]] to <4 x half>
  ...
  // [[B_CAST]], <8 x half> [[LANE]], <8 x half> [[A_CAST]]) LLVM:      ret <8 x
  // half> [[FMA]]
...
float16x4_t test_vfma_laneq_f16(float16x4_t a, float16x4_t b, float16x8_t c) {
  ...
  // #cir.int<7> : !s32i] : !cir.vector<4 x !cir.f16> CIR:
  // cir.call_llvm_intrinsic "fma" [[LANE]], %{{.*}}, %{{.*}} : (!cir.vector<4 x
  // !cir.f16>, !cir.vector<4 x !cir.f16>, !cir.vector<4 x !cir.f16>) ->
  // !cir.vector<4 x !cir.f16>
  ...
  // half> {{.*}} [[C:%.*]]) {{.*}} { LLVM:      [[A_I:%.*]] = bitcast <4 x
  // half> [[A]] to <4 x i16> LLVM-NEXT: [[B_I:%.*]] = bitcast <4 x half> [[B]]
  // to <4 x i16> LLVM-NEXT: [[C_I:%.*]] = bitcast <8 x half> [[C]] to <8 x i16>
  // LLVM-NEXT: [[A_BYTES:%.*]] = bitcast <4 x i16> [[A_I]] to <8 x i8>
  // LLVM-NEXT: [[B_BYTES:%.*]] = bitcast <4 x i16> [[B_I]] to <8 x i8>
  // LLVM-NEXT: [[C_BYTES:%.*]] = bitcast <8 x i16> [[C_I]] to <16 x i8>
  // LLVM-NEXT: [[A_CAST:%.*]] = bitcast <8 x i8> [[A_BYTES]] to <4 x half>
  ...
  // {{.*}}, <4 x i32> <i32 7, i32 7, i32 7, i32 7> LLVM:      [[FMA:%.*]] =
  // call <4 x half> @llvm.fma.v4f16(<4 x half> [[LANE]], <4 x half> [[B_CAST]],
  // <4 x half> [[A_CAST]]) LLVM:      ret <4 x half> [[FMA]]
...
float16x8_t test_vfmaq_laneq_f16(float16x8_t a, float16x8_t b, float16x8_t c) {
  ...
  // : !s32i, #cir.int<7> : !s32i] : !cir.vector<8 x !cir.f16> CIR:
  // cir.call_llvm_intrinsic "fma" [[LANE]], %{{.*}}, %{{.*}} : (!cir.vector<8 x
  // !cir.f16>, !cir.vector<8 x !cir.f16>, !cir.vector<8 x !cir.f16>) ->
  // !cir.vector<8 x !cir.f16>
  ...
  // LLVM-NEXT: [[FMA:%.*]] = call <8 x half> @llvm.fma.v8f16(<8 x half>
  // [[LANE]], <8 x half> [[B_CAST]], <8 x half> [[A_CAST]]) LLVM:      ret <8 x
  // half> [[FMA]]
...
float16x4_t test_vfma_n_f16(float16x4_t a, float16x4_t b, float16_t c) {
  ...
  // {{.*}} [[C:%.*]]) {{.*}} { LLVM:      [[SPLAT:%.*]] = insertelement <4 x
  // half> {{.*}}, half [[C]], i{{32|64}} 3 LLVM-NEXT: [[A_I:%.*]] = bitcast <4
  // x half> [[A]] to <4 x i16> LLVM-NEXT: [[B_I:%.*]] = bitcast <4 x half>
  // [[B]] to <4 x i16> LLVM-NEXT: [[C_I:%.*]] = bitcast <4 x half> [[SPLAT]] to
  // <4 x i16> LLVM-NEXT: [[A_BYTES:%.*]] = bitcast <4 x i16> [[A_I]] to <8 x
  // i8> LLVM-NEXT: [[B_BYTES:%.*]] = bitcast <4 x i16> [[B_I]] to <8 x i8>
  // LLVM-NEXT: [[C_BYTES:%.*]] = bitcast <4 x i16> [[C_I]] to <8 x i8>
  // LLVM-NEXT: [[A_CAST:%.*]] = bitcast <8 x i8> [[A_BYTES]] to <4 x half>
  // LLVM-NEXT: [[B_CAST:%.*]] = bitcast <8 x i8> [[B_BYTES]] to <4 x half>
  // LLVM-NEXT: [[C_CAST:%.*]] = bitcast <8 x i8> [[C_BYTES]] to <4 x half>
  // LLVM-NEXT: [[FMA:%.*]] = call <4 x half> @llvm.fma.v4f16(<4 x half>
  // [[B_CAST]], <4 x half> [[C_CAST]], <4 x half> [[A_CAST]]) LLVM:      ret <4
  // x half> [[FMA]]
...
float16x8_t test_vfmaq_n_f16(float16x8_t a, float16x8_t b, float16_t c) {
  ...
  // {{.*}} [[C:%.*]]) {{.*}} { LLVM:      [[SPLAT:%.*]] = insertelement <8 x
  // half> {{.*}}, half [[C]], i{{32|64}} 7 LLVM-NEXT: [[A_I:%.*]] = bitcast <8
  // x half> [[A]] to <8 x i16> LLVM-NEXT: [[B_I:%.*]] = bitcast <8 x half>
  // [[B]] to <8 x i16> LLVM-NEXT: [[C_I:%.*]] = bitcast <8 x half> [[SPLAT]] to
  // <8 x i16> LLVM-NEXT: [[A_BYTES:%.*]] = bitcast <8 x i16> [[A_I]] to <16 x
  // i8> LLVM-NEXT: [[B_BYTES:%.*]] = bitcast <8 x i16> [[B_I]] to <16 x i8>
  // LLVM-NEXT: [[C_BYTES:%.*]] = bitcast <8 x i16> [[C_I]] to <16 x i8>
  // LLVM-NEXT: [[A_CAST:%.*]] = bitcast <16 x i8> [[A_BYTES]] to <8 x half>
  // LLVM-NEXT: [[B_CAST:%.*]] = bitcast <16 x i8> [[B_BYTES]] to <8 x half>
  // LLVM-NEXT: [[C_CAST:%.*]] = bitcast <16 x i8> [[C_BYTES]] to <8 x half>
  // LLVM-NEXT: [[FMA:%.*]] = call <8 x half> @llvm.fma.v8f16(<8 x half>
  // [[B_CAST]], <8 x half> [[C_CAST]], <8 x half> [[A_CAST]]) LLVM:      ret <8
  // x half> [[FMA]]
...
float16_t test_vfmah_lane_f16(float16_t a, float16_t b, float16x4_t c) {
  ...
  // !cir.vector<4 x !cir.f16> CIR: cir.call_llvm_intrinsic "fma" %{{.*}},
  // [[LANE]], %{{.*}} : (!cir.f16, !cir.f16, !cir.f16) -> !cir.f16

  // LLVM-SAME: half {{.*}} [[A:%.*]], half {{.*}} [[B:%.*]], <4 x half> {{.*}}
  // [[C:%.*]]) {{.*}} { LLVM:      [[LANE:%.*]] = extractelement <4 x half>
  // [[C]], i{{32|64}} 3 LLVM:      [[FMA:%.*]] = call half @llvm.fma.f16(half
  // [[B]], half [[LANE]], half [[A]]) LLVM:      ret half [[FMA]]
...
float16_t test_vfmah_laneq_f16(float16_t a, float16_t b, float16x8_t c) {
  // CIR: [[LANE:%.*]] = cir.vec.extract %{{.*}}[%{{.*}} : !u64i] :
  // !cir.vector<8 x !cir.f16> CIR: cir.call_llvm_intrinsic "fma" %{{.*}},
  // [[LANE]], %{{.*}} : (!cir.f16, !cir.f16, !cir.f16) -> !cir.f16

  // LLVM-SAME: half {{.*}} [[A:%.*]], half {{.*}} [[B:%.*]], <8 x half> {{.*}}
  // [[C:%.*]]) {{.*}} { LLVM:      [[LANE:%.*]] = extractelement <8 x half>
  // [[C]], i{{32|64}} 7 LLVM:      [[FMA:%.*]] = call half @llvm.fma.f16(half
  // [[B]], half [[LANE]], half [[A]]) LLVM:      ret half [[FMA]]
...
float16x4_t test_vfms_lane_f16(float16x4_t a, float16x4_t b, float16x4_t c) {
  ...
  // !cir.f16>, !cir.ptr<!cir.vector<4 x !cir.f16>> CIR: [[NEG_PTR:%.*]] =
  // cir.cast bitcast [[NEG_SLOT]] : !cir.ptr<!cir.vector<4 x !cir.f16>> ->
  // !cir.ptr<!cir.vector<8 x !s8i>> CIR: [[NEG_BYTES:%.*]] = cir.load align(8)
  // [[NEG_PTR]] : !cir.ptr<!cir.vector<8 x !s8i>>, !cir.vector<8 x !s8i> CIR:
  // [[NEG_CAST:%.*]] = cir.cast bitcast [[NEG_BYTES]] : !cir.vector<8 x !s8i>
  // -> !cir.vector<4 x !cir.f16> CIR: [[LANE:%.*]] = cir.vec.shuffle(%{{.*}},
  // %{{.*}} : !cir.vector<4 x !cir.f16>) [#cir.int<3> : !s32i, #cir.int<3> :
  // !s32i, #cir.int<3> : !s32i, #cir.int<3> : !s32i] : !cir.vector<4 x
  // !cir.f16> CIR: cir.call_llvm_intrinsic "fma" [[NEG_CAST]], [[LANE]],
  // %{{.*}} : (!cir.vector<4 x !cir.f16>, !cir.vector<4 x !cir.f16>,
  // !cir.vector<4 x !cir.f16>) -> !cir.vector<4 x !cir.f16>

  // LLVM-SAME: <4 x half> {{.*}} [[A:%.*]], <4 x half> {{.*}} [[B:%.*]], <4 x
  // half> {{.*}} [[C:%.*]]) {{.*}} { LLVM:      [[A_I:%.*]] = bitcast <4 x
  // half> [[A]] to <4 x i16> LLVM-NEXT: [[FNEG:%.*]] = fneg <4 x half> [[B]]
  // LLVM-NEXT: [[B_I:%.*]] = bitcast <4 x half> [[FNEG]] to <4 x i16>
  // LLVM-NEXT: [[C_I:%.*]] = bitcast <4 x half> [[C]] to <4 x i16>
  // LLVM-NEXT: [[A_BYTES:%.*]] = bitcast <4 x i16> [[A_I]] to <8 x i8>
  // LLVM-NEXT: [[B_BYTES:%.*]] = bitcast <4 x i16> [[B_I]] to <8 x i8>
...
float16x8_t test_vfmsq_lane_f16(float16x8_t a, float16x8_t b, float16x4_t c) {
  ...
  // !cir.f16>, !cir.ptr<!cir.vector<8 x !cir.f16>> CIR: [[NEG_PTR:%.*]] =
  // cir.cast bitcast [[NEG_SLOT]] : !cir.ptr<!cir.vector<8 x !cir.f16>> ->
  // !cir.ptr<!cir.vector<16 x !s8i>> CIR: [[NEG_BYTES:%.*]] = cir.load
  // align(16) [[NEG_PTR]] : !cir.ptr<!cir.vector<16 x !s8i>>, !cir.vector<16 x
  // !s8i> CIR: [[NEG_CAST:%.*]] = cir.cast bitcast [[NEG_BYTES]] :
  // !cir.vector<16 x !s8i> -> !cir.vector<8 x !cir.f16> CIR: [[LANE:%.*]] =
  // cir.vec.shuffle(%{{.*}}, %{{.*}} : !cir.vector<4 x !cir.f16>) [#cir.int<3>
  // : !s32i, #cir.int<3> : !s32i, #cir.int<3> : !s32i, #cir.int<3> : !s32i,
  // #cir.int<3> : !s32i, #cir.int<3> : !s32i, #cir.int<3> : !s32i, #cir.int<3>
  // : !s32i] : !cir.vector<8 x !cir.f16> CIR: cir.call_llvm_intrinsic "fma"
  // [[NEG_CAST]], [[LANE]], %{{.*}} : (!cir.vector<8 x !cir.f16>, !cir.vector<8
  // x !cir.f16>, !cir.vector<8 x !cir.f16>) -> !cir.vector<8 x !cir.f16>

  // LLVM-SAME: <8 x half> {{.*}} [[A:%.*]], <8 x half> {{.*}} [[B:%.*]], <4 x
  // half> {{.*}} [[C:%.*]]) {{.*}} { LLVM:      [[A_I:%.*]] = bitcast <8 x
  // half> [[A]] to <8 x i16> LLVM-NEXT: [[FNEG:%.*]] = fneg <8 x half> [[B]]
  // LLVM-NEXT: [[B_I:%.*]] = bitcast <8 x half> [[FNEG]] to <8 x i16>
  // LLVM-NEXT: [[C_I:%.*]] = bitcast <4 x half> [[C]] to <4 x i16>
  // LLVM-NEXT: [[A_BYTES:%.*]] = bitcast <8 x i16> [[A_I]] to <16 x i8>
  // LLVM-NEXT: [[B_BYTES:%.*]] = bitcast <8 x i16> [[B_I]] to <16 x i8>
...
float16x4_t test_vfms_laneq_f16(float16x4_t a, float16x4_t b, float16x8_t c) {
  ...
  // [[NEG_CAST:%.*]] = cir.cast bitcast [[NEG_BYTES]] : !cir.vector<8 x !s8i>
  // -> !cir.vector<4 x !cir.f16> CIR: [[LANE:%.*]] = cir.vec.shuffle(%{{.*}},
  // %{{.*}} : !cir.vector<8 x !cir.f16>) [#cir.int<7> : !s32i, #cir.int<7> :
  // !s32i, #cir.int<7> : !s32i, #cir.int<7> : !s32i] : !cir.vector<4 x
  // !cir.f16> CIR: cir.call_llvm_intrinsic "fma" [[LANE]], [[NEG_CAST]],
  // %{{.*}} : (!cir.vector<4 x !cir.f16>, !cir.vector<4 x !cir.f16>,
  // !cir.vector<4 x !cir.f16>) -> !cir.vector<4 x !cir.f16>
  ...
  // LLVM-NEXT: [[C_CAST:%.*]] = bitcast <16 x i8> [[C_BYTES]] to <8 x half>
  // LLVM-NEXT: [[LANE:%.*]] = shufflevector <8 x half> [[C_CAST]], <8 x half>
  // {{.*}}, <4 x i32> <i32 7, i32 7, i32 7, i32 7> LLVM-NEXT: [[FMA:%.*]] =
  // call <4 x half> @llvm.fma.v4f16(<4 x half> [[LANE]], <4 x half> [[B_CAST]],
  // <4 x half> [[A_CAST]]) LLVM:      ret <4 x half> [[FMA]]
...
float16x8_t test_vfmsq_laneq_f16(float16x8_t a, float16x8_t b, float16x8_t c) {
  ...
  // !s8i> CIR: [[NEG_CAST:%.*]] = cir.cast bitcast [[NEG_BYTES]] :
  // !cir.vector<16 x !s8i> -> !cir.vector<8 x !cir.f16> CIR: [[LANE:%.*]] =
  // cir.vec.shuffle(%{{.*}}, %{{.*}} : !cir.vector<8 x !cir.f16>) [#cir.int<7>
  // : !s32i, #cir.int<7> : !s32i, #cir.int<7> : !s32i, #cir.int<7> : !s32i,
  // #cir.int<7> : !s32i, #cir.int<7> : !s32i, #cir.int<7> : !s32i, #cir.int<7>
  // : !s32i] : !cir.vector<8 x !cir.f16> CIR: cir.call_llvm_intrinsic "fma"
  // [[LANE]], [[NEG_CAST]], %{{.*}} : (!cir.vector<8 x !cir.f16>, !cir.vector<8
  // x !cir.f16>, !cir.vector<8 x !cir.f16>) -> !cir.vector<8 x !cir.f16>
...
float16x4_t test_vfms_n_f16(float16x4_t a, float16x4_t b, float16_t c) {
  ...
  // LLVM-SAME: <4 x half> {{.*}} [[A:%.*]], <4 x half> {{.*}} [[B:%.*]], half
  // {{.*}} [[C:%.*]]) {{.*}} { LLVM:      [[FNEG:%.*]] = fneg <4 x half> [[B]]
  // LLVM:      [[SPLAT:%.*]] = insertelement <4 x half> {{.*}}, half [[C]],
  // i{{32|64}} 3 LLVM-NEXT: [[A_I:%.*]] = bitcast <4 x half> [[A]] to <4 x i16>
  // LLVM-NEXT: [[B_I:%.*]] = bitcast <4 x half> [[FNEG]] to <4 x i16>
...
float16x8_t test_vfmsq_n_f16(float16x8_t a, float16x8_t b, float16_t c) {
  ...
  // LLVM-SAME: <8 x half> {{.*}} [[A:%.*]], <8 x half> {{.*}} [[B:%.*]], half
  // {{.*}} [[C:%.*]]) {{.*}} { LLVM:      [[FNEG:%.*]] = fneg <8 x half> [[B]]
  // LLVM:      [[SPLAT:%.*]] = insertelement <8 x half> {{.*}}, half [[C]],
  // i{{32|64}} 7 LLVM-NEXT: [[A_I:%.*]] = bitcast <8 x half> [[A]] to <8 x i16>
  // LLVM-NEXT: [[B_I:%.*]] = bitcast <8 x half> [[FNEG]] to <8 x i16>
...
float16_t test_vfmsh_lane_f16(float16_t a, float16_t b, float16x4_t c) {
  ...
  // !cir.ptr<!cir.f16> CIR: [[NEG_ARG:%.*]] = cir.load align(2) [[NEG_SLOT]] :
  // !cir.ptr<!cir.f16>, !cir.f16 CIR: [[LANE:%.*]] = cir.vec.extract
  // %{{.*}}[%{{.*}} : !u64i] : !cir.vector<4 x !cir.f16> CIR:
  // cir.call_llvm_intrinsic "fma" [[NEG_ARG]], [[LANE]], %{{.*}} : (!cir.f16,
  // !cir.f16, !cir.f16) -> !cir.f16

  // LLVM-SAME: half {{.*}} [[A:%.*]], half {{.*}} [[B:%.*]], <4 x half> {{.*}}
  // [[C:%.*]]) {{.*}} { LLVM:      [[CONV:%.*]] = fpext half [[B]] to float
  // LLVM-NEXT: [[FNEG:%.*]] = fneg float [[CONV]]
  // LLVM-NEXT: [[NEG:%.*]] = fptrunc float [[FNEG]] to half
  // LLVM-NEXT: [[LANE:%.*]] = extractelement <4 x half> [[C]], i{{32|64}} 3
  // LLVM-NEXT: [[FMA:%.*]] = call half @llvm.fma.f16(half [[NEG]], half
  // [[LANE]], half [[A]]) LLVM:      ret half [[FMA]]
...
float16_t test_vfmsh_laneq_f16(float16_t a, float16_t b, float16x8_t c) {
  ...
  // !cir.ptr<!cir.f16> CIR: [[NEG_ARG:%.*]] = cir.load align(2) [[NEG_SLOT]] :
  // !cir.ptr<!cir.f16>, !cir.f16 CIR: [[LANE:%.*]] = cir.vec.extract
  // %{{.*}}[%{{.*}} : !u64i] : !cir.vector<8 x !cir.f16> CIR:
  // cir.call_llvm_intrinsic "fma" [[NEG_ARG]], [[LANE]], %{{.*}} : (!cir.f16,
  // !cir.f16, !cir.f16) -> !cir.f16
