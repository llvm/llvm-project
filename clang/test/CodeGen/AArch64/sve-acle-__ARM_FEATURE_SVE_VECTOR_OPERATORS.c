...
// CHECK-SAME:   ptr dead_on_unwind noalias nofree writable writeonly sret(<[[#div(VBITS,8)]] x i8>) align 16 captures(none) initializes((0, [[#div(VBITS,8)]])) %agg.result, ptr nofree noundef readonly align 16 captures(none) dead_on_return %0)
// CHECK-NEXT: entry:
...
// CHECK-NEXT:   call void @f3(ptr noundef nonnull align 16 dead_on_return [[INDIRECT_ARG_TEMP]]) [[ATTR5:#.*]]
...
// CHECK-SAME:   ptr noundef align 16 dead_on_return)
