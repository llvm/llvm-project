...
// CHECK-NEXT:    [[TMP3:%.*]] = load ptr, ptr [[P_ADDR]], align 8
// CHECK-NEXT:    [[TMP4:%.*]] = load <8 x i32>, ptr [[T_ADDR]], align 32
// CHECK-NEXT:    [[MASKED_EXPAND_LOAD:%.*]] = call <8 x i32> @llvm.masked.expandload.v8i32.p0(ptr align 4 [[TMP3]], <8 x i1> [[TMP2]], <8 x i32> [[TMP4]])
...
// CHECK-NEXT:    ret void
//
void gtest_store(v8b m, gv8i v, int *p) {
  __builtin_masked_store(m, v, p);
}
...
// CHECK-NEXT:    [[TMP3:%.*]] = load <8 x i32>, ptr [[V_ADDR]], align 32
// CHECK-NEXT:    [[TMP4:%.*]] = load ptr, ptr [[P_ADDR]], align 8
// CHECK-NEXT:    call void @llvm.masked.compressstore.v8i32.p0(<8 x i32> [[TMP3]], ptr align 4 [[TMP4]], <8 x i1> [[TMP2]])
// CHECK-NEXT:    ret void
//
...
// CHECK-NEXT:    ret void
//
void gtest_scatter(v8b mask, gv8i val, gv8i idx, int *ptr) {
  __builtin_masked_scatter(mask, val, idx, ptr);
}
...
// CHECK-NEXT:    ret void
//
void gtest_store_as(v8b m, gv8i v, int __attribute__((address_space(42))) *p) {
  __builtin_masked_store(m, v, p);
}
...
// CHECK-NEXT:    ret void
//
void gtest_scatter_as(v8b mask, gv8i val, gv8i idx, int __attribute__((address_space(42))) *ptr) {
  __builtin_masked_scatter(mask, val, idx, ptr);
}
