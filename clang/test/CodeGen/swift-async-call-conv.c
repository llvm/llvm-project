...
// RUN: %clang_cc1 -no-opaque-pointers -no-enable-noundef-analysis -triple i386-apple-watchos2 -emit-llvm -o - %s | FileCheck %s
...
// RUN: %clang_cc1 -no-opaque-pointers -no-enable-noundef-analysis -x c++ -triple i386-apple-watchos2 -emit-llvm -o - %s | FileCheck %s --check-prefix=CHECK --check-prefix=CPPONLY
