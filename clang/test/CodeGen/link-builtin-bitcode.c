...
// without RUN: %clang_cc1 -triple amdgpu8.03-- -DBITCODE -emit-llvm-bc -o
// %t-lib.bc %s RUN: %clang_cc1 -triple amdgpu-- -DBITCODE -emit-llvm-bc -o
// %t-lib.no-cpu.bc %s
...
int __attribute__((target("dot8-insts"))) attr_not_in_target(void) {
  return 42;
}
