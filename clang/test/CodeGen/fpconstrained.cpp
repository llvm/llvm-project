...
float f0, f1, f2;
  ...
  // CHECK-LABEL: define {{.*}}void @_ZN4aaaaIiED2Ev{{.*}}
    ...
    // MAYTRAP: llvm.fadd.f32(float %{{.*}}, float %{{.*}}) #{{.*}} [ "fp.control"(metadata !"rte"), "fp.except"(metadata !"maytrap") ]
    // EXCEPT: llvm.fadd.f32(float %{{.*}}, float %{{.*}}) #{{.*}} [ "fp.control"(metadata !"rte") ]
    // FPMODELSTRICT: fadd float
    // STRICTEXCEPT: fadd float
    // STRICTNOEXCEPT: llvm.fadd.f32(float %{{.*}}, float %{{.*}}) #{{.*}} [ "fp.except"(metadata !"ignore") ]
