...
// CHECK: call void {{.*}}foo@P1{{.*}}(ptr nofree noundef{{[^,]*}} %ref.tmp, ptr dead_on_unwind inreg writable sret(%class.P1) align 1 %agg.result, i8 %0)
...
// CHECK: call void {{.*}}foo@P2{{.*}}(ptr nofree noundef{{[^,]*}} %ref.tmp, ptr dead_on_unwind inreg writable sret(%class.P2) align 1 %retval, i8 %0)
class P2 {
public:
  P2 foo(P2 x);
};
