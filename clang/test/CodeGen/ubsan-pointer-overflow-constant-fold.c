// RUN: %clang_cc1 -triple msp430 -emit-llvm -fsanitize=pointer-overflow -O0 %s
// -o - | FileCheck %s
...
void test_constant_offset_overflow(void) {
  // sizeof(struct sensor_reading) == 8, index 4096: 4096 * 8 = 32768 overflows
  // i16.
  readings[0][4096];
}
...
void test_constant_offset_overflow_wraps_to_zero(void) {
  // sizeof(struct sensor_reading) == 8, index 8192: 8192 * 8 = 65536 wraps to 0
  // in i16.
  readings[0][8192];
}
