...
// RUN: %clang_cc1 -emit-llvm -fdebug-prefix-map=%S=clang/test/CodeGen -fno-ident -fdebug-compilation-dir=%S -fsanitize=array-bounds -fsanitize-trap=array-bounds -triple x86_64 -debug-info-kind=limited %s -o - | FileCheck --check-prefix=CHECK-TRAP %s
// RUN: %clang_cc1 -emit-llvm -fdebug-prefix-map=%S=clang/test/CodeGen -fno-ident -fdebug-compilation-dir=%S -fsanitize=array-bounds                              -triple x86_64 -debug-info-kind=limited %s -o - | FileCheck --check-prefix=CHECK-NOTRAP %s
...
// CHECK-TRAP: [[META5]] = !DIFile(filename: "clang/test/CodeGen/bounds-checking-debuginfo.c", directory: {{.*}})
...
// CHECK-NOTRAP: [[META5]] = !DIFile(filename: "clang/test/CodeGen/bounds-checking-debuginfo.c", directory: {{.*}})
