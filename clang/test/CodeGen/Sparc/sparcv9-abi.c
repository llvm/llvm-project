...
// CHECK-LABEL: define{{.*}} %struct.medium @f_medium(ptr nofreeobj noundef align 8 dead_on_return dereferenceable(32) %x)
...
// CHECK-LABEL: define{{.*}} %struct.medium_aligned @f_medium_aligned(ptr nofreeobj noundef align 16 dead_on_return dereferenceable(32) %x)
...
// CHECK-LABEL: define{{.*}} void @f_large(ptr dead_on_unwind noalias writable sret(%struct.large) align 8 %agg.result, ptr nofreeobj noundef align 8 dead_on_return dereferenceable(40) %x)
