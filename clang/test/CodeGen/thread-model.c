...
// RUN: %clang_cc1 -triple wasm32-unknown-unknown -mthread-model single -emit-llvm %s -o - | FileCheck %s --check-prefix=SINGLE
// RUN: %clang_cc1 -triple wasm32-unknown-unknown -mthread-model posix -emit-llvm %s -o - | FileCheck %s --check-prefix=POSIX
// RUN: %clang_cc1 -triple wasm32-unknown-unknown -emit-llvm %s -o - | FileCheck %s --check-prefix=POSIX
