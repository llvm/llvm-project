...
// -emit-llvm-bc %s -o %t-ppc-host.bc RUN: %clang_cc1 -fopenmp -x c++ -std=c++11
...
// %t-ppc-host.bc -o - | FileCheck %s --check-prefix CHK-EXCEPTION RUN:
...
// CHK-EXCEPTION RUN: %clang_cc1 -fopenmp -x c++ -std=c++11 -triple
// x86_64-unknown-unknown -fopenmp-targets=spirv64 -emit-llvm %s
// -fopenmp-is-target-device -fopenmp-host-ir-file-path %t-ppc-host.bc -o - |
// FileCheck %s --check-prefix CHK-EXCEPTION
