...
// -fopenmp-targets=amdgpu-amd-amdhsa -o - %s
...
#pragma omp declare target enter(fun1)                                         \
    device_type(nohost) // omp52-note {{marked as 'device_type(nohost)' here}}
#pragma omp declare variant(not_a_host_function)                               \
    match(device = {kind(                                                      \
                  host)}) // omp52-error {{function with 'device_type(nohost)'
                          // is not available on host}}
void fun1() {}
#pragma omp begin declare target device_type(                                  \
        nohost) // omp52-note {{marked as 'device_type(nohost)' here}}
void not_a_host_function() {}
#pragma omp end declare target
void failed_call_to_host_function() {
  fun1();
} // omp52-error {{function with 'device_type(nohost)' is not available on
  // host}}
