...
// FileCheck %s --check-prefix=OMP45 --check-prefix=CHECK RUN: %clang_cc1
// -fopenmp -fopenmp-version=45 -x c++ -std=c++11 -triple x86_64-unknown-unknown
// -fexceptions -fcxx-exceptions -emit-pch -o %t %s RUN: %clang_cc1 -fopenmp
// -fopenmp-version=45 -x c++ -triple x86_64-unknown-unknown -fexceptions
// -fcxx-exceptions -debug-info-kind=limited -std=c++11 -include-pch %t -verify
// %s -emit-llvm -o - | FileCheck %s RUN: %clang_cc1 -verify -triple
// x86_64-apple-darwin10 -fopenmp -fopenmp-version=45 -fexceptions
// -fcxx-exceptions -debug-info-kind=line-tables-only -x c++ -emit-llvm %s -o -
// | FileCheck %s --check-prefix=TERM_DEBUG

// RUN: %clang_cc1 -verify -fopenmp -DOMP5 -x c++ -triple x86_64-unknown-unknown
// -emit-llvm %s -fexceptions -fcxx-exceptions -o - | FileCheck %s
// --check-prefix=OMP50 --check-prefix=CHECK RUN: %clang_cc1 -fopenmp -DOMP5 -x
// c++ -std=c++11 -triple x86_64-unknown-unknown -fexceptions -fcxx-exceptions
// -emit-pch -o %t %s RUN: %clang_cc1 -fopenmp -DOMP5 -x c++ -triple
...
// %clang_cc1 -verify -triple x86_64-apple-darwin10 -fopenmp -DOMP5 -fexceptions
// -fcxx-exceptions -debug-info-kind=line-tables-only -x c++ -emit-llvm %s -o -
// | FileCheck %s --check-prefix=TERM_DEBUG
...
// -fopenmp-version=45 -x c++ -std=c++11 -triple x86_64-unknown-unknown
// -fexceptions -fcxx-exceptions -emit-pch -o %t %s RUN: %clang_cc1
// -fopenmp-simd -fopenmp-version=45 -x c++ -triple x86_64-unknown-unknown
// -fexceptions -fcxx-exceptions -debug-info-kind=limited -std=c++11
// -include-pch %t -verify %s -emit-llvm -o - | FileCheck --check-prefix
// SIMD-ONLY0 %s RUN: %clang_cc1 -verify -triple x86_64-apple-darwin10
...
// -x c++ -std=c++11 -triple x86_64-unknown-unknown -fexceptions
// -fcxx-exceptions -emit-pch -o %t %s RUN: %clang_cc1 -fopenmp-simd -DOMP5 -x
...
// - | FileCheck --check-prefix SIMD-ONLY0 %s RUN: %clang_cc1 -verify -triple
// x86_64-apple-darwin10 -fopenmp-simd -DOMP5 -fexceptions -fcxx-exceptions
// -debug-info-kind=line-tables-only -x c++ -emit-llvm %s -o - | FileCheck
// --check-prefix SIMD-ONLY0 %s expected-no-diagnostics SIMD-ONLY0-NOT:
// {{__kmpc|__tgt}}
#ifndef HEADER
#define HEADER
...
#pragma omp parallel for simd
  // CHECK: call void @__kmpc_for_static_init_4(ptr {{[^,]+}}, i32 %{{[^,]+}},
  // i32 34, ptr %{{[^,]+}}, ptr [[LB:%[^,]+]], ptr [[UB:%[^,]+]], ptr
  // [[STRIDE:%[^,]+]], i32 1, i32 1) CHECK: [[UB_VAL:%.+]] = load i32, ptr
  // [[UB]], CHECK: [[CMP:%.+]] = icmp sgt i32 [[UB_VAL]], 5 CHECK: br i1
  // [[CMP]], label %[[TRUE:.+]], label %[[FALSE:[^,]+]] CHECK: [[TRUE]]: CHECK:
  // br label %[[SWITCH:[^,]+]] CHECK: [[FALSE]]: CHECK: [[UB_VAL:%.+]] = load
  // i32, ptr [[UB]], CHECK: br label %[[SWITCH]] CHECK: [[SWITCH]]: CHECK:
  // [[UP:%.+]] = phi i32 [ 5, %[[TRUE]] ], [ [[UB_VAL]], %[[FALSE]] ] CHECK:
  // store i32 [[UP]], ptr [[UB]], CHECK: [[LB_VAL:%.+]] = load i32, ptr [[LB]],
...
#pragma omp parallel for simd linear(k : 3) schedule(dynamic)
  ...
  // CHECK: call void @__kmpc_dispatch_init_4(ptr {{.+}}, i32 %{{.+}}, i32
  // {{35|1073741859}}, i32 0, i32 8, i32 1, i32 1) CHECK: [[NEXT:%.+]] = call
  // i32 @__kmpc_dispatch_next_4(ptr {{.+}}, i32 %{{.+}}, ptr %{{.+}}, ptr
  // [[LB:%.+]], ptr [[UB:%.+]], ptr %{{.+}}) CHECK: [[COND:%.+]] = icmp ne i32
  // [[NEXT]], 0 CHECK: br i1 [[COND]], label %[[CONT:.+]], label %[[END:.+]]
  // CHECK: [[CONT]]:
...
#pragma omp parallel for simd linear(lin : get_val()), linear(g_ptr)
  ...
  // CHECK: call void @__kmpc_for_static_init_8u(ptr {{[^,]+}}, i32 %{{[^,]+}},
  // i32 34, ptr %{{[^,]+}}, ptr [[LB:%[^,]+]], ptr [[UB:%[^,]+]], ptr
  // [[STRIDE:%[^,]+]], i64 1, i64 1) CHECK: [[UB_VAL:%.+]] = load i64, ptr
  // [[UB]], CHECK: [[CMP:%.+]] = icmp ugt i64 [[UB_VAL]], 3 CHECK: br i1
  // [[CMP]], label %[[TRUE:.+]], label %[[FALSE:[^,]+]] CHECK: [[TRUE]]: CHECK:
  // br label %[[SWITCH:[^,]+]] CHECK: [[FALSE]]: CHECK: [[UB_VAL:%.+]] = load
  // i64, ptr [[UB]], CHECK: br label %[[SWITCH]] CHECK: [[SWITCH]]: CHECK:
  // [[UP:%.+]] = phi i64 [ 3, %[[TRUE]] ], [ [[UB_VAL]], %[[FALSE]] ] CHECK:
  // store i64 [[UP]], ptr [[UB]], CHECK: [[LB_VAL:%.+]] = load i64, ptr [[LB]],
  ...
  for (unsigned long long it = 2000; it >= 600; it -= 400) {
    ...
    // variable. CHECK: [[LINSTART:.+]] = load i32, ptr [[LIN_START]] CHECK:
    // [[LINSTEP:.+]] = load i64, ptr [[LIN_STEP]] CHECK-NOT: store i32 {{.+}},
    // ptr [[LIN_VAR]] CHECK: store i32 {{.+}}, ptr [[LIN_PRIV:%[^,]+]], CHECK:
    // [[GLINSTART:.+]] = load ptr, ptr [[GLIN_START]] CHECK-NEXT: [[IV3_1:%.+]]
    // = load i64, ptr [[OMP_IV3]] CHECK-NEXT: [[MUL:%.+]] = mul i64 [[IV3_1]],
    // 1 CHECK: [[GEP:%.+]] = getelementptr{{.*}}[[GLINSTART]] CHECK-NEXT: store
    // ptr [[GEP]], ptr [[G_PTR_CUR:%[^,]+]]
    *g_ptr++ = 0.0;
  ...
  // variables. CHECK: [[LIN:%.+]] = load i32, ptr [[LIN_PRIV]] CHECK: store i32
  // [[LIN]], ptr [[LIN_VAR]], CHECK: [[GLIN:%.+]] = load ptr, ptr [[G_PTR_CUR]]
...
#pragma omp parallel for simd
  ...
  // i32 34, ptr %{{[^,]+}}, ptr [[LB:%[^,]+]], ptr [[UB:%[^,]+]], ptr
  // [[STRIDE:%[^,]+]], i32 1, i32 1) CHECK: [[UB_VAL:%.+]] = load i32, ptr
  // [[UB]], CHECK: [[CMP:%.+]] = icmp sgt i32 [[UB_VAL]], 3 CHECK: br i1
  // [[CMP]], label %[[TRUE:.+]], label %[[FALSE:[^,]+]] CHECK: [[TRUE]]: CHECK:
  // br label %[[SWITCH:[^,]+]] CHECK: [[FALSE]]: CHECK: [[UB_VAL:%.+]] = load
  // i32, ptr [[UB]], CHECK: br label %[[SWITCH]] CHECK: [[SWITCH]]: CHECK:
  // [[UP:%.+]] = phi i32 [ 3, %[[TRUE]] ], [ [[UB_VAL]], %[[FALSE]] ] CHECK:
  // store i32 [[UP]], ptr [[UB]], CHECK: [[LB_VAL:%.+]] = load i32, ptr [[LB]],
...
#pragma omp parallel for simd
  ...
  // i32 34, ptr %{{[^,]+}}, ptr [[LB:%[^,]+]], ptr [[UB:%[^,]+]], ptr
  // [[STRIDE:%[^,]+]], i32 1, i32 1) CHECK: [[UB_VAL:%.+]] = load i32, ptr
  // [[UB]], CHECK: [[CMP:%.+]] = icmp sgt i32 [[UB_VAL]], 25 CHECK: br i1
  // [[CMP]], label %[[TRUE:.+]], label %[[FALSE:[^,]+]] CHECK: [[TRUE]]: CHECK:
  // br label %[[SWITCH:[^,]+]] CHECK: [[FALSE]]: CHECK: [[UB_VAL:%.+]] = load
  // i32, ptr [[UB]], CHECK: br label %[[SWITCH]] CHECK: [[SWITCH]]: CHECK:
  // [[UP:%.+]] = phi i32 [ 25, %[[TRUE]] ], [ [[UB_VAL]], %[[FALSE]] ] CHECK:
  // store i32 [[UP]], ptr [[UB]], CHECK: [[LB_VAL:%.+]] = load i32, ptr [[LB]],
...
#pragma omp parallel for simd lastprivate(A)
    // CHECK: call void @__kmpc_for_static_init_8(ptr {{[^,]+}}, i32 %{{[^,]+}},
    // i32 34, ptr %{{[^,]+}}, ptr [[LB:%[^,]+]], ptr [[UB:%[^,]+]], ptr
    // [[STRIDE:%[^,]+]], i64 1, i64 1) CHECK: [[UB_VAL:%.+]] = load i64, ptr
    // [[UB]], CHECK: [[CMP:%.+]] = icmp sgt i64 [[UB_VAL]], 6 CHECK: br i1
    // [[CMP]], label %[[TRUE:.+]], label %[[FALSE:[^,]+]] CHECK: [[TRUE]]:
...
// noundef {{.+}}) CHECK: call void (ptr, i32, ptr, ...) @__kmpc_fork_call(
void inst_templ1() {
  float a;
  float z[100];
  templ1<float, 2>(a, z);
...
// OMP50: call void @__kmpc_for_static_init_8(ptr {{[^,]+}}, i32 %{{[^,]+}}, i32
// 34, ptr %{{[^,]+}}, ptr [[LB:%[^,]+]], ptr [[UB:%[^,]+]], ptr
// [[STRIDE:%[^,]+]], i64 1, i64 1) OMP50: [[UB_VAL:%.+]] = load i64, ptr
// [[UB]], OMP50: [[CMP:%.+]] = icmp sgt i64 [[UB_VAL]], 15 OMP50: br i1
// [[CMP]], label %[[TRUE:.+]], label %[[FALSE:[^,]+]] OMP50: [[TRUE]]: OMP50:
// br label %[[SWITCH:[^,]+]] OMP50: [[FALSE]]: OMP50: [[UB_VAL:%.+]] = load
// i64, ptr [[UB]], OMP50: br label %[[SWITCH]] OMP50: [[SWITCH]]: OMP50:
// [[UP:%.+]] = phi i64 [ 15, %[[TRUE]] ], [ [[UB_VAL]], %[[FALSE]] ] OMP50:
// store i64 [[UP]], ptr [[UB]], OMP50: [[LB_VAL:%.+]] = load i64, ptr [[LB]],
...
#pragma omp parallel for simd
  ...
  // CHECK: call void @__kmpc_for_static_init_4(ptr {{[^,]+}}, i32 %{{[^,]+}},
  // i32 34, ptr %{{[^,]+}}, ptr [[LB:%[^,]+]], ptr [[UB:%[^,]+]], ptr
  // [[STRIDE:%[^,]+]], i32 1, i32 1) CHECK-DAG: [[UB_VAL:%.+]] = load i32, ptr
  // [[UB]], CHECK-DAG: [[OMP_LAST_IT_VAL:%.+]] = load i32, ptr [[OMP_LAST_IT]],
  ...
  // CHECK: br label %[[SWITCH]]
  // CHECK: [[SWITCH]]:
  // CHECK: [[UP:%.+]] = phi i32 [ [[OMP_LAST_IT_VAL]], %[[TRUE]] ], [
  // [[UB_VAL]], %[[FALSE]] ] CHECK: store i32 [[UP]], ptr [[UB]], CHECK:
  // [[LB_VAL:%.+]] = load i32, ptr [[LB]], CHECK: store i32 [[LB_VAL]], ptr
  // [[IT_OMP_IV:%[^,]+]],

  // CHECK: [[IV:%.+]] = load i32, ptr [[IT_OMP_IV]]
  // CHECK-NEXT: [[UB_VAL:%.+]] = load i32, ptr [[UB]]
  // CHECK-NEXT: [[CMP:%.+]] = icmp sle i32 [[IV]], [[UB_VAL]]
  // CHECK-NEXT: br i1 [[CMP]], label %[[IT_BODY:[^,]+]], label
  // %[[IT_END:[^,]+]]
...
void collapsed(float *a, float *b, float *c, float *d) {
  ...
  // i32 34, ptr %{{[^,]+}}, ptr [[LB:%[^,]+]], ptr [[UB:%[^,]+]], ptr
  // [[STRIDE:%[^,]+]], i32 1, i32 1) CHECK: [[UB_VAL:%.+]] = load i32, ptr
  // [[UB]], CHECK: [[CMP:%.+]] = icmp ugt i32 [[UB_VAL]], 119 CHECK: br i1
  // [[CMP]], label %[[TRUE:.+]], label %[[FALSE:[^,]+]] CHECK: [[TRUE]]: CHECK:
  // br label %[[SWITCH:[^,]+]] CHECK: [[FALSE]]: CHECK: [[UB_VAL:%.+]] = load
  // i32, ptr [[UB]], CHECK: br label %[[SWITCH]] CHECK: [[SWITCH]]: CHECK:
  // [[UP:%.+]] = phi i32 [ 119, %[[TRUE]] ], [ [[UB_VAL]], %[[FALSE]] ] CHECK:
  // store i32 [[UP]], ptr [[UB]], CHECK: [[LB_VAL:%.+]] = load i32, ptr [[LB]],
  // CHECK: store i32 [[LB_VAL]], ptr [[OMP_IV:%[^,]+]],
  //
...
void widened(float *a, float *b, float *c, float *d) {
  ...
  // CHECK: call void @__kmpc_for_static_init_8(ptr {{[^,]+}}, i32 %{{[^,]+}},
  // i32 34, ptr %{{[^,]+}}, ptr [[LB:%[^,]+]], ptr [[UB:%[^,]+]], ptr
  // [[STRIDE:%[^,]+]], i64 1, i64 1) CHECK-DAG: [[UB_VAL:%.+]] = load i64, ptr
  // [[UB]], CHECK-DAG: [[OMP_LAST_IT_VAL:%.+]] = load i64, ptr [[OMP_LAST_IT]],
  ...
  // CHECK: br label %[[SWITCH]]
  // CHECK: [[SWITCH]]:
  // CHECK: [[UP:%.+]] = phi i64 [ [[OMP_LAST_IT_VAL]], %[[TRUE]] ], [
  // [[UB_VAL]], %[[FALSE]] ] CHECK: store i64 [[UP]], ptr [[UB]], CHECK:
  // [[LB_VAL:%.+]] = load i64, ptr [[LB]], CHECK: store i64 [[LB_VAL]], ptr
  // [[OMP_IV:%[^,]+]],
  //
...
// OMP45: call void @__kmpc_for_static_init_8(ptr {{[^,]+}}, i32 %{{[^,]+}}, i32
// 34, ptr %{{[^,]+}}, ptr [[LB:%[^,]+]], ptr [[UB:%[^,]+]], ptr
// [[STRIDE:%[^,]+]], i64 1, i64 1) OMP45: [[UB_VAL:%.+]] = load i64, ptr
// [[UB]], OMP45: [[CMP:%.+]] = icmp sgt i64 [[UB_VAL]], 15 OMP45: br i1
// [[CMP]], label %[[TRUE:.+]], label %[[FALSE:[^,]+]] OMP45: [[TRUE]]: OMP45:
// br label %[[SWITCH:[^,]+]] OMP45: [[FALSE]]: OMP45: [[UB_VAL:%.+]] = load
// i64, ptr [[UB]], OMP45: br label %[[SWITCH]] OMP45: [[SWITCH]]: OMP45:
// [[UP:%.+]] = phi i64 [ 15, %[[TRUE]] ], [ [[UB_VAL]], %[[FALSE]] ] OMP45:
// store i64 [[UP]], ptr [[UB]], OMP45: [[LB_VAL:%.+]] = load i64, ptr [[LB]],
...
// OMP50: %{{[0-9]+}} = atomicrmw add ptr %{{[0-9]+}}, i32 1 monotonic, align 4,
