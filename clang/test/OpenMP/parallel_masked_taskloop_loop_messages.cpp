...
// expected-error@+2 {{loop iteration variable in the associated loop of 'omp parallel masked taskloop' directive may not be firstprivate, predetermined as private}}
#pragma omp parallel masked taskloop firstprivate(ii)
  for (ii = 0; ii < 10; ii++)
    c[ii] = a[ii];

#pragma omp parallel
// expected-error@+1 {{unexpected OpenMP clause 'linear' in directive '#pragma omp parallel masked taskloop'}}
#pragma omp parallel masked taskloop linear(ii)
  for (ii = 0; ii < 10; ii++)
    c[ii] = a[ii];

#pragma omp parallel
#pragma omp parallel masked taskloop private(ii)
  for (ii = 0; ii < 10; ii++)
    c[ii] = a[ii];

#pragma omp parallel
#pragma omp parallel masked taskloop lastprivate(ii)
  for (ii = 0; ii < 10; ii++)
    c[ii] = a[ii];

#pragma omp parallel
  {
// expected-error@+2 {{loop iteration variable in the associated loop of 'omp parallel masked taskloop' directive may not be threadprivate or thread local, predetermined as private}}
#pragma omp parallel masked taskloop
...
#pragma omp parallel
// omp4-error@+2 {{statement after '#pragma omp parallel masked taskloop' must be a for loop}}
#pragma omp parallel masked taskloop
  for (auto &item : a) {
    item = item + 1;
  }
...
void test_loop_firstprivate_lastprivate() {
  ...
  int c;
#pragma omp parallel
#pragma omp parallel masked taskloop lastprivate(s) firstprivate(s)
  for (int i = 0; i < 16; ++i)
    ;
#pragma omp parallel masked taskloop if(c) default(none) // expected-error {{variable 'c' must have explicitly specified data sharing attributes}} expected-note {{explicit data sharing attribute requested here}}
  for (int i = 0; i < 16; ++i)
    ;
#pragma omp parallel masked taskloop if(taskloop:c) default(none) // expected-error {{variable 'c' must have explicitly specified data sharing attributes}} expected-note {{explicit data sharing attribute requested here}}
  for (int i = 0; i < 16; ++i)
    ;
#pragma omp parallel masked taskloop if(parallel:c) default(none)
  for (int i = 0; i < 16; ++i)
    ;
}
