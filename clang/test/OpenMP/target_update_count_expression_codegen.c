...
// RUN: %clang_cc1 -fopenmp -fopenmp-enable-irbuilder -x c -triple x86_64-unknown-unknown -fopenmp-targets=amdgpu-amd-amdhsa -emit-pch -o %t %s
// RUN: %clang_cc1 -fopenmp -fopenmp-enable-irbuilder -x c -triple x86_64-unknown-unknown -fopenmp-targets=amdgpu-amd-amdhsa -include-pch %t -emit-llvm %s -o - \
// RUN: | FileCheck %s
