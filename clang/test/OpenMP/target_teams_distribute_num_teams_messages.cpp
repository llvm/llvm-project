...
#pragma omp target teams distribute num_teams(1, 2, 3) // expected-error {{unexpected number of expressions in 'num_teams' clause}}
  for (int i=0; i<100; i++) foo();

#pragma omp target teams distribute thread_limit(1, 2, 3) // expected-error {{unexpected number of expressions in 'thread_limit' clause}}
  for (int i=0; i<100; i++) foo();

#pragma omp target teams ompx_bare num_teams(1, 2, 3, 4) thread_limit(1) // expected-error {{unexpected number of expressions in 'num_teams' clause}}
  for (int i=0; i<100; i++) foo();

#pragma omp target teams ompx_bare num_teams(1) thread_limit(1, 2, 3, 4) // expected-error {{unexpected number of expressions in 'thread_limit' clause}}
  for (int i=0; i<100; i++) foo();
