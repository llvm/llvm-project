...
#pragma omp target teams distribute simd nowait device (-10u)
  for (i = 0; i < argc; ++i) foo();
#pragma omp target teams distribute simd nowait (3.14) device (-10u) // expected-error {{arguments of OpenMP clause 'nowait' with bitwise operators cannot be of floating type}}
  for (i = 0; i < argc; ++i) foo();
