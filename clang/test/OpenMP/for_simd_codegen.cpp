...
// FileCheck %s --check-prefix=CHECK --check-prefix=OMP45 RUN: %clang_cc1
...
// -emit-llvm -o - < %s | FileCheck %s --check-prefix=CHECK --check-prefix=OMP45
// RUN: %clang_cc1 -verify -triple x86_64-apple-darwin10 -fopenmp
// -fopenmp-version=45 -fexceptions -fcxx-exceptions
// -debug-info-kind=line-tables-only -x c++ -emit-llvm -o - < %s | FileCheck %s
// --check-prefix=TERM_DEBUG RUN: %clang_cc1 -verify -fopenmp -x c++ -triple
// x86_64-unknown-unknown -emit-llvm -fexceptions -fcxx-exceptions -o - < %s
// -DOMP5 | FileCheck %s --check-prefix=CHECK --check-prefix=OMP50 RUN:
// %clang_cc1 -fopenmp -x c++ -std=c++11 -triple x86_64-unknown-unknown
// -fexceptions -fcxx-exceptions -emit-pch -o %t < %s -DOMP5 RUN: %clang_cc1
// -fopenmp -x c++ -triple x86_64-unknown-unknown -fexceptions -fcxx-exceptions
// -debug-info-kind=limited -std=c++11 -include-pch %t -verify -emit-llvm -o -
// -DOMP5 < %s | FileCheck %s --check-prefix=CHECK --check-prefix=OMP50 RUN:
// %clang_cc1 -verify -triple x86_64-apple-darwin10 -fopenmp -fexceptions
// -fcxx-exceptions -debug-info-kind=line-tables-only -x c++ -emit-llvm -o - <
// %s -DOMP5 | FileCheck %s --check-prefix=TERM_DEBUG
...
// -include-pch %t -verify -emit-llvm -o - < %s | FileCheck --check-prefix
...
// -debug-info-kind=line-tables-only -x c++ -emit-llvm -o - < %s | FileCheck
// --check-prefix SIMD-ONLY0 %s RUN: %clang_cc1 -verify -fopenmp-simd -x c++
// -triple x86_64-unknown-unknown -emit-llvm -fexceptions -fcxx-exceptions -o -
// < %s -DOMP5 | FileCheck --check-prefix SIMD-ONLY0 %s RUN: %clang_cc1
// -fopenmp-simd -x c++ -std=c++11 -triple x86_64-unknown-unknown -fexceptions
// -fcxx-exceptions -emit-pch -o %t < %s -DOMP5 RUN: %clang_cc1 -fopenmp-simd -x
// c++ -triple x86_64-unknown-unknown -fexceptions -fcxx-exceptions
// -debug-info-kind=limited -std=c++11 -include-pch %t -verify -DOMP5 -emit-llvm
// -o - < %s | FileCheck --check-prefix SIMD-ONLY0 %s RUN: %clang_cc1 -verify
// -triple x86_64-apple-darwin10 -fopenmp-simd -fexceptions -fcxx-exceptions
// -debug-info-kind=line-tables-only -x c++ -emit-llvm -o - < %s -DOMP5 |
...
#pragma omp for simd linear(k : 3) schedule(simd, nonmonotonic : dynamic)
  ...
  // 1073741859, i32 0, i32 8, i32 1, i32 1) CHECK: [[NEXT:%.+]] = call i32
  // @__kmpc_dispatch_next_4(ptr {{.+}}, i32 %{{.+}}, ptr %{{.+}}, ptr
  ...
  // store i64 [[LIN0_2]], ptr [[K_VAR]] CHECK: call void @__kmpc_barrier(ptr
  // {{.+}}, i32 %{{.+}})
...
#pragma omp for simd linear(lin : get_val()), linear(g_ptr)
  ...
  // [[G_PTR_CUR]], CHECK-NEXT: store ptr [[GLIN]], ptr [[GLIN_VAR]], CHECK:
  // call void @__kmpc_barrier(ptr {{.+}}, i32 %{{.+}})
...
#pragma omp for simd reduction(* : R)
    ...
    for (long long i = -10; i < 10; i += 3) {
      ...
      // CHECK-NEXT: br label {{%?}}[[SIMD_LOOP8_COND]], {{.*}}!llvm.loop
      // ![[SIMD_LOOP:.+]]
    }
    // CHECK: [[SIMPLE_LOOP8_END]]:
    // OMP50:  br label {{%?}}[[IF_EXIT:[^,]+]]
    // OMP50:  [[ELSE]]:
    // OMP50: call void @__kmpc_for_static_init_8(ptr {{[^,]+}}, i32 %{{[^,]+}},
    // i32 34, ptr %{{[^,]+}}, ptr [[LB:%[^,]+]], ptr [[UB:%[^,]+]], ptr
    // [[STRIDE:%[^,]+]], i64 1, i64 1) OMP50: [[UB_VAL:%.+]] = load i64, ptr
    // [[UB]], OMP50: [[CMP:%.+]] = icmp sgt i64 [[UB_VAL]], 6 OMP50: br i1
    // [[CMP]], label %[[TRUE:.+]], label %[[FALSE:[^,]+]] OMP50: [[TRUE]]:
    ...
    // %[[SIMPLE_LOOP8_END:[^,]+]] OMP50: [[SIMPLE_LOOP8_BODY]]: Start of body:
    // calculate i from IV: OMP50: [[IV8_0:%.+]] = load i64, ptr [[OMP_IV8]]
    ...
    // OMP50-NEXT: br label {{%?}}[[SIMD_LOOP8_COND]], {{.*}}!llvm.loop
    // ![[NOSIMD_LOOP:.+]] OMP50: [[SIMPLE_LOOP8_END]]: OMP50:  br label
    // {{%?}}[[IF_EXIT]] OMP50:  [[IF_EXIT]]:
...
// noundef {{.+}}) CHECK: call void @__kmpc_for_static_init_8(ptr {{[^,]+}}, i32
// %{{[^,]+}}, i32 45, ptr %{{[^,]+}}, ptr [[LB:%[^,]+]], ptr [[UB:%[^,]+]], ptr
// [[STRIDE:%[^,]+]], i64 1, i64 2) CHECK: [[UB_VAL:%.+]] = load i64, ptr
// [[UB]], CHECK: [[CMP:%.+]] = icmp sgt i64 [[UB_VAL]], 15 CHECK: br i1
// [[CMP]], label %[[TRUE:.+]], label %[[FALSE:[^,]+]] CHECK: [[TRUE]]: CHECK:
// br label %[[SWITCH:[^,]+]] CHECK: [[FALSE]]: CHECK: [[UB_VAL:%.+]] = load
// i64, ptr [[UB]], CHECK: br label %[[SWITCH]] CHECK: [[SWITCH]]: CHECK:
// [[UP:%.+]] = phi i64 [ 15, %[[TRUE]] ], [ [[UB_VAL]], %[[FALSE]] ] CHECK:
// store i64 [[UP]], ptr [[UB]], CHECK: [[LB_VAL:%.+]] = load i64, ptr [[LB]],
...
// CHECK-DAG: ![[SIMD_LOOP]] = distinct !{![[SIMD_LOOP]],
// {{.*}}![[VECT_LOOP:[^,]+]]} CHECK-DAG: ![[VECT_LOOP]] =
// !{!"llvm.loop.vectorize.enable"} OMP45-NOT: !"llvm.loop.vectorize.disable"
// OMP50-DAG: ![[NOSIMD_LOOP]] = distinct !{![[NOSIMD_LOOP]],
// {{.*}}![[NOVECT_LOOP:[^,]+]]} OMP50-DAG: ![[NOVECT_LOOP]] =
