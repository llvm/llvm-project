...
#pragma omp target
#pragma omp teams
#pragma omp distribute parallel for reduction(- : da) // expected-error 2 {{const-qualified variable cannot be reduction}} omp52-warning 3 {{minus(-) operator for reductions is deprecated; use + or user defined reduction instead}}
  for (int i = 0; i < 10; ++i)
    foo();
#pragma omp target
#pragma omp teams
...
#pragma omp target
#pragma omp teams
#pragma omp distribute parallel for reduction(- : da) // expected-error {{const-qualified variable cannot be reduction}} omp52-warning {{minus(-) operator for reductions is deprecated; use + or user defined reduction instead}}
  for (int i = 0; i < 10; ++i)
    foo();
#pragma omp target
#pragma omp teams
...
#pragma omp target
#pragma omp teams
#pragma omp distribute parallel for reduction(task, + : m) // omp45-error 2 {{expected expression}} omp45-warning {{missing ':' after reduction identifier - ignoring}}
  for (int i = 0; i < 10; ++i)
    m++;
