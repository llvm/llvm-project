...
// so the upper bound is materialized as a `min()` (cond.true/cond.false/phi over
// the two `.tile_*.iv`/`.floor_*.iv` counters). There is no `.tile.cnt`
// rectangular counter and no body-guard predicate here; those only appear when a
...
// Nested loop in the tile body: the tiled `i` dim is the floor/tile pair in
// min-bounded form; the inner `j` loop stays a plain nested loop.
...
// Exact division (trip count 12, tile 4): a standalone tile still emits the
// min() cond-phi upper bound (it is always present, regardless of divisibility)
// and never a body-guard predicate.
