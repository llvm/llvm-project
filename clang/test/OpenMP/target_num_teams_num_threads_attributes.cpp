...
// -fopenmp -x c++ -std=c++11 -triple amdgpu9.00-amd-amdhsa
// -fopenmp-targets=amdgpu-amd-amdhsa -emit-llvm %s -fopenmp-is-target-device
// -fopenmp-host-ir-file-path %t-ppc-host.bc -o - | FileCheck %s RUN: %clang_cc1
// -fopenmp -x c++ -std=c++11 -triple x86_64-unknown-unknown
// -fopenmp-targets=nvptx64 -emit-llvm-bc %s -o %t-ppc-host.bc RUN: %clang_cc1
// -fopenmp -x c++ -std=c++11 -triple nvptx64 -fopenmp-targets=nvptx64
// -emit-llvm %s -fopenmp-is-target-device -fopenmp-host-ir-file-path
// %t-ppc-host.bc -o - | FileCheck %s RUN: %clang_cc1 -target-cpu sm_80 -fopenmp
// -x c++ -std=c++11 -triple nvptx64 -fopenmp-targets=nvptx64 -emit-llvm %s
// -fopenmp-is-target-device -fopenmp-host-ir-file-path %t-ppc-host.bc -o - |
// FileCheck %s RUN: %clang_cc1 -fopenmp -x c++ -std=c++11 -triple
// x86_64-unknown-unknown -fopenmp-targets=spirv64 -emit-llvm-bc %s -o
// %t-ppc-host.bc RUN: %clang_cc1 -fopenmp -x c++ -std=c++11 -triple spirv64
...
void num_threads() {
#pragma omp target teams distribute parallel for thread_limit(22)              \
    num_threads(11)
  for (int i = 0; i < 22; i++)
    int a_var;
}
