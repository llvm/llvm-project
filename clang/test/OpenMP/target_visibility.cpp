...
// -fopenmp-is-target-device -o - | FileCheck %s RUN: %clang_cc1
// -debug-info-kind=limited -verify -fopenmp -x c++ -triple amdgpu-amd-amdhsa
// -fopenmp-targets=amdgpu-amd-amdhsa -emit-llvm %s -fopenmp-is-target-device -o
// - | FileCheck %s RUN: %clang_cc1 -debug-info-kind=limited -verify -fopenmp -x
// c++ -triple spirv64 -fopenmp-targets=spirv64 -emit-llvm %s
// -fopenmp-is-target-device -o - | FileCheck %s expected-no-diagnostics
