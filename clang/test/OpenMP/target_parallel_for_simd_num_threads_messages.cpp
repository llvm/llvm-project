...
T tmain(T argc, S **argv) {
  ...
  for (i = 0; i < argc; ++i) foo();
  #pragma omp target parallel for simd num_threads (argv[1]=2) // expected-error 2 {{incompatible integer to pointer conversion assigning to 'char *' from 'int'}}
  for (i = 0; i < argc; ++i) foo();
...
int main(int argc, char **argv) {
  ...
  for (i = 0; i < argc; ++i) foo();
  #pragma omp target parallel for simd num_threads (argv[1]=2) // expected-error {{incompatible integer to pointer conversion assigning to 'char *' from 'int'}}
  for (i = 0; i < argc; ++i) foo();
