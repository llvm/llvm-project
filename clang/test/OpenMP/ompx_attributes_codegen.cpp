...
// --check-prefix=AMD RUN: %clang_cc1 -fopenmp -x c++ -std=c++11 -triple
// amdgpu9.00-amd-amdhsa -fopenmp-targets=amdgpu-amd-amdhsa -emit-llvm %s
// -fopenmp-is-target-device -fopenmp-host-ir-file-path %t-ppc-host.bc -o - |
// FileCheck %s --check-prefix=AMD RUN: %clang_cc1 -fopenmp -x c++ -std=c++11
// -triple amdgpu9.00-amd-amdhsa -fopenmp-targets=amdgpu-amd-amdhsa
// -dwarf-version=5 -emit-llvm %s -fopenmp-is-target-device
// -fopenmp-host-ir-file-path %t-ppc-host.bc -o - | FileCheck %s
// --check-prefix=AMD RUN: %clang_cc1 -fopenmp -x c++ -std=c++11 -triple nvptx64
// -fopenmp-targets=nvptx64 -emit-llvm %s -fopenmp-is-target-device
// -fopenmp-host-ir-file-path %t-ppc-host.bc -o - | FileCheck %s
// --check-prefix=NVIDIA RUN: %clang_cc1 -fopenmp -x c++ -std=c++11 -triple
// nvptx64 -fopenmp-targets=nvptx64 -emit-llvm %s -fopenmp-is-target-device
// -dwarf-version=5 -fopenmp-host-ir-file-path %t-ppc-host.bc -o - | FileCheck
// %s --check-prefix=NVIDIA RUN: %clang_cc1 -fopenmp -x c++ -std=c++11 -triple
// spirv64 -fopenmp-targets=spirv64 -emit-llvm %s -fopenmp-is-target-device
// -fopenmp-host-ir-file-path %t-ppc-host.bc -o - | FileCheck %s
// --check-prefix=SPIRV RUN: %clang_cc1 -fopenmp -x c++ -std=c++11 -triple
// spirv64 -fopenmp-targets=spirv64 -emit-llvm %s -fopenmp-is-target-device
// -dwarf-version=5 -fopenmp-host-ir-file-path %t-ppc-host.bc -o - | FileCheck
// %s --check-prefix=SPIRV expected-no-diagnostics
...
void func() {
  ...
  // #[[ATTR0:[0-9]+]] NVIDIA: ptx_kernel void
  // @__omp_offloading[[HASH:.*]]_l30(ptr {{[^,]+}}) #[[ATTR1:[0-9]+]] NVIDIA:
  // ptx_kernel void @__omp_offloading[[HASH:.*]]_l32(ptr {{[^,]+}})
  // #[[ATTR2:[0-9]+]]
...
#pragma omp target ompx_attribute(                                             \
        [[clang::amdgpu_flat_work_group_size(10, 20)]])
  {}
...
#pragma omp target teams distribute parallel for simd ompx_attribute(          \
        [[clang::amdgpu_flat_work_group_size(3, 17)]]) device(3)               \
    ompx_attribute(__attribute__((amdgpu_waves_per_eu(3, 7))))
...
// It is unclear if we should use the AMD annotations for other targets, we do
// for now. NVIDIA: attributes #[[ATTR0]] NVIDIA-SAME: "nvvm.maxntid"="20"
