...
// CHECK-DAG: [[CHECK_ZERO_ARG_TY:%[a-zA-Z0-9_.]+]] = type { i32, i32 }
...
int CheckZeroArg() {
  // size passed to printf_alloc (Hello, world + \0) 13 bytes + 4 bytes + 4 bytes = 21

  // CHECK: [[PTR0:%[a-zA-Z0-9_]+]] = call i8* @printf_alloc(i32 21)
  // CHECK: [[ARGS_CAST:%[a-zA-Z0-9_]+]] = addrspacecast i8* [[PTR0]] to [[CHECK_ZERO_ARG_TY]] addrspace(1)*
  #pragma omp target
  {
    // CHECK: [[PTR1:%[0-9]+]] = getelementptr inbounds [[CHECK_ZERO_ARG_TY]], [[CHECK_ZERO_ARG_TY]] addrspace(1)* [[ARGS_CAST]], i32 0, i32 0
    // CHECK: store i32 8, i32 addrspace(1)* [[PTR1]]
    ...
    // CHECK: [[PTR4:%[0-9]+]] = getelementptr inbounds i8, i8 addrspace(1)* [[PTR3]], i64 8
    ...
    // CHECK: [[PTR6:%[0-9]+]] = call i32 @printf_execute(i8* [[PTR0]], i32 21)
    printf("Hello, world");
  }
