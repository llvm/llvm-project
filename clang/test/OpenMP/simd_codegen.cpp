...
// -emit-llvm %s -fexceptions -fcxx-exceptions -o - -fopenmp-version=45 |
// FileCheck %s RUN: %clang_cc1 -fopenmp -x c++ -std=c++11 -triple
// x86_64-unknown-unknown -fexceptions -fcxx-exceptions -emit-pch -o %t %s
// -fopenmp-version=45 RUN: %clang_cc1 -fopenmp -x c++ -triple
// x86_64-unknown-unknown -fexceptions -fcxx-exceptions -debug-info-kind=limited
// -std=c++11 -include-pch %t -verify %s -emit-llvm -o - -fopenmp-version=45 |
// FileCheck %s RUN: %clang_cc1 -verify -triple x86_64-apple-darwin10 -fopenmp
// -fexceptions -fcxx-exceptions -debug-info-kind=line-tables-only -x c++
// -emit-llvm %s -o - -fopenmp-version=45 | FileCheck %s
// --check-prefix=TERM_DEBUG RUN: %clang_cc1 -verify -fopenmp -x c++ -triple
// x86_64-unknown-unknown -emit-llvm %s -fexceptions -fcxx-exceptions -o -
// -fopenmp-version=50 -DOMP5 | FileCheck %s --check-prefix=CHECK
// --check-prefix=OMP5 --check-prefix=OMP5RT RUN: %clang_cc1 -fopenmp -x c++
// -std=c++11 -triple x86_64-unknown-unknown -fexceptions -fcxx-exceptions
// -emit-pch -o %t %s -fopenmp-version=50 -DOMP5 RUN: %clang_cc1 -fopenmp -x c++
...
// - -fopenmp-version=50 -DOMP5 | FileCheck %s --check-prefix=CHECK
// --check-prefix=OMP5 --check-prefix=OMP5RT

// RUN: %clang_cc1 -verify -fopenmp-simd -x c++ -triple x86_64-unknown-unknown
// -emit-llvm %s -fexceptions -fcxx-exceptions -o - -fopenmp-version=45 |
// FileCheck %s RUN: %clang_cc1 -fopenmp-simd -x c++ -std=c++11 -triple
// x86_64-unknown-unknown -fexceptions -fcxx-exceptions -emit-pch -o %t %s
// -fopenmp-version=45 RUN: %clang_cc1 -fopenmp-simd -x c++ -triple
// x86_64-unknown-unknown -fexceptions -fcxx-exceptions -debug-info-kind=limited
// -std=c++11 -include-pch %t -verify %s -emit-llvm -o - -fopenmp-version=45 |
// FileCheck %s RUN: %clang_cc1 -verify -triple x86_64-apple-darwin10
// -fopenmp-simd -fexceptions -fcxx-exceptions -debug-info-kind=line-tables-only
// -x c++ -emit-llvm %s -o - -fopenmp-version=45 | FileCheck
// --check-prefix=TERM_DEBUG %s RUN: %clang_cc1 -verify -fopenmp-simd -x c++
// -triple x86_64-unknown-unknown -emit-llvm %s -fexceptions -fcxx-exceptions -o
// - -fopenmp-version=50 -DOMP5 | FileCheck %s --check-prefix=CHECK
// --check-prefix=OMP5 RUN: %clang_cc1 -fopenmp-simd -x c++ -std=c++11 -triple
// x86_64-unknown-unknown -fexceptions -fcxx-exceptions -emit-pch -o %t %s
// -fopenmp-version=50 -DOMP5 RUN: %clang_cc1 -fopenmp-simd -x c++ -triple
// x86_64-unknown-unknown -fexceptions -fcxx-exceptions -debug-info-kind=limited
// -std=c++11 -include-pch %t -verify %s -emit-llvm -o - -fopenmp-version=50
// -DOMP5 | FileCheck %s --check-prefix=CHECK --check-prefix=OMP5
...
// CHECK-LABEL: define {{.*void}} @{{.*}}simple{{.*}}(ptr noundef {{.+}}, ptr
// noundef {{.+}}, ptr noundef {{.+}}, ptr noundef {{.+}})
...
#pragma omp simd
#endif
  // CHECK: store i32 0, ptr [[OMP_IV:%[^,]+]]

  // CHECK: [[IV:%.+]] = load i32, ptr [[OMP_IV]]{{.*}}!llvm.access.group
  ...
  for (int i = 3; i < 32; i += 5) {
    ...
    // CHECK: [[IV1_1:%.+]] = load i32, ptr [[OMP_IV]]{{.*}}!llvm.access.group
    // CHECK: [[CALC_I_1:%.+]] = mul nsw i32 [[IV1_1]], 5
    // CHECK-NEXT: [[CALC_I_2:%.+]] = add nsw i32 3, [[CALC_I_1]]
    // CHECK-NEXT: store i32 [[CALC_I_2]], ptr
    // [[LC_I:.+]]{{.*}}!llvm.access.group
    // ... loop body ...
    ...
    // CHECK: [[IV1_2:%.+]] = load i32, ptr [[OMP_IV]]{{.*}}!llvm.access.group
    // CHECK-NEXT: [[ADD1_2:%.+]] = add nsw i32 [[IV1_2]], 1
    // CHECK-NEXT: store i32 [[ADD1_2]], ptr [[OMP_IV]]{{.*}}!llvm.access.group
    // br label %{{.+}}, !llvm.loop !{{.+}}
  }
...
#pragma omp simd linear(k : 3)
  ...
  for (int i = 10; i > 1; i--) {
    ...
    // FIXME: It is interesting, why the following "mul 1" was not constant
    // folded? CHECK-NEXT: [[IV2_1:%.+]] = mul nsw i32 [[IV2_0]], 1 CHECK-NEXT:
    // [[LC_I_1:%.+]] = sub nsw i32 10, [[IV2_1]] CHECK-NEXT: store i32
    // [[LC_I_1]], ptr {{.+}}, !llvm.access.group
    //
    // CHECK-NEXT: [[LIN0_1:%.+]] = load i64, ptr
    // [[LIN0]]{{.*}}!llvm.access.group CHECK-NEXT: [[IV2_2:%.+]] = load i32,
    // ptr [[OMP_IV2]]{{.*}}!llvm.access.group CHECK-NEXT: [[LIN_MUL1:%.+]] =
    // mul nsw i32 [[IV2_2]], 3 CHECK-NEXT: [[LIN_EXT1:%.+]] = sext i32
    // [[LIN_MUL1]] to i64 CHECK-NEXT: [[LIN_ADD1:%.+]] = add nsw i64
    // [[LIN0_1]], [[LIN_EXT1]] Update of the privatized version of linear
    // variable! CHECK-NEXT: store i64 [[LIN_ADD1]], ptr [[K_PRIVATIZED:%[^,]+]]
    a[k]++;
    k = k + 3;
  ...
  // version. CHECK: [[LIN0_2:%.+]] = load i64, ptr [[K_PRIVATIZED]],
...
#pragma omp simd linear(lin : get_val()), linear(g_ptr)
  ...
  // CHECK: [[IV3:%.+]] = load i64, ptr [[OMP_IV3]]{{.*}}!llvm.access.group
  ...
  for (unsigned long long it = 2000; it >= 600; it -= 400) {
    ...
    // CHECK: [[IV3_0:%.+]] = load i64, ptr [[OMP_IV3]]{{.*}}!llvm.access.group
    // CHECK-NEXT: [[LC_IT_1:%.+]] = mul i64 [[IV3_0]], 400
    // CHECK-NEXT: [[LC_IT_2:%.+]] = sub i64 2000, [[LC_IT_1]]
    // CHECK-NEXT: store i64 [[LC_IT_2]], ptr {{.+}}, !llvm.access.group
    //
    // Linear start and step are used to calculate current value of the linear
    // variable. CHECK: [[LINSTART:.+]] = load i32, ptr
    // [[LIN_START]]{{.*}}!llvm.access.group CHECK: [[LINSTEP:.+]] = load i64,
    // ptr [[LIN_STEP]]{{.*}}!llvm.access.group CHECK-NOT: store i32 {{.+}}, ptr
    // [[LIN_VAR]],{{.*}}!llvm.access.group CHECK: store i32 {{.+}}, ptr
    // [[LIN_PRIV:%.+]],{{.*}}!llvm.access.group CHECK: [[GLINSTART:.+]] = load
    // ptr, ptr [[GLIN_START]]{{.*}}!llvm.access.group CHECK-NEXT: [[IV3_1:%.+]]
    // = load i64, ptr [[OMP_IV3]]{{.*}}!llvm.access.group CHECK-NEXT:
    // [[MUL:%.+]] = mul i64 [[IV3_1]], 1 CHECK: [[GEP:%.+]] =
    // getelementptr{{.*}}[[GLINSTART]] CHECK-NEXT: store ptr [[GEP]], ptr
    // [[G_PTR_CUR:%[^,]+]]{{.*}}!llvm.access.group
    *g_ptr++ = 0.0;
    // CHECK: [[GEP_VAL:%.+]] = load ptr, ptr
    // [[G_PTR_CUR]]{{.*}}!llvm.access.group CHECK: store
    // double{{.*}}[[GEP_VAL]]{{.*}}!llvm.access.group
    a[it + lin]++;
    // CHECK: [[FLT_INC:%.+]] = fadd float
    // CHECK-NEXT: store float [[FLT_INC]],{{.*}}!llvm.access.group
    // CHECK: [[IV3_2:%.+]] = load i64, ptr [[OMP_IV3]]{{.*}}!llvm.access.group
    // CHECK-NEXT: [[ADD3_2:%.+]] = add i64 [[IV3_2]], 1
    // CHECK-NEXT: store i64 [[ADD3_2]], ptr [[OMP_IV3]]{{.*}}!llvm.access.group
  }
  // CHECK: [[SIMPLE_LOOP3_END]]:
  //
  // Linear start and step are used to calculate final value of the linear
  // variables. CHECK: [[LIN:%.+]] = load i32, ptr [[LIN_PRIV]] CHECK-NEXT:
  // store i32 [[LIN]], ptr [[LIN_VAR]], CHECK: [[GLIN:%.+]] = load ptr, ptr
  // [[G_PTR_CUR]] CHECK-NEXT: store ptr [[GLIN]], ptr [[GLIN_VAR]]

#pragma omp simd
  // CHECK: store i32 0, ptr [[OMP_IV4:%[^,]+]]

  // CHECK: [[IV4:%.+]] = load i32, ptr [[OMP_IV4]]{{.*}}!llvm.access.group
  // CHECK-NEXT: [[CMP4:%.+]] = icmp slt i32 [[IV4]], 4
  // CHECK-NEXT: br i1 [[CMP4]], label %[[SIMPLE_LOOP4_BODY:.+]], label
  // %[[SIMPLE_LOOP4_END:[^,]+]]
  for (short it = 6; it <= 20; it -= -4) {
    // CHECK: [[SIMPLE_LOOP4_BODY]]:
    // Start of body: calculate it from IV:
    // CHECK: [[IV4_0:%.+]] = load i32, ptr [[OMP_IV4]]{{.*}}!llvm.access.group
    ...
    // CHECK-NEXT: store i16 [[LC_IT_3]], ptr {{.+}}, !llvm.access.group

    // CHECK: [[IV4_2:%.+]] = load i32, ptr [[OMP_IV4]]{{.*}}!llvm.access.group
    // CHECK-NEXT: [[ADD4_2:%.+]] = add nsw i32 [[IV4_2]], 1
    // CHECK-NEXT: store i32 [[ADD4_2]], ptr [[OMP_IV4]]{{.*}}!llvm.access.group
  }
  // CHECK: [[SIMPLE_LOOP4_END]]:

#pragma omp simd
  // CHECK: store i32 0, ptr [[OMP_IV5:%[^,]+]]

  // CHECK: [[IV5:%.+]] = load i32, ptr [[OMP_IV5]]{{.*}}!llvm.access.group
  // CHECK-NEXT: [[CMP5:%.+]] = icmp slt i32 [[IV5]], 26
  // CHECK-NEXT: br i1 [[CMP5]], label %[[SIMPLE_LOOP5_BODY:.+]], label
  // %[[SIMPLE_LOOP5_END:[^,]+]]
  for (unsigned char it = 'z'; it >= 'a'; it += -1) {
    // CHECK: [[SIMPLE_LOOP5_BODY]]:
    // Start of body: calculate it from IV:
    // CHECK: [[IV5_0:%.+]] = load i32, ptr [[OMP_IV5]]{{.*}}!llvm.access.group
    // CHECK-NEXT: [[IV5_1:%.+]] = mul nsw i32 [[IV5_0]], 1
    // CHECK-NEXT: [[LC_IT_1:%.+]] = sub nsw i32 122, [[IV5_1]]
    // CHECK-NEXT: [[LC_IT_2:%.+]] = trunc i32 [[LC_IT_1]] to i8
    // CHECK-NEXT: store i8 [[LC_IT_2]], ptr {{.+}}, !llvm.access.group

    // CHECK: [[IV5_2:%.+]] = load i32, ptr [[OMP_IV5]]{{.*}}!llvm.access.group
    // CHECK-NEXT: [[ADD5_2:%.+]] = add nsw i32 [[IV5_2]], 1
    // CHECK-NEXT: store i32 [[ADD5_2]], ptr [[OMP_IV5]]{{.*}}!llvm.access.group
  }
...
#pragma omp simd lastprivate(CONDITIONAL A)
  // CHECK: store i64 0, ptr [[OMP_IV7:%[^,]+]]
  // CHECK: br label %[[SIMD_LOOP7_COND:[^,]+]]
  // CHECK: [[SIMD_LOOP7_COND]]:
  // CHECK-NEXT: [[IV7:%.+]] = load i64, ptr [[OMP_IV7]]{{.*}}!llvm.access.group
  // CHECK-NEXT: [[CMP7:%.+]] = icmp slt i64 [[IV7]], 7
  // CHECK-NEXT: br i1 [[CMP7]], label %[[SIMPLE_LOOP7_BODY:.+]], label
  // %[[SIMPLE_LOOP7_END:[^,]+]]
  for (long long i = -10; i < 10; i += 3) {
    // CHECK: [[SIMPLE_LOOP7_BODY]]:
    // Start of body: calculate i from IV:
    // CHECK: [[IV7_0:%.+]] = load i64, ptr [[OMP_IV7]]{{.*}}!llvm.access.group
    // CHECK-NEXT: [[LC_IT_1:%.+]] = mul nsw i64 [[IV7_0]], 3
    // CHECK-NEXT: [[LC_IT_2:%.+]] = add nsw i64 -10, [[LC_IT_1]]
    // CHECK-NEXT: store i64 [[LC_IT_2]], ptr
    // [[LC:%[^,]+]],{{.+}}!llvm.access.group CHECK-NEXT: [[LC_VAL:%.+]] = load
    // i64, ptr [[LC]]{{.+}}!llvm.access.group CHECK-NEXT: [[A_VAL:%.+]] = load
    // i32, ptr [[A_PRIV:%[^,]+]],{{.+}}!llvm.access.group CHECK-NEXT:
    // [[CAST:%.+]] = sext i32 [[A_VAL]] to i64 CHECK-NEXT: [[ADD:%.+]] = add
    // nsw i64 [[CAST]], [[LC_VAL]] CHECK-NEXT: [[CONV:%.+]] = trunc i64 [[ADD]]
    // to i32 CHECK-NEXT: store i32 [[CONV]], ptr
    // [[A_PRIV]],{{.+}}!llvm.access.group OMP5-NEXT: [[IV:%.+]] = load i64, ptr
    // [[OMP_IV7]],{{.+}}!llvm.access.group OMP5RT:    call void
    // @__kmpc_critical(ptr {{.+}}, i32 [[GTID:%.+]], ptr
    // [[A_REGION:@.+]]),{{.+}}!llvm.access.group OMP5-NEXT: [[LAST_IV_VAL:%.+]]
    // = load i64, ptr [[LAST_IV]],{{.+}}!llvm.access.group OMP5-NEXT:
    // [[CMP:%.+]] = icmp sle i64 [[LAST_IV_VAL]], [[IV]] OMP5-NEXT: br i1
    // [[CMP]], label %[[LP_THEN:.+]], label %[[LP_DONE:[^,]+]] OMP5:
    // [[LP_THEN]]: OMP5-NEXT: store i64 [[IV]], ptr
    // [[LAST_IV]],{{.+}}!llvm.access.group OMP5-NEXT: [[A_VAL:%.+]] = load i32,
    // ptr [[A_PRIV]],{{.+}}!llvm.access.group OMP5-NEXT: store i32 [[A_VAL]],
    // ptr [[LAST_A]],{{.+}}!llvm.access.group OMP5-NEXT: br label %[[LP_DONE]]
    // OMP5:      [[LP_DONE]]:
    // OMP5RT-NEXT: call void @__kmpc_end_critical(ptr {{.+}}, i32 [[GTID]], ptr
    // [[A_REGION]]),{{.+}}!llvm.access.group
    A += i;
    // CHECK: [[IV7_2:%.+]] = load i64, ptr [[OMP_IV7]]{{.*}}!llvm.access.group
    // CHECK-NEXT: [[ADD7_2:%.+]] = add nsw i64 [[IV7_2]], 1
    // CHECK-NEXT: store i64 [[ADD7_2]], ptr [[OMP_IV7]]{{.*}}!llvm.access.group
  }
...
#pragma omp simd reduction(* : R)
  ...
  // OMP5-NEXT: br i1 [[COND]], label {{%?}}[[THEN:[^,]+]], label
  // {{%?}}[[ELSE:[^,]+]] OMP5:      [[THEN]]:

  // CHECK: br label %[[SIMD_LOOP8_COND:[^,]+]]
  // CHECK: [[SIMD_LOOP8_COND]]:
  // CHECK-NEXT: [[IV8:%.+]] = load i64, ptr [[OMP_IV8]]{{.*}}!llvm.access.group
  // CHECK-NEXT: [[CMP8:%.+]] = icmp slt i64 [[IV8]], 7
  // CHECK-NEXT: br i1 [[CMP8]], label %[[SIMPLE_LOOP8_BODY:.+]], label
  // %[[SIMPLE_LOOP8_END:[^,]+]]
  for (long long i = -10; i < 10; i += 3) {
    // CHECK: [[SIMPLE_LOOP8_BODY]]:
    // Start of body: calculate i from IV:
    // CHECK: [[IV8_0:%.+]] = load i64, ptr [[OMP_IV8]]{{.*}}!llvm.access.group
    // CHECK-NEXT: [[LC_IT_1:%.+]] = mul nsw i64 [[IV8_0]], 3
    // CHECK-NEXT: [[LC_IT_2:%.+]] = add nsw i64 -10, [[LC_IT_1]]
    // CHECK-NEXT: store i64 [[LC_IT_2]], ptr
    // [[LC:%[^,]+]],{{.+}}!llvm.access.group CHECK-NEXT: [[LC_VAL:%.+]] = load
    // i64, ptr [[LC]]{{.+}}!llvm.access.group CHECK: store i32 %{{.+}}, ptr
    // [[R_PRIV]],{{.+}}!llvm.access.group
    R *= i;
    // CHECK: [[IV8_2:%.+]] = load i64, ptr [[OMP_IV8]]{{.*}}!llvm.access.group
  ...
  // OMP5-NEXT: br i1 [[CMP8]], label %[[SIMPLE_LOOP8_BODY:.+]], label
  // %[[SIMPLE_LOOP8_END:[^,]+]] OMP5: [[SIMPLE_LOOP8_BODY]]: Start of body:
  // calculate i from IV: OMP5: [[IV8_0:%.+]] = load i64, ptr
  // [[OMP_IV8]],{{[^!]*}} OMP5-NEXT: [[LC_IT_1:%.+]] = mul nsw i64 [[IV8_0]], 3
  ...
  // OMP5:      br label {{%?}}[[SIMD_LOOP8_COND]], {{.*}}!llvm.loop
  // ![[DISABLE_VECT:.+]] OMP5: [[SIMPLE_LOOP8_END]]: OMP5: br label
  // {{%?}}[[EXIT]] OMP5: [[EXIT]]:

  // CHECK-DAG: [[R_VAL:%.+]] = load i32, ptr [[R]],
  // CHECK-DAG: [[R_PRIV_VAL:%.+]] = load i32, ptr [[R_PRIV]],
  // CHECK: [[RED:%.+]] = mul nsw i32 [[R_VAL]], [[R_PRIV_VAL]]
  // CHECK-NEXT: store i32 [[RED]], ptr [[R]],
  // CHECK-NEXT: ret void
}

template <class T, unsigned K> T tfoo(T a) { return a + K; }

template <typename T, unsigned N> int templ1(T a, T *z) {
...
// CHECK-LABEL: define {{.*i32}} @{{.*}}templ1{{.*}}(float noundef {{.+}}, ptr
// noundef {{.+}}) CHECK: store i64 0, ptr [[T1_OMP_IV:[^,]+]]
// ...
// CHECK: [[IV:%.+]] = load i64, ptr [[T1_OMP_IV]]{{.*}}!llvm.access.group
// CHECK-NEXT: [[CMP1:%.+]] = icmp slt i64 [[IV]], 16
// CHECK-NEXT: br i1 [[CMP1]], label %[[T1_BODY:.+]], label %[[T1_END:[^,]+]]
// CHECK: [[T1_BODY]]:
// Loop counters i and j updates:
// CHECK: [[IV1:%.+]] = load i64, ptr [[T1_OMP_IV]]{{.*}}!llvm.access.group
...
// CHECK-NEXT: store i32 [[I_2]], ptr {{%.+}}{{.*}}!llvm.access.group
// CHECK: [[IV2:%.+]] = load i64, ptr [[T1_OMP_IV]]{{.*}}!llvm.access.group
// CHECK: [[IV2_1:%.+]] = load i64, ptr [[T1_OMP_IV]]{{.*}}!llvm.access.group
...
// CHECK-NEXT: store i64 [[J_2_ADD0]], ptr {{%.+}}{{.*}}!llvm.access.group
// simd.for.inc:
// CHECK: [[IV3:%.+]] = load i64, ptr [[T1_OMP_IV]]{{.*}}!llvm.access.group
// CHECK-NEXT: [[INC:%.+]] = add nsw i64 [[IV3]], 1
// CHECK-NEXT: store i64 [[INC]], ptr [[T1_OMP_IV]]{{.*}}!llvm.access.group
// CHECK-NEXT: br label {{%.+}}
// CHECK: [[T1_END]]:
// CHECK: ret i32 0
//
...
public:
  ...
  bool operator<(const IterDouble &that) const { return Ptr < that.Ptr; }
...
void iter_simple(IterDouble ia, IterDouble ib, IterDouble ic) {
  ...
  // CHECK: store i32 0, ptr [[IT_OMP_IV:%[^,]+]]
#pragma omp simd

  // CHECK: [[IV:%.+]] = load i32, ptr [[IT_OMP_IV]]{{.+}} !llvm.access.group
  // CHECK-NEXT: [[LAST_IT:%.+]] = load i32, ptr
  // [[OMP_LAST_IT]]{{.+}}!llvm.access.group CHECK-NEXT: [[NUM_IT:%.+]] = add
  // nsw i32 [[LAST_IT]], 1 CHECK-NEXT: [[CMP:%.+]] = icmp slt i32 [[IV]],
  // [[NUM_IT]] CHECK-NEXT: br i1 [[CMP]], label %[[IT_BODY:[^,]+]], label
  // %[[IT_END:[^,]+]]
  for (IterDouble i = ia; i < ib; ++i) {
    ...
    *i = *ic * 0.5;
    ...
    ++ic;
    //
    // CHECK: [[IV2:%.+]] = load i32, ptr [[IT_OMP_IV]]{{.+}}!llvm.access.group
    // CHECK-NEXT: [[ADD2:%.+]] = add nsw i32 [[IV2]], 1
    // CHECK-NEXT: store i32 [[ADD2]], ptr [[IT_OMP_IV]]{{.+}}!llvm.access.group
...
#pragma omp simd collapse(4)
  ...
  // CHECK-NEXT: br i1 [[CMP]], label %[[COLL1_BODY:[^,]+]], label
  // %[[COLL1_END:[^,]+]]
    ...
    for (j = 2u; j < 5u; j++)      // 3 iterations
      for (int k = 3; k <= 6; k++) // 4 iterations
        for (l = 4; l < 9; ++l)    // 5 iterations
        {
          // CHECK: [[COLL1_BODY]]:
          // Start of body: calculate i from index:
          // CHECK: [[IV1:%.+]] = load i32, ptr
          // [[OMP_IV]]{{.+}}!llvm.access.group Calculation of the loop counters
          // values. CHECK: [[CALC_I_1:%.+]] = udiv i32 [[IV1]], 60 CHECK-NEXT:
          // [[CALC_I_1_MUL1:%.+]] = mul i32 [[CALC_I_1]], 1 CHECK-NEXT:
          // [[CALC_I_2:%.+]] = add i32 1, [[CALC_I_1_MUL1]] CHECK-NEXT: store
          // i32 [[CALC_I_2]], ptr [[LC_I:.+]]

          // CHECK: [[IV1_2:%.+]] = load i32, ptr
          // [[OMP_IV]]{{.+}}!llvm.access.group CHECK: [[IV1_2_1:%.+]] = load
          // i32, ptr [[OMP_IV]]{{.+}}!llvm.access.group CHECK-NEXT:
          // [[CALC_J_1:%.+]] = udiv i32 [[IV1_2_1]], 60 CHECK-NEXT:
          // [[MUL_1:%.+]] = mul i32 [[CALC_J_1]], 60 CHECK-NEXT: [[SUB_3:%.+]]
          // = sub i32 [[IV1_2]], [[MUL_1]] CHECK-NEXT: [[CALC_J_2:%.+]] = udiv
          // i32 [[SUB_3]], 20 CHECK-NEXT: [[CALC_J_2_MUL1:%.+]] = mul i32
          // [[CALC_J_2]], 1 CHECK-NEXT: [[CALC_J_3:%.+]] = add i32 2,
          // [[CALC_J_2_MUL1]] CHECK-NEXT: store i32 [[CALC_J_3]], ptr
          // [[LC_J:.+]]

          // CHECK: [[IV1_3:%.+]] = load i32, ptr
          // [[OMP_IV]]{{.+}}!llvm.access.group CHECK: [[IV1_3_1:%.+]] = load
          // i32, ptr [[OMP_IV]]{{.+}}!llvm.access.group CHECK-NEXT:
          // [[DIV_1:%.+]] = udiv i32 [[IV1_3_1]], 60 CHECK-NEXT: [[MUL_2:%.+]]
          // = mul i32 [[DIV_1]], 60 CHECK-NEXT: [[ADD_3:%.+]] = sub i32
          // [[IV1_3]], [[MUL_2]]
          ...
          // CHECK: [[IV1_5:%.+]] = load i32, ptr
          // [[OMP_IV]]{{.+}}!llvm.access.group CHECK: [[IV1_5_1:%.+]] = load
          // i32, ptr [[OMP_IV]]{{.+}}!llvm.access.group CHECK-NEXT:
          // [[DIV_5:%.+]] = udiv i32 [[IV1_5_1]], 60 CHECK-NEXT: [[MUL_6:%.+]]
          // = mul i32 [[DIV_5]], 60 CHECK-NEXT: [[SUB_7:%.+]] = sub i32
          // [[IV1_5]], [[MUL_6]]
          ...
          // [[RESULT_ADDR:%.+]]{{.+}}!llvm.access.group
          float res = b[j] * c[k];
          a[i] = res * d[l];
          // CHECK: [[IV2:%.+]] = load i32, ptr
          // [[OMP_IV]]{{.*}}!llvm.access.group CHECK-NEXT: [[ADD2:%.+]] = add
          // i32 [[IV2]], 1 CHECK-NEXT: store i32 [[ADD2]], ptr
          // [[OMP_IV]]{{.*}}!llvm.access.group br label %{{[^,]+}}, !llvm.loop
          // ![[COLL1_LOOP_ID]] CHECK: [[COLL1_END]]:
        }
  // i,j,l are updated; k is not updated.
  // CHECK: store i32 3, ptr
  // CHECK-NEXT: store i32 5, ptr
  // CHECK-NEXT: store i32 7, ptr
...
void widened(float *a, float *b, float *c, float *d) {
  ...
  // CHECK: store i64 0, ptr [[OMP_IV:[^,]+]]
  //
#pragma omp simd collapse(2) private(globalfloat, localint)

  // CHECK: [[IV:%.+]] = load i64, ptr [[OMP_IV]]{{.+}}!llvm.access.group
  // CHECK-NEXT: [[LI:%.+]] = load i64, ptr
  // [[OMP_LI:%[^,]+]]{{.+}}!llvm.access.group CHECK-NEXT: [[NUMIT:%.+]] = add
  // nsw i64 [[LI]], 1 CHECK-NEXT: [[CMP:%.+]] = icmp slt i64 [[IV]], [[NUMIT]]
  // CHECK-NEXT: br i1 [[CMP]], label %[[WIDE1_BODY:[^,]+]], label
  // %[[WIDE1_END:[^,]+]]
  for (i = 1; i < 3; i++)       // 2 iterations
    for (j = 0; j < foo(); j++) // foo() iterations
    {
      // CHECK: [[WIDE1_BODY]]:
      // Start of body: calculate i from index:
      // CHECK: [[IV1:%.+]] = load i64, ptr [[OMP_IV]]{{.+}}!llvm.access.group
      // Calculation of the loop counters values...
      // CHECK: store i32 {{[^,]+}}, ptr [[LC_I:.+]]
      // CHECK: [[IV1_2:%.+]] = load i64, ptr [[OMP_IV]]{{.+}}!llvm.access.group
      ...
      // CHECK: store i32 {{.+}}, ptr
      // [[LOCALINT:%[^,]+]]{{.+}}!llvm.access.group
      localint = (int)j;
      // CHECK: [[IV2:%.+]] = load i64, ptr [[OMP_IV]]{{.*}}!llvm.access.group
      // CHECK-NEXT: [[ADD2:%.+]] = add nsw i64 [[IV2]], 1
      // CHECK-NEXT: store i64 [[ADD2]], ptr [[OMP_IV]]{{.*}}!llvm.access.group
      //
...
// CHECK-LABEL: define {{.*void}} @{{.*}}linear{{.*}}(ptr noundef {{.+}})
void linear(float *a) {
  // CHECK: [[VAL_ADDR:%.+]] = alloca i64,
  // CHECK: [[K_ADDR:%.+]] = alloca ptr,
  long long val = 0;
  long long &k = val;

#pragma omp simd linear(k : 3)
  // CHECK: store ptr [[VAL_ADDR]], ptr [[K_ADDR]],
  // CHECK: [[VAL_REF:%.+]] = load ptr, ptr [[K_ADDR]],
  // CHECK: store ptr [[VAL_REF]], ptr [[K_ADDR_REF:%.+]],
  // CHECK: store i32 0, ptr [[OMP_IV:%[^,]+]]
  // CHECK: [[K_REF:%.+]] = load ptr, ptr [[K_ADDR_REF]],
  // CHECK: [[K0LOAD:%.+]] = load i64, ptr [[K_REF]]
  // CHECK-NEXT: store i64 [[K0LOAD]], ptr [[LIN0:%[^,]+]]

  // CHECK: [[IV:%.+]] = load i32, ptr [[OMP_IV]]{{.*}}!llvm.access.group
  // CHECK-NEXT: [[CMP2:%.+]] = icmp slt i32 [[IV]], 9
  // CHECK-NEXT: br i1 [[CMP2]], label %[[SIMPLE_LOOP_BODY:.+]], label
  // %[[SIMPLE_LOOP_END:[^,]+]]
  for (int i = 10; i > 1; i--) {
    // CHECK: [[SIMPLE_LOOP_BODY]]:
    // Start of body: calculate i from IV:
    // CHECK: [[IV_0:%.+]] = load i32, ptr [[OMP_IV]]{{.*}}!llvm.access.group
    // FIXME: It is interesting, why the following "mul 1" was not constant
    // folded? CHECK-NEXT: [[IV_1:%.+]] = mul nsw i32 [[IV_0]], 1 CHECK-NEXT:
    // [[LC_I_1:%.+]] = sub nsw i32 10, [[IV_1]] CHECK-NEXT: store i32
    // [[LC_I_1]], ptr {{.+}}, !llvm.access.group
    //
    // CHECK-NEXT: [[LIN0_1:%.+]] = load i64, ptr
    // [[LIN0]]{{.*}}!llvm.access.group CHECK-NEXT: [[IV_2:%.+]] = load i32, ptr
    // [[OMP_IV]]{{.*}}!llvm.access.group CHECK-NEXT: [[LIN_MUL1:%.+]] = mul nsw
    // i32 [[IV_2]], 3 CHECK-NEXT: [[LIN_EXT1:%.+]] = sext i32 [[LIN_MUL1]] to
    // i64 CHECK-NEXT: [[LIN_ADD1:%.+]] = add nsw i64 [[LIN0_1]], [[LIN_EXT1]]
    // Update of the privatized version of linear variable!
    // CHECK-NEXT: store i64 [[LIN_ADD1]], ptr [[K_PRIVATIZED:%[^,]+]]
    a[k]++;
    k = k + 3;
    // CHECK: [[IV_2:%.+]] = load i32, ptr [[OMP_IV]]{{.*}}!llvm.access.group
    // CHECK-NEXT: [[ADD2_2:%.+]] = add nsw i32 [[IV_2]], 1
    // CHECK-NEXT: store i32 [[ADD2_2]], ptr [[OMP_IV]]{{.*}}!llvm.access.group
    // br label {{.+}}, !llvm.loop ![[SIMPLE_LOOP_ID]]
  }
  // CHECK: [[SIMPLE_LOOP_END]]:
  //
  // Update linear vars after loop, as the loop was operating on a private
  // version. CHECK: [[K_REF:%.+]] = load ptr, ptr [[K_ADDR_REF]], CHECK: store
  // ptr [[K_REF]], ptr [[K_PRIV_REF:%.+]], CHECK: [[LIN0_2:%.+]] = load i64,
  // ptr [[K_PRIVATIZED]] CHECK-NEXT: [[K_REF:%.+]] = load ptr, ptr
  // [[K_PRIV_REF]], CHECK-NEXT: store i64 [[LIN0_2]], ptr [[K_REF]]
  //
...
#pragma omp simd linear(uval(k) : 3)
  // CHECK: store i32 0, ptr [[OMP_IV:%[^,]+]]
  // CHECK: [[K0LOAD:%.+]] = load i64, ptr [[VAL_ADDR]]
  // CHECK-NEXT: store i64 [[K0LOAD]], ptr [[LIN0:%[^,]+]]
  ...
  //
  // Update linear vars after loop, as the loop was operating on a private
  // version. CHECK: [[LIN0_2:%.+]] = load i64, ptr [[K_PRIVATIZED]] CHECK-NEXT:
  // store i64 [[LIN0_2]], ptr [[VAL_ADDR]]
  //
}
...
template <typename T> class S7 : public T {
protected:
  T *a;
  T b[2];
  S7() : a(0) {}
...
// "_GLOBAL__sub_I_simd_codegen.cpp", OMP5-DAG: ![[NOVECT:.+]] =
// !{!"llvm.loop.vectorize.disable"} OMP5-DAG: ![[DISABLE_VECT]] = distinct
// !{{.*}}![[NOVECT]]{{[,}]}}
#endif // HEADER
