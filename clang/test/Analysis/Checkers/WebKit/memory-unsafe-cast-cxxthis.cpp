...
struct GrandchildPtr : T1 {
  int more;
};
void T1::bogus_ptr() {
  auto *g = static_cast<GrandchildPtr *>(this);
  // expected-warning@-1 {{Unsafe cast from base type 'T1' to derived type
  // 'GrandchildPtr'}}
}
...
struct GrandchildRef : T2 {
  int more;
};
void T2::bogus_ref() {
  auto &g = static_cast<GrandchildRef &>(*this);
  // expected-warning@-1 {{Unsafe cast from base type 'T2' to derived type
  // 'GrandchildRef'}}
}
