...
Target &downcast_ref(Source &source) {
  [[clang::suppress]] return static_cast<Target &>(source);
}
...
void test_dynamic_casts(BaseVirtual *base_ptr, BaseVirtual &base_ref) {
  ...
  // expected-warning@-1{{Unsafe cast from base type 'BaseVirtual' to derived
  // type 'DerivedVirtual'}}
...
void test_struct_pointers(struct BaseStruct *base_struct) {
  struct DerivedStruct *derived_static =
      static_cast<struct DerivedStruct *>(base_struct);
  // expected-warning@-1{{Unsafe cast from base type 'BaseStruct' to derived
  // type 'DerivedStruct'}}
  struct DerivedStruct *derived_reinterpret =
      reinterpret_cast<struct DerivedStruct *>(base_struct);
  // expected-warning@-1{{Unsafe cast from base type 'BaseStruct' to derived
  // type 'DerivedStruct'}}
...
template <typename T> class DerivedTemplate : public BaseTemplate<T> {};
...
void test_templates(BaseTemplate<int> *base, BaseTemplate<int> &base_ref) {
  ...
  // expected-warning@-1{{Unsafe cast from base type 'BaseTemplate' to derived
...
void test_macro_reinterpret(Base *base, Derived *derived, Base &base_ref) {
  ...
  // expected-warning@-1{{Unsafe cast from base type 'Base' to derived type
  // 'Derived'}}
  Derived &derived_reinterpret_ref =
      CAST_MACRO_REINTERPRET(base_ref, Derived &);
  // expected-warning@-1{{Unsafe cast from base type 'Base' to derived type
  // 'Derived'}}
  Base *base_reinterpret_same =
      CAST_MACRO_REINTERPRET(base, Base *); // no warning
  Base *base_reinterpret_upcast =
      CAST_MACRO_REINTERPRET(derived, Base *); // no warning
}
...
void test_struct_cpp_pointers(struct BaseStructCpp *base_struct) {
  struct DerivedStructCpp *derived_static =
      static_cast<struct DerivedStructCpp *>(base_struct);
  // expected-warning@-1{{Unsafe cast from base type 'BaseStructCpp' to derived
  // type 'DerivedStructCpp'}}
  struct DerivedStructCpp *derived_reinterpret =
      reinterpret_cast<struct DerivedStructCpp *>(base_struct);
  // expected-warning@-1{{Unsafe cast from base type 'BaseStructCpp' to derived
  // type 'DerivedStructCpp'}}
...
void test_struct_cpp_refs(BStructCpp &base_struct, DStructCpp &derived_struct) {
  ...
  BStructCpp &base_static =
      static_cast<BStructCpp &>(derived_struct); // no warning
  BStructCpp &base_reinterpret =
      reinterpret_cast<BStructCpp &>(derived_struct); // no warning
}
...
void test_stack(stack_st *base) {
  ...
  // expected-warning@-1{{Unsafe cast from type 'stack_st' to an unrelated type
  // 'stack_st_void'}}
}
...
void test_common_parent(Child1 *c1, Child2 *c2) {
  ...
  // expected-warning@-1{{Unsafe cast from type 'Child1' to an unrelated type
  // 'Child2'}}
...
void test_unrelated_ref(Type1 &t1, Type2 &t2) {
  ...
  // expected-warning@-1{{Unsafe cast from type 'Type1' to an unrelated type
  // 'Type2'}}
...
void test_unrelated_virtual(VirtualClass1 &v1) {
  ...
  // expected-warning@-1{{Unsafe cast from type 'VirtualClass1' to an unrelated
  // type 'VirtualClass2'}}
}
...
void test_struct_unrelated_refs(StA &a, StB &b) {
  ...
  // expected-warning@-1{{Unsafe cast from type 'StructA' to an unrelated type
  // 'StructB'}}
  StB &b_c = (StB &)a;
  // expected-warning@-1{{Unsafe cast from type 'StructA' to an unrelated type
  // 'StructB'}}
  StA &a_local = (StA &)b;
  // expected-warning@-1{{Unsafe cast from type 'StructB' to an unrelated type
  // 'StructA'}}
...
template <typename T> class DeferrableRefCounted {
public:
  void deref() const {
...
void test_this_to_template(SomeArrayClass *ptr) { ptr->deref(); };

template <typename WeakPtrFactoryType> class CanMakeWeakPtrBase {
public:
  ...
  void initializeWeakPtrFactoryReinterpret() const {
    ...
    // expected-warning@-1{{Unsafe cast from type 'CanMakeWeakPtrBase' to an
    // unrelated type 'SomeArrayClass'}}
  }
};

template <typename T> using CanMakeWeakPtr = CanMakeWeakPtrBase<T>;
