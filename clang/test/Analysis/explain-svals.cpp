...
void test_2(char *ptr, int ext) {
  ...
  clang_analyzer_explain(glob); // expected-warning-re{{{{^value derived from \(invalidation artifact \(conservative-call\) of type 'int' at CFG element 'conjure\(\)'\) for global variable 'glob'$}}}}
  clang_analyzer_explain(glob_ptr); // expected-warning-re{{{{^value derived from \(invalidation artifact \(conservative-call\) of type 'int' at CFG element 'conjure\(\)'\) for global variable 'glob_ptr'$}}}}
...
void test_6() {
  ...
  clang_analyzer_explain(conjure_S().z); // expected-warning-re{{{{^value derived from \(invalidation artifact \(partial-call\) of type 'int' at CFG element 'conjure_S\(\) \(CXXRecordTypedCall, \)'\) for field 'z' of temporary object constructed at statement 'conjure_S\(\)'$}}}}
}
