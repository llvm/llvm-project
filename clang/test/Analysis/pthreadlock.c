...
void noCrash(void) {
  if (((global_var & 137) == 2) && ((global_var & 137) & 8)) {
...
void bad3(void) {
  pthread_mutex_lock(&mtx1);   // no-warning
  pthread_mutex_lock(&mtx2);   // no-warning
  pthread_mutex_unlock(&mtx1); // lor-warning{{This was not the most recently
...
void bad7(void) {
  ...
  lck_mtx_unlock(&lck1); // lor-warning{{This was not the most recently acquired
...
void ok_non_lifo_unlock_three(void) {
  ...
  pthread_mutex_unlock(&mtx2); // lor-warning{{This was not the most recently
                               // acquired lock. Possible lock order reversal}}
