...
int *direct_return() {
  ...
  // expected-note@-3 {{Value bound to 'i' here}}
...
int &dangling_sources_ref() {
  ...
  // expected-note@-5 {{Value bound to 'x' here}}
  // expected-note@-6 {{Lifetime of 'x' ended here}}
  // expected-note@-7 {{Value bound to 'y' here}}
...
int &one_dangling_source_ref(int &a [[clang::lifetimebound]]) {
  ...
  // expected-note@-3 {{Value bound to 'x' here}}
