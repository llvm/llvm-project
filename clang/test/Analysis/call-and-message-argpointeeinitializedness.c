...
void uninit_mbrlen(const char *mbs) {
  mbstate_t state;
  mbrlen(mbs, 1,
         &state); // expected-warning{{3rd function call argument points to an
                  // uninitialized value; function 'mbrlen' expects memory
                  // pointed to by this argument to be initialized}}
}
...
void uninit_wcsnrtombs(const wchar_t *src) {
  char dst[10];
  mbstate_t state;
  wcsnrtombs(dst, &src, 1, 2,
             &state); // expected-warning{{5th function call argument points to
                      // an uninitialized value; function 'wcsnrtombs' expects
                      // memory pointed to by this argument to be initialized}}
}
...
void uninit_mbrtoc16(const char *s) {
  ...
  mbstate_t state;
  mbrtoc16(pc16, s, 1,
           &state); // expected-warning{{4th function call argument points to an
                    // uninitialized value; function 'mbrtoc16' expects memory
                    // pointed to by this argument to be initialized}}
}
...
void uninit_mktime() {
  ...
  mktime(&time); // expected-warning{{1st function call argument points to an
                 // uninitialized value; function 'mktime' expects memory
                 // pointed to by this argument to be initialized}}
}
