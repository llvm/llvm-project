...
void f3(void *dest) {
  ...
  memcpy(dest, src,
         1); // expected-warning{{2nd function call argument points to an
             // uninitialized value; function 'memcpy' expects memory pointed to
...
void f4(char *array) {
  ...
  clang_analyzer_dump_int(
      __builtin_bit_cast(int, b)); // expected-warning {{Unknown}}
  clang_analyzer_dump_int(
