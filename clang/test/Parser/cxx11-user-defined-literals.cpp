// RUN: %clang_cc1 -std=c++11 -verify %s -fms-extensions -triple
...
#pragma comment(                                                               \
    lib,                                                                       \
    "foo"_bar) // expected-error {{user-defined suffix cannot be used here}}
    _Pragma("comment(lib, \"foo\"_bar)") // expected-error {{user-defined suffix
                                         // cannot be used here}}
#pragma message "hi"_there // expected-error {{user-defined suffix cannot be
                           // used here}} expected-warning {{hi}}
#pragma push_macro(                                                            \
    "foo"_bar) // expected-error {{user-defined suffix cannot be used here}}
#if __has_warning("-Wan-island-to-discover"_bar) // expected-error
                                                 // {{user-defined suffix cannot
                                                 // be used here}}
#elif __has_include(                                                           \
...
int f() {
  asm("mov %eax, %rdx"_foo); // expected-error {{user-defined suffix cannot be
                             // used here}}
}

static_assert(true, "foo"_bar); // expected-error {{no matching literal operator
                                // for call to 'operator""_bar'}}
// expected-warning@-1 {{'static_assert' with a user-generated message is a
// C++26 extension}}

int cake() __attribute__((
    availability(macosx, unavailable,
                 message = "is a lie"_x))); // expected-error {{user-defined
                                            // suffix cannot be used here}}

// A ud-suffix cannot be used on character literals in preprocessor constant
// expressions:
#if 'x'_y -                                                                    \
    u'x'_z // expected-error 2{{character literal with user-defined suffix
...
// expressions:
#if 0_foo // expected-error {{integer literal with user-defined suffix cannot be
          // used in preprocessor constant expression}}
#error error
#endif
...
constexpr const char16_t operator""_id(const char16_t *p, size_t n) {
  return *p;
}
constexpr const char32_t operator""_id(const char32_t *p, size_t n) {
  return *p;
}
...
// expected-warning@-1{{identifier '_foo' preceded by whitespace in a literal
...
void operatorL""_not_char(
    const char *); // expected-error {{cannot have an encoding prefix}}
void operator""
             ""
...
int operator""_\u00b5s(unsigned long long) {
} // expected-error {{redefinition of 'operator""_µs'}}

int operator""_\U0000212B(long double) {} // expected-note {{previous}}
int hundred_Å = 50.0_Å + 50._\U0000212B;
int operator""_Å(long double) {
} // expected-error {{redefinition of 'operator""_Å'}}

int operator""_𐀀(char) {} // expected-note {{previous}}
int 𐀀 = '4'_𐀀 + '2'_\U00010000;
int operator""_\U00010000(char) {
} // expected-error {{redefinition of 'operator""_𐀀'}}
...
void operator""_℮
             ""_ℯ(unsigned long long) {
} // expected-error {{differing user-defined suffixes ('_℮' and '_ℯ') in string
...
constexpr int operator""_¢(long double d) {
  return d * 100;
} // expected-error {{character '¢' U+00A2 not allowed in an identifier}}
constexpr int operator""_¢(unsigned long long n) {
  return n;
} // expected-error {{character '¢' U+00A2 not allowed in an identifier}}
static_assert(0.02_¢ == 2_¢, ""); // expected-error 2{{character '¢' U+00A2 not
