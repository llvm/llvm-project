...
// -index-file %s | FileCheck %s CHECK: [indexDeclaration]: kind:
// c++-class-template | name: Foo CHECK-NEXT: [indexDeclaration]: kind:
// c++-instance-method | name: f CHECK-NEXT: [indexDeclaration]: kind: function
// | name: g CHECK-NEXT: [indexEntityReference]: kind: c++-class-template |
// name: Foo | USR: c:@ST>1#T@Foo CHECK-NEXT: [indexEntityReference]: kind:
// c++-instance-method | name: f | USR: c:@ST>1#T@Foo@F@f#t0.0#
