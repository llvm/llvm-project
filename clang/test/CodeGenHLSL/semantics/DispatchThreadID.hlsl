...
// RUN: %clang_cc1 -triple spirv-linux-vulkan-library -x hlsl -emit-llvm -finclude-default-header -disable-llvm-passes -o - %s | FileCheck %s

// Make sure SV_DispatchThreadID translated into hlsl.thread.id.
...
// CHECK:  %[[#ID_Y:]] = call i32 @llvm.hlsl.thread.id(i32 1)
