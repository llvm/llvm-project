...
// CHECK: %[[RET:.*]] = call i64 @llvm.[[TARGET]].quad.read.lane.at.i64(i64 %[[#]], i32 %[[#]])
// CHECK: ret i64 %[[RET]]
int64_t test_int64_t(int64_t expr, uint idx) { return QuadReadLaneAt(expr, idx); }

// CHECK: %[[RET:.*]] = call <2 x i64> @llvm.[[TARGET]].quad.read.lane.at.v2i64(<2 x i64> %[[#]], i32 %[[#]])
// CHECK: ret <2 x i64> %[[RET]]
int64_t2 test_int64_t2(int64_t2 expr, uint idx) { return QuadReadLaneAt(expr, idx); }

// CHECK: %[[RET:.*]] = call <3 x i64> @llvm.[[TARGET]].quad.read.lane.at.v3i64(<3 x i64> %[[#]], i32 %[[#]])
// CHECK: ret <3 x i64> %[[RET]]
int64_t3 test_int64_t3(int64_t3 expr, uint idx) { return QuadReadLaneAt(expr, idx); }

// CHECK: %[[RET:.*]] = call <4 x i64> @llvm.[[TARGET]].quad.read.lane.at.v4i64(<4 x i64> %[[#]], i32 %[[#]])
...
// CHECK: %[[RET:.*]] = call reassoc nnan ninf nsz arcp afn float @llvm.[[TARGET]].quad.read.lane.at.f32(float %[[#]], i32 %[[#]])
// CHECK: ret float %[[RET]]
float test_float(float expr, uint idx) { return QuadReadLaneAt(expr, idx); }

// CHECK: %[[RET:.*]] = call reassoc nnan ninf nsz arcp afn <2 x float> @llvm.[[TARGET]].quad.read.lane.at.v2f32(<2 x float> %[[#]], i32 %[[#]])
// CHECK: ret <2 x float> %[[RET]]
float2 test_float2(float2 expr, uint idx) { return QuadReadLaneAt(expr, idx); }

// CHECK: %[[RET:.*]] = call reassoc nnan ninf nsz arcp afn <3 x float> @llvm.[[TARGET]].quad.read.lane.at.v3f32(<3 x float> %[[#]], i32 %[[#]])
// CHECK: ret <3 x float> %[[RET]]
float3 test_float3(float3 expr, uint idx) { return QuadReadLaneAt(expr, idx); }

// CHECK: %[[RET:.*]] = call reassoc nnan ninf nsz arcp afn <4 x float> @llvm.[[TARGET]].quad.read.lane.at.v4f32(<4 x float> %[[#]], i32 %[[#]])
// CHECK: ret <4 x float> %[[RET]]
float4 test_float4(float4 expr, uint idx) { return QuadReadLaneAt(expr, idx); }

// CHECK: %[[RET:.*]] = call reassoc nnan ninf nsz arcp afn double @llvm.[[TARGET]].quad.read.lane.at.f64(double %[[#]], i32 %[[#]])
// CHECK: ret double %[[RET]]
double test_double(double expr, uint idx) { return QuadReadLaneAt(expr, idx); }

// CHECK: %[[RET:.*]] = call reassoc nnan ninf nsz arcp afn <2 x double> @llvm.[[TARGET]].quad.read.lane.at.v2f64(<2 x double> %[[#]], i32 %[[#]])
// CHECK: ret <2 x double> %[[RET]]
double2 test_double2(double2 expr, uint idx) { return QuadReadLaneAt(expr, idx); }

// CHECK: %[[RET:.*]] = call reassoc nnan ninf nsz arcp afn <3 x double> @llvm.[[TARGET]].quad.read.lane.at.v3f64(<3 x double> %[[#]], i32 %[[#]])
// CHECK: ret <3 x double> %[[RET]]
double3 test_double3(double3 expr, uint idx) { return QuadReadLaneAt(expr, idx); }

// CHECK: %[[RET:.*]] = call reassoc nnan ninf nsz arcp afn <4 x double> @llvm.[[TARGET]].quad.read.lane.at.v4f64(<4 x double> %[[#]], i32 %[[#]])
// CHECK: ret <4 x double> %[[RET]]
double4 test_double4(double4 expr, uint idx) { return QuadReadLaneAt(expr, idx); }

// CHECK-NATIVE_HALF: %[[RET:.*]] = call reassoc nnan ninf nsz arcp afn half @llvm.[[TARGET]].quad.read.lane.at.f16(half %[[#]], i32 %[[#]])
// CHECK-NATIVE_HALF: ret half %[[RET]]
// CHECK-NO_HALF: %[[RET:.*]] = call reassoc nnan ninf nsz arcp afn float @llvm.[[TARGET]].quad.read.lane.at.f32(float %[[#]], i32 %[[#]])
// CHECK-NO_HALF: ret float %[[RET]]
half test_half(half expr, uint idx) { return QuadReadLaneAt(expr, idx); }

// CHECK-NATIVE_HALF: %[[RET:.*]] = call reassoc nnan ninf nsz arcp afn <2 x half> @llvm.[[TARGET]].quad.read.lane.at.v2f16(<2 x half> %[[#]], i32 %[[#]])
// CHECK-NATIVE_HALF: ret <2 x half> %[[RET]]
// CHECK-NO_HALF: %[[RET:.*]] = call reassoc nnan ninf nsz arcp afn <2 x float> @llvm.[[TARGET]].quad.read.lane.at.v2f32(<2 x float> %[[#]], i32 %[[#]])
// CHECK-NO_HALF: ret <2 x float> %[[RET]]
half2 test_half2(half2 expr, uint idx) { return QuadReadLaneAt(expr, idx); }

// CHECK-NATIVE_HALF: %[[RET:.*]] = call reassoc nnan ninf nsz arcp afn <3 x half> @llvm.[[TARGET]].quad.read.lane.at.v3f16(<3 x half> %[[#]], i32 %[[#]])
// CHECK-NATIVE_HALF: ret <3 x half> %[[RET]]
// CHECK-NO_HALF: %[[RET:.*]] = call reassoc nnan ninf nsz arcp afn <3 x float> @llvm.[[TARGET]].quad.read.lane.at.v3f32(<3 x float> %[[#]], i32 %[[#]])
// CHECK-NO_HALF: ret <3 x float> %[[RET]]
half3 test_half3(half3 expr, uint idx) { return QuadReadLaneAt(expr, idx); }

// CHECK-NATIVE_HALF: %[[RET:.*]] = call reassoc nnan ninf nsz arcp afn <4 x half> @llvm.[[TARGET]].quad.read.lane.at.v4f16(<4 x half> %[[#]], i32 %[[#]])
// CHECK-NATIVE_HALF: ret <4 x half> %[[RET]]
// CHECK-NO_HALF: %[[RET:.*]] = call reassoc nnan ninf nsz arcp afn <4 x float> @llvm.[[TARGET]].quad.read.lane.at.v4f32(<4 x float> %[[#]], i32 %[[#]])
// CHECK-NO_HALF: ret <4 x float> %[[RET]]
half4 test_half4(half4 expr, uint idx) { return QuadReadLaneAt(expr, idx); }

#ifdef __HLSL_ENABLE_16_BIT
// CHECK-NATIVE_HALF: %[[RET:.*]] = call i16 @llvm.[[TARGET]].quad.read.lane.at.i16(i16 %[[#]], i32 %[[#]])
// CHECK-NATIVE_HALF: ret i16 %[[RET]]
int16_t test_int16_t(int16_t expr, uint idx) { return QuadReadLaneAt(expr, idx); }

// CHECK-NATIVE_HALF: %[[RET:.*]] = call <2 x i16> @llvm.[[TARGET]].quad.read.lane.at.v2i16(<2 x i16> %[[#]], i32 %[[#]])
// CHECK-NATIVE_HALF: ret <2 x i16> %[[RET]]
int16_t2 test_int16_t2(int16_t2 expr, uint idx) { return QuadReadLaneAt(expr, idx); }

// CHECK-NATIVE_HALF: %[[RET:.*]] = call <3 x i16> @llvm.[[TARGET]].quad.read.lane.at.v3i16(<3 x i16> %[[#]], i32 %[[#]])
// CHECK-NATIVE_HALF: ret <3 x i16> %[[RET]]
int16_t3 test_int16_t3(int16_t3 expr, uint idx) { return QuadReadLaneAt(expr, idx); }

// CHECK-NATIVE_HALF: %[[RET:.*]] = call <4 x i16> @llvm.[[TARGET]].quad.read.lane.at.v4i16(<4 x i16> %[[#]], i32 %[[#]])
