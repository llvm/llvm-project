...
// CHECK-LABEL: define hidden noundef nofpclass(nan inf) <5 x float> @{{.*}}test_smoothstep_float5
// CHECK: [[SUB_I:%.*]] = fsub reassoc nnan ninf nsz arcp afn <5 x float> %{{.*}}, %{{.*}}
// CHECK-NEXT: [[SUB1_I:%.*]] = fsub reassoc nnan ninf nsz arcp afn <5 x float> %{{.*}}, %{{.*}}
...
//
// SPVCHECK-LABEL: define hidden spir_func noundef nofpclass(nan inf) <5 x float> @{{.*}}test_smoothstep_float5
// SPVCHECK: [[SPV_SMOOTHSTEP_I:%.*]] = tail call reassoc nnan ninf nsz arcp afn noundef nofpclass(nan inf) <5 x float> @llvm.spv.smoothstep.v5f32(<5 x float> nofpclass(nan inf) %{{.*}}, <5 x float> nofpclass(nan inf) %{{.*}}, <5 x float> nofpclass(nan inf) %{{.*}})
