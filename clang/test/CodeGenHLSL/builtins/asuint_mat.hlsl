...
// CHECK-NOT: bitcast
// CHECK: ret <16 x i32> [[VAL]]
uint4x4 test_vector_uint(uint4x4 p0) {
  return asuint(p0);
}
...
// CHECK-NOT: bitcast
// CHECK: ret <16 x i32> [[VAL]]
uint4x4 test_vector_int(int4x4 p0) {
  return asuint(p0);
}

// CHECK: define {{.*}}test_vector_float{{.*}}(<16 x float> {{.*}} [[VAL:%.*]]){{.*}}
// CHECK: bitcast <16 x float> [[VAL]] to <16 x i32>
uint4x4 test_vector_float(float4x4 p0) {
  return asuint(p0);
}
