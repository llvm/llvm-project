...
// CHECK: %hlsl.ddy.fine = call {{.*}} <2 x half> @llvm.[[TGT]].ddy.fine.v2f16(<2 x half> %{{.*}})
...
// CHECK: %hlsl.ddy.fine = call {{.*}} <3 x half> @llvm.[[TGT]].ddy.fine.v3f16(<3 x half> %{{.*}})
...
// CHECK: %hlsl.ddy.fine = call {{.*}} <4 x half> @llvm.[[TGT]].ddy.fine.v4f16(<4 x half> %{{.*}})
...
// CHECK: %hlsl.ddy.fine = call {{.*}} <2 x float> @llvm.[[TGT]].ddy.fine.v2f32(<2 x float> %{{.*}})
...
// CHECK: %hlsl.ddy.fine = call {{.*}} <3 x float> @llvm.[[TGT]].ddy.fine.v3f32(<3 x float> %{{.*}})
...
// CHECK: %hlsl.ddy.fine = call {{.*}} <4 x float> @llvm.[[TGT]].ddy.fine.v4f32(<4 x float> %{{.*}})
