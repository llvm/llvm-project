...
void f(char * p, char * q, std::span<char> s) {
  ...
  snprintf(q, 10, "%s%d", "hello", *p); // expected-warning{{function snprintf introduces unsafe buffer access}} expected-note{{use 'std::string::c_str' as pointer to guarantee null-termination}}
  snprintf(s.data(), s.size(), "%s%d", "hello", *p); // expected-warning{{function snprintf introduces unsafe buffer access}} expected-note{{use 'std::string::c_str' as pointer to guarantee null-termination}}
