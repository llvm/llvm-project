...
bool Foo::operator==(const Foo &) const =
    default; // expected-warning {{comparing floating point with == or != is
             // unsafe}} expected-note {{in defaulted equality comparison
             // operator for 'Foo' first required here}}
...
// Declare the defaulted comparison function as a non-member function. Arguments
// are passed by value.
...
struct ConstCheck : ConstOnly {
  friend std::strong_ordering operator<=>(const ConstCheck &,
                                          const ConstCheck &) = default;
...
private:
  std::strong_ordering operator<=>(const HasPrivateSpaceship &)
      const; // expected-note 2 {{declared private here}}
};

struct S {
  HasPrivateSpaceship member; // expected-note 2 {{because it would invoke a
                              // private 'operator<=>' member of
                              // 'immediate_deletion_check::HasPrivateSpaceship'
                              // to compare member 'member'}}
  auto operator<=>(const S &) const =
      default; // expected-warning {{explicitly defaulted three-way comparison
               // operator is implicitly deleted}} expected-note {{replace
               // 'default' with 'delete'}} expected-note {{explicitly defaulted
               // function was implicitly deleted here}}
};

bool b = (S{} < S{}); // expected-error {{object of type 'S' cannot be compared
                      // because its 'operator<=>' is implicitly deleted}}
} // namespace immediate_deletion_check
