...
constexpr bool test_const_violation_array() { // ref-error{{constexpr function never produces a constant expression}}
  const unsigned char buf[4] = {0x01, 0x02, 0x03, 0x04};
  __builtin_stdc_memreverse8(4, const_cast<unsigned char *>(buf)); // ref-note 2{{modification of object of const-qualified type 'const unsigned char' is not allowed in a constant expression}} \
                                                                    // expected-note{{modification of object of const-qualified type 'const unsigned char' is not allowed in a constant expression}}
  return true;
}
