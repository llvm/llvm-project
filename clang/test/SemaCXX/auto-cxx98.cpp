...
void f() {
  auto int a; // expected-warning {{'auto' storage class specifier is redundant
              // and incompatible with C++11}}
  int auto b; // expected-warning {{'auto' storage class specifier is redundant
              // and incompatible with C++11}}
...
void g() {
  ...
  auto typedef int t2; // expected-error {{cannot combine with previous
                       // 'typedef' declaration specifier}}
}
