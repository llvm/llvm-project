...
void user_memcpy(U *d, const U *s) {
  __builtin_memcpy(
      d, s, sizeof(U)); // expected-warning {{first argument in call to
                        // '__builtin_memcpy' is a pointer to non-trivially
                        // copyable type 'U'}} expected-note {{explicitly cast
...
union V {
  int i;
  ~V() {} // expected-warning {{definition of implicit copy assignment operator
          // for 'V' is deprecated because it has a user-provided destructor}}
};

void use_deprecated_copy(V &a, const V &b) {
  a = b; // expected-note {{in implicit copy assignment operator for 'V' first
         // required here}}
}
