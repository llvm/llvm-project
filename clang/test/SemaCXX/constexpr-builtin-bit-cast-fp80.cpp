...
namespace builtin_bit_cast {
  ...
  // expected-warning@+2 {{is always undefined because it unconditionally maps a padding bit onto a non-padding bit}}
  ...
  // expected-error@+3 {{constexpr variable 'bad_str_fp80_0' must be initialized by a constant expression}}
  // expected-warning@+2 {{is always undefined because it unconditionally maps a padding bit onto a non-padding bit}}
  // expected-note@+1 {{constexpr bit cast involving type 'long double' is not yet supported}}
  constexpr static char bad_str_fp80_0 = __builtin_bit_cast(fp80x2_s, test_vec_fp80)._pad[0];

  // expected-error@+3 {{constexpr variable 'bad_str_fp80_1' must be initialized by a constant expression}}
  // expected-warning@+2 {{is always undefined because it unconditionally maps a padding bit onto a non-padding bit}}
  // expected-note@+1 {{constexpr bit cast involving type 'long double' is not yet supported}}
  constexpr static char bad_str_fp80_1 = __builtin_bit_cast(fp80x2_s, test_vec_fp80)._pad[1];

  // expected-error@+3 {{constexpr variable 'bad_str_fp80_11' must be initialized by a constant expression}}
  // expected-warning@+2 {{is always undefined because it unconditionally maps a padding bit onto a non-padding bit}}
  // expected-note@+1 {{constexpr bit cast involving type 'long double' is not yet supported}}
