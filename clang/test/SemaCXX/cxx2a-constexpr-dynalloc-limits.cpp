...
constexpr std::size_t s = S<std::size_t>(~0UL)
    [42]; // expected-error {{constexpr variable 's' must be initialized by a constant expression}} \
...
constexpr std::size_t s4 = S<std::size_t>(1024)
    [42]; // expected-error {{constexpr variable 's4' must be initialized by a constant expression}} \
...
constexpr std::size_t s5 = S<std::size_t>(1025)
    [42]; // expected-error{{constexpr variable 's5' must be initialized by a constant expression}} \
...
template <auto N> constexpr int stack_array() {
  [[maybe_unused]] char BIG[N] = {
      1}; // expected-note  3{{cannot allocate array; evaluated array bound 1025 exceeds the limit (1024)}} \
