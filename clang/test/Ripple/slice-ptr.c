...
// Slicing of a block of pointers
void extract_2nd_row_from_32x4_blocks(char * input[N], char * output[N/4]) {
  ...
  for (size_t block_idx = 0; block_idx < n_blocks; ++block_idx) {
    ...
    char * block = input[block_size * block_idx + block_size0 * v1 + v0];
    char * extracted = ripple_slice_ptr(block, -1, 2);
