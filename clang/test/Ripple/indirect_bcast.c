...
// RUN: %clang -ffreestanding -g -S -fenable-ripple -O0 -emit-llvm %s -o %t 2>&1
// RUN: %clang -ffreestanding -g -S -fenable-ripple -O1 -emit-llvm %s -o - 2>&1 | FileCheck %s
// RUN: %clang -ffreestanding -g -S -fenable-ripple -O2 -emit-llvm %s -o %t 2>&1
// RUN: %clang -ffreestanding -g -S -fenable-ripple -O3 -emit-llvm %s -o %t 2>&1
...
void indirectStore(float In[32] , float (*Out[32])[32]) {
  ...
  for (unsigned i = 0; i < 32; ++i) {
    ...
    // CHECK: %[[OutOffset:[a-zA-Z0-9_.]+]] = getelementptr inbounds nuw [{{4|8}} x i8], ptr %[[OUT]], i{{[0-9]+}} %[[IndVar]]
