...
// RUN: %clang -ffreestanding -S -O2 -emit-llvm %s -DCOMPILE_LIB=1 -o %t.ll
// RUN: %clang -ffreestanding -S -O2 -emit-llvm -fenable-ripple -fripple-lib=%t.ll -mllvm -ripple-disable-link %s &> %t.warnings; FileCheck %s --input-file=%t.warnings
