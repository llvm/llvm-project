...
// RUN: %clang -ffreestanding --target=x86_64-linux-gnu -g -c -O2 -emit-llvm %S/external_library.c -o external_library_masked_x86.bc
...
// CHECK: br label %[[FOR_BODY:[a-zA-Z0-9_.]+]]
// CHECK: [[FOR_BODY]]:
// CHECK: br i1 %{{.*}}, label %[[FOR_BODY]], label %[[FOR_END:[a-zA-Z0-9_.]+]]
// CHECK: [[FOR_END]]:
