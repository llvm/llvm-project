...
// RUN: %clang -ffreestanding -g -c -O2 -emit-llvm %s -DCOMPILE_LIB=1 -o %t.rlib.bc
// RUN: %clang -ffreestanding -g -O2 -fenable-ripple -fripple-lib=%t.rlib.bc -emit-llvm -S %s -o - -mllvm -ripple-disable-link | FileCheck %s
