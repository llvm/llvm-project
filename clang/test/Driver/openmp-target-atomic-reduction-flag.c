...
// -Xopenmp-target=amdgcn-amd-amdhsa -march=gfx90a -nogpulib %s 2>&1 \ RUN:   |
// FileCheck -check-prefix=DEFAULT %s

// Passed through to -cc1 when requested.
// RUN:   %clang -### -fopenmp=libomp -fopenmp-targets=amdgcn-amd-amdhsa
// -Xopenmp-target=amdgcn-amd-amdhsa -march=gfx90a -nogpulib
// -fopenmp-target-atomic-reduction %s 2>&1 \ RUN:   | FileCheck
// -check-prefix=ENABLE %s

// Explicit disable wins over a preceding enable and is not passed through.
// RUN:   %clang -### -fopenmp=libomp -fopenmp-targets=amdgcn-amd-amdhsa
// -Xopenmp-target=amdgcn-amd-amdhsa -march=gfx90a -nogpulib
// -fopenmp-target-atomic-reduction -fno-openmp-target-atomic-reduction %s 2>&1
