...
// RUN: %clang -### -c --target=x86_64-unknown-linux-gnu -fveclib=HVML %s 2>&1 | FileCheck --check-prefix=CHECK-HVML %s
...
// RUN: not %clang --target=x86 -c -fveclib=HVML %s 2>&1 | FileCheck --check-prefix=CHECK-ERROR %s
...
// RUN: not %clang --target=aarch64 -c -fveclib=HVML %s 2>&1 | FileCheck --check-prefix=CHECK-HVML-ERROR %s
// CHECK-HVML-ERROR: unsupported option 'HVML' for target 'aarch64'
...
// RUN: %clang -### --target=x86_64-unknown-linux-gnu -fveclib=HVML -flto %s 2>&1 | FileCheck --check-prefix=CHECK-HVML-LTO %s
// CHECK-HVML-LTO: "-plugin-opt=-vector-library=HVML"
...
// RUN: %clang -### --target=x86_64-unknown-linux-gnu -fveclib=HVML %s 2>&1 | FileCheck --check-prefix=CHECK-ERRNO-HVML %s
// CHECK-ERRNO-HVML: "-fveclib=HVML"
// CHECK-ERRNO-HVML-SAME: "-fmath-errno"
// RUN: %clang -### --target=x86_64-unknown-linux-gnu -fveclib=HVML -fno-math-errno %s 2>&1 | FileCheck --check-prefix=CHECK-ERRNO2-HVML %s
// CHECK-ERRNO2-HVML: "-fveclib=HVML"
// CHECK-ERRNO2-HVML-NOT: "-fmath-errno"
