...
// --target=x86_64-unknown-linux-gnu -fsycl --sysroot=/nonexistent-prefix %s
// 2>&1 \ RUN:   | FileCheck -check-prefix=CHECK-FLAT %s
...
// RUN: %t/bin/clang -### -no-canonical-prefixes
// --target=x86_64-unknown-linux-gnu -fsycl --sysroot=/nonexistent-prefix %s
// 2>&1 \ RUN:   | FileCheck -check-prefix=CHECK-PER-TARGET %s
