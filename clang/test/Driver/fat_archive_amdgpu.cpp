...
// RUN: %clang -O2 -### -nogpuinc -nogpulib  -nogpulib  -fopenmp --no-offload-new-driver -fopenmp-targets=amdgcn-amd-amdhsa -Xopenmp-target=amdgcn-amd-amdhsa -march=gfx906 %s -L%S/Inputs/openmp_static_device_link -lFatArchive 2>&1 | FileCheck %s
...
//  FIXME: these prelink and link commands need to be properly speced
//  FIXME: llvm-link{{.*}}" "[[DEVICESPECIFICARCHIVE]]" "-o" "[[PRELINKED:.*-[[GPU]]-prelinked-.*.bc]]"
//  FIXME: llvm-link "[[PRELINKED]]" "--internalize" "--only-needed" {{.*}} "-o" "{{.*}}-[[GPU]]-linked-{{.*}}.bc"
