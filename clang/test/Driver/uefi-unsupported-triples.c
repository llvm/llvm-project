// RUN: not %clang --target=aarch64-unknown-uefi < %s 2>&1 | FileCheck -check-prefixes=CHECK-AARCH64 %s
...
// RUN: not %clang --target=arm-unknown-uefi < %s 2>&1 | FileCheck -check-prefixes=CHECK-ARM %s
...
// RUN: not %clang --target=x86-unknown-uefi < %s 2>&1 | FileCheck -check-prefixes=CHECK-x86 %s
