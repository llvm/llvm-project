...
// CHECK: lld{{.*}}"-flavor" "gnu" "--no-undefined" "-shared" "-o" "[[GFX906OUT:.*.out.*-gfx906]]" "{{.*}}openmp-offload-multi-{{.*}}-gfx906-{{.*}}.o"
...
// CHECK: lld{{.*}}"-flavor" "gnu" "--no-undefined" "-shared" "-o" "[[GFX908OUT:.*.out.*-gfx908]]" "{{.*}}openmp-offload-multi-{{.*}}-gfx908-{{.*}}.o"
