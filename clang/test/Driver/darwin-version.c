...
// CHECK-VERSION-TNO-OSV1-NOT: argument unused during compilation: '-mmacosx-version-min=10.6'

// RUN: %clang -target x86_64-apple-macos -miphoneos-version-min=9.1 -c %s -### 2>&1 | \
// RUN:   FileCheck --check-prefix=CHECK-VERSION-TNO-OSV2 %s
// CHECK-VERSION-TNO-OSV2-NOT: argument unused during compilation: '-miphoneos-version-min=9.1'
