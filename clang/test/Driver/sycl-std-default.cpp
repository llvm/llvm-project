...
// RUN: %clangxx -### -fsycl -resource-dir %S/Inputs/spirv64-sycl -c %s 2>&1 | FileCheck %s --check-prefixes=CHECK-DEVICE,CHECK-HOST
...
// RUN: %clangxx -### -fsycl -resource-dir %S/Inputs/spirv64-sycl -std=c++20 -c %s 2>&1 | FileCheck %s --check-prefixes=CHECK-OVERRIDE-DEVICE,CHECK-OVERRIDE-HOST
...
// RUN: %clang_cl --target=x86_64-pc-windows-msvc -### -fsycl -resource-dir %S/Inputs/spirv64-sycl -- %s 2>&1 | FileCheck %s --check-prefixes=CHECK-MSVC-DEVICE,CHECK-MSVC-HOST
...
// RUN: %clang_cl --target=x86_64-pc-windows-msvc -### -fsycl -resource-dir %S/Inputs/spirv64-sycl /std:c++20 -- %s 2>&1 | FileCheck %s --check-prefixes=CHECK-MSVC-OVERRIDE-DEVICE,CHECK-MSVC-OVERRIDE-HOST
