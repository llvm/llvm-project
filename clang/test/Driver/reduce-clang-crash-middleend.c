...
// RUN: echo "%clang -target x86_64-unknown-linux-gnu -O2 -mllvm -opt-pipeline-trigger-crash -mllvm=-enable-scalar-pre -c %t/test.c" > %t/crash.sh
...
// CHECK-CMD: {{^.*}}opt{{(\.exe)?}} '-passes=function(trigger-crash-function)' {{[^ ]*}}test.reduced.ll{{$}}
