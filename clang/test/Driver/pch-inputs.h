...
// UNUSED-L: warning: -lfoo: 'linker' input unused when '' is present [-Wunused-command-line-argument]

// RUN: %clang %s -Wl,--whole-archive -o %t/tmp1.pch -### 2>&1 | FileCheck %s --check-prefix=UNUSED-WL,SINGLEHEADER
// UNUSED-WL: warning: -Wl,--whole-archive: 'linker' input unused when '' is present [-Wunused-command-line-argument]
...
// UNUSED-XLINKER: warning: -Xlinker -somelinkerflag: 'linker' input unused when '' is present [-Wunused-command-line-argument]
