...
// RUN: %clang -### -target amdgcn-amd-amdhsa -x cl -c -emit-llvm -g %s 2>&1 | FileCheck -check-prefix=CHECK-SIMPLE %s
...
// RUN: %clang -### -target amdgcn-amd-amdhsa -x cl -c -emit-llvm -g -gno-heterogeneous-dwarf %s 2>&1 | FileCheck -check-prefix=CHECK-NO-HETEROGENEOUS %s
