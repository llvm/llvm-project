...
// RUN: %clang -### -fsanitize=address -fsanitize-compilation-dir=. %s 2>&1 | FileCheck %s --check-prefix=RELATIVE
// RUN: %clang -### -fsanitize=address -fsanitize-compilation-dir=/foo/bar/.. %s 2>&1 | FileCheck %s --check-prefix=DOTS
