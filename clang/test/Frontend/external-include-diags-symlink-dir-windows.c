...
// RUN: rm -rf %t
// RUN: split-file %s %t
// RUN: cmd /c mklink /D %t\inc1\foo ..\inc2\foo
// RUN: cmd /c mklink /D %t\inc1\goo\baz .
...
// The first case below is disabled because MSVC does not follow the
// symbolic link and ".." path component to arrive at a directory where
// "foo" is present and thus fails to resolve the header file.
//
// XXX: %clang_cc1 \
// XXX:     -fsyntax-only -verify -Wall -Wno-system-headers \
// XXX:     -I%t\inc1\goo\baz\..\foo \
// XXX:     -iexternal %t\inc2\fo \
// XXX:     %t\should-warn.c
// RUN: %clang_cc1 \
// RUN:     -fsyntax-only -verify -Wall -Wno-system-headers \
// RUN:     -I%t\inc2\foo \
