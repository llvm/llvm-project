...
// PGOGEN: store {{.*}} @[[THC]]
void throws() {
  // PGOGEN: store {{.*}} @[[THC]], i64 8
  // PGOUSE: br {{.*}} !prof ![[TH1:[0-9]+]]
  for (int i = 0; i < 100; ++i) {
    try {
      // PGOGEN: store {{.*}} @[[THC]], i64 24
      // PGOUSE: br {{.*}} !prof ![[TH2:[0-9]+]]
      if (i % 3) {
        // PGOGEN: store {{.*}} @[[THC]], i64 32
      ...
      // PGOGEN-EXC: store {{.*}} @[[THC]], i64 48
      // PGOGEN-EXC: store {{.*}} @[[THC]], i64 56
      // PGOUSE-EXC: br {{.*}} !prof ![[TH5:[0-9]+]]
      if (e) {}
    }
    // PGOGEN: store {{.*}} @[[THC]], i64 16

    // PGOGEN: store {{.*}} @[[THC]], i64 64
...
void unreachable(int i) {
  // PGOGEN: store {{.*}} @[[UNC]], i64 8
  // PGOUSE: br {{.*}} !prof ![[UN1:[0-9]+]]
  if (i)
    throw i;

  // PGOGEN: store {{.*}} @[[UNC]], i64 16
