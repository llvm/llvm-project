// RUN: %clang_cc1 -std=c++26 -fsyntax-only -Wunreachable-code -verify=ref,both
// %s RUN: %clang_cc1 -std=c++26 -fsyntax-only -Wunreachable-code
...
static_assert((delete(float *)(void *)new int,
               true)); // both-error {{not an integral constant expression}} \
...
static_assert((delete[] (float *)(void *)new int[2],
               true)); // both-error {{not an integral constant expression}} \
                                                             // both-note {{cast from 'void *' is not allowed in a constant expression because the pointed object type 'int' is not similar to the target type 'float'}}
} // namespace VoidCast
...
struct T {
  constexpr int a() const { return 42; }
};
...
constexpr const char foo[] = {
    bar}; // both-error {{use of undeclared identifier}} \
...
static_assert(
    false,
    S{}); // both-error {{the message in a static assertion must be produced by a constant expression}} \
