...
// -std=c++26 %s RUN: %clang_cc1 -fexperimental-new-constant-interpreter
...
struct C : A, C2 {
  A4 *c = dynamic_cast<A4 *>(static_cast<C2 *>(this));
};
...
constexpr S s{};
static_assert(&dynamic_cast<const A &>((L &)s) ==
...
constexpr G g;
static_assert(&dynamic_cast<A &>((D &)g) ==
              nullptr); // both-error {{not an integral constant expression}} \
...
constexpr int b_unrelated =
    (dynamic_cast<Unrelated &>((B &)g),
...
constexpr int e_unrelated =
    (dynamic_cast<Unrelated &>((E &)g),
     0); // both-error {{must be initialized by a constant expression}} \
                                                                    // both-note {{reference dynamic_cast failed: dynamic type 'Unrelated::G' of operand does not have a base class of type 'Unrelated'}}
...
// both-note@+1 {{reference dynamic_cast failed: 'A' is an ambiguous base class
// of dynamic type 'Field2::G' of operand}}
...
// both-note@+1 {{reference dynamic_cast failed: 'A' is an ambiguous base class
// of dynamic type 'Field2::G' of operand}}
static_assert(&dynamic_cast<A &>((D &)ss.g) ==
              &(A &)(B &)ss.g); // both-error {{}}
...
// Cannot cast from B to private sibling E.
// both-note@+1 {{reference dynamic_cast failed: 'E' is a non-public base class
// of dynamic type 'Field2::G' of operand}}
...
constexpr int b_unrelated =
    (dynamic_cast<Unrelated &>((B &)ss.g), 0); // both-error {{}}
// both-note@+1 {{reference dynamic_cast failed: dynamic type 'Field2::G' of
// operand does not have a base class of type 'Unrelated'}}
constexpr int e_unrelated =
    (dynamic_cast<Unrelated &>((E &)ss.g), 0); // both-error {{}}
} // namespace Field2
...
static_assert(&dynamic_cast<A &>((S &)(B &)g) ==
              &(A &)(B &)
                  g); // both-error {{not an integral constant expression}} \
                                                             // both-note {{cast that performs the conversions of a reinterpret_cast is not allowed in a constant expression}}
...
static_assert(&dynamic_cast<S &>((X &)x),
              ""); // both-error {{not an integral constant expression}} \
...
constexpr D d;
constexpr A &a = (B &)d;
constexpr auto p = dynamic_cast<C &>(
    a); // both-error {{must be initialized by a constant expression}} \
