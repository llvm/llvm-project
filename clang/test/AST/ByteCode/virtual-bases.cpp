// RUN: %clang_cc1 -std=c++26 -fexperimental-new-constant-interpreter
// -verify=both,expected %s RUN: %clang_cc1 -std=c++26 -verify=both,ref      %s
...
struct Left : virtual Common {
  ...
  constexpr const unsigned &get_counter() const { return Common::counter; }
};
...
struct B : public A {};
constexpr B
    b{}; // both-error {{must be initialized by a constant expression}} \
...
struct C : public virtual A, public virtual B {
  ...
  constexpr C(int *results, int &i)
      : A(results, i), B(results, i), results(results), i(i), f(results, i) {}
...
constexpr int foo() {
  ...
  int i = 0;
  { C c = C(results, i); }
  return i == 5 && results[0] == R_C && results[1] == R_F &&
         results[2] == R_G && results[3] == R_B && results[4] == R_A;
}
static_assert(foo() == 1);
} // namespace DtorOrder
...
constexpr int foo() {
  ...
  int i = 0;
  { C c = C(results, i); }
  return i == 5 && results[0] == R_A && results[1] == R_B &&
         results[2] == R_G && results[3] == R_F && results[4] == R_C;
}
static_assert(foo() == 1);
} // namespace CtorOrder
...
constexpr Ints2
    ints22; // both-error {{without a user-provided default constructor}}
...
struct V {
  bool b;
  constexpr ~V() {
    __builtin_abort(); // both-note {{subexpression not valid in a constant
                       // expression}}
  }
};
...
constexpr X
    x; // both-error {{constexpr variable 'x' must have constant destruction}} \
