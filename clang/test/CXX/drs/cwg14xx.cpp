// RUN: %clang_cc1 -std=c++98 %s -fexceptions -fcxx-exceptions -pedantic-errors
// -verify-directives -verify=expected RUN: %clang_cc1 -std=c++11 %s
// -fexceptions -fcxx-exceptions -pedantic-errors -verify-directives
// -verify=expected,cxx11-17,since-cxx11 RUN: %clang_cc1 -std=c++14 %s
// -fexceptions -fcxx-exceptions -pedantic-errors -verify-directives
// -verify=expected,cxx14-17,cxx11-17,since-cxx11,since-cxx14 RUN: %clang_cc1
// -std=c++17 %s -fexceptions -fcxx-exceptions -pedantic-errors
// -verify-directives -verify=expected,cxx14-17,cxx11-17,since-cxx11,since-cxx14
// RUN: %clang_cc1 -std=c++20 %s -fexceptions -fcxx-exceptions -pedantic-errors
// -verify-directives -verify=expected,since-cxx11,since-cxx14,since-cxx20 RUN:
// %clang_cc1 -std=c++23 %s -fexceptions -fcxx-exceptions -pedantic-errors
// -verify-directives -verify=expected,since-cxx11,since-cxx14,since-cxx20 RUN:
// %clang_cc1 -std=c++2c %s -fexceptions -fcxx-exceptions -pedantic-errors
// -verify-directives -verify=expected,since-cxx11,since-cxx14,since-cxx20

// RUN: %clang_cc1 -std=c++98 %s -fexceptions -fcxx-exceptions -pedantic-errors
// -fexperimental-new-constant-interpreter -verify-directives -verify=expected
// RUN: %clang_cc1 -std=c++11 %s -fexceptions -fcxx-exceptions -pedantic-errors
// -fexperimental-new-constant-interpreter -verify-directives
// -verify=expected,cxx11-17,since-cxx11 RUN: %clang_cc1 -std=c++14 %s
...
// -fexperimental-new-constant-interpreter -verify-directives
// -verify=expected,cxx14-17,cxx11-17,since-cxx11,since-cxx14 RUN: %clang_cc1
// -std=c++20 %s -fexceptions -fcxx-exceptions -pedantic-errors
// -fexperimental-new-constant-interpreter -verify-directives
// -verify=expected,since-cxx11,since-cxx14,since-cxx20 RUN: %clang_cc1
// -std=c++23 %s -fexceptions -fcxx-exceptions -pedantic-errors
// -fexperimental-new-constant-interpreter -verify-directives
// -verify=expected,since-cxx11,since-cxx14,since-cxx20 RUN: %clang_cc1
// -std=c++2c %s -fexceptions -fcxx-exceptions -pedantic-errors
// -fexperimental-new-constant-interpreter -verify-directives
...
template <typename T> struct A : T {
  ...
  void f() {
    ...
    // value-dependent because of the value-dependent '&' operator, not because
    // of 'A::d'
...
template <typename T> struct S {
  typedef T F;
  typedef T *P;
  // expected-error@-1 {{pointer to function type 'void () const' cannot have
  // 'const' qualifier}} expected-note@#cwg1417-s {{in instantiation of template
  // class 'cwg1417::S<void () const>' requested here}}
  typedef T &R;
  // expected-error@-1 {{reference to function type 'void () const' cannot have
  // 'const' qualifier}}
};
...
bool b1 = nullptr;
// since-cxx11-error@-1 {{cannot initialize a variable of type 'bool' with an
...
struct X {
  ...
  // cxx11-17-error@-1 {{constexpr declaration of 'B' follows non-constexpr
  // declaration}}
  ...
  // cxx11-17-error@-1 {{constexpr declaration of 'E' follows non-constexpr
  // declaration}}
...
struct H {
  ...
  constexpr H() {}
  // cxx11-17-error@-1 {{constexpr constructor that does not initialize all
  // members is a C++20 extension}}
...
// cxx14-17-error@-1 {{static assertion expression is not an integral constant
// expression}}
//   cxx14-17-note@#cwg1460-t {{non-constexpr constructor 'B' cannot be used in
//   a constant expression}} cxx14-17-note@#cwg1460-check-B {{in call to
//   'check<cwg1460::B>()'}} cxx14-17-note@#cwg1460-B {{declared here}}
static_assert(check<C>(), "");
static_assert(check<D>(), "");
static_assert(check<E>(), ""); // #cwg1460-check-E
// cxx14-17-error@-1 {{static assertion expression is not an integral constant
// expression}}
//   cxx14-17-note@#cwg1460-t {{non-constexpr constructor 'E' cannot be used in
//   a constant expression}} cxx14-17-note@#cwg1460-check-E {{in call to
//   'check<cwg1460::E>()'}} cxx14-17-note@#cwg1460-E {{declared here}}
...
// constant expression}}
//   since-cxx11-note@-2 {{read of member 'b' of union with active member 'a' is
//   not allowed in a constant expression}}
static_assert(B('x').a == 0, "");
// since-cxx11-error@-1 {{static assertion expression is not an integral
// constant expression}}
//   since-cxx11-note@-2 {{read of member 'a' of union with active member 'b' is
//   not allowed in a constant expression}}
static_assert(B('x').b == 4, "");
static_assert(B(123).b == 2, "");
// since-cxx11-error@-1 {{static assertion expression is not an integral
// constant expression}}
//   since-cxx11-note@-2 {{read of member 'b' of union with active member 'c' is
//   not allowed in a constant expression}}
...
// since-cxx11-error@-1 {{static assertion expression is not an integral
// constant expression}}
//   since-cxx11-note@-2 {{read of member 'c' of union with active member 'b' is
//   not allowed in a constant expression}}
...
// since-cxx11-error@-1 {{static assertion expression is not an integral
// constant expression}}
//   since-cxx11-note@-2 {{read of member 'd' of union with active member 'e' is
//   not allowed in a constant expression}}
static_assert(C().e == 5, "");
...
// since-cxx11-error@-1 {{static assertion expression is not an integral
// constant expression}}
//   since-cxx11-note@-2 {{read of member 'e' of union with active member 'd' is
//   not allowed in a constant expression}}
...
// since-cxx11-error@-1 {{static assertion expression is not an integral
// constant expression}}
//   since-cxx11-note@-2 {{read of member 'e' of union with active member 'f' is
//   not allowed in a constant expression}}
...
// since-cxx11-error@-1 {{static assertion expression is not an integral
// constant expression}}
//   since-cxx11-note@-2 {{read of member 'f' of union with active member 'e' is
//   not allowed in a constant expression}}
...
//   since-cxx11-note@#cwg1467-f1-deleted {{candidate function has been
//   explicitly deleted}}
...
//   since-cxx11-note@#cwg1467-f2-deleted {{candidate function has been
//   explicitly deleted}}
} // namespace dr_example
...
void g() {
  ...
  //   since-cxx11-note@#cwg1467-f-char-5 {{candidate function has been
  //   explicitly deleted}} since-cxx11-note@#cwg1467-f-char-4 {{candidate
  //   function not viable: expects an rvalue for 1st argument}}
  //   since-cxx11-note@#cwg1467-f-wchar-4 {{candidate function not viable: no
  //   known conversion from 'const char[4]' to 'const wchar_t' for 1st
  //   argument}} since-cxx11-note@#cwg1467-f-wchar-5 {{candidate function not
  //   viable: no known conversion from 'const char[4]' to 'const wchar_t' for
  //   1st argument}} since-cxx11-note@#cwg1467-f-char16-4 {{candidate function
  //   not viable: no known conversion from 'const char[4]' to 'const char16_t'
  //   for 1st argument}} since-cxx11-note@#cwg1467-f-char16-5 {{candidate
  //   function not viable: no known conversion from 'const char[4]' to 'const
  //   char16_t' for 1st argument}} since-cxx11-note@#cwg1467-f-char32-4
  //   {{candidate function not viable: no known conversion from 'const char[4]'
  //   to 'const char32_t' for 1st argument}}
  //   since-cxx11-note@#cwg1467-f-char32-5 {{candidate function not viable: no
  //   known conversion from 'const char[4]' to 'const char32_t' for 1st
  //   argument}}
  f({((("abc")))});
  // since-cxx11-error@-1 {{call to deleted function 'f'}}
  //   since-cxx11-note@#cwg1467-f-char-5 {{candidate function has been
  ...
  f({L"abc"});
  // since-cxx11-error@-1 {{call to deleted function 'f'}}
  //   since-cxx11-note@#cwg1467-f-wchar-5 {{candidate function has been
  //   explicitly deleted}} since-cxx11-note@#cwg1467-f-char-4 {{candidate
  //   function not viable: no known conversion from 'const wchar_t[4]' to
  //   'const char' for 1st argument}} since-cxx11-note@#cwg1467-f-char-5
  //   {{candidate function not viable: no known conversion from 'const
  //   wchar_t[4]' to 'const char' for 1st argument}}
  //   since-cxx11-note@#cwg1467-f-wchar-4 {{candidate function not viable:
  //   expects an rvalue for 1st argument}} since-cxx11-note@#cwg1467-f-char16-4
  //   {{candidate function not viable: no known conversion from 'const
  //   wchar_t[4]' to 'const char16_t' for 1st argument}}
  //   since-cxx11-note@#cwg1467-f-char16-5 {{candidate function not viable: no
  //   known conversion from 'const wchar_t[4]' to 'const char16_t' for 1st
  //   argument}} since-cxx11-note@#cwg1467-f-char32-4 {{candidate function not
  //   viable: no known conversion from 'const wchar_t[4]' to 'const char32_t'
  //   for 1st argument}} since-cxx11-note@#cwg1467-f-char32-5 {{candidate
  //   function not viable: no known conversion from 'const wchar_t[4]' to
  //   'const char32_t' for 1st argument}}
#if __cplusplus >= 202002L
  f2({u8"abc"});
  // since-cxx20-error@-1 {{call to deleted function 'f2'}}
  //   since-cxx20-note@#cwg1467-f2-char8-5 {{candidate function has been
  //   explicitly deleted}} since-cxx20-note@#cwg1467-f2-char8-4 {{candidate
  //   function not viable: expects an rvalue for 1st argument}}
#endif
  f({uR"(abc)"});
  // since-cxx11-error@-1 {{call to deleted function 'f'}}
  //   since-cxx11-note@#cwg1467-f-char16-5 {{candidate function has been
  //   explicitly deleted}} since-cxx11-note@#cwg1467-f-char-4 {{candidate
  //   function not viable: no known conversion from 'const char16_t[4]' to
  //   'const char' for 1st argument}} since-cxx11-note@#cwg1467-f-char-5
  //   {{candidate function not viable: no known conversion from 'const
  //   char16_t[4]' to 'const char' for 1st argument}}
  //   since-cxx11-note@#cwg1467-f-wchar-4 {{candidate function not viable: no
  //   known conversion from 'const char16_t[4]' to 'const wchar_t' for 1st
  //   argument}} since-cxx11-note@#cwg1467-f-wchar-5 {{candidate function not
  //   viable: no known conversion from 'const char16_t[4]' to 'const wchar_t'
  //   for 1st argument}} since-cxx11-note@#cwg1467-f-char16-4 {{candidate
  //   function not viable: expects an rvalue for 1st argument}}
  //   since-cxx11-note@#cwg1467-f-char32-4 {{candidate function not viable: no
  //   known conversion from 'const char16_t[4]' to 'const char32_t' for 1st
  //   argument}} since-cxx11-note@#cwg1467-f-char32-5 {{candidate function not
  //   viable: no known conversion from 'const char16_t[4]' to 'const char32_t'
  //   for 1st argument}}
  f({(UR"(abc)")});
  // since-cxx11-error@-1 {{call to deleted function 'f'}}
  //   since-cxx11-note@#cwg1467-f-char32-5 {{candidate function has been
  //   explicitly deleted}} since-cxx11-note@#cwg1467-f-char-4 {{candidate
  //   function not viable: no known conversion from 'const char32_t[4]' to
  //   'const char' for 1st argument}} since-cxx11-note@#cwg1467-f-char-5
  //   {{candidate function not viable: no known conversion from 'const
  //   char32_t[4]' to 'const char' for 1st argument}}
  //   since-cxx11-note@#cwg1467-f-wchar-4 {{candidate function not viable: no
  //   known conversion from 'const char32_t[4]' to 'const wchar_t' for 1st
  //   argument}} since-cxx11-note@#cwg1467-f-wchar-5 {{candidate function not
  //   viable: no known conversion from 'const char32_t[4]' to 'const wchar_t'
  //   for 1st argument}} since-cxx11-note@#cwg1467-f-char16-4 {{candidate
  //   function not viable: no known conversion from 'const char32_t[4]' to
  //   'const char16_t' for 1st argument}} since-cxx11-note@#cwg1467-f-char16-5
  //   {{candidate function not viable: no known conversion from 'const
  //   char32_t[4]' to 'const char16_t' for 1st argument}}
  //   since-cxx11-note@#cwg1467-f-char32-4 {{candidate function not viable:
...
struct A { // #cwg1482-A
  struct B {
    using A::A;
    // since-cxx11-error@-1 {{using declaration refers into 'A', which is not a
    // base class of 'B'}}
  };

  struct C : A {
    // since-cxx11-error@-1 {{base class has incomplete type}}
    //   since-cxx11-note@#cwg1482-A {{definition of 'cwg1487::A' is not
...
// since-cxx11-error@-1 {{class template partial specialization is not more
...
// since-cxx14-error@-1 {{variable template partial specialization is not more
// specialized than the primary template}}
