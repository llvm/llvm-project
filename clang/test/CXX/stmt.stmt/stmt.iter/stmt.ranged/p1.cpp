...
void g() {
  ...
  for (constexpr int a : A()) {} // expected-error {{constexpr variable 'a' must be initialized by a constant expression}} expected-note {{range-based for loop variable is initialized on each iteration from the loop's iterator, whose value is not known at compile time}}
  int arr[] = {1, 2, 3};
  for (constexpr int a : arr) {} // expected-error {{constexpr variable 'a' must be initialized by a constant expression}} expected-note {{range-based for loop variable is initialized on each iteration}}
