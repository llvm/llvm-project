...
__global__ void test_kernel(int *p, float *f, double *d, unsigned *u) {
  ...
  i += __clz(*p) + __clzll(*p);
  i += __ffs(*p) + __ffs(*u) + __ffsll((long long)*p) +
       __ffsll((unsigned long long)*u);
  ...
  i += __bitinsert_u32(*u, *u, *u, *u) +
       (unsigned)__bitinsert_u64(*u, *u, *u, *u);
  ...
  *d = __int2double_rn(*p) + __uint2double_rn(*u);
  ...
  int s =
      __shfl(*p, 1) + __shfl_up(*p, 1) + __shfl_down(*p, 1) + __shfl_xor(*p, 1);
  s +=
      (int)__shfl(*u, 1, 32) + (int)__shfl_down(*f, 1) + (int)__shfl_xor(*d, 1);
  ...
  s += __all_sync(mask, *p) + __any_sync(mask, *p) +
       (int)__ballot_sync(mask, *p);
  int pred;
