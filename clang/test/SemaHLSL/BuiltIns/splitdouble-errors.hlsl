...
void test_const_second_arg(double D) {
  ...
  uint B;
  __builtin_hlsl_elementwise_splitdouble(D, A, B);
 // expected-error@-1 {{cannot bind non-lvalue argument 'A' to out parameter}}
}
...
void test_const_third_arg(double D) {
  ...
  const uint B = 1;
  __builtin_hlsl_elementwise_splitdouble(D, A, B);
 // expected-error@-1 {{cannot bind non-lvalue argument 'B' to out parameter}}
}

void test_number_second_arg(double D) {
  uint B;
  __builtin_hlsl_elementwise_splitdouble(D, (uint)1, B);
 // expected-error@-1 {{cannot bind non-lvalue argument '(uint)1' to out parameter}}
}
