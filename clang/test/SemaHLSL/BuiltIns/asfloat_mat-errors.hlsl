...
float4x4 test_float_too_many_arg(float4x4 p0, float4x4 p1) {
...
float4x4 test_float_double(double4x4 p1) {
    return asfloat(p1);
    // expected-error@hlsl/hlsl_intrinsics.h:* {{no matching function for call to 'bit_cast'}}
    // expected-note@-2 {{in instantiation of function template specialization 'hlsl::asfloat<double, 4, 4>'}}
    // expected-note@hlsl/hlsl_detail.h:* {{candidate template ignored: substitution failure [with U = float, T = double, R = 4, C = 4]}}
...
float4x4 test_float_half(half4x4 p1) {
    return asfloat(p1);
     // expected-error@hlsl/hlsl_intrinsics.h:* {{no matching function for call to 'bit_cast'}}
    // expected-note@-2 {{in instantiation of function template specialization 'hlsl::asfloat<half, 4, 4>'}}
    // expected-note@hlsl/hlsl_detail.h:* {{candidate template ignored: substitution failure [with U = float, T = half, R = 4, C = 4]}}
    // expected-note@hlsl/hlsl_detail.h:* {{candidate template ignored: substitution failure: too many template arguments for function template 'bit_cast'}}
    // expected-note@hlsl/hlsl_detail.h:* {{candidate template ignored: substitution failure: too many template arguments for function template 'bit_cast'}}
}
