// RUN: %clang_cc1 -Wno-hlsl-implicit-binding -triple dxil-pc-shadermodel6.0-compute -x hlsl -fsyntax-only -verify -DTEXTURE=Texture2D %s
// RUN: %clang_cc1 -Wno-hlsl-implicit-binding -triple dxil-pc-shadermodel6.0-compute -x hlsl -fsyntax-only -verify -DTEXTURE=Texture2DArray %s
...
// expected-error-re@+3 {{constraints not satisfied for class template 'Texture{{.*}}'}}
// expected-note-re@*:* {{because 'Texture{{.*}}<int>' does not satisfy '__is_typed_resource_element_compatible'}}
// expected-note-re@*:* {{because '__builtin_hlsl_is_typed_resource_element_compatible(hlsl::Texture{{.*}}<int>)' evaluated to false}}
...
[numthreads(1,1,1)]
void main() {
  (void)Tex.__handle; // expected-error-re {{'__handle' is a private member of 'hlsl::Texture{{.*}}<vector<float, 3>>'}}
