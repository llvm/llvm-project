...
float2x2 test_too_few_arg(float2x2 p0) {
  return __builtin_elementwise_pow(p0);
  // expected-error@-1 {{too few arguments to function call, expected 2, have 1}}
}

float2x2 test_too_many_arg(float2x2 p0) {
  return __builtin_elementwise_pow(p0, p0, p0);
  // expected-error@-1 {{too many arguments to function call, expected 2, have 3}}
}

int2x2 test_int_mat(int2x2 p0) {
  return __builtin_elementwise_pow(p0, p0);
  // expected-error@-1 {{1st argument must be a scalar or vector of floating-point types (was 'int2x2' (aka 'matrix<int, 2, 2>'))}}
}
