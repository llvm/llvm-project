...
int4x4 test_float_too_many_arg(float4x4 p0, float4x4 p1) {
  return asint(p0, p1);
...
int4x4 test_float_double(double4x4 p1) {
    return asint(p1);
    // expected-error@hlsl/hlsl_intrinsics.h:* {{no matching function for call to 'bit_cast'}}
    // expected-note@-2 {{in instantiation of function template specialization 'hlsl::asint<double, 4, 4>'}}
    // expected-note@hlsl/hlsl_detail.h:* {{candidate template ignored: substitution failure [with U = int, T = double, R = 4, C = 4]}}
...
int4x4 test_float_half(half4x4 p1) {
    return asint(p1);
     // expected-error@hlsl/hlsl_intrinsics.h:* {{no matching function for call to 'bit_cast'}}
    // expected-note@-2 {{in instantiation of function template specialization 'hlsl::asint<half, 4, 4>'}}
    // expected-note@hlsl/hlsl_detail.h:* {{candidate template ignored: substitution failure [with U = int, T = half, R = 4, C = 4]}}
    // expected-note@hlsl/hlsl_detail.h:* {{candidate template ignored: substitution failure: too many template arguments for function template 'bit_cast'}}
    // expected-note@hlsl/hlsl_detail.h:* {{candidate template ignored: substitution failure: too many template arguments for function template 'bit_cast'}}
}
