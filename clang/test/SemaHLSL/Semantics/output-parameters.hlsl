...
// expected-error@-1 {{semantic 'SV_Position' is not supported in pixel shader outputs; it is only available as an input}}

[shader("compute")][numthreads(1,1,1)]
void cs_group_index_out(out uint GI : SV_GroupIndex) { GI = 0; }
// expected-error@-1 {{semantic 'SV_GroupIndex' is not supported in compute shader outputs; it is only available as an input}}
