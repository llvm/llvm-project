...
// expected-error@+1{{'hlsl::is_arrayed' attribute cannot be applied to a declaration}}
...
__hlsl_resource_t [[hlsl::resource_class(UAV)]] [[hlsl::is_arrayed(gibberish)]] res3;

// expected-warning@+1{{attribute 'hlsl::is_arrayed' is already applied}}
__hlsl_resource_t [[hlsl::resource_class(UAV)]] [[hlsl::is_arrayed]] [[hlsl::is_arrayed]] res4;

// expected-error@+2{{attribute 'hlsl::resource_class' can be used only on HLSL intangible type '__hlsl_resource_t'}}
// expected-error@+1{{attribute 'hlsl::is_arrayed' can be used only on HLSL intangible type '__hlsl_resource_t'}}
