...
// CHECK: FieldDecl 0x{{[0-9a-f]+}} <line:[[# @LINE + 3]]:3, col:53> col:53 h '__hlsl_resource_t
// CHECK-SAME{LITERAL}: [[hlsl::resource_class("UAV")]]
struct MyBuffer {
  __hlsl_resource_t [[hlsl::resource_class("UAV")]] h;
};

// CHECK: VarDecl 0x{{[0-9a-f]+}} <line:[[# @LINE + 2]]:1, col:51> col:51 res '__hlsl_resource_t
...
// CHECK: VarDecl 0x{{[0-9a-f]+}} <col:3, col:57> col:57 r '__hlsl_resource_t
...
// CHECK: ClassTemplateSpecializationDecl 0x{{[0-9a-f]+}} <line:[[# @LINE - 4]]:1, line:[[# @LINE - 2]]:1> line:[[# @LINE - 4]]:29 referenced struct MyBuffer2 definition external-linkage instantiated_from 0x{{.+}} implicit_instantiation
// CHECK: TemplateArgument type 'float'
// CHECK: BuiltinType 0x{{[0-9a-f]+}} 'float'
// CHECK: CXXRecordDecl 0x{{[0-9a-f]+}} <col:22, col:29> col:29 implicit struct MyBuffer2
// CHECK: FieldDecl 0x{{[0-9a-f]+}} <line:[[# @LINE - 7]]:3, col:53> col:53 h '__hlsl_resource_t
// CHECK-SAME{LITERAL}: [[hlsl::resource_class("UAV")]]
