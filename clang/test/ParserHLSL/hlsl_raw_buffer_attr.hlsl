...
// CHECK: FieldDecl 0x{{[0-9a-f]+}} <line:[[# @LINE + 4]]:3, col:74> col:74 h1 '__hlsl_resource_t
...
// CHECK: VarDecl 0x{{[0-9a-f]+}} <line:[[# @LINE + 3]]:1, col:72> col:72 h2 '__hlsl_resource_t
...
// CHECK: VarDecl 0x{{[0-9a-f]+}} <col:3, col:74> col:74 h3 '__hlsl_resource_t
// CHECK-SAME{LITERAL}: [[hlsl::resource_class("UAV")]]
// CHECK-SAME{LITERAL}: [[hlsl::raw_buffer]]
void f() {
