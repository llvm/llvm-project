...
; AOT-INTEL-CPU-NOT:  {{.+}}
;
; Test that AOT temp file names ("<stem>_<index>-<unique>.spv"/".out") are
; derived from -o's basename only, not its full path, so they land in the
...
; AOT-STEM-BASENAME:      LLVM backend: input: {{.*}}.bc, output: [[STEM:nested_0]]{{.*}}.spv
