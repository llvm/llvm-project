...
// -fembed-offload-object=%t.out
// RUN: clang-linker-wrapper --host-triple=x86_64-unknown-linux-gnu --dry-run
// --device-compiler=-g \ RUN:   --linker-path=/usr/bin/ld %t.o -o a.out 2>&1 |
// FileCheck %s --check-prefix=NVPTX-LINK-DEBUG

// NVPTX-LINK-DEBUG: clang{{.*}} --target=nvptx64-nvidia-cuda -march=sm_70
// {{.*}}-g
...
// AMDGPU-LINK: clang{{.*}} -o {{.*}}.img -dumpdir a.out.amdgpu9.08.gfx908.img.
// --target=amdgpu9.08-amd-amdhsa -mcpu=gfx908 -Wl,--no-undefined {{.*}}.o
// {{.*}}.o

// RUN: llvm-offload-binary -o %t.out \
// RUN:   --image=file=%t.amdgpu.bc,kind=openmp,triple=amdgpu10.30-amd-amdhsa,arch=gfx1030 \
// RUN:   --image=file=%t.amdgpu.bc,kind=openmp,triple=amdgpu10.30-amd-amdhsa,arch=gfx1030
// RUN: %clang -cc1 %s -triple x86_64-unknown-linux-gnu -emit-obj -o %t.o
// -fembed-offload-object=%t.out
// RUN: clang-linker-wrapper --host-triple=x86_64-unknown-linux-gnu --dry-run
// --device-compiler=--save-temps \ RUN:   --linker-path=/usr/bin/ld %t.o -o
// a.out 2>&1 | FileCheck %s --check-prefix=AMDGPU-LTO-TEMPS

// AMDGPU-LTO-TEMPS: clang{{.*}} --target=amdgpu10.30-amd-amdhsa -mcpu=gfx1030
// {{.*}}-save-temps
...
// RUN:   --image=file=%t.elf.o,kind=openmp,triple=x86_64-unknown-linux-gnu
// RUN: %clang -cc1 %s -triple x86_64-unknown-linux-gnu -emit-obj -o %t.o
// -fembed-offload-object=%t.out RUN: llvm-ar rcs %t.a %t.o
// RUN: clang-linker-wrapper --host-triple=x86_64-unknown-linux-gnu --dry-run \
...
// -fembed-offload-object=%t.out
// RUN: clang-linker-wrapper --host-triple=x86_64-unknown-linux-gnu --dry-run \
// RUN:   --linker-path=/usr/bin/ld %t.a %t-obj.o -o a.out 2>&1 | FileCheck %s
// --check-prefix=STATIC-LIBRARY
...
// --target=nvptx64-nvidia-cuda -march=sm_70 CUDA: clang{{.*}} -o
// [[IMG_SM52:.+]] -dumpdir a.out.nvptx64.sm_52.img.
// --target=nvptx64-nvidia-cuda -march=sm_52 CUDA: fatbinary{{.*}}-64 --create
// {{.*}}.fatbin --image3=kind=elf,sm=70,file=[[IMG_SM70]]
// --image3=kind=elf,sm=52,file=[[IMG_SM52]] CUDA: usr/bin/ld{{.*}}
// {{.*}}.openmp.image.{{.*}}.o {{.*}}.cuda.image.{{.*}}.o
...
// HIP: clang{{.*}} -o [[IMG_GFX90A:.+]] -dumpdir a.out.amdgpu9.0a.gfx90a.img.
// --target=amdgpu9.0a-amd-amdhsa -mcpu=gfx90a HIP: clang{{.*}} -o
// [[IMG_GFX908:.+]] -dumpdir a.out.amdgpu9.08.gfx908.img.
// --target=amdgpu9.08-amd-amdhsa -mcpu=gfx908 HIP:
// clang-offload-bundler{{.*}}-type=o -bundle-align=4096 -compress
// -compression-level=6
// -targets=host-x86_64-unknown-linux-gnu,hip-amdgpu9.0a-amd-amdhsa--gfx90a,hip-amdgpu9.08-amd-amdhsa--gfx908
...
// LINKER-ARGS: clang{{.*}}--target=amdgpu9.08-amd-amdhsa{{.*}}-Xlinker
// foo=bar{{.*}}-Xlinker a{{.*}}foo LINKER-ARGS:
// clang{{.*}}--target=nvptx64-nvidia-cuda{{.*}}-Xlinker foo=bar{{.*}}-Xlinker a
// -Xlinker b{{.*}}foo
...
// RUN: llvm-offload-binary -o %t.out \
// RUN:   --image=file=%t.amdgpu.bc,kind=openmp,triple=amdgpu9.08-amd-amdhsa,arch=gfx908 \
// RUN:   --image=file=%t.amdgpu.bc,kind=openmp,triple=amdgpu9.08-amd-amdhsa,arch=gfx908
// RUN: %clang -cc1 %s -triple x86_64-unknown-linux-gnu -emit-obj -o %t.o
// -fembed-offload-object=%t.out
// RUN: clang-linker-wrapper --host-triple=x86_64-unknown-linux-gnu --dry-run
...
// CLANG-BACKEND: clang{{.*}} -o {{.*}}.img -dumpdir
// a.out.amdgpu9.08.gfx908.img. --target=amdgpu9.08-amd-amdhsa -mcpu=gfx908
// -Wl,--no-undefined {{.*}}.o
...
// RUN:
// --image=file=%t.elf.o,kind=openmp,triple=amdgpu9.0a-amd-amdhsa,arch=gfx90a:xnack+
...
// a.out.amdgpu9.0a.gfx90a:xnack+.img. --target=amdgpu9.0a-amd-amdhsa
// -mcpu=gfx90a:xnack+ -Wl,--no-undefined {{.*}}.o {{.*}}.o AMD-TARGET-ID:
// clang{{.*}} -o {{.*}}.img -dumpdir a.out.amdgpu9.0a.gfx90a:xnack-.img.
// --target=amdgpu9.0a-amd-amdhsa -mcpu=gfx90a:xnack- -Wl,--no-undefined
// {{.*}}.o {{.*}}.o
...
// -fembed-offload-object=%t-off.out
// RUN: clang-linker-wrapper --host-triple=x86_64-unknown-linux-gnu --dry-run \
// RUN:   --linker-path=/usr/bin/ld %t-on.o %t-off.o %t-generic.o -o a.out 2>&1
// | FileCheck %s --check-prefix=AMD-XNACK-SPLIT
...
// AMD-XNACK-SPLIT-DAG: clang{{.*}} --target=amdgcn-amd-amdhsa
// -mcpu=gfx90a:xnack+ -Wl,--no-undefined {{.*}}.o AMD-XNACK-SPLIT-DAG:
// clang{{.*}} --target=amdgcn-amd-amdhsa -mcpu=gfx90a:xnack- -Wl,--no-undefined
// {{.*}}.o AMD-XNACK-SPLIT-DAG: clang{{.*}} --target=amdgcn-amd-amdhsa
// -mcpu=gfx90a -Wl,--no-undefined {{.*}}.o
...
// FileCheck %s --check-prefix=AMD-SRAMECC-ANY-SPLIT

// AMD-SRAMECC-ANY-SPLIT-DAG: clang{{.*}} --target=amdgcn-amd-amdhsa
// -mcpu=gfx90a:sramecc+ -Wl,--no-undefined {{.*}}.o AMD-SRAMECC-ANY-SPLIT-DAG:
// clang{{.*}} --target=amdgcn-amd-amdhsa -mcpu=gfx90a -Wl,--no-undefined
// {{.*}}.o
...
// a.out 2>&1 | FileCheck %s --check-prefix=AMD-SRAMECC-SPLIT

// AMD-SRAMECC-SPLIT-DAG: clang{{.*}} --target=amdgcn-amd-amdhsa
// -mcpu=gfx90a:sramecc+ -Wl,--no-undefined {{.*}}.o AMD-SRAMECC-SPLIT-DAG:
// clang{{.*}} --target=amdgcn-amd-amdhsa -mcpu=gfx90a:sramecc-
// -Wl,--no-undefined {{.*}}.o AMD-SRAMECC-SPLIT-DAG: clang{{.*}}
// --target=amdgcn-amd-amdhsa -mcpu=gfx90a -Wl,--no-undefined {{.*}}.o
...
// RUN: %clang -cc1 %s -triple x86_64-unknown-linux-gnu -emit-obj -o %t-gfx900.o
// -fembed-offload-object=%t-gfx900.out
...
// -Wl,--no-undefined {{.*}}.o AMD-DISTINCT-PROC-DAG: clang{{.*}}
// --target=amdgcn-amd-amdhsa -mcpu=gfx9-generic -Wl,--no-undefined {{.*}}.o

// RUN: llvm-offload-binary -o %t-lib.out \
// RUN:   --image=file=%t.elf.o,kind=openmp,triple=amdgpu-amd-amdhsa,arch=generic
// RUN: %clang -cc1 %s -triple x86_64-unknown-linux-gnu -emit-obj -o %t.o
// -fembed-offload-object=%t-lib.out RUN: llvm-ar rcs %t.a %t.o
...
// RUN:   --image=file=%t.elf.o,kind=openmp,triple=amdgpu-amd-amdhsa,arch=gfx908
...
// ARCH-ALL: clang{{.*}} -o {{.*}}.img -dumpdir a.out.amdgpu.gfx90a.img.
// --target=amdgpu-amd-amdhsa -mcpu=gfx90a -Wl,--no-undefined {{.*}}.o {{.*}}.o
// ARCH-ALL: clang{{.*}} -o {{.*}}.img -dumpdir a.out.amdgpu.gfx908.img.
// --target=amdgpu-amd-amdhsa -mcpu=gfx908 -Wl,--no-undefined {{.*}}.o {{.*}}.o
...
// RUN:   --image=file=%t.elf.o,kind=openmp,triple=amdgcn-amd-amdhsa,arch=gfx90a
// RUN: %clang -cc1 %s -triple x86_64-unknown-linux-gnu -emit-obj -o
// %t-exact-libobj.o -fembed-offload-object=%t-exact-lib.out RUN: llvm-ar rcs
// %t-exact.a %t-exact-libobj.o
// RUN: llvm-offload-binary -o %t-exact-obj.out \
// RUN:   --image=file=%t.elf.o,kind=openmp,triple=amdgcn-amd-amdhsa,arch=gfx90a
// RUN: %clang -cc1 %s -triple x86_64-unknown-linux-gnu -emit-obj -o
// %t-exact-obj.o -fembed-offload-object=%t-exact-obj.out
// RUN: clang-linker-wrapper --host-triple=x86_64-unknown-linux-gnu --dry-run \
// RUN:   --linker-path=/usr/bin/ld %t-exact-obj.o %t-exact.a -o a.out 2>&1 |
// FileCheck %s --check-prefix=AMD-ARCHIVE-EXACT

// AMD-ARCHIVE-EXACT: clang{{.*}} --target=amdgcn-amd-amdhsa -mcpu=gfx90a
// -Wl,--no-undefined {{[^ ]+}}.o {{[^ ]+}}.o{{$}} AMD-ARCHIVE-EXACT-NOT:
// clang{{.*}} -mcpu=gfx90a{{.*}}.o {{.*}}.o {{.*}}.o
...
// RUN:   --image=file=%t.elf.o,kind=openmp,triple=amdgcn-amd-amdhsa,arch=gfx90a
// RUN: %clang -cc1 %s -triple x86_64-unknown-linux-gnu -emit-obj -o
// %t-subexact-libobj.o -fembed-offload-object=%t-subexact-lib.out RUN: llvm-ar
// rcs %t-subexact.a %t-subexact-libobj.o
// RUN: llvm-offload-binary -o %t-subexact-obj.out \
// RUN:   --image=file=%t.elf.o,kind=openmp,triple=amdgpu9.0a-amd-amdhsa
// RUN: %clang -cc1 %s -triple x86_64-unknown-linux-gnu -emit-obj -o
// %t-subexact-obj.o -fembed-offload-object=%t-subexact-obj.out
// RUN: clang-linker-wrapper --host-triple=x86_64-unknown-linux-gnu --dry-run \
// RUN:   --linker-path=/usr/bin/ld %t-subexact-obj.o %t-subexact.a -o a.out
// 2>&1 | FileCheck %s --check-prefix=AMD-ARCHIVE-SUBARCH-EXACT

// AMD-ARCHIVE-SUBARCH-EXACT: clang{{.*}} --target=amdgpu9.0a-amd-amdhsa
// -Wl,--no-undefined {{[^ ]+}}.o {{[^ ]+}}.o{{$}}
// AMD-ARCHIVE-SUBARCH-EXACT-NOT: clang{{.*}} --target={{.*}}-amd-amdhsa
// -Wl,--no-undefined {{.*}}.o {{.*}}.o {{.*}}.o
...
// RUN: %clang -cc1 %s -triple x86_64-unknown-linux-gnu -emit-obj -o
// %t-genlibobj.o -fembed-offload-object=%t-genlib.out RUN: llvm-ar rcs %t-gen.a
// %t-genlibobj.o
// RUN: llvm-offload-binary -o %t-gen-obj.out \
// RUN:   --image=file=%t.elf.o,kind=openmp,triple=amdgcn-amd-amdhsa,arch=gfx900
// RUN: %clang -cc1 %s -triple x86_64-unknown-linux-gnu -emit-obj -o
// %t-gen-obj.o -fembed-offload-object=%t-gen-obj.out
// RUN: clang-linker-wrapper --host-triple=x86_64-unknown-linux-gnu --dry-run \
// RUN:   --linker-path=/usr/bin/ld %t-gen-obj.o %t-gen.a -o a.out 2>&1 |
// FileCheck %s --check-prefix=AMD-ARCHIVE-GENERIC

// AMD-ARCHIVE-GENERIC: clang{{.*}} --target=amdgcn-amd-amdhsa -mcpu=gfx900
// -Wl,--no-undefined {{[^ ]+}}.o {{[^ ]+}}.o{{$}} AMD-ARCHIVE-GENERIC-NOT:
// clang{{.*}} --target=amdgpu9-amd-amdhsa
...
// RUN:   --image=file=%t.elf.o,kind=openmp,triple=amdgcn-amd-amdhsa,arch=gfx90a
// RUN: %clang -cc1 %s -triple x86_64-unknown-linux-gnu -emit-obj -o %t-legacy.o
// -fembed-offload-object=%t-legacy.out
// RUN: llvm-offload-binary -o %t-canon.out \
// RUN:   --image=file=%t.elf.o,kind=openmp,triple=amdgpu-amd-amdhsa,arch=gfx90a
// RUN: %clang -cc1 %s -triple x86_64-unknown-linux-gnu -emit-obj -o %t-canon.o
// -fembed-offload-object=%t-canon.out
// RUN: clang-linker-wrapper --host-triple=x86_64-unknown-linux-gnu --dry-run \
// RUN:   --linker-path=/usr/bin/ld %t-legacy.o %t-canon.o -o a.out 2>&1 |
// FileCheck %s --check-prefix=AMDGPU-TRIPLE-MERGE

// AMDGPU-TRIPLE-MERGE: clang{{.*}} --target=amdgcn-amd-amdhsa -mcpu=gfx90a
// -Wl,--no-undefined {{.*}}.o {{.*}}.o AMDGPU-TRIPLE-MERGE-NOT: clang{{.*}}
// --target=amdgcn-amd-amdhsa -mcpu=gfx90a -Wl,--no-undefined

// Two images whose triples encode the ISA in the subarch
// (amdgpu9.0a-amd-amdhsa) and carry no separate arch must merge into a single
// device link.
// RUN: llvm-offload-binary -o %t-sub1.out \
// RUN:   --image=file=%t.elf.o,kind=openmp,triple=amdgpu9.0a-amd-amdhsa
// RUN: %clang -cc1 %s -triple x86_64-unknown-linux-gnu -emit-obj -o %t-sub1.o
// -fembed-offload-object=%t-sub1.out
// RUN: llvm-offload-binary -o %t-sub2.out \
// RUN:   --image=file=%t.elf.o,kind=openmp,triple=amdgpu9.0a-amd-amdhsa
// RUN: %clang -cc1 %s -triple x86_64-unknown-linux-gnu -emit-obj -o %t-sub2.o
// -fembed-offload-object=%t-sub2.out
// RUN: clang-linker-wrapper --host-triple=x86_64-unknown-linux-gnu --dry-run \
// RUN:   --linker-path=/usr/bin/ld %t-sub1.o %t-sub2.o -o a.out 2>&1 |
// FileCheck %s --check-prefix=AMDGPU-SUBARCH-MERGE

// AMDGPU-SUBARCH-MERGE: clang{{.*}} --target=amdgpu9.0a-amd-amdhsa
// -Wl,--no-undefined {{.*}}.o {{.*}}.o AMDGPU-SUBARCH-MERGE-NOT: clang{{.*}}
// --target=amdgpu9.0a-amd-amdhsa -Wl,--no-undefined
...
// RUN: %clang -cc1 %s -triple x86_64-unknown-linux-gnu -emit-obj -o
// %t-specific.o -fembed-offload-object=%t-specific.out
// RUN: llvm-offload-binary -o %t-major.out \
// RUN:   --image=file=%t.elf.o,kind=openmp,triple=amdgpu9-amd-amdhsa
// RUN: %clang -cc1 %s -triple x86_64-unknown-linux-gnu -emit-obj -o %t-major.o
// -fembed-offload-object=%t-major.out
// RUN: clang-linker-wrapper --host-triple=x86_64-unknown-linux-gnu --dry-run \
// RUN:   --linker-path=/usr/bin/ld %t-specific.o %t-major.o -o a.out 2>&1 |
// FileCheck %s --check-prefix=AMDGPU-MAJOR-SUBARCH-SPLIT

// AMDGPU-MAJOR-SUBARCH-SPLIT-DAG: clang{{.*}} --target=amdgpu9.00-amd-amdhsa
// -Wl,--no-undefined {{.*}}.o AMDGPU-MAJOR-SUBARCH-SPLIT-DAG: clang{{.*}}
// --target=amdgpu9-amd-amdhsa -Wl,--no-undefined {{.*}}.o
...
// RUN: %clang -cc1 %s -triple x86_64-unknown-linux-gnu -emit-obj -o %t-gfx906.o
// -fembed-offload-object=%t-gfx906.out
// RUN: clang-linker-wrapper --host-triple=x86_64-unknown-linux-gnu --dry-run \
// RUN:   --linker-path=/usr/bin/ld %t-gfx900.o %t-gfx906.o -o a.out 2>&1 |
// FileCheck %s --check-prefix=AMDGPU-SAME-MAJOR-NO-MERGE

// AMDGPU-SAME-MAJOR-NO-MERGE-DAG: clang{{.*}} -mcpu=gfx900 -Wl,--no-undefined
// {{.*}}.o AMDGPU-SAME-MAJOR-NO-MERGE-DAG: clang{{.*}} -mcpu=gfx906
// -Wl,--no-undefined {{.*}}.o
...
// --target=x86_64-unknown-linux-gnu RELOCATABLE-LINK: /usr/bin/ld.lld{{.*}}-r
...
// a.out.amdgpu9.0a.gfx90a.img. --target=amdgpu9.0a-amd-amdhsa
// RELOCATABLE-LINK-HIP: clang-offload-bundler{{.*}} -type=o -bundle-align=4096
// -targets=host-x86_64-unknown-linux-gnu,hip-amdgpu9.0a-amd-amdhsa--gfx90a
// -input={{/dev/null|NUL}} -input={{.*}} -output={{.*}} RELOCATABLE-LINK-HIP:
// /usr/bin/ld.lld{{.*}}-r RELOCATABLE-LINK-HIP: llvm-objcopy{{.*}}a.out
// --remove-section .llvm.offloading RELOCATABLE-LINK-HIP: --rename-section
...
// a.out.nvptx64.sm_89.img. --target=nvptx64-nvidia-cuda RELOCATABLE-LINK-CUDA:
// fatbinary{{.*}} -64 --create {{.*}}.fatbin
// --image3=kind=elf,sm=89,file={{.*}}.img RELOCATABLE-LINK-CUDA:
// /usr/bin/ld.lld{{.*}}-r RELOCATABLE-LINK-CUDA: llvm-objcopy{{.*}}a.out
// --remove-section .llvm.offloading

// RUN: %clang -cc1 %s -triple x86_64-unknown-linux-gnu -emit-obj -o %t.o
// RUN: clang-linker-wrapper --host-triple=x86_64-unknown-linux-gnu --dry-run \
// RUN:   --linker-path=/usr/bin/ld --override=image=openmp=%t.o %t.o -o a.out
// 2>&1 \ RUN: | FileCheck %s --check-prefix=OVERRIDE OVERRIDE-NOT: clang
// OVERRIDE: /usr/bin/ld
...
// --check-prefix=LINKER-PATH-NOT-PROVIDED %s LINKER-PATH-NOT-PROVIDED: linker
// path missing, must pass 'linker-path'
