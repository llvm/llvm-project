...
// RUN:
// --image=file=%t.amdgpu.bc,kind=hip,triple=amdgpu9.4-amd-amdhsa,arch=gfx9-4-generic:xnack+
// \ RUN:
// --image=file=%t.amdgpu.bc,kind=hip,triple=amdgpu12.00-amd-amdhsa,arch=gfx1200

// Test that linker wrapper outputs .hipfb file without -r option for HIP
// non-RDC The linker wrapper is called directly with the packaged device binary
// (not embedded in host object) Note: When called directly (not through the
// driver), the linker wrapper processes architectures from the packaged binary.
// The test verifies it can process at least one architecture correctly.
// RUN: %if system-windows %{ \
// RUN:   clang-linker-wrapper --host-triple=x86_64-unknown-linux-gnu
// --wrapper-verbose --device-linker=amdgpu-amd-amdhsa=-v --device-compiler=-v
// --emit-fatbin-only --linker-path=/usr/bin/ld %t.out -o %t.hipfb 2>&1 |
// FileCheck %s --check-prefix=CMD-WIN \
// RUN: %} %else %{ \
// RUN:   clang-linker-wrapper --host-triple=x86_64-unknown-linux-gnu
// --wrapper-verbose --device-linker=amdgpu-amd-amdhsa=-v --device-compiler=-v
// --emit-fatbin-only --linker-path=/usr/bin/ld %t.out -o %t.hipfb 2>&1 |
// FileCheck %s --check-prefix=CMD-LINUX \ RUN: %}

// On Linux, ':' is preserved in file names
// CMD-LINUX-DAG: clang{{.*}} -o
// {{.*}}hipfb.amdgpu9.4.gfx9-4-generic:xnack+{{.*}}.img CMD-LINUX-DAG:
// clang{{.*}} -o {{.*}}hipfb.amdgpu12.00.gfx1200{{.*}}.img CMD-LINUX-DAG:
// ld.lld{{.*}} -o {{.*}}hipfb.amdgpu9.4.gfx9-4-generic:xnack+{{.*}}.img
// CMD-LINUX-DAG: ld.lld{{.*}} -o {{.*}}hipfb.amdgpu12.00.gfx1200{{.*}}.img

// On Windows, ':' is replaced with '@' in file names
// CMD-WIN-DAG: clang{{.*}} -o
// {{.*}}hipfb.amdgpu9.4.gfx9-4-generic@xnack+{{.*}}.img CMD-WIN-DAG:
// clang{{.*}} -o {{.*}}hipfb.amdgpu12.00.gfx1200{{.*}}.img CMD-WIN-DAG:
// ld.lld{{.*}} -o {{.*}}hipfb.amdgpu9.4.gfx9-4-generic@xnack+{{.*}}.img
...
// -targets=hip-amdgpu9.4-amd-amdhsa--gfx9-4-generic:xnack+,hip-amdgpu12.00-amd-amdhsa--gfx1200
