...
// Verify while loop is recognized after sequence of pragma clang loop
// directives.
...
template <typename A> void for_template_test(A *List, int Length, A Value) {
...
#pragma clang loop vectorize_width(V * 2 + VECWIDTH)                           \
    interleave_count(I * 2 + INTCOUNT) unroll_count(U * 2 + UNROLLCOUNT)
  for (int i = 0; i < Length; i++) {
  ...
  const int Scale = 4;
#pragma clang loop vectorize_width(Scale *V) interleave_count(Scale *I)        \
    unroll_count(Scale *U)
  for (int i = 0; i < Length; i++) {
...
#pragma clang loop vectorize_width(16, scalable) interleave_count(4)           \
    unroll(disable) distribute(disable)
  for (int i = 0; i < Length; i++) {
...
#pragma clang loop vectorize_width(fixed) interleave_count(4) unroll(disable)  \
    distribute(disable)
  for (int i = 0; i < Length; i++) {
...
#pragma clang loop vectorize_width(scalable) interleave_count(4)               \
    unroll(disable) distribute(disable)
  for (int i = 0; i < Length; i++) {
...
#pragma clang loop vectorize_width(1, scalable) interleave_count(4)            \
    unroll(disable) distribute(disable)
  for (int i = 0; i < Length; i++) {
...
#pragma clang loop vectorize_width(1) interleave_count(4) unroll(disable)      \
    distribute(disable)
  for (int i = 0; i < Length; i++) {
...
#pragma clang loop vectorize_width(1, fixed) interleave_count(4)               \
    unroll(disable) distribute(disable)
  for (int i = 0; i < Length; i++) {
...
// 10} CHECK-DAG: ![[INTERLEAVE_16:[0-9]+]] = !{!"llvm.loop.interleave.count",
// i32 16}

// CHECK-DAG: ![[VECTORIZE_ENABLE:[0-9]+]] = !{!"llvm.loop.vectorize.enable"}
// CHECK-DAG: ![[FIXED_VEC:[0-9]+]] = !{!"llvm.loop.vectorize.scalable.enable",
// i1 false} CHECK-DAG: ![[SCALABLE_VEC:[0-9]+]] =
// !{!"llvm.loop.vectorize.scalable.enable", i1 true} CHECK-DAG:
// ![[WIDTH_1:[0-9]+]] = !{!"llvm.loop.vectorize.width", i32 1} CHECK-DAG:
// ![[WIDTH_2:[0-9]+]] = !{!"llvm.loop.vectorize.width", i32 2} CHECK-DAG:
// ![[WIDTH_5:[0-9]+]] = !{!"llvm.loop.vectorize.width", i32 5} CHECK-DAG:
// ![[WIDTH_6:[0-9]+]] = !{!"llvm.loop.vectorize.width", i32 6} CHECK-DAG:
// ![[WIDTH_8:[0-9]+]] = !{!"llvm.loop.vectorize.width", i32 8} CHECK-DAG:
// ![[WIDTH_10:[0-9]+]] = !{!"llvm.loop.vectorize.width", i32 10} CHECK-DAG:
// ![[WIDTH_16:[0-9]+]] = !{!"llvm.loop.vectorize.width", i32 16}
...
// ![[UNROLL_DISABLE]], ![[DISTRIBUTE_DISABLE]], ![[WIDTH_8]], ![[FIXED_VEC]],
// ![[INTERLEAVE_4]], ![[VECTORIZE_ENABLE]]}

// CHECK-DAG: ![[LOOP_3]] = distinct !{![[LOOP_3]], ![[MP]], ![[INTERLEAVE_4]],
// ![[VECTORIZE_ENABLE]], ![[FOLLOWUP_VECTOR_3:[0-9]+]]} CHECK-DAG:
// ![[FOLLOWUP_VECTOR_3]] = !{!"llvm.loop.vectorize.followup_all", ![[MP]],
// ![[ISVECTORIZED]], ![[UNROLL_8]]}

// CHECK-DAG: ![[LOOP_4]] = distinct !{![[LOOP_4]], ![[WIDTH_2]],
// ![[FIXED_VEC]], ![[INTERLEAVE_2]], ![[VECTORIZE_ENABLE]]}

// CHECK-DAG: ![[LOOP_5]] = distinct !{![[LOOP_5]], ![[MP]],
// ![[UNROLL_DISABLE]], ![[DISTRIBUTE_DISABLE]], ![[WIDTH_1]]}

// CHECK-DAG: ![[LOOP_6]] = distinct !{![[LOOP_6]], ![[MP]], ![[WIDTH_2]],
// ![[FIXED_VEC]], ![[INTERLEAVE_2]], ![[VECTORIZE_ENABLE]],
// ![[FOLLOWUP_VECTOR_3]]}

// CHECK-DAG: ![[LOOP_7]] = distinct !{![[LOOP_7]], ![[MP]], ![[WIDTH_5]],
// ![[FIXED_VEC]], ![[VECTORIZE_ENABLE]]}

// CHECK-DAG: ![[LOOP_8]] = distinct !{![[LOOP_8]], ![[MP]], ![[WIDTH_5]],
// ![[FIXED_VEC]], ![[VECTORIZE_ENABLE]]}

// CHECK-DAG: ![[LOOP_9]] = distinct !{![[LOOP_9]], ![[MP]], ![[WIDTH_8]],
// ![[FIXED_VEC]], ![[INTERLEAVE_8]], ![[VECTORIZE_ENABLE]],
// ![[FOLLOWUP_VECTOR_3]]}

// CHECK-DAG: ![[LOOP_10]] = distinct !{![[LOOP_10]], ![[MP]], ![[WIDTH_2]],
// ![[FIXED_VEC]], ![[INTERLEAVE_2]], ![[VECTORIZE_ENABLE]],
// ![[FOLLOWUP_VECTOR_3]]}

// CHECK-DAG: ![[LOOP_11]] = distinct !{![[LOOP_11]], ![[MP]], ![[WIDTH_2]],
// ![[FIXED_VEC]], ![[INTERLEAVE_4]], ![[VECTORIZE_ENABLE]],
// ![[FOLLOWUP_VECTOR_3]]}

// CHECK-DAG: ![[LOOP_12]] = distinct !{![[LOOP_12]], ![[MP]], ![[WIDTH_6]],
// ![[FIXED_VEC]], ![[INTERLEAVE_10]], ![[VECTORIZE_ENABLE]],
// ![[FOLLOWUP_VECTOR_12:[0-9]+]]} CHECK-DAG: ![[FOLLOWUP_VECTOR_12]] =
// !{!"llvm.loop.vectorize.followup_all", ![[MP]], ![[ISVECTORIZED]],
// ![[UNROLL_24]]}

// CHECK-DAG: ![[LOOP_13]] = distinct !{![[LOOP_13]], ![[MP]], ![[WIDTH_8]],
// ![[FIXED_VEC]], ![[INTERLEAVE_16]], ![[VECTORIZE_ENABLE]],
// ![[FOLLOWUP_VECTOR_13:[0-9]+]]} CHECK-DAG: ![[FOLLOWUP_VECTOR_13]] =
// !{!"llvm.loop.vectorize.followup_all", ![[MP]], ![[ISVECTORIZED]],
// ![[UNROLL_32]]}

// CHECK-DAG: ![[LOOP_14]] = distinct !{![[LOOP_14]], ![[MP]], ![[WIDTH_10]],
// ![[FIXED_VEC]], ![[VECTORIZE_ENABLE]]}

// CHECK-DAG: ![[LOOP_15]] = distinct !{![[LOOP_15]], ![[MP]],
// ![[UNROLL_DISABLE]], ![[DISTRIBUTE_DISABLE]], ![[WIDTH_16]], ![[FIXED_VEC]],
// ![[INTERLEAVE_4]], ![[VECTORIZE_ENABLE]]}

// CHECK-DAG: ![[LOOP_16]] = distinct !{![[LOOP_16]], ![[MP]],
// ![[UNROLL_DISABLE]], ![[DISTRIBUTE_DISABLE]], ![[WIDTH_16]],
// ![[SCALABLE_VEC]], ![[INTERLEAVE_4]], ![[VECTORIZE_ENABLE]]}

// CHECK-DAG: ![[LOOP_17]] = distinct !{![[LOOP_17]], ![[MP]],
// ![[UNROLL_DISABLE]], ![[DISTRIBUTE_DISABLE]], ![[FIXED_VEC]],
// ![[INTERLEAVE_4]], ![[VECTORIZE_ENABLE]]} CHECK-DAG: ![[LOOP_18]] = distinct
// !{![[LOOP_18]], ![[MP]], ![[UNROLL_DISABLE]], ![[DISTRIBUTE_DISABLE]],
// ![[SCALABLE_VEC]], ![[INTERLEAVE_4]], ![[VECTORIZE_ENABLE]]} CHECK-DAG:
// ![[LOOP_19]] = distinct !{![[LOOP_19]], ![[MP]], ![[UNROLL_DISABLE]],
// ![[DISTRIBUTE_DISABLE]], ![[WIDTH_1]], ![[SCALABLE_VEC]], ![[INTERLEAVE_4]],
// ![[VECTORIZE_ENABLE]]} CHECK-DAG: ![[LOOP_20]] = distinct !{![[LOOP_20]],
// ![[MP]], ![[UNROLL_DISABLE]], ![[DISTRIBUTE_DISABLE]], ![[WIDTH_1]],
// ![[FIXED_VEC]], ![[INTERLEAVE_4]]} CHECK-DAG: ![[LOOP_21]] = distinct
// !{![[LOOP_21]], ![[MP]], ![[UNROLL_DISABLE]], ![[DISTRIBUTE_DISABLE]],
// ![[WIDTH_1]], ![[FIXED_VEC]], ![[INTERLEAVE_4]]} CHECK-DAG: ![[LOOP_22]] =
// distinct !{![[LOOP_22]], ![[MP]], ![[WIDTH_1]], ![[ISVECTORIZED]],
// ![[UNROLL_8]]} CHECK-DAG: ![[LOOP_23]] = distinct !{![[LOOP_23]], ![[MP]],
// ![[WIDTH_1]], ![[INTERLEAVE_4]], ![[ISVECTORIZED]], ![[UNROLL_8]]}
