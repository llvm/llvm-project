...
// FileCheck -check-prefixes=X64,CHECK %s RUN: %clang_cc1 -std=c++11 -triple
// amdgpu %s -emit-llvm -o - | FileCheck -check-prefixes=AMDGCN,CHECK %s
...
void test0(void *array, int n) {
  ...
  // AMDGCN-NEXT:   [[ARRAY:%.*]] = addrspacecast ptr addrspace(5) [[ARRAY0]] to
  // ptr AMDGCN-NEXT:   [[N:%.*]] = addrspacecast ptr addrspace(5) [[N0]] to ptr
...
void test2(int b) {
  ...
  // CHECK-NEXT: [[VLA_NUM_ELEMENTS_PRE:%.*]] = zext i32
  // [[VLA_NUM_ELEMENTS_PREZEXT]]
  ...
  // CHECK-NEXT: [[VLA_END_PTR:%.*]] = getelementptr inbounds nuw i32, ptr
  // {{%.*}}, i64 [[VLA_NUM_ELEMENTS_POST]] X64-NEXT: store ptr [[VLA_END_PTR]],
  // ptr %__end1 AMDGCN-NEXT: store ptr [[VLA_END_PTR]], ptr [[END]]
  for (int d : varr)
    0;
}

void test3(int b, int c) {
  // CHECK-LABEL: define{{.*}} void {{.*}}test3{{.*}}(i32 noundef %b, i32
  // noundef %c)
  int varr[b][c];
  // AMDGCN: %__end1 = alloca ptr, align 8, addrspace(5)
  // AMDGCN: [[END:%.*]] = addrspacecast ptr addrspace(5) %__end1 to ptr
  ...
  // multiply the two dimensions, then by the element type and then divide by
  // the sizeof dim2
  // CHECK: [[VLA_DIM1_X_DIM2:%.*]] = mul nuw i64 [[VLA_DIM1_PRE]],
  // [[VLA_DIM2_PRE]] CHECK-NEXT: [[VLA_SIZEOF:%.*]] = mul nuw i64 4,
  // [[VLA_DIM1_X_DIM2]] CHECK-NEXT: [[VLA_SIZEOF_DIM2:%.*]] = mul nuw i64 4,
  // [[VLA_DIM2_PRE]] CHECK-NEXT: [[VLA_NUM_ELEMENTS:%.*]] = udiv i64
  // [[VLA_SIZEOF]], [[VLA_SIZEOF_DIM2]] CHECK-NEXT: [[VLA_END_INDEX:%.*]] = mul
  // nsw i64 [[VLA_NUM_ELEMENTS]], [[VLA_DIM2_PRE]] CHECK-NEXT:
  // [[VLA_END_PTR:%.*]] = getelementptr inbounds nuw i32, ptr {{%.*}}, i64
  // [[VLA_END_INDEX]] X64-NEXT: store ptr [[VLA_END_PTR]], ptr %__end
