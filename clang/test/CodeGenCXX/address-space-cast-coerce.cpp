...
template <typename T> struct my_vector_base<T, 1> {
...
struct my_vector_type : public my_vector_base<T, rank> {
  ...
  template <typename U> my_vector_type(U x) noexcept {
...
inline my_vector_type<T, n> operator+(const my_vector_type<T, n> &x,
                                      const my_vector_type<T, n> &y) noexcept {
