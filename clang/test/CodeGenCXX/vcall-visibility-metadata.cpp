...
// CHECK-MS: @anon.{{.*}} = private constant {{.*}}@"??_R4A@?A{{.*}}@@6B@"{{.*}} !vcall_visibility [[VIS_TU:![0-9]+]]
struct A {
  A() {}
...
// CHECK-MS: @anon.{{.*}} = private constant {{.*}}@"??_R4B@@6B@"{{.*}} !vcall_visibility [[VIS_DSO:![0-9]+]]
...
// CHECK-MS: @anon.{{.*}} = private constant {{.*}}@"??_R4C@@6B@"{{.*}} !vcall_visibility [[VIS_DSO]]
...
// CHECK-MS-NOT: @anon.{{.*}} = private constant {{.*}}@"??_R4D@@6B@"{{.*}} !vcall_visibility
...
// On MS default is hidden
// CHECK-MS: @anon.{{.*}} = private constant {{.*}}@"??_R4E@@6B@"{{.*}} !vcall_visibility [[VIS_DSO]]
...
// On MS default is hidden
// CHECK-MS: @anon.{{.*}} = private constant {{.*}}@"??_R4F@?A{{.*}}@@6B@"{{.*}} !vcall_visibility [[VIS_DSO]]
namespace {
...
// CHECK-MS: @anon.{{.*}} = private constant {{.*}}@"??_R4G@?A{{.*}}@@6B@"{{.*}} !vcall_visibility [[VIS_DSO]]
namespace {
