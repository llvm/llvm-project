...
// FileCheck %s -check-prefix PREPROCESSOR PREPROCESSOR: #define
// __WASM_EXCEPTIONS__ 1
...
// CHECK-NEXT:           to label %[[NORMAL_BB:.*]] unwind label
// %[[CATCHDISPATCH_BB:.*]]
...
// CHECK: [[CATCHSTART_BB]]:
// CHECK-NEXT:   %[[CATCHPAD:.*]] = catchpad within %[[CATCHSWITCH]] [ptr
// @_ZTIi, ptr @_ZTId] CHECK-NEXT:   %[[EXN:.*]] = call ptr
// @llvm.wasm.get.exception(token %[[CATCHPAD]]) CHECK-NEXT:   store ptr
// %[[EXN]], ptr %exn.slot CHECK-NEXT:   %[[SELECTOR:.*]] = call i32
// @llvm.wasm.get.ehselector(token %[[CATCHPAD]]) CHECK-NEXT:   %[[TYPEID:.*]] =
// call i32 @llvm.eh.typeid.for.p0(ptr @_ZTIi) {{.*}} CHECK-NEXT:
// %[[MATCHES:.*]] = icmp eq i32 %[[SELECTOR]], %[[TYPEID]] CHECK-NEXT:   br i1
// %[[MATCHES]], label %[[CATCH_INT_BB:.*]], label %[[CATCH_FALLTHROUGH_BB:.*]]

// CHECK: [[CATCH_INT_BB]]:
// CHECK-NEXT:   %[[EXN:.*]] = load ptr, ptr %exn.slot
// CHECK-NEXT:   %[[ADDR:.*]] = call ptr @__cxa_begin_catch(ptr %[[EXN]]) {{.*}}
// [ "funclet"(token %[[CATCHPAD]]) ] CHECK-NEXT:   %[[INT_VAL:.*]] = load i32,
// ptr %[[ADDR]] CHECK-NEXT:   store i32 %[[INT_VAL]], ptr %[[INT_ALLOCA]]
// CHECK-NEXT:   call void @_Z10dont_throwv() {{.*}} [ "funclet"(token
// %[[CATCHPAD]]) ] CHECK-NEXT:   call void @__cxa_end_catch() {{.*}} [
// "funclet"(token %[[CATCHPAD]]) ] CHECK-NEXT:   catchret from %[[CATCHPAD]] to
// label %[[CATCHRET_DEST_BB0:.*]]
...
// CHECK: [[RETHROW_BB]]:
// CHECK-NEXT:   call void @llvm.wasm.rethrow() {{.*}} [ "funclet"(token
// %[[CATCHPAD]]) ] CHECK-NEXT:   unreachable
...
// CHECK: [[CATCHSTART_BB]]:
// CHECK-NEXT:   %[[CATCHPAD:.*]] = catchpad within %[[CATCHSWITCH]] [ptr
// @_ZTIi, ptr null] CHECK:   br i1 %{{.*}}, label %[[CATCH_INT_BB:.*]], label
// %[[CATCH_ALL_BB:.*]]
...
// CHECK-NEXT:   call noundef ptr @_ZN7CleanupD1Ev(ptr {{[^,]*}} %{{.*}}) {{.*}}
// [ "funclet"(token %[[CLEANUPPAD]]) ] CHECK-NEXT:   cleanupret from
// %[[CLEANUPPAD]] unwind to caller
...
// CHECK-NEXT:           to label %[[INVOKE_CONT_BB:.*]] unwind label
// %[[EHCLEANUP_BB:.*]]

// CHECK: [[INVOKE_CONT_BB]]:
// CHECK-NEXT:   call void @__cxa_end_catch() {{.*}} [ "funclet"(token
// %[[CATCHPAD]]) ] CHECK-NEXT:   catchret from %[[CATCHPAD]] to label

// CHECK: [[EHCLEANUP_BB]]:
// CHECK-NEXT:   %[[CLEANUPPAD:.*]] = cleanuppad within %[[CATCHPAD]] []
// CHECK-NEXT:   call void @__cxa_end_catch() {{.*}} [ "funclet"(token
// %[[CLEANUPPAD]]) ] CHECK-NEXT:   cleanupret from %[[CLEANUPPAD]] unwind to
// caller
...
// CHECK:   invoke void @_Z9may_throwv() [ "funclet"(token %[[CATCHPAD]]) ]
// CHECK-NEXT:           to label %[[INVOKE_CONT_BB0:.*]] unwind label
// %[[EHCLEANUP_BB:.*]]
...
// CHECK-NEXT:   invoke void @__cxa_end_catch() [ "funclet"(token
// %[[CLEANUPPAD0]]) ] CHECK-NEXT:           to label %[[INVOKE_CONT_BB1:.*]]
// unwind label %[[TERMINATE_BB:.*]]
...
// CHECK-NEXT:   call void @_ZSt9terminatev() {{.*}} [ "funclet"(token
// %[[CLEANUPPAD1]]) ] CHECK-NEXT:   unreachable
...
// CHECK-NEXT:   call noundef ptr @_ZN7CleanupD1Ev(ptr {{.*}}) {{.*}} [
// "funclet"(token %[[CLEANUPPAD0]]) ] CHECK-NEXT:   cleanupret from
// %[[CLEANUPPAD0]] unwind label %[[CATCH_DISPATCH_BB:.*]]

// CHECK: [[CATCH_DISPATCH_BB]]:
// CHECK-NEXT:  %[[CATCHSWITCH:.*]] = catchswitch within none [label
// %[[CATCHSTART_BB:.*]]] unwind label %[[EHCLEANUP_BB1:.*]]

// CHECK: [[CATCHSTART_BB]]:
// CHECK-NEXT:   %[[CATCHPAD:.*]] = catchpad within %[[CATCHSWITCH]] [ptr
// @_ZTIi] CHECK:   br i1 %{{.*}}, label %[[CATCH_INT_BB:.*]], label
// %[[RETHROW_BB:.*]]
...
// CHECK: [[RETHROW_BB]]:
// CHECK-NEXT:   invoke void @llvm.wasm.rethrow() {{.*}} [ "funclet"(token
// %[[CATCHPAD]]) ] CHECK-NEXT:          to label %[[UNREACHABLE_BB:.*]] unwind
// label %[[EHCLEANUP_BB1:.*]]

// CHECK: [[EHCLEANUP_BB2]]:
// CHECK-NEXT:   %[[CLEANUPPAD2:.*]] = cleanuppad within %[[CATCHPAD]] []
// CHECK-NEXT:   call noundef ptr @_ZN7CleanupD1Ev(ptr {{[^,]*}} %{{.*}}) {{.*}}
// [ "funclet"(token %[[CLEANUPPAD2]]) ] CHECK-NEXT:   cleanupret from
// %[[CLEANUPPAD2]] unwind label %[[EHCLEANUP_BB3:.*]]
...
// CHECK-NEXT:   call noundef ptr @_ZN7CleanupD1Ev(ptr {{[^,]*}} %{{.*}}) {{.*}}
// [ "funclet"(token %[[CLEANUPPAD1]]) ] CHECK-NEXT:   cleanupret from
// %[[CLEANUPPAD1]] unwind to caller
...
// CHECK:   invoke void @llvm.wasm.rethrow() {{.*}} [ "funclet"(token
// %[[CATCHPAD0]]) ]
...
// CHECK:   call void @_ZSt9terminatev() {{.*}} [ "funclet"(token
// %[[CLEANUPPAD7]]) ] CHECK:   unreachable
...
// CHECK:   call void @llvm.wasm.rethrow() {{.*}} [ "funclet"(token
// %[[CATCHPAD0]]) ] CHECK:   unreachable
...
void noexcept_throw() noexcept { throw 3; }
...
// %[[CATCHPAD]]) CHECK-NEXT:   store ptr %[[EXN]], ptr %exn.slot CHECK-NEXT:
// {{.*}} call i32 @llvm.wasm.get.ehselector(token %[[CATCHPAD]]) CHECK-NEXT: br
// label %[[CATCH_ALL_BB:.*]]

// CHECK: [[CATCH_ALL_BB]]:
// CHECK-NEXT:   %[[EXN_LOAD:.*]] = load ptr, ptr %exn.slot
// CHECK-NEXT:   call ptr @__cxa_begin_catch(ptr %[[EXN_LOAD]]) {{.*}} [
// "funclet"(token %[[CATCHPAD]]) ] CHECK-NEXT:   call {{.*}} @_ZTW3tls() [
...
// RUN: %clang_cc1 %s -triple wasm32-unknown-unknown -fms-extensions
// -fexceptions -fcxx-exceptions -mllvm -wasm-enable-eh -exception-model=wasm
// -target-feature +exception-handling -emit-llvm -o - -std=c++11 2>&1 |
// FileCheck %s --check-prefix=WARNING-DEFAULT RUN: %clang_cc1 %s -triple
// wasm32-unknown-unknown -fms-extensions -fexceptions -fcxx-exceptions -mllvm
// -wasm-enable-eh -exception-model=wasm -target-feature +exception-handling
// -Wwasm-exception-spec -emit-llvm -o - -std=c++11 2>&1 | FileCheck %s
// --check-prefix=WARNING-ON RUN: %clang_cc1 %s -triple wasm32-unknown-unknown
// -fms-extensions -fexceptions -fcxx-exceptions -mllvm -wasm-enable-eh
// -exception-model=wasm -target-feature +exception-handling
// -Wno-wasm-exception-spec -emit-llvm -o - -std=c++11 2>&1 | FileCheck %s
// --check-prefix=WARNING-OFF RUN: %clang_cc1 %s -triple wasm32-unknown-unknown
// -fexceptions -fcxx-exceptions -emit-llvm -o - -std=c++11 2>&1 | FileCheck %s
...
// currently ignored in wasm WARNING-ON: warning: dynamic exception
// specifications with types are currently ignored in wasm WARNING-OFF-NOT:
// warning: dynamic exception specifications with types are currently ignored in
// wasm EM-EH-WARNING: warning: dynamic exception specifications with types are
...
void exception_spec_throw_empty() throw() { throw 3; }
// WARNING-DEFAULT: warning: 'exception_spec_throw_empty' has a non-throwing
// exception specification but can still throw WARNING-DEFAULT: function
