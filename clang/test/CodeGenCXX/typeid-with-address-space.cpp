// RUN: %clang_cc1 -I%S %s -triple amdgcn-amd-amdhsa -emit-llvm -fcxx-exceptions -fexceptions -o - | FileCheck %s --check-prefix=AMDGCN-DEFAULT-AS --check-prefix=CHECK
// RUN: %clang_cc1 -I%S %s -triple spirv64-amd-amdhsa -emit-llvm -fcxx-exceptions -fexceptions -o - | FileCheck %s --check-prefix=WITH-NONZERO-DEFAULT-AS --check-prefix=CHECK
#include <typeinfo>
...
// AMDGCN-DEFAULT-AS: define{{.*}} ptr @_ZN5Test11fEv
// AMDGCN-DEFAULT-AS-SAME:  personality ptr @__gxx_personality_v0
// WITH-NONZERO-DEFAULT-AS: define{{.*}} ptr @_ZN5Test11fEv() addrspace(4)
