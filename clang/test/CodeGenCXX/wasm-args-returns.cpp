...
test(two_fields);
// CHECK: define void @_Z7forward10two_fields(ptr dead_on_unwind noalias nofree writable writeonly sret(%struct.two_fields) align 8 captures(none) initializes((0, 16)) %{{.*}}, ptr nofree readonly byval(%struct.two_fields) align 8 captures(none) %{{.*}})
//
...
test(copy_ctor);
// CHECK: define void @_Z7forward9copy_ctor(ptr dead_on_unwind noalias {{[^,]*}} sret(%struct.copy_ctor) align 8 %{{.*}}, ptr nofreeobj align 8 dead_on_return dereferenceable(8) %{{.*}})
//
...
// CHECK: call void @_Z3use9copy_ctor(ptr nofreeobj align 8 dead_on_return dereferenceable(8) %[[tmp]])
// CHECK: ret void
//
// CHECK: declare void @_Z3use9copy_ctor(ptr nofreeobj align 8 dead_on_return dereferenceable(8))
...
// CHECK: define void @_Z7forward17aligned_copy_ctor(ptr dead_on_unwind noalias {{[^,]*}} sret(%struct.aligned_copy_ctor) align 16 %{{.*}}, ptr nofreeobj align 16 dead_on_return dereferenceable(16) %{{.*}})
//
...
// CHECK: call void @_Z3use17aligned_copy_ctor(ptr nofreeobj align 16 dead_on_return dereferenceable(16) %[[tmp]])
// CHECK: ret void
//
// CHECK: declare void @_Z3use17aligned_copy_ctor(ptr nofreeobj align 16 dead_on_return dereferenceable(16))
