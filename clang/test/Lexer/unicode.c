...
// RUN: %clang_cc1 -fsyntax-only -verify=expected,c2x,math -x c -std=c2x %s
// RUN: %clang_cc1 -fsyntax-only -verify=expected,cxx,math,ext -x c++ -std=c++11
// %s RUN: %clang_cc1 -fsyntax-only -verify=expected,cxx -x c++ -std=c++2d %s
// RUN: %clang_cc1 -fsyntax-only -verify=expected,cxx,math,compat -x c++
// -std=c++2d -Wpre-c++2d-compat %s RUN: %clang_cc1 -std=c99 -E -DPP_ONLY=1 %s |
// FileCheck %s --strict-whitespace RUN: %clang_cc1 -E -DPP_ONLY=1 %s |
// FileCheck %s --strict-whitespace UNSUPPORTED: system-zos

// This file contains Unicode characters; please do not "fix" them!

extern int x; // expected-error {{treating character ' ' U+00A0 as whitespace}}
extern int　x; // expected-error {{treating character '　' U+3000 as
               // whitespace}}
...
extern int a\N{ZWJ};
extern int
    a\N{TANGSALETTERGA}; // expected-error {{'TANGSALETTERGA' is not a valid Unicode character name}} \
                                // expected-error {{expected ';' after top level declarator}} \
                                // expected-note {{character names in Unicode escape sequences are sensitive to case and whitespace}}

// Mathematical notation characters are standard in C++ (P3658R1) and a Clang
// extension in C. The 'math' prefix matches whichever warning applies.
extern int
    𝛛; // c2x-warning {{mathematical notation character '𝛛' U+1D6DB in an identifier is a Clang extension}} \
                 ext-warning {{mathematical notation character '𝛛' U+1D6DB in an identifier is a C++2d extension}} \
                 compat-warning {{mathematical notation character '𝛛' U+1D6DB in an identifier is incompatible with C++ standards before C++2d}}
extern int ₉; // expected-error {{character '₉' U+2089 not allowed at the start of an identifier}} \\
                 expected-warning {{declaration does not declare anything}}

extern int a\N{PICKLE}; // expected-error {{character '🫝' U+1FADD not allowed
                        // in an identifier}}

int a¹b₍₄₂₎∇; // math-warning 6{{mathematical notation character}}

int \u{221E} = 1; // math-warning {{mathematical notation character}}
int \N{MATHEMATICAL SANS-SERIF BOLD ITALIC PARTIAL DIFFERENTIAL} = 1;
// math-warning@-1 {{mathematical notation character}}

int a\N{SUBSCRIPT EQUALS SIGN} =
    1; // math-warning {{mathematical notation character}}
...
int n; = 3; // expected-warning {{treating character ';' U+037E as an identifier
            // character rather than as ';' symbol}}
int *n꞉꞉v = &n;; // expected-warning 2{{treating character '꞉' U+A789 as an
                 // identifier character rather than as ':' symbol}}
                 // expected-warning@-1 {{treating character ';' U+037E as an
                 // identifier character rather than as ';' symbol}}
int v＝［＝］（auto）｛return～x；｝（）; // expected-warning 12{{treating
                                          // character}}

int ⁠x﻿x‍;
// expected-warning@-1 {{identifier contains character U+2060 that is invisible
// in some environments}} expected-warning@-2 {{identifier contains character
// U+FEFF that is invisible in some environments}} expected-warning@-3
// {{identifier contains character U+200D that is invisible in some
// environments}}
int foo​bar = 0; // expected-warning {{identifier contains character U+200B
