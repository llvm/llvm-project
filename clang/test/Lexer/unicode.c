...
// RUN: %clang_cc1 -fsyntax-only -verify=expected,cxx,math,ext -x c++ -std=c++11 %s
// RUN: %clang_cc1 -fsyntax-only -verify=expected,cxx -x c++ -std=c++2d %s
// RUN: %clang_cc1 -fsyntax-only -verify=expected,cxx,math,compat -x c++ -std=c++2d -Wpre-c++2d-compat %s
...
extern int 𝛛; // c2x-warning {{mathematical notation character '𝛛' U+1D6DB in an identifier is a Clang extension}} \
...
int a\N{SUBSCRIPT EQUALS SIGN} = 1; // math-warning {{mathematical notation character}}
