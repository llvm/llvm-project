...
template <typename T>
void f(T *); // expected-note 2{{could not match 'T *' against 'double'}}
template <typename T> void nondep();

template <typename> struct X1 {
  friend void qualified_friend::f(
      double); // expected-error {{friend declaration of 'f' does not match any
               // declaration in namespace 'qualified_friend'}}
  friend void qualified_friend::g(); // expected-error {{friend declaration of
                                     // 'g' does not match any declaration in
                                     // namespace 'qualified_friend'}}
};
template <typename T> struct X2 {
  friend void qualified_friend::f(
      T); // expected-error {{friend declaration of 'f' does not match any
          // declaration in namespace 'qualified_friend'}}
};
X1<int> xi;
...
template <typename> struct Z1 {
  friend void
  Y::f(double); // expected-error {{friend declaration of 'f' does not match any
                // declaration in 'qualified_friend::Y'}}
  friend void Y::g(); // expected-error {{friend declaration of 'g' does not
                      // match any declaration in 'qualified_friend::Y'}}
};
template <typename T> struct Z2 {
  friend void Y::f(T); // expected-error {{friend declaration of 'f' does not
                       // match any declaration in 'qualified_friend::Y'}}
};
Z1<int> zi;
...
template <typename T> struct A {
  friend void
      N::f(T); // expected-error {{friend declaration of 'f' does not match}}
};
namespace N {
void f();
} // namespace N
...
int n =
    f((X1 *)nullptr); // expected-error {{cannot initialize a variable of type
                      // 'int' with an rvalue of type 'std::nullptr_t'}}
} // namespace PR42513_comment3
...
template <typename T> class Bar {
  template <typename U> friend void foo(U &&arg) {} // no diagnostic expected
};
} // namespace A

void foobar() { A::Bar<int> b; }
} // namespace GH61851
