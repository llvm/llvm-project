...
struct CheckSimpleTypes {
  static_assert(!__is_same( // expected-error {{static assertion failed}}
                            // expected-note@* {{in instantiation of template class 'CheckSimpleTypes}}
    Types<__builtin_sort_pack<int, double, int, double>...>,
    Types<double, double, int, int>)); 
};
template struct CheckSimpleTypes<int>;

__builtin_sort_pack<int, double> err1; // expected-error {{contains an unexpanded}}
Types<__builtin_sort_pack<int, double>*> err2; // expected-error {{contains an unexpanded}}
Types<const __builtin_sort_pack<int, double>> *err3; // expected-error {{contains an unexpanded}}

template <template <class...> class Inner>
struct Wrapper {
  using result = Inner<int,int,int>*;
};
Types<Wrapper<__builtin_sort_pack>::result> *err11; // expected-error  {{use of template '__builtin_sort_pack' requires template arguments}} \
