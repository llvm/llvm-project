...
template <int I, int J, int K> void car() {
  ...
  __attribute__((
      address_space(I))) int local; // expected-error {{automatic variable
                                    // qualified with an address space}}
  __attribute__((
      address_space(J))) int array[5]; // expected-error {{automatic variable
                                       // qualified with an address space}}
  __attribute__((address_space(
      I))) int arrarr[5][5]; // expected-error {{automatic variable qualified
                             // with an address space}}

  __attribute__((address_space(J))) *
  ...
  struct HasASFields {
    AS2Int typedef_as_field; // expected-error {{field may not be qualified with
                             // an address space}}
  };
...
template <int I> struct HasASTemplateFields {
  __attribute__((
      address_space(I))) int as_field; // expected-error {{field may not be
                                       // qualified with an address space}}
};

template <int I, int J>
void foo(__attribute__((address_space(I))) float
             *a, // expected-note {{candidate template ignored: substitution
                 // failure [with I = 1, J = 2]: parameter may not be qualified
                 // with an address space}}
...
template void
foo<1, 2>(float *,
          float); // expected-error {{explicit instantiation of 'foo' does not
                  // refer to a function template, variable template, member
                  // function, member class, or static data member}}

template <int I> void neg() {
  __attribute__((address_space(
      I))) int *bounds; // expected-error {{address space is negative}}
}

template <long int I> void tooBig() {
  __attribute__((address_space(
      I))) int *bounds; // expected-error {{address space is larger than the
                        // maximum supported (8388579)}}
}

template <long int I> void correct() {
...
char *cmp(__attribute__((address_space(I))) char *x,
  ...
  return x < y ? x : y; // expected-error {{comparison of distinct pointer types
...
template <int I> struct fooFunction {
  __attribute__((address_space(I))) void **const base = 0;

  void *get_0(void) {
    return base[0]; // expected-error {{cannot initialize return object of type
                    // 'void *' with an lvalue of type
                    // '__attribute__((address_space(1))) void *}}
  }

  __attribute__((
      address_space(I))) ft qf; // expected-error {{function type may not be
                                // qualified with an address space}}
  ...
  void test3(void) {
    ...
    test3_helper(test3_val); // expected-error {{cannot initialize a parameter
                             // of type 'char *' with an lvalue of type
...
template <unsigned A>
    int __attribute__((address_space(A))) *
    different_template(); // expected-note {{candidate function [with A = 0]}}
template <unsigned B>
    int __attribute__((address_space(B + 1))) *
    different_template(); // expected-note {{candidate function [with B = 0]}}
void test_different_template() {
  (void)different_template<0>();
...
int main() {
  ...
  static_assert(
      partial_spec_deduce_as<int __attribute__((address_space(3))) *>::value ==
          3,
      "address space value has been incorrectly deduced");

  return 0;
}

namespace gh101685 {
template <int AS> using ASPtrTy = void [[clang::address_space(AS)]] *;

template <int AS> struct EntryTy {
  ASPtrTy<AS> Base;
};
