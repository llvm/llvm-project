...
// RUN: cd %t/tmp && %clang_cc1 -fmodules -fimplicit-module-maps -fsyntax-only -I %t %t/tu4.c -Rmodule-build -Rmodule-import -verify -fmodules-cache-path=%t/cache-symlink
//--- tu4.c
...
// RUN: cd %t/tmp && %clang_cc1 -fmodules -fimplicit-module-maps -fsyntax-only -I %t %t/tu5.c -Rmodule-build -Rmodule-import -verify -fmodules-cache-path=%t/cache
//--- tu5.c
#include "f.h" // expected-remark{{building module 'f'}}           \
               // expected-remark{{finished building module 'f'}}  \
               // expected-remark{{importing module 'f'}}          \
               // expected-remark{{importing module 'e' into 'f'}} \
               // expected-remark{{importing module 'd' into 'e'}} \
               // expected-remark{{importing module 'c' into 'd'}} \
               // expected-remark{{importing module 'b' into 'c'}} \
               // expected-remark{{importing module 'a' into 'b'}}
