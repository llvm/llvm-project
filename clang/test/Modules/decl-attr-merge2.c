...
module First{header "first.h" export * } module Second {
...
void test(void) {
  // Check the attribute from "second.h" doesn't hide the attribute from
  // "first.h".
  additiveAttr();
  // expected-error@-1 {{'additiveAttr' is unavailable: not available on macOS}}
  // expected-note@first.h:* {{'additiveAttr' has been explicitly marked
  // unavailable here}}

  // Check calling a function with `MutualExclusions` attributes.
  exclusiveAttr();
  // expected-error@second.h:* {{'cold' and 'hot' attributes are not
  // compatible}} expected-note@first.h:* {{conflicting attribute is here}}
}
