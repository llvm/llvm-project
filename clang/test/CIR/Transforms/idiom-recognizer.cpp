...
// -check-prefix=CIR CIR: IR Dump After IdiomRecognizer: cir-idiom-recognizer
...
// implicit-check-not proves no raised operation leaked past LoweringPrepare.
...
// --check-prefix=RAISED '--implicit-check-not=cir.call @_ZSt4find' RUN:
// FileCheck %s --check-prefix=FINAL --input-file=%t.cir
...
// RUN: %clang_cc1 -std=c++17 -triple aarch64-unknown-linux-gnu -fno-signed-char
...
// -fno-builtin-memcpy -fclangir -clangir-enable-idiom-recognizer -emit-cir %s
// -o %t.no-builtin-memcpy.cir RUN: FileCheck %s
// --check-prefix=NO-BUILTIN-MEMCPY --input-file=%t.no-builtin-memcpy.cir
...
// -fclangir -clangir-enable-idiom-recognizer -emit-cir -mmlir
// --mlir-print-ir-after=cir-idiom-recognizer %s -o /dev/null 2>&1 | FileCheck
// %s --check-prefix=NO-BUILTINS --implicit-check-not=cir.std.strlen
...
// %[[VALUE]]) FINAL-SAME: nothrow side_effect(pure) FINAL-SAME: {llvm.noundef}
