...
// RUN: FileCheck %s --input-file=%t.ll --check-prefix=LLVM-NO-ARGINFO
// RUN: %clang_cc1 %s -fclangir -cl-std=CL2.0 -triple spirv64-unknown-unknown -emit-llvm -cl-kernel-arg-info -o %t.arginfo.ll
// RUN: FileCheck %s --input-file=%t.arginfo.ll --check-prefix=LLVM
...
// RUN: FileCheck %s --input-file=%t.ogcg.ll --check-prefix=LLVM-NO-ARGINFO
// RUN: %clang_cc1 %s -cl-std=CL2.0 -triple spirv64-unknown-unknown -emit-llvm -cl-kernel-arg-info -o %t.ogcg.arginfo.ll
// RUN: FileCheck %s --input-file=%t.ogcg.arginfo.ll --check-prefix=LLVM
...
// LLVM-ARGINFO-DAG: define{{.*}} void @global_qualifier_kernel_args{{.*}} !kernel_arg_name ![[GLOBAL_ARG_NAMES:[0-9]+]]
...
// LLVM-NO-ARGINFO: define{{.*}} void @global_qualifier_kernel_args
// LLVM-NO-ARGINFO-NOT: !kernel_arg_name
...
// LLVM-ARGINFO-DAG: define{{.*}} void @constant_kernel_args{{.*}} !kernel_arg_name ![[CONSTANT_ARG_NAMES:[0-9]+]]
...
// LLVM-ARGINFO-DAG: define{{.*}} void @local_qualifier_kernel_args{{.*}} !kernel_arg_name ![[LOCAL_ARG_NAMES:[0-9]+]]
...
// LLVM-ARGINFO-DAG: define{{.*}} void @private_qualifier_kernel_args{{.*}} !kernel_arg_name ![[PRIVATE_ARG_NAMES:[0-9]+]]
...
// LLVM-ARGINFO-DAG: define{{.*}} void @typedef_kernel_args{{.*}} !kernel_arg_name ![[TYPEDEF_ARG_NAMES:[0-9]+]]
...
// LLVM-DAG: ![[VECTOR_TYPEDEF_ADDR_SPACES]] = !{i32 1}
// LLVM-DAG: ![[VECTOR_TYPEDEF_ACCESS_QUALS]] = !{!"none"}
// LLVM-DAG: ![[VECTOR_TYPEDEF_ARG_TYPES]] = !{!"char16*"}
// LLVM-DAG: ![[VECTOR_TYPEDEF_BASE_TYPES]] = !{!"char __attribute__((ext_vector_type(16)))*"}
// LLVM-DAG: ![[VECTOR_TYPEDEF_TYPE_QUALS]] = !{!""}
...
// LLVM-ARGINFO-DAG: define{{.*}} void @signed_char_kernel_args{{.*}} !kernel_arg_name ![[SIGNED_CHAR_ARG_NAMES:[0-9]+]]
