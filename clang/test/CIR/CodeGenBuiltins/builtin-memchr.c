...
// RUN: %clang_cc1 -triple x86_64-apple-darwin -fclangir -emit-llvm %s -o %t.x64mac.ll
// RUN: FileCheck --check-prefix=LLVM --input-file=%t.x64mac.ll %s
// RUN: %clang_cc1 -triple x86_64-apple-darwin -emit-llvm %s -o %t.x64mac-classic.ll
// RUN: FileCheck --check-prefix=LLVM --input-file=%t.x64mac-classic.ll %s
// RUN: %clang_cc1 -triple aarch64-apple-darwin -fclangir -emit-llvm %s -o %t.a64mac.ll
// RUN: FileCheck --check-prefix=LLVM --input-file=%t.a64mac.ll %s
// RUN: %clang_cc1 -triple aarch64-apple-darwin -emit-llvm %s -o %t.a64mac-classic.ll
// RUN: FileCheck --check-prefix=LLVM --input-file=%t.a64mac-classic.ll %s
...
// ILP32: cir.libc.memchr({{.*}}, %[[PATTERN32]], %[[LEN32]]) : !s32i, !u32i
...
// I16: cir.libc.memchr({{.*}}, %[[PATTERN16]], %[[LEN16]]) : !s16i, !u16i
...
// ILP32: cir.libc.memchr({{.*}}) : !s32i, !u32i
// I16-LABEL: @test_memchr
// I16: cir.libc.memchr({{.*}}) : !s16i, !u16i
