...
// CIR-HOST: @_Z24__device_stub__global_fni{{.*}}attributes
// {{{.*}}cu.kernel_name = #cir.cu.kernel_name<"_Z9global_fni">{{.*}}} CIR-HOST:
// %[[#CIRKernelArgs:]] = cir.alloca {{.*}}"kernel_args" CIR-HOST:
// %[[#Decayed:]] = cir.cast array_to_ptrdecay %[[#CIRKernelArgs]] CIR-HOST:
// cir.call @__hipPopCallConfiguration CIR-HOST: cir.get_global @_Z9global_fni :
// !cir.ptr<!cir.ptr<!cir.func<(!s32i)>>> CIR-HOST: cir.call @hipLaunchKernel
...
// @_Z9global_fni OGCG-HOST-PTH: %call = call noundef i32
// @hipLaunchKernel_spt(ptr noundef @_Z9global_fni

int main() { global_fn<<<1, 1>>>(1); }
