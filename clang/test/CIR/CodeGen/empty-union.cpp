...
// | FileCheck %s --check-prefix=CIR RUN: %clang_cc1 -triple
// x86_64-unknown-linux-gnu -fclangir -emit-llvm %s -o - | FileCheck %s
// --check-prefixes=LLVM,LLVMCIR RUN: %clang_cc1 -triple
// x86_64-unknown-linux-gnu -emit-llvm %s -o - | FileCheck %s
// --check-prefixes=LLVM,OGCG
...
// {!cir.array<!u8i x 16>}> LLVM-DAG: %union.EmptyAligned = type { [16 x i8] }
...
// {!u8i}> LLVM-DAG: %union.OnlyZeroBitfield = type { i8 }
...
// !s32i}> CIR-DAG: !rec_WrapEmpty = !cir.struct<"WrapEmpty"
// {!rec_OuterWithEmpty, !s32i}> CIR-DAG: cir.global external @w = #cir.zero :
// !rec_WrapEmpty {alignment = 4 : i64} LLVM-DAG: %struct.WrapEmpty = type {
// %union.OuterWithEmpty, i32 } LLVM-DAG: %union.OuterWithEmpty = type { i32 }
...
// {!rec_EmptyAligned, !s32i, !cir.array<!u8i x 12>}> CIR-DAG:
// !rec_LeadingZeroBitfield = !cir.struct<"LeadingZeroBitfield"
...
// i32 } LLVMCIR-DAG: %struct.LeadingOver = type { %union.EmptyAligned, i32, [12
// x i8] } OGCG-DAG:    %struct.Leading = type { [4 x i8], i32 } OGCG-DAG:
// %struct.Trailing = type { i32, [4 x i8] } OGCG-DAG:    %struct.Middle = type
// { i32, [4 x i8], i32 } OGCG-DAG:    %struct.LeadingZeroBitfield = type { [4 x
// i8], i32 } OGCG-DAG:    %struct.LeadingOver = type { [16 x i8], i32, [12 x
// i8] } LLVM-DAG:    @lead = global %struct.Leading zeroinitializer, align 4
// LLVM-DAG:    @leadOver = global %struct.LeadingOver zeroinitializer, align 16

void useEmpty() { Empty e; }
...
void useEmptyAligned() { EmptyAligned e; }
...
void useZeroBitfield() { OnlyZeroBitfield e; }
...
// !cir.ptr<!rec_Leading> -> !cir.ptr<!s32i> LLVM: define dso_local noundef i32
// @_Z10getLeadingv() LLVM:   load i32, ptr getelementptr inbounds nuw (i8, ptr
// @lead, i64 4), align 4
...
// !cir.ptr<!rec_Trailing> -> !cir.ptr<!s32i> LLVM: define dso_local noundef i32
// @_Z11getTrailingv() LLVM:   load i32, ptr @trail, align 4
...
// !cir.ptr<!rec_Middle> -> !cir.ptr<!s32i> LLVM: define dso_local noundef i32
// @_Z9getMiddlev() LLVM:   load i32, ptr getelementptr inbounds nuw (i8, ptr
// @mid, i64 8), align 4
...
// !cir.ptr<!rec_LeadingOver> -> !cir.ptr<!s32i> LLVM: define dso_local noundef
// i32 @_Z14getLeadingOverv() LLVM:   load i32, ptr getelementptr inbounds nuw
// (i8, ptr @leadOver, i64 16), align 16
...
// !cir.ptr<!rec_LeadingZeroBitfield> CIR:    %{{.*}} = cir.get_member %[[Z]][1]
// {name = "x"} : !cir.ptr<!rec_LeadingZeroBitfield> -> !cir.ptr<!s32i> LLVM:
// define dso_local noundef i32 @_Z22getLeadingZeroBitfieldv() LLVM:   load i32,
// ptr getelementptr inbounds nuw (i8, ptr @leadZero, i64 4), align 4
...
// !cir.ptr<!cir.array<!rec_Leading x 2>> CIR:    %[[E:.*]] = cir.get_element
// %[[A]][%{{.*}} : !s64i] : !cir.ptr<!cir.array<!rec_Leading x 2>> ->
// !cir.ptr<!rec_Leading> CIR:    %{{.*}} = cir.get_member %[[E]][1] {name =
// "x"} : !cir.ptr<!rec_Leading> -> !cir.ptr<!s32i> LLVM: define dso_local
// noundef i32 @_Z8getArrayv() LLVM:   load i32, ptr getelementptr inbounds nuw
// (i8, ptr @leadArr, i64 12), align 4
