...
// CIR-DAG: ![[STRUCT_D:.*]] = !cir.struct<"D" {data ![[STRUCT_A]], data ![[STRUCT_A]], empty_bitfield !u8i, data ![[STRUCT_A]]}>
struct D {
  A a;
  A b = A{2, 2.0};
  unsigned : 2;
...
// CIR-DAG: ![[UNION_U:.*]] = !cir.union<"U" {empty_bitfield !u8i, data ![[STRUCT_A]], data !s8i}>
union U {
  unsigned : 1;
  A a;
  char b;
