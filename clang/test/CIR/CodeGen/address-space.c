...
// %clang_cc1 -triple x86_64-unknown-linux-gnu -fclangir -emit-llvm %s -o %t.ll
...
// target_address_space(1)> LLVM: @gvar = global ptr addrspace(1) null OGCG:
// @gvar = global ptr addrspace(1) null
...
void foo(int __attribute__((address_space(1))) * arg) { return; }
...
void bar(int __attribute__((address_space(0))) * arg) { return; }
...
// target_address_space(1)>> LLVM: define dso_local ptr addrspace(1) @get_gvar()
// LLVM:   load ptr addrspace(1), ptr @gvar
// OGCG: define dso_local ptr addrspace(1) @get_gvar()
// OGCG:   load ptr addrspace(1), ptr @gvar
int __attribute__((address_space(1))) * get_gvar() { return gvar; }
...
// CIR: cir.copy {{.*}} to {{.*}} : !cir.ptr<!rec_S>, !cir.ptr<!rec_S,
// target_address_space(1)> LLVM-LABEL: define dso_local void @copy_to_as LLVM:
// call void @llvm.memcpy.p1.p0.i64(ptr addrspace(1) align 4 %{{.*}}, ptr align
// 4 %{{.*}}, i64 16, i1 false) OGCG-LABEL: define dso_local void @copy_to_as
// OGCG: call void @llvm.memcpy.p1.p0.i64(ptr addrspace(1) align 4 %{{.*}}, ptr
// align 4 %{{.*}}, i64 16, i1 false)
