...
// %t.cir RUN: FileCheck --check-prefix=CIR --input-file=%t.cir %s RUN:
...
// FileCheck --check-prefix=LLVM --input-file=%t.ll %s
...
// CIR:         %[[R1:.+]] = cir.call @_Z3Bari(%{{.+}}) musttail : (!s32i{{.*}})
// -> (!s32i{{.*}}) CIR-NEXT:    cir.return %[[R1]] : !s32i CIR: %[[R2:.+]] =
// cir.call @_Z3Bazi(%{{.+}}) musttail : (!s32i{{.*}}) -> (!s32i{{.*}})
...
int Nested(int x) { [[clang::musttail]] return Bar(Bar(x)); }

// CIR-LABEL: cir.func{{.*}} @_Z6Nestedi
// CIR:         %[[INNER:.+]] = cir.call @_Z3Bari(%{{.+}}) : (!s32i{{.*}}) ->
// (!s32i{{.*}}) CIR-NEXT:    %[[OUTER:.+]] = cir.call @_Z3Bari(%[[INNER]])
// musttail : (!s32i{{.*}}) -> (!s32i{{.*}}) CIR-NEXT:    cir.return %[[OUTER]]
// : !s32i

// LLVM-LABEL: define{{.*}} i32 @_Z6Nestedi
// LLVM:         %[[INNER:.+]] = call{{.*}} i32 @_Z3Bari
// LLVM:         %[[OUTER:.+]] = musttail call{{.*}} i32 @_Z3Bari(i32{{.*}}
// %[[INNER]]) LLVM-NEXT:    ret i32 %[[OUTER]]
...
// CIR:         %[[R:.+]] = cir.call @_Z3Bari(%{{.+}}) musttail : (!s32i{{.*}})
// -> (!s32i{{.*}}) CIR-NEXT:    cir.return %[[R]] : !s32i
...
void ReturnsVoid();
void VoidTail() { [[clang::musttail]] return ReturnsVoid(); }
...
// CIR:         cir.call %{{.+}}(%{{.+}}) musttail :
// (!cir.ptr<!cir.func<(!s32i)>>, !s32i{{.*}}) -> () CIR-NEXT:    cir.return
...
int IndirectField(Data *data) { [[clang::musttail]] return data->fptr(data); }

// CIR-LABEL: cir.func{{.*}} @_Z13IndirectFieldP4Data
// CIR:         %[[R:.+]] = cir.call %{{.+}}(%{{.+}}) musttail :
// (!cir.ptr<{{.*}}>, !cir.ptr<!rec_Data>{{.*}}) -> (!s32i{{.*}}) CIR-NEXT:
// cir.return %[[R]] : !s32i
...
int Foo::TailFrom(int x) { [[clang::musttail]] return MemberFunction(x); }

// CIR-LABEL: cir.func{{.*}} @_ZN3Foo8TailFromEi
// CIR:         %[[R:.+]] = cir.call @_ZN3Foo14MemberFunctionEi(%{{.+}},
// %{{.+}}) musttail : (!cir.ptr<!rec_Foo>{{.*}}, !s32i{{.*}}) -> (!s32i{{.*}})
// CIR-NEXT:    cir.return %[[R]] : !s32i

// LLVM-LABEL: define{{.*}} i32 @_ZN3Foo8TailFromEi
// LLVM:         %[[R:.+]] = musttail call{{.*}} i32
// @_ZN3Foo14MemberFunctionEi(ptr{{.*}}, i32{{.*}}) LLVM-NEXT:    ret i32 %[[R]]

int StaticViaClass(int x) { [[clang::musttail]] return Foo::StaticMethod(x); }

// CIR-LABEL: cir.func{{.*}} @_Z14StaticViaClassi
// CIR:         %[[R:.+]] = cir.call @_ZN3Foo12StaticMethodEi(%{{.+}}) musttail
// : (!s32i{{.*}}) -> (!s32i{{.*}}) CIR-NEXT:    cir.return %[[R]] : !s32i

// LLVM-LABEL: define{{.*}} i32 @_Z14StaticViaClassi
// LLVM:         %[[R:.+]] = musttail call{{.*}} i32
// @_ZN3Foo12StaticMethodEi(i32{{.*}}) LLVM-NEXT:    ret i32 %[[R]]
...
template <class T> T TplQualified(T x) { [[clang::musttail]] return ::Bar(x); }
...
// CIR:         %[[R:.+]] = cir.call @_Z10ReturnsInti(%{{.+}}) musttail :
// (!s32i{{.*}}) -> (!s32i{{.*}}) CIR-NEXT:    cir.return %[[R]] : !s32i
...
int (Foo::*pmf)(int);
int Foo::TailFrom2(int x) { [[clang::musttail]] return (this->*pmf)(x); }

// CIR-LABEL: cir.func{{.*}} @_ZN3Foo9TailFrom2Ei
// CIR:         %[[R:.+]] = cir.call %{{.+}}(%{{.+}}, %{{.+}}) musttail :
// (!cir.ptr<{{.*}}>, !cir.ptr<!void>{{.*}}, !s32i{{.*}}) -> (!s32i{{.*}})
// CIR-NEXT:    cir.return %[[R]] : !s32i

// LLVM-LABEL: define{{.*}} i32 @_ZN3Foo9TailFrom2Ei
// LLVM:         %[[R:.+]] = musttail call{{.*}} i32 %{{.+}}(ptr{{.*}},
// i32{{.*}}) LLVM-NEXT:    ret i32 %[[R]]

struct V {
  virtual void t();
  virtual void f();
};
void V::f() { [[clang::musttail]] return t(); }

// CIR-LABEL: cir.func{{.*}} @_ZN1V1fEv
// CIR:         cir.call %{{.+}}(%{{.+}}) musttail : (!cir.ptr<{{.*}}>,
// !cir.ptr<!rec_V>{{.*}}) -> () CIR-NEXT:    cir.return
...
// CIR:         %{{.+}} = cir.call @_ZZ3LamvENK3$_0cvPFivEEv(%{{.+}}){{.*}} :
// (!cir.ptr<{{.*}}>{{.*}}) -> (!cir.ptr<!cir.func<() -> !s32i>>{{.*}}) CIR:
// %[[R:.+]] = cir.call %{{.+}}() musttail : (!cir.ptr<!cir.func<() -> !s32i>>)
// -> (!s32i{{.*}}) CIR-NEXT:    cir.return %[[R]] : !s32i
