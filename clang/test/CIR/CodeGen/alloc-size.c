...
// CIR: cir.func{{.*}}@my_malloc(!s32i){{.*}} attributes {alloc_size = array<i32: 0>}
extern void *my_malloc(int) __attribute__((alloc_size(1)));
// CIR: cir.func{{.*}}@my_calloc(!s32i, !s32i){{.*}} attributes {alloc_size = array<i32: 0, 1>}
...
void call_direct(void) {
  my_malloc(50);
  // CIR: cir.call @my_malloc(%{{.*}}) {alloc_size = array<i32: 0>}
  // LLVM: call ptr @my_malloc(i32{{.*}} 50) [[DIRECT_MALLOC_ATTR:#[0-9]+]]
  my_calloc(1, 16);
  // CIR: cir.call @my_calloc(%{{.*}}) {alloc_size = array<i32: 0, 1>}
...
void call_function_pointer(void) {
  ...
  // CIR: cir.call %[[MALLOC_FN_PTR]]({{.*}}) {alloc_size = array<i32: 1>}
  //
  ...
  // CIR: cir.call %[[CALLOC_FN_PTR]]({{.*}}) {alloc_size = array<i32: 1, 2>}
  //
...
void call_function_pointer_typedef(void) {
  ...
  // CIR: cir.call %[[INDIRECT_TYPEDEF_MALLOC_FN_PTR]]({{.*}}) {alloc_size = array<i32: 2>}
  //
  ...
  // CIR: cir.call %[[INDIRECT_TYPEDEF_CALLOC_FN_PTR]]({{.*}}) {alloc_size = array<i32: 2, 3>}
  //
