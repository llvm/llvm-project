...
// CIR: cir.func {{.*}}@ext_schar(%arg0: !s8i {{.*}}llvm.signext{{.*}}) -> (!s8i
// {{.*}}llvm.signext LLVM: define dso_local signext i8 @ext_schar(i8 noundef
// signext %{{.+}})

// Narrow unsigned integer zero-extended in a register.
unsigned char ext_uchar(unsigned char c) { return c; }

// CIR: cir.func {{.*}}@ext_uchar(%arg0: !u8i {{.*}}llvm.zeroext{{.*}}) -> (!u8i
// {{.*}}llvm.zeroext LLVM: define dso_local zeroext i8 @ext_uchar(i8 noundef
// zeroext %{{.+}})
...
// CIR: cir.func {{.*}}@take_int_sse(%arg0: !s64i{{.*}}, %arg1:
// !cir.double{{.*}}) LLVM: define dso_local void @take_int_sse(i64 %{{.+}},
// double %{{.+}})

// Struct split into two SSE eightbytes.
void take_sse2(SSE2 s) { (void)s; }

// CIR: cir.func {{.*}}@take_sse2(%arg0: !cir.double{{.*}}, %arg1:
// !cir.double{{.*}}) LLVM: define dso_local void @take_sse2(double %{{.+}},
// double %{{.+}})
...
// CIR: cir.func {{.*}}@ret_big(%arg0: !cir.ptr<!rec_Big> {{.*}}llvm.sret =
// !rec_Big{{.*}}) LLVM: define dso_local void @ret_big(ptr dead_on_unwind
// noalias writable sret(%struct.Big) align 8 %{{.+}})
...
// !rec_Big{{.*}}) LLVM-CIR: define dso_local void @take_big(ptr noalias noundef
// byval(%struct.Big) align 8 %{{.+}}) LLVM-OGCG: define dso_local void
// @take_big(ptr noundef byval(%struct.Big) align 8 %{{.+}})
