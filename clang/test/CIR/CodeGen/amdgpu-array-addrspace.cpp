...
// !cir.ptr<!cir.array<!s32i x 10>, target_address_space(1)> CIR-NEXT:
// %[[DECAY:.*]] = cir.cast array_to_ptrdecay %[[ARR]] :
// !cir.ptr<!cir.array<!s32i x 10>, target_address_space(1)> -> !cir.ptr<!s32i,
// target_address_space(1)> CIR-NEXT:    %[[FLAT:.*]] = cir.cast address_space
// %[[DECAY]] : !cir.ptr<!s32i, target_address_space(1)> -> !cir.ptr<!s32i>
...
// addrspace(1) @globalArr to ptr))
void pass_global_array() { takes_ptr(globalArr); }
...
// CIR:         %[[ARR:.*]] = cir.get_global @globalArr :
// !cir.ptr<!cir.array<!s32i x 10>, target_address_space(1)> CIR-NEXT:
// %[[ELEM:.*]] = cir.get_element %[[ARR]][%{{.*}} : !s64i] :
// !cir.ptr<!cir.array<!s32i x 10>, target_address_space(1)> -> !cir.ptr<!s32i,
// target_address_space(1)> CIR-NEXT:    %{{.*}} = cir.load align(4) %[[ELEM]] :
...
// @globalArr, i32 0, i64 %{{.*}} LLVM-NEXT:    %{{.*}} = load i32, ptr
...
// addrspace(1) @globalArr to ptr) OGCG:         load i32, ptr %{{.*}}, align 4
int index_global_array(int i) { return globalArr[i]; }
