// RUN: %clang_cc1 -std=c11 -triple x86_64-unknown-linux-gnu -fclangir -emit-cir
// %s -o %t.cir RUN: FileCheck --check-prefix=CIR --input-file=%t.cir %s RUN:
// %clang_cc1 -std=c11 -triple x86_64-unknown-linux-gnu -fclangir -emit-llvm %s
// -o %t-cir.ll RUN: FileCheck --check-prefix=LLVM --input-file=%t-cir.ll %s
// RUN: %clang_cc1 -std=c11 -triple x86_64-unknown-linux-gnu -emit-llvm %s -o
// %t.ll RUN: FileCheck --check-prefix=OGCG --input-file=%t.ll %s
...
// CIR:  %[[GET_USED:.*]] = cir.get_global @noproto_used : !cir.ptr<!cir.func<()
// -> !s32i>> CIR:  cir.call %[[GET_USED]]() : (!cir.ptr<!cir.func<() ->
// !s32i>>) -> !s32i
...
// alias(@noproto_used_target) LLVM sorts these at the top of the file.
...
// -> !s32i>> CIR: %[[TO_VARIADIC:.*]] = cir.cast bitcast %[[GET_NPA]] :
// !cir.ptr<!cir.func<() -> !s32i>> -> !cir.ptr<!cir.func<(...) -> !s32i>> CIR:
// %[[TO_TYPED:.*]] = cir.cast bitcast %[[TO_VARIADIC]] :
// !cir.ptr<!cir.func<(...) -> !s32i>> -> !cir.ptr<!cir.func<(!s32i, !s32i,
// !s32i) -> !s32i>> CIR: cir.call %[[TO_TYPED]](%{{.*}}, %{{.*}}, %{{.*}}) :
...
// OGCG: call i32 (i32, i32, i32, ...) @noproto_args(i32 noundef 1, i32 noundef
// 2, i32 noundef 3)
...
// alias(@noproto_args_target) LLVM sorts these at the top of the file.
...
// !cir.ptr<!cir.func<(!s32i, !s32i, !s32i) -> !s32i>> CIR: cir.call %4(%{{.*}},
// %{{.*}}, %{{.*}}) : (!cir.ptr<!cir.func<(!s32i, !s32i, !s32i) -> !s32i>>,
// !s32i {llvm.noundef}, !s32i {llvm.noundef}, !s32i {llvm.noundef}) -> !s32i
//
...
// OGCG: call i32 (i32, i32, i32, ...) @noproto_args2(i32 noundef 1, i32 noundef
// 2, i32 noundef 3)
...
// !s32i alias(@noproto_args_target2) LLVM sorts these at the top of the file.
