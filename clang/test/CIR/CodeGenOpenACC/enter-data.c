...
#pragma acc enter data copyin(parmVar)
  // CHECK-NEXT: %[[COPYIN1:.*]] = acc.copyin varPtr(%[[PARM]] : !cir.ptr<!s32i>) structured(false) name("parmVar") -> !cir.ptr<!s32i>
  // CHECK-NEXT: acc.enter_data dataOperands(%[[COPYIN1]] : !cir.ptr<!s32i>)

#pragma acc enter data copyin(readonly, alwaysin: parmVar)
  // CHECK-NEXT: %[[COPYIN1:.*]] = acc.copyin varPtr(%[[PARM]] : !cir.ptr<!s32i>) structured(false) name("parmVar") <{modifiers = #acc<data_clause_modifier readonly,alwaysin>}> -> !cir.ptr<!s32i>
  // CHECK-NEXT: acc.enter_data dataOperands(%[[COPYIN1]] : !cir.ptr<!s32i>)

#pragma acc enter data copyin(readonly, alwaysin: parmVar) async
  // CHECK-NEXT: %[[COPYIN1:.*]] = acc.copyin varPtr(%[[PARM]] : !cir.ptr<!s32i>) async structured(false) name("parmVar") <{modifiers = #acc<data_clause_modifier readonly,alwaysin>}> -> !cir.ptr<!s32i>
...
#pragma acc enter data copyin(readonly, alwaysin: parmVar) async(parmVar)
  ...
  // CHECK-NEXT: %[[COPYIN1:.*]] = acc.copyin varPtr(%[[PARM]] : !cir.ptr<!s32i>) async(%[[PARM_CAST]] : si32) structured(false) name("parmVar") <{modifiers = #acc<data_clause_modifier readonly,alwaysin>}> -> !cir.ptr<!s32i>
...
#pragma acc enter data create(parmVar)
  // CHECK-NEXT: %[[CREATE1:.*]] = acc.create varPtr(%[[PARM]] : !cir.ptr<!s32i>) structured(false) name("parmVar") -> !cir.ptr<!s32i>
  // CHECK-NEXT: acc.enter_data dataOperands(%[[CREATE1]] : !cir.ptr<!s32i>)

#pragma acc enter data create(zero: parmVar)
  // CHECK-NEXT: %[[CREATE1:.*]] = acc.create varPtr(%[[PARM]] : !cir.ptr<!s32i>) structured(false) name("parmVar") <{modifiers = #acc<data_clause_modifier zero>}> -> !cir.ptr<!s32i>
  // CHECK-NEXT: acc.enter_data dataOperands(%[[CREATE1]] : !cir.ptr<!s32i>)

#pragma acc enter data create(zero: parmVar) async
  // CHECK-NEXT: %[[CREATE1:.*]] = acc.create varPtr(%[[PARM]] : !cir.ptr<!s32i>) async structured(false) name("parmVar") <{modifiers = #acc<data_clause_modifier zero>}> -> !cir.ptr<!s32i>
  // CHECK-NEXT: acc.enter_data async dataOperands(%[[CREATE1]] : !cir.ptr<!s32i>)

#pragma acc enter data create(zero: parmVar) async(parmVar)
  // CHECK-NEXT: %[[PARM_LOAD:.*]] = cir.load{{.*}} %[[PARM]]
  // CHECK-NEXT: %[[PARM_CAST:.*]] = cir.builtin_int_cast %[[PARM_LOAD]]
  // CHECK-NEXT: %[[CREATE1:.*]] = acc.create varPtr(%[[PARM]] : !cir.ptr<!s32i>) async(%[[PARM_CAST]] : si32) structured(false) name("parmVar") <{modifiers = #acc<data_clause_modifier zero>}> -> !cir.ptr<!s32i>
  // CHECK-NEXT: acc.enter_data async(%[[PARM_CAST]] : si32) dataOperands(%[[CREATE1]] : !cir.ptr<!s32i>)

#pragma acc enter data attach(ptrParmVar)
  // CHECK-NEXT: %[[ATTACH1:.*]] = acc.attach varPtr(%[[PTRPARM]] : !cir.ptr<!cir.ptr<!s32i>>) structured(false) name("ptrParmVar") -> !cir.ptr<!cir.ptr<!s32i>>
  // CHECK-NEXT: acc.enter_data dataOperands(%[[ATTACH1]] : !cir.ptr<!cir.ptr<!s32i>>)

#pragma acc enter data attach(ptrParmVar) async
  // CHECK-NEXT: %[[ATTACH1:.*]] = acc.attach varPtr(%[[PTRPARM]] : !cir.ptr<!cir.ptr<!s32i>>) async structured(false) name("ptrParmVar") -> !cir.ptr<!cir.ptr<!s32i>>
...
#pragma acc enter data attach(ptrParmVar) async(parmVar)
  // CHECK-NEXT: %[[PARM_LOAD:.*]] = cir.load{{.*}} %[[PARM]]
  // CHECK-NEXT: %[[PARM_CAST:.*]] = cir.builtin_int_cast %[[PARM_LOAD]]
  // CHECK-NEXT: %[[ATTACH1:.*]] = acc.attach varPtr(%[[PTRPARM]] : !cir.ptr<!cir.ptr<!s32i>>) async(%[[PARM_CAST]] : si32) structured(false) name("ptrParmVar") -> !cir.ptr<!cir.ptr<!s32i>>
...
#pragma acc enter data async if (parmVar == 1) copyin(parmVar)
  ...
  // CHECK-NEXT: %[[CMP:.*]] = cir.cmp eq %[[PARM_LOAD]], %[[ONE_CONST]]
  // CHECK-NEXT: %[[CMP_CAST:.*]] = builtin.unrealized_conversion_cast %[[CMP]]
  // CHECK-NEXT: %[[COPYIN1:.*]] = acc.copyin varPtr(%[[PARM]] : !cir.ptr<!s32i>) async structured(false) name("parmVar") -> !cir.ptr<!s32i>
...
#pragma acc enter data if (parmVar == 1) async(parmVar) copyin(parmVar)
  ...
  // CHECK-NEXT: %[[PARM_LOAD:.*]] = cir.load{{.*}} %[[PARM]]
  // CHECK-NEXT: %[[PARM_CAST:.*]] = cir.builtin_int_cast %[[PARM_LOAD]]
  // CHECK-NEXT: %[[COPYIN1:.*]] = acc.copyin varPtr(%[[PARM]] : !cir.ptr<!s32i>) async(%[[PARM_CAST]] : si32) structured(false) name("parmVar") -> !cir.ptr<!s32i>
