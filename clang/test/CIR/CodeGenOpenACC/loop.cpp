...
#pragma acc loop device_type(nvidia, radeon) seq
  ...
  // CHECK: acc.loop {
  // CHECK: acc.yield
  // CHECK-NEXT: } seq ([#acc.device_type<nvidia>, #acc.device_type<radeon>]) independent loc
#pragma acc loop device_type(radeon) seq
  for(unsigned I = 0; I < N; ++I);
  // CHECK: acc.loop {
  // CHECK: acc.yield
  // CHECK-NEXT: } seq ([#acc.device_type<radeon>]) independent loc
...
#pragma acc loop device_type(nvidia, radeon) independent
  ...
  // CHECK: acc.loop {
  // CHECK: acc.yield
  // CHECK-NEXT: } independent ([#acc.device_type<nvidia>, #acc.device_type<radeon>, #acc.device_type<none>]) loc
#pragma acc loop device_type(radeon) independent
  for(unsigned I = 0; I < N; ++I);
  // CHECK: acc.loop {
  // CHECK: acc.yield
  // CHECK-NEXT: } independent ([#acc.device_type<radeon>, #acc.device_type<none>]) loc
...
#pragma acc loop device_type(nvidia, radeon) auto
  ...
  // CHECK: acc.loop {
  // CHECK: acc.yield
  // CHECK-NEXT: } independent auto_ ([#acc.device_type<nvidia>, #acc.device_type<radeon>]) loc
#pragma acc loop device_type(radeon) auto
  for(unsigned I = 0; I < N; ++I);
  // CHECK: acc.loop {
  // CHECK: acc.yield
  // CHECK-NEXT: } independent auto_ ([#acc.device_type<radeon>]) loc
...
#pragma acc loop auto device_type(radeon)
  ...
  // CHECK: acc.loop {
  // CHECK: acc.yield
  // CHECK-NEXT: } collapse([1, 2]) collapseDeviceType([#acc.device_type<none>, #acc.device_type<radeon>]) independent
  ...
  // CHECK: acc.loop {
  // CHECK: acc.yield
  // CHECK-NEXT: } collapse([1, 2, 2]) collapseDeviceType([#acc.device_type<none>, #acc.device_type<radeon>, #acc.device_type<nvidia>]) independent
