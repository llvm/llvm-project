...
#pragma acc shutdown device_type(*)
  // CHECK-NEXT: acc.shutdown device_types([#acc.device_type<star>])
#pragma acc shutdown device_type(nvidia)
  // CHECK-NEXT: acc.shutdown device_types([#acc.device_type<nvidia>])
#pragma acc shutdown device_type(NVIDIA)
  // CHECK-NEXT: acc.shutdown device_types([#acc.device_type<nvidia>])
#pragma acc shutdown device_type(HoSt) device_type(MuLtIcORe)
  // CHECK-NEXT: acc.shutdown device_types([#acc.device_type<host>, #acc.device_type<multicore>])
...
#pragma acc shutdown if(cond) device_num(cond) device_type(*)
  ...
  // CHECK-NEXT: %[[COND_LOAD:.*]] = cir.load{{.*}} %[[COND]] : !cir.ptr<!s32i>, !s32i
  // CHECK-NEXT: %[[COND_CONV:.*]] = cir.builtin_int_cast %[[COND_LOAD]] : !s32i -> si32
  // CHECK-NEXT: acc.shutdown device_num(%[[COND_CONV]] : si32) if(%[[BOOL_CONV]]) device_types([#acc.device_type<star>])
}
