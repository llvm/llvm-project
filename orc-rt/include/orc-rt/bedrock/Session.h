...
public:
  using ErrorReporterFn = move_only_function<void(Session &, Error) noexcept>;
  ...
  ///
  /// The ReportError function is passed the reporting Session along with
  /// the error, so a single reporter can serve many Sessions, and can
  /// identify the one reporting (e.g. by address) without needing to be
  ...
  void reportError(Error Err) noexcept { ReportError(*this, std::move(Err)); }
  ...
  /// Error reporter that logs errors via ORC_RT_LOG at Error level in the
  /// Session category, tagged with the reporting Session's address.
  ///
  /// Only available if Error-level logging is compiled in: code that must
  /// build under every logging configuration should guard uses with
  /// `#if ORC_RT_LOG_ENABLED(Error)`, and pick another reporter otherwise.
  /// Errors may still be suppressed at runtime by logging configuration.
  ///
  /// NOTE: This is an error reporter, not a general-purpose logging API: pass
  /// it as the Session's error reporter, and report errors via reportError.
#if ORC_RT_LOG_ENABLED(Error)
  static void logErrors(Session &S, Error Err) noexcept;
#endif
