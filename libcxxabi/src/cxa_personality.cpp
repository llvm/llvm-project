...
_GCC_specific_handler(PEXCEPTION_RECORD, void*, PCONTEXT, PDISPATCHER_CONTEXT, _Unwind_Personality_Fn);
#endif
...
/// @returns decoded value
static uintptr_t readEncodedPointer(const uint8_t** data, uint8_t encoding, uintptr_t base = 0) {
  uintptr_t result = 0;
...
static void call_terminate(bool native_exception, _Unwind_Exception* unwind_exception) {
...
static const __shim_type_info* get_shim_type_info(
  ...
  return (const __shim_type_info*)readEncodedPointer(&classInfo, ttypeEncoding, base);
}
...
static bool exception_spec_can_catch(
  ...
  const void** temp = reinterpret_cast<const void**>(
      reinterpret_cast<uintptr_t>(classInfo) + static_cast<uintptr_t>(specIndex) * sizeof(uintptr_t));
  ...
  while (true) {
    ...
    const __shim_type_info* catchType = static_cast<const __shim_type_info*>(read_target2_value(ttypePtr));
...
static bool exception_spec_can_catch(
  ...
  while (true) {
    ...
    const __shim_type_info* catchType =
        get_shim_type_info(ttypeIndex, classInfo, ttypeEncoding, true, unwind_exception, base);
    void* tempPtr = adjustedPtr;
    if (catchType->can_catch(excpType, tempPtr))
      return false;
  }
...
#endif
  _Unwind_SetGR(context, __builtin_eh_return_data_regno(0), reinterpret_cast<uintptr_t>(unwind_exception));
  _Unwind_SetGR(context, __builtin_eh_return_data_regno(1), static_cast<uintptr_t>(results.ttypeIndex));
#if __has_feature(ptrauth_calls)
...
static void
scan_eh_tab(scan_results& results,
      ...
      while (true) {
        ...
        if (ttypeIndex > 0) {
          ...
          const __shim_type_info* catchType = get_shim_type_info(
              static_cast<uint64_t>(ttypeIndex), classInfo, ttypeEncoding, native_exception, unwind_exception, base);
          if (catchType == 0) {
          ...
          //    catch a foreign exception
            ...
            const __shim_type_info* excpType  = static_cast<const __shim_type_info*>(exception_header->exceptionType);
          ...
          if (actions & _UA_FORCE_UNWIND) {
            ...
            if (exception_spec_can_catch(
                    ttypeIndex, classInfo, ttypeEncoding, excpType, adjustedPtr, unwind_exception, base)) {
...
#  if __has_feature(ptrauth_calls)
  ...
  uintptr_t reauthenticatedPointer    = (uintptr_t)ptrauth_auth_and_resign(
      *reinterpret_cast<void* const*>(&results.landingPad),
      ...
      targetDiscriminator);
  memmove(
      reinterpret_cast<void*>(&dest), reinterpret_cast<void*>(&reauthenticatedPointer), sizeof(reauthenticatedPointer));
#  else
...
#  if defined(__SEH__) && !defined(__USING_SJLJ_EXCEPTIONS__)
extern "C" _LIBCXXABI_FUNC_VIS EXCEPTION_DISPOSITION __gxx_personality_seh0(
    PEXCEPTION_RECORD ms_exc, void* this_frame, PCONTEXT ms_orig_context, PDISPATCHER_CONTEXT ms_disp) {
  return _GCC_specific_handler(ms_exc, this_frame, ms_orig_context, ms_disp, __gxx_personality_imp);
}
#  endif
...
static _Unwind_Reason_Code continue_unwind(_Unwind_Exception* unwind_exception, _Unwind_Context* context) {
...
static void save_results_to_barrier_cache(_Unwind_Exception* unwind_exception, const scan_results& results) {
...
static void load_results_from_barrier_cache(scan_results& results, const _Unwind_Exception* unwind_exception) {
...
__gxx_personality_v0(_Unwind_State state, _Unwind_Exception* unwind_exception, _Unwind_Context* context) {
...
[[noreturn]] _LIBCXXABI_FUNC_VIS void __cxa_call_unexpected(void* arg) {
  ...
  try {
    ...
    if (native_old_exception) {
      ...
      //   uint8_t ttypeEncoding
      uint8_t lpStartEncoding = *lsda++;
      const uint8_t* lpStart  = (const uint8_t*)readEncodedPointer(&lsda, lpStartEncoding, base);
      ...
      const __shim_type_info* excpType = static_cast<const __shim_type_info*>(&typeid(std::bad_exception));
...
_LIBCXXABI_FUNC_VIS _Unwind_Reason_Code __xlcxx_personality_v1(
    ...
    uint64_t exceptionClass,
    _Unwind_Exception* unwind_exception,
    _Unwind_Context* context) __attribute__((__alias__("__gxx_personality_v0")));
#endif
