...
ScriptedHookPythonInterface::GetSupportedMethods() {
  ...
  methods.do_resolve_addr =
      implementor.HasAttribute("do_resolve_addr");
  return methods;
}
...
ScriptedHookPythonInterface::HandleResolveAddress(lldb::addr_t load_addr,
  ...
  Status error;
  Address addr =
      Dispatch<Address>("do_resolve_addr", error, load_addr, output_sp);
  if (error.Fail()) {
    LLDB_LOG(GetLog(LLDBLog::Script), "do_resolve_addr failed: {0}",
             error.AsCString());
  }
