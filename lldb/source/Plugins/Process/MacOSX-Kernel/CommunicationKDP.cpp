...
CommunicationKDP::CommunicationKDP(const char *comm_name)
    : Communication(comm_name), m_addr_byte_size(4),
      m_byte_order(eByteOrderLittle), m_packet_timeout(5),
      m_sequence_mutex(Mutex::eMutexTypeRecursive), m_is_running(false),
      m_session_key(0u), m_request_sequence_id(0u), m_exception_sequence_id(0u),
      m_kdp_version_version(0u), m_kdp_version_feature(0u),
      m_kdp_hostinfo_cpu_mask(0u), m_kdp_hostinfo_cpu_type(0u),
...
bool CommunicationKDP::GetSequenceMutex(Mutex::Locker &locker) {
  return locker.TryLock(m_sequence_mutex);
}

bool CommunicationKDP::WaitForNotRunningPrivate(const TimeValue *timeout_ptr) {
  ...
  while (IsConnected() && !timed_out) {
    ...
    if (log)
      log->Printf("%s: Read (buffer, (sizeof(buffer), timeout_usec = 0x%x, "
                  ...
                  __PRETTY_FUNCTION__, timeout_usec,
