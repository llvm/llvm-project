...
ProcessGDBRemote::ProcessGDBRemote(lldb::TargetSP target_sp,
    ...
    : Process(target_sp, listener_sp), m_flags(0), m_gdb_comm(),
      ...
      m_last_stop_packet_mutex(Mutex::eMutexTypeRecursive), m_register_info(),
      m_async_broadcaster(NULL, "lldb.process.gdb-remote.async-broadcaster"),
      m_async_listener_sp(
          Listener::MakeListener("lldb.process.gdb-remote.async-listener")),
      m_async_thread_state_mutex(Mutex::eMutexTypeRecursive), m_thread_ids(),
      m_thread_pcs(), m_jstopinfo_sp(), m_jthreadsinfo_sp(),
      ...
      m_thread_create_bp_sp(), m_waiting_for_attach(false),
      m_destroy_tried_resuming(false), m_command_sp(),
      ...
      if (response_type == StringExtractorGDBRemote::eResponse) {
        ...
        while (response.GetNameColonValue(name, value)) {
          if (name.compare("name") == 0) {
            ...
            reg_info.byte_size =
                StringConvert::ToUInt32(value.c_str(), 0, 0) / CHAR_BIT;
            ...
            reg_info.kinds[eRegisterKindEHFrame] =
                StringConvert::ToUInt32(value.c_str(), LLDB_INVALID_REGNUM, 0);
          } else if (name.compare("dwarf") == 0) {
            reg_info.kinds[eRegisterKindDWARF] =
                StringConvert::ToUInt32(value.c_str(), LLDB_INVALID_REGNUM, 0);
          } else if (name.compare("generic") == 0) {
            reg_info.kinds[eRegisterKindGeneric] =
                Args::StringToGenericRegister(value.c_str());
  ...
  case 'S': {
    ...
    while (stop_packet.GetNameColonValue(key, value)) {
      if (key.compare("metype") == 0) {
        ...
        // thread in big endian hex
        tid =
            StringConvert::ToUInt64(value.c_str(), LLDB_INVALID_THREAD_ID, 16);
        ...
        while ((comma_pos = value.find(',')) != std::string::npos) {
          value[comma_pos] = '\0';
          // thread in big endian hex
          tid = StringConvert::ToUInt64(value.c_str(), LLDB_INVALID_THREAD_ID,
                                        16);
          if (tid != LLDB_INVALID_THREAD_ID)
            m_thread_ids.push_back(tid);
          value.erase(0, comma_pos + 1);
        ...
        if (!pair.first.empty() && !pair.second.empty()) {
          ...
          const lldb::addr_t mem_cache_addr = StringConvert::ToUInt64(
              addr_str.c_str(), LLDB_INVALID_ADDRESS, 0);
          if (mem_cache_addr != LLDB_INVALID_ADDRESS) {
            StringExtractor bytes;
        ...
        lldb::addr_t wp_addr =
            StringConvert::ToUInt64(value.c_str(), LLDB_INVALID_ADDRESS, 16);
        WatchpointSP wp_sp =
            GetTarget().GetWatchpointList().FindByAddress(wp_addr);
...
Error ProcessGDBRemote::LaunchAndConnectToDebugserver(
  ...
  if (m_debugserver_pid == LLDB_INVALID_PROCESS_ID) {
    ...
    error = m_gdb_comm.StartDebugserverProcess(
        url, GetTarget().GetPlatform().get(), debugserver_launch_info, &port);
...
bool ProcessGDBRemote::MonitorDebugserverProcess(
    void *callback_baton, lldb::pid_t debugserver_pid,
  ...
  if (log)
    log->Printf(
        "ProcessGDBRemote::MonitorDebugserverProcess (baton=%p, pid=%" PRIu64
        ", signo=%i (0x%x), exit_status=%i)",
        callback_baton, debugserver_pid, signo, signo, exit_status);
  ...
  if (target_sp) {
    ...
    // looking for.
    if (process_sp && process == process_sp.get() &&
        process->m_debugserver_pid == debugserver_pid) {
      ...
      if (process->m_debugserver_pid != LLDB_INVALID_PROCESS_ID &&
        ...
        if (signo) {
          const char *signal_cstr =
              process->GetUnixSignals()->GetSignalAsCString(signo);
          if (signal_cstr)
            ::snprintf(error_str, sizeof(error_str),
                       DEBUGSERVER_BASENAME " died with signal %s",
                       signal_cstr);
          else
            ::snprintf(error_str, sizeof(error_str),
          ...
          ::snprintf(error_str, sizeof(error_str), DEBUGSERVER_BASENAME
                     " died with an exit status of 0x%8.8x",
                     exit_status);
        }
          ...
          if (continue_packet) {
            ...
            if (process->GetTarget().GetNonStopModeEnabled()) {
              ...
              if (!process->GetGDBRemote().SendvContPacket(
                      process, continue_cstr, continue_cstr_len, response)) {
    ...
    if (process) {
      for (size_t i = 0; i < argc; ++i) {
        ...
        if (strstr(packet_cstr, "qGetProfileData") != NULL) {
          response_str =
              process->GetGDBRemote().HarmonizeThreadIdsForProfileData(
                  process, response);
        }
