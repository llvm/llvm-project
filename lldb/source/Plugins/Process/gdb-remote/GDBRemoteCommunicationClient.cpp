...
GDBRemoteCommunicationClient::GDBRemoteCommunicationClient()
    : GDBRemoteCommunication("gdb-remote.client",
                             "gdb-remote.client.rx_packet"),
      ...
      m_async_packet_predicate(false), m_async_packet(),
      m_async_result(PacketResult::Success), m_async_response(),
      m_async_signal(-1), m_interrupt_sent(false),
      m_thread_id_to_used_usec_map(), m_host_arch(), m_process_arch(),
...
GDBRemoteCommunicationClient::SendPacketsAndConcatenateResponses(
  ...
  Mutex::Locker locker;
  if (!GetSequenceMutex(locker, "ProcessGDBRemote::"
                                "SendPacketsAndConcatenateResponses() failed "
                                "due to not getting the sequence mutex")) {
  ...
  for (unsigned int offset = 0; true; offset += response_size) {
    ...
    snprintf(sizeDescriptor, sizeof(sizeDescriptor), "%x,%x", offset,
             response_size);
    PacketResult result = SendPacketAndWaitForResponse(
        (payload_prefix_str + sizeDescriptor).c_str(), this_response,
        /*send_async=*/false);
...
GDBRemoteCommunicationClient::SendPacketAndWaitForResponse(
    const char *payload, StringExtractorGDBRemote &response, bool send_async) {
  return SendPacketAndWaitForResponse(payload, ::strlen(payload), response,
                                      send_async);
}
...
GDBRemoteCommunicationClient::SendPacketAndWaitForResponseNoLock(
  ...
  if (packet_result == PacketResult::Success) {
    ...
    for (size_t i = 0; i < max_response_retries; ++i) {
      ...
      if (log)
        log->Printf("error: packet with payload \"%*s\" got invalid response "
                    "\"%s\": %s",
...
GDBRemoteCommunicationClient::SendPacketAndWaitForResponse(
  ...
  // In order to stop async notifications from being processed in the middle of
  // the
  // send/receive sequence Hijack the broadcast. Then rebroadcast any events
  // when we are done.
  static ListenerSP hijack_listener_sp(
      Listener::MakeListener("lldb.NotifyHijacker"));
  HijackBroadcaster(hijack_listener_sp, eBroadcastBitGdbReadThreadGotNotify);

  if (GetSequenceMutex(locker)) {
    packet_result =
        SendPacketAndWaitForResponseNoLock(payload, payload_length, response);
  } else {
    if (send_async) {
      if (IsRunning()) {
        ...
        if (SendInterrupt(locker, 2, timed_out)) {
          if (m_interrupt_sent) {
            ...
            // Make sure we wait until the continue packet has been sent
            // again...
            if (m_private_is_running.WaitForValueEqualTo(true, &timeout_time,
                                                         &timed_out)) {
              if (log) {
                if (timed_out)
                  log->Printf("async: timed out waiting for process to resume, "
                              "but process was resumed");
                else
            ...
            packet_result = SendPacketAndWaitForResponseNoLock(
  ...
  // If a notification event occurred, rebroadcast since it can now be processed
  // safely.
  EventSP event_sp;
...
std::string GDBRemoteCommunicationClient::HarmonizeThreadIdsForProfileData(
    ProcessGDBRemote *process, StringExtractorGDBRemote &profileDataExtractor) {
    ...
    if (name.compare("thread_used_id") == 0) {
      ...
      if (has_used_usec) {
        ...
        bool good_subsequent_time =
            (prev_used_usec > 0) &&
            ((real_used_usec > 0) ||
             (process->HasAssignedIndexIDToThread(thread_id)));
...
bool GDBRemoteCommunicationClient::SendvContPacket(
    ProcessGDBRemote *process, const char *payload, size_t packet_length,
    StringExtractorGDBRemote &response) {
  ...
  if (log)
    log->Printf("GDBRemoteCommunicationClient::%s () sending vCont packet: %s",
...
StateType GDBRemoteCommunicationClient::SendContinuePacketAndWaitForResponse(
    ProcessGDBRemote *process, const char *payload, size_t packet_length,
    StringExtractorGDBRemote &response) {
  m_curr_tid = LLDB_INVALID_THREAD_ID;
  Log *log(ProcessGDBRemoteLog::GetLogIfAllCategoriesSet(GDBR_LOG_PROCESS));
  ...
  while (state == eStateRunning) {
    if (!got_async_packet) {
      if (log)
        log->Printf(
            "GDBRemoteCommunicationClient::%s () sending continue packet: %s",
            __FUNCTION__, continue_packet.c_str());
      if (SendPacketNoLock(continue_packet.c_str(), continue_packet.size()) !=
          PacketResult::Success)
        state = eStateInvalid;
    ...
    if (ReadPacket(response, UINT32_MAX, false) == PacketResult::Success) {
      if (response.Empty())
        ...
        if (log)
          log->Printf("GDBRemoteCommunicationClient::%s () got packet: %s",
        ...
        case 'S': {
          ...
          bool continue_after_async =
              m_async_signal != -1 || m_async_packet_predicate.GetValue();
          if (continue_after_async || m_interrupt_sent) {
            ...
            const bool received_nonstop_signal =
                signo != sigint_signo && signo != sigstop_signo;
            if (m_interrupt_sent || received_nonstop_signal) {
              ...
              if (ReadPacket(extra_stop_reply_packet, timeout_usec, false) ==
                  PacketResult::Success) {
          ...
          if (m_async_signal != -1) {
            if (log)
              log->Printf("async: send signo = %s",
                          Host::GetSignalAsCString(m_async_signal));
            ...
            if (signo == async_signal) {
              if (log)
                log->Printf(
                    "async: stopped with signal %s, we are done running",
              ...
              signal_packet_len = ::snprintf(
                  signal_packet, sizeof(signal_packet), "C%2.2x", async_signal);
            ...
            Log *packet_log(ProcessGDBRemoteLog::GetLogIfAllCategoriesSet(
                GDBR_LOG_PACKETS));
            ...
            if (m_async_packet.empty()) {
              ...
              m_async_result = SendPacketAndWaitForResponse(
                  &m_async_packet[0], m_async_packet.size(), m_async_response,
                  false);
            }
            ...
            if (packet_log)
              packet_log->Printf(
                  "async: sent packet, continue_after_async = %i",
        ...
        case 'A':
          // Async miscellaneous reply. Right now, only profile data is coming
          // through this channel.
          {
            got_async_packet = true;
            std::string input =
                response.GetStringRef().substr(1); // '1' to move beyond 'A'
            ...
            while ((found = input.find(end_delimiter, pos)) !=
                   std::string::npos) {
              StringExtractorGDBRemote profileDataExtractor(
                  input.substr(pos, found).c_str());
              std::string profile_data = HarmonizeThreadIdsForProfileData(
                  process, profileDataExtractor);
        ...
        default:
          if (log)
            log->Printf(
                "GDBRemoteCommunicationClient::%s () unrecognized async packet",
                __FUNCTION__);
          state = eStateInvalid;
          break;
        }
      ...
      if (log)
        log->Printf(
            "GDBRemoteCommunicationClient::%s () ReadPacket(...) => false",
            __FUNCTION__);
      state = eStateInvalid;
    }
  }
  if (log)
    log->Printf("GDBRemoteCommunicationClient::%s () => %s", __FUNCTION__,
...
bool GDBRemoteCommunicationClient::SendAsyncSignal(int signo) {
  Mutex::Locker async_locker(m_async_mutex);
...
// This function takes a mutex locker as a parameter in case the
// GetSequenceMutex
// actually succeeds. If it doesn't succeed in acquiring the sequence mutex
// (the expected result), then it will send the halt packet. If it does succeed
// then the caller that requested the interrupt will want to keep the sequence
// locked down so that no one else can send packets while the caller has
// control.
...
bool GDBRemoteCommunicationClient::SendInterrupt(
    Mutex::Locker &locker, uint32_t seconds_to_wait_for_stop, bool &timed_out) {
  timed_out = false;
  Log *log(ProcessGDBRemoteLog::GetLogIfAnyCategoryIsSet(GDBR_LOG_PROCESS |
                                                         GDBR_LOG_PACKETS));
  ...
  if (IsRunning()) {
    ...
    if (GetSequenceMutex(locker)) {
      if (log)
        log->Printf("SendInterrupt () - got sequence mutex without having to "
                    "interrupt");
    } else {
      ...
      if (bytes_written > 0) {
        ...
        if (seconds_to_wait_for_stop) {
          ...
          if (m_private_is_running.WaitForValueEqualTo(false, &timeout,
                                                       &timed_out)) {
            if (log)
              log->PutCString(
            ...
            if (log)
              log->Printf("SendInterrupt () - sent interrupt, timed out wating "
                          "for async thread resume");
          }
        } else {
          if (log)
            log->Printf(
                "SendInterrupt () - sent interrupt, not waiting for stop...");
          return true;
        }
      } else {
        if (log)
        ...
        while (response.GetNameColonValue(name, value)) {
          if (name.compare("name") == 0) {
            ...
            const uint32_t version =
                StringConvert::ToUInt32(value.c_str(), UINT32_MAX, 0);
      ...
      if (response.IsNormalResponse()) {
        ...
        while (response.GetNameColonValue(name, value)) {
          if (name.compare("cputype") == 0) {
            ...
            cpu =
                StringConvert::ToUInt32(value.c_str(), LLDB_INVALID_CPUTYPE, 0);
            ...
            if (pointer_byte_size != 0)
              ++num_keys_decoded;
                     ...
                     (name.compare("version") == 0)) // Older debugserver
                                                     // binaries used the
                                                     // "version" key instead of
                                                     // "os_version"...
          {
            Args::StringToVersion(value.c_str(), m_os_version_major,
            ...
            m_default_packet_timeout =
                StringConvert::ToUInt32(value.c_str(), 0);
    ...
    if (SendPacketAndWaitForResponse(packet, packet_len, response, false) ==
      ...
      while (success && response.GetNameColonValue(name, value)) {
        if (name.compare("start") == 0) {
          addr_value = StringConvert::ToUInt64(
              value.c_str(), LLDB_INVALID_ADDRESS, 16, &success);
          if (success)
    ...
    while (response.GetNameColonValue(name, value)) {
      if (name.compare("pid") == 0) {
        process_info.SetProcessID(
            StringConvert::ToUInt32(value.c_str(), LLDB_INVALID_PROCESS_ID, 0));
  ...
  if (SendPacketAndWaitForResponse("qProcessInfo", response, false) ==
    ...
    if (response.IsNormalResponse()) {
      ...
      while (response.GetNameColonValue(name, value)) {
        if (name.compare("cputype") == 0) {
          cpu =
              StringConvert::ToUInt32(value.c_str(), LLDB_INVALID_CPUTYPE, 16);
          if (cpu != LLDB_INVALID_CPUTYPE)
            ++num_keys_decoded;
        } else if (name.compare("cpusubtype") == 0) {
          sub = StringConvert::ToUInt32(value.c_str(), 0, 16);
  ...
  if (SendPacketAndWaitForResponse(packet, packet_len, response, false) ==
    ...
    while (response.GetNameColonValue(name, value)) {
      if (name.compare("port") == 0)
        ...
        pid =
            StringConvert::ToUInt64(value.c_str(), LLDB_INVALID_PROCESS_ID, 0);
...
size_t GDBRemoteCommunicationClient::GetCurrentThreadIDs(
  ...
  if (GetSequenceMutex(locker, "ProcessGDBRemote::UpdateThreadList() failed "
                               "due to not getting the sequence mutex")) {
    sequence_mutex_unavailable = false;
    StringExtractorGDBRemote response;

    PacketResult packet_result;
    for (packet_result = SendPacketAndWaitForResponseNoLock(
             "qfThreadInfo", strlen("qfThreadInfo"), response);
         packet_result == PacketResult::Success && response.IsNormalResponse();
         packet_result = SendPacketAndWaitForResponseNoLock(
             "qsThreadInfo", strlen("qsThreadInfo"), response)) {
...
lldb::addr_t GDBRemoteCommunicationClient::GetShlibInfoAddr() {
  if (!IsRunning()) {
    StringExtractorGDBRemote response;
    if (SendPacketAndWaitForResponse("qShlibInfoAddr",
                                     ::strlen("qShlibInfoAddr"), response,
                                     false) == PacketResult::Success) {
      if (response.IsNormalResponse())
        return response.GetHexMaxU64(false, LLDB_INVALID_ADDRESS);
    }
...
bool GDBRemoteCommunicationClient::ReadRegister(
    lldb::tid_t tid, uint32_t reg, StringExtractorGDBRemote &response) {
  Mutex::Locker locker;
...
bool GDBRemoteCommunicationClient::ReadAllRegisters(
    lldb::tid_t tid, StringExtractorGDBRemote &response) {
  Mutex::Locker locker;
  if (GetSequenceMutex(locker, "Didn't get sequence mutex for g packet.")) {
    ...
    if (thread_suffix_supported || SetCurrentThread(tid)) {
      ...
      if (thread_suffix_supported)
        packet_len =
            ::snprintf(packet, sizeof(packet), "g;thread:%4.4" PRIx64 ";", tid);
      else
...
bool GDBRemoteCommunicationClient::SaveRegisterState(lldb::tid_t tid,
  ...
  Mutex::Locker locker;
  if (GetSequenceMutex(locker,
                       "Didn't get sequence mutex for QSaveRegisterState.")) {
    const bool thread_suffix_supported = GetThreadSuffixSupported();
    if (thread_suffix_supported || SetCurrentThread(tid)) {
      char packet[256];
      if (thread_suffix_supported)
        ::snprintf(packet, sizeof(packet),
                   "QSaveRegisterState;thread:%4.4" PRIx64 ";", tid);
      else
...
bool GDBRemoteCommunicationClient::RestoreRegisterState(lldb::tid_t tid,
  ...
  Mutex::Locker locker;
  if (GetSequenceMutex(
          locker, "Didn't get sequence mutex for QRestoreRegisterState.")) {
    const bool thread_suffix_supported = GetThreadSuffixSupported();
    if (thread_suffix_supported || SetCurrentThread(tid)) {
      char packet[256];
      if (thread_suffix_supported)
        ::snprintf(packet, sizeof(packet),
                   "QRestoreRegisterState:%u;thread:%4.4" PRIx64 ";", save_id,
                   tid);
      else
        ::snprintf(packet, sizeof(packet),
                   "QRestoreRegisterState:%u" PRIx64 ";", save_id);
...
void GDBRemoteCommunicationClient::ServeSymbolLookups(
  ...
  if (m_supports_qSymbol && m_qSymbol_requests_done == false) {
    Mutex::Locker locker;
    if (GetSequenceMutex(locker, "GDBRemoteCommunicationClient::"
                                 "ServeSymbolLookups() failed due to not "
                                 "getting the sequence mutex")) {
      StreamString packet;
      packet.PutCString("qSymbol::");
      StringExtractorGDBRemote response;
      while (SendPacketAndWaitForResponseNoLock(packet.GetData(),
                                                packet.GetSize(), response) ==
             PacketResult::Success) {
        if (response.IsOKResponse()) {
