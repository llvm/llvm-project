...
public:
  ...
  PacketResult SendPacketAndWaitForResponse(const char *send_payload,
  ...
  lldb::StateType SendContinuePacketAndWaitForResponse(
      ProcessGDBRemote *process, const char *packet_payload,
      size_t packet_length, StringExtractorGDBRemote &response);

  bool SendvContPacket(ProcessGDBRemote *process, const char *payload,
                       size_t packet_length,
                       StringExtractorGDBRemote &response);

  bool GetThreadSuffixSupported() override;
  ...
  bool SendInterrupt(Mutex::Locker &locker, uint32_t seconds_to_wait_for_stop,
                     bool &timed_out);
  ...
  bool ReadRegister(lldb::tid_t tid, uint32_t reg_num,
                    StringExtractorGDBRemote &response);

  bool ReadAllRegisters(lldb::tid_t tid, StringExtractorGDBRemote &response);
...
protected:
  ...
  int m_async_signal; // We were asked to deliver a signal to the inferior
                      // process.
  ...
  PacketResult
  SendPacketAndWaitForResponseNoLock(const char *payload, size_t payload_length,
                                     StringExtractorGDBRemote &response);
