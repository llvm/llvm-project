...
GDBRemoteCommunication::GDBRemoteCommunication(const char *comm_name,
    ...
    : Communication(comm_name),
      ...
      m_sequence_mutex(Mutex::eMutexTypeRecursive), m_public_is_running(false),
      m_private_is_running(false), m_history(512), m_send_acks(true),
...
char GDBRemoteCommunication::CalculcateChecksum(const char *payload,
...
GDBRemoteCommunication::PacketResult
GDBRemoteCommunication::SendPacketNoLock(const char *payload,
                                         size_t payload_length) {
  if (IsConnected()) {
...
bool GDBRemoteCommunication::GetSequenceMutex(Mutex::Locker &locker,
...
bool GDBRemoteCommunication::WaitForNotRunningPrivate(
    const TimeValue *timeout_ptr) {
      ...
      case eConnectionStatusInterrupted:
        if (sync_on_timeout) {
          ...
          PacketResult echo_packet_result =
              SendPacketNoLock(echo_packet, echo_packet_len);
...
bool GDBRemoteCommunication::DecompressPacket() {
  ...
  if (GetSendAcks()) {
    ...
    long actual_checksum =
        CalculcateChecksum(m_bytes.data() + 1, hash_mark_idx - 1);
        ...
        if (::isxdigit(m_bytes[checksum_idx + 0]) ||
          ...
          if (GetSendAcks()) {
            ...
            char actual_checksum =
                CalculcateChecksum(packet_str.c_str(), packet_str.size());
