...
// Only send out MachO info when lldb-platform/llgs is running on a MachO host.
  ...
  if (cpu == ArchSpec::kCore_arm_any)
    response.Printf(
        "watchpoint_exceptions_received:before;"); // On armv7 we use
                                                   // "synchronous" watchpoints
                                                   // which means the exception
  ...
  if (packet.GetChar() == ':') {
    ...
    while (packet.GetNameColonValue(key, value)) {
      ...
      if (key.compare("name") == 0) {
        ...
        match_info.GetProcessInfo().GetExecutableFile().SetFile(value.c_str(),
                                                                false);
        ...
        match_info.GetProcessInfo().SetProcessID(StringConvert::ToUInt32(
            value.c_str(), LLDB_INVALID_PROCESS_ID, 0, &success));
        ...
        match_info.GetProcessInfo().SetUserID(
            StringConvert::ToUInt32(value.c_str(), UINT32_MAX, 0, &success));
        ...
        match_info.SetMatchAllUsers(
            Args::StringToBoolean(value.c_str(), false, &success));
...
GDBRemoteCommunicationServerCommon::Handle_qSpeedTest(
  ...
  if (success && key.compare("response_size") == 0) {
    uint32_t response_size =
        StringConvert::ToUInt32(value.c_str(), 0, 0, &success);
    if (success) {
      if (response_size == 0)
