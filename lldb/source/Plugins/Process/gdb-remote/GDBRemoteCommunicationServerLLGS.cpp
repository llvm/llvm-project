...
GDBRemoteCommunicationServerLLGS::GDBRemoteCommunicationServerLLGS(
    const lldb::PlatformSP &platform_sp, MainLoop &mainloop)
    : GDBRemoteCommunicationServerCommon("gdb-remote.server",
                                         "gdb-remote.server.rx_packet"),
      m_platform_sp(platform_sp), m_mainloop(mainloop),
...
Error GDBRemoteCommunicationServerLLGS::LaunchProcess() {
  ...
  Error error;
  {
    ...
    assert(!m_debugged_process_sp &&
           "lldb-gdbserver creating debugged process but one already exists");
  ...
  if (m_process_launch_info.GetFileActionForFD(STDIN_FILENO) == nullptr ||
