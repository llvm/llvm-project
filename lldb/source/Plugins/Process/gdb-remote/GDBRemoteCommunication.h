...
public:
  ...
  char CalculcateChecksum(const char *payload, size_t payload_length);

  bool GetSequenceMutex(Mutex::Locker &locker,
                        const char *failure_message = nullptr);

  PacketType CheckForPacket(const uint8_t *src, size_t src_len,
                            StringExtractorGDBRemote &packet);

  bool IsRunning() const { return m_public_is_running.GetValue(); }
  ...
  //------------------------------------------------------------------
  virtual bool GetThreadSuffixSupported() = 0;
  ...
  //------------------------------------------------------------------
  Error StartDebugserverProcess(const char *url,
                                Platform *platform, // If non nullptr, then
                                                    // check with the platform
                                                    // for the GDB server binary
                                                    // if it can't be located
                                ProcessLaunchInfo &launch_info, uint16_t *port,
...
protected:
...
#else
  Mutex m_sequence_mutex; // Restrict access to sending/receiving packets to a
  ...
  PacketResult SendPacket(const char *payload, size_t payload_length);

  PacketResult SendPacketNoLock(const char *payload, size_t payload_length);
  ...
  bool WaitForNotRunningPrivate(const TimeValue *timeout_ptr);
