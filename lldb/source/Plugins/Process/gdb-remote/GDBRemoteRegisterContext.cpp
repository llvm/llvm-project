...
bool GDBRemoteRegisterContext::PrivateSetRegisterValue(
    uint32_t reg, StringExtractor &response) {
  ...
  const size_t bytes_copied =
      response.GetHexBytes(const_cast<uint8_t *>(m_reg_data.PeekData(
                               reg_info->byte_offset, reg_byte_size)),
                           reg_byte_size, '\xcc');
  ...
  if (!GetRegisterIsValid(reg)) {
    if (m_read_all_at_once) {
      ...
      if (response.IsNormalResponse())
        if (response.GetHexBytes(
                const_cast<void *>(
                    reinterpret_cast<const void *>(m_reg_data.GetDataStart())),
...
bool GDBRemoteRegisterContext::SetPrimordialRegister(
  ...
  if (gdb_comm.SendPacketAndWaitForResponse(
          packet.GetString().c_str(), packet.GetString().size(), response,
...
void GDBRemoteRegisterContext::SyncThreadState(Process *process) {
...
bool GDBRemoteRegisterContext::WriteRegisterBytes(const RegisterInfo *reg_info,
  ...
  GDBRemoteCommunicationClient &gdb_comm(
      ((ProcessGDBRemote *)process)->GetGDBRemote());
// FIXME: This check isn't right because IsRunning checks the Public state, but
// this
// is work you need to do - for instance in ShouldStop & friends - before the
// public
...
//        return false;
                               ...
                               m_reg_data.GetByteOrder())) // dst byte order
  {
    ...
    if (gdb_comm.GetSequenceMutex(
            locker, "Didn't get sequence mutex for write register.")) {
      ...
      if (thread_suffix_supported ||
        ...
        if (m_read_all_at_once) {
          ...
          if (reg_info->value_regs) {
            ...
            // the actual
            // register data for any "value_regs", and once all that data is
            // read, we will
...
bool GDBRemoteRegisterContext::ReadAllRegisterValues(
  ...
  Mutex::Locker locker;
  if (gdb_comm.GetSequenceMutex(
          locker, "Didn't get sequence mutex for read all registers.")) {
    ...
    if (thread_suffix_supported ||
      ...
      if (use_g_packet &&
          gdb_comm.SendPacketAndWaitForResponse(packet, packet_len, response,
                                                false) ==
              GDBRemoteCommunication::PacketResult::Success) {
        int packet_len = 0;
        if (thread_suffix_supported)
        ...
        if (gdb_comm.SendPacketAndWaitForResponse(packet, packet_len, response,
                                                  false) ==
            GDBRemoteCommunication::PacketResult::Success) {
          ...
          if (isxdigit(response_str[0])) {
            ...
            if (thread_suffix_supported) {
              ...
              ::snprintf(thread_id_cstr, sizeof(thread_id_cstr),
                         ";thread:%4.4" PRIx64 ";", m_thread.GetProtocolID());
              response_str.append(thread_id_cstr);
            }
            data_sp.reset(
                new DataBufferHeap(response_str.c_str(), response_str.size()));
            return true;
          }
        }
      } else {
        // For the use_g_packet == false case, we're going to read each register
        // individually and store them as binary data in a buffer instead of as
        // ascii
        // characters.
        const RegisterInfo *reg_info;

        // data_sp will take ownership of this DataBufferHeap pointer soon.
        DataBufferSP reg_ctx(
            new DataBufferHeap(m_reg_info.GetRegisterDataByteSize(), 0));
        ...
        memcpy(reg_ctx->GetBytes(), m_reg_data.GetDataStart(),
...
bool GDBRemoteRegisterContext::WriteAllRegisterValues(
  ...
  Mutex::Locker locker;
  if (gdb_comm.GetSequenceMutex(
          locker, "Didn't get sequence mutex for write all registers.")) {
    const bool thread_suffix_supported = gdb_comm.GetThreadSuffixSupported();
    ProcessSP process_sp(m_thread.GetProcess());
    if (thread_suffix_supported ||
        static_cast<ProcessGDBRemote *>(process_sp.get())
      ...
      if (use_g_packet &&
          gdb_comm.SendPacketAndWaitForResponse(G_packet, G_packet_len,
                                                response, false) ==
              GDBRemoteCommunication::PacketResult::Success) {
        // The data_sp contains the entire G response packet including the
        // G, and if the thread suffix is supported, it has the thread suffix
        ...
        const char *G_packet = (const char *)data_sp->GetBytes();
        size_t G_packet_len = data_sp->GetByteSize();
        if (gdb_comm.SendPacketAndWaitForResponse(G_packet, G_packet_len,
                                                  response, false) ==
            GDBRemoteCommunication::PacketResult::Success) {
          if (response.IsOKResponse())
            return true;
            ...
            // G_packet_len is hex-ascii characters plus prefix 'G' plus suffix
            // thread specifier.
            // This means buffer will be a little more than 2x larger than
            // necessary but we resize
            // it down once we've extracted all hex ascii chars from the packet.
            DataBufferHeap buffer(G_packet_len, 0);

            const uint32_t bytes_extracted = response.GetHexBytes(
                buffer.GetBytes(), buffer.GetByteSize(), '\xcc');

            DataExtractor restore_data(buffer.GetBytes(), buffer.GetByteSize(),
                                       m_reg_data.GetByteOrder(),
                                       m_reg_data.GetAddressByteSize());

            if (bytes_extracted < restore_data.GetByteSize())
              restore_data.SetData(restore_data.GetDataStart(), bytes_extracted,
                                   m_reg_data.GetByteOrder());
            ...
            // terms of other registers, e.g. eax is a subset of rax) or not.
            // The slice registers
            // should NOT be in the g packet, but some implementations may
            // incorrectly include them.
            //
            // If the slice registers are included in the packet, we must step
            // over the slice registers
            // when parsing the packet -- relying on the RegisterInfo
            // byte_offset field would be incorrect.
            // If the slice registers are not included, then using the
            // byte_offset values into the
            ...
            if (size_by_highest_offset == restore_data.GetByteSize()) {
              // The size of the packet agrees with the highest offset: + size
              ...
              use_byte_offset_into_buffer = true;
            } else if (size_including_slice_registers ==
                       restore_data.GetByteSize()) {
              // The slice registers are present in the packet (when they
              // shouldn't be).
              // Don't try to use the RegisterInfo byte_offset into the
              // restore_data, it will
              // point to the wrong place.
              use_byte_offset_into_buffer = false;
            } else {
              // None of our expected sizes match the actual g packet data we're
              // looking at.
              // The most conservative approach here is to use the running total
              // byte offset.
              use_byte_offset_into_buffer = false;
            }

            // In case our register definitions don't include the correct
            // offsets,
            // keep track of the size of each reg & compute offset based on
            // that.
            uint32_t running_byte_offset = 0;
            for (uint32_t reg_idx = 0;
                 (reg_info = GetRegisterInfoAtIndex(reg_idx)) != NULL;
                 ++reg_idx, running_byte_offset += reg_info->byte_size) {
              // Skip composite aka slice registers (e.g. eax is a slice of
              // rax).
              if (reg_info->value_regs)
                continue;
              ...
              const char *restore_src = (const char *)restore_data.PeekData(
                  register_offset, reg_byte_size);
              if (restore_src) {
                StreamString packet;
                packet.Printf("P%x=", reg);
                ...
                SetRegisterIsValid(reg, false);
                if (gdb_comm.SendPacketAndWaitForResponse(
                        packet.GetString().c_str(), packet.GetString().size(),
                        response, false) ==
                    GDBRemoteCommunication::PacketResult::Success) {
                  const char *current_src = (const char *)m_reg_data.PeekData(
                      register_offset, reg_byte_size);
                  if (current_src)
                    write_reg =
                        memcmp(current_src, restore_src, reg_byte_size) != 0;
                }
        ...
        // For the use_g_packet == false case, we're going to write each
        // register
        // individually.  The data buffer is binary data in this case, instead
        // of
        // ascii characters.
        ...
        for (uint32_t i = 0; (reg_info = GetRegisterInfoAtIndex(i)) != NULL;
          ...
          // writing to
          // work around a bug in an older version of debugserver that would
          // lead to
...
void GDBRemoteDynamicRegisterInfo::HardcodeARMRegisters(bool from_scratch) {
  ...
  static RegisterInfo g_register_infos[] = {
      ...
      {"r0",
       ...
       eEncodingUint,
       eFormatHex,
       {ehframe_r0, dwarf_r0, LLDB_REGNUM_GENERIC_ARG1, 0, 0},
       NULL,
       NULL},
      {"r1",
       "arg2",
       ...
       eEncodingUint,
       eFormatHex,
       {ehframe_r1, dwarf_r1, LLDB_REGNUM_GENERIC_ARG2, 1, 1},
       NULL,
       NULL},
      {"r2",
       "arg3",
       ...
       eEncodingUint,
       eFormatHex,
       {ehframe_r2, dwarf_r2, LLDB_REGNUM_GENERIC_ARG3, 2, 2},
       NULL,
       NULL},
      {"r3",
       "arg4",
       ...
       eEncodingUint,
       eFormatHex,
       {ehframe_r3, dwarf_r3, LLDB_REGNUM_GENERIC_ARG4, 3, 3},
       NULL,
       NULL},
      {"r4",
       NULL,
       ...
       eEncodingUint,
       eFormatHex,
       {ehframe_r4, dwarf_r4, LLDB_INVALID_REGNUM, 4, 4},
       NULL,
       NULL},
      {"r5",
       NULL,
       ...
       eEncodingUint,
       eFormatHex,
       {ehframe_r5, dwarf_r5, LLDB_INVALID_REGNUM, 5, 5},
       NULL,
       NULL},
      {"r6",
       NULL,
       ...
       eEncodingUint,
       eFormatHex,
       {ehframe_r6, dwarf_r6, LLDB_INVALID_REGNUM, 6, 6},
       NULL,
       NULL},
      {"r7",
       "fp",
       ...
       eEncodingUint,
       eFormatHex,
       {ehframe_r7, dwarf_r7, LLDB_REGNUM_GENERIC_FP, 7, 7},
       NULL,
       NULL},
      {"r8",
       NULL,
       ...
       eEncodingUint,
       eFormatHex,
       {ehframe_r8, dwarf_r8, LLDB_INVALID_REGNUM, 8, 8},
       NULL,
       NULL},
      {"r9",
       NULL,
       ...
       eEncodingUint,
       eFormatHex,
       {ehframe_r9, dwarf_r9, LLDB_INVALID_REGNUM, 9, 9},
       NULL,
       NULL},
      {"r10",
       NULL,
       ...
       eEncodingUint,
       eFormatHex,
       {ehframe_r10, dwarf_r10, LLDB_INVALID_REGNUM, 10, 10},
       NULL,
       NULL},
      {"r11",
       NULL,
       ...
       eEncodingUint,
       eFormatHex,
       {ehframe_r11, dwarf_r11, LLDB_INVALID_REGNUM, 11, 11},
       NULL,
       NULL},
      {"r12",
       NULL,
       ...
       eEncodingUint,
       eFormatHex,
       {ehframe_r12, dwarf_r12, LLDB_INVALID_REGNUM, 12, 12},
       NULL,
       NULL},
      {"sp",
       "r13",
       ...
       eEncodingUint,
       eFormatHex,
       {ehframe_sp, dwarf_sp, LLDB_REGNUM_GENERIC_SP, 13, 13},
       NULL,
       NULL},
      {"lr",
       "r14",
       ...
       eEncodingUint,
       eFormatHex,
       {ehframe_lr, dwarf_lr, LLDB_REGNUM_GENERIC_RA, 14, 14},
       NULL,
       NULL},
      {"pc",
       "r15",
       ...
       eEncodingUint,
       eFormatHex,
       {ehframe_pc, dwarf_pc, LLDB_REGNUM_GENERIC_PC, 15, 15},
       NULL,
       NULL},
      {"f0",
       NULL,
       ...
       eEncodingUint,
       eFormatHex,
       {LLDB_INVALID_REGNUM, LLDB_INVALID_REGNUM, LLDB_INVALID_REGNUM, 16, 16},
       NULL,
       NULL},
      {"f1",
       NULL,
       ...
       eEncodingUint,
       eFormatHex,
       {LLDB_INVALID_REGNUM, LLDB_INVALID_REGNUM, LLDB_INVALID_REGNUM, 17, 17},
       NULL,
       NULL},
      {"f2",
       NULL,
       ...
       eEncodingUint,
       eFormatHex,
       {LLDB_INVALID_REGNUM, LLDB_INVALID_REGNUM, LLDB_INVALID_REGNUM, 18, 18},
       NULL,
       NULL},
      {"f3",
       NULL,
       ...
       eEncodingUint,
       eFormatHex,
       {LLDB_INVALID_REGNUM, LLDB_INVALID_REGNUM, LLDB_INVALID_REGNUM, 19, 19},
       NULL,
       NULL},
      {"f4",
       NULL,
       ...
       eEncodingUint,
       eFormatHex,
       {LLDB_INVALID_REGNUM, LLDB_INVALID_REGNUM, LLDB_INVALID_REGNUM, 20, 20},
       NULL,
       NULL},
      {"f5",
       NULL,
       ...
       eEncodingUint,
       eFormatHex,
       {LLDB_INVALID_REGNUM, LLDB_INVALID_REGNUM, LLDB_INVALID_REGNUM, 21, 21},
       NULL,
       NULL},
      {"f6",
       NULL,
       ...
       eEncodingUint,
       eFormatHex,
       {LLDB_INVALID_REGNUM, LLDB_INVALID_REGNUM, LLDB_INVALID_REGNUM, 22, 22},
       NULL,
       NULL},
      {"f7",
       NULL,
       ...
       eEncodingUint,
       eFormatHex,
       {LLDB_INVALID_REGNUM, LLDB_INVALID_REGNUM, LLDB_INVALID_REGNUM, 23, 23},
       NULL,
       NULL},
      {"fps",
       NULL,
       ...
       eEncodingUint,
       eFormatHex,
       {LLDB_INVALID_REGNUM, LLDB_INVALID_REGNUM, LLDB_INVALID_REGNUM, 24, 24},
       NULL,
       NULL},
      {"cpsr",
       "flags",
       ...
       eEncodingUint,
       eFormatHex,
       {ehframe_cpsr, dwarf_cpsr, LLDB_INVALID_REGNUM, 25, 25},
       NULL,
       NULL},
      {"s0",
       NULL,
       ...
       eEncodingIEEE754,
       eFormatFloat,
       {LLDB_INVALID_REGNUM, dwarf_s0, LLDB_INVALID_REGNUM, 26, 26},
       NULL,
       NULL},
      {"s1",
       NULL,
       ...
       eEncodingIEEE754,
       eFormatFloat,
       {LLDB_INVALID_REGNUM, dwarf_s1, LLDB_INVALID_REGNUM, 27, 27},
       NULL,
       NULL},
      {"s2",
       NULL,
       ...
       eEncodingIEEE754,
       eFormatFloat,
       {LLDB_INVALID_REGNUM, dwarf_s2, LLDB_INVALID_REGNUM, 28, 28},
       NULL,
       NULL},
      {"s3",
       NULL,
       ...
       eEncodingIEEE754,
       eFormatFloat,
       {LLDB_INVALID_REGNUM, dwarf_s3, LLDB_INVALID_REGNUM, 29, 29},
       NULL,
       NULL},
      {"s4",
       NULL,
       ...
       eEncodingIEEE754,
       eFormatFloat,
       {LLDB_INVALID_REGNUM, dwarf_s4, LLDB_INVALID_REGNUM, 30, 30},
       NULL,
       NULL},
      {"s5",
       NULL,
       ...
       eEncodingIEEE754,
       eFormatFloat,
       {LLDB_INVALID_REGNUM, dwarf_s5, LLDB_INVALID_REGNUM, 31, 31},
       NULL,
       NULL},
      {"s6",
       NULL,
       ...
       eEncodingIEEE754,
       eFormatFloat,
       {LLDB_INVALID_REGNUM, dwarf_s6, LLDB_INVALID_REGNUM, 32, 32},
       NULL,
       NULL},
      {"s7",
       NULL,
       ...
       eEncodingIEEE754,
       eFormatFloat,
       {LLDB_INVALID_REGNUM, dwarf_s7, LLDB_INVALID_REGNUM, 33, 33},
       NULL,
       NULL},
      {"s8",
       NULL,
       ...
       eEncodingIEEE754,
       eFormatFloat,
       {LLDB_INVALID_REGNUM, dwarf_s8, LLDB_INVALID_REGNUM, 34, 34},
       NULL,
       NULL},
      {"s9",
       NULL,
       ...
       eEncodingIEEE754,
       eFormatFloat,
       {LLDB_INVALID_REGNUM, dwarf_s9, LLDB_INVALID_REGNUM, 35, 35},
       NULL,
       NULL},
      {"s10",
       NULL,
       ...
       eEncodingIEEE754,
       eFormatFloat,
       {LLDB_INVALID_REGNUM, dwarf_s10, LLDB_INVALID_REGNUM, 36, 36},
       NULL,
       NULL},
      {"s11",
       NULL,
       ...
       eEncodingIEEE754,
       eFormatFloat,
       {LLDB_INVALID_REGNUM, dwarf_s11, LLDB_INVALID_REGNUM, 37, 37},
       NULL,
       NULL},
      {"s12",
       NULL,
       ...
       eEncodingIEEE754,
       eFormatFloat,
       {LLDB_INVALID_REGNUM, dwarf_s12, LLDB_INVALID_REGNUM, 38, 38},
       NULL,
       NULL},
      {"s13",
       NULL,
       ...
       eEncodingIEEE754,
       eFormatFloat,
       {LLDB_INVALID_REGNUM, dwarf_s13, LLDB_INVALID_REGNUM, 39, 39},
       NULL,
       NULL},
      {"s14",
       NULL,
       ...
       eEncodingIEEE754,
       eFormatFloat,
       {LLDB_INVALID_REGNUM, dwarf_s14, LLDB_INVALID_REGNUM, 40, 40},
       NULL,
       NULL},
      {"s15",
       NULL,
       ...
       eEncodingIEEE754,
       eFormatFloat,
       {LLDB_INVALID_REGNUM, dwarf_s15, LLDB_INVALID_REGNUM, 41, 41},
       NULL,
       NULL},
      {"s16",
       NULL,
       ...
       eEncodingIEEE754,
       eFormatFloat,
       {LLDB_INVALID_REGNUM, dwarf_s16, LLDB_INVALID_REGNUM, 42, 42},
       NULL,
       NULL},
      {"s17",
       NULL,
       ...
       eEncodingIEEE754,
       eFormatFloat,
       {LLDB_INVALID_REGNUM, dwarf_s17, LLDB_INVALID_REGNUM, 43, 43},
       NULL,
       NULL},
      {"s18",
       NULL,
       ...
       eEncodingIEEE754,
       eFormatFloat,
       {LLDB_INVALID_REGNUM, dwarf_s18, LLDB_INVALID_REGNUM, 44, 44},
       NULL,
       NULL},
      {"s19",
       NULL,
       ...
       eEncodingIEEE754,
       eFormatFloat,
       {LLDB_INVALID_REGNUM, dwarf_s19, LLDB_INVALID_REGNUM, 45, 45},
       NULL,
       NULL},
      {"s20",
       NULL,
       ...
       eEncodingIEEE754,
       eFormatFloat,
       {LLDB_INVALID_REGNUM, dwarf_s20, LLDB_INVALID_REGNUM, 46, 46},
       NULL,
       NULL},
      {"s21",
       NULL,
       ...
       eEncodingIEEE754,
       eFormatFloat,
       {LLDB_INVALID_REGNUM, dwarf_s21, LLDB_INVALID_REGNUM, 47, 47},
       NULL,
       NULL},
      {"s22",
       NULL,
       ...
       eEncodingIEEE754,
       eFormatFloat,
       {LLDB_INVALID_REGNUM, dwarf_s22, LLDB_INVALID_REGNUM, 48, 48},
       NULL,
       NULL},
      {"s23",
       NULL,
       ...
       eEncodingIEEE754,
       eFormatFloat,
       {LLDB_INVALID_REGNUM, dwarf_s23, LLDB_INVALID_REGNUM, 49, 49},
       NULL,
       NULL},
      {"s24",
       NULL,
       ...
       eEncodingIEEE754,
       eFormatFloat,
       {LLDB_INVALID_REGNUM, dwarf_s24, LLDB_INVALID_REGNUM, 50, 50},
       NULL,
       NULL},
      {"s25",
       NULL,
       ...
       eEncodingIEEE754,
       eFormatFloat,
       {LLDB_INVALID_REGNUM, dwarf_s25, LLDB_INVALID_REGNUM, 51, 51},
       NULL,
       NULL},
      {"s26",
       NULL,
       ...
       eEncodingIEEE754,
       eFormatFloat,
       {LLDB_INVALID_REGNUM, dwarf_s26, LLDB_INVALID_REGNUM, 52, 52},
       NULL,
       NULL},
      {"s27",
       NULL,
      ...
      {"s28",
       ...
       eEncodingIEEE754,
       eFormatFloat,
       {LLDB_INVALID_REGNUM, dwarf_s28, LLDB_INVALID_REGNUM, 54, 54},
       NULL,
       NULL},
      {"s29",
       NULL,
       ...
       eEncodingIEEE754,
       eFormatFloat,
       {LLDB_INVALID_REGNUM, dwarf_s29, LLDB_INVALID_REGNUM, 55, 55},
       NULL,
       NULL},
      {"s30",
       NULL,
       ...
       eEncodingIEEE754,
       eFormatFloat,
       {LLDB_INVALID_REGNUM, dwarf_s30, LLDB_INVALID_REGNUM, 56, 56},
       NULL,
       NULL},
      {"s31",
       NULL,
       ...
       eEncodingIEEE754,
       eFormatFloat,
       {LLDB_INVALID_REGNUM, dwarf_s31, LLDB_INVALID_REGNUM, 57, 57},
       NULL,
       NULL},
      {"fpscr",
       NULL,
       ...
       eEncodingUint,
       eFormatHex,
       {LLDB_INVALID_REGNUM, LLDB_INVALID_REGNUM, LLDB_INVALID_REGNUM, 58, 58},
       NULL,
       NULL},
      {"d16",
       NULL,
       ...
       eEncodingIEEE754,
       eFormatFloat,
       {LLDB_INVALID_REGNUM, dwarf_d16, LLDB_INVALID_REGNUM, 59, 59},
       NULL,
       NULL},
      {"d17",
       NULL,
       ...
       eEncodingIEEE754,
       eFormatFloat,
       {LLDB_INVALID_REGNUM, dwarf_d17, LLDB_INVALID_REGNUM, 60, 60},
       NULL,
       NULL},
      {"d18",
       NULL,
       ...
       eEncodingIEEE754,
       eFormatFloat,
       {LLDB_INVALID_REGNUM, dwarf_d18, LLDB_INVALID_REGNUM, 61, 61},
       NULL,
       NULL},
      {"d19",
       NULL,
       ...
       eEncodingIEEE754,
       eFormatFloat,
       {LLDB_INVALID_REGNUM, dwarf_d19, LLDB_INVALID_REGNUM, 62, 62},
       NULL,
       NULL},
      {"d20",
       NULL,
       ...
       eEncodingIEEE754,
       eFormatFloat,
       {LLDB_INVALID_REGNUM, dwarf_d20, LLDB_INVALID_REGNUM, 63, 63},
       NULL,
       NULL},
      {"d21",
       NULL,
       ...
       eEncodingIEEE754,
       eFormatFloat,
       {LLDB_INVALID_REGNUM, dwarf_d21, LLDB_INVALID_REGNUM, 64, 64},
       NULL,
       NULL},
      {"d22",
       NULL,
       ...
       eEncodingIEEE754,
       eFormatFloat,
       {LLDB_INVALID_REGNUM, dwarf_d22, LLDB_INVALID_REGNUM, 65, 65},
       NULL,
       NULL},
      {"d23",
       NULL,
       ...
       eEncodingIEEE754,
       eFormatFloat,
       {LLDB_INVALID_REGNUM, dwarf_d23, LLDB_INVALID_REGNUM, 66, 66},
       NULL,
       NULL},
      {"d24",
       NULL,
       ...
       eEncodingIEEE754,
       eFormatFloat,
       {LLDB_INVALID_REGNUM, dwarf_d24, LLDB_INVALID_REGNUM, 67, 67},
       NULL,
       NULL},
      {"d25",
       NULL,
       ...
       eEncodingIEEE754,
       eFormatFloat,
       {LLDB_INVALID_REGNUM, dwarf_d25, LLDB_INVALID_REGNUM, 68, 68},
       NULL,
       NULL},
      {"d26",
       NULL,
       ...
       eEncodingIEEE754,
       eFormatFloat,
       {LLDB_INVALID_REGNUM, dwarf_d26, LLDB_INVALID_REGNUM, 69, 69},
       NULL,
       NULL},
      {"d27",
       NULL,
       ...
       eEncodingIEEE754,
       eFormatFloat,
       {LLDB_INVALID_REGNUM, dwarf_d27, LLDB_INVALID_REGNUM, 70, 70},
       NULL,
       NULL},
      {"d28",
       NULL,
       ...
       eEncodingIEEE754,
       eFormatFloat,
       {LLDB_INVALID_REGNUM, dwarf_d28, LLDB_INVALID_REGNUM, 71, 71},
       NULL,
       NULL},
      {"d29",
       NULL,
       ...
       eEncodingIEEE754,
       eFormatFloat,
       {LLDB_INVALID_REGNUM, dwarf_d29, LLDB_INVALID_REGNUM, 72, 72},
       NULL,
       NULL},
      {"d30",
       NULL,
       ...
       eEncodingIEEE754,
       eFormatFloat,
       {LLDB_INVALID_REGNUM, dwarf_d30, LLDB_INVALID_REGNUM, 73, 73},
       NULL,
       NULL},
      {"d31",
       NULL,
       ...
       eEncodingIEEE754,
       eFormatFloat,
       {LLDB_INVALID_REGNUM, dwarf_d31, LLDB_INVALID_REGNUM, 74, 74},
       NULL,
       NULL},
      {"d0",
       NULL,
       ...
       eEncodingIEEE754,
       eFormatFloat,
       {LLDB_INVALID_REGNUM, dwarf_d0, LLDB_INVALID_REGNUM, 75, 75},
       g_d0_regs,
       NULL},
      {"d1",
       NULL,
       ...
       eEncodingIEEE754,
       eFormatFloat,
       {LLDB_INVALID_REGNUM, dwarf_d1, LLDB_INVALID_REGNUM, 76, 76},
       g_d1_regs,
       NULL},
      {"d2",
       NULL,
       ...
       eEncodingIEEE754,
       eFormatFloat,
       {LLDB_INVALID_REGNUM, dwarf_d2, LLDB_INVALID_REGNUM, 77, 77},
       g_d2_regs,
       NULL},
      {"d3",
       NULL,
       ...
       eEncodingIEEE754,
       eFormatFloat,
       {LLDB_INVALID_REGNUM, dwarf_d3, LLDB_INVALID_REGNUM, 78, 78},
       g_d3_regs,
       NULL},
      {"d4",
       NULL,
       ...
       eEncodingIEEE754,
       eFormatFloat,
       {LLDB_INVALID_REGNUM, dwarf_d4, LLDB_INVALID_REGNUM, 79, 79},
       g_d4_regs,
       NULL},
      {"d5",
       NULL,
       ...
       eEncodingIEEE754,
       eFormatFloat,
       {LLDB_INVALID_REGNUM, dwarf_d5, LLDB_INVALID_REGNUM, 80, 80},
       g_d5_regs,
       NULL},
      {"d6",
       NULL,
       ...
       eEncodingIEEE754,
       eFormatFloat,
       {LLDB_INVALID_REGNUM, dwarf_d6, LLDB_INVALID_REGNUM, 81, 81},
       g_d6_regs,
       NULL},
      {"d7",
       NULL,
       ...
       eEncodingIEEE754,
       eFormatFloat,
       {LLDB_INVALID_REGNUM, dwarf_d7, LLDB_INVALID_REGNUM, 82, 82},
       g_d7_regs,
       NULL},
      {"d8",
       NULL,
       ...
       eEncodingIEEE754,
       eFormatFloat,
       {LLDB_INVALID_REGNUM, dwarf_d8, LLDB_INVALID_REGNUM, 83, 83},
       g_d8_regs,
       NULL},
      {"d9",
       NULL,
       ...
       eEncodingIEEE754,
       eFormatFloat,
       {LLDB_INVALID_REGNUM, dwarf_d9, LLDB_INVALID_REGNUM, 84, 84},
       g_d9_regs,
       NULL},
      {"d10",
       NULL,
       ...
       eEncodingIEEE754,
       eFormatFloat,
       {LLDB_INVALID_REGNUM, dwarf_d10, LLDB_INVALID_REGNUM, 85, 85},
       g_d10_regs,
       NULL},
      {"d11",
       NULL,
       ...
       eEncodingIEEE754,
       eFormatFloat,
       {LLDB_INVALID_REGNUM, dwarf_d11, LLDB_INVALID_REGNUM, 86, 86},
       g_d11_regs,
       NULL},
      {"d12",
       NULL,
       ...
       eEncodingIEEE754,
       eFormatFloat,
       {LLDB_INVALID_REGNUM, dwarf_d12, LLDB_INVALID_REGNUM, 87, 87},
       g_d12_regs,
       NULL},
      {"d13",
       NULL,
       ...
       eEncodingIEEE754,
       eFormatFloat,
       {LLDB_INVALID_REGNUM, dwarf_d13, LLDB_INVALID_REGNUM, 88, 88},
       g_d13_regs,
       NULL},
      {"d14",
       NULL,
       ...
       eEncodingIEEE754,
       eFormatFloat,
       {LLDB_INVALID_REGNUM, dwarf_d14, LLDB_INVALID_REGNUM, 89, 89},
       g_d14_regs,
       NULL},
      {"d15",
       NULL,
       ...
       eEncodingIEEE754,
       eFormatFloat,
       {LLDB_INVALID_REGNUM, dwarf_d15, LLDB_INVALID_REGNUM, 90, 90},
       g_d15_regs,
       NULL},
      {"q0",
       NULL,
