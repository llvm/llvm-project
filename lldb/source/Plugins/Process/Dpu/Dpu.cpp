...
const std::string bkp_fault_description[6] = {
    "", "Heap Full", "Division by Zero", "Assert", "Halt", "Printf Overflow"};
  ...
  if (bkp_fault && bkp_fault_thread_index == thread_index) {
    if (bkp_fault_id == 0) {
      ...
      description = "fault " + std::to_string(bkp_fault_id) + " (" +
                    bkp_fault_description[bkp_fault_id] + ")";
    ...
    // check if it is a memory fault generated by mem_alloc_nolock, in which
    // case it is better to print a fault 1
    uint32_t thread_pc = m_context->GetPcOfThread(thread_index);
    if (thread_pc > k_dpu_iram_base) {
