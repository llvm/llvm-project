...
namespace {
Error ResolveProcessArchitecture(lldb::pid_t pid, Platform &platform,
                                 ArchSpec &arch) {
...
NativeProcessLinux::LaunchArgs::LaunchArgs(Module *module, char const **argv,
                                           char const **envp,
    ...
    : m_module(module), m_argv(argv), m_envp(envp),
...
Error NativeProcessProtocol::Launch(
  ...
  Error error =
      platform_sp->ResolveExecutable(ModuleSpec(launch_info.GetExecutableFile(),
                                                launch_info.GetArchitecture()),
                                     exe_module_sp, nullptr);
  ...
  if (log) {
    if (stdin_file_spec)
      log->Printf("NativeProcessLinux::%s setting STDIN to '%s'", __FUNCTION__,
                  stdin_file_spec.GetCString());
    else
      log->Printf("NativeProcessLinux::%s leaving STDIN as is", __FUNCTION__);

    if (stdout_file_spec)
      log->Printf("NativeProcessLinux::%s setting STDOUT to '%s'", __FUNCTION__,
                  stdout_file_spec.GetCString());
    else
      log->Printf("NativeProcessLinux::%s leaving STDOUT as is", __FUNCTION__);

    if (stderr_file_spec)
      log->Printf("NativeProcessLinux::%s setting STDERR to '%s'", __FUNCTION__,
                  stderr_file_spec.GetCString());
    else
  ...
  if (log) {
    int i = 0;
    for (const char **
             args = launch_info.GetArguments().GetConstArgumentVector();
         *args; ++args, ++i) {
      log->Printf("NativeProcessLinux::%s arg %d: \"%s\"", __FUNCTION__, i,
                  *args ? *args : "nullptr");
      ++i;
    }
  }
  ...
  std::static_pointer_cast<NativeProcessLinux>(native_process_sp)
      ->LaunchInferior(
          mainloop, exe_module_sp.get(),
...
Error NativeProcessProtocol::Attach(
  ...
  Error error =
      ResolveProcessArchitecture(pid, *platform_sp.get(), process_arch);
  if (!error.Success())
    return error;
...
void NativeProcessLinux::LaunchInferior(
    MainLoop &mainloop, Module *module, const char *argv[], const char *envp[],
    const FileSpec &stdin_file_spec, const FileSpec &stdout_file_spec,
  ...
  std::unique_ptr<LaunchArgs> args(
      new LaunchArgs(module, argv, envp, stdin_file_spec, stdout_file_spec,
                     stderr_file_spec, working_dir, launch_info));
...
::pid_t NativeProcessLinux::Launch(LaunchArgs *args, Error &error) {
  ...
  if (pid == 0) {
    ...
    // FIXME consider opening a pipe between parent/child and have this forked
    // child
    ...
    if (args->m_stdout_file_spec)
      if (!DupDescriptor(args->m_stdout_file_spec, STDOUT_FILENO,
                         O_WRONLY | O_CREAT | O_TRUNC))
        exit(eDupStdoutFailed);

    if (args->m_stderr_file_spec)
      if (!DupDescriptor(args->m_stderr_file_spec, STDERR_FILENO,
                         O_WRONLY | O_CREAT | O_TRUNC))
    ...
    if (args->m_launch_info.GetFlags().Test(lldb::eLaunchFlagDisableASLR)) {
      const int old_personality =
          personality(LLDB_PERSONALITY_GET_CURRENT_SETTINGS);
      if (old_personality == -1) {
        // Can't retrieve Linux personality.  Cannot disable ASLR.
      } else {
        const int new_personality =
            personality(ADDR_NO_RANDOMIZE | old_personality);
  ...
  if ((wpid = waitpid(pid, &status, 0)) < 0) {
    ...
    if (log) {
      log->Printf("NativeProcessLinux::%s inferior exited with status %d "
                  "before issuing a STOP",
...
void NativeProcessLinux::MonitorSIGTRAP(const siginfo_t &info,
  ...
  case (SIGTRAP | (PTRACE_EVENT_EXEC << 8)): {
    ...
    // which only copies the main thread.  Mutexes are in undefined state.
    if (log)
...
void NativeProcessLinux::DoStopIDBumped(uint32_t newBumpId) {
...

  {
    ...
    if (log)
      log->Printf(
          "NativeProcessLinux::%s clearing %" PRIu64 " entries from the cache",
...
// Return the image info address for the exe module
  ...
  if (error.Fail()) {
    if (log)
      log->Warning("NativeProcessLinux::%s failed to retrieve exe module: %s",
                   __FUNCTION__, error.AsCString());
    return LLDB_INVALID_ADDRESS;
  }
  ...
  if (module_sp == nullptr) {
    if (log)
      log->Warning("NativeProcessLinux::%s exe module returned was NULL",
                   __FUNCTION__);
    return LLDB_INVALID_ADDRESS;
  }
  ...
  if (object_file_sp == nullptr) {
    if (log)
      log->Warning(
          "NativeProcessLinux::%s exe module returned a NULL object file",
          __FUNCTION__);
    return LLDB_INVALID_ADDRESS;
  }
...
bool NativeProcessLinux::DupDescriptor(const FileSpec &file_spec, int fd,
                                       int flags) {
