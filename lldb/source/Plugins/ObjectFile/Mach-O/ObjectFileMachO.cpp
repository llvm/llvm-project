...
bool ObjectFileMachO::ParseHeader() {
  ...
  bool can_parse = false;
  lldb::offset_t offset = 0;
  m_data_sp->SetByteOrder(endian::InlHostByteOrder());
  // Leave magic in the original byte order
  m_header.magic = m_data_sp->GetU32(&offset);
  switch (m_header.magic) {
  case MH_MAGIC:
    m_data_sp->SetByteOrder(endian::InlHostByteOrder());
    m_data_sp->SetAddressByteSize(4);
    can_parse = true;
    break;

  case MH_MAGIC_64:
    m_data_sp->SetByteOrder(endian::InlHostByteOrder());
    m_data_sp->SetAddressByteSize(8);
    can_parse = true;
    break;

  case MH_CIGAM:
    m_data_sp->SetByteOrder(endian::InlHostByteOrder() == eByteOrderBig
                                ? eByteOrderLittle
                                : eByteOrderBig);
    m_data_sp->SetAddressByteSize(4);
    can_parse = true;
  ...
  if (can_parse) {
    m_data_sp->GetU32(&offset, &m_header.cputype, 6);

    ModuleSpecList all_specs;
    ModuleSpec base_spec;
    GetAllArchSpecs(m_header, *m_data_sp.get(),
  ...
  if (m_dysymtab.cmd == 0) {
    ...
    if (module_sp) {
      ...
      for (uint32_t i = 0; i < m_header.ncmds; ++i) {
        ...
        if (m_data_sp->GetU32(&offset, &lc.cmd, 2) == nullptr)
          break;
        if (lc.cmd == LC_DYSYMTAB) {
          m_dysymtab.cmd = lc.cmd;
          m_dysymtab.cmdsize = lc.cmdsize;
          if (m_data_sp->GetU32(&offset, &m_dysymtab.ilocalsym,
...
ObjectFileMachO::EncryptedFileRanges ObjectFileMachO::GetEncryptedFileRanges() {
  ...
  for (uint32_t i = 0; i < m_header.ncmds; ++i) {
    const lldb::offset_t load_cmd_offset = offset;
    if (m_data_sp->GetU32(&offset, &encryption_cmd, 2) == nullptr)
      break;
    ...
    if (encryption_cmd.cmd == LC_ENCRYPTION_INFO ||
      ...
      if (m_data_sp->GetU32(&offset, &encryption_cmd.cryptoff, 3)) {
...
void ObjectFileMachO::ProcessSegmentCommand(
  ...
  if (!m_data_sp->GetU8(&offset, (uint8_t *)load_cmd.segname, 16))
    return;
  ...
  load_cmd.vmaddr = m_data_sp->GetAddress(&offset);
  load_cmd.vmsize = m_data_sp->GetAddress(&offset);
  load_cmd.fileoff = m_data_sp->GetAddress(&offset);
  load_cmd.filesize = m_data_sp->GetAddress(&offset);
  if (!m_data_sp->GetU32(&offset, &load_cmd.maxprot, 4))
    return;
  ...
  for (segment_sect_idx = 0; segment_sect_idx < load_cmd.nsects;
       ++segment_sect_idx) {
    if (m_data_sp->GetU8(&offset, (uint8_t *)sect64.sectname,
                         sizeof(sect64.sectname)) == nullptr)
      break;
    if (m_data_sp->GetU8(&offset, (uint8_t *)sect64.segname,
                         sizeof(sect64.segname)) == nullptr)
      break;
    sect64.addr = m_data_sp->GetAddress(&offset);
    sect64.size = m_data_sp->GetAddress(&offset);

    if (m_data_sp->GetU32(&offset, &sect64.offset, num_u32s) == nullptr)
      break;
...
void ObjectFileMachO::ProcessDysymtabCommand(
  ...
  m_data_sp->GetU32(&offset, &m_dysymtab.ilocalsym,
                    (sizeof(m_dysymtab) / sizeof(uint32_t)) - 2);
}
...
void ObjectFileMachO::CreateSections(SectionList &unified_section_list) {
  ...
  for (uint32_t i = 0; i < m_header.ncmds; ++i) {
    const lldb::offset_t load_cmd_offset = offset;
    if (m_data_sp->GetU32(&offset, &load_cmd, 2) == nullptr)
      break;
...
void ObjectFileMachO::ParseSymtab(Symtab &symtab) {
  ...
  for (i = 0; i < m_header.ncmds; ++i) {
    ...
    llvm::MachO::load_command lc;
    if (m_data_sp->GetU32(&offset, &lc, 2) == nullptr)
      break;
    // Watch for the symbol table load command
    switch (lc.cmd) {
    case LC_SYMTAB: {
      llvm::MachO::symtab_command lc_obj;
      if (m_data_sp->GetU32(&offset, &lc_obj.symoff, 4)) {
        lc_obj.cmd = lc.cmd;
    ...
    case LC_DYLD_INFO_ONLY: {
      ...
      if (m_data_sp->GetU32(&offset, &lc_obj.rebase_off, 10)) {
        lc_obj.cmd = lc.cmd;
        lc_obj.cmdsize = lc.cmdsize;
        dyld_info = lc_obj;
      }
    ...
    case LC_LOAD_UPWARD_DYLIB: {
      uint32_t name_offset = cmd_offset + m_data_sp->GetU32(&offset);
      const char *path = m_data_sp->PeekCStr(name_offset);
      if (path) {
    ...
    case LC_DYLD_EXPORTS_TRIE: {
      ...
      lc_obj.cmd = lc.cmd;
      lc_obj.cmdsize = lc.cmdsize;
      if (m_data_sp->GetU32(&offset, &lc_obj.dataoff, 2))
    ...
    case LC_UUID: {
      const uint8_t *uuid_bytes = m_data_sp->PeekData(offset, 16);
  ...
  const uint32_t addr_byte_size = m_data_sp->GetAddressByteSize();
  const ByteOrder byte_order = m_data_sp->GetByteOrder();
  ...
  if (process && m_header.filetype != llvm::MachO::MH_OBJECT &&
    ...
    nlist_data.SetData(*m_data_sp.get(), symtab_load_command.symoff,
                       nlist_data_byte_size);
    strtab_data.SetData(*m_data_sp.get(), symtab_load_command.stroff,
    ...
    if (dyld_info.export_size > 0) {
      dyld_trie_data.SetData(*m_data_sp.get(), dyld_info.export_off,
                             dyld_info.export_size);
    } else if (exports_trie_load_command.datasize > 0) {
      dyld_trie_data.SetData(*m_data_sp.get(),
                             exports_trie_load_command.dataoff,
                             exports_trie_load_command.datasize);
    }

    if (dysymtab.nindirectsyms != 0) {
      indirect_symbol_index_data.SetData(*m_data_sp.get(),
                                         dysymtab.indirectsymoff,
                                         dysymtab.nindirectsyms * 4);
    }
    if (function_starts_load_command.cmd) {
      function_starts_data.SetData(*m_data_sp.get(),
...
UUID ObjectFileMachO::GetUUID() {
  ...
  if (module_sp) {
    std::lock_guard<std::recursive_mutex> guard(module_sp->GetMutex());
    lldb::offset_t offset = MachHeaderSizeFromMagic(m_header.magic);
    return GetUUID(m_header, *m_data_sp.get(), offset);
  }
  return UUID();
}
    ...
    for (i = 0; i < m_header.ncmds; ++i) {
      ...
      case LC_THREAD: {
        while (offset < cmd_offset + load_cmd.cmdsize) {
          uint32_t flavor = m_data_sp->GetU32(&offset);
          uint32_t count = m_data_sp->GetU32(&offset);
          if (count == 0) {
          ...
          case llvm::MachO::CPU_TYPE_ARM:
                             ...
                             // from mach/arm/thread_status.h
            {
              offset += 60; // This is the offset of pc in the GPR thread state
                            // data structure.
              start_address = m_data_sp->GetU32(&offset);
              done = true;
            }
            break;
          ...
          case llvm::MachO::CPU_TYPE_ARM64_32:
            if (flavor == 6) // ARM_THREAD_STATE64 from mach/arm/thread_status.h
            {
              offset += 256; // This is the offset of pc in the GPR thread state
                             // data structure.
              start_address = m_data_sp->GetU64(&offset);
              done = true;
            }
            break;
      ...
      case LC_MAIN: {
        uint64_t entryoffset = m_data_sp->GetU64(&offset);
        SectionSP text_segment_sp =
            GetSectionList()->FindSectionByName(GetSegmentNameTEXT());
        if (text_segment_sp) {
          done = true;
...
uint32_t ObjectFileMachO::GetNumThreadContexts() {
  ...
  if (module_sp) {
    ...
    if (!m_thread_context_offsets_valid) {
      ...
      for (uint32_t i = 0; i < m_header.ncmds; ++i) {
        const uint32_t cmd_offset = offset;
        if (m_data_sp->GetU32(&offset, &thread_cmd, 2) == nullptr)
          break;
...
ObjectFileMachO::FindLC_NOTEByName(std::string name) {
  ...
  if (module_sp) {
    ...
    for (uint32_t i = 0; i < m_header.ncmds; ++i) {
      ...
      if (lc.cmd == LC_NOTE) {
        char data_owner[17];
        m_data_sp->CopyData(offset, 16, data_owner);
        data_owner[16] = '\0';
        offset += 16;

        if (name == data_owner) {
          offset_t payload_offset = m_data_sp->GetU64_unchecked(&offset);
          offset_t payload_size = m_data_sp->GetU64_unchecked(&offset);
...
std::string ObjectFileMachO::GetIdentifierString() {
  ...
  if (module_sp) {
    ...
    for (auto lc_note : lc_notes) {
      ...
      uint32_t version;
      if (m_data_sp->GetU32(&payload_offset, &version, 1) != nullptr) {
        if (version == 1) {
          uint32_t strsize = payload_size - sizeof(uint32_t);
          std::string result(strsize, '\0');
          m_data_sp->CopyData(payload_offset, strsize, result.data());
    ...
    for (uint32_t i = 0; i < m_header.ncmds; ++i) {
      ...
      if (m_data_sp->GetU32(&offset, &ident_command, 2) == nullptr)
        break;
      if (ident_command.cmd == LC_IDENT && ident_command.cmdsize != 0) {
        std::string result(ident_command.cmdsize, '\0');
        if (m_data_sp->CopyData(offset, ident_command.cmdsize, result.data()) ==
...
AddressableBits ObjectFileMachO::GetAddressableBits() {
  ...
  if (module_sp) {
    ...
    for (auto lc_note : lc_notes) {
      ...
      if (m_data_sp->GetU32(&payload_offset, &version, 1) != nullptr) {
        if (version == 3) {
          uint32_t num_addr_bits = m_data_sp->GetU32_unchecked(&payload_offset);
        ...
        if (version == 4) {
          uint32_t lo_addr_bits = m_data_sp->GetU32_unchecked(&payload_offset);
          uint32_t hi_addr_bits = m_data_sp->GetU32_unchecked(&payload_offset);
  ...
  if (module_sp) {
    ...
    for (auto lc_note : lc_notes) {
      ...
      if (m_data_sp->GetU32(&payload_offset, &version, 1) != nullptr &&
        ...
        if (!m_data_sp->GetU32(&payload_offset, &binspec_type, 1))
          return false;
        if (!m_data_sp->GetU64(&payload_offset, &value, 1))
          return false;
        uint64_t slide = LLDB_INVALID_ADDRESS;
        if (version > 1 && !m_data_sp->GetU64(&payload_offset, &slide, 1))
          return false;
        ...
        if (m_data_sp->CopyData(payload_offset, sizeof(uuid_t), raw_uuid) !=
            0) {
          ...
          if (!m_data_sp->GetU32(&payload_offset, &log2_pagesize, 1))
            return false;
          if (version > 1 && !m_data_sp->GetU32(&payload_offset, &platform, 1))
            return false;
          return true;
        }
      }
...
StructuredData::ObjectSP ObjectFileMachO::GetCorefileProcessMetadata() {
  ...
  if (m_data_sp->CopyData(payload_offset, strsize, buf.data()) != strsize) {
    LLDB_LOGF(log,
...
ObjectFileMachO::GetThreadContextAtIndex(uint32_t idx,
  ...
  if (module_sp) {
    ...
    if (thread_context_file_range) {
      ...
      DataExtractor data(*m_data_sp.get(),
    ...
    for (i = 0; i < m_header.ncmds; ++i) {
      ...
      if (load_cmd.cmd == LC_ID_DYLIB) {
        if (version_cmd == 0) {
          ...
          if (m_data_sp->GetU32(&offset, &load_cmd.dylib, 4) == nullptr)
            break;
...
ArchSpec ObjectFileMachO::GetArchitecture() {
  ...
  if (module_sp) {
    ...
    return GetArchitecture(module_sp, m_header, *m_data_sp.get(),
...
llvm::VersionTuple ObjectFileMachO::GetMinimumOSVersion() {
  if (!m_min_os_version)
    m_min_os_version = FindMinimumVersionInfo(
        *m_data_sp.get(), MachHeaderSizeFromMagic(m_header.magic),
        m_header.ncmds);
...
ObjectFileMachO::GetCorefileAllImageInfos() {
  ...
  for (auto lc_note : lc_notes) {
    ...
    uint32_t version = m_data_sp->GetU32(&payload_offset);
    if (version != 1) {
      return image_infos;
    }
    uint32_t imgcount = m_data_sp->GetU32(&payload_offset);
    uint64_t entries_fileoff = m_data_sp->GetU64(&payload_offset);
    ...
    for (uint32_t i = 0; i < imgcount; i++) {
      ...
      offset_t filepath_offset = m_data_sp->GetU64(&payload_offset);
      uuid_t uuid;
      memcpy(&uuid, m_data_sp->GetData(&payload_offset, sizeof(uuid_t)),
             sizeof(uuid_t));
      uint64_t load_address = m_data_sp->GetU64(&payload_offset);
      offset_t seg_addrs_offset = m_data_sp->GetU64(&payload_offset);
      uint32_t segment_count = m_data_sp->GetU32(&payload_offset);
      uint32_t currently_executing = m_data_sp->GetU32(&payload_offset);

      MachOCorefileImageEntry image_entry;
      image_entry.filename = (const char *)m_data_sp->GetCStr(&filepath_offset);
      ...
      for (uint32_t j = 0; j < segment_count; j++) {
        char segname[17];
        m_data_sp->CopyData(seg_vmaddrs_offset, 16, segname);
        segname[16] = '\0';
        seg_vmaddrs_offset += 16;
        uint64_t vmaddr = m_data_sp->GetU64(&seg_vmaddrs_offset);
  ...
  for (auto lc_note : lc_notes) {
    ...
    if (version == 1) {
      ...
      memcpy(&uuid, m_data_sp->GetData(&payload_offset, sizeof(uuid_t)),
             sizeof(uuid_t));
      uint64_t load_address = m_data_sp->GetU64(&payload_offset);
      uint64_t slide = m_data_sp->GetU64(&payload_offset);
      std::string filename = m_data_sp->GetCStr(&payload_offset);
