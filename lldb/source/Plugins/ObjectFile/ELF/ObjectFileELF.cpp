  ...
  while (true) {
    ...
    // Process CSR kalimba notes
      ...
      if (note.n_type == NT_FILE) {
        ...
        offset +=
            8 + 3 * 8 * count; // Skip page size and all start/end/file_ofs
...
unsigned ObjectFileELF::ParseSymbols(Symtab *symtab, user_id_t start_id,
  ...
  // On Android the oatdata and the oatexec symbols in system@framework@boot.oat
  // covers the full
  ...
  bool skip_oatdata_oatexec =
      m_file.GetFilename() == ConstString("system@framework@boot.oat");
  ...
  for (i = 0; i < num_symbols; ++i) {
    ...
    if (arch.IsValid()) {
       ...
       * MIPS).
       * This allows processer to switch between microMIPS and MIPS without any
       * need
    ...
    if (symbol_name && symbol_name[0] &&
        (symbol_name[0] == '_' && symbol_name[1] == 'Z')) {
      is_mangled = true;
    }
