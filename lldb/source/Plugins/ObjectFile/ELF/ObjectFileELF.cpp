...
bool ELFNote::Parse(const DataExtractor &data, lldb::offset_t *offset) {
  ...
  // The name field is required to be null-terminated, and n_namesz includes the
  ...
  // which write a note named "CORE" without a null terminator and n_namesz = 4.
  if (n_namesz == 4) {
    char buf[4];
  ...
  if (cstr == nullptr) {
    ...
    LLDB_LOGF(log, "Failed to parse note name lacking null terminator");
