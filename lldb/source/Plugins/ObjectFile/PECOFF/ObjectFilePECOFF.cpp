...
bool ObjectFilePECOFF::CreateBinary() {
  ...
  auto binary = llvm::object::createBinary(llvm::MemoryBufferRef(
      toStringRef(m_data_sp->GetData()), m_file.GetFilename().GetStringRef()));
  if (!binary) {
    LLDB_LOG_ERROR(log, binary.takeError(),
...
bool ObjectFilePECOFF::ParseHeader() {
  ...
  if (module_sp) {
    ...
    if (ParseDOSHeader(*m_data_sp.get(), m_dos_header)) {
      offset = m_dos_header.e_lfanew;
      uint32_t pe_signature = m_data_sp->GetU32(&offset);
      if (pe_signature != IMAGE_NT_SIGNATURE)
        return false;
      if (ParseCOFFHeader(*m_data_sp.get(), &offset, m_coff_header)) {
...
bool ObjectFilePECOFF::ParseCOFFOptionalHeader(lldb::offset_t *offset_ptr) {
  ...
  if (*offset_ptr < end_offset) {
    success = true;
    m_coff_header_opt.magic = m_data_sp->GetU16(offset_ptr);
    m_coff_header_opt.major_linker_version = m_data_sp->GetU8(offset_ptr);
    m_coff_header_opt.minor_linker_version = m_data_sp->GetU8(offset_ptr);
    m_coff_header_opt.code_size = m_data_sp->GetU32(offset_ptr);
    m_coff_header_opt.data_size = m_data_sp->GetU32(offset_ptr);
    m_coff_header_opt.bss_size = m_data_sp->GetU32(offset_ptr);
    m_coff_header_opt.entry = m_data_sp->GetU32(offset_ptr);
    m_coff_header_opt.code_offset = m_data_sp->GetU32(offset_ptr);
    ...
    if (*offset_ptr < end_offset) {
      if (m_coff_header_opt.magic == OPT_HEADER_MAGIC_PE32) {
        // PE32 only
        m_coff_header_opt.data_offset = m_data_sp->GetU32(offset_ptr);
      } else
        m_coff_header_opt.data_offset = 0;

      if (*offset_ptr < end_offset) {
        m_coff_header_opt.image_base =
            m_data_sp->GetMaxU64(offset_ptr, addr_byte_size);
        m_coff_header_opt.sect_alignment = m_data_sp->GetU32(offset_ptr);
        m_coff_header_opt.file_alignment = m_data_sp->GetU32(offset_ptr);
        m_coff_header_opt.major_os_system_version =
            m_data_sp->GetU16(offset_ptr);
        m_coff_header_opt.minor_os_system_version =
            m_data_sp->GetU16(offset_ptr);
        m_coff_header_opt.major_image_version = m_data_sp->GetU16(offset_ptr);
        m_coff_header_opt.minor_image_version = m_data_sp->GetU16(offset_ptr);
        m_coff_header_opt.major_subsystem_version =
            m_data_sp->GetU16(offset_ptr);
        m_coff_header_opt.minor_subsystem_version =
            m_data_sp->GetU16(offset_ptr);
        m_coff_header_opt.reserved1 = m_data_sp->GetU32(offset_ptr);
        m_coff_header_opt.image_size = m_data_sp->GetU32(offset_ptr);
        m_coff_header_opt.header_size = m_data_sp->GetU32(offset_ptr);
        m_coff_header_opt.checksum = m_data_sp->GetU32(offset_ptr);
        m_coff_header_opt.subsystem = m_data_sp->GetU16(offset_ptr);
        m_coff_header_opt.dll_flags = m_data_sp->GetU16(offset_ptr);
        ...
        m_coff_header_opt.heap_commit_size =
            m_data_sp->GetMaxU64(offset_ptr, addr_byte_size);
        m_coff_header_opt.loader_flags = m_data_sp->GetU32(offset_ptr);
        uint32_t num_data_dir_entries = m_data_sp->GetU32(offset_ptr);
        m_coff_header_opt.data_dirs.clear();
        m_coff_header_opt.data_dirs.resize(num_data_dir_entries);
        uint32_t i;
        for (i = 0; i < num_data_dir_entries; i++) {
          m_coff_header_opt.data_dirs[i].vmaddr = m_data_sp->GetU32(offset_ptr);
          m_coff_header_opt.data_dirs[i].vmsize = m_data_sp->GetU32(offset_ptr);
        }
...
DataExtractor ObjectFilePECOFF::ReadImageData(uint32_t offset, size_t size) {
  ...
  if (m_data_sp->ValidOffsetForDataOfSize(offset, size))
    return DataExtractor(*m_data_sp.get(), offset, size);
...
llvm::StringRef ObjectFilePECOFF::GetSectionName(const section_header_t &sect) {
  ...
  if (hdr_name.consume_front("/")) {
    ...
    if (const char *name = m_data_sp->GetCStr(&string_file_offset))
      return name;
    return "";
  }
  return hdr_name;
