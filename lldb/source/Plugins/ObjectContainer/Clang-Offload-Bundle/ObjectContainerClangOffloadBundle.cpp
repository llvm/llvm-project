...
static ArchSpec ParseArchFromBundleEntryID(llvm::StringRef id) {
  // Bundle entry IDs use the format: <offload-kind>-<target-id>
  // e.g. "hip-amdgcn-amd-amdhsa--gfx906",
  // "host-x86_64-unknown-linux-gnu"
  llvm::StringRef target = id.split('-').second;
  if (target.empty())
    return {};

  if (auto target_id = llvm::AMDGPU::TargetID::parseTargetIDString(target)) {
    llvm::Triple triple(target_id->getTargetTripleString());
    triple.setArch(llvm::Triple::amdgpu,
                   llvm::AMDGPU::getSubArch(target_id->getGPUKind()));
    return ArchSpec(triple);
  }

  return ArchSpec(target);
}
