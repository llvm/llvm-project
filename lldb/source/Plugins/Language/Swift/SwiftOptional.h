...
namespace lldb_private
{
    ...
    {
        // ExtractSomeIfAny() can return EITHER a child member or some other long-lived ValueObject
        // OR an entirely consed-up ValueObject
        // The lifetime of these two is radically different, and there is no trivial way to do the right
        // thing for both cases - except have a class that can wrap either and is safe to store and pass around
        class PointerOrSP
        {
        public:
            PointerOrSP  (std::nullptr_t) :
            m_raw_ptr(nullptr),
            m_shared_ptr(nullptr)
            {
            }
            
            PointerOrSP (ValueObject *valobj) :
            m_raw_ptr(valobj),
            m_shared_ptr(nullptr)
            {
            }
