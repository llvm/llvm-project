...
static void DumpPlaceholderArguments (lldb_private::Stream &wrapped_stream, llvm::ArrayRef<Expression::SwiftGenericInfo::Binding> generic_bindings)
{
    if (generic_bindings.empty())
        return;
...
static void DumpPlaceholdersIntoCall (lldb_private::Stream &wrapped_stream, llvm::ArrayRef<Expression::SwiftGenericInfo::Binding> generic_bindings)
{
    if (generic_bindings.empty())
        return;
...
bool
SwiftASTManipulator::VariableInfo::GetIsLet () const
{
    if (m_decl)
        return m_decl->isLet();
    else
...
@_silgen_name ("playground_print_T_stream") func $builtin_print<T, TargetStream : OutputStream> (object: T, inout _ stream: TargetStream) -> AnyObject
@_silgen_name ("playground_print_T_appendNewline") func $builtin_print<T> (object: T, appendNewline: Bool) -> AnyObject
@_silgen_name ("playground_print_T_stream_appendNewline") func $builtin_print<T, TargetStream : OutputStream> (object: T, inout _ stream: TargetStream, appendNewline: Bool) -> AnyObject
@_silgen_name ("playground_debugPrint_T") func $builtin_debugPrint<T> (object: T) -> AnyObject
@_silgen_name ("playground_debugPrint_T_stream") func $builtin_debugPrint<T, TargetStream : OutputStream> (object: T, inout _ stream: TargetStream) -> AnyObject
@_silgen_name ("playground_debugPrint_T_appendNewline") func $builtin_debugPrint<T> (object: T, appendNewline: Bool) -> AnyObject
@_silgen_name ("playground_debugPrint_T_stream_appendNewline") func $builtin_debugPrint<T, TargetStream : OutputStream> (object: T, inout _ stream: TargetStream, appendNewline: Bool) -> AnyObject
...
$builtin_logger_initialize()
    ...
    // Note: All the wrapper functions we make are marked with the @LLDBDebuggerFunction macro so that the compiler
    // can do whatever special treatment it need to do on them.  If you add new variants be sure to mark them this way.
    // Also, any function that might end up being in an extension of swift class needs to be marked final, since otherwise
...
void SwiftASTManipulatorBase::VariableInfo::Print(lldb_private::Stream &stream) const
{
    stream.Printf("[name=%s, type = ",
                  m_name.str().str().c_str());
...
void SwiftASTManipulatorBase::DoInitialization()
{
    ...
    {
    public:
        swift::FuncDecl *m_function_decl = nullptr;  // This is the function in which the expression code is inserted.
                                                     // It is always marked with the DebuggerFunction attribute.
        swift::ExtensionDecl *m_extension_decl = nullptr; // This is an optional extension holding the function
        swift::FuncDecl *m_wrapper_decl = nullptr;  // This is an optional wrapper function that calls m_function_decl.
        llvm::StringRef m_wrapper_func_prefix;   // This is the prefix name for the wrapper function.  One tricky bit
        ...
        virtual bool walkToDeclPre(swift::Decl *D)
        {
            ...
            if (func_decl)
            {
                if (func_decl->getAttrs().hasAttribute<swift::LLDBDebuggerFunctionAttr>())
                {
                    m_function_decl = func_decl;
                    
                    // Now walk back up the containing DeclContexts, and if we find an extension Decl, that's our extension:
                    swift::DeclContext *cur_ctx = m_function_decl->getDeclContext();
                    while (cur_ctx)
                    {
                        swift::ExtensionDecl *extension_decl = llvm::dyn_cast<swift::ExtensionDecl>(cur_ctx);
                        if (extension_decl)
                        {
                ...
                else if (func_decl->hasName() && func_decl->getName().str().startswith(m_wrapper_func_prefix))
                    m_wrapper_decl = func_decl;

                // There's nothing buried in a function that we need to find in this search.
                return false;
            }
            return true;
        }
...
swift::BraceStmt *
SwiftASTManipulatorBase::GetUserBody ()
{
    if (!IsValid())
        return nullptr;
...
void
SwiftASTManipulator::FindSpecialNames (llvm::SmallVectorImpl<swift::Identifier> &names, llvm::StringRef prefix)
{
    names.clear();
    ...
    public:
        ...
        SpecialNameFinder (NameVector &names, llvm::StringRef &prefix) : m_names(names), m_prefix(prefix)
        {
        }
...
// It records what it has done in a ResultLocationInfo, which gets pushed to the back of the ResultLocationInfo stack
// maintained by the SwiftASTManipulator, and returns the statement which effects the change.
//
// May return NULL if we can't make an appropriate variable assignment (e.g. for a bare "nil".)

swift::Stmt *
SwiftASTManipulator::ConvertExpressionToTmpReturnVarAccess (swift::Expr *expr,
                                                            const swift::SourceLoc &source_loc,
                                                            bool in_return,
                                                            swift::DeclContext *decl_context)
{
    // swift doesn't know how to infer the type of a variable by assignment to "nil".  So if the
    ...
    result_loc_info.tmp_var_decl = new (ast_context) swift::VarDecl(is_static,
                                                                    is_let,
                                                                    source_loc,
                                                                    name,
                                                                    swift::Type(),
    ...
    result_loc_info.tmp_var_decl->setAccessibility(swift::Accessibility::Internal);
    result_loc_info.tmp_var_decl->setSetterAccessibility(swift::Accessibility::Internal);

    swift::NamedPattern *var_pattern = new (ast_context) swift::NamedPattern (result_loc_info.tmp_var_decl, true);
    
    const swift::StaticSpellingKind static_spelling_kind = swift::StaticSpellingKind::KeywordStatic;
    result_loc_info.binding_decl = swift::PatternBindingDecl::create(ast_context,
                                                                     source_loc,
                                                                     static_spelling_kind,
                                                                     source_loc,
                                                                     var_pattern,
                                                                     expr,
                                                                     new_decl_context);
    ...
    if (in_return)
    {
        result_loc_info.return_stmt  = new (ast_context) swift::ReturnStmt(source_loc, nullptr);
    ...
    swift::SequenceExpr *zero_equals_one_expr = swift::SequenceExpr::create (ast_context, llvm::ArrayRef<swift::Expr *>(false_body));
    
    
    zero_equals_one_expr->setImplicit();
    swift::BraceStmt *body_stmt = swift::BraceStmt::create (ast_context,
                                                            source_loc,
                                                            llvm::ArrayRef<swift::ASTNode>(body),
                                                            source_loc,
                                                            true);
    
     // Default construct a label info that contains nothing for the while statement
    swift::LabeledStmtInfo label_info;
    
    swift::RepeatWhileStmt *assign_stmt =  new (ast_context) swift::RepeatWhileStmt (label_info,
                                                                             source_loc,
                                                                             zero_equals_one_expr,
                                                                             source_loc,
                                                                             body_stmt,
                                                                             true);
...
bool
SwiftASTManipulator::RewriteResult ()
{
    ...
    public:
        ...
        {
            // Don't step into function declarations, they may have returns, but we don't want
            // to instrument them.
        ...
        virtual std::pair<bool, swift::Expr *>
        walkToExprPre(swift::Expr *expr)
        {
            // Don't step into closure definitions, they may have returns, but we don't want
        ...
        virtual swift::Stmt *
        walkToStmtPost(swift::Stmt *stmt)
        {
            swift::ReturnStmt *possible_return = swift::dyn_cast<swift::ReturnStmt>(stmt);
            if (possible_return && possible_return->hasResult())
            {
                ...
                if (return_expr)
                {
                    ...
                    return_stmt = m_manipulator.ConvertExpressionToTmpReturnVarAccess (return_expr,
                                                                                       possible_return->getStartLoc(),
                                                                                       add_return,
                                                                                       m_decl_context);
                    if (return_stmt)
                        stmt = return_stmt;
    ...
    if (m_repl)
    {
        ...
        // First step, walk the function body converting returns to assignments to temp variables + return:

        for (swift::Decl *decl : m_source_file.Decls)
        {
            if (auto top_level_code_decl = llvm::dyn_cast<swift::TopLevelCodeDecl>(decl))
            {
        ...
        // Second step, fetch the last expression, and if it is non-null, set it to a temp result as well:
        ...
        if (!m_source_file.Decls.empty())
        {
            ...
            if (auto last_top_level_code_decl = llvm::dyn_cast<swift::TopLevelCodeDecl>(last_decl))
            {
                ...
                back_iterator = last_top_level_code_decl->getBody()->getElements().end() - 1;
                ...
                if (last_expr)
                {
                    swift::Stmt *temp_result_decl = ConvertExpressionToTmpReturnVarAccess (last_expr,
                                                                                           last_expr->getStartLoc(),
                                                                                           false,
    ...
    else
    {
        ...
        llvm::MutableArrayRef<swift::ASTNode> orig_elements = user_body->getElements();
        llvm::SmallVector<swift::Expr *, 1> return_values;
        
        // The function body is wrapped in an "if (true)" when constructed, so the function body can not be empty
        // or it was one we didn't make (or the optimizer is getting smart on us when it has no business doing that.)
...
namespace {
    ...
    public:
        void MakeOneAssignment(swift::VarDecl *var_decl, swift::Expr *initializer, swift::SourceLoc location)
        {
            ...
            swift::AssignExpr *assignment = new (m_ast_context) swift::AssignExpr(decl_ref,
                                                                                  location,
                                                                                  initializer,
                                                                                  implicit);
...
void
SwiftASTManipulator::MakeDeclarationsPublic ()
{
    if (!IsValid())
        return;
    ...
    class Publicist : public swift::ASTWalker
    {
        virtual bool walkToDeclPre(swift::Decl *decl)
        {
            if (swift::ValueDecl *value_decl = llvm::dyn_cast<swift::ValueDecl>(decl))
            {
                ...
                if (swift::AbstractStorageDecl *var_decl = llvm::dyn_cast<swift::AbstractStorageDecl>(decl))
...
static bool
hasInit (swift::PatternBindingDecl *pattern_binding)
{
...
static swift::Expr*
getFirstInit (swift::PatternBindingDecl *pattern_binding)
{
    for (unsigned i = 0, e = pattern_binding->getNumPatternEntries(); i != e; ++i)
        if (pattern_binding->getInit(i))
...
bool
SwiftASTManipulator::CheckPatternBindings ()
{
    for (swift::Decl *top_level_decl : m_source_file.Decls)
    {
        if (swift::TopLevelCodeDecl *top_level_code = llvm::dyn_cast<swift::TopLevelCodeDecl>(top_level_decl))
        {
            for (swift::ASTNode &node : top_level_code->getBody()->getElements())
            {
                if (swift::Decl *decl = node.dyn_cast<swift::Decl *>())
                {
                    if (swift::PatternBindingDecl *pattern_binding = llvm::dyn_cast<swift::PatternBindingDecl>(decl))
                    {
                        if (!(pattern_binding->isImplicit() || hasInit(pattern_binding)))
                        {
                            m_source_file.getASTContext().Diags.diagnose(pattern_binding->getStartLoc(), swift::diag::repl_must_be_initialized);
...
void
SwiftASTManipulator::FindVariableDeclarations (llvm::SmallVectorImpl<size_t> &found_declarations, bool repl)
{
    if (!IsValid())
        return;
    
    auto register_one_var = [this, &found_declarations](swift::VarDecl *var_decl) {
        ...
        persistent_info.m_type = CompilerType(&var_decl->getASTContext(), var_decl->getType().getPointer());
    ...
    else
    {
        ...
        llvm::SmallVector<swift::ASTNode, 3> elements(body_elements.begin(), body_elements.end());
        
        for (swift::ASTNode &element : elements)
        {
            if (swift::Decl *element_decl = element.dyn_cast<swift::Decl*>())
            {
                if (swift::VarDecl *var_decl = llvm::dyn_cast<swift::VarDecl>(element_decl))
                {
                    if (!var_decl->isDebuggerVar()) // skip bona fide external variables or variables we've already tagged
                    {
                        swift::Identifier name = var_decl->getName();
...
void
SwiftASTManipulator::FindNonVariableDeclarations(llvm::SmallVectorImpl<swift::ValueDecl *> &non_variables)
{
    if (!IsValid())
        return;
...
void
SwiftASTManipulator::InsertResult (swift::VarDecl *result_var,
                                   ...
                                   SwiftASTManipulator::ResultLocationInfo &result_info)
{
    ...
    // Finally, go reset the return expression to the new result variable for each of the return expressions.
    ...
    swift::BraceStmt *body_stmt = swift::BraceStmt::create (ast_context,
                                                            result_info.source_loc,
                                                            llvm::ArrayRef<swift::ASTNode>(new_body),
                                                            result_info.source_loc,
                                                            true);
...
void
SwiftASTManipulator::InsertError (swift::VarDecl *error_var,
                                   swift::Type &error_type)
{
    ...
    swift::AssignExpr *assign_expr = new (ast_context) swift::AssignExpr (lhs_expr, error_loc, init_expr, true);
    assign_expr->setType(ast_context.TheEmptyTupleType);
...
bool
SwiftASTManipulator::FixupResultAfterTypeChecking(Error &error)
{
    if (!IsValid())
    {
    ...
    for (size_t i = 0; i < num_results; i++)
    {
        ...
        if (the_decl->hasType())
        {
            ...
            else if (!its_type.getPointer()->isEqual(result_type))
            {
                ...
                error.SetErrorStringWithFormat("Type for %zuth return value is inconsistent, previous type: %s, current type: %s.",
                                               i,
                                               prev_type_name.c_str(),
                                               cur_type_name.c_str());
                return false;
            }
        }
        ...
        {
            error.SetErrorStringWithFormat("Type of %zuth return value could not be determined.", i);
            return false;
        }
    }
    ...
    CompilerType return_ast_type (&ast_context, result_type.getPointer());
    swift::Identifier result_var_name = ast_context.getIdentifier(GetResultName());
    SwiftASTManipulatorBase::VariableMetadataSP metadata_sp(new VariableMetadataResult());

    swift::VarDecl *result_var = AddExternalVariable (result_var_name, return_ast_type, metadata_sp);
    ...
    // Finally we have to do pretty much the same transformation on the error object.
    // First we need to find it:
    if (m_catch_stmt)
    {
        // Search for the error variable, so we can read it and its type,
        // then call InsertError to replace it with an assignment to the error variable.
        swift::BraceStmt *catch_body = llvm::dyn_cast<swift::BraceStmt>(m_catch_stmt->getBody());
        llvm::ArrayRef<swift::ASTNode> body_elements = catch_body->getElements();
        ...
        for (swift::ASTNode &element : elements)
        {
            if (swift::Decl *element_decl = element.dyn_cast<swift::Decl*>())
            {
                if (swift::VarDecl *var_decl = llvm::dyn_cast<swift::VarDecl>(element_decl))
                {
                    if (var_decl->hasType())
                    {
                        swift::Identifier error_var_name = ast_context.getIdentifier(GetErrorName());
                        if (error_var_name != var_decl->getName())
                            continue;

                        swift::Type error_type = var_decl->getType();
                        CompilerType error_ast_type (&ast_context, error_type.getPointer());
                        SwiftASTManipulatorBase::VariableMetadataSP error_metadata_sp(new VariableMetadataError());

                        swift::VarDecl *error_var = AddExternalVariable(error_var_name, error_ast_type, error_metadata_sp);
...
swift::VarDecl *
SwiftASTManipulator::AddExternalVariable (swift::Identifier name, CompilerType &type, VariableMetadataSP &metadata_sp)
{
    if (!IsValid())
        return nullptr;
...
GetPatternBindingForVarDecl (swift::VarDecl *var_decl, swift::DeclContext *containing_context)
{
    swift::ASTContext &ast_context = var_decl->getASTContext();
    
    const bool is_implicit = true;
    
    swift::NamedPattern *named_pattern = new (ast_context) swift::NamedPattern(var_decl,
                                                                               is_implicit);
    
    swift::TypedPattern *typed_pattern = new (ast_context) swift::TypedPattern(named_pattern,
                                                                               swift::TypeLoc::withoutLoc(var_decl->getType()));
    
    swift::PatternBindingDecl *pattern_binding = swift::PatternBindingDecl::create(ast_context,
                                                                                   swift::SourceLoc(),
...
bool
SwiftASTManipulator::AddExternalVariables (llvm::MutableArrayRef<VariableInfo> variables)
{
    if (!IsValid())
        return false;
    ...
    if (m_repl)
    {
        ...
        // If the type is an inout or lvalue type (happens if this is an argument) strip that part off:
        
        swift::VarDecl *redirected_var_decl = new (ast_context) swift::VarDecl(is_static,
                                                                               is_let,
                                                                               loc,
                                                                               name,
                                                                               var_type,
        ...
        swift::TopLevelCodeDecl *top_level_code = new (ast_context) swift::TopLevelCodeDecl(&m_source_file);
        
        swift::PatternBindingDecl *pattern_binding = GetPatternBindingForVarDecl(redirected_var_decl, top_level_code);
        
        swift::ASTNode elements[] = { pattern_binding };
        
        swift::BraceStmt *brace_stmt = swift::BraceStmt::create(ast_context,
                                                                loc,
                                                                elements,
                                                                loc,
                                                                true);
        ...
        m_source_file.Decls.insert(m_source_file.Decls.begin(), redirected_var_decl);
        ...
        if (log)
        {
            ...
            log->Printf("[SwiftASTManipulator::AddExternalVariables] Injected variable %s",
                        s.c_str());
        }
    ...
    else
    {
        ...
        llvm::SmallVector<swift::ASTNode, 3> elements(body_elements.begin(), body_elements.end());
        llvm::SmallVectorImpl<swift::ASTNode>::iterator element_iterator = elements.begin();
        const bool is_static = false;
        ...
        for (SwiftASTManipulator::VariableInfo &variable : variables)
        {
            ...
            // The access pattern for these types is the same as for the referent type, so it is fine to
            // just strip it off.
            // FIXME: If this is a weak managed type, then it could ostensibly go away out from under us,
            // but for now we aren't playing with reference counts to keep things alive in the expression parser.
            SwiftASTContext *swift_ast_ctx = llvm::dyn_cast_or_null<SwiftASTContext>(variable.m_type.GetTypeSystem());
            ...
            // One tricky bit here is that this var may be an argument to the function whose context we are
            // emulating, and that argument might be of "inout" type.  We need to strip the inout off the type
            // or the initial parse will fail.  Fortunately, the variable access goes the same regardless of whether
            // it is inout or not, so we don't have to do anything more to get this to work.
            swift::Type var_type = GetSwiftType(referent_type)->getLValueOrInOutObjectType();
            if (is_self)
            {
                // Another tricky bit is that the Metatype types we get have the "Representation" already attached (i.e.
                // "@thick", "@thin".)  But the representation is a SIL level thing, and if it is attached to types that
                // we hand the parser, it throws a verifier error & aborts.  So we strip it off here:
                swift::MetatypeType *metatype_type = llvm::dyn_cast<swift::MetatypeType>(var_type.getPointer());
                if (metatype_type)
                {
                    var_type = swift::Type(swift::MetatypeType::get(metatype_type->getInstanceType()));
                }
            }
            ...
            if (is_self)
            {
                ...
                llvm::ArrayRef<swift::ASTNode> wrapper_elements = wrapper_body->getElements();
                
                llvm::SmallVector<swift::ASTNode, 3> wrapper_elements_copy(wrapper_elements.begin(), wrapper_elements.end());
                llvm::SmallVectorImpl<swift::ASTNode>::iterator wrapper_element_iterator = wrapper_elements_copy.begin();
                
                wrapper_element_iterator = wrapper_elements_copy.insert(wrapper_element_iterator, swift::ASTNode(pattern_binding));
                wrapper_element_iterator = wrapper_elements_copy.insert(wrapper_element_iterator, swift::ASTNode(redirected_var_decl));
                
                m_wrapper_decl->setBody(swift::BraceStmt::create(ast_context, wrapper_body->getLBraceLoc(), ast_context.AllocateCopy(wrapper_elements_copy), wrapper_body->getRBraceLoc()));
            }
            else
            {
                element_iterator = elements.insert(element_iterator, swift::ASTNode(pattern_binding));
                element_iterator = elements.insert(element_iterator, swift::ASTNode(redirected_var_decl));
            }
        ...
        m_function_decl->setBody(swift::BraceStmt::create(ast_context, body->getLBraceLoc(), ast_context.AllocateCopy(elements), body->getRBraceLoc()));
    }
...
static void
AppendToCaptures(swift::ASTContext &ast_context, swift::FuncDecl *func_decl, swift::VarDecl *capture_decl)
{
    llvm::ArrayRef<swift::CapturedValue> old_captures = func_decl->getCaptureInfo().getCaptures();
    llvm::SmallVector<swift::CapturedValue, 2> captures(old_captures.begin(), old_captures.end());
...
bool
SwiftASTManipulator::FixCaptures ()
{
    if (!IsValid())
        return false;
    ...
    swift::VarDecl *function_arg_decl = FindArgInFunction(ast_context, m_function_decl);
    ...
    for (VariableInfo &variable : m_variables)
    {
        ...
        if (variable.m_decl->getStorageKind() != swift::AbstractStorageDecl::Computed)
            continue;
...
swift::ValueDecl *
SwiftASTManipulator::MakeGlobalTypealias(swift::Identifier name, CompilerType &type, bool make_private)
{
    if (!IsValid())
        return nullptr;
