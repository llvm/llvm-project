...
void
SwiftREPL::EnumerateSupportedLanguages (std::set<lldb::LanguageType> &languages)
{
...
{
    PluginManager::RegisterPlugin(ConstString("swift"), "The Swift REPL", &CreateInstance, &EnumerateSupportedLanguages);
}
...
SwiftREPL::SwiftREPL (Target &target) :
    REPL(LLVMCastKind::eKindSwift, target),
    m_swift_ast_sp ()
{
}
...
Error
SwiftREPL::DoInitialization()
{
    Error error;
    
    if (!m_compiler_options.empty())
    {
        (void)m_target.GetScratchSwiftASTContext(error, true, m_compiler_options.c_str());
    }
...
bool
SwiftREPL::SourceIsComplete (const std::string &source)
{
    std::unique_ptr<llvm::MemoryBuffer> source_buffer_ap (llvm::MemoryBuffer::getMemBuffer (source));
    swift::ide::SourceCompleteResult result = swift::ide::isSourceInputComplete (std::move(source_buffer_ap));
...
lldb::offset_t
SwiftREPL::GetDesiredIndentation (const StringList &lines,
                                  int cursor_position,
                                  int tab_size)
{
    ...
    std::unique_ptr<llvm::MemoryBuffer> source_buffer_ap (llvm::MemoryBuffer::getMemBuffer (source_string));
    swift::ide::SourceCompleteResult result = swift::ide::isSourceInputComplete (std::move(source_buffer_ap));
    
    int desired_indent = (result.IndentLevel * tab_size) + result.IndentPrefix.length();
    
    const std::string &last_line = lines[lines.GetSize() - 1];
    
    // Unindent for an initial closed brace on a line break, or when the brace was just typed
    if (cursor_position == 0 || last_line[cursor_position - 1] == '}') {
        ...
        if (last_line.length() > actual_indent && last_line[actual_indent] == '}')
        {
            ...
            if (desired_indent < 0) desired_indent = 0;
            return desired_indent;
        }
    }
    
    // Unindent for a case clause on a line break, or when the colon was just typed
    if (cursor_position == 0 || last_line[cursor_position - 1] == ':')
    {
        ...
        if (colon_pos != std::string::npos)
        {
            ...
            if (!line.empty())
            {
                ...
                if (GetIdentifier (line, identifier))
                {
                    ...
                    // and if it isn't we have "case" followed by a space, then we handle this as a cast statement
                    bool outdent = false;
                    if (line.empty())
                    ...
                    else
                    {
                        ...
                        // Check for any identifier followed by an optional paren expression and a colon
