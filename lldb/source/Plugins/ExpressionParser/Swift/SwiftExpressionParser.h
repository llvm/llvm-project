...
//----------------------------------------------------------------------
/// @class SwiftExpressionParser SwiftExpressionParser.h "lldb/Expression/SwiftExpressionParser.h"
...
private:
    bool
    PerformAutoImport (swift::SourceFile &source_file, bool user_imports, Error &error);
    
    Expression                          &m_expr;                ///< The expression to be parsed
    std::string                         m_triple;               ///< The triple to use when compiling
    std::unique_ptr <llvm::LLVMContext> m_llvm_context;         ///< The context to use for IR generation
    std::unique_ptr <llvm::Module>      m_module;               ///< The module to build IR into
    lldb::IRExecutionUnitSP             m_execution_unit_sp;    ///< The container for the IR, to be JIT-compiled or interpreted
    SwiftASTContext                    *m_swift_ast_context;    ///< The AST context to build the expression into
    SymbolContext                       m_sc;                   ///< The symbol context to use when parsing
    lldb::StackFrameWP                  m_stack_frame_wp;       ///< The stack frame to use (if possible) when determining dynamic types.
