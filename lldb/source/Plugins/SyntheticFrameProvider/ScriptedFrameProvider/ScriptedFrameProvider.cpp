...
ScriptedFrameProvider::GetStackFrames(StackFrameListSP real_frames) {
  ...
  // Call get_frame_at_index for each index until we get None/null
  for (uint32_t idx = 0; idx < UINT32_MAX; idx++) {
    StructuredData::ObjectSP obj_sp =
        m_interface_sp->GetFrameAtIndex(real_frames, idx);

    // None/null means no more frames
    if (!obj_sp || !obj_sp->IsValid())
      break;

    StackFrameSP synth_frame_sp = nullptr;

    // Check if it's an integer (real frame index)
    if (auto *int_obj = obj_sp->GetAsUnsignedInteger()) {
      ...
      // It's a dictionary describing a frame
      auto frame_from_dict_or_err = create_frame_from_dict(dict, idx);
      if (!frame_from_dict_or_err) {
        return llvm::createStringError(
            llvm::Twine("Couldn't create frame from dictionary at index " +
                        llvm::Twine(idx) + ": " +
      ...
      auto frame_from_script_obj_or_err =
          create_frame_from_script_object(obj_sp);
    ...
    if (!scripted_frames->SetFrameAtIndex(idx, synth_frame_sp))
      return llvm::createStringError(
