...
// The `elem_size` paramter represents the size of a single element, including
// padding.
...
bool RenderScriptRuntime::JITAllocationSize(AllocationDetails *allocation,
  ...
  if (allocation->element.children.size() > 0) {
    ...
    if (log)
      log->Printf("%s - infered size of struct allocation %" PRIu32 ".",
...
public:
  ...
  public:
    ...
    void OptionParsingStarting() override { m_outfile.Clear(); }
...
public:
  ...
  public:
    ...
    void OptionParsingStarting() override { m_id = 0; }
