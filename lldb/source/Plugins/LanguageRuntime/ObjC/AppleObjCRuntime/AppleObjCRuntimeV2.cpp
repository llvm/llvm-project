...
// We'll substitute in class_getName or objc_debug_class_getNameRaw 
...
AppleObjCRuntimeV2::UpdateISAToDescriptorMapSharedCache() {
  ...
  if (!m_get_shared_cache_class_info_code) {
    ...
    // If the inferior objc.dylib has the objc_debug_class_getNameRaw 
    // function, use that in our jitted expression.  Else fall back to 
    // the old class_getName.
    ...
    // Substitute in the correct class_getName / objc_debug_class_getNameRaw 
    // function name, concatenate the two parts of our expression text.  
    // The format string has two %s's, so provide the name twice.
