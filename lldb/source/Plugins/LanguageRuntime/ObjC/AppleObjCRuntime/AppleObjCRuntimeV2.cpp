...
AppleObjCRuntimeV2::AppleObjCRuntimeV2(Process *process,
    ...
    : AppleObjCRuntime(process), m_get_class_info_code(),
      ...
      m_decl_vendor_ap(), m_isa_hash_table_ptr(LLDB_INVALID_ADDRESS),
      m_hash_signature(), m_has_object_getClass(false),
      ...
      m_encoding_to_type_sp(), m_warn_if_no_classes_cached{false, false},
...
public:
  ...
  public:
    CommandOptions(CommandInterpreter &interpreter)
        : Options(interpreter), m_verbose(false, false) {}
    ...
    void OptionParsingStarting() override { m_verbose.Clear(); }
        ...
        if (iterator->second) {
          ...
          if (m_options.m_verbose) {
            ...
            iterator->second->Describe(
                nullptr,
                [objc_runtime, &out](const char *name,
...
AppleObjCRuntimeV2::UpdateISAToDescriptorMapSharedCache() {
  ...
  if (num_classes == 0) {
    if (log)
      log->Printf(
          "No dynamic classes found in gdb_objc_realized_classes_addr.");
    return DescriptorMapUpdateResult::Fail();
  }
    ...
    if (results == eExpressionCompleted) {
      ...
      if (num_class_infos > 0) {
        ...
        if (process->ReadMemory(class_infos_addr, buffer.GetBytes(),
          ...
          any_found =
              (ParseClassInfoArray(class_infos_data, num_class_infos) > 0);
        }
      } else {
        success = true;
      }
...
bool AppleObjCRuntimeV2::UpdateISAToDescriptorMapFromMemory(
    RemoteNXMapTable &hash_table, uint32_t &discovered_classes_count) {
  Log *log(GetLogIfAllCategoriesSet(LIBLLDB_LOG_PROCESS));
...
void AppleObjCRuntimeV2::WarnIfNoClassesFound(bool globally) {
  ...
  if (m_process && m_process->GetTarget().GetPlatform() &&
      m_process->GetTarget()
          .GetPlatform()
          ->GetPluginName()
          .GetStringRef()
  ...
  if (debugger_async_out_sp) {
    if (globally) {
      debugger_async_out_sp->PutCString("warning: could not load any "
                                        "Objective-C class information. This "
                                        "will significantly reduce the quality "
                                        "of type information available.\n");
      ...
      if (DoesProcessHaveSharedCache(*m_process))
        debugger_async_out_sp->PutCString(
            "warning: could not load any Objective-C class information from "
            "the dyld shared cache. This will significantly reduce the quality "
            "of type information available.\n");
