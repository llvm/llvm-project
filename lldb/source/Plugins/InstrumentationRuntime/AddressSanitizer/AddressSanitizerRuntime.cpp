...
AddressSanitizerRuntime::AddressSanitizerRuntime(const ProcessSP &process_sp)
    : m_is_active(false), m_runtime_module(), m_process_wp(),
      m_breakpoint_id(0) {
  if (process_sp)
...
void AddressSanitizerRuntime::ModulesDidLoad(
  ...
  for (size_t i = 0; i < num_modules; ++i) {
    ...
    static RegularExpression g_asan_runtime_regex(
        "libclang_rt.asan_(.*)_dynamic\\.dylib");
    if (g_asan_runtime_regex.Execute(file_spec.GetFilename().GetCString()) ||
        module_pointer->IsExecutable()) {
...
bool AddressSanitizerRuntime::IsActive() { return m_is_active; }
...
void AddressSanitizerRuntime::Activate() {
  ...
  const Symbol *symbol = m_runtime_module->FindFirstSymbolWithNameAndType(
