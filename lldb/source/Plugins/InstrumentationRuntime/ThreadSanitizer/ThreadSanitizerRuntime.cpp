...
ThreadSanitizerRuntime::ThreadSanitizerRuntime(const ProcessSP &process_sp)
    : m_is_active(false), m_runtime_module_wp(), m_process_wp(),
      m_breakpoint_id(0) {
  if (process_sp)
...
static bool ModuleContainsTSanRuntime(ModuleSP module_sp) {
...
void ThreadSanitizerRuntime::ModulesDidLoad(
  ...
  module_list.ForEach([this](const lldb::ModuleSP module_sp) -> bool {
    ...
    if (module_sp->IsExecutable() ||
        module_basename.startswith("libclang_rt.tsan_")) {
...
bool ThreadSanitizerRuntime::IsActive() { return m_is_active; }
...
static StructuredData::Array *ConvertToStructuredArray(
    ValueObjectSP return_value_sp, std::string items_name,
...
static std::string GenerateThreadName(std::string path,
...
static void AddThreadsForPath(std::string path, ThreadCollectionSP threads,
                              ProcessSP process_sp,
