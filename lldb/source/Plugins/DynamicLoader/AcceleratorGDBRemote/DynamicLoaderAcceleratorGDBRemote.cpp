...
DynamicLoaderAcceleratorGDBRemote::CreateInstance(Process *process,
                                                  bool force) {
  // Only create an instance when asked for by name; this loader is selected via
  // the target's jLLDBSettings dyld_plugin_name, never auto-selected.
  if (force)
...
bool DynamicLoaderAcceleratorGDBRemote::LoadModulesFromGDBServer(bool full) {
  ...
  std::optional<AcceleratorDynamicLoaderResponse> response =
      gpu_process->GetGDBRemote().GetAcceleratorDynamicLoaderLibraryInfos(args);
  if (!response) {
  ...
  for (const AcceleratorDynamicLoaderLibraryInfo &info :
    ...
    // The object file is either a whole file (pathname), or a slice of a
    // containing file located by file_offset/file_size (e.g. a library embedded
    // in the executable via an llvm-objcopy section).
    ...
    if (info.load_address) {
      // Slide the whole object file to this base load address.
      module_sp->SetLoadAddress(target, *info.load_address,
                                /*value_is_offset=*/true, changed);
    } else if (!info.loaded_sections.empty()) {
      // Load individual sections at their own addresses. Each entry names a
      // section, or a path of nested section names to descend.
      ...
      // Load at the file addresses with no slide.
...
DynamicLoaderAcceleratorGDBRemote::GetPluginDescriptionStatic() {
  return "Dynamic loader for accelerator (GPU) targets. Gets library load and "
         "unload info from an lldb-server accelerator plugin over gdb-remote "
         "rather than from a rendezvous breakpoint.";
}
