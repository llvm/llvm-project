...
void DynamicLoaderDarwin::UpdateDYLDImageInfoFromNewImageInfos(
    ImageInfo::collection &image_infos) {
...
void DynamicLoaderDarwin::AddExecutableModuleIfInImageInfos(
    ImageInfo::collection &image_infos) {
  const size_t image_infos_size = image_infos.size();
  for (size_t i = 0; i < image_infos_size; i++) {
    if (image_infos[i].header.filetype == llvm::MachO::MH_EXECUTE) {
      ...
      ModuleSP exe_module_sp(
          FindTargetModuleForImageInfo(image_infos[i], can_create, NULL));
      ...
      if (exe_module_sp) {
        ...
        if (exe_module_sp.get() != target.GetExecutableModulePointer()) {
          ...
          // and find out about all images that are loaded. Also when setting
          // the
          ...
          // dyld module, make it into a shared pointer, then add the
          // executable,
