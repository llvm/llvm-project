                                            ...
                                            CompilerType &return_compiler_type) const
{
    ...
    else if ((type_flags & eTypeIsPointer) || (type_flags & eTypeInstanceIsPointer))
    {
...
static bool
ExtractBytesFromRegisters (ExecutionContext& exe_ctx,
                           CompilerType &clang_type,
                           const DataExtractor &rax_data,
                           const DataExtractor &rdx_data,
                           const DataExtractor &rcx_data,
                           const DataExtractor &xmm0_data,
                           const DataExtractor &xmm1_data,
                           const DataExtractor &xmm2_data,
                           ...
                           bool &is_memory)
{
    ...
    for (uint32_t idx = 0; idx < num_children; ++idx)
    {
        ...
        const bool is_swift_enum = (field_type_flags & eTypeIsEnumeration) && (field_type_flags & eTypeIsSwift);
        // HACK: Swift enums are really hard to figure out the return value ABI for.
        // We really need compiler assistance but we don't have it. We need Swift.Dictionary
        // objects to work though for return values, so we check for that simple case here
        // and just assume that of the value is less than a pointer size that it can be treated
        // as a integer type...
        ...
        if (field_type_flags & eTypeIsInteger        ||
            ...
            (field_type_flags & eTypeInstanceIsPointer && child_is_base_class == false) ||
            ...
            else if (field_bit_width == 32)
            {
                ...
                if (field_byte_offset % 8 == 0)
                {
                    ...
                    else
                    {
                        ...
                        CompilerType next_field_clang_type = clang_type.GetFieldAtIndex (idx + 1,
                                                                                         name,
                ...
                else if (field_byte_offset % 4 == 0)
                {
                    ...
                    else
                    {
                        ...
                        CompilerType prev_field_clang_type = clang_type.GetFieldAtIndex (idx - 1,
                                                                                         name,
        ...
        {
            // Swift enums are unions and we can't just iterate through the fields. We need
            // help from the compiler to do enums correctly.
            if (is_swift_enum)
                return false;

            if (ExtractBytesFromRegisters (exe_ctx,
                                           field_clang_type,
                                           rax_data,
                                           rdx_data,
                                           rcx_data,
                                           ...
                                           byte_order,
                                           data_sp,
                                           data_byte_offset + child_byte_offset,
                                           integer_bytes,
                                           fp_bytes,
                                           is_memory) == false)
            {
        ...
        if (copy_from_extractor && copy_from_offset + field_byte_width <= copy_from_extractor->GetByteSize())
        {
...
ABISysV_x86_64::GetReturnValueObjectImpl (Thread &thread, CompilerType &return_compiler_type) const
{
    ...
    if (return_compiler_type.IsAggregateType())
    {
        ...
        bool is_memory = true;
        const bool is_swift_type = (return_compiler_type.GetTypeInfo() & eTypeIsSwift);
        ...
        if (bit_width <= max_register_value_bit_width)
        {
            ...
            DataBufferSP data_sp (new DataBufferHeap(max_register_value_bit_width/8, 0));
            ...
            const RegisterInfo *rcx_info = reg_ctx_sp->GetRegisterInfoByName("rcx", 0);
            ...
            const RegisterInfo *xmm2_info = reg_ctx_sp->GetRegisterInfoByName("xmm2", 0);
            ...
            uint32_t data_byte_offset = 0; // The offset into "data_sp" where to place the next chunk of data
