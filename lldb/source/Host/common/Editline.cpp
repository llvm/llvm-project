  ...
  if (!IsInputPending(m_input_file)) {
    ...
    if (m_current_line_index == m_input_lines.size() - 1 &&
      ...
      if (m_is_input_complete_callback) {
        ...
        end_of_input = m_is_input_complete_callback(
            this, lines, m_is_input_complete_callback_baton);

        // The completion test is allowed to change the input lines when
        // complete
        if (end_of_input) {
          m_input_lines.clear();
...
void Editline::ConfigureEditor(bool multiline) {
  ...
  // clients using
  // custom .editrc files (largely becuase libedit has a bad bug where if you
  // have a bind command
