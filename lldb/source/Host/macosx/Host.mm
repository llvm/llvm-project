...
static Mutex &GetCrashReporterMutex() {
...
Error Host::LaunchProcess(ProcessLaunchInfo &launch_info) {
  ...
  if (pid != LLDB_INVALID_PROCESS_ID) {
    ...
    if (!launch_info.MonitorProcess()) {
      ...
      StartMonitoringChildProcess(callback, NULL, pid, monitor_signals);
    }
  } else {
...
Host::StartMonitoringChildProcess(Host::MonitorChildProcessCallback callback,
  ...
  if (log)
    log->Printf("Host::StartMonitoringChildProcess (callback=%p, baton=%p, "
                "pid=%i, monitor_signals=%i) source = %p\n",
                callback, callback_baton, (int)pid, monitor_signals, source);
