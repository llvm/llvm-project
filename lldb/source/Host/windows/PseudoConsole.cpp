...
llvm::Error PseudoConsole::DrainInitSequences() {
  ...
  static constexpr char kDrainMarker[] = "LLDB_CONPTY_DRAIN_DONE";
  static constexpr wchar_t kDrainMarkerW[] = L"LLDB_CONPTY_DRAIN_DONE";
  std::wstring cmdline_str =
      std::wstring(comspec) + L" /c echo " + kDrainMarkerW;
  ...
  while (GetOverlappedResult(m_conpty_output, &ov, &read, /*bWait=*/TRUE) &&
    ...
    if (accumulated.find(kDrainMarker) != std::string::npos)
      break;
