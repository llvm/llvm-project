...
CommandObjectDisassemble::CommandOptions::CommandOptions(
    ...
    : Options(interpreter), num_lines_context(0), num_instructions(0),
      func_name(), current_function(false), start_addr(), end_addr(),
      at_pc(false), frame_line(false), plugin_name(), flavor_string(), arch(),
...
Error CommandObjectDisassemble::CommandOptions::SetOptionValue(
  ...
  case 's': {
    ...
    start_addr = Args::StringToAddress(&exe_ctx, option_arg,
  ...
  case 'e': {
    ExecutionContext exe_ctx(m_interpreter.GetExecutionContext());
    end_addr = Args::StringToAddress(&exe_ctx, option_arg, LLDB_INVALID_ADDRESS,
                                     &error);
  ...
  case 'a': {
    ...
    symbol_containing_addr = Args::StringToAddress(
        &exe_ctx, option_arg, LLDB_INVALID_ADDRESS, &error);
