...
protected:
  bool DoExecute(Args &command, CommandReturnObject &result) override {
    ...
    if (argc == 1 || core_file || remote_file) {
      ...
      Timer scoped_timer(__PRETTY_FUNCTION__, "(lldb) target create '%s'",
                         file_path);
      FileSpec file_spec;
...
public:
  ...
  int HandleArgumentCompletion(Args &input, int &cursor_index,
    ...
    CommandCompletions::InvokeCommonCompletionCallbacks(
        m_interpreter, CommandCompletions::eModuleCompletion,
        completion_str.c_str(), match_start_point, max_return_elements, nullptr,
        word_complete, matches);
    return matches.GetSize();
  }
...
public:
  ...
  int HandleArgumentCompletion(Args &input, int &cursor_index,
    ...
    CommandCompletions::InvokeCommonCompletionCallbacks(
        m_interpreter, CommandCompletions::eSourceFileCompletion,
        completion_str.c_str(), match_start_point, max_return_elements, nullptr,
        word_complete, matches);
    return matches.GetSize();
  }
...
public:
  ...
  public:
    CommandOptions(CommandInterpreter &interpreter)
        : Options(interpreter), m_sort_order(eSortOrderNone),
...
public:
  ...
  public:
    CommandOptions(CommandInterpreter &interpreter)
        : Options(interpreter), m_format_array(),
    ...
    Error SetOptionValue(uint32_t option_idx, const char *option_arg) override {
      ...
      if (short_option == 'g') {
        ...
        m_module_addr = Args::StringToAddress(&exe_ctx, option_arg,
...
protected:
  bool DoExecute(Args &command, CommandReturnObject &result) override {
    ...
    if (target == nullptr && !use_global_module_list) {
      ...
      Mutex::Locker locker; // This locker will be locked on the mutex in
                            // module_list_ptr if it is non-nullptr.
...
public:
  ...
  public:
    CommandOptions(CommandInterpreter &interpreter)
        : Options(interpreter), m_type(eLookupTypeInvalid), m_str(),
...
public:
  ...
  public:
    CommandOptions(CommandInterpreter &interpreter)
        : Options(interpreter), m_line_start(0), m_line_end(UINT_MAX),
