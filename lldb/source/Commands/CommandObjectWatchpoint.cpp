...
public:
  ...
  public:
    ...
    void OptionParsingStarting() override { m_ignore_count = 0; }
...
public:
  ...
  public:
    CommandOptions(CommandInterpreter &interpreter)
        : Options(interpreter), m_condition(), m_condition_passed(false) {}
...
public:
  CommandObjectWatchpointSetVariable(CommandInterpreter &interpreter)
      : CommandObjectParsed(
        ...
        m_option_group(interpreter), m_option_watchpoint() {
    SetHelpLong(
        R"(
Examples:
...
protected:
  bool DoExecute(const char *raw_command,
    ...
    m_option_group.NotifyOptionParsingStarting(); // This is a raw command, so
                                                  // notify the option group
