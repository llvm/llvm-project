...
public:
  CommandObjectTypeFormatAdd(CommandInterpreter &interpreter)
      : CommandObjectParsed(interpreter, "type format add",
        ...
        m_option_group(interpreter), m_format_options(eFormatInvalid),
        m_command_options() {
...
public:
  CommandObjectTypeFormatterDelete(CommandInterpreter &interpreter,
      ...
      : CommandObjectParsed(interpreter, name, help, nullptr),
        m_options(interpreter), m_formatter_kind_mask(formatter_kind_mask) {
    CommandArgumentEntry type_arg;
    CommandArgumentData type_style_arg;
...
private:
  ...
  public:
    ...
    void OptionParsingStarting() override { m_delete_all = false; }
...
class CommandObjectTypeFormatterList : public CommandObjectParsed {
  ...
  public:
    CommandOptions(CommandInterpreter &interpreter)
        : Options(interpreter), m_category_regex("", ""),
...
class CommandObjectTypeCategoryDefine : public CommandObjectParsed {
  ...
  public:
    CommandOptions(CommandInterpreter &interpreter)
        : Options(interpreter), m_define_enabled(false, false),
...
public:
  CommandObjectTypeLookup(CommandInterpreter &interpreter)
      : CommandObjectRaw(interpreter, "type lookup",
        ...
        m_option_group(interpreter), m_command_options() {
    m_option_group.Append(&m_command_options);
    m_option_group.Finalize();
  }
  ...
  bool DoExecute(const char *raw_command_line,
    ...
    TargetSP target_sp(
        GetCommandInterpreter().GetDebugger().GetSelectedTarget());
    ...
    if (StackFrame *frame = m_exe_ctx.GetFramePtr()) {
      ...
      if (lang != eLanguageTypeUnknown) {
        std::sort(
            ...
            [lang](Language *lang1, Language *lang2) -> bool {
              if (!lang1 || !lang2)
                return false;
