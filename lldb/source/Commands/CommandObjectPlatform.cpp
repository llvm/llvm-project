...
public:
  ...
  int HandleCompletion(Args &input, int &cursor_index,
    ...
    CommandCompletions::PlatformPluginNames(
        m_interpreter, completion_str.c_str(), match_start_point,
...
protected:
  ...
  public:
    CommandOptions(CommandInterpreter &interpreter)
        : Options(interpreter), match_info(), show_args(false), verbose(false) {
    ...
    Error SetOptionValue(uint32_t option_idx, const char *option_arg) override {
      ...
      case 'a':
        match_info.GetProcessInfo().GetArchitecture().SetTriple(
            option_arg, m_interpreter.GetDebugger()
                            .GetPlatformList()
                            .GetSelectedPlatform()
                            .get());
        break;
...
public:
  ...
  public:
    CommandOptions(CommandInterpreter &interpreter)
        : Options(interpreter), timeout(10) {}
    ...
    Error SetOptionValue(uint32_t option_idx,
