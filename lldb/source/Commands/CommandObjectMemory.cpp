...
public:
  CommandObjectMemoryRead(CommandInterpreter &interpreter)
      : CommandObjectParsed(
        ...
        m_format_options(eFormatBytesWithASCII, 1, 8), m_memory_options(),
        m_outfile_options(), m_varobj_options(),
...
public:
  ...
  CommandObjectMemoryFind(CommandInterpreter &interpreter)
      : CommandObjectParsed(
        ...
        m_option_group(interpreter), m_memory_options() {
    CommandArgumentEntry arg1;
    CommandArgumentEntry arg2;
...
protected:
  bool DoExecute(Args &command, CommandReturnObject &result) override {
    ...
    while (count) {
      found_location = Search(found_location, high_addr, buffer.GetBytes(),
                              buffer.GetByteSize());
  ...
  lldb::addr_t Search(lldb::addr_t low, lldb::addr_t high, uint8_t *buffer,
                      size_t buffer_size) {
    Process *process = m_exe_ctx.GetProcessPtr();
...
public:
  ...
  CommandObjectMemoryWrite(CommandInterpreter &interpreter)
      : CommandObjectParsed(
        ...
        m_option_group(interpreter),
        m_format_options(eFormatBytes, 1, UINT64_MAX), m_memory_options() {
    CommandArgumentEntry arg1;
    CommandArgumentEntry arg2;
    CommandArgumentData addr_arg;
    CommandArgumentData value_arg;
