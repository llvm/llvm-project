...
bool BreakpointLocation::ConditionSaysStop(ExecutionContext &exe_ctx,
  ...
  if (condition.GetHash() != m_condition_hash && exe_ctx.HasFrameScope() &&
    ...
    if (lex_or_err) {
      ...
      if (tree_or_error) {
        ...
        LLDB_LOGF(log, "DIL successfully parsed the condition: %s.",
                  condition.GetText().str().c_str());
      } else {
        m_dil_expr_tree.reset();
        error = Status::FromError(tree_or_error.takeError());
        LLDB_LOGF(log, "Parsing condition with DIL failed:\n%s.",
                  error.AsCString());
      }
    } else {
      m_dil_expr_tree.reset();
      error = Status::FromError(lex_or_err.takeError());
      LLDB_LOGF(log, "Lexing condition with DIL failed:\n%s.",
                error.AsCString());
    }
  }
  ...
  if (m_dil_expr_tree) {
    ...
    auto valobj_or_error = interpreter.Evaluate(*m_dil_expr_tree);
    if (valobj_or_error) {
      ...
      if (value_sp && value_sp->GetError().Success()) {
        ...
        if (bool_result) {
          LLDB_LOGF(log,
                    "Condition successfully evaluated by DIL, result is %s.\n",
    ...
    // Evaluation failed.
    LLDB_LOGF(log, "Evaluating condition with DIL failed:\n%s.",
              error.AsCString());
    m_dil_expr_tree.reset();
    m_condition_hash = 0;
  }
