...
Error OptionGroupFormat::SetOptionValue(CommandInterpreter &interpreter,
                                        uint32_t option_idx,
  ...
  case 'G': {
    ...
    while (ParserGDBFormatLetter(interpreter, gdb_format_cstr[0], format,
                                 byte_size)) {
      ++gdb_format_cstr;
    }
...
bool OptionGroupFormat::ParserGDBFormatLetter(CommandInterpreter &interpreter,
                                              char format_letter,
                                              Format &format,
                                              uint32_t &byte_size) {
...
void OptionGroupFormat::OptionParsingStarting(CommandInterpreter &interpreter) {
  m_format.Clear();
  m_byte_size.Clear();
  m_count.Clear();
