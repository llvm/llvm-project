...
Mutex *Module::GetAllocationModuleCollectionMutex() {
  ...
  if (g_module_collection_mutex == nullptr)
    g_module_collection_mutex =
        new Mutex(Mutex::eMutexTypeRecursive); // NOTE: known leak
...
Module::Module(const ModuleSpec &module_spec)
    : m_mutex(Mutex::eMutexTypeRecursive), m_mod_time(), m_arch(), m_uuid(),
      m_file(), m_platform_file(), m_remote_install_file(), m_symfile_spec(),
      m_object_name(), m_object_offset(), m_object_mod_time(), m_objfile_sp(),
...
Module::Module(const FileSpec &file_spec, const ArchSpec &arch,
    ...
    : m_mutex(Mutex::eMutexTypeRecursive),
      m_mod_time(file_spec.GetModificationTime()), m_arch(arch), m_uuid(),
      m_file(file_spec), m_platform_file(), m_remote_install_file(),
...
Module::Module()
    : m_mutex(Mutex::eMutexTypeRecursive), m_mod_time(), m_arch(), m_uuid(),
      m_file(), m_platform_file(), m_remote_install_file(), m_symfile_spec(),
      m_object_name(), m_object_offset(0), m_object_mod_time(), m_objfile_sp(),
      m_symfile_ap(), m_type_system_map(), m_source_mappings(), m_sections_ap(),
      m_did_load_objfile(false), m_did_load_symbol_vendor(false),
      m_did_parse_uuid(false), m_file_has_changed(false),
      m_first_file_changed_log(false) {
