...
Broadcaster::BroadcasterImpl::BroadcasterImpl(Broadcaster &broadcaster)
    : m_broadcaster(broadcaster), m_listeners(),
      m_listeners_mutex(Mutex::eMutexTypeRecursive), m_hijacking_listeners(),
      m_hijacking_masks() {}
...
void Broadcaster::BroadcasterImpl::ListenerIterator(
    std::function<bool(const lldb::ListenerSP &listener_sp,
                       uint32_t &event_mask)> const &callback) {
  // Private iterator that should be used by everyone except
  // BroadcasterImpl::RemoveListener().
  // We have weak pointers to our listeners which means that at any point the
  // listener can
  // expire which means we will need to take it out of our list. To take care of
  // this, we
  // iterate and check that the weak pointer can be made into a valid shared
  // pointer before
  // we call the callback. If the weak pointer has expired, we remove it from
  // our list.
...
void Broadcaster::BroadcasterImpl::Clear() {
  ...
  ListenerIterator([this](const lldb::ListenerSP &curr_listener_sp,
                          uint32_t &curr_event_mask) -> bool {
...
Broadcaster::BroadcasterImpl::AddListener(const lldb::ListenerSP &listener_sp,
  ...
  ListenerIterator([this, &listener_sp, &handled,
                    event_mask](const lldb::ListenerSP &curr_listener_sp,
                                uint32_t &curr_event_mask) -> bool {
...
bool Broadcaster::BroadcasterImpl::EventTypeHasListeners(uint32_t event_type) {
  ...
  bool result = false;
  ListenerIterator(
      [this, event_type, &result](const lldb::ListenerSP &curr_listener_sp,
                                  uint32_t &curr_event_mask) -> bool {
  ...
  if (listener) {
    ...
    while (pos != m_listeners.end()) {
      ...
      if (curr_listener_sp) {
        ...
        // The listener has been destroyed since we couldn't turn the
        // std::weak_ptr
...
void Broadcaster::BroadcasterImpl::PrivateBroadcastEvent(EventSP &event_sp,
  ...
  if (hijacking_listener_sp) {
    ...
    ListenerIterator([this, unique, event_type,
                      &event_sp](const lldb::ListenerSP &curr_listener_sp,
                                 uint32_t &curr_event_mask) -> bool {

      if (event_type & curr_event_mask) {
        if (!unique ||
            curr_listener_sp->PeekAtNextEventForBroadcasterWithType(
                &m_broadcaster, event_type) == nullptr)
