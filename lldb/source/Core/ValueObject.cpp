...
lldb::ValueObjectSP ValueObject::CastDerivedToBaseType(
    CompilerType type,
...
lldb::ValueObjectSP ValueObject::CastBaseToDerivedType(CompilerType type,
  ...
  uintptr_t addr = type.IsPointerType() ? GetValueAsUnsigned(0)
                                        : GetLoadAddress();

  lldb::ValueObjectSP value =
      ValueObject::CreateValueObjectFromPointer(target, addr - offset,
                                                pointer_type);
...
lldb::ValueObjectSP ValueObject::CastScalarToBasicType(CompilerType type,
  ...
  assert(GetCompilerType().IsScalarType()
...
lldb::ValueObjectSP ValueObject::CastEnumToBasicType(CompilerType type) {
  ...
  if (type.IsBoolean()) {
    return ValueObject::CreateValueObjectFromBool(target, GetValueAsUnsigned(0)
                                                  != 0);
  }
  ...
  if (type.IsFloat()) {
    ...
    llvm::APFloat f =
        scalar_int.CreateAPFloatFromAPSInt(
            type.GetCanonicalType().GetBasicTypeEnumeration());
    return ValueObject::CreateValueObjectFromAPFloat(target, f, type);
  }
  assert(false && "invalid target type: must be a scalar");
...
lldb::ValueObjectSP ValueObject::CastPointerToBasicType(CompilerType type) {
  ...
  // Get the value as APSInt and extend or truncate it to the requested size.
  llvm::APSInt ext =
      GetValueAsAPSInt().extOrTrunc(type_byte_size * CHAR_BIT);
  return ValueObject::CreateValueObjectFromAPInt(target, ext, type);
}

lldb::ValueObjectSP ValueObject::CastIntegerOrEnumToEnumType(CompilerType type) {
  lldb::TargetSP target = GetTargetSP();

  assert(type.IsEnumerationType() && "target type must be an enum");
  assert((GetCompilerType().IsInteger() || GetCompilerType().IsEnumerationType())
...
lldb::ValueObjectSP ValueObject::CastFloatToEnumType(CompilerType type,
  ...
  llvm::APFloatBase::opStatus status =
      GetValueAsFloat().convertToInteger(integer, llvm::APFloat::rmTowardZero,
                                         &is_exact);
...
lldb::ValueObjectSP ValueObject::CreateValueObjectFromBytes (
    lldb::TargetSP target_sp, const void* bytes, CompilerType type) {
  ExecutionContext exe_ctx(
...
lldb::ValueObjectSP ValueObject::CreateValueObjectFromPointer (
    lldb::TargetSP target, uintptr_t addr, CompilerType type) {
  return CreateValueObjectFromBytes(target, &addr, type);
}

lldb::ValueObjectSP ValueObject::CreateValueObjectFromBool (
    lldb::TargetSP target, bool value) {
...
lldb::ValueObjectSP ValueObject::CreateValueObjectFromNullptr(
    lldb::TargetSP target, CompilerType type) {
