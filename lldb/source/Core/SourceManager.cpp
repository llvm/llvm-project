...
// FindEntryPoint() will attempt to resolve these to a valid line entry in the order
// in which they are provided. This could probably be extended with a notion
// of "main language", i.e. the language in which the main executable module is coded
...
static lldb_private::LineEntry FindEntryPoint (Module *exe_module)
{
    if (!exe_module)
        return LineEntry();
    const std::vector<std::pair<ConstString,bool>>& entry_points(GetEntryPointNames());
    for (std::pair<ConstString,bool> entry_point : entry_points)
    {
        ...
        bool append = false;
        size_t num_matches = exe_module->FindFunctions (entry_point_name,
                                                        NULL,
        ...
        for (size_t idx = 0; idx < num_matches; idx++)
        {
            ...
            if (sc.function)
            {
                ...
                if (skip_prologue)
                    base_address.SetOffset(sc.function->GetPrologueByteSize() + base_address.GetOffset());
