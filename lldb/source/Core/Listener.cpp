...
Listener::Listener(const char *name)
    : m_name(name), m_broadcasters(),
      m_broadcasters_mutex(Mutex::eMutexTypeRecursive), m_events(),
...
Listener::PeekAtNextEventForBroadcasterWithType(Broadcaster *broadcaster,
  ...
  Mutex::Locker lock(m_events_mutex);
  EventSP event_sp;
  if (FindNextEventInternal(lock, broadcaster, nullptr, 0, event_type_mask,
                            event_sp, false))
    return event_sp.get();
  return nullptr;
}
...
bool Listener::WaitForEventForBroadcasterWithType(const TimeValue *timeout,
...
bool Listener::WaitForEventForBroadcaster(const TimeValue *timeout,
                                          Broadcaster *broadcaster,
                                          EventSP &event_sp) {
  return WaitForEventsInternal(timeout, broadcaster, nullptr, 0, 0, event_sp);
}

bool Listener::WaitForEvent(const TimeValue *timeout, EventSP &event_sp) {
