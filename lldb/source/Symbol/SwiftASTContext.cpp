...
SwiftASTContext::GetTypeFromMangledTypename (const char *mangled_typename, Error &error)
{
    ...
    if (mangled_typename && mangled_typename[0] == '_' && mangled_typename[1] == 'T')
    {
        ...
        {
            if (log)
                log->Printf ("((SwiftASTContext*)%p)->GetTypeFromMangledTypename('%s') -- null Swift AST Context", this, mangled_typename);
...
bool
SwiftASTContext::IsSelfArchetypeType (const CompilerType& compiler_type)
{
    if (!compiler_type.IsValid())
        return false;
    ...
    if (llvm::dyn_cast_or_null<SwiftASTContext>(compiler_type.GetTypeSystem()))
    {
        if (swift::ArchetypeType *archetype = llvm::dyn_cast<swift::ArchetypeType>((swift::TypeBase*)compiler_type.GetOpaqueQualType())) {
                  ...
                  if (arg_idx == 0) {
                      return CompilerType(GetASTContext(),
                          swift::ArchetypeBuilder::mapTypeIntoContext(
                              nominal_type_decl, depTy)->castTo<swift::ArchetypeType>());
                  }
