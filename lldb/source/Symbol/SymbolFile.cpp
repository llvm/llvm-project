...
bool
SymbolFile::ForceInlineSourceFileCheck ()
{
    ...
    // file. Returning true for JIT files means all breakpoints set by file and line
    // will be found correctly.
    return m_obj_file->GetType() == ObjectFile::eTypeJIT;
}

bool
SymbolFile::SetLimitSourceFileRange (const FileSpec &file,
                                     uint32_t first_line,
                                     uint32_t last_line)
{
...
bool
SymbolFile::SymbolContextShouldBeExcluded (const SymbolContext &sc, uint32_t actual_line)
{
