...
static TemplateParameterList *CreateTemplateParameterList(
  ...
  TemplateParameterList *template_param_list =
      TemplateParameterList::Create(*ast, SourceLocation(), SourceLocation(),
                                    template_param_decls, SourceLocation());
...
void ClangASTContext::CreateFunctionTemplateSpecializationInfo(
  ...
  TemplateArgumentList template_args(TemplateArgumentList::OnStack,
                                     infos.args.data(), infos.args.size());
...
ClangASTContext::CreateClassTemplateSpecializationDecl(
  ...
  ClassTemplateSpecializationDecl *class_template_specialization_decl =
      ClassTemplateSpecializationDecl::Create(
          *ast, (TagDecl::TagKind)kind, decl_ctx, SourceLocation(),
          SourceLocation(), class_template_decl,
          &template_param_infos.args.front(), template_param_infos.args.size(),
          nullptr);
...
static inline bool check_op_param(uint32_t op_kind, bool unary, bool binary,
                                  uint32_t num_params) {
...
bool ClangASTContext::CheckOverloadedOperatorKindParameterCount(
    uint32_t op_kind, uint32_t num_params) {
  switch (op_kind) {
  default:
    break;
...
#define OVERLOADED_OPERATOR(Name, Spelling, Token, Unary, Binary, MemberOnly)  \
  case OO_##Name:                                                              \
    return check_op_param(op_kind, Unary, Binary, num_params);
  switch (op_kind) {
...
FunctionDecl *ClangASTContext::CreateFunctionDeclaration(
  ...
  if (name && name[0]) {
    func_decl =
        FunctionDecl::Create(*ast, decl_ctx, SourceLocation(), SourceLocation(),
                             DeclarationName(&ast->Idents.get(name)),
                             ClangUtil::GetQualType(function_clang_type),
                             nullptr, (clang::StorageClass)storage, is_inline,
          ...
          clang::IndirectFieldDecl *indirect_field =
              clang::IndirectFieldDecl::Create(
                  *ast->getASTContext(), record_decl, clang::SourceLocation(),
                  ...
                  nested_indirect_field_decl->getType(), chain,
                  nested_chain_size + 1);
