...
public:
  ...
  JavaObjectType(const ConstString &name, const ConstString &linkage_name,
                 uint32_t byte_size)
      : JavaType(JavaType::eKindObject), m_name(name),
        m_linkage_name(linkage_name), m_byte_size(byte_size),
        m_base_class_offset(0), m_is_complete(false),
...
public:
  JavaArrayType(CompilerType element_type,
                const DWARFExpression &length_expression,
                const lldb::addr_t data_offset)
      : JavaType(JavaType::eKindArray), m_element_type(element_type),
  ...
  uint32_t GetNumElements(const ValueObject *value_obj) {
    ...
    Value result;
    if (m_length_expression.Evaluate(nullptr, nullptr, nullptr, nullptr, 0,
...
CompilerType
JavaASTContext::CreateArrayType(const CompilerType &element_type,
                                const DWARFExpression &length_expression,
                                const lldb::addr_t data_offset) {
  ConstString name = element_type.GetTypeName();
  auto it = m_array_type_map.find(name);
  if (it == m_array_type_map.end()) {
    std::unique_ptr<JavaType> array_type(
        new JavaArrayType(element_type, length_expression, data_offset));
