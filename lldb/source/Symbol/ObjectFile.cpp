...
ObjectFile::ObjectFile(const lldb::ModuleSP &module_sp,
    ...
    : ModuleChild(module_sp),
      ...
      m_data_sp(std::make_shared<DataExtractor>()), m_process_wp(),
...
ObjectFile::ObjectFile(const lldb::ModuleSP &module_sp,
    ...
    : ModuleChild(module_sp), m_file(), m_type(eTypeInvalid),
      ...
      m_data_sp(std::make_shared<DataExtractor>()), m_process_wp(process_sp),
      m_memory_addr(header_addr), m_sections_up(), m_symtab_up(),
      m_symtab_once_up(new llvm::once_flag()) {
  if (header_data_sp)
    m_data_sp->SetData(header_data_sp, 0, header_data_sp->GetByteSize());
  Log *log = GetLog(LLDBLog::Object);
  LLDB_LOGF(log,
...
size_t ObjectFile::GetData(lldb::offset_t offset, size_t length,
  ...
  // The entire file has already been mmap'ed into m_data_sp, so just copy from
  // there as the back mmap buffer will be shared with shared pointers.
  return data.SetData(*m_data_sp.get(), offset, length);
}
...
size_t ObjectFile::CopyData(lldb::offset_t offset, size_t length,
  ...
  return m_data_sp->CopyData(offset, length, dst);
}
