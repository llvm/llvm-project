...
SwiftLanguageRuntime::SwiftLanguageRuntime(Process *process)
    : LanguageRuntime(process), m_negative_cache_mutex(Mutex::eMutexTypeNormal),
    ...
    if (mangled_cstr_len > 3) {
      ...
      if (mangled_ref.startswith("_TF")) {
        swift::Demangle::NodePointer node =
            swift::Demangle::demangleSymbolAsNode(mangled_cstr,
                                                  mangled_cstr_len);
        StreamString identifier;
        if (node) {
          switch (node->getKind()) {
...
SwiftLanguageRuntime::MemberVariableOffsetResolver::ResolveOffset(
    ValueObject *valobj, ConstString ivar_name, Error *error) {
  if (error)
    error->Clear();
        ...
        if (symbol_name[2] == 'T' && symbol_name[3] == 'o') {
          ...
          swift::Demangle::NodePointer demangled_nodes =
              swift::Demangle::demangleSymbolAsNode(
                  symbol_name, symbol_mangled_name.GetLength());
...
ValueObjectSP SwiftLanguageRuntime::CalculateErrorValueFromFirstArgument(
  ...
  if (success) {
    ...
    if (swift_error_proto_type.IsValid()) {
      ...
      Error extract_error = arg0->GetValueAsData(&exe_ctx, data, 0, nullptr);
...
Error SwiftLanguageRuntime::SwiftExceptionPrecondition::ConfigurePrecondition(
    Args &args) {
  Error error;
...
public:
  ...
  public:
    CommandOptions(CommandInterpreter &interpreter)
        : Options(interpreter), m_expand(false, false) {
    ...
    void OptionParsingStarting() { m_expand.Clear(); }
