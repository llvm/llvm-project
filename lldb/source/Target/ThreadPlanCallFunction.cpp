        ...
        if (GetExpressionLanguage() == eLanguageTypeSwift)
        {
            ...
            if (swift_runtime)
            {
                ...
                if (!backstop_name.IsEmpty())
                {
                    ...
                    stdlib_module_list.Append(FileSpec (swift_runtime->GetStandardLibraryName().AsCString(), false));
                    const LazyBool skip_prologue = eLazyBoolNo;
                    const bool is_internal   = true;
                    const bool is_hardware   = false;
                    m_error_backstop_bp_sp = process_sp->GetTarget().CreateBreakpoint(&stdlib_module_list,
                                                                                      NULL,
