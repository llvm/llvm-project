    ...
    m_calculate_return_value(gather_return_value)
{
    ...
    // If we are about to step out of a swift frame, we need to store away the location that swift will use for
    // any error return:
    // FIXME: I'm only doing this if you are stepping out ONE frame at present.  I'll have to change the stepping
    // code so that it first runs to the frame we are stepping out FROM, then capture the error return pointer, then
    ...
    if (frame_idx == 0)
    {
        ...
        {
            CompileUnit *comp_unit = frame_sp->GetSymbolContext(eSymbolContextCompUnit).comp_unit;
        ...
        if (stepping_from_swift)
        {
            ...
            if (swift_runtime)
            {
                m_swift_error_return_addr = swift_runtime->GetErrorReturnLocationForFrame(frame_sp);
            }
        }
    }
}
...
ThreadPlanStepOut::CalculateReturnValue ()
{
    ...
    if (m_swift_error_return_addr != LLDB_INVALID_ADDRESS)
    {
        ...
        if (swift_runtime)
        {
            ...
            m_return_valobj_sp = swift_runtime->CalculateErrorValueObjectAtAddress(m_swift_error_return_addr,
                                                                                   name,
                                                                                   true);
