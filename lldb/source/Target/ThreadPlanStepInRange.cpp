...
bool
ThreadPlanStepInRange::MischiefManaged()
{
...
bool
ThreadPlanStepInRange::StepInDeepBreakpointExplainsStop (lldb::StopInfoSP stop_info_sp)
{
    Log *log(lldb_private::GetLogIfAllCategoriesSet (LIBLLDB_LOG_STEP));
    ...
    for (size_t i = 0; i < num_step_in_deep_bps && hit_step_in_deep_bp == false; i++)
    {
        if (bp_site_sp->IsBreakpointAtThisSite (m_step_in_deep_bps[i]))
        {
            // We've hit a step in deep breakpoint, see if it is the only breakpoint at this site:
            explains_stop = true;
            ...
            // Of course, if there's only one owner, it's us so we don't need to check.
            ...
            for (size_t i = 0; i < num_owners; i++)
            {
                ...
                if (owner_loc_sp->ValidForThisThread(&GetThread()) && !owner_bp.IsInternal())
                {
                    explains_stop = false;
                    break;
                }
            ...
            if (log)
                log->Printf ("ThreadPlanStepRange::StepInDeepBreakpointExplainsStop - Hit step in deep breakpoint %d which has %zu owners - explains stop: %u.",
                            m_step_in_deep_bps[i],
                            num_owners,
                            explains_stop);
    ...
    // For now, if we trigger one of our "step in deep" breakpoints we delete them all:
...
void
ThreadPlanStepInRange::ClearStepInDeepBreakpoints ()
{
    size_t num_step_in_deep_bps = m_step_in_deep_bps.size();
...
ThreadPlanStepInRange::DefaultShouldStopHereCallback (ThreadPlan *current_plan, Flags &flags, FrameComparison operation, void *baton)
{
    ...
    if (should_stop_here && current_plan->GetKind() == eKindStepInRange && operation == eFrameCompareYounger)
    {
        ...
        should_stop_here = step_in_range_plan->DefaultShouldStopHereImpl(flags, !should_stop_here);
...
bool
ThreadPlanStepInRange::DefaultShouldStopHereImpl (Flags &flags, bool should_step_out)
{
    StackFrame *frame = GetThread().GetStackFrameAtIndex(0).get();
    Log *log(lldb_private::GetLogIfAllCategoriesSet (LIBLLDB_LOG_STEP));
    ...
    if (should_step_out)
    {
        // We are going to step out, but first let's examine the function we are stepping past to see if it tells us
        // about any interesting places we could stop while running it.  For instance, if we can tell from the signature
        // that we're being passed a function pointer that points to user code, we'll prospectively stop there.
        // We only know how to do this for Swift at present.
        // FIXME: We could probably do this for C++ mangled names as well, if we could come up with some
        // good heuristic to identify function pointers in the mangled function arguments.
        ...
        if (sc.symbol)
        {
            ...
            if (mangled_name.GuessLanguage() == lldb::eLanguageTypeSwift)
            {
                ...
                if (swift_runtime)
                {
                    ...
                    swift_runtime->FindFunctionPointersInCall(*frame, interesting_addresses);
                    size_t num_addresses = interesting_addresses.size();
                    if (num_addresses)
                    {
                        // Run through the addresses we found, make sure they have debug info, and if so set breakpoints
                        // on all these addresses.
                        for (size_t i = 0; i < num_addresses; i++)
                        {
                            LineEntry line_entry;
                            if (interesting_addresses[i].CalculateSymbolContextLineEntry(line_entry))
                            {
                                // It has debug information, use it:
                                const bool internal = true;
                                const bool hardware = false;
                                BreakpointSP bkpt_sp = GetTarget().CreateBreakpoint(interesting_addresses[i], internal, hardware);
    ...
    // We're returning an answer to "Should Stop Here" which is the opposite of "should_step_out".
    ...
    else
    {
        ...
        if (stop_info_sp)
        {
            ...
            if (reason ==  eStopReasonBreakpoint)
            {
                ...
                bool hit_step_in_deep_bp = StepInDeepBreakpointExplainsStop(stop_info_sp);
