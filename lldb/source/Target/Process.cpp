...
Error ProcessLaunchCommandOptions::SetOptionValue(uint32_t option_idx,
  ...
  case 'a':
    if (!launch_info.GetArchitecture().SetTriple(
...
Process::Process(lldb::TargetSP target_sp, ListenerSP listener_sp,
    ...
    : ProcessProperties(this), UserID(LLDB_INVALID_PROCESS_ID),
      ...
      m_exit_status_mutex(), m_thread_mutex(Mutex::eMutexTypeRecursive),
      m_thread_list_real(this), m_thread_list(this),
      m_extended_thread_list(this), m_extended_thread_stop_id(0),
      m_queue_list(this), m_queue_list_stop_id(0), m_notifications(),
      m_image_tokens(), m_listener_sp(listener_sp), m_breakpoint_site_list(),
      m_dynamic_checkers_ap(), m_unix_signals_sp(unix_signals_sp), m_abi_sp(),
      m_process_input_reader(), m_stdio_communication("process.stdio"),
      m_stdio_communication_mutex(Mutex::eMutexTypeRecursive),
      m_stdin_forward(false), m_stdout_data(), m_stderr_data(),
      m_profile_data_comm_mutex(Mutex::eMutexTypeRecursive), m_profile_data(),
      m_iohandler_sync(0), m_memory_cache(*this),
      m_allocated_memory_cache(*this), m_should_detach(false),
      m_next_event_action_ap(), m_public_run_lock(), m_private_run_lock(),
      m_stop_info_override_callback(nullptr), m_finalizing(false),
      m_finalize_called(false), m_clear_thread_plans_on_stop(false),
      m_force_next_event_delivery(false), m_destroy_in_process(false),
      m_destroy_complete(false), m_last_broadcast_state(eStateInvalid),
...
void Process::SyncIOHandler(uint32_t iohandler_id, uint64_t timeout_msec) {
  ...
  m_iohandler_sync.WaitForValueNotEqualTo(iohandler_id, new_iohandler_id,
                                          &timeout);
...
StateType Process::WaitForProcessToStop(const TimeValue *timeout,
...
StateType Process::WaitForState(const TimeValue *timeout,
...
StateType Process::WaitForStateChangedEvents(const TimeValue *timeout,
                                             EventSP &event_sp,
                                             ListenerSP hijack_listener_sp) {
  Log *log(lldb_private::GetLogIfAllCategoriesSet(LIBLLDB_LOG_PROCESS));
  ...
  if (log)
    log->Printf("Process::%s (timeout = %p, event_sp) => %s", __FUNCTION__,
                static_cast<const void *>(timeout), StateAsCString(state));
  return state;
}
...
StateType Process::WaitForStateChangedEventsPrivate(const TimeValue *timeout,
                                                    EventSP &event_sp) {
  Log *log(lldb_private::GetLogIfAllCategoriesSet(LIBLLDB_LOG_PROCESS));
...
bool Process::WaitForEventsPrivate(const TimeValue *timeout, EventSP &event_sp,
                                   bool control_only) {
  Log *log(lldb_private::GetLogIfAllCategoriesSet(LIBLLDB_LOG_PROCESS));
...
bool Process::SetProcessExitStatus(
  ...
  if (log)
    log->Printf("Process::SetProcessExitStatus (baton=%p, pid=%" PRIu64
                ", exited=%i, signal=%i, exit_status=%i)\n",
                callback_baton, pid, exited, signo, exit_status);
...
Error Process::ResumeSynchronous(Stream *stream) {
  ...
  if (error.Success()) {
    StateType state =
        WaitForProcessToStop(NULL, NULL, true, listener_sp, stream);
...
StateType Process::WaitForProcessStopPrivate(const TimeValue *timeout,
                                             EventSP &event_sp) {
  StateType state;
...
Error Process::Halt(bool clear_thread_plans, bool use_run_lock) {
  ...
  StateType state = WaitForProcessToStop(
      &timeout_time, &event_sp, true, halt_listener_sp, nullptr, use_run_lock);
  RestoreProcessEvents();
...
Error Process::StopForDestroyOrDetach(lldb::EventSP &exit_event_sp) {
  ...
  if (m_public_state.GetValue() == eStateRunning) {
    ...
    StateType state =
        WaitForProcessToStop(&timeout, &exit_event_sp, true, listener_sp);
...
void Process::ControlPrivateStateThread(uint32_t signal) {
  ...
  // when it exits
  {
    ...
    if (private_state_thread.IsJoinable()) {
      ...
      // Send the control event and wait for the receipt or for the private
      // state
      // thread to exit
...
thread_result_t Process::PrivateStateThread(void *arg) {
  PrivateStateThreadArgs real_args =
...
public:
  IOHandlerProcessSTDIO(Process *process, int write_fd)
      : IOHandler(process->GetTarget().GetDebugger(),
        ...
        m_pipe(), m_waiting_on_pipe(false) {
...
// So don't call return anywhere within it.
    ...
    while (true) {
      ...
      if (log) {
        if (timeout_ptr) {
          log->Printf(
              "Process::RunThreadPlan(): about to wait - now is %" PRIu64
              " - endpoint is %" PRIu64,
