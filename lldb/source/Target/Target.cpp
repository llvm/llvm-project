...
Target::Target(Debugger &debugger, const ArchSpec &target_arch,
    ...
    : TargetProperties(this),
      ...
      m_mutex(Mutex::eMutexTypeRecursive), m_arch(target_arch), m_images(this),
      m_section_load_history(), m_breakpoint_list(false),
  ...
  if (rc.Success()) {
    ...
    if (num_current_watchpoints >= num_supported_hardware_watchpoints)
      error.SetErrorStringWithFormat(
          "number of supported hardware watchpoints (%u) has been reached",
          num_supported_hardware_watchpoints);
  }
  return false;
}
...
ClangModulesDeclVendor *Target::GetClangModulesDeclVendor() {
  static Mutex s_clang_modules_decl_vendor_mutex; // If this is contended we can
                                                  // make it per-target

  {
    Mutex::Locker clang_modules_decl_vendor_locker(
        s_clang_modules_decl_vendor_mutex);
  ...
  if (error.Success()) {
    if (synchronous_execution ||
      ...
      StateType state = m_process_sp->WaitForProcessToStop(
          nullptr, nullptr, false, hijack_listener_sp, nullptr);
          ...
          if (synchronous_execution) {
            ...
            if (error.Success()) {
              state = m_process_sp->WaitForProcessToStop(
                  nullptr, nullptr, true, hijack_listener_sp, stream);
  ...
  if (error.Success() && process_sp) {
    if (async) {
      ...
      state = process_sp->WaitForProcessToStop(
          nullptr, nullptr, false, attach_info.GetHijackListener(), stream);
      process_sp->RestoreProcessEvents();
