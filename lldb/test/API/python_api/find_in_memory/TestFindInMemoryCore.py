...
class FindInMemoryTestCase(TestBase):
    ...
    def CreateCoreProcess(self, plugin_name: str) -> ProcessInfo:
        core_path = self.getBuildArtifact("a.out.core")
        if plugin_name == "elf":
            self.yaml2obj("linux-x86_64.yaml", core_path)
        else:
        ...
        core_pi = ProcessInfo()
        core_pi.target = self.dbg.CreateTarget(None)
    ...
    def test_elf_find_ranges_in_memory_core_heap_ok(self):
        ...
        """
        _ = self.BuildAndRunToBreakpoint()
        core_pi = self.CreateCoreProcess("elf")
        core_var = self.GetVariable(core_pi, "heap_pointer1")
