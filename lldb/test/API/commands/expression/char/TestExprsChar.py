...
class ExprCharTestCase(TestBase):
    ...
    @expectedFailureAll(archs=["powerpc64le", "s390x"], bugnumber="llvm.org/pr23069")
    def test_signed_char(self):
        self.do_test(dictionary={"CFLAGS_EXTRAS": "-fsigned-char"})

    @expectedFailureAll(
        archs=["i[3-6]86", "x86_64", "arm64", "arm64e", "armv7", "armv7k", "arm64_32"],
        bugnumber="llvm.org/pr23069, <rdar://problem/28721938>",
    )
    @expectedFailureAll(triple="mips*", bugnumber="llvm.org/pr23069")
    @expectedFailureAll(
        oslist=["windows"], archs=["aarch64"], bugnumber="llvm.org/pr23069"
    )
