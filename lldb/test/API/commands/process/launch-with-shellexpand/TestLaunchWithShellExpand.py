...
class LaunchWithShellExpandTestCase(TestBase):
    ...
    def test(self):
        ...
        self.runCmd(
            "process launch -X true -w %s -- fi*.tx? () > <" % (self.getSourceDir())
        )
        ...
        self.assertTrue(
            thread.IsValid(), "Process stopped at 'main' should have a valid thread"
        )
        ...
        self.runCmd('process launch -X true -w %s -- "foo bar"' % (self.getSourceDir()))
