...
The server selects the loader by name during accelerator initialization. The
loader then asks the server for the loaded libraries via
...
class AcceleratorResponder(MockGDBServerResponder):
    ...
    def qSupported(self, client_supported):
        return (
            ...
            + ";qXfer:features:read+;accelerator-plugins+"
        )
    ...
    def other(self, packet):
        if packet == "jAcceleratorPluginInitialize":
            action = {
                "plugin_name": "mock",
                "session_name": "",
                "identifier": 0,
            ...
            if self.dyld_plugin_name is not None:
                response["dyld_plugin_name"] = self.dyld_plugin_name
...
class TestAcceleratorDynamicLoader(GDBRemoteTestBase):
    def make_container(self):
        ...
        bundle, offsets = make_clang_offload_bundle(
            [
                ...
                ("hipv4-amdgpu-amd-amdhsa--gfx942", embedded_bytes),
            ]
        )
    ...
    def test_not_selected_without_server_request(self):
    ...
    def test_selected_by_name_for_host_target(self):
        """An explicit server selection does not depend on target architecture."""
