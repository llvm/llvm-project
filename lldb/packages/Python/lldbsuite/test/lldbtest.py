...
class TestVariant:
    ...
    `_xfailForVariant` / `_skipForVariant` in `decorators.py` to attach
    a predicate function to the **original** test method.  During expansion the
    metaclass evaluates that predicate for each value and wraps the copy
    with `unittest.expectedFailure` or `unittest.skip` accordingly.
    ...
    def __init__(
        ...
        Args:
            name: Attribute name set on each expanded method to record which
                value it was created for (e.g. `"debug_info"`).  Also
                used to derive the xfail/skip attribute names
                (`__xfail_for_<name>_fn__`, `__skip_for_<name>_fn__`).
        ...
        self.xfail_attr = f"__xfail_for_{name}_fn__"
        self.skip_attr = f"__skip_for_{name}_fn__"
...
def _expand_test_variants(attrname, methods, variant, xfail_fn, skip_fn, no_reason):
    ...
    `unittest.expectedFailure` or `unittest.skip` based on *xfail_fn* /
    *skip_fn*.
    ...
    Args:
        ...
        xfail_fn: Callable `(**{variant.name: value_name}) -> reason | None`.
        skip_fn:  Callable `(**{variant.name: value_name}) -> reason | None`.
        no_reason: Sentinel callable (unused here, kept for signature parity).
            ...
            if attrname.startswith("test") and not getattr(
                ...
                if not original_testcase.NO_DEBUG_INFO_TESTCASE:
                    ...
                    xfail_for_debug_info_cat_fn = getattr(
                        attrvalue, "__xfail_for_debug_info_fn__", no_reason
                    )
                    skip_for_debug_info_cat_fn = getattr(
                        attrvalue, "__skip_for_debug_info_fn__", no_reason
                    )
                ...
                for variant in _test_variants:
                    if variant.should_expand(attrvalue):
                        xfail_fn = getattr(attrvalue, variant.xfail_attr, no_reason)
                        skip_fn = getattr(attrvalue, variant.skip_attr, no_reason)
