...
class CPPBreakpointCommandsTestCase(TestBase):
    ...
    def test(self):
        ...
        plain_method_break = target.BreakpointCreateByName(
            "Function", lldb.eFunctionNameTypeMethod, a_out_module, nested_comp_unit)
        self.assertTrue(plain_method_break.GetNumLocations() == 3)

        plain_method_break = target.BreakpointCreateByName(
            "Baz::Function", lldb.eFunctionNameTypeMethod, a_out_module, nested_comp_unit)
        ...
        plain_method_break = target.BreakpointCreateByName(
            "Function", lldb.eFunctionNameTypeBase, a_out_module, nested_comp_unit)
        self.assertTrue(plain_method_break.GetNumLocations() == 2)

        plain_method_break = target.BreakpointCreateByName(
            "Bar::Function", lldb.eFunctionNameTypeBase, a_out_module, nested_comp_unit)
        self.assertTrue(plain_method_break.GetNumLocations() == 1)
