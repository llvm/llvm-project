...
class TestSimpleSwiftExpressions(TestBase):
    ...
    def do_test(self):
        ...
        value_obj = self.frame.EvaluateExpression ("iff is_five === 5 { return is_five")
        ...
        self.check_expression ("if (1 == 1) { return is_five + is_six }", "11", use_summary=False)
        ...
        self.check_expression ("if is_five == 5 { return is_five + is_six } else { return is_five }", "11", use_summary=False)

        # Make sure we get an error if we don't give homogenous return types:
        bool_or_int = self.frame.EvaluateExpression ("if is_five == 5 { return is_five + is_six } else { return false }")
        self.assertTrue(bool_or_int.IsValid(), "if is_five == 5 { return is_five + is_six } else { return false } is invalid")

        # Make sure we get the correct branch of a complex result expression:
        self.check_expression ("if is_five == 6 {return is_five} else if is_six == 5 {return is_six} ; is_eleven", "11", use_summary=False)
        ...
        # Non-simple names:
        self.check_expression (u"\u20ac_varname".encode("utf-8"), "5", use_summary=False)

        # See if we can do the same manipulations with tuples:
        # Commented out due to: <rdar://problem/15476525> Expressions with tuple elements assert
        self.check_expression ("a_tuple.0 + a_tuple.1", "11", use_summary=False)

        # See if we can do some manipulations with dicts:
        self.check_expression ('str_int_dict["five"]! + str_int_dict["six"]!', "11", use_summary=False)
        self.check_expression ('int_str_dict[Int(is_five + is_six)]!', '"eleven"')
        # Commented out, touching the dict twice causes it to die, probably the same problem 
        # as <rdar://problem/15306399>
        self.check_expression ('str_int_dict["five"] = 6; str_int_dict["five"]! + str_int_dict["six"]!', "12", use_summary=False)

        # See if we can use a switch statement in an expression:
        self.check_expression ("switch is_five { case 0..<6: return 1; case 7..<11: return 2; case _: return 4; }; 3;", "1", use_summary=False)

        # These ones are int-convertible and Equatable so we can do some things with them anyway:
        self.check_expression ("enum_eleven", "Eleven", False)
        self.check_expression ("enum_eleven == SomeValues.Eleven", "true")
        self.check_expression ("SomeValues.Five.toInt() + SomeValues.Six.toInt()", "11", use_summary=False)
        self.check_expression ("enum_eleven = .Five; return enum_eleven == .Five", "true")

        # Test expressions with a simple object:
        self.check_expression ("a_obj.x", "6", use_summary=False)
        # Should not have to make a second object here. This is another side-effect of <rdar://problem/15306399>
        ...
        self.check_expression ('"Five: \(is_five) " + "Six: \(is_six)"', '"Five: 5 Six: 6"')
        ...
        failed_value = self.frame.EvaluateExpression ("b_struct.b_read_only = 34")
        self.assertTrue(failed_value.IsValid() == True,  "Get something back from the evaluation.")
        self.assertTrue(failed_value.GetError().Success() == False, "But it is an error.")
        ...
        self.check_expression ("var enum_six : SomeValues = SomeValues.Six; return enum_six == .Six", "true")
