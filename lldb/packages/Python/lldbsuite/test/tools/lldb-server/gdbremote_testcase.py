...
class GdbRemoteTestCaseBase(TestBase):
    ...
    def tearDown(self):
        ...
        if self._remote_server_log_file is not None:
            lldb.remote_platform.Get(
                lldb.SBFileSpec(
                    self._remote_server_log_file), lldb.SBFileSpec(
                    self.getLocalServerLogFile()))
            lldb.remote_platform.Run(
                lldb.SBPlatformShellCommand(
                    "rm " + self._remote_server_log_file))
            self._remote_server_log_file = None
    ...
    def setUpServerLogging(self, is_llgs):
        ...
        if is_llgs:
            ...
            self.debug_monitor_extra_args = [
                "--log-file=" + self.log_file, "--log-flags=0x800000"]
