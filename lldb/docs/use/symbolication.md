...
address of the \_\_TEXT segment for each binary. Specifying a slide requires
requires that you first find the original (file) address for the \_\_TEXT
segment, and subtract the two values. If you specify the address of the \_\_TEXT
...
We specified that the \_\_TEXT section is loaded at 0x100123000. Now that we have
...
Then we set the load addresses for each \_\_TEXT section (note the colors of the
