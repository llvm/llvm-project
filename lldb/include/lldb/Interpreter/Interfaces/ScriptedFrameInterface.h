...
public:
  ...
  /// declared variable is subject to, so a ValueObject with no storage behind
  /// it will be filtered out of an in-scope-only listing without it. Leave it
  /// off to present a variable that really does exist in the frame.
  ///
  /// Returning std::nullopt keeps the ValueType the ValueObject already
  /// reports, with the synthetic flag added.
