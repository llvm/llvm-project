...
private:
    ...
    bool
    DefaultShouldStopHereImpl (Flags &flags, bool should_step_out);
    
    bool
    StepInDeepBreakpointExplainsStop(lldb::StopInfoSP stop_info_sp);
    ...
    std::vector<lldb::break_id_t> m_step_in_deep_bps; // Places where we might want to stop when we do a step out.
