...
protected:
  ...
  Mutex m_mutex; // Mutex for modifying Platform data structures that should
                 // only be used for non-reentrant code
...
public:
  PlatformList()
      : m_mutex(Mutex::eMutexTypeRecursive), m_platforms(),
...
public:
  ...
  lldb_private::Error SetOptionValue(CommandInterpreter &interpreter,
                                     uint32_t option_idx,
