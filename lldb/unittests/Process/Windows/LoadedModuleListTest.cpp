...
namespace {
const FileSpec g_dll("C:\\a\\one.dll", FileSpec::Style::windows);
const FileSpec g_other("C:\\a\\two.dll", FileSpec::Style::windows);
} // namespace

TEST(LoadedModuleListTest, ReportsTheMappingItWasGiven) {
  LoadedModuleList modules;
  modules.Add(g_dll, 0x1000);

  EXPECT_EQ(modules.GetSize(), 1u);
  EXPECT_EQ(modules.GetBaseAddress(g_dll), 0x1000u);
  EXPECT_EQ(*modules.FindFile(g_dll), g_dll);
}
...
TEST(LoadedModuleListTest, DoesNotFindAnUnmappedFile) {
  ...
  EXPECT_EQ(modules.GetBaseAddress(g_other), std::nullopt);
  EXPECT_EQ(modules.FindFile(g_other), nullptr);
}

TEST(LoadedModuleListTest, ReportsTheFirstOfSeveralMappings) {
  LoadedModuleList modules;
  modules.Add(g_dll, 0x1000);
  modules.Add(g_dll, 0x9000);
...
TEST(LoadedModuleListTest, KeepsTheFileWhileAnyMappingRemains) {
  ...
  EXPECT_EQ(modules.Remove(0x1000), g_dll);
  EXPECT_TRUE(modules.IsEmpty());
}
...
TEST(LoadedModuleListTest, FallsBackToASurvivingMapping) {
  ...
  EXPECT_EQ(modules.GetBaseAddress(g_dll), 0x9000u);
}

TEST(LoadedModuleListTest, RemovesOnlyTheGivenMapping) {
  LoadedModuleList modules;
  modules.Add(g_dll, 0x1000);
  modules.Add(g_other, 0x2000);

  EXPECT_EQ(modules.Remove(0x1000), g_dll);
  EXPECT_EQ(modules.GetSize(), 1u);
  EXPECT_EQ(modules.GetBaseAddress(g_other), 0x2000u);
}
