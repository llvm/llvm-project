...
static void dump_available_platforms(FILE *output_file) {
...
static lldb::PlatformSP setup_platform(const std::string &platform_name) {
  ...
  Error error;
  platform_sp =
      Platform::Create(lldb_private::ConstString(platform_name), error);
  if (error.Fail()) {
    ...
    if (Platform::GetHostPlatform() &&
        (Platform::GetHostPlatform()->GetPluginName() ==
         ConstString(platform_name.c_str()))) {
      platform_sp = Platform::GetHostPlatform();
    } else {
