    ...
    for (const auto &bp : *breakpoints) {
      ...
      if (bp_obj) {
        ...
        if (existing_source_bps != g_dap.source_breakpoints.end()) {
          const auto &existing_bp = existing_source_bps->second.find(bp_pos);
        ...
        g_dap.source_breakpoints[path][bp_pos] = src_bp;
        SourceBreakpoint &new_bp = g_dap.source_breakpoints[path][bp_pos];
...
void RegisterRequestCallbacks() {
  ...
  g_dap.RegisterRequestCallback("breakpointLocations",
