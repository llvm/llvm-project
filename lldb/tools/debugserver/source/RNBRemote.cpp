...
std::string binary_encode_string(const std::string &s) {
  std::string output;
  ...
  if (IsValid() && dispatch_qaddr != INVALID_NUB_ADDRESS &&
    ...
    if (dispatch_queue_t) {
      ...
      if (dqo_version >= 4) {
        ...
        queue_name = std::move(DNBProcessMemoryReadCStringFixed(
...
rnb_err_t RNBRemote::HandlePacket_DeallocateMemory(const char *p) {
  ...
  packet.SetFilePos(2); // Skip the "_m"
  nub_addr_t addr =
      packet.GetHexMaxU64(StringExtractor::BigEndian, INVALID_NUB_ADDRESS);
