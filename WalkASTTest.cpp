...
std::vector<Decl::Kind> testWalk(llvm::StringRef TargetCode,
  ...
  // Changed from getFileLoc to getExpansionLoc, to fix broken explicit and implicit reference detection .
  ...
  for (Decl *D : AST.context().getTranslationUnitDecl()->decls()) {
    ...
    walkAST(*D, [&](SourceLocation Loc, NamedDecl &ND, RefType RT) {
      if (SM.getExpansionLoc(Loc) != ReferencingLoc)
        return;
      auto NDLoc = SM.getDecomposedLoc(SM.getExpansionLoc(ND.getLocation()));
