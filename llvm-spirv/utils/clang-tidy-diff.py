...
# https://llvm.org/svn/llvm-project/clang-tools-extra:
# * Do not run the test on the files modified by the patch, but on all the
#   files specified on the command-line;
# * Set the header-filter to all header files;
# * Remove the "--" when forwarding the input files to clang-tidy.

r"""
ClangTidy Diff Checker
======================

This script reads input from a unified diff, runs clang-tidy on all specified
files and outputs clang-tidy warnings in changed lines only. This is useful to
detect clang-tidy regressions in the lines touched by a specific patch.
Example usage for git/svn users:

  git diff -U0 HEAD^ | clang-tidy-diff.py -p1 -- <source0> [... <sourceN>]
...
def main():
  parser = argparse.ArgumentParser(description=
                                   'Run clang-tidy against specified files, and '
  ...
  command.append('-header-filter=' + quote + '.*' + quote)
  if args.fix:
    command.append('-fix')
  if args.checks != '':
