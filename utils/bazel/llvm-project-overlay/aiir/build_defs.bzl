...
def if_cuda_available(if_true, if_false = []):
    return select({
        ...
        "//aiir:enable_cuda_config": if_true,
...
def aiir_c_api_cc_library(
        ...
        capi_deps = [],
        **kwargs):
    """Macro that generates three targets for AIIR C API libraries.
    ...
    * An implementation cc_library target tagged `alwayslink` suitable for
    ...
    aiir_c_api_cc_library targets only depend on other aiir_c_api_cc_library
