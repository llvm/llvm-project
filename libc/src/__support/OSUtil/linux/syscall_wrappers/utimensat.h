...
LIBC_INLINE ErrorOr<int> utimensat(int dirfd, const char *path,
  ...
  return syscall_checked<int>(SYS_utimensat_time64, dirfd, path, times, flags);
  ...
  static_assert(
      ...
      "with 64-bit tv_nsec.");
  return syscall_checked<int>(SYS_utimensat, dirfd, path, times, flags);
#else
