...
///
/// \file
/// Implementation header for single-precision erfc(x).
///
//===----------------------------------------------------------------------===//
...
LIBC_INLINE float erfcf(float x) {
  ...
  // for k = 0..31.  All 32 polynomials have relative errors between 1e-16
  // and 1e-15, i.e. well below the half-ULP of float.
  ...
  // For |x| < 2^-11, erfc(x) is evaluated to double precision using the first
  // terms of its Taylor series.  Also avoids the shift below being >= 32.
  if (LIBC_UNLIKELY(x_abs < 0x3A000000U)) { // |x| < 2^-11
    constexpr double TWO_OVER_SQRT_PI = 0x1.20dd750429b6dp0;
    constexpr double TWO_OVER_THREE_SQRT_PI = 0x1.812746b0379e7p-2;
    double xd = fputil::cast<double>(x);
    double xd2 = xd * xd;
    double erf_x = xd * fputil::multiply_add(xd2, -TWO_OVER_THREE_SQRT_PI,
                                             TWO_OVER_SQRT_PI);
    return fputil::cast<float>(1.0 - erf_x);
  }
