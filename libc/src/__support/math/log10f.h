...
LIBC_INLINE float log10f(float x) {
  ...
  constexpr float INV_LN10_HI = 0x1.bcb7b2p-2f;
  constexpr float INV_LN10_LO = -0x1.5b239ap-26f;
  constexpr float LOG10_2_HI = 0x1.344100p-2f;
  constexpr float LOG10_2_LO = 0x1.a84fbcp-21f;

  float prod_hi = d * INV_LN10_HI;
  float prod_err = fputil::multiply_add(d, INV_LN10_HI, -prod_hi);
  float prod_lo = fputil::multiply_add(
      lo, INV_LN10_HI, fputil::multiply_add(d, INV_LN10_LO, prod_err));

  float m_hi = mf * LOG10_2_HI;
  float m_lo = mf * LOG10_2_LO;

  float sum_hi = m_hi + prod_hi;
  float v = sum_hi - m_hi;
  float err = (m_hi - (sum_hi - v)) + (prod_hi - v);
  return sum_hi + (err + m_lo + prod_lo);
#else
