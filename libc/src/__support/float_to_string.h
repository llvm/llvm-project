...
// block.
template <typename T>
class FloatToString<T, cpp::enable_if_t<cpp::disjunction_v<
                           cpp::is_same<T, float>, cpp::is_same<T, double>>>> {
...
template <typename T>
struct IsFloat128 : cpp::is_same<T, float128> {};
#else
template <typename T>
struct IsFloat128 : cpp::bool_constant<false> {};
#endif

template <typename T>
class FloatToString<T, cpp::enable_if_t<cpp::disjunction_v<
                           cpp::is_same<T, long double>, IsFloat128<T>>>> {
  fputil::FPBits<T> float_bits;
  bool is_negative = 0;
  int exponent = 0;
...
public:
  LIBC_INLINE constexpr FloatToString(T init_float)
