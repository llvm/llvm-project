...
TEST(LlvmLibcSysmacrosTest, StandardLinuxDevices) {
  ...
  dev_t dev_pts0 = LIBC_NAMESPACE::__llvm_libc_makedev(136, 0);
  EXPECT_EQ(dev_pts0, static_cast<dev_t>(0x8800));
  EXPECT_EQ(LIBC_NAMESPACE::__llvm_libc_major(dev_pts0), 136u);
  EXPECT_EQ(LIBC_NAMESPACE::__llvm_libc_minor(dev_pts0), 0u);

  // /dev/sda1: major 8, minor 1 -> dev = 0x801
  dev_t dev_sda1 = LIBC_NAMESPACE::__llvm_libc_makedev(8, 1);
  EXPECT_EQ(dev_sda1, static_cast<dev_t>(0x801));
  EXPECT_EQ(LIBC_NAMESPACE::__llvm_libc_major(dev_sda1), 8u);
  EXPECT_EQ(LIBC_NAMESPACE::__llvm_libc_minor(dev_sda1), 1u);
}
...
TEST(LlvmLibcSysmacrosTest, LargeMinorOrMajor) {
  ...
  // 0x100000.
  dev_t dev1 = LIBC_NAMESPACE::__llvm_libc_makedev(0, 0x100);
  EXPECT_EQ(dev1, static_cast<dev_t>(0x100000));
  EXPECT_EQ(LIBC_NAMESPACE::__llvm_libc_major(dev1), 0u);
  EXPECT_EQ(LIBC_NAMESPACE::__llvm_libc_minor(dev1), 0x100u);

  // Major >= 4096 uses the upper bits of dev_t (bits 44..63).
  // major = 4096 (0x1000): lower 12 bits = 0, upper bits shifted by 32 =
  // 0x100000000000.
  dev_t dev2 = LIBC_NAMESPACE::__llvm_libc_makedev(0x1000, 0);
  EXPECT_EQ(dev2, static_cast<dev_t>(0x100000000000ULL));
  EXPECT_EQ(LIBC_NAMESPACE::__llvm_libc_major(dev2), 0x1000u);
  EXPECT_EQ(LIBC_NAMESPACE::__llvm_libc_minor(dev2), 0u);
}

TEST(LlvmLibcSysmacrosTest, MaxValues) {
  // All bits set
  dev_t dev_max = LIBC_NAMESPACE::__llvm_libc_makedev(0xffffffffu, 0xffffffffu);
  EXPECT_EQ(dev_max, static_cast<dev_t>(0xffffffffffffffffULL));
  EXPECT_EQ(LIBC_NAMESPACE::__llvm_libc_major(dev_max), 0xffffffffu);
  EXPECT_EQ(LIBC_NAMESPACE::__llvm_libc_minor(dev_max), 0xffffffffu);
}
...
TEST(LlvmLibcSysmacrosTest, RoundTrip) {
  ...
  for (const auto &tc : test_cases) {
    dev_t d = LIBC_NAMESPACE::__llvm_libc_makedev(tc.maj, tc.min);
    EXPECT_EQ(LIBC_NAMESPACE::__llvm_libc_major(d), tc.maj);
    EXPECT_EQ(LIBC_NAMESPACE::__llvm_libc_minor(d), tc.min);
  }
}
