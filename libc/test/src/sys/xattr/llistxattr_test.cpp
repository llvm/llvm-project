...
TEST_F(LlvmLibcLlistxattrTest, NoExtendedAttributes) {
  const LIBC_NAMESPACE::CString TEST_FILE_NAME =
      libc_make_test_file_path("testdata/llistxattr_no_xattrs.txt");
...
TEST_F(LlvmLibcLlistxattrTest, WithUserExtendedAttribute) {
  const LIBC_NAMESPACE::CString TEST_FILE_NAME =
      libc_make_test_file_path("testdata/llistxattr_with_user_xattr.txt");
  constexpr const char *TEST_SYMLINK_TARGET = "llistxattr_with_user_xattr.txt";
  const LIBC_NAMESPACE::CString TEST_SYMLINK_NAME =
      libc_make_test_file_path("testdata/llistxattr_symlink.txt");
  ...
  {
    ASSERT_THAT(LIBC_NAMESPACE::llistxattr(TEST_FILE_NAME, nullptr, 0),
  ...
  // internally; llistxattr instead of listxattr.
  {
    EXPECT_THAT(LIBC_NAMESPACE::llistxattr(TEST_SYMLINK_NAME, nullptr, 0),
                Succeeds<ssize_t>(0));
...
TEST(LlvmLibcLlistxattrTest, CrashOnNullPath) {
  EXPECT_DEATH(
      [] {
        constexpr size_t BUFFER_SIZE = 32;
        char buffer[BUFFER_SIZE] = {};
